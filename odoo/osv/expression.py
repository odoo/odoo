...
TERM_OPERATORS = set(odoo.domains.TERM_OPERATORS)

# A subset of the above operators, with a 'negative' semantic. When the
# expressions 'in NEGATIVE_TERM_OPERATORS' or 'not in NEGATIVE_TERM_OPERATORS' are used in the code
# below, this doesn't necessarily mean that any of those NEGATIVE_TERM_OPERATORS is
# legal in the processed term.
NEGATIVE_TERM_OPERATORS = set(odoo.domains.NEGATIVE_TERM_OPERATORS)

# Negation of domain expressions
TERM_OPERATORS_NEGATION = odoo.domains._TERM_OPERATORS_NEGATION

TRUE_LEAF = odoo.domains._TRUE_LEAF
FALSE_LEAF = odoo.domains._FALSE_LEAF
...
def normalize_domain(domain):
    ...
    """
    # TODO deprecate use D()
    if isinstance(domain, odoo.domains.Domain):
        # already normalized
        return list(domain)
...
def is_false(model, domain):
    ...
    # TODO deprecate
    return D(domain).const() is False
...
def AND(domains):
    ...
    # TODO deprecate
    return list(D.AND(*domains))


def OR(domains):
    """OR([D1,D2,...]) returns a domain representing D1 or D2 or ... """
    # TODO deprecate
    return list(D.OR(*domains))
...
def _anyfy_leaves(domain, model):
    ...
    """
    # TODO same for remaining methods _tree*
    result = []
    for item in domain:
        if is_operator(item):
            result.append(item)
...
class expression(object):
    ...
    def __init__(self, domain, model, alias=None, query=None):
        ...
        self.result = result = domain._build_sql(self.root_model, self.root_alias, query)
