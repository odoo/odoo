...
class TestMonetaryAccess(TransactionCaseWithUserDemo):
    def test_monetary_access_create(self):
        ...
        new_user = user_demo.copy({"test_access_monetary": 1 / 3})
        self.assertEqual(
            new_user.test_access_currency_id.id,
            False,
        ...
        self.assertEqual(
            new_user.test_access_currency_id,
            new_user.company_id.currency_id,
            "the currency declares company_id.currency_id, so the write is seen at once",
        )
        self.assertEqual(
            new_user.test_access_monetary,
            1 / 3,
        ...
        self.assertEqual(
            new_user.test_access_currency_id.rounding,
            0.01,
