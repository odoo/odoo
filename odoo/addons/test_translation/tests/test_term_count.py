...
class TestImport(common.TransactionCase):
    ...
    def test_import_colliding_xmlids_later_overwrites(self):
        ...
        self.env['res.lang']._activate_lang('fr_FR')
        record = self.env.ref('test_translation.test_translation_model1_record1')
        alias = self.env.ref('test_translation.test_translation_model1_record1_alias')
        self.assertEqual(record, alias)

        self.env['ir.module.module']._load_module_terms(
            ['test_translation'], ['fr_FR'], overwrite=True,
        )
    ...
    def test_import_from_po_file(self):
        ...
        import_tlh = self.env["base.language.import"].create({
            'name': 'Klingon',
            'code': 'tlh',
            'data': self.file_read('test_translation/i18n/tlh.po'),
            'filename': 'tlh.po',
        })
    ...
    def test_import_from_csv_file(self):
        ...
        import_tlh = self.env["base.language.import"].create({
            'name': 'Dothraki',
            'code': 'dot',
            'data': self.file_read('test_translation/i18n/dot.csv'),
            'filename': 'dot.csv',
        })
        with mute_logger('odoo.addons.base.models.res_lang'):
            import_tlh.import_lang()
