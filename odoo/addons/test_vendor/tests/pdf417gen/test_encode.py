...
from odoo.tools._vendor.pdf417gen.compaction import TEXT_LATCH, NUMERIC_LATCH
from odoo.tools._vendor.pdf417gen.encoding import encode, encode_high, to_bytes
...
class TestEncode(common.TransactionCase):
    ...
    def test_encode_high(self):
        ...
        self.assertEqual(encode_high(to_bytes(TEST_DATA), 6, 2), expected)
    ...
    def test_encode_low(self):
        ...
        self.assertEqual(list(encode(TEST_DATA, 6, 2)), expected)
    ...
    def test_encode_unicode(self):
        ...
        self.assertEqual(encode(uc), expected)
        self.assertEqual(encode(by), expected)
    ...
    def test_max_barcode_size(self):
        ...
        self.assertEqual(str(cm.exception), "Data too long. Generated bar code has length descriptor of 944. Maximum is 928.")

        # Too few rows
        with self.assertRaises(ValueError) as cm:
            encode("x", columns=16, security_level=1)
        self.assertEqual(str(cm.exception), "Generated bar code has 1 rows. Minimum is 3 rows. Try decreasing column count.")

        # Too many rows
        with self.assertRaises(ValueError) as cm:
            encode("x" * 1853, columns=8, security_level=6)
        self.assertEqual(str(cm.exception), "Generated bar code has 132 rows. Maximum is 90 rows. Try increasing column count.")
