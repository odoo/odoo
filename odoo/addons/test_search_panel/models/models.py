...
class Test_Search_PanelSource_Model(models.Model):
    ...
    folder_id = fields.Many2one(comodel_name="test_search_panel.category_target_model")
    categ_id = fields.Many2one(
        comodel_name="test_search_panel.category_target_model_no_parent_name"
    )
    tag_ids = fields.Many2many(
        comodel_name="test_search_panel.filter_target_model",
        relation="rel_table",
        string="Tags",
    )
...
class Test_Search_PanelCategory_Target_Model(models.Model):
    ...
    parent_name_id = fields.Many2one(
...
class Test_Search_PanelFilter_Target_Model(models.Model):
    ...
    status = fields.Selection(selection=[("cool", "Cool"), ("unknown", "Unknown")])
    color = fields.Char()
    folder_id = fields.Many2one(comodel_name="test_search_panel.category_target_model")
