...
class TestSessionStore(HttpCaseWithUserDemo):
    ...
    def test01_session_nan(self):
        ...
        with self.assertLogs('odoo.addons.base.models.ir_config_parameter', level='ERROR') as logs:
            self.assertEqual(odoo.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "ERROR:odoo.addons.base.models.ir_config_parameter:ir.config_parameter with key sessions.max_inactivity_seconds has invalid value adminCantSetupThisValueLikeANormalPerson for type int")
