...
class TestHttpRegistry(BaseCase):
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('odoo.orm.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:odoo.orm.registry:Reloading the model registry after database signaling.",
            Like("INFO:odoo.orm.registry:Registry loaded in ...s"),
        ])
    ...
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('odoo.orm.registry', logging.ERROR) as capture1, \
        ...
        self.assertEqual(capture1.output, [
            "ERROR:odoo.orm.registry:Failed to load registry",
        ])
