...
class TestPerformance(SavepointCaseWithUserDemo):
    ...
    def test_write_base_one2many(self):
        ...
        with self.assertQueryCount(2):
            rec1.write({'line_ids': [(fields.X2ManyCmd.CREATE, 0, {'value': 0})]})
        self.assertEqual(len(rec1.line_ids), 1)

        rec1.invalidate_cache()
        with self.assertQueryCount(15):
            rec1.write({'line_ids': [(fields.X2ManyCmd.CREATE, 0, {'value': val}) for val in range(1, 12)]})
        ...
        with self.assertQueryCount(6):
            rec1.write({'line_ids': [(fields.X2ManyCmd.UPDATE, line.id, {'value': 42}) for line in lines[0]]})
        self.assertEqual(rec1.line_ids, lines)

        rec1.invalidate_cache()
        with self.assertQueryCount(26):
            rec1.write({'line_ids': [(fields.X2ManyCmd.UPDATE, line.id, {'value': 42 + line.id}) for line in lines[1:]]})
        self.assertEqual(rec1.line_ids, lines)

        # delete N lines: O(1) queries
        rec1.invalidate_cache()
        with self.assertQueryCount(__system__=14, demo=14):
            rec1.write({'line_ids': [(fields.X2ManyCmd.DELETE, line.id) for line in lines[0]]})
        self.assertEqual(rec1.line_ids, lines[1:])

        rec1.invalidate_cache()
        with self.assertQueryCount(__system__=12, demo=12):
            rec1.write({'line_ids': [(fields.X2ManyCmd.DELETE, line.id) for line in lines[1:]]})
        self.assertFalse(rec1.line_ids)
        self.assertFalse(lines.exists())

        rec1.write({'line_ids': [(fields.X2ManyCmd.CREATE, 0, {'value': val}) for val in range(12)]})
        lines = rec1.line_ids

        # unlink N lines: O(1) queries
        rec1.invalidate_cache()
        with self.assertQueryCount(__system__=11, demo=11):
            rec1.write({'line_ids': [(fields.X2ManyCmd.UNLINK, line.id) for line in lines[0]]})
        self.assertEqual(rec1.line_ids, lines[1:])

        rec1.invalidate_cache()
        with self.assertQueryCount(__system__=12, demo=12):
            rec1.write({'line_ids': [(fields.X2ManyCmd.UNLINK, line.id) for line in lines[1:]]})
        self.assertFalse(rec1.line_ids)
        self.assertFalse(lines.exists())
        ...
        rec1.invalidate_cache()
        with self.assertQueryCount(2):
            rec2.write({'line_ids': [(fields.X2ManyCmd.LINK, line.id) for line in lines[0]]})
        self.assertEqual(rec1.line_ids, lines[1:])
        self.assertEqual(rec2.line_ids, lines[0])

        rec1.invalidate_cache()
        with self.assertQueryCount(7):
            rec2.write({'line_ids': [(fields.X2ManyCmd.LINK, line.id) for line in lines[1:]]})
        self.assertFalse(rec1.line_ids)
        ...
        with self.assertQueryCount(__system__=13, demo=13):
            rec2.write({'line_ids': [(fields.X2ManyCmd.CLEAR,)]})
        ...
        with self.assertQueryCount(4):
            rec2.write({'line_ids': [(fields.X2ManyCmd.SET, 0, lines[0].ids)]})
        self.assertEqual(rec1.line_ids, lines[1:])
        self.assertEqual(rec2.line_ids, lines[0])

        with self.assertQueryCount(5):
            rec2.write({'line_ids': [(fields.X2ManyCmd.SET, 0, lines.ids)]})
        self.assertFalse(rec1.line_ids)
        self.assertEqual(rec2.line_ids, lines)
    ...
    def test_write_base_one2many_with_constraint(self):
        ...
        rec.write({'line_ids': [(fields.X2ManyCmd.CREATE, 0, {'value': val}) for val in range(12)]})

        # This write() will raise because of the unique index if the unlink() is
        # not performed before the create()
        rec.write({'line_ids': [(fields.X2ManyCmd.CLEAR,)] + [(fields.X2ManyCmd.CREATE, 0, {'value': val}) for val in range(6)]})
    ...
    def test_write_base_many2many(self):
        ...
        rec1.invalidate_cache()
        with self.assertQueryCount(4):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.CREATE, 0, {'name': 0})]})
        self.assertEqual(len(rec1.tag_ids), 1)

        rec1.invalidate_cache()
        with self.assertQueryCount(14):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.CREATE, 0, {'name': val}) for val in range(1, 12)]})
        ...
        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.UPDATE, tag.id, {'name': 'X'}) for tag in tags[0]]})
        self.assertEqual(rec1.tag_ids, tags)

        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.UPDATE, tag.id, {'name': 'X'}) for tag in tags[1:]]})
        self.assertEqual(rec1.tag_ids, tags)

        # delete N tags: O(1) queries
        rec1.invalidate_cache()
        with self.assertQueryCount(__system__=8, demo=8):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.DELETE, tag.id) for tag in tags[0]]})
        self.assertEqual(rec1.tag_ids, tags[1:])

        rec1.invalidate_cache()
        with self.assertQueryCount(__system__=8, demo=8):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.DELETE, tag.id) for tag in tags[1:]]})
        self.assertFalse(rec1.tag_ids)
        self.assertFalse(tags.exists())

        rec1.write({'tag_ids': [(fields.X2ManyCmd.CREATE, 0, {'name': val}) for val in range(12)]})
        tags = rec1.tag_ids

        # unlink N tags: O(1) queries
        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.UNLINK, tag.id) for tag in tags[0]]})
        self.assertEqual(rec1.tag_ids, tags[1:])

        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec1.write({'tag_ids': [(fields.X2ManyCmd.UNLINK, tag.id) for tag in tags[1:]]})
        self.assertFalse(rec1.tag_ids)
        ...
        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec2.write({'tag_ids': [(fields.X2ManyCmd.LINK, tag.id) for tag in tags[0]]})
        self.assertEqual(rec2.tag_ids, tags[0])

        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec2.write({'tag_ids': [(fields.X2ManyCmd.LINK, tag.id) for tag in tags[1:]]})
        ...
        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec2.write({'tag_ids': [(fields.X2ManyCmd.CLEAR,)]})
        ...
        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec2.write({'tag_ids': [(fields.X2ManyCmd.SET, 0, tags.ids)]})
        self.assertEqual(rec2.tag_ids, tags)

        rec1.invalidate_cache()
        with self.assertQueryCount(3):
            rec2.write({'tag_ids': [(fields.X2ManyCmd.SET, 0, tags[:8].ids)]})
        self.assertEqual(rec2.tag_ids, tags[:8])

        rec1.invalidate_cache()
        with self.assertQueryCount(4):
            rec2.write({'tag_ids': [(fields.X2ManyCmd.SET, 0, tags[4:].ids)]})
    ...
    def test_create_base_with_lines(self):
        ...
        with self.assertQueryCount(__system__=12, demo=12):
            self.env['test_performance.base'].create({
                'name': 'X',
                'line_ids': [(fields.X2ManyCmd.CREATE, 0, {'value': val}) for val in range(10)],
            })
    ...
    def test_create_base_with_tags(self):
        ...
        with self.assertQueryCount(13):
            self.env['test_performance.base'].create({
                'name': 'X',
                'tag_ids': [(fields.X2ManyCmd.CREATE, 0, {'name': val}) for val in range(10)],
            })
        ...
        with self.assertQueryCount(3):
            self.env['test_performance.base'].create({
                'name': 'X',
                'tag_ids': [(fields.X2ManyCmd.LINK, tag.id) for tag in tags],
            })

        with self.assertQueryCount(2):
            self.env['test_performance.base'].create({
                'name': 'X',
                'tag_ids': [(fields.X2ManyCmd.SET, 0, [])],
            })

        with self.assertQueryCount(3):
            self.env['test_performance.base'].create({
                'name': 'X',
                'tag_ids': [(fields.X2ManyCmd.SET, 0, tags.ids)],
            })
...
class TestMapped(TransactionCase):
    ...
    def test_relational_mapped(self):
        ...
        recs = self.env['test_performance.base'].create([
            {'name': 'foo%d' % index, 'line_ids': [(fields.X2ManyCmd.CREATE, 0, {'value': index})]}
