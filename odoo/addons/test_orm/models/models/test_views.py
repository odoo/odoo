...
class TestViewsCategory(models.Model):
    ...
    parent = fields.Many2one('test_views.category', ondelete='cascade')
    parent_path = fields.Char(index=True)
    depth = fields.Integer(compute="_compute_depth")
    root_categ = fields.Many2one('test_views.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_views.discussion', 'test_views_discussion_category',
                                   'category', 'discussion')
...
class TestViewsDiscussion(models.Model):
    ...
    categories = fields.Many2many('test_views.category',
        'test_views_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('test_views.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('test_views.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_views.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_views.emailmessage', 'discussion')
    important_emails = fields.One2many('test_views.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class TestViewsMessage(models.Model):
    ...
    discussion = fields.Many2one('test_views.discussion', ondelete='cascade')
...
class TestViewsEmailmessage(models.Model):
    ...
    message = fields.Many2one('test_views.message', 'Message',
...
class BinaryTest(models.Model):
    _name = _description = "test_views.binary.test"
...
class CalendarTest(models.Model):
    _name = _description = "test_views.calendar.test"
...
class TestViewsModel2SomeAccess(models.Model):
    ...
    g_id = fields.Many2one('test_views.model.some_access', domain='[("a", "=", g_d)]')
...
class TestViewsModel3SomeAccess(models.Model):
    ...
    xxx_id = fields.Many2one('test_views.model2.some_access')
