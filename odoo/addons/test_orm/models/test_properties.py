...
class Test_Properties_Category(models.Model):
    ...
    parent = fields.Many2one('test_properties.category', ondelete='cascade')
    parent_path = fields.Char(index=True)
    depth = fields.Integer(compute="_compute_depth")
    root_categ = fields.Many2one('test_properties.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_properties.discussion', 'test_properties_discussion_category',
                                   'category', 'discussion')
...
class Test_Properties_Discussion(models.Model):
    ...
    categories = fields.Many2many('test_properties.category',
        'test_properties_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('test_properties.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('test_properties.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_properties.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_properties.emailmessage', 'discussion')
    important_emails = fields.One2many('test_properties.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Properties_Message(models.Model):
...
class Test_Properties_Emailmessage(models.Model):
...
class Test_Properties_MultiLine(models.Model):
    _name = 'test_properties.multi.line'
    _description = 'Test ORM Multi Line'

    multi = fields.Many2one('test_properties.multi', ondelete='cascade')
    name = fields.Char()
    partner = fields.Many2one(related='multi.partner', store=True)
    tags = fields.Many2many('test_properties.multi.tag')


class Test_Properties_MultiTag(models.Model):
...
class Test_Properties_Transient_Model(models.TransientModel):
