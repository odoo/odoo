...
class TestReadGroupPrivateOrder(models.Model):
    ...
    many2one_id = fields.Many2one('test_read_group_private.order')
    name = fields.Char()
    fold = fields.Boolean()

    @property
    def _order(self):
        if self.env.context.get('test_read_group_private_order_company_dependent'):
...
class TestReadGroupPrivateOrderLine(models.Model):
    ...
    order_expand_id = fields.Many2one('test_read_group_private.order', group_expand='_read_group_expand_full')
...
class TestReadGroupPrivateTask(models.Model):
    ...
    customer_ids = fields.Many2many(
        'test_read_group_private.user',
        'test_read_group_private_task_user_rel_2',
        'task_id',
        'user_id',
        string="Customers",
    )
