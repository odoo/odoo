...
class Test_Json_Field_Category(models.Model):
    ...
    parent = fields.Many2one('test_json_field.category', ondelete='cascade')
    parent_path = fields.Char(index=True)
    depth = fields.Integer(compute="_compute_depth")
    root_categ = fields.Many2one('test_json_field.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_json_field.discussion', 'test_json_field_discussion_category',
                                   'category', 'discussion')
...
class Test_Json_Field_Discussion(models.Model):
    ...
    categories = fields.Many2many('test_json_field.category',
        'test_json_field_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('test_json_field.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('test_json_field.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_json_field.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_json_field.emailmessage', 'discussion')
    important_emails = fields.One2many('test_json_field.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Json_Field_Message(models.Model):
    _name = 'test_json_field.message'
    _description = 'Test ORM Message'

    discussion = fields.Many2one('test_json_field.discussion', ondelete='cascade')
...
class Test_Json_Field_Emailmessage(models.Model):
    ...
    message = fields.Many2one('test_json_field.message', 'Message',
