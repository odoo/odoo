...
class Test_Attributes_Category(models.Model):
    _name = 'test_attributes.category'
    ...
    parent = fields.Many2one('test_attributes.category', ondelete='cascade')
    parent_path = fields.Char(index=True)
    depth = fields.Integer(compute="_compute_depth")
    root_categ = fields.Many2one('test_attributes.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_attributes.discussion', 'test_attributes_discussion_category',
                                   'category', 'discussion')
...
class Test_Attributes_Discussion(models.Model):
    _name = 'test_attributes.discussion'
    ...
    categories = fields.Many2many('test_attributes.category',
        'test_attributes_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('test_attributes.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('test_attributes.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_attributes.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_attributes.emailmessage', 'discussion')
    important_emails = fields.One2many('test_attributes.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Attributes_Message(models.Model):
    _name = 'test_attributes.message'
    _description = 'Test ORM Message'

    discussion = fields.Many2one('test_attributes.discussion', ondelete='cascade')
...
class Test_Attributes_Emailmessage(models.Model):
    _name = 'test_attributes.emailmessage'
    _description = 'Test ORM Email Message'
    _inherits = {'test_attributes.message': 'message'}
    _inherit = 'properties.base.definition.mixin'

    message = fields.Many2one('test_attributes.message', 'Message',
