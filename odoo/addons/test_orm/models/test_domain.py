...
class Test_Domain_Category(models.Model):
    ...
    root_categ = fields.Many2one('test_domain.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_domain.discussion', 'test_domain_discussion_category',
                                   'category', 'discussion')
...
class Test_Domain_Discussion(models.Model):
    ...
    categories = fields.Many2many('test_domain.category',
        'test_domain_discussion_category', 'discussion', 'category')
    ...
    important_messages = fields.One2many('test_domain.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_domain.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_domain.emailmessage', 'discussion')
    important_emails = fields.One2many('test_domain.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Domain_Message(models.Model):
...
class Test_Domain_Emailmessage(models.Model):
...
class Test_Domain_Bool(models.Model):
...
class Test_Domain_Foo(models.Model):
...
class Test_Domain_Bar(models.Model):
...
class Test_Domain_Model_A(models.Model):
    _name = 'test_domain.model_a'
    _description = 'Model A'

    name = fields.Char()
    a_restricted_b_ids = fields.Many2many('test_domain.model_b', relation='rel_domain_model_a_model_b_1')
    b_restricted_b_ids = fields.Many2many('test_domain.model_b', relation='rel_domain_model_a_model_b_2', ondelete='restrict')


class Test_Domain_Model_B(models.Model):
    _name = 'test_domain.model_b'
    _description = 'Model B'

    name = fields.Char()
    a_restricted_a_ids = fields.Many2many('test_domain.model_a', relation='rel_domain_model_a_model_b_1', ondelete='restrict')
    b_restricted_a_ids = fields.Many2many('test_domain.model_a', relation='rel_domain_model_a_model_b_2')


class Test_Domain_Model_Active_Field(models.Model):
    ...
    children_ids = fields.One2many('test_domain.model_active_field', 'parent_id')
    all_children_ids = fields.One2many('test_domain.model_active_field', 'parent_id',
                                       context={'active_test': False})
    active_children_ids = fields.One2many('test_domain.model_active_field', 'parent_id',
                                          context={'active_test': True})
    relatives_ids = fields.Many2many(
        'test_domain.model_active_field',
        'model_test_domain_active_field_relatives_rel', 'source_id', 'dest_id',
    )
...
class Test_Domain_Indexed_Translation(models.Model):
...
class Test_Domain_Empty_Char(models.Model):
...
class Test_Domain_Empty_Int(models.Model):
...
class Test_Domain_AnyParent(models.Model):
    _name = 'test_domain.any_parent'
    _description = 'Any Parent'

    name = fields.Char()
    child_ids = fields.One2many('test_domain.any_child', 'parent_id')


class Test_Domain_AnyChild(models.Model):
    _name = 'test_domain.any_child'
    _description = 'Any Child'
    _inherits = {
        'test_domain.any_parent': 'parent_id',
    }

    parent_id = fields.Many2one('test_domain.any_parent', required=True, ondelete='cascade')
    link_sibling_id = fields.Many2one('test_domain.any_child')
    quantity = fields.Integer()
    tag_ids = fields.Many2many('test_domain.any_tag')


class Test_Domain_AnyTag(models.Model):
    _name = 'test_domain.any_tag'
    _description = 'Any tag'

    name = fields.Char()
    child_ids = fields.Many2many('test_domain.any_child')
