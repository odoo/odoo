...
class Test_Schema_Category(models.Model):
    _name = 'test_schema.category'
    ...
    parent = fields.Many2one('test_schema.category', ondelete='cascade')
    parent_path = fields.Char(index=True)
    depth = fields.Integer(compute="_compute_depth")
    root_categ = fields.Many2one('test_schema.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_schema.discussion', 'test_schema_discussion_category',
                                   'category', 'discussion')
...
class Test_Schema_Discussion(models.Model):
    _name = 'test_schema.discussion'
    ...
    categories = fields.Many2many('test_schema.category',
        'test_schema_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('test_schema.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('test_schema.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_schema.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_schema.emailmessage', 'discussion')
    important_emails = fields.One2many('test_schema.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Schema_Message(models.Model):
    _name = 'test_schema.message'
    _description = 'Test ORM Message'

    discussion = fields.Many2one('test_schema.discussion', ondelete='cascade')
...
class Test_Schema_Emailmessage(models.Model):
    _name = 'test_schema.emailmessage'
    _description = 'Test ORM Email Message'
    _inherits = {'test_schema.message': 'message'}
    _inherit = 'properties.base.definition.mixin'

    message = fields.Many2one('test_schema.message', 'Message',
                              required=True, ondelete='cascade')
    email_to = fields.Char('To')
    active = fields.Boolean('Active Message', related='message.active', store=True, related_sudo=False)


class Test_Schema_Foo(models.Model):
    _name = 'test_schema.foo'
...
class Test_Schema_Mixed(models.Model):
    _name = 'test_schema.mixed'
...
class Test_Schema_Model_Many2one_Reference(models.Model):
    _name = 'test_schema.model_many2one_reference'
...
class Test_Schema_Inverse_M2o_Ref(models.Model):
    _name = 'test_schema.inverse_m2o_ref'
    _description = 'dummy m2oref inverse model'

    model_ids = fields.One2many(
        'test_schema.model_many2one_reference', 'res_id',
    ...
    model_computed_ids = fields.One2many(
        'test_schema.model_many2one_reference',
