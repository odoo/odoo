...
class Test_Sort_Category(models.Model):
    ...
    root_categ = fields.Many2one('test_sort.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('test_sort.discussion', 'test_sort_discussion_category',
                                   'category', 'discussion')
...
class Test_Sort_Discussion(models.Model):
    ...
    categories = fields.Many2many('test_sort.category',
        'test_sort_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('test_sort.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('test_sort.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'test_sort.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('test_sort.emailmessage', 'discussion')
    important_emails = fields.One2many('test_sort.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Sort_Message(models.Model):
    _name = 'test_sort.message'
    _description = 'Test ORM Message'

    discussion = fields.Many2one('test_sort.discussion', ondelete='cascade')
...
class Test_Sort_Emailmessage(models.Model):
    ...
    message = fields.Many2one('test_sort.message', 'Message',
                              required=True, ondelete='cascade')
    email_to = fields.Char('To')
    active = fields.Boolean('Active Message', related='message.active', store=True, related_sudo=False)


class Test_Sort_Model_A(models.Model):
    _name = 'test_sort.model_a'
    _description = 'Model A'

    name = fields.Char()
    a_restricted_b_ids = fields.Many2many('test_sort.model_b', relation='rel_sort_model_a_model_b_1')
    b_restricted_b_ids = fields.Many2many('test_sort.model_b', relation='rel_sort_model_a_model_b_2', ondelete='restrict')


class Test_Sort_Model_B(models.Model):
    _name = 'test_sort.model_b'
    _description = 'Model B'

    name = fields.Char()
    a_restricted_a_ids = fields.Many2many('test_sort.model_a', relation='rel_sort_model_a_model_b_1', ondelete='restrict')
    b_restricted_a_ids = fields.Many2many('test_sort.model_a', relation='rel_sort_model_a_model_b_2')


class Test_Sort_Model_Active_Field(models.Model):
    _name = 'test_sort.model_active_field'
    _description = 'A model with active field'

    name = fields.Char()
    active = fields.Boolean(default=True)
    parent_id = fields.Many2one('test_sort.model_active_field')
    children_ids = fields.One2many('test_sort.model_active_field', 'parent_id')
    all_children_ids = fields.One2many('test_sort.model_active_field', 'parent_id',
                                       context={'active_test': False})
    active_children_ids = fields.One2many('test_sort.model_active_field', 'parent_id',
                                          context={'active_test': True})
    relatives_ids = fields.Many2many(
        'test_sort.model_active_field',
        'model_test_sort_active_field_relatives_rel', 'source_id', 'dest_id',
    )
...
class Test_Sort_Country(models.Model):
...
class Test_Sort_City(models.Model):
