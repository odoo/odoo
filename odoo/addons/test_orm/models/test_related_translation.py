...
class Test_Related_Translation_Category(models.Model):
    _name = 'r_translation.category'
    ...
    parent = fields.Many2one('r_translation.category', ondelete='cascade')
    parent_path = fields.Char(index=True)
    depth = fields.Integer(compute="_compute_depth")
    root_categ = fields.Many2one('r_translation.category', compute='_compute_root_categ')
    display_name = fields.Char(
        inverse='_inverse_display_name',
        recursive=True,
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many('r_translation.discussion', 'r_translation_discussion_category',
                                   'category', 'discussion')
...
class Test_Related_Translation_Discussion(models.Model):
    _name = 'r_translation.discussion'
    ...
    categories = fields.Many2many('r_translation.category',
        'r_translation_discussion_category', 'discussion', 'category')
    participants = fields.Many2many('res.users', context={'active_test': False})
    messages = fields.One2many('r_translation.message', 'discussion', copy=True)
    message_concat = fields.Text(string='Message concatenate')
    important_messages = fields.One2many('r_translation.message', 'discussion',
                                         domain=[('important', '=', True)])
    very_important_messages = fields.One2many(
        'r_translation.message', 'discussion',
        domain=lambda self: self._domain_very_important())
    emails = fields.One2many('r_translation.emailmessage', 'discussion')
    important_emails = fields.One2many('r_translation.emailmessage', 'discussion',
                                       domain=[('important', '=', True)])
...
class Test_Related_Translation_Message(models.Model):
    _name = 'r_translation.message'
    _description = 'Test ORM Message'

    discussion = fields.Many2one('r_translation.discussion', ondelete='cascade')
...
class Test_Related_Translation_Emailmessage(models.Model):
    _name = 'r_translation.emailmessage'
    _description = 'Test ORM Email Message'
    _inherits = {'r_translation.message': 'message'}
    _inherit = 'properties.base.definition.mixin'

    message = fields.Many2one('r_translation.message', 'Message',
                              required=True, ondelete='cascade')
    email_to = fields.Char('To')
    active = fields.Boolean('Active Message', related='message.active', store=True, related_sudo=False)

    
class Test_Related_Translation_Related(models.Model):
    _name = 'r_translation.related'
    ...
    message = fields.Many2one('r_translation.message')
    ...
    foo_id = fields.Many2one('r_translation.related_foo')
    foo_ids = fields.Many2many('r_translation.related_foo', string='Foos')
...
class Test_Related_Translation_Related_Foo(models.Model):
    _name = 'r_translation.related_foo'
    _description = 'r_translation.related_foo'

    name = fields.Char()
    bar_id = fields.Many2one('r_translation.related_bar')
    foo_ids = fields.One2many('r_translation.related', 'foo_id', string='Foos')
    bar_ids = fields.Many2many('r_translation.related_bar', string='Bars')
...
class Test_Related_Translation_Related_Bar(models.Model):
    _name = 'r_translation.related_bar'
    _description = 'r_translation.related_bar'

    name = fields.Char()
    active = fields.Boolean(default=True)


class Test_Related_Translation_Related_Translation_1(models.Model):
    _name = 'r_translation.related_translation_1'
...
class Test_Related_Translation_Related_Translation_2(models.Model):
    _name = 'r_translation.related_translation_2'
    _description = 'A model to test translation for related fields'

    related_id = fields.Many2one('r_translation.related_translation_1', string='Parent Model')
...
class Test_Related_Translation_Related_Translation_3(models.Model):
    _name = 'r_translation.related_translation_3'
    _description = 'A model to test translation for related fields'

    related_id = fields.Many2one('r_translation.related_translation_2', string='Parent Model')
    name = fields.Char('Name Related', related='related_id.name', readonly=False)
    html = fields.Html('HTML Related', related='related_id.html', readonly=False)
