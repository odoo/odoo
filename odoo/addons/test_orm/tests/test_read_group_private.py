...
class TestPrivateReadGroup(common.TransactionCase):
    ...
    def test_simple_private_read_group(self):
        Model = self.env['test_read_group_private.aggregate']
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_aggregate"."key",
                   SUM("test_read_group_private_aggregate"."value")
            FROM "test_read_group_private_aggregate"
            GROUP BY "test_read_group_private_aggregate"."key"
            ORDER BY "test_read_group_private_aggregate"."key" ASC
        """]):
            self.assertEqual(
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_aggregate"."key",
                   "test_read_group_private_aggregate"."partner_id",
                   SUM("test_read_group_private_aggregate"."value")
            FROM "test_read_group_private_aggregate"
            LEFT JOIN "res_partner" AS "test_read_group_private_aggregate__partner_id"
                ON ("test_read_group_private_aggregate"."partner_id" = "test_read_group_private_aggregate__partner_id"."id")
            GROUP BY "test_read_group_private_aggregate"."key",
                     "test_read_group_private_aggregate"."partner_id",
                     "test_read_group_private_aggregate__partner_id"."complete_name",
                     "test_read_group_private_aggregate__partner_id"."id"
            ORDER BY "test_read_group_private_aggregate"."key" ASC,
                     "test_read_group_private_aggregate__partner_id"."complete_name" ASC,
                     "test_read_group_private_aggregate__partner_id"."id" DESC
        """]):
            self.assertEqual(
        ...
        with self.assertQueries(["""
            ...
            GROUP BY "test_read_group_private_aggregate"."key",
                     "test_read_group_private_aggregate"."partner_id"
            ORDER BY "test_read_group_private_aggregate"."key" ASC,
                     "test_read_group_private_aggregate"."partner_id" ASC
        """]):
            self.assertEqual(
    ...
    def test_ambiguous_field_name(self):
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_aggregate"."display_name",
                   "test_read_group_private_aggregate"."partner_id",
                   COUNT(*)
            FROM "test_read_group_private_aggregate"
            LEFT JOIN "res_partner" AS "test_read_group_private_aggregate__partner_id"
                ON ("test_read_group_private_aggregate"."partner_id" = "test_read_group_private_aggregate__partner_id"."id")
            GROUP BY "test_read_group_private_aggregate"."display_name",
                     "test_read_group_private_aggregate"."partner_id",
                     "test_read_group_private_aggregate__partner_id"."complete_name",
                     "test_read_group_private_aggregate__partner_id"."id"
            ORDER BY "test_read_group_private_aggregate__partner_id"."complete_name" DESC,
                     "test_read_group_private_aggregate__partner_id"."id" ASC
        """]):
            result = Model._read_group(
                [],
    ...
    def test_bool_read_groups(self):
        Model = self.env['test_read_group_private.aggregate.boolean']
    ...
    def test_having_clause(self):
        ...
        with self.assertQueries(["""
            ...
            FROM "test_read_group_private_aggregate"
            GROUP BY "test_read_group_private_aggregate"."key"
            HAVING SUM("test_read_group_private_aggregate"."value") > %s
            ORDER BY "test_read_group_private_aggregate"."key" ASC
        """]):
            self.assertEqual(
                Model._read_group(
        ...
        with self.assertQueries(["""
            ...
            FROM "test_read_group_private_aggregate"
            GROUP BY "test_read_group_private_aggregate"."key"
            HAVING (COUNT(*) < %s AND SUM("test_read_group_private_aggregate"."value") > %s)
            ORDER BY "test_read_group_private_aggregate"."key" ASC
        """]):
            self.assertEqual(
                Model._read_group(
    ...
    def test_malformed_params(self):
        Model = self.env['test_read_group_private.order.line']
        ...
        with self.assertRaisesRegex(ValueError, "Invalid field 'not_another_field' on model 'test_read_group_private.order.line' for 'not_another_field:sum'."):
    ...
    def test_groupby_date(self):
        ...
        Model = self.env['test_read_group_private.fill_temporal']
    ...
    def test_field_bypass_search_access(self):
        ...
        model = self.env['test_read_group_private.order']
        records = model.create([{
    ...
    def test_groupby_many2many(self):
        User = self.env['test_read_group_private.user']
        mario, luigi = User.create([{'name': 'Mario'}, {'name': 'Luigi'}])
        tasks = self.env['test_read_group_private.task'].create([
            {   # both users
        ...
        expected = """
            SELECT
                "test_read_group_private_task__user_ids__rel"."user_id",
                ARRAY_AGG("test_read_group_private_task"."name" ORDER BY "test_read_group_private_task"."id")
            FROM "test_read_group_private_task"
                LEFT JOIN "test_read_group_private_task_user_rel" AS "test_read_group_private_task__user_ids__rel"
                    ON ("test_read_group_private_task"."id" = "test_read_group_private_task__user_ids__rel"."task_id")
            WHERE "test_read_group_private_task"."id" IN %s
            GROUP BY "test_read_group_private_task__user_ids__rel"."user_id"
            ORDER BY "test_read_group_private_task__user_ids__rel"."user_id" ASC
        """
        ...
        expected = """
            ...
            WHERE "test_read_group_private_task"."id" IN %s
            GROUP BY "test_read_group_private_task__user_ids__rel"."user_id"
            ORDER BY "test_read_group_private_task__user_ids__rel"."user_id" DESC
        """
        with self.assertQueries([expected]):
            self.assertEqual(tasks._read_group(
                    [('id', 'in', tasks.ids)],
        ...
        expected = """
            ...
            LEFT JOIN "test_read_group_private_task_user_rel" AS "test_read_group_private_task__user_ids__rel"
                ON (
                    "test_read_group_private_task"."id" = "test_read_group_private_task__user_ids__rel"."task_id"
                    AND "test_read_group_private_task__user_ids__rel"."user_id" IN (
                        SELECT "test_read_group_private_user"."id"
                        FROM "test_read_group_private_user"
                        WHERE "test_read_group_private_user"."id" IN %s
                    )
                )
            GROUP BY "test_read_group_private_task__user_ids__rel"."user_id"
            ORDER BY "test_read_group_private_task__user_ids__rel"."user_id" ASC
    ...
    def test_groupby_many2many_active_test_domain(self):
        Tag = self.env['test_read_group_private.tag']
        ...
        with self.assertQueries([
            """
            SELECT "test_read_group_private_task__tag_ids__rel"."tag_id",
                ARRAY_AGG("test_read_group_private_task"."name" ORDER BY "test_read_group_private_task"."id")
            FROM "test_read_group_private_task"
            LEFT JOIN "test_read_group_private_task_tag_rel" AS "test_read_group_private_task__tag_ids__rel"
                ON (
                    "test_read_group_private_task"."id" = "test_read_group_private_task__tag_ids__rel"."task_id"
                    AND "test_read_group_private_task__tag_ids__rel"."tag_id" IN (
                        SELECT
                            "test_read_group_private_tag"."id"
                        FROM
                            "test_read_group_private_tag"
                        WHERE
                            "test_read_group_private_tag"."active" IS TRUE
                    )
                )
            WHERE "test_read_group_private_task"."id" IN %s
            GROUP BY "test_read_group_private_task__tag_ids__rel"."tag_id"
            ORDER BY "test_read_group_private_task__tag_ids__rel"."tag_id" ASC
            """,
        ]):
            self.assertEqual(
                tasks._read_group(
        ...
        with self.assertQueries([
            """
            SELECT "test_read_group_private_task__active_tag_ids__rel"."tag_id",
                ARRAY_AGG("test_read_group_private_task"."name" ORDER BY "test_read_group_private_task"."id")
            FROM "test_read_group_private_task"
            LEFT JOIN "test_read_group_private_task_tag_rel" AS "test_read_group_private_task__active_tag_ids__rel"
                ON (
                    "test_read_group_private_task"."id" = "test_read_group_private_task__active_tag_ids__rel"."task_id"
                    AND "test_read_group_private_task__active_tag_ids__rel"."tag_id" IN (
                        SELECT
                            "test_read_group_private_tag"."id"
                        FROM
                            "test_read_group_private_tag"
            ...
            WHERE "test_read_group_private_task"."id" IN %s
            GROUP BY "test_read_group_private_task__active_tag_ids__rel"."tag_id"
            ORDER BY "test_read_group_private_task__active_tag_ids__rel"."tag_id" ASC
            """,
        ]):
            self.assertEqual(
                tasks._read_group(
        ...
        with self.assertQueries([
            """
            SELECT "test_read_group_private_task__all_tag_ids__rel"."tag_id",
                ARRAY_AGG("test_read_group_private_task"."name" ORDER BY "test_read_group_private_task"."id")
            FROM "test_read_group_private_task"
            LEFT JOIN "test_read_group_private_task_tag_rel" AS "test_read_group_private_task__all_tag_ids__rel"
                ON (
                    "test_read_group_private_task"."id" = "test_read_group_private_task__all_tag_ids__rel"."task_id"
                )
            WHERE "test_read_group_private_task"."id" IN %s
            GROUP BY "test_read_group_private_task__all_tag_ids__rel"."tag_id"
            ORDER BY "test_read_group_private_task__all_tag_ids__rel"."tag_id" ASC
            """,
        ]):
            self.assertEqual(
                tasks._read_group(
    ...
    def test_float_aggregate(self):
        records = self.env['test_read_group_private.aggregate'].create({'numeric_value': 42.42})
        [[result]] = records._read_group([('id', 'in', records.ids)], [], ['numeric_value:array_agg'])
        self.assertIsInstance(result, list)
        self.assertIsInstance(result[0], float)

    def test_related(self):
        RelatedBar = self.env['test_read_group_private.related_bar']
        RelatedFoo = self.env['test_read_group_private.related_foo']
        RelatedBase = self.env['test_read_group_private.related_base']
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_related_base__foo_id"."name",
                    COUNT(*)
            FROM "test_read_group_private_related_base"
            LEFT JOIN "test_read_group_private_related_foo" AS "test_read_group_private_related_base__foo_id"
                ON ("test_read_group_private_related_base"."foo_id" = "test_read_group_private_related_base__foo_id"."id")
            GROUP BY "test_read_group_private_related_base__foo_id"."name"
            ORDER BY "test_read_group_private_related_base__foo_id"."name" ASC
        """]):
            self.assertEqual(
        ...
        foo_bar_name_query = """
            SELECT "test_read_group_private_related_base__foo_id__bar_id"."name",
                    COUNT(*)
            FROM "test_read_group_private_related_base"
            LEFT JOIN "test_read_group_private_related_foo" AS "test_read_group_private_related_base__foo_id"
                ON ("test_read_group_private_related_base"."foo_id" = "test_read_group_private_related_base__foo_id"."id")
            LEFT JOIN "test_read_group_private_related_bar" AS "test_read_group_private_related_base__foo_id__bar_id"
                ON ("test_read_group_private_related_base__foo_id"."bar_id" = "test_read_group_private_related_base__foo_id__bar_id"."id")
            GROUP BY "test_read_group_private_related_base__foo_id__bar_id"."name"
            ORDER BY "test_read_group_private_related_base__foo_id__bar_id"."name" ASC
        """
        ...
        with self.assertQueries(["""
            SELECT COUNT(DISTINCT "test_read_group_private_related_base__foo_id__bar_id"."name")
            FROM "test_read_group_private_related_base"
            LEFT JOIN "test_read_group_private_related_foo" AS "test_read_group_private_related_base__foo_id"
                ON ("test_read_group_private_related_base"."foo_id" = "test_read_group_private_related_base__foo_id"."id")
            LEFT JOIN "test_read_group_private_related_bar" AS "test_read_group_private_related_base__foo_id__bar_id"
    ...
    def test_inherited(self):
        RelatedBase = self.env['test_read_group_private.related_base']
        RelatedInherits = self.env['test_read_group_private.related_inherits']
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_related_inherits__base_id"."name",
                    COUNT(*)
            FROM "test_read_group_private_related_inherits"
            JOIN "test_read_group_private_related_base" AS "test_read_group_private_related_inherits__base_id"
                ON ("test_read_group_private_related_inherits"."base_id" = "test_read_group_private_related_inherits__base_id"."id")
            GROUP BY "test_read_group_private_related_inherits__base_id"."name"
            ORDER BY "test_read_group_private_related_inherits__base_id"."name" ASC
        """]):
            self.assertEqual(
                RelatedInherits._read_group([], ['name'], ['__count']),
                [('a', 3), ('b', 1), (False, 1)],
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_related_inherits__base_id"."name",
                    SUM("test_read_group_private_related_inherits__base_id"."value")
            FROM "test_read_group_private_related_inherits"
            JOIN "test_read_group_private_related_base" AS "test_read_group_private_related_inherits__base_id"
                ON ("test_read_group_private_related_inherits"."base_id" = "test_read_group_private_related_inherits__base_id"."id")
            GROUP BY "test_read_group_private_related_inherits__base_id"."name"
        ...
        expected_query = """
            SELECT "test_read_group_private_related_inherits__base_id__foo_id"."name",
                    COUNT(*)
            FROM "test_read_group_private_related_inherits"
            JOIN "test_read_group_private_related_base" AS "test_read_group_private_related_inherits__base_id"
                ON ("test_read_group_private_related_inherits"."base_id" = "test_read_group_private_related_inherits__base_id"."id")
            LEFT JOIN "test_read_group_private_related_foo" AS "test_read_group_private_related_inherits__base_id__foo_id"
                ON ("test_read_group_private_related_inherits__base_id"."foo_id" = "test_read_group_private_related_inherits__base_id__foo_id"."id")
            GROUP BY "test_read_group_private_related_inherits__base_id__foo_id"."name"
            ORDER BY "test_read_group_private_related_inherits__base_id__foo_id"."name" ASC
        """
    ...
    def test_related_many2many_groupby(self):
        RelatedFoo = self.env['test_read_group_private.related_foo'].with_user(self.base_user)

        # warmup
        RelatedFoo._read_group([], ['bar_base_ids'], ['__count'])
        with self.assertQueries(["""
            SELECT "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id",
                    COUNT(*)
            FROM "test_read_group_private_related_foo"
            LEFT JOIN "test_read_group_private_related_bar" AS "test_read_group_private_related_foo__bar_id"
                ON ("test_read_group_private_related_foo"."bar_id" = "test_read_group_private_related_foo__bar_id"."id")
            LEFT JOIN "trgp_related_bar_trgp_related_base_rel" AS "test_read_group_private_related_foo__bar_id__base_ids__rel"
                ON ("test_read_group_private_related_foo__bar_id"."id" = "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_bar_id")
            GROUP BY "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id"
            ORDER BY "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id" ASC
        """]):
            RelatedFoo._read_group([], ['bar_base_ids'], ['__count'])
        ...
        related_base_model = self.env['ir.model']._get('test_read_group_private.related_base')
        self.env['ir.rule'].create({
            'name': "Only The Lone Wanderer allowed",
        ...
        with self.assertQueries(["""
            SELECT "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id", COUNT(*)
            FROM "test_read_group_private_related_foo"
            LEFT JOIN "test_read_group_private_related_bar" AS "test_read_group_private_related_foo__bar_id"
                ON ("test_read_group_private_related_foo"."bar_id" = "test_read_group_private_related_foo__bar_id"."id")
            LEFT JOIN "trgp_related_bar_trgp_related_base_rel" AS "test_read_group_private_related_foo__bar_id__base_ids__rel"
                ON ("test_read_group_private_related_foo__bar_id"."id" = "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_bar_id"
                    AND "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id" IN (
                        SELECT "test_read_group_private_related_base"."id"
                        FROM "test_read_group_private_related_base"
                        WHERE "test_read_group_private_related_base"."id" IN %s
                    )
                )
            GROUP BY "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id"
            ORDER BY "test_read_group_private_related_foo__bar_id__base_ids__rel"."test_read_group_private_related_base_id" ASC
    ...
    def test_many2many_aggregate(self):
        ...
        Model = self.env['test_read_group_private.task']
    ...
    def test_many2many_compute_not_groupable(self):
        Model = self.env['test_read_group_private.related_bar']
    ...
    def test_groupby_chain_fnames_many2one(self):
        ...
        expected_query = """
            SELECT "test_read_group_private_related_base__foo_id"."bar_id",
                    COUNT(*)
            FROM "test_read_group_private_related_base"
            LEFT JOIN "test_read_group_private_related_foo" AS "test_read_group_private_related_base__foo_id"
                ON ("test_read_group_private_related_base"."foo_id" = "test_read_group_private_related_base__foo_id"."id")
            GROUP BY "test_read_group_private_related_base__foo_id"."bar_id"
            ORDER BY "test_read_group_private_related_base__foo_id"."bar_id" ASC
        """
        ...
        alias_join = f"test_read_group_private_related_base__foo_id__{self.base_user.id}"
        with self.assertQueries([f"""
            SELECT "{alias_join}"."bar_id",
                   COUNT(*)
            FROM "test_read_group_private_related_base"
            LEFT JOIN (
                SELECT "test_read_group_private_related_foo".*
                FROM "test_read_group_private_related_foo"
                WHERE "test_read_group_private_related_foo"."id" IN %s
            ) AS "{alias_join}"
            ON (
                "test_read_group_private_related_base"."foo_id" = "{alias_join}"."id"
            )
    ...
    def test_groupby_chain_fnames_char(self):
        ...
        expected_query = f"""
            ...
            LEFT JOIN "test_read_group_private_related_bar" AS "{alias_join}__bar_id" ON (
