...
class TestFields(TransactionCaseWithUserDemo, TransactionExpressionCase):
    ...
    def test_18_flush_precommit(self):
        ...
        cr = self.env.cr
        record = self.env['test_fields.compute.created'].create({'name': 'foo'})
    ...
    def test_25_related_sudo(self):
        model = self.env['test_fields.related'].with_user(self.env.ref('base.user_admin'))
        bar1 = self.env['test_fields.related_bar'].create({'name': 'B1'})
        bar2 = self.env['test_fields.related_bar'].create({'name': 'B2'})
        foo1 = self.env['test_fields.related_foo'].create({'name': 'F1', 'bar_id': bar1.id})
        foo2 = self.env['test_fields.related_foo'].create({'name': 'F2', 'bar_id': bar2.id})
        record1 = model.create({'name': 'A1', 'foo_id': foo1.id})
        record2 = model.create({'name': 'A2', 'foo_id': foo2.id})
        self.env['ir.rule'].create({
            'name': 'related_foo',
            'model_id': self.env['ir.model']._get('test_fields.related_foo').id,
            ...
            for operator, values in operations.items():
                for value in values:
                    ...
                    with self.subTest(domain=domain, default=default, check_not_null=True):
                        ...
                        if queries:
                            self.assertIn(f'"test_fields_company"."{field_name}" IS NOT NULL', queries[-1])
...
%endif"""
    ...
    def test_61_one2many_domain(self):
        ...
        self.assertEqual(
            ...
            "([('const', '=', True)]) + ([('res_model', '=', 'test_fields.inverse_m2o_ref')])",
    ...
    def test_81_binary_assign(self):
        ...
        record = self.env['test_fields.model_binary'].create({'binary': binary_value1})
    ...
    def test_100_rename_custom_field_inherited(self):
        manual_origin = self.env["ir.model.fields"].create({
            "name": "x_manual_hazel",
            "model_id": self.env["ir.model"]._get("test_fields.related").id,
            "ttype": "char",
        })
        self.assertEqual(manual_origin.state, "manual")
        manual_inherited = self.env["ir.model.fields"]._get("test_fields.related_inherits", "x_manual_hazel")
        ...
        manual_inherited = self.env["ir.model.fields"]._get("test_fields.related_inherits", 'x_manual_hazel_2')
        self.assertTrue(manual_inherited.exists())
    ...
    def test_related_column_type(self):
        related_float_field = self.env['test_fields.related']._fields['foo_float_id']
...
class TestSelectionOndelete(TransactionCase):
    ...
    MODEL_COMPANY_DEPENDENT = 'test_fields.model_selection_company_dependent'
...
class TestModifiedPerformance(TransactionCase):
    ...
    def test_modified_create_no_inverse(self):
        LinePositive = self.env['test_fields.modified.line.positive']
