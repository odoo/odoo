...
class TestFields(TransactionCaseWithUserDemo, TransactionExpressionCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.env.ref('test_orm.test_fields_discussion_0').write({'participants': [Command.link(self.user_demo.id)]})
        # YTI FIX ME: The cache shouldn't be inconsistent (rco is gonna fix it)
        # self.env.ref('test_orm.test_fields_discussion_0').participants -> 1 user
        # self.env.ref('test_orm.test_fields_discussion_0').invalidate()
        # self.env.ref('test_orm.test_fields_discussion_0').with_context(active_test=False).participants -> 2 users
        self.env.ref('test_orm.test_fields_message_0_1').write({'author': self.user_demo.id})

    def test_00_basics(self):
        """ test accessing new fields """
        # find a discussion
        discussion = self.env.ref('test_orm.test_fields_discussion_0')
    ...
    def test_01_basic_get_assertion(self):
        ...
        record = self.env.ref('test_orm.test_fields_message_0_0')
    ...
    def test_01_basic_set_assertion(self):
        ...
        records = self.env['test_fields.message'].search([])
    ...
    def test_05_unknown_fields(self):
        ...
        cat = self.env['test_fields.category'].create({'name': 'Foo'})
    ...
    def test_10_computed(self):
        ...
        field = self.env['test_fields.message']._fields['size']
        self.assertFalse(field.store)
        self.assertFalse(field.compute_sudo)
        self.assertTrue(field.readonly)
        self.assertFalse(field.copy)

        field = self.env['test_fields.message']._fields['name']
        ...
        field = self.env['test_fields.compute.onchange']._fields['baz']
        self.assertTrue(field.store)
        self.assertTrue(field.compute_sudo)
        self.assertFalse(field.readonly)
        self.assertTrue(field.copy)

        field = self.env['test_fields.compute.onchange']._fields['line_ids']
        self.assertTrue(field.store)
        self.assertTrue(field.compute_sudo)
        self.assertFalse(field.readonly)
        self.assertFalse(field.copy)  # like a regular one2many field

        field = self.env['test_fields.compute.onchange']._fields['tag_ids']
        self.assertTrue(field.store)
        self.assertTrue(field.compute_sudo)
        self.assertFalse(field.readonly)
    ...
    def test_10_computed_custom(self):
        ...
        self.env['ir.model.fields'].create({
            ...
            'model_id': self.env.ref('test_orm.model_test_fields_message').id,
            'field_description': 'A boolean computed to false',
            'compute': "for r in self: r['x_bool_false_computed'] = False",
            'store': False,
            'ttype': 'boolean',
        })
        field = self.env['test_fields.message']._fields['x_bool_false_computed']
    ...
    def test_10_computed_custom_invalid_transitive_depends(self):
        ...
        self.env["ir.model.fields"].create(
            {
                ...
                "model_id": self.env.ref("test_orm.model_test_fields_foo").id,
                "state": "manual",
        ...
        fields = self.env["test_fields.foo"]._fields
    ...
    def test_10_context_dependent_related(self):
        ...
        container = self.env['test_fields.compute.container'].create({'name': 'test', 'name_translated': 'test_en'})
        container.with_context(lang='fr_FR').name_translated = 'test_fr'

        member_root = self.env['test_fields.compute.member'].create({'name': 'test'})
    ...
    def test_10_display_name(self):
        ...
        field = self.env.registry['test_fields.discussion'].display_name
    ...
    def test_10_non_stored(self):
        ...
        field = self.env['test_fields.category']._fields['dummy']
        self.assertFalse(field.store)
        self.assertFalse(field.compute)
        self.assertFalse(field.inverse)

        # find messages
        for message in self.env['test_fields.message'].search([]):
        ...
        message1 = self.env['test_fields.message'].create({})
        ...
        record = self.env['test_fields.mixed'].create({})
    ...
    def test_11_stored(self):
        ...
        discussion1 = self.env.ref('test_orm.test_fields_discussion_0')
        ...
        discussion3 = self.env['test_fields.discussion'].create({
            'name': 'Stuff',
    ...
    def test_11_stored_protected(self):
        ...
        model = self.env['test_fields.compute.readonly']
    ...
    def test_12_recursive(self):
        ...
        Category = self.env['test_fields.category']
    ...
    def test_12_recursive_recompute(self):
        ...
        a = self.env['test_fields.recursive'].create({'name': 'A'})
        b = self.env['test_fields.recursive'].create({'name': 'B', 'parent': a.id})
        c = self.env['test_fields.recursive'].create({'name': 'C', 'parent': b.id})
        d = self.env['test_fields.recursive'].create({'name': 'D', 'parent': c.id})
    ...
    def test_12_recursive_tree(self):
        foo = self.env['test_fields.recursive.tree'].create({'name': 'foo'})
    ...
    def test_12_recursive_unlink(self):
        order = self.env['test_fields.recursive.order'].create({'value': 42})
        line = self.env['test_fields.recursive.line'].create({'order_id': order.id})
        task = self.env['test_fields.recursive.task'].create({'value': 42})
    ...
    def test_12_cascade(self):
        ...
        message = self.env.ref('test_orm.test_fields_message_0_0')
        self.env.invalidate_all()
        double_size = message.double_size
        self.assertEqual(double_size, message.size)

        record = self.env['test_fields.cascade'].create({'foo': "Hi"})
    ...
    def test_12_unlink_cascade_active_store(self):
        ...
        """
        message = self.env['test_fields.message'].create({
            'active': False,
        })
        self.env['test_fields.emailmessage'].create(
    ...
    def test_12_unlink_cascade_ir_rule_using_related(self):
        ...
        self.env['ir.rule'].create({
            'model_id': self.env['ir.model']._get_id('test_fields.emailmessage'),
    ...
    def test_12_dynamic_depends(self):
        Model = self.registry['test_fields.compute.dynamic.depends']
        ...
        self.env['ir.config_parameter'].set_str('test_fields.full_name', 'name1,name2')

        # this must re-evaluate the field's dependencies
        self.env.flush_all()
        self.registry._setup_models__(self.cr, ['test_fields.compute.dynamic.depends'])
        self.assertEqual(self.registry.field_depends[Model.full_name], ('name1', 'name2'))

    def test_12_one2many_reference_domain(self):
        model = self.env['test_fields.inverse_m2o_ref']
    ...
    def test_13_inverse(self):
        ...
        log = []
        model = self.env['test_fields.compute.inverse'].with_context(log=log)
        ...
        log = []
        model = self.env['test_fields.multi_compute_inverse'].with_context(log=log)
        record = model.create({
            'bar1': '1',
            'bar2': '2',
            'bar3': '3',
    ...
    def test_13_inverse_access(self):
        ...
        foo = self.env['test_fields.category'].create({'name': 'Foo'})
    ...
    def test_14_search(self):
        ...
        for size in sizes:
            messages0 = self.env['test_fields.message'].search(
                [('discussion', '=', discussion.id), ('size', '<=', size)])

            messages1 = self.env['test_fields.message'].browse()
    ...
    def test_15_constraint(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['test_fields.message'].create({'discussion': discussion.id, 'body': 'Whatever'})
    ...
    def test_15_constraint_inverse(self):
        ...
        log = []
        model = self.env['test_fields.compute.inverse'].with_context(log=log, log_constraint=True)
    ...
    def test_16_compute_unassigned(self):
        model = self.env['test_fields.compute.unassigned']
    ...
    def test_16_compute_unassigned_access_error(self):
        # create two records
        records = self.env['test_fields.compute.unassigned'].create([{}, {}])
        self.env.flush_all()

        # alter access rights: regular users cannot read 'records'
        access = self.env.ref('test_orm.access_test_fields_compute_unassigned')
    ...
    def test_17_compute_depends_on_many2many(self):
        user1, user2, user3 = self.env['test_fields.user'].create([{}, {}, {}])
        group = self.env['test_fields.group'].create({'user_ids': [Command.link(user1.id)]})
        self.env.flush_all()
        ...
        user1 = self.env['test_fields.user'].new({})
        user2 = self.env['test_fields.user'].new({})
        group = self.env['test_fields.group'].new({'user_ids': [user1.id]})
    ...
    def test_18_flush_all(self):
        ...
        self.env.invalidate_all()
        self.assertFalse(self.env['test_fields.compute.created'].search_count([('name', '=', 'foo')]))

        # the computation of field 'created_id' should create a new record with
        # a stored computed field
        record = self.env['test_fields.compute.creator'].create({'name': 'foo'})
    ...
    def test_20_float(self):
        ...
        record = self.env['test_fields.mixed'].create({})
        query = "SELECT 1 FROM test_fields_mixed WHERE id=%s AND number=%s"
    ...
    def test_21_float_digits(self):
        ...
        precision = self.env.ref('test_orm.test_fields_decimal_orm_number')
        description = self.env['test_fields.mixed'].fields_get()['number2']
    ...
    def check_monetary(self, record, amount, currency, msg=None):
        ...
        self.env.flush_all()
        self.cr.execute('SELECT amount FROM test_fields_mixed WHERE id=%s', [record.id])
        value = self.cr.fetchone()[0]
        self.assertEqual(value, samount, msg)

    def test_20_monetary(self):
        """ test monetary fields """
        model = self.env['test_fields.mixed']
    ...
    def test_20_monetary_opw_2223134(self):
        ...
        model = self.env['test_fields.monetary_order']
        currency = self.env.ref('base.USD')

        def check(value):
            self.assertEqual(record.total, value)
            self.env.flush_all()
            self.cr.execute('SELECT total FROM test_fields_monetary_order WHERE id=%s', [record.id])
    ...
    def test_20_monetary_related(self):
        ...
        currency = self.env.ref('base.USD')
        monetary_base = self.env['test_fields.monetary_base'].create({
            'base_currency_id': currency.id,
        })
        monetary_related = self.env['test_fields.monetary_related'].create({
            'monetary_id': monetary_base.id,
            'total': 1 / 3,
        })
        self.env.cr.execute(
            "SELECT total FROM test_fields_monetary_related WHERE id=%s",
    ...
    def test_20_like(self):
        ...
        record = self.env['test_fields.multi.tag'].create({'name': 'Foo'})
    ...
    def test_20_like_multiline(self):
        ...
        record = self.env['test_fields.mixed'].create({'comment1': 'Foo\nBar'})
    ...
    def test_21_date_dynamic(self):
        record = self.env['test_fields.mixed'].create({'moment': fields.Datetime.now()})
    ...
    def test_22_selection(self):
        ...
        record_list = self.env['test_fields.selection'].create({})
        ...
        record_call = self.env['test_fields.mixed'].create({})
    ...
    def test_25_related(self):
        ...
        search_on_related = self.env['test_fields.message'].search([('discussion_name', '=', 'Bar')])
        search_on_regular = self.env['test_fields.message'].search([('discussion.name', '=', 'Bar')])
    ...
    def test_25_related_attributes(self):
        ...
        text = self.registry['test_fields.foo'].text
        ...
        text1 = self.registry['test_fields.bar'].text1
        ...
        text2 = self.registry['test_fields.bar'].text2
        ...
        self.registry._setup_models__(self.cr, ['test_fields.foo'])
        self.assertTrue(self.registry['test_fields.foo'].text.trim)
        self.assertTrue(self.registry['test_fields.bar'].text1.trim)

    def test_25_related_single(self):
        """ test related fields with a single field in the path. """
        record = self.env['test_fields.related'].create({'name': 'A'})
    ...
    def test_25_related_multi(self):
        ...
        foo = self.env['test_fields.foo'].create({'name': 'A', 'value1': 1, 'value2': 2})
        oof = self.env['test_fields.foo'].create({'name': 'B', 'value1': 1, 'value2': 2})
        bar = self.env['test_fields.bar'].create({'name': 'A'})
    ...
    def test_25_related_many2one(self):
        bar = self.env['test_fields.related_bar'].create({'name': 'A'})
        foo = self.env['test_fields.related_foo'].create({'name': 'A', 'bar_id': bar.id})
    ...
    def test_25_one2many_inverse_related(self):
        left = self.env['test_fields.trigger.left'].create({})
        right = self.env['test_fields.trigger.right'].create({})
        ...
        middle = self.env['test_fields.trigger.middle'].create({
            'left_id': left.id,
            'right_id': right.id,
        })
    ...
    def test_27_company_dependent(self):
        ...
        null_record = self.env['test_fields.company'].create({})
        null_record_normal = self.env['test_fields.mixed'].create({})
        ...
        tag0 = self.env['test_fields.multi.tag'].create({'name': 'Qux'})
        tag1 = self.env['test_fields.multi.tag'].create({'name': 'Quux'})
        tag2 = self.env['test_fields.multi.tag'].create({'name': 'Quuz'})

        # create default values for the company-dependent fields
        self.env['ir.default'].set('test_fields.company', 'foo', 'default')
        self.env['ir.default'].set('test_fields.company', 'foo', 'default1', company_id=company1.id)
        self.env['ir.default'].set('test_fields.company', 'tag_id', tag0.id)
        ...
        record = self.env['test_fields.company'].create({
            'foo': 'main',
            'date': '1932-11-09',
        ...
        company_record = self.env['test_fields.company'].create({'foo': 'ABC'})
        attribute_record = self.env['test_fields.company.attr'].create({
        ...
        company_records = self.env['test_fields.company'].search([('foo', '=', 'DEF')])
        self.assertEqual(len(company_records), 1)

    def test_27_company_dependent_bool_integer_float(self):
        company0 = self.env.ref('base.main_company')
        company1 = self.env['res.company'].create({'name': 'A'})
        Model = self.env['test_fields.company']
        record = Model.create({})
        record.invalidate_recordset()
        cr = self.env.cr
        cr.execute("SELECT truth, count, phi FROM test_fields_company WHERE id = %s", (record.id,))
    ...
    def test_27_company_dependent_missing_many2one(self):
        ...
        self.env.cr.execute(
            'SELECT id FROM test_fields_company WHERE id = %s and (tag_id->%s)::int = %s',
    ...
    def test_28_company_dependent_search(self):
                    ...
                    if company_dependent_column_not_null:
                        with self.subTest(domain=domain, default=default):
                            ...
                            if current_thread.query_count:
                                ...
                                expected_contained_sqls = [''] * (current_thread.query_count - 1) + [f'"test_fields_company"."{field_name}" IS NOT NULL']
        ...
        # many2one fields
        tag1, tag2 = self.env['test_fields.multi.tag'].create([{'name': 'one'}, {'name': 'two'}])
...
%endif"""
    ...
    def test_32_prefetch_missing_error(self):
        ...
        Discussion = self.env['test_fields.discussion']
    ...
    def test_40_real_vs_new(self):
        ...
        Model = self.env['test_fields.category']
        ...
        Model = self.env['test_fields.recursive']
        real_record = Model.create({'name': 'Foo'})
        new_origin = Model.new({'name': 'Bar'}, origin=real_record)
        new_record = Model.new({'name': 'Baz'})
        self.assertEqual(real_record.display_name, 'Foo')
    ...
    def test_40_new_defaults(self):
        ...
        new_msg = self.env['test_fields.message'].new({'body': "XXX"})
        ...
        cat1 = self.env['test_fields.category'].create({'name': "Cat1"})
        cat2 = self.env['test_fields.category'].create({'name': "Cat2"})
    ...
    def test_40_new_fields(self):
        ...
        msg0 = self.env['test_fields.message'].create({'body': "XXX"})
        msg1 = self.env['test_fields.message'].create({'body': "WWW"})
        cat0 = self.env['test_fields.category'].create({'name': 'AAA'})
        cat1 = self.env['test_fields.category'].create({'name': 'DDD'})
        new_disc = self.env['test_fields.discussion'].new({
            'name': "Stuff",
        ...
        new_email = self.env['test_fields.emailmessage'].new({'body': "XXX"})
        ...
        new_email = self.env['test_fields.emailmessage'].new({'message': msg0.id})
        self.assertFalse(new_email.id)
    ...
    def test_40_new_inherited_fields(self):
        ...
        email = self.env['test_fields.emailmessage'].new({'body': 'XXX'})
    ...
    def test_40_new_ref_origin(self):
        ...
        disc = self.env.ref('test_orm.test_fields_discussion_0')
        ...
        tag = self.env['test_fields.multi.tag'].create({'name': 'Foo'})
        rec = self.env['test_fields.multi'].create({
    ...
    def test_41_new_compute(self):
        ...
        move = self.env['test_fields.move'].create({
    ...
    def test_41_new_one2many(self):
        ...
        move = self.env['test_fields.move'].create({})
        line = self.env['test_fields.move_line'].create({'move_id': move.id, 'quantity': 1})
        self.env.flush_all()
    ...
    def test_41_new_many2many(self):
        group = self.env['test_fields.group'].create({})
        user0 = self.env['test_fields.user'].create({'group_ids': [Command.link(group.id)]})
        ...
        with self.assertQueryCount(0):
            new_user1 = self.env['test_fields.user'].new({'group_ids': [Command.link(group.id)]})
        ...
        # it is in cache
        with self.assertQueryCount(0):
            new_user2 = self.env['test_fields.user'].new({'group_ids': [Command.link(group.id)]})
    ...
    def test_41_new_related(self):
        ...
        discussion = self.env.ref('test_orm.test_fields_discussion_0')
        message = env['test_fields.message'].new({'discussion': discussion})
    ...
    def test_42_new_related(self):
        ...
        discussion = env['test_fields.discussion'].new({'name': 'Stuff'})
        message = env['test_fields.message'].new({'discussion': discussion})
        self.assertEqual(message.discussion, discussion)
    ...
    def test_43_new_related(self):
        ...
        multi = self.env['test_fields.multi'].new()
    ...
    def test_50_defaults(self):
        ...
        defaults = self.env['test_fields.message'].default_get(fields)
        self.assertEqual(defaults, {'author': self.env.uid})

        defaults = self.env['test_fields.mixed'].default_get(['number'])
        self.assertEqual(defaults, {'number': 3.14})

    def test_50_search_many2one(self):
        """ test search through a path of computed fields"""
        messages = self.env['test_fields.message'].search(
            [('author_partner.name', '=', 'Marc Demo')])
        self.assertEqual(messages, self.env.ref('test_orm.test_fields_message_0_1'))

    def test_51_search_many2one_ordered(self):
        """ test search on many2one ordered by id """
        with self.assertQueries(['''
            SELECT "test_fields_message"."id" FROM "test_fields_message"
            WHERE "test_fields_message"."active" IS TRUE
            ORDER BY  "test_fields_message"."discussion"
        ''']):
            self.env['test_fields.message'].search([], order='discussion')

    def test_52_search_many2one_active_test(self):
        Model = self.env['test_fields.model_active_field']
    ...
    def test_53_boolean_query(self):
        ...
        with self.assertQueries(["""
            SELECT "test_fields_model_active_field"."id"
            FROM "test_fields_model_active_field"
            WHERE "test_fields_model_active_field"."active" IS TRUE
            ORDER BY "test_fields_model_active_field"."id"
        """, """
            SELECT "test_fields_model_active_field"."id"
            FROM "test_fields_model_active_field"
            WHERE "test_fields_model_active_field"."active" IS NOT TRUE
            ORDER BY "test_fields_model_active_field"."id"
        """]):
    ...
    def test_54_not_null_id_query(self):
        ...
        Model = self.env['test_fields.model_active_field'].with_context(active_test=False)
        ...
        with self.assertQueries(["""
            SELECT "test_fields_model_active_field"."id"
            FROM "test_fields_model_active_field"
            WHERE "test_fields_model_active_field"."id" NOT IN %s
            ORDER BY "test_fields_model_active_field"."id"
        """]):
            Model.search([('id', '!=', 1)])
    ...
    def test_60_many2many_domain(self):
        ...
        tag = self.env['test_fields.multi.tag'].create({'name': 'bar'})
        record = self.env['test_fields.multi'].create({'tags': tag.ids})
        self.env.flush_all()
        self.env.invalidate_all()
    ...
    def test_70_x2many_write(self):
        ...
        Message = self.env['test_fields.message']
    ...
    def test_71_relational_inverse(self):
        """ Check the consistency of relational fields with inverse(s). """
        move1 = self.env['test_fields.move'].create({})
        move2 = self.env['test_fields.move'].create({})
        line = self.env['test_fields.move_line'].create({'move_id': move1.id})
        self.env.flush_all()
        self.env.invalidate_all()
    ...
    def test_72_relational_inverse(self):
        ...
        line = self.env['test_fields.move_line'].create({'move_id': move1.id})
        moves = self.env['test_fields.move'].search([('line_ids', 'in', line.id)])
    ...
    def test_73_relational_inverse(self):
        """ Check the consistency of relational fields with inverse(s). """
        discussion1, discussion2 = self.env['test_fields.discussion'].create([
            {'name': "discussion1"}, {'name': "discussion2"},
        ])
        category1, category2 = self.env['test_fields.category'].create([
    ...
    def test_80_copy(self):
        discussion = self.env.ref('test_orm.test_fields_discussion_0')
        message = self.env.ref('test_orm.test_fields_message_0_0')
        message1 = self.env.ref('test_orm.test_fields_message_0_1')

        email = self.env.ref('test_orm.test_fields_emailmessage_0_0')
    ...
    def test_85_binary_guess_zip(self):
        ...
        self.env['test_fields.binary_svg'].with_user(self.user_demo).create({
    ...
    def test_90_binary_svg(self):
        ...
        self.env['test_fields.binary_svg'].create({
            'name': 'Test without attachment',
            'image_wo_attachment': SVG,
        })
        # And this gives error
        with self.assertRaises(UserError):
            self.env['test_fields.binary_svg'].with_user(
                self.user_demo,
            ).create({
                'name': 'Test without attachment',
                'image_wo_attachment': SVG,
    ...
    def test_91_binary_svg_attachment(self):
        ...
        record = self.env['test_fields.binary_svg'].create({
            'name': 'Test without attachment',
        ...
        record = self.env['test_fields.binary_svg'].with_user(
            self.user_demo,
        ).create({
            'name': 'Test without attachment',
            'image_attachment': SVG,
    ...
    def test_93_monetary_related(self):
        ...
        # check base field
        model = self.env['test_fields.monetary_base']
        ...
        model = self.env['test_fields.monetary_inherits']
        field = model._fields['amount']
        self.assertEqual(field.related, 'monetary_id.amount')
        self.assertEqual(field.get_currency_field(model), 'base_currency_id')
    ...
    def test_94_image(self):
        ...
        record = self.env['test_fields.model_image'].create({
            'name': 'image',
            'image': image_w,
            'image_128': image_w,
        })
        ...
        record = self.env['test_fields.model_image'].with_context(image_no_postprocess=True).create({
            'name': 'image',
            'image_256': image_w,
        })
        self.assertEqual(Image.open(io.BytesIO(base64.b64decode(record.image_512))).size, (512, 256))
    ...
    def test_95_binary_bin_size_create(self):
        ...
        record = self.env['test_fields.model_binary'].create({'binary': binary_value})
        ...
        record_no_bin_size = self.env['test_fields.model_binary'].with_context(bin_size=False).create({'binary': binary_value})
        record = self.env['test_fields.model_binary'].browse(record.id)
        record_bin_size = record.with_context(bin_size=True)
        ...
        record_bin_size = self.env['test_fields.model_binary'].with_context(bin_size=True).create({'binary': binary_value})
        record = self.env['test_fields.model_binary'].browse(record.id)
        record_no_bin_size = record.with_context(bin_size=False)
        ...
        record = self.env['test_fields.model_binary'].create({})
        record_no_bin_size = record.with_context(bin_size=False)
        record_bin_size = record.with_context(bin_size=True)
    ...
    def test_96_order_m2o(self):
        belgium, congo = self.env['test_fields.country'].create([
            {'name': "Duchy of Brabant"},
            {'name': "Congo"},
        ])
        cities = self.env['test_fields.city'].create([
    ...
    def test_97_ir_rule_m2m_field(self):
        ...
        tag = self.env['test_fields.multi.tag'].create({})
        record = self.env['test_fields.multi.line'].create({
            'name': 'image',
    ...
    def test_98_prefetch_translate(self):
        Model = self.registry['test_fields.prefetch']
    ...
    def test_99_prefetch_group(self):
        records = self.env['test_fields.prefetch'].create([{} for _ in range(10)])
        self.env.flush_all()
        self.env.invalidate_all()

        with self.assertQueries(["""
            SELECT "test_fields_prefetch"."id",
                   "test_fields_prefetch"."name"->>%s,
                   "test_fields_prefetch"."description"->>%s,
                   "test_fields_prefetch"."html_description"->>%s,
                   "test_fields_prefetch"."create_uid",
                   "test_fields_prefetch"."create_date",
                   "test_fields_prefetch"."write_uid",
                   "test_fields_prefetch"."write_date"
            FROM "test_fields_prefetch"
            WHERE "test_fields_prefetch"."id" IN %s
        """]):
            records.mapped('name')  # fetch all fields with prefetch=True

        with self.assertQueries(["""
            SELECT
                "test_fields_prefetch"."id",
                "test_fields_prefetch"."harry",
                "test_fields_prefetch"."hermione",
                "test_fields_prefetch"."ron"
            FROM "test_fields_prefetch"
            WHERE "test_fields_prefetch"."id" IN %s
        """]):
        ...
        with self.assertQueries(["""
            SELECT
                "test_fields_prefetch"."id",
                "test_fields_prefetch"."hansel",
                "test_fields_prefetch"."gretel"
            FROM "test_fields_prefetch"
            WHERE "test_fields_prefetch"."id" IN %s
        """]):
    ...
    def test_cache_key_invalidation(self):
        ...
        # this uses company0
        record = self.env['test_fields.company'].with_user(user0).create({
            'foo': 'main',
        })
    ...
    def test_field_set_prefetch(self):
        records = self.env['test_fields.prefetch'].create([
...
class TestX2many(TransactionExpressionCase):
    ...
    def test_definition_many2many(self):
        ...
        field = self.env['test_fields.multi.line']._fields['tags']
        self.assertEqual(field.relation, 'test_fields_multi_line_test_fields_multi_tag_rel')
        self.assertEqual(field.column1, 'test_fields_multi_line_id')
        self.assertEqual(field.column2, 'test_fields_multi_tag_id')

        field = self.env['test_fields.multi.line2']._fields['tags']
        self.assertEqual(field.relation, 'test_fields_multi_line2_test_fields_multi_tag_rel')
        self.assertEqual(field.column1, 'test_fields_multi_line2_id')
        self.assertEqual(field.column2, 'test_fields_multi_tag_id')

    def test_10_ondelete_many2many(self):
        """Test A can't be deleted when used on the relation."""
        record_a = self.env['test_fields.model_a'].create({'name': 'a'})
        record_b = self.env['test_fields.model_b'].create({'name': 'b'})
        record_a.write({
    ...
    def test_search_many2many(self):
        ...
        tags = self.env['test_fields.multi.tag']
        tagA = tags.create({})
        tagB = tags.create({})
        tagC = tags.create({})
        recs = self.env['test_fields.multi.line']
        recW = recs.create({})
    ...
    def test_search_one2many(self):
        ...
        recs = self.env['test_fields.multi']
    ...
    def test_create_batch_m2m(self):
        lines = self.env['test_fields.multi.line'].create([{
    ...
    def test_sudo_commands(self):
        ...
        # 2. many2many field `test_fields.discussion.participants`
        # Sanity checks
        # `test_fields.user` must be flagged as `_allow_sudo_commands = False` otherwise the test is pointless
        self.assertEqual(self.env['test_fields.group']._allow_sudo_commands, False)
        # in case the type of `test_fields.discussion.participants` changes in a future release.
        # if `test_fields.discussion.participants` is no longer a many2many, this test must be adapted.
        self.assertEqual(self.env['test_fields.user']._fields['group_ids'].type, 'many2many')
        public_group = self.env['test_fields.group'].with_user(admin_user).create({
            'name': 'public',
        }).with_user(self.env.user)
        with self.assertRaises(AccessError):
            # the default user on the transaction has no access
            self.env['test_fields.user'].with_user(admin_user).create({
                'name': 'foo',
                'group_ids': [public_group.id],
            })
        u = self.env['test_fields.user'].with_user(admin_user).sudo().create({
            'name': 'foo',
            'group_ids': [public_group.id],
        }).with_user(self.env.user)

        for User, my_user in [
            (self.env['test_fields.user'].with_user(admin_user), u.with_user(admin_user)),
            (self.env['test_fields.user'].sudo(), u.sudo()),
        ]:
            # 2.0 Command.CREATE
            # Case: a public/portal user creating a new users with arbitrary values
            with self.assertRaisesRegex(AccessError, "not allowed to create 'test_fields.group'"):
                User.create({
                    'name': 'foo',
            ...
            # 2.1 Command.UPDATE
            # Case: a public/portal updating his user to add himself a group
            with self.assertRaisesRegex(AccessError, "not allowed to modify 'test_fields.group'"):
                my_user.write({
                    'group_ids': [Command.update(my_user.group_ids[0].id, {})],
                })
            # 2.2 Command.DELETE
            # Case: a public user deleting the public user to mess with the database
            with self.assertRaisesRegex(AccessError, "not allowed to delete 'test_fields.group'"):
                my_user.write({
...
class TestHtmlField(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        self.model = self.env['test_fields.mixed']
...
%endif"""
    ...
    def test_onchange_sanitize(self, patch):
        self.assertTrue(self.registry['test_fields.mixed'].comment2.sanitize)
...
class TestMagicFields(TransactionCase):
    ...
    def test_write_date(self):
        record = self.env['test_fields.discussion'].create({'name': 'Booba'})
    ...
    def test_mro_mixin(self):
        ...
        #                                |
        #                                |
        #   ExtendedDisplay    'test_fields.mixin'    Display    'base'
        #         |                      |            |         |
        #         +----------------------+------------+---------+
        #                                |
        #                       'test_fields.display'
        ...
        # above.  The field 'display_name' on the model 'test_fields.mixin' is
        # expected to be automatic and non-stored.  But the field 'display_name'
        # on the model 'test_fields.display' should not be automatic: it must
        ...
        field = registry['test_fields.display'].display_name
        self.assertTrue(field.store)
...
class TestParentStore(TransactionCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        #      /|\
        #     7 8 9
        Cat = cls.env['test_fields.category']
    ...
    def test_with_ir_rule_behavior(self):
        self.env['ir.rule'].create({
            'name': 'category rule',
            'model_id': self.env['ir.model']._get('test_fields.category').id,
        ...
        Dis = self.env['test_fields.discussion']
...
class TestRequiredMany2oneTransient(TransactionCase):
    ...
    def test_explicit_ondelete(self):
        field = self.env['test_fields.req_m2o_transient']._fields['foo']
        self.assertEqual(field.ondelete, 'restrict')

    def test_implicit_ondelete(self):
        field = self.env['test_fields.req_m2o_transient']._fields['bar']
        self.assertEqual(field.ondelete, 'cascade')

    def test_explicit_set_null(self):
        Model = self.env['test_fields.req_m2o_transient']
        field = Model._fields['foo']
...
class TestMany2oneReference(TransactionExpressionCase):
    ...
    def test_delete_m2o_reference_records(self):
        m = self.env['test_fields.model_many2one_reference']
        self.env.cr.execute("SELECT max(id) FROM test_fields_model_many2one_reference")
    ...
    def test_search_inverse_one2many_bypass_search_access(self):
        record = self.env['test_fields.inverse_m2o_ref'].create({})
        ...
        reference = self.env['test_fields.model_many2one_reference'].create({'res_id': record.id})
...
class TestSelectionDeleteUpdate(TransactionCase):
    ...
    MODEL_ABSTRACT = 'test_fields.state_mixin'
...
class TestSelectionUpdates(TransactionCase):
    MODEL_BASE = 'test_fields.model_selection_base'
    MODEL_RELATED = 'test_fields.model_selection_related'
    MODEL_RELATED_UPDATE = 'test_fields.model_selection_related_updatable'
...
class TestSelectionOndelete(TransactionCase):
    ...
    MODEL_BASE = 'test_fields.model_selection_base'
    MODEL_REQUIRED = 'test_fields.model_selection_required'
    MODEL_NONSTORED = 'test_fields.model_selection_non_stored'
    MODEL_WRITE_OVERRIDE = 'test_fields.model_selection_required_for_write_override'
...
class TestFieldParametersValidation(TransactionCase):
    def test_invalid_parameter(self):
        ...
        class Foo(models.Model):
            _module = None
            _name = _description = 'test_fields.field_parameter_validation'
        ...
        self.assertTrue(cm.output[0].startswith(
            "WARNING:odoo.fields:Field test_fields.field_parameter_validation.name: "
...
class TestComputeQueries(TransactionCase):
    ...
    def test_compute_readwrite(self):
        model = self.env['test_fields.compute.readwrite']
        model.create({})
    ...
    def test_compute_inverse(self):
        model = self.env['test_fields.compute.inverse']
        model.create({})
    ...
    def test_x2many_computed_inverse(self):
        record = self.env['test_fields.compute.inverse'].create(
    ...
    def test_multi_create(self):
        model = self.env['test_fields.foo']
        model.create({})
    ...
    def test_create_cache_consistency(self):
        ...
        """
        record = self.env['test_fields.create.performance'].create({})
    ...
    def test_create_cache_of_compute_store_fields(self):
        model = self.env['test_fields.create.performance']
        model.create({})  # warmup
    ...
    def test_partial_compute_batching(self):
        ...
        """
        order = self.env['test_fields.order'].new({
            'line_ids': [Command.create({'reward': False})] * 100,
        })

        OrderLine = self.env.registry['test_fields.order.line']
        with patch.object(
            OrderLine,
...
class TestComputeSudo(TransactionCaseWithUserDemo):
    def test_compute_sudo_depends_context_uid(self):
        record = self.env['test_fields.compute.sudo'].create({})
...
class test_shared_cache(TransactionCaseWithUserDemo):
    def test_shared_cache_computed_field(self):
        ...
        task = self.env['test_fields.model_shared_cache_compute_parent'].create({
            'name': 'Shared Task'})
        self.env['test_fields.model_shared_cache_compute_line'].create({
...
class TestUnlinkConstraints(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        MODEL = cls.env['test_fields.model_constrained_unlinks']
...
class TestWrongRelatedError(TransactionCase):
    def test_wrong_related_path(self):
        ...
        class Foo(models.Model):
            _module = None
            _name = _description = 'test_fields.wrong_related_path'

            foo_id = fields.Many2one('test_fields.foo')
        ...
        errMsg = (
            ...
            "test_fields.wrong_related_path.foo_non_existing does not exist."
        )
...
class TestPrecomputeModel(TransactionCase):
    ...
    def test_precompute_consistency(self):
        Model = self.registry['test_fields.precompute']
        ...
        with self.assertWarns(UserWarning):
            self.registry._setup_models__(self.cr, ['test_fields.precompute'])
    ...
    def test_precompute_dependencies_one2many(self):
        Model = self.registry['test_fields.precompute']
        Line = self.registry['test_fields.precompute.line']
        ...
        with self.assertWarns(UserWarning):
            self.registry._setup_models__(self.cr, ['test_fields.precompute', 'test_fields.precompute.line'])
            self.registry.get_trigger_tree(Model._fields.values())
...
class TestPrecompute(TransactionCase):
    ...
    def test_precompute(self):
        ...
        model = self.env['test_fields.precompute']
        Model = self.registry['test_fields.precompute']
        self.assertTrue(Model.lower.precompute)
        self.assertTrue(Model.upper.precompute)
        self.assertTrue(Model.lowup.precompute)
    ...
    def test_precompute_combo(self):
        model = self.env['test_fields.precompute.combo']
    ...
    def test_precompute_editable(self):
        model = self.env['test_fields.precompute.editable']
    ...
    def test_precompute_readonly(self):
        ...
        """
        model = self.env['test_fields.precompute.readonly']
    ...
    def test_precompute_required(self):
        model = self.env['test_fields.precompute.required']
    ...
    def test_precompute_monetary(self):
        ...
        model = self.env['test_fields.precompute.monetary']
...
class TestModifiedPerformance(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.Modified = cls.env['test_fields.modified']
        cls.ModifiedLine = cls.env['test_fields.modified.line']
    ...
    def test_modified_trigger_recursive_partial_invalidate(self):
        ...
        with self.assertQueries(["""
            SELECT "test_fields_modified_line"."id",
                   "test_fields_modified_line"."modified_id",
                   "test_fields_modified_line"."quantity",
                   "test_fields_modified_line"."parent_id",
                   "test_fields_modified_line"."create_uid",
                   "test_fields_modified_line"."create_date"
            FROM "test_fields_modified_line"
            WHERE "test_fields_modified_line"."id" IN %s
        """, """
            SELECT "test_fields_modified_line"."id",
                   "test_fields_modified_line"."parent_id"
            FROM "test_fields_modified_line"
