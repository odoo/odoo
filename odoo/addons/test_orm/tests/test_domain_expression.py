...
class TestExpression(TestOrmPartnerCommon, SavepointCaseWithUserDemo, TransactionExpressionCase):
    ...
    def test_10_hierarchy_access(self):
        Partner = self.env['test_orm.domain_expression.partner'].with_user(self.user_demo)
        ...
        Bank = self.env['test_orm.domain_expression.partner.bank'].with_user(self.user_demo)
        bank_top, bank_med, bank_bot = Bank.create([
            {'formatted_account_number': '1', 'partner_id': top.id},
            {'formatted_account_number': '2', 'partner_id': med.id},
            {'formatted_account_number': '3', 'partner_id': bot.id},
        ])
    ...
    def test_15_m2o(self):
        ...
        partners = self._search(Partner, [('business_id', 'in', [])])
        ...
        # create new business with partners, and partners with no business
        business2 = self.env['test_orm.business'].create({
            'name': 'Acme 2',
            'partner_id': self.env['test_orm.partner'].create({'name': 'Acme 2'}).id,
        })
        for i in range(4):
            Partner.create({'name': 'P of Acme %s' % i, 'business_id': business2.id})
            Partner.create({'name': 'P of All %s' % i, 'business_id': False})

        # check if many2one works with negative empty list
        all_partners = Partner.search([])
        res_partners = self._search(Partner, ['|', ('business_id', 'not in', []), ('business_id', '=', False)])
        self.assertEqual(all_partners, res_partners, "not in [] fails")

        # check that many2one will pick the correct records with a list
        partners = self._search(Partner, [('business_id', 'in', [False])])
        self.assertTrue(len(partners) >= 4, "We should have at least 4 partners with no business")

        # check that many2one will exclude the correct records with a list
        partners = self._search(Partner, [('business_id', 'not in', [1])])
        self.assertTrue(len(partners) >= 4, "We should have at least 4 partners not related to business #1")

        # check that many2one will exclude the correct records with a list and False
        partners = self._search(Partner, ['|', ('business_id', 'not in', [1]),
                                        ('business_id', '=', False)])
        self.assertTrue(len(partners) >= 8, "We should have at least 8 partners not related to business #1")

        # check that multi-level expressions also work
        partners = self._search(Partner, [('business_id.partner_id', 'in', [])])
        self.assertFalse(partners)
        ...
        all_partners = self._search(Partner, [('business_id', '!=', False)])

        # check with empty list
        res_partners = self._search(Partner, [('business_id.partner_id', 'not in', [])])
        self.assertEqual(all_partners, res_partners, "not in [] fails")
        ...
        business = self.env['test_orm.business'].search([])
        res_101 = self._search(business, [('partner_id', 'not ilike', '')])  # get no business
        self.assertFalse(res_101)
        res_102 = self._search(business, [('partner_id', 'ilike', '')])  # get all business
        self.assertEqual(res_102, business)
    ...
    def test_15_o2m_subselect(self):
        ...
        state_us_1, state_us_2, state_us_3 = self.env['test_orm.country.state'].create([
            {'name': 'Alabama', 'country_id': country_us.id},
            {'name': 'Alaska', 'country_id': country_us.id},
            {'name': 'Arizona', 'country_id': country_us.id},
        ])
    ...
    def test_15_equivalent_one2many_1(self):
        Business = self.env['test_orm.business']
        business3 = Business.create({
            'name': 'Acme 3',
            'partner_id': self.env['test_orm.partner'].create({'name': 'Acme 3'}).id,
        })
        ...
        res_1 = self._search(Business, [('child_ids', 'in', business3.child_ids.ids)])  # any business having a child of business3 as child
        self.assertEqual(res_1, business3)
        res_2 = self._search(Business, [('child_ids', 'in', business3.child_ids[0].ids)])  # any business having the first child of business3 as child
        self.assertEqual(res_2, business3)

        # child_of x returns x and its children (direct or not).
        expected = business3 + business4
        res_1 = self._search(Business, [('id', 'child_of', [business3.id])])
        self.assertEqual(res_1, expected)
        res_2 = self._search(Business, [('id', 'child_of', business3.id)])
        self.assertEqual(res_2, expected)
        res_3 = self._search(Business, [('id', 'child_of', [business3.name])])
        self.assertEqual(res_3, expected)
        res_4 = self._search(Business, [('id', 'child_of', business3.name)])
        self.assertEqual(res_4, expected)

        # parent_of x returns x and its parents (direct or not).
        expected = business3 + business4
        res_1 = self._search(Business, [('id', 'parent_of', [business4.id])])
        self.assertEqual(res_1, expected)
        res_2 = self._search(Business, [('id', 'parent_of', business4.id)])
        self.assertEqual(res_2, expected)
        res_3 = self._search(Business, [('id', 'parent_of', [business4.name])])
        self.assertEqual(res_3, expected)
        res_4 = self._search(Business, [('id', 'parent_of', business4.name)])
        self.assertEqual(res_4, expected)

        # try testing real subsets with IN/NOT IN
        Partner = self.env['test_orm.domain_expression.partner']
        Users = self.env['test_orm.domain_expression.users']
        up1, up2 = Users.create([
            {'name': 'Dédé Boitaclou'},
            {'name': "Raoulette Pizza O'poil"},
        ])
        p1, p2 = up1.partner_id.id, up2.partner_id.id
        u1a, u1b, u2 = Users.create([
            {'partner_id': p1},
            {'partner_id': p1},
            {'partner_id': p2},
        ]).ids
    ...
    def test_15_equivalent_one2many_2(self):
        ...
        country_state1 = self._search(State, [('country_id', 'not like', 'probably_unexisting_name')])
        country_state2 = self._search(State, [('id', 'not in', [non_country_id])])
        self.assertEqual(country_state1, country_state2)
        country_state3 = self._search(State, [('id', 'not in', [])])
        self.assertEqual(country_state1, country_state3)

        # one2many towards another model
        self.assertEqual(country_fr, self._search(Country, [('state_ids', 'in', country_fr.state_ids.ids)]))
        self.assertEqual(country_fr, self._search(Country, [('state_ids', 'in', country_fr.state_ids[0].ids)]))
        self.assertEqual(country_fr, self._search(Country, [('state_ids', 'in', country_fr.state_ids[0].id)]))
        ...
        self.assertEqual(expected, self._search(Country, [('state_ids', 'not in', [non_country_id])]))
        self.assertEqual(expected, self._search(Country, [('state_ids', '!=', False)]))
        self.assertEqual(expected, self._search(Country, [('state_ids', 'not in', [])]))

    def test_15_hierarchy_ilike(self):
        Business = self.env['test_orm.business']
        business1 = Business.create({
            'name': 'Hierarchy Parent',
            'partner_id': self.env['test_orm.partner'].create({'name': 'Hierarchy Parent'}).id,
        })
        Business.create({
            'name': 'Child',
            'partner_id': self.env['test_orm.partner'].create({'name': 'Child'}).id,
            'parent_id': business1.id,
        })
        base_domain = Domain('id', 'child_of', business1.id)
        businesses = self._search(Business, base_domain)

        self.assertEqual(businesses, self._search(Business, base_domain & Domain('id', 'child_of', 'Parent')))
        self.assertEqual(businesses, self._search(Business, base_domain & Domain('id', 'parent_of', 'Chi')))

    def test_20_inherited_field_search(self):
        ...
        self.assertTrue(Users._fields['name'].inherited)

        # Create users
        a = Users.create({'name': 'test_A'})
        b1 = Users.create({'name': 'test_B'})
        b2 = Users.create({'name': 'test_B2', 'parent_id': b1.partner_id.id})
...
class TestExpression2(TransactionExpressionCase):
    ...
    def test_long_table_alias(self):
        ...
        self.patch(self.registry['test_orm.domain_expression.users'], '_order', 'partner_id')
        self.patch(self.registry['test_orm.domain_expression.partner'], '_order', 'commercial_partner_id,country_id,name')
        self.env['test_orm.domain_expression.users'].search([('name', '=', 'test')])
...
class TestBypassAccess(TransactionExpressionCase):
    ...
    def test_bypass_search_access(self):
        # Get models
        partner_obj = self.env['test_orm.domain_expression.partner']
        category_obj = self.env['test_orm.partner.category']
        country_obj = self.env['test_orm.domain_expression.country']
        state_obj = self.env['test_orm.domain_expression.country.state']
        bank_obj = self.env['test_orm.domain_expression.partner.bank']
        ...
        country_us = country_obj.create([{'name': 'United States', 'code': 'US'}])
        states = state_obj.create([{
            'name': name,
            'code': code,
            'country_id': country_us.id,
        } for name, code in (('AL', 'Alabama'), ('AK', 'Alaska'))])
        categories = category_obj.create([{'name': name} for name in ('foo', 'bar')])
        ...
        b_aa = bank_obj.create({'formatted_account_number': '123', 'partner_id': p_aa.id})
        b_ab = bank_obj.create({'formatted_account_number': '456', 'partner_id': p_ab.id})
        b_ba = bank_obj.create({'formatted_account_number': '789', 'partner_id': p_ba.id})
        ...
        partners = self._search(partner_obj, [('bank_ids.account_number', 'like', name_test)])
        self.assertEqual(partners, p_aa,
            "bypass_search_access off: ('bank_ids.account_number', 'like', '..'): incorrect result")

        partners = self._search(partner_obj, ['|', ('name', 'like', 'C'), ('bank_ids.account_number', 'like', name_test)])
        self.assertIn(p_aa, partners,
            "bypass_search_access off: '|', ('name', 'like', 'C'), ('bank_ids.account_number', 'like', '..'): incorrect result")
        ...
        partners = self._search(partner_obj, [('bank_ids.account_number', 'like', name_test)])
        self.assertEqual(partners, p_aa,
            "bypass_search_access on: ('bank_ids.account_number', 'like', '..') incorrect result")

        partners = self._search(partner_obj, ['|', ('name', 'like', 'C'), ('bank_ids.account_number', 'like', name_test)])
        self.assertIn(p_aa, partners,
            "bypass_search_access on: '|', ('name', 'like', 'C'), ('bank_ids.account_number', 'like', '..'): incorrect result")
        self.assertIn(p_c, partners,
            "bypass_search_access on: '|', ('name', 'like', 'C'), ('bank_ids.account_number', 'like', '..'): incorrect result")
        ...
        patch_domain(partner_obj, 'bank_ids', [('account_number', 'like', '2')])
...
class TestQueries(TransactionCase):
    ...
    def test_order(self):
        ...
        with self.assertQueries(['''
            ...
            FROM "test_orm_partner"
            WHERE ...
            ORDER BY "test_orm_partner"."business_id"
        ''']):
            Model.search(domain, order='business_id.id')

        with self.assertQueries(['''
            SELECT "test_orm_partner"."id"
            FROM "test_orm_partner"
            WHERE ...
            ORDER BY "test_orm_partner"."business_id" DESC
        ''']):
            Model.search(domain, order='business_id.id DESC')

        with self.assertRaises(ValueError):
            Model.search(domain, order='business_id.name')
...
class TestMany2one(TransactionCase):
    def setUp(self):
        ...
        self.partner = self.env['test_orm.partner'].create([{'name': 'foo'}])
        self.business = self.env['test_orm.business'].create([{'partner_id': self.partner.id}])
    ...
    def test_regular(self):
        self.Partner.search([('business_id.partner_id.name', 'like', self.business.name)])
        ...
        with self.assertQueries(['''
            SELECT "test_orm_partner"."id"
            FROM "test_orm_partner"
            LEFT JOIN "test_orm_business" AS "test_orm_partner__business_id"
            ON ("test_orm_partner"."business_id" = "test_orm_partner__business_id"."id")
            WHERE ("test_orm_partner"."business_id" IS NOT NULL
                AND "test_orm_partner__business_id"."id" IS NOT NULL
                AND "test_orm_partner__business_id"."name" LIKE %s)
            ORDER BY ...
        ''']):
        ...
        with self.assertQueries(['''
            ...
            LEFT JOIN "test_orm_business" AS "test_orm_partner__business_id"
            ON ("test_orm_partner"."business_id" = "test_orm_partner__business_id"."id")
            LEFT JOIN "test_orm_partner" AS "test_orm_partner__business_id__partner_id"
            ON ("test_orm_partner__business_id"."partner_id" = "test_orm_partner__business_id__partner_id"."id")
            WHERE ("test_orm_partner"."business_id" IS NOT NULL
                AND "test_orm_partner__business_id"."id" IS NOT NULL
                AND "test_orm_partner__business_id__partner_id"."name" LIKE %s)
            ORDER BY ...
        ''']):
            self.Partner.search([('business_id.partner_id.name', 'like', self.business.name)])
        ...
        with self.assertQueries(['''
            ...
            WHERE (
                ("test_orm_partner"."business_id" IS NOT NULL
                    AND "test_orm_partner__business_id"."id" IS NOT NULL
                    AND "test_orm_partner__business_id"."name" LIKE %s)
                OR ("test_orm_partner"."country_id" IS NOT NULL
        ...
        ''']):
            self.Partner.search([
                '|',
                ('business_id.name', 'like', self.business.name),
    ...
    def test_explicit_subquery(self):
        ...
        ''']):
            ...
            self.Partner.search([('business_id', 'in', business_ids)])
        ...
        ''']):
            business_ids = self.business._search([('name', 'like', self.business.name)], order='id')
            business_ids = tuple(business_ids)
            self.Partner.search([('business_id', 'in', business_ids)])
        ...
        ''']):
            ...
            business_ids = companies._as_query(ordered=False)
            self.Partner.search([('business_id', 'in', business_ids)])
        ...
        ''']):
            business_ids = self.business._search([('name', 'like', self.business.name)], order='id')
            self.Partner.search([('business_id', 'in', business_ids.subselect())])

    def test_bypass_search_access(self):
        # bypass_search_access on the first many2one
        self.patch(self.Partner._fields['business_id'], 'bypass_search_access', True)
        self.patch(self.business._fields['partner_id'], 'bypass_search_access', False)
        self.Partner.search([('business_id.partner_id.name', 'like', self.business.name)])

        with self.assertQueries(['''
            SELECT "test_orm_partner"."id"
            FROM "test_orm_partner"
            LEFT JOIN "test_orm_business" AS "test_orm_partner__business_id" ON
                ("test_orm_partner"."business_id" = "test_orm_partner__business_id"."id")
            WHERE ("test_orm_partner"."business_id" IS NOT NULL
                AND "test_orm_partner__business_id"."id" IS NOT NULL
                AND "test_orm_partner__business_id"."name" LIKE %s)
            ORDER BY ...
        ...
        with self.assertQueries(['''
            ...
            LEFT JOIN "test_orm_business" AS "test_orm_partner__business_id" ON
                ("test_orm_partner"."business_id" = "test_orm_partner__business_id"."id")
            LEFT JOIN "test_orm_partner" AS "test_orm_partner__business_id__partner_id" ON
                ("test_orm_partner__business_id"."partner_id" = "test_orm_partner__business_id__partner_id"."id")
            WHERE ("test_orm_partner"."business_id" IS NOT NULL
                AND "test_orm_partner__business_id"."id" IS NOT NULL
                AND "test_orm_partner__business_id__partner_id"."name" LIKE %s)
            ORDER BY ...
        ...
        with self.assertQueries(['''
            ...
            WHERE ("test_orm_partner"."business_id" IS NOT NULL
                AND "test_orm_partner__business_id"."id" IS NOT NULL
                AND "test_orm_partner__business_id"."parent_id" IS NULL)
            ORDER BY ...
        ''']):
            self.Partner.search([('business_id.parent_id', '=', False)])

        # bypass_search_access on the second many2one
        self.patch(self.Partner._fields['business_id'], 'bypass_search_access', False)
        self.patch(self.business._fields['partner_id'], 'bypass_search_access', True)
        self.Partner.search([('business_id.partner_id.name', 'like', self.business.name)])
    ...
    def test_name_search(self):
        self.Partner.search([('business_id', 'like', self.business.name)])
...
class TestOne2many(TransactionCase):
    def setUp(self):
        ...
        self.partner = self.Partner.create({
            'name': 'Foo',
            'bank_ids': [
                Command.create({'formatted_account_number': '123'}),
                Command.create({'formatted_account_number': '456'}),
                Command.create({'formatted_account_number': '789'}),
            ],
        })
    ...
    def test_regular(self):
        ...
        self.Partner.search([('bank_ids.account_number', 'like', '12')])
        self.Partner.search([('child_ids.bank_ids.account_number', 'like', '12')])
        ...
        with self.assertQueries(['''
            ...
            WHERE EXISTS (SELECT FROM (
                SELECT "test_orm_domain_expression_partner_bank"."partner_id" AS __inverse
                FROM "test_orm_domain_expression_partner_bank"
                WHERE "test_orm_domain_expression_partner_bank"."account_number" LIKE %s
            ) AS __sub WHERE __inverse = "test_orm_domain_expression_partner"."id")
            ORDER BY ...
        ''']):
            self.Partner.search([('bank_ids.account_number', 'like', '12')])
        ...
        with self.assertQueries(['''
            ...
            WHERE EXISTS (SELECT FROM (
                ...
                FROM "test_orm_domain_expression_partner"
                WHERE (
                    "test_orm_domain_expression_partner"."active" IS TRUE
    ...
    def test_bypass_search_access(self):
        ...
        ''']):
            self.Partner.search([
                ('bank_ids.account_number', 'like', '12'),
                ('bank_ids.account_number', 'like', '45'),
            ])
        ...
        self.patch(self.Partner._fields['bank_ids'], 'domain', [('account_number', 'like', '2')])
            ...
            WHERE EXISTS (SELECT FROM (
                ...
                WHERE (
                    EXISTS (SELECT FROM (
                        ...
                        FROM "test_orm_domain_expression_partner_bank"
                        WHERE (
                            "test_orm_domain_expression_partner_bank"."account_number" LIKE %s
                            AND "test_orm_domain_expression_partner_bank"."id" IN %s
                        )
                    ) AS __sub WHERE __inverse = "test_orm_domain_expression_partner"."id")
                    AND (
...
class TestAnyfy(TransactionCase):
    ...
    def test_and_multiple_many2one_with_subfield(self):
        self._test_combine_anies([
            '&', '&',
                ('business_id.name', '=', 'SGC'),
                ('business_id.name', '=', 'NID'),
                ('business_id.name', '=', 'Free Jaffa Nation'),
        ], [
            ('business_id', 'any', [
                '&', '&',
                    ('name', '=', 'SGC'),
