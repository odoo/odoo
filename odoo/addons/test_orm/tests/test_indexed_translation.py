...
class TestIndexedTranslation(TransactionExpressionCase):
    ...
    def test_search_ilike(self):
        record_en = self.env['test_indexed_translation.indexed_translation'].with_context(lang='en_US').create({})
        ...
        with self.assertQueries(["""
            SELECT "test_indexed_translation_indexed_translation"."id"
            FROM "test_indexed_translation_indexed_translation"
            WHERE (jsonb_path_query_array("test_indexed_translation_indexed_translation"."name", '$.*')::text ILIKE %s
            AND "test_indexed_translation_indexed_translation"."name"->>%s ILIKE %s)
            ORDER BY "test_indexed_translation_indexed_translation"."id"
        """, """
            SELECT "test_indexed_translation_indexed_translation"."id"
            FROM "test_indexed_translation_indexed_translation"
            WHERE (jsonb_path_query_array("test_indexed_translation_indexed_translation"."name", '$.*')::text ILIKE %s
            AND COALESCE("test_indexed_translation_indexed_translation"."name"->>%s, "test_indexed_translation_indexed_translation"."name"->>%s) ILIKE %s)
            ORDER BY "test_indexed_translation_indexed_translation"."id"
        """, """
            SELECT "test_indexed_translation_indexed_translation"."id"
            FROM "test_indexed_translation_indexed_translation"
