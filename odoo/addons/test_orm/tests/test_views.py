...
class TestDefaultView(common.TransactionCase):
    ...
    def test_default_view_with_binaries(self):
        self.assertEqual(
            etree.tostring(self.env['test_views.binary.test']._get_default_form_view()),
            b'<form><sheet string="test_views.binary.test"><group><group><field name="img"/></group><group><field name="bin1"/></group></group><group><separator/></group></sheet></form>'
        )

    def test_default_calender_view(self):
        self.assertEqual(
            etree.tostring(self.env['test_views.calendar.test']._get_default_calendar_view()),
            b'<calendar string="test_views.calendar.test" date_start="x_date_start" date_stop="x_date_end"><field name="id"/></calendar>'
        )
