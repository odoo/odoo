...
class TestDefaultView(common.TransactionCase):
    ...
    def test_default_form_view(self):
        self.assertEqual(
            etree.tostring(self.env['test_views.message']._get_default_form_view()),
        ...
        self.assertEqual(
            etree.tostring(self.env['test_views.creativework.edition']._get_default_form_view()),
        ...
        self.assertEqual(
            etree.tostring(self.env['test_views.mixed']._get_default_form_view()),
    ...
    def test_default_view_with_binaries(self):
        self.assertEqual(
            etree.tostring(self.env['test_views.binary']._get_default_form_view()),
            b'<form><sheet string="test_views.binary"><group><group><field name="img"/></group><group><field name="bin1"/></group></group><group><separator/></group></sheet></form>'
        )

    def test_default_calender_view(self):
        self.assertEqual(
            etree.tostring(self.env['test_views.calendar']._get_default_calendar_view()),
            b'<calendar string="test_views.calendar" date_start="x_date_start" date_stop="x_date_end"><field name="id"/></calendar>'
        )
...
class TestViewGroups(ViewCase):
    def test_attrs_groups(self):
        ...
        f = Form(self.env['test_views.model.some_access'], view='test_orm.view_model_views_some_access')
        f.a = 1
        f.b = 2
        ...
        self.assertWarning("""
            ...
            model='test_views.model.some_access')
        ...
        self.assertWarning("""
            ...
            """,
            model='test_views.model.all_access')

        # must raise for does not exists error
        with self.assertRaisesRegex(ValidationError, 'Field "ab" does not exist in model "test_views.model.some_access"'):
    ...
    def test_model_access(self):
        ...
        view = self.env.ref('test_orm.view_model_views_all_access').with_user(user)
        arch = self.env['test_views.model.all_access']._get_view_cache(view_id=view.id)['arch']
    ...
    def test_tree(self):
        view = self.env.ref('test_orm.view_model_views_some_access_tree')
        arch = self.env['test_views.model.some_access'].get_views([(view.id, 'list')])['views']['list']['arch']
    ...
    def test_related_field_and_groups(self):
        ...
        self.assertWarning("""
            ...
            model='test_views.model2.some_access')

        # should not fail, the domain is not applied on xxx_sub_id
        self.env['ir.ui.view'].create({
            'name': 'stuff',
            'model': 'test_views.model3.some_access',
            'arch': """
                <form>
    ...
    def test_computed_invisible_modifier(self):
        self.env['ir.ui.view'].create({
                'name': 'stuff',
                'model': 'test_views.computed.modifier',
                'arch': """
                    <form>
                        <field name="foo"/>
                        <field name="bar"/>
        ...
        with Form(self.env['test_views.computed.modifier']) as form:
            form.name = 'toto'
    ...
    def test_add_properties_definition_field(self):
        ...
        views = self.env['test_views.message'].get_views([(view.id, 'form')])
    ...
    def test_auto_add_filename_with_binary_requesting(self):
        ...
        views = self.env['test_views.model_binary'].get_views([(view.id, 'form')])
        arch = views['views']['form']['arch']
        form = etree.fromstring(arch)
