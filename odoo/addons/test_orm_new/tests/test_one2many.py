...
class One2manyCase(TransactionExpressionCase):
    def setUp(self):
        super().setUp()
        self.Line = self.env["test_orm.one2many.multi_line"]
        self.multi = self.env["test_orm.one2many.multi"].create({
            "name": "What is up?",
        })

        # data for One2many with inverse field Integer
        self.Edition = self.env["test_orm.one2many.creativework_edition"]
        self.Book = self.env["test_orm.one2many.creativework_book"]
        self.Movie = self.env["test_orm.one2many.creativework_movie"]
    ...
    def test_rpcstyle_one_by_one_on_new(self):
        self.multi = self.env["test_orm.one2many.multi"].new({
            "name": "What is up?",
        })
        for name in range(10):
            self.multi.lines = [Command.create({"name": str(name)})]
    ...
    def test_merge_partner(self):
        model = self.env['test_orm.one2many.field_with_caps']
    ...
    def test_cache_invalidation(self):
        ...
        record0 = self.env['test_orm.one2many.attachment_host'].create({})
        ...
        attachment = self.env['test_orm.one2many.attachment'].create({
        ...
        with self.assertQueryCount(1):
            record1 = self.env['test_orm.one2many.attachment_host'].create({})
        with self.assertQueryCount(0):
    ...
    def test_recompute(self):
        ...
        discussion = self.env.ref('test_orm_new.one2many_discussion_0')
    ...
    def test_dont_write_the_existing_childs(self):
        ...
        parent = self.env['test_orm.one2many.model_parent_m2o'].create({
            'name': 'parent',
    ...
    def test_create_with_commands(self):
        ...
        order = self.env['test_orm.one2many.order'].create({
        ...
        with self.assertQueryCount(2):
            self.env['test_orm.one2many.order'].create({
        ...
        with self.assertQueryCount(3):
            ...
            thief = self.env['test_orm.one2many.order'].create({
    ...
    def test_recomputation_ends(self):
        ...
        parent = self.env['test_orm.one2many.model_parent_m2o'].create({'name': 'parent'})
        child = self.env['test_orm.one2many.model_child_m2o'].create({'name': 'A', 'parent_id': parent.id})
    ...
    def test_compute_stored_many2one_one2many(self):
        container = self.env['test_orm.one2many.compute_container'].create({'name': 'Foo'})
        self.assertFalse(container.member_ids)
        member = self.env['test_orm.one2many.compute_member'].create({'name': 'Foo'})
    ...
    def test_new_real_interactions(self):
        ...
        # REAL - NEW #
        ##############
        parent = self.env['test_orm.one2many.model_parent_m2o'].create({'name': 'parentB'})
        new_child = self.env['test_orm.one2many.model_child_m2o'].new({'name': 'B', 'parent_id': parent.id})
        ...
        new_parent = self.env['test_orm.one2many.model_parent_m2o'].new({
            "name": 'parentC3PO',
        ...
        new_child = self.env['test_orm.one2many.model_child_m2o'].new({
            'name': 'PO',
        })
        ...
        new_child2 = self.env['test_orm.one2many.model_child_m2o'].new({
            'name': 'R2D2',
    ...
    def test_parent_id(self):
        Team = self.env['test_orm.one2many.team']
        Member = self.env['test_orm.one2many.team_member']
    ...
    def test_create_one2many_with_unsearchable_field(self):
        unsearchableO2M = self.env['test_orm.one2many.unsearchable_o2m']
    ...
    def test_computed_inverse_one2many(self):
        record = self.env['test_orm.one2many.computed_inverse_one2many'].create({
            'name': 'SuperRecord',
