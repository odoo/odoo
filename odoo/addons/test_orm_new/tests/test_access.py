...
class TestAccess(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['ir.access'].search([('model_id', '=', 'test_orm.access')]).unlink()

        # create records: Mario, Luigi, Peach, Toad, Yoshi, Bowser
        cls.records = cls.env['test_orm.access'].create([{}] * 6).with_user(cls.user)
    ...
    def make_access(self, name="", records=None, group=None, operation='r', **kwargs):
        ...
        return self.env['ir.access'].create({
            'name': name,
            'model_id': self.env['ir.model']._get_id('test_orm.access'),
    ...
    def test_error_message_partial_access(self):
        ...
        with self.assertAccessError(
            ...
            r"- Test ORM Access \(test_orm\.access\)\s*"
            r"If you really"
        ):
            self.records.check_access('read')
        ...
        with self.debug_mode():
            with self.assertAccessError(
                r"Uh-oh.*"
                rf"Sorry, Bob Bobman \(id={self.user.id}\) doesn't have 'read' access to:\s*"
                rf"- Test ORM Access, {self.toad.display_name}.*"
                rf"- Test ORM Access, {self.yoshi.display_name}.*"
                rf"- Test ORM Access, {self.bowser.display_name}.*"
    ...
    def test_get_groups_with_access(self):
        ...
        self.assertGreater(len(comodel_groups_write), 1)
