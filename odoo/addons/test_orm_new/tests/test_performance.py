...
class TestPerformance(TestOrmPartnerCommon, SavepointCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.env['test_orm.performance.base'].create([{
            'name': 'Object 0',
            'value': 0,
    ...
    def test_read_base(self):
        """ Read records. """
        records = self.env['test_orm.performance.base'].search([])
    ...
    def test_search_read(self):
        """ Search and fetch all at once. """
        Model = self.env['test_orm.performance.base']
    ...
    def test_write_base_one2many(self):
        ...
        rec1 = self.env['test_orm.performance.base'].create({'name': 'X'})
        ...
        rec1.write({'line_ids': [Command.create({'value': val}) for val in range(12)]})
        lines = rec1.line_ids
        rec2 = self.env['test_orm.performance.base'].create({'name': 'X'})
    ...
    def test_write_base_one2many_with_constraint(self):
        ...
        rec = self.env['test_orm.performance.base'].create({'name': 'Y'})
    ...
    def test_create_base(self):
        ...
        with self.assertQueryCount(__system__=2, demo=2):
            self.env['test_orm.performance.base'].create({'name': 'X'})
    ...
    def test_create_base_with_lines(self):
        ...
        with self.assertQueryCount(__system__=3, demo=3):
            self.env['test_orm.performance.base'].create({
                'name': 'X',
    ...
    def test_create_base_with_tags(self):
        ...
        tags = self.env['test_orm.performance.tag'].create([{'name': val} for val in range(10)])
    ...
    def test_several_prefetch(self):
        initial_records = self.env['test_orm.performance.base'].search([])
        self.assertEqual(len(initial_records), 5)
        for _i in range(8):
            self.env.cr.execute(
                'insert into test_orm_performance_base(value) select value from test_orm_performance_base'
            )
        records = self.env['test_orm.performance.base'].search([])
        ...
        self.env.cr.execute(
            'delete from test_orm_performance_base where id not in %s',
            (tuple(initial_records.ids),)
        )

    def test_prefetch_compute(self):
        records = self.env['test_orm.performance.base'].create([
        ...
        queries = [
            ''' SELECT "test_orm_performance_base"."id",
                       "test_orm_performance_base"."name",
                       "test_orm_performance_base"."value",
                       "test_orm_performance_base"."value_pc",
                       "test_orm_performance_base"."partner_id",
                       "test_orm_performance_base"."total",
                       "test_orm_performance_base"."create_uid",
                       "test_orm_performance_base"."create_date",
                       "test_orm_performance_base"."write_uid",
                       "test_orm_performance_base"."write_date"
                FROM "test_orm_performance_base"
                WHERE "test_orm_performance_base"."id" IN %s
            ''',
        ]
...
class TestIrPropertyOptimizations(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        self.Bacon = self.env['test_orm.performance.bacon']
        self.Eggs = self.env['test_orm.performance.eggs']

    def test_with_falsy_default(self):
        self.assertFalse(self.env['ir.default']._get('test_orm.performance.bacon', 'property_eggs', company_id=True))
    ...
    def test_with_truthy_default(self):
        eggs = self.Eggs.create({})
        self.env['ir.default'].set("test_orm.performance.bacon", "property_eggs", eggs.id)

        self.assertEqual(eggs.id, self.env['ir.default']._get('test_orm.performance.bacon', 'property_eggs'))
...
class TestMapped(TransactionCase):
    ...
    def test_relational_mapped(self):
        ...
        recs = self.env['test_orm.performance.base'].create([
...
class TestIncrementFieldsSkipLock(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.record = cls.env['test_orm.performance.mozzarella'].create([{
            'value': 1,
            'value_plus_one': 2,
        }])
        cls.other_record = cls.env['test_orm.performance.mozzarella'].create([{
            'value': 10,
            'value_plus_one': 11,
        }])
    ...
    def test_increment_fields_skiplock_null_field(self):
        ...
        self.env.cr.execute("SELECT value_null_by_default FROM test_orm_performance_mozzarella WHERE id = %s", (self.record.id,))
