...
class TestSudo(Feedback):
    ...
    def test_sudo(self):
        record = self.env['test_orm.access_feedback'].create({'val': 5})
        user1 = self.user
...
class TestACLFeedback(Feedback):
    ...
    def setUpClass(cls):
        ...
        model = cls.env['ir.model']._get('test_orm.access_feedback')
        ...
        cls.record = cls.env['test_orm.access_feedback'].create({'val': 5})
    ...
    def test_no_groups(self):
        ...
        self.assertEqual(
            ...
            """You are not allowed to modify 'Object For Test Access Right' (test_orm.access_feedback) records.
...
Contact your administrator to request access if necessary.""",
    ...
    def test_one_group(self):
        with self.assertRaises(AccessError) as ctx:
            self.env(user=self.user)['test_orm.access_feedback'].create({
                'val': 1,
            })
        self.assertEqual(
            ctx.exception.args[0],
            """You are not allowed to create 'Object For Test Access Right' (test_orm.access_feedback) records.
...
Contact your administrator to request access if necessary.""",
    ...
    def test_two_groups(self):
        ...
        expected = """You are not allowed to access 'Object For Test Access Right' (test_orm.access_feedback) records.
...
class TestIRRuleFeedback(Feedback):
    ...
    def setUpClass(cls):
        ...
        cls.model = cls.env['ir.model']._get('test_orm.access_feedback')
        cls.record = cls.env['test_orm.access_feedback'].create({
            'val': 0,
        }).with_user(cls.user)
        cls.maxDiff = None
...
If you really, really need access, perhaps you can win over your friendly administrator with a batch of freshly baked cookies.""",
        ...
        ChildModel = self.env['test_orm.access_feedback.inherits']
        with self.debug_mode(), self.assertRaises(AccessError) as ctx:
...
If you really, really need access, perhaps you can win over your friendly administrator with a batch of freshly baked cookies.""",
    ...
    def test_warn_company_no_company_field(self):
        ...
        then no information about the company is shown.
        """
        ChildModel = self.env['test_orm.access_feedback.child'].sudo()
...
This seems to be a multi-company issue, you might be able to access the record by switching to the company: {self.record.sudo().company_id.display_name}.""",
        ...
        p = self.env['test_orm.access_feedback.inherits'].create({'some_id': self.record.id})
        self.env.flush_all()
        self.env.invalidate_all()
        with self.assertRaisesRegex(
            AccessError,
            r"Implicitly accessed through 'Object for testing related access rights' \(test_orm.access_feedback.inherits\)\.",
        ):
    ...
    def test_warn_company_access_multi_record(self):
        ...
        records = self.env["test_orm.access_feedback"].create([
...
class TestFieldGroupFeedback(Feedback):
    ...
    def setUpClass(cls):
        ...
        cls.inherits_record = cls.env['test_orm.access_feedback.inherits'].create({
    ...
    def test_read(self):
        ...
        self.assertEqual(
            ctx.exception.args[0],
            f"""You do not have enough rights to access the field "forbidden" on Object For Test Access Right (test_orm.access_feedback). Please contact your system administrator.

Operation: read
User: {self.user.id}
Groups: allowed for groups 'Role / Portal', 'Test Access Feedback Group'""",
        )
        ...
        self.assertEqual(
            ctx.exception.args[0],
            f"""You do not have enough rights to access the field "forbidden3" on Object For Test Access Right (test_orm.access_feedback). Please contact your system administrator.
