...
class TestRules(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        ObjCateg = cls.env['test_orm.ir_rules.category']
        SomeObj = cls.env['test_orm.ir_rules']
        ...
        cls.env['ir.access'].create({
            ...
            'model_id': cls.env.ref('test_orm_new.model_test_orm_ir_rules').id,
            'operation': 'crud',
            'domain': "[('val', '>', 0)]",
        })
        ...
        cls.env['ir.access'].create({
            ...
            'model_id': cls.env.ref('test_orm_new.model_test_orm_ir_rules').id,
            'operation': 'crud',
            'domain': "[('categ_id', 'in', user.env['test_orm.ir_rules.category'].search([]).ids)]",
        })
    ...
    def test_group_rule(self):
        ...
        access = self.env.ref('test_orm_new.access_test_orm_ir_rules_public')
        access.domain = "[(0, '=', 1)]"
    ...
    def test_many2many(self):
        ...
        container_admin = self.env['test_orm.ir_rules.container'].create({'some_ids': [Command.set(ids)]})
    ...
    def test_many2many_no_read_access(self):
        ...
        container_admin = self.env['test_orm.ir_rules.container'].create({'some_ids': [Command.set([self.allowed.id])]})
    ...
    def test_one2many_no_read_access(self):
        ...
        child_admin = self.env['test_orm.ir_rules.child'].create({'parent_id': parent_admin.id})
    ...
    def test_no_context_in_ir_rules(self):
        ...
        ObjCateg = self.env['test_orm.ir_rules.category']
        SomeObj = self.env['test_orm.ir_rules']
    ...
    def test_check_access_rule_with_inherits(self):
        ...
        """
        ChildModel = self.env['test_orm.ir_rules.inherits']
    ...
    def test_flush_with_inherits(self):
        ...
        self.env['ir.access'].create({
            'name': 'Forbid 0 value',
            'model_id': self.env['ir.model']._get('test_orm.ir_rules').id,
            'operation': 'crud',
            'domain': str([('val', '!=', 0)]),
        })
    ...
    def test_domain_constrains(self):
        ...
        access = self.env['ir.access'].create({
            ...
            'model_id': self.env.ref('test_orm_new.model_test_orm_ir_rules').id,
            'operation': 'crud',
        })
        invalid_domains = [
            'A really bad domain!',
