...
class ManifestLinter(BaseCase):
    ...
    def test_manifests(self):
        for module in Manifest.all_addon_manifests():
            with self.subTest(module=module.name):
                # we want to check the content of the manifest directly without
                # parsed values
                self._test_manifest_keys(module)
                self._test_manifest_values(module)

    def _test_manifest_keys(self, manifest_data):
    ...
    def _test_manifest_values(self, manifest_data):
