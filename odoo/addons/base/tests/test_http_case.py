...
class TestRequestRemainingCrossClass(HttpCase):
    thread_a = None
    main_lock = threading.Lock()
    main_lock.acquire()

class TestRequestRemainingCrossClassA(TestRequestRemainingCrossClass): 
    def test_requests_a(self):
        class Dummycontroller(Controller):
            @route('/web/concurrent', type='http', auth='public', sitemap=False)
            def wait(c, **params):
        ...
        def late_request_thread():
            ...
            if self.main_lock.acquire(timeout=10):
                ...
                _logger.error('Something went wrong and thread was not able to aquire lock')
        TestRequestRemainingCrossClass.thread_a = threading.Thread(target=late_request_thread)
        self.thread_a.start()

class TestRequestRemainingCrossClassB(TestRequestRemainingCrossClass):
    def test_requests_b(self):
        self.env.cr.execute('SELECT 1')
        self.main_lock.release()
        _logger.info('B started, waiting for A to finish')
