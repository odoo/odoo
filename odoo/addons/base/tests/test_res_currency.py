...
class TestResConfig(TransactionCase):
    ...
    def test_res_currency_name_search(self):
        ...
        self.assertEqual(len(self.env["res.currency"].search([["rate_ids","=","1971-01-01"]])), 2)
        self.assertEqual(len(self.env["res.currency"].search([["rate_ids","=","0.69"]])), 1)
        self.assertEqual(len(self.env["res.currency"].search([["rate_ids","=","irrelevant"]])), 0)
