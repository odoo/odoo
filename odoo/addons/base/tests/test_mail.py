    ...
    def test_basic_sanitizer(self):
        cases = [
            ...
            (
                ...
                u"<p>Merci à l'intérêt pour notre produit.nous vous contacterons bientôt. Merci</p>",
            ),  # unicode
        ]
   ...
   font-size: 30px; max-width: 100%; after: "Not sure
    ...
    def test_quote_blockquote(self):
        ...
        for ext in test_mail_examples.QUOTE_BLOCKQUOTE_OUT:
            self.assertIn(
                u'<span data-o-mail-quote="1">%s' % misc.html_escape(ext), html
            )
    ...
    def test_quote_thunderbird(self):
        ...
        for ext in test_mail_examples.QUOTE_THUNDERBIRD_1_OUT:
            self.assertIn(
                u'<span data-o-mail-quote="1">%s</span>' % misc.html_escape(ext), html
            )
    ...
    def test_quote_basic_text(self):
        ...
        for test, in_lst, out_lst in test_data:
            ...
            for text in out_lst:
                self.assertIn(
                    u'<span data-o-mail-quote="1">%s</span>' % misc.html_escape(text),
                    new_html,
                )
