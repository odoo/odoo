...
class TestHasGroup(TransactionCase):
    ...
    def setUp(self):
        ...
        group0 = self.env["ir.model.data"]._update(
            "res.groups", "test_user_has_group", {"name": "group0"}, xml_id="group0"
        )
        self.group0 = "test_user_has_group.group0"
        self.env["ir.model.data"]._update(
            "res.groups", "test_user_has_group", {"name": "group1"}, xml_id="group1"
        )
    ...
    def test_env_uid(self):
        ...
        self.assertTrue(
            Users.has_group(self.group0), "the test user should belong to group0"
        )
        self.assertFalse(
            Users.has_group(self.group1), "the test user should *not* belong to group1"
        )
