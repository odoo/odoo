...
class TestOwnDefaults(FiltersCase):
    ...
    def test_new_filter_existing_default(self):
        ...
        self.build(
            ...
            dict(name="a", user_id=self.USER_ID, model_id="ir.filters"),
            dict(
                name="b", is_default=True, user_id=self.USER_ID, model_id="ir.filters"
            ),
        )
...
class TestReadGroup(TransactionCase):
    ...
    def test_read_group_1(self):
        ...
        self.assertEqual(
            Users._order,
            "name, login",
            "Model res.users must be ordered by name, login",
        )
        self.assertFalse(
            Users._fields["name"].store, "Field name is not stored in res.users"
        )
        ...
        self.assertTrue(
            any(val["user_id"] == False for val in res),
            "At least one group must contain val['user_id'] == False.",
        )
