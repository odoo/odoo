...
class TestBase(TransactionCase):
    ...
    def test_10_res_partner_find_or_create(self):
        ...
        self.assertNotEqual(
            partner_id2, found_id2, "john@connor.com match sarah.john@connor.com"
        )
        new_id = res_partner.find_or_create(SAMPLES[1][0])
        self.assertTrue(
            new_id > partner_id, "find_or_create failed - should have created new one"
        )
        new_id2 = res_partner.find_or_create(SAMPLES[2][0])
        self.assertTrue(
            new_id2 > new_id,
            "find_or_create failed - should have created new one again",
        )
    ...
    def test_50_res_partner_commercial_sync(self):
        ...
        sunhelm = res_partner.create(
            {
                ...
                "child_ids": [
                    (4, p0.id),
                    (0, 0, {"name": "Alrik Greenthorn", "email": "agr@sunhelm.com"}),
                ],
            }
        )
        p1 = res_partner.create(
        ...
        sunhelm.write(
            {
                "child_ids": [
                    (0, 0, {"name": "Ulrik Greenthorn", "email": "ugr@sunhelm.com"})
                ]
            }
        )
