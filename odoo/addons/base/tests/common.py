...
class MockSmtplibCase:
    ...
    def assertSMTPEmailsSent(self,
                             smtp_from=None, smtp_to_list=None,
                             message_from=None,
                             msg_from=None,
        ...
        :param message_values: values to compare against email.message;
        ...
        if msg_cc is not None:
            if not msg_cc:
                for email in matching_emails:
                    self.assertFalse(email['msg_cc'])
            else:
                for email in matching_emails:
                    self.assertEqual(
                        sorted(tools.mail.email_split_and_format(email['msg_cc'])),
                        sorted(tools.mail.email_split_and_format(msg_cc))
                    )
        if msg_to is not None:
            if not msg_to:
                for email in matching_emails:
                    self.assertFalse(email['msg_to'])
            else:
                for email in matching_emails:
                    self.assertEqual(
                        sorted(tools.mail.email_split_and_format(email['msg_to'])),
                        sorted(tools.mail.email_split_and_format(msg_to))
                    )
