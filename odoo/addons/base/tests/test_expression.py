...
class TestExpression(TransactionCase):
    ...
    def test_00_in_not_in_m2m(self):
        ...
        a = partners.create(
            {"name": "test_expression_partner_A", "category_id": [(6, 0, [cat_a.id])]}
        )
        b = partners.create(
            {"name": "test_expression_partner_B", "category_id": [(6, 0, [cat_b.id])]}
        )
        ab = partners.create(
            {
                "name": "test_expression_partner_AB",
                "category_id": [(6, 0, [cat_a.id, cat_b.id])],
            }
        )
        ...
        with_a_or_with_b = partners.search(
        ...
        without_a_or_b = partners.search(
            [("category_id", "not in", [cat_a.id, cat_b.id])]
        )
        self.assertFalse(
    ...
    def test_05_not_str_m2m(self):
        ...
        for name, cat_ids in partners_config.items():
            pids[name] = partners.create(
                {"name": name, "category_id": [(6, 0, cat_ids)]}
            ).id
    ...
    def test_10_hierarchy_in_m2m(self):
        ...
        partners = Partner.search(
            [("category_id", "child_of", self.ref("base.res_partner_category_0"))]
        )
    ...
    def test_15_m2o(self):
        ...
        res_10 = Partner.search(
            ["!", ("parent_id", "like", "probably_unexisting_name")]
        )
        ...
        res_101 = companies.search(
            [("currency_id", "not ilike", "")]
        )  # get no companies
    ...
    def test_15_equivalent_one2many_1(self):
        ...
        self.assertEqual(
            [p1],
            Partner.search([("user_ids", "in", u1a)]).ids,
            "o2m IN accept single int on right side",
        )
        self.assertEqual(
            [p1],
        ...
        self.assertEqual(
            [],
            Partner.search([("user_ids", "in", [10000])]).ids,
            "o2m NOT IN matches none on the right side",
        )
        self.assertEqual(
            [p1, p2],
            Partner.search([("user_ids", "in", [u1a, u2])]).ids,
            "o2m IN matches any on the right side",
        )
    ...
    def test_15_equivalent_one2many_2(self):
        ...
        currency_rate1 = CurrencyRate.search(
            [("name", "not like", "probably_unexisting_name")]
        )
    ...
    def test_active(self):
        ...
        partner = Partner.search(
            [("category_id", "ilike", "vendor"), ("active", "=", False)]
        )
        self.assertTrue(
            partner, "Record not Found with category vendor and active False."
        )
    ...
    def test_lp1071710(self):
        ...
        not_be = Partner.with_context(lang="fr_FR").search(
            [("country_id", "!=", "Belgique")]
        )
        self.assertNotIn(agrolait, not_be)
...
class TestAutoJoin(TransactionCase):
    ...
    def test_auto_join(self):
        ...
        states = self.env["res.country.state"].search(
            [("country_id", "=", country_us.id)], limit=2
        )
        ...
        expected = "%s like %s" % (
            unaccent('"res_partner_bank"."sanitized_acc_number"::text'),
            unaccent("%s"),
        )
        self.assertIn(
            expected,
        ...
        expected = "%s like %s" % (
            unaccent('"res_partner__bank_ids"."sanitized_acc_number"::text'),
            unaccent("%s"),
        )
        self.assertIn(
            expected,
        ...
        expected = "%s like %s" % (
            unaccent('"res_country"."code"::text'),
            unaccent("%s"),
        )
        self.assertIn(
            expected,
        ...
        expected = "%s like %s" % (
            unaccent('"res_country_state__country_id"."code"::text'),
            unaccent("%s"),
        )
        self.assertIn(
            expected,
        ...
        expected = "%s like %s" % (
            unaccent('"res_partner__state_id__country_id"."code"::text'),
            unaccent("%s"),
        )
        self.assertIn(
            expected,
