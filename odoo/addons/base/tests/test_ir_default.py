...
class TestIrDefault(TransactionCase):
    ...
    def test_defaults(self):
        ...
        self.assertEqual(
            IrDefault1.get_model_defaults("res.partner"),
            {"ref": "GLOBAL"},
            "Can't retrieve the created default value for all users.",
        )
        self.assertEqual(
            IrDefault2.get_model_defaults("res.partner"),
            {"ref": "GLOBAL"},
            "Can't retrieve the created default value for all users.",
        )
        self.assertEqual(
            IrDefault3.get_model_defaults("res.partner"),
            {"ref": "GLOBAL"},
            "Can't retrieve the created default value for all users.",
        )
    ...
    def test_conditions(self):
        ...
        self.assertEqual(IrDefault.get_model_defaults("res.partner"), {"ref": "X"})
        self.assertEqual(
            IrDefault.get_model_defaults("res.partner", condition="name=Agrolait"), {}
        )
        ...
        self.assertEqual(
            IrDefault.get_model_defaults("res.partner.title"), {"shortcut": "X"}
        )
        self.assertEqual(
            IrDefault.get_model_defaults("res.partner.title", condition="name=Miss"), {}
        )
        self.assertEqual(
            IrDefault.get_model_defaults("res.partner.title", condition="name=Mister"),
            {"shortcut": "Mr"},
        )
    ...
    def test_removal(self):
        ...
        self.assertEqual(
            IrDefault.get_model_defaults("res.partner"), {"title": title.id}
        )
