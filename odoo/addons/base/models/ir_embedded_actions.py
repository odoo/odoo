    ...
    def _compute_is_visible(self):
        ...
        for parent_res_model, records in self.grouped('parent_res_model').items():
            ...
            for record in records:
                ...
                python_method_exists = not record.python_method or hasattr(self.env[parent_res_model], str(record.python_method))
                if python_method_exists and not action_groups or (action_groups & self.env.user.all_group_ids):
