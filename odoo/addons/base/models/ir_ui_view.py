...
def get_view_arch_from_file(filename, xmlid):
    ...
    if xmlid.endswith(suffix):
        xmlid_search += (
            xmlid.rsplit(suffix, 1)[0],
            xmlid.split(".")[1].rsplit(suffix, 1)[0],
        )

    for node in doc.xpath(
        "//*[%s]" % " or ".join(["@id='%s'" % _id for _id in xmlid_search])
    ):
...
* if False, the view currently does not extend its parent but can be enabled
        ...
        def resolve_external_ids(arch_fs, view_xml_id):
            ...
            def replacer(m):
                ...
                return m.group("prefix") + str(
                    self.env["ir.model.data"].xmlid_to_res_id(xmlid)
                )
    ...
    def write(self, vals):
        ...
        if (
            "arch" in vals or "arch_base" in vals
        ) and "install_mode_data" not in self._context:
            vals["arch_fs"] = False
    ...
    def get_inheriting_views_arch(self, view_id, model):
        ...
        if self.pool._init and not self._context.get("load_all_views"):
            ...
            modules = tuple(self.pool._init_modules) + (
                self._context.get("install_mode_data", {}).get("module"),
            )
            ...
            views_cond = [
                (
                    "id",
                    "in",
                    list(self._context.get("check_view_ids") or (0,)) + views.ids,
                )
            ]
    ...
    def postprocess(self, model, node, view_id, in_tree_view, model_fields):
        ...
        if node.tag in ("field", "node", "arrow"):
            if node.get("object"):
                ...
                ).postprocess_and_fields(node.get("object"), xml_form, view_id)
            ...
            if node.get("name"):
                ...
                if field:
                    editable = self.env.context.get(
                        "view_is_editable", True
                    ) and self._field_is_editable(field, node)
                    children = False
                    views = {}
                    for f in node:
                        if f.tag in ("form", "tree", "graph", "kanban", "calendar"):
                            node.remove(f)
                            xarch, xfields = self.with_context(
                                base_model_name=model, view_is_editable=editable
                    ...
                    if field.comodel_name in self.env and field.type in (
                        ...
                        node.set(
                            "can_create",
                            "true"
                            if Comodel.check_access_rights(
        ...
        for f in node:
            if children or (node.tag == "field" and f.tag in ("filter", "separator")):
                fields.update(
                    self.postprocess(model, f, view_id, in_tree_view, model_fields)
                )
    ...
    def postprocess_and_fields(self, model, node, view_id):
        ...
        if node.tag == "diagram":
            if node.getchildren()[0].tag == "node":
                ...
                if (
                    not node.get("create")
                    and not node_model.check_access_rights(
                        "create", raise_exception=False
                    )
            ...
            if node.getchildren()[1].tag == "arrow":
                arrow_fields = self.env[node.getchildren()[1].get("object")].fields_get(
                    None
                )
    ...
    @api.model
    def graph_get(
        self, id, model, node_obj, conn_obj, src_node, des_node, label, scale
    ):

        def rec_name(rec):
            return (
                rec.name if "name" in rec else rec.x_name if "x_name" in rec else None
            )
        ...
        if self.pool._init:
            ...
            names = tuple(
                ...
                for (xmod, name), (
                    model,
                    res_id,
                ) in self.pool.model_data_reference_ids.items()
