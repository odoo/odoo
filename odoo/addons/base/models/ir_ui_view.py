    ...
    def _compute_invalid_locators(self):
        ...
        for view in self:
            ...
            while specs:
                ...
                invalid_locator = assess_locator(source, spec)
                if invalid_locator is not None:
                    invalid_locators.append(invalid_locator)
                else:
                    position, mode = spec.get("position"), spec.get("mode")
                    for sub_spec in spec:
                        if sub_spec.get("position"):
                            if sub_spec.get("position") != "move" or (position == "replace" and mode == "inner"):
                                ...
                                invalid_move = assess_locator(source, sub_spec)
                                if invalid_move:
