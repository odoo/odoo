...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def create(self, vals_list):
        valid_types = [key for key, _ in self.fields_get()['type']['selection']]
            ...
            if not values.get('type'):
                if values.get('inherit_id'):
                    ...
                    try:
                        ...
                        view_type = etree.fromstring(values.get('arch') or values.get('arch_base')).tag
                        if view_type not in valid_types:
                            raise ValidationError(_(
                                "Invalid view type: '%(view_type)s'.\n"
                                "Allowed types are: %(valid_types)s",
                                view_type=view_type, valid_types=', '.join(valid_types)
                            ))
