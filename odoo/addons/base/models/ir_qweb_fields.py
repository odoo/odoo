...
class IntegerConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        return pycompat.to_text(
            self.user_lang().format("%d", value, grouping=True).replace(r"-", u"\u2011")
        )
    ...
    def value_to_html(self, value, options):
        if "decimal_precision" in options:
            precision = (
                ...
                .search([("name", "=", options["decimal_precision"])])
                .digits
            )
        else:
        ...
        formatted = (
            self.user_lang().format(fmt, value, grouping=True).replace(r"-", u"\u2011")
        )
...
class ImageConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        return u'<img src="data:%s;base64,%s">' % (
    ...
    def value_to_html(self, value, options=None):
        ...
        barcode = self.env["ir.actions.report"].barcode(
            ...
            **{
                ...
                if key in ["width", "height", "humanreadable"]
            }
        )
        return u'<img src="data:png;base64,%s">' % base64.b64encode(barcode).decode(
            "ascii"
        )
...
class QwebView(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        ...
        if view._name != "ir.ui.view":
            _logger.warning(
                "%s.%s must be a 'ir.ui.view' model." % (record, field_name)
            )
            return None
