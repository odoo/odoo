...
class IrAccess(models.Model):
    ...
    # TODO operation: required maybe? default='rwcd'?
    operation = fields.Selection(OPERATION_SELECTION, help="Which operation(s) this access applies to, a subset of 'rwcd'.")
    domain = fields.Char()

    for_read = fields.Boolean(
        compute='_compute_for_read', inverse='_inverse_for_fields', store=True, precompute=True,
        help="Whether this access record applies for operation 'read'",
    )
    for_write = fields.Boolean(
        compute='_compute_for_write', inverse='_inverse_for_fields', store=True, precompute=True,
        help="Whether this access record applies for operation 'write'",
    )
    for_create = fields.Boolean(
        compute='_compute_for_create', inverse='_inverse_for_fields', store=True, precompute=True,
        help="Whether this access record applies for operation 'create'",
    )
    for_unlink = fields.Boolean(
        compute='_compute_for_unlink', inverse='_inverse_for_fields', store=True, precompute=True,
    ...
    def _compute_for_read(self):
        for access in self:
            access.for_read = 'r' in (access.operation or '')

    @api.depends('operation')
    def _compute_for_write(self):
        for access in self:
            access.for_write = 'w' in (access.operation or '')

    @api.depends('operation')
    def _compute_for_create(self):
        for access in self:
            access.for_create = 'c' in (access.operation or '')

    @api.depends('operation')
    def _compute_for_unlink(self):
        for access in self:
            access.for_unlink = 'd' in (access.operation or '')

    def _inverse_for_fields(self):
        for access in self:
            access.operation = ''.join([
                'r' if access.for_read else '',
                'w' if access.for_write else '',
        ...
        for access in self:
            if access.active and access.domain:
                try:
                    ...
                    expression.expression(domain, self.env[access.model_id.model].sudo())
