...
class IrAccess(models.Model):
    ...
    mode = fields.Char(
        ...
        help="Code specifying which operations this access applies to, a subset of 'crwd'.",
    )
    domain = fields.Char()

    @api.depends('for_create', 'for_read', 'for_write', 'for_unlink')
    ...
    @ormcache('self.env.uid', 'model_name', 'operation')
