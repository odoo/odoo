...
class ResUsersDeletion(models.Model):
    ...
    def _gc_portal_users(self, batch_size=50):
        ...
        self.env.cron_commit_progress(len(done_requests), remaining=len(todo_requests))
        todo_requests = todo_requests[:batch_size]

        for delete_request in todo_requests:
            delete_request = delete_request.with_prefetch().exists()
            if not delete_request or delete_request.state != 'todo':
                continue
            ...
            try:
                ...
                if self.env.cron_commit_progress(1):
                    continue
                else:
                    break
            ...
            if not self.env.cron_commit_progress():
                break
