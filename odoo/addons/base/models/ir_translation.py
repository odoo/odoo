...
class IrTranslation(models.Model):
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        if any(self.env[model]._fields[field].translate for model, ids in self.env.transaction.towrite.items() for record_id, fields in ids.items() for field in fields):
            self.flush()
