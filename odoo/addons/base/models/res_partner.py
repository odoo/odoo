...
class ResPartner(models.Model):
    ...
    def _run_vat_checks(self, country, vat, partner_name='', validation='error'):
        ...
        """
        if validation and validation not in ('error', 'setnull'):
            raise ValidationError(_("The validation argument should be either False, 'error' or 'setnull'"))
    ...
    def _get_vat_required_valid(self, company=None, fp_exists=False):
        ...
        :param company: the company we are checking this for
        :param fp_exists: if a fiscal position exists with a VAT number for the country of the partner (so VIES should be checked)
        """
        self.ensure_one()
