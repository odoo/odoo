...
class PartnerCategory(models.Model):
    ...
    def _name_search(
        ...
        partner_category_ids = self._search(
...
class Partner(models.Model):
    ...
    child_ids = fields.One2many(
        "res.partner", "parent_id", string="Contacts", domain=[("active", "=", True)]
    ...
    tz_offset = fields.Char(
        compute="_compute_tz_offset", string="Timezone offset", invisible=True
    )
    ...
    def _compute_tz_offset(self):
        for partner in self:
            partner.tz_offset = datetime.datetime.now(
                pytz.timezone(partner.tz or "GMT")
            ).strftime("%z")
    ...
    def view_header_get(self, view_id, view_type):
        ...
        return (
            _("Partners: ")
            + self.env["res.partner.category"].browse(self._context["category_id"]).name
        )
    ...
    def _display_address(self, without_company=False):
        ...
        address_format = (
            self.country_id.address_format or self._get_default_address_format()
        )
        args = {
