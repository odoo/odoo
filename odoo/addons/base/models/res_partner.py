...
class Partner(models.Model):
    ...
    @api.model_cr
    def init(self):
        self._cr.execute("""SELECT indexname FROM pg_indexes WHERE indexname = 'res_partner_vat_uppercase_index_'""")
        if not self._cr.fetchone():
            self._cr.execute("""CREATE INDEX res_partner_vat_uppercase_index_ ON res_partner (regexp_replace(upper(vat), '[^A-Z0-9]+', '', 'g'))""")
