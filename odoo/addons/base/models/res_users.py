...
class ResGroups(models.Model):
    ...
    implied_ids: ResGroups = fields.Many2many('res.groups', 'res_groups_implied_rel', 'gid', 'hid',
        string='Inherits', help='Users of this group automatically inherit those groups')
    all_implied_ids: ResGroups = fields.Many2many('res.groups', 'res_groups_all_implied_rel', 'gid', 'hid',
        string='Transitive and recursive closure', compute='_compute_all_implied_ids', store=True)
    implied_by_ids: ResGroups = fields.Many2many('res.groups', 'res_groups_implied_rel', 'hid', 'gid', string='Automatically added from')

    @api.constrains('users', 'implied_ids', 'implied_by_ids')
    def _check_one_user_type(self):
        self.users._check_one_user_type()

    @api.constrains('implied_ids', 'implied_by_ids')
    def _check_no_recursive_groups(self):
        if self._has_cycle('implied_ids'):
            raise ValidationError(_('Groups shold not imply themselves.'))
...
class ResUsers(models.Model):
    ...
    def _compute_view_implied_group_ids(self):
        ...
        for user in self:
            ...
            user.view_implied_group_ids = list(group_definitions.get_superset_ids(user.group_ids.ids))
    ...
    def _check_one_user_type(self):
        ...
        group_definitions = self.env['res.groups']._get_group_definitions()
        for user in self:
            if group_definitions.get_disjoint_ids(user.group_ids.all_implied_ids.ids).intersection(user.group_ids.all_implied_ids.ids):
