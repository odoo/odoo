...
class IrModelConstraint(models.Model):
    ...
    name = fields.Char(string='Constraint', required=True, index=True,
                       help="PostgreSQL index, constraint or foreign key name.")
    definition = fields.Char(help="PostgreSQL object definition")
    ...
    type = fields.Char(string='Constraint Type', required=True, size=1, index=True,
                       help="Type of the constraint: `f` for a foreign key, "
                            "`i` for indexes, `u` for other constraints.")
    ...
    def _reflect_constraints(self, model_names):
        """ Reflect the SQL objects of the given models. """
        for model_name in model_names:
            self._reflect_model(self.env[model_name])

    def _reflect_model(self, model):
        """ Reflect the _sql_objects of the given model. """
        ...
        constraint_module = {
            ...
            for name in getattr(cls, '_local_sql_objects', ())
        }

        data_list = []
        for cons in model._sql_objects.values():
