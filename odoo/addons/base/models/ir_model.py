...
SAFE_EVAL_BASE = {"datetime": datetime, "dateutil": dateutil, "time": time}
...
class IrModel(models.Model):
    ...
    def _default_field_id(self):
        ...
        return [
            (0, 0, {"name": "x_name", "field_description": "Name", "ttype": "char"})
        ]
    ...
    def unlink(self):
        ...
        if not self._context.get(MODULE_UNINSTALL_FLAG):
            for model in self:
                if model.state != "manual":
                    raise UserError(
                        _("Model '%s' contains module data and cannot be removed!")
                        % model.name
                    )
    ...
    def write(self, vals):
        if "__last_update" in self._context:
            self = self.with_context(
                {k: v for k, v in self._context.items() if k != "__last_update"}
            )
    ...
    def _reflect_model(self, model):
        ...
        if model._module == self._context.get("module"):
            ...
            cr.execute(
                "SELECT * FROM ir_model_data WHERE name=%s AND module=%s",
    ...
    def _add_manual_models(self):
        ...
        custom_models = [
            name for name, model_class in self.pool.items() if model_class._custom
        ]
...
class IrModelFields(models.Model):
    ...
    copy = fields.Boolean(
        string="Copied", help="Whether the value is copied when duplicating a record."
    )
    related = fields.Char(
    ...
    def _check_selection(self, selection):
        try:
            ...
            if not (
                ...
                and all(
                    isinstance(item, (tuple, list)) and len(item) == 2 for item in items
                )
            ):
    ...
    def _drop_column(self):
        ...
        for field in self:
            ...
            if (
                tools.column_exists(self._cr, model._table, field.name)
    ...
    def unlink(self):
        ...
        if not self._context.get(MODULE_UNINSTALL_FLAG) and any(
            field.state != "manual" for field in self
        ):
            raise UserError(
    ...
    def create(self, vals):
        ...
        if vals.get("state", "manual") == "manual":
            if vals.get("relation") and not self.env["ir.model"].search(
                [("model", "=", vals["relation"])]
            ):
    ...
    def _reflect_model(self, model):
        ...
        for field in model._fields.values():
            if field.string in duplicate_fields_label:
                _logger.warning(
                    ...
                    field.name,
                    duplicate_fields_label[field.string],
                    model,
                    field.string,
                )
            else:
        ...
        if not self.pool._init:
            ...
            if extra_names:
                ...
                records = self.browse(
                    [fields_data.pop(name)["id"] for name in extra_names]
                )
...
class IrModelConstraint(models.Model):
    ...
    def _module_data_uninstall(self):
        ...
        """
        if not (
            self._uid == SUPERUSER_ID or self.env.user.has_group("base.group_system")
        ):
            raise AccessError(
        ...
        for data in self.sorted(key="id", reverse=True):
            ...
            if typ == "f":
                ...
                if self._cr.fetchone():
                    self._cr.execute(
                        'ALTER TABLE "%s" DROP CONSTRAINT "%s"' % (table, name)
                    )
...
class IrModelAccess(models.Model):
    ...
    group_id = fields.Many2one(
        "res.groups", string="Group", ondelete="cascade", index=True
    )
    ...
    def group_names_with_access(self, model_name, access_mode):
        ...
        self._cr.execute(
            """SELECT c.name, g.name
                            ...
                            WHERE m.model=%s AND a.active IS TRUE AND a.perm_"""
            + access_mode,
            (model_name,),
        )
    ...
    def check(self, model, mode="read", raise_exception=True):
        ...
        assert isinstance(model, pycompat.string_types), "Not a model name: %s" % (
            model,
        )
        assert mode in ("read", "write", "create", "unlink"), "Invalid access mode"
...
class IrModelData(models.Model):
    ...
    def _update(
        ...
        if xml_id:
            self._cr.execute(
                """SELECT imd.id, imd.res_id, md.id, imd.model, imd.noupdate
                                FROM ir_model_data imd LEFT JOIN %s md ON (imd.res_id = md.id)
                                WHERE imd.module=%%s AND imd.name=%%s"""
                % record._table,
                (module, xml_id),
            )
            ...
            for imd_id, imd_res_id, real_id, imd_model, imd_noupdate in results:
                ...
                if not real_id:
                    ...
                    assert model == imd_model, (
                        "External ID conflict, %s already refers to a `%s` record,"
            ...
            if xml_id:
                ...
                while inherit_models:
                    ...
                    for (
                        parent_model_name,
                        parent_field,
                    ) in current_model._inherits.items():
    ...
    def _module_data_uninstall(self, modules_to_remove):
            ...
            for model, res_id in to_unlink:
                ...
                if model == "ir.model.fields":
                    ...
                    if not field.exists():
                        ...
                        continue
                    if (
                        field.name in models.LOG_ACCESS_COLUMNS
                        and field.model in self.env
                        and self.env[field.model]._log_access
                    ):
                        continue
                    if field.name == "id":
                        continue
        ...
        undeletable += unlink_if_refcount(
            item
            for item in to_unlink
            if item[0] not in ("ir.model", "ir.model.fields", "ir.model.constraint")
        )
        undeletable += unlink_if_refcount(
            item for item in to_unlink if item[0] == "ir.model.constraint"
        )
        ...
        undeletable += unlink_if_refcount(
            item for item in to_unlink if item[0] == "ir.model.fields"
        )
        ...
        undeletable += unlink_if_refcount(
            item for item in to_unlink if item[0] == "ir.model"
        )
