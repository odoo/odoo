...
class QWebException(Exception):
    ...
    def __init_subclass__(cls):
        assert QWebException is cls.__bases__[0]
        QWEB_ERROR_CLASSES[cls.__bases__[1]] = cls
...
class QWebValidationError(QWebException, ValidationError):
    pass


class QWebMissingError(QWebException, MissingError):
    pass


class QWebAccessError(QWebException, AccessError):
    pass


class QWebAccessDenied(QWebException, AccessDenied):
    pass


class QWebUserError(QWebException, UserError):
    pass


class QWebAssertionError(QWebException, AssertionError):
    pass
...
class QwebCallParameters(NamedTuple):
    ...
    method: str | None
    values: dict | None
    scope: Literal[False, True, 'root']
    directive: str
    path_xml: tuple[str | int, str, str] | None


class QwebStackItem(NamedTuple):
...
class IrQweb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render_iterall(self, view_ref, method, values) -> Iterator[str | QwebCallParameters]:
        ...
        params = QwebCallParameters({}, view_ref, method, None, False, 'render', None)
        stack = [QwebStackItem(params, self, iter([params]), values, None)]
        ...
        while stack:
            ...
            try:
                ...
                dont_fetch_directive = stack_item.irQweb.env.context.get('__qweb_dont_fetch_directive')

                # traverse the iterator
                for item in stack_item.iterator:
                    if isinstance(item, str) or item.directive == dont_fetch_directive:
                        yield item
                    else:
                        # add new QwebStackItem from QwebCallParameters
                        values = stack_item.values
                        irQweb = stack_item.irQweb
                        params = item
                        ...
                        stack.append(QwebStackItem(params, irQweb, iterator, values, options))
                        break
                else:
                    stack.pop()
                ...
                if error.ref is None:
                    error.ref = stack_item.params.view_ref
                raise
            except Exception as error:
                loaded_codes = self.env.context['__qweb_loaded_codes']
                if (stack_item.params.view_ref in loaded_codes and not isinstance(error, RecursionError)) or len(stack) <= 1:
                    options = stack_item.options or {}  # The compilation may have failed before the compilation options were loaded.
                    ...
                    code = loaded_codes.get(stack_item.params.view_ref) or loaded_codes.get(False)
                    ...
                    path_xml = stack_item.params.path_xml
        ...
        def_wrap = [indent_code(f"""
            def {def_name}_wrap(self, values):
                ...
                def {def_name}_cache():
                    text = []
                    result = []
                    for item in self.with_context(__qweb_dont_fetch_directive='t-nocache')._render_iterall({compile_context['template']!r}, {def_name!r}, values):
    ...
    def _compile_directive_nocache(self, el, compile_context, level):
        ...
        code.append(indent_code(f"yield QwebCallParameters({{'__qweb_dont_fetch_directive': None}}, {compile_context['template']!r}, {def_name!r}, cached_values, self.env.context.get('__qweb_dont_fetch_directive') and 'root', 't-nocache', (template_options['ref'], {path!r}, {xml!r}))", level))
