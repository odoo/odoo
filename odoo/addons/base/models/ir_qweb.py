...
class IrQWeb(models.AbstractModel, QWeb):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def default_values(self):
        ...
        default.update(
            request=request, cache_assets=round(time() / 180), true=True, false=False
            ...
            self._append(
                ast.Call(
                    ...
                    keywords=[
                        ...
                        ast.keyword(
                            "async", self._get_attr_bool(el.get("async", False))
                        ),
    ...
    def _compile_widget_options(self, el, directive_type):
        ...
        if ("t-%s-options" % directive_type) in el.attrib:
            if tools.config["dev_mode"]:
                _logger.warning(
                    "Use new syntax t-options instead of t-%s-options for '%s'"
                    % (directive_type, etree.tostring(el))
                )
    ...
    def _get_asset(
        self, xmlid, options, css=True, js=True, debug=False, async=False, values=None
    ):
    ...
    @tools.ormcache_context(
        "xmlid", 'options.get("lang", "en_US")', keys=("website_id",)
    )
    def _get_asset_content(self, xmlid, options):
        ...
        for el in html.fragments_fromstring(template):
            if isinstance(el, pycompat.string_types):
                ...
                can_aggregate = not urls.url_parse(href).netloc and not href.startswith(
                    "/web/content"
                )
                if el.tag == "style" or (
                    el.tag == "link" and el.get("rel") == "stylesheet" and can_aggregate
                ):
