...
class QWebException(Exception):
    ...
    def __repr__(self):
        return f"QWebException({repr(self.message)})"
...
class IrQWeb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _compile(self, template, options):
        ...
        try:
            ...
            raise QWebException("Error when compiling xml template",
                self, options, e, template, path=_options.get('last_path_node')) from e
        try:
            code = '\n'.join(code_lines)
        except QWebException as e:
            raise e
            ...
            raise QWebException("Error when compiling xml template",
                self, options, e, template, code=code) from e
        ...
        def render_template(self, values):
            try:
                ...
                raise QWebException("Error when render the template",
                    self, options, e, template, path=log.get('last_path_node'), code=code) from e
    ...
    def _get_template(self, template, options):
        ...
        if isinstance(template, etree._Element):
            ...
            try:
                ...
                raise QWebException("load could not load template",
                    self, options, e, template) from e

        if document is None:
            raise QWebException("Template not found",
    ...
    def _is_static_node(self, el, options):
        ...
        attributes.
        """
        return el.tag != 't' and not any(att.startswith('t-') and att not in ['t-tag', 't-inner-content'] for att in el.attrib)
    ...
    def _compile_expr(self, expr, raise_on_missing=False):
        ...
        method.
        """
        readable = io.BytesIO(f"({expr or ''})".strip().encode('utf-8'))
        try:
    ...
    def _compile_static_node(self, el, options, indent):
        ...
        if el.nsmap:
            ...
            body = self._compile_directive_inner_content(el, options, indent)
    ...
    def _compile_directive_set(self, el, options, indent):
        ...
        * whose value is the content of the tag (being Markup safe).
        ...
        if 't-value' in el.attrib:
            ...
            content = self._compile_directive_inner_content(el, options, indent + 1) + self._flushText(options, indent + 1, True)
            if content:
    ...
    def _compile_directive_value(self, el, options, indent):
        ...
        This method only check if this attributes is on the same node of a
         `t-set` attribute.
        """
        if not options.get('t-set') is None:
            raise ValueError("t-value must be on the same node of t-set")
        return []

    def _compile_directive_valuef(self, el, options, indent):
        ...
        """
        if not options.get('t-set') is None:
            raise ValueError("t-valuef must be on the same node of t-set")
        return []

    def _compile_directive_inner_content(self, el, options, indent):
    ...
    def _compile_directive_else(self, el, options, indent):
        ...
        if not options.pop('t_if', None):
            raise ValueError("t-else directive must be preceded by t-if or t-elif directive")
    ...
    def _compile_directive_elif(self, el, options, indent):
        ...
        if _elif == '_t_skip_else_':
            ...
            return []
        if not options.pop('t_if', None):
            raise ValueError("t-elif directive must be preceded by t-if or t-elif directive")
    ...
    def _compile_directive_as(self, el, options, indent):
        """Compile `t-as` expressions into a python code as a list of strings.

        This method only check if this attributes is on the same node of a
         `t-foreach` attribute.
        """
        if not options.get('t-foreach') is None:
            raise ValueError("t-as must be on the same node of t-foreach")
        return []
    ...
    def _compile_widget_value(self, el, options, indent=0, without_attributes=False):
        ...
        default_body = self._compile_directive_inner_content(el, options, indent + 1)
    ...
    def _compile_directive_call(self, el, options, indent):
        ...
        content = self._compile_directive_inner_content(el, options, indent + 1)
    ...
    def _compile_directive_call_assets(self, el, options, indent):
        ...
        code = self._flushText(options, indent)
        code.append(self._indent("""t_call_assets_nodes = self._get_asset_nodes(%(xmlid)s, css=%(css)s, js=%(js)s, debug=values.get("debug"), async_load=%(async_load)s, defer_load=%(defer_load)s, lazy_load=%(lazy_load)s, media=%(media)s)""".strip() % {
