...
class IrActionsReport(models.Model):
    ...
    def _run_wkhtmltoimage(self, bodies, width, height, image_format="jpg"):
        ...
        """
        if (modules.module.current_test or tools.config.is_testing) and not self.env.context.get('force_image_rendering'):
                ...
                case 1:
                    if len(bodies) > 1:
                        ...
                        if '(with patched qt)' not in v.stdout:
                            if modules.module.current_test or tools.config.is_testing:
    ...
    def _pre_render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if (modules.module.current_test or tools.config.is_testing) and not self.env.context.get('force_report_rendering'):
