...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if is_pdf_engine_required:
            ...
            if report_type == 'qweb-pdf':
                if self.env.company.report_rendering_engine == 'none':
                    raise UserError(_("No rendering engine selected. Please contact your administrator."))
                engine_name = self.env.company.report_rendering_engine
            else:
                engine_name = report_type.split(separator='-', maxsplit=3)[2]
    ...
    def _run_pdf_engine(self, engine_name, html, report_ref=False, landscape=False):
        raise NotImplementedError("PDF engine not implemented")
