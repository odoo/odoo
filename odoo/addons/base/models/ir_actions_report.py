...
from odoo.tools import check_barcode_encoding, is_html_empty, parse_version
...
class IrActionsReport(models.Model):
    ...
    help='The type of the report that will be rendered, each one having its own'
        ...
        ' browser PDF means the report will be rendered using a pdf engine and'
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        # Call pdf engine to generate the missing streams.
        ...
        is_pdfrendering_needed = not res_ids or res_ids_wo_stream

        if is_pdfrendering_needed:
            pdf_engine = self._get_render_pdf_engine(data.get('report_subtype'))
            ...
            pdf_content_stream = io.BytesIO(pdf_engine(
                report_ref=report_ref,
                bodies=bodies, header=header, footer=footer,
