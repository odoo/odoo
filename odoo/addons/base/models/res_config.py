...
class ResConfigConfigurable(models.TransientModel):
    ...
    def action_cancel(self):
        """ Action handler for the ``cancel`` event.

        That event isn't generated by the res.config.view.base inheritable view,
        the inherited view has to overload one of the buttons (or add one
        more).
...
class ResConfigSettings(models.TransientModel, ResConfigModuleInstallationMixin):
    ...
    *   For the other fields, the method ``execute`` invokes `set_values`.
    ...
    It also invokes all methods with a name that starts with 'get_default';
    ...
    def get_config_warning(self, msg):
        r"""
        :param string msg: Warning content, may contain field or menus references.

        Return a warning exception with the given message where the %(field:xxx)s
