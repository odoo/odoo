...
class IrAsset(models.Model):
    ...
    def _parse_bundle_name(self, bundle_name, debug_assets):
        if bundle_name.endswith('.xml.js'):
            bundle_name = bundle_name.removesuffix('.xml.js')
            asset_type = 'xml'
        else:
        ...
        if asset_type == 'css':
            ...
            if bundle_name.endswith('.rtl'):
                ...
                rtl = True
        elif asset_type not in ('js', 'xml'):
            raise ValueError('Only js, xml and css assets bundle are supported for now')
