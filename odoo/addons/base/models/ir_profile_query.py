...
class IrProfileQuery(models.Model):
    ...
    profile_id = fields.Many2one('ir.profile', required=True, ondelete='cascade', index=True)
    time = fields.Float(string="Time", help="Time spent executing the query (seconds)", min_display_digits=6)
    query = fields.Text(readonly=True, help="Query with placeholders, before injecting the parameters")
    full_query = fields.Text(required=True, prefetch=False, help="Query with its parameters injected, ready to execute")
    query_preview = fields.Text(compute='_compute_query_preview')
    formatted_query = fields.Text(compute='_compute_formatted_query')
    plan = fields.Text("Query Plan", prefetch=False)
