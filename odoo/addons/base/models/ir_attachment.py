...
class IrAttachment(models.Model):
    ...
    res_id = fields.Integer(
        "Resource ID", readonly=True, help="The record id this is attached to."
    )
    ...
    def check(self, mode, values=None):
        ...
        if require_employee:
            if not (
                self.env.user._is_admin() or self.env.user.has_group("base.group_user")
            ):
                raise AccessError(
