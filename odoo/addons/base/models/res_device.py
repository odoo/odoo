...
class ResDeviceLog(models.Model):
    ...
    def _check_location(self, request):
        """
            ...
            This method is based on the order of the traces and not the activity of the IP addresses.
            This method should therefore be called when a new trace has been added.
        ...
        geoip = GeoIP(last_ip)
        last_latitude, last_longitude, last_accuracy_radius = geoip['latitude'], geoip['longitude'], geoip['accuracy_radius']
        ...
        for trace in filter(lambda t: t['last_activity'] >= time_, reversed(request.session['_trace'][:-1])):
            ip = trace['ip_address']
            geoip = GeoIP(ip)
            latitude, longitude, accuracy_radius = geoip['latitude'], geoip['longitude'], geoip['accuracy_radius']
            ...
            distance = self._haversine_formula(last_latitude, last_longitude, latitude, longitude)
