...
class Module(models.Model):
    ...
    state = fields.Selection(
        STATES, string="Status", default="uninstalled", readonly=True, index=True
    )
        ...
        for mod_name in modules.get_modules():
            ...
            if mod:
                ...
                for key in values:
                    old = getattr(mod, key)
                    updated = (
                        tools.ustr(values[key])
                        if isinstance(values[key], pycompat.string_types)
                        else values[key]
                    )
...
class ModuleExclusion(models.Model):
    ...
    exclusion_id = fields.Many2one(
        "ir.module.module", "Exclusion Module", compute="_compute_exclusion"
    )
    state = fields.Selection(DEP_STATES, string="Status", compute="_compute_state")
