...
class AssetsBundle(object):
        ...
        for f in files:
            ...
            if css and not binary and extension in STYLE_EXTENSIONS:
                if css_urls is None and self.binaries:
                    ...
                    css_urls = {}
                    web_dir = params['url'].rpartition('/')[0]
                    ...
                    for definition_bundle, bins in definition_bundles.items():
                        links = self.env['ir.qweb']._get_asset_links(definition_bundle, binary=True)
                        for link, bin in zip(links, bins):
                            rel_name = bin.name
                            if bin.name.startswith(web_dir + '/'):
                                rel_name = bin.name[len(web_dir) + 1:]
                            css_urls.setdefault(bin.name, link)
                            css_urls.setdefault(rel_name, link)
    ...
    def get_attachments(self, extension, ignore_version=False):
        ...
        """
        extension_type = os.path.splitext(extension)[1].removeprefix('.')
...
""" % error_message
    ...
    def bins(self):
        ...
        for binary in self.binaries:
            ...
            attachments = self.get_attachments(extension)
            if not attachments:
                attachments = self.save_attachment(extension, binary._fetch_content())
...
class WebAsset(object):
    ...
    def __init__(self, bundle, inline=None, url=None, filename=None, last_modified=None, params={}):
        self.bundle = bundle
        self.inline = inline
...
class StylesheetAsset(WebAsset):
    rx_full_url = re.compile(r"""\burl\(['"]?(.*?)['"]?\)""", re.U)
    rx_import = re.compile(r"""@import\s+('|")(?!'|"|/|https?://)""", re.U)
    rx_url = re.compile(r"""(?<!")url\s*\(\s*('|"|)(?!'|"|/|https?://|data:|#{str)""", re.U)
    rx_sourceMap = re.compile(r'(/\*# sourceMappingURL=.*)', re.U)
    rx_charset = re.compile(r'(@charset "[^"]+";)', re.U)

    def __init__(self, *args, rtl=False, autoprefix=False, css_urls=False, **kw):
        self.rtl = rtl
        self.autoprefix = autoprefix
    ...
    def _fetch_content(self):
        try:
            ...
            if self.css_urls:
                def map_url(matched):
                    plain_url = matched.group(1).split('?')[0]
...
class BinaryAsset(WebAsset):
    def __init__(self, bundle, definition_bundle, **kwargs):
        super().__init__(bundle, **kwargs)
        self.extension = os.path.splitext(self._filename)[1].removeprefix('.')
