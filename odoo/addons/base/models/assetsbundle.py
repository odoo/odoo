...
class AssetsBundle(object):
        ...
        if debug == "assets":
            if css and self.stylesheets:
                ...
                if not is_css_preprocessed:
                    ...
                    if self.css_errors:
                        ...
                        response.append(
                            JavascriptAsset(
                                self, inline=self.dialog_message(msg)
                            ).to_html()
                        )
                        response.append(
                            StylesheetAsset(
                                self, url="/web/static/lib/bootstrap/css/bootstrap.css"
                            ).to_html()
                        )
            ...
            if css and self.stylesheets:
                ...
                for attachment in css_attachments:
                    response.append(
                        u'<link href="%s" rel="stylesheet"/>' % url_for(attachment.url)
                    )
                if self.css_errors:
                    msg = "\n".join(self.css_errors)
                    response.append(
                        JavascriptAsset(self, inline=self.dialog_message(msg)).to_html()
                    )
            if js and self.javascripts:
                response.append(
                    u'<script %s type="text/javascript" src="%s"></script>'
                    % (async and u'async="async"' or "", url_for(self.js().url))
                )
    ...
    def clean_attachments(self, type):
        ...
        domain = [
            (
                "url",
                "=like",
                "/web/content/%-%/{0}%.{1}".format(self.name, type),
    ...
    def get_attachments(self, type, ignore_version=False):
        ...
        url_pattern = "/web/content/%-{0}/{1}{2}.{3}".format(
            version, self.name, ".%" if type == "css" else "", type
        )
        self.env.cr.execute(
            """
             SELECT max(id)
            ...
            if assets:
                ...
                for attachment in attachments:
                    ...
                    if asset._content is None:
                        asset._content = (
                            attachment.datas
                            and base64.b64decode(attachment.datas).decode("utf8")
                            or ""
                        )
    ...
    def get_preprocessor_error(self, stderr, source=None):
        ...
        error += (
            "This error occured while compiling the bundle '%s' containing:" % self.name
        )
        for asset in self.stylesheets:
    ...
    def stat(self):
        if not (self.inline or self._filename or self._ir_attach):
            ...
            try:
                ...
                attach = (
                    self.bundle.env["ir.attachment"].sudo().search_read(domain, fields)
                )
...
class JavascriptAsset(WebAsset):
    ...
    def to_html(self):
        if self.url:
            ...
            return (
                '<script type="text/javascript" charset="utf-8">%s</script>'
                % self.with_header()
            )
...
class StylesheetAsset(WebAsset):
    ...
    def _fetch_content(self):
        try:
            ...
            if self.rx_import:
                content = self.rx_import.sub(r"""@import \1%s/""" % (web_dir,), content)

            if self.rx_url:
                content = self.rx_url.sub(r"url(\1%s/" % (web_dir,), content)
    ...
    def to_html(self):
        ...
        if self.url:
            href = self.html_url
            return '<link rel="stylesheet" href="%s" type="text/css"%s/>' % (
                href,
                media,
            )
        else:
...
class PreprocessedCSS(StylesheetAsset):
    ...
    def compile(self, source):
        ...
        if compiler.returncode:
            ...
            if not cmd_output:
                cmd_output = (
                    u"Process exited with return code %d\n" % compiler.returncode
                )
...
class ScssStylesheetAsset(PreprocessedCSS):
    ...
    def bootstrap_components_path(self):
        return get_resource_path(
            "web", "static", "lib", "bootstrap", "scss", "bootstrap"
        )
    ...
    def compile(self, source):
        ...
        try:
            return libsass.compile(
                string=source,
                include_paths=[self.bootstrap_path, self.bootstrap_components_path],
