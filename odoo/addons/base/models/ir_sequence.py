...
class IrSequence(models.Model):
    ...
    number_next = fields.Integer(string='Next Number', required=True, default=1, bigint=True, help="Next number of this sequence")
    ...
    def _onchange_prefix_suffix_ascii(self):
        for field_name in ('prefix', 'suffix'):
            ...
            if value and not value.isascii():
                return {
                    'warning': {
                        'title': _("Non-ASCII characters in %s", field_name),
                        'message': _(
