...
from odoo.domains import D, Domain
...
class IrRule(models.Model):
        ...
        for rule in self:
            if rule.active and rule.domain_force:
                try:
                    ...
                    self.env[rule.model_id.model].sudo()._search(domain)
    ...
    def _get_failing(self, for_records, mode='read'):
        ...
        if Model.search_count(group_domains & D('id', 'in', for_records.ids)) == len(for_records):
        ...
        def is_failing(r, ids=for_records.ids):
            dom = D(safe_eval(r.domain_force, eval_context) if r.domain_force else [])
            return Model.search_count(dom & D('id', 'in', ids)) < len(ids)
    ...
    def _compute_domain(self, model_name: str, mode: str = "read") -> Domain:
        ...
        for parent_model_name, parent_field_name in model._inherits.items():
            if domain := self._compute_domain(parent_model_name, mode):
                global_domains.append(D(parent_field_name, 'any', domain))

        rules = self._get_rules(model_name, mode=mode)
        if not rules:
            return D.AND(*global_domains)
        ...
        for rule in rules.sudo():
            if not rule.groups:
                dom_list = global_domains
            elif rule.groups & user_groups:
                dom_list = group_domains
            else:
                continue
            # evaluate the domain for the current user
            dom = safe_eval(rule.domain_force, eval_context) if rule.domain_force else []
            dom_list.append(D(dom))

        # combine global domains and group domains
        if not group_domains:
            return D.AND(*global_domains)
        return D.AND(*global_domains, D.OR(*group_domains))
