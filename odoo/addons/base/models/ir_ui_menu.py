...
class IrUiMenu(models.Model):
    ...
    def _get_full_name(self, level=6):
        ...
        if self.parent_id:
            return (
                self.parent_id._get_full_name(level - 1)
                + MENU_ITEM_SEPARATOR
                + (self.name or "")
            )
        else:
    ...
    def _search(
        ...
        menu_ids = super(IrUiMenu, self)._search(
            args,
            offset=0,
            limit=None,
            order=order,
    ...
    def unlink(self):
        ...
        direct_children = self.with_context(**extra).search(
    ...
    def _set_menuitems_xmlids(self, menu_root):
        menuitems = (
            ...
            .sudo()
            .search([("res_id", "in", self.ids), ("model", "=", "ir.ui.menu")])
        )
