...
class Country(models.Model):
    ...
    def _name_search(
        ...
        if args is None:
            args = []
        country_ids = self._search(
            [("code", "=", name)] + args, limit=limit, access_rights_uid=name_get_uid
        )
        if not country_ids:
            search_domain = [("name", operator, name)]
            country_ids = self._search(
                search_domain + args, limit=limit, access_rights_uid=name_get_uid
            )
        return [
            (country.id, country.display_name) for country in self.browse(country_ids)
        ]
