...
class TestFormattedReadGroup(common.TransactionCase):
    ...
    def test_groupby_sequence_field_sudo(self):
        RelatedBar = self.env['test_read_group.related_bar']
        RelatedFoo = self.env['test_read_group.related_foo']
        RelatedBase = self.env['test_read_group.related_base']
        ...
        RelatedBase.formatted_read_group([], ['foo_id.name'], ['__count'])
        ...
        """]):
            self.assertEqual(
                ...
                [
                    {
                        "__count": 2,
                        "__extra_domain": [("foo_id.name", "=", "foo_a_bar_a")],
                        "foo_id.name": "foo_a_bar_a",
                    },
                    {
                        "__count": 1,
                        "__extra_domain": [("foo_id.name", "=", "foo_b_bar_false")],
                        "foo_id.name": "foo_b_bar_false",
                    },
                    {"__count": 2, "__extra_domain": [("foo_id.name", "=", False)], "foo_id.name": False},
                ],
            )
        ...
        with self.assertQueries([foo_bar_name_query] * 3):
            self.assertEqual(
                ...
                [
                    ...
                    {
                        "__count": 2,
                        "__extra_domain": [("foo_id.bar_id.name", "=", False)],
                        "foo_id.bar_id.name": False,
                    },
                ],
            )
            self.assertEqual(
                RelatedBase.formatted_read_group([], ['foo_id.bar_name_sudo'], ['__count']),
                [
                    {
                        "__count": 3,
                        "__extra_domain": [("foo_id.bar_name_sudo", "=", "bar_a")],
                        "foo_id.bar_name_sudo": "bar_a",
                    },
                    {
                        "__count": 2,
                        "__extra_domain": [("foo_id.bar_name_sudo", "=", False)],
                        "foo_id.bar_name_sudo": False,
                    },
                ],
            )
            self.assertEqual(
                ...
                [
                    {
                        "__count": 3,
                        "__extra_domain": [("foo_id.bar_name", "=", "bar_a")],
                        "foo_id.bar_name": "bar_a",
                    },
                    {
                        "__count": 2,
                        "__extra_domain": [("foo_id.bar_name", "=", False)],
                        "foo_id.bar_name": False,
                    },
                ],
            )
    ...
    def test_groupby_sequence_field_no_rules(self):
        RelatedBar = self.env['test_read_group.related_bar']
        RelatedFoo = self.env['test_read_group.related_foo']
        RelatedBase = self.env['test_read_group.related_base']
        ...
        RelatedBase.formatted_read_group([], ['foo_id.bar_id.name'], ['__count'])
        with self.assertQueries([foo_bar_name_query] * 2):
            self.assertEqual(
                RelatedBase.formatted_read_group([], ['foo_id.bar_id.name'], ['__count']),
                [
                    {
    ...
    def test_groupby_sequence_field_with_ir_rules(self):
        RelatedBar = self.env['test_read_group.related_bar']
        RelatedFoo = self.env['test_read_group.related_foo']
        RelatedBase = self.env['test_read_group.related_base']
        ...
        with self.assertQueries([f"""
            SELECT "{alias_join}"."name",
                   COUNT(*)
            FROM "test_read_group_related_base"
            LEFT JOIN "test_read_group_related_foo" AS "{alias_join}"
                ON (
                    "test_read_group_related_base"."foo_id" = "{alias_join}"."id"
                    AND "{alias_join}"."id" IN (
                        SELECT "test_read_group_related_foo"."id"
                        FROM "test_read_group_related_foo"
                        WHERE "test_read_group_related_foo"."id" IN %s
                    )
                )
            GROUP BY "{alias_join}"."name"
            ORDER BY "{alias_join}"."name" ASC
        """]):
            # foos[0] not accessible, then foo_a_bar_a result into False result
            self.assertEqual(
                RelatedBase.formatted_read_group([], ['foo_id.name'], ['__count']),
                [
                    {
                    ...
                    {
                        "__count": 4,
                        "__extra_domain": [("foo_id.name", "=", False)],
                        "foo_id.name": False,
                    },
                ],
            )
        ...
        # Same query generated by grouping foo_id.bar_id.name/foo_id.bar_name/foo_id.bar_name_sudo
        foo_bar_name_query = f"""
            SELECT "{alias_join}__bar_id"."name",
                   COUNT(*)
            FROM "test_read_group_related_base"
            LEFT JOIN "test_read_group_related_foo" AS "{alias_join}" ON (
                "test_read_group_related_base"."foo_id" = "{alias_join}"."id"
                AND "{alias_join}"."id" IN (
                    SELECT "test_read_group_related_foo"."id"
                    FROM "test_read_group_related_foo"
...
class TestFormattedReadGroupMonetary(common.TransactionCase):
    ...
    def test_different_currencies(self):
        ...
        read_group_res = self.MonetaryAgg.formatted_read_group(
            [], groupby=['name'],
            aggregates=['total_in_currency_id:sum', 'total_in_related_stored_currency_id:sum'],
        )

        self.assertEqual(len(read_group_res), 1)
        group = read_group_res[0]
    ...
    def test_mixed_one_and_none_currencies(self):
        records_name = 'Some name'
        self.MonetaryAgg.create({
            'name': records_name,
            'currency_id': self.usd.id,
    ...
    def test_non_stored_currency(self):
        self.MonetaryAgg.create({
            'name': "Some name",
            'related_model_id': self.related_model_usd.id,
        ...
        with self.assertRaises(ValueError):
            self.MonetaryAgg.formatted_read_group(
                [], groupby=['name'],
