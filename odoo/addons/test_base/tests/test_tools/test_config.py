...
EMPTY_CONFIG_PATH = file_path('test_base/tests/test_tools/config/empty.conf')
...
class TestConfigManager(TransactionCase):
    ...
    def test_02_config_file(self):
        config_path = file_path('test_base/tests/test_tools/config/non_default.conf')
    ...
    def test_03_save_default_options(self):
        with file_open_temporary_directory(self.env) as temp_dir:
            ...
            with (file_open(config_path, env=self.env) as config_file,
                  file_open('test_base/tests/test_tools/config/save_posix.conf', env=self.env) as save_file):
    ...
    def test_04_odoo16_config_file(self):
        ...
        config_path = file_path('test_base/tests/test_tools/config/16.0.conf')
        with self.assertLogs('odoo.tools.config', 'WARNING') as capture:
    ...
    def test_06_cli(self):
        with file_open('test_base/tests/test_tools/config/cli') as file:
            with self.assertLogs('odoo.tools.config', 'WARNING') as capture:
    ...
    def test_07_environ(self):
        with file_open('test_base/tests/test_tools/config/environ') as file:
            os.environ.update({
                x[0]: x[2]
    ...
    def test_06_syslog_logfile_exclusive_cli(self, error):
        ...
        self.parse_reset(['-c', file_path('test_base/tests/test_tools/config/sysloglogfile.conf')])
    ...
    def test_10_init_update_incompatible_with_multidb(self, error):
        ...
        self.parse_reset(['-c', file_path('test_base/tests/test_tools/config/multidb.conf'), '-i', 'base'])
        self.parse_reset(['-c', file_path('test_base/tests/test_tools/config/multidb.conf'), '-u', 'base'])
    ...
    def test_15_addons_path_glob(self):
