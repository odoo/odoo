...
@tagged('at_install', '-post_install')  # LEGACY at_install
class TestFloatExport2(common.TransactionCase):
...
class TestCurrencyExport(TestExport):
    ...
    def test_currency_post(self):
        ...
        self.assertEqual(
            converted, '<span class="oe_currency_value">-\N{ZERO WIDTH NO-BREAK SPACE}0.12</span>'
...
class TestBinaryExport(TestBasicExport):
    def test_image(self):
        ...
        binary = self.file_read('test_orm/tests/test_vectors/image')
        ...
        binary = self.file_read('test_orm/tests/test_vectors/pdf')

        with self.assertRaises(ValueError):
            converter.value_to_html(binary, {})

        binary = self.file_read('test_orm/tests/test_vectors/pptx')
