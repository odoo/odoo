...
class TestTransactionCase(TransactionCase):
    ...
    def test_00(self):
        """ Create a partner. """
        partners = self.env['test_testing_utilities.c'].search([('name', '=', 'test_per_class_teardown_partner')])
        self.assertEqual(0, len(partners), "Test partner found.")
        self.env['test_testing_utilities.c'].create({'name': 'test_per_class_teardown_partner'})
        partners = self.env['test_testing_utilities.c'].search([('name', '=', 'test_per_class_teardown_partner')])
    ...
    def test_20a(self):
        ...
        pid, _ = self.env['test_testing_utilities.c'].name_create('Mr Yellow')
        self.env['ir.model.data'].create({'name': 'test_partner_yellow',
                                          'module': 'base',
                                          'model': 'test_testing_utilities.c',
                                          'res_id': pid})
