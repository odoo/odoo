...
class TestOnChange(common.TransactionCase):
    ...
    def test_onchange(self):
        ...
        self.assertEqual(
            result["value"]["name"], "[%s] %s" % (discussion.name, USER.name)
        )
    ...
    def test_onchange_specific(self):
        ...
        self.assertItemsEqual(
            strip_prefix("participants.", field_onchange), ["display_name"]
        )
        ...
        self.assertItemsEqual(
            ...
            [(5,)]
            + [
                (1, user.id, {"display_name": user.display_name})
    ...
    def test_onchange_default(self):
        ...
        self.env["ir.default"].set(
            "test_new_api.foo", "value2", 666, condition="value1=42"
        )
    ...
    def test_onchange_one2many_with_domain_on_related_field(self):
        ...
        view_info = self.Discussion.fields_view_get(
            view_id=self.env.ref("test_new_api.discussion_form").id, view_type="form"
        )
        ...
        values = {
            ...
            "message_changes": 0,
            "important_messages": [
                (4, msg.id) for msg in discussion.important_messages
            ],
        ...
        self.assertEqual(
            result["value"]["important_messages"],
    ...
    def test_onchange_related(self):
        value = {"message": 1, "message_name": False, "message_currency": 2}
        field_onchange = {
            "message": "1",
            "message_name": None,
        ...
        result = Message.onchange(
            value, ["message", "message_name", "message_currency"], field_onchange
        )

        self.assertEqual(result["value"], onchange_result)

        self.env.cache.invalidate()
        Message = self.env(user=self.env.ref("base.user_demo").id)[
            "test_new_api.related"
        ]
        result = Message.onchange(
            value, ["message", "message_name", "message_currency"], field_onchange
        )
