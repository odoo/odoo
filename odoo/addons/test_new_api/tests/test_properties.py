...
class PropertiesCase(TransactionCase):
    ...
    def test_properties_field_performance(self):
        ...
        values = self.env['test_new_api.message'].browse((self.message_1.id, 133337))._read_format(['attributes'])
...
class PropertiesSearchCase(PropertiesCase):
    ...
    @mute_logger('odoo.fields')
    def test_properties_field_search_basic(self):
        ...
        for forbidden_char in '! ()"\'.':
            ...
            for search in searches:
                with (self.assertRaises(AssertionError if forbidden_char == "." else ValueError),
