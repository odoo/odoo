...
class PropertiesSearchCase(TransactionExpressionCase, TestPropertiesMixin):
    ...
    def test_properties_field_search_boolean(self):
        ...
        messages = self._search(self.env['test_new_api.message'], [('attributes.myboolean', '=', True)])
        self.assertEqual(messages, self.message_1)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myboolean', '!=', False)])
        self.assertEqual(messages, self.message_1)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myboolean', '=', False)])
    ...
    def test_properties_field_search_char(self):
        ...
        messages = self._search(self.env['test_new_api.message'], [('attributes.mychar', '=', 'Test')])
        self.assertEqual(messages, self.message_1, "Should be able to search on a properties field")
        messages = self._search(self.env['test_new_api.message'], [('attributes.mychar', '=', '"Test"')])
        self.assertFalse(messages)
        messages = self._search(self.env['test_new_api.message'], [('attributes.mychar', 'ilike', 'test')])
        self.assertEqual(messages, self.message_1 | self.message_2)
        messages = self.env['test_new_api.message'].search([('attributes.mychar', 'not ilike', 'test')])  # FIXME return message_3
        self.assertFalse(messages)
        messages = self._search(self.env['test_new_api.message'], [('attributes.mychar', 'ilike', '"test"')])
        self.assertFalse(messages)
        ...
        messages = self._search(self.env['test_new_api.message'], [('attributes.mychar', '=', False)])
    ...
    def test_properties_field_search_float(self):
        ...
        messages = self._search(self.env['test_new_api.message'], [('attributes.myfloat', '>', 4.4)])
        self.assertEqual(messages, self.message_2)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myfloat', '<', 4.4)])
        self.assertEqual(messages, self.message_1)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myfloat', '>', 1.1)])
        self.assertEqual(messages, self.message_1 | self.message_2)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myfloat', '<=', 1.1)])
        self.assertFalse(messages)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myfloat', '=', 3.14)])
        self.assertEqual(messages, self.message_1)
    ...
    def test_properties_field_search_integer(self):
        ...
        messages = self._search(self.env['test_new_api.message'], [('attributes.myint', '>', 4)])
        self.assertEqual(messages, self.message_1 | self.message_2)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myint', '<', 4)])
        self.assertEqual(messages, self.message_3)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myint', '=', 111)])
        self.assertEqual(messages, self.message_2)
        # search on the JSONified value (operator "->>")
        messages = self._search(self.env['test_new_api.message'], [('attributes.myint', 'ilike', '1')])
        self.assertEqual(messages, self.message_2)
        messages = self._search(self.env['test_new_api.message'], [('attributes.myint', 'not ilike', '1')])
    ...
    def test_properties_field_search_many2many(self):
        ...
        # TODO implement filtered and use self._search in test
...
class PropertiesGroupByCase(TestPropertiesMixin):
    ...
    def test_properties_field_read_group_selection(self):
        ...
        self.assertEqual(
            self.env['test_new_api.message'].search(result[1]['__extra_domain']),
    ...
    def _check_domains_count(self, result):
        ...
        for line in result:
            records = self.env['test_new_api.message'].search(line['__extra_domain'])
