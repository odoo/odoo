...
from odoo.domains import D, Domain, NEGATIVE_TERM_OPERATORS
        ...
        def make_domain(path, model):
            if '.' not in path:
                if isinstance(value, Domain):
                    ...
                    final_value = value
                return D(path, operator, final_value)
            ...
            domain = D(prefix, 'any', comodel._search(sub_domain))
            if can_be_null and field.type == 'many2one' and not field.required:
                domain |= D(prefix, '=', False)
    ...
    def convert_to_record(self, value, record):
        ...
        for property_definition in value:
            ...
            if property_domain:
                ...
                try:
                    dom = D(ast.literal_eval(property_domain))
                    record.env[property_model].sudo()._search(dom)
                except ValueError:
                    del property_definition['domain']
