...
@check_db_management_enabled
def dump_db(db_name, stream, backup_format='zip', filestore=True):
    """Dump database `db` into file-like object `stream` if stream is None
    return a file object with the dump """

    _logger.info('DUMP DB: %s format %s %s', db_name, backup_format, 'with filestore' if filestore else 'without filestore')
