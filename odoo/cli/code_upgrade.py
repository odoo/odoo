...
class FileManager:
    ...
    def __init__(self, addons_path: list[str], glob: str = '**/*') -> None:
        ...
        for addon_path in addons_path:
            for path_addon in Path(addon_path).iterdir():
                for path in path_addon.glob(glob):
                    if path.is_file() and path.name.rsplit('.')[-1] in AVAILABLE_EXT:
                        self._files[str(path)] = FileDescriptor(path, path_addon)
...
def get_version(value: str) -> tuple[int, ...]:
    return tuple(int(x) if x.isnumeric() else x for x in value.split('.'))


def get_upgrade_code_scripts(from_version: tuple[int, ...], to_version: tuple[int, ...]) -> list[ModuleType]:
    modules = []
    script_paths = list(Path(__file__).parent.parent.glob('code_upgrade/*.py'))
...
def migrate(
    ...
    for (name, module) in modules:
        print(f'update script: {name}')  # noqa: T201
    ...
    for file in file_manager:
        if file.dirty:
            ...
            if not test:
                file.path.open("w").write(file.content)
