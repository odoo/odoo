...
class Db(Command):
    ...
    def run(self, cmdargs):
        ...
        dump.add_argument(
            '--no-filestore', action='store_const', dest='no_filestore', default=False, const=True,
            help="if true, zip database is dumped without filestore (default: false)"
        )
    ...
    def dump(self, args):
        if args.dump_path == '-':
            ...
            with open(args.dump_path, 'wb') as f:
                dump_db(args.database, f, args.dump_format, not args.no_filestore)
