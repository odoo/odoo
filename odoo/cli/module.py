...
class Module(Command):
    ...
    def _install(self, parsed_args):
        with self._create_env_context(parsed_args.db_name) as env:
            ...
            non_installable_modules = set(parsed_args.modules or []) - set(installable_modules.mapped("name"))
    ...
    def _upgrade(self, parsed_args):
        with self._create_env_context(parsed_args.db_name) as env:
            if parsed_args.modules:
                ...
                valid_module_names = env['ir.module.module'].search([('state', '=', 'installed')]).mapped('name')
