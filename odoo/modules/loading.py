...
def load_module_graph(
    cr, graph, status=None, perform_checks=True, skip_modules=None, report=None
):
        ...
        try:
            ...
            for filename in _get_files_of_kind(kind):
                ...
                if kind in ("demo", "demo_xml") or (
                    ...
                    noupdate = True
                tools.convert_file(
                    cr, module_name, filename, idref, mode, noupdate, kind, report
                )
        finally:
            if kind in ("demo", "test"):
    ...
    for index, package in enumerate(graph, 1):
        ...
        if (
            ...
            has_demo = hasattr(package, "demo") or (
                package.dbdemo and package.state != "installed"
            )
            if has_demo:
                _load_data(cr, module_name, idref, mode, kind="demo")
                cr.execute(
                    "update ir_module_module set demo=%s where id=%s", (True, module_id)
                )
            ...
            if has_demo:
                ...
                if tools.config.options["test_enable"]:
                    ...
                    env[
                        "ir.http"
                    ]._clear_routing_map()  # force routing map to be rebuilt
                    report.record_result(
                        odoo.modules.module.run_unit_tests(module_name, cr.dbname)
                    )
    ...
    _logger.log(
        25,
        "%s modules loaded in %.2fs, %s queries",
        len(graph),
        time.time() - t0,
...
def load_marked_modules(
    cr, graph, states, force, progressdict, report, loaded_modules, perform_checks
):
    ...
    try:
        ...
        if processed_modules:
            ...
            for (model, name) in cr.fetchall():
                if (
                    model in registry
                    and not registry[model]._abstract
                    and not registry[model]._transient
                ):
                    _logger.warning(
