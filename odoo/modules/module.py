...
class Manifest(Mapping[str, typing.Any]):
    ...
    def _get_manifest_from_addons(module: str) -> Manifest | None:
        ...
        for adp in odoo.addons.__path__:
            if manifest := Manifest._get_manifest_from_path(opj(adp, module)):
                return manifest
        return None
    ...
    def for_addon(module_name: str, *, downloaded: bool = False, display_warning: bool = True) -> Manifest | None:
        ...
        if downloaded:
            ...
            if mod := Manifest._get_manifest_from_path(path):
                return mod
        if display_warning:
    ...
    @staticmethod
    def _get_manifest_from_path(path: str, env=None) -> Manifest | None:
    ...
    def all_addon_manifests() -> list[Manifest]:
        ...
        for adp in odoo.addons.__path__:
            ...
            modules.extend(
                ...
                if (mod := Manifest._get_manifest_from_path(opj(adp, file_name)))
            )
...
def load_manifest(module: str, mod_path: str | None = None) -> dict:
    ...
    if mod_path:
        mod = Manifest._get_manifest_from_path(mod_path)
