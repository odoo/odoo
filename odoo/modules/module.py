...
def load_openerp_module(module_name):
    ...
    try:
        ...
        tace = traceback.format_exc()
        matching = re.search(r' *(\w+)(?: *: ([^ ]*))? *= *fields[.](One2many|Many2many|Many2one)(?:\[([^\]]*)\])?', tace)
        if 'most likely due to a circular import' in tace and matching:
            ...
            raise AttributeError((
                ...
                "    %(field_name)s: '%(module_name)s.%(ModelClass)s' = fields.%(ttype)s(...)"
                ) % {
