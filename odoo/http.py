...
def redirect_with_hash(url, code=303):
    ...
    return (
        "<html><head><script>window.location = '%s' + location.hash;</script></head></html>"
        % url
    )
...
class WebRequest(object):
    ...
    def _call_function(self, *args, **kwargs):
        ...
        if self.endpoint.routing["type"] != self._request_type:
            msg = (
                "%s, %s: Function declared as capable of handling request of type '%s' but called with a request of type '%s'"
            )
            params = (
    ...
    * a ``str`` or ``unicode``, will be wrapped in a Response object and
    ...
    def dispatch(self):
        if (
            ...
            and request.endpoint.routing.get("cors")
        ):
            headers = {
                ...
                if hasattr(mv, "routing"):
                    ...
                    if not nodb_only or routing["auth"] == "none":
                        ...
                        for url in routing["routes"]:
                            ...
                            xtra_keys = (
                                "defaults subdomain build_only strict_slashes redirect_to alias host".split()
                            )
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def save_request_data(self):
        ...
        self["serialized_request_data"] = {"form": req.form, "files": files}
...
class Root(object):
    ...
    def load_addons(self):
        ...
        app = werkzeug.wsgi.SharedDataMiddleware(
            self.dispatch, statics, cache_timeout=STATIC_CACHE
        )
        ...
        rv.set_etag(
            ...
            % (
                ...
                adler32(
                    ...
                    if isinstance(filename, pycompat.text_type)
                    else filename
                )
                & 0xffffffff,
            )
