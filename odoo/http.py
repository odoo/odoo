...
class Root(object):
    ...
    def get_response(self, httprequest, result, explicit_session):
        ...
        if not explicit_session and hasattr(response, 'set_cookie'):
            max_age = float(odoo.tools.config['session_max_age'])
            if max_age:
                response.set_cookie('session_id', httprequest.session.sid, max_age=max_age, httponly=True)
            else:
                response.set_cookie('session_id', httprequest.session.sid, httponly=True)
