...
def compile_expr(expr, bound_variables):
    ...
    # print([(t.value, t.type) for t in tokens])
    i = 0
    while i < len(tokens):
        tok = tokens[i]
...
def fix_template(root: etree._ElementTree, bound_variables):
    ...
    for node in root.xpath("//xpath[@expr]"):
        expr = node.get("expr")
        if expr:
            node.set("expr", process_xpath_expr(expr, bound_variables))

        for n in root.xpath("//attribute[@name]"):
            attr = n.get("name")
            if attr in DIRECTIVES:
                n.text = compile_expr(n.text, bound_variables)
...
def fix_rendering_context(root: etree._ElementTree, vars):
    template_nodes = set(root.xpath("//*[@t-name]"))
...
def replace_x_path_only(content: str, variables: dict):
    def fix_xpath(root: etree._ElementTree):
        for node in root.xpath("//*[@t-inherit]"):
            attr = node.get("t-inherit")
            if attr.startswith("web."):
                for xp in node.xpath(".//xpath[@expr]"):
                    expr = xp.get("expr")
                    if expr:
                        xp.set("expr", process_xpath_expr(expr, variables[attr], SKIP_XPATH_ATTRS.union({"call"})))
        return
...
def _aggregate_call_vars(root: etree._ElementTree, vars):
    ...
    for call_node in root.xpath("//*[@t-call]"):
        ...
        for set_node in call_node.xpath(".//*[@t-set]"):
            var_name = set_node.get("t-set")
            if var_name:
                var_set.add(var_name)

    for call_node in root.xpath("//*[@t-foreach]"):
        print(call_node)
...
def _aggregate_inside_vars(root: etree._ElementTree, inside_vars: dict[str, set[str]]):
    ...
    templates = root.xpath("//*[@t-name] | //template[@id]")
    for tpl in templates:
        ...
        for set_node in tpl.xpath(".//*[@t-set]"):
            _add_var(var_set, set_node.get("t-set"))

        # Loops introduce locals via t-as (and maybe an index var)
        for loop_node in tpl.xpath(".//*[@t-foreach] | .//*[@t-for-each]"):
            _add_var(var_set, loop_node.get("t-as"))
            _add_var(var_set, loop_node.get("t-foreach-index") or loop_node.get("t-index"))
...
if __name__ == "__main__":
    ...
    for test in tests:
        ...
        if WHITELIST and name not in WHITELIST:
            continue  # skip tests not in whitelist
        content = test["content"]
        output = update_template(content, {})
        name = test.get("name")
    ...
    for test in test_xpath:
        name = test.get("name")
        if WHITELIST and name not in WHITELIST:
            continue  # skip tests not in whitelist
        content = test["content"]
        output = replace_x_path_only(content)
        name = test.get("name")
        if test["expected"] != output:
            fail += 1
            print(f"{name}: fail")
            ...
            success += 1
    if not fail:
        print(f"Yep, {success} tests passed")

    c1 = '<t t-name="abc"><t t-out="a"/><t t-out="b"/></t>'
    e1 = '<t t-name="abc"><t t-out="a"/><t t-out="this.b"/></t>'
    r1 = update_template(c1, {"abc": set(["a"])})

    if r1 == e1:
        print("success for vars")
    else:
        print("nope")
