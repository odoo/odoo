...
class upgrade:
    ...
    def generate(self, module: str):
            ...
            for index, group in enumerate(implying.follow(rule.group)):
                ...
                if index == 0:
                    operations = rule.operations & set().union(*(
                        permissions[rule.model, impl] for impl in implied.follow(group)
                    ))
                else:
        ...
        for access in self.extract_accesses(module, remove=True):
            ...
            operations = access.operations - set().union(*(
                ops for ops, _, _ in ir_access_rules[access.model][access.group]
            ))
            if operations:
    ...
    def extract_accesses_from_csv(self, module: str, file_name: str, remove: bool) -> Iterator[Access]:
        ...
        for line in reader:
            ...
            if not operations:
                logger.info("INFO %s: ir.model.access without operations, skipping %s in %s", module, xid, file_name)
                continue
    ...
    def extract_rules_from_xml(self, module: str, file_name: str, remove: bool) -> Iterator[Access]:
        ...
        for node in tree.xpath("//record[@model='ir.rule']"):
            ...
            if not operations:
                logger.info("INFO %s: ir.rule without operations, skipping %s in %s", module, xid, file_name)
                nodes.append(node)
                continue
...
def extract_model_names(file) -> list[str]:
    ...
    for line in file.content.splitlines():
        ...
        if match['class']:
            ...
            model_name = RE_DOT_PLACE.sub(r'.\1', class_name).lower()
