...
def upgrade_t_model(file_manager, log_info, log_error):
    ...
    reg_t_model = re.compile(r'\b(?<!-)t-model(\s*=)')
    for fileno, file in enumerate(files, start=1):
        try:

            raw_content = file.path.read_bytes()
            content = raw_content.decode("utf-8", errors="ignore")
            new_content, count = reg_t_model.subn(r't-custom-model\1', content)
...
def upgrade_t_ref(file_manager, log_info, log_error):
    files = [file for file in file_manager if file.path.suffix == '.xml']
    reg_t_ref = re.compile(r'\b(?<!-)t-ref(\s*=)')
    for fileno, file in enumerate(files, start=1):
        try:

            raw_content = file.path.read_bytes()
            content = raw_content.decode("utf-8", errors="ignore")
            new_content, count = reg_t_ref.subn(r't-custom-ref\1', content)
