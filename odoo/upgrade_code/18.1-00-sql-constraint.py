...
def upgrade(file_manager: FileManager):
    ...
    sql_expression_re = re.compile(r"_sql_constraints\s*=\s*\[([^\]]+)]")
    ind = ' ' * 4
    ...
    def build_sql_object(match):
        ...
        for name, definition, *messages in constraints:
            ...
            if m := re.match(r"(unique\s+)?index", definition, re.IGNORECASE):
                definition = definition[len(m.group(0)):].strip()
                constructor = "UniqueIndex" if m.group(1) else "Index"
            if message:
                # format on 2 lines
