...
def upgrade(file_manager):
    ...
    for addon_init, imported_modules in class_to_import.items():
        ...
        module_paths = reversed(sorted(imported_modules.keys(), key=lambda module_path: get_module_order_key(file_manager, addon_init, module_path)))
        ...
        import_ref = re.findall(rf"(from.* import .*)", content)[-1]
...
def get_module_order_key(file_manager, addon_init, module_paths: tuple[str]) -> tuple[int]:
    if module_paths not in order:
        ...
        order[module_paths] = tuple(indexes)
    return order[module_paths]
