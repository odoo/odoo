...
class BinaryRelation:
    ...
    def __init__(self, items: Iterable[tuple[str, str]]):
        self.__map = rel = defaultdict(dict)  # type: ignore[var-annotated]
        for x, y in items:
            rel[x].setdefault(y, None)

    def __call__(self, x: str) -> Iterable[str]:
        """ Return the elements related to `x`. """
        return self.__map.get(x, ())

    def closure(self) -> Closure:
        """ Return the reflexive transitive closure of `self`. """
        return Closure(self)
    ...
    def inverse(self) -> BinaryRelation:
        ...
        return BinaryRelation(
            (y, x)
            for x, ys in self.__map.items()
            for y in ys
        )


class Closure(BinaryRelation):
    """ Represents the reflexive transitive closure of a binary relation. """
    def __init__(self, relation: BinaryRelation):
    ...
    def __call__(self, x: str) -> Iterable[str]:
        ...
        for y in self.relation(x):
            for z in self(y):
                result.setdefault(z, None)
        return result

    def closure(self) -> Closure:
        return self

    def inverse(self) -> BinaryRelation:  # type: ignore[override]
        return Closure(self.relation.inverse())
...
class upgrade:
    ...
    def generate(self, module: str):
        ...
        # is: if group A implies group B, every ir.model.access on B applies to
        ...
        from_rules = defaultdict(lambda: defaultdict(list))
        ...
        for rule in self.extract_rules(module, remove=True):
            ...
            if rule.group is None:
                from_rules[rule.model][rule.group].append(
            ...
            for group in implying(rule.group):
                ...
                added = True
                entries = from_rules[rule.model][group]
        ...
        # {model: {group: [(operations, domain, name)]}}
        from_acls = defaultdict(lambda: defaultdict(list))
        for access in self.extract_accesses(module, remove=True):
            ...
            entries = from_acls[access.model][access.group]
            # only consider operations from access that do not appear in rules
            operations = access.operations - {
                op for ops, _, _ in from_rules[access.model][access.group] for op in ops
            }
            if operations:
    ...
    def get_groups_implied(self) -> BinaryRelation:
        ...
        return BinaryRelation(
            ...
            for x_implies_y in self._get_groups_implied_from_xml(module, file_name)
        )

    def _get_groups_implied_from_xml(self, module: str, file_name: str) -> Iterator[tuple[str, str]]:
        """ Retrieve the group implications from a given XML file. """
        try:
            tree = etree.fromstring(self.get_file_content(module, file_name).encode())
        ...
        for node in tree.xpath("//record[@model='res.groups']"):
            ...
            for field_node in node.findall("./field[@name='implied_ids']"):
                for match in REF_RE.finditer(field_node.get('eval')):
                    yield (xid, with_prefix(module, match['ref']))
