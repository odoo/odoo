...
class _Cleaner(clean.Cleaner):
    ...
    def __call__(self, doc):
        ...
        # force attribute sanitization of specified tags
...
def html_sanitize(src, silent=True, sanitize_tags=True, sanitize_attributes=False, sanitize_style=False, sanitize_form=True, sanitize_conditional_comments=True, strip_style=False, strip_classes=False, output_method="html"):
    ...
    def sanitize_handler(doc):
        ...
        if sanitize_form:
            # For bootstrap/snippet we need to allow BUTTON tag that
            # are removed by lxml forms=False
            # We reproduce lxml cleaner forms=True without BUTTON
            ...
            # We forcibly apply attribute sanitization to BUTTON tags to
            # prevent dangerous attributes (autofocus, formaction, …)
