...
allowed_tags = clean.defs.tags | frozenset(
    "article section header footer hgroup nav aside figure main".split()
    + [etree.Comment]
)
...
def html_sanitize(
    ...
    src = part.sub(
        ...
        and misc.html_escape(m.group(1))
        or m.group(1),
        src,
    )
    ...
    if sanitize_attributes and etree.LXML_VERSION >= (
        ...
        kwargs.update({"safe_attrs_only": True, "safe_attrs": current_safe_attrs})
    else:
        kwargs.update(
            {
...
def email_references(references):
    ref_match, model, thread_id, hostname, is_private = (
        False,
        False,
        False,
        False,
...
def decode_message_header(message, header, separator=" "):
    return separator.join(
        decode_smtp_header(h) for h in message.get_all(header, []) if h
    )
