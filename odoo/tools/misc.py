...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    if not timestamp:
        ...
        days_offset = int.from_bytes(hashlib.sha1(f"d-{unique_str}".encode()).digest(), "little")
        seconds_offset = int.from_bytes(hashlib.sha1(f"s-{unique_str}".encode()).digest(), "little")
        date = start_of_period + datetime.timedelta(
            days=28 + days_offset % 14, seconds=seconds_offset % (24 * 60 * 60)
        )
