...
def create_model_table(cr, tablename, comment=None):
    ...
    cr.execute(
        'CREATE TABLE "{}" (id SERIAL NOT NULL, PRIMARY KEY(id))'.format(tablename)
    )
    if comment:
...
def create_column(cr, tablename, columnname, columntype, comment=None):
    ...
    cr.execute(
        'ALTER TABLE "{}" ADD COLUMN "{}" {}'.format(tablename, columnname, columntype)
    )
    if comment:
        cr.execute(
...
def add_constraint(cr, tablename, constraintname, definition):
    ...
    query = 'ALTER TABLE "{}" ADD CONSTRAINT "{}" {}'.format(
