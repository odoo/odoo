...
def try_report(
    ...
    if res_format == "pdf":
        if res_data[:5] != b"%PDF-":
            raise ValueError(
                "Report %s produced a non-pdf header, %r" % (rname, res_data[:10])
            )
        res_text = None
        try:
        ...
        _logger.warning(
            'Report %s produced a "%s" chunk, cannot examine it',
            rname,
            res_format,
        )
        return False
...
def try_report_action(
    ...
    if isinstance(action_id, str):
        if "." in action_id:
            ...
            if not our_module:
                raise ValueError(
                    'You cannot only specify action_id "%s" without a module name'
                    % action_id
                )
            act_xmlid = action_id
    ...
    def _exec_action(action, datas, env):
        ...
        if action["type"] in ["ir.actions.act_window", "ir.actions.submenu"]:
            ...
            assert view_res and view_res.get(
                "arch"
            ), "Did not return any arch for the view"
            view_data = {}
            arch = etree.fromstring(view_res["arch"])
            fields = [
                el.get("name") for el in arch.xpath("//field[not(ancestor::field)]")
            ]
            if fields:
            ...
            for fk in fields:
                ...
                if (
                    model._fields[fk].type in ("one2many", "many2many")
            ...
            _logger.debug(
                "Buttons are: %s",
                ", ".join(["%s: %d" % (b["string"], b["weight"]) for b in buttons]),
            )
            ...
            while buttons and not res:
                ...
                if not b["type"]:
                    log_test(
                        'the "%s" button has no type, cannot use it',
                        b["string"],
                    )
                    continue
                if b["type"] == "object":
