...
class graph(object):
    ...
    def rev_edges(self, tree):
        ...
        for link in self.links:
            ...
            des = link[1]
            edge_len = (
                self.partial_order[des]["level"] - self.partial_order[src]["level"]
            )
            if edge_len < 0:
                del self.links[i]
            ...
            if self.edge_wt[edge] > 1:
                ...
                for rank in range(start, end):
                    if start == rank:
                        ...
                        self.transitions.setdefault((rank, "temp"), []).append(
                            (rank + 1, "temp")
                        )
        ...
        if self.Is_Cyclic:
            ...
            if max_level % 2:
                self.result[self.start]["y"] = (
                    (max_level + 1) / 2 + self.max_order + (self.max_order and 1)
                )
            else:
                self.result[self.start]["y"] = (
