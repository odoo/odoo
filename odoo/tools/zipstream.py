...
def generate_zip(
    attachments: Iterable['IrAttachment | ZipAttachment'],
    ...
    :param attachments: The attachments, either as a recordset or as a
        list of :class:`ZipAttachment`.
    ...
    :returns: An iterator (something you can iter on, but only once)
        that yields the pieces of the zipfile as it is being generated.
    ...
    .. example::
        ...
        .. code-block::
            ...
            attachments.fetch(ZipAttachment.__required_fields__)
            stream = generate_zip(attachments)
    ...
    for attach in attachments:
        ...
        compression = (
            ...
            if attach['raw'].mimetype.startswith('text/') else
        ...
        dd = local_file.data_descriptor = DataDescriptor(
            crc32=0,
            compressed_size=0 if compression else attach['raw'].size,
            uncompressed_size=attach['raw'].size,
        )
        if compression:
            compressor = compression.compressor()
        with attach['raw'].open() as file:
...
class DataDescriptorBytes(bytes):
    pass
...
class DataDescriptor:
    ...
    def pack(self, zip64=None):
        ...
        return DataDescriptorBytes(
            struct.pack(self._struct_64 if zip64 else self._struct,
                self.signature,
                self.crc32,
...
class LocalFileHeaderBytes(bytes):
    pass
...
class CentralDirectoryFileHeaderBytes(bytes):
    pass
...
class EndOfCentralDirectory64Bytes(bytes):
    pass
...
class EndOfCentralDirectory64:
    ...
    def pack(self, encoding='utf-8'):
        return EndOfCentralDirectory64Bytes(
            struct.pack(self._struct,
                self.signature,
...
class EndOfCentralDirectoryLocatorBytes(bytes):
    pass
...
class EndOfCentralDirectoryLocator:
    ...
    def pack(self):
        return EndOfCentralDirectoryLocatorBytes(
            struct.pack(self.struct,
                self.signature,
...
class EndOfCentralDirectoryBytes(bytes):
    pass
...
class EndOfCentralDirectory:
    ...
    def pack(self):
        return EndOfCentralDirectoryBytes(
            struct.pack(self._struct,
                self.signature,
                self.disk_no,
                self.central_directory_disk_no,
