...
def parse_date(value: str, env: Environment) -> date | datetime:
    ...
    if terms[0] == 'today':
        terms.pop(0)
        now = Date.context_today(env['base'], now)
    else:
        if terms[0] == 'now':
            terms.pop(0)
        now = Datetime.context_timestamp(env['base'], now)
    ...
    for term in terms:
        ...
        # Weekday
        if (weekday_offset := WEEKDAY_NUMBER.get(term[1:])) is not None:
            # current week starting on Monday
            weekday_offset -= now.weekday()
            if operator in ('+', '-'):
                if weekday_offset > 0 and operator == '-':
                    weekday_offset -= 7
                elif weekday_offset < 0 and operator == '+':
                    weekday_offset += 7
            elif isinstance(now, datetime):
                now += TRUNCATE_TODAY
            now += timedelta(weekday_offset)
            continue
        # Operations on dates
        try:
