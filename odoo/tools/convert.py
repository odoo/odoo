...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_template(self, el):
        ...
        if 'track' in attrs:
            record.append(Field(attrs['track'], name='track'))
        if 'priority' in attrs:
            record.append(Field(attrs['priority'], name='priority'))
        if 'inherit_id' in attrs:
            record.append(Field(name='inherit_id', ref=attrs['inherit_id']))
        if 'website_id' in attrs:
            record.append(Field(name='website_id', ref=attrs['website_id']))
        if 'key' in attrs:
            record.append(Field(attrs['key'], name='key'))
        if attrs.get('active') in ("True", "False"):
            view_id = self.id_get(tpl_id, raise_if_not_found=False)
            if self.mode != "update" or not view_id:
                record.append(Field(name='active', eval=attrs['active']))
        if attrs.get('customize_show') in ("True", "False"):
            record.append(Field(name='customize_show', eval=attrs['customize_show']))
        groups = attrs.get('groups')
        if groups:
            el.set('groups', groups)
            grp_lst = [("ref('%s')" % x) for x in groups.split(',')]
            record.append(Field(name="group_ids", eval="[Command.set(["+', '.join(grp_lst)+"])]"))
        if attrs.get('primary') == 'True':
