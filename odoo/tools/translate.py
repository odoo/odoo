...
def translate_xml_node(node, callback, parse, serialize):
    ...
    def process(node):
        ...
        if (
            ...
            or node.tag == "attribute"
            and node.get("name") not in TRANSLATED_ATTRS
            or node.getparent() is None
            and avoid_pattern.match(node.text or "")
        ):
            return (None, node)
        ...
        for child in node:
            ...
            if child_has_text is None:
                ...
                append_content(
                    result, translate_content(todo) if todo_has_text else todo
                )
        ...
        if (
            ...
            and not (result.text or len(result))
            and not any(name.startswith("t-") for name in node.attrib)
        ):
...
def translate(cr, name, source_type, lang, source=None):
    if source and name:
        cr.execute(
            "select value from ir_translation where lang=%s and type=%s and name=%s and src=%s and md5(src)=md5(%s)",
            (lang, source_type, str(name), source, source),
        )
    elif name:
        cr.execute(
            "select value from ir_translation where lang=%s and type=%s and name=%s",
            (lang, source_type, str(name)),
        )
    elif source:
        cr.execute(
            "select value from ir_translation where lang=%s and type=%s and src=%s and md5(src)=md5(%s)",
            (lang, source_type, source, source),
        )
            ...
            if lang:
                ...
                if cr:
                    ...
                    res = env["ir.translation"]._get_source(
                        None, ("code", "sql_constraint"), lang, source
                    )
                else:
                    _logger.debug(
                        'no context cursor detected, skipping translation for "%r"',
                        source,
                    )
            else:
                _logger.debug(
                    'no translation language detected, skipping translation for "%r" ',
                    source,
                )
        except Exception:
...
class PoFile(object):
    ...
    def next(self):
        ...
        if name is None:
            if not fuzzy:
                _logger.warning(
                    'Missing "#:" formatted comment at line %d for the following source:\n\t%s',
                    self.cur_line(),
                    source[:30],
                )
            return next(self)
...
def trans_export(lang, modules, buffer, format, cr):
    ...
    def _process(format, modules, rows, buffer, lang):
        if format == "csv":
                    ...
                    with io.BytesIO() as buf:
                        ...
                        info = tarfile.TarInfo(
                            join(
                                ...
                                "i18n",
                                "{basename}.{ext}".format(
                                    basename=lang or mod, ext="po" if lang else "pot"
                                ),
                            )
                        )
            ...
            raise Exception(
                _(
                    "Unrecognized extension: must be one of "
...
def trans_generate(lang, modules, cr):
    ...
    if "all_installed" in modules:
        query += (
            " WHERE module IN ( SELECT name FROM ir_module_module WHERE state = 'installed') "
        )
        query_models += (
            " AND imd.module in ( SELECT name FROM ir_module_module WHERE state = 'installed') "
        )
    ...
    def get_module_from_path(path):
        ...
        return (
            "base"
        )  # files that are not in a module are considered as being in 'base' module
    ...
    def babel_extract_terms(
        ...
        try:
            for extracted in extract.extract(
                extract_method, src_file, keywords=extract_keywords
            ):
        ...
        if fileformat == "csv":
            ...
            if isinstance(fileobj.name, str) and fileobj.name.endswith(".po"):
                try:
                    ...
                    pot_handle = file_open(
                        os.path.join(addons, module, i18n_dir, module + ".pot"),
                        mode="rb",
                    )
        ...
        for src, target in pot_targets.items():
            if target.value:
                for type, name, res_id in target.targets:
                    pot_rows.append(
                        (type, name, res_id, src, target.value, target.comments)
                    )
        pot_targets.clear()
        for row in pot_rows:
            process_row(row)
