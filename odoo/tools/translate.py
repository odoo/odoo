...
def is_translatable_attrib(key):
    return key in TRANSLATED_ATTRS or key.endswith(('.translate', '.translate.f'))
...
def translate_xml_node(node, callback, parse, serialize):
    ...
    def translatable(node):
        ...
        return (
            ...
            and not any(key.startswith("t-") or key.endswith((".translate", ".translate.f")) for key in node.attrib)
