...
class IncrementalWriter:
    ...
    def get_next_object_id(self, pdf_reader: PdfFileReader) -> int:
        ...
        # If size is missing, the xref was damaged and pypdf rebuilt it by scanning the file,
        # so recover the highest ID from the entries it parsed. The uncompressed ones in
        # ``xref`` (keyed by generation, then by ID) and the compressed ones in ``xref_objStm``.
        if not size:
            # default=0 guards against a PDF with no parsed xref entry at all
            size = max(itertools.chain(pdf_reader.xref_objStm, *pdf_reader.xref.values()), default=0) + 1
