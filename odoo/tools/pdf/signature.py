...
# Number of bytes reserved for the /Contents of the signature field. It has to be big
# enough to hold the CMS object, i.e. the signature and the whole certificate chain.
SIGNATURE_PLACEHOLDER_SIZE = 16384
...
class PdfSigner:
    ...
    def _load_key_and_certificate(self) -> tuple[Optional[PrivateKeyTypes], Optional[Certificate]]:
        """Loads the private key and the signing certificate

        Returns:
            Optional[PrivateKeyTypes]: a private key object, or None if the key couldn't be loaded.
        """
        warnings.warn("Deprecated since 18.3, use _load_key_and_certificates instead", DeprecationWarning, stacklevel=2)
        private_key, leaf_cert, _chain = self._load_key_and_certificates()
        return private_key, leaf_cert
    ...
    def _load_key_and_certificates(self) -> tuple[PrivateKeyTypes | None, Certificate | None, list[Certificate] | None]:
        ...
        for cert in certificate._get_certificate_chain():
            all_certs.append(load_pem_x509_certificate(base64.decodebytes(cert.pem_certificate)))
    ...
    def _setup_form(self, visible_signature: bool, field_name: str, signer: Optional[ResUsers] = None) -> tuple[DictionaryObject, DictionaryObject] | None:
        ...
        # The /Contents are pre-filled with placeholder values of an arbitrary size (i.e. 16KB) to ensure that
        ...
        signature_field_value.update({
            NameObject("/Contents"): ByteStringObject(b"\0" * SIGNATURE_PLACEHOLDER_SIZE),
    ...
    def _perform_signature(self, sig_field_value: DictionaryObject) -> bool:
        ...
        signature = cms_content_info.dump()
        if len(signature) > SIGNATURE_PLACEHOLDER_SIZE:
            _logger.warning(
                "The signature takes %s bytes, more than the %s reserved for it",
                len(signature), SIGNATURE_PLACEHOLDER_SIZE,
            )
            return False

        signature_hex = signature.hex().ljust(SIGNATURE_PLACEHOLDER_SIZE * 2, "0")
