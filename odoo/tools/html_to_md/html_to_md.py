...
def html_to_markdown(root, **options):
    ...
    """
    return MarkdownRenderer(root, get_node).render_markdown(options=options)


def get_node(tag: str, options, **kwargs) -> Union[NodeWithChildren, TextNode]:
    node_class = TAG_TO_NODE_CLASS.get(tag)
    if not node_class:
        _logger.debug("html_to_md: no match found for tag %s, using base NodeWithChildren", tag)
