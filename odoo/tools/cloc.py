...
class Cloc(object):
    ...
    def count_customization(self, env):
        imported_module_sa = SQL()
        if env['ir.module.module']._fields.get('imported'):
            imported_module_sa = SQL("OR (m.imported = TRUE AND m.state = 'installed')")
        query = SQL("""
             ...
             LEFT JOIN ir_module_module AS m
                    ON m.name = d.module
                 WHERE s.state = 'code' AND (m.name IS null %s)
              GROUP BY s.id
        """, imported_module_sa)
        ...
        imported_module_field = (SQL("'odoo/studio'"), SQL())
        if env['ir.module.module']._fields.get('imported'):
            imported_module_field = (SQL("min(m.name)"), SQL("AND m.imported = TRUE AND m.state = 'installed'"))
