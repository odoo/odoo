...
# match the exact form of 'count' field generated by studio for smart
COUNT_FIELD_REGEX = """results = self\.env\[''.*''\]\.read_group\(\[\(''.*'', ''in'', self\.ids\)\], .*, .*\)
dic = {}
for x in results: dic\[x\[''.*''\]\[0\]\] = x\[''.*''\]
for record in self: record\[''.*''\] = dic\.get\(record\.id, 0\)"""
    ...
    def count_customization(self, env):
        ...
        query = """
            SELECT f.id, m.name FROM ir_model_fields AS f
                ...
                LEFT JOIN ir_module_module AS m ON m.name = d.module
            WHERE f.compute IS NOT null AND (m.name IS null {}) AND f.compute !~ '{}'
        """.format(imported_module, COUNT_FIELD_REGEX)
        env.cr.execute(query)
        data = {r[0]: r[1] for r in env.cr.fetchall()}
