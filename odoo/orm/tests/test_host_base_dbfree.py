...
def test_base_hosts_its_groups_rules_and_acls(base_env):
    env = base_env
    assert env["ir.model.access"].sudo().search_count([]) > 150
    assert env["ir.rule"].sudo().search_count([]) > 30
...
def test_an_internal_user_reads_what_the_acls_and_rules_allow(base_env):
    ...
    with pytest.raises(AccessError):
        as_user["ir.model.access"].create({"name": "x", "model_id": 1})
