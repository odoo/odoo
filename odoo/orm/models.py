...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _read_grouping_set(
        ...
        groupby_mask = {groupby: 2 ** i for i, groupby in enumerate(groupbys)}
        # for GROUPING SET ((a, b, c), (a, c), (b), ())
        # GROUPING(a, b, c) = ((2 ^ len(groupby)) - 1) - sum(mask(field) for field in group)
        # (7 - 1 - 2 - 4, 7 - 1 - 4, 7 - 2, 7)
        # {0: 0, 2: 1, 5: 2, 7: 3}
        max_mask = ((2 ** len(groupbys)) - 1)  # or sum(groupby_mask.values())
