...
def add_field(model_cls: type[BaseModel], name: str, field: Field, is_inverse: bool = False):
    ...
    if not (is_class_field or model_cls.pool['ir.model.fields']._is_manual_name(None, name) or is_inverse):
    ...
    # generate inverse One2many for each Many2one so callers always have the inverse at hand
    if field.type == 'many2one' and field.comodel_name and not is_inverse:
        comodel_cls = model_cls.pool.get(field.comodel_name)
        if comodel_cls is not None and not comodel_cls._abstract and not model_cls._abstract:
            from .fields_relational import One2many  # noqa: PLC0415
            inverse_field = One2many(
                string=f'Inverse of {model_cls._name}.{name}',
                comodel_name=model_cls._name,
                inverse_name=field.name,
                readonly=True,
            )
            inverse_field._is_autogenerated = True
            add_field(comodel_cls, field._get_inverse_name(model_cls._name), inverse_field, is_inverse=True)
...
def pop_field(model_cls: type[BaseModel], name: str) -> Field | None:
    ...
    if field is not None and field.type == 'many2one' and field.comodel_name:
        comodel_cls = model_cls.pool.get(field.comodel_name)
        if comodel_cls is not None:
            pop_field(comodel_cls, field._get_inverse_name(model_cls._name))
    return field
