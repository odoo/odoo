...
class Registry(Mapping[str, type["BaseModel"]]):
    ...
    def init(self, db_name: str) -> None:
        ...
        # must be reloaded.
        # The `orm_signaling_... sequence` indicates a caches must be
    ...
    def setup_signaling(self) -> None:
        ...
        with self.cursor() as cr:
            ...
            signaling_tables = tuple(f'orm_signaling_{cache_name}' for cache_name in ['registry', *_CACHES_BY_KEY.keys()])
            ...
            for table_name in signaling_tables:
                if table_name not in existing_sequences:
                    cr.execute(SQL(
    ...
    def get_sequences(self, cr: BaseCursor) -> tuple[int, dict[str, int]]:
        signaling_tables = tuple(f'orm_signaling_{cache_name}' for cache_name in ['registry', *_CACHES_BY_KEY.keys()])
        signaling_selects = ', '.join([f'( SELECT max(id) FROM {signaling_table})' for signaling_table in signaling_tables])
        cr.execute(f"SELECT {signaling_selects}")
        row = cr.fetchone()
