...
class DomainNary(Domain):
    ...
    def map_conditions(self, function) -> Domain:
        new_children = [child.map_conditions(function) for child in self.children]
        if all(itertools.starmap(py_operator.is_, zip(self.children, new_children))):
            # unchanged, keep current instance
            return self
...
def _optimize_any_domain(condition, model):
    ...
    try:
        ...
        raise ValueError(f"Cannot determine the relation for {condition.field_expr}")
