...
if typing.TYPE_CHECKING:
    ...
    _cursor_proxy_type = psycopg2.extensions.cursor
else:
    _cursor_proxy_type = object
...
class Savepoint:
    ...
    _cr: BaseCursor | psycopg2.extensions.cursor
    name: str
    closed: bool

    def __init__(self, cr: BaseCursor | psycopg2.extensions.cursor):
...
class BaseCursor(_cursor_proxy_type):
...
class Cursor(BaseCursor):
    ...
    def __build_dict(self, row: tuple) -> dict[str, typing.Any]:
        ...
        return {d.name: row[i] for i, d in enumerate(description)}
...
class ConnectionPool:
    ...
    def __repr__(self):
        used = sum(c._pool_used for c in self._connections)
    ...
    def borrow(self, connection_info: dict) -> PsycoConnection:
        ...
        for i, cnx in tools.reverse_enumerate(self._connections):
            if not cnx._pool_used and not cnx.closed and time.time() - cnx._pool_last_used > MAX_IDLE_TIMEOUT:
