...
class TestCursor(BaseCursor):
    ...
    @contextmanager
    def allow_readwrite(self):
        """
        Temporary grant writes (insert/update/delete) on this test
        cursor. Only works when the underlying connection's transaction
        was open in read/write.
        """
        assert not self._cursor.readonly, "the underlying connection doesn't allow writes"
        previous_mode = self.readonly
        self.readonly = False
        try:
            yield
        finally:
            self.readonly = previous_mode
...
def db_connect(to, allow_uri=False, mode='read/write'):
    ...
    if _Oeuf is None and mode == 'read-only':
        _Oeuf = ConnectionPool(int(maxconn), readonly=True)
