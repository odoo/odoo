...
class Cache:
    ...
    def contains(self, field, context, id_):
    ...
    def get_cached_ids(self, field):
        """ Return the ids if it is cached in any context. """
        context_cache = self._data.get(field, EMPTY_DICT)
        return OrderedSet(id_ for field_cache in context_cache.values() for id_ in field_cache)
