...
| `_session_store.py` | serving | `SessionStore`, the policy every backend shares — keys, load, save with merge, soft and hard rotation, successor adoption, family revocation, keep-alive — over a seven-method storage surface (`_lock`, `_read`, `_write`, `_unlink`, `_touch`, `_sids_in_family`, `vacuum`); `FilesystemSessionStore` (stripe locks shared across processes, atomic and optionally durable writes, sharded by the sid's first two characters) and `MemorySessionStore` (one `RLock`, for tests and embedding) implement it. A third backend implements the surface and nothing else |
