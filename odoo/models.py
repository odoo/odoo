...
def check_pg_name(name):
    ...
    if not regex_pg_name.match(name):
        raise ValidationError("Invalid characters in table name %r" % name)  # pylint: disable=missing-gettext
    if len(name) > 63:
        raise ValidationError("Table name %r is too long" % name)  # pylint: disable=missing-gettext
