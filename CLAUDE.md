...
Each is an exact-match ratchet, so an undone fix fails as loudly as a new offence. The AST and XML rules run at the narrow scope below; the registry-dependent classes need a fuller install. The AST rules run `E8501`–`E8528`; none is advisory and none fails outright — the floor decides.
