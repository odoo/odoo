...
class HrAeMasterReport(models.Model):
    ...
    def default_get(self, field_list=None):
        if self.env.company.country_id.code != 'AE':
            raise UserError(_('You must be logged in a U.A.E. company to use this feature'))
        ...
        for struct in payslips_data:
            ...
            for employee in payslips_data[struct]:
                ...
                if employee.contract_id.date_start:
                    joining_date = datetime.strptime(employee.contract_id.date_start.strftime('%Y-%m-%d'), '%Y-%m-%d').date()
                ...
                row.extend((XLSX['TEXT'], company.currency_id.format(t)) for t in payslips_data[struct][employee].line_ids.mapped('total'))
