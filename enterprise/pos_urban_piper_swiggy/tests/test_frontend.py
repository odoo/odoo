...
class TestPosUrbanPiperSwiggyCommon(TestTaxCommonPOS):
    ...
    def setUpClass(cls):
        ...
        cls.urban_piper_config = cls.env['pos.config'].create({
            ...
            'urbanpiper_delivery_provider_ids': [Command.set([cls.env.ref('pos_urban_piper_swiggy.pos_delivery_provider_swiggy').id])],
            'urbanpiper_webhook_url': cls.env['pos.config'].get_base_url()
        })
        cls.product_tag_2 = cls.env['product.tag'].create({
            'name': 'allergen-peanut',
        })
        cls.product_1 = cls.env['product.template'].create({
            'name': 'Product 1',
            'available_in_pos': True,
            'taxes_id': [Command.set([cls.env['account.chart.template'].ref("sgst_sale_5").id])],
            'product_tag_ids': [cls.product_tag_2.id],
            'type': 'consu',
            'list_price': 100.0,
        })
    ...
    def test_swiggy_tags_included_in_urban_piper_items(self):
        """
        ...
        self.assertEqual(items[0]['tags'].get('swiggy', []), ['alcohol-absent', 'allergen-peanut'])
        self.assertEqual(
