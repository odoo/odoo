...
QUnit.module('Social Formatter Regex', {}, () => {
    QUnit.test('Facebook Message', (assert) => {
        ...
        const testMessage = 'Hello @[542132] Odoo-Social, check this out: https://www.odoo.com?utm=mail&param=1,2,3 #crazydeals #odoo';
    ...
    QUnit.test('URL regex supports special characters across all media types', function (assert) {
        ...
        assert.expect(mediaTypes.length);
        ...
        mediaTypes.forEach(mediaType => {
            ...
            assert.ok(
                finalMessage.includes(expectedUrlPattern),
                `URL with special characters (parentheses, asterisk, dollar sign, comma, apostrophe) should be properly formatted for ${mediaType}`
            );
        });
    });
});
