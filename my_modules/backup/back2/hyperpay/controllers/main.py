...
from odoo.addons.hyperpay.const import HANDLED_WEBHOOK_EVENTS
...
class HyperpayController(http.Controller):
    ...
    # @http.route('/shop/hyperpay', auth='public', website=True)
    # def index(self, **kw):
    #     return http.request.render('hyperpay.index', {})
    ...
    def hyperpay_return_from_checkout(self, reference, **data):
    ...
    def hyperpay_webhook(self):
        ...
        _logger.info("Notification received from Hyperpay with data:\n%s", pprint.pformat(data))
        ...
        if event_type in HANDLED_WEBHOOK_EVENTS:
            ...
            try:
                ...
                received_signature = request.httprequest.headers.get('X-Hyperpay-Signature')
                tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data(
                    'hyperpay', entity_data
                )
                ...
                tx_sudo._handle_notification_data('hyperpay', entity_data)
    ...
    def _verify_notification_signature(
        ...
        expected_signature = tx_sudo.provider_id._hyperpay_calculate_signature(
