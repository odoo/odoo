...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_processing_values(self, processing_values):
        ...
        uid = '8a8294174b7ecb28014b9699220015ca'
        order_id = self._hyperpay_create_order(customer_id, uid)['id']
        # stats = self._hyperpay_get_status(order_id)
        # print(stats)

        # namespace_uuid= uuid.uuid4()
        # uid = uuid.uuid5(namespace_uuid, customer_id).hex
        # customer_id = self._hyperpay_create_customer()['id']
    ...
    # def _hyperpay_create_customer(self):
    ...
    #     )
    #     customer_data = self.provider_id._hyperpay_make_request('customers', payload=payload)
    #     _logger.info(
    ...
    #         raise ValidationError("Hyperpay: " + _("The phone number is missing."))
    #
    #     try:
    ...
    #         )
    #     except Exception:
    #         raise ValidationError("Hyperpay: " + _("The phone number is invalid."))
    #     return phone

    def _hyperpay_create_order(self, customer_id=None, uid=None):
        ...
        :rtype: dict
        """
        payload = self._hyperpay_prepare_order_payload(customer_id=customer_id, uid=uid)
        _logger.info(
    ...
    def _hyperpay_prepare_order_payload(self, customer_id=None, uid=None):
        ...
        if self.operation in ['online_direct', 'validation']:
            payload['entityId'] = uid  # Required for only non-subsequent payments.
            if self.tokenize:
                payload['token'] = {
                    'max_amount': payment_utils.to_minor_currency_units(
                        self._hyperpay_get_mandate_max_amount(), self.currency_id
                    ),
    ...
    def _process_notification_data(self, notification_data):
        ...
        if 'id' in notification_data:  # We have the full entity data (S2S request or webhook).
            ...
            # stats = self._hyperpay_get_status(order_id)
            # print(stats)
            stats = self._hyperpay_get_status(notification_data[order_id])
            _logger.info(
        ...
        entity_status = entity_data.get('result')
        self._set_done()
        ...
        #         self._hyperpay_tokenize_from_notification_data(notification_data)
        #     self._set_done()
        #
        ...
        #     )
        #     self._set_error(
        #         "Hyperpay: " + _("Received data with invalid status: %s", entity_status)
        #     )
