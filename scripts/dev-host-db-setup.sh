...
CREATE_APP_DATABASES="${CREATE_APP_DATABASES:-1}"
...
should_create_databases() {
    case "$CREATE_APP_DATABASES" in
        1|true|TRUE|yes|YES)
            return 0
            ;;
        0|false|FALSE|no|NO)
            ...
            ;;
        *)
            echo "[dev-host-db-setup] Invalid CREATE_APP_DATABASES value: '$CREATE_APP_DATABASES'"
            exit 1
            ;;
    esac
}
...
if should_create_databases; then
    ...
    echo "[dev-host-db-setup] local PostgreSQL role is ready. Database creation skipped for Odoo database manager mode."
fi
