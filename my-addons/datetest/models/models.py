...
class FactoryData(models.Model):
    ...
    @api.model
    def get_report_init(self):
        rpt_init=dict()
        return rpt_init

    @api.model
    def get_report_param(self):
        rpt_param=dict([('startdate','2020/11/25'),('end_date','2020/11/30')])
    ...
    def sqlImportData(self):
        ...
        select = ''
        #转换成查询数据库语句需要的格式
        for orderRow, j in enumerate(getData):
            if orderRow == len(getData) - 1:
                select += j
            else:
                select += j + ','
        ...
        mainorderDict = {}
        # 主表模型数据插入
        for orderRow in rows:
            orderNo = orderRow['order_numbers']
            if orderNo not in mainOrderList.keys():
                mainorderDict = {}
                for j in mainCols:
                    mainorderDict[j] = orderRow[j]
                mainorderDict['factory_data_from_ids'] = []
                mainOrderList[orderNo] = mainorderDict
            else:
                mainorderDict = mainOrderList[orderNo]
            ciOrderList = {}
            for k in salveCols:
                ciOrderList[k] = orderRow[k]
            mainorderDict['factory_data_from_ids'].append((0, 0, ciOrderList))
        ...
        # 需要更新的订单编号
        updateOrderNumbers = set(order_numbers_list) & dicts_list
        #直接插入的订单编号
        insertOrderNumbers = set(order_numbers_list) | dicts_list
        # 开始更新，新增，删除操作
        for orderRow in mainOrderList.values():
            # 2
            if orderRow['order_numbers'] in updateOrderNumbers:
                existRows = self.env['factory.data'].search([('order_numbers', '=', orderRow['order_numbers'])])
                ...
                for factory_data_from_id in orderRow['factory_data_from_ids']:
                ...
                for factory_data_from_id in orderRow['factory_data_from_ids']:
                    nowData = factory_data_from_id[2]
                    if nowData['mxid'] in updates:
                        nowData['factory_data_id'] = id
                        nowData['mxstatus'] = '更新'
                        ...
                        existDatas={}
                        for key in nowData.keys():
                           existDatas[key] = from_update[key]

                        updDatas = nowData.items() - (nowData.items() & existDatas.items())
                        from_update.write(dict(updDatas))
                ...
                if deletes:
                    ...
                    from_deletes.write({'mxstatus': "删除"})
                        # from_delete.unlink()
                existMxDatas={}
                for key in orderRow.keys():
                    if key == 'factory_data_from_ids':continue
                    existMxDatas[key] = existRows[key]
                ak = orderRow
                del ak["factory_data_from_ids"]
                mainUpds = ak.items() - (ak.items() & existMxDatas.items())
                mainUpds = dict(mainUpds)
                mainUpds.update({'status':'更新'})
               # del mainUpds["factory_data_from_ids"]
                existRows.write(mainUpds)
            else:
                super(FactoryData, self).create(orderRow)
                print('直接插入')
