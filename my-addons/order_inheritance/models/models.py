...
class order_inheritance(models.Model):
    ...
    old_order_number=fields.Char(string="旧订单编号")
    # 客户PO号就是订单号
    ...
    def order_import_data(self,start_time = None,end_time= None,is_order_time=False):
        ...
        sql = f'select TOP 30  * from odoo_订单主表查询 where {where_filter}'.replace('\\','')
        print(sql)
        ...
        for mx_row in mx_rows:
            ...
            md5 = hashlib.md5()
            md5_str = mx_row['订单编号']+mx_row['订单号']+mx_row['装箱配码']+mx_row['工厂款号']
        ...
        for mx_fz_row in mx_fz_rows:
            md5 = hashlib.md5()
            md5_str = mx_fz_row['订单编号'] + mx_fz_row['订单号']
            md5.update(md5_str.encode("utf-8"))
        ...
        for mx_fz_row in mx_fz_rows:
            ...
            if comp_id in exist_comp_ids:
                ...
                exist_order_lines =[]
                for mx_row in mx_rows:
                    if mx_row['订单编号'] == name and mx_row['订单号'] == customer_po:
        ...
        return_data = super(order_inheritance, self).create(create_order_data_list)
...
class product_inheritance(models.Model):
    ...
    def product_import_data(self):
        ...
        # productList = []
        # for row in rows:
        #     productExist = self.env['product.template'].search([('name', '=', row['工厂款号'])])
        #     type = '成品 / ' + row['鞋码标准']
        #     existRows = self.env['product.category'].search([('complete_name', '=', type)])
        #     categ_id = existRows.id
        #     if productExist:
        #         # print('更新')
        #         update_product = {
        #             'name':row['工厂款号'],
        ...
        #             'color_en':row['配色En'],
        #             'color_cn':row['配色Cn'],
        #             'customer_number':row['客户款号'],
        #             'trademark':row['商标'],
        #             'type':'product'
        #         }
        #         productExist.write(update_product)
        #     else:
        #         product = {
        #             'name':row['工厂款号'],
        #             'categ_id':categ_id,
        ...
        # # print(productList)
        # super(product_inheritance, self).create(productList)
        ...
        for data in res:
            ...
            if existRows:
                ...
                # }
                # product_supplier_list.append(product_supplier_dict)
                rowsList.append(create_rowDict)
        if rowsList:
            # self.env['product.supplierinfo'].create(product_supplier_list)
            super(product_inheritance, self).create(rowsList)
        # 明细数量
