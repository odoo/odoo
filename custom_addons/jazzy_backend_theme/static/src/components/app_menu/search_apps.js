...
patch(NavBar.prototype, {
    ...
    async fetch_data() {
        ...
        let result;
        try {
            result = await this.orm.call("res.config.settings", "config_color_settings", [0]);
        } catch {
            return;
        }
        ...
        if (result.full_bg_img !== false) {
            ...
            const appComponentsDivs = document.getElementsByClassName("app_components");

            for (let i = 0; i < appComponentsDivs.length; i++) {
    ...
    getNavbarItems() {
        const children = this.app_components.el?.parentElement?.children || [];

        for (const child of children) {
            if (child.classList?.contains("o_navbar")) {
    ...
    toggleNavbarItems(visible) {
        for (const item of this.getNavbarItems()) {
            if (
                item.classList.contains("o_menu_brand") ||
                item.classList.contains("o_menu_sections")
            ) {
                item.classList.toggle("d-none", !visible);
            }
        }
    },
    toggleSidebarPanel(visible) {
        if (this.sidebar_panel.el) {
            this.sidebar_panel.el.style.display = visible ? "block" : "none";
        }
    },
