...
class DoctorAllocation(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if doctor_group in self.env.user.group_ids:
    ...
    def _compute_patient_limit(self):
        ...
        for record in self:
            if record.work_from is not None and record.work_from != 0 and record.work_to is not None and record.work_to != 0 and record.time_avg:
                work_from = float(record.work_from)
                work_to = float(record.work_to)
                time_avg = float(record.time_avg)
                if time_avg <= 0:
                    record.patient_limit = 0
                    continue
                duration = (work_to - work_from)*60
                if duration <= 0:
                    raise ValidationError("Work From must be less than Work To.")
                if duration > 0:
                    patient_slots = int(duration / time_avg)
                    record.patient_limit = max(patient_slots, 0)
                else:
