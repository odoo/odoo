...
class SifJurnalEntry(models.Model):
    ...
    name = fields.Char(string='Nomor Bukti', required=True, copy=False, readonly=True, default=lambda self: _('New'))
    date = fields.Date(string='Tanggal', required=True, default=fields.Date.context_today)
    ref = fields.Char(string='Referensi / Dokumen Sumber')
    kwitansi_ref = fields.Char(string='No. Kwitansi / Bukti Fisik')
    unit_name = fields.Char(string='Unit Kerja', default='KANTOR')
    ...
    line_ids = fields.One2many('sif.jurnal.line', 'entry_id', string='Baris Jurnal')
    total_debit = fields.Float(string='Total Debet', compute='_compute_totals', store=True)
    total_credit = fields.Float(string='Total Kredit', compute='_compute_totals', store=True)
    period_month = fields.Integer(string='Bulan Periode', compute='_compute_period', store=True)
    period_year = fields.Integer(string='Tahun Periode', compute='_compute_period', store=True)
    period_key = fields.Char(string='Kunci Periode', compute='_compute_period', store=True)
    ...
    def create_journal_from_ppl(self, vals):
        if hasattr(vals, '_name'):
            ...
            if exp_acc and pay_acc:
                lines = []
                if dpp > 0:
                    lines.append({'account_id': exp_acc.id, 'name': f"Biaya Pengadaan {vals['ref']}", 'debit': dpp, 'credit': 0.0})
                if ppn > 0 and ppn_acc:
                    lines.append({'account_id': ppn_acc.id, 'name': f"PPN Masukan (Aset) {vals['ref']}", 'debit': ppn, 'credit': 0.0})
                lines.append({'account_id': pay_acc.id, 'name': f"Pembayaran {vals['ref']}", 'debit': 0.0, 'credit': total})
                vals['lines'] = lines

        lines_command = []
        for l in vals.get('lines', []):
    ...
    @api.model
    def create_asset_purchase_journal(self, asset_name, asset_code, amount, asset_account_id, credit_account_id, date=False, unit_name='KANTOR', vendor_name='', kwitansi=''):
        txn_date = date or fields.Date.today()
        lines = [
            (0, 0, {'account_id': asset_account_id, 'name': f"Perolehan [{asset_code}] {asset_name}", 'debit': amount, 'credit': 0.0}),
            (0, 0, {'account_id': credit_account_id, 'name': f"Pembayaran / Kewajiban Perolehan [{asset_code}]", 'debit': 0.0, 'credit': amount})
        ]
    ...
    @api.model
    def create_asset_depreciation_journal(self, asset_name, asset_code, amount, dep_account_id, exp_account_id, date, period_name, unit_name='KANTOR'):
        desc = f"Penyusutan [{asset_code}] {asset_name} - Periode {period_name}"
        lines = [
            (0, 0, {'account_id': exp_account_id, 'name': desc, 'debit': amount, 'credit': 0.0}),
            (0, 0, {'account_id': dep_account_id, 'name': desc, 'debit': 0.0, 'credit': amount})
        ]
        entry = self.sudo().create({
...
class SifJurnalLine(models.Model):
    ...
    entry_id = fields.Many2one('sif.jurnal.entry', string='Voucher Jurnal', ondelete='cascade', required=True)
    entry_number = fields.Char(related='entry_id.name', string='Bukti / Nomer', store=True)
    date = fields.Date(related='entry_id.date', string='Tanggal', store=True, index=True)
    unit_name = fields.Char(related='entry_id.unit_name', string='Unit Kerja', store=True)
    ...
    account_name = fields.Char(related='account_id.name', string='Nama Account', store=True)
    ...
    period_key = fields.Char(related='entry_id.period_key', string='Periode', store=True)
...
class SifBukuBesar(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('posted', 'Posted'), ('cancel', 'Dibatalkan')], string='Status', readonly=True)
...
class SifBukuBesarWizard(models.TransientModel):
    ...
    date_from = fields.Date(string='Tanggal Awal', required=False)
    date_to = fields.Date(string='Tanggal Akhir', required=False)
    account_id = fields.Many2one('sif.coa', string='Akun (Opsional)', required=False)
    unit_name = fields.Char(string='Unit Kerja (Opsional)', required=False)
