...
class ColorAssetsEditor(models.AbstractModel):
    """Read, write, and reset customized SCSS color variable assets."""
    ...
    def _get_custom_colors_url(self, url: str, bundle: str) -> str:
        """Build the custom attachment URL for a bundled color asset."""
        return f'/_custom/{bundle}{url}'

    @api.model
    def _get_color_info_from_url(self, url: str) -> dict | bool:
        """Parse a custom color URL into its module, path, and bundle parts.

        :return: the parsed URL components, or ``False`` when it does not match
        """
        regex = re.compile(r'^(/_custom/([^/]+))?/(\w+)/([/\w]+\.\w+)$')
    ...
    def _get_colors_asset(self, custom_url: str) -> models.BaseModel:
        """Return the ``ir.asset`` record overriding the color asset."""
        return self.env['ir.asset'].search([('path', 'like', custom_url)])

    @api.model
    def _get_colors_from_url(self, url: str, bundle: str) -> bytes:
        """Return the raw color asset content, preferring a customized copy."""
    ...
    def _get_color_variable(self, content: str, variable: str) -> str | bool:
        """Extract the value of a single ``$mk_`` color variable from content."""
        value = re.search(rf'\$mk_{variable}\:?\s(.*?);', content)
        return value and value.group(1)

    def _get_color_variables(self, content: str, variables: list[str]) -> dict:
        """Extract the values of several color variables from content."""
        return {var: self._get_color_variable(content, var) for var in variables}

    def _replace_color_variables(self, content: str, variables: list[dict]) -> str:
        """Return content with each given color variable set to its new value."""
    ...
    @api.model
    def _save_color_asset(self, url: str, bundle: str, content: str) -> None:
    ...
    def reset_color_asset(self, url: str, bundle: str) -> None:
        """Remove the customized attachment and asset for a color bundle."""
        custom_url = self._get_custom_colors_url(url, bundle)
