...
class TestHrPayrollAccount(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.payslip_action_id = self.ref(
            'hr_payroll_community.menu_department_tree')
        ...
        self.hr_contract_john = self.env['hr.contract'].create({
            ...
            'journal_id': self.ref(
                'hr_payroll_account_community.expenses_journal'),
        })
    ...
    def test_00_hr_payslip(self):
        ...
        date_to = str(datetime.now() + relativedelta.relativedelta(
            months=+1, day=1, days=-1))[:10]
        res = self.hr_payslip.onchange_employee_id(
            date_from, date_to, self.hr_employee_john.id)
        vals = {'struct_id': res['value']['struct_id'],
                'contract_id': res['value']['contract_id'],
                'name': res['value']['name'],
                'worked_days_line_ids': [(0, 0, i) for i in
                                         res['value']['worked_days_line_ids']],
                'input_line_ids': [(0, 0, i) for i in
                                   res['value']['input_line_ids']]}
        ...
        self.assertEqual(
            self.hr_payslip.state, 'draft', 'State not changed!')

        # I click on "Compute Sheet" button.
        context = {"lang": "en_US", "tz": False, "active_model": 'hr.payslip',
                   "department_id": False,
                   "active_ids": [self.payslip_action_id],
                   "section_id": False, "active_id": self.payslip_action_id}
        self.hr_payslip.with_context(context).compute_sheet()

        # I want to check cancel button. So I first cancel the sheet then make
        # it set to draft.
        ...
        self.assertTrue(self.hr_payslip.move_id, 'Accounting Entries has'
                                                 ' not been created')
