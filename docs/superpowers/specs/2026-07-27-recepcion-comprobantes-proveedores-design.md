...
**Corrección posterior (validación + UX del motivo):** dos ajustes agregados tras pruebas manuales:
1. **"Aceptar parcial" sin cambios reales queda bloqueado.** Si el usuario no edita ninguna línea/cantidad/precio respecto al XML original, se rechaza con un `UserError` sugiriendo usar "Aceptar total" — evita dejar un Mensaje Receptor "parcial" en Hacienda que en realidad no ajustó nada. La validación (`_l10n_cr_fe_mr_parcial_sin_cambios()`) compara **el subtotal sin impuesto** (`amount_untaxed` de Odoo vs. `l10n_cr_fe_proveedor_subtotal`, capturado del `TotalVentaNeta` del XML original) — **no** los montos con impuesto (`amount_tax`/`amount_total`). Se probó con el primer diseño comparando montos con impuesto y falló: si el impuesto de compra no tiene una tarifa configurada que haga match automático (mismo problema que motivó el fix de "montos auténticos" de la sección 2.5), `amount_tax`/`amount_total` de Odoo quedan distintos del XML original **aunque el usuario no haya tocado nada**, dejando pasar aceptaciones "parciales" sin cambios reales (falso negativo). El subtotal sin impuesto no depende del emparejamiento de impuestos, así que es la señal correcta de si el usuario editó cantidades/precios/líneas. Se aplica tanto en el botón (`action_l10n_cr_fe_abrir_aceptar_parcial`) como dentro de `_l10n_cr_fe_generate_and_send` (para que no se pueda saltar poniendo la decisión a mano y llamando `action_post`, mismo patrón ya usado para el motivo obligatorio). Si no hay `l10n_cr_fe_proveedor_subtotal` capturado (facturas antiguas o creadas por otra vía), no se bloquea — no hay base de comparación.
2. **Popup de motivo en vez de tener que ir a la pestaña "Factura Electrónica CR".** Los botones "Aceptar parcial" y "Rechazar" ahora abren un wizard (`l10n_cr.fe.mr.motivo.wizard`, `target='new'`) que pide el motivo en una ventana emergente, en vez de exigir que el usuario navegue hasta esa pestaña a llenar el campo antes de hacer clic. Al confirmar, el wizard escribe el motivo en la factura y llama al método existente (`action_l10n_cr_fe_aceptar_parcial`/`action_l10n_cr_fe_rechazar`), reutilizando toda la validación y el flujo de envío ya construidos — no se duplica lógica.
