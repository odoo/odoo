...
1. **"Aceptar parcial" sin cambios reales queda bloqueado.** Si el usuario no edita ninguna línea/monto respecto al XML original (`amount_tax`/`amount_total` idénticos a `l10n_cr_fe_proveedor_monto_impuesto`/`l10n_cr_fe_proveedor_total`), se rechaza con un `UserError` sugiriendo usar "Aceptar total" — evita dejar un Mensaje Receptor "parcial" en Hacienda que en realidad no ajustó nada. La validación vive en `_l10n_cr_fe_mr_parcial_sin_cambios()`, y se aplica tanto en el botón (`action_l10n_cr_fe_abrir_aceptar_parcial`) como dentro de `_l10n_cr_fe_generate_and_send` (para que no se pueda saltar poniendo la decisión a mano y llamando `action_post`, mismo patrón ya usado para el motivo obligatorio). Si no hay `l10n_cr_fe_proveedor_total` capturado (facturas antiguas o creadas por otra vía), no se bloquea — no hay base de comparación.
