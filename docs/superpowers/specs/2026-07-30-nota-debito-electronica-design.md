...
**Fuera de alcance de este wizard:** no se restringe el botón nativo "Debit Note" a que solo aparezca sobre facturas `out_invoice` aceptadas — se deja visible tal cual lo trae `account_debit_note` (cualquier factura posteada). Si el usuario lo usa sobre un origen no soportado (factura de proveedor, nota de crédito), el resultado es una ND puramente contable de Odoo, sin envío a Hacienda — comportamiento correcto por diseño, no un caso de error.
