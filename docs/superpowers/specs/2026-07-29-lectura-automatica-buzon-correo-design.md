...
El endurecimiento de `ValueError` a `UserError` (y, en una ronda de fix posterior, también envolver el `account.move.create()`) no es solo higiene de código: `fetchmail.server` marca el correo entrante como `\Seen` en el servidor IMAP incluso cuando la transacción se revierte por una excepción no capturada. Si el parseo o la creación del `account.move` lanzan una excepción sin manejar, esa transacción completa se revierte -- incluyendo el propio registro de auditoría `l10n_cr.fe.proveedor.email` -- pero el correo ya quedó marcado como leído, así que se pierde para siempre sin dejar rastro. Por eso ambos pasos deben terminar en `state='sin_xml_valido'` con el motivo real, nunca dejar escapar la excepción.
