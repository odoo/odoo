...
class ShopeeShop(models.Model):
    ...
    shopee_code = fields.Char(string='Code')
    shopee_access_token = fields.Char(string='Access Token')
    shopee_refresh_token = fields.Char(string='Refresh Token')
    ...
    #     res = super(ShopeeShop, self).create(vals)
    #     return res

    # def write(self, vals):
    #     res = super(ShopeeShop, self).write(vals)
    #     for shop in self:
    #         if len(shop.boost_product_ids) > 5:
    #             raise UserError(_('You can only have up to 5 boost products per shop!'))
    #     return res

    def shopee_generate_new_token(self):
        ...
        for shop in self:
            ...
            if 'access_token' in values:
                shop.write({'shopee_access_token': values['access_token']})
            if 'refresh_token' in values:
                shop.write({'shopee_refresh_token': values['refresh_token']})

    def shopee_generate_refresh_token(self):
        company = self.env.user.company_id
        ...
        for shop in self:
            ...
            refresh_token = shop.shopee_refresh_token
