...
class ResPartner(models.Model):
    ...
    def _get_eas_mapping(self, eas, include_given_eas=True):
        ...
        mappings = EAS_MAPPING.get(self.country_code)
    ...
    def write(self, vals):
        ...
        if ((vals.get('peppol_eas') or vals.get('peppol_endpoint'))
    ...
    def button_account_peppol_check_partner_endpoint(self, company=None):
        ...
        if new_value == 'not_valid':
            eas_to_try = self._get_eas_mapping(eas=self.peppol_eas, include_given_eas=False)
            if eas_to_try:
                # Cannot store verification state directly in partner because of the compute method, that depends on eas and endpoint
