...
class AccountInvoiceSend(models.TransientModel):
        ...
        for wizard in self:
            wizard.peppol_invoice_ids = wizard.invoice_ids.filtered(
                lambda invoice: (
                    ...
                    and invoice._get_peppol_document()
    ...
    def _compute_checkbox_send_peppol(self):
        for wizard in self:
            wizard.checkbox_send_peppol = wizard.peppol_invoice_ids

    @api.depends('peppol_invoice_ids')
    def _compute_checkbox_send_peppol_readonly(self):
        for wizard in self:
            wizard.checkbox_send_peppol_readonly = not wizard.peppol_invoice_ids

    @api.depends('invoice_ids')
    def _compute_enable_peppol(self):
        for wizard in self:
            wizard.enable_peppol = bool(wizard.invoice_ids)

    @api.depends('invoice_ids', 'peppol_invoice_ids')
