...
class TestPeppolMessage(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.main_company = cls.company_data['company']
    ...
    def create_move(self, company, partner, account_revenue=False, account_expense=False):
        return self.env['account.move'].create({
            ...
            'invoice_line_ids': [
                (0, 0, {
                    ...
                    'debit': 100.0,
                    'credit': 0.0,
                    'account_id': account_revenue or self.company_data['default_account_revenue'].id,
                ...
                (0, 0, {
                    ...
                    'debit': 0.0,
                    'credit': 100.0,
                    'account_id': account_expense or self.company_data['default_account_expense'].id,
                    'tax_ids': company.account_sale_tax_id,
                }),
            ],
        })

    def _get_form(self, move):
        context = move.action_send_and_print()['context']
        send_and_print_form = Form(
            self.env['account.move.send'].with_context(context))
        send_and_print_form.checkbox_ubl_cii_xml = True
        send_and_print_form.checkbox_send_peppol = True

        return send_and_print_form
    ...
    def test_attachment_placeholders(self):
        self.env.company = self.main_company
        move = self.create_move(self.main_company, self.valid_partner)
        move.action_post()

        builder = self.valid_partner._get_edi_builder()
        filename = builder._export_invoice_filename(move)
        send_and_print_form = self._get_form(move)

        send_and_print_form.checkbox_ubl_cii_xml = False
        # sending with peppol should not be possible if checkbox_ubl_cii_xml is unchecked
        # because sending via peppol requires a proper ubl xml creation
        self.assertTrue(send_and_print_form._get_modifier(
            'checkbox_send_peppol', 'readonly'))

        send_and_print_form.checkbox_ubl_cii_xml = True
        send_and_print_form.checkbox_send_peppol = False

        # the ubl xml placeholder should be generated
        self.assertTrue(bool(
            [file for file in send_and_print_form.mail_attachments_widget if file['name'] == filename]
        ))
    ...
    def test_send_peppol_invalid_partner(self):
        ...
        move = self.create_move(self.main_company, self.invalid_partner)
        move.action_post()

        send_and_print_form = self._get_form(move)

        self.assertTrue(bool(send_and_print_form.peppol_warning))
    ...
    def test_resend_error_peppol_message(self):
        ...
        with self._patch_peppol_requests(error=True):
            send_wizard = send_and_print_form.save()
            send_wizard.action_send_and_print()

            self.env.ref(
                'account_peppol.ir_cron_peppol_get_message_status')._trigger()
    ...
    def test_send_invalid_edi_user(self):
        ...
        self.company_data_2['company'].write({
            'is_account_peppol_participant': True,
            'account_peppol_proxy_state': 'rejected',
            'peppol_eas': '0208',
            'peppol_endpoint': '0202239951',
        })

        move = self.create_move(
            self.company_data_2['company'],
            self.valid_partner,
            self.company_data_2['default_account_revenue'].id,
            self.company_data_2['default_account_expense'].id
        )
        move.action_post()
        ...
        self.assertTrue(send_and_print_form._get_modifier(
            'checkbox_send_peppol', 'invisible'))

    def test_receive_error_peppol(self):
        # an error peppol message should be created
        with self._patch_peppol_requests(error=True):
            self.env.ref(
                'account_peppol.ir_cron_peppol_get_new_documents').with_company(self.main_company)._trigger()
