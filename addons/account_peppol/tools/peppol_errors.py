...
ERROR_MESSAGES = MappingProxyType({
    'PEPPOL-EN16931-P0105': lambda move: Markup('%s') % move.env._(
    ...
    'BR-O-02': _lt(
        "If you are not subject to VAT, you should not have a VAT number on your company/contacts and your clients should not have one either."
    ),
    'BR-O-05': _lt(
        "Invoice lines using Tax Category 'O' cannot use a percentage-based tax. Please update the tax type in Advanced Options."
    ),
    'BR-CO-04': _lt(
...
def _humanize_error_line(move, raw_line):
    for error_code, error_message in ERROR_MESSAGES.items():
        if f'[{error_code}]' in raw_line:
            return error_message(move) if callable(error_message) else move.env._("%s", error_message)
    return raw_line


def render_peppol_errors(env, move, raw_message):
    raw_message = str(raw_message)

    if move:
        error_lines = [line for line in raw_message.split('\n') if line.startswith('[')]
        if error_lines:
            items = Markup().join(
                Markup('<li>%s</li>') % _humanize_error_line(move, line)
                for line in error_lines
            )
            return Markup('<ul class="mb-0">%s</ul>') % items

    return Markup('<ul class="mb-0"><li>%s</li></ul>') % raw_message
