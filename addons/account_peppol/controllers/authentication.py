...
class PeppolAuthentication(http.Controller):
    ...
    def peppol_authentication_callback(self, auth_type=None, connect_token=None, auth_token=None, state=None):
        ...
        if not conn_data:
            ...
            _logger.info("Peppol callback decision %s (auth_type=%s)", state, auth_type)
        elif not auth_token:
            _logger.warning("Peppol registration auth_token missing (auth_type=%s)", auth_type)
        elif conn_data['company'].sudo().account_peppol_edi_user:
            _logger.info("Peppol connection already created by the webhook (auth_type=%s)", auth_type)
        else:
            try:
                conn_data['company'].sudo()._peppol_create_connection(conn_data['peppol_identifier'], auth_token=auth_token)
            except Exception as e:  # noqa: BLE001
                _logger.warning("Failed to create Peppol connection: %s", e)
    ...
    def peppol_authentication_webhook(self, auth_type=None, connect_token=None, auth_token=None):
