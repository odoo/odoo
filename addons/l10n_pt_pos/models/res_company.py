    ...
    def _verify_hashed_pos_order(self, order, previous_hash, versioning_list, current_versioning_index):
        ...
        try:
            message = pt_hash_utils.get_message_to_hash(
                order.date_order, order.l10n_pt_hashed_on, order.l10n_pt_document_number, order.amount_total, previous_hash,
            )
            return pt_hash_utils.verify_integrity(
                message, order.l10n_pt_pos_inalterable_hash, versioning_list[current_versioning_index],
            )
        except AccessError as e:
            raise UserError(_("This company has AT Series shared across branches, and other companies also have hashed documents under this series. "
