    ...
    def _check_l10n_pt_pos_at_series_id(self):
        for config in self.filtered(lambda c: c.company_id.country_id.code == 'PT'):
            if (
                ...
                raise RedirectWarning(
                    _("There is no AT series for this POS Configuration registered under the series name %(series_name)s. Create a new series or view existing series via the Accounting Settings.",
