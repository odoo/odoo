    ...
    def write(self, vals):
        if 'name' in vals or 'training_series' in vals:
            for at_series in self:
                pos_at_series_line = at_series.at_series_line_ids.filtered(lambda s: s.type == 'pos_order')
                if self.env['pos.order'].search_count([
                    ('l10n_pt_document_number', '=like', f'{pos_at_series_line.document_identifier}/%'),
                    ('state', "!=", 'draft'),
                ], limit=1):
