...
class PosOrder(models.Model):
    ...
    l10n_pt_inalterable_hash_short = fields.Char(string="Short version of the Portuguese hash", compute='_compute_l10n_pt_pos_inalterable_hash_info')
    ...
    def _compute_l10n_pt_at_series_id(self):
        for order in self:
            if order.country_code == 'PT' and order.config_id.l10n_pt_pos_at_series_id:
                order.l10n_pt_at_series_id = order.config_id.l10n_pt_pos_at_series_id
            else:
                order.l10n_pt_at_series_id = False
    ...
    @api.model
    def _find_last_order(self, company_id, at_series):
    ...
    def l10n_pt_pos_compute_missing_hashes(self, company_id, pos_config_id=None, at_series=None):
        """
        ...
        if len(self) == 1 and self.l10n_pt_pos_inalterable_hash:
            ...
            if pos_config_id:
                at_series = self.env['l10n_pt.at.series.line'].search([
            ...
            previous_order = self._find_last_order(company_id, at_series)
            try:
        ...
        return {
            ...
            "hash_short": last_order.l10n_pt_inalterable_hash_short,
            "atcud": last_order.l10n_pt_pos_atcud,
            "document_identifier": last_order.l10n_pt_document_number,
    ...
    def _compute_l10n_pt_pos_qr_code_str(self):
        ...
        for order in self.filtered(lambda o: (
            ...
            qr_code_dict['D:'] = "FR*"
            qr_code_dict['G:'] = f"{order.l10n_pt_document_number}*"
