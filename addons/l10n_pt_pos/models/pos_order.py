...
class PosOrder(models.Model):
    ...
    def post_reprint_reason(self, reason):
        self.ensure_one()
        msg = Markup(_("Reason for reprinting document %(name)s:<br/>%(reason)s")) % {'name': self.name, 'reason': self.reason}
    ...
    def l10n_pt_pos_compute_missing_hashes(self, company_id, pos_config_id=None, at_series_line=None):
        """
        ...
        previous_order = self._find_last_order(company_id, at_series_line)
        try:
