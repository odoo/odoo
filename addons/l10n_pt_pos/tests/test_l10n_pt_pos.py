...
class TestL10nPtPos(TestPoSCommon):
    ...
    def test_l10n_pt_pos_hash_inalterability(self):
        ...
        with self.assertRaisesRegex(UserError, f"{expected_error_msg}Inalterability Hash"):
            order.l10n_pt_pos_inalterable_hash = '$1$fake_hash'
    ...
    def test_l10n_pt_pos_hash_integrity_report(self):
        ...
        integrity_check = next(filter(lambda r: r['name'] == order1.config_id.name, self.company_pt._l10n_pt_pos_check_hash_integrity()['results']))
    ...
    def test_l10n_pt_pos_dont_hash_linked_stock_pickings(self):
        ...
        self.assertNotEqual(order1.l10n_pt_pos_inalterable_hash, False)
        self.assertEqual(order1.picking_ids.l10n_pt_stock_inalterable_hash, False)
