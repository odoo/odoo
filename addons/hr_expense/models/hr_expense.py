...
class HrExpense(models.Model):
    ...
    def _compute_attachment_number(self):
        attachment_data = self.env['ir.attachment']._aggregate([('res_model', '=', 'hr.expense'), ('res_id', 'in', self.ids)], ['*:count'], ['res_id'])
        for expense in self:
            expense.attachment_number = attachment_data.get_agg(expense._origin.id, '*:count', 0)
    ...
    def get_expense_dashboard(self):
        ...
        expenses = self._aggregate(
            [
                ('employee_id', 'in', self.env.user.employee_ids.ids),
                ('payment_mode', '=', 'own_account'),
                ('state', 'in', ['draft', 'reported', 'approved'])
            ], ['total_amount:sum'], ['state', 'currency_id'])
        for [state, currency], [total_amount_sum] in expenses.items(as_records=True):
            currency = currency or target_currency
            amount = currency._convert(
                total_amount_sum or 0.0, target_currency, self.env.company, fields.Date.today())
...
class HrExpenseSheet(models.Model):
    ...
    def _compute_expense_number(self):
        result = self.env['hr.expense']._aggregate([('sheet_id', 'in', self.ids)], ['*:count'], ['sheet_id'])
        for sheet in self:
            sheet.expense_number = result.get_agg(sheet, '*:count', 0)
