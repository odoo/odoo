...
class HrExpenseSheet(models.Model):
    ...
    def action_sheet_move_create(self):
        ...
        if any(not sheet.journal_id for sheet in self if sheet.payment_model == 'own_account'):
            raise UserError(_("Specify expense journal to generate accounting entries."))

        if any(not sheet.bank_journal_id for sheet in self if sheet.payment_model == 'company_account'):
            raise UserError(_("Specify bank journal to generate accounting entries."))
