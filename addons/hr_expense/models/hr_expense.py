...
class HrExpense(models.Model):
    ...
    def write(self, vals):
        ...
        if enforced_non_zero_expenses:
            self.check_amount_not_zero(vals, enforced_non_zero_expenses)
    ...
    def create(self, vals_list):
        ...
        for expense, vals in zip(expenses, vals_list):
            if self.env.context.get('check_total_amount_not_zero'):
                self.check_amount_not_zero(vals, expense)
        return expenses
    ...
    def check_amount_not_zero(self, vals, expenses):
        error_msgs = []
        if 'total_amount' in vals:
            if any(expense.company_currency_id.is_zero(vals['total_amount']) for expense in expenses):
                error_msgs.append(_("You cannot set the expense total to 0 if it's linked to a report."))
        if 'total_amount_currency' in vals:
            if any(expense.currency_id.is_zero(vals['total_amount_currency']) for expense in expenses):
