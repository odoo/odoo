...
class HrEmployee(models.Model):
    ...
    def _compute_expense_manager(self):
        ...
        employee_id_to_valid_manager_cache = {}  # Because on large recordset we will probably search the same parent path several times
        def get_closest_valid_manager(employee):
            """ Search in the given employee parent_id chain, and store it in cache to avoid """
            manager_in_cache = employee_id_to_valid_manager_cache.get(employee.id)
            if manager_in_cache:
                return manager_in_cache
            valid_manager = self.env['res.users']

            employees_to_cache = {employee.id}
            next_parent = employee_to_parent_map.get(employee, self.env['hr.employee'])
            while next_parent:
                ...
                if next_parent.user_id and next_parent.user_id.has_group('hr_expense.group_hr_expense_team_approver'):
                    ...
                    employee_id_to_valid_manager_cache.update({employee_id: valid_manager for employee_id in employees_to_cache})
                    break

                # If the current parent is already in the cache
                valid_manager = employee_id_to_valid_manager_cache.get(next_parent.id, self.env['res.users'])
                if valid_manager:
                    employee_id_to_valid_manager_cache.update({employee_id: valid_manager for employee_id in employees_to_cache})
                    break

                # Add the current parent to the caching queue and continue with their parent
                employees_to_cache.add(next_parent.id)
