...
class HrEmployeeBase(models.AbstractModel):
    ...
    def _search_filter_for_expense(self, operator, value):
        ...
        if user.has_groups('hr_expense.group_hr_expense_user') or user.has_groups('account.group_account_user'):
            domain = Domain(['|', ('company_id', '=', False), ('company_id', 'child_of', self.env.company.root_id.id)])  # Then, domain accepts everything
            ...
            domain = Domain([('id', '=', employee.id), '|', ('company_id', '=', False), ('company_id', '=', employee.company_id.id)])
        return domain
