...
class TestExpenses(TestExpenseCommon):
    ...
    def test_expense_main_flow(self):
        ...
        self.assertRecordValues(all_expenses, [
            {'name': 'Employee PA 2*800 + 15%', 'total_amount_currency': 1600.00, 'untaxed_amount_currency': 1391.30, 'price_unit':  800.00, 'tax_amount_currency': 208.70, 'state': 'draft', 'accounting_date': date(2021, 10, 10)},
            {'name': 'Employee PB 160 + 2*15%', 'total_amount_currency':  160.00, 'untaxed_amount_currency':  123.08, 'price_unit':  160.00, 'tax_amount_currency':  36.92, 'state': 'draft', 'accounting_date': False},
            {'name': 'Company PC 1000 + 15%',   'total_amount_currency': 1000.00, 'untaxed_amount_currency':  869.57, 'price_unit': 1000.00, 'tax_amount_currency': 130.43, 'state': 'draft', 'accounting_date': False},
            {'name': 'Company PB 160 + 2*15%',  'total_amount_currency':  160.00, 'untaxed_amount_currency':  123.08, 'price_unit':  160.00, 'tax_amount_currency':  36.92, 'state': 'draft', 'accounting_date': False},
        ])
        ...
        self.assertRecordValues(all_expenses, [
            {'state': 'approved', 'accounting_date': date(2021, 10, 10)},
            {'state': 'approved', 'accounting_date': eom_dates_1_2},
            # Company expenses don't have an accounting date
            {'state': 'approved', 'accounting_date': False},
            {'state': 'approved', 'accounting_date': False},
        ])
        employee_partner_id = self.expense_user_employee.partner_id.id
        self.assertRecordValues(all_expenses.account_move_id.sorted(lambda move: (move.expense_ids, move)), [
            {'amount_total':  1600.00, 'ref': 'Employee PA 2*800 + 15%', 'date': eom_dates_1_2,      'invoice_date': date(2021, 10, 10), 'partner_id': employee_partner_id, 'state': 'draft'},
            {'amount_total':   160.00, 'ref': 'Employee PB 160 + 2*15%', 'date': eom_dates_1_2,      'invoice_date': eom_dates_1_2,      'partner_id': employee_partner_id, 'state': 'draft'},
            {'amount_total':  1000.00, 'ref': 'Company PC 1000 + 15%',   'date': date(2021, 10, 12), 'invoice_date': False,              'partner_id': False,               'state': 'draft'},
            {'amount_total':   160.00, 'ref': 'Company PB 160 + 2*15%',  'date': date(2021, 10, 11), 'invoice_date': False,              'partner_id': False,               'state': 'draft'},
        ])
        ...
        self.assertRecordValues(all_expenses, [
            ...
            {'payment_mode': 'own_account',     'state': 'posted', 'accounting_date': date(2021, 10, 10)},
            {'payment_mode': 'own_account',     'state': 'posted', 'accounting_date': eom_dates_1_2},
            # Expenses paid by company don't use accounting date since they are already paid and posted directly
            {'payment_mode': 'company_account', 'state': 'paid',   'accounting_date': False},
            {'payment_mode': 'company_account', 'state': 'paid',   'accounting_date': False},
        ])

        self.assertRecordValues(expenses_by_employee.account_move_id, [
            {'amount_total': 1600.00, 'ref': 'Employee PA 2*800 + 15%', 'state': 'posted'},
            {'amount_total':  160.00, 'ref': 'Employee PB 160 + 2*15%', 'state': 'posted'},
        ])

        self.assertRecordValues(expenses_by_company.account_move_id, [
            {'amount_total':  1000.00, 'ref': 'Company PC 1000 + 15%',    'state': 'posted'},
            {'amount_total':   160.00, 'ref': 'Company PB 160 + 2*15%', 'state': 'posted'},
        ])
