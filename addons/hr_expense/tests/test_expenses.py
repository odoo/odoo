...
class TestExpenses(TestExpenseCommon):
    ...
    def test_expense_main_flow(self):
        ...
        self.assertRecordValues(all_expenses, [
            {'total_amount_currency': 1600.00, 'untaxed_amount_currency': 1391.30, 'price_unit':  800.00, 'tax_amount_currency': 208.70, 'state': 'reported', 'accounting_date': date(2021, 10, 11)},
            {'total_amount_currency':  160.00, 'untaxed_amount_currency':  123.08, 'price_unit':  160.00, 'tax_amount_currency':  36.92, 'state': 'reported', 'accounting_date': date(2021, 10, 13)},
            {'total_amount_currency': 1000.00, 'untaxed_amount_currency':  869.57, 'price_unit': 1000.00, 'tax_amount_currency': 130.43, 'state': 'reported', 'accounting_date': False},
            {'total_amount_currency':  160.00, 'untaxed_amount_currency':  123.08, 'price_unit':  160.00, 'tax_amount_currency':  36.92, 'state': 'reported', 'accounting_date': False},
        ])

        # Submitting properly change states
        all_expenses.action_submit_to_manager()
        self.assertRecordValues(all_expenses, [
            {'state': 'submitted'},
            {'state': 'submitted'},
            {'state': 'submitted'},
        ...
        self.assertRecordValues(all_expenses.account_move_id.sorted(lambda move: (move.expense_id, move)), [
            {'amount_total':  1600.00, 'ref': 'Employee PA 2*800 + 15%', 'date': date(2021, 10, 31), 'partner_id': employee_partner_id, 'state': 'draft'},
            {'amount_total':   160.00, 'ref': 'Employee PB 160 + 2*15%', 'date': date(2021, 10, 31), 'partner_id': employee_partner_id, 'state': 'draft'},
            {'amount_total':   160.00, 'ref': 'Company PB 160 + 2*15%',  'date': date(2021, 10, 12), 'partner_id': False,               'state': 'draft'},
            {'amount_total':  1000.00, 'ref': 'Company 1000 + 15%',      'date': date(2021, 10, 11), 'partner_id': False,               'state': 'draft'},
        ])

        # Post a payment for 'company_account' (and its move(s)) and a vendor bill for 'own_account'
        all_expenses.action_post()
        self.assertRecordValues(all_expenses, [
            # As the payment is not done yet those are still in "posted"
            {'payment_mode': 'own_account',     'state': 'posted', 'payment_state': 'not_paid',    'accounting_date': date(2021, 10, 11)},
            {'payment_mode': 'own_account',     'state': 'posted', 'payment_state': 'not_paid',    'accounting_date': date(2021, 10, 13)},
            # Expensse paid by company don't use accounting date since they are already paid and posted directly
            {'payment_mode': 'company_account', 'state': 'done',     'payment_state': 'paid',        'accounting_date': False},
            {'payment_mode': 'company_account', 'state': 'done',     'payment_state': 'paid',        'accounting_date': False},
        ])
        ...
        self.assertRecordValues(all_expenses.account_move_id.line_ids.sorted(lambda line: (line.expense_id, line)), [
            # own_account expense 1 move
            {'balance':  1391.30, 'account_id': self.expense_account.id,    'name': 'expense_employee: Employee PA 2*800 + 15%', 'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 11)},
            {'balance':   208.70, 'account_id': tax_account_id,             'name': '15%',                                       'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 11)},
            {'balance': -1600.00, 'account_id': default_account_payable_id, 'name': '',                                          'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 11)},

            # own_account expense 2 move
            {'balance':   123.08, 'account_id': product_b_account_id,       'name': 'expense_employee: Employee PB 160 + 2*15%', 'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 13)},
            {'balance':    18.46, 'account_id': tax_account_id,             'name': '15%',                                       'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 13)},
            {'balance':    18.46, 'account_id': tax_account_id,             'name': '15% (Copy)',                                'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 13)},
            {'balance':  -160.00, 'account_id': default_account_payable_id, 'name': '',                                          'date': date(2021, 10, 31), 'invoice_date': date(2021, 10, 13)},

            # company_account expense 2 move
            {'balance':  123.08, 'account_id': product_b_account_id,        'name': 'expense_employee: Company PB 160 + 2*15%',  'date': date(2021, 10, 12), 'invoice_date': False},
            ...
            {'balance': -160.00, 'account_id': company_payment_account_id,  'name': 'expense_employee: Company PB 160 + 2*15%',  'date': date(2021, 10, 12), 'invoice_date': False},

            # company_account expense 1 move
            {'balance':   869.57, 'account_id': product_c_account_id,       'name': 'expense_employee: Company PC 1000 + 15%',   'date': date(2021, 10, 11), 'invoice_date': False},
            {'balance':   130.43, 'account_id': tax_account_id,             'name': '15%',                                       'date': date(2021, 10, 11), 'invoice_date': False},
            {'balance': -1000.00, 'account_id': company_payment_account_id, 'name': 'expense_employee: Company PC 1000 + 15%',   'date': date(2021, 10, 11), 'invoice_date': False},
        ])

        # Own_account partial payment
        payment_1 = self.get_new_payment(expenses_by_employee, 1700.0)
        liquidity_lines1 = payment_1._seek_for_lines()[0]
        self.assertRecordValues(expenses_by_employee, [{'payment_state': 'partial', 'state': 'done'}] * 2)

        # own_account remaining payment
        payment_2 = self.get_new_payment(expenses_by_employee, 60.0)
        ...
        self.assertRecordValues(expenses_by_employee, [{'payment_state': in_payment_state, 'state': 'done'}] * 2)
        ...
        self.assertRecordValues(expenses_by_employee, [{'payment_state': 'paid', 'state': 'done'}] * 2)
        ...
        self.assertRecordValues(expenses_by_employee, [{'payment_state': 'not_paid', 'state': 'post'}] * 2)
        ...
        expenses_by_company.account_move_id.origin_payment_id.unlink()
        self.assertFalse(expenses_by_company.account_move_id)

        self.assertRecordValues(all_expenses.sorted('id'), [
            {'payment_mode': 'own_account',     'state': 'approved', 'payment_state': 'not_paid', 'account_move_id': []},
            {'payment_mode': 'own_account',     'state': 'approved', 'payment_state': 'not_paid', 'account_move_id': []},
            {'payment_mode': 'company_account', 'state': 'approved', 'payment_state': 'not_paid', 'account_move_id': []},
            {'payment_mode': 'company_account', 'state': 'approved', 'payment_state': 'not_paid', 'account_move_id': []},
        ])

        expenses_by_employee.action_reset()
        self.assertRecordValues(expenses_by_employee, [{'state': 'draft', 'payment_state': 'not_paid', 'account_move_id': []}] * 2)
        expenses_by_employee.unlink()
    ...
    def test_expense_split_flow(self):
        ...
        expense = self.create_expenses({'analytic_distribution': {self.analytic_account_1.id: 100}})
    ...
    def test_expense_multi_currencies(self):
        ...
        foreign_expense_1.total_amount_currency = 100.00
        self.assertRecordValues(foreign_expense_1, [
            {'total_amount': 50.00, 'total_amount_currency': 100.00, 'currency_rate': 0.5},  # Rate revert back to the default one
        ])
    ...
    def test_expense_company_dates(self):
        ...
        expenses.action_move_post()

        move_twelve_january, move_first_january = expenses.account_move_id
        ...
        self.assertRecordValues(expenses, [
            {'total_amount': 350.00, 'payment_state': 'paid'},
            {'total_amount':  90.00, 'payment_state': 'paid'},
        ])

    @freeze_time('2022-01-25')
    def test_computation_expense_date_user_input(self):
        """
        ...
        self.assertEqual(expense.accounting_date, fields.Date.from_string('2024-03-10'))

    @freeze_time('2022-01-25')
    def test_computation_expense_date_this_month(self):
        """
        ...
        expense.action_approve()
        expense.action_post()
        self.assertEqual(expense.accounting_date, fields.Date.from_string('2022-01-25'))

    @freeze_time('2022-02-25')
    def test_computation_expense_date_previous_month(self):
        """
            Test the accounting date if the most recent expense is before this month and there is no lock date
            The accounting date should then be the last day of the most recent expense month
        """
        expense = self.create_expenses({
            'name': 'Expense for John Smith',
            'employee_id': self.expense_employee.id,
        ...
        self.assertEqual(expense.accounting_date, fields.Date.from_string('2021-12-31'))
        ...
        expense_2.action_post()

        self.assertEqual(expense_2.accounting_date, fields.Date.from_string('2022-01-31'))

    @freeze_time('2022-01-25')
    def test_computation_expense_date_with_lock_date(self):
        """
           Test the accounting date if there is a lock date
           The accounting date should then be the min(max(the day of the expense AND last day of month after lock date) AND today)
       """
        ...
        # today, because is the earliest date between the last day of the month following the lock date and today
        self.assertEqual(expense.accounting_date, fields.Date.from_string('2022-01-25'))
        ...
        with freeze_time('2022-02-25'):  # Changing today to be later than the last day of the month following the lock date
            expense_2.action_approve()
            expense_2.action_post()

        # Last day of the month following the lockdate
        self.assertEqual(expense_2.accounting_date, fields.Date.from_string('2022-01-31'))

    @freeze_time('2022-01-25')
    def test_accounting_date_reset_after_draft_reset(self):
        """
        Test that the accounting date is reset to False when we reset the expense to draft
        """
        expense = self.create_expenses({
            'name': 'Expense for John Smith',
            'employee_id': self.expense_employee.id,
        ...
        expense.action_approve()
        expense.action_post()
        self.assertEqual(expense.accounting_date, False, "Accounting date must be reset when expense is reset to draft")
        expense.action_reset()
        self.assertEqual(expense.state, 'draft', "The expense expense must be reset to draft")
        self.assertEqual(expense.accounting_date, date.fromisoformat('2022-01-25'), "Accounting date must be reset when expense is reset to draft")
    ...
    def test_expense_with_employee_of_no_work_email(self):
        """
        ...
        employee.bank_account_id = employee_bank_account.id,
    ...
    def test_foreign_currencies_total(self):
        """ Check that the dashboard computes amount properly in companny currency """
        ...
        expense_data = self.env['hr.expense'].get_expense_dashboard()
        self.assertEqual(expense_data['to_submit']['amount'], 3000.00)
    ...
    def test_update_expense_price_on_product_standard_price(self):
        ...
        self.assertRecordValues(expenses.sorted('name'), [
            ...
            {'name':    'test update', 'price_unit': 200.0, 'quantity': 0.5, 'total_amount': 100.0},  # price_unit is updated
        ])

        expense_update.quantity = 5
        self.assertRecordValues(expenses.sorted('name'), [
            {'name': 'test no update', 'price_unit': 100.0, 'quantity': 1, 'total_amount': 100.0},
            {'name':    'test update', 'price_unit': 200.0, 'quantity': 5, 'total_amount': 250.0},  # total is updated
        ])

        product.standard_price = 0.0
        self.assertRecordValues(expenses.sorted('name'), [
            {'name': 'test no update', 'price_unit': 100.0, 'quantity': 1, 'total_amount': 100.0},
            {'name':    'test update', 'price_unit': 200.0, 'quantity': 1, 'total_amount': 200.0},  # quantity & price_unit only are updated
        ])
        ...
        self.assertRecordValues(expenses.sorted('name'), [
            {'name': 'test no update', 'price_unit': 100.0, 'quantity': 1, 'total_amount': 100.0},
            {'name':    'test update', 'price_unit': 200.0, 'quantity': 1, 'total_amount': 200.0},  # no update
        ])
    ...
    def test_expense_multi_company(self):
        ...
        # to company_2.
        with self.assertRaises(UserError):
            expense_approve.with_user(self.env.user).with_context(allowed_company_ids=main_company.ids).action_post()

        # An expense manager having accounting access rights is able to post the journal entry with access to
        # company_2.
        expense_approve.with_user(self.env.user).with_context(allowed_company_ids=other_company.ids).action_post()
    ...
    def test_expense_mandatory_analytic_plan_product_category(self):
        ...
        expense.expense_line_ids.analytic_distribution = {self.analytic_account_1.id: 100.00}
        expense.with_context(validate_analytic=True).action_approve()
