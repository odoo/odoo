...
class TestExpenses(TestExpenseCommon):
    ...
    def test_expense_sheet_payment_state_after_payment_cancel(self):
        sheet = self.env['hr.expense.sheet'].create({
            'company_id': self.env.company.id,
            'employee_id': self.expense_employee.id,
            'name': 'test sheet',
        ...
        self.assertEqual(sheet.state, 'cancel', 'The payment state of the expense sheet is not correct')
