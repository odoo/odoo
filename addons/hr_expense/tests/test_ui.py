...
class TestUi(TestExpenseCommon, HttpCase):
    ...
    def test_not_create_zero_amount_expense_in_expense(self):
        """
        ...
        """
        expense = self.create_expenses({'name': 'report_for_tour'})
        with mute_logger("odoo.http"):
            self.start_tour('/odoo', 'do_not_create_zero_amount_expense', login=self.env.user.login)
        self.assertEqual(expense.total_amount, 10.0, "Expense amount should have been set by tour")
