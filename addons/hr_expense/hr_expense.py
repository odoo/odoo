...
class hr_expense_expense(osv.osv):
    ...
    def unlink(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.state != 'draft':
                raise UserError(_('Warning!'),_('You can only delete draft expenses!'))
    ...
    def expense_confirm(self, cr, uid, ids, context=None):
        for expense in self.browse(cr, uid, ids):
            if not expense.line_ids:
                raise UserError(_('Error!'), _('You cannot submit expense which has no expense line.'))
    ...
    def account_move_get(self, cr, uid, expense_id, context=None):
        ...
        if expense.journal_id:
            ...
            if not journal_id:
                raise UserError(_('Error!'), _("No expense journal found. Please make sure you have a journal with type 'purchase' configured."))
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        for exp in self.browse(cr, uid, ids, context=context):
            if not exp.employee_payable_account_id:
                raise UserError(_('Error!'), _('No employee account payable found for the expense '))
    ...
    def move_line_get_item(self, cr, uid, line, context=None):
        ...
        if line.product_id:
            ...
            if not acc:
                raise UserError(_('Error!'), _('No purchase account found for the product %s (or for his category), please configure one.') % (line.product_id.name))
        else:
            acc = property_obj.get(cr, uid, 'property_account_expense_categ', 'product.category', context={'force_company': company.id})
            if not acc:
                raise UserError(_('Error!'), _('Please configure Default Expense account for Product purchase: `property_account_expense_categ`.'))
        return {
            'type':'src',
