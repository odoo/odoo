...
class HrExpenseSplitWizard(models.TransientModel):
    ...
    def action_split_expense(self):
        ...
        if self.expense_id.split_expense_id:
            split_expense_ids = self.env['hr.expense'].search([('split_expense_id', '=', self.expense_id.split_expense_id.id)])
        all_related_expenses = copied_expenses | self.expense_id | split_expense_ids
        all_related_expenses.split_expense_id = self.expense_id
