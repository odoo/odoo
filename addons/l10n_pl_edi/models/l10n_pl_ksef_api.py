...
class KsefApiService:
    def __init__(self, company):
        ...
        self.mode = company.env['ir.config_parameter'].sudo().get_param('l10n_pl_edi_ksef.mode') or 'prod'
        self.refresh_token = company.l10n_pl_refresh_token
        self.api_url = self._get_api_url()
        self.raw_symmetric_key = base64.b64decode(company.l10n_pl_ksef_session_key) if company.l10n_pl_ksef_session_key else None
        self.raw_iv = base64.b64decode(company.l10n_pl_ksef_session_iv) if company.l10n_pl_ksef_session_iv else None
    ...
    def _make_request(self, method, endpoint, is_auth_retry=False, **kwargs):
        ...
        kwargs['headers'].update(self._make_headers(self.company.l10n_pl_access_token))
        try:
    ...
    def _get_public_keys(self):
        ...
        try:
            ...
            if not public_keys['symmetric'] or not public_keys['token']:
                raise UserError(self.company.env._("Could not find all required KSeF public keys ('SymmetricKeyEncryption' and 'KsefTokenEncryption')."))
            return public_keys

        except requests.exceptions.RequestException as e:
            raise UserError(self.company.env._("Could not fetch KSeF public keys: %s", e.response.text if e.response else e))

    def open_ksef_session(self):
        """Builds the encrypted request and opens an interactive session, with one retry on token expiry."""
        if self.company.l10n_pl_ksef_session_id and self.get_session_status().get('code') == 100:
            return
        ...
        try:
            ...
            self.company.sudo().write(
                {
                    'l10n_pl_ksef_session_id': response.json().get('referenceNumber'),
                    'l10n_pl_ksef_session_key': base64.b64encode(self.raw_symmetric_key),
                    'l10n_pl_ksef_session_iv': base64.b64encode(self.raw_iv),
                })
        except UserError as e:
            raise UserError(self.company.env._("Failed to open KSeF session: %s", e))

    def refresh_access_token(self):
        """Uses a refresh token to obtain a new access token and updates the service and company."""
        if not self.refresh_token:
            raise UserError(self.company.env._("No refresh token found to renew the session."))
        ...
        try:
            ...
            if not new_access_token:
                raise UserError(self.company.env._("Failed to retrieve a new access token from KSeF response."))

            self.company.l10n_pl_access_token = new_access_token
    ...
    def send_invoice(self, xml_content_bytes):
        ...
        endpoint = f"{self.api_url}/sessions/online/{self.company.l10n_pl_ksef_session_id}/invoices"
        headers = {'Content-Type': 'application/json'}
    ...
    def close_ksef_session(self):
        ...
        if not self.company.l10n_pl_ksef_session_id:
            _logger.warning("No KSeF session data found to close.")
            return

        endpoint = f"{self.api_url}/sessions/online/{self.company.l10n_pl_ksef_session_id}/close"
        try:
    ...
    def get_session_status(self):
        if not self.company.l10n_pl_ksef_session_id:
            raise UserError(self.company.env._("No active KSeF session found. Please open a session first."))
        endpoint = f"{self.api_url}/sessions/{self.company.l10n_pl_ksef_session_id}"
        try:
            response = self._make_request('GET', endpoint)
            return response.json().get('status')
        except UserError as e:
            raise UserError(self.company.env._("Failed to check KSeF session: %s", e))
    ...
    def get_invoice_status(self, invoice_reference_number):
        ...
        """
        endpoint = f"{self.api_url}/sessions/{self.company.l10n_pl_ksef_session_id}/invoices/{invoice_reference_number}"

        response = self._make_request('GET', endpoint)
        return response.json()

    def get_invoice_upo(self, invoice_reference_number):
        endpoint = f"{self.api_url}/sessions/{self.company.l10n_pl_ksef_session_id}/invoices/{invoice_reference_number}/upo"
        response = self._make_request('GET', endpoint)
    ...
    def get_challenge(self):
        ...
        try:
            ...
            raise UserError(self.company.env._("Failed to get challenge code: %s", e.response.text if e.response else e))
    ...
    def authenticate_xades(self, signed_xml):
        ...
        try:
            ...
            raise UserError(self.company.env._("Failed to authenticate with XAdES: %s", e.response.text if e.response else e))
    ...
    def authenticate_token(self, nip, challenge, encrypted_token_b64):
        ...
        try:
            ...
            raise UserError(self.company.env._("Failed to start token authentication: %s", e.response.text if e.response else e))
    ...
    def check_auth_status(self, ref_number, temp_token):
        ...
        for _attempt in range(5):
            try:
                ...
                raise UserError(self.company.env._("Failed to check KSeF auth status: %s", e.response.text if e.response else e))

        raise UserError(self.company.env._("KSeF Authentication timed out. Please try again in a moment."))
    ...
    def redeem_token(self, temp_token):
        ...
        try:
            ...
            raise UserError(self.company.env._("Failed to redeem token: %s", e.response.text if e.response else e))
