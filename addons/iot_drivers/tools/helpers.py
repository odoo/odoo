    ...
    def devtools_wrapper(*args, **kwargs):
        if args and args[0].__class__.__name__ == "DriverController":
            if system.get_conf("longpolling", section="devtools"):
                ...
                msg = "Longpolling disabled by devtools"
                raise Locked(msg)  # raise to make the http request fail
        elif function.__name__ == "action":
            action = kwargs.get(
                "action", "default"
            )  # first argument is self (containing Driver instance), second is 'data'
            disabled_actions = (
                system.get_conf("actions", section="devtools") or ""
            ).split(",")
    ...
    try:
        response = requests.post(
            ...
            data={
                ...
                "auto": True,
            },  # TODO: remove `auto` when v19.0 is deprecated
            timeout=8,
