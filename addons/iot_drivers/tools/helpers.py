...
def download_from_url(download_url, path_to_filename):
    ...
    try:
        ...
        system.write_file(path_to_filename, request_response.content, 'wb')
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    """
    try:
        path = system.path_file(path_to_filename)
        with zipfile.ZipFile(path) as zip_file:
            zip_file.extractall(system.path_file(path_to_extract))
        Path(path).unlink()
