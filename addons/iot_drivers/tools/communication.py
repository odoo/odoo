...
from odoo.addons.iot_drivers.tools.system import IOT_IDENTIFIER, IS_WINDOWS
...
def handle_message(message_type: str, communication_type: str, **kwargs: dict) -> dict:
    ...
    base_response = {
        "owner": kwargs.get(
            "session_id", "0"
        ),  # TODO: remove 'owner' in future versions
    ...
    _logger.info("(%s) Received message of type %s", communication_type, message_type)
    ...
    match message_type:
        case "iot_action":
            if device_identifier not in main.iot_devices:
                ...
                _logger.warning(
                    "No IoT device with identifier '%s' found", device_identifier
                )
                return {**base_response, "status": "disconnected"}
            start_operation_time = time.perf_counter()
            _logger.info(
                "(%s) device '%s' action started", communication_type, device_identifier
            )
