...
class IotBoxOwlHomePage(Controller):
    ...
    def index(self):
        return Stream.from_path("iot_drivers/views/index.html").get_response(
            content_security_policy=CONTENT_SECURITY_POLICY
        )

    @route.iot_route("/logs", type="http")
    def logs_page(self):
        return Stream.from_path("iot_drivers/views/logs.html").get_response(
            content_security_policy=CONTENT_SECURITY_POLICY
        )

    @route.iot_route("/status", type="http")
    def status_page(self):
        return Stream.from_path("iot_drivers/views/status_display.html").get_response(
            content_security_policy=CONTENT_SECURITY_POLICY
        )
    ...
    @route.iot_route(
        [
            "/iot_drivers/restart_odoo_service",
    ...
    @route.iot_route(
        "/iot_drivers/is_remote_debug_enabled", type="http", linux_only=True
    )
    ...
    @route.iot_route(
        "/iot_drivers/enable_remote_debug",
        type="jsonrpc",
        methods=["POST"],
        linux_only=True,
    )
    def enable_remote_connection(self, auth_token):
        return {
            "status": "success" if system.toggle_remote_debug(auth_token) else "failure"
        }

    @route.iot_route(
        "/iot_drivers/disable_remote_debug",
        type="jsonrpc",
        methods=["POST"],
        linux_only=True,
    )
    ...
    @route.iot_route(
        "/iot_drivers/enable_custom_handlers",
        type="jsonrpc",
        methods=["POST"],
        cors="*",
    )
    ...
    def _update_logger_level(
        ...
        LOG_HANDLERS = (
            system.get_conf(ODOO_TOOL_CONFIG_HANDLER_NAME, section="options") or []
        ).split(",")
