...
export class FooterButtons extends Component {
    ...
    <div class="w-100 d-flex flex-wrap align-items-cente gap-2 justify-content-center" t-translation="off">
        <a t-if="this.store.isLinux() and !this.store.base().is_access_point_up" class="btn btn-primary btn-sm" href="/status" target="_blank">
            Status Display
        </a>
        <a t-if="this.store.isLinux() and !this.store.base().is_access_point_up" class="btn btn-primary btn-sm" t-att-href="'http://' + this.store.base().ip + ':631'" target="_blank">
            Printer Server
        </a>
