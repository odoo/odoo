...
class DisplayDriver(Driver):
    ...
    def get_url_from_db(self, server_url=None):
        ...
        try:
            response = requests.get(
                f"{server_url}/iot/box/{IOT_IDENTIFIER}/display_url", timeout=5
            )
    ...
    def _action_open_customer_display(self, data):
        ...
        self.update_url(
            f"{origin}/pos_customer_display/{data['pos_id']}/{data['access_token']}"
        )
    ...
    def set_orientation(self, orientation=Orientation.NORMAL):
        if type(orientation) is not Orientation:
            msg = "orientation must be of type Orientation"
            raise TypeError(msg)
...
class DisplayController(http.Controller):
    ...
    def customer_facing_display(
        self, action, pos_id=None, access_token=None, data=None
    ):
