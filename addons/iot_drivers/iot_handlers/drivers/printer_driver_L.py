...
class PrinterDriver(PrinterDriverBase):
    def __init__(self, identifier, device):
        ...
        if (
            any(cmd in device_id for cmd in ["CMD:STAR;", "CMD:ESC/POS;"])
            or "tm-m30" in self.device_name.lower()
        ):
            self.device_subtype = "receipt_printer"
        elif (
            any(cmd in device_id for cmd in ["COMMAND SET:ZPL;", "CMD:ESCLABEL;"])
        ...
        try:
            with self.cups_lock:
                ...
                job_id = self.conn.createJob(
                    self.device_identifier, "Odoo print job", options
                )
