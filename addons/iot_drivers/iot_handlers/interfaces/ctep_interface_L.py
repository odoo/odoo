...
if not path_file('odoo/addons/iot_drivers/iot_handlers/drivers/ctep/libeasyctep.so').exists():
    ...
    try:
        ...
        with open("/etc/ld.so.conf.d/worldline-ctep.conf", "w", encoding="utf-8") as f:
            f.write(str(extract_path / "ctep" / "lib"))
        with open("/root_bypass_ramdisks/etc/ld.so.conf.d/worldline-ctep.conf", "w", encoding="utf-8") as f:
            f.write(str(extract_path / "ctep" / "lib"))
