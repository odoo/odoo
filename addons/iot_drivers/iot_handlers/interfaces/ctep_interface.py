...
def _configure_ctep_lib(lib_name: str) -> bool:
    ...
    source_zip_name = (
        "worldline-ctepv21_07.zip" if IS_RPI else "worldline-ctepv23_02_w.zip"
    )
    ...
    if IS_WINDOWS:
        ...
        os.environ["PATH"] = (
            str(drivers_path / "ctep") + os.pathsep + os.environ["PATH"]
        )
        return True
    ...
    try:
        ...
        subprocess.run(
            [
                "sudo",
                "tee",
                worldline_ctep_conf_path,
                f"/root_bypass_ramdisks{worldline_ctep_conf_path}",
            ],
            text=True,
...
class CTEPInterface(Interface):
    ...
    def __init__(self):
        ...
        self.easy_ctep.connectedTerminal.argtypes = [
            ctypes.c_void_p,
            ctypes.c_char_p,
            *extra_args,
        ]
    ...
    def get_devices(self):
        ...
        try:
            if self.manager and self.easy_ctep.connectedTerminal(
                self.manager, terminal_id, *extra_args
            ):
                return {
