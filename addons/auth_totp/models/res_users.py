...
TOTP_RATE_LIMIT_ERROR_MESSAGES = {
...
class ResUsers(models.Model):
    ...
    def _totp_rate_limit(self, limit_type):
        self.ensure_one()
        if not self.env['rate.limit.log'].sudo()._consume_rate_limit([{'scope': limit_type, 'key_vals': [self.id], 'limit': 5, 'interval': 3600}]):
            raise AccessDenied(TOTP_RATE_LIMIT_ERROR_MESSAGES[limit_type])

    def _totp_rate_limit_purge(self, limit_type):
        self.ensure_one()
        self.env['rate.limit.log'].sudo()._reset_rate_limit([{'scope': limit_type, 'key_vals': [self.id]}])
