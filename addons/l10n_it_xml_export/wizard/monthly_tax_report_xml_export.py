...
class L10nItMonthlyTaxReportXmlExportWizard(models.TransientModel):
    ...
    declarant_fiscal_code = fields.Char(
        ...
        default=lambda self: self.env.company.account_representative_id.l10n_it_codice_fiscale if self.env.company.account_representative_id else self.env.company.l10n_it_codice_fiscale,
    )
    ...
    parent_vat_number = fields.Char(
        string="Parent VAT Number",
        help="VAT number of the parent company.",
        default=lambda self: "".join([char for char in self.env.company.parent_id.vat if char.isdigit()]) if self.env.company.parent_id and self.env.company.parent_id.vat else False,
    )
