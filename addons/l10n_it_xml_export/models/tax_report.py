...
class ItalianReportCustomHandler(models.AbstractModel):
    ...
    def export_tax_report_to_xml(self, options):
        ...
        with file_open("l10n_it_xml_export/data/validation/fornituraIvp_2018_v1.xsd", 'rb') as xsd:
            xsd_schema = etree.XMLSchema(etree.parse(xsd, parser=etree.XMLParser()))
            try:
    ...
    def _get_xml_export_data(self, options):
        ...
        report_line2amount = {
            line['columns'][colname_to_idx['balance']]['report_line_id']: "{:.2f}".format(
        ...
        report_lines_data = {}
        for record in self.env['account.report.line'].browse(report_line2amount.keys()):
            report_lines_data[record.code] = report_line2amount[record.id]
        ...
        return {
            ...
            "parent_vat_number": options["parent_vat_number"],
