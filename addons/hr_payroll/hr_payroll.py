    ...
    def unlink(self, cr, uid, ids, context=None):
        for payslip in self.browse(cr, uid, ids, context=context):
            if payslip.state not in  ['draft','cancel']:
                raise UserError(_('Warning!'),_('You cannot delete a payslip which is not draft or cancelled!'))
    ...
    def compute_rule(self, cr, uid, rule_id, localdict, context=None):
        ...
        if rule.amount_select == 'fix':
            try:
                ...
                raise UserError(_('Error!'), _('Wrong quantity defined for salary rule %s (%s).')% (rule.name, rule.code))
        elif rule.amount_select == 'percentage':
            try:
                return eval(rule.amount_percentage_base, localdict), eval(rule.quantity, localdict), rule.amount_percentage
            except:
                raise UserError(_('Error!'), _('Wrong percentage base or quantity defined for salary rule %s (%s).')% (rule.name, rule.code))
        else:
            try:
                ...
                raise UserError(_('Error!'), _('Wrong python code defined for salary rule %s (%s).')% (rule.name, rule.code))
    ...
    def satisfy_condition(self, cr, uid, rule_id, localdict, context=None):
        ...
        if rule.condition_select == 'none':
            ...
            try:
                ...
                raise UserError(_('Error!'), _('Wrong range condition defined for salary rule %s (%s).')% (rule.name, rule.code))
        else: #python code
            try:
                eval(rule.condition_python, localdict, mode='exec', nocopy=True)
                return 'result' in localdict and localdict['result'] or False
            except:
                raise UserError(_('Error!'), _('Wrong python condition defined for salary rule %s (%s).')% (rule.name, rule.code))
