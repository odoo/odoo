...
class SaleOrder(models.Model):
    ...
    def _activity_cancel_on_purchase(self):
        ...
        purchase_order_lines = self.env['purchase.order.line'].search([('sale_line_ids', 'in', self.mapped('order_line').ids), ('state', '!=', 'cancel')])
        for purchase_line in purchase_order_lines:
            ...
            purchase_to_notify_map[purchase_line.order_id] |= purchase_line.sale_line_ids
...
class SaleOrderLine(models.Model):
    ...
    def _compute_purchase_count(self):
        database_data = self.env['purchase.order.line'].sudo().read_group([('sale_line_ids', 'in', self.ids)], ['sale_line_ids'], ['sale_line_ids'])
        po_lines_by_so_line = defaultdict(float)
        for group in database_data:
            po_lines_by_so_line[group['sale_line_ids']] += group['sale_line_ids_count']

        for line in self:
            line.purchase_line_count = po_lines_by_so_line.get(line.id, 0)
    ...
    def _purchase_decrease_ordered_qty(self, new_qty, origin_values):
        ...
        last_purchase_lines = self.env['purchase.order.line'].search([('sale_line_ids', 'in', self.ids)])
    ...
    def _purchase_increase_ordered_qty(self, new_qty, origin_values):
        ...
        """
        for line in self:
            last_purchase_line = self.env['purchase.order.line'].search([('sale_line_ids', 'in', line.id)], order='create_date DESC', limit=1)
