...
class TestTimesheetGlobalTimeOff(common.TransactionCase):
    ...
    def test_timeoff_task_creation_with_global_leave(self):
        ...
        # The company's time off task is already flagged as a time off task as soon as the company is created.
        self.assertIn(self.test_company.leave_timesheet_task_id, self.env['project.task'].search([('is_timeoff_task', '!=', False)]))
        ...
        # Its timesheets are generated on the existing company task, no new task is created.
    ...
    def test_global_time_off_timesheet_creation_after_leave_refusal(self):
        ...
        hr_work_entry_type_with_ts = self.env['hr.work.entry.type'].create({
            ...
            'code': 'Leave Type with timesheet generation',
