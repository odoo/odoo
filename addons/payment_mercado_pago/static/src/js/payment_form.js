...
paymentForm.include({
    ...
    async _prepareInlineForm(providerId, providerCode, paymentOptionId, paymentMethodCode, flow) {
        ...
        // Create the checkout object if not already done for another payment method.
            try {
                const mp = new MercadoPago(inlineFormValues['mercado_pago_public_key'], {
                    locale: 'en-US'
                });
                ...
                const method_container = `o_mercado_pago_checkout_container_${providerId}_${paymentOptionId}`
    ...
    async _initiatePaymentFlow(providerCode, paymentOptionId, paymentMethodCode, flow) {
        if (providerCode !== 'mercado_pago' || flow === 'token' || paymentMethodCode !== 'card') {
            await this._super(...arguments); // Tokens and other methods are handled by the generic flow.
            return;
        }
