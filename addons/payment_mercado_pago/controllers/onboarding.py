...
class MercadoPagoOnboardingController(Controller):
    ...
    def mercado_pago_return_from_authorization(self, **data):
        ...
        try:
            response_content = provider.with_context(
                mercado_pago_test_mode=test_mode
            )._send_api_request(
                "POST", "2/get_access_token", json=proxy_payload, is_proxy_request=True
            )
            # Verify the account before storing its credentials: the connection mode is chosen
            # before the OAuth round-trip, and Mercado Pago does not enforce it.
            provider._mercado_pago_verify_account_mode(response_content["access_token"], test_mode)
