...
class BuckarooController(http.Controller):
    ...
    def buckaroo_webhook(self, **raw_data):
        ...
        if tx_sudo:
            self._verify_signature(raw_data, tx_sudo)
            tx_sudo._process('buckaroo', data)
        return ''
    ...
    @staticmethod
    def _verify_signature(raw_payment_data, tx_sudo):
        """Check that the received signature matches the expected one.

        :param dict raw_payment_data: The raw payment data.
        :param payment.transaction tx_sudo: The sudoed transaction referenced by the payment data.
        :return: None
        :raise Forbidden: If the signatures don't match.
        """
        payment_data = BuckarooController._normalize_data_keys(raw_payment_data)
        received_signature = payment_data.get('brq_signature')
