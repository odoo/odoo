...
class PaymentWizard(models.TransientModel):
    ...
    def _set_manual_post_msg_value(self):
        self.manual_post_msg = _('Please make a payment to <ul><li>Bank: %s</li><li>Account Number: %s</li><li>Account Holder: %s</li></ul>') %\
                               (self.journal_name or _("Bank") , self.acc_number or _("Account"), self.env['res.company']._get_current_company().name)
    ...
    def _get_manual_payment_acquirer(self, env=None):
        ...
        return env['payment.acquirer'].search([('module_id', '=', module_id),
            ('company_id', '=', env['res.company']._get_current_company().id)], limit=1)
    ...
    def _get_default_payment_acquirer_onboarding_value(self, key):
        ...
        self._payment_acquirer_onboarding_cache['payment_method'] = self.env['res.company']._get_current_company().payment_onboarding_payment_method
    ...
    def add_payment_methods(self):
        ...
        if self.payment_method  in ('paypal', 'stripe', 'manual', 'other'):
            ...
            self.env['res.company']._get_current_company().payment_onboarding_payment_method = self.payment_method
    ...
    def _set_payment_acquirer_onboarding_step_done(self):
        self.env['res.company']._get_current_company().set_onboarding_step_done('payment_acquirer_onboarding_state')
