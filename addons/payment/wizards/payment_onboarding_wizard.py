...
class PaymentWizard(models.TransientModel):
    ...
    def _get_default_payment_provider_onboarding_value(self, key):
        ...
        if 'payment_paypal' in installed_modules:
            provider = self.env['payment.provider'].search(
                [('company_id', '=', self.env.company.id), ('code', '=', 'paypal')],
                limit=1
            )
    ...
    def add_payment_methods(self):
        ...
        if self.payment_method in ('paypal', 'manual'):
            ...
            if self.payment_method == 'paypal':
                provider_count = new_env['payment.provider'].search_count(
                    [('company_id', '=', self.env.company.id), ('code', '=', 'paypal')]
                )
                if provider_count == 0:
                    new_provider = self.env.ref('payment.payment_provider_paypal').with_company(self.env.company.id).sudo().copy()
                ...
                new_env['payment.provider'].search([('company_id', '=', self.env.company.id), ('code', '=', 'paypal')], limit=1).write({
