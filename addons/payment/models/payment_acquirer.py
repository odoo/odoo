...
class PaymentAcquirer(models.Model):
    ...
    save_token = fields.Selection([
        ...
        "If you manage subscriptions (recurring invoicing), you need it to automatically charge the customer when you issue an invoice.")
    ...
    def _check_authorization_support(self):
        for acquirer in self:
            if acquirer.auto_confirm == 'authorize' and acquirer.provider not in self._get_feature_support()['authorize']:
                raise ValidationError(""" You cannot capture payments manually with this payment method.
                                    Please choose another Order Confirmation mode (in Configuration tab).""")
        return True
...
class PaymentTransaction(models.Model):
    ...
    def _check_reference(self):
        for transaction in self.filtered(lambda tx: tx.state not in ('cancel', 'error')):
            ...
            if transaction.state == 'authorized' and transaction.acquirer_id.auto_confirm != 'authorize':
                    raise exceptions.ValidationError(_('The %s payment acquirer is not allowed to manual capture mode!' % transaction.acquirer_id.name))
        return True
