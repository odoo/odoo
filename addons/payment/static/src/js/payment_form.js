            ...
            if (checked_radio.length === 1) {
                ...
                if (this.isNewPaymentRadio(checked_radio)) {
                    ...
                    ajax.jsonRpc(ds.dataset.createRoute, 'call', form_data).then(function (data) {
                        ...
                        self.displayError(
                            _t('Server Error'),
                            _t("<p>We are not able to add your payment method at the moment.</p>") + "<div style='margin-top:-25px'>" + (core.debug ? (data.data.message.replace(/\n/g, "<br />")): '') + "</div>"
                        );
                    });
                }
                // if the user is going to pay with a form payment, then
                    ...
                    if ($tx_url.length === 1) {
                        ...
                        ajax.jsonRpc($tx_url[0].value, 'call', {
                            ...
                            self.displayError(
                                _t('Server Error'),
                                _t("<p>We are not able to redirect you to the payment form.</p>") + "<div style='margin-top:-25px'>" + (core.debug ? (data.data.message.replace(/\n/g, "<br />")): '') + "</div>"
                            );
                        });
                    }
                    else {
