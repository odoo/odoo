...
export class BackgroundShapeOptionPlugin extends Plugin {
    ...
    resources = {
        ...
        on_bg_color_changed: this.handleBgColorChanged.bind(this),
    };
    static shared = [
        "getShapeStyleUrl",
        "getShapeData",
        ...
        return {
            ...
            flipShape: {
                apply: ({ editingElement, param: { mainParam: axis } }) => {
                    if (axis === "y") {
                        this.flipYPending = true;
                    }
                    // Might want to prevent the colors computing when flipping x.
                    this.applyShape(editingElement, () => {
                    ...
                    if (axis === "y") {
                        delete this.flipYPending;
                    }
                },
                clean: ({ editingElement, param: { mainParam: axis } }) => {
                    if (axis === "y") {
                        this.flipYPending = false;
                    }
                    this.applyShape(editingElement, () => {
                        let { shape, flip } = this.getShapeData(editingElement);
                        flip = new Set(flip);
            ...
            dispatchChange: {
                apply: ({ editingElement }) => {
                    this.dispatchTo("on_bg_color_changed", editingElement);
                },
            },
        };
    }
    ...
    handleSnippetMoved(editingElement) {
        this.updateShapeForNeighborBeforeMoving(editingElement);
    }
    /**
    ...
    showBackgroundShapes(editingElements) {
        this.dependencies.customizeTab.openCustomizeComponent(ShapeSelector, editingElements, {
            ...
            shapeData: this.getShapeData(editingElements[0]),
        });
    }
    ...
    getElementsFromPoint(editingElement, viewportX, viewportY) {
        ...
        const isHeaderTop = !header.classList.contains("o_header_sidebar");
        if (pageY < headerHeight && isHeaderTop) {
    ...
    updateShapeForNeighborBeforeMoving(editingElement) {
        const targetRect = editingElement.getBoundingClientRect();
        ...
        [...topNeighbors, ...bottomNeighbors].forEach((element, index) => {
            const elRect = element.getBoundingClientRect();
            const x = elRect.left + elRect.width / 2;
            const y = index < topNeighbors.length ? targetRect.bottom + 1 : targetRect.top - 1;

            let newColor = getComputedStyle(
                this.getBackgroundElementFromPoint(editingElement, x, y)
            ).backgroundColor;
            let curBgColorEl = element;
            while (getComputedStyle(curBgColorEl).backgroundColor.includes("rgba(0, 0, 0, 0)")) {
                curBgColorEl = curBgColorEl.parentElement;
            }
            const curBgColor = getComputedStyle(curBgColorEl).backgroundColor;
            if (curBgColor === newColor) {
                newColor = this.getContrastingColor(newColor);
            }
            this.updateShapeForNeighbor(element, newColor);
        });
    }
    ...
    // Is the shape next to the current element ?
