export async function getActionRecords(records, orm) {
    const actionIds = [];
    (records || []).map((record) => {
        if (!record.callback) {
            actionIds.push(record.id || record.action.id);
        }
    });
    return await orm.call("hr.employee.base", "get_server_action_records", [[], actionIds]);
}
