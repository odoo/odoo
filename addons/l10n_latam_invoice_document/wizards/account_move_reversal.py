        ...
        for record in self:
            if len(record.move_ids) > 1:
                ...
                if move_ids_use_document:
                    raise UserError(_('You can only reverse documents.py with legal invoicing documents.py from Latin America one at a time.\nProblematic documents.py: %s') % ", ".join(move_ids_use_document.mapped('name')))
            else:
