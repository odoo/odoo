...
class TestResUsersCalendars(TransactionCase):
    ...
    def test_new_user_has_one_owned_primary_calendar(self):
        ...
        self.assertTrue(self.user.primary_calendar_id)
        self.assertEqual(self.user, self.user.primary_calendar_id.owner_id)
        primary_memberships = self.user.calendar_user_ids.filtered(
    ...
    def test_get_secondary_calendars(self):
        ...
        self.assertNotIn(self.user.primary_calendar_id, secondaries)
