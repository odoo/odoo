...
class TestAccessRights(TransactionCase):
    ...
    def test_read_group_public(self):
        ...
        data = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', event.id)], groupby=['start:month'])
        self.assertTrue(data, "It should be able to read group")
        data = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', event.id)], groupby=['name'])
        self.assertTrue(data, "It should be able to read group")

    def test_read_group_private(self):
        event = self.create_event(self.john, privacy='private')
        result = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', event.id)], groupby=['name'])
        self.assertFalse(result, "Private events should not be fetched")

    def test_read_group_agg(self):
        event = self.create_event(self.john)
        data = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', event.id)], groupby=['start:week'])
        self.assertTrue(data, "It should be able to read group")
    ...
    def test_event_default_privacy_as_private(self):
        ...
        query_default_event = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', default_event.id)], groupby=['name'])
        query_public_event = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', public_event.id)], groupby=['name'])
        query_private_event = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', private_event.id)], groupby=['name'])
        query_confidential_event = self.env['calendar.event'].with_user(self.raoul).base_read_group([('id', '=', confidential_event.id)], groupby=['name'])
