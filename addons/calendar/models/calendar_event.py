...
class Meeting(models.Model):
    ...
    # Overide of recurrence mixin. Timing redefining the start and stop datetime to use tracking
    start_datetime = fields.Datetime(
        'Start', tracking=True, help="Start date of an event, without time for full days events")
    ...
    def _compute_display_time(self):
        for meeting in self:
            meeting.display_time = self._get_display_time(meeting.start_datetime, meeting.stop_datetime, meeting.duration, meeting.allday)

    @api.depends('allday', 'start_datetime', 'stop_datetime')
    def _compute_dates(self):
        ...
        """
        for meeting in self:
            if meeting.allday and meeting.start_datetime and meeting.stop_datetime:
                meeting.start_date = meeting.start_datetime.date()
                meeting.stop_date = meeting.stop_datetime.date()
            else:
    ...
    def _compute_stop(self):
        ...
        for event in self:
            ...
            event.stop_datetime = event.start_datetime and event.start_datetime + timedelta(minutes=round((event.duration or 1.0) * 60))
            if event.allday:
                event.stop_datetime -= timedelta(seconds=1)
    ...
    def _inverse_dates(self):
        ...
        for meeting in self:
            if meeting.allday:
                ...
                meeting.write({
                    'start_datetime': startdate.replace(tzinfo=None),
                    'stop_datetime': enddate.replace(tzinfo=None)
                })
    ...
    def _check_closing_date(self):
        for meeting in self:
            if not meeting.allday and meeting.start_datetime and meeting.stop_datetime and meeting.stop_datetime < meeting.start_datetime:
                raise ValidationError(
                    _('The ending date and time cannot be earlier than the starting date and time.') + '\n' +
                    _("Meeting '%(name)s' starts '%(start_datetime)s' and ends '%(stop_datetime)s'",
                      name=meeting.name,
    ...
    def create(self, vals_list):
        ...
        # Create the events, the recurrency events will be handled too
        events = super().create(vals_list)

        events.filtered(lambda event: event.start_datetime > fields.Datetime.now()).attendee_ids._send_mail_to_attendees(
    ...
    def write(self, values):
        ...
        update_recurrence = values.get('recurrence_update') in ('all_records', 'future_records') and len(self) == 1
        if not update_recurrence:
        ...
        if 'start_datetime' in values:
            start_date = fields.Datetime.to_datetime(values.get('start_datetime'))
    ...
    def action_mass_deletion(self, recurrence_update_setting):
        ...
        if recurrence_update_setting == 'all_records':
            events = self.recurrence_id._get_recurrent_records(model=self._name)
            self.recurrence_id.unlink()
            events.unlink()
        elif recurrence_update_setting == 'future_records':
            future_events = self.recurrence_id._get_recurrent_records(model=self._name).filtered(lambda ev: ev.start_datetime >= self.start_datetime)
    ...
    def _setup_alarms(self):
        ...
        for event in self:
            for alarm in (alarm for alarm in event.alarm_ids if alarm.alarm_type in alarm_types):
                at = event.start_datetime - timedelta(minutes=alarm.duration_minutes)
            ...
            if any(alarm.alarm_type == 'notification' for alarm in event.alarm_ids):
                ...
                events_to_notify |= event.filtered(lambda ev: ev.alarm_ids and ev.stop_datetime >= fields.Datetime.now())
        if events_to_notify:
    ...
    def change_attendee_status(self, status, recurrence_update_setting):
        ...
        if recurrence_update_setting == 'all_records':
            ...
            events = self.recurrence_id._get_recurrent_records(model=self._name).filtered(lambda ev: ev.start >= self.start)
        else:
            events = self
    ...
    def _get_start_date(self):
        ...
        if self.recurrence_id.record_tz:
            ...
            start = self.start_datetime if not self.allday else self.start_datetime.replace(hour=12)
    ...
    def _get_ics_file(self):
        ...
        for meeting in self:
            ...
            if not meeting.start_datetime or not meeting.stop_datetime:
                raise UserError(_("First you have to specify the date of the invitation."))
            event.add('created').value = ics_datetime(fields.Datetime.now())
            event.add('dtstart').value = ics_datetime(meeting.start_datetime, meeting.allday)
            event.add('dtend').value = ics_datetime(meeting.stop_datetime, meeting.allday)
    ...
    @api.model
    def _get_display_time(self, start_datetime, stop_datetime, zduration, zallday):
        ...
        date = fields.Datetime.context_timestamp(self_tz, fields.Datetime.from_string(start_datetime))
        date_deadline = fields.Datetime.context_timestamp(self_tz, fields.Datetime.from_string(stop_datetime))
    ...
    def _get_time_fields(self):
        return {'start_datetime', 'stop_datetime', 'start_date', 'stop_date'}
    ...
    def _rewrite_start_time_fields(self, update_dict, base_time_values, start_update):
        ...
        """
        update_dict = super()._rewrite_start_time_fields(update_dict, base_time_values, start_update)
        start_date = base_time_values['start_datetime'].date() + (start_update.date() - self.start_datetime.date())
        stop_date = base_time_values['stop_datetime'].date() + (start_update.date() - self.start_datetime.date())
        update_dict.update(
    ...
    def _rewrite_stop_time_fields(self, update_dict, base_time_values, start_update, stop_update, ):
        update_dict = super()._rewrite_stop_time_fields(update_dict, base_time_values, start_update, stop_update)
        if not start_update:
            # Apply the same shift for start
            start_date = base_time_values['start_date'] + (stop_update.date() - self.stop_datetime.date())
            update_dict.update({'start_date': start_date})
        stop_date = base_time_values['stop_datetime'].date() + (stop_update.date() - self.stop_datetime.date())
        update_dict.update({'stop_date': stop_date})
        return update_dict
