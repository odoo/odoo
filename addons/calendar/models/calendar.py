...
from odoo.tools.datetime import date, datetime, relativedelta, timedelta,\
    rrulestr, parse, DEFAULT_SERVER_DATE_FORMAT, UTC, datetime_types
...
class Meeting(models.Model):
    ...
    def _get_recurrent_date_by_event(self, date_field='start'):
        ...
        for meetings in self.env.cr.split_for_in_conditions(recurring_meetings, size=50000):
            ...
            invalidate = True
        return [d.astimezone(UTC) if d.tzinfo else d for d in rset1]
                ...
                for arg in domain:
                    if str(arg[0]) in ('start', 'stop', 'final_date'):
                        if str(arg[0]) == 'start':
                            ...
                            r_date = r_stop_date
                        l_date = datetime.from_string(arg[2]) if arg[2] else False
                        if l_date and not l_date.tzinfo:
                            l_date = l_date.replace(tzinfo=r_date.tzinfo)
                        if arg[1] == '=':
                            ok = r_date == l_date
                        elif arg[1] == '!=':
                            ok = r_date != l_date
