...
class ResUsers(models.Model):
    ...
    calendar_ids = fields.One2many('calendar.calendar', compute='_compute_calendar_ids', compute_sudo=True, readonly=False, user_writeable=True)
    owned_calendar_ids = fields.One2many('calendar.calendar', compute='_compute_owned_calendar_ids')
    writable_calendar_ids = fields.One2many('calendar.calendar', compute='_compute_writable_calendar_ids')
    primary_calendar_id = fields.Many2one('calendar.calendar', compute='_compute_primary_calendar', store=True)
    default_calendar_id = fields.Many2one('calendar.calendar', help='The default calendar for all newly created events',
        domain='[("id", "in", writable_calendar_ids)]', user_writeable=True)
    ...
    def _generate_primary_calendar(self):
        self.env['calendar.calendar'].sudo().create([
            {
                'name': _('%s\'s Calendar', user.display_name),
    ...
    def _compute_calendar_ids(self):
        for user in self:
            user.calendar_ids = user.calendar_user_ids.calendar_id

    @api.depends('calendar_user_ids')
    def _compute_owned_calendar_ids(self):
        for user in self:
            user.owned_calendar_ids = user.calendar_user_ids.filtered(lambda l: l.access_role == 'owner').calendar_id

    @api.depends('calendar_user_ids')
    def _compute_writable_calendar_ids(self):
        for user in self:
            user.writable_calendar_ids = user.calendar_user_ids.filtered(lambda l: l.access_role in ('owner', 'writer')).calendar_id

    def get_secondary_calendars(self):
        return self.calendar_user_ids.filtered(lambda l: not (l.is_primary and l.access_role == 'owner')).calendar_id
