...
class Controller(openerp.addons.bus.controllers.main.BusController):
    def _poll(self, dbname, channels, last, options):
        if request.session.uid:
            ...
            channels.append((request.db, 'calendar.alarm', partner_id))
...
class meeting_invitation(http.Controller):
    ...
    def notify_ack(self, event_id, type=''):
        registry = request.registry
        uid = request.session.uid
        context = request.session.context
        with registry.cursor() as cr:
            res = registry.get("calendar.attendee")._set_calendar_last_notif_ack(cr, uid, event_id, context=context)
            return res
