...
class SMSResend(models.TransientModel):
    ...
    def action_resend(self):
        ...
        if to_resend_ids:
            ...
            notify_sms_pid_to_number = dict((r.partner_id.id, r.sms_number) for  r in self.recipient_ids if r.resend and r.partner_id)
            pids = list(notify_sms_pid_to_number.keys())
            ...
            if rdata or numbers:
                record.with_context(notify_check_existing=True, notify_sms_pid_to_number=notify_sms_pid_to_number, notify_sms_numbers=numbers)._notify_records_by_sms(self.mail_message_id, {'partners': rdata}, put_in_queue=False)
