...
class SendSMS(models.TransientModel):
    ...
    def _compute_description(self):
        if self.partner_ids:
            description = ''
            description = ','.join('%s - %s' % (partner.display_name, partner.mobile or partner.phone or 'Missing number') for partner in self.partner_ids[:self._RECIPIENTS_DISPLAY_NBR])
