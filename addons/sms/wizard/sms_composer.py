...
class SendSMS(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if fields == 'partner_ids':
            # TDE FIXME: why the fuck is default get called several times ... bloody retarded ORM
            return result
    ...
    @api.depends('partner_ids', 'res_model', 'res_id', 'composition_mode', 'number_field_name', 'sanitized_numbers')
    def _compute_description(self):
        if self.partner_ids:
            description = ''
            description = ','.join('%s - %s' % (partner.display_name, partner.mobile or partner.phone) for partner in self.partner_ids[:3])
            if len(self.partner_ids) > 3:
                description += _(' (and %s more)') % (len(self.partner_ids) - 3)
            ...
            if records and issubclass(type(records), self.pool['mail.thread']):
                ...
                self.recipient_description = ', '.join('%s (%s)' % (
                    ...
                    for record in records if record.id in valid_ids[:3]
                ) or False
                if len(valid_ids) > 3:
                    self.recipient_description += _(', and %s more') % (len(valid_ids) - 3)
                self.recipient_invalid = ', '.join('%s (%s)' % (
                    res[record.id]['partner'].display_name or record.display_name,
                    res[record.id]['number'])
                    for record in records if record.id in invalid_ids[:3]
                ) or False
                if len(invalid_ids) > 3:
                    self.recipient_description += _(', and %s more') % (len(invalid_ids) - 3)
            else:
