odoo.define('web_clearbit.field', function (require) {
    ...
    var FieldClearbit = FieldChar.extend({
        ...
        _selectCompany: function (company) {
            ...
            var enrichPromise = Clearbit.enrichCompany(company.domain);

            // Get logo
            var logoPromise = Clearbit.getCompanyLogo(company.domain);
        ...
        _suggestCompanies: function (value) {
            var self = this;
            if (value.length > 0 && Clearbit.isOnline()) {
                return Clearbit.autocomplete(value).then(function(suggestions){
