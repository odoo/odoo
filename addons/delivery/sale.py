...
class sale_order(osv.Model):
    ...
    def delivery_set(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            if not grid_id:
                raise Warning(_('No Grid Available!'), _('No grid matching for this carrier!'))

            if order.state not in ('draft', 'sent'):
                raise Warning(_('Order not in Draft State!'), _('The order state have to be draft to add delivery lines.'))
