...
class SaleOrder(models.Model):
    ...
    def _compute_partner_shipping_id(self):
        pickup_orders_with_wrong_parent = self.filtered(lambda so: so.partner_shipping_id.is_pickup_location and so.partner_shipping_id.parent_id != so.partner_id)
        if pickup_orders_with_wrong_parent:
            for order in pickup_orders_with_wrong_parent:
                order.partner_shipping_id = order.partner_shipping_id.copy({'parent_id': order.partner_id.id})
        super(SaleOrder, self - pickup_orders_with_wrong_parent)._compute_partner_shipping_id()
    ...
    def _remove_delivery_line(self):
        ...
        if self.partner_shipping_id.location_data:
            self._compute_partner_shipping_id()  # Reset the pickup location data.
        to_delete.unlink()
