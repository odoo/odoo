...
class SaleOrder(models.Model):
    ...
    delivery_method = fields.Selection([
        ...
        ('store', 'Store'),
    ], string='Delivery Type', compute='_compute_delivery_method')
    delivery_address_id = fields.Many2one(
        'res.partner', string="Delivery Address", check_company=True,
        compute='_compute_delivery_address_id', store=True, readonly=False, tracking=True)
    ...
    def _compute_delivery_method(self):
        for order in self:
            ...
            elif order.carrier_id.delivery_type == 'in_store':
    ...
    def _compute_delivery_address_id(self):
        for order in self:
            if order.delivery_method == 'classic':
    ...
    def action_open_delivery_wizard(self):
        ...
        if self.delivery_method in ['pickup_point', 'store']:
            context['default_delivery_address_id'] = self.delivery_address_id.id
        return {
            'name': name,
