...
class StockMove(models.Model):
    ...
    def _key_assign_picking(self):
        ...
        return keys + (self.picking_id.carrier_id,)
...
class StockMoveLine(models.Model):
    ...
    @api.depends('qty_done', 'product_uom_id', 'product_id', 'move_id.picking_id.sale_id.order_line.price_reduce_taxinc', 'move_id.picking_id.sale_id.order_line.product_uom')
    def _compute_sale_price(self):
        for move_line in self:
            picking = move_line.move_id.picking_id
            sale_lines = False
            if picking.sale_id and picking.location_dest_id.usage == 'customer':
                sale_lines = picking.sale_id.order_line.filtered(
                    lambda l: l.product_id == move_line.product_id)
            if sale_lines:
                unit_price = sale_lines[0].price_reduce_taxinc
                qty = move_line.product_uom_id._compute_quantity(
                    move_line.qty_done, sale_lines[0].product_uom)
            else:
