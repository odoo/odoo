...
class PaymentTransaction(models.Model):
    ...
    def _post_process(self):
        """Override of ``payment`` to confirm orders paid on delivery."""
        pod_txs = self._filtered_pending_pay_on_delivery()
        if orders := pod_txs.sale_order_ids.filtered(lambda so: so.state in {"draft", "sent"}):
    ...
    def _should_create_payment(self):
        ...
        return super()._should_create_payment() and not bool(
            self._filtered_pending_pay_on_delivery()
        )

    def _filtered_pending_pay_on_delivery(self):
