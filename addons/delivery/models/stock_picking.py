...
class StockPicking(models.Model):
    ...
    def _get_upstream_documents_and_responsibles(self, visited):
        return [(self, self.env.user, visited)]
    ...
    def _send_confirmation_email(self):
        ...
        def _carrier_exception_note(picking, exception):
            line_1 = _("Exception occurred with respect to carrier on the picking")
            line_2 = _("Manual actions might be needed.")
            line_3 = _("Exception:")
            return Markup('<div> {line_1} <a href="#" data-oe-model="stock.picking" data-oe-id="{picking_id}"> {picking_name}</a>. {line_2}<div class="mt16"><p>{line_3} {exception}</p></div></div>').format(line_1=line_1, line_2=line_2, line_3=line_3, picking_id=picking.id, picking_name=picking.name, exception=exception)
            ...
            try:
                ...
                if processed_carrier_picking:
                    ...
                    pick.sudo().activity_schedule(
                        ...
                        date.today(),
                        note=_carrier_exception_note(pick, exception_message),
                        user_id=self.env.user.id or SUPERUSER_ID,
                        )
                else:
                    raise e
