...
class PaymentProvider(models.Model):
    ...
    def copy_data(self, default=None):
        ...
        for provider, vals in zip(self, vals_list):
            if provider.custom_mode == "cash_on_delivery":
                vals["is_live"] = True
        return vals_list
    ...
    def _find_available_providers(
        ...
        reason = None
        sale_order = sale_order_id and self.env["sale.order"].browse(sale_order_id).exists()
        ...
        elif sale_order.currency_id.compare_amounts(amount, sale_order.amount_unpaid) < 0:
            reason = self.env._(
