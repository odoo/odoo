...
class TestDeliveryCost(DeliveryCommon):
    ...
    def test_01_taxes_on_delivery_cost(self):
        ...
        self.env.user.sudo().group_ids += self.env.ref("product.group_product_pricelist")
    ...
    def test_delivery_product_taxes_on_branch(self):
        ...
        # branch company + taxes are master-data setup -> sudo
        branch = self.env["res.company"].sudo().create({
            "name": "Branch",
            "country_id": company.country_id.id,
            "parent_id": company.id,
        })
        # Multi-company test: grant the test user access to the branch company
        self.env.user.sudo().company_ids += branch
    ...
    def test_base_on_rule_currency_is_converted(self):
        ...
        # Multi-company test: grant the test user access to the extra company
        self.env.user.sudo().company_ids += nook_inc
