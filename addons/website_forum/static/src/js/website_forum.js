...
publicWidget.registry.websiteForum = publicWidget.Widget.extend({
    ...
    _onExpandAnswerClick: function (ev) {
        ...
        if (ev.target === expandableWindow.querySelector('.o_wforum_expand_toggle')) {
            expandableWindow.classList.toggle('o_expand min-vh-100', true);
            expandableWindow.classList.toggle('w-lg-50');
        } else if (ev.target === expandableWindow.querySelector('.o_wforum_discard_btn')){
            expandableWindow.classList.toggle('o_expand min-vh-100', false);
