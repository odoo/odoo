...
class TestAttachmentController(HttpCase):
    ...
    def test_01_portal_attachment(self):
        ...
        response = self.url_open('/web_editor/media/add_data', data=json.dumps(payload), headers=self.headers, timeout=60000)
    ...
    def test_02_admin_attachment(self):
        ...
        attachment = self.env['web_editor.media'].create([{'name': 'test_pixel', 'public': True, 'res_id': False,
                                                        'mimetype': 'text/plain', 'res_model': 'forum.post',
                                                        'datas': self.pixel, 'website_id': self.env.ref('website.default_website').id}])
        ...
        self.assertEqual(result.id, attachment._get_attachment().id)
