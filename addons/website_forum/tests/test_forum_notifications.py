...
class TestForumNotification(MailCase, TestForumCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['mail.alias.domain'].create({"name": "test.com", "default_from": "notifications"})
    ...
    def test_post_create_question(self):
        ...
        self.assertSentEmail(f'"{self.env.user.partner_id.name}" <notifications@test.com>', self.partner_3)
    ...
    def test_post_create_answer(self):
        ...
        self.assertSentEmail(f'"{self.env.user.partner_id.name}" <notifications@test.com>', self.partner_1)
        self.assertSentEmail(f'"{self.env.user.partner_id.name}" <notifications@test.com>', self.partner_3)
    ...
    def test_post_comment_answer(self):
        ...
        self.assertSentEmail(f'"{self.env.user.partner_id.name}" <notifications@test.com>', self.partner_2)
        self.assertEqual(len(self._new_mails), 1)
        self.assertIn("comment body", self._new_mails.body_html)
