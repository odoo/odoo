...
class res_partner(osv.osv):
    ...
    def _purchase_invoice_count(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for partner_id in ids:
            for field, getter in {
                'purchase_order_count': lambda: (
                    PurchaseOrder.search_count(cr,uid, [('partner_id', 'child_of', partner_id)], context=context)
                ),
                'supplier_invoice_count': lambda: (
                    Invoice.search_count(cr,uid, [('partner_id', 'child_of', partner_id), ('type','=','in_invoice')], context=context)
                ),
            }.items():
                try:
