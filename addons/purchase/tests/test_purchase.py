...
class TestPurchase(AccountTestInvoicingCommon):
    ...
    def test_vendor_price_by_purchase_order_company(self):
        ...
        self.product_a.create({'seller_ids': [
            (0, 0, {'partner_id': self.partner_a, 'product_code': 'A', 'company_id': company_a.id, 'price': 10.0}),
            (0, 0, {'partner_id': self.partner_a, 'product_code': 'B', 'company_id': company_b.id, 'price': 15.0}),
        ]})

        po = self.env['purchase.order'].with_company(company_b).create({
            'partner_id': self.partner_a.id,
            'company_id': company_b.id,
            'order_line': [Command.create({
                'name': self.product_a.name,
