...
class TestValuationReconciliation(ValuationReconciliationTestCase):
    ...
    def check_reconciliation(self, invoice, purchase_order):
        ...
        valuation_line = self.env['stock.picking'].search([('purchase_id','=',purchase_order.id)]).move_lines.stock_account_valuation_account_move_id.line_ids.filtered(lambda x: x.account_id == self.input_account)
