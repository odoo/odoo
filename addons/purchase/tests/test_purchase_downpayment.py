...
class TestPurchaseDownpayment(TestPurchaseToInvoiceCommon):
    ...
    def test_downpayment_basic(self):
        ...
        self.assertRecordValues(generated_bill.invoice_line_ids, [
            ...
            {'product_id': self.product_order.id, 'display_type': 'product',      'quantity': 10, 'balance': 10.0 * self.product_order.standard_price, 'account_id': custom_expense_account.id},
            {'product_id': False,                 'display_type': 'line_section', 'quantity': 0,  'balance': 0.0, 'account_id': False},
            {'product_id': False,                 'display_type': 'downpayment',  'quantity': -1, 'balance': -69.0, 'account_id': custom_expense_account.id},
        ])
