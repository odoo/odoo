                ...
                for move in line.move_ids:
                    if move.state == 'done':
                        if move.product_uom != line.product_uom:
                            total += move.product_uom._compute_qty_obj(move.product_uom_qty, line.product_uom)
                        else:
    ...
    def _get_bom_delivered(self, line):
        ...
        if 'mrp.bom' in self.env:
            ...
            for bom in line.product_id.product_tmpl_id.bom_ids:
                ...
                product_uom_qty_bom = line.product_uom._compute_qty_obj(line.product_qty, bom.product_uom)
...
class ProcurementOrder(models.Model):
    ...
    def _prepare_purchase_order_line(self, po, supplier):
        ...
        procurement_uom_po_qty = self.product_uom._compute_qty_obj(self.product_qty, self.product_id.uom_po_id)
        seller = self.product_id._select_seller(
        ...
        for procurement in self:
            ...
            for line in po.order_line:
                if line.product_id == procurement.product_id and line.product_uom == procurement.product_id.uom_po_id:
                    procurement_uom_po_qty = procurement.product_uom._compute_qty_obj(procurement.product_qty, procurement.product_id.uom_po_id)
                    seller = procurement.product_id._select_seller(
                        partner_id=partner,
