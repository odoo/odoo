...
class ResConfigSettings(models.TransientModel):
    ...
    lock_confirmed_po = fields.Boolean("Lock Confirmed Orders", default=lambda self: self.env['res.company']._get_current_company().po_lock == 'lock')
    po_lock = fields.Selection(related='company_id.po_lock', string="Purchase Order Modification *", readonly=False)
    po_order_approval = fields.Boolean("Purchase Order Approval", default=lambda self: self.env['res.company']._get_current_company().po_double_validation == 'two_step')
