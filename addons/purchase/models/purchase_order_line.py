...
class PurchaseOrderLine(models.Model):
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            if values.get('price_unit') and not values.get('price_unit_manual'):
                values['price_unit_manual'] = values['price_unit']
    ...
    def _compute_price_unit_and_date_planned_and_name(self):
        for line in self:
            if not line.product_id or line.invoice_lines or not line.company_id or self.env.context.get('skip_uom_conversion') or (line.price_unit_manual != line.price_unit):
                continue
            params = line._get_select_sellers_params()
