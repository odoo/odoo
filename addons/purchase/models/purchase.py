...
class PurchaseOrder(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', required=True, index=True, states=READONLY_STATES, default=lambda self: self.env['res.company']._get_current_company().id)
    ...
    def onchange_partner_id(self):
        if not self.partner_id:
            ...
            self.currency_id = self.partner_id.property_purchase_currency_id.id or self.env['res.company']._get_current_company().currency_id.id
        return {}
        ...
        for order in self:
            ...
            if order.company_id.po_double_validation == 'one_step'\
                    or (order.company_id.po_double_validation == 'two_step'\
                        and order.amount_total < self.env['res.company']._get_current_company().currency_id._convert(
    ...
    def _add_supplier_to_product(self):
        ...
        for line in self.order_line:
            ...
            if partner not in line.product_id.seller_ids.mapped('name') and len(line.product_id.seller_ids) <= 10:
                currency = partner.property_purchase_currency_id or self.env['res.company']._get_current_company().currency_id
                supplierinfo = {
                    'name': partner.id,
