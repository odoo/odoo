...
class PurchaseOrder(models.Model):
    ...
    def action_create_invoice(self):
        ...
        for order in self:
            ...
            order = order.with_context(force_company=order.company_id)
            ...
            for line in order.order_line:
                if line.display_type == 'line_section':
                    ...
                    continue
                if not float_is_zero(line.qty_received, precision_digits=precision):
                    if pending_section:
                        invoice_vals['invoice_line_ids'].append((0, 0, pending_section._prepare_account_move_line(False)))
                        pending_section = None
                    invoice_vals['invoice_line_ids'].append((0, 0, line._prepare_account_move_line(False)))
        ...
        for vals in invoice_vals_list:
            moves |= AccountMove.with_context(force_comany=vals['company_id']).create(vals)
    ...
    def action_view_invoice(self, invoices=False):
        ...
        result = self.env.ref('account.action_move_in_invoice_type').read()[0]
