        ...
        for line in new_po.order_line:
            if line.product_id:
                seller = line.product_id._select_seller(
                    ...
                    date=line.order_id.date_order and line.order_id.date_order.date(), uom_id=line.uom_id)
    ...
    def _add_supplier_to_product(self):
        ...
        for line in self.order_line:
            ...
            if line.product_id and partner not in line.product_id.seller_ids.partner_id and len(line.product_id.seller_ids) <= 10:
                ...
                if line.product_id.product_tmpl_id.uom_po_id != line.uom_id:
...
class PurchaseOrderLine(models.Model):
    ...
    @api.depends('product_qty', 'uom_id')
        ...
        for line in self:
            ...
            if not seller:
                ...
                if not unavailable_seller and line.price_unit and line.uom_id == line._origin.uom_id:
                    ...
                    continue
                po_line_uom = line.uom_id or line.product_id.uom_po_id
            ...
            line.price_unit = seller.uom_id._compute_price(price_unit, line.uom_id)
    ...
    def _compute_product_packaging_id(self):
        for line in self:
            ...
            if line.product_id and line.product_qty and line.uom_id:
                line.product_packaging_id = line.product_id.packaging_ids.filtered('purchase')._find_suitable_product_packaging(line.product_qty, line.uom_id) or line.product_packaging_id

    @api.onchange('product_packaging_id')
    def _onchange_product_packaging_id(self):
        if self.product_packaging_id and self.product_qty:
            newqty = self.product_packaging_id._check_qty(self.product_qty, self.uom_id, "UP")
            if float_compare(newqty, self.product_qty, precision_rounding=self.uom_id.rounding) != 0:
                return {
                    'warning': {
                        'title': _('Warning'),
                        'message': _(
    ...
    @api.depends('product_packaging_id', 'uom_id', 'product_qty')
    ...
    def _prepare_add_missing_fields(self, values):
        ...
        res = {}
        onchange_fields = ['name', 'price_unit', 'product_qty', 'uom_id', 'taxes_id', 'date_planned']
