...
class TestL10nEsEdiVerifactuPosFrontend(TestL10nEsEdiVerifactuPosCommon, TestPointOfSaleHttpCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # Ensure the pos user can create Veri*Factu Documents
        # TODO: Should probably "just work"?
        cls.pos_user.groups_id = [Command.link(cls.env.ref('account.group_account_invoice').id)]
    ...
    def test_tour_order_with_refund_reason(self):
        ...
        with self._mock_zeep_registration_operation_certificate_issue():
            self.start_tour(
                f'/pos/web?config_id={self.config.id}',
