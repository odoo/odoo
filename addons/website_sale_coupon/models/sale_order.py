...
class SaleOrder(models.Model):
    ...
    def _compute_website_order_line(self):
        ...
        for program in self._get_applied_programs_with_rewards_on_current_order():
            program_lines = self.order_line.filtered(lambda line: line.product_id == program.discount_line_product_id)
            if len(program_lines) > 1:
                price_unit = sum(l.price_subtotal if self.env.user.has_group('sale.group_show_price_subtotal') else l.price_total for l in program_lines)
                self.website_order_line += self.env['sale.order.line'].new({
                ...
                self.website_order_line -= program_lines
