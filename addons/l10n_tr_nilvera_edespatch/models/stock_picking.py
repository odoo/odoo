...
class StockPicking(models.Model):
    ...
    def _compute_l10n_tr_nilvera_despatch_scenario(self):
        self.filtered(
            lambda p: p.company_id.account_fiscal_country_id.code == 'TR'
    ...
    def _compute_l10n_tr_nilvera_despatch_state(self):
        self.filtered(
            lambda p: p.state == 'done' and p.company_id.account_fiscal_country_id.code == 'TR'
    ...
    def _l10n_tr_validate_edelivery_fields(self):
        ...
        error_messages = {
            **partners._l10n_tr_nilvera_validate_partner_details(self.partner_id.id)
        }
        ...
        if drivers := len(invalid_country_drivers):
            error_messages["invalid_driver_country"] = {
                ...
                "action_text": _(
                    "View %s",
                    drivers == 1 and invalid_country_drivers.name or "Drivers",
                ),
        ...
        if drivers := len(invalid_tckn_drivers):
            error_messages["invalid_driver_tckn"] = {
                "message": (
                    drivers == 1
                    and _("%s's", invalid_tckn_drivers.name)
                    or _("Drivers")
                )
                + _(" TCKN is required."),
    ...
    def action_export_l10n_tr_delivery_note(self):
        errors = []
        for picking in self:
            if picking.company_id.account_fiscal_country_id.code == 'TR' and picking.picking_type_code == 'outgoing':
