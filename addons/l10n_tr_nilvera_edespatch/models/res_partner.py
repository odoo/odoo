...
class ResPartner(models.Model):
    ...
    l10n_tr_nilvera_edespatch_customs_zip = fields.Char(
        string="Customs ZIP",
        size=5,
        compute='_compute_l10n_tr_nilvera_edespatch_customs_zip',
        store=True,
        readonly=False
    )

    @api.depends('country_id')
    def _compute_l10n_tr_nilvera_edespatch_customs_zip(self):
        for partner in self:
            if partner.country_id and partner.country_id.code == 'TR':
                partner.l10n_tr_nilvera_edespatch_customs_zip = False
    ...
    def _l10n_tr_nilvera_validate_partner_details(self, delivery_partner_id):
        ...
        for record in self:
            ...
            if (
                record.country_id.code != "TR"
                and record.l10n_tr_nilvera_edespatch_customs_zip
                and len(record.l10n_tr_nilvera_edespatch_customs_zip) != 5
            ):
