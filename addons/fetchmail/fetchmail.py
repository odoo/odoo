...
class email_server(osv.osv):
    ...
    _columns = {
        ...
        'attach':fields.boolean('Add Attachments ?', required=False, help="Fetches attachments from mail if true."),
        ...
        'action_id':fields.many2one('ir.actions.server', 'Reply Email', required=False, domain="[('state','=','email')]", 
                                    help="A reply e-mail will be sent to specified email address here whenever an e-mail is fetched from server."),
        'object_id': fields.many2one('ir.model', "Model", required=True, help="Generates a record for each mail in selected model."),
