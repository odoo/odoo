...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        # When a parsing error occurs with Nuvei or the user cancels the order,
        # they do not send the checksum back, as such we need to pass the token in the url.
        cancel_error_url_params = {
            'tx_ref': self.reference,
            'return_access_tkn': payment_utils.generate_access_token(
                self.provider_id.nuvei_secret_key, self.reference
            ),
        }
        ...
        # Some payment methods don't support float values, even for currencies that does.
        # Therefore we must round them.
        mandatory_integer_pm = self.payment_method_code in const.INTEGER_METHODS
        rounding = 0 if mandatory_integer_pm else self.currency_id.decimal_places
    ...
    def _process_notification_data(self, notification_data):
        ...
        if status in const.PAYMENT_STATUS_MAPPING['pending']:
            ...
            self._set_error(_(
                "An error occurred during the processing of your payment (%(reason)s). "
                "Please try again.",
