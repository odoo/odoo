...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[('nuvei', "Nuvei Payment Provider")], ondelete={'nuvei': 'set default'}
    )
    ...
    def _nuvei_calculate_signature(self, data, incoming=True):
        ...
        :param bool incoming: If the signature must be generated for an incoming (Nuvei to Odoo)
                              or outgoing (Odoo to Nuvei) communication.
        :return: The calculated signature.
        :rtype: str
        """
        self.ensure_one()
        signature_keys = const.INCOMING_KEYS if incoming else data.keys()
