...
class NuveiController(http.Controller):
    ...
    def nuvei_return_from_checkout(self, tx_ref=None, return_access_tkn=None, **data):
        ...
        self._verify_notification_signature(tx_sudo, data, return_access_tkn)
    ...
    @staticmethod
    def _verify_notification_signature(tx_sudo, notification_data, token=None):
        ...
        if token:
            # We only enter this flow on errored or canceled order because Nuvei doesn't send
            # the checksum on these cases.
            secret = tx_sudo.provider_id.nuvei_secret_key
            ref = tx_sudo.reference
            if not payment_utils.check_access_token(token, secret, ref):
