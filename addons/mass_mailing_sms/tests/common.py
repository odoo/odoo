...
class MockSMS(sms_common.MockSMS):
    ...
    def assertSMSOutgoingStatistics(self, partners, numbers, records, mailing):
        ...
        found_stats = self.env['sms.statistics'].sudo().search([
            ('sms_sms_id_int', 'in', found_sms.ids),
        ])
        self.assertEqual(len(found_sms), len(found_stats))
        self.assertTrue(all(s.state == 'outgoing' for s in found_stats))
        self.assertTrue(all(s.res_model == records._name for s in found_stats))
        self.assertEqual(set(found_stats.mapped('res_id')), set(records.ids))
        self.assertTrue(all(s.mass_mailing_id == mailing for s in found_stats))
    ...
    def assertSMSStatistics(self, recipients_info, mailing, records, check_sms=True):
        ...
        """
        statistics = self.env['sms.statistics'].search([('mass_mailing_id', 'in', mailing.ids), ('res_id', 'in', records.ids)])

        self.assertTrue(all(s.res_model == records._name for s in statistics))
        self.assertTrue(all(s.utm_campaign_id == mailing.campaign_id for s in statistics))
        self.assertEqual(set(s.res_id for s in statistics), set(records.ids))
        ...
        for recipient_info in recipients_info:
            ...
            notif = statistics.filtered(lambda s: s.sms_number == number and s.state == state)
