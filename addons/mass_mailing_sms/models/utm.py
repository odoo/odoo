...
class UtmCampaign(models.Model):
    ...
    def _compute_mailing_sms_count(self):
        mailing_sms_data = self.env['mailing.mailing']._aggregate(
            [('campaign_id', 'in', self.ids), ('mailing_type', '=', 'sms')],
            ['*:count'],
            ['campaign_id', 'ab_testing_enabled'],
        )
        for campaign in self:
            campaign_id = campaign._origin.id or campaign.id
            campaign.ab_testing_mailings_sms_count = mailing_sms_data.get_agg((campaign_id, True), '*:count', 0)
            campaign.mailing_sms_count = mailing_sms_data.get_agg((campaign_id, False), '*:count', 0) + mailing_sms_data.get_agg((campaign_id, True), '*:count', 0)
