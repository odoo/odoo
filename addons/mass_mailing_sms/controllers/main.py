...
class MailingSMSController(http.Controller):
    ...
    def blacklist_page(self, mailing_id, trace_code, **post):
        ...
        if sms_number:
            ...
            ) or sms_number  # Use original if sanitization fails

            trace = check_res['trace'].filtered(lambda r: r.sms_number == sanitized_number)[:1] if sanitized_number else False
            ...
            unsubscribe_error = _('Invalid number %s', sms_number) if not sanitized_number else _('Number %s not found', sanitized_number)
    ...
    def blacklist_number(self, mailing_id, trace_code, **post):
        ...
        sms_number = post.get('sms_number')
