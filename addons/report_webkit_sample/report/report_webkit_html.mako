...
<body>
    ...
    <table class="dest_address">
        <tr><td ><b>${_(inv.partner_id.title and inv.partner_id.title.name) or ''|entity}  ${_(inv.partner_id.name) |entity}</b></td></tr>
        <tr><td>${_(inv.address_invoice_id.street) or ''|entity}</td></tr>
        <tr><td>${_(inv.address_invoice_id.street2) or ''|entity}</td></tr>
        <tr><td>${inv.address_invoice_id.zip or ''|entity} ${_(inv.address_invoice_id.city) or ''|entity}</td></tr>
        %if inv.address_invoice_id.country_id :
        <tr><td>${_(inv.address_invoice_id.country_id and inv.address_invoice_id.country_id.name) or ''|entity} </td></tr>
        %endif
        ...
        <tr><td>${_("VAT")}: ${_(inv.partner_id.vat) or ''|entity}</td></tr>
        %endif
    </table>
    <br />
    ...
    <table class="basic_table" width="90%">
        ...
        <tr><td>${_(inv.name) or ''}</td><td>${formatLang(inv.date_invoice, date=True)|entity}</td><td>&nbsp;</td></tr>
    </table>
    <h1><br /></h1>
    <table class="list_table"  width="90%">
        ...
        <tbody>
        <tr><td>${_(line.name)|entity}</td><td>${ ', '.join([ _(tax.name) or '' for tax in line.invoice_line_tax_id ])|entity}</td><td>${line.quantity}</td><td style="text-align:right;">${formatLang(line.price_unit)}</td><td  style="text-align:center;">${line.discount or 0.00}</td><td style="text-align:right;">${formatLang(line.price_subtotal)}</td></tr>
        %if line.note :
        <tr><td colspan="6" style="border-style:none"><pre style="font-family:Helvetica;padding-left:20px">${_(line.note) |entity}</pre></td></tr>
        %endif
        %endfor
