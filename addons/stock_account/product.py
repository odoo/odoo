...
class product_template(osv.osv):
    ...
    def get_product_accounts(self, cr, uid, product_id, context=None):
        ...
        if not all([stock_input_acc, stock_output_acc, account_valuation, journal_id]):
            raise UserError(_('Error!'), _('''One of the following information is missing on the product or product category and prevents the accounting valuation entries to be created:
    Product: %s
    ...
    def do_change_standard_price(self, cr, uid, ids, new_price, context=None):
        ...
        for rec_id in ids:
            ...
            for location in location_obj.browse(cr, uid, loc_ids, context=context):
                ...
                if not diff:
                    raise UserError(_('Error!'), _("No difference between standard price and new price!"))
