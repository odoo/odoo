...
class TestLotValuation(TestStockValuationCommon):
    ...
    def test_real_time_valuation(self):
        ...
        self.assertRecordValues(aml, [
            {'debit': 0.0, 'credit': 25.0, 'lot_id': self.lot1.id},
            {'debit': 25.0, 'credit': 0.0, 'lot_id': self.lot1.id},
            {'debit': 0.0, 'credit': 25.0, 'lot_id': self.lot2.id},
            {'debit': 25.0, 'credit': 0.0, 'lot_id': self.lot2.id},
            {'debit': 0.0, 'credit': 70.0, 'lot_id': self.lot3.id},
            {'debit': 70.0, 'credit': 0.0, 'lot_id': self.lot3.id},
            {'debit': 0.0, 'credit': 10.0, 'lot_id': self.lot1.id},
            {'debit': 10.0, 'credit': 0.0, 'lot_id': self.lot1.id},
            ])
    ...
    def test_disable_lot_valuation(self):
        ...
        self.assertEqual(self.product1.value_svl, 250)
        self.assertEqual(self.product1.quantity_svl, 25)
        self.assertEqual(len(self.product1.stock_valuation_layer_ids), 15)
    ...
    # def test_enable_lot_valuation_fifo(self):
    #     """ Enabling lot valuation will split current value on current stock.
    #         product valuation is fifo """
    #     cls.product1.product_tmpl_id.categ_id.property_cost_method = 'fifo'
    #     cls.product1.product_tmpl_id.standard_price = 10

    #     cls.product1.is_lot_valuated = False

    #     self._make_in_move(self.product1, 10, 5, [self.lot1, self.lot2])
    #     self._make_in_move(self.product1, 10, 7, [self.lot3])
    #     self._make_out_move(self.product1, 2, [self.lot1])
    #     self._make_out_move(self.product1, 2, [self.lot3])
    #     self._make_in_move(self.product1, 9, 8, [self.lot1, self.lot2, self.lot3])
    ...
    #     cls.product1.lot_valuated = False
    ...
    def test_lot_change_lot_after_done(self):
        ...
        update the valuation accordingly."""
        move = self._make_in_move(self.product1, 8, 5, create_picking=True, lot_ids=[self.lot1, self.lot2])
        move.picking_id.action_toggle_is_locked()
        move.move_line_ids = [
        ...
        move = self._make_out_move(self.product1, 8, 5, create_picking=True, lot_ids=[self.lot1, self.lot2])
        ...
        move.picking_id.action_toggle_is_locked()
        move.move_line_ids = [
            Command.update(move.move_line_ids[1].id, {'quant_id': quant.id}),
        ]
        self.assertRecordValues(move.stock_valuation_layer_ids, [
            {'value': -20, 'lot_id': self.lot1.id, 'quantity': -4},
            {'value': -20, 'lot_id': self.lot2.id, 'quantity': -4},
