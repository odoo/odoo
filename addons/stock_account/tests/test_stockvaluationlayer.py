...
class TestStockValuationStandard(TestStockValuationCommon):
    ...
    def test_multicompany(self):
        ...
        other_currency = self.env['res.currency'].sudo().create({
            'name': 'Other curr',
            'symbol': 'O',
            'rounding': 0.1,
            'rate_ids': [
        ...
        # SETUP: changing a company's currency is master-data configuration -> sudo
        self.other_company.sudo().currency_id = other_currency
        self.product.invalidate_recordset(['total_value'])
...
class TestStockValuationFIFO(TestStockValuationCommon):
    ...
    def test_fifo_avg_cost_fallback_zero_valued_qty(self):
        ...
        move_in.move_line_ids.owner_id = self.env['res.partner'].sudo().create({'name': 'External Owner'})
        ...
        move_out.move_line_ids.owner_id = self.env['res.partner'].sudo().create({'name': 'External Owner'})
        self.product.invalidate_recordset(['total_value', 'qty_available'])
    ...
    def test_fifo_consignment_valuation(self):
        owner = self.env['res.partner'].sudo().create({'name': 'External Owner'})
...
class TestStockValuationChangeValuation(TestStockValuationCommon):
    ...
    def test_return_delivery_fifo(self):
        self.product = self.product_fifo
        # SETUP: decimal.precision master-data -> sudo
