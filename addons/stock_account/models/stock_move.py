...
class StockMove(models.Model):
    ...
    def _account_entry_move(self, qty, description, svl_id, cost, move_directions = False):
        ...
        company_from = 'out' in move_directions and self.mapped('move_line_ids.location_id.company_id') or False
        company_to = 'in' in move_directions and self.mapped('move_line_ids.location_dest_id.company_id') or False

        journal_id, acc_src, acc_dest, acc_valuation = self._get_accounting_data_for_valuation()
        # Create Journal Entry for products arriving in the company; in case of routes making the link between several
        # warehouse of the same company, the transit location belongs to this company, so we don't need to create accounting entries
        if 'in' in move_directions:
