...
class StockMove(models.Model):
    ...
    def product_price_update_before_done(self, forced_qty=None):
        ...
        for move in self:
            ...
            if float_is_zero(product_tot_qty_available, precision_rounding=rounding) \
                ...
                new_std_price = ((amount_unit * product_tot_qty_available) + (move_cost * qty)) / (product_tot_qty_available + qty)
            ...
            for lot, qty in quantity_by_lot.items():
                ...
                if float_is_zero(qty_avail, precision_rounding=rounding) \
                    ...
                    new_std_price = ((amount_unit * qty_avail) + (move_cost * qty)) / (qty_avail + qty)
    ...
    def _get_in_svl_vals(self, forced_quantity):
        ...
        for move in self:
            ...
            if move.product_id.lot_valuated:
                vals = []
                for lot_id, qty in quantities.items():
                    vals.append(move.product_id._prepare_in_svl_vals(qty, unit_cost, lot_id=lot_id))
            else:
                vals = [move.product_id._prepare_in_svl_vals(sum(quantities.values()), unit_cost)]
            for val in vals:
                val.update(move._prepare_common_svl_vals())
                if forced_quantity:
                    val['description'] = _('Correction of %s (modification of past move)', move.picking_id.name or move.name)
    ...
    def _generate_valuation_lines_data(self, partner_id, qty, debit_value, credit_value, debit_account_id, credit_account_id, svl_id, description):
        ...
        line_vals['lot_id'] = svl.lot_id.id
