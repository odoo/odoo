...
from odoo.tools import float_round, float_compare, float_is_zero, OrderedSet
...
class StockMove(models.Model):
    ...
    def _get_price_unit(self):
        ...
        if self.origin_returned_move_id and self.origin_returned_move_id.sudo().stock_valuation_layer_ids:
            ...
            price_unit = float_round(sum(layers.mapped("value")) / quantity if not float_is_zero(
                quantity, precision_rounding=layers.uom_id.rounding) else 0, precision_digits=product_price_digits)
            return price_unit
