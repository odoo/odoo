...
class StockMoveLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        for move_line in move_lines:
            ...
            diff = move.uom_id._compute_quantity(move_line.qty_done, move.product_id.uom_id)
    ...
    def write(self, vals):
        ...
        if 'qty_done' in vals:
            for move_line in self:
                ...
                move = move_line.move_id
                rounding = move.product_id.uom_id.rounding
                diff = move.uom_id._compute_quantity(vals['qty_done'] - move_line.qty_done, move.product_id.uom_id)
                if float_is_zero(diff, precision_rounding=rounding):
                    continue
                self._create_correction_svl(move, diff)
