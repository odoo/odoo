...
class ProductValue(models.Model):
    ...
    new_value = fields.Monetary(string='New Value', currency_field='currency_id', default=0.0)
    ...
    def write(self, vals):
        ...
        if 'date' in vals or 'new_cost' in vals:
            for pv in self:
                if pv.move_id:
                    if 'new_cost' in vals:
                        vals.update({
                            'new_value': vals['new_cost'] * pv.quantity,
                            'value': vals['new_cost'] * pv.move_id.quantity,
                        })
                    move_ids.add(pv.move_id.id)
                else:
                    if 'new_cost' in vals:
        ...
        for pv in self:
            if pv.account_move_id and (pv.account_move_id.date != pv.date.date() or pv.account_move_id.amount_total != abs(pv.adjustment)):
