...
class ProductProduct(models.Model):
    ...
    def do_change_standard_price(self, new_price, account_id):
        ...
        locations = self.env['stock.location'].search([('usage', '=', 'internal'), ('company_id', '=', self.env['res.company']._get_current_company().id), ('id', 'in', quant_loc_ids)])
    ...
    def _compute_stock_value(self):
        ...
        params = (self.env['res.company']._get_current_company().id,)
        if to_date:
            ...
            if product.cost_method in ['standard', 'average']:
                ...
                if to_date:
                    price_used = product.get_history_price(
                        self.env['res.company']._get_current_company().id,
                        date=to_date,
                    )
