...
class ProductProduct(models.Model):
    ...
    def _get_prefetch_move_fields(self):
        return ['date', 'is_dropship', 'is_in', 'is_out', 'location_dest_id', 'location_id', 'move_line_ids', 'picked', 'value', 'product_id', 'manual_value_id']

    def _get_prefetch_move_line_fields(self):
        return ['company_id', 'location_id', 'location_dest_id', 'lot_id', 'owner_id', 'picked', 'quantity_product_uom']
    ...
    def _run_average_batch(self, at_date=None, lot=None, force_recompute=False):
        ...
        move_fields = self._get_prefetch_move_fields()
        move_line_fields = self._get_prefetch_move_line_fields()
        ...
        for product, move_ids in move_ids_by_product.items():
            ...
            product_data[product]['quantity'] = quantity_by_product_id.get(product.id, 0)
            product_data[product]['average_cost'] = std_price_by_product_id.get(product.id, first_move.value / first_move._get_valued_qty() if first_move._get_valued_qty() else 0)
            product_data[product]['value'] = value_by_product_id.get(product.id, 0)
            ...
            for move in moves_batch:
                ...
                if move.is_in or move.is_dropship:
                    ...
                    if at_date or move.is_dropship:
                        in_value = move._get_value(at_date=at_date, forced_std_price=product_data[product]['average_cost'])
                    if lot:
                        lot_qty = move._get_valued_qty(lot)
                        in_value = (in_value * lot_qty / in_qty) if in_qty else 0
                        in_qty = lot_qty
                    previous_qty = product_data[product]['quantity']
                    product_data[product]['quantity'] += in_qty
                    # Regular case, value from accumulation
                    if previous_qty > 0:
                        product_data[product]['value'] += in_value
                        product_data[product]['average_cost'] = product_data[product]['value'] / product_data[product]['quantity']
                    # From negative quantity case, value from last_in
                    elif previous_qty <= 0:
                        product_data[product]['average_cost'] = in_value / in_qty if in_qty else product_data[product]['average_cost']
                        product_data[product]['value'] = product_data[product]['average_cost'] * product_data[product]['quantity']
                if move.is_out or move.is_dropship:
                    out_qty = move._get_valued_qty()
                    out_value = out_qty * product_data[product]['average_cost']
                    if lot:
                        lot_qty = move._get_valued_qty(lot)
                        out_value = (out_value * lot_qty / out_qty) if out_qty else 0
                        out_qty = lot_qty
                    product_data[product]['value'] -= out_value
                    product_data[product]['quantity'] -= out_qty

            self.env['stock.move'].invalidate_model()  # Avoid keeping too many records in cache
            self.env['stock.move.line'].invalidate_model()

        for product in move_ids_by_product.keys():
            std_price_by_product_id[product.id] = product_data[product]['average_cost']
            value_by_product_id[product.id] = product_data[product]['value']
