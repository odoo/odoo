            ...
            for move in moves_batch:
                ...
                if move.is_in or move.is_dropship:
                    ...
                    if at_date or move.is_dropship:
                        in_value = move._get_value(at_date=at_date, forced_std_price=std_price_by_product_id[product.id])
                    if lot:
                        lot_qty = move._get_valued_qty(lot)
                        in_value = (in_value * lot_qty / in_qty) if in_qty else 0
                        in_qty = lot_qty
                    previous_qty = quantity_by_product_id[product.id]
                    quantity_by_product_id[product.id] += in_qty
                    # Regular case, value from accumulation
                    if previous_qty > 0:
                        value_by_product_id[product.id] += in_value
                        std_price_by_product_id[product.id] = value_by_product_id[product.id] / quantity_by_product_id[product.id]
                    # From negative quantity case, value from last_in
                    elif previous_qty <= 0:
                        std_price_by_product_id[product.id] = in_value / in_qty if in_qty else std_price_by_product_id[product.id]
                        value_by_product_id[product.id] = std_price_by_product_id[product.id] * quantity_by_product_id[product.id]
                if move.is_out or move.is_dropship:
                    out_qty = move._get_valued_qty()
                    out_value = out_qty * std_price_by_product_id[product.id]
                    if lot:
                        lot_qty = move._get_valued_qty(lot)
                        out_value = (out_value * lot_qty / out_qty) if out_qty else 0
                        out_qty = lot_qty
                    value_by_product_id[product.id] -= out_value
                    quantity_by_product_id[product.id] -= out_qty
