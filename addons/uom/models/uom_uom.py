...
class UomUom(models.Model):
    ...
    def _unprotected_uom_xml_ids(self):
        return [
            ...
            "product_uom_pack_of_6",
        ]

    name = fields.Char('Unit', required=True, translate=True)
    relative_factor = fields.Float(
        'Contains', default=1.0, digits='Product Unit of Measure', required=True,  # force NUMERIC with unlimited precision
    ...
    factor = fields.Float(
        'Absolute Quantity', compute='_compute_factor', readonly=False,
        store=True, digits=0)
