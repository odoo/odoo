...
class MassMailing(models.Model):
    ...
    def _compute_sale_invoiced_amount(self):
        for mass_mailing in self:
            if self.user_has_groups('sales_team.group_sale_salesman') and self.user_has_groups('account.group_account_invoice'):
                ...
                invoices = self.env['account.move'].search_read(domain, ['amount_untaxed'])
                mass_mailing.sale_invoiced_amount = sum(i['amount_untaxed'] for i in invoices)
            else:
    ...
    def action_redirect_to_invoiced(self):
        ...
        invoices = self.env['account.move'].search(self._get_sale_utm_domain())
        action['domain'] = [
            ('id', 'in', invoices.ids),
