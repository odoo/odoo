...
class ForumForum(models.Model):
    ...
    slide_channel_id = fields.Many2one('slide.channel', 'Course', compute='_compute_slide_channel_id', store=True, index='btree_not_null')
    ...
    def _compute_can_access(self):
        ...
        if not_accessible := self - accessible:
            super(ForumForum, not_accessible)._compute_can_access()
