...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_to_deliver(self):
        ...
        order_lines = self.filtered(lambda l: l.state in ['draft', 'sent'] and l.product_id)
        kit_boms = self.env['mrp.bom']._bom_find(order_lines.product_id, company_ids=order_lines.company_id.ids, bom_type='phantom')
        for line in self:
            ...
            if line.state == 'sale':
                ...
                boms = kit_boms[(line.product_id, line.company_id.id)]
    ...
    def _compute_qty_delivered(self):
        ...
        order_lines = self.filtered(lambda l: l.qty_delivered_method == 'stock_move')
        kit_boms = self.env['mrp.bom']._bom_find(order_lines.product_id, company_ids=order_lines.company_id.ids, bom_type='phantom')
        for order_line in order_lines:
            ...
            if not boms and dropship:
                boms = kit_boms[(order_line.product_id, order_line.company_id.id)]
            ...
            if not relevant_bom:
                relevant_bom = boms._bom_find(order_line.product_id, company_ids=order_line.company_id.ids, bom_type='phantom')[order_line.product_id, order_line.company_id.id]
            if relevant_bom:
    ...
    def _get_qty_procurement(self, previous_product_uom_qty=False):
        ...
        bom = self.env['mrp.bom']._bom_find(self.product_id, bom_type='phantom')[(self.product_id, False)]
        if bom:
