...
class SaleOrder(models.Model):
    ...
    def _get_mos_generated(self):
        """Helper to fetch the manufacture order generated within the sale order"""
        self.ensure_one()
        productions = self.env['mrp.production']
        for move in self.procurement_group_id.stock_move_ids:
            while move.move_orig_ids and not move.created_production_id:
                move = move.move_orig_ids
            productions |= move.created_production_id.procurement_group_id.mrp_production_ids
        return productions

    @api.depends('procurement_group_id.stock_move_ids.created_production_id.procurement_group_id.mrp_production_ids')
    def _compute_mrp_production_count(self):
        for sale in self:
            productions = sale._get_mos_generated()
            sale.mrp_production_count = len(productions)

    def action_view_mrp_production(self):
        self.ensure_one()
        mrp_productions = self._get_mos_generated()
        action = {
            'res_model': 'mrp.production',
            'type': 'ir.actions.act_window',
        }
        if len(mrp_productions) == 1:
            action.update({
                'view_mode': 'form',
                'res_id': mrp_productions.id,
            })
        else:
            action.update({
                'name': _("Manufacturing Orders Generated by %s", self.name),
                'domain': [('id', 'in', mrp_productions.ids)],
                'view_mode': 'tree,form',
            })
        return action
