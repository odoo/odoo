...
class WebsiteRewrite(models.Model):
        ...
        for rewrite in self:
            if rewrite.redirect_type in ['301', '302', '308']:
                if not rewrite.url_to:
                    raise ValidationError(self.env._('"URL to" cannot be empty.'))
                ...
                if rewrite.url_to.split('#')[0] == rewrite.url_from.split('#')[0]:
                    raise ValidationError(self.env._("base URL of 'URL to' should not be same as 'URL from'."))

            if rewrite.redirect_type == '308':
                if not rewrite.url_to.startswith('/'):
                    raise ValidationError(self.env._('"URL to" must start with a leading slash.'))
                for param in re.findall('/<.*?>', rewrite.url_from):
                    if param not in rewrite.url_to:
                        raise ValidationError(self.env._('"URL to" must contain parameter %s used in "URL from".', param))
                for param in re.findall('/<.*?>', rewrite.url_to):
                    if param not in rewrite.url_from:
                        raise ValidationError(self.env._('"URL to" cannot contain parameter %s which is not used in "URL from".', param))

                if rewrite.url_to == '/':
                    raise ValidationError(self.env._('"URL to" cannot be set to "/". To change the homepage content, use the "Homepage URL" field in the website settings or the page properties on any custom page.'))
                ...
                if any(
                    ...
                    raise ValidationError(self.env._('"URL to" cannot be set to an existing page.'))
    ...
    def _generate_name(url_from, url_to, redirect_type):
        if redirect_type == '404':
            return self.env._('%(url)s not found', url=url_from)
        return self.env._('%(url_from)s to %(url_to)s', url_from=url_from, url_to=url_to)
