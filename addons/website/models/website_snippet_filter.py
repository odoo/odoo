...
class WebsiteSnippetFilter(models.Model):
    ...
    def _prepare_values(self, limit=None, search_domain=None, **options):
        ...
        single_record_filter = limit == 1 and model and res_id

        # Either a multi-record filter is provided, or a single record is specified.
        if self.filter_id or single_record_filter:
            model_id = model if single_record_filter else self.filter_id.sudo().model_id
            domain = [] if single_record_filter else self.filter_id.sudo()._get_eval_domain()
            if 'website_id' in self.env[model_id]:
                domain = expression.AND([domain, self.env['website'].get_current_website().website_domain()])
            if 'company_id' in self.env[model_id]:
                website = self.env['website'].get_current_website()
                domain = expression.AND([domain, [('company_id', 'in', [False, website.company_id.id])]])
            if 'is_published' in self.env[model_id]:
                domain = expression.AND([domain, [('is_published', '=', True)]])
            if search_domain:
                domain = expression.AND([domain, search_domain])
            try:
                filter_sudo = self.filter_id.sudo()
                records = self.env[model_id].sudo(False).with_context(**literal_eval(filter_sudo.context)).search(
                    domain,
                    order=','.join(literal_eval(filter_sudo.sort)) or None,
                    limit=limit
                ) if not single_record_filter else self.env[model_id].browse([res_id])
                return self._filter_records_to_values(records.sudo(), res_model=model_id)
            except MissingError:
    ...
    def _get_filter_meta_data(self, model):
        ...
        default_field_names = self.with_context(model=model._name).default_get(['field_names']).get('field_names')
        field_names = (self.field_names or default_field_names).split(",")
        ...
        for record in records:
            ...
            for field_name, field_widget in meta_data.items():
                ...
                if field and field.type in ('binary', 'image'):
                    if options.get('is_sample', False):
