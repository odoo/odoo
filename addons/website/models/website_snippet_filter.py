...
class WebsiteSnippetFilter(models.Model):
    ...
    def _validate_template_keys(self, content_template, wrapper_template):
        if content_template and '.dynamic_filter_template_' not in content_template:
            _logger.warning("You can only use template prefixed by dynamic_filter_template_ inside website.editable_call_template_dynamic_snippet_content, not %s", content_template)
            return False

        if not wrapper_template or '.s_dynamic_snippet_wrapper_' not in wrapper_template:
            _logger.warning("You can only use template prefixed by s_dynamic_snippet_wrapper_ to wrap content in website.editable_call_template_dynamic_snippet_content, not %s", wrapper_template)
            return False
        ...
        if content_template and self.model_name and self.model_name.replace('.', '_') not in content_template:
            _logger.warning("You can only use template specific to the model of the filter inside website.editable_call_template_dynamic_snippet_content, not %s for %s", content_template, self.model_name)
            return False
