...
class WebsitePageProperties(models.TransientModel):
    ...
    parent_id = fields.Many2one(related="target_model_id.parent_id", readonly=False,
        domain="[('website_id', '=?', website_id), ('id', '!=', target_model_id), ('parent_id', '!=', target_model_id)]", help="Make sure your page is assigned to your website")
    old_url = fields.Char()
    ...
    def _onchange_has_parent_page(self):
        ...
        if self.has_parent_page:
            self.parent_id = self.parent_id or homepage
        else:
