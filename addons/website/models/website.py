...
class Website(models.Model):
    ...
    def configurator_apply(self, **kwargs):
        ...
        html_text_processor = self.env['html.text.processor'].with_processing_context(
            IrQweb=IrQweb,
            cta_data=cta_data,
        ...
        for page_code in requested_pages - {'privacy_policy'}:
            ...
            page_group_code = f'website.configurator_{page_code}'
            html_text_processor, page_generated_content, page_translated_content = html_text_processor.get_snippets_content(snippet_list, page_group_code)
            generated_content.update(page_generated_content)
            translated_content.update(page_translated_content)
        ...
        for page_code in requested_pages:
            ...
            for i, snippet in enumerate(snippet_list, start=1):
                try:
                    el = html_text_processor.update_snippet_content(f'website.configurator_{page_code}_{snippet}', generated_content, snippet)
