...
class Website(models.CachedModel):
    ...
    def configurator_recommended_themes(self, industry_id, result_nbr_max=6,
        ...
        for theme_name in themes_suggested:
            preview_url = self._get_configurator_theme_preview_url(theme_name)
            if preview_url:
                themes_with_previews.append({
                    'name': theme_name,
    ...
    def _enumerate_pages(self, query_string=None, force=False, ignore_custom_homepage=False):
        """ Available pages in the website/CMS. This is mostly used for links
            ...
            in the section named by ``@sitemap_group``, on itself or on the
            function it overrides, else after its module.
            ...
            :returns: one mapping per URL: ``loc`` is the URL, ``group`` its
                      sitemap section, ``lastmod`` its last change date when
                      known
            :rtype: Iterator[dict]
        """
