...
class WebsiteConfigSettings(models.TransientModel):
    ...
    number_of_languages = fields.Integer(string='Number of languages', compute='_compute_number_of_languages')
    ...
    has_google_analytics = fields.Boolean("Google Analytics", required=True)
    has_google_analytics_dashboard = fields.Boolean("Google Analytics in Dashboard", required=True)
    has_google_maps = fields.Boolean("Google Maps", required=True)

    @api.depends('language_ids')
    def _compute_number_of_languages(self):
        language_count = len(self.language_ids)
        for config in self:
            config.number_of_languages = language_count
