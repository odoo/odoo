...
class IrHttp(models.AbstractModel):
    ...
    def _auth_method_public(self):
        if not request.session.uid:
            env = api.Environment(request.cr, odoo.SUPERUSER_ID, context=request.context)
        ...
        if cache_enable:
            ...
            try:
                ...
                if r['time'] + cache_time > time.time():
                    cache_response = http.Response(r['content'], mimetype=r['mimetype'])
                else:
                    del request.registry.cache[key]
            except KeyError:
                pass
    ...
    def _handle_exception(self, exception, code=500):
        ...
        if not is_website_request:
            ...
            if isinstance(exception, QWebException):
                ...
                if isinstance(exception.qweb.get('cause'), AccessError):
                    code = 403
    ...
    def binary_content(self, xmlid=None, model='ir.attachment', id=None, field='datas', unique=False, filename=None, filename_field='datas_fname', download=False, mimetype=None, default_mimetype='application/octet-stream', env=None):
        ...
        _model = None
        if xmlid:
            _model = env.ref(xmlid, False)
        elif id and env.registry.get(model):
            _model = env[model].browse(int(id))
        if _model and 'website_published' in _model._fields:
            if env[_model._name].sudo().search([('id', '=', _model.id), ('website_published', '=', True)]):
