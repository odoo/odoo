...
def test_01_cow_views_inherit_on_module_update(env):
    ...
    View = env['ir.ui.view']
    View.with_context(force_delete=True, active_test=False).search([('website_id', '=', env.ref('base.default_website').id)]).unlink()
    ...
    child_view.write({
        ...
        'arch': child_view.arch_db.replace('id="language_selector_hook"', 't-if="custom_html"'),
    })
    # Trigger COW on view
...
def test_02_cow_views_inherit_on_module_update(env):
    ...
    view_D.with_context(website_id=env.ref('base.default_website').id).write({'name': 'D Website 1'})
    view_B.with_context(website_id=env.ref('base.default_website').id).write({'name': 'B Website 1'})
