...
class TestViewSavingCommon(common.TransactionCase):
    def _create_imd(self, Qweb):
        xml_id = Qweb.key.split('.')
...
class TestCustomizeView(common.HttpCase):
    ...
    def test_disabled_optional_template_t_call(self):
        ...
        # theme_customize_data will clone the default Qweb to save the configuration
        self.url_open_authenticate('/website/theme_customize_data', {'enable': [], 'disable': ['website.test_view']})
    ...
    def test_enabling_optional_template_with_editor(self):
        """ Ensure that a non-admin editor can enable a Qweb """
        website = self.env['website'].search([], limit=1)
...
class TestViewSaving(TestViewSavingCommon):
    ...
    def test_save(self):
        ...
        self.assertFalse(imd.noupdate, "Qweb's xml_id shouldn't be set to 'noupdate' in a website context as `save` method will COW")
        # remove newly created COW Qweb so next `save()`` wont be redirected to COW Qweb
        ...
        # the xml_id of the Qweb should be flagged as 'noupdate'
    ...
    def test_save_escaped_text(self):
        ...
        Qweb.save(replacement, xpath='/t/p/h1')
        self.assertIn(
        ...
        self.assertIn(
            replacement,
            self.env['ir.qweb']._render(Qweb.id),
        ...
        Qweb.save(replacement, xpath='/t/p')
        self.assertIn(replacement, Qweb.arch, 'common text node should not be escaped server side')
        self.assertIn(
            replacement,
            str(self.env['ir.qweb']._render(Qweb.id)).replace(u'&', u'&amp;'),
...
class TestCowViewSaving(TestViewSavingCommon, HttpCase):
    ...
    def test_cow_on_base_after_extension(self):
        ...
        self.assertEqual(v2.key == v3.key, True, "Making specific a generic inherited Qweb should copy it's key (just change the website_id)")
        self.assertEqual(v3.key != v4.key != v5.key, True, "Copying a Qweb should generate a new key for the new Qweb (not the case when triggering COW)")
        self.assertEqual('website.extension_view' in v3.key and 'website.extension_view' in v4.key and 'website.extension_view' in v5.key, True, "The copied views should have the key from the Qweb it was copied from but with an unique suffix")
        ...
        self.assertEqual(total_views + 4 - 2, Qweb.search_count([]), "It should have duplicated the Qweb tree with a website_id, taking only most specific (only specific `b` key), and removing website_specific from generic tree")
    ...
    def test_cow_leaf(self):
        ...
        # website-specific inactive Qweb should take preference over active generic one when viewing the website
    ...
    #     # edit on backend, regular write
    #     self.base_view.write({'arch': '<div>modified base content</div>'})
    #     self.assertEqual(self.base_view.key, False, "Writing on a keyless Qweb should not set a key on it if there is no website in context")
    #
    #     # edit on frontend, copy just the leaf
    #     self.base_view.with_context(website_id=1).write({'arch': '<div position="replace"><div>website 1 content</div></div>'})
    #     self.assertEqual('website.key_' in self.base_view.key, True, "Writing on a keyless Qweb should set a key on it if there is a website in context")
    #     total_views_with_key = QwebView.search_count([('key', '=', self.base_view.key)])
    #     self.assertEqual(total_views_with_key, 2, "It should have set the key on generic Qweb then copy to specific Qweb (with they key)")

    def test_cow_generic_view_with_already_existing_specific(self):
        """ Writing on a generic Qweb should check if a website specific Qweb already exists
            (The flow of this test will happen when editing a generic Qweb in the front end and changing more than one element)
        """
        ...
        base_view.with_context(website_id=1).write({'name': 'New Name'})  # This will not write on `base_view` but will copy it to a specific Qweb on which the `name` change will be applied
        ...
        self.assertEqual(total_views + 1, Qweb.with_context(active_test=False).search_count([]), "Subsequent writes should have written on the Qweb copied during first write")
        ...
        self.assertEqual(total_views + 1, Qweb.with_context(active_test=False).search_count([]), "Second save should have written on the Qweb copied during first save")
        ...
        self.assertEqual(total_specific_view, 1, "both editable_part should have been replaced on a created specific Qweb")
    ...
    def test_cow_complete_flow(self):
        ...
        arch = base_specific.with_context(website_id=1).get_combined_arch()
        self.assertEqual('Bye Nobody' in arch, True, "Write on generic `inherit_view` should have been diverted to already existing specific Qweb")
        ...
        self.assertEqual(base_arch, base_arch_w1, "Reading a top level Qweb with or without a website_id in the context should render that exact Qweb..")  # ..even if there is a specific Qweb for that one, as get_combined_arch is supposed to render specific inherited Qweb over generic but not specific top level instead of generic top level
    ...
    def test_cow_cross_inherit(self):
        ...
        Qweb.create({
            ...
            'arch': '<xpath expr="//div" position="replace"><div>Qweb<p>B</p></div></xpath>',
        ...
        self.assertEqual(total_views + 3 + 1, Qweb.search_count([]), "It should have created the 3 initial generic views and created a child_view_2 specific Qweb")
        main_view.with_context(website_id=1).write({'arch': '<body>SPECIFIC<div>Z</div></body>'})
        self.assertEqual(total_views + 3 + 3, Qweb.search_count([]), "It should have duplicated the Main Qweb tree as a specific tree and then removed the specific Qweb from the generic tree as no more needed")
        ...
        self.assertEqual(generic_view_arch, '<body>GENERIC<div>Qweb<span>C</span></div></body>')
        self.assertEqual(specific_view_arch, '<body>SPECIFIC<div>Qweb<span>D</span></div></body>', "Writing on top level Qweb hierarchy with a website in context should write on the Qweb and clone it's inherited views")

    def test_multi_website_view_active(self):
        ''' With the following structure:
            * A generic active parent Qweb
            * A generic active child Qweb, that is inactive on website 1
        ...
        Qweb = Qweb.with_context(active_test=False).search([('key', '=', self.inherit_view.key)]).filter_duplicate()
        self.assertEqual(Qweb.active, True, "filter_duplicate should return the generic one")
        Qweb = Qweb.with_context(active_test=False, website_id=1).search([('key', '=', self.inherit_view.key)]).filter_duplicate()
        self.assertEqual(Qweb.active, False, "filter_duplicate should return the specific one")
    ...
    def test_get_related_views_tree_recursive_t_call_and_inherit_inactive(self):
        """ If a Qweb A was doing a t-call on a Qweb B and Qweb B had Qweb C as child.
            And Qweb A had Qweb D as child.
            And Qweb D also t-call Qweb B (that as mentionned above has Qweb C as child).
            And Qweb D was inactive (`d` in bellow schema).
        ...
        views = Qweb.with_context(website_id=1).get_related_views('_website_sale.products')
        self.assertEqual(views, products + products_item + add_to_wishlist_cow + products_list_view, "The generic wishlist Qweb should have been replaced by the COW one.")
    ...
    def test_cow_inherit_children_order(self):
        """ COW method should loop on inherit_children_ids in correct order
            ...
            Correct order is the same as the one when applying Qweb arch:
            PRIORITY, ID
    ...
    def test_write_order_vs_cow_inherit_children_order(self):
        """ When both a specific inheriting Qweb and a non-specific base Qweb
            are written simultaneously, the specific inheriting base Qweb
    ...
    def test_module_new_inherit_view_on_parent_already_forked(self):
        """ If a generic parent Qweb is copied (COW) and that another module
            creates a child Qweb for that generic parent, all the COW views
            should also get a copy of that new child Qweb.

            Typically, a parent Qweb (website_sale.product) is copied (COW)
        ...
        # Simulate website_sale product Qweb
        ...
        self.assertEqual(specific_view.website_id.id, 1, "Ensure we got specific Qweb to perform the checks against")
        self.assertEqual(specific_view_arch, '<p>COMPARE</p>', "When a module creates an inherited Qweb (on a generic tree), it should also create that Qweb in the specific COW'd tree.")
        ...
        self.assertEqual(specific_view_arch, '<p>COMPARE EDITED</p>', "When a module updates an inherited Qweb (on a generic tree), it should also update the copies of that Qweb (COW).")
        ...
        self.assertEqual(generic_child_view.inherit_id, random_views[0], "prohibited fields only concerned write on COW'd Qweb. Generic should still considere these fields")
        ...
        # Set back the generic Qweb as parent for the rest of the test
        ...
        # Don't update fields from COW'd Qweb if these fields have been modified from original Qweb
        ...
        self.assertEqual(w1_specific_child_view.priority, 3, "XML update should be written on the specific Qweb if the fields have not been modified on that specific Qweb")
        self.assertEqual(new_website_specific_child_view.priority, 6, "XML update should NOT be written on the specific Qweb if the fields have been modified on that specific Qweb")

        # Simulate website_sale update on top level Qweb
    ...
    def test_module_new_inherit_view_on_parent_already_forked_xpath_replace(self):
        """ Deeper, more specific test of above behavior.
            A module install should add/update the COW Qweb (if allowed fields,
            eg not modified or prohibited (website_id, inherit_id..)).
            This test ensure it does not crash if the child Qweb is a primary Qweb.
        """
        Qweb = self.env['ir.qweb']
    ...
    def test_specific_view_translation(self):
        ...
        self.assertEqual(specific_view.with_context(lang='fr_BE').arch, '<div>bonjour</div>',
                         "updating translation of base Qweb doesn't update specific Qweb")
        ...
        self.assertEqual(specific_view.with_context(lang='fr_BE').arch, '<div>salut</div>',
                         "loading module translation copy translation from base to specific Qweb")

        self.assertEqual(specific_view.with_context(lang='es_ES').arch, '<div>hola</div>',
                         "loading module translation should not remove specific translations that are not available on base Qweb")
    ...
    def test_view_to_translate_tag(self):
        ...
        # generic Qweb without website_id but with website for request
        ...
        # generic Qweb without website_id
    ...
    def test_soc_complete_flow(self):
        ...
        self.assertEqual(len(created_views), 1, "Should only have created one Qweb")
        self.assertEqual(created_views.website_id.id, 1, "The created Qweb should be specific to website 1")

        with self.assertRaises(ValueError, msg="Should not allow to create generic Qweb explicitely from website 1 specific context"):
            Qweb.with_context(website_id=1).create({
                'name': 'Name',
        ...
        with self.assertRaises(ValueError, msg="Should not allow to create specific Qweb for website 2 from website 1 specific context"):
            Qweb.with_context(website_id=1).create({
                'name': 'Name',
    ...
    def test_specific_view_module_update_inherit_change(self):
        ...
        # 2. Ensure Qweb trees are as expected
    ...
    def test_no_cow_on_translate(self):
        ...
        Qweb = self.base_view.with_context(lang=french.code, website_id=1)
        ...
        self.assertEqual(len(old_specific_views), len(new_specific_views), "No additional specific Qweb must have been created")
        self.assertTrue(Qweb.arch.index('contenu de base') > 0, "New translation must appear in Qweb")
...
class Crawler(HttpCase):
    ...
    def test_get_switchable_related_views(self):
        ...
        self.assertEqual(
            [v['name'] for v in res],
            ['Sign In', 'Affix Top Menu', 'Show Logo', 'Filters', 'Photos', 'Quotes', 'Filter by Category', 'Filter by Country'],
            "multi-website COW should not impact customize views order (COW Qweb will have a bigger ID and should not be last)",
        )
        self.assertEqual(
            [v['inherit_id'][1] for v in res],
            ['Main Frontend Layout', 'Main layout', 'Main layout', 'Events', 'Events', 'Events', 'Filters', 'Filters'],
            "multi-website COW should not impact customize views menu header position or split (COW Qweb will have a bigger ID and should not be last)",
        )
        ...
        self.assertEqual(
            [v['name'] for v in res],
            ['Sign In', 'Affix Top Menu', 'Show Logo', 'Filters', 'Photos', 'Quotes', 'Filter by Category', 'Filter by Country'],
            "multi-website COW should not impact customize views order (COW Qweb will have a bigger ID and should not be last) (2)",
        )
        self.assertEqual(
            [v['inherit_id'][1] for v in res],
            ['Main Frontend Layout', 'Main layout', 'Main layout', 'Events', 'Events', 'Events', 'Filters', 'Filters'],
            "multi-website COW should not impact customize views menu header position or split (COW Qweb will have a bigger ID and should not be last) (2)",
        )
    ...
    def test_multi_website_views_retrieving(self):
        ...
        # Simulate theme Qweb instal + load on website
        ...
        with self.assertRaises(MissingError):
            # It should crash as it should not find a Qweb on website 1 for '_theme_kea_sale.products', !!and certainly not a theme.ir.qweb!!.
            Qweb = Qweb.with_context(website_id=website_1.id)._get_template_view('_theme_kea_sale.products')
        Qweb = Qweb.with_context(website_id=website_2.id)._get_template_view('_theme_kea_sale.products')
        self.assertEqual(len(Qweb), 1, "It should find the ir.qweb with key '_theme_kea_sale.products' on website 2..")
        self.assertEqual(Qweb._name, 'ir.qweb', "..and not a theme.ir.qweb")
        ...
        self.assertEqual(len(response.json()['result']), 1, "Only '_theme_kea_sale.products' should be returned as it is the only customize_show related Qweb in website 2 context")
        ...
        response = self.url_open(url=url, json=json)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()['result']), 1, "Only '_website_sale.child_view_w1' should be returned as it is the only customize_show related Qweb in website 1 context")
...
class TestThemeViews(common.TransactionCase):
    def test_inherit_specific(self):
        ...
        # 2. Simulate a theme install with a child Qweb of `main_view`
