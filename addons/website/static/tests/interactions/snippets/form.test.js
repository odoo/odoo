...
test("contains conditional visibility(multiple checkbox)", async () => {
    ...
    expect(core.interactions).toHaveLength(1);
    const fieldB = queryOne(".s_website_form_field:has(input[name=b])");
    const optionA = queryOne("input[value='Option 1']");
    const optionB = queryOne("input[value='Option 2']");
    const optionC = queryOne("input[value='Option 3']");
...
test("contains conditional visibility(text input)", async () => {
    ...
    expect(core.interactions).toHaveLength(1);
    const fieldB = queryOne(".s_website_form_field:has(input[name=b])");
    const fieldA = queryOne("input[name=a]");
    expect(fieldB).not.toBeVisible();
    await click(fieldA);
