import { expect, test, queryOne } from "@odoo/hoot";
...
test("add custom fields to signup form via builder options", async () => {
    ...
    const { getEditor } = await setupWebsiteBuilder(signupFormTemplate);
    ...
    setSelectionOnNodeContent(queryOne(":iframe .s_website_form_label_content:last"));
    await insertText(getEditor(), "file_1");
    ...
    await addNewField();
    setSelectionOnNodeContent(queryOne(":iframe .s_website_form_label_content:last"));
    await insertText(getEditor(), "field_1");
