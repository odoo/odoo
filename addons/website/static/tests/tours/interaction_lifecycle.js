...
registerWebsitePreviewTour("interaction_lifecycle", {
    url: "/",
    edition: true,
}, () => [
    ...insertSnippet({
    ...
    {
        content: "Wait for the interaction to be started and empty the interactionAndWysiwygLifecycle list",
        trigger: ":iframe .s_countdown.interaction_started",
        run: () => {
            // Start recording the calls to the "start" and "destroy" method of
            // the interaction and the wysiwyg.
    ...
    ...clickOnEditAndWaitEditMode(),
    {
        content: "Wait for the interaction to be started and check the order of the lifecycle method call of the interaction and the wysiwyg",
        trigger: ":iframe .s_countdown.interaction_started",
        run() {
            ...
            if (!(resultIsEqualTo(expected) || resultIsEqualTo(alternative))) {
                ...
                // the interaction at the page reload, two sequences are
