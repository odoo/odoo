...
registry.category("web_tour.tours").add("parent_child_menu", {
    ...
    steps: () => [
        ...
        },
        {
            ...
            trigger: "div[name='url']",
            run: function () {
                const url = this.anchor.textContent.trim();
                if (url !== "#") {
                    console.error(`Error: Parent URL should be '#' but found '${url}'`);
                }
            },
        },
        {
            ...
            run: function () {
                const url = this.anchor.textContent.trim();
                if (url !== "/child") {
                    console.error(`Error: Child URL should be '/child' but found '${url}'`);
                }
            },
        },
    ],
