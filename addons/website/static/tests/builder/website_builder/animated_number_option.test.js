...
test("changing font size class on the animated number display replaces the previous one", async () => {
    const { displayEl, editor, valueEl } = await setupAnimatedNumber();
    const valueTextNode = valueEl.firstChild;
    setSelection({
        anchorNode: valueTextNode,
    ...
    await animationFrame();
    expect(displayEl.classList.contains("h1-fs")).toBe(true);
    expect(displayEl.classList.contains("h2-fs")).toBe(false);

    setFontSizeClassName("display-2-fs")(editor);
    await animationFrame();
    expect(displayEl.classList.contains("display-2-fs")).toBe(true);
    expect(displayEl.classList.contains("h1-fs")).toBe(false);
