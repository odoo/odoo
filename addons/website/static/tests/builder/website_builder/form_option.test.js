...
const sendCopyOptionSelector =
    ".options-container[data-container-title='Form'] [data-action-id='prepareEmailField'][data-class-action='o_website_form_copy_enabled'] input[type='checkbox']";
const sendCopyEmailFieldSelector =
    ":iframe .s_website_form form.o_website_form_copy_enabled .s_website_form_field.s_website_form_copy_email[data-type='email']";
...
test("Send a copy option creates/removes email field and adapts on action change", async () => {
    ...
    expect(".options-container[data-container-title='Field'] .alert-info").toHaveText(
    ...
    await toggleSendCopyOption();
    expect(sendCopyEmailFieldSelector).not.toHaveClass("s_website_form_required");
});
