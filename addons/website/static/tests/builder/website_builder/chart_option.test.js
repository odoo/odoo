import { describe, expect, test, advanceTime, manuallyDispatchProgrammaticEvent } from "@odoo/hoot";
import {
    defineWebsiteModels,
    setupWebsiteBuilder,
...
test("Adding / Deleting multiple columns updates the data and available cells", async () => {
    const type = "bar";
    await setupWebsiteBuilder(chartTemplate(type, getData(type)));
    let data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
    expect(data.datasets).toHaveLength(2);
...
test("Pasting extra values extends rows and columns", async () => {
    ...
    const data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
    expect(data.labels).toHaveLength(4);
...
test("Pasting on row labels uses text and can add rows", async () => {
    ...
    const data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
    expect(data.labels).toHaveLength(4);
    expect(data.labels[0]).toBe("North");
    expect(data.labels[1]).toBe("South");
