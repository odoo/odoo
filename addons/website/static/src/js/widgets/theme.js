            ...
            _.each($items, function (item) {
                ...
                switch (item.tagName) {
                    case 'OPT':
                        ...
                        var className = $container.hasClass('form-row') ? (icon ? 'col' : (colorPalette ? 'col-12' : 'col-6')) : '';
                        var font = $item.data('font');
                        if (font) {
                            className += (' o_theme_customize_option_font_' + font);
                        }
        ...
        colorpicker.on('colorpicker:saved', this, function (ev) {
            ...
            self._rpc({
                ...
                var file = _.find(files, function (file) {
                    return file.url === '/website/static/src/scss/user_color_palette.scss';
                });
