...
publicWidget.registry.CarouselSingleScrollMode = publicWidget.Widget.extend({
    ...
    /**
     * Returns if the image that will appear is loaded or not
     *
     * @private
     */
    _isSlideLoaded(dir) {
        if (dir === "right") {
            return Array.from(
                this.carouselInnerEl.lastElementChild.querySelectorAll("img")
            ).every(img => img.complete);
        }
        if (dir === "left") {
            return Array.from(
                this.carouselInnerEl.children[
                    Math.min(this.itemsPerSlide + 1, this.carouselInnerEl.children.length-1)
                    ].querySelectorAll("img")
            ).every(img => img.complete);
        }
        return Array.from(
                this.carouselInnerEl.querySelectorAll('img[loading="eager"]')
            ).every(img => img.complete);
    },
    /**
