 ...
 * @returns {String}
 */
function _getWebsiteLabelPosition(labelEl) {
    if (!labelEl) {
        return "";
    }
    ...
    if (labelClassList.contains("d-none")) {
        return "hide";
    }
    if (labelEl.previousElementSibling?.matches(".form-check > input[type='checkbox']")) {
        return "after-checkbox";
    }
    if (labelEl.closest(".form-floating")) {
        return "floating";
    }
    if (labelEl.nextElementSibling.matches("input, .form-control, .form-select, .form-check-input, .s_website_form_multiple")) {
        return "top";
    }
    return "";
}
...
export function adaptFormLabel(formEl, newPosition, width) {
    for (const inputEl of formEl.querySelectorAll(
        ...
        const oldPosition = _getWebsiteLabelPosition(labelEl);
