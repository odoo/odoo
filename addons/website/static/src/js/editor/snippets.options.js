...
class GoogleFontAutoComplete extends AutoComplete {
    setup() {
        super.setup();
        // Prevent from closing when clicking on the scrollbar.
        const sourcesList = useRef("sourcesList");
        const onMouseEnter = (ev) => {
            this.ignoreBlur = true;
        };
        const onMouseLeave = (ev) => {
            this.ignoreBlur = false;
            if (document.hasFocus(ev.target)) {
...
const FontFamilyPickerUserValueWidget = SelectUserValueWidget.extend({
    ...
    start: async function () {
        ...
        if (this.uploadedLocalFonts.length) {
            ...
            uploadedLocalFontsEls.forEach((el, index) => {
                $(el).append(renderToFragment('website.delete_font_btn', {
                    index: index,
                    local: "uploaded",
                }));
            });
    ...
    async _onAddFontClick(ev) {
        const addFontDialog = class extends Component {
            ...
            async onGoogleFontChange(selected) {
            ...
            async onUploadChange(e) {
                ...
                reader.onload = (e) => {
                    const base64 = e.srcElement.result.split(',')[1];
                ...
                if (uploadedFontName && uploadedFontFaces) {
                    ...
                    if (fontExistsLocally) {
                        this.dialog.add(ConfirmationDialog, {
                            ...
                            body: _t("This font already exists."),
                        });
                        return;
                    }
                    // Create attachment.
