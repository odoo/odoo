...
export function clickOnSave(position = "bottom", timeout) {
    return [
        ...
        {
            trigger: "nav.o_main_navbar a:contains(edit), nav.o_main_navbar button:contains(edit)",
            noPrepend: true,
        },
    ];
}
...
export function selectElementInWeSelectWidget(widgetName, elementName, searchNeeded = false) {
    ...
    steps.push({
        ...
        trigger: `${we_select} we-button:contains("${elementName}"), ${we_select} we-button[data-select-label="${elementName}"]`,
        run: "click",
    });
    steps.push({
        content: "Check we-select is set and wait a delay before continue the tour",
