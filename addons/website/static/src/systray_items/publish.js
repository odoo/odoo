...
class PublishSystray extends Component {
    ...
    async publishContent() {
        ...
        if (this.state.published) {
            const closeNotification = this.notificationService.add("Your page is now published", {
                type: "info",
                sticky: false,
                autocloseDelay: 3000,
                buttons: [
            ...
            this.activeNotification = closeNotification;
        }

        this.timeout = setTimeout(async () => {
            this.state.processing = true;
            this.orm.call(mainObject.model, "website_publish_button", [[mainObject.id]]).then(
                async (published) => {
