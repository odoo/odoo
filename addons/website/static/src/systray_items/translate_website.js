...
class TranslateWebsiteSystray extends Component {
    ...
    attemptStartTranslate() {
        if (this.websiteService.isRestrictedEditor && !this.websiteService.isDesigner) {
            if (!this.websiteService.websiteRootInstance.el) {
                // Root element might not be there yet if user clicks too fast.
            ...
            const otherRecordEls = this.websiteService.websiteRootInstance.el.querySelectorAll("[data-res-model][data-res-id]:not([data-res-model='ir.ui.view'])");
            for (const el of otherRecordEls) {
                if (!objects[el.dataset.resModel]) {
                    // Keep one record of each type.
                    objects[el.dataset.resModel] = parseInt(el.dataset.resId);
                }
            }
