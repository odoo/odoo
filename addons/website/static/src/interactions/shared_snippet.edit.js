...
class SharedSnippetContent extends Interaction {
    static selector = "[data-oe-editable-call]";

    async willStart() {
        const editableCallName = this.el.dataset.oeEditableCall;
        const editableCallArgs = Object.fromEntries(
            this.el
                .getAttributeNames()
        ...
        this.content = markup(
            await this.services.orm.silent.call(
                "ir.ui.view",
                "render_editable_call",
                [editableCallName, editableCallArgs, mainObject.model, mainObject.id],
                {
                    context: {
...
registry.category("public.interactions.edit").add("website.editable_call_content", {
