...
export class SeoChecks extends Component {
    ...
    async setup() {
        ...
        onWillStart(async () => {
            const altAttributes = await rpc("/website/get_alt_images", {
...
export class OptimizeSEODialog extends Component {
    ...
    async save() {
        ...
        if (
            ...
            const footerEl = this.pageDocumentElement.querySelector("#footer");
            let inFooter = false;
            if (footerEl) {
                for (const brokenLink of seoContext.brokenLinks) {
                    if (brokenLink.oldLink !== brokenLink.newLink || brokenLink.remove) {
                        if (footerEl.querySelector(`a[href="${brokenLink.oldLink}"]`)) {
                            inFooter = true;
                            break;
                        }
                    }
            ...
            if (inFooter) {
                rpcCalls.push(
                    rpc("/website/update_broken_links", {
                        res_id: parseInt(footerEl.dataset.oeId),
                        res_model: footerEl.dataset.oeModel,
                        links: seoContext.brokenLinks,
                    })
                );
            }
        }
        if (seoContext.updatedAlts.length > 0) {
            rpcCalls.push(
