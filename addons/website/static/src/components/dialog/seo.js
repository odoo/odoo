...
class TitleDescription extends Component {
    ...
    onDescriptionBlur() {
        const descLength = this.seoContext.description.length;
        if (descLength && descLength < this.minRecommendedDescriptionSize) {
            this.state.descriptionWarning = _t("Your description is too short (min 50 characters).");
        } else if (descLength > this.maxRecommendedDescriptionSize) {
            this.state.descriptionWarning = _t("Your description is too long (max 160 characters).");
        } else {
...
export class SeoChecks extends Component {
    ...
    async setup() {
        ...
        onWillStart(async () => {
            ...
            this.state.hasEnoughWords = this.checkWordCount();
        });
    }
    ...
    getHeadingsScan() {
        ...
        if (h1Els.length !== 0 && allHeadingsEls.length
            && allHeadingsEls[0].tagName.toLowerCase() !== "h1") {
            // The h1 tag must be at the top of the hierarchy
            ret.misplacedH1 = true;
            ret.issuesLength++;
        }

        // Checks that the first <h2> is not preceded by an <h3>
        const firstH2Index = allHeadingsEls
            .findIndex(el => el.tagName.toLowerCase() === "h2");

        if (firstH2Index !== -1) {
            ret.h3BeforeH2 = allHeadingsEls
                .slice(0, firstH2Index)
                .some(el => el.tagName.toLowerCase() === "h3");
            ret.issuesLength += !!ret.h3BeforeH2;
        }
    ...
    checkWordCount() {
        // Minimum 300 words per language for SEO: enough content to rank well,
        // even if languages differ (chinese, arabic, etc.), this ensures
        // meaningful content without being too short.
        const documentEl = this.website.pageDocument.documentElement;
        return documentEl.textContent.trim().split(/\s+/).filter(Boolean).length >= 300;
    }
