...
class ImageSelector extends Component {
    ...
    selectImage(src) {
        ...
        if (this.isActiveImageDefault) {
            this.props.hasSocialDefaultImage = true;
        } else {
            this.props.hasSocialDefaultImage = false;
        }
    }

    get isSocialDefaultImageActive() {
        return this.areSameImages(this.activeMetaImage, this.props.socialDefaultImage);
    }

    setDefaultCoverImage() {
        if (this.isSocialDefaultImageActive) {
            this.setDefaultSocialImage(null); // Unset default
        } else {
            this.setDefaultSocialImage(this.activeMetaImage); // Set current as default
        }
    }
...
export class OptimizeSEODialog extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            this.socialDefaultImage = this.data.social_default_image;
    ...
    async save() {
        ...
        if (seoContext.defaultImage) {
            const base64Image = await this.fetchAsDataURL(seoContext.defaultImage);
            rpcCalls.push(
