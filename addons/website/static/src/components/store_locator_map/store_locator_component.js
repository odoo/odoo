...
export class StoreLocatorComponent extends Component {
    ...
    getLocations(zipCode) {
        this.state.locations = [];
        const rawLocations = this.props.rootEl.attributes["locations_data"]?.nodeValue;
        if (rawLocations) {
            this.state.locations = JSON.parse(rawLocations).filter(({ zip_code }) =>
                zip_code?.match(zipCode)
            );
        }
    }
