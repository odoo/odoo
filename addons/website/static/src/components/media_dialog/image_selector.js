...
patch(AutoResizeImage.prototype, {
     ...
     */
    async remove() {
        const {attachmentsIds: ids, originalMedia} =
            await this.props.getOriginalAndCopies(this.props.id, true);
        // True if the media can be used on website pages.
        const canHideMedia = originalMedia
            && originalMedia.res_model === "ir.ui.view" && originalMedia.res_id === 0;
...
patch(ImageSelector.prototype, {
    ...
    onRemoved(attachmentId) {
        this.state.attachments = this.state.attachments.filter(attachment =>
            attachment.id !== attachmentId && attachment.original_id?.[0] !== attachmentId
        );
    },
});
