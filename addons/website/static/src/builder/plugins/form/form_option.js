    ...
    static async cleanForSave(el, { dependencies, services }) {
        ...
        for (const formEl of selectElements(el, ".s_website_form form[data-model_name]")) {
            ...
            const fields = [
                ...
                .map((el) => el.name)
                // Fields without a name (e.g. hand-edited HTML) would make
                // `formbuilder_whitelist` raise and break the whole save.
                .filter((name) => name && !authorizedFields[name]?._property);
            if (fields.length) {
