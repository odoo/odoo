...
export function dynamicContentOfDynamicSnippet(el) {
    return el.querySelector(
        '[data-oe-editable-call="website.editable_call_template_dynamic_snippet_content"]'
    );
}
...
export class DynamicSnippetOptionPlugin extends Plugin {
    ...
    resources = {
        builder_actions: {
            ...
            DynamicSearchInfoAction,
            DynamicSearchInfoJsonAction,
            DynamicSearchInfoByIdsAction,
        },
...
export class DynamicSearchInfoAction extends BuilderAction {
    static id = "dynamicSearchInfo";
    getValue({ editingElement, params: { mainParam: key } }) {
        const dynamicEl = dynamicContentOfDynamicSnippet(editingElement);
        return getSharedSnippetArg(dynamicEl, "search_info")?.[key];
    }
    isApplied(args) {
        return deepEqual(this.getValue(args), args.value);
    }
    apply(args) {
        args.value === "" ? this.clean(args) : this.writeValue(args);
    }
    clean(args) {
    ...
    writeValue({ editingElement, params: { mainParam: key }, value }) {
        const dynamicEl = dynamicContentOfDynamicSnippet(editingElement);
        const searchInfo = getSharedSnippetArg(dynamicEl, "search_info") ?? {};
        if (value === undefined) {
            delete searchInfo[key];
        } else {
            searchInfo[key] = value;
        }
        setSharedSnippetArg(dynamicEl, "search_info", searchInfo);
    }
}

export class DynamicSearchInfoJsonAction extends DynamicSearchInfoAction {
    static id = "dynamicSearchInfoJson";
    getValue(args) {
        const searchInfoValue = super.getValue(args);
        return searchInfoValue && JSON.stringify(searchInfoValue);
    }
    apply(args) {
...
export class DynamicSearchInfoByIdsAction extends DynamicSearchInfoAction {
    static id = "dynamicSearchInfoByIds";
    getValue(args) {
        const searchInfoValue = super.getValue(args);
        return searchInfoValue && JSON.stringify(searchInfoValue.map((id) => ({ id })));
    }
    apply(args) {
        const value = JSON.parse(args.value).map(({ id }) => id);
        value.length ? super.apply({ ...args, value }) : super.clean(args);
    }
}
