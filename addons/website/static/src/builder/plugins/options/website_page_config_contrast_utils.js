...
function parseLength(value, reference) {
    return value === "auto"
        ? null
        : value.endsWith("%")
        ? (parseFloat(value) / 100) * reference
        : parseFloat(value);
}

function parsePosition(value, reference, renderedSize) {
    if (value.endsWith("%")) {
...
function getRenderedBackgroundImageSize(style, sectionRect, image) {
    ...
    const width = parseLength(sizeX, sectionRect.width);
    const height = parseLength(sizeY, sectionRect.height);
    if (!width && !height) {
        return { width: image.naturalWidth, height: image.naturalHeight };
    }
    return {
        width: width || (height * image.naturalWidth) / image.naturalHeight,
        height: height || (width * image.naturalHeight) / image.naturalWidth,
    };
}
...
function getBackgroundImageSourceArea(style, sectionRect, sampleHeight, renderedSize, image) {
    const [positionX = "50%", positionY = "50%"] = style.backgroundPosition
        ...
        .trim()
        .split(/\s+/);
    const offsetX = parsePosition(positionX, sectionRect.width, renderedSize.width);
    const offsetY = parsePosition(positionY, sectionRect.height, renderedSize.height);
...
export async function getAverageBackgroundImageColor(sectionEl, sampleEl) {
    ...
    const sampleHeight = Math.min(sectionRect.height, sampleEl.getBoundingClientRect().height);
