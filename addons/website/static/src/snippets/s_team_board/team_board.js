...
export class TeamBoard extends Interaction {
    ...
    updateState(cardEl) {
        ...
        this.memberName = cardEl.querySelector(".o_team_board_card_name")?.textContent.trim() || "";

        const memberRole =
            cardEl.querySelector(".o_team_board_card_designation")?.textContent.trim() || "";

        const memberDetails =
            cardEl.querySelector(".o_team_board_card_summary")?.textContent.trim() || "";

        const memberImageSrc = cardEl.querySelector(".o_team_board_member_image")?.src || "";

        this.modalEl.querySelector(".o_team_board_member_modal_name").textContent = this.memberName;
        this.modalEl.querySelector(".o_team_board_member_modal_role").textContent = memberRole;
        this.modalEl.querySelector(".o_team_board_member_modal_summary").textContent =
            memberDetails;

        this.modalEl.querySelector(".o_team_board_member_modal_img").src = memberImageSrc;
    }

    disableMsgBtn() {
        const msgBtn = this.modalEl.querySelector(".o_team_board_member_send_message_btn");
