...
options.registry.facebookPage = options.Class.extend({
    ...
    willStart: function () {
        ...
        const defaults = {
            ...
            id: '',
            height: 700,
            width: Math.min(this.$target[0].firstElementChild.offsetWidth, 500), // Default width is the width of the parent element
            tabs: "timeline",
            small_header: false,
            hide_cover: "false",
            show_facepile: true,
    ...
    setHeight(previewMode, widgetValue, params) {
        // Cases when widgetValue is invalid e.g. empty, not a number, or greater than 700
        if (!widgetValue || isNaN(widgetValue) || JSON.parse(widgetValue) > 700) {
            widgetValue = this.fbData.height;
        } else {
            this.fbData.height = widgetValue;
        }
        return this._markFbElement();
    },
    ...
    setWidth(previewMode, widgetValue, params) {
        // Cases when widgetValue is invalid e.g. empty, not a number, or greater than 500
        if (!widgetValue || isNaN(widgetValue) || JSON.parse(widgetValue) > 500) {
            widgetValue = this.fbData.width;
        } else {
            this.fbData.width = widgetValue;
        }
        return this._markFbElement();
    },
