...
options.registry.facebookPage = options.Class.extend({
    ...
    setHeight(previewMode, widgetValue, params) {
        if (!widgetValue || isNaN(widgetValue)) {
            widgetValue = this.fbData.height || 700; // Retain previous value or default to 700
            return;
        }
    ...
    setWidth(previewMode, widgetValue, params) {
        if (!widgetValue || isNaN(widgetValue)) {
            widgetValue = this.fbData.width || 500; // Retain previous value or default to 500
            return;
        }
    ...
    _checkURL: function () {
        ...
        if (match) {
            ...
            .then((res) => {
                ...
                .catch(() => {
                    const fbPageElement = this._getFbPageElement();
                    // Notify the user if the Facebook page is not reachable
                    // The content will be replaced by a blockquote element in this case
                    if (fbPageElement?.firstElementChild.classList.contains("replaceable")) {
                        ...
                        fbPageElement.replaceChildren(fallbackBlock);
