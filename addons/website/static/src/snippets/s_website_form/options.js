...
options.registry.WebsiteFieldEditor = FieldEditor.extend({
    ...
    async selectDataAttribute(previewMode, widgetValue, params) {
        ...
        if (["requirementCondition", "requirementBetween"].includes(params.attributeName)) {
            const { 
                ...
                type } = this.$target[0].dataset;
            this.$target[0].dataset.errorMessage = this._defaultMessage(comparator, condition, between, type);
        }
    },
        ...
        switch (widgetName) {
            ...
            case "require_condition_additional_text":
                return this.$target[0].dataset.requirementComparator && 
                    ...
                    (currentEl.nodeName === "TEXTAREA" ||
                        ["text", "email", "tel", "url", "search", "password", "number"].includes(currentEl.type));
    ...
    _defaultMessage(comparator, condition, between, type) {
        const textMessages = {
            "contains": "This field must include keyword %s.",
            "!contains": "This field must not include keyword %s.",
            "substring": "This field must include keyword %s.",
            "!substring": "This field must not include keyword %s.",
            "greater": "Invalid: field is not greater than %s.",
            "less": "Invalid: field is not less than %s.",
            "greater or equal": "Invalid: field is not greater than or equal to %s.",
            "less or equal": "Invalid: field is not less than or equal to %s."
        };
        
        if (textMessages[comparator]) {
            return _t(textMessages[comparator], condition);
        }
        ...
        if (["date","datetime"].includes(type)) {
            ...
            const dateMessages = {
                "dateEqual": "Entered date or time is not correct! It must be %(start)s (%(format)s).",
                "date!equal": "Entered date or time is not correct! It must not be %(start)s (%(format)s).",
                "before": "Entered date or time is not correct! It must be before %(start)s (%(format)s).",
                "after": "Entered date or time is not correct! It must be after %(start)s (%(format)s).",
                "equal or before": "Entered date or time is not correct! It must be before or equal to %(start)s (%(format)s).",
                "equal or after": "Entered date or time is not correct! It must be after or equal to %(start)s (%(format)s).",
                "between": "Entered date or time is not correct! It must be within %(start)s and %(end)s (%(format)s).",
                "!between": "Entered date or time is not correct! It must not be within %(start)s and %(end)s (%(format)s)."
            };
            
            if (dateMessages[comparator]) {
                return _t(dateMessages[comparator], { start, end, format });
            }
        }
