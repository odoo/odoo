...
const dynamicSnippetOptions = options.Class.extend({
        ...
        if (params.attributeName === "numberOfRecords" && previewMode === false) {
            ...
            if (switchMode) {
                ...
                if (isSingleRecord) {
                    ...
                    this.$target[0].dataset.filterId = Object.entries(this.dynamicFilters).filter(
                        (filter) => filter[1].model_name === this.currentModelName
                    )[0][0];
    ...
    _computeWidgetVisibility(widgetName, params) {
        switch (widgetName) {
            // Hide if exaclty one filter is available: show when none to help
    ...
    _computeWidgetState(methodName, params) {
        ...
        if (methodName === "setSnippetResId" && this._isSingleRecordSnippet()) {
            ...
            const firstPossibleId = params.possibleValues.find(Boolean);
            const id = data.snippetResId || firstPossibleId || "";
            ...
            // display sample data).
            if (!("snippetResId" in this.$target[0].dataset)) {
                this.$target[0].dataset.snippetResId = id;
                this._refreshPublicWidgets();
            }
            return id;
        }
    ...
    _renderCustomXML: async function (uiFragment) {
        ...
        if (defaultModel) {
            ...
            modeldSelectorEl.dataset.attributeDefaultValue = defaultModel;
            recordSelectorEl.dataset.model = this.$target[0].dataset.snippetModel || defaultModel;
        } else {
            modeldSelectorEl.remove();
    ...
    _renderDynamicFilterTemplatesSelector(uiFragment) {
        ...
        for (const templateOption of templatesSelectorEl.children) {
            ...
            const extraSnippetClasses = this.dynamicFilterTemplates[templateOption.dataset.selectDataAttribute]?.extraSnippetClasses;
            templateOption.dataset.selectClass = extraSnippetClasses || "";
        }
    },
    /**
    ...
    _getDefaultTemplateKey() {
        return Object.keys(this.dynamicFilterTemplates).find((key) => {
            ...
            const currentModel = this.$target[0].dataset.snippetModel || this.currentModelName;
            const isModelTemplate = key.includes(currentModel.replaceAll(".", "_"));
            return (
                isModelTemplate &&
                (this._isSingleRecordSnippet() ? isSingleTemplate : !isSingleTemplate)
            );
        });
    },
    /**
    ...
    _getSelectDefaultOption(name) {
        return this.el.querySelectorAll(
            `we-select[data-name='${name}'] we-selection-items we-button`
        )[0];
    },
});
