...
export class AnnouncementScroll extends Interaction {
    ...
    setup() {
        ...
        this.scrollText = this.el.dataset.scrollcontent || _t("Write your text ");
        this.windowHeight = window.innerHeight;
        this.parallaxPosition = -50;
        this.lastScrollY = window.scrollY;
        this.scrollDelta = 0;
        this.marqueeCloneEls = [];
        this.currentScrollTimeoutId = null;
    }
    ...
    start() {
        ...
        !this.el.dataset.scrollcontent && (this.el.dataset.scrollcontent = _t("Write your text "));
        ...
        this.el.classList.toggle('s_announcement_scroll_in_viewport', this.isElementInViewport());
    ...
    onScroll() {
        ...
        if (this.isElementInViewport()) {
            this.el.classList.add('s_announcement_scroll_in_viewport', 's_announcement_scroll_page_scrolling');
            const timeoutId = Date.now();
            this.currentScrollTimeoutId = timeoutId;
            this.waitForTimeout(() => {
                if (this.currentScrollTimeoutId === timeoutId) {
    ...
    destroy() {
        this.marqueeCloneEls.forEach(el => el.remove());
        this.marqueeCloneEls = [];
    ...
    updateMarqueeLayout() {
        ...
        if (this.el.dataset.scrollcontent) {
            ...
            for (let i = 0; i < cloneCount; i++) {
                ...
                this.marqueeCloneEls.push(clone);
            }
        }
    }
