...
options.registry.StepsConnector = options.Class.extend({
     ...
     */
    _computeVisibility() {
        // Hide the options if the device is too small (connectors are hidden).
        const isSmallDevice = config.device.size_class < config.device.SIZES.LG;
        return !isSmallDevice && this._super(...arguments);
    },
    /**
