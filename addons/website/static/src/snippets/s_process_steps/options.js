...
options.registry.StepsConnector = options.Class.extend({
    ...
    _reloadConnectors() {
        ...
        for (let i = 0; i < stepsEls.length - 1; i++) {
            ...
            const stepHeightDifference = stepMainElementRect.top - nextStepMainElementRect.top;
            ...
            connectorEl.style.height = `${stepMainElementRect.height + Math.abs(stepHeightDifference)}px`;
            ...
            connectorEl.style[marginType] = `${30 - stepHeightDifference}px`;
            ...
            connectorEl.querySelector('path').setAttribute('d', this._getPath(type, width, height, stepHeightDifference));
        ...
        switch (type) {
            case 's_process_steps_connector_line': {
                if (stepHeightDifference >= 0){
                    return `M 0 ${stepHeightDifference + this.hCurrentStepIconHeight} L ${width} ${this.hNextStepIconHeight}`;
                }
                return `M 0 ${this.hCurrentStepIconHeight} L ${width} ${this.hNextStepIconHeight - stepHeightDifference}`;
            }
            case 's_process_steps_connector_arrow': {
                ...
                if (stepHeightDifference >= 0) {
                    return `M ${0.05 * width} ${stepHeightDifference + this.hCurrentStepIconHeight - verticalPaddingFactor} L ${0.95 * width - 6} ${(this.hNextStepIconHeight) + verticalPaddingFactor}`;
                }
                return `M ${0.05 * width} ${this.hCurrentStepIconHeight + verticalPaddingFactor} L ${0.95 * width - 6} ${Math.abs(stepHeightDifference) + (this.hNextStepIconHeight) - verticalPaddingFactor}}`;
            }
            case 's_process_steps_connector_curved_arrow': {
                if (stepHeightDifference == 0) {
                    ...
                    return `M ${0.05 * width} ${stepHeightDifference + this.hCurrentStepIconHeight} Q ${width * 0.75} ${height * 0.75}, ${(0.5 * width - 6)} ${hHeight} T ${0.95 * width - 6} ${this.hNextStepIconHeight}`;
                }
                return `M ${0.05 * width} ${this.hCurrentStepIconHeight} Q ${width * 0.75} ${height * 0.005},  ${(0.5 * width - 6)} ${hHeight} T ${0.95 * width - 6} ${Math.abs(stepHeightDifference) + this.hNextStepIconHeight}`;
            }
        }
        return '';
    },
