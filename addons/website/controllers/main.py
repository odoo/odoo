...
class Website(Home):
    ...
    def update_footer_template(self, template_key, possible_values):
        """ Enables the footer template and its corresponding copyright template on template change
        The goal is to ensure that the content width of the copyright aligns with the footer. """

        # 1. Define templates views to enable/disable
        views_enable = [template_key]
        views_disable = self.theme_customize_data_get(possible_values, is_view_data=True)

        # 2. Define the possible footer classes and corresponding views
        width_views = {'container-fluid': 'website.footer_copyright_content_width_fluid',
                       'o_container_small': 'website.footer_copyright_content_width_small'}

        # 3. Parse new footer template and get the content width
        ...
        res = tree.xpath(
            ...
            "or contains(concat(' ', normalize-space(@class), ' '), ' container-fluid ') "
            "or contains(concat(' ', normalize-space(@class), ' '), ' o_container_small ')]")

        # 4. Define copyright views to enable/disable
        if res:
            classes = res[0].get('class').split()
            width = next((c for c in ['container', 'container-fluid', 'o_container_small'] if
                          c in classes), False)
            if width:
