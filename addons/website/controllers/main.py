...
class Website(Home):
    ...
    @http.route("/website/http_error/<int:status_code>", type="http", auth="public", website=True, sitemap=False)
    def website_error_template_render(self, status_code, **kwargs):
        """
        Generic error page renderer for 400, 403, 404, 500, etc.
        """
        template = f"http_routing.{status_code}"
        IrUiView = request.env["ir.ui.view"]
        if not IrUiView.search_count([("key", "=", template)]):
