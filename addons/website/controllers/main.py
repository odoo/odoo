...
MAX_FONT_FILE_SIZE = 100 * 1024 * 1024
...
class Website(Home):
    ...
    def google_font_metadata(self):
        ...
        if not metadata or metadata.write_date < yesterday:
            with urlopen('https://fonts.google.com/metadata/fonts') as response:
                json_text = response.read()
                if metadata:
                    metadata.raw = json_text
                else:
    ...
    def theme_upload_font(self, name, data):
        ...
        def check_content(filename, data):
            ...
            if ext == 'otf':
                ...
                TOC_OFFSET = 12
                TOC_ENTRY_LENGTH = 16
                table_count = int(data[4] << 8) + data[5]  # uint16
                if TOC_OFFSET + table_count * TOC_ENTRY_LENGTH > len(data):
                    return False
                mandatory_tags = {b'cmap', b'glyf', b'head', b'hhea', b'hmtx', b'loca', b'maxp', b'name', b'post'}
                for offset in range(TOC_OFFSET, TOC_OFFSET + table_count * TOC_ENTRY_LENGTH, TOC_ENTRY_LENGTH):
        ...
        if zipfile.is_zipfile(readable_data):
            with zipfile.ZipFile(readable_data, "r") as zip_file:
                for entry in zip_file.filelist:
                    ...
                    if entry.filename.split('.')[-1].lower() not in SUPPORTED_FONT_EXTENSIONS \
        ...
        elif name.split('.')[-1].lower() in SUPPORTED_FONT_EXTENSIONS and check_content(name, binary_data):
            result.append(create_attachment({
                'name': name,
            }, binary_data))
        if not result:
