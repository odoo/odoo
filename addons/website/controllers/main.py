...
class Website(Home):
    ...
    def get_alt_images(self, res_id, res_model):
        ret = []
        ...
        for editable_field in editable_fields:
            ...
            for index, element in enumerate(tree.xpath('//img')):
                decorative = element.attrib.get('role') == 'presentation'
                alt = element.attrib.get('alt')
                if not decorative or not alt:
                    # If an alt attribute is missing, set decorative as False
                    # to ensure a correct attribute setting
                    if not alt:
                        decorative = False
                    ret.append({
                        'id': f"{editable_field}-{index}",
    ...
    def update_alt_images(self, res_id, res_model, imgs):
        ...
        for editable_field in editable_fields:
            ...
            modified = False
            for index, element in enumerate(tree.xpath('//img')):
                img = next((item for item in imgs if item['id'] == f"{editable_field}-{index}"), None)
                if not img:
                    continue
                if (img['decorative']):
                    element.set('alt', '')
