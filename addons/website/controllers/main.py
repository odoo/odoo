...
SITEMAP_CACHE_TIME = datetime.timedelta(seconds=10)
...
class Website(Home):
    ...
    def sitemap_xml_index(self, **kwargs):
        ...
        if not content:
            ...
            for loc in locs:
                relative_url = loc.get('loc')
                full_url = url_root.rstrip('/') + relative_url
                try:
                    resp = requests.get(full_url)
                    logger.info("%s -> %s", full_url, resp.status_code)
                    if 200 <= resp.status_code < 300:
                        valid_locs.append(loc)
                    else:
                        logger.warning("Skipping URL with non-2XX status: %s (%s)", full_url, resp.status_code)
                except Exception as e:
                    logger.error("Failed to check %s: %s", full_url, e)
            pages = 0
            while True:
                current_slice = list(islice(valid_locs, pages * LOC_PER_SITEMAP, (pages + 1) * LOC_PER_SITEMAP))
