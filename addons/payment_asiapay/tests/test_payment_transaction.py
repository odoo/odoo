...
class TestPaymentTransaction(AsiaPayCommon, PaymentHttpCommon):
    ...
    def test_reference_is_singularized(self):
        ...
        reference = self.env['payment.transaction']._compute_reference(self.asiapay.provider)
    ...
    def test_reference_is_computed_based_on_document_name(self):
        ...
        reference = self.env['payment.transaction']._compute_reference(
            self.asiapay.provider, invoice_ids=[Command.set([invoice.id])]
        )
    ...
    def test_reference_is_stripped_at_max_length(self):
        ...
        reference = self.env['payment.transaction']._compute_reference(
            self.asiapay.provider, prefix='this is a long reference of more than 35 characters'
        )
    ...
    def test_no_item_missing_from_rendering_values(self):
        ...
        with patch(
            'odoo.addons.payment_asiapay.models.payment_acquirer.PaymentAcquirer'
    ...
    def test_no_input_missing_from_redirect_form(self):
        ...
        self.assertEqual(form_info['action'], tx.acquirer_id._asiapay_get_api_url())
