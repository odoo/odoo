...
class TestAbaPayway(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.aba_payway_qr_invoice = cls.env['account.move'].create({
    ...
    def test_aba_payway_qr_code_generation(self):
        ...
        with patch(
            ...
            result = self.aba_payway_qr_invoice.with_context({'is_online_qr': True})._generate_qr_code()
        ...
        with patch(
            ...
            self.aba_payway_qr_invoice.with_context({'is_online_qr': True})._generate_qr_code()
            ...
            self.assertEqual(len(self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids), 2)

        with patch(
            'odoo.addons.l10n_id.models.res_bank._l10n_id_make_qris_request', return_value=self.success_qris_get_second
        ):
            self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids[-1]['qris_creation_datetime'] = '2024-02-27 03:00:00'
            self.aba_payway_qr_invoice.with_context({'is_online_qr': True})._generate_qr_code()
            ...
            self.assertEqual(len(self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids), 3)
    ...
    def test_fetch_payment_status_success(self):
        ...
        with patch(
            ...
            self.aba_payway_qr_invoice.action_l10n_id_update_payment_status()
            self.assertEqual(self.aba_payway_qr_invoice.payment_state, self.env['account.move']._get_invoice_in_payment_state())
            ...
            self.assertTrue(any(self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids.mapped('paid')))
