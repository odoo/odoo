...
@contextmanager
def mock_payway_request(*args, **kwargs):
    def patched_payway_request(base_url: str, endpoint: str, payload: dict):
        if endpoint == "/api/payment-gateway/v1/payments/close-transaction":
            return {
            "status": {"code": "00", "message": "Success!", "tran_id": "1729573626"}
        }
        if endpoint == "/api/payment-gateway/v1/payments/generate-qr":
            return {
    ...
    with patch("odoo.addons.l10n_kh_aba_payway.models.res_bank._make_payway_api_request", side_effect=patched_payway_request):
        yield


@tagged("post_install_l10n", "post_install", "-at_install")
class TestAbaPayway(AccountTestInvoicingCommon):
    """Test ABA QR generation on invoices and auto-payment registration"""

    @classmethod
    @AccountTestInvoicingCommon.setup_chart_template("kh")
    def setUpClass(cls):
        super().setUpClass()
        cls.company_data["company"].qr_code = True
        cls.company_data["company"].partner_id.update(
            {
                "country_id": cls.env.ref("base.kh").id,
                "city": "Phnom Penh",
            }
        )

        cls.acc_aba_payway_id = cls.env["res.partner.bank"].create(
            {
                "acc_number": "123456789012345678",
                "partner_id": cls.company_data["company"].partner_id.id,
                "sandbox_payway_merchant_id": "mId",
                "sandbox_payway_key": "apikey",
                "payway_environment": "sandbox",
            }
        )

        cls.aba_payway_qr_invoice = (
            cls.env["account.move"]
            .create(
                {
                    "move_type": "out_invoice",
                    "partner_id": cls.partner_a.id,
                    "currency_id": cls.env.ref("base.KHR").id,
                    "partner_bank_id": cls.acc_aba_payway_id.id,
                    "company_id": cls.company_data["company"].id,
                    "invoice_line_ids": [
                        Command.create({"quantity": 1, "price_unit": 100})
                    ],
                    "qr_code_method": "abapay_khqr",
                }
            )
            ._post()
        )
    ...
    def test_aba_payway_qr_code_generation(self):
        ...
        """
        with mock_payway_request():
            result = self.aba_payway_qr_invoice._generate_qr_code()
            self.assertIsNotNone(result)

        self.fail("Todo: Complete the test cases for success, failure and other cases")
        # with patch(
    ...
    #         result = self.aba_payway_qr_invoice.with_context(
    #             {"is_online_qr": True}
    #         )._generate_qr_code()
    #         self.assertIsNotNone(result)

    #         # Confirm that a transaction should be registered on the invoice
    #         qr_details = self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids
    ...
    #         self.aba_payway_qr_invoice.with_context(
    #             {"is_online_qr": True}
    #         )._generate_qr_code()
    ...
    #         self.assertEqual(
    #             len(self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids), 2
    #         )
    ...
    #     ):
    #         self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids[-1][
    ...
    #         self.assertEqual(
    #             len(self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids), 3
    #         )
    ...
    #     ):
    #         self.aba_payway_qr_invoice.action_l10n_id_update_payment_status()
    #         self.assertEqual(self.aba_payway_qr_invoice.payment_state, "not_paid")
    ...
    #         self.assertTrue(
    #             any(
    #                 self.aba_payway_qr_invoice.l10n_id_qris_transaction_ids.mapped(
    #                     "paid"
    #                 )
    #             )
    #         )
