...
class TestMassMailEventValues(EventCase):
    ...
    def test_mailing_event_computed_fields_form(self):
        if not self.env['ir.module.module'].search([('name', '=', 'mass_mailing_sms'), ('state', '=', 'installed')]):
            self.skipTest("This test requires the installation of the mass_mailing_sms module")
        ...
        mailing_form = Form(self.env['mailing.mailing'].with_context(
            ...
            force_mailing_model_id=self.env['ir.model']._get('event.registration').id,
            force_mailing_domain=f"[('event_id', 'in', {test_event.ids}), ('state', 'not in', ['cancel', 'draft'])]",
