...
def get_user_delegation_key(
    ...
    if token_response.status_code != 200:
        raise ValidationError(f"Failed to get access token: {token_response.content}")  # nosem: missing-gettext
    ...
    try:
        ...
        raise ValidationError("Failed to get user delegation key: the account name may be incorrect")  # nosem: missing-gettext
    ...
    if key_response.status_code != 200:
        raise ValidationError(f"Failed to get user delegation key: {key_response.content}")  # nosem: missing-gettext
