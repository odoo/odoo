...
class TestSelfOrderCombo(SelfOrderCommonTest):
    ...
    def test_product_dont_display_all_variants(self):
        ...
        coke_template = self.env['product.template'].create({
            'name': 'Coke',
            'available_in_pos': True,
            'list_price': 3.0,
            'attribute_line_ids': [
                ...
                Command.create({
                    'attribute_id': color_attribute.id,
                    'value_ids': [Command.set([attribute_values[2].id])],
                })
            ],
        })
        coke_large = coke_template.product_variant_ids[1]
        combo = self.env['product.combo'].create({
            'name': 'Drink Combo',
            'combo_item_ids': [
                Command.create({
                    'product_id': coke_large.id,
                    'extra_price': 0,
                }),
            ],
        })
