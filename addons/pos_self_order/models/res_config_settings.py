...
class ResConfigSettings(models.TransientModel):
    ...
    def _onchange_pos_self_order_kiosk(self):
        if self.pos_self_ordering_mode == 'kiosk':
            ...
            self.pos_self_ordering_pay_after = "each"
            cash_payment_methods = self.pos_payment_method_ids.filtered(lambda x: x.type == 'cash' and not x.payment_provider)
    ...
    def _onchange_pos_payment_method_ids(self):
        if self.pos_self_ordering_mode == 'kiosk' and any(pm.type == 'cash' and not pm.payment_provider for pm in self.pos_payment_method_ids):
