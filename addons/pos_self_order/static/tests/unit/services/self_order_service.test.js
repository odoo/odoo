...
describe("self_order_service", () => {
    ...
    describe("initData", () => {
        test("hide special products", async () => {
            ...
            expect(store.productByCategIds["0"]).toBeEmpty();
        ...
        test("availableCategories", async () => {
            const store = await setupSelfPosEnv();
            const models = store.models;
            ...
            expect(store.availableCategories.map((c) => c.id)).toEqual([1, 2, 3, 5, 4, 0]);
            ...
            expect(store.availableCategories.map((c) => c.id)).toEqual([1, 2, 3, 5, 4]);
    ...
    test("showComboSelectionPage", async () => {
        ...
        expect(store.showComboSelectionPage(product)).toEqual(defaultReturnValue);
        // only One choice
    ...
    test("createNewOrder", async () => {
        ...
        models["pos.preset"].get(10).delete();
        models["pos.preset"].get(2).delete();
        {
    ...
    describe("addToCart", () => {
        ...
        test("Combo Products", async () => {
            ...
            store.addToCart(productCombo, 21, "", {}, {}, comboValues);
            ...
            expect(parent.qty).toBe(21);
            expect(child1.qty).toBe(21);
            expect(child2.qty).toBe(21);
        });
    });
    ...
    describe("cancelOrder", () => {
        test("simple cancel order", async () => {
            const store = await setupSelfPosEnv();
            const models = store.models;
            const order = await getFilledSelfOrder(store);
