...
describe("printKioskChanges", () => {
    beforeEach(async () => {
        ...
        const printedReceipts = [];
        store.kitchenPrinters = [
            {
                config: this.posPrinter,
        ...
        this.getPrintedOrderLines = () => {
            expect(this.printedReceipts.length).toBe(1);
            return Array.from(this.printedReceipts[0].querySelectorAll(".orderline")).map((node) =>
    ...
    test("all matching lines", async () => {
        ...
        await this.store.printKioskChanges();
    ...
    test("no category matches", async () => {
        ...
        await this.store.printKioskChanges();
        expect(this.printedReceipts.length).toBe(0);
    });
