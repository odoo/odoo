...
const comparePricesWithBackend = {
    trigger: "body",
    run: async () => {
        posmodel.currentOrder.recomputeOrderData();
        const orderTotal = posmodel.currentOrder.amount_total;
        const allUnitPrices = posmodel.currentOrder.lines.map((l) => l.price_unit);
        const result = await posmodel.sendDraftOrderToServer();
        if (!result) {
            throw new Error("Failed to sync order with server");
        }

        const orderTotalAfterSync = posmodel.currentOrder.amount_total;
        const allUnitPricesAfterSync = posmodel.currentOrder.lines.map((l) => l.price_unit);
    ...
    steps: () => [
        ...
        ...commonStepWithSpecificPrice,
        {
            ...
            run: async () => {
                ...
                line.price_unit = 0;
                line.tax_ids = [];
                const orderTotal = posmodel.currentOrder.getTotalWithTax();
                const allUnitPrices = posmodel.currentOrder.lines.map(
                    (l) => l.getAllPrices().priceWithTax
                );
...
registry.category("web_tour.tours").add("test_pricelist_should_not_be_changed_from_frontend", {
    steps: () => [
        ...
        ...commonStepWithSpecificPrice,
        {
            trigger: "body",
            run: async () => {
                const amountTotal = posmodel.currentOrder.getTotalWithTax();
                const freePricelist = posmodel.models["product.pricelist"].find(
                    (p) => p.name === "Free Pricelist"
                );

                if (posmodel.currentOrder.pricelist_id.name !== "10% Pricelist") {
                    throw new Error(
                        `The pricelist should be "10% Pricelist", but it is ${posmodel.currentOrder.pricelist_id.name}`
                    );
                }

                if (amountTotal !== 231.84) {
                    throw new Error(
                        `The total price should be 231.84 with 10% discount, but it is ${amountTotal}`
                    );
                }

                posmodel.currentOrder.pricelist_id = freePricelist;
                posmodel.currentOrder.lines = [];
            },
        },
        ...commonStepWithSpecificPrice,
        {
            ...
            run: async () => {
                ...
                const amountTotalAfterSync = posmodel.currentOrder.amount_total;
                if (amountTotalAfterSync === 0) {
                    throw new Error(
                        `The total price shouldn't be 0 after sync, but it is ${amountTotalAfterSync}`
                    );
                }
            },
        },
