...
import { useScrollDirection } from "@pos_self_order/common/hooks/use_scroll_direction";
import { useDetection } from "@pos_self_order/mobile/hooks/use_detection";
...
export class ProductList extends Component {
    ...
    setup() {
        ...
        this.productGroups = Object.fromEntries(
            Array.from(this.selfOrder.categoryList).map((category) => {
                return [category.name, useRef(`productsWithCategory_${category.name}`)];
            })
        );
    ...
    categoryOnClick(category_name) {
        ...
        this.scrollTo(this.productGroups[category_name]);
    }
