...
export class ProductList extends Component {
    ...
    filteredProducts = () => {
        ...
        return this.props.productList.filter(product => { 
            return this.itemHasAllOfTheTags(product, this.private_state.selected_tags) &&
    ...
    itemHasAllOfTheTags = (item, selected_tags) => {
        if (!selected_tags.size) return true;
        for( const tag of selected_tags ){
            if (!item.tag_list.has(tag)) return false;
        }
        return true;
    }
    /**
