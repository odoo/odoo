...
patch(PosStore.prototype, {
    ...
    _checkForSelfOrderNotifications(orders) {
        for (const orderData of orders) {
            const order = this.models["pos.order"].get(orderData.id);
            if (!order || !["mobile", "kiosk"].includes(order.source)) {
                continue;
            }
            const payAfter = order.config_id.self_ordering_pay_after;
            const hasOnlinePayment = order.config_id.self_order_online_payment_method_id;

            if (payAfter === "each" && hasOnlinePayment) {
                continue;
            } else {
                this._handleSelfOrder(order.id);
            }
        }
    },
    ...
    async _notifySelfOrderReviewed(orderId) {
        try {
            ...
            logPosMessage(
                "Self Order",
                "orderNotificationReviewed",
                "Failed to get sync reviewed self order notification ",
                CONSOLE_COLOR,
                [error]
            );
        }
