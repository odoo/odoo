...
EventSpecificOptions.include({
    ...
    _getCheckboxFields: function () {
        ...
        fields = _.union(fields, ['website_exhibitor']);
        return fields;
    },

    _getCheckboxFieldMatch: function (checkboxField) {
        if (checkboxField === 'website_exhibitor') {
    ...
    _initCheckboxCallback: function (rpcData) {
        this._super(rpcData);
        if (rpcData[0]['website_exhibitor']) {
            var submenuInput = this._getCheckboxFieldMatch('website_exhibitor');
