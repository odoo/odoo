...
class TestPersonalStages(TestProjectCommon):
    ...
    def test_personal_stage_read_group(self):
        ...
        self.env.flush_all()
        read_group_user = self.env['project.task'].with_user(self.user_projectuser).base_read_group(
        ...
        read_group_manager = self.env['project.task'].with_user(self.user_projectmanager).base_read_group(
