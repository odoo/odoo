...
class TestProjectTaskType(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        cls.stage_created = cls.env['project.task.type'].create({
            ...
            'project_ids': [cls.project_goats.id],
        })
    ...
    def test_modify_existing_stage(self):
        ...
        - case 1: [`user_id`: not set, `project_ids`: not set] | Add `user_id` and `project_ids` => user_id reset
        - case 4: [`user_id`: not set, `project_ids`: set] | Add `user_id` => UserError
        - case 3: [`user_id`: set, `project_ids`: not set]  | Add `project_ids` => user_id reset
        '''
        # case 1
