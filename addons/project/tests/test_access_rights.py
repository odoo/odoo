...
class TestPortalProjectBase(TestProjectBase):
    ...
    def setUp(self):
        ...
        self.task_3 = self.Task.with_context(mail_create_nolog=True).create({
            'name': 'Test3', 'user_id': self.user_portal.id, 'project_id': self.project_pigs.id})
        self.task_4 = self.Task.with_context(mail_create_nolog=True).create({
            'name': 'Test4', 'user_id': self.user_public.id, 'project_id': self.project_pigs.id})
        self.task_5 = self.Task.with_context(mail_create_nolog=True).create({
            'name': 'Test5', 'user_id': False, 'project_id': self.project_pigs.id})
        self.task_6 = self.Task.with_context(mail_create_nolog=True).create({
            'name': 'Test5', 'user_id': False, 'project_id': self.project_pigs.id})
...
class TestPortalProject(TestPortalProjectBase):
    ...
    def test_portal_project_access_rights(self):
        ...
        tasks = self.Task.sudo(self.user_projectuser).search([('project_id', '=', pigs.id)])
        ...
        self.assertRaises(AccessError, self.Task.sudo(self.user_noone).search, [('project_id', '=', pigs.id)])
        ...
        self.assertRaises(AccessError, self.Task.sudo(self.user_public).search, [])
    ...
    def test_employee_project_access_rights(self):
        ...
        tasks = self.Task.sudo(self.user_portal).search([('project_id', '=', pigs.id)])
        ...
        tmp_task = self.Task.sudo(self.user_projectuser).with_context(mail_create_nolog=True).create({
            'name': 'Pigs task',
            'project_id': pigs.id})
    ...
    def test_followers_project_access_rights(self):
        ...
        self.Task.sudo(self.user_projectuser.id).with_context(mail_create_nolog=True).create({
        ...
        self.assertRaises(except_orm, self.Task.sudo(self.user_projectuser).with_context(mail_create_nolog=True).create, {'name': 'Pigs task', 'project_id': pigs.id})
