...
class TestPortalProject(TestPortalProjectBase):
    ...
    @mute_logger('odoo.addons.base.models.ir_model')
    def test_portaledit_project_access_rights(self):
        """Note: Portaledit privacy_visibility affects only token options. Without a token, the access rights
           should be exactly the same as with portal privacy_visibility. This test is therefore exactly the
           same as test_portal_project_access_rights."""
        pigs = self.project_pigs
        pigs.write({'privacy_visibility': 'portaledit'})
