...
class TestProjectUpdateAccessRights(TestProjectCommon):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.base_user = mail_new_test_user(
            cls.env, "Base user", groups="base.group_user"
        )
    ...
    def test_project_update_user_can_read(self) -> None:
        _ = self.project_update_1.with_user(self.env.user).name

    @users("Base user")
    def test_project_update_user_no_write(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to write in the project update"
            % self.env.user.name,
        ):
            self.project_update_1.with_user(self.env.user).name = "Test write"

    @users("Project admin")
    def test_project_update_admin_can_write(self) -> None:
        self.project_update_1.with_user(self.env.user).name = "Test write"

    @users("Base user")
    def test_project_update_user_no_unlink(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to unlink in the project update"
            % self.env.user.name,
        ):
            self.project_update_1.with_user(self.env.user).unlink()

    @users("Project admin")
    def test_project_update_admin_unlink(self) -> None:
        self.project_update_1.with_user(self.env.user).unlink()

    @users("Portal user")
    def test_project_update_portal_user_no_read(self) -> None:
        with self.assertRaises(
            AccessError,
    ...
    @users("Portal user")
    def test_project_update_portal_user_no_write(self) -> None:
        with self.assertRaises(
            AccessError,
    ...
    @users("Portal user")
    def test_project_update_portal_user_no_create(self) -> None:
        with self.assertRaises(
            AccessError,
    ...
    @users("Portal user")
    def test_project_update_portal_user_no_unlink(self) -> None:
        with self.assertRaises(
            AccessError,
    ...
    @users("Portal user")
    def test_project_milestone_portal_user_no_read(self) -> None:
        with self.assertRaises(
            AccessError,
            msg=f"{self.env.user.name} should not be able to read in the project update",
        ):
            _ = self.project_milestone.with_user(self.env.user).name

    @users("Portal user")
    def test_project_milestone_portal_user_no_write(self) -> None:
        with self.assertRaises(
            AccessError,
            msg=f"{self.env.user.name} should not be able to write in the project update",
        ):
    ...
    @users("Portal user")
    def test_project_milestone_portal_user_no_create(self) -> None:
        with self.assertRaises(
            AccessError,
            msg=f"{self.env.user.name} should not be able to create in the project update model",
        ):
    ...
    @users("Portal user")
    def test_project_milestone_portal_user_no_unlink(self) -> None:
        with self.assertRaises(
            AccessError,
            msg=f"{self.env.user.name} should not be able to unlink in the project update",
        ):
