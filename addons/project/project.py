...
class project_task_type(osv.osv):
    ...
    def _get_tooltips(self, cr, uid, ids, field, args, context=None):
        res = {}
        for rec in self.read(cr, uid, ids, ['legend_blocked', 'legend_done', 'description'],context=context):
            value = ""
            if rec['legend_blocked']:
                value += "<img src='/web/static/src/img/icons/gtk-no.png'/> " + rec['legend_blocked'] + "<br/>"
            if rec['legend_done']:
                value += "<img src='/web/static/src/img/icons/gtk-yes.png'/> " + rec['legend_done'] + "<br/>"
            if rec['description']:
...
class account_analytic_account(osv.osv):
    ...
    _columns = {
        ...
        'task_label': fields.char('Use Task as', help="Gives label to tasks on project's kanaban view.", required=True),
    }
