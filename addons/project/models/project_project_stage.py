...
class ProjectProjectStage(models.Model):
    ...
    def copy_data(self, default: dict | None = None) -> list[dict]:
        ...
        return [
            dict(vals, name=self.env._("%s (copy)", stage.name))
            for stage, vals in zip(self, vals_list, strict=True)
        ]

    def unlink_wizard(self, stage_view: bool = False) -> dict:
        wizard = (
            self.with_context(active_test=False)
            .env["project.project.stage.delete.wizard"]
            .create({"stage_ids": self.ids})
        )
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref("project.view_project_project_stage_delete_wizard").id,
                    "form",
                )
            ],
    ...
    def write(self, vals: dict) -> bool:
        if vals.get("company_id"):
            ...
            if project:
                ...
                raise UserError(
                    _(
                        "You are not able to switch the company of this stage to %(company_name)s since it currently "
        ...
        if "active" in vals and not vals["active"]:
            self.env["project.project"].search([("stage_id", "in", self.ids)]).write(
                {"active": False}
            )
    ...
    def action_unarchive(self) -> dict | bool:
        ...
        if stage_active and self.env["project.project"].with_context(
            active_test=False
        ).search_count(
            [("active", "=", False), ("stage_id", "in", stage_active.ids)],
            limit=1,
        ):
            ...
            return {
                ...
                "views": [
                    (
                        self.env.ref(
                            "project.view_project_project_stage_unarchive_wizard"
                        ).id,
                        "form",
                    )
                ],
