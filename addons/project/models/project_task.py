...
class ProjectTask(models.Model):
    ...
    def message_new(self, msg, custom_values=None):
        ...
        defaults = {
            ...
            'email_cc': ", ".join(self._mail_cc_sanitized_raw_dict(msg.get('cc')).values()) if custom_values.get('project_id') else ""
        }
        defaults.update(custom_values)

        # users having email address matched from emails recepients are filtered out and added as assignees to the task
        if msg.get('to'):
            internal_users, partner_emails_without_users, unmatched_partner_emails = self._find_internal_users_from_address_mail(msg.get('to'), defaults.get('project_id'))
        ...
        partners = task._partner_find_from_emails_single(tools.email_split((msg.get('to') or '') + ',' + (msg.get('cc', '') if task.project_id else '')), no_create=True)
