...
class Task(models.Model):
    ...
    milestone_id = fields.Many2one(
        'project.milestone',
        'Milestone',
        domain="[('project_id', '=', project_root_id)]",
    ...
    @api.depends('project_root_id.analytic_account_id')
    def _compute_analytic_account_id(self):
        for task in self:
            task.analytic_account_id = task.project_root_id.analytic_account_id
    ...
    def _compute_stage_id(self):
        for task in self:
            if task.project_root_id:
                if task.project_root_id not in task.stage_id.project_ids:
                    task.stage_id = task.stage_find(task.project_root_id.id, [('fold', '=', False)])
            else:
                task.stage_id = False
    ...
    def default_get(self, default_fields):
        ...
        project_id = vals.get('project_id', self.env.context.get('default_project_id') or\
                              'parent_id' in vals and self.env['project.task'].browse(vals['parent_id']).project_root_id.id)
        if project_id:
    ...
    def _load_records_create(self, vals_list):
        ...
        for task in tasks:
            if task.stage_id and task.stage_id not in task.project_root_id.type_ids and task.stage_id.id not in stage_ids_per_project[task.project_root_id]:
                stage_ids_per_project[task.project_root_id].append(task.stage_id.id)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            project_id = vals.get('project_id', self.env.context.get('default_project_id') or\
                                  "parent_id" in vals and self.browse(vals["parent_id"]).project_root_id.id)
    ...
    def write(self, vals):
        ...
        if 'stage_id' in vals:
            if not 'project_id' in vals and self.filtered(lambda t: not t.project_root_id):
    ...
    def _compute_partner_id(self):
        ...
        for task in self:
            ...
            if not task.partner_id:
                task.partner_id = self._get_default_partner_id(task.project_root_id, task.parent_id)

    @api.depends('project_root_id')
    def _compute_milestone_id(self):
        for task in self:
            if task.project_root_id != task.milestone_id.project_id:
