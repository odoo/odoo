...
class ProjectProject(models.Model):
    ...
    def _inverse_allow_recurring_tasks(self):
        ...
        for project in self:
            if not project.allow_recurring_tasks:
                project.task_ids.recurring_task = False
    ...
    def _check_is_project_feature_enabled(self, field_name):
        return bool(self.env['project.project'].search_count([(field_name, '=', True)], limit=1))

    def _get_project_feature_fields(self):
        return {
    ...
    def check_features_enabled(self, updated_features=None):
        if not self.env.user.has_group('project.group_project_user'):
            return {}
        if updated_features:
            return {field_name: self._check_is_project_feature_enabled(field_name) for field_name in self._get_project_feature_fields() if field_name in updated_features}
        return {field_name: self._check_is_project_feature_enabled(field_name) for field_name in self._get_project_feature_fields()}
