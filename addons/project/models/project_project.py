...
class ProjectProject(models.Model):
    ...
    def create(self, vals_list):
        ...
        for project in projects:
            ...
            if project.privacy_visibility in ['followers', 'invited_users']:
                # Add the current user as follower so that creating a private project does not lock them out.
                project.sudo().message_subscribe(partner_ids=self.env.user.partner_id.ids)
        return projects
    ...
    def _change_privacy_visibility(self, new_visibility):
        ...
        for project in self:
            ...
            if new_visibility in ['followers', 'invited_users']:
                # Keep the current user as follower so that restricting the visibility does not lock them out.
    ...
    def _toggle_template_mode(self, is_template):
        ...
        if not is_template and self.privacy_visibility in ['followers', 'invited_users']:
            # Keep the current user as follower so that converting the template back to a regular project does not lock them out.
