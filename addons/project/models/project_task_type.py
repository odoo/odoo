...
class ProjectTaskType(models.Model):
    ...
    def _default_user_id(self) -> int | bool:
    ...
    rotting_threshold_days = fields.Integer(
        "Days to rot",
        default=0,
        help="Day count before tasks in this stage become stale. Set to 0 to disable \
    ...
    def _compute_rating_request_deadline(self) -> None:
        ...
        for stage in self:
            stage.rating_request_deadline = fields.Datetime.now() + timedelta(
                days=periods.get(stage.rating_status_period, 0)
            )
    ...
    def write(self, vals: dict) -> bool:
        ...
        if "rating_active" in vals:
            ...
            if (not task_types and rating_active) or (
                task_types and task_types <= self and not rating_active
            ):
                ...
                self.env.ref("project.rating_project_request_email_template").active = (
                    rating_active
                )
    ...
    def copy_data(self, default: dict | None = None) -> list[dict]:
        ...
        return [
            ...
            for task_type, vals in zip(self, vals_list, strict=True)
        ]

    @api.ondelete(at_uninstall=False)
    def _unlink_if_remaining_personal_stages(self) -> None:
        ...
        personal_stages_to_update = self.env["project.task.stage.personal"]._read_group(
            [("stage_id", "in", personal_stages.ids)],
            ["stage_id"],
            ["id:recordset"],
        )
        for user in personal_stages.user_id:
            ...
            user_stages_to_unlink = personal_stages.filtered(
                lambda stage, u=user: stage.user_id == u
            )
    ...
    def _prepare_personal_stages_deletion(
        self, remaining_stages_dict: list[dict], personal_stages_to_update: Any
    ) -> None:
    ...
    def _check_personal_stage_not_linked_to_projects(self) -> None:
