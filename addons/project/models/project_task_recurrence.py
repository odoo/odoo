...
class ProjectTaskRecurrence(models.Model):
    ...
    def _get_recurrence_delta(self, deadline):
        ...
        if self.repeat_unit == 'weekday':
            ...
            if not weekdays:
                raise UserError(_("You have to choose at least one day in the week to schedule tasks based on days of the week"))
    ...
    def _create_next_occurrences_values(self, recurrence_by_task):
        ...
        for task, copy_data, fields_to_copy, fields_to_postpone in zip(
            ...
            if (hasattr(task, "planned_date_begin") and task.planned_date_begin):
                delta_ref = task.planned_date_begin
            else:
                delta_ref = task.date_deadline
            if delta_ref:
                delta = recurrence._get_recurrence_delta(delta_ref)
                create_values.update({
