...
QUnit.module('Subtask Kanban List tests', {
    beforeEach: async function () {
        ...
        pyEnv['project.task'].create([
            { name: 'task one', project_id: projectId, open_subtask_count: 2, subtask_count: 3, child_ids: [2, 3, 4], state: '01_in_progress', user_ids: [userId] },
            { name: 'task two', parent_id: 1, open_subtask_count: 0, subtask_count: 0, child_ids: [], state: '03_approved' },
            { name: 'task three', parent_id: 1, open_subtask_count: 0, subtask_count: 0, child_ids: [], state: '02_changes_requested' },
            { name: 'task four', parent_id: 1, open_subtask_count: 0, subtask_count: 0, child_ids: [], state: '1_done' },
            { name: 'task five', open_subtask_count: 0, subtask_count: 1, child_ids: [6], state: '03_approved' },
            { name: 'task six', parent_id: 5, open_subtask_count: 0, subtask_count: 0, child_ids: [], state: '1_canceled' },
        ]);
        this.views = {
            "project.task,false,kanban":
                `<kanban js_class="project_task_kanban">
                    <field name="subtask_count"/>
                    <field name="open_subtask_count"/>
                    <templates>
                        <t t-name="kanban-box">
                            <div>
                                ...
                                <a t-if="record.open_subtask_count.raw_value">
                                    <span title="See Subtasks" class="subtask_list_button fa fa-check-square-o me-1"/>
                                    <t t-out="record.open_subtask_count.value"/>/<t t-out="record.subtask_count.value"/>
                                </a>
                            </div>
