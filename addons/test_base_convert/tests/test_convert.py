...
class TestEvalXML(common.TransactionCase):
    ...
    def test_function_eval(self):
        ...
        obj = Obj('test_base_convert', {}, id_get)

        try:
            test_datetime = ET.XML("<function name='action_test_date' model='test_base_convert.test_model' eval='[datetime.now().strftime(\"%Y-%m-%d %H:%M:%S\")]'/>")
            self.eval_xml(node=test_datetime, obj=obj)
            test_time = ET.XML("<function name='action_test_time' model='test_base_convert.test_model' eval='[time.strftime(\"%Y-%m-%d %H:%M:%S\")]'/>")
            self.eval_xml(node=test_time, obj=obj)
            test_timedelta = ET.XML("<function name='action_test_date' model='test_base_convert.test_model' eval='[(datetime.today()-timedelta(days=365)).strftime(\"%Y-%m-%d %H:%M:%S\")]'/>")
            self.eval_xml(node=test_timedelta, obj=obj)
            test_relativedelta = ET.XML("<function name='action_test_date' model='test_base_convert.test_model' eval='[(datetime.today()+relativedelta(months=3)).strftime(\"%Y-%m-%d %H:%M:%S\")]'/>")
            self.eval_xml(node=test_relativedelta, obj=obj)
            test_timezone = ET.XML("<function name='action_test_timezone' model='test_base_convert.test_model' eval='[pytz.timezone(\"Asia/Calcutta\")]'/>")
    ...
    def test_file(self):
        ...
        obj = Obj('test_base_convert', None)
        self.assertEqual(
            self.eval_xml(Field('test_file.txt', type='file'), obj),
            'test_base_convert,test_file.txt')

        with self.assertRaises(IOError):
            self.eval_xml(Field('test_nofile.txt', type='file'), obj)

    def test_function(self):
        obj = xml_import(self.cr, 'test_base_convert', None, 'init')
    ...
    def test_function_function(self):
        ...
        xml = E.function(
            E.function(model="test_base_convert.usered", name="search", eval="[[]]"),
            model="test_base_convert.usered",
            name="method",
        )
        rec, args, kwargs = self.eval_xml(xml, obj)
