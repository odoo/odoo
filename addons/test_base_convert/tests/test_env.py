...
class TestEnv(common.TransactionCase):
    def setUp(self):
        super().setUp()
        self._importer = xml_import(self.env.cr, 'test_base_convert', None, 'init')
    ...
    def test_uid_data_record(self):
        self.importer(
            odoo(
                record(
                    field("a", name="name"),
                    model="test_base_convert.usered",
                    id="test_base_convert.testing"
                ),
                uid="base.default_user"
            )
        )

        r = self.env.ref('test_base_convert.testing')
    ...
    def test_uid_data_function(self):
        ...
        r = self.env['test_base_convert.usered'].search([])
