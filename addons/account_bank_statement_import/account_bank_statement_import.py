...
class account_bank_statement_import(osv.TransientModel):
    ...
    _columns = {
        ...
        'account_id': fields.many2one('account.account', 'Account', domain="[('type', '!=', 'view')]", help=" Choose the account to be credited/debited for the entries of the statement.", required=True),
    }
    ...
    def import_bank_statement(self, cr, uid, bank_statement_vals=False, context=None):
        statement_id = self.pool.get('account.bank.statement').create(cr, uid, bank_statement_vals, context=context)
        return statement_id

    def process_ofx(self, cr, uid, data_file, journal_id=False, account_id=False, context=None):
        try:
    ...
    def parse_file(self, cr, uid, ids, context=None):
        ...
        vals = getattr(self, "process_%s" % data.file_type)(cr, uid, data.data_file, data.journal_id.id, data.account_id.id, context=context)
        statement_id = self.import_bank_statement(cr, uid, vals, context=context)
        ...
        action['domain'] = "[('id', 'in', ["+', '.join(map(str, [statement_id]))+"])]"
        return action
