    ...
    def get_leave_intervals(self, resource_id=None,
        ...
        for leave in self.leave_ids:
            ...
            if start_datetime and date_to < start_datetime:
                continue
            leaves += [(date_from, date_to, leave.group_id.id)]
        return leaves
    ...
    def interval_remove_leaves(self, interval, leave_intervals):
        ...
        for leave in leave_intervals:
            if len(leave) > 2:
                ...
                if leave[2]:
                    if len(current_interval) > 2:
                        current_group = current_interval[2] and self.env['resource.calendar.attendance'].browse(current_interval[2]).group_id.id or False
            ...
            if current_interval[0] < leave[0] < current_interval[1]:
                ...
                intervals += [(current_interval[0], current_interval[1])]
        ...
        if current_interval and current_interval[0] < interval[1]:  # remove intervals moved outside base interval due to leaves
            if len(interval) > 2:
                intervals += [(current_interval[0], current_interval[1], interval[2])]
            else:
                intervals += [(current_interval[0], current_interval[1], )]
        return intervals
