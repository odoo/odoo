...
class TestAccrualAllocationsWithContracts(TestAccrualAllocations):
    ...
    def setUpClass(cls):
        ...
        cls.test_user = mail_new_test_user(cls.env, login='test_user', company_id=cls.belgian_company.id, groups='base.group_user,hr_holidays.group_hr_holidays_manager')
    ...
    def test_accrual_plan_with_changing_working_schedules_2(self):
        ...
        0,5 (accrual period) * 10 (accrual value) * 100% (working rate) = 5 days (* based on worked time) = 2,5 days
        + 0,5 (accrual period) * 10 (accrual value) * 50% (working rate) = 2,5 days (* based on worked time) = 2,5 days
    ...
    def test_accrual_plan_with_changing_working_schedules_3(self):
        ...
        Value should be computed like this :
        0,5 (accrual period) * 10 (accrual value) * 100% (working rate) = 5 days (* based on worked time) = 5 days
        +0,5 (accrual period) * 10 (accrual value) * 50% (working rate) = 2,5 days (* based on worked time) = 1,25 days
