...
class PaymentTransaction(models.Model):
    ...
    def _extract_reference(self, provider_code, payment_data):
        """Override of `payment` to extract the reference from the Redsys data."""
        if provider_code != 'redsys':
    ...
    def _extract_amount_data(self, payment_data):
        ...
        amount = int(payment_data.get('Ds_Amount')) / 100
        currency = payment_utils.get_currency_alphabetic_code_from_numeric_code(
            payment_data.get('Ds_Currency')
        )
        return {
            'amount': amount,
    ...
    def _apply_updates(self, payment_data):
        ...
        status_code = payment_data.get('Ds_Response')
        if status_code in const.PAYMENT_STATUS_MAPPING['done']:
            ...
            self._set_error(_("Received data with code %s.", payment_data.get('Ds_ErrorCode')))
        else:
            _logger.warning(
