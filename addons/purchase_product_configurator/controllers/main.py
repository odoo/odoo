...
class ProductConfiguratorController(Controller):
    ...
    def get_product_configurator_values(
        ...
        :param str po_date: The date of the `purchase.order`, to compute the price at the right rate.
        ...
        :param int|None partner_id:  The partner_id to use, as a `res.partner`.
    ...
    def purchase_product_configurator_update_combination(
        ...
        if company_id:
            request.update_context(allowed_company_ids=[company_id])
        product_template = request.env['product.template'].browse(product_template_id)
        pricelist = request.env['product.supplierinfo'].search([('partner_id', '=', partner_id), ('product_tmpl_id', '=', product_template_id)], limit=1)
        ...
        return self._get_basic_product_information(
            ...
            uom=product_uom,
            currency=currency,
            date=datetime.fromisoformat(po_date),
        )
    ...
    def _get_product_information(
        ...
        """
        pricelist = request.env['product.supplierinfo'].search([('partner_id', '=', partner_id), ('product_tmpl_id', '=', product_template.id)], limit=1)
        product_uom = request.env['uom.uom'].browse(product_uom_id)
        currency = request.env['res.currency'].browse(currency_id)
        product = product_template._get_variant_for_combination(combination)
                ...
                attribute_values=[dict(
                    **ptav.read(['name', 'html_color', 'is_custom'])[0],
                    price_extra=ptav.currency_id._convert(
                        ...
                        currency,
                        request.env.company,
                        datetime.fromisoformat(po_date).date(),
                    ),
    ...
    def _get_basic_product_information(self, product_or_template, pricelist, combination, **kwargs):
        ...
        return dict(
            **basic_information,
            price=pricelist.price if pricelist else product_or_template.standard_price
        )
