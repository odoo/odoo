...
class TestUI(HttpCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        if cls.env['ir.module.module']._get('website_sale').state != 'installed':
            raise unittest.SkipTest("website_sale module is not installed")
        cls.product = cls.env['product.product'].create({  # noqa: OLS03001
            'name': 'A test product',
    ...
    def test_autocomplete(self):
        with patch.object(AutoCompleteController, '_perform_complete_place_search',
            ...
            self.env.ref('base.default_website').google_places_api_key = MOCK_API_KEY
