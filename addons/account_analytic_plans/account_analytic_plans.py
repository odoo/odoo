...
class account_analytic_plan_instance(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context and context.get('journal_id'):
            ...
            if pids:
                raise Warning(_('Error!'), _('A model with this name and code already exists.'))
            ...
            for i in res:
                ...
                if total_per_plan < item.min_required or total_per_plan > item.max_required:
                    raise Warning(_('Error!'),_('The total should be between %s and %s.') % (str(item.min_required), str(item.max_required)))
        ...
        for line in self.browse(cr, uid, ids, context=context):
           if line.analytics_id:
               if not line.journal_id.analytic_journal_id:
                   raise Warning(_('No Analytic Journal!'),_("You have to define an analytic journal on the '%s' journal.") % (line.journal_id.name,))
            ...
            for st_line in st.line_ids:
                if st_line.analytics_id:
                    if not st.journal_id.analytic_journal_id:
                        raise Warning(_('No Analytic Journal!'),_("You have to define an analytic journal on the '%s' journal.") % (st.journal_id.name,))
