...
class HrEmployee(models.Model):
    ...
    def get_internal_resume_lines(self, res_id, res_model):
        ...
        current_version = employee_versions[0]
        while current_version.next_version_id:
            next_version = current_version.next_version_id
            if not current_version.job_title:
                if interval_date_start:
                    ...
                    res.append({
                        ...
                        'date_end': current_version.date_start + relativedelta(days=-1),
                    })
                    interval_date_start = False
            elif current_version.job_title != next_version.job_title or current_version.date_end + relativedelta(days=1) != next_version.date_version:
                res.append({
                    'id': current_version.id,
                    'job_title': current_version.job_title,
                    'date_start': interval_date_start or current_version.date_start,
                    'date_end': current_version.date_end,
                })
                interval_date_start = False
            else:
                interval_date_start = interval_date_start or current_version.date_start
            current_version = next_version

        if current_version.job_title:
            res.append({
                'id': current_version.id,
                'job_title': current_version.job_title,
                'date_start': interval_date_start or current_version.date_start,
                'date_end': current_version.contract_date_end or False,
            })
