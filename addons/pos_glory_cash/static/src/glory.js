...
export class GloryService extends PaymentInterface {
    setup() {
        ...
        this.state = reactive({ status: "DISCONNECTED", inventory: [], amountInserted: 0 });
    ...
    setupWebsocket() {
        const websocketEndpoint = `ws://${this.payment_method_id.glory_websocket_address}:${WEBSOCKET_PORT}${WEBSOCKET_URL}`;
    ...
    async onConnect() {
        await this.sendWebsocketRequest(WEBSOCKET_REQUESTS.checkCredentials);
        ...
        this.statusChangeHandler(initialStatus);
    ...
    statusChangeHandler(statusResponse) {
    ...
    async sendPaymentRequest() {
        ...
        return await this.handlePaymentResponse(paymentResponse);
    }
    ...
    sendWebsocketRequest(request) {
        this.socketIo.sendMessage(request.requestName);
