...
@contextmanager
def _keep_to_compute(env, fields):
    """ Keep the computed fields depending on ``fields`` as-is, so that they are
    recomputed once the values of ``fields`` are set.
    """
    to_compute = {
        dep: comp
        for f in fields
    ...
    def _register_hook(self):
        ...
        def make_compute_field_value():
            ...
            def _compute_field_value(self, field):
                ...
                # see `test_computation_inside_computation`
                with _keep_to_compute(self.env, changed_fields):
                    for automation in automations.with_context(old_values=old_values):
                        _logger.debug(
