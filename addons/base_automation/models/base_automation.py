...
from odoo.tools import DEFAULT_SERVER_DATETIME_FORMAT, safe_eval, ormcache
...
class BaseAutomation(models.Model):
    ...
    @ormcache()
    def _get_allowed_models_domain(self):
        # Excluding Abstract, Transient, _auto = False models from Automation rules.
        disallow_names = tuple(m._name for m in self.env.registry.values() if (not m._auto or m._abstract))
        model_ids = self.env["ir.model"].sudo().search([("model", "in", disallow_names)]).ids
        return [("id", "not in", model_ids)]

    model_id = fields.Many2one(
        "ir.model", string="Model", domain=_get_allowed_models_domain, required=True, ondelete="cascade",
