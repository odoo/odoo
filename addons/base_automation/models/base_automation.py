...
class BaseAutomation(models.Model):
    ...
    def _compute_trg_date_id(self):
        ...
        for record in (self - to_reset):
            record.trg_date_id = record._get_trigger_related_field()
    ...
    def _compute_trg_field_ref_model_name(self):
        ...
        for automation in to_compute:
            relation = automation._get_trigger_related_field().relation
            if not relation:
    ...
    def _compute_filter_pre_domain(self):
        ...
        for automation in (self - to_reset):
            field = automation._get_trigger_related_field().name
    ...
    def _compute_filter_domain(self):
        for automation in self:
            field = (
                automation._get_trigger_related_field()
    ...
    def _compute_trigger_field_ids(self):
        for automation in self:
            ...
            automation.trigger_field_ids = field
    ...
    def _onchange_filter_domain(self):
        if self.filter_domain and self.trigger == 'on_create_or_write':
            # for "On save", append the fields to the existing list as the
            # trigger fields are visible and may be modified by the user
            domain_fields = [
                part[0].split(".")[0]
                for part in safe_eval.safe_eval(self.filter_domain, self._get_eval_context())
                if is_leaf(part)
            ]
            if domain_fields:
                self.trigger_field_ids |= self.env['ir.model.fields'].search([
    ...
    def _get_trigger_related_field(self):
        self.ensure_one()
        match self.trigger:
            case 'on_stage_set':
