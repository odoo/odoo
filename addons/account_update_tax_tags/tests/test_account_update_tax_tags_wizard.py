...
class TestAccountUpdateTaxTagsWizard(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.tax_1 = cls._create_tax('update_test_tax', tag_names=cls.tag_names)
    ...
    def _change_tax_tag(cls, tax, new_tag, invoice=True, base=True):
        if invoice:
            rep_line = tax.invoice_repartition_line_ids
        else:
            rep_line = tax.refund_repartition_line_ids
        if base:
            filtered_rep_line = rep_line.filtered(lambda rep_line: rep_line.repartition_type == 'base')
        else:
            filtered_rep_line = rep_line.filtered(lambda rep_line: rep_line.repartition_type == 'tax')
        filtered_rep_line.write({'tag_ids': [(6, 0, cls._create_tax_tag(new_tag).ids)]})
    ...
    def test_update_tax_tags(self):
        ...
        self.assertTrue(all('invoice_tax_tag_changed' in tax_line.tax_audit for tax_line in tax_lines), 'Tax audit string sould hva changed.')
        self.assertEqual(invoice_lines.tax_tag_ids.name, 'invoice_base_tag', 'Invoice lines tags should not have changed.')
    ...
    def test_update_multiple_taxes(self):
        ...
        tax_2 = self._create_tax('update_test_tax_2', tag_names={
        ...
        self.assertEqual(
            ...
            'Invoice lines tags should not have changed.'
        )
        self.assertEqual(
        ...
        for move_type in move_types:
            for line_type in ['base', 'tax']:
                with self.subTest(f'Update tax tag on {move_type}-{line_type}'):
                    ...
                    tax_2 = self._create_tax('update_test_tax_2', type_tax_use=type_tax_use, tag_names={
                    ...
                    invoice_line, tax_line, counterpart_line = self._get_amls_by_type(move)
                    if line_type == 'base':
                        self.assertEqual(invoice_line.tax_tag_ids.name, f'{move_type}_{line_type}_tag_changed', 'Invoice lines tags should have changed.')
                        self.assertEqual(tax_line.tax_tag_ids.name, f'test_tag_{super_type}_tax', 'Tax lines tags should not have changed.')
                        self.assertFalse(counterpart_line.tax_tag_ids, 'Counterpart lines tags should not have changed.')
                    else:
                        self.assertEqual(invoice_line.tax_tag_ids.name, f'test_tag_{super_type}_base', 'Invoice lines tags should not have changed.')
    ...
    def test_update_move_type_entry(self):
        ...
        for tax_type in ['sale', 'purchase']:
            tax = self._create_tax(f'test_{tax_type}_tax', amount=10, type_tax_use=tax_type, tag_names=self.tag_names)
            for balance in [-1000, 1000]:
                with self.subTest(f'Testing move type entry {tax_type}: {balance}'):
                    ...
                    if (balance < 0 and tax_type == 'sale') or (balance > 0 and tax_type == 'purchase'):
                        self.assertEqual(move.invoice_line_ids.tax_tag_ids.name, 'invoice_base_tag_changed')
                    elif (balance < 0 and tax_type == 'purchase') or (balance > 0 and tax_type == 'sale'):
                        self.assertEqual(move.invoice_line_ids.tax_tag_ids.name, 'refund_base_tag_changed')
    ...
    def test_update_amls_all_states(self):
        ...
        for move in moves:
            with self.subTest(f'Update tax tag on move with state: {move.state}'):
                invoice_line, tax_line, counterpart_line = self._get_amls_by_type(move)
                self.assertEqual(invoice_line.tax_tag_ids.name, 'invoice_base_tag', 'Invoice lines tags should not have changed.')
    ...
    def test_update_no_tag_before(self):
        ...
        tax = self._create_tax('test_tax_no_tag')
    ...
    def test_update_no_tag_after(self):
        ...
        self.tax_1.invoice_repartition_line_ids.filtered(lambda rep_line: rep_line.repartition_type == 'base').write({'tag_ids': [(5, 0, 0)]})
        self.wizard.update_amls_tax_tags()

        invoice_line, tax_line, counterpart_line = self._get_amls_by_type(move)
        self.assertFalse(invoice_line.tax_tag_ids, 'Invoice lines tags should be empty.')
        self.assertTrue(tax_line.tax_tag_ids, 'Tax lines tags should not have changed.')
        self.assertFalse(counterpart_line.tax_tag_ids, 'Counterpart lines tags should not have changed.')

    def test_update_child_tax(self):
        """ Make sure tax that have children are correctly handled. """
        tax_child_1 = self._create_tax('tax_child_1', type_tax_use='purchase', tag_names={
        ...
        tax_child_2 = self._create_tax('tax_child_2', type_tax_use='none', tag_names={
        ...
        tax_parent = self._create_tax('tax_parent', amount_type='group', type_tax_use='purchase', children_taxes=(tax_child_1 + tax_child_2))
    ...
    def test_update_with_caba_taxes(self):
        tax = self._create_tax('caba_tax', tag_names=self.tag_names, tax_exigibility='on_payment')
        move = self._create_invoice(taxes=tax)
        self._change_tax_tag(tax, 'invoice_tax_tag_changed', invoice=True, base=False)
        self.wizard.update_amls_tax_tags()

        invoice_lines, tax_lines, counterpart_lines = self._get_amls_by_type(move)
        self.assertEqual(invoice_lines.tax_tag_ids.name, 'invoice_base_tag', 'Invoice lines tags should not have changed.')
        self.assertEqual(tax_lines.tax_tag_ids.name, 'invoice_tax_tag_changed', 'Tax lines tags should have changed.')
        self.assertFalse(counterpart_lines.tax_tag_ids, 'Counterpart lines should not have changed.')

    def test_child_tax_multiple_parent_raises(self):
        tax_child = self._create_tax('tax_child_1')
        tax_parent_1 = self._create_tax('tax_parent', amount_type='group', children_taxes=tax_child)
        self._create_tax('tax_parent_2', amount_type='group', children_taxes=tax_child)
