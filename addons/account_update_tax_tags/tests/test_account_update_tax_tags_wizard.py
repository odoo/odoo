...
class TestAccountUpdateTaxTagsWizard(AccountTestInvoicingCommon):
    ...
    def _create_invoice_upd_tags(self, partner_id=None, state=None, **invoice_args):
    ...
    def test_update_tax_tags(self):
        ...
        """
        moves = self._create_invoice_upd_tags(taxes=self.tax_1) + self._create_invoice_upd_tags(taxes=self.tax_1)
    ...
    def test_update_date_from(self):
        ...
        with freeze_time('2023-02-23'):
            move_included = self._create_invoice_upd_tags(taxes=self.tax_1)
        with freeze_time('2023-01-23'):
            move_not_included = self._create_invoice_upd_tags(taxes=self.tax_1)
        self._change_tax_tag(self.tax_1, 'invoice_tax_tag_changed', invoice=True, base=False)
        self.wizard.update_amls_tax_tags()
    ...
    def test_update_multiple_taxes(self):
        ...
        move = self._create_invoice_upd_tags(taxes=self.tax_1 + tax_2)
        self._change_tax_tag(self.tax_1, 'invoice_tax_tag_changed', invoice=True, base=False)
    ...
    def test_update_multi_company(self):
        ...
        move_1 = self._create_invoice_upd_tags(taxes=self.tax_1)
        ...
        move_2 = self._create_invoice_upd_tags(taxes=tax_2)
        ...
        for move_type in move_types:
            for line_type in ['base', 'tax']:
                with self.subTest(f'Update tax tag on {move_type}-{line_type}'):
                    ...
                    move = self._create_invoice_upd_tags(move_type=move_type, taxes=tax_2)
    ...
    def test_update_amls_all_states(self):
        ...
        moves = [self._create_invoice_upd_tags(partner_id=self.partner_a.id, taxes=self.tax_1, state=state) for state in move_states]
    ...
    def test_update_no_tag_before(self):
        ...
        move = self._create_invoice_upd_tags(taxes=tax)
    ...
    def test_update_no_tag_after(self):
        move = self._create_invoice_upd_tags(taxes=self.tax_1)
    ...
    def test_update_child_tax(self):
        ...
        move = self._create_invoice_upd_tags(taxes=tax_parent)
    ...
    def test_update_with_caba_taxes(self):
        ...
        invoice = self._create_invoice_upd_tags(taxes=tax, state='posted')
        # make payment
    ...
    def test_child_tax_multiple_parent_raises(self):
        ...
        self._create_invoice_upd_tags(taxes=tax_parent_1)
