    ...
    def _recompute_tax_audit_string(self, aml_ids):
        ...
        query = f"""
            WITH tags AS (
                ...
                WHERE aml.id IN %s -- FIXME @oco is this really ok ? :/ probably we should "explode" it manually ?
            ),
            tag_values AS (
                SELECT id,
                    array_to_string(
    ...
    def _modify_tag_to_aml_relation(self, company_id, date_from):
        ...
        self.env.cr.execute("""
            ...
            WITH base_aml_id_rep_tag_to_insert AS (
                ...
                LEFT JOIN account_move move_caba
                ON move.tax_cash_basis_move_id = move_caba.id
                ...
                ON
                    ...
                    AND (
                        -- invoice type doc
                        (COALESCE(move_caba.move_type, move.move_type) IN ('in_invoice','out_invoice', 'in_receipt', 'out_receipt') AND tax_to_rep_line.invoice_tax_id = tax.id)
                        OR
                        -- refund type doc
                        (COALESCE(move_caba.move_type, move.move_type) IN ('in_refund','out_refund') AND tax_to_rep_line.refund_tax_id = tax.id) 
                        OR
            ...
            tax_aml_id_rep_tag_to_insert AS (
                SELECT aml.id AS aml_id, rep_tags.account_account_tag_id AS tag_id
                FROM account_move_line aml
                JOIN account_move move ON aml.move_id = move.id AND move.company_id = %(company_id)s AND aml.date >= %(date_from)s
