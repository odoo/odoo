...
class L10nLatamDocumentType(models.Model):
    ...
    sequence = fields.Integer(
        default=10, required=True, help='To set in which order show the'
        ' documents type taking into account the most commonly used first')
    country_id = fields.Many2one(
        'res.country', required=True, index=True, help='Country in which this'
        ' type of document is valid')
    ...
    doc_code_prefix = fields.Char(
        ...
        " and Account Moves. For eg. 'FA ' will build 'FA 0001-0000001'"
        " Document Number")
    code = fields.Char(help='Code used by different localizations')
    report_name = fields.Char(
        'Name on Reports', help='Name that will be printed in reports, for example "CREDIT NOTE"')
    internal_type = fields.Selection(
        [('invoice', 'Invoices'), ('debit_note', 'Debit Notes'), ('credit_note', 'Credit Notes')], index=True,
        help='Analog to odoo account.invoice.type but with more options allowing to identify the kind of document'
        ' we are working with. (not only related to account.invoice, could be for documents of other models like'
        ' stock.picking)')
