...
class AccountAccount(models.Model):
    ...
    def _build_spreadsheet_formula_domain(self, formula_params, default_accounts=False):
        ...
        balance_domain = [("account_id.include_initial_balance", "=", True), ("date", "<=", end)]
        pnl_domain = [("account_id.include_initial_balance", "=", False), ("date", ">=", start), ("date", "<=", end)]
        ...
        if 'account_tag_ids' in formula_params:
            tag_ids = [int(tag_id) for tag_id in formula_params.get("account_tag_ids", []) if tag_id]
            account_id_domain = [('account_id.tag_ids', 'in', tag_ids)] if tag_ids else expression.FALSE_DOMAIN
            ...
            code_domain = expression.OR([("code", "=like", f"{code}%")] for code in codes)
            ...
            account_id_domain = expression.FALSE_DOMAIN

        posted_domain = [("move_id.state", "!=", "cancel")] if formula_params.get("include_unposted") else [("move_id.state", "=", "posted")]

        domain = expression.AND([account_id_domain, period_domain, [("company_id", "=", company_id)], posted_domain])
