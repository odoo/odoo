...
class TestTimezones(TestResourceCommon):
    ...
    def test_unavailable_intervals(self):
        ...
        self.assertEqual(list(next(iter(intervals.values()))), [
            (datetime(2022, 9, 21, 2, 0, tzinfo=UTC), datetime(2022, 9, 21, 3, 0, tzinfo=UTC), self.env['resource.calendar']),
            (datetime(2022, 9, 21, 7, 0, tzinfo=UTC), datetime(2022, 9, 21, 22, 0, tzinfo=UTC), self.env['resource.calendar']),
        ])
    ...
    def test_flexible_resource_leave_interval(self):
        ...
        intervals = self.env['resource.calendar']._leave_intervals_batch(start_dt, end_dt, {UTC: flex_resource})
