...
class WebsitePartnerPage(http.Controller):
    ...
    def partners_detail(self, partner_id, **post):
        ...
        _, partner_id = self.env['ir.http']._unslug(partner_id)
        if partner_id:
            ...
            if partner_sudo.exists() and (partner_sudo.website_published or is_website_restricted_editor):
                if self.env['ir.http']._slug(partner_sudo) != current_slug:
                    return request.redirect('/partners/%s' % self.env['ir.http']._slug(partner_sudo))
                values = {
