...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_default_extra_edi_checkboxes(self, move):
        ...
        if self._is_ro_edi_applicable(move):
            extra_edi['ro_edi'] = {'checked': True, 'readonly': False, 'label': label}
        elif move.l10n_ro_edi_state == 'invoice_sent':
            extra_edi['ro_edi'] = {'checked': False, 'readonly': True, 'label': label, 'question_circle': _("You can't send now. Invoice is waiting for an answer.")}
        return extra_edi
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            if 'ro_edi' in invoice_data['extra_edi']:
