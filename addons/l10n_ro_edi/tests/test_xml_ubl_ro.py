...
class TestUBLRO(TestUBLROCommon):
    ...
    def test_export_constraints(self):
        for required_field in ("city", "street", "state_id", "vat"):
            ...
            invoice = self.create_move("out_invoice", send=False)
            # after the move is built, because creating one writes the seller's
            # own country and VAT back onto the company, and a Romanian
            # company's registry is recomputed from its VAT
