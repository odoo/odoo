...
class mrp_bom(osv.osv):
    ...
    _columns = {
        ...
        'product_tmpl_id': fields.many2one('product.template', 'Product', domain="[('type', 'not in', ['service', 'digital'])]", required=True),
        'product_id': fields.many2one('product.product', 'Product Variant',
            domain="['&', ('product_tmpl_id','=',product_tmpl_id), ('type','not in', ['service', 'digital'])]",
...
class mrp_production(osv.osv):
    ...
    _columns = {
        ...
        'product_id': fields.many2one('product.product', 'Product', required=True, readonly=True, states={'draft': [('readonly', False)]}, 
                                      domain=[('type','not in', ['service', 'digital'])]),
    ...
    def _calculate_qty(self, cr, uid, production, product_qty=0.0, context=None):
        ...
        for scheduled in production.product_lines:
            if scheduled.product_id.type in ['service', 'digital']:
                continue
