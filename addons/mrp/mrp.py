...
class mrp_bom(osv.osv):
    ...
    def _bom_explode(self, cr, uid, bom, product, factor, properties=None, level=0, routing_id=False, previous_products=None, master_bom=None, context=None):
        ...
        for bom_line_id in bom.bom_line_ids:
            ...
            if previous_products and bom_line_id.product_id.product_tmpl_id.id in previous_products:
                raise UserError(_('Invalid Action!'), _('BoM "%s" contains a BoM line with a product recursion: "%s".') % (master_bom.name,bom_line_id.product_id.name_get()[0][1]))
            ...
            if bom_line_id.type != "phantom" and (not bom_id or self.browse(cr, uid, bom_id, context=context).type != "phantom"):
                ...
                raise UserError(_('Invalid Action!'), _('BoM "%s" contains a phantom BoM line but the product "%s" does not have any BoM defined.') % (master_bom.name,bom_line_id.product_id.name_get()[0][1]))
    ...
    def unlink(self, cr, uid, ids, context=None):
        if self.pool['mrp.production'].search(cr, uid, [('bom_id', 'in', ids), ('state', 'not in', ['done', 'cancel'])], context=context):
            raise UserError(_('Warning!'), _('You can not delete a Bill of Material with running manufacturing orders.\nPlease close or cancel it first.'))
...
class mrp_production(osv.osv):
    ...
    def unlink(self, cr, uid, ids, context=None):
        for production in self.browse(cr, uid, ids, context=context):
            if production.state not in ('draft', 'cancel'):
                raise UserError(_('Invalid Action!'), _('Cannot delete a manufacturing order in state \'%s\'.') % production.state)
    ...
    def _action_compute_lines(self, cr, uid, ids, properties=None, context=None):
        ...
        for production in self.browse(cr, uid, ids, context=context):
            ...
            if not bom_id:
                raise UserError(_('Error!'), _("Cannot find a bill of material for this product."))
