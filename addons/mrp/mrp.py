...
class mrp_production(osv.osv):
    ...
    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'state' in init_values and self.state == 'draft' and self.product_qty > 0:
            return 'mrp.mt_manufacturing_order'
        if 'state' in init_values and self.state == 'draft' and self.product_qty < 0:
            return 'mrp.mt_disassemble_order'
        return super(mrp_production, self)._track_subtype(init_values)
    ...
    def action_disassemble(self, cr, uid, ids, qty, context=None):
        mo = self.browse(cr, uid, ids, context=context)
        values = {
            'disassemble': True,
            'origin': mo.name,
            'routing_id': False,
