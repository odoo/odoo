...
class MrpBomLine(models.Model):
    ...
    def _compute_possible_product_template_attribute_value_ids(self):
        for line in self:
            line.possible_product_template_attribute_value_ids = line.product_tmpl_id.valid_product_template_attribute_line_ids._without_no_variant_attributes().product_template_value_ids._only_active()
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            bom_ptav_ids = []
            for orm_command in values.get('bom_product_template_attribute_value_ids', []):
                if orm_command[0] == Command.SET:
                    bom_ptav_ids += orm_command[2]
                else:
                    bom_ptav_ids.append(orm_command[1])
            ...
            selected_ptavs_ids = []
            for orm_command in values.get('selected_product_template_attribute_value_ids', []):
                if orm_command[0] == Command.SET:
                    selected_ptavs_ids += orm_command[2]
                else:
                    selected_ptavs_ids.append(orm_command[1])
            selected_ptavs = self.env["product.template.attribute.value"].browse(selected_ptavs_ids)

            self._check_attribute_values(product_tmpl_id, bom_id, possible_bom_ptavs, bom_ptavs, possible_ptavs, selected_ptavs)
    ...
    def get_product_variant(self, product):
        ...
        template_attribute_lines_values = self.selected_product_template_attribute_value_ids.filtered(lambda v: v.attribute_id not in product.product_template_attribute_value_ids.attribute_id
            or len(self.selected_product_template_attribute_value_ids.filtered(lambda w: w.attribute_id == v.attribute_id)) == 1)
        ...
        for bptav in self.bom_product_template_attribute_value_ids:
            ...
            if not ptav:
                raise UserError(_("Missing attribute value for %(att_name)s on the component %(tmpl_name)s", att_name=bptav.attribute_id.name, tmpl_name=self.product_tmpl_id.name))
            template_attribute_lines_values |= ptav

        # check if there are attributes that need to be specified on the component
        for pal in compo_additional_pal:
            ...
            if len(ptavs) > 1:
                raise UserError(_("Too many attribute values for %(pal)s on the component %(tmpl_name)s", pal=pal.name, tmpl_name=self.product_tmpl_id.name))
    ...
    def _check_attribute_values(self, product_tmpl_id, bom_id, possible_bom_ptavs, bom_ptavs, possible_ptavs, selected_ptavs):
        # Check if too many value selected on component
        ...
        if attribute_values_duplicate:
            ...
            if valid_values_on_component:
                return
            attribute_values_duplicate = bom_ptavs.filtered(lambda v: v.attribute_id in attribute_values_duplicate.attribute_id).product_attribute_value_id
            raise UserError(_("Impossible to match attribute value %(av_name)s found for %(a_name)s on the component %(tmpl_name)s",
                av_name=attribute_values_duplicate[:1].name,
                a_name=attribute_values_duplicate[:1].attribute_id.name,
                tmpl_name=product_tmpl_id.name)
            )

        # Check for missing values on common attributes
        common_pal = (possible_ptavs.attribute_id & possible_bom_ptavs.attribute_id).filtered(lambda pal: pal not in selected_ptavs.attribute_id)
        for pa in common_pal:
            ...
            # The only safe case is when there is values selected for that attribute on both the bom and component product.
            # The length of the bom selected attributes value has to be 1 as this is the only case where it does not need matching
            # The length of the component selected attributes value has already been checked previously
