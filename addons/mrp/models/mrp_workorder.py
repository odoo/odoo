...
class MrpWorkorder(models.Model):
        ...
        for wo in self:
            ...
            if self.env.context.get('prefix_product'):
                if variant := wo.product_id.product_template_attribute_value_ids._get_combination_name():
                    wo.display_name = f"{wo.product_id.name}({variant}) - {wo.production_id.name} - {wo.name}"
                else:
    ...
    def _plan_workorders(self, from_date=False, alternative=True, blocked_by=True):
        ...
        :type Boolean:
        :param blocked_by: An optional `boolean` parameter allowing the planning of the blocked_by_workorder_ids
        :type Boolean:
    ...
    def action_plan(self):
        ...
        self._plan_workorders(from_date=date_to_plan_on, alternative=False, blocked_by=False)
