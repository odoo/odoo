...
class MrpWorkorder(models.Model):
    ...
    def _onchange_finished_lot_id(self):
        ...
        if self.previous_work_order_ids:
            previous_finished_line = self.previous_work_order_ids.finished_workorder_line_ids.filtered(
                lambda line: line.lot_id == self.finished_lot_id
            )
            if previous_finished_line:
                finished_line = self.finished_workorder_line_ids.filtered(
                    lambda line: line.lot_id == self.finished_lot_id
                )
                self.qty_producing = max(0.0, previous_finished_line.qty_done - finished_line.qty_done)
    ...
    def _defaults_from_finished_workorder_line(self):
        ...
        previous_wo = self.previous_work_order_ids
        if previous_wo:
            suggested_qty = previous_wo.qty_produced - self.qty_produced
        else:
            suggested_qty = self.qty_remaining

        if previous_wo and self.product_tracking != 'none':
            for r_line in previous_wo.finished_workorder_line_ids:
                # see which lot we could suggest and its related qty_producing
                if not r_line.lot_id:
                    continue
                candidate = self.finished_workorder_line_ids.filtered(lambda line: line.lot_id == r_line.lot_id)
                if float_compare(candidate.qty_done, r_line.qty_done, precision_rounding=self.product_uom_id.rounding) < 0:
                    suggested_lot = r_line.lot_id.id
                    suggested_qty = min(r_line.qty_done - candidate.qty_done, self.qty_remaining)
                    break
