    ...
    def action_confirm(self):
        ...
        for move in self:
            ...
            if move.picking_type_id:
                moves.extend(self.env['stock.move']._action_explode(move))
            else:
                moves.append(move.id)

        #we go further with the list of ids potentially changed by action_explode
        return super(StockMove, self.browse(moves)).action_confirm()
    ...
    def action_scrap(self, product_qty, location_id, restrict_lot_id=False, restrict_partner_id=False):
        ...
        for move in self:
            ...
            for new_move in new_moves:
                production_ids.write({'move_line_ids': [(4, new_move)]})
                res.append(new_move)
        return res
    ...
    def write(self, vals):
        ...
        if vals.get('state') == 'assigned':
            ...
            for order_id in orders:
                if MrpProduction.browse(order_id).test_ready():
