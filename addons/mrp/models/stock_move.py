    ...
    def _compute_kit_quantities(self, product_id, kit_qty, kit_bom, filters):
        ...
        for bom_line, bom_line_data in bom_sub_lines:
            ...
            if float_is_zero(bom_line_data['qty'], precision_rounding=bom_line.product_uom_id.rounding):
                ...
                continue
            if 'bom_lines' in filters:
                bom_line_filter = lambda move: filters['bom_lines'](move, bom_line)
            else:
                bom_line_filter = lambda m: m.bom_line_id == bom_line
            bom_line_moves = self.filtered(bom_line_filter)
            if bom_line_moves:
