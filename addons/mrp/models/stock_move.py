...
class StockMove(models.Model):
    ...
    def action_explode(self):
        ...
        move_by_company = defaultdict(lambda: self.env['stock.move'])
        for move in self:
            move_by_company[move.company_id] |= move
        for company_id, move_ids in move_by_company.items():
            bom_by_product = self.env['mrp.bom'].sudo()._bom_find(move_ids.product_id, company_id=company_id.id, bom_type='phantom')
            for move in move_ids:
                if (not move.picking_type_id and not self.env.context.get('is_scrap')) or (move.production_id and move.production_id.product_id == move.product_id):
                    ...
                    continue
                bom = bom_by_product[move.product_id]
                if not bom:
                    moves_ids_to_return.add(move.id)
                    continue
                ...
                _boms, lines = bom.sudo().explode(move.product_id, factor, picking_type=bom.picking_type_id)
