...
class MrpBom(models.Model):
    ...
    def _get_bom_ids_from_product_template_ids(self, product_template_ids):
        return self.env['mrp.bom'].search([('product_tmpl_id', 'in', product_template_ids)])
    ...
    def _bom_explode(self, bom, product, factor, properties=None, level=0, routing_id=False, previous_products=None, master_bom=None):
        ...
        if routing:
            for workcenter_use in routing.workcenter_line_ids:
                ...
                result2.append({
                    'name': tools.ustr(workcenter_use.name) + ' - ' + tools.ustr(bom.product_tmpl_id.name_get()[0][1]),
        ...
        for bom_line_id in bom.bom_line_ids:
            ...
            if bom_line_id.bom_type != "phantom" and (not bom_id or self.browse(bom_id).bom_type != "phantom"):
                result.append({
                    'name': bom_line_id.product_id.name,
                    'product_id': bom_line_id.product_id.id,
                    'product_qty': quantity,
                    'product_uom_id': bom_line_id.product_uom_id.id,
                    'product_uos_qty': bom_line_id.product_uos_id and _factor(bom_line_id.product_uos_qty * factor, bom_line_id.product_efficiency, bom_line_id.product_rounding) or False,
                    'product_uos_id': bom_line_id.product_uos_id and bom_line_id.product_uos_id.id or False,
                })
            elif bom_id:
                all_product = [bom.product_tmpl_id.id] + (previous_products or [])
                bom2 = self.browse(bom_id)
                # We need to convert to units/UoM of chosen BoM
                factor2 = self.env['product.uom']._compute_qty(bom_line_id.product_uom_id.id, quantity, bom2.product_uom_id.id)
...
class MrpBomLine(models.Model):
    ...
    product_uos_id = fields.Many2one('product.uom', string='Product UOS', help="Product UOS (Unit of Sale) is the unit of measurement for the invoicing and promotion of stock.", oldname='product_uos')
    product_qty = fields.Float(string='Product Quantity', required=True, default=1.0, digits_compute=dp.get_precision('Product Unit of Measure'))
