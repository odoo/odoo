...
class MrpBom(models.Model):
    ...
    def _bom_find_domain(self, products, picking_type=None, company_id=False, bom_type=False):
        ...
        companies = company_id or self.env.context.get('company_id') or self.env.context.get('allowed_company_ids') or self.env.companies.ids
        if not isinstance(companies, (list, tuple)):
            companies = [companies]
        domain = AND([domain, ['|', ('company_id', '=', False), ('company_id', 'in', companies)]])
        if picking_type:
