...
class MrpBom(models.Model):
    ...
    def _update_order_line_info(self, product, quantity, uom=False, *, child_field=False, **kwargs):
        ...
        bom_line_uom_id = entity.uom_id if entity else product.uom_id
        ...
        if entity:
            if quantity != 0:
                ...
                entity.unlink()
                return self._get_product_catalog_price_and_data(product)
        elif quantity > 0:
        ...
        return {'price': price_unit, 'productUnitPrice': product.standard_price}
...
class MrpBomLine(models.Model):
    ...
    def _get_product_catalog_lines_data(self, default=False, **kwargs):
        if self and not default:
            ...
            product_data = self[0].bom_id._get_product_catalog_price_and_data(self.product_id)
            product_data['price'] = self[0].product_id.uom_id._compute_price(product_data['price'], self[0].uom_id)
            return {
                **product_data,
                'quantity': sum(
                    self.mapped(
                ...
                'productUnitPrice': self[0].product_id.standard_price,
            }
        return {
            'quantity': 0,
        }
