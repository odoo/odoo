...
class StockRule(models.Model):
    ...
    def _get_matching_bom(self, product_id, company_id, values):
        ...
        return product_id.with_context(picking_type=self.picking_type_id, bom_type='normal', company_id=company_id.id).current_bom_id
...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        for company, product_ids in product_by_company.items():
            # Cache and used the prefetch by company
            product_ids.with_context(company_id=company.id, bom_type='phantom').current_bom_id
        for procurement in procurements:
            bom_kit = procurement.product_id.with_context(company_id=procurement.company.id, bom_type='phantom').current_bom_id
            if bom_kit:
