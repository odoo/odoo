...
class StockWarehouse(models.Model):
    ...
    def _get_sequence_values(self, name=False, code=False):
        ...
        values.update({
            'pbm_type_id': {'name': _('%(name)s Sequence picking before manufacturing', name=self.name), 'prefix': self.code + '/' + (self._extract_sequence_code(self.pbm_type_id.sequence_code) or 'PC/'), 'padding': 5, 'company_id': self.company_id.id},
            'sam_type_id': {'name': _('%(name)s Sequence stock after manufacturing', name=self.name), 'prefix': self.code + '/' + (self._extract_sequence_code(self.sam_type_id.sequence_code) or 'SFP/'), 'padding': 5, 'company_id': self.company_id.id},
            'manu_type_id': {'name': _('%(name)s Sequence production', name=self.name), 'prefix': self.code + '/' + (self._extract_sequence_code(self.manu_type_id.sequence_code) or 'MO/'), 'padding': 5, 'company_id': self.company_id.id},
        })
        return values
