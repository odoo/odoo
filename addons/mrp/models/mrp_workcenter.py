...
class MrpWorkcenter(models.Model):
    ...
    def _get_first_available_slot(self, start_datetime, duration, forward=True, leaves_to_ignore=False, simulated_leaves=[]):
        ...
        :param simulated_leaves: fake leaves (start, stop) to consider (see '_simulate_planning')
        :rtype: tuple
        """
        self.ensure_one()
        ...
        simulated_intervals = Intervals([(make_aware(start)[0], make_aware(stop)[0], self.env['resource.calendar.attendance']) for start, stop in simulated_leaves])
            ...
            if forward:
                ...
                for start, stop, _records in available_intervals:
                    ...
                    while (interval := Intervals([(start_interval or start, start + timedelta(minutes=min(remaining, interval_minutes)), _records)])) \
                      and (conflict := interval & workorder_intervals or interval & simulated_intervals):
