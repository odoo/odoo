...
class MrpWorkcenter(models.Model):
    ...
    def _log_block_status(self, loss_reason='', description='', blocking=True):
        ...
        description_template = _('Description: %s') % description_template

        if not blocking:
            message = Markup("<div>%s</div>") % title
        elif not description:
            message = Markup("<div>%(title)s<ul><li>%(reason_template)s</li></div>") % { title: title, reason_template: reason_template }
        else:
