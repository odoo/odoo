...
class MrpProduction(models.Model):
    ...
    def _compute_show_serial_mass_produce(self):
        ...
        for order in self:
            if order.state == 'confirmed' and order.product_id.tracking == 'serial' and \
                ...
                have_serial_components, dummy, dummy = order._check_serial_mass_produce_components()
    ...
    def action_serial_mass_produce_wizard(self):
        ...
        if self.product_id.tracking != 'serial':
            return
        have_serial_components, missing_components, multiple_lot_components = self._check_serial_mass_produce_components()
        if have_serial_components:
            raise UserError(_("Can't work with components tracked by serial number"))
        message = ""
    ...
    def _check_serial_mass_produce_components(self):
        ...
        for production in self:
            ...
            missing_components.update([move.product_id for move in production.move_raw_ids if move.reserved_availability < move.product_uom_qty])
            components = {}
        ...
        return (have_serial_components, missing_components, multiple_lot_components)
    ...
    def _generate_backorder_productions_multi(self, serial_numbers, cancel_remaining_quantities=False):
        ...
        for serial_number in serial_numbers:
            ...
            # ------------------------------------
            # from _generate_backorder_productions
            # ------------------------------------
            ...
            if backorder:
                ...
                # backorder.move_raw_ids.move_line_ids.filtered(lambda ml: ml.product_id.tracking == 'serial' and ml.product_qty == 0).unlink()
