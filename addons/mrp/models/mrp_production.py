...
class MrpProduction(models.Model):
    ...
    def _production_calc(self):
        ...
        for record in self:
            ...
            for workcenter in record.workcenter_line_ids:
                record.hour_total += workcenter.hour
                record.cycle_total += workcenter.cycle
    ...
    def _mrp_from_move(self):
        ...
        res = []
        for move in self:
            res += self.env["mrp.production"].search([('move_line_ids', 'in', move.id)])
        return res

    def _get_mo_ids_from_product_template_ids(self, product_template_ids):
        return self.env["mrp.production"].search([('product_id.product_tmpl_id', 'in', product_template_ids)])
    ...
    product_uos_id = fields.Many2one('product.uom', string='Product UoS', readonly=True, states={'draft': [('readonly', False)]}, oldname='product_uos')
    ...
    def product_id_change(self):
        ...
        if product.uos_id.id:
            ...
            self.product_uos_id = product.uos_id.id
    ...
    def _prepare_lines(self, production, properties=None):
        ...
        if not bom_point:
            bom_id = MrpBom._bom_find(product_id=production.product_id.id, properties=properties)
            if bom_id:
        ...
        factor = self.env['product.uom']._compute_qty(production.product_uom_id.id, production.product_qty, bom_point.product_uom_id.id)
        # product_line_ids, workcenter_line_ids
        return MrpBom._bom_explode(bom_point, production.product_id, factor / bom_point.product_qty, properties, routing_id=production.routing_id.id)
    ...
    def action_cancel(self):
        ...
        for production in self:
            ...
            procs = self.env['procurement.order'].search([('move_dest_id', 'in', [record.id for record in production.move_line_ids])])
            if procs:
                procs.cancel()
    ...
    def _calculate_qty(self, production, product_qty=0.0):
        ...
        if not product_qty:
            product_qty = self.env["product.uom"]._compute_qty(production.product_uom_id.id, production.product_qty, production.product_id.uom_id.id) - produced_qty
        production_qty = self.env["product.uom"]._compute_qty(production.product_uom_id.id, production.product_qty, production.product_id.uom_id.id)

        scheduled_qty = OrderedDict()
        for scheduled in production.product_line_ids:
            if scheduled.product_id.type == 'service':
                continue
            qty = self.env["product.uom"]._compute_qty(scheduled.product_uom_id.id, scheduled.product_qty, scheduled.product_id.uom_id.id)
    ...
    def _make_production_produce_line(self, production):
        ...
        data = {
            ...
            'product_uos_qty': production.product_uos_id and production.product_uos_qty or False,
            'product_uos': production.product_uos_id and production.product_uos_id.id or False,
    ...
    def _make_production_consume_line(self, line):
        return self._make_consume_line_from_data(line.production_id, line.product_id, line.product_uom_id.id, line.product_qty, line.product_uos_id.id, line.product_uos_qty)
...
class MrpProductionProductLine(models.Model):
    ...
    product_uos_id = fields.Many2one('product.uom', string='Product UOS', oldname='product_uos')
