...
class MrpProduction(models.Model):
    ...
    def button_plan(self):
        ...
        for order in orders_to_plan:
            # Compute the in-degree of the queued orders. It is important to
            # only count edges generated by dependencies queued for planning.
            created_id = order.procurement_group_id.stock_move_ids.created_production_id
            dependencies = created_id.procurement_group_id.mrp_production_ids - order
            finished_dates = dependencies.mapped('date_planned_finished')
            edge_counts[order] = len(dependencies & orders_to_plan)
            start_dates[order] = max(finished_dates, default=fields.Datetime.now())
        ...
        while len(queued_orders) > 0:
            ...
            if start_dates[order] > order.date_planned_start:
                order.date_planned_start = start_dates[order]
            ...
            for dependent in dependents.filtered(lambda order: order in orders_to_plan):
                start_dates[dependent] = max(start_dates[dependent], order.date_planned_finished)
