...
class MrpProduction(models.Model):
    ...
    def _compute_bom_id(self):
        ...
        boms = self.env['mrp.bom'].with_context(active_test=True)._bom_find(self.product_id, picking_type=picking_type, company_ids=self.company_id.ids, bom_type='normal')
        for production in self:
            if not production.product_id:
                production.bom_id = False
            elif not production.bom_id or production.bom_id.product_tmpl_id != production.product_tmpl_id or (production.bom_id.product_id and production.bom_id.product_id != production.product_id):
                bom = boms[(production.product_id, production.company_id.id)]
