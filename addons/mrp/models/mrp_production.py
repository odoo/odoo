...
class MrpProduction(models.Model):
    ...
    def action_reset_to_draft(self):
        ...
        self.env['stock.picking'].search([('group_id', '=', self.procurement_group_id.id)]).write({
            'group_id': False,
        })
        ...
        for move in cancelled_moves:
            move.write({'move_dest_ids': [(3, m.id) for m in move.move_dest_ids.filtered(lambda m: m.state == 'cancel')]})
            move.copy()
