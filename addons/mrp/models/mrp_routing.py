...
class MrpRoutingWorkcenter(models.Model):
    ...
    def _simulate_planning(self, product, start_date, quantity, planning_per_operation=False, simulated_leaves_per_workcenter=False):
        ...
        for op in self.blocked_by_operation_ids:
            if op._skip_operation_line(product):
                continue
            if op not in planning_per_operation:
                op._simulate_planning(product, start_date, quantity, planning_per_operation, simulated_leaves_per_workcenter)
        ...
        for workcenter in workcenters:
            ...
            duration_expected = self.with_context(product=product, quantity=quantity, workcenter=workcenter).time_total
