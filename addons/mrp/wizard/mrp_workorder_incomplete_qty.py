...
class MrpWorkorderIncompleteQty(models.TransientModel):
    ...
    def action_validate(self):
        ...
        res_model = 'mrp.production' if self.calling_method == 'pre_button_mark_done' else 'mrp.workorder'
        if self.calling_method in allowed_methods and hasattr(self.env[res_model], self.calling_method):
            try:
                if res_model == 'mrp.production':
                    return getattr(self.workorder_ids.production_id.with_context(skip_check_qty_on_set_state_done=True), self.calling_method)(**caller_kwargs)
                return getattr(self.workorder_ids.with_context(skip_check_qty_on_set_state_done=True), self.calling_method)(**caller_kwargs)
            except AttributeError as e:
                raise ValidationError(str(e)) from e
        else:
            raise ValidationError(
                f"Method '{self.calling_method}' is not allowed or not found in '{res_model}'. "
