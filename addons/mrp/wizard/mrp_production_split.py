    ...
    def _compute_details(self):
        for wizard in self:
            ...
            if wizard.counter < 1 or not wizard.production_id:
                ...
                continue
            quantity = float_round(wizard.product_qty / wizard.counter, precision_rounding=wizard.uom_id.rounding)
            ...
            for _ in range(wizard.counter - 1):
                ...
                remaining_quantity = float_round(remaining_quantity - quantity, precision_rounding=wizard.uom_id.rounding)
            commands.append(Command.create({
    ...
    def _compute_valid_details(self):
        ...
        for wizard in self:
            if wizard.production_detailed_vals_ids:
                wizard.valid_details = float_compare(wizard.product_qty, sum(wizard.production_detailed_vals_ids.mapped('quantity')), precision_rounding=wizard.uom_id.rounding) == 0
