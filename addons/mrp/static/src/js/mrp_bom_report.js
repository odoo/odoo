...
var MrpBomReport = stock_report_generic.extend({
    ...
    get_html: function() {
        ...
        var args = [
            ...
            parseInt(this.given_context.searchVariant),
            this.given_context.report_name || 'all',
        ];
        return this._rpc({
    ...
    renderSearch: function () {
        ...
        this.$searchView.find('.bom_report').on('change', this._onChangeType.bind(this));
    },
    _onClickPrint: function (ev) {
        ...
        framework.blockUI();
        var reportname = 'mrp.report_bom_structure?docids=' + this.given_context.active_id + '&report_name=' + (this.given_context.report_type || 'pdf' || 'qweb-pdf') + '&quantity=' + (this.given_context.searchQty || 1) +
                          '&childs=' + JSON.stringify(childBomIDs);
        reportname += '&variant=' + (this.given_context.searchVariant || 1);
        var action = {
