...
class ReportMrpLabelProductionPdf(models.AbstractModel):
    ...
    def _prepare_label(self, move_line, barcode, label_quantity):
        return {
            'barcode': barcode,
    ...
    def _prepare_labels(self, docs, uom_unit):
        labels = []
        ...
        for move_line in move_lines:
            ...
            for _index in range(int(move_line.quantity) if is_unit else 1):
                labels.append(self._prepare_label(move_line, barcode, label_quantity))
        return labels

    def _prepare_data(self, docids, data):
        docs = self.env['mrp.production'].browse(docids)
        uom_unit = self.env.ref('uom.product_uom_unit')
        label_pages = self._organize_labels(self._prepare_labels(docs, uom_unit), SimpleNamespace(rows=12, columns=4))
        return {
            'doc_ids': docids,
