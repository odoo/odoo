    ...
    def _get_bom_lines(self, bom, bom_quantity, product, line_id, level):
        ...
        for line in bom.bom_line_ids:
            ...
            if line.child_bom_id:
                ...
                sub_total = price
            sub_total = self.env['res.company']._get_current_company().currency_id.round(sub_total)
            components.append({
                ...
                'prod_cost': self.env['res.company']._get_current_company().currency_id.round(price),
                'parent_id': bom.id,
                'line_id': line.id,
                'level': level or 0,
                'total': sub_total,
    ...
    def _get_operation_line(self, routing, qty, level):
        ...
        for operation in routing.operation_ids:
            ...
            operations.append({
                ...
                'total': self.env['res.company']._get_current_company().currency_id.round(total),
            })
        return operations
    ...
    def _get_price(self, bom, factor):
        ...
        for line in bom.bom_line_ids:
            if line.child_bom_id:
                ...
                price += self.env['res.company']._get_current_company().currency_id.round(not_rounded_price)
        return price
