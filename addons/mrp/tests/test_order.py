...
class TestMrpOrder(TestMrpCommon):
    ...
    def test_compute_picking_type_id(self):
        ...
        # All stock BoMs in tests have a picking type defined, so we need to remove it first
        self.bom_2.picking_type_id = False
        mo_form = Form(self.env['mrp.production'])
        mo_form.bom_id = self.bom_2
        mo_2 = mo_form.save()
    ...
    def test_batch_production_03(self):
        ...
        mo_form.bom_id = self.env['mrp.bom'].create({
            ...
            'type': 'normal',
            'bom_line_ids': [
                Command.create({'product_id': self.product_2.id, 'product_qty': 2, 'manual_consumption': True}),
                Command.create({'product_id': self.product_1.id, 'product_qty': 4, 'manual_consumption': True}),
            ]})
        mo = mo_form.save()
        mo.action_confirm()
        mo.action_assign()
