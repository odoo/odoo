...
class TestMrpOrder(TestMrpCommon, MailCase):
    ...
    def test_components_availability(self):
        ...
        mo = self.env['mrp.production'].create({
            'bom_id': self.bom_3.id,  # product_5 (2), product_4 (8), product_2 (12)
            'date_start': now,
        })
    ...
    def test_product_produce_different_uom(self):
        ...
        mo_product_final_form = self.env['mrp.production'].create({
            'product_id': product_final.id,
            'uom_id': uom_L.id,
            'bom_id': test_bom.id,
            'product_qty': 0.5,
    ...
    def test_compute_tracked_time_2_under_capacity(self):
        ...
        production = self.env['mrp.production'].create({
            'bom_id': self.bom_5.id,
            'product_qty': 2,  # We want to produce 2 items (the capacity) now
        })
