...
class TestMrpStockReports(TestReportsCommon):
    ...
    def test_mo_overview_same_component(self):
        ...
        #    - 1 'kit':
        #        - 1 'part'
        part, kit, finished = self.env['product.product'].create([
            {
                'name': name,
                'type': 'product',
            } for name in ['Part', 'Kit', 'Finished']
        ])
        _, bom = self.env['mrp.bom'].create([
            {
                ...
                'type': bom_type,
                'bom_line_ids': [
                    (0, 0, {'product_id': part.id, 'product_qty': 1.0})
                ],
            } for product, bom_type in zip([kit, finished], ['phantom', 'normal'])
        ])
        bom.bom_line_ids += self.env['mrp.bom.line'].create({
            'product_id': kit.id,
            'product_qty': 1.0,
            'bom_id': bom.id,
        })
