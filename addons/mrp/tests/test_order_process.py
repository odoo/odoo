...
class TestOrderProcess(TestOrderDemo):
   ...
   # I check details of Produce Move of Production Order to trace Final Product.
        ...
        self.assertEqual(move.product_uos_qty, self.mrp_production_test1.product_uos_id and self.mrp_production_test1.product_qty or self.mrp_production_test1.product_uos_qty, "UOS qty does not correspond.")
        if self.mrp_production_test1.product_uos_id:
            self.assertEqual(move.product_uos.id, self.mrp_production_test1.product_uos_id.id, "UOS does not correspond.")
        ...
        for move_line in self.mrp_production_test1.move_line_ids:
            for order_line in self.mrp_production_test1.product_line_ids:
                ...
                if move_line.product_id.id == order_line.product_id.id:
                    ...
                    self.assertEqual(move_line.product_uos_qty, order_line.product_uos_id and order_line.product_qty or order_line.product_uos_qty, "UOS qty is not correspond in 'To consume line'.")
                    if order_line.product_uos_id:
                        self.assertEqual(move_line.product_uos.id, order_line.product_uos_id.id, "UOS is not correspond in 'To consume line'.")
        ...
        for proc in self.env['procurement.order'].browse(procurements):
            for order_line in self.mrp_production_test1.product_line_ids:
                ...
                if proc.product_id.id == order_line.product_id.id:
                    ...
                    self.assertEqual(proc.product_uos_qty ,order_line.product_uos_id and order_line.product_uos_qty or order_line.product_qty, "UOS qty does not correspond in procurement.")
                  ...
                  # case it could already be in `running` or `exception` state (not enough stock)
                    ...
                    if order_line.product_uos_id:
                        self.assertEqual(proc.product_uos_id.id, order_line.product_uos_id.id, "UOS is not correspond in procurement.")
