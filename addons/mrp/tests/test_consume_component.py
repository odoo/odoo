...
class TestConsumeComponent(TestConsumeComponentCommon):
    ...
    def test_automatic_consume_new_added_component(self):
        ...
        """
        sfg_product = self.env['product.product'].create({
            'name': 'SFG Product',
            'is_storable': True,
        ...
        compo1 = self.env['product.product'].create({
            'name': 'Compo 1',
            'is_storable': True,
            'categ_id': self.env.ref('product.product_category_all').id,
            'tracking': 'none',
        ...
        compo2 = self.env['product.product'].create({
            'name': 'Compo 2',
            'is_storable': True,
            'categ_id': self.env.ref('product.product_category_all').id,
            'tracking': 'none',
        ...
        compo3 = self.env['product.product'].create({
            'name': 'Compo 3',
            'is_storable': True,
            'categ_id': self.env.ref('product.product_category_all').id,
            'tracking': 'none',
        ...
        quant |= self.create_quant(compo3, 10)
        quant.action_apply_inventory()
        ...
        self.create_bom_lines(bom, [compo1, compo2], [1, 1])
