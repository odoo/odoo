...
class TestSmsTwilio(MockSmsTwilioApi, SMSCase, common.TransactionCase):
    ...
    def test_multi_company_diff_providers(self):
        ...
        with patch.object(SmsSms, 'send', autospec=True, wraps=SmsSms, side_effect=patch_send):
            # First create the SMS by going through the mail.thread
            ...
            sms_twilio_mail = self.env['sms.sms'].sudo().search([], limit=1)  # Last one created
            ...
            sms_iap_mail = self.env['sms.sms'].sudo().search([], limit=1)  # Last one created

            # Second, create the "raw" SMS (for other flows that do not fill the `sms.mail_message_id.record_company_id`
            sms_twilio = self.env['sms.sms'].with_company(company_twilio).create({
                'body': 'SMS by Twilio',
                'number': self.valid_phone_number,
            })
            sms_iap = self.env['sms.sms'].with_company(company_iap).create({
                'body': 'SMS by IAP',
                'number': self.valid_phone_number,
            })
        ...
        self.assertRecordValues(sms_twilio_mail, [{"state": "outgoing", "body": "SMS by Twilio"}])
        self.assertRecordValues(sms_iap_mail, [{"state": "outgoing", "body": "SMS by IAP"}])
        ...
        self.assertRecordValues(sms_twilio_mail, [{"state": "pending", "sms_twilio_sid": "SMfake"}])
        self.assertRecordValues(sms_iap_mail, [{"state": "pending", "sms_twilio_sid": False}])

        self.assertRecordValues(sms_twilio, [{"state": "pending", "sms_twilio_sid": False}])  # Will be deleted anyway, so no need to store the SID as it's on the tracker and we have don't have any in this case
