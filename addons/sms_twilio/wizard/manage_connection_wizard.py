...
class SmsTwilioManageConnectionWizard(models.TransientModel):
    ...
    sms_twilio_success_msg = fields.Char("Success message", readonly=True)
    sms_twilio_error_msg = fields.Char("Error message", readonly=True)
    ...
    def action_test(self):
        ...
        if not message_notif.failure_type:
            self.sms_twilio_success_msg = _("The SMS has been sent from %s", get_twilio_from_number(self.company_id, self.sms_twilio_to_number).display_name)
            self.sms_twilio_error_msg = False
        else:
            self.sms_twilio_success_msg = False
            if message_notif.failure_type != "unknown":
                self.sms_twilio_error_msg = dict(message_notif._fields["failure_type"].selection).get(message_notif.failure_type)  # Translated error msg
            else:
                self.sms_twilio_error_msg = message_notif.failure_reason  # Not translated, coming from Twilio
        temp_partner.unlink()
        return self.company_id._action_sms_twilio_open_manage_connection_wizard(self)
