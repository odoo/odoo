...
import { SCHEME_TEMPLATE, FIELD_TEMPLATE, PARTITION} from "./epc_model.js";
...
function decodeString(model, fieldName){
    ...
    for (let i = 0; i < charLength; i++) {
        let res = Number(readBits(value, i * 7, 7, field.bitSize));
        if (res === 0){
            break;
        }
        charCodes[i] = Number(readBits(value, i * 7, 7, field.bitSize));
    }
    // parse to UTF string
...
function toPureUri(model){
    ...
    for (let i = 0; i < fieldEntries.length; i++) {
        let index = uriBody.indexOf(fieldEntries[i][1].uriPortion);
        if(index !== -1){
...
function getAi414(model){
    ...
    return ' (414) ' + glnToCheck + checkdigit
}
...
//public
export async function decode(hexString, asElementString = false){
    // Prepare values
    ...
    for (let i = 1; i < fieldEntries.length; i++) {
        ...
        let previousField = fieldEntries[i - 1][1];
...
registry.category("services").add("epc_service", EPCService);
