...
test("Test Alternative GS1 Separator (fnc1)", async () => {
    ...
    expect(() => {
        barcodeNomenclature.gs1_decompose_extended(BarcodePlugin.prototype.cleanBarcode(code128));
    }).toThrow();

    // Reload the nomenclature but this time using '#' as separator.
    barcodeNomenclature = buildBarcodeParser({ gs1_separator_fnc1: "#" });
    const res = barcodeNomenclature.gs1_decompose_extended(BarcodePlugin.prototype.cleanBarcode(code128));
    expect(res.length).toBe(3);
