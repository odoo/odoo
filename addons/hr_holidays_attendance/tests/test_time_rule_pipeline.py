...
class TestTimeRulePipeline(TransactionCase):
    ...
    def test_output_attendance_counts_in_overtime(self):
        ...
        data = self.cal_emp.get_attendance_data_by_employee(start, stop)
    ...
    def test_flex_overlapping_leaves_no_singleton(self):
        ...
        sick_type = self.env['hr.work.entry.type'].search([('code', '=', '013.00')], limit=1)
        public_type = self.env['hr.work.entry.type'].create({
    ...
    def test_get_attendance_data_worked_hours_and_overtime_hours(self):
        ...
        data = self.cal_emp.get_attendance_data_by_employee(
    ...
    def test_two_quantity_rules_priority(self):
        ...
        # sources may be type-changed in-place (time_rule_id set but no child link)
...
class TestTimeRuleCronBehavior(TransactionCase):
    ...
    def _outputs_for(self, att_id, ot_type=None):
        ...
        if att.time_rule_id and not att.source_attendance_id:
            all_outputs = att
            if ot_type:
...
class TestTimeRulePipelineLeaves(TransactionCase):
    ...
    def _make_leave(self, date_from_dt, date_to_dt, *, state='validate', skip_rules=False, emp=None, wet=None):
        ctx = dict(leave_fast_create=True, leave_skip_date_from_to_computation=True, leave_skip_state_check=True)
        if skip_rules:
    ...
    def test_write_check_out_extended_triggers_reprocess(self):
        ...
        try:
            ...
            outputs = self.env['hr.attendance'].search([
                ('source_attendance_id', '=', att.id), ('time_rule_id', '!=', False),
            ])
    ...
    def test_write_date_extended_triggers_reprocess(self):
        ...
        try:
            ...
            leave.with_context(skip_time_rules=False, leave_skip_date_from_to_computation=True, leave_skip_state_check=True).sudo().write({
    ...
    def test_day_and_week_cron_independence_undertime_attendance(self):
        ...
        def _att_outputs(att, wet=None):
            domain = [('source_attendance_id', '=', att.id), ('time_rule_id', '!=', False)]
            if wet:
    ...
    def test_deficit_partial_fill_when_period_mostly_blocked(self):
        """Deficit gap trims from the START of the available schedule; a blocker before
        the gap shrinks the schedule but does not interfere with the gap itself.

        The company default calendar is duration-based 8h/day centered at 12:00 → 08:00-16:00.
        Both source and blocker are subtracted from the schedule before computing the gap.

        Source:  08:00-09:00 (1h, src_type) -> threshold=3h -> deficit=2h.
        Blocker: 10:00-11:00 (out_type, not in condition).
        Schedule after leaves subtracted: (09:00-10:00, 11:00-16:00) = 6h.
        Trim 6h-2h=4h from start -> deficit gap at 14:00-16:00.
        """
        undertime_type = self.env['hr.work.entry.type'].create({
        ...
        try:
            ...
            # blocker within working hours but before the deficit window
            self._make_leave(
                datetime(2022, 12, 12, 10), datetime(2022, 12, 12, 11),
            ...
            total_hours = sum((o.date_to - o.date_from).total_seconds() / 3600 for o in outputs)
            self.assertAlmostEqual(total_hours, 2.0, places=5,
                msg="Full 2h deficit placed at 14:00-16:00; blocker at 10:00-11:00 ends before the gap")
            self.assertEqual(len(outputs), 1, "Single contiguous output record expected")
            self.assertEqual(outputs.date_from, datetime(2022, 12, 12, 14, 0),
                             "Deficit output starts at 14:00 (end of trimmed gap)")
            self.assertEqual(outputs.date_to, datetime(2022, 12, 12, 16, 0),
                             "Deficit output ends at 16:00 (end of working day)")
        finally:
            rule.write({'active': False})
