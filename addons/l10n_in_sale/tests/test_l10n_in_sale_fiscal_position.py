...
class TestSaleFiscal(L10nInTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        # Remove fiscal position 'property_account_position_id' so it does not show up on invoice
    ...
    def test_l10n_in_sale_fiscal_position(self):
        ...
        with self.subTest(scenario="Inter-State"):
            ...
            # Here fpos should Intra-State. But due to `l10n_in_gst_treatment` it will be Export/SEZ
            sale_order.write({
                'partner_id': self.partner_a.id,  # Intra-State Partner
                'l10n_in_sale_state_id': company_state.id,  # Company State
    ...
    def test_l10n_in_sale_fpos_in_bill_to_ship_to(self):
        self.env.company = self.default_company
        company_state = self.env.company.state_id
        ...
        # Inter-State
        sale_order.write({
            'l10n_in_sale_state_id': other_state.id,
        })
        self.assertEqual(
            sale_order.fiscal_position_id,
        ...
        sale_order.write({
            'l10n_in_sale_state_id': self.env.ref('l10n_in.state_in_oc').id,  # Other Country State
        })
        self.assertEqual(
            sale_order.fiscal_position_id,
