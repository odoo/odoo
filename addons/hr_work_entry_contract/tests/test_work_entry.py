...
class TestWorkEntry(TestWorkEntryBase):
    ...
    def test_work_entry_different_calendars(self):
        ...
        emp_flex_std = self.env['hr.employee'].create({'name': 'emp flex std'})
        self.env['hr.contract'].create({
        ...
        emp_std_flex = self.env['hr.employee'].create({'name': 'emp std flex'})
        self.env['hr.contract'].create({
        ...
        emp_fullyflex_std = self.env['hr.employee'].create({'name': 'emp fullyflex std'})
        self.env['hr.contract'].create({
        ...
        emp_std_fullyflex = self.env['hr.employee'].create({'name': 'emp std fullyflex'})
        self.env['hr.contract'].create({
        ...
        emp_flex_std.generate_work_entries(date(2025, 9, 1), date(2025, 9, 30))
        flex_std_work_entries = self.env['hr.work.entry'].search([('employee_id', '=', emp_flex_std.id)])
        ...
        half2_entries = flex_std_work_entries.filtered(lambda e:
            e.date_start.astimezone() >= half2_date_start
        ...
        self.assertEqual(len(half2_entries), 22)
        ...
        emp_std_flex.generate_work_entries(date(2025, 9, 1), date(2025, 9, 30))
        std_flex_work_entries = self.env['hr.work.entry'].search([('employee_id', '=', emp_std_flex.id)])
        ...
        half2_entries = std_flex_work_entries.filtered(lambda e:
            ...
            and e.date_stop.astimezone(tz) <= half2_date_end)
        self.assertEqual(len(half1_entries), 22)
        ...
        emp_fullyflex_std.generate_work_entries(date(2025, 9, 1), date(2025, 9, 30))
        fullyflex_std_work_entries = self.env['hr.work.entry'].search([('employee_id', '=', emp_fullyflex_std.id)])
        ...
        half2_entries = fullyflex_std_work_entries.filtered(lambda e:
            e.date_start.astimezone(tz) >= half2_date_start
            and e.date_stop.astimezone(tz) <= half2_date_end)
        self.assertEqual(len(half1_entries), 1)
        ...
        emp_std_fullyflex.generate_work_entries(date(2025, 9, 1), date(2025, 9, 30))
        std_fullyflex_work_entries = self.env['hr.work.entry'].search([('employee_id', '=', emp_std_fullyflex.id)])
        ...
        self.assertTrue(all(entry.duration == 4 for entry in half1_entries))
        self.assertTrue(all(entry.contract_id.name == 'Std Contract' for entry in half1_entries))
        self.assertEqual(len(half2_entries), 1)
