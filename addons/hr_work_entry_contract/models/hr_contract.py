...
class HrContract(models.Model):
    ...
    def _get_attendance_intervals(self, start_dt, end_dt):
        ...
        for calendar, employees in employees_by_calendar.items():
            if not calendar:
                result.update({employees.resource_id.id: WorkIntervals([(start_dt, end_dt, self.env['resource.calendar.attendance'])])})
            else:
    ...
    def generate_work_entries(self, date_start, date_stop, force=False):
        ...
        for contract in self:
            contracts_by_company_tz[(
                contract.company_id,
                ((contract.resource_calendar_id or contract.employee_id.resource_calendar_id).tz or contract.employee_id.tz)
            )] += contract
    ...
    def _generate_work_entries(self, date_start, date_stop, force=False):
        ...
        utc = pytz.timezone('UTC')
        for contract in self:
            contract_tz = (contract.resource_calendar_id or contract.employee_id.resource_calendar_id).tz or contract.employee_id.tz
            tz = pytz.timezone(contract_tz) if contract_tz else pytz.utc
