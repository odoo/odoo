...
class RestaurantPrinter(models.Model):
    ...
    printer_type = fields.Selection(string='Printer Type', default='iot',
        selection=[('iot', ' Use a printer connected to the IoT Box'), ('star_webprnt', 'Use a Star printer')])
    proxy_ip = fields.Char('Proxy IP Address', help="The IP Address or hostname of the Printer's hardware proxy")
    star_printer_ip = fields.Char(string='Star Receipt Printer IP Address', help="Local IP address of a Star receipt printer. This should only be set if you want to use a Star Printer witout IoT Box.")
