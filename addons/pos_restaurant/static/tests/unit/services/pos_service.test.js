...
describe("restaurant pos_store.js", () => {
    ...
    test("preparation receipt order_name", async () => {
        const store = await setupPosEnv();
        ...
        expect(orderChange[0].extra_data.order_name).toBe(partner.name);
        ...
        expect(tableOrderChange[0].extra_data.order_name).toBe(partner.name);
