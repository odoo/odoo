...
export class FloorScreen extends Component {
        ...
        useDraggable({
            ...
            onDrag: ({ element, x, y }) => {
                ...
                if (!suggestLinkingPositions()) {
                    // FIXME: this is hacky; we shouldn't use data.withoutSyncing here
                    const move = () => {
                        table.position_h =
                    ...
                    this.pos.data.withoutSyncing(move);
            ...
            onDrop: ({ element }) => {
                ...
                const table = this.getPosTable(element);
                if (this.pos.isEditMode) {
                    // This is a hack in order to trigger the syncing mechanism
                    table.position_h = table.position_h + 0.1;
                    table.position_v = table.position_v + 0.1;
                    return;
                }
                ...
                console.log(this.state.potentialLink);
                if (!suggestLinkingPositions()) {
                ...
                console.log("drop");
                table.parent_id = this.state.potentialLink.parent;
                this.state.potentialLink = null;
            },
        });
        this.useResizeHook();
    ...
    async _createTableHelper(copyTable, duplicateFloor = false) {
        ...
        let newTableData;
        if (copyTable) {
            newTableData = omit(copyTable.serializeForORM(), "uuid", "id");
            if (!duplicateFloor) {
    ...
    get activeTables() {
        ...
        return this.pos.models["restaurant.table"].filter(
            (table) => table.floor_id?.id === this.activeFloor?.id
        );
    }
    get selectedTables() {
    ...
    async addFloor() {
        // FIXME: Unable to modify this PoS Configuration because you can't modify Restaurant Floors while a session is open.
    ...
    async duplicateFloor() {
        ...
        for (const table of tables) {
            const tableSerialized = omit(table.serializeForORM(), "uuid", "id");
            tableSerialized.floor_id = copyFloor.id;
    ...
    async deleteTable() {
        ...
        if (this.selectedTables.some((table) => table.getOrder())) {
            this.dialog.add(AlertDialog, {
                title: _t("User Error"),
    ...
    async uploadImage(event) {
        ...
        if (!file.type.match(/image.*/)) {
            ...
            if (loadedImage) {
                this.activeFloor.floor_background_image = imageUrl.split(",")[1];
            } else {
                this.dialog.add(AlertDialog, {
