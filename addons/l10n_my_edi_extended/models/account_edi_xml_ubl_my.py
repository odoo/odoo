...
class AccountEdiXmlUBLMyInvoisMY(models.AbstractModel):
    ...
    def _export_invoice_vals(self, invoice):
        ...
        if vals['vals']['document_type_code'] in ('11', '12', '13'):
            ...
            # Switch from foreign buyer to foreign supplier
            for identification_val in other_party['party_identification_vals']:
                if identification_val.get('id_attrs', {}).get('schemeID') == 'TIN' and identification_val.get('id') == 'EI00000000020':
                    identification_val['id'] = 'EI00000000030'
        else:
            other_party = vals['vals']['accounting_customer_party_vals']['party_vals']
            # Switch from foreign supplier to foreign buyer
            for identification_val in other_party['party_identification_vals']:
                if identification_val.get('id_attrs', {}).get('schemeID') == 'TIN' and identification_val.get('id') == 'EI00000000030':
                    identification_val['id'] = 'EI00000000020'
        return vals
    ...
    def _l10n_my_edi_get_document_type_code(self, invoice):
        ...
        if 'debit_origin_id' in self.env['account.move']._fields and invoice.debit_origin_id:
            ...
            is_paid = invoice.payment_state in ('paid', 'in_payment') and invoice.currency_id.compare_amounts(invoice.amount_residual, 0) == 0
            reconcile_lines = invoice.line_ids.filtered(lambda x: x.account_id.reconcile or x.account_id.account_type in ('asset_cash', 'liability_credit_card'))
            reconciled_entry = reconcile_lines.full_reconcile_id.reconciled_line_ids.move_id.filtered(lambda x: x.is_entry())
            if not is_paid or not reconcile_lines.full_reconcile_id or not reconciled_entry:
