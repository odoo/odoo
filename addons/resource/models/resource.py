...
def to_user_tz(dt, record):
    if not isinstance(dt, datetime):
        dt = datetime.from_datetime(dt)
    tz_name = record._context.get('tz') or record.env.user.tz
    return dt.astimezone(tz_name)
...
class ResourceCalendar(models.Model):
    ...
    def plan_hours(self, hours, day_dt, compute_leaves=False, domain=None):
        ...
        if hours > 0:
            ...
            # In case we want zero hours, return the start of the next attendance
            for start, stop, meta in get_intervals(day_dt, day_dt + timedelta(days=1)):
                break
            return start
    ...
    def plan_days(self, days, day_dt, compute_leaves=False, domain=None):
        ...
        if days > 0:
            ...
            # In case we want zero days, return the start of the next attendance
            for start, stop, meta in get_intervals(day_dt, day_dt + timedelta(days=1)):
                break
            return start
