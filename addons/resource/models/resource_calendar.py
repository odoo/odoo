...
class ResourceCalendar(models.Model):
        ...
        for tz, tz_resources in resources_per_tz.items():
            ...
            for resource in chain(tz_resources, (self.env['resource.resource'],) if self else ()):
                ...
                if resource and is_fully_flexible:
                    ...
                    while current_start_day <= end_date:
                        ...
                        while current_day <= week_end:
                            if remaining_hours > 0:
                                start_time, end_time, allocate_hours = self._get_centered_duration_interval(current_day, tz, start_datetime, end_datetime, min(max_hours_per_day, remaining_hours))
                    ...
                    if not_flex_res is None:
                        not_flex_res = self._get_not_flex_schedule_attendance_intervals(
                            tz, start_datetime, end_datetime, bounds_per_tz[tz],
                            days, attendances_per_day, duration_based_attendances, all_duration_based_attendances,
                        )
                    result_per_resource_id[resource.id] = Intervals(not_flex_res, keep_distinct=True)
        return result_per_resource_id

    def _get_not_flex_schedule_attendance_intervals(self, tz, start_datetime, end_datetime, bounds,
                                                     days, attendances_per_day, duration_based_attendances, all_duration_based_attendances):
        """
        Compute attendance intervals for non-flexible schedules, handling both fixed-time and duration-based attendances and
        respecting the requested datetime bounds.
        """
        result = []
        for day in days:
            ...
            if all(att.duration_based for att in attendances):
                ...
                current_hour_from, end_time, _ = self._get_centered_duration_interval(day, tz, start_datetime, end_datetime, total_hours)
                for att in day_all_duration_based_attendances:
                    ...
                    hours_per_attendance[att] = (current_hour_from, next_current_hour_from)
                    current_hour_from = next_current_hour_from
                for att in day_duration_based_attendances:
                    date_from, date_to = hours_per_attendance[att]
                    result.append((date_from, date_to, att))
                continue

            for attendance in attendances:
                if attendance.duration_based:
                    day_from, day_to, _ = self._get_centered_duration_interval(day, tz, start_datetime, end_datetime, attendance.duration_hours)
                    result.append((day_from, day_to, attendance))
                else:
                    ...
                    result.append((day_from, day_to, attendance))
        return [(
                    max(bounds[0], val[0].replace(tzinfo=tz)),
                    min(bounds[1], val[1].replace(tzinfo=tz)),
                    val[2],
                ) for val in result]

    def _get_centered_duration_interval(self, day, tz, min_border, max_border, total_hours):
        day_start = datetime.combine(day, time.min, tzinfo=tz)
        day_end = datetime.combine(day, time.max, tzinfo=tz)
        day_period_start = max(min_border, day_start)
        day_period_end = min(max_border, day_end)
    ...
    def _get_fixed_hours_on_date(self, date, day_period=None):
        week_type = False
        if self.two_weeks_calendar:
            week_type = str(self.env['resource.calendar.attendance'].get_week_type(date))
        attendances = self.attendance_ids.filtered(
            lambda a: a.week_type == week_type and int(a.dayofweek) == date.weekday() and (not day_period or a.day_period == day_period))
        return sum(attendances.mapped('duration_hours'))

    def _get_half_day_leave_hours_on_date(self, date, date_from, date_to, date_from_period, date_to_period):
        self.ensure_one()
        is_half = (
            date == date_from and date_from_period == 'pm'
        ) or (
            date == date_to and date_to_period == 'am'
        )
        if self._is_duration_based_on_date(date):
            day_hours = self._get_duration_based_work_hours_on_date(date)
            hours = day_hours * 0.5 if is_half else day_hours
        else:
            day_period = None
            if is_half:
                day_period = 'afternoon' if date == date_from else 'morning'
            hours = self._get_fixed_hours_on_date(date, day_period=day_period)
