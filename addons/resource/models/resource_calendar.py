...
class ResourceCalendar(models.Model):
    ...
    def _check_company_id(self):
        for calendar, resources in self.env['resource.resource']._read_group(
            ...
            mismatched_resources = resources.filtered(
                lambda resource: resource.company_id and resource.company_id != calendar.company_id
            )
            if not mismatched_resources:
                continue
            raise ValidationError(self.env._(
                ...
                resource_names=format_list(self.env, mismatched_resources.mapped('name')),
            ))
