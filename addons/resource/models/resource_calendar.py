...
class ResourceCalendar(models.Model):
    ...
    def get_hours_for_date(self, target_date, day_period=None):
        """
        ...
        attendances = sorted([DummyAttendance(hour_from, hour_to, dayofweek, None, week_type)
            for week_type, dayofweek, hour_from, hour_to in attendances], key=lambda att: (att.dayofweek))
        ...
        week_type = 0
        if self.two_weeks_calendar:
            week_type = self.env['resource.calendar.attendance'].get_week_type(target_date)
