...
class ResourceCalendar(models.Model):
    ...
    def _load_fixed_time_calendar(self):
        DAYS = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
        unique_days = set()
        filtered_attendance_ids = self.env['resource.calendar.attendance']
        for attendance in self.attendance_ids:
            day = int(attendance.dayofweek)
            if day not in unique_days:
                unique_days.add(day)
                attendance.write(
                    {'name': DAYS[day], 'day_period': 'day'}
                )
                filtered_attendance_ids |= attendance
        return filtered_attendance_ids

    @api.depends('schedule_type')
    def _compute_attendance_ids(self):
        for calendar in self:
            if calendar.schedule_type == 'fixed_time':
                calendar.attendance_ids = calendar._load_fixed_time_calendar()
