...
class ResourceCalendar(models.Model):
    ...
    monday_hours = fields.Float(default=0)
    tuesday = fields.Boolean(default=False)
    tuesday_hours = fields.Float(default=0)
    wednesday = fields.Boolean(default=False)
    wednesday_hours = fields.Float(default=0)
    thursday = fields.Boolean(default=False)
    thursday_hours = fields.Float(default=0)
    friday = fields.Boolean(default=False)
    friday_hours = fields.Float(default=0)
    saturday = fields.Boolean(default=False)
    saturday_hours = fields.Float(default=0)
    sunday = fields.Boolean(default=False)
    sunday_hours = fields.Float(default=0)
    ...
    def _onchange_schedule_type(self):
        if self.schedule_type == 'fixed_time':
            ...
            for attendance in self.attendance_ids.filtered(lambda a: a.day_period != 'lunch'):
                attendance_days[attendance.dayofweek] = attendance.hour_to - attendance.hour_from + attendance_days.get(attendance.dayofweek, 0)

            self.attendance_ids = [(5, 0, 0)]
            day_name = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']
            for day, hours in attendance_days.items():
                self.attendance_ids |= self.env['resource.calendar.attendance'].new({
                    'name': f'{day_name[int(day)]}',
                    'dayofweek': day,
                    'hour_from': 8,
                    'hour_to': 8 + hours,
                    'day_period': 'full_day'
                })
                self[f'{day_name[int(day)]}'] = True
                self[f'{day_name[int(day)]}_hours'] = hours
