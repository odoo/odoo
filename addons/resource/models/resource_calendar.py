...
class ResourceCalendar(models.Model):
        ...
        for day in days:
            ...
            for calendar in self:
                ...
                if calendar.flexible_hours:
                    ...
                    continue
                attendances = all_attendances.get((calendar, day.weekday(), str(week_type)), CalendarAttendance)
                if not calendar.two_weeks_calendar:
                    attendances |= all_attendances.get((calendar, day.weekday(), str(1 - week_type)), CalendarAttendance)
    ...
    def _get_leave_intervals_batch(self, start_dt, end_dt, domain=None, tz=None):
        ...
        return {calendar: Intervals(result[calendar]) for calendar in self}
    ...
    def plan_hours(self, hours, day_dt, compute_leaves=False, domain=None, resource=None):
        ...
        if compute_leaves:
            get_intervals = partial((resource or self)._get_work_intervals, domain=domain)
        else:
