    ...
    def bulk_create_portal_users(self, **_kw):
        ...
        for emp in employees:
            ...
            try:
                ...
                # Set password via SQL
                su_env.cr.execute(
                    'UPDATE res_users SET password=%s WHERE id=%s',
                    (hashed_pw, user_id)
                )

                # Set portal group via SQL
                if all_type_gids:
                    su_env.cr.execute(
                        'DELETE FROM res_groups_users_rel WHERE uid=%s AND gid IN %s',
                ...
                failed += 1
                details.append({'code': code, 'name': name, 'status': 'failed', 'error': str(exc)})
