...
class DigestDigestKpi(models.Model):
    ...
    def _inverse_is_opted_in(self):
        ...
        for digest_kpi in self:
            ...
            if digest_kpi.is_opted_in:
                ...
                to_add |= digest_kpi
        to_add.write({'opt_out_user_ids': [Command.link(self.env.user.id)]})
        to_remove.write({'opt_out_user_ids': [Command.unlink(self.env.user.id)]})

    def action_open_kpi_form(self):
        self.ensure_one()
        return {
    ...
    def action_opt_in(self, is_opted_in=True):
        self.ensure_one()
        self.sudo().is_opted_in = is_opted_in
    ...
    def _action_send(self, update_periodicity=True, ignore_error=False):
        ...
        for digest in self:
            for user in digest.user_ids:
                ...
                except MailDeliveryException as mail_delivery_exception:
                    if not ignore_error:
                        raise mail_delivery_exception
                    _logger.warning(
