...
action = ({company.id: 0 for company in env.context.get('companies')}, 'integer')"""
    ...
    @api.constrains('res_model_id', 'date_field')
    def _check_date_field(self):
        for kpi in self:
            if not kpi.date_field:
                continue
            allowed_field = {
                field.name
    ...
    @api.constrains('res_model_id', 'company_field')
    def _check_company_field(self):
        for kpi in self:
            if not kpi.company_field:
                continue
            allowed_field = {
                field.name
                for field_name, field in self.env[kpi.res_model_id.model]._fields.items()
    ...
    @api.constrains('res_model_id', 'domain')
