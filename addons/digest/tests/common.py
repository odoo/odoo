...
class TestDigestCommon(mail_test.MailCommon):
    ...
    def _get_values(self, digests, kpi_name, field_name):
        """ :return list: values of field_name of kpi_name for each digest. """
        # There are 2 ways of computing the values
        # 1. for a digest mainly to display it in the interface
        field_computed = [
            digest.digest_kpi_ids.filtered(lambda kpi: kpi.name == kpi_name)[field_name]
            for digest in digests
        ]
        # 2. in batch for multiple digests when sending them
        key_value = 'value'
        ...
        self.assertEqual(field_computed, batch_computed, 'Field and batch computed kpi must be equals')
        return field_computed[0] if len(field_computed) == 1 else field_computed
