...
class L10nPhDiscountPrivilegeWizard(models.TransientModel):
    ...
    def _recompute_line_previews(self):
        ...
        for line in self.line_ids:
            ...
            if not privilege:
                ...
                mapped_price_unit = inv._l10n_ph_compute_mapped_price_unit(privilege)
                vals["discounted_amount"] = inv._l10n_ph_get_special_discount_amount(
                    privilege=privilege,
                    price_unit=mapped_price_unit,
                )
    ...
    def action_confirm(self):
        ...
        if not self.privilege_id:
            raise UserError(
                self.env._("Please select a privilege before applying discounts."),
            )
        ...
        for inv_line in self.line_ids.invoice_line_id:
            if not self._line_matches_scope(inv_line):
                continue
            mapped_price_unit = inv_line._l10n_ph_compute_mapped_price_unit(privilege)
            vals = {
            ...
            # Store the VAT-inclusive mapped price in the internal field so the tax
            # engine can use it, without touching price_unit (which stays as-is for the user).
            if inv_line.currency_id.compare_amounts(
                mapped_price_unit,
                inv_line.price_unit,
            ):
                vals["l10n_ph_internal_price_unit"] = mapped_price_unit
            if privilege.tax_id:
