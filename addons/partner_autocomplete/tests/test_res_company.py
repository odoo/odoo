...
class TestResCompany(MockIAPEnrich):

    def test_enrich(self):
        company = self.env['res.company'].create({'name': "Test Company 1"})
        with self.mockPartnerAutocomplete():
            res = company.partner_id._iap_perform_autocomplete(include_logo=True)
    ...
    def test_extract_company_domain(self):
        ...
        self.assertEqual(company_1.partner_id._get_company_domain(), "proximus.be")

        company_1.email = 'info@waterlink.be'
        self.assertEqual(company_1.partner_id._get_company_domain(), "waterlink.be")

        company_1.website = False
        company_1.email = False
        self.assertEqual(company_1.partner_id._get_company_domain(), False)
        ...
        self.assertEqual(company_1.partner_id._get_company_domain(), 'superwebsite.com')
        ...
        self.assertEqual(company_1.partner_id._get_company_domain(), 'odoo.com')

        company_1.website = "http://www.example.com/biniou"
        self.assertEqual(company_1.partner_id._get_company_domain(), False)

        company_1.website = "http://www.cwi.nl:80/%7Eguido/Python.html"
        self.assertEqual(company_1.partner_id._get_company_domain(), "cwi.nl")
