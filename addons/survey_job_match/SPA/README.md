...
- The top-ranked profile is presented as the best match: image, name, match
  ...
  to the job posting.
- Below it, the next two profiles are listed with their own percentages,
  smaller meters and their own link to the posting, as roles the visitor would
  also fit.
- One closing line ("Thanks for playing!") ends the screen, whichever of the
  three endings it showed. It lives in exactly one place so it cannot appear
  twice — do not repeat it inside a message text.
...
**The six scoring questions are mandatory, and that is a deliberate
divergence.** The demo data leaves everything optional, so a visitor could press
Enter straight through and be shown a 0% match as though it were a result — the
reference does exactly that, returning all fourteen profiles at zero. Both
screens refuse to advance without an answer and the toolbar's forward arrow goes
through the same check, so there is no way past. A welcome side effect: the
...
`data/questions.json` no longer matches the module's demo data. Where they
disagree the spreadsheet wins, and three edits were made on that basis:

| Change | Effect |
| --- | --- |
| "A job at Odoo" also eliminates HR and Marketing | a job seeker sees 8 survivors, not 10 |
| "English & nordic languages" no longer eliminates the Business developer internship | fixes a dead end: internship seeker + Nordic went from **no match at all** to a surviving role |
| "Following up on clients/partners" grants 1 point to five more profiles | scores rise where that answer is picked; ceilings are untouched, so nobody else's percentage moves |

So the reference comparison below is a record of parity *at the time the data was
ported*, not a standing invariant. Re-running it now shows differences in exactly
those three places and nowhere else, which is the useful check: it confirms the
edits landed and that nothing else drifted with them.
