...
class MrpRepair(models.Model):
    ...
    def _get_default_address(self):
        for repair in self:
            ...
            if repair.partner_id:
                adr_id = repair.partner_id.address_get(['default'])['default']
            repair.default_address_id = adr_id
    ...
    def action_invoice_create(self, group=False):
        ...
        for repair in self.filtered(lambda r: not (r.state in ('draft', 'cancel') or r.invoice_id)):
            res[repair.id] = False
            if not (repair.partner_id and repair.partner_invoice_id):
                ...
                for operation in repair.operations_ids:
                    if operation.to_invoice:
                        ...
                        if operation.product_id.property_account_income_id:
                            account = operation.product_id.property_account_income_id
                        elif operation.product_id.categ_id.property_account_income_categ_id:
                            account = operation.product_id.categ_id.property_account_income_categ_id
                        else:
                        ...
                        invoice_line = AccountInvoiceLine.create({
                            ...
                            'invoice_line_tax_ids': [(6, 0, operation.tax_id.ids)],
                ...
                for fee in repair.fees_lines_ids:
                    if fee.to_invoice:
                        ...
                        if fee.product_id.property_account_income_id:
                            account = fee.product_id.property_account_incomed_id
                        elif fee.product_id.categ_id.property_account_income_categ_id:
                            account = fee.product_id.categ_id.property_account_income_categ_id
                        else:
...
class MrpRepairLine(models.Model, ProductChangeMixin):
    ...
    def _amount_line(self):
        ...
        for line in self:
            if line.to_invoice:
                ...
                line.price_subtotal = line.tax_id.compute_all(line.price_unit, cur, line.product_uom_qty, line.product_id.id, line.repair_id.partner_id.id)['total_included']
            else:
    ...
    def onchange_operation_type(self, repair_type, guarantee_limit, company_id=False):
        ...
        location = StockLocation.search([('usage', '=', 'production')])
