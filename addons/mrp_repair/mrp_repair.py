            ...
            if (o.invoice_method == 'b4repair'):
                ...
                for line in o.operations:
                    if line.product_id.track_production:
                        raise Warning(_('Warning!'), _("Serial number is required for operation line with product '%s'") % (line.product_id.name))
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        for repair in self.browse(cr, uid, ids, context=context):
            if not repair.invoiced:
                ...
                raise Warning(_('Warning!'), _('Repair order is already invoiced.'))
    ...
    def action_invoice_create(self, cr, uid, ids, group=False, context=None):
        ...
        for repair in self.browse(cr, uid, ids, context=context):
            ...
            if not (repair.partner_id.id and repair.partner_invoice_id.id):
                raise Warning(_('No partner!'), _('You have to select a Partner Invoice Address in the repair form!'))
            ...
            if (repair.invoice_method != 'none'):
                if group and repair.partner_invoice_id.id in invoices_group:
                    ...
                    if not repair.partner_id.property_account_receivable:
                        raise Warning(_('Error!'), _('No account defined for partner "%s".') % repair.partner_id.name)
                ...
                for operation in repair.operations:
                    if operation.to_invoice:
                        ...
                        if operation.product_id.property_account_income:
                            ...
                            raise Warning(_('Error!'), _('No account defined for product "%s".') % operation.product_id.name)
                ...
                for fee in repair.fees_lines:
                    if fee.to_invoice:
                        ...
                        if not fee.product_id:
                            raise Warning(_('Warning!'), _('No product defined on Fees!'))
                        ...
                        if fee.product_id.property_account_income:
                            ...
                            raise Warning(_('Error!'), _('No account defined for product "%s".') % fee.product_id.name)
