...
class test_domain(common.TransactionCase):
    ...
    def test_00_test_bool_undefined(self):
        ...
        self.env['ir.model.fields'].create({
            ...
            'model_id': self.env.ref('test_base_new_api.model_domain_bool').id,
            'field_description': 'A new boolean column',
            'ttype': 'boolean'
        })

        self.env.ref('test_base_new_api.bool_3').write({'x_bool_new_undefined': True})
        self.env.ref('test_base_new_api.bool_4').write({'x_bool_new_undefined': False})
