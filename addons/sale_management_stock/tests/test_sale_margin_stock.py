...
class TestSaleManagementStock(TestStockValuationCommon):
    _test_user_groups = (
    ...
    def test_sale_management_stock_1(self):
    ...
    def test_sale_management_stock_2(self):
        sale_order = self._create_sale_order()
        product = self._create_product()
    ...
    def test_sale_management_stock_3(self):
        sale_order = self._create_sale_order()
        product = self._create_product()
    ...
    def test_sale_management_stock_4(self):
        sale_order = self._create_sale_order()
        product = self._create_product()
    ...
    def test_sale_management_stock_5(self):
        sale_order = self._create_sale_order()
    ...
    def test_sale_management_stock_6(self):
    ...
    def test_so_and_multicompany(self):
        ...
        new_company = (
            ...
            .env["res.company"]
            .sudo()
            .create({"name": "Super Company", "currency_id": new_company_currency.id})
        )
    ...
    def test_add_standard_product_on_delivery_cost_on_sale_order(self):
        """Test that if product with standard cost method is added in delivery, the cost is
        computed.
        """
        product = self.product_standard
        product.write({"standard_price": 20, "list_price": 25, "invoice_policy": "order"})
        ...
        delivery.button_validate()
        self.assertEqual(
            sale_order.order_line.filtered(lambda sol: sol.product_id == product2).purchase_price,
            10,
        )

    def test_add_avco_product_on_delivery_cost_on_sale_order(self):
        """Test that if product with avco cost method and an order "invoice_policy" is added in
        delivery, the cost is computed.
        """
        ...
        self.product.write({"standard_price": 20, "list_price": 25, "invoice_policy": "order"})
        product2 = self.env["product.product"].create({
            "name": "product2",
            "type": "consu",
            "is_storable": True,
    ...
    def test_avco_different_uom(self):
        ...
        self.product_avco.write({"standard_price": 1, "list_price": 3, "uom_ids": [pack_of_6]})
        sale_order = self._create_sale_order()
    ...
    def test_avco_does_not_mix_products_on_compute_avg_price(self):
        ...
        self.assertEqual(
            second_delivery.move_ids.sale_line_id, sale_order.order_line - sale_order_line
        )
    ...
    def test_avco_calc(self):
        ...
        # need to freezetime due to test being too fast resulting in inconsistent AVCO
        # calculation for in/out moves having the same exact validation date
        with freeze_time() as freeze:
            ...
            self.assertEqual(
                sol.purchase_price, 30, "purchase_price should match product's standard_price"
            )
            self.assertEqual(
                sol.margin,
                140,
            ...
            backorder_wizard = (
                self
                .env[(backorder_wizard_values.get("res_model"))]
                .browse(backorder_wizard_values.get("res_id"))
                .with_context(backorder_wizard_values["context"])
            )
            backorder_wizard.process()
            # purchase_unit_from_delivery = line.move_ids(done)._get_price_unit = (1 * 30) / (1)
            # = 30 ; qty_from_std_price = max(SOL quantity - qty_from_delivery, 0) = 2 - 1 = 1
            self.assertEqual(
                sol.purchase_price,
                30,
                "purchase_price = (qty_delivered * purchase_unit_from_delivery + "
                "qty_from_std_price * standard_price)/(qty_from_delivery + qty_from_std_price) "
                "= (1 * 30 + 1 * 30)/ (1 + 1) = 30",
            )
            self.assertEqual(
                sol.margin,
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                75,
                "standard_price for avco = (3 * 30 + 2 * 142.5) / (3 + 2) = 75: "
                "3 remaining + 2 added to stock",
            )
            ...
            delivery = move.picking_id
            delivery.button_validate()
            # purchase_unit_from_delivery = line.move_ids(done)._get_price_unit
            # = (1 * 30 + 2 * 75) / (1 + 2) = 60
    ...
    def test_avco_zero_quantity(self):
        """Test that the purchase_price and margin are still calculated correctly when 0 quantity
        SOL, including when a return is done for avco valuated product.
        """
        # need to freezetime due to test being too fast resulting in inconsistent AVCO
        # calculation for in/out moves having the same exact validation date
        with freeze_time() as freeze:
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                40,
                "standard_price for avco = (4 * 30 + 2 * 60) / (4 + 2) = 40: "
                "4 existing + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            move = sol.move_ids
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                30,
                "standard_price for avco = (5 * 40 + 2 * 5) / (5 + 2) = 30: "
                "1 delivered, 5 remaining + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                33,
                "standard_price for avco = (7 * 30 + 3 * 40) / (7 + 3)) = 33: "
                "7 remaining + 3 returned",
            )
            self.assertEqual(
                sol.purchase_price,
                self.product_avco_auto.standard_price,
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                32.5,
                "standard_price for avco = (10 * 33 + 2 * 30) / 12 = 32.5: "
                "10 remaining + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                sol3.margin,
                -65,
                "margin = SOL qty * sale price - purchase_price * qty_delivered "
                "= (0 - 32.5) * 2 = -65",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                30,
                "standard_price for avco = (10 * 32.5 + 2 * 17.5) / (10 + 2) = 30: "
                "2 delivered, 10 remaining + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            return_pick = delivery._create_return()
            ...
            # purchase_unit_from_delivery = line.move_ids(done)._get_price_unit
            # = (2 * 32.5 + 1 * 32.5) / (2 + 1) = 32.5
            self.assertEqual(
                sol3.purchase_price, 32.5, "purchase_price = 2 * 32.5 + 1 * 32.5) / (2 + 1) = 32.5"
            )
            self.assertEqual(
                sol3.margin,
                -32.5,
    ...
    def test_dropship_fifo_purchase_price(self):
        ...
        so = self.env["sale.order"].create({
            "partner_id": self.customer.id,
            "order_line": [
                Command.create({"product_id": self.product_fifo_auto.id, "product_uom_qty": 1.0})
            ],
        })
        so.action_confirm()
        po = self.env["purchase.order"].search(  # noqa: OLS03001
            [("origin", "=", so.name), ("partner_id", "=", self.vendor.id)], limit=1
        )
        po.button_confirm()

        # untill dropship validation, purchase price of the sale order line is the
