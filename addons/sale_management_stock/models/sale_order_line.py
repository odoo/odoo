        ...
        for line in self:
            ...
            if not line.has_valued_move_ids():
                ...
                price_unit_from_delivery = (
                    line.move_ids.filtered(lambda m: m.state == "done")._get_price_unit(product=product)
                    if qty_from_delivery > 0
                    else 0
                )
                price_unit_from_delivery = line.product_id.uom_id._compute_price(
                    price_unit_from_delivery, product.uom_id
                )
                if qty_from_delivery <= 0:
                ...
                line.purchase_price = line._convert_to_sol_currency(
                    purch_price_uom, product.cost_currency_id
                )
