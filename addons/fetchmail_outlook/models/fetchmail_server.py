...
class FetchmailServer(models.Model):
    ...
    @api.constrains('use_microsoft_outlook_service', 'server_type', 'password')
    def _check_use_microsoft_outlook_service(self):
        for server in self:
            if server.use_microsoft_outlook_service and server.server_type != 'imap':
                raise UserError(_('Outlook mail server %r only supports IMAP server type.' % server.name))

            if server.use_microsoft_outlook_service and server.password:
                raise UserError(_('Please leave the password empty for Outlook mail server %r.' % server.name))
