...
def _l10n_account_wth_post_init(env):
    ...
    for template_code, companies in companies.grouped('chart_template').items():
        ...
        for company in companies:
            if company.parent_id or company.l10n_account_wth_tax_base_account_id:
                continue

            code_digits = int(template_data.get('code_digits', 6))
            company.l10n_account_wth_tax_base_account_id = env['account.account']._load_records([{
...
def _make_demo_tax(env, chart_template_data, company):
    ...
    wh_tax = env['account.tax'].create({
        ...
        'l10n_account_wth_sequence_id': env['ir.sequence'].create({
