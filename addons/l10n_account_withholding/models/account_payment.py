...
class AccountPayment(models.Model):
    ...
    def _get_valid_liquidity_accounts(self):
        """ The liquidity account may come from a field on the wizard when it's not used outside of withholdings.
        In such case, it could be any liquidity account in the company.
        To make sure the flows keep working, we add the outstanding_account_id to the list if there are withholding lines on the payment.
        This way, it should work without new fields and also shouldn't impact other flows.
        """
        # EXTENDS account
        result = super()._get_valid_liquidity_accounts()
        return result | self.outstanding_account_id
