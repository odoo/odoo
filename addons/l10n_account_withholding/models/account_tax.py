...
class AccountTax(models.Model):
    ...
    def _add_tax_details_in_base_line(self, base_line, company, rounding_method=None):
        ...
        taxes = base_line['tax_ids']._flatten_taxes_and_sort_them()[0].filtered('is_withholding_tax_on_payment')
        if not taxes:
            super()._add_tax_details_in_base_line(base_line, company, rounding_method=rounding_method)
            return

        # Neutralize the taxes that are withholding for forcing a manual tax amounts for them.
        manual_tax_amounts = dict(base_line['manual_tax_amounts'] or {})
        for tax in taxes:
            manual_tax_amounts[str(tax.id)] = {'tax_amount_currency': 0.0}
        new_base_line = self._prepare_base_line_for_taxes_computation(base_line, manual_tax_amounts=manual_tax_amounts, special_mode=base_line['special_mode'], special_type=base_line['special_type'])
        ...
        new_base_line.pop('manual_tax_amounts')
        tax_details = base_line['tax_details'] = new_base_line['tax_details']
        tax_details['taxes_data'] = [
            tax_data
            for tax_data in tax_details['taxes_data']
            if not tax_data['tax'].is_withholding_tax_on_payment
        ]
