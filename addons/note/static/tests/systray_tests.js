...
QUnit.test('note activity menu widget: create note from activity menu', function (assert) {
    ...
    testUtils.addMockEnvironment(activityMenu, {
        ...
        mockRPC: function (route, args) {
            ...
            if (args.method === 'quick_create_note') {
                if (step == 1) {
                    assert.deepEqual(args.args, [[], "New Note", false], 'Create note should get proper value');
                }
                if (step == 2) {
                    assert.strictEqual(args.args[2] == false, false, 'Create note with date should get proper value');
                }
                return $.when();
            }
