...
def get_default_thumbnail():
    with file_open('web/static/img/default_embed_thumbnail.png', 'rb', filter_ext=('.png')) as fd:
...
def get_video_metadata(video_url, resize=False):
    ...
    if source is None:
        return None
    metadata_url = thumbnail = name = None
        ...
        if res.ok:
            if platform != 'youku':
                ...
                if platform != 'instagram':
                    ...
                    name = _("%s's %s Video - %s", author_name.capitalize(), platform.capitalize(), video_id)
    ...
    return {
        ...
        'thumbnail': thumbnail or get_default_thumbnail(),
    }
