...
describe('Copy and paste', () => {
        ...
        describe('range collapsed', async () => {
            ...
            it('should paste and transform an image URL in an existing link', async () => {
                await testEditor(BasicEditor, {
                    ...
                    stepFunction: async editor => {
                        await pasteText(editor, 'https://download.odoocdn.com/icons/website/static/description/icon.png');
                        // Ensure the powerbox is active with 2 commands (embed image and paste as text)
                        window.chai.expect(editor.commandBar._active).to.be.true;
                        window.chai.expect(editor.commandBar._currentFilteredCommands.length).to.be.equal(2);
                        // Force commandBar validation on the default first choice
                        await editor.commandBar._currentValidate();
                    },
            ...
            it('should paste and not transform an image URL in an existing link', async () => {
                await testEditor(BasicEditor, {
                    contentBefore: '<p>a<a href="http://existing.com">b[]c</a>d</p>',
                    stepFunction: async editor => {
                        await pasteText(editor, 'https://download.odoocdn.com/icons/website/static/description/icon.png');
                    ...
                    contentAfter: '<p>a<a href="http://existing.com">bhttps://download.odoocdn.com/icons/website/static/description/icon.png[]c</a>d</p>',
                });
            });
            ...
            it('should paste an image URL as simple text in a p', async () => {
                const url = 'https://download.odoocdn.com/icons/website/static/description/icon.png';
                await testEditor(BasicEditor, {
                    contentBefore: '<p>[]</p>',
                    stepFunction: async editor => {
                        ...
                        // Ensure the powerbox is active
                        window.chai.expect(editor.commandBar._active).to.be.true;
                        // Pick the third command (Paste as text)
                        triggerEvent(editor.editable, 'keydown', { key: 'ArrowDown' });
                        triggerEvent(editor.editable, 'keydown', { key: 'ArrowDown' });
                        triggerEvent(editor.editable, 'keydown', { key: 'Enter' });
                    },
                    contentAfter: `<p>${url}&nbsp;[]</p>`,
                });
            });
        });
        describe('range not collapsed', async () => {
            ...
            it('should paste an image URL as simple text in a p', async () => {
                ...
                await testEditor(BasicEditor, {
                    ...
                    stepFunction: async editor => {
                        ...
                        // Ensure the powerbox is active
                        window.chai.expect(editor.commandBar._active).to.be.true;
                        // Pick the third command (Paste as URL)
                        triggerEvent(editor.editable, 'keydown', { key: 'ArrowDown' });
                        triggerEvent(editor.editable, 'keydown', { key: 'ArrowDown' });
                        triggerEvent(editor.editable, 'keydown', { key: 'Enter' });
                    },
                    contentAfter: `<p>ab${url}[]cd</p>`,
                });
            });
            it('should paste an image URL as simple text and add a space when at end of a line', async () => {
                const url = 'https://download.odoocdn.com/icons/website/static/description/icon.png';
                await testEditor(BasicEditor, {
                    contentBefore: '<p>[]</p>',
                    stepFunction: async editor => {
            ...
            it('should restore selection after pasting image URL followed by UNDO', async () => {
                ...
                await testEditor(BasicEditor, {
                    ...
                    stepFunction: async editor => {
                        ...
                        // Ensure the powerbox is active
                        window.chai.expect(editor.commandBar._active).to.be.true;
                        // Pick third command (Paste as text)
                        triggerEvent(editor.editable, 'keydown', { key: 'ArrowDown' });
                        triggerEvent(editor.editable, 'keydown', { key: 'ArrowDown' });
                        triggerEvent(editor.editable, 'keydown', { key: 'Enter' });
                        // Undo
    ...
    describe('youtube video', () => {
        describe('range collapsed', async () => {
            ...
            it('should paste a youtube URL as simple text in a p', async () => {
                const url = 'https://youtu.be/dQw4w9WgXcQ';
                await testEditor(BasicEditor, {
                    contentBefore: '<p>[] abc</p>',
                    stepFunction: async editor => {
                    ...
                    contentAfter: `<p>${url}[] abc</p>`,
                });
            });
            it('should paste a youtube URL as simple text and add a space when at end of a line', async () => {
                const url = 'https://youtu.be/dQw4w9WgXcQ';
                await testEditor(BasicEditor, {
                    contentBefore: '<p>[]</p>',
                    stepFunction: async editor => {
