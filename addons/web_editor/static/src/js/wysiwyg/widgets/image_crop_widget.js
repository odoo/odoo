...
const ImageCropWidget = Widget.extend({
    ...
    async start() {
        ...
        const cropperWrapper = this.$('.o_we_cropper_wrapper')[0];
        ...
        // The cropper takes the size of the container, including padding
        // The image doesn't fill its padding, so we use margin on the container
        // to replicate the effect on the cropper preview.
        ...
        this.document.addEventListener('mousedown', this._onOutsideClick, {capture: true});
    ...
    destroy() {
        if (this.$cropperImage) {
            ...
            this.$el.remove();
            this.document.removeEventListener('mousedown', this._onOutsideClick, {capture: true});
        }
    ...
    _save() {
        ...
        // see 'saveCroppedImages' in rte.summernote.js
