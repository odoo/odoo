...
const ColorPaletteWidget = Widget.extend({
     ...
     * @param {JQuery} [options.$target] The target element which has the class or css attribute color on it.
    ...
    start: function () {
        ...
        this.el.querySelectorAll('button[data-color]').forEach(elem => {
            ...
            if (this.options.$target && this.options.$target.hasClass(className)) {
    ...
    _createColorButton: function (color, classes) {
        ...
        if (this.options.$target && this.options.$target.css('background-color') === cssColor) {
    ...
    _onCustomColorButtonClick: function (ev) {
        ...
        colorpicker.on('colorpicker:saved', this, ev => {
            ...
            this._addCustomColorButton(ev.data.cssColor, this.options.$target ? ['selected'] : undefined);
