...
// Time in ms to wait when trying to aggregate other peer snapshots and
...
const Wysiwyg = Widget.extend({
    ...
    startEdition: async function () {
        ...
        // This count is used to detect when we reset the editor (e.g. from a
        // peer or from the server). When doing an async request that require
        // informations, we might want to discard the response if another reset
        // has been done.
        this._lastResetCollabCount = 0;
        // This ID correspond to the person that initiated the document. It is
        // usefull to know if we receive a step that comes from an history from
        // wich we based (with is not the same as document that had a step id at
        // some point). If a step comes from a different history, we should not apply it.

        // todo: Share history id between peers and check against it to detect
        // if a peer send a step from a totally different history although they
        // both could have the lastDocumenId in their history.
        this._historyShareId = Math.floor(Math.random() * Math.pow(2,52)).toString();

        // The ID is the latest step ID that the server know through
        ...
        // stale.
        this._serverLastStepId = options.value && this._getServerLastStepId(options.value);
    ...
    setupCollaboration(collaborationChannel) {
        ...
        const collaborationBusListener = ({ detail: notifications}) => {
            for (const { payload, type } of notifications) {
                if (
                    ...
                    if (payload.notificationName === 'document_write') {
                        this._onDocumentWrite(payload.notificationPayload.last_step_id);
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                if (missingSteps === REQUEST_ERROR) return false;
                this._processMissingSteps(missingSteps.concat(step));
            },
        };
    ...
    _signalOnline: async function () {
        ...
        // Ask for potential missing steps for all peers.
    ...
    async _processMissingSteps(missingSteps) {
        ...
        //   - because the step.clientId has reset from the server is not synced
        //     with this client
        //   - because the step.clientId is in a cluster
        //
        // Todo: Detect and do something if the peer is in a cluster. Check that
        // the history has the same id.
    ...
    _showResetDialog() {
        if (this._resetDialogOpened) return;
        ...
        this._resetDialogOpened = true;
        dialog.open({shouldFocusButtons: true});
        dialog.on('closed', undefined, () => {
            this._resetDialogOpened = false;
        });
    },
    _getServerLastStepId: function (value) {
                    ...
                    case 'rtc_data_channel_open': {
                        ...
                        if (!this._historySyncAtLeastOnce) {
                            ...
                            if (isClientFirst(localClient, remoteClient)) {
                                ...
                                const response = await this._resetFromClient(fromClientId, this._lastResetCollabCount);
    ...
    async _recoverFromStaleDocument() {
        return new Promise((resolve) => {
            ...
            const resetCollabCount = this._lastResetCollabCount;
            ...
            let snapshots = [];
            let nbOfResponseToReceive = allPeers.length;
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    nbOfResponseToReceive--;
                    if (
                        response === REQUEST_ERROR ||
                        resetCollabCount !== this._lastResetCollabCount ||
            ...
            // Wait PTP_MAX_RECOVERY_TIME to retrieve the maximum number of
            // informations from other peers to avoid reseting from the server if
            // possible.
            const timeout = setTimeout(() => {
                if (resetCollabCount !== this._lastResetCollabCount) {
                    return;
                }
    ...
    /**
     * Callback for when the timeout PTP_MAX_RECOVERY_TIME fire.
     *
    ...
    async _recoverFromStaleDocumentFromServer() {
        ...
        const lastHistoryId = content && this._getServerLastStepId(content);
        ...
        // proper stnapshot (e.g. This case could arises if we tried to recover
        ...
        await Promise.all(this._getPtpClients().map((client) => {
            ...
            // resets.
            return this._resetFromClient(client.id, resetCollabCount);
        }));
        return true;
    },
    _resetCollabRequests() {
        this._lastResetCollabCount++;
        ...
        // calls `Wysiwyg.requestClient` might want to check the if the response
        // is REQUEST_ERROR.
    ...
    resetValue: async function (value) {
        this.setValue(value);
        this.odooEditor.historyReset();
        this._historyShareId = Math.floor(Math.random() * Math.pow(2,52)).toString();
        this._serverLastStepId = value && this._getServerLastStepId(value);
    ...
    _applySnapshot(snapshot) {
        ...
        const isStaleDocument = !historyIds.includes(this._serverLastStepId);
        if (isStaleDocument) {
            return;
        }
        this._historySyncAtLeastOnce = true;
