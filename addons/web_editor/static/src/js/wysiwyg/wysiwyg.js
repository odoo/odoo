...
const createWebRtcCollaboration = async (
    configuration,
    myUserId,
    module,
    documentId,
    ...
    module.call('bus_service', 'startPolling');
    const channel = `${model}_${documentId}`;
    module.call('bus_service', 'addChannel', channel);

// Map of userId -> (rtc connection, data channel, isInitiator)
    const peerMap = new Map();
    const newConnectionEmitter = new EventTarget();

    module.call('bus_service', 'onNotification', module, (notifications) => {
        notifications.forEach(
            ([notificationChannel, {type, message, userId}]) => {
                if (notificationChannel === channel) {
                    if (type === 'message') {
                        if (message.to === myUserId) {
                            signalingMessageCallback(message.from, message.message);
                        }
                    } else if (type === 'join') {
                        if (userId !== myUserId) {
    ...
    // Joining a room.
    await module._rpc({
        route: '/web_editor/collaboration/join',
        params: {document_id: documentId, model, user_id: myUserId},
    });
   ...
   */
    function sendMessage(message) {
        return module._rpc({
            route: '/web_editor/collaboration/message',
            params: {document_id: documentId, model, message},
        });
    }
    ...
    function createPeerConnection(isInitiator, userId, config) {
        ...
        peerConn.addEventListener('connectionstatechange', function (e) {
            // TODO: handle disconnection
        });
        if (isInitiator) {
...
const Wysiwyg = Widget.extend({
    ...
    start: async function () {
        ...
        const {broadcast, newConnectionEmitter} = await createWebRtcCollaboration(
            null,
            new Date().getTime(),
            this,
            documentId,
        ...
        newConnectionEmitter.addEventListener(
            'newDataChannel',
            ({connectionObject}) => {
                const {isInitiator, dataChannel} = connectionObject;
                    ...
                    switch (message.type) {
                        ...
                        case 'historyRequest':
                            dataChannel.send(
                                JSON.stringify({
                                    type: 'historyReset',
                                    history: this.odooEditor.historyGetSnapshot(),
                                })
                            );
                            break;
                        case 'historyReset':
                            this.odooEditor.historySynchronise(message.history);
                            break;
                        case 'selection':
                            this.odooEditor.receiveCollaboratorSelection(message.selection);
                    }
                });

                if (!isInitiator && !isInitialised) {
                    isInitialised = true;
                    dataChannel.addEventListener('open', () => {
                        dataChannel.send(JSON.stringify({type: 'historyRequest'}));
                    });
                }
            }
        );
        ...
        if (!this.options.preventLinkDoubleClick) {
            this.$editable.on('dblclick', 'a', function () {
                    if (
                        !this.getAttribute('data-oe-model') &&
          self.toolbar.$el.is(':visible')
                    ) {
                        self.showTooltip = false;
            ...
            if (options.snippets) {
                ...
                this.snippetsMenu = new snippetsEditor.SnippetsMenu(
                    this,
                    Object.assign(
                        {
                            wysiwyg: this,
   ...
   */
        focus: function () {
            if (!this.odooEditor.resetCursorOnLastHistoryCursor()) {
                ...
                const elementToTarget = this.$editable[0].lastElementChild
                    ? this.$editable[0].lastElementChild
                    : this.$editable[0];
   ...
   */
        _observeOdooFieldChanges: function () {
            ...
            if (this.odooFieldObservers) {
                ...
                $node.data('oe-translation-id') +
                '"]'
                            );
                        }
                        if ($node.data('oe-type')) {
                            $nodes = $nodes.filter(
                                '[data-oe-type="' + $node.data('oe-type') + '"]'
                            );
                        }
                        if ($node.data('oe-expression')) {
                            $nodes = $nodes.filter(
                                '[data-oe-expression="' + $node.data('oe-expression') + '"]'
                            );
                        } else if ($node.data('oe-xpath')) {
                            $nodes = $nodes.filter(
                                '[data-oe-xpath="' + $node.data('oe-xpath') + '"]'
                            );
                        }
                        if ($node.data('oe-contact-options')) {
                            $nodes = $nodes.filter(
                                "[data-oe-contact-options='" +
                $node[0].dataset.oeContactOptions +
                "']"
                            );
                        }
                        ...
                        if ($node.data('oe-type') === 'many2one') {
                            $nodes = $nodes.add(
                                $('[data-oe-model]')
                                    ...
                                    .filter(
                                        '[data-oe-many2one-model="' +
                    $node.data('oe-many2one-model') +
                    '"]'
                                    )
                                    .filter(
                                        '[data-oe-many2one-id="' + $node.data('oe-many2one-id') + '"]'
                                    )
                            ...
                            $nodes = $nodes.add(
                                $('[data-oe-model]')
                                    ...
                                    .filter(
                                        '[data-oe-model="' + $node.data('oe-many2one-model') + '"]'
                                    )
   ...
   */
        toggleLinkTools(options = {}) {
            if (this.snippetsMenu && !options.forceDialog) {
                ...
                if (options.forceOpen || !this.linkTools) {
                    ...
                    this.linkTools = new weWidgets.LinkTools(
                        ...
                        {},
                        $btn,
                        options.link || this.lastMediaClicked
                    );
            ...
            !ev.target.closest('.ui-autocomplete')
                            ...
                            this.odooEditor.document.removeEventListener(
                                'mousedown',
                                _onMousedown,
                                true
                            );
                    ...
                    this.odooEditor.document.addEventListener(
                        'mousedown',
                        _onMousedown,
                        true
                    );
                ...
                const linkDialog = new weWidgets.LinkDialog(
                    this,
                    {
   ...
   */
        openMediaDialog(params = {}) {
            ...
            if (fontawesomeIcon && sel.toString().trim() === '') {
                ...
                // save layouting classes from icons to not break the page if you edit an icon
                params.htmlClass = [...fontawesomeIcon.classList]
                    .filter((className) => {
                        return !className.startsWith('fa') || faZoomClassRegex.test(className);
                    })
                    .join(' ');
            ...
            const $editable = $(
                OdooEditorLib.closestElement(range.startContainer, '.o_editable')
            );
            ...
            const mediaParams = Object.assign(
                {
                    ...
                    useMediaLibrary:
          field &&
          ((model === 'ir.ui.view' && field === 'arch') || type === 'html'),
                },
        ...
        _configureToolbar: function (options) {
            ...
            $toolbar
                .find('#create-link, #media-insert, #media-replace, #media-description')
                .click(openTools);
            $toolbar.find('#image-shape div, #fa-spin').click((e) => {
                ...
                e.target.classList.toggle(
                    'active',
                    $(this.lastMediaClicked).hasClass(e.target.id)
                );
            });
            ...
            $imageWidthButtons.click((e) => {
                ...
                for (const button of $imageWidthButtons) {
                    button.classList.toggle(
                        'active',
                        this.lastMediaClicked.style.width === button.id
                    );
                }
            });
            ...
            $toolbar.on('mousedown', (e) => {
                ...
                const classes =
        mode === 'center' ? ['d-block', 'mx-auto'] : ['float-' + mode];
                const doAdd = classes.some(
                    (className) => !this.lastMediaClicked.classList.contains(className)
                );
            ...
            $toolbar.find('#image-crop').click((e) => {
                ...
                new weWidgets.ImageCropWidget(this, this.lastMediaClicked).appendTo(
                    this.$editable
                );
            });
            $toolbar.find('#image-transform').click((e) => {
                ...
                const mouseup = () => {
                    $('#image-transform').toggleClass(
                        'active',
                        $image.is('[style*="transform"]')
                    );
                };
        ...
        _createPalette() {
            const $dropdownContent = this.toolbar.$el.find(
                '#colorInputButtonGroup .colorPalette'
            );
            ...
            for (const elem of $dropdownContent) {
                ...
                $dropdown.on('hide.bs.dropdown', (ev) => {
                    return !(
                        ev.clickEvent &&
          ev.clickEvent.originalEvent &&
          ev.clickEvent.originalEvent.__isColorpickerClick
                    );
                });
            ...
            targetNode && targetNode.nodeType === Node.ELEMENT_NODE
                ...
                : targetNode && targetNode.parentNode;
                        colorpicker = new ColorPaletteWidget(this, {
                            ...
                            selectedColor: $(targetElement).css(
                                eventName === 'foreColor' ? 'color' : 'backgroundColor'
                            ),
                        });
                ...
                toolbarPos.top +
                this.toolbar.$el.outerHeight() +
                colorpickerHeight;
                                const clientHeight =
                this.odooEditor.document.documentElement.clientHeight;
                                top =
                colorpickerBottom > clientHeight
                    ? colorpickerHeight - clientHeight
                    : '';
                            }
                            $colorpickerMenu.css({
                                top,
                                height: $colorpicker.outerHeight() + 2,
                            });
                            manualOpening = false;
                        });
                    });
        ...
        _colorToHex: function (color) {
            if (color.startsWith('#')) {
                ...
                if (color.startsWith('rgb')) {
                    ...
                    const propertyName = color.startsWith('text')
                        ? 'color'
                        : 'backgroundColor';
   ...
   */
        _updateEditorUI: function (e) {
            ...
            this.toolbar.$el
                .find('#mediaParagraphDropdownButton')
                .attr('id', 'paragraphDropdownButton');
            ...
            const spansBlocks =
      range &&
      !!$rangeContainer.contents().filter((i, node) => isBlock(node)).length;
            this.toolbar.$el
                .find('#create-link')
                .toggleClass('d-none', !range || spansBlocks);
            ...
            this.toolbar.$el
                .find(['#image-transform', '#image-crop', '#media-description'].join(','))
                .toggleClass('d-none', !$target.is('img'));
            ...
            this.toolbar.$el
                .find('#colorInputButtonGroup, #create-link')
                .toggleClass('d-none', isInMedia && !$target.is('.fa'));
            ...
            if (isInMedia) {
                ...
                for (const button of this.toolbar.$el.find(
                    '#image-shape div, #fa-spin'
                )) {
        ...
        _updateMediaJustifyButton: function (commandState) {
            ...
            const $paragraphDropdownButton = this.toolbar.$el.find(
                '#paragraphDropdownButton, #mediaParagraphDropdownButton'
            );
            ...
            if (commandState) {
                ...
                resetAlignment = !['float-left', 'mx-auto', 'float-right'].some(
                    (className) => this.lastMediaClicked.classList.contains(className)
                );
            }
            for (const button of $buttons) {
                button.classList.toggle(
                    'active',
                    !resetAlignment && button.id === commandState
                );
            }
        ...
        _getSnippetsCommands: function () {
            ...
            return [
                ...
                },
                {
                    ...
                    fontawesome: 'fa-sticky-note',
                    callback: () => {
                        snippetCommandCallback(
                            '.oe_snippet_body[data-snippet="s_text_highlight"]'
                        );
                    },
                },
                {
                ...
                },
                {
                    ...
                    callback: () => {
                        snippetCommandCallback(
                            '.oe_snippet_body[data-snippet="s_progress_bar"]'
                        );
                    },
                },
                {
                ...
                },
                {
                    ...
                    callback: () => {
                        snippetCommandCallback(
                            '.oe_snippet_body[data-snippet="s_blockquote"]'
                        );
                    },
                },
                {
   ...
   */
        _saveViewBlocks: function (context) {
            ...
            $('.o_editable').removeClass(
                'o_editable o_is_inline_editable o_editable_date_field_linked o_editable_date_field_format_changed'
            );
   ...
   */
        _getEscapedElement: function ($el) {
            ...
            to_escape = to_escape.not(
                to_escape
                    .filter(
                        'object,iframe,script,style,[data-oe-model][data-oe-model!="ir.ui.view"]'
                    )
                    .find('*')
                    .addBack()
            );
