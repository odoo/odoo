...
export const MASONRY = {
    CLASSES: {
        MODE: "o_masonry",
        CONTAINER: ["o_container_small", "container", "container-fluid"],
        ROW: "o_masonry_mode",
...
function _getElementIndex(el) {
    ...
    // Handle case where the element is wrapped in an anchor tag
...
async function _createMasonryLayout(rowEl, desktopColumnCount, mobileColumnCount) {
    ...
    console.debug("originalEls: ", originalEls);
    ...
    return new Promise(async (resolve) => {
        ...
        for (const el of originalEls) {
            ...
            console.debug("pushed el in column: ", el);
        }
 ...
 *                                MASONRY.CLASSES.CONTAINER, consisting of a
 *                                child with the class "row" that holds all the
 *                                the masonry items.
 * @returns {Promise} Resolves when all images are loaded and placed
 */
...
export function isValidMasonryLayout(sectionEl) {
    ...
    const isContainerClass = MASONRY.CLASSES.CONTAINER.some((containerClass) => 
...
export function observeMasonryContainerResize(sectionEl, debounceTime = 100) {
    ...
    const resizeObserver = new ResizeObserver(
        debounce(toggleMasonryMode.bind(this, masonryContainerEl), debounceTime)
    );
