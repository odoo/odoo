...
var SnippetEditor = Widget.extend({
    ...
    custom_events: {
        ...
        'toggle_overlay': '_onToggleOverlay',
    },
    overlayColors: ['lightblue', 'lightgreen', 'lightsalmon', 'plum'],
    ...
    init: function (parent, target, templateOptions, $editable, options) {
        ...
        this.maxParentingLevels = this.overlayColors.length;
    ...
    start: function () {
        ...
        if (this.isTargetMovable) {
            ...
            this.$el.draggable({
                ...
                stop: this._onDragAndDropStop.bind(this),
            });
        } else {
            this.$('.o_move_handle').addClass('d-none');
            this._customize$Elements.forEach($el => {
                $el.find('.oe_snippet_clone').addClass('d-none');
            });
        }

        if (!this.isTargetParentEditable) {
            this._customize$Elements.forEach($el => {
                $el.find('.oe_snippet_remove').addClass('d-none');
            });
        }
     ...
     *        If set to true, only displays the current editor overlay (not its
     *        parent ones) and do not toggle the customize options.
     */
    toggleFocus: function (focus, previewMode) {
        ...
        if (!previewMode) {
            ...
            [].slice.call(this._customize$Elements).reverse().forEach(($el, index) => {
                var styles = _.values($el.data('editor').styles);
                $el.toggleClass('d-none', styles.length === 0 || index >= this.maxParentingLevels);
                $el.css('border-color', this.overlayColors[index]);
                $el.css('color', this.overlayColors[index]);
            });
            this._toggleOptions(focus ? (this._customize$Elements.length - 1) : false);
        }

        // Activate the overlay(s)
        this.trigger_up('toggle_overlay', {
            enable: !!focus,
            color: 0,
    ...
    _toggleOptions: function (index) {
        this._customize$Elements.forEach(($el, i) => {
            $el.toggleClass('o_we_closed', i !== index);

            var editor = $el.data('editor');
            editor.$el.toggleClass('o_we_options_opened', i === index);

            var styles = _.values(editor.styles);
            _.each(_.sortBy(styles, '__order').reverse(), function (style) {
                if (i === index) {
                    _doActionFocus(style, $el);
                } else {
                    _doActionBlur(style, $el);
                }
            });
        });

        function _doActionFocus(style, $dest) {
            style.$el.appendTo($dest);
            style.onFocus();
            return (style.$el.length > 0);
        }
        function _doActionBlur(style, $dest) {
            style.$el.detach();
            style.onBlur();
            return false;
        }
     ...
     * @private
     */
    _toggleOverlay: function (enable, color, previewMode) {
        this.$el.toggleClass('oe_active', enable);
        if (!previewMode) {
            this.$el.toggleClass('o_we_sticky', enable);
            this.$el.css('border-color', enable ? this.overlayColors[color] : '');
        }
    ...
    _onOptionsSectionClick: function (ev) {
        ...
        // TODO improve: here is an hack to get the last shown editor
        var $neighbors = $optionsSection.nextAll().last();
        var editor = $neighbors.length ? $neighbors.data('editor') : this;
     ...
     * @private
     */
    _onToggleOverlay: function (ev) {
        if (ev.data.previewMode) {
            // In preview mode, we only want to toggle one overlay and leave the
            // parents untouched.
            ev.stopPropagation();
        }

        var enable = ev.data.enable;
        if (enable && (ev.data.color + 1) >= this.maxParentingLevels) {
            ev.data.enable = false;
        }
        this._toggleOverlay(enable, ev.data.color++, ev.data.previewMode);
    },
    /**
...
var SnippetsMenu = Widget.extend({
    ...
    start: function () {
        ...
        var lastElement, lastEvent;
        this.$document.on('click.snippets_menu mouseover.snippets_menu', '*', ev => {
            var srcElement = ev.target || (ev.originalEvent && (ev.originalEvent.target || ev.originalEvent.originalTarget)) || ev.srcElement;
            if (!srcElement || (lastElement === srcElement && lastEvent === ev.type)) {
                return;
            }
            ...
            var previewMode = (ev.type !== 'click');
            if (!previewMode && $target.closest('#snippets_menu').length) {
            ...
            this._activateSnippet($target, previewMode);
        });
     ...
     * @param {boolean} [previewMode=false]
     *        If set to true, only enable the overlay for the current $snippet
     *        (not its parents) and do not toggle the customize options.
    ...
    _onCollapseTogglerClick: function (ev) {
        this.$('we3-collapse-area > we3-toggler').not(ev.currentTarget).removeClass('active');
    ...
    _onUpdateCustomizeElements: function (ev) {
        ...
        this.customizePanel.classList.toggle('d-none', !ev.data.customize$Elements.length);
    },
});
