...
registry.colorpicker = SnippetOption.extend({
    ...
    start: function () {
        ...
        if (!this.$el.find('.colorpicker').length) {
            ...
            var $clpicker = $(qweb.render('web_editor.colorpicker')).prepend("<h6 class='text-muted'>" + _t("Theme Colors") + "</h6>")
            ...
            if (!('common' in excluded)) {
                var $commonColorTab = $clpicker.find('[data-name="common"]').prepend("<h6 class='text-muted'>" + _t("Common Colors") + "</h6>");
                var colors = ['#000000', '#424242', '#636363', '#9C9C94', '#CEC6CE', '#EFEFEF', '#F7F7F7', '#FFFFFF',
                ...
                _.each(colors, function (color) {
                    $commonColorTab.append('<button class="o_custom_color" style="background-color: ' + color + '" />')
                });
            }
