...
var SnippetOption = Widget.extend({
     ...
     * @returns {HTMLElement}
     */
    buildGroupElement: function (title, options) {
        const groupEl = SnippetOption.prototype.buildElement('we-group', title, options);

        if (options && options.childNodes) {
            options.childNodes.forEach(node => groupEl.appendChild(node));
        }
