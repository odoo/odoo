...
registry.BackgroundShape = SnippetOptionWidget.extend({
    ...
    // Are they any other elements with a shape in the same snippet ?
    _findElementsWithShape(element) {
    ...
    _getNeighborShape(target = this.$target[0], direction) {
        ...
        switch (direction) {
            ...
            case "bottom":
                ...
                break;
            case "inner": {
                const innerElements = this._findElementsWithShape(target);
        ...
        const ShapeElements = this._findElementsWithShape(element);
    ...
    notify(name, data) {
        ...
        } else if (name === "updateNeighbor" && data.target === this.$target[0]) {
            this._updateShapeForNeighborBeforeMoving();
        } else {
            this._super(...arguments);
        }
     ...
     * @param {string} newColor Indicates if the connection shape has to be flip to be next
