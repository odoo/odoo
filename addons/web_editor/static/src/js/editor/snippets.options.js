...
var SnippetOption = Widget.extend({
    events: {
        ...
        'mouseenter we3-button': '_onLinkEnter',
        'click': '_onLinkClick',
        'click we3-button': '_onLinkClick',
        'mouseleave': '_onMouseleave',
        'mouseleave we3-button': '_onMouseleave',
    },
    ...
    selectClass: function (previewMode, value, $opt) {
        var $group = $opt && $opt.parents('we3-collapse-area').last();
    ...
    _reset: function () {
        var self = this;
        var $actives = this.$el.find('we3-button.active');
    ...
    _setActive: function () {
        ...
        var $submenus = this.$el.find('we3-collapse-area')
            .not('we3-collapse-area we3-collapse-area');
    ...
    _onLinkEnter: function (ev) {
        var $opt = $(ev.target).closest('we3-button');
        if (!$opt.length) {
            return;
        }
...
registry.colorpicker = SnippetOption.extend({
    ...
    start: function () {
        ...
        if (!this.$el.find('.colorpicker').length) {
            ...
            this.$el.find('we3-collapse').append($pt);
        }
