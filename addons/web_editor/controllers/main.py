...
def get_existing_attachment(IrAttachment, vals):
    ...
    """
    fields = dict(vals)
    if fields['res_model'] == 'web_editor.media':
...
class Web_Editor(http.Controller):
    ...
    @http.route('/web_editor/media/add_data', type='json', auth='user', methods=['POST'], website=True)
    def add_media_data(self, name, data, is_image, quality=0, width=0, height=0, res_id=False, res_model='ir.ui.view', **kwargs):
        """Creates a `web_editor.media` and its related `ir.attachment`."""
        ...
        media = self._media_create(
            ...
            mimetype=preprocessed_data.get('mimetype'),
            res_id=res_id,
            res_model=res_model
        )
    ...
    @http.route('/web_editor/media/add_url', type='json', auth='user', methods=['POST'], website=True)
    def add_url(self, url, res_id=False, res_model='ir.ui.view', **kwargs):
        """Creates a `web_editor.media` and its related `ir.attachment` when
        the media is given through a link.
        """
        self._clean_context()
        media = self._media_create(url=url, res_id=res_id, res_model=res_model)
        return media._get_media_info()

    @http.route('/web_editor/media/fetch', type='json', auth='user', methods=['POST'], website=True)
    def fetch_media_attachments(self, media_domain, attachments_domain, offset, limit, **kwargs):
        return request.env['web_editor.media']._fetch_media_attachments(media_domain, attachments_domain, offset, limit)
    ...
    def remove(self, ids, **kwargs):
        ...
        Media = media = media_to_remove = request.env['web_editor.media']
    ...
    def _prepare_create(self, name='', data=False, url=False, res_id=False, res_model='ir.ui.view', mimetype=None, **kwargs):
    ...
    def _media_create(self, name='', data=False, mimetype=None, url=False, res_id=False, res_model='ir.ui.view'):
        """Create and return a new media attachment."""
        Media = request.env['web_editor.media']
        IrAttachment = request.env['ir.attachment']

        attachment_data = Web_Editor._prepare_create(self, name, data, url, res_id, res_model, mimetype)
        ...
        if (
            ...
            existing_attachment_data = dict(attachment_data)
            existing_attachment_data.update({
    ...
    def _attachment_create(self, name='', data=False, url=False, res_id=False, res_model='ir.ui.view'):
        ...
        attachment_data = Web_Editor._prepare_create(self, name, data, url, res_id, res_model)
        # Despite the user having no right to create an attachment, he can still
        # create an image attachment through some flows
        if (
            not request.env.is_admin()
    ...
    def save_library_media(self, media):
        ...
        for id, url in response.json().items():
            ...
            media_attachment = request.env['web_editor.media'].with_user(SUPERUSER_ID).create({
                'name': name,
            ...
            attachment = media_attachment._get_attachment()
