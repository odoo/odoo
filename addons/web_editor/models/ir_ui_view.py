    ...
    def _views_get(self, view_id, get_children=True, bundles=False, root=True, visited=None):
        ...
        for child in node.xpath(xpath):
            try:
                ...
                continue
            if called_view and called_view not in views_to_return and called_view not in visited:
                views_to_return += self._views_get(called_view, get_children=get_children, bundles=bundles, visited=visited + views_to_return)
        ...
        for extension in extensions.sorted(key=lambda v: v.id):
            ...
            if extension not in visited:
                for ext_view in self._views_get(extension, get_children=extension.active, root=False, visited=visited + views_to_return):
