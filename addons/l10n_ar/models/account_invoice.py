        ...
        for rec in self.filtered(lambda x: x.company_id.country_id == self.env.ref('base.ar')):
            if rec.company_id.currency_id == rec.currency_id:
                ...
                l10n_ar_currency_rate = rec.currency_id._convert(
                    1.0, rec.company_id.currency_id, rec.company_id, rec.date_invoice or fields.Date.today(),
                    round=False)
    ...
    def set_date_afip(self):
        for rec in self.filtered('date_invoice'):
            ...
            if not rec.l10n_ar_afip_service_start:
                vals['l10n_ar_afip_service_start'] = date_invoice + relativedelta(day=1)
            if not rec.l10n_ar_afip_service_end:
                vals['l10n_ar_afip_service_end'] = date_invoice + relativedelta(day=1, days=-1, months=+1)
            if vals:
                rec.write(vals)
