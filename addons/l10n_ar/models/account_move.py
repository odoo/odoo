...
class AccountMove(models.Model):
    ...
    def _get_vat(self, base_lines=None):
        ...
        for grouping_key, values in vat_afip_code_aggregated_tax_details.items():
            if grouping_key['vat_afip_code'] not in (False, '0', '1', '2') and (values['base_amount'] or values['tax_amount']):
                res.append({
                    'BaseImp': float_round(amount_sign * values['base_amount'], precision_digits=2),
                    'Id': grouping_key['vat_afip_code'],
                    'Importe': float_round(amount_sign * values['tax_amount'], precision_digits=2),
                })
