...
class AccountMove(models.Model):
    ...
    def _compute_l10n_latam_document_type(self):
        ...
        for rec in self.filtered(lambda x: x.company_id.account_fiscal_country_id.code == "AR" and x.state == 'draft' and x.move_type in ['in_invoice', 'in_refund']):
            if rec.l10n_latam_document_type_id and rec.partner_id.l10n_ar_afip_responsibility_type_id.code == '8':
                # Foreign Vendor
                rec.l10n_latam_document_type_id = rec.l10n_latam_available_document_type_ids.filtered(lambda x: x.l10n_ar_letter in ['I'])[0].id
