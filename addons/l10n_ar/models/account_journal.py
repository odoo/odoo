...
class AccountJournal(models.Model):
    ...
    def _compute_l10n_ar_afip_pos_system(self):
        self.filtered(lambda x: not x.l10n_ar_is_pos).l10n_ar_afip_pos_system = False
    ...
    def _check_afip_pos_system(self):
        journals = self.filtered(
            lambda x: x.l10n_ar_is_pos and x.type == 'purchase' and
    ...
    def _check_afip_pos_number(self):
        if self.filtered(lambda x: x.l10n_ar_is_pos and x.l10n_ar_afip_pos_number == 0):
            raise ValidationError(_('Please define an AFIP POS number'))

        if self.filtered(lambda x: x.l10n_ar_is_pos and x.l10n_ar_afip_pos_number > 99999):
