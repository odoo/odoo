    ...
    switch (f.getAttribute("name")) {
        ...
        case "secret":
            ...
            copyButton.onclick = async function (event) {
                ...
                const copyBtnPopover = window.Tooltip.getOrCreateInstance(copyButton, {
                    title: _t("Copied!"),
                    trigger: "manual",
                    placement: "bottom",
                });
                await browser.navigator.clipboard.writeText(secretSpan.textContent);
                copyBtnPopover.show();
                setTimeout(() => copyBtnPopover.hide(), 800);
            };
