...
class TestOrderEdiUbl(TestAccountEdiUblCii):
    ...
    def get_xml_attchament_of_po(self, po_line_vals, **po_vals):
    ...
    def test_so_fallback_partner(self):
        ...
        xml_attachment = self.get_xml_attchament_of_po([])
    ...
    def test_import_product_from_po(self):
        ...
        xml_attachment = self.get_xml_attchament_of_po(line_vals)
        so = self.env['sale.order']._create_order_from_attachment(xml_attachment.ids)
    ...
    def test_product_unit_price_with_different_uom(self):
        ...
        for line in line_vals:
            ...
            line['product_uom_qty'] = line['product_qty']
            product_uom_factor = product_uom._compute_quantity(1.0, line_product.uom_id)
            # Price unit based on UOM factor
            line['price_unit'] = line_product.list_price * product_uom_factor
            del line['product_qty']
    ...
    def test_import_payment_terms(self):
        ...
        xml_attachment = self.get_xml_attchament_of_po([], payment_term_id=payment_term.id)
        so = self.env['sale.order']._create_order_from_attachment(xml_attachment.ids)
