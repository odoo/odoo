...
class SaleOrder(models.Model):
    ...
    def _compute_delivery_status(self):
        for order in self:
            out_pickings = order.picking_ids.filtered(lambda a: a.picking_type_id.code == 'outgoing')
            if not order.picking_ids or all(p.state == 'cancel' for p in order.picking_ids) or (out_pickings and all(p.state == 'cancel' for p in out_pickings)):
                order.delivery_status = False
            elif all(p.state == 'done' for p in order.picking_ids):
                order.delivery_status = 'full'
            elif any(p.state == 'done' for p in out_pickings) and any(line.qty_delivered > 0 for line in order.order_line):
                order.delivery_status = 'partial'
            elif any(p.state == 'done' for p in order.picking_ids):
                order.delivery_status = 'preparation'
            elif any(p.state == 'confirmed' for p in order.picking_ids):
    ...
    def _get_action_view_picking(self, pickings):
        ...
        action['context'] = dict(default_partner_id=self.partner_id.id, default_picking_type_id=picking_id.picking_type_id.id, default_origin=self.name, default_group_id=picking_id.group_id.id, create=False)
        return action
