...
class TestSaleStockLeadTime(TestStockCommon):
    ...
    def test_02_delivery_date_propagation(self):
        ...
        # Update company with Sales Safety Days
        company.write({'security_lead': 2.00})

        # Update warehouse_1 with Outgoing Shippings pick + pack + ship
        self.warehouse_1.write({'delivery_steps': 'pick_pack_ship'})

        # Set delay on pull rule
        self.warehouse_1.delivery_route_id.rule_ids.write({'delay': 5})

        # Update the product_1 with type and Customer Lead Time
        self.product_1.write({'type': 'product', 'sale_delay': 30.0})
        ...
        out = order.picking_ids.filtered(lambda r: r.picking_type_id == self.warehouse_1.out_type_id)
        out_deadline_date = order.date_order + timedelta(days=self.product_1.sale_delay) - timedelta(days=out.move_lines[0].rule_id.delay)
        out_scheduled_date = order.date_order + timedelta(days=self.product_1.sale_delay) - timedelta(days=out.move_lines[0].rule_id.delay) - timedelta(days=company.security_lead)
        self.assertAlmostEquals(
            out.scheduled_date, out_scheduled_date, timedelta(seconds=1),
            'Schedule date of ship type picking should be equal to: order date + Customer Lead Time - pull rule delay - security_lead')
        self.assertAlmostEquals(
            out.date_deadline, out_deadline_date, timedelta(seconds=1),
            'Deadline date of ship type picking should be equal to: order date + Customer Lead Time - pull rule delay.')
        ...
        pack_deadline_date = pack_scheduled_date + timedelta(days=company.security_lead)
        self.assertAlmostEquals(
            pack.scheduled_date, pack_scheduled_date, timedelta(seconds=1),
            'Schedule date of pack type picking should be equal to: Schedule date of ship type picking - pull rule delay.')
        self.assertAlmostEquals(
            pack.date_deadline, pack_deadline_date, timedelta(seconds=1),
            'Deadline date of pack type picking should be equal to: Deadline date of ship type picking - pull rule delay.')
        ...
        pick_deadline_date = pick_scheduled_date + timedelta(days=company.security_lead)
        self.assertAlmostEquals(
            pick.scheduled_date, pick_scheduled_date, timedelta(seconds=1),
            'Schedule date of pack type picking should be equal to: Schedule date of ship type picking - pull rule delay.')
        self.assertAlmostEquals(
            pick.date_deadline, pick_deadline_date, timedelta(seconds=1),
            'Deadline date of pack type picking should be equal to: Deadline date of ship type picking - pull rule delay.')

        # Now change the commitment date (Delivery Date) of the sale order
        new_deadline = out_deadline_date + timedelta(days=5)
        order.write({'commitment_date': new_deadline})

        # Now check date_deadline of pick, pack and out are forced
        # TODO doesn't take in account `delay` of the rules ?
        self.assertEquals(out.date_deadline, new_deadline)
        self.assertEquals(pick.date_deadline, new_deadline)
        self.assertEquals(pack.date_deadline, new_deadline)
