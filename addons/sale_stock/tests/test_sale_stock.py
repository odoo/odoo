...
class TestSaleStock(TestSaleStockCommon, ValuationReconciliationTestCommon):
    ...
    def test_so_cancellation_creates_reverse_pickings(self):
        """Ensure that cancelling an SO creates the appropriate reverse pickings
        and allows them to be partially reserved and completed.
        """
        warehouse = self.company_data['default_warehouse']
        warehouse.delivery_steps = 'pick_pack_ship'
        stock_location = warehouse.lot_stock_id
        ...
        pick_backorder = pick.backorder_ids
        pack = sale_order.picking_ids - pick - pick_backorder
        (pack | pick_backorder).button_validate()
        sale_order.action_cancel()
