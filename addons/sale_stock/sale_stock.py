    ...
    def product_id_change_with_wh(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        if product_obj.type == 'product':
            ...
            if not is_available:
                ...
                if compare_qty == -1:
                    warn_msg = _('You plan to sell %.2f %s but you only have %.2f %s in stock ! \nThe total quantity of incoming stock is %.2f %s.') % \
                        (qty, uom_record.name,
                         max(0,product_obj.qty_available), uom_record.name,
                         max(0,product_obj.incoming_qty), uom_record.name)
