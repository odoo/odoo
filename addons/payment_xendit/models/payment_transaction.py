...
class PaymentTransaction(models.Model):
    ...
    def _xendit_tokenize_notification_data(self, notification_data):
        ...
        token = notification_data.get('credit_card_token')
        response = self.provider_id._xendit_make_request('TOKEN', endpoint_param={'token_id': token}, method='GET')
        card_info = response.get('masked_card_number')[-4:]

        _logger.info("Creating token for card: %s", token)
