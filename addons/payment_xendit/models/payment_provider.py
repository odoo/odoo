...
class PaymentProvider(models.Model):
    ...
    def _autovacuum_notify_xendit_webhook_migration(self):
        ...
        summary = self.env._("Update the Xendit webhook configuration")
        ...
        webhook_link = Markup('<a href="%s" target="_blank">%s</a>') % (
            doc_url,
            self.env._("webhook configuration"),
        )
        dashboard_link = Markup('<a href="%s" target="_blank">%s</a>') % (
            dashboard_url,
            self.env._("Xendit Dashboard"),
        )

        # Archived providers are excluded, and unconfigured ones have no webhook to update.
        for provider in self.search([("code", "=", "xendit"), ("xendit_secret_key", "!=", False)]):
