...
class TestPaymentTransaction(PaymentHttpCommon, XenditCommon):
    ...
    def test_rounded_amount_idr(self):
        ...
        self.assertEqual(tx._xendit_get_rounded_amount(), 1000)
    ...
    def test_get_pending_authentication_url_when_requires_action(self):
        ...
        with patch(
            ...
            '._xendit_send_request', return_value=self.payment_request_requires_action_data
        ) as mock_req:
    ...
    def test_sync_from_provider_checks_session_for_redirect_operation(self):
        ...
        with patch.object(
            PaymentProvider, '_xendit_send_request', return_value=self.webhook_notification_data
        ) as mock_req:
            tx._xendit_sync_from_provider()
    ...
    def test_no_item_missing_from_legacy_token_charge_payload(self):
        ...
        with `is_recurring` set to avoid a repeated 3DS challenge on every renewal. """
        currency_idr = self.env.ref('base.IDR')
        tx = self._create_transaction('redirect', amount=1000.50, currency_id=currency_idr.id)
        with patch(
            'odoo.addons.payment_xendit.models.payment_provider.PaymentProvider'
            '._xendit_make_request', return_value=self.legacy_charge_notification_data
        ) as mock_req:
            tx._xendit_create_legacy_token_charge('legacy-token-123')
        self.assertEqual(mock_req.call_args.args[0], 'credit_card_charges')
        self.assertDictEqual(mock_req.call_args.kwargs.get('payload'), {
    ...
    def test_processing_legacy_charge_notification_confirms_transaction(self):
        ...
        tx = self._create_transaction('token')
        tx._process_notification_data(self.legacy_charge_notification_data)
    ...
    def test_get_tx_from_notification_data_falls_back_to_external_id(self):
    ...
    def test_tokenize_from_notification_data_fetches_masked_card_number(self):
        ...
        with patch(
            'odoo.addons.payment_xendit.models.payment_provider.PaymentProvider'
            '._xendit_send_request', return_value=self.payment_token_data
        ) as mock_req:
