...
class EventTrackController(http.Controller):
    ...
    def _split_track_by_days(self, track, local_tz):
        ...
        """
        start_date = fields.Datetime.from_string(track.date).replace(tzinfo=pytz.utc).astimezone(local_tz)
        track_start_time_slot = start_time_slot = self.time_slot_rounder(start_date, 15)
        end_datetime = self.time_slot_rounder(start_time_slot + timedelta(hours=(track.duration or 0.25)), 15)
        time_slots_count = int(((end_datetime - start_time_slot).total_seconds() / 3600) * 4)

        time_slots_by_day_start_time = {start_time_slot: 0}
        next_day = (start_time_slot + timedelta(days=1)).date()
        for i in range(0, time_slots_count):
            # If the new time slot is still on the current day
            time_slots = track_start_time_slot + timedelta(minutes=15 * i)
            if time_slots.date() < next_day:
                time_slots_by_day_start_time[start_time_slot] += 1
            else:
                start_time_slot = time_slots
                time_slots_by_day_start_time.update({start_time_slot: 0})
                time_slots_by_day_start_time[start_time_slot] += 1
