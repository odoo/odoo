...
class EventTrackController(http.Controller):
    ...
    def _prepare_calendar_values(self, event):
        ...
        for track, time_slots in time_slots_by_tracks.items():
            ...
            for time_slot, duration in time_slots.items():
                tracks_by_rounded_times[time_slot][track.location_id][track] = {
                    ...
                    'occupied_cells': self._get_occupied_cells(track, time_slot, duration, locations, local_tz)
                }
        ...
        for day, time_slots in time_slots_by_day.items():
            ...
            next_day = (start_time_slot + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(minutes=15)
            end_time_slot = min(max(time_slots['end']), next_day)
    ...
    def _split_track_by_days(self, track, local_tz):
        ...
        start_date = fields.Datetime.from_string(track.date).replace(tzinfo=UTC).astimezone(local_tz)
        track_start = day_start = self.time_slot_rounder(start_date, 15)
        track_end = self.time_slot_rounder(day_start + timedelta(hours=(track.duration or 0.25)), 15)
        total_slot_count = int(((track_end - day_start).total_seconds() / 3600) * 4)

        slot_starts_by_day = {day_start: 0}
        next_day = (day_start + timedelta(days=1)).date()
        slot_end = track_start - timedelta(minutes=15)

        for i in range(0, total_slot_count):
            # If the new time slot is still on the current day
            slot_end += timedelta(minutes=15)
            if slot_end.date() < next_day:
                slot_starts_by_day[day_start] += 1
            else:
                day_start = slot_end
                slot_starts_by_day[day_start] = 1
                next_day += timedelta(days=1)

        return slot_starts_by_day

    def _get_occupied_cells(self, track, start_time_slot, rowspan, locations, local_tz):
        """
        ...
        for i in range(0, rowspan):
            time_slot = start_time_slot + timedelta(minutes=15 * i)
