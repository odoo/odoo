...
class EventTrack(models.Model):
    ...
    def _get_track_calendar_external_description(self):
    ...
    def _get_track_calendar_ics_file(self):
        """ Returns iCalendar file for the event track.
            :returns a dict of .ics file content for each event track
        """
        result = {}
        if not vobject:
            return result
        ...
        for track in self:
            ...
            cal_track.add('description').value = track._get_track_external_description()
    ...
    def _get_track_calendar_resource_urls(self):
        date_tz = self.event_id.date_tz
        reminder_dates = self._get_event_track_reminder_dates()
        ...
        params = {
            ...
            'details': self._get_track_calendar_external_description(),
        }
        ...
        return {'google_url': google_url, 'iCal_url': iCal_url, 'yahoo_url': self._get_track_calendar_yahoo_resource_url()}

    def _get_track_calendar_yahoo_resource_url(self):
        date_tz = self.event_id.date_tz
        ...
        params = {
            ...
            'st': reminder_dates['date_begin'].astimezone(timezone(date_tz)).strftime('%Y%m%dT%H%M%S'),
            'et': reminder_dates['date_end'].astimezone(timezone(date_tz)).strftime('%Y%m%dT%H%M%S'),
