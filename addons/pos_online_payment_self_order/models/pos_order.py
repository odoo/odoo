...
class PosOrder(models.Model):
    ...
    def get_order_to_print(self):
        ...
        self.nb_print += 1
        pos_data = self.read_pos_data([], self.config_id.id)

        if self.partner_id and 'res.partner' not in pos_data:
            pos_data['res.partner'] = self.partner_id._load_pos_data_read(self.partner_id, self.config_id)
        return pos_data
