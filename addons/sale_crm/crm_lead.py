...
class crm_lead(models.Model):
    ...
    def retrieve_sales_dashboard(self, cr, uid, context=None):
        ...
        # Invoiced
        invoice_ids = self.pool.get('account.invoice').search_read(cr, uid, account_invoice_domain, context=context)
        for inv in invoice_ids:
            if inv['date']:
                ...
                if inv_date <= date.today() and inv_date >= date.today().replace(day=1):
                    res['invoiced']['this_month'] = res['invoiced']['this_month'] + inv['amount_untaxed_signed']
                elif inv_date < date.today().replace(day=1) and inv_date >= date.today().replace(day=1) - relativedelta(months=+1):
                    res['invoiced']['last_month'] = res['invoiced']['last_month'] + inv['amount_untaxed_signed']
