    ...
    def process_sheet(self, cr, uid, ids, context=None):
        ...
        for slip in self.browse(cr, uid, ids, context=context):
            ...
            if float_compare(credit_sum, debit_sum, precision_digits=precision) == -1:
                ...
                if not acc_id:
                    raise UserError(_('Configuration Error!'),_('The Expense Journal "%s" has not properly configured the Credit Account!')%(slip.journal_id.name))
                adjust_credit = (0, 0, {
                ...
                if not acc_id:
                    raise UserError(_('Configuration Error!'),_('The Expense Journal "%s" has not properly configured the Debit Account!')%(slip.journal_id.name))
                adjust_debit = (0, 0, {
                    'name': _('Adjustment Entry'),
                    'date': timenow,
                    'partner_id': False,
