...
class TestAccountValidateAccount(AccountingTestCase):
    ...
    def test_account_validate_account(self):
        ...
        account_cash = self.env['account.account'].search([('user_type_id.account_type', '=', 'liquidity')], limit=1)
        journal = self.env['account.journal'].search([('journal_type', '=', 'bank')], limit=1)
