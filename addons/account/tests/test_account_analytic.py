...
class TestAccountAnalyticAccount(AccountTestInvoicingCommon, AnalyticCommon):
    ...
    def test_model_score(self):
        ...
        invoice = self._create_invoice(partner_id=self.partner_a, products=[self.product_a])
        ...
        invoice = self._create_invoice(partner_id=self.partner_a, products=[self.product_b])
        self.assertEqual(invoice.invoice_line_ids.analytic_distribution, False)

        # Product match, score 1
        invoice = self._create_invoice(partner_id=self.partner_b, products=[self.product_a])
        ...
        invoice = self._create_invoice(partner_id=self.partner_b, products=[self.product_b])
        self.assertEqual(invoice.invoice_line_ids.analytic_distribution, False)
    ...
    def test_model_application(self):
        ...
        invoice = self._create_invoice(partner_id=self.env['res.partner'], products=[self.product_a])
    ...
    def test_mandatory_plan_validation_mass_posting(self):
        ...
        """
        invoice1 = self._create_invoice(partner_id=self.partner_a, products=[self.product_a])
        invoice2 = self._create_invoice(partner_id=self.partner_b, products=[self.product_a])
        self.analytic_plan_2.write({
            'applicability_ids': [Command.create({
                'business_domain': 'invoice',
                'product_categ_id': self.product_a.categ_id.id,
