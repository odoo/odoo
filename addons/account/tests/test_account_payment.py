...
class TestAccountPayment(AccountTestInvoicingCommon, MailCommon):
    ...
    def test_payment_communication_on_child_accounting_on_commercial(self):
        ...
        self.assertEqual(payment.move_id.partner_id, self.invoice_contact)
    ...
    def test_commercial_partner_id_search(self):
        """ Domain on commercial_partner_id works via the _search method. """
        payment = self.init_payment(-10.0, partner=self.invoice_contact)
        found = self.env['account.payment'].search([
            ('commercial_partner_id', '=', self.commercial_partner.id),
            ('id', '=', payment.id),
        ])
        self.assertEqual(found, payment)
