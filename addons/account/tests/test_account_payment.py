...
class TestAccountPayment(AccountTestInvoicingCommon, MailCommon):
    ...
    def test_attachments_send_multiple(self):
        ...
        attachment_record_ids = self.env['ir.attachment'].search([('res_model', '=', 'account.payment')]).mapped('res_id')
        self.assertEqual(self.env['account.payment'].browse(attachment_record_ids), payments)
