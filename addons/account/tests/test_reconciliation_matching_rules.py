...
class TestReconciliationMatchingRules(AccountingTestCase):
    ...
    def _check_statement_matching(self, rules, expected_values):
        ...
        matching_values = rules._apply_criteria(statement_lines)
        for st_line_id, values in matching_values.items():
            if not values.get('model'):
                # A line found a unexpected match.
                self.assertTrue(st_line_id not in expected_values)
                continue

            # Check values.
            self.assertEqual(values.get('model'), expected_values[st_line_id].get('model'))
            self.assertEqual(values.get('status'), expected_values[st_line_id].get('status'))
            self.assertEqual(set(values.get('model', [])), set(expected_values[st_line_id].get('model', [])))

        # A match is missing.
        self.assertFalse(set(expected_values.keys()) - set(matching_values.keys()))
    ...
    def setUp(self):
        ...
        self.invoice_line_3 = self._create_invoice_line(300, self.partner_1, 'out_invoice')
        self.invoice_line_4 = self._create_invoice_line(400, self.partner_1, 'out_invoice')
        self.invoice_line_5 = self._create_invoice_line(400, self.partner_1, 'out_refund')
        self.invoice_line_6 = self._create_invoice_line(1000, self.partner_2, 'in_invoice')
    ...
    def test_matching_fields(self):
        ...
        self._check_statement_matching(self.rule_1, {
            ...
            self.bank_line_2.id: {'aml_ids': [
                ...
                self.invoice_line_4.id,
                self.invoice_line_5.id,
            ], 'model': self.rule_1},
            self.cash_line_1.id: {'aml_ids': [self.invoice_line_6.id], 'model': self.rule_1},
        })
        ...
        self.rule_1.match_amount_max = 100
        self._check_statement_matching(self.rule_1, {
            self.bank_line_1.id: {'aml_ids': [self.invoice_line_1.id], 'model': self.rule_1},
        })
