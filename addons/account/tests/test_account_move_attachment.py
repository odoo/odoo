...
def read_file(file_path):
    ...
    with file_open(file_path, 'rb') as file:
        contents = BinaryBytes(file.read())
    if not contents:
...
def extract_pdf_from_xml(xml_file):
    ...
    """
    xml_root = etree.parse(BinaryBytes(xml_file).open()).getroot()
    embedded_documents = [
            {
    ...
    return [BinaryBytes(doc.get('data') or b'\0') for doc in pdf_documents]
...
class TestAccountMoveAttachment(HttpCase):
    ...
    def test_check_create_from_pdf_match(self):
        ...
        self.assertEqual(account_move.invoice_pdf_report_id.id, attachment.id)
    ...
    def test_check_create_from_xml_match(self):
        ...
        extracted_pdf_file_data = BinaryBytes(extract_pdf_from_xml(xml_file)[0])
        final_pdf_file_data = account_move.invoice_pdf_report_file or account_move.invoice_pdf_report_id.raw

        # compares the contents of the file (not very efficient)
        self.assertEqual(extracted_pdf_file_data.open().read(), BinaryBytes(final_pdf_file_data).open().read())
