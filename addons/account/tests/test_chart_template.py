...
class TestChartTemplate(AccountTestInvoicingCommon):
    ...
    def test_remove_fiscal_position_try_loading_force_create_false(self):
        ...
        # Remove related fiscal position tax mappings
        fiscal_position_taxes = self.env['account.fiscal.position.tax'].search([
            ('position_id', '=', fiscal_position.id)
        ])
        fiscal_position_taxes.unlink()

        # Now remove the fiscal position safely
        fiscal_position.unlink()

        # Ensure the fiscal position is removed
        fiscal_position_check = self.env['account.fiscal.position'].search([
            ('company_id', '=', self.company.id),
            ('name', '=', 'Fiscal Position')
        ])
        self.assertFalse(fiscal_position_check, "Fiscal Position should be deleted")
