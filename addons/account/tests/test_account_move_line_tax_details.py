...
class TestAccountTaxDetailsReport(AccountTestInvoicingCommon):
    ...
    def _get_tax_details(self, extra_domain=None, include_without_tax_line=False):
        domain = [('company_id', '=', self.env.company.id)] + (extra_domain or [])
        tax_details_query = self.env['account.move.line']._get_query_tax_details_from_domain(domain, include_without_tax_line)
    ...
    def test_include_base_lines_without_tax_lines(self):
        ...
        details = {row['base_line_id']: row for row in self._get_tax_details(domain, include_without_tax_line=True)}
