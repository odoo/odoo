...
class TestAccountTax(AccountTestInvoicingCommon, MailCase):
    ...
    def test_asymmetric_repartition_lines_neutral_tail(self):
        ...
        with self.assertRaisesRegex(ValidationError, "Invoice and credit note distribution should have the same total factor."):
            self.env['account.tax'].create({
                'name': "tax with mismatching aggregate factors",
                'amount': 15.0,
                'invoice_repartition_line_ids': [
                    Command.create({'repartition_type': 'base'}),
                    Command.create({'repartition_type': 'tax', 'factor_percent': 100.0}),
    ...
    def test_zero_amount_tax_without_tax_repartition_lines(self):
        ...
        with self.assertRaisesRegex(ValidationError, "Invoice distribution should have at least one non-zero tax repartition line."):
            self.env['account.tax'].create({
                'name': "tax",
                'amount': 15.0,
                'invoice_repartition_line_ids': [
