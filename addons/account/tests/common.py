...
class AccountTestSetup:
    ...
    @staticmethod
    def setup_company_data(obj, company_name, chart_template=None, **kwargs):
        ...
        def search_account(company, chart_template, field_name, domain):
            ...
            account = None
            if template_code:
                account = obj.env['account.account'].search(domain + [('code', '=like', template_code + '%')], limit=1)

            if not account:
                account = obj.env['account.account'].search(domain, limit=1)
            return account

        chart_template = chart_template or obj.env.company.chart_template_id
        currency = obj.env.user.company_id.currency_id
        company = obj.env['res.company'].create({
            'name': company_name,
            'currency_id': currency.id,
            **kwargs,
        })
        obj.env.user.company_ids |= company
        ...
        return {
            ...
            'default_account_revenue': obj.env['account.account'].search([
                    ('company_id', '=', company.id),
                    ('user_type_id', '=', obj.env.ref('account.data_account_type_revenue').id)
                ], limit=1),
            'default_account_expense': obj.env['account.account'].search([
                    ('company_id', '=', company.id),
            ...
            'default_account_payable': obj.env['account.account'].search([
                    ('company_id', '=', company.id),
                    ('user_type_id.type', '=', 'payable')
                ], limit=1),
            'default_account_assets': obj.env['account.account'].search([
                    ('company_id', '=', company.id),
                    ('user_type_id', '=', obj.env.ref('account.data_account_type_current_assets').id)
                ], limit=1),
            ...
            'default_journal_misc': obj.env['account.journal'].search([
                    ('company_id', '=', company.id),
                    ('type', '=', 'general')
                ], limit=1),
            'default_journal_sale': obj.env['account.journal'].search([
                    ('company_id', '=', company.id),
                    ('type', '=', 'sale')
                ], limit=1),
            'default_journal_purchase': obj.env['account.journal'].search([
                    ('company_id', '=', company.id),
                    ('type', '=', 'purchase')
                ], limit=1),
            'default_journal_bank': obj.env['account.journal'].search([
                    ('company_id', '=', company.id),
                    ('type', '=', 'bank')
                ], limit=1),
            'default_journal_cash': obj.env['account.journal'].search([
                    ('company_id', '=', company.id),
                    ('type', '=', 'cash')
                ], limit=1),
    ...
    @staticmethod
    def setup_multi_currency_data(obj, default_values={}, rate2016=3.0, rate2017=2.0):
        foreign_currency = obj.env['res.currency'].create({
            'name': 'Gold Coin',
            'symbol': '☺',
            'rounding': 0.001,
            'position': 'after',
        ...
        rate1 = obj.env['res.currency.rate'].create({
            'name': '2016-01-01',
            'rate': rate2016,
            'currency_id': foreign_currency.id,
            'company_id': obj.env.company.id,
        })
        rate2 = obj.env['res.currency.rate'].create({
            'name': '2017-01-01',
            'rate': rate2017,
            'currency_id': foreign_currency.id,
            'company_id': obj.env.company.id,
    ...
    @staticmethod
    def setup_armageddon_tax(obj, tax_name, company_data):
        return obj.env['account.tax'].create({
    ...
    @staticmethod
    def setup(obj, chart_template_ref=None):
        if chart_template_ref:
            chart_template = obj.env.ref(chart_template_ref)
        else:
            chart_template = obj.env.ref('l10n_generic_coa.configurable_chart_template', raise_if_not_found=False)
        if not chart_template:
            obj.skipTest(obj, "Accounting Tests skipped because the user's company has no chart of accounts.")

        # Create user.
        user = obj.env['res.users'].create({
            'name': 'Because I am accountman!',
            'login': 'accountman',
            'password': 'accountman',
            'groups_id': [(6, 0, obj.env.user.groups_id.ids), (4, obj.env.ref('account.group_account_user').id)],
        })
        user.partner_id.email = 'accountman@test.com'

        # Shadow the current environment/cursor with one having the report user.
        # This is mandatory to test access rights.
        obj.env = obj.env(user=user)
        obj.cr = obj.env.cr

        obj.company_data_2 = AccountTestSetup.setup_company_data(obj, 'company_2_data', chart_template=chart_template)
        obj.company_data = AccountTestSetup.setup_company_data(obj, 'company_1_data', chart_template=chart_template)

        user.write({
            'company_ids': [(6, 0, (obj.company_data['company'] + obj.company_data_2['company']).ids)],
            'company_id': obj.company_data['company'].id,
        })

        obj.currency_data = AccountTestSetup.setup_multi_currency_data(obj)

        # ==== Taxes ====
        obj.tax_sale_a = obj.company_data['default_tax_sale']
        obj.tax_sale_b = obj.company_data['default_tax_sale'].copy()
        obj.tax_purchase_a = obj.company_data['default_tax_purchase']
        obj.tax_purchase_b = obj.company_data['default_tax_purchase'].copy()
        obj.tax_armageddon = AccountTestSetup.setup_armageddon_tax(obj, 'complex_tax', obj.company_data)

        # ==== Products ====
        obj.product_a = obj.env['product.product'].create({
            'name': 'product_a',
            'uom_id': obj.env.ref('uom.product_uom_unit').id,
            'lst_price': 1000.0,
            'standard_price': 800.0,
            'property_account_income_id': obj.company_data['default_account_revenue'].id,
            'property_account_expense_id': obj.company_data['default_account_expense'].id,
            'taxes_id': [(6, 0, obj.tax_sale_a.ids)],
            'supplier_taxes_id': [(6, 0, obj.tax_purchase_a.ids)],
        })
        obj.product_b = obj.env['product.product'].create({
            'name': 'product_b',
            'uom_id': obj.env.ref('uom.product_uom_dozen').id,
            'lst_price': 200.0,
            'standard_price': 160.0,
            'property_account_income_id': obj.copy_account(obj.company_data['default_account_revenue']).id,
            'property_account_expense_id': obj.copy_account(obj.company_data['default_account_expense']).id,
            'taxes_id': [(6, 0, (obj.tax_sale_a + obj.tax_sale_b).ids)],
            'supplier_taxes_id': [(6, 0, (obj.tax_purchase_a + obj.tax_purchase_b).ids)],
        })

        # ==== Fiscal positions ====
        obj.fiscal_pos_a = obj.env['account.fiscal.position'].create({
            'name': 'fiscal_pos_a',
            'tax_ids': [
                (0, None, {
                    'tax_src_id': obj.tax_sale_a.id,
                    'tax_dest_id': obj.tax_sale_b.id,
                }),
                (0, None, {
                    'tax_src_id': obj.tax_purchase_a.id,
                    'tax_dest_id': obj.tax_purchase_b.id,
                }),
            ],
            'account_ids': [
                (0, None, {
                    'account_src_id': obj.product_a.property_account_income_id.id,
                    'account_dest_id': obj.product_b.property_account_income_id.id,
                }),
                (0, None, {
                    'account_src_id': obj.product_a.property_account_expense_id.id,
                    'account_dest_id': obj.product_b.property_account_expense_id.id,
                }),
            ],
        })

        # ==== Payment terms ====
        obj.pay_terms_a = obj.env.ref('account.account_payment_term_immediate')
        obj.pay_terms_b = obj.env['account.payment.term'].create({
        ...
        # ==== Partners ====
        obj.partner_a = obj.env['res.partner'].create({
            'name': 'partner_a',
            'property_payment_term_id': obj.pay_terms_a.id,
            'property_supplier_payment_term_id': obj.pay_terms_a.id,
            'property_account_receivable_id': obj.company_data['default_account_receivable'].id,
            'property_account_payable_id': obj.company_data['default_account_payable'].id,
            'company_id': False,
        })
        obj.partner_b = obj.env['res.partner'].create({
            'name': 'partner_b',
            'property_payment_term_id': obj.pay_terms_b.id,
            'property_supplier_payment_term_id': obj.pay_terms_b.id,
            'property_account_position_id': obj.fiscal_pos_a.id,
            'property_account_receivable_id': obj.company_data['default_account_receivable'].copy().id,
            'property_account_payable_id': obj.company_data['default_account_payable'].copy().id,
            'company_id': False,
        })

        # ==== Cash rounding ====
        obj.cash_rounding_a = obj.env['account.cash.rounding'].create({
            'name': 'add_invoice_line',
            'rounding': 0.05,
            'strategy': 'add_invoice_line',
            'profit_account_id': obj.company_data['default_account_revenue'].copy().id,
            'loss_account_id': obj.company_data['default_account_expense'].copy().id,
            'rounding_method': 'UP',
        })
        obj.cash_rounding_b = obj.env['account.cash.rounding'].create({
            'name': 'biggest_tax',
            'rounding': 0.05,
    ...
    def copy_account(cls, account):
        ''' Helper to copy an account without risking a collision regarding their code.
        :param account: The account.account record to copy.
        :return:        The newly copied account.account record.
        '''
        suffix_nb = 1
        while True:
...
class AccountTestInvoicingCommon(SavepointCase, AccountTestSetup):

    @classmethod
    def setUpClass(cls, chart_template_ref=None):
        super().setUpClass()
        cls.setup(cls, chart_template_ref=chart_template_ref)
...
@tagged('post_install', '-at_install')
class AccountTestInvoicingHttpCommon(HttpCase, AccountTestSetup):

    def setUp(self, chart_template_ref=None):
        super().setUp()
        self.setup(self, chart_template_ref=chart_template_ref)
