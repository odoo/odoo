...
class TestAccountMoveInalterableHash(AccountTestInvoicingCommon):
    ...
    def check_blockchain_integrity(self):
        return self.env['report.account.report_accounting_blockchain_integrity'].with_company(self.company_data['company'].id)._check_blockchain_integrity()['results'][0]
    ...
    def test_account_move_inalterable_hash(self):
        ...
        common_error = "You cannot edit the following fields due to restrict mode being activated: "
        with self.assertRaisesRegex(UserError, f"{common_error}Inalterable hash."):
            move.blockchain_inalterable_hash = 'fake_hash'
        with self.assertRaisesRegex(UserError, f"{common_error}Inalterable no-gap sequence number."):
            move.blockchain_secure_sequence_number = 666
        with self.assertRaisesRegex(UserError, f"{common_error}Number."):
            move.name = "fake name"
        with self.assertRaisesRegex(UserError, f"{common_error}Date."):
            move.date = fields.Date.from_string('2023-01-02')
        with self.assertRaisesRegex(UserError, f"{common_error}Company."):
            move.company_id = 666
        with self.assertRaisesRegex(UserError, f"{common_error}(Company.*Date)|(Date.*Company)."):
            move.write({
                'company_id': 666,
    ...
    def test_account_move_hash_integrity_report(self):
        ...
        self.assertEqual(self.check_blockchain_integrity()['status'], 'not_checked')

        # No records to be hashed even if the restrict mode is activated because the hashing is not retroactive
        self.company_data['default_journal_sale'].restrict_mode_hash_table = True
        self.assertEqual(self.check_blockchain_integrity()['status'], 'no_record')
        ...
        integrity_check = self.check_blockchain_integrity()
        self.assertRegex(integrity_check['msg'], f'Entries are hashed from {moves[2].name}.*')
        self.assertEqual(integrity_check['first_date'], format_date(self.env, fields.Date.to_string(moves[2].date)))
        self.assertEqual(integrity_check['last_date'], format_date(self.env, fields.Date.to_string(moves[-1].date)))
        ...
        self.assertEqual(self.check_blockchain_integrity()['msg'], f'Corrupted data on record {moves[4].name} with id {moves[4].id}.')

        # Revert the previous change
        Model.write(moves[4], {'date': fields.Date.from_string("2023-01-05")})
        self.assertRegex(self.check_blockchain_integrity()['msg'], f'Entries are hashed from {moves[2].name}.*')

        # Let's try with the one of the subfields
        Model.write(moves[-1].line_ids[0], {'partner_id': self.partner_b.id})
        self.assertEqual(self.check_blockchain_integrity()['msg'], f'Corrupted data on record {moves[-1].name} with id {moves[-1].id}.')
        ...
        Model.write(moves[-1], {'blockchain_inalterable_hash': 'fake_hash'})
        self.assertEqual(self.check_blockchain_integrity()['msg'], f'Corrupted data on record {moves[-1].name} with id {moves[-1].id}.')
    ...
    def test_account_move_hash_versioning_1(self):
        ...
        self.company_data['default_journal_sale'].restrict_mode_hash_table = True
        moves = (
            self.init_invoice("out_invoice", self.partner_a, "2023-01-02", amounts=[1000, 2000], company=self.company_data['company'])
            | self.init_invoice("out_invoice", self.partner_b, "2023-01-03", amounts=[1000, 2000], company=self.company_data['company'])
            | self.init_invoice("out_invoice", self.partner_b, "2023-01-04", amounts=[1000, 2000], company=self.company_data['company'])
        )
        moves.with_context(hash_version=1).action_post()
        integrity_check = self.check_blockchain_integrity()
        self.assertRegex(integrity_check['msg'], f'Entries are hashed from {moves[0].name}.*')
        self.assertEqual(integrity_check['first_date'], format_date(self.env, fields.Date.to_string(moves[0].date)))
        self.assertEqual(integrity_check['last_date'], format_date(self.env, fields.Date.to_string(moves[-1].date)))
    ...
    def test_account_move_hash_versioning_v1_to_v2(self):
        ...
        moves_v1.with_context(hash_version=1).flush_recordset()
        fields_v1 = moves_v1.with_context(hash_version=1)._get_blockchain_inalterable_hash_fields()
        moves_v2 = (
            self.init_invoice("out_invoice", self.partner_a, "2023-01-01", amounts=[1000, 2000])
            | self.init_invoice("out_invoice", self.partner_b, "2023-01-02", amounts=[1000, 2000])
            | self.init_invoice("out_invoice", self.partner_b, "2023-01-03", amounts=[1000, 2000])
        ...
        moves_v2.with_context(hash_version=2).flush_recordset()
        fields_v2 = moves_v2._get_blockchain_inalterable_hash_fields()
        ...
        # Revert the previous change
        Model.write(moves[4], {'date': fields.Date.from_string("2023-01-02")})
        self.assertRegex(integrity_check['msg'], f'Entries are hashed from {moves[0].name}.*')
        ...
        moves_v1_bis.with_context(hash_version=1).flush_recordset()
        self.assertEqual(self.check_blockchain_integrity()['msg'], f'Corrupted data on record {moves_v1_bis[0].name} with id {moves_v1_bis[0].id}.')
    ...
    def test_account_move_hash_versioning_3(self):
        ...
        self.assertRegex(self.check_blockchain_integrity()['msg'], f'Entries are hashed from {moves_v3[0].name}.*')
    ...
    def test_account_move_hash_versioning_v2_to_v3(self):
        ...
        moves_v3.with_context(hash_version=3).flush_recordset()
