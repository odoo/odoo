...
from odoo.addons.account.wizard.account_move_send_batch_wizard import AccountMoveSendBatchWizard
...
class TestAccountMoveSend(TestAccountMoveSendCommon):
    ...
    def test_invoice_multi_with_edi(self):
        ...
        with (
            patch.object(AccountMoveSend, '_get_default_extra_edis', get_default_extra_edis),
            patch.object(AccountMoveSend, '_get_all_extra_edis', get_all_extra_edis)
        ):
            wizard = self.create_send_and_print(invoice1 + invoice2)
            self.assertEqual(wizard.summary_data, {
                'manual': {'count': 1, 'label': 'Download'},
