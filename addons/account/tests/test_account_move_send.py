...
class TestAccountComposerPerformance(AccountTestInvoicingCommon, MailCommon):
    ...
    def test_move_composer_single(self):
        ...
        self.assertEqual(composer.sending_method, 'email')
...
class TestAccountMoveSendCommon(AccountTestInvoicingCommon):
    ...
    def _set_extra_edi(self, single_wizard, edi_key, enable=True):
        """ Helper method to enable/disable a specific EDI on a single send wizard."""
        if edi_key in single_wizard.extra_edi_checkboxes:
            checkboxes = single_wizard.extra_edi_checkboxes
            checkboxes[edi_key]['checked'] = enable
            single_wizard.extra_edi_checkboxes = {**checkboxes}  # we are forced to reassign the whole json to trigger dependencies
...
class TestAccountMoveSend(TestAccountMoveSendCommon):
    ...
    def test_invoice_single(self):
        for sending_method in ('email', 'manual'):
            invoice = self.init_invoice("out_invoice", amounts=[1000], post=True)
            wizard = self.create_send_and_print(invoice, sending_method=sending_method, invoice_edi_format=False, extra_edi_checkboxes=False)
            ...
            self.assertEqual(results['type'], 'ir.actions.act_window_close' if sending_method == 'email' else 'ir.actions.act_url')
    ...
    def test_invoice_single_email_missing(self):
        ...
        wizard = self.create_send_and_print(invoice, invoice_edi_format=False, extra_edi_checkboxes=False)
        self.assertEqual(wizard.sending_method, 'manual')  # preferred method is overriden since it makes no sense
        wizard.sending_method = 'email'  # user selects email anyway
        ...
        wizard = self.create_send_and_print(invoice, invoice_edi_format=False, extra_edi_checkboxes=False)
        self.assertFalse(wizard.alerts)
        self.assertEqual(wizard.sending_method, 'email')
        wizard.action_send_and_print()
        self.assertTrue(self._get_mail_message(invoice))
    ...
    def test_invoice_multi(self):
        ...
        self.assertEqual(wizard.summary_data, {
            'manual': {'count': 1, 'label': 'Manually (download)'},
        ...
        self.assertEqual(wizard.summary_data, {
            'manual': {'count': 2, 'label': 'Manually (download)'},
            'email': {'count': 1, 'label': 'by Email'},
        })
        wizard.action_send_and_print()
        self.env.ref('account.ir_cron_account_move_send').method_direct_trigger()
    ...
    def test_invoice_multi_email_missing(self):
        ...
        wizard = self.create_send_and_print(invoice1 + invoice2)
        self.assertTrue('account_missing_email' in wizard.alerts and wizard.alerts['account_missing_email']['level'] == 'warning')
        wizard.action_send_and_print()
        self.env.ref('account.ir_cron_account_move_send').method_direct_trigger()
