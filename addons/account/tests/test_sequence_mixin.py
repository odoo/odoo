...
class TestSequenceMixinCommon(AccountTestInvoicingCommon):
    ...
    def assertMoveName(cls, move, expected_name):
        if move.name_placeholder_form:
            ...
            cls.assertEqual(move.name_placeholder_form, expected_name, f"This move is potentially the first of the sequence, it doesn't have a name but a placeholder name which is currently '{move.name_placeholder_form}'. You expected '{expected_name}'.")
        else:
...
class TestSequenceMixin(TestSequenceMixinCommon):
    ...
    def test_sequence_change_date(self):
        ...
        self.assertEqual(self.test_move.name_placeholder_form, 'MISC/15-16/01/0001')
