...
class TestTaxCommon(AccountTestInvoicingCommon):
    ...
    def _assert_tax_results(self, results, expected_values, rounding):
        """ Compare the results of the taxes computation with the expected ones.
    ...
    def _check_tax_results(self, taxes, expected_values, price_unit, evaluation_context_kwargs=None, compute_kwargs=None):
        ...
        if is_round_globally:
            ...
            compare_values(results, expected_values, rounding)
            self.assertEqual(
                float_round(results['total_excluded'] + results['price_include_added_base'], precision_rounding=rounding),
