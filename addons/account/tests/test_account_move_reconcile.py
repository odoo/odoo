...
class TestAccountMoveReconcile(AccountTestInvoicingCommon):
    ...
    def test_links_between_move_and_payment(self):
        ...
        # No links when reset to raft
        self.assertFalse(payment.invoice_ids)  # No Many2many links when there's an outstanding account
        ...
        # 2 payments linked to the invoice delete the right links when 1 of both payment is reset to draft
        payment2 = self.create_move_payment(invoice_outstanding, 150, True)
        payment3 = self.create_move_payment(invoice_outstanding, 150, True)
        self.assertFalse(payment2.invoice_ids)
        self.assertFalse(payment3.invoice_ids)
        self.assertEqual(payment2.reconciled_invoice_ids, invoice_outstanding)
        self.assertEqual(payment3.reconciled_invoice_ids, invoice_outstanding)
        payment2.action_draft()
        self.assertFalse(payment2.reconciled_invoice_ids)
        self.assertEqual(payment3.reconciled_invoice_ids, invoice_outstanding)
