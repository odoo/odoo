...
class TestAccountMoveReconcile(AccountTestInvoicingCommon):
    ...
    def test_reconcile_partial_reconciliations(self):
        ...
        a line not belonging to a matching_group with lines of a partially reconciled group not fully reconciled.
        """
        ...
        (line_1 + line_3).reconcile()
        self.assertNotIn(True, [bool(line.full_reconcile_id) for line in (line_1 + line_3)], msg="Both reconciled lines must not be fully reconciled")
        self.assertEqual(line_1.matching_number.startswith("P") and line_1.matching_number, line_3.matching_number, msg="Both reconciled lines must be part of the same partially reconciled group")

        with self.assertRaises(UserError, msg="Can't reconcile a partial reconcile group if there's not at least one not fully reconciled line from that group."):
            (line_1 + line_2).reconcile()

        (line_2 + line_3).reconcile()
        self.assertNotIn(False, [bool(line.full_reconcile_id) for line in (line_1 + line_2 + line_3)], msg="All reconciled lines must now be fully reconciled")
        self.assertEqual(not line_2.matching_number.startswith("P") and line_2.matching_number, line_3.matching_number, msg="All reconciled lines must be part of the same fully reconciled group")
