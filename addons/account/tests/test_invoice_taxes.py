...
class TestInvoiceTaxes(AccountTestInvoicingCommon):
    ...
    def _create_invoice_taxes_per_line(self, taxes_per_line, **invoice_args):
        ...
        return self._create_invoice(
            invoice_line_ids=self._create_line_list_vals([
                {
    ...
    def test_setting_tax_separately(self):
        ...
        21%         | /         | 100       | 21
        '''
        invoice = self._create_invoice(amounts=[100])
    ...
    def test_tax_repartition(self):
        ...
        invoice = self._create_invoice(amounts=[100], taxes=tax)
        invoice.action_post()
        ...
        refund = self._create_invoice(amounts=[100], taxes=tax, move_type='out_refund')
        refund.action_post()
    ...
    def test_division_tax(self):
        ...
        invoice = self._create_invoice(amounts=[100], taxes=sale_tax)
        self.assertRecordValues(invoice.line_ids.filtered('tax_line_id'), [{
            'name': sale_tax.name,
            'tax_base_amount': 0.0,
            'balance': -100,
    ...
    def test_fixed_tax_with_zero_price(self):
        ...
        invoice = self._create_invoice(amounts=[0], taxes=fixed_tax)
        self.assertRecordValues(invoice.line_ids.filtered('tax_line_id'), [{
            'credit': 5.0,
            'debit': 0,
        }])
    ...
    def test_tax_repartition_lines_dispatch_amount_1(self):
        ...
        invoice = self._create_invoice(amounts=[1], taxes=tax)
    ...
    def test_tax_repartition_lines_dispatch_amount_2(self):
        ...
        invoice = self._create_invoice(amounts=[1], taxes=tax, move_type='out_refund')
        self.assertRecordValues(invoice, [{
            'amount_untaxed': 1.0,
            'amount_tax': 0.0,
            'amount_total': 1.0,
