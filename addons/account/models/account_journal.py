...
class AccountJournal(models.Model):
    ...
    @api.onchange('payment_sequence')
    def _onchange_payment_sequence(self):
        if self.payment_sequence:
            new_move = self.env['account.move'].new({
                'journal_id': self.id,
                'line_ids': [
                    Command.create({'name': 'line_debit', 'account_id': self.loss_account_id.id, 'debit': 100, 'credit': 0}),
                    Command.create({'name': 'line_credit', 'account_id': self.profit_account_id.id, 'debit': 0, 'credit': 100}),
                ]
            })
            new_move_last_seq = new_move._get_last_sequence(with_prefix=new_move.sequence_prefix)
            if not new_move_last_seq:  # there are no moves yet, except those linked to a payment
                already_existing_move_w_same_name = self.env['account.move'].search([('journal_id', '=', self._origin.id), ('name', '=', new_move.name)])
                if already_existing_move_w_same_name:
                    return {
                        'warning': {
                            'title': _("Warning!"),
                            'message': _("You already have an entry that has the same name as the one that would be generated by this journal when creating/importing first statement line.\n"
                                         "Name of the problematic entry: %s", new_move.name),
                        }
                    }
