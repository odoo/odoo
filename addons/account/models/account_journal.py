...
class AccountJournal(models.Model):
    ...
    show_fetch_einvoices_button = fields.Boolean(
        string="Show E-Invoice Buttons",
        compute='_compute_show_fetch_einvoices_button',
    )
    show_refresh_einvoices_status_button = fields.Boolean(
        string="Show E-Invoice Status Buttons",
        compute='_compute_show_refresh_einvoices_status_button',
    )
    ...
    @api.depends('type')
    def _compute_show_fetch_einvoices_button(self):
        # TO OVERRIDE
        self.show_fetch_einvoices_button = False

    @api.depends('type')
    def _compute_show_refresh_einvoices_status_button(self):
        # TO OVERRIDE
        self.show_refresh_einvoices_status_button = False
    ...
    def _notify_einvoices_received(self, moves):
        ...
        if not (einvoice_count := len(moves)):
            return

        self._send_einvoice_received_mail_if_needed(einvoice_count)

    def _send_einvoice_received_mail_if_needed(self, einvoice_count: int):
        self.ensure_one()
        if not all((
            einvoice_count,
        ...
        if not mail_template:
            return
        mail_template.with_context(einvoice_count=einvoice_count).send_mail(self.id, force_send=True)

    def button_fetch_einvoices(self):
        # TO OVERRIDE
        """
    ...
    def button_refresh_einvoices_status(self):
        # TO OVERRIDE
        """
