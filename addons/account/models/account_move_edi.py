...
class AccountMove(models.Model):
    ...
    def _get_document_type(self):
        """ Differentiate between account.move used for payment and invoice. """
        self.ensure_one()
        if self._name != 'account.move':
            return super()._get_document_type()
    ...
    def _update_payments_edi_documents(self):
        ''' Update the edi flows linked to the current journal entries. These journal entries must be linked to an
        account.payment or an account.bank.statement.line. This additional method is needed because the payment flow is
        not the same as the invoice one: the edi documents must be updated when the reconciliation with some
        invoices is changing.
        '''
        edi_flow_vals_list = []
        for payment in self:
            edi_formats = payment._get_reconciled_invoices().journal_id.edi_format_ids | payment.edi_flow_ids.edi_format_id
            for edi_format in edi_formats:
                existing_edi_flow = payment.edi_flow_ids.filtered(lambda x: x.edi_format_id == edi_format)

                if edi_format._is_format_required(payment, 'payment'):
                    if existing_edi_flow:
                        existing_edi_flow.write({
                            'state': 'to_send',
                            'error': False,
        ...
        self.edi_flow_ids._process_documents_no_web_services()

    def _post(self, soft=True):
        posted = super()._post(soft=soft)
        self._hook_initiate_flows(posted)
        posted.edi_flow_ids._process_documents_no_web_services()
        self.env.ref('edi.ir_cron_edi_network')._trigger()
        return posted

    def button_cancel(self):
        res = super().button_cancel()
        self._hook_cancel_flows()
        self.edi_flow_ids._process_documents_no_web_services()
        self.env.ref('edi.ir_cron_edi_network')._trigger()
        return res
        ...
        for move in self:
            if move.edi_show_cancel_button:
                raise UserError(_(
                    ...
                    "sent. Please use the 'Request EDI Cancellation' button instead.", move.display_name
                ))
        self.edi_flow_ids.write({'error': False, 'blocking_level': False})
        return super().button_draft()

    def button_cancel_posted_moves(self):
        ''' Cancel the existing edi flow, and create a new cancellation flow for them.
        '''
    ...
    def _get_create_invoice_from_attachment_decoders(self):
        ...
        res.append((10, self.env['edi.format'].search([('applicability', 'like', 'accounting')])._create_document_from_attachment))
        return res
    ...
    def _get_update_invoice_from_attachment_decoders(self, invoice):
        ...
        res.append((10, self.env['edi.format'].search([('applicability', 'like', 'accounting')])._update_document_from_attachment))
        return res
    ...
    def _get_edi_format_by_code(self, edi_format_code):
        ...
        """
        self.ensure_one()
        if self._get_document_type() not in ('invoice', 'payment'):
            return super()._get_edi_format_by_code(edi_format_code)
        return self.journal_id.edi_format_ids.filtered(lambda f: f.code == edi_format_code)[:1]

    @api.model
    def _hook_initiate_flows(self, documents):
        invoices_edi_formats = self.env['edi.format'].search([('applicability', 'like', 'accounting')])
        for edi_format in invoices_edi_formats:
            self._create_flow_for_format_code(edi_format.code, documents)
