...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _post_load_data(self, template_code, company, template_data):
        ...
        # We need to browse the product in sudo to check for the tax_ids and supplier_tax_ids fields regardless of the companies record rules
        ...
        if company.account_sale_tax_id:
            sudoed_products_sale = sudoed_products.filtered(
                lambda p: p.tax_ids and not p.tax_ids.filtered_domain(p.tax_ids._check_company_domain(company)))
            sudoed_products_sale._force_default_sale_tax(company)
        if company.account_purchase_tax_id:
            sudoed_products_purchase = sudoed_products.filtered(
                lambda p: p.supplier_tax_ids and not p.supplier_tax_ids.filtered_domain(p.tax_ids._check_company_domain(company)))
