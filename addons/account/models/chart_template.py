...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _pre_reload_data(self, company, template_data, data, force_create=True, force_update=False):
        ...
        for xmlid, journal_data in list(data.get('account.journal', {}).items()):
            if self.ref(xmlid, raise_if_not_found=False):
                ...
                translated_code = self._get_untranslatable_field_translation('account.journal', journal_data, 'code', lang)
            ...
            for record in records.values():
                for field in untranslatable_fields:
                    if field not in record:
                        continue
                    translation = self._get_untranslatable_field_translation(model_name, record, field, untranslatable_target_lang)
                    if translation:
    ...
    def _get_untranslatable_field_translation(self, model_name, record, fname, lang):
        """Return the translation of an untranslatable field, if it fits the field.

        Unlike translatable fields, the fields listed by
        `_get_untranslatable_fields_to_translate` are size constrained: a journal
        code is a `Char(size=7)`. Nothing validates the length of a translation,
        and several exceed it, `BILL` being `RACHUNEK` in Polish for instance.

        Fall back to the source value when the translation does not fit. It is not
        truncated, because a truncated code is both meaningless and liable to
        collide with another journal of the same company.

        :param model_name: the model the field belongs to
        :type model_name: str
        :param record: record formatted like in the template data (generated by _get_chart_template_data)
        :type record: dict
        :param fname: the name of a field (in record) as string
        ...
        :param lang: the code of a res.lang
        :type lang: str
        :return: the translation of record[fname] into lang, or None
        :rtype: str | None
        """
        translation = self._get_field_translation(record, fname, lang)
        size = getattr(self.env[model_name]._fields[fname], 'size', None)  # only `Char` defines one
        if translation and size and len(translation) > size:
            _logger.warning(
                "Ignoring the %s translation %r of %s.%s, it is longer than the %s characters the field holds",
                lang, translation, model_name, fname, size,
            )
            return None
        return translation
