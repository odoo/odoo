...
class AccountTax(models.Model):
    ...
    def _check_repartition_lines(self, lines):
        ...
        total_factor = sum(tax_reps.mapped('factor'))
        if (
            float_compare(total_factor, 0.0, precision_digits=2)
            and float_compare(total_factor, 1.0, precision_digits=2)
        ):
            raise ValidationError(_("Invoice and credit note distribution should have a total factor equals to 0% or 100%."))
    ...
    def _validate_repartition_lines(self):
        for record in self:
            ...
            invoice_tax_repartition_lines = invoice_repartition_line_ids.filtered(lambda x: x.repartition_type == 'tax')
            refund_tax_repartition_lines = refund_repartition_line_ids.filtered(lambda x: x.repartition_type == 'tax')
            if (
                not float_is_zero(record.amount, precision_digits=4)
                and not any(
                    not float_is_zero(line.factor_percent, precision_digits=2)
                    for line in invoice_tax_repartition_lines
                )
            ):
                raise ValidationError(_("Invoice distribution should have at least one non-zero tax repartition line."))

            min_length = min(len(invoice_repartition_line_ids), len(refund_repartition_line_ids))
            for inv_rep_ln, ref_rep_ln in zip(invoice_repartition_line_ids, refund_repartition_line_ids):
            ...
            extra_repartition_lines = invoice_repartition_line_ids[min_length:] or refund_repartition_line_ids[min_length:]
            if any(line.repartition_type != 'tax' for line in extra_repartition_lines):
                raise ValidationError(_("Exceeding invoice or credit note repartition lines must be tax lines."))

            invoice_total_factor = sum(invoice_tax_repartition_lines.mapped('factor'))
            refund_total_factor = sum(refund_tax_repartition_lines.mapped('factor'))
            if float_compare(invoice_total_factor, refund_total_factor, precision_digits=2):
                raise ValidationError(_("Invoice and credit note distribution should have the same total factor."))
