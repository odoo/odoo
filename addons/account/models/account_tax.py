...
class AccountTax(models.Model):
        ...
        for tax in reversed(self):
            if batch:
                same_batch = (
                    ...
                    and taxes_data[tax.id]['price_include'] == taxes_data[batch[0].id]['price_include']
    ...
    def _evaluate_taxes_computation(
        self,
        price_unit,
        quantity,
        currency=None,
        ...
        :param currency:            Optional currency to retrieve the 'precision_rounding' from it.
        ...
        """
        precision_rounding = currency.rounding if currency else precision_rounding or 0.01
        ...
        # We need to take care of the order when group of taxes are involved.
        # Suppose tax/sequence as t1/5 t2/6 t3/7 and a group of taxes t4/1 containing t2 + t3.
        # If self is t4 + t1, we need to evaluate taxes as t2 -> t3 -> t1 according the sequence of t4.
        # If we flatten all taxes, then sort them, we'll get t1 -> t2 -> t3 but we don't want that.
        taxes_data = {}
        ...
        sorted_taxes._prepare_taxes_batches(taxes_data)
    ...
    def _adapt_price_unit_to_another_taxes(self, price_unit, product, original_taxes, new_taxes):
        ...
        taxes_computation = original_taxes._evaluate_taxes_computation(
            price_unit,
            1.0,
        ...
        taxes_computation = new_taxes._evaluate_taxes_computation(
            price_unit,
            1.0,
            rounding_method='round_globally',
            product=product,
        ...
        delta = sum(x['tax_amount_factorized'] for x in taxes_computation['taxes_data'] if x['tax'].price_include)
    ...
    def _apply_taxes_computation_split_repartition_lines(
        ...
        for i, tax_data in enumerate(taxes_data):
            ...
            if tax.include_base_amount:
                for next_tax_data in taxes_data[i + 1:]:
                    next_tax = next_tax_data['tax']
                    subsequent_taxes |= next_tax
                    if include_caba_tags or next_tax.tax_exigibility != 'on_payment':
                        subsequent_tags |= next_tax[repartition_lines_field].filtered(lambda x: x.repartition_type == "base").tag_ids
            tax_data['taxes'] = subsequent_taxes
            tax_data['tags'] = subsequent_tags
    ...
    def _prepare_base_line_tax_details(
        ...
        taxes_data_map = {tax_data['tax'].id: tax_data for tax_data in taxes_computation['taxes_data']}
        for tax_data in taxes_data:
            if 'display_base' not in tax_data:
                tax_data['tax']._add_batch_tax_totals_display_base(taxes_data_map)
        ...
        for tax_data in taxes_data:
            tax_data['group'] = tax_data.get('group') or self.env['account.tax']
    ...
    def _add_batch_tax_totals_display_base(self, taxes_data):
        tax_data = taxes_data[self.id]
        if self.amount_type == 'division' and self.price_include:
            total_tax_amount = sum(taxes_data.get(tax.id, {}).get('tax_amount_factorized', 0.0) for tax in tax_data['batch'])
            tax_data['display_base'] = tax_data['base'] + total_tax_amount
        elif 'display_base' not in tax_data:
