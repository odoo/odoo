...
Best Regards,'''))
    ...
    def action_close_account_invoice_onboarding(self):
        ...
        self.env['res.company']._get_current_company().account_invoice_onboarding_state = 'closed'

    @api.model
    def action_close_account_dashboard_onboarding(self):
        """ Mark the dashboard onboarding panel as closed. """
        self.env['res.company']._get_current_company().account_dashboard_onboarding_state = 'closed'
    ...
    def _get_sample_invoice(self):
        ...
        if len(sample_invoice) == 0:
            ...
            account = self.env['res.company']._get_current_company().get_chart_of_accounts_or_fail()
