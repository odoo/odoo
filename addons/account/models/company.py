...
Best Regards,'''))
    ...
    def _compute_has_chart_of_accounts(self):
        for company in self:
            company.has_chart_of_accounts = bool(company.chart_template_id)
            company.chart_template_id = company.chart_template_id or False
            company.has_accounting_entries = self.env['wizard.multi.charts.accounts'].existing_accounting(company)
    ...
    def mark_company_setup_as_done_action(self):
        ...
        if self.chart_template_id and not self.has_accounting_entries:
