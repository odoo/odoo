...
class AccountAccountTag(models.Model):
    ...
    tax_tag_template_id = fields.Many2one(string="Parent Tax Tag Template", comodel_name='account.tax.tag.template', help="The template that generated this tax tag")
    tax_negate = fields.Boolean(string="Negate Tax Balance", help="Check this boox to ")
...
class AccountTaxTagTemplate(models.Model):
    _name = "account.tax.tag.template"
    _description = 'Account Tax Tag Template'

    name = fields.Char(string="Name", required=True, help="Short name for this tag.")
    complete_name = fields.Char(string="Complete Name", help="Complete name for this tag, to be used in reports.")
    children_tag_ids = fields.One2many(string="Generated Tags", comodel_name='account.account.tag', inverse_name='tax_tag_template_id', help="Tax tags generated by this template")
    country_id = fields.Many2one(string="Country", comodel_name='res.country', required=True, help="Country for which this tag is available.")
    report_action_id = fields.Many2one(string="Report Action", comodel_name='ir.actions.act_window', help="The optional action to call when clicking on this tag in accounting reports.")
    ...
    def create(self, vals):
        ...
        vals['children_tag_ids'] = [(0, 0, minus_tag_vals), (0, 0, plus_tag_vals)]
        return super(AccountTaxTagTemplate, self).create(vals)

    def write(self, vals):
        rslt = super(AccountTaxTagTemplate, self).write(vals)
        if 'name' in vals:
            minus_child_tags = self.mapped('children_tag_ids').filtered(lambda x: x.tax_negate)
            minus_child_tags.write({'name': '- ' + vals['name']})

            plus_child_tags = self.mapped('children_tag_ids').filtered(lambda x: not x.tax_negate)
            plus_child_tags.write({'name': '+ ' + vals['name']})
        return rslt
...
class AccountTax(models.Model):
    ...
    @api.constrains('refund_repartition_line_ids')
    def _validate_refund_repartition(self):
        for record in self:
