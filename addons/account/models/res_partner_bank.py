...
class ResPartnerBank(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            archived_res_partner_bank = self.env['res.partner.bank'].search([('active', '=', False), ('partner_id', '=', vals['partner_id']), ('acc_number', '=', vals['acc_number'])])
            if archived_res_partner_bank:
                raise UserError(_("A bank account with Account Number %(number)s already exists for Partner %(partner)s, but is archived. Please unarchive it instead.", number=vals['acc_number'], partner=vals['acc_holder_name']))
