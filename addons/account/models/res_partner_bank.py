    ...
    def write(self, vals):
        ...
        for account, initial_values in account_initial_values.items():
            ...
            if tracking_values:
                ...
                account.partner_id._message_log(body=msg, tracking_value_ids=[(0, 0, values) for values in tracking_values])
                if 'partner_id' in initial_values:  # notify previous partner as well
                    initial_values['partner_id']._message_log(body=msg, tracking_value_ids=[(0, 0, values) for values in tracking_values])
        return res
