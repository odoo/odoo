...
class account_journal(models.Model):
    ...
    def get_journal_dashboard_datas(self):
        ...
        #TODO need to check if all invoices are in the same currency than the journal!!!!
            ...
            today = datetime.today()
            query = """SELECT ABS(residual_signed) AS amount_total, currency_id AS currency, type FROM account_invoice WHERE journal_id = %s AND date < %s AND state = 'open';"""
    ...
    def _get_open_bills_to_pay_query(self):
        ...
        it as its second.
        """
        return ("""SELECT state, ABS(residual_signed) AS amount_total, currency_id AS currency, type
                  FROM account_invoice
                  WHERE journal_id = %(journal_id)s AND state = 'open';""", {'journal_id':self.id})
