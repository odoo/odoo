        ...
        for record in self:
            if record.foreign_vat:
                ...
                if record.country_group_id:
                    foreign_vat_country_id = next(country_id for country_id in [record.country_id] + [country for country in self.country_group_id.country_ids] if country_id.code.upper() == self.foreign_vat[:2])
