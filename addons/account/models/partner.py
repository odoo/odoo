...
class ResPartner(models.Model):
    ...
    def _invoice_total(self):
        ...
        user_currency_id = self.env['res.company']._get_current_company().currency_id.id
    ...
    def get_followup_lines_domain(self, date, overdue_only=False, only_unblocked=False):
        domain = [('reconciled', '=', False), ('account_id.deprecated', '=', False), ('account_id.internal_type', '=', 'receivable'), '|', ('debit', '!=', 0), ('credit', '!=', 0), ('company_id', '=', self.env['res.company']._get_current_company().id)]
        if only_unblocked:
    ...
    def mark_as_reconciled(self):
        ...
        return self.sudo().with_context(company_id=self.env['res.company']._get_current_company().id).write({'last_time_entries_checked': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
    ...
    def _get_company_currency(self):
        if self.company_id:
            ...
            self.currency_id = self.env['res.company']._get_current_company().currency_id
