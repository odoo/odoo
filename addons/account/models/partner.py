...
class AccountFiscalPosition(models.Model):
    ...
    def _onchange_foreign_vat(self):
        self.foreign_vat, country_code = self.env['res.partner']._run_vat_checks(self.country_id, self.foreign_vat, validation=False)
    ...
    def _inverse_foreign_vat(self):
        for record in self:
            ...
            if record.country_id:
                ...
                vat, country_code = self.env['res.partner']._run_vat_checks(record.country_id, record.foreign_vat, partner_name=fp_label)
                record.foreign_vat = vat
    ...
    def _get_fpos_ranking_functions(self, partner):
        ...
        return [
            ('vat_required', lambda fpos: (
                ...
                or (partner._get_vat_required_valid(company=self.env.company, fp_exists=fp_exists) and 2)
            )),
