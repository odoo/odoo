...
class AccountBankStatementLine(models.Model):
    ...
    def base_read_group(self, domain, groupby=(), aggregates=(), limit=None, offset=0, order=''):
        # Add latest running_balance in the base_read_group
        result = super().base_read_group(
            domain, groupby, aggregates, offset=offset,
        ...
        if show_running_balance:
            for group_line in result:
                group_line['running_balance'] = self.search(group_line['__domain'] + domain, limit=1).running_balance or 0.0
        return result
