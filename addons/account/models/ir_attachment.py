...
class IrAttachment(models.Model):
    ...
    def _identify_and_unwrap_file(self, file_data):
        ...
        * attachment:   (optional) The associated ir.attachment, if any.
        * xml_tree:     (optional) The tree of the xml if the file is an xml.
        * root_file:    (optional) The dict representing the file this was extracted from, if this is a sub-file.
        ...
        * priority:     (optional) The priority with which the file should be decoded.
        ...
        if 'attachment' in file_data and 'pdf' in file_data['attachment'].mimetype or file_data['filename'].endswith('.pdf'):
            file_data = {**file_data, 'type': 'pdf'}
            embedded_files_data = []
            for embedded_filename, embedded_content in self._unwrap_pdf(file_data['content'], file_data['filename']):
                embedded_file_data = {
                    'filename': embedded_filename,
                    'content': embedded_content,
                    'root_file': file_data.get('root_file', file_data),
                }
                with contextlib.suppress(etree.ParseError):
                    embedded_file_data['xml_tree'] = etree.fromstring(embedded_content)

                # Recursively unwrap embedded files.
                embedded_files_data += self._identify_and_unwrap_file(embedded_file_data)

            return [file_data, *embedded_files_data]

        # If the file didn't match any known format, we just pass it on without setting the type.
        return [file_data]
    ...
    def _identify_and_unwrap_edi_attachments(self):
        ...
        * attachment:   (optional) The associated ir.attachment, if there is one.
        * xml_tree:     (optional) The tree of the xml if the file is an xml.
        * type:         (optional) The type of document, if it was identified.
        * root_file:    (optional) The dict representing the file this was extracted from, if this is an embedded file.
        * priority:     (optional) The priority with which the file should be applied on a record.
        """
        files_data = []
        for attachment in self:
            file_data = {
