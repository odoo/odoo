...
class AccountPayment(models.Model):
    ...
    def _get_outstanding_account(self, payment_type):
        ...
        chart_template = self.with_context(company_id=self.company_id.root_id).env['account.chart.template']
        outstanding_account = (
            chart_template.ref(account_ref, raise_if_not_found=False)
            or self.company_id.transfer_account_id
        )
        if not outstanding_account:
            raise UserError("No outstanding account could be found to make the payment")
