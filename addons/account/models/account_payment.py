...
class AccountPayment(models.Model):
    ...
    def write(self, vals):
        ...
        vals_list = []
        for pay in self:
            vals_list.append(OrmUtils(pay).cleanup_write_values(vals))
            snapshots0.append(pay._create_snapshot(fields=[
        ...
        if vals_list and all(vals == vals_list[0] for vals in vals_list):
            ...
            res = super(AccountPayment, self_ctx).write(vals_list[0])
        else:
            res = True
            for pay, vals in zip(self, vals_list):
                st_line_ctx = self.with_context(skip_account_move_synchronization=True)
                res |= super(AccountPayment, st_line_ctx).write(vals)
        ...
        if not self._context.get('write_recursion'):
            for pay, snapshot0 in zip(self.with_context(write_recursion=True), snapshots0):
                pay._synchronize_to_moves(snapshot0)
    ...
    def _synchronize_from_moves(self):
        ...
        for pay in self.with_context(skip_account_move_synchronization=True):
            ...
            pay.write(OrmUtils(pay).cleanup_write_values(payment_vals_to_write))
    ...
    def _synchronize_to_moves(self, snapshot0):
        ...
        line_vals_list = self._prepare_move_line_default_vals(write_off_line_vals=write_off_line_vals)
