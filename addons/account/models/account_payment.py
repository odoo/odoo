...
class account_payment(models.Model):
    ...
    journal_id = fields.Many2one(
        ...
        domain="[('id', 'in', _suitable_journal_ids)]"
    )
    ...
    def _onchange_payment_type(self):
        ...
        self._compute_journal_types()

    def _compute_journal_types(self):
        journal_type = ['bank', 'cash']
        if self.currency_id.is_zero(self.amount) and self.has_invoices:
            # In case of payment with 0 amount, allow to select a journal of
            # type 'general' like 'Miscellaneous Operations' and set this
            # journal by default.
    ...
    def _onchange_amount(self):
        journal_types = self._compute_journal_types()
