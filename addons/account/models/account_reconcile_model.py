...
class AccountReconcileModelPartnerMapping(models.Model):
    ...
    model_id = fields.Many2one(comodel_name='account.reconcile.model',
...
class AccountReconcileModelLine(models.Model):
    ...
    company_id = fields.Many2one(
        related='model_id.company_id', store=True, default=lambda self: self.env.company)
    ...
    amount = fields.Float(string="Float Amount", compute='_compute_float_amount',
                          store=True, help="Technical shortcut to parse the amount to a float")
    ...
    * From Label: There is no need for regex delimiter, only the regex is needed. For instance if you want to extract the amount from\nR:9672938 10/07 AX 9415126318 T:5L:NA BRT: 3358,07 C:\nYou could enter\nBRT: ([\d,]+)""")
    ...
    analytic_account_id = fields.Many2one(
        'account.analytic.account', string='Analytic Account', ondelete='set null', check_company=True)
    ...
    def _compute_show_force_tax_included(self):
        for record in self:
            record.show_force_tax_included = False if len(
                record.tax_ids) != 1 else True
...
class AccountReconcileModel(models.Model):
    ...
    line_ids = fields.One2many(
        'account.reconcile.model.line', 'model_id', copy=True)
    ...
    past_months_limit = fields.Integer(string="Past Months Limit", default=18,
                                       help="Number of months in the past to consider entries from when applying this model.")

    decimal_separator = fields.Char(default=lambda self: self.env['res.lang']._lang_get(
        self.env.user.lang).decimal_point, help="Every character that is nor a digit nor this separator will be removed from the matching string")
    ...
    def _get_taxes_move_lines_dict(self, tax, base_line_dict):
        ...
        if (tax_type == 'sale' and not is_refund) or (tax_type == 'purchase' and is_refund):
            base_tags = self.env['account.account.tag'].browse(
                res['base_tags'])
            res['base_tags'] = self.env['account.move.line']._revert_signed_tags(
                base_tags).ids

            for tax_result in res['taxes']:
                tax_tags = self.env['account.account.tag'].browse(
                    tax_result['tag_ids'])
                tax_result['tag_ids'] = self.env['account.move.line']._revert_signed_tags(
                    tax_tags).ids
    ...
    def _get_write_off_move_lines_dict(self, st_line, residual_balance):
        ...
        for line in self.line_ids:
                ...
                if match:
                    ...
                    try:
                        extracted_match_group = re.sub(
                            r'[^\d' + self.decimal_separator + ']', '', match.group(1))
                        extracted_balance = float(
                            extracted_match_group.replace(self.decimal_separator, '.'))
                        balance = copysign(
                            extracted_balance * sign, residual_balance)
                    except ValueError:
                        balance = 0
                else:
                    balance = 0
            ...
            if line.tax_ids:
                ...
                tax_vals_list = self._get_taxes_move_lines_dict(
                    tax, writeoff_line)
    ...
    def _prepare_reconciliation(self, st_line, aml_ids=[], partner=None):
        ...
        if st_line.to_check:
            ...
            st_line_residual_currency = st_line._prepare_move_line_default_vals()[
                1]['amount_currency']
        elif suspense_lines.account_id.reconcile:
            st_line_residual = sum(suspense_lines.mapped('amount_residual'))
            st_line_residual_currency = sum(
                suspense_lines.mapped('amount_residual_currency'))
        else:
            st_line_residual = sum(suspense_lines.mapped('balance'))
            st_line_residual_currency = sum(
                suspense_lines.mapped('amount_currency'))

        has_full_write_off = any(
            rec_mod_line.amount == 100.0 for rec_mod_line in self.line_ids)
        ...
        for aml in amls:
            ...
            if aml.balance * st_line_residual > 0:
                ...
                assigned_balance = -aml.amount_residual
                assigned_amount_currency = -aml.amount_residual_currency
                st_line_residual -= min(assigned_balance,
                                        st_line_residual, key=abs)
                st_line_residual_currency -= min(
                    assigned_amount_currency, st_line_residual_currency, key=abs)
            else:
                assigned_balance = min(-aml.amount_residual,
                                       st_line_residual, key=abs)
                assigned_amount_currency = min(
                    -aml.amount_residual_currency, st_line_residual_currency, key=abs)
        ...
        write_off_amount = max(aml_total_residual, -st_line_residual_before,
                               key=abs) + st_line_residual_before + st_line_residual

        reconciliation_overview, open_balance_vals = st_line._prepare_reconciliation(
            lines_vals_list)

        writeoff_vals_list = self._get_write_off_move_lines_dict(
            st_line, write_off_amount)

        for line_vals in writeoff_vals_list:
            st_line_residual -= st_line.company_currency_id.round(
                line_vals['balance'])
    ...
    def _apply_rules(self, st_lines, excluded_ids=None, partner_map=None):
        ...
        available_models = self.filtered(
            lambda m: m.rule_type != 'writeoff_button').sorted()
        ...
        for st_line in st_lines:
            ...
            mapped_partner = (partner_map and partner_map.get(
                st_line.id) and self.env['res.partner'].browse(partner_map[st_line.id])) or st_line.partner_id

            for rec_model in available_models:
                partner = mapped_partner or rec_model._get_partner_from_mapping(
                    st_line)

                if rec_model._is_applicable_for(st_line, partner):
                    lines_with_partner_per_model[rec_model].append(
                        (st_line, partner))
        ...
        for rec_model in available_models:
            ...
            filtered_st_lines_with_partner = [
                x for x in lines_with_partner_per_model[rec_model] if x[0] not in matched_lines]

            if not filtered_st_lines_with_partner:
                # No unreconciled statement line for this model
                continue

            all_model_candidates = rec_model._get_candidates(
                filtered_st_lines_with_partner, excluded_ids)

            for st_line, partner in filtered_st_lines_with_partner:
                candidates = all_model_candidates[st_line.id]
                if candidates:
                    model_rslt, new_reconciled_aml_ids, new_treated_aml_ids = rec_model._get_rule_result(
                        st_line, candidates, aml_ids_to_exclude, reconciled_amls_ids, partner)
    ...
    def _get_candidates(self, st_lines_with_partner, excluded_ids):
        ...
        slice_size_param = self.env['ir.config_parameter'].sudo(
        ).get_param('account.reconcile_model_forced_slice_size')
        if slice_size_param:
        ...
        while slice_start < len(st_lines_with_partner):
            ...
            treatment_slices.append(
                st_lines_with_partner[slice_start:slice_end])
    ...
    def _get_partner_from_mapping(self, st_line):
        ...
        for partner_mapping in self.partner_mapping_line_ids:
            match_payment_ref = re.match(
                partner_mapping.payment_ref_regex, st_line.payment_ref) if partner_mapping.payment_ref_regex else True
            match_narration = re.match(
                partner_mapping.narration_regex, st_line.narration or '') if partner_mapping.narration_regex else True
    ...
    def _get_writeoff_suggestion_query(self, st_lines_with_partner, excluded_ids=None):
        ...
        if self.rule_type != 'writeoff_suggestion':
            raise UserError(
                _("Programmation Error: Can't call _get_writeoff_suggestion_query() for different rules than 'writeoff_suggestion'"))
    ...
    def _get_invoice_matching_rule_result(self, st_line, candidates, aml_ids_to_exclude, reconciled_amls_ids, partner):
        ...
        candidates, priorities = self._filter_candidates(
            candidates, aml_ids_to_exclude, reconciled_amls_ids)
        ...
        candidate_currencies = set(
            candidate['aml_currency_id'] or st_line.company_id.currency_id.id for candidate in candidates)
        if candidate_currencies == {st_line_currency.id}:
            for candidate in candidates:
                residual_amount = candidate['aml_currency_id'] and candidate[
                    'aml_amount_residual_currency'] or candidate['aml_amount_residual']
                if st_line_currency.is_zero(residual_amount + st_line.amount_residual):
                    candidates, priorities = self._filter_candidates(
                        [candidate], aml_ids_to_exclude, reconciled_amls_ids)
                    break
        ...
        disable_bypass = self.env['ir.config_parameter'].sudo(
        ).get_param('account.disable_rec_models_bypass')
        if (not disable_bypass and priorities & {1, 2}) or self._check_rule_propositions(st_line, candidates):
            ...
            lines_vals_list = self._prepare_reconciliation(
                st_line, aml_ids=rslt['aml_ids'], partner=partner)

            # A write-off must be applied if there are some 'new' lines to propose.
            write_off_lines_vals = list(
                filter(lambda x: 'id' not in x, lines_vals_list))
    ...
    def _check_rule_propositions(self, statement_line, candidates):
        ...
        for reconciliation_vals in reconciliation_overview:
            ...
            if line_vals['currency_id']:
                ...
                line_residual_after_reconciliation -= line_vals['debit'] - \
                    line_vals['credit']
        ...
        reconciled_percentage = 100 - abs(line_residual_after_reconciliation) / abs(
            residual_difference) * 100 if (residual_difference != 0) else 0
    ...
    def _filter_candidates(self, candidates, aml_ids_to_exclude, reconciled_amls_ids):
        ...
        """
        candidates_by_priority = self._sort_reconciliation_candidates_by_priority(
            candidates, aml_ids_to_exclude, reconciled_amls_ids)
