    ...
    def action_switch_invoice_into_refund_credit_note(self):
        ...
        for move in self:
            ...
            for cmd, virtualid, line_vals in reversed_move['line_ids']:
                if (line_vals.get('line_type') or '').startswith('invl'):
...
class AccountMoveLine(models.Model):
    ...
    line_type = fields.Selection(
        selection=[
            ...
            ('bank_statement_liquidity_line', 'Liquidity line'),
            ('pay_counterpart_line', 'Counterpart line'),
            ('bank_statement_suspense_line', 'Suspense line'),
        ],
    ...
    _sql_constraints = [
        ...
        (
            'check_accountable_required_fields',
             "CHECK(COALESCE(line_type, 'false') IN ('invl_section', 'invl_note') OR account_id IS NOT NULL)",
             "Missing required account on accountable invoice line."
        ),
        (
            'check_non_accountable_fields_null',
             "CHECK(COALESCE(line_type, 'false') NOT IN ('invl_section', 'invl_note') OR (amount_currency = 0 AND debit = 0 AND credit = 0 AND account_id IS NULL))",
    ...
    def is_invoice_line(self, with_display_line_check=False):
        ...
        self.ensure_one()
        is_invoice_line = (self.line_type or '').startswith('invl')
    ...
    def is_display_line(self):
        ...
        self.ensure_one()
        return (self.line_type or '') in ('invl_section', 'invl_note')
