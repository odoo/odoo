...
from odoo.tools import float_compare, float_is_zero, float_round, date_utils, email_split, email_re
...
class AccountMove(models.Model):
    ...
    def _onchange_currency(self):
        if self.is_invoice(include_receipts=True):
            ...
            lines = self._get_lines_onchange_currency()
            lines.currency_id = currency
            lines._onchange_currency()
        else:
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False):
        ...
        :param recompute_tax_base_amount: A flag indicating if only the 'tax_base_amount' should be recomputed.
        '''
        self.ensure_one()
        in_draft_mode = self != self._origin
        base_lines = self.line_ids.filtered(lambda line: not line.display_type and not line.tax_repartition_line_id)
        tax_lines = self.line_ids.filtered(lambda line: not line.display_type and line.tax_repartition_line_id)
        line_ids_commands = []
        ...
        if not recompute_tax_base_amount:
            for line in base_lines:
                ...
                line_ids_commands.append(Command.update(line.id, {
                    ...
                    'tax_tag_ids': [Command.set(tax_reps.tag_ids.ids)],
                }))

        # Update the tax lines.

        for diff_vals in (base_lines + tax_lines)._prepare_diff_tax_lines_from_tax_details():
            orm_command = diff_vals['command']

            if recompute_tax_base_amount and orm_command[0] != Command.UPDATE:
                continue

            if orm_command[0] in (Command.CREATE, Command.UPDATE):
                balance = amount_currency = tax_base_amount = 0.0
                for tax_detail_vals in diff_vals['source_dicts']:
                    tax_detail = tax_detail_vals['source_record']
                    balance += tax_detail.tax_amount
                    amount_currency += tax_detail.tax_amount_currency
                ...
                has_zero_amount = diff_vals['source_dicts'][0]['source_record'].currency_id.is_zero(amount_currency)
                if not recompute_tax_base_amount and has_zero_amount:
                    # Don't create/update tax line having a zero amount.
                    if orm_command[0] == Command.UPDATE:
                        orm_command = Command.delete(orm_command[1])
                    else:
                        continue
                else:
                    update_vals = {
                    ...
                    if orm_command[0] == Command.CREATE:
                        orm_command = Command.create({
                            'move_id': self.id,
                            'name': orm_command[2]['name'],
                            'tax_repartition_line_id': orm_command[2]['tax_repartition_line_id'],
                            'account_id': orm_command[2]['account_id'],
                            'partner_id': orm_command[2]['partner_id'],
                            'currency_id': orm_command[2]['currency_id'],
                            'tax_exigible': orm_command[2]['tax_exigible'],
                            'analytic_tag_ids': orm_command[2]['analytic_tag_ids'],
                            'analytic_account_id': orm_command[2]['analytic_account_id'],
                            'tax_ids': orm_command[2]['tax_ids'],
                            'tax_tag_ids': orm_command[2]['tag_ids'],
                            **update_vals,
                        })
                    elif recompute_tax_base_amount:
                        orm_command = Command.update(orm_command[1], {'tax_base_amount': tax_base_amount})
                    else:
                        orm_command = Command.update(orm_command[1], update_vals)

            line_ids_commands.append(orm_command)

        to_write = {'line_ids': line_ids_commands}
        (self.update if in_draft_mode else self.write)(to_write)
    ...
    def _reverse_move_vals(self, default_values, cancel=True):
        ...
        for line_command in move_vals.get('line_ids', []):
            ...
            if line_vals.get('tax_repartition_line_id'):
                ...
                account_id = refund_repartition_line._get_business_account().id
                if not account_id:
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ...
        for move, reverse_move in zip(self, reverse_moves.with_context(check_move_validity=False)):
            ...
            if cancel and move.move_type == 'entry':
                continue
...
class AccountMoveLine(models.Model):
    ...
    tax_detail_ids = fields.One2many(
        string="Tax Details", store=True,
        comodel_name='account.tax.detail',
    ...
    def _get_price_unit(self):
        # OVERRIDE
        self.ensure_one()
        return self.price_unit if self.move_id.is_invoice(include_receipts=True) else self.amount_currency

    def _get_quantity(self):
        # OVERRIDE
        self.ensure_one()
        return self.quantity if self.move_id.is_invoice(include_receipts=True) else 1.0
    ...
    def _get_fiscal_position(self):
        # OVERRIDE
        self.ensure_one()
        return self.move_id.fiscal_position_id
    ...
    def _is_refund_document(self):
        ...
        if self.move_id.is_invoice(include_receipts=True):
            return self.move_id.move_type in ('out_refund', 'in_refund')
        else:
            tax_type = self.tax_ids[0].type_tax_use if self.tax_ids else None
            return (tax_type == 'sale' and self.balance > 0.0) or (tax_type == 'purchase' and self.balance < 0.0)
    ...
    def _onchange_product_id(self):
        for line in self:
            ...
            is_invoice_line = not line.exclude_from_invoice_tab and line.display_type not in ('line_section', 'line_note')

            if not is_invoice or not is_invoice_line:
                continue
    ...
    def _onchange_uom_id(self):
        ...
        self.tax_ids = self._get_default_taxes()
    ...
    def _onchange_account_id(self):
        ...
        for line in self:
            ...
            if is_invoice and line.account_id.internal_type in ('receivable', 'payable'):
                continue
    ...
    def _onchange_price_subtotal(self):
        for line in self:
            if line.move_id.is_invoice(include_receipts=True) \
                ...
                line._update_accounting_fields_from_price_subtotal()
    ...
    def _onchange_currency(self):
        for line in self:
            ...
            # Round with the new currency. This is a limitation of the current ORM:
            # Monetary values are not rounded automatically in cache in case of a change of the currency.
            line.amount_currency = line.currency_id.round(line.amount_currency)
    ...
    @api.depends('move_id.move_type', 'exclude_from_invoice_tab',
                 'price_unit', 'quantity', 'discount', 'tax_ids', 'analytic_account_id', 'analytic_tag_ids',
                 'currency_id', 'partner_id', 'amount_currency')
    def _compute_tax_details(self):

        def _get_precision_digits(line):
        ...
        base_lines = self.filtered(lambda line: not line.display_type and not line.tax_repartition_line_id)
        other_lines = self - base_lines

        # Display lines.
        for line in other_lines:
            line.price_subtotal = 0.0
            line.price_total = 0.0
            line.tax_detail_ids = [Command.clear()]

        # Base lines (must be computed before the tax lines).
        for line in base_lines:
            price_subtotal = price_total = 0.0
            tax_detail_ids_commands = [Command.clear()]
            is_invoice_line = line.move_id.is_invoice(include_receipts=True) and not line.exclude_from_invoice_tab

            if line.tax_ids or is_invoice_line:
                # Base line.
                ...
                pd1, pd2 = _get_precision_digits(line)

                if line.tax_ids:
                    for tax_detail_vals in line_detail['tax_details']:
                        tax_detail_ids_commands.append(Command.create({
                            'account_id': tax_detail_vals['account_id'],
                            'tax_amount': float_round(sign * tax_detail_vals['tax_amount'] / (rate or 1.0), precision_digits=pd1),
                            'tax_amount_currency': float_round(sign * tax_detail_vals['tax_amount'], precision_digits=pd2),
                            'tax_base_amount': sign * tax_detail_vals['tax_base_amount'] / (rate or 1.0),
                            'tax_base_amount_currency': sign * tax_detail_vals['tax_base_amount'],
                            'tax_ids': tax_detail_vals['tax_ids'],
                            'tag_ids': tax_detail_vals['tag_ids'],
                            'tax_repartition_line_id': tax_detail_vals['tax_repartition_line_id'],
                            'tax_id': tax_detail_vals['tax_id'],
                        }))
                if is_invoice_line:
                    price_subtotal = line_detail['total_excluded']
                    price_total = line_detail['total_included']

            line.tax_detail_ids = tax_detail_ids_commands
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            # Fill the missing 'currency_id' and ensure 'balance' == 'amount_currency' in that case.
            if 'currency_id' not in vals:
                if move.is_invoice(include_receipts=True) and not vals.get('exclude_from_invoice_tab', False):
        ...
        # Synchronize accounting fields <-> business fields for invoices
