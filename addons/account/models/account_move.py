...
class AccountMove(models.Model):
    ...
    def _is_eligible_for_early_payment_discount_batched(self, currency, reference_date):
        ...
        move_types_for_epd = self._early_payment_discount_move_types()
        ...
        # A move is not eligible if any of its payment term lines are reconciled
        unreconciled_pt_lines = self.env['account.move.line'].search([
            ('move_id', 'in', eligible_moves.ids),
            ('display_type', '=', 'payment_term'),
        ]).filtered(lambda line: line.amount_residual != 0)

        pt_lines_by_move = unreconciled_pt_lines.grouped('move_id')
        for move in eligible_moves:
            move_payment_terms = pt_lines_by_move.get(move)
            if not move_payment_terms:
                continue

            first_payment_term = fields.first(move_payment_terms)
            if (
                not reference_date
                or not move.invoice_date
                or (
                    (existing_discount_date := first_payment_term.discount_date)
                    and existing_discount_date >= reference_date
                )
            ):
