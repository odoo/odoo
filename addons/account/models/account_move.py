...
class AccountMove(models.Model):
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        followers = self._partner_find_from_emails_single(cc_mail_addresses, filter_found=is_right_company, force_create=False)

        # Search for partner that sent the mail.
        from_mail_addresses = email_split(msg_dict.get('from', ''))
        senders = partners = self._partner_find_from_emails_single(from_mail_addresses, filter_found=is_right_company, force_create=False)

        # Search for partners using the user.
        if not senders:
            senders = partners = self._partner_find_from_emails_single(
                from_mail_addresses, filter_found=is_internal_partner, force_create=False
            )
        ...
        if partners:
            ...
            if is_internal_partner(partners[0]):
                ...
                partners = self._partner_find_from_emails_single(
                    body_mail_addresses, filter_found=is_right_company, force_create=False
