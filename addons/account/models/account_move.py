...
class AccountMove(models.Model):
        ...
        for move in moves:
            try:
                ...
                if not move:
                    continue
                move._post()
                remaining_time = self.env['ir.cron']._commit_progress(1)
    ...
    def _cron_account_move_send(self, job_count=10):
        ...
        if not to_process:
            return
        self.env['account.move.send']._generate_and_send_invoices(to_process, allow_raising=False, notification=get_account_notification)
