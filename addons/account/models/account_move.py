...
class AccountMove(models.Model):
    ...
    def _compute_payments_widget_reconciled_info(self):
        for move in self:
            payments_widget_vals = {'title': self.env._('Less Payment'), 'outstanding': False, 'content': []}
    ...
    def _compute_narration(self):
        ...
        for move in invoice_to_update_terms:
            ...
            if move.company_id.terms_type != 'html':
                ...
                context = {'lang': lang}
                narration = self.with_context(context).env._('Terms & Conditions: %s', baseurl)
                del context
    ...
    def _get_copy_message_content(self, default):
        ...
        """
        return self.env._('This entry has been reversed from %s', self._get_html_link()) if default.get('reversed_entry_id') else self.env._('This entry has been duplicated from %s', self._get_html_link())
    ...
    def write(self, vals):
        ...
        for move in self:
            ...
            if vals.get('state') == 'draft' and (
                ...
                raise ValidationError(self.env._("This entry has already been reviewed. You need the bookkeeper role to change it."))
            ...
            if (
                ...
                raise UserError(self.env._('You cannot edit the journal of an account move if it has been posted once, unless the name is removed or set to "/". This might create a gap in the sequence.'))
            if (
                    move.name and move.name != '/'
                ...
                raise UserError(self.env._('You cannot edit the journal of an account move with a sequence number assigned, unless the name is removed or set to "/". This might create a gap in the sequence.'))
    ...
    def _get_move_display_name(self, show_ref=False):
        ...
        if self.env.context.get('name_as_amount_total'):
            ...
            if self.state == 'posted':
                return self.env._("%(ref)s (%(currency_amount)s)", ref=(self.ref or self.name), currency_amount=currency_amount)
            else:
    ...
    def action_switch_move_type(self):
        if any(move.posted_before for move in self):
            raise ValidationError(self.env._("Once a document has been posted once, its type is set in stone and you can't change it anymore."))
    ...
    def button_draft(self):
        ...
        if any(move.need_cancel_request for move in self):
            raise UserError(self.env._("You can't reset to draft those journal entries. You need to request a cancellation instead."))
    ...
    def _check_draftable(self):
        ...
        for move in self:
            ...
            if move.tax_cash_basis_rec_id or move.tax_cash_basis_origin_move_id:
                ...
                raise UserError(self.env._('You cannot reset to draft a tax cash basis journal entry.'))
    ...
    def get_import_templates(self):
        ...
        match move_type:
            case 'entry':
                return [{
                    'label': self.env._('Import Template for Misc. Operations'),
                    'template': '/account/static/xls/misc_operations_import_template.xlsx',
                }]
            case 'out_invoice':
                return [{
                    'label': self.env._('Import Template for Invoices'),
                    'template': '/account/static/xls/customer_invoices_credit_notes_import_template.xlsx',
                }]
            case 'out_refund':
                return [{
                    'label': self.env._('Import Template for Credit Notes'),
                    'template': '/account/static/xls/customer_invoices_credit_notes_import_template.xlsx',
                }]
            case 'in_invoice':
                return [{
                    'label': self.env._('Import Template for Bills'),
                    'template': '/account/static/xls/vendor_bills_refunds_import_template.xlsx',
                }]
            case 'in_refund':
                return [{
                    'label': self.env._('Import Template for Refunds'),
                    'template': '/account/static/xls/vendor_bills_refunds_import_template.xlsx',
                }]
            case _:
                return []
