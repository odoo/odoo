...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        if not self.env.user.has_group('account.group_purchase_deductible_amount') and \
                self.filtered(lambda move: move.move_type == 'in_invoice' and move.invoice_line_ids.filtered(lambda l: l.deductible_amount != 100)):
            self.env.user.sudo().group_ids = [Command.link(self.env.ref('account.group_purchase_deductible_amount').id)]
