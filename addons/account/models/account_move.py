...
class AccountMove(models.Model):
    _name = "account.move"
    _inherit = ['portal.mixin', 'mail.thread.main.attachment', 'mail.activity.mixin', 'sequence.mixin', 'blockchain.mixin']
    ...
    def _get_blockchain_inalterable_hash_fields(self):
        ...
        hash_version = self._context.get('hash_version', self._get_blockchain_max_version())
        if hash_version == 1:
    ...
    def _get_blockchain_secure_sequence(self):
        self.ensure_one()
        self.env['blockchain.mixin']._create_blockchain_secure_sequence(self.journal_id, 'secure_sequence_id', self.company_id.id)
    ...
    def _get_blockchain_record_dict_to_hash(self, lines=()):
        return super()._get_blockchain_record_dict_to_hash(self.line_ids)
    ...
    def _unlink_or_reverse(self):
        ...
        for move in self:
            if move.blockchain_inalterable_hash or move.date <= lock_date:
                to_reverse += move
            else:
                to_cancel += move
    ...
    def _post(self, soft=True):
        ...
        to_post._compute_payment_reference()
        to_post._compute_blockchain_must_hash()
        return to_post
    ...
    def button_draft(self):
        ...
        for move in self:
            ...
            if move.blockchain_inalterable_hash:
