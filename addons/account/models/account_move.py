...
class AccountMove(models.Model):
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if not relaxed:
            ...
            # Format      | Fixed | Yearly | Monthly | Year Range | Year range Monthly |
            # ----------- | ----- | ------ | ------- | ---------- | ------------------ |
            # Fixed       |   X   |        |         |            |                    |
            # Yearly      |   X   |   X    |         |            |                    |
            # Monthly     |   X   |   X    |    X    |     X      |                    |
            # Year Range  |   X   |   X    |         |     X      |                    |
            # Year range  |   X   |   X    |    X    |     X      |          X         |
            # Monthly
    ...
    def _get_starting_sequence(self):
        ...
        if last_month != 12 or last_day != 31:
            if self.date > date(self.date.year, last_month, last_day):
                year_part = "%04d-%04d" % (self.date.year, (self.date + relativedelta(years=1)).year)
            else:
                year_part = "%04d-%04d" % ((self.date + relativedelta(years=-1)).year, self.date.year)
        ...
        if self.journal_id.type in ['sale', 'bank', 'cash']:
            starting_sequence = "%s/%s/00000" % (self.journal_id.code, year_part)
        else:
    ...
    def _get_sequence_date_range(self, reset):
        ...
        fiscal_year = date_utils.get_fiscal_year(self.date, day=fiscalyear_last_day, month=int(fiscalyear_last_month))

        if reset == 'year_range':
            return fiscal_year + (None, None)

        forced_year_range = (fiscal_year[0].year, fiscal_year[1].year)
        month_range = date_utils.get_month(self.date)
        fiscalyear_last_month_max_day = calendar.monthrange(self.date.year, int(fiscalyear_last_month))[1]
        ...
        if fiscalyear_last_day < fiscalyear_last_month_max_day and fiscalyear_last_month == self.date.month:
            if self.date.month <= fiscalyear_last_day:
