...
class AccountMove(models.Model):
    ...
    def _get_ir_default_invoice_cash_rounding_id(self):
        return self.env['ir.default']._get(self._name, 'invoice_cash_rounding_id', company_id=self.company_id.id, condition=f'currency={self.currency_id.name}') \
            or self.env['ir.default']._get(self._name, 'invoice_cash_rounding_id', condition=f'currency={self.currency_id.name}') \
            or self.env['ir.default']._get(self._name, 'invoice_cash_rounding_id', company_id=self.company_id.id)
    ...
    def _compute_invoice_cash_rounding_id(self):
        ...
        for move in self:
            ...
            # Try to find an existing `ir.default` record for the cash rounding value, where the search will be sorted by priority of:
            # - Have company_id and currency condition set
            # - Have currency condition set (without company_id)
            # - Have company_id set (without currency condition)
            elif ir_default_cash_rounding_id := move._get_ir_default_invoice_cash_rounding_id():
