...
class AccountMove(models.Model):
    ...
    adjusting_entry_origin_moves_type = fields.Char(compute='_compute_adjusting_entry_origin_moves_type')
    ...
    @api.depends('adjusting_entry_origin_move_ids')
    def _compute_adjusting_entry_origin_moves_type(self):
        for move in self:
            move.adjusting_entry_origin_moves_type = move.adjusting_entry_origin_move_ids.move_type if len(move.adjusting_entry_origin_move_ids) <= 1 else 'many_invoices'
    ...
    def open_adjusting_entry_origin_moves(self):
        self.ensure_one()
        match self.adjusting_entry_origin_moves_type:
            case 'in_invoice':
                move_name = 'Bill'
            case 'out_refund':
                move_name = 'Credit Note'
            case 'in_refund':
                move_name = 'Refund'
            case 'out_invoice':
                move_name = 'Invoice'
            case _:
                move_name = 'Invoices'
        return self.adjusting_entry_origin_move_ids._get_records_action(name=move_name)
