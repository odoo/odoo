...
class AccountInvoice(models.Model):
    ...
    def _compute_next_recurrency_date(self):
        for invoice in self.filtered(lambda inv: inv.date_invoice and inv.is_recurrency_enabled and inv.recurrency_interval and inv.recurrency_type):
    ...
    is_recurrency_enabled = fields.Boolean(string="Generate Recurring Document(s)", copy=False)
    recurrency_interval = fields.Integer(string='Every', default=1, copy=False)
    ...
    def _get_next_recurrency_date(self, current_date):
        ...
        recurring_invoice_day = fields.Date.from_string(self.date_invoice).day
        # In case of monthly recurrent invoice, future invoices should always be created on same day as of it's main invoice
        # if the next month doesn't have the same day then the invoice should be created on last day of that month
        ...
        if self.recurrency_type == 'months':
            ending_date = recurring_next_date + relativedelta(day=31)
            if ending_date.day >= recurring_invoice_day:
    ...
    def _cron_generate_recurring_vendor_bill(self):
        ...
        for invoice in self.search(domain):
            while invoice.next_recurrency_date <= fields.Date.today():
                invoice.with_context(recurrent_invoice=True)._create_draft_invoice()
    ...
    def _create_draft_invoice(self):
        ...
        invoice.message_post(body=_('This bill has been generated by <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a>') % (self.id, self.number))
        return invoice
