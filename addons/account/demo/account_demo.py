...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _get_demo_data_products(self, company):
        ...
        if not taxes:
            return {}
        product_xmls = self.sudo().env['ir.model.data'].search([('model', '=', 'product.template')])
        existing_products = [d.module + '.' + d.name for d in product_xmls]
        product_templates = list(set(existing_products) - set(self._get_demo_exception_product_template_xml_ids()))
        product_xmls = self.sudo().env['ir.model.data'].search([('model', '=', 'product.product')])
        existing_products = [d.module + '.' + d.name for d in product_xmls]
        product_variants = list(set(existing_products) - set(self._get_demo_exception_product_variant_xml_ids()))
        return {
