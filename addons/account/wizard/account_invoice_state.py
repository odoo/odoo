    ...
    def invoice_confirm(self, cr, uid, ids, context=None):
        ...
        for record in proxy.browse(cr, uid, active_ids, context=context):
            if record.state not in ('draft', 'proforma', 'proforma2'):
                raise Warning(_('Warning!'), _("Selected invoice(s) cannot be confirmed as they are not in 'Draft' or 'Pro-Forma' state."))
    ...
    def invoice_cancel(self, cr, uid, ids, context=None):
        ...
        for record in proxy.browse(cr, uid, active_ids, context=context):
            if record.state in ('cancel','paid'):
                raise Warning(_('Warning!'), _("Selected invoice(s) cannot be cancelled as they are already in 'Cancelled' or 'Done' state."))
