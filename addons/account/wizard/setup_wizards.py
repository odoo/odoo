...
class SetupBarBankConfigWizard(models.TransientModel):
    ...
    linked_journal_id = fields.Many2one(string="Journal",
        ...
        check_company=True,
        domain=[('type', 'in', ('bank', 'credit')), ('bank_account_id', '=', False)])
    ...
    def _number_unlinked_journal(self):
        return self.env['account.journal'].search_count([
            ...
            ('id', '!=', self.default_linked_journal_id('bank')),
        ])
    ...
    def default_linked_journal_id(self, journal_type):
        for journal_id in self.env['account.journal'].search([('type', '=', journal_type), ('bank_account_id', '=', False)]):
