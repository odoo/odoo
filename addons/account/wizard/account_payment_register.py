...
class AccountPaymentRegister(models.TransientModel):
    ...
    wizard_change_mode = fields.Char(
        help="'code_change_amount' if the amount changed and 'payment_date_change' if the payment date change",
    )
    is_amount_forced_by_user = fields.Boolean(
        help="Specify if the user changed the amount or not based on 'wizard_change_mode'",
    )
    ...
    def _onchange_amount(self):
        if self.wizard_change_mode not in ('code_change_amount', 'payment_date_change'):
        ...
        for wizard in self:
            if wizard.wizard_change_mode != 'payment_date_change' or not wizard.is_amount_forced_by_user:
                if not wizard.journal_id or not wizard.currency_id or not wizard.payment_date:
                    ...
                    wizard.amount = None
                wizard.wizard_change_mode = 'code_change_amount'
