...
class AccountInvoiceSend(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if 'invoice_ids' in fields_list and not result.get('invoice_ids'):
            result['invoice_ids'] = res_ids
        if 'composer_id' in fields_list and not result.get('composer_id'):
        ...
        if self.composition_mode == 'mass_mail' and self.template_id:
            ...
            for lang in sorted((set(langs)) or [default_lang]):
                ...
                self_lang = self.with_context(
                    composer_force_res_ids=active_ids_lang,
                    active_ids=active_ids_lang,
                    lang=lang
                )
