    ...
    def view_init(self, cr , uid , fields_list, context=None):
        ...
        if context.get('active_id',False):
            if obj_inv.browse(cr, uid, context['active_id']).state != 'draft':
                raise Warning(_('Error!'), _('You can only change currency for Draft Invoice.'))
            pass
    ...
    def change_currency(self, cr, uid, ids, context=None):
        ...
        for line in invoice.invoice_line:
            ...
            if invoice.company_id.currency_id.id == invoice.currency_id.id:
                ...
                if new_price <= 0:
                    raise Warning(_('Error!'), _('New currency is not configured properly.'))
            ...
            if invoice.company_id.currency_id.id != invoice.currency_id.id and invoice.company_id.currency_id.id == new_currency:
                ...
                if old_rate <= 0:
                    raise Warning(_('Error!'), _('Current currency is not configured properly.'))
