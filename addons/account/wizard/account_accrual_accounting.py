...
class AccrualAccountingWizard(models.TransientModel):
    ...
    def amend_entries(self):
        ...
        for aml in self.active_move_line_ids:
            ...
            to_reconcile.append(number_to_reconcile)
        created_ids = self.env['account.move'].create(move_data)
        created_ids.post()

        self.active_move_line_ids.with_context(no_update_check=True).write({'account_id': (self.revenue_accrual_account if self.account_type == 'income' else self.expense_accrual_account).id})
        for move, msg in zip(created_ids, message_data):
            move.message_post(body=msg)

        move_list = list(created_ids)
        for number_to_reconcile, origin_aml in zip(to_reconcile, self.active_move_line_ids):
            move_ids = reduce(lambda a, b: a + b, move_list[0:number_to_reconcile])
            del move_list[0:number_to_reconcile]
            (move_ids.line_ids.filtered(lambda line: line.account_id in (self.revenue_accrual_account + self.expense_accrual_account)) + origin_aml).reconcile()
