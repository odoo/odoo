        ...
        for st in self.browse(cr, uid, ids, context=context):
            ...
            for st_line in st.line_ids:
                ...
                if st_line.account_id and not st_line.journal_entry_id.id:
                    ...
                    raise Warning(_('Error!'), _('All the account entries lines must be processed in order to close the statement.'))
...
class account_bank_statement_line(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        if vals.get('amount_currency', 0) and not vals.get('amount', 0):
            raise Warning(_('Error!'), _('If "Amount Currency" is specified, then "Amount" must be as well.'))
    ...
    def process_reconciliation(self, cr, uid, id, mv_line_dicts, context=None):
        ...
        if st_line.journal_entry_id.id:
            raise Warning(_('Error!'), _('The bank statement line was already reconciled.'))
        for mv_line_dict in mv_line_dicts:
            ...
            if mv_line_dict.get('counterpart_move_line_id'):
                ...
                if mv_line.reconcile_id:
                    raise Warning(_('Error!'), _('A selected move line was already reconciled.'))
