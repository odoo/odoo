...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        partner_clause = ''
        cr = self.env.cr
        user_company = self.env['res.company']._get_current_company()
        ...
        for i in range(5):
            ...
            for line in self.env['account.move.line'].browse(aml_ids).with_context(prefetch_fields=False):
                ...
                if not self.env['res.company']._get_current_company().currency_id.is_zero(line_amount):
        ...
        for partner in partners:
            ...
            if not float_is_zero(values['direction'], precision_rounding=self.env['res.company']._get_current_company().currency_id.rounding):
            ...
            for i in range(5):
                ...
                if not float_is_zero(values[str(i)], precision_rounding=self.env['res.company']._get_current_company().currency_id.rounding):
                    at_least_one_amount = True
