...
class aged_trial_report(report_sxw.rml_parse, common_report_header):
    ...
    def _get_lines(self, form):
        ...
        self.cr.execute('SELECT DISTINCT res_partner.id AS id,\
                ...
                WHERE (l.account_id=account_account.id) \
                    ...
                    AND ((reconcile_id IS NULL)\
                       OR (reconcile_id IN (SELECT recon.id FROM account_move_reconcile AS recon WHERE recon.create_date::Date > %s )))\
        ...
        self.cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit) \
                    ...
                    OR (l.reconcile_id IN (SELECT recon.id FROM account_move_reconcile AS recon WHERE recon.create_date::Date > %s )))\
                    AND ' + self.query + '\
                    AND account_account.active\
                    AND (l.date <= %s)\
