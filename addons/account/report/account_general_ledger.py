...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts):
        """
        :param:
                accounts: List of account's recordset
        :retrun {
                'code': account code,
                'name': account name,
        ...
        for account in accounts:
            ...
            if self.display_account == 'not_zero' and not currency.is_zero(res['balance']):
                account_res.append(res)
    ...
    def render_html(self, data):
        ...
        accounts_res = self._get_account_move_entry(self.env['account.account'].search([]))
        docargs = {
            'doc_ids': self.ids,
