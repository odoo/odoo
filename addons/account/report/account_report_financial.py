...
class ReportFinancial(models.AbstractModel, CommonReportHeader):
    _name = 'report.account.report_financial'

    def _balance(self, reports):
        ...
        for report in reports:
            ...
            if report.type == 'accounts':
                ...
                res[report.id]['account'] = self._compute(report.account_ids)
                ...
                res[report.id]['account'] = self._compute(accounts)
                for value in res[report.id]['account'].values():
                    for field in fields:
                        res[report.id][field] += value.get(field)
            elif report.type == 'account_report' and report.account_report_id:
                # it's the amount of the linked report
                res2 = self._balance(report.account_report_id)
                for key, value in res2.items():
                    for field in fields:
                        res[report.id][field] += value[field]
            elif report.type == 'sum':
                # it's the sum of the children of this account.report
                res2 = self._balance(report.children_ids)
                for key, value in res2.items():
                    for field in fields:
                        res[report.id][field] += value[field]
        return res
