...
var bankStatementReconciliationLine = Widget.extend({
    ...
    createFormWidgets: function() {
        ...
        var field_manager = new FormView (
            this, dataset, false, {
                initial_mode: 'edit',
        ...
        var FieldMany2One = core.form_widget_registry.get('many2one');
        self.change_partner_field = new FieldMany2One(field_manager, change_partner_node);
    ...
    makeMoveLineDicts: function() {
        ...
        var created_lines = _.filter(self.getCreatedLines(), function(l) { return ! l.is_tax_line; }); // Tax lines are created via account_tax_id
...
var ReconciliationListView = ListView.extend({
    init: function() {
    ...
    do_search: function(domain, context, group_by) {
        ...
        var mod = new Model("account.move.line", context, domain);
    ...
    reconcile: function() {
        ...
        new Model("ir.model.data").call("get_object_reference", ["account", "action_view_account_move_line_reconcile"]).then(function(result) {
    ...
    mark_as_reconciled: function() {
        ...
        new Model("res.partner").call("mark_as_reconciled", [[id]]).then(function() {
            self.do_search(self.last_domain, self.last_context, self.last_group_by);
        });
    },
...
core.view_registry.add('tree_account_reconciliation', ReconciliationListView);


return {
    bankStatementReconciliation: bankStatementReconciliation,
    bankStatementReconciliationLine: bankStatementReconciliationLine,
    ReconciliationListView: ReconciliationListView,
};
