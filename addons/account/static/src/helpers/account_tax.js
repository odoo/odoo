 ...
 * PLZ KEEP BOTH METHODS CONSISTENT WITH EACH OTHERS.
*/
export function ascending_process_fixed_taxes_batch(batch, fixed_multiplicator=1) {
    if (batch.amount_type === "fixed") {
        batch.computed = "tax";
        for (const tax_values of batch.taxes) {
            tax_values.eval_context.quantity_multiplicator = tax_values.amount * Math.abs(fixed_multiplicator);
        }
    }
}
...
export function descending_process_price_included_taxes_batch(batch) {
    ...
    if (amount_type === "percent") {
        ...
        for (const tax_values of batch.taxes) {
            total_reverse_percentage += tax_values.amount * tax_values.factor / 100.0;
        }
        for (const tax_values of batch.taxes) {
...
export function ascending_process_taxes_batch(batch) {
    ...
    if (amount_type === "percent") {
        ...
        for (const tax_values of batch.taxes) {
            total_percentage += tax_values.amount * tax_values.factor / 100.0;
        }
        for (const tax_values of batch.taxes) {
            const percentage = tax_values.amount / 100.0;
...
export function prepare_taxes_computation(tax_values_list, kwargs={}) {
    // Kwargs.
    const fixed_multiplicator = kwargs.hasOwnProperty("fixed_multiplicator") ? kwargs.fixed_multiplicator : 1;
    ...
    for (const tax_values of sorted_tax_values_list) {
        if (tax_values.amount_type === "group") {
            const sorted_children_tax_values_list = tax_values.children_tax_ids.sort(
                (v1, v2) => v1.sequence - v2.sequence || v1.tax_id - v2.tax_id
            );
            for (const child_tax_values of sorted_children_tax_values_list) {
    ...
    for (const batch of ascending_batches) {
        ...
        ascending_process_fixed_taxes_batch(batch, fixed_multiplicator);
    ...
    for (const [i, batch] of ascending_batches.entries()) {
        ...
        const base_tags_field = is_refund ? "refund_base_tag_ids" : "invoice_base_tag_ids";
...
export function eval_taxes_computation(taxes_computation, eval_context) {
    ...
    let price_include_added_base = 0.0;
    ...
    for (const [quid, index] of eval_order_indexes) {
        ...
        if (quid === "tax") {
            ...
            tax_values.tax_amount_factorized = tax_values.tax_amount * tax_values.factor;
            if (rounding_method === "round_per_line") {
                tax_values.tax_amount_factorized = roundPrecision(tax_values.tax_amount_factorized, prec_rounding);
            }
            if (tax_values.price_include && reverse) {
                price_include_added_base += tax_values.tax_amount_factorized;
            }
        } else if (quid === "base") {
            let extra_base = 0.0;
    ...
    return {
        ...
        price_include_added_base: price_include_added_base,
    }
}
...
export function adaptPriceUnitToAnotherTaxes(priceUnit, originalTaxValuesList, newTaxValuesList) {
    const originalTaxIds = new Set(originalTaxValuesList.map(x => x.id));
    const newTaxIds = new Set(newTaxValuesList.map(x => x.tax_id));
    if (
        (
            originalTaxIds.size === newTaxIds.size
            && [...originalTaxIds].every(value => newTaxIds.has(value))
        )
        || originalTaxValuesList.some(x => !x.price_include)
     ) {
        return priceUnit;
    }

    let taxesComputation = prepare_taxes_computation(originalTaxValuesList);
    let evalContext = eval_taxes_computation_prepare_context(priceUnit, 1.0, { rounding_method: 'round_globally' });
    taxesComputation = eval_taxes_computation(taxesComputation, evalContext);
    priceUnit = taxesComputation.total_excluded;

    taxesComputation = prepare_taxes_computation(newTaxValuesList);
    evalContext = eval_taxes_computation_prepare_context(priceUnit, 1.0, {
        rounding_method: 'round_globally',
        reverse: true,
    });
    taxesComputation = eval_taxes_computation(taxesComputation, evalContext);

    return priceUnit + taxesComputation.price_include_added_base;
}
