...
export class SectionAndNoteListRenderer extends ListRenderer {
    ...
    getCellClass(column, record) {
        ...
        if (this.isSection(record) && column.widget !== "handle" && column.name !== this.titleField && !this.sectionCols.includes(column.name)) {
            return `${classNames} o_hidden`;
        } else if (this.isNote(record) && column.widget !== "handle" && column.name !== this.titleField) {
            return `${classNames} o_hidden`;
        }
        return classNames;
    }
    ...
    getSectionColumns(columns) {
        ...
        return sectionCols.map((col) => {
            ...
            } else if (col.name === "print_details" && !hasProductRelatedColumn) {
                return { ...col, colspan: this.columns.length - sectionCols.length + 1 };
            } else {
                return { ...col };
            }
    ...
    getNoteColumns(columns) {
        const noteCols = columns.filter((col) => col.widget === "handle" || col.type === "field" && col.name === this.titleField);
