...
export function common_kwargs() {
    ...
    return {
        ...
        invoice_repartition_line_ids: [1.0],
        refund_repartition_line_ids: [1.0],
        factor: 1.0,
        invoice_base_tag_ids: [],
        refund_base_tag_ids: [],
    }
}
...
export function group_of_taxes(taxes, kwargs={}) {
    return {
        ...
        ...kwargs,
        amount_type: "group",
        children_tax_ids: taxes.map(tax_values => Object.assign({group_id: number}, tax_values)),
    }
}
...
export function checkTaxResults(assert, taxes, expectedValues, priceUnit, evalContextKwargs={}, computeKwargs={}) {
    ...
    const quantity = evalContextKwargs.hasOwnProperty("quantity") ? evalContextKwargs.quantity : 1;
    let evalContext = eval_taxes_computation_prepare_context(priceUnit, quantity, evalContextKwargs);
    let results = computeSingleLineTaxes(taxes, evalContext, computeKwargs);
    const isRoundGlobally = evalContext.rounding_method === "round_globally";
    const rounding = isRoundGlobally ? 0.000001 : 0.01;
    compare_values(results, expectedValues, rounding);

    if (isRoundGlobally) {
        let evalContext = eval_taxes_computation_prepare_context(results.total_excluded, quantity, Object.assign(
            {reverse: true},
            evalContextKwargs,
        ));
        results = computeSingleLineTaxes(taxes, evalContext, computeKwargs);
        compare_values(results, expectedValues, rounding);
        assert.strictEqual(
            roundPrecision(results.total_excluded + results.price_include_added_base, rounding),
...
QUnit.module("test_account_tax", ({}) => {
    ...
    QUnit.test("test_adapt_price_unit_to_another_taxes", async (assert) => {
        ...
        let product_price_unit = adaptPriceUnitToAnotherTaxes(
            121.0,
            [tax_include_src],
            [tax_include_dst],
        );
        ...
        product_price_unit = adaptPriceUnitToAnotherTaxes(
            100.0,
            [tax_exclude_src],
            [tax_include_dst],
        );
