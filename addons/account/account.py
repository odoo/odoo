...
class account_model(osv.osv):
    ...
    def generate(self, cr, uid, ids, data=None, context=None):
        ...
        period_obj = self.pool.get('account.period')
        account_model_line_obj = self.pool.get('account.model.line')
...
class account_model_line(osv.osv):
    ...
    def _prepare_move_line_values(self, cr, uid, line_id, context=None):
        pt_obj = self.pool.get('account.payment.term')

        line = self.browse(cr, uid, line_id, context=context)
        analytic_account_id = False
        if line.analytic_account_id:
            if not line.model_id.journal_id.analytic_journal_id:
                raise osv.except_osv(_('No Analytic Journal!'),_("You have to define an analytic journal on the '%s' journal!") % (line.model_id.journal_id.name,))
        ...
        if line.date_maturity == 'partner':
            if not line.partner_id:
                raise osv.except_osv(_('Error!'), _("Maturity date of entry line generated by model line '%s' of model '%s' is based on partner payment term!" \
                                                        "\nPlease define partner on it!")%(line.name, line.model_id.name))

            payment_term_id = False
            if line.model_id.journal_id.type in ('purchase', 'purchase_refund') and line.partner_id.property_supplier_payment_term:
