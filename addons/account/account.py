...
class account_model(osv.osv):
    ...
    def generate_line_values(self, cr, uid, line, context=None):
        ...
        date_maturity = ctx2.get('date',time.strftime('%Y-%m-%d'))
        ...
        val.update({
            ...
            'date': ctx2.get('date', fields.date.context_today(self,cr,uid,context=ctx2)),
    ...
    def generate(self, cr, uid, ids, data=None, context=None):
        ...
        for model in self.browse(cr, uid, ids, context=context):
            ...
            move_id = account_move_obj.create(cr, uid, {
                ...
                'date': context.get('date', fields.date.context_today(self,cr,uid,context=ctx))
            })
            move_ids.append(move_id)
            for line in model.lines_id:
                val, ctx2 = self.generate_line_values(cr, uid, line, context=ctx)
                val.update({
                    'move_id': move_id,
                    'journal_id': model.journal_id.id,
                    'period_id': period_id,
                })
                account_move_line_obj.create(cr, uid, val, context=ctx2)
