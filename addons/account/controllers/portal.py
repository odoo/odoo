...
class PortalAccount(CustomerPortal):
    ...
    def portal_my_invoice_report(self, invoice_id, access_token=None, **kw):
        ...
        AccountInvoice = request.env.ref('account.account_invoices').sudo()
        report_type = kw.get('report_type', 'pdf')
        method_name = 'render_qweb_%s' % (report_type)
        if hasattr(AccountInvoice, method_name):
            invoice_report = getattr(AccountInvoice, method_name)([invoice_sudo.id], data={'report_type': report_type})[0]
            reporthttpheaders = [
