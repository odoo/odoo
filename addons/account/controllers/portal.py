...
class PortalAccount(CustomerPortal):
    ...
    def portal_my_journal_unsubscribe(self, journal_id, access_token=None):
        def _render(ctx, status=200):
            return request.render('account.portal_my_journal_mail_notifications', ctx, status=status)

        email_to_unsubscribe = False

        if access_token:
            token_data = verify_hash_signed(request.env, request.env['account.journal']._get_journal_notification_unsubscribe_scope(), access_token)
            if not token_data or token_data.get('journal_id') != journal_id:
                return _render({'error': _('Invalid token')}, 403)

            if not (email_to_unsubscribe := email_normalize(token_data.get('email_to_unsubscribe'), strict=False)):
                return _render({'error': _('Invalid token')}, 403)

            journal = request.env['account.journal'].sudo().browse(journal_id)
            if journal.exists():
                journal = journal.with_company(journal.company_id.id)
        else:
            ...
            if not journal.exists() or not journal.has_access('write'):
                return _render({'error': _('Invalid token')}, 403)

            journal = journal.with_company(journal.company_id.id)
            emails = email_normalize_all(journal.invoice_notified_emails or '')
            if len(emails) != 1:
        ...
        all_recipients = email_normalize_all(journal.invoice_notified_emails or '')
        ...
        if request.httprequest.method == 'POST':
            journal.sudo()._unsubscribe_invoice_notification_email(email_to_unsubscribe)
