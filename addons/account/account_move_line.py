        ...
        if context.get('periods'):
            ...
            if initial_bal:
                ...
                if period_ids and period_ids[0]:
                    ...
                    if periods:
                        query_params['initial_period_ids'] = tuple(periods)
                        query = obj+".state <> 'draft' AND "+obj+".period_id IN (SELECT id FROM account_period WHERE fiscalyear_id IN %(fiscalyear_ids)s AND id IN %(initial_period_ids)s) " + where_move_state + where_move_lines_by_date
            else:
