...
class CRMLeadMiningRequest(models.Model):
    ...
    def _check_lead_number(self):
        if self.lead_number > MAX_LEAD_NUMBER:
            raise ValidationError(_("The number of leads to generate can't exceed %s," % MAX_LEAD_NUMBER))

    @api.constrains('contact_number')
    def _check_contact_number(self):
        if self.contact_number > MAX_CONTACT_NUMBER:
            raise ValidationError(_("The number of contacts to look for can't exceed %s," % MAX_CONTACT_NUMBER))

    @api.constrains('company_size_min', 'company_size_max')
    def _check_company_size(self):
        if self.company_size_min > self.company_size_max:
            raise ValidationError(_("The minimum size of the company should be less than or equal to the maximum size."))
