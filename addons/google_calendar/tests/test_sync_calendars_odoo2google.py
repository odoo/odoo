...
class TestSyncCalendarsGoogle2Odoo(TestSyncGoogle):
    ...
    def test_o2g_existing_synced_calendar_calls_patch(self):
        ...
        self.organizer_user.primary_calendar_id._sync_calendars_odoo2google(self.google_service)
        self.organizer_user.primary_calendar_id.with_user(self.organizer_user).calendar_default_privacy = 'confidential'
        self.assertGoogleCalendarPatched(
            self.organizer_user.primary_calendar_id.id,
            self.organizer_user.primary_calendar_id._google_values()
        )
    ...
    def test_o2g_moving_calendar_in_odoo_moves_event_in_google(self):
        ...
        event.calendar_id = self.organizer_user.primary_calendar_id
    ...
    def test_o2g_changing_calendar_organizer_removes_and_recreates_calendar_in_google(self):
        ...
        self.assertEqual(event.calendar_id, self.attendee_user.primary_calendar_id)
    ...
    @patch_api
    def test_o2g_unsetting_calendar_or_organizer_removes_event_in_google(self):
        google_id = '4'
        event = self.env['calendar.event'].with_user(self.organizer_user).create({
            'name': 'Event',
            'start': datetime(2020, 1, 15, 8, 0),
        ...
        # Event should not be inserted again
        event._sync_odoo2google(self.google_service)
        self.assertEqual(len(self._gsync_insert_values), 1)

        # Reset the event user
        event.user_id = self.organizer_user
        event._sync_odoo2google(self.google_service)
        self.assertEqual(len(self._gsync_insert_values), 2)
        google_id = event.google_id

        # Unsetting the calendar should also remove the event from Google
