...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        odoo_events_ids = (events - recurrences).odoo_ids(self.env['calendar.event'].env)
        odoo_recurrences_ids = recurrences.odoo_ids(self.env['calendar.recurrence'].env)
        odoo_records_by_class = {
            'calendar.event': self.env['calendar.event'].browse(odoo_events_ids),
            'calendar.recurrence': self.env['calendar.recurrence'].browse(odoo_recurrences_ids)
        }
        write_dates = {}
        for class_name in odoo_records_by_class:
            for record in odoo_records_by_class[class_name]:
                write_dates[(class_name, record.id)] = record.write_date
        synced_recurrences = self.env['calendar.recurrence'].with_context(write_dates=write_dates)._sync_google2odoo(recurrences)
        synced_events = self.env['calendar.event'].with_context(write_dates=write_dates)._sync_google2odoo(events - recurrences, default_reminders=default_reminders)
