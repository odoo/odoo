...
class ResUsers(models.Model):
    ...
    def _sync_google_calendars(self, calendar_service: GoogleCalendarService):
        ...
        if google_calendar_values := self._sync_calendars_request(calendar_service):
            new_calendars = self.env['calendar.calendar']._sync_calendars_google2odoo(google_calendar_values)
    ...
    def _sync_google_events(self, calendar_service: GoogleCalendarService):
        ...
        for calendar in self.env.user.calendar_ids:
            ...
            events, default_reminders, full_sync = self._sync_request(calendar_service, calendar)
        ...
        for calendar, calendar_data in events_by_calendar.items():
            ...
            fake_cancelled_events = events.filter(lambda e: e.cancelled() and e.id in persisted_events)
        ...
        for calendar in self.env.user.calendar_ids:
            if calendar.user_access_role not in ['owner', 'writer'] or not calendar.google_sync_enabled:
                continue
            full_sync = events_by_calendar.get(calendar.id, {}).get('full_sync', False)
    ...
    def _sync_request(self, calendar_service, calendar, event_id=None):
        self.ensure_one()
        if self._get_google_sync_status() != "sync_active":
