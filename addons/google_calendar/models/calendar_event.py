...
class CalendarEvent(models.Model):
    ...
    def _get_calendar_id_domain(self):
        ...
        return Domain.AND([domain, [("is_import_pending", "=", False)]])
    ...
    def _google_values(self):
        ...
        if self.privacy:
            values['visibility'] = 'private' if self.privacy == 'members_only' else self.privacy
        if self.show_as:
    ...
    def _before_calendar_cascade_unlink(self):
        # Detach the events from the calendar before mass unlinking them to avoid removing all the events
        # on Google's side. If the user wants to remove all events from Google, they should unlink them
