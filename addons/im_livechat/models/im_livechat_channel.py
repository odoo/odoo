...
class Im_LivechatChannel(models.Model):
    ...
    concurrent_session = fields.Selection([
        ("unlimited", "Unlimited"),
        ("limited", "Limited"),
    ], default="unlimited")
    ...
    _concurrent_session_greater_than_zero = models.Constraint(
        "CHECK(concurrent_session_number > 0)",
    ...
    def _compute_available_operator_ids(self):
        ...
        if limited_channel := self.filtered(lambda c: c.concurrent_session == "limited"):
            old_livechat_dt = fields.Datetime.now() - timedelta(minutes=30)
            available_user_operators = limited_channel.user_ids.filtered(
        ...
        for record in self:
            ...
            if record.concurrent_session == "limited":
                operator_with_too_many_sessions = [
