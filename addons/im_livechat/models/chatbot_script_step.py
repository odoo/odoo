...
class ChatbotScriptStep(models.Model):
    ...
    def _process_step_forward_operator(self, discuss_channel):
        ...
        if human_operator and human_operator != self.env.user:
            ...
            # sudo: discuss.channel - setting the operator to the actual operator is part of the flow.
            discuss_channel.sudo().livechat_operator_id = human_operator.partner_id
            # sudo - discuss.channel.member: finding bot member to remove it from the members is part of the flow.
            if bot_member := discuss_channel.sudo().channel_member_ids.filtered(
                ...
                # sudo - discuss.channel: removing bot from the channel is part of the flow.
                discuss_channel.sudo()._action_unfollow(partner=bot_member.partner_id)
            # finally, rename the channel to include the operator's name.
            discuss_channel.sudo().name = (human_operator.livechat_username or human_operator.name,)
