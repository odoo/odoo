...
class DiscussChannel(models.Model):
    ...
    def _get_channel_history_format_author(self, message_author):
        ...
        return (
            username
            if message_author._name == "res.partner" and (username := message_author.user_livechat_username)
            else message_author.name
        )
    ...
    def _get_recent_channels_match_sql(self):
        return SQL(
            """
            SELECT source_history.source_channel_id,
                   candidate_history.channel_id AS candidate_channel_id
              FROM source_histories source_history
            ...
            SELECT source_history.source_channel_id,
              ...
              FROM source_histories source_history
              JOIN im_livechat_channel_member_history candidate_history
                ON candidate_history.guest_id = source_history.guest_id
               AND candidate_history.livechat_member_type = 'visitor'
            """,
        )
            ...
            SQL(
                ...
                WITH source_channels AS (
                    SELECT id
                     ...
                     WHERE id IN %(source_channel_ids)s
                ),
                source_histories AS (
                    SELECT source_channel.id AS source_channel_id,
                           source_history.partner_id,
                           source_history.guest_id
                      FROM source_channels source_channel
                      JOIN im_livechat_channel_member_history source_history
                        ON source_history.channel_id = source_channel.id
                ...
                recent_channels AS (
                    SELECT channel_match.source_channel_id,
                           ARRAY_AGG(
                               candidate_channel.id
                               ORDER BY
                                   candidate_channel.livechat_end_dt IS NOT NULL,
                                   candidate_channel.last_interest_dt DESC,
                                   candidate_channel.id DESC
                           ) AS channel_ids,
                           COUNT(*) AS channel_count
                      FROM channel_matches channel_match
                      JOIN discuss_channel candidate_channel
                        ON candidate_channel.id = channel_match.candidate_channel_id
                     WHERE candidate_channel.id != channel_match.source_channel_id
                       AND candidate_channel.channel_type = 'livechat'
                       AND candidate_channel.last_interest_dt >= %(recent_channels_since)s
                  GROUP BY channel_match.source_channel_id
                )
                SELECT recent_channels.source_channel_id,
                       CASE
                           WHEN %(limit)s IS NULL THEN recent_channels.channel_ids
                           ELSE recent_channels.channel_ids[1:%(limit)s]
                       END AS channel_ids,
    ...
    def action_recent_channels(self):
        ...
        action["domain"] = Domain("id", "in", recent_channels_by_channel[self].ids)
        action["name"] = self.env._("Recent Conversations")
        return action
