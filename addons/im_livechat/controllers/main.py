    ...
    def _poll(self, dbname, channels, last, options):
        ...
        if not request.session.get('uid'):
            ...
            for channel in request.env['mail.channel'].sudo().search([('channel_type', '=', 'livechat'), ('uuid', 'in', list(channels))]):
                livechat_operator = channel.livechat_operator_id
                operator_ids = channel.livechat_channel_id.user_ids.mapped('partner_id').ids
                no_operator = livechat_operator.id not in operator_ids or livechat_operator.im_status == 'offline'
                if no_operator != options.get('no_operator'):
