...
class LivechatController(http.Controller):
    ...
    def visitor_leave_session(self, uuid):
        ...
        if not discuss_channel:
            return
        channel_member_sudo = request.env["discuss.channel.member"]._get_as_sudo_from_context(channel_id=discuss_channel.id)
        if discuss_channel.id in channel_member_sudo.rtc_session_ids.channel_id.ids:
