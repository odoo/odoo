...
class CorsLivechatController(LivechatController):
    ...
    @route("/im_livechat/cors/download_transcript/<int:channel_id>", type="http", auth="force_guest", cors="*")
    def cors_download_livechat_transcript(self, guest_token, channel_id):
        return self.download_livechat_transcript(channel_id)

    @route("/im_livechat/cors/get_session", methods=["POST"], type="jsonrpc", auth="force_guest_optional", cors="*")
    ...
    @route("/im_livechat/cors/init", type="jsonrpc", auth="force_guest_optional", cors="*")
