...
class CorsLivechatController(LivechatController):
    @route("/im_livechat/cors/visitor_leave_session", type="jsonrpc", auth="force_guest", save_session=False, cors="*")
    def cors_visitor_leave_session(self, guest_token, channel_id):
        self.visitor_leave_session(channel_id)

    @route("/im_livechat/cors/feedback", type="jsonrpc", auth="force_guest", save_session=False, cors="*")
    ...
    @route("/im_livechat/cors/get_session", methods=["POST"], type="jsonrpc", auth="force_guest_optional", save_session=False, cors="*")
    def cors_get_session(
        self, channel_id, previous_operator_id=None, chatbot_script_id=None, persisted=True, guest_token="", **kwargs
    ):
        return self.get_session(
            channel_id, previous_operator_id, chatbot_script_id, persisted, **kwargs
        )

    @route("/im_livechat/cors/init", type="jsonrpc", auth="force_guest_optional", save_session=False, cors="*")
