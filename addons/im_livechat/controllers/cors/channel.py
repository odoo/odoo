...
class LivechatChannelController(ChannelController):
    @route("/im_livechat/cors/channel/mark_as_read", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
    def livechat_channel_mark_as_read(self, guest_token, **kwargs):
        return self.discuss_channel_mark_as_read(**kwargs)

    @route("/im_livechat/cors/channel/notify_typing", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
