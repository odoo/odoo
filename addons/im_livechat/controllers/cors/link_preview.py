...
class LivechatLinkPreviewController(LinkPreviewController):
    @route("/im_livechat/cors/link_preview", methods=["POST"], type="jsonrpc", auth="force_guest", save_session=False, cors="*")
    def livechat_link_preview(self, guest_token, message_id):
        self.mail_link_preview(message_id)

    @route("/im_livechat/cors/link_preview/hide", methods=["POST"], type="jsonrpc", auth="force_guest", save_session=False, cors="*")
