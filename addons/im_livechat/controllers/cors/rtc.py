...
class LivechatRtcController(RtcController):
    @route("/im_livechat/cors/rtc/channel/join_call", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
    ...
    @route("/im_livechat/cors/rtc/channel/leave_call", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
    def livechat_channel_call_leave(self, guest_token, channel_id):
        return self.channel_call_leave(channel_id)

    @route("/im_livechat/cors/rtc/session/update_and_broadcast", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
    ...
    @route("/im_livechat/cors/rtc/session/notify_call_members", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
    def livechat_session_call_notify(self, guest_token, peer_notifications):
        self.session_call_notify(peer_notifications)

    @route("/im_livechat/cors/channel/ping", methods=["POST"], type="jsonrpc", auth="force_guest", cors="*")
