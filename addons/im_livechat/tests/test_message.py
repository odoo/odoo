...
class TestImLivechatMessage(HttpCase, MailCommon):
    ...
    def test_message_to_store(self):
        ...
        self.env["mail.presence"].sudo().create({"user_id": self.users[0].id, "status": "online"})  # make available for livechat (ignore leave)
    ...
    def test_feedback_message(self):
        ...
        self.env["mail.presence"].sudo().create({"user_id": self.users[0].id, "status": "online"})
