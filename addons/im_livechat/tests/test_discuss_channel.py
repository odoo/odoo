...
class TestDiscussChannel(TestImLivechatCommon):
    ...
    def test_human_operator_failure_states(self):
        ...
        chat = self.env["discuss.channel"].browse(data["channel_id"])
        self.assertEqual(len(chat.chatbot_current_step_id), 0) # assert there is no chatbot
        self.assertEqual(chat.livechat_failure, "never_answered")
    ...
    def test_chatbot_failure_states(self):
        ...
        chat = self.env["discuss.channel"].browse(data["channel_id"])
        self.assertEqual(len(chat.chatbot_current_step_id), 1) # assert there is a chatbot
        self.assertEqual(chat.livechat_failure, "no_failure")
        ...
        self.assertEqual(chat.livechat_failure, "no_one_available")
