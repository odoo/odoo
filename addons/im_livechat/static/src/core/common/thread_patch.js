...
patch(Thread.prototype, {
    setup() {
        ...
        useEffect(
            () => {
                if (
                    !this.props.thread.livechatVisitorMember?.partner_id?.im_status &&
                    !this.props.thread.livechatVisitorMember?.guest_id?.im_status
                ) {
                    return;
                }
                clearTimeout(this.imStatusTimeoutId);
                if (
                    this.props.thread.livechatVisitorMember.partner_id?.im_status.includes(
                        "offline"
                    ) ||
                    this.props.thread.livechatVisitorMember.guest_id?.im_status.includes("offline")
                ) {
