...
QUnit.test('livechat - avatar: should have a smily face avatar for an anonymous livechat item', async function (assert) {
    assert.expect(2);
    ...
    `);
    assert.strictEqual(
        livechatItem.querySelectorAll(`:scope .o_DiscussSidebarCategoryItem_image`).length,
        1,
...
QUnit.test('livechat - avatar: should have a parnter profile picture for a livechat item linked with a partner', async function (assert) {
    assert.expect(2);
