...
var LiveChatWindow = require('mail.widget.LiveChatWindow');
...
var LivechatButton = Widget.extend({
    ...
    custom_events: {
        save_conversation: '_onSaveConversation',
    },
    events: {
        'click': '_openChat'
    },
    ...
    willStart: function () {
        ...
        var ready;
        if (!cookie) {
            ready = session.rpc('/im_livechat/init', {channel_id: this.options.channelID})
    ...
    _addMessage: function (data, options) {
        ...
        msg.avatar_src = this._serverURL;
    ...
    _askFeedback: function () {
        this._chatWindow.$('.o_conversation_composer input').prop('disabled', true);

        var feedback = new Feedback(this, this._channelUUID);
    ...
    _loadQWebTemplate: function () {
        var xml_files = ['/mail/static/src/xml/abstract_conversation_window.xml',
    ...
    _openChatWindow: function (channel) {
        ...
        this._chatWindow.on('close_chat_session', this, function () {
            var input_disabled = this._chatWindow.$('.o_conversation_composer input').prop('disabled');
            var ask_fb = !input_disabled && _.find(this._messages, function (msg) {
        ...
        this._chatWindow.threadWidget.$el.on('scroll', null, _.debounce(function () {
            if (self._chatWindow.threadWidget.isAtBottom()) {
                self._chatWindow.updateUnreadCounter(0);
            }
        }, 100));
    },
    /**
...
var Feedback = Widget.extend({
    ...
    init: function (parent, channelUUID) {
        this._super(parent);
