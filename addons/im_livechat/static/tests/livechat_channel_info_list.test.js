...
test("Show recent conversations in channel info list", async () => {
    ...
    mockService("action", {
        doAction(action) {
            if (action.name === "Recent Conversations") {
                expect(action.domain).toHaveLength(1);
                const [[field, operator, ids]] = action.domain;
                expect(field).toBe("id");
                expect(operator).toBe("in");
                expect(ids).toEqual(channelIds.slice(0, -1), { ignoreOrder: true });
                expect.step("open recent conversations");
                return;
            }
    ...
    await click(".o-livechat-LivechatChannelInfoList-recentConversation button:text('View all')");
    await expect.waitForSteps(["open recent conversations"]);
});
