...
class MailResendMessage(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if message_id:
            ...
            notification_ids = mail_message_id.notification_ids.filtered(lambda notif: notif.notification_type == 'email' and notif.notification_status in ('exception', 'bounced'))
            partner_ids = [(0, 0, {
    ...
    def resend_mail_action(self):
        ...
        for wizard in self:
            ...
            notif_to_cancel = wizard.notification_ids.filtered(lambda notif: notif.notification_type == 'email' and notif.res_partner_id in to_cancel and notif.notification_status in ('exception', 'bounced'))
    ...
    def cancel_mail_action(self):
        for wizard in self:
            for notif in wizard.notification_ids:
                notif.filtered(lambda notif: notif.notification_type == 'email' and notif.notification_status in ('exception', 'bounced')).sudo().write({'notification_status': 'canceled'})
