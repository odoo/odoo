...
class MailTemplatePreview(models.TransientModel):
        ...
        if not self.resource_ref or not self.resource_ref.id:
            ...
            try:
                ...
                if mail_template.email_layout_xmlid:
                    ...
                    ActiveModel = self.resource_ref if hasattr(self.resource_ref, '_notify_get_recipients') else self.env['mail.thread']
                    # Create a message without db entry for the layout.
                    message = self.env['mail.message'].new(msg_vals)
                    recipients_data = ActiveModel._notify_get_recipients(message, msg_vals)
                    if not recipients_data:
                        recipients_data.append(self._partner_data_for_preview())
                    _lang, render_values, recipients_group = next(ActiveModel._notify_get_classified_recipients_iterator(message, recipients_data, msg_vals=msg_vals))
                    # We don't add signature in the mail template
                    render_values['email_add_signature'] = False
                    # Some custom template layouts use record variable
                    render_values['record'] = self.resource_ref
                    render_values['email_notification_force_header'] = mail_template.force_header
                    render_values['email_notification_force_footer'] = mail_template.force_footer

                    mail_values['body_html'] = self.env['ir.qweb']._render(
    ...
    def _partner_data_for_preview(self):
        return {
                'active': True,
                'id': 0,
                'is_follower': False,
    ...
    @api.autovacuum
    def _gc_previewed_attachments(self):
