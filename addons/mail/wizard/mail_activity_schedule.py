    ...
    def _compute_error(self):
        for scheduler in self:
            ...
            if scheduler.res_model:
                ...
                if (
                    applied_on
                    and scheduler.activity_user_id != self.env.user
                    and applied_on != applied_on._filtered_access('write')
                ):
                    errors.add(
                        self.env._(
                            "You don't have write access on this record to schedule activity for others."
                        )
                    )
