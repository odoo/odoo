...
class MailServerConfigurator(models.TransientModel):
        ...
        for link in self:
            ...
            if link.email != default_alias.default_from_email:
                is_default_warning.append(_(
                    "The current default email is %r and it will be overwritten with %r.",
                    default_alias.default_from_email,
                    link.email,
                ))
            if link.catchall_domain != default_alias.name:
                new_catchall_email = f"{default_alias.catchall_alias}@{link.catchall_domain}"
                is_default_warning.append(_(
                    "The current catchall email is %r and it will be overwritten with %r.",
                    default_alias.catchall_email,
                    new_catchall_email,
                ))
    ...
    def action_setup(self):
        ...
        if not self.email or not email_normalize(self.email):
            raise UserError(_("Invalid email: %r", self.email))

        module = self._get_module()
        if module and module.state != "installed":
            raise UserError(_("Please install %r first.", module.name))
    ...
    def _get_module(self):
        ...
        if module := self.env["ir.module.module"].search([("name", "=", module_name)], limit=1):
            return module
        raise UserError(_("The module %r is needed.", module_name))
    ...
    def _prepare_ir_mail_server_values(self):
        ...
        if not normalized_email:
            raise UserError(_("Wrong email address %r.", self.email))
