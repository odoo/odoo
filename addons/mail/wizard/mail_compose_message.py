...
class MailComposer(models.TransientModel):
    ...
    def _compute_force_send(self):
        ...
        the email queue (lot of potentially different emails to craft). Mailing
        mode depends on number of recipients, defaults to 50. Using a domain
        automatically uses the email queue as it depends on actual evaluation
        and is generally used for big batches anyway. """
        for composer in self:
            if not composer.composition_batch:
                ...
                force_send_limit = int(
                    self.env['ir.config_parameter'].sudo().get_param('mail.mail_force_send_limit')
                ) or 100
                res_ids = composer._evaluate_res_ids()
    ...
    def _action_send_mail_mass_mail(self, res_ids, auto_commit=False):
        ...
        gen_batch_size = int(self.env['ir.config_parameter'].sudo().get_param('mail.batch_size')) or self._batch_size
        for res_ids_iter in tools.split_every(gen_batch_size, res_ids):
            mail_values_all = self._prepare_mail_values(res_ids_iter)

            iter_mails_sudo = self.env['mail.mail'].sudo().create([
                mail_values for _res_id, mail_values in mail_values_all.items()
            ])
