...
class MailComposer(models.TransientModel):
    ...
    def onchange_template_id(self, template_id, composition_mode, model, res_id):
        ...
        if template_id and composition_mode == 'mass_mail':
            ...
            values['attachment_ids'] = values.get('attachment_ids', []) + attachment_ids
            if keep:
                checksum = Attachment.browse(values['attachment_ids']).mapped('checksum')
                checkdict = {res['checksum']: res['id'] for res in Attachment.search_read([('checksum','in', checksum)], ['checksum'])}
                values['attachment_ids'] = [checkdict[c] for c in checksum]
        else:
        ...
        values['attachment_ids'] = [(5,)] + values.get('attachment_ids', [])
