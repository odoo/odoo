    ...
    QUnit.module("model_field_commands", {}, function () {
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                const address10 = messaging.models["TestAddress"].findFromIdentifyingData({
                    id: 10,
                });
        ...
        QUnit.test("link: should link a record to an empty x2many field", async function (assert) {
            ...
            assert.strictEqual(contact.tasks.length, 1, "should have 1 record");
            assert.strictEqual(contact.tasks[0], task, "the record should be linked");
            assert.strictEqual(
                task.responsible,
                contact,
                "the inverse relation should be set as well"
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.strictEqual(contact.tasks.length, 2, "should have 2 records");
                assert.strictEqual(contact.tasks[0], task10, "the original record should be kept");
                assert.strictEqual(contact.tasks[1], task20, "the new record should be added");
                assert.ok(
