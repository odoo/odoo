...
(function (module, exports) {
    ...
    (function (window, undefined$1) {
        ...
        var isWindow = typeof window !== UNDEF_TYPE,
            NAVIGATOR = isWindow && window.navigator ? window.navigator : undefined$1,
            NAVIGATOR_UADATA =
                NAVIGATOR && NAVIGATOR.userAgentData ? NAVIGATOR.userAgentData : undefined$1;
...
const npm_events_package_1 = eventsExports;
const Logger_1$l = Logger$5;
const enhancedEventEmitterLogger = new Logger_1$l.Logger("EnhancedEventEmitter");
...
utils$i.clone = clone;
utils$i.generateRandomNumber = generateRandomNumber;
utils$i.deepFreeze = deepFreeze;
/**
...
const Logger_1$k = Logger$3;
const logger$k = new Logger_1$k.Logger();
/**
 * Supported profiles.
 */
var Profile;
...
function profileToString(profile) {
    switch (profile) {
        ...
        default: {
            logger$k.warn(`profileToString() | unrecognized profile ${profile}`);
            return undefined;
        }
    }
}
...
function levelToString(level) {
    switch (level) {
        ...
        default: {
            logger$k.warn(`levelToString() | unrecognized level ${level}`);
            return undefined;
        }
    }
}
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$i(result, mod, k[i]);
                    }
                }
            }
...
Object.defineProperty(ortc$e, "__esModule", { value: true });
ortc$e.validateRtpCapabilities = validateRtpCapabilities;
ortc$e.validateRtpParameters = validateRtpParameters;
ortc$e.validateSctpStreamParameters = validateSctpStreamParameters;
ortc$e.validateSctpCapabilities = validateSctpCapabilities;
ortc$e.getExtendedRtpCapabilities = getExtendedRtpCapabilities;
ortc$e.getRecvRtpCapabilities = getRecvRtpCapabilities;
ortc$e.getSendingRtpParameters = getSendingRtpParameters;
ortc$e.getSendingRemoteRtpParameters = getSendingRemoteRtpParameters;
ortc$e.reduceCodecs = reduceCodecs;
ortc$e.generateProbatorRtpParameters = generateProbatorRtpParameters;
ortc$e.canSend = canSend;
ortc$e.canReceive = canReceive;
const h264 = __importStar$i(lib$2);
const utils$h = __importStar$i(utils$i);
...
function generateProbatorRtpParameters(videoRtpParameters) {
    ...
    videoRtpParameters = utils$h.clone(videoRtpParameters);
    // This may throw.
...
const Logger_1$j = Logger$1;
const errors_1$e = errors;
const logger$j = new Logger_1$j.Logger("AwaitQueue");
class AwaitQueue {
    constructor() {
        ...
        return new Promise((resolve, reject) => {
            const pendingTask = {
                ...
                resolve: (result) => {
                    ...
                    logger$j.debug(`resolving task [name:${pendingTask.name}]`);
                ...
                reject: (error) => {
                    ...
                    // Remove the task from the queue.
                    this.pendingTasks.delete(pendingTask.id);
                    logger$j.debug(`rejecting task [name:${pendingTask.name}]: %s`, String(error));
    ...
    stop() {
        ...
        for (const pendingTask of this.pendingTasks.values()) {
            logger$j.debug(`stop() | stopping task [name:${pendingTask.name}]`);
            pendingTask.reject(new errors_1$e.AwaitQueueStoppedError());
        }
        this.stopping = false;
    }
    remove(taskIdx) {
        logger$j.debug(`remove() [taskIdx:${taskIdx}]`);
        const pendingTask = Array.from(this.pendingTasks.values())[taskIdx];
        if (!pendingTask) {
            logger$j.debug(`stop() | no task with given idx [taskIdx:${taskIdx}]`);
            return;
        }
        pendingTask.reject(new errors_1$e.AwaitQueueRemovedTaskError());
    }
    dump() {
        const now = Date.now();
        let idx = 0;
    ...
    async execute(pendingTask) {
        logger$j.debug(`execute() [name:${pendingTask.name}]`);
...
const Logger_1$i = Logger$5;
const enhancedEvents_1$6 = enhancedEvents;
const errors_1$d = errors$1;
const logger$i = new Logger_1$i.Logger("Producer");
class Producer extends enhancedEvents_1$6.EnhancedEventEmitter {
    constructor({
        id,
        localId,
        rtpSender,
        ...
        this._closed = false;
        // Observer instance.
        this._observer = new enhancedEvents_1$6.EnhancedEventEmitter();
        logger$i.debug("constructor()");
        this._id = id;
    ...
    transportClosed() {
        ...
        logger$i.debug("transportClosed()");
        this._closed = true;
        this.destroyTrack();
    ...
    async getStats() {
        if (this._closed) {
            throw new errors_1$d.InvalidStateError("closed");
        }
        return new Promise((resolve, reject) => {
     ...
     */
    pause() {
        logger$i.debug("pause()");
        if (this._closed) {
            logger$i.error("pause() | Producer closed");
            return;
        }
        this._paused = true;
     ...
     */
    resume() {
        logger$i.debug("resume()");
        if (this._closed) {
            logger$i.error("resume() | Producer closed");
            return;
        }
        this._paused = false;
        if (this._track && this._disableTrackOnPause) {
    ...
    async replaceTrack({ track }) {
        logger$i.debug("replaceTrack() [track:%o]", track);
        if (this._closed) {
            ...
            throw new errors_1$d.InvalidStateError("track ended");
        }
        // Do nothing if this is the same track as the current handled one.
        if (track === this._track) {
            logger$i.debug("replaceTrack() | same track, ignored");
            return;
        }
    ...
    async setMaxSpatialLayer(spatialLayer) {
        if (this._closed) {
            ...
            throw new errors_1$d.UnsupportedError("not a video Producer");
    ...
    onTrackEnded() {
        logger$i.debug('track "ended" event');
...
const Logger_1$h = Logger$5;
const enhancedEvents_1$5 = enhancedEvents;
const errors_1$c = errors$1;
const logger$h = new Logger_1$h.Logger("Consumer");
class Consumer extends enhancedEvents_1$5.EnhancedEventEmitter {
    constructor({ id, localId, producerId, rtpReceiver, track, rtpParameters, appData }) {
        super();
        // Closed flag.
        this._closed = false;
        // Observer instance.
        this._observer = new enhancedEvents_1$5.EnhancedEventEmitter();
    ...
    pause() {
        ...
        if (this._closed) {
            logger$h.error("pause() | Consumer closed");
            return;
        }
        if (this._paused) {
            logger$h.debug("pause() | Consumer is already paused");
            return;
        }
        this._paused = true;
        this._track.enabled = false;
    ...
    resume() {
        ...
        if (this._closed) {
            logger$h.error("resume() | Consumer closed");
            return;
        }
        if (!this._paused) {
            logger$h.debug("resume() | Consumer is already resumed");
            return;
        }
        this._paused = false;
        this._track.enabled = true;
...
const Logger_1$g = Logger$5;
const enhancedEvents_1$4 = enhancedEvents;
const errors_1$b = errors$1;
const logger$g = new Logger_1$g.Logger("DataProducer");
class DataProducer extends enhancedEvents_1$4.EnhancedEventEmitter {
    constructor({ id, dataChannel, sctpStreamParameters, appData }) {
        super();
        // Closed flag.
        this._closed = false;
        // Observer instance.
        this._observer = new enhancedEvents_1$4.EnhancedEventEmitter();
    ...
    handleDataChannel() {
        this._dataChannel.addEventListener("open", () => {
            ...
            logger$g.debug('DataChannel "open" event');
            this.safeEmit("open");
        });
        this._dataChannel.addEventListener("error", (event) => {
            ...
            if (error.errorDetail === "sctp-failure") {
                ...
                logger$g.error('DataChannel "error" event: %o', error);
            }
        ...
        this._dataChannel.addEventListener("close", () => {
            ...
            logger$g.warn('DataChannel "close" event');
            this._closed = true;
            this.emit("@close");
            this.safeEmit("close");
            // Emit observer event.
        ...
        this._dataChannel.addEventListener("message", () => {
            ...
            logger$g.warn('DataChannel "message" event in a DataProducer, message discarded');
        });
...
const logger$f = new Logger_1$f.Logger("DataConsumer");
class DataConsumer extends enhancedEvents_1$3.EnhancedEventEmitter {
    constructor({ id, dataProducerId, dataChannel, sctpStreamParameters, appData }) {
        super();
        // Closed flag.
        this._closed = false;
        // Observer instance.
        this._observer = new enhancedEvents_1$3.EnhancedEventEmitter();
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$h(result, mod, k[i]);
                    }
                }
            }
...
const Logger_1$e = Logger$5;
const enhancedEvents_1$2 = enhancedEvents;
const errors_1$a = errors$1;
const utils$g = __importStar$h(utils$i);
const ortc$d = __importStar$h(ortc$e);
...
const logger$e = new Logger_1$e.Logger("Transport");
...
class Transport extends enhancedEvents_1$2.EnhancedEventEmitter {
    constructor({
        direction,
        id,
        iceParameters,
        ...
        // Observer instance.
        this._observer = new enhancedEvents_1$2.EnhancedEventEmitter();
        logger$e.debug("constructor() [id:%s, direction:%s]", id, direction);
        this._id = id;
        ...
        const clonedAdditionalSettings = utils$g.clone(additionalSettings) ?? {};
    ...
    async updateIceServers({ iceServers } = {}) {
        logger$e.debug("updateIceServers()");
        if (this._closed) {
            throw new errors_1$a.InvalidStateError("closed");
    ...
    async produce({
        ...
        logger$e.debug("produce() [track:%o]", track);
        if (this._closed) {
            throw new errors_1$a.InvalidStateError("closed");
        } else if (!track) {
            throw new TypeError("missing track");
        } else if (this._direction !== "send") {
            throw new errors_1$a.UnsupportedError("not a sending Transport");
        } else if (!this._canProduceByKind[track.kind]) {
            throw new errors_1$a.UnsupportedError(`cannot produce ${track.kind}`);
            ...
            this._awaitQueue
                .push(async () => {
                    ...
                    try {
                        ...
                        ortc$d.validateRtpParameters(rtpParameters);
    ...
    async consume({ id, producerId, kind, rtpParameters, streamId, onRtpReceiver, appData = {} }) {
        logger$e.debug("consume()");
        if (this._closed) {
            throw new errors_1$a.InvalidStateError("closed");
        } else if (this._direction !== "recv") {
            throw new errors_1$a.UnsupportedError("not a receiving Transport");
        ...
        const clonedRtpParameters = utils$g.clone(rtpParameters);
        // Ensure the device can consume it.
        const canConsume = ortc$d.canReceive(clonedRtpParameters, this._extendedRtpCapabilities);
        if (!canConsume) {
            throw new errors_1$a.UnsupportedError("cannot consume this Producer");
        }
    ...
    async produceData({
        ...
        logger$e.debug("produceData()");
        if (this._closed) {
            throw new errors_1$a.InvalidStateError("closed");
        } else if (this._direction !== "send") {
            throw new errors_1$a.UnsupportedError("not a sending Transport");
        } else if (!this._maxSctpMessageSize) {
            throw new errors_1$a.UnsupportedError("SCTP not enabled by remote Transport");
        } else if (this.listenerCount("connect") === 0 && this._connectionState === "new") {
            throw new TypeError('no "connect" listener set into this transport');
        ...
        return this._awaitQueue.push(async () => {
            ...
            ortc$d.validateSctpStreamParameters(sctpStreamParameters);
            const { id } = await new Promise((resolve, reject) => {
                this.safeEmit(
                    "producedata",
                    {
    ...
    async consumeData({
        ...
        const clonedSctpStreamParameters = utils$g.clone(sctpStreamParameters);
        // This may throw.
        ortc$d.validateSctpStreamParameters(clonedSctpStreamParameters);
        // Enqueue command.
        return this._awaitQueue.push(async () => {
        ...
        this._awaitQueue
            .push(async () => {
                if (this._pendingConsumerTasks.length === 0) {
                    logger$e.debug("createPendingConsumers() | there is no Consumer to be created");
                    return;
                }
                ...
                if (videoConsumerForProbator) {
                    try {
                        const probatorRtpParameters = ortc$d.generateProbatorRtpParameters(
        ...
        this._awaitQueue
            .push(async () => {
                if (this._pendingPauseConsumers.size === 0) {
                    logger$e.debug("pausePendingConsumers() | there is no Consumer to be paused");
                    return;
                }
                ...
                try {
                    ...
                    logger$e.error("pausePendingConsumers() | failed to pause Consumers:", error);
                }
        ...
        this._awaitQueue
            .push(async () => {
                if (this._pendingResumeConsumers.size === 0) {
                    logger$e.debug("resumePendingConsumers() | there is no Consumer to be resumed");
                    return;
                }
                ...
                try {
                    ...
                    logger$e.error("resumePendingConsumers() | failed to resume Consumers:", error);
                }
        ...
        this._awaitQueue
            .push(async () => {
                if (this._pendingCloseConsumers.size === 0) {
                    logger$e.debug("closePendingConsumers() | there is no Consumer to be closed");
                    return;
                }
                ...
                try {
                    ...
                    logger$e.error("closePendingConsumers() | failed to close Consumers:", error);
                }
    ...
    handleHandler() {
        ...
        handler.on("@connect", ({ dtlsParameters }, callback, errback) => {
            if (this._closed) {
                errback(new errors_1$a.InvalidStateError("closed"));
                return;
            }
        ...
        handler.on("@icegatheringstatechange", (iceGatheringState) => {
            ...
            logger$e.debug("ICE gathering state changed to %s", iceGatheringState);
        ...
        handler.on("@connectionstatechange", (connectionState) => {
            ...
            logger$e.debug("connection state changed to %s", connectionState);
    ...
    handleProducer(producer) {
        producer.on("@close", () => {
            ...
            this._awaitQueue
                ...
                .catch((error) => logger$e.warn("producer.close() failed:%o", error));
        });
        ...
        producer.on("@getstats", (callback, errback) => {
            if (this._closed) {
                return errback(new errors_1$a.InvalidStateError("closed"));
            }
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$g(result, mod, k[i]);
                    }
                }
            }
...
const sdpTransform$d = __importStar$g(lib);
/**
...
function extractRtpCapabilities({ sdpObject }) {
    ...
    for (const m of sdpObject.media) {
        ...
        for (const fmtp of m.fmtp ?? []) {
            const parameters = sdpTransform$d.parseParams(fmtp.config);
...
utils$f.addNackSupportForOpus = addNackSupportForOpus;
/**
...
class HandlerInterface extends enhancedEvents_1$1.EnhancedEventEmitter {
    constructor() {
        super();
    }
}
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$f(result, mod, k[i]);
                    }
                }
            }
...
MediaSection$1.OfferMediaSection =
    MediaSection$1.AnswerMediaSection =
    MediaSection$1.MediaSection =
        void 0;
const sdpTransform$c = __importStar$f(lib);
const utils$e = __importStar$f(utils$i);
class MediaSection {
...
class AnswerMediaSection extends MediaSection {
        ...
        switch (offerMediaObject.type) {
            ...
            case "video": {
                ...
                for (const codec of answerRtpParameters.codecs) {
                    ...
                    const codecParameters = utils$e.clone(codec.parameters) ?? {};
                    let codecRtcpFeedback = utils$e.clone(codec.rtcpFeedback) ?? [];
                    if (codecOptions) {
                        const {
                            opusStereo,
                            opusFec,
    ...
    muxSimulcastStreams(encodings) {
        ...
        const simulcastStreams = sdpTransform$c.parseSimulcastStreamList(raw);
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$e(result, mod, k[i]);
                    }
                }
            }
...
const Logger_1$d = Logger$5;
const MediaSection_1 = MediaSection$1;
const logger$d = new Logger_1$d.Logger("RemoteSdp");
class RemoteSdp {
    constructor({
        iceParameters,
        iceCandidates,
    ...
    updateIceParameters(iceParameters) {
        logger$d.debug("updateIceParameters() [iceParameters:%o]", iceParameters);
        this._iceParameters = iceParameters;
    ...
    updateDtlsRole(role) {
        logger$d.debug("updateDtlsRole() [role:%s]", role);
    ...
    getSdp() {
        ...
        return sdpTransform$b.write(this._sdpObject);
    }
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$d(result, mod, k[i]);
                    }
                }
            }
...
const utils$d = __importStar$d(utils$i);
const ortc$c = __importStar$d(ortc$e);
const sdpCommonUtils$a = __importStar$d(commonUtils);
const sdpUnifiedPlanUtils$6 = __importStar$d(unifiedPlanUtils);
const ortcUtils$3 = __importStar$d(utils$f);
const errors_1$9 = errors$1;
const HandlerInterface_1$b = HandlerInterface$1;
const RemoteSdp_1$a = RemoteSdp$1;
const scalabilityModes_1$6 = scalabilityModes;
const logger$c = new Logger_1$c.Logger("Chrome111");
const NAME$b = "Chrome111";
const SCTP_NUM_STREAMS$a = { OS: 1024, MIS: 1024 };
class Chrome111 extends HandlerInterface_1$b.HandlerInterface {
    /**
    ...
    async getNativeRtpCapabilities() {
        logger$c.debug("getNativeRtpCapabilities()");
        ...
        try {
            ...
            const sdpObject = sdpTransform$a.parse(offer.sdp);
            const nativeRtpCapabilities = sdpCommonUtils$a.extractRtpCapabilities({
                sdpObject,
            });
    ...
    async getNativeSctpCapabilities() {
        logger$c.debug("getNativeSctpCapabilities()");
        return {
            numStreams: SCTP_NUM_STREAMS$a,
        };
    }
    run({
        direction,
        ...
        logger$c.debug("run()");
        this._direction = direction;
        this._remoteSdp = new RemoteSdp_1$a.RemoteSdp({
            iceParameters,
            iceCandidates,
            dtlsParameters,
            sctpParameters,
        ...
        this._sendingRtpParametersByKind = {
            audio: ortc$c.getSendingRtpParameters("audio", extendedRtpCapabilities),
            video: ortc$c.getSendingRtpParameters("video", extendedRtpCapabilities),
        };
        this._sendingRemoteRtpParametersByKind = {
            audio: ortc$c.getSendingRemoteRtpParameters("audio", extendedRtpCapabilities),
            video: ortc$c.getSendingRemoteRtpParameters("video", extendedRtpCapabilities),
        };
        ...
        if (this._pc.connectionState) {
            ...
            logger$c.warn("run() | pc.connectionState not supported, using pc.iceConnectionState");
    ...
    async updateIceServers(iceServers) {
        this.assertNotClosed();
        logger$c.debug("updateIceServers()");
    ...
    async restartIce(iceParameters) {
        ...
        if (this._direction === "send") {
            ...
            logger$c.debug("restartIce() | calling pc.setLocalDescription() [offer:%o]", offer);
            await this._pc.setLocalDescription(offer);
            const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
            logger$c.debug("restartIce() | calling pc.setRemoteDescription() [answer:%o]", answer);
            await this._pc.setRemoteDescription(answer);
        } else {
            const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
            logger$c.debug("restartIce() | calling pc.setRemoteDescription() [offer:%o]", offer);
            await this._pc.setRemoteDescription(offer);
            const answer = await this._pc.createAnswer();
            logger$c.debug("restartIce() | calling pc.setLocalDescription() [answer:%o]", answer);
    ...
    async send({ track, encodings, codecOptions, codec, onRtpSender }) {
        ...
        logger$c.debug("send() [kind:%s, track.id:%s]", track.kind, track.id);
        if (encodings && encodings.length > 1) {
            ...
            for (const encoding of encodings) {
                const temporalLayers = encoding.scalabilityMode
                    ? (0, scalabilityModes_1$6.parse)(encoding.scalabilityMode).temporalLayers
                    : 3;
        ...
        const sendingRtpParameters = utils$d.clone(this._sendingRtpParametersByKind[track.kind]);
        // This may throw.
        sendingRtpParameters.codecs = ortc$c.reduceCodecs(sendingRtpParameters.codecs, codec);
        const sendingRemoteRtpParameters = utils$d.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$c.reduceCodecs(
        ...
        let localSdpObject = sdpTransform$a.parse(offer.sdp);
        if (!this._transportReady) {
        ...
        logger$c.debug("send() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        // We can now get the transceiver.mid.
        const localId = transceiver.mid;
        // Set MID.
        sendingRtpParameters.mid = localId;
        localSdpObject = sdpTransform$a.parse(this._pc.localDescription.sdp);
        const offerMediaObject = localSdpObject.media[mediaSectionIdx.idx];
        // Set RTCP CNAME.
        sendingRtpParameters.rtcp.cname = sdpCommonUtils$a.getCname({
            offerMediaObject,
        });
        // Set RTP encodings by parsing the SDP offer if no encodings are given.
        if (!encodings) {
            sendingRtpParameters.encodings = sdpUnifiedPlanUtils$6.getRtpEncodings({
                offerMediaObject,
            });
        }
        ...
        // one if just a single encoding has been given.
            ...
            const newEncodings = sdpUnifiedPlanUtils$6.getRtpEncodings({
                offerMediaObject,
            });
        ...
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("send() | calling pc.setRemoteDescription() [answer:%o]", answer);
        await this._pc.setRemoteDescription(answer);
        // Store in the map.
    ...
    async stopSending(localId) {
        this.assertSendDirection();
        logger$c.debug("stopSending() [localId:%s]", localId);
        if (this._closed) {
            return;
        }
        ...
        const offer = await this._pc.createOffer();
        logger$c.debug("stopSending() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("stopSending() | calling pc.setRemoteDescription() [answer:%o]", answer);
        await this._pc.setRemoteDescription(answer);
    ...
    async pauseSending(localId) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$c.debug("pauseSending() [localId:%s]", localId);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
        }
        ...
        const offer = await this._pc.createOffer();
        logger$c.debug("pauseSending() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("pauseSending() | calling pc.setRemoteDescription() [answer:%o]", answer);
        await this._pc.setRemoteDescription(answer);
    }
    async resumeSending(localId) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$c.debug("resumeSending() [localId:%s]", localId);
        const transceiver = this._mapMidTransceiver.get(localId);
        ...
        const offer = await this._pc.createOffer();
        logger$c.debug("resumeSending() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("resumeSending() | calling pc.setRemoteDescription() [answer:%o]", answer);
        await this._pc.setRemoteDescription(answer);
    }
    async replaceTrack(localId, track) {
        ...
        this.assertSendDirection();
        if (track) {
            logger$c.debug("replaceTrack() [localId:%s, track.id:%s]", localId, track.id);
        } else {
            logger$c.debug("replaceTrack() [localId:%s, no track]", localId);
        }
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
    ...
    async setMaxSpatialLayer(localId, spatialLayer) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$c.debug("setMaxSpatialLayer() [localId:%s, spatialLayer:%s]", localId, spatialLayer);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
        }
        ...
        const offer = await this._pc.createOffer();
        logger$c.debug("setMaxSpatialLayer() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug(
    ...
    async setRtpEncodingParameters(localId, params) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$c.debug("setRtpEncodingParameters() [localId:%s, params:%o]", localId, params);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
        }
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        logger$c.debug("sendDataChannel() [options:%o]", options);
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$a.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
            const localSdpObject = sdpTransform$a.parse(offer.sdp);
    ...
    async receive(optionsList) {
        ...
        for (const options of optionsList) {
            ...
            logger$c.debug("receive() [trackId:%s, kind:%s]", trackId, kind);
        ...
        const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("receive() | calling pc.setRemoteDescription() [offer:%o]", offer);
        await this._pc.setRemoteDescription(offer);
        for (const options of optionsList) {
        ...
        const localSdpObject = sdpTransform$a.parse(answer.sdp);
        for (const options of optionsList) {
            ...
            sdpCommonUtils$a.applyCodecParameters({
                offerRtpParameters: rtpParameters,
                answerMediaObject,
            });
        }
        answer = { type: "answer", sdp: sdpTransform$a.write(localSdpObject) };
        if (!this._transportReady) {
            await this.setupTransport({
                localDtlsRole: this._forcedLocalDtlsRole ?? "client",
                localSdpObject,
        ...
        logger$c.debug("receive() | calling pc.setLocalDescription() [answer:%o]", answer);
        await this._pc.setLocalDescription(answer);
        for (const options of optionsList) {
    ...
    async stopReceiving(localIds) {
        ...
        for (const localId of localIds) {
            logger$c.debug("stopReceiving() [localId:%s]", localId);
            const transceiver = this._mapMidTransceiver.get(localId);
            if (!transceiver) {
                throw new Error("associated RTCRtpTransceiver not found");
            }
        ...
        const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("stopReceiving() | calling pc.setRemoteDescription() [offer:%o]", offer);
        await this._pc.setRemoteDescription(offer);
        const answer = await this._pc.createAnswer();
        logger$c.debug("stopReceiving() | calling pc.setLocalDescription() [answer:%o]", answer);
        await this._pc.setLocalDescription(answer);
        for (const localId of localIds) {
            this._mapMidTransceiver.delete(localId);
        }
    ...
    async pauseReceiving(localIds) {
        ...
        this.assertRecvDirection();
        for (const localId of localIds) {
            logger$c.debug("pauseReceiving() [localId:%s]", localId);
            const transceiver = this._mapMidTransceiver.get(localId);
            if (!transceiver) {
                throw new Error("associated RTCRtpTransceiver not found");
            }
        ...
        const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("pauseReceiving() | calling pc.setRemoteDescription() [offer:%o]", offer);
        await this._pc.setRemoteDescription(offer);
        const answer = await this._pc.createAnswer();
        logger$c.debug("pauseReceiving() | calling pc.setLocalDescription() [answer:%o]", answer);
        await this._pc.setLocalDescription(answer);
    }
    async resumeReceiving(localIds) {
        ...
        this.assertRecvDirection();
        for (const localId of localIds) {
            logger$c.debug("resumeReceiving() [localId:%s]", localId);
            const transceiver = this._mapMidTransceiver.get(localId);
            if (!transceiver) {
                throw new Error("associated RTCRtpTransceiver not found");
            }
        ...
        const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
        logger$c.debug("resumeReceiving() | calling pc.setRemoteDescription() [offer:%o]", offer);
        await this._pc.setRemoteDescription(offer);
        const answer = await this._pc.createAnswer();
        logger$c.debug("resumeReceiving() | calling pc.setLocalDescription() [answer:%o]", answer);
        await this._pc.setLocalDescription(answer);
    }
    ...
    async receiveDataChannel({ sctpStreamParameters, label, protocol }) {
        ...
        logger$c.debug("receiveDataChannel() [options:%o]", options);
        const dataChannel = this._pc.createDataChannel(label, options);
    ...
    async setupTransport({ localDtlsRole, localSdpObject }) {
        ...
        const dtlsParameters = sdpCommonUtils$a.extractDtlsParameters({
    ...
    assertNotClosed() {
        if (this._closed) {
            throw new errors_1$9.InvalidStateError("method called in a closed handler");
        }
    }
    assertSendDirection() {
        if (this._direction !== "send") {
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$c(result, mod, k[i]);
                    }
                }
            }
...
const utils$c = __importStar$c(utils$i);
const ortc$b = __importStar$c(ortc$e);
const sdpCommonUtils$9 = __importStar$c(commonUtils);
const sdpUnifiedPlanUtils$5 = __importStar$c(unifiedPlanUtils);
const ortcUtils$2 = __importStar$c(utils$f);
...
const logger$b = new Logger_1$b.Logger("Chrome74");
const NAME$a = "Chrome74";
const SCTP_NUM_STREAMS$9 = { OS: 1024, MIS: 1024 };
class Chrome74 extends HandlerInterface_1$a.HandlerInterface {
    /**
     * Creates a factory function.
     */
    static createFactory() {
    ...
    async getNativeRtpCapabilities() {
        ...
        try {
            ...
            const nativeRtpCapabilities = sdpCommonUtils$9.extractRtpCapabilities({
                sdpObject,
            });
            // libwebrtc supports NACK for OPUS but doesn't announce it.
            ortcUtils$2.addNackSupportForOpus(nativeRtpCapabilities);
            return nativeRtpCapabilities;
        } catch (error) {
            try {
                pc.close();
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        // This may throw.
        sendingRtpParameters.codecs = ortc$b.reduceCodecs(sendingRtpParameters.codecs, codec);
        const sendingRemoteRtpParameters = utils$c.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$b.reduceCodecs(
            sendingRemoteRtpParameters.codecs,
            codec
        );
        const mediaSectionIdx = this._remoteSdp.getNextMediaSectionIdx();
        ...
        const layers = (0, scalabilityModes_1$5.parse)((encodings ?? [{}])[0].scalabilityMode);
        if (
            encodings &&
            encodings.length === 1 &&
            ...
            logger$b.debug("send() | enabling legacy simulcast for VP9 SVC");
            hackVp9Svc = true;
            localSdpObject = sdpTransform$9.parse(offer.sdp);
            offerMediaObject = localSdpObject.media[mediaSectionIdx.idx];
            sdpUnifiedPlanUtils$5.addLegacySimulcast({
                offerMediaObject,
                numStreams: layers.spatialLayers,
            });
            offer = { type: "offer", sdp: sdpTransform$9.write(localSdpObject) };
        }
        ...
        offerMediaObject = localSdpObject.media[mediaSectionIdx.idx];
        // Set RTCP CNAME.
        sendingRtpParameters.rtcp.cname = sdpCommonUtils$9.getCname({
            offerMediaObject,
        });
        // Set RTP encodings by parsing the SDP offer if no encodings are given.
        if (!encodings) {
            sendingRtpParameters.encodings = sdpUnifiedPlanUtils$5.getRtpEncodings({
                offerMediaObject,
            });
        }
        // Set RTP encodings by parsing the SDP offer and complete them with given
        // one if just a single encoding has been given.
            ...
            let newEncodings = sdpUnifiedPlanUtils$5.getRtpEncodings({
                offerMediaObject,
            });
            Object.assign(newEncodings[0], encodings[0]);
            // Hack for VP9 SVC.
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$9.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    async setupTransport({ localDtlsRole, localSdpObject }) {
        ...
        // Get our local DTLS parameters.
        const dtlsParameters = sdpCommonUtils$9.extractDtlsParameters({
            sdpObject: localSdpObject,
        });
        // Set our DTLS role.
        dtlsParameters.role = localDtlsRole;
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$b(result, mod, k[i]);
                    }
                }
            }
...
const utils$b = __importStar$b(utils$i);
const ortc$a = __importStar$b(ortc$e);
...
const logger$a = new Logger_1$a.Logger("Chrome70");
const NAME$9 = "Chrome70";
const SCTP_NUM_STREAMS$8 = { OS: 1024, MIS: 1024 };
class Chrome70 extends HandlerInterface_1$9.HandlerInterface {
    /**
     * Creates a factory function.
     */
    static createFactory() {
    ...
    async getNativeRtpCapabilities() {
        ...
        try {
            ...
            const nativeRtpCapabilities = sdpCommonUtils$8.extractRtpCapabilities({
                sdpObject,
            });
            return nativeRtpCapabilities;
        } catch (error) {
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        // This may throw.
        sendingRtpParameters.codecs = ortc$a.reduceCodecs(sendingRtpParameters.codecs, codec);
        const sendingRemoteRtpParameters = utils$b.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$a.reduceCodecs(
            sendingRemoteRtpParameters.codecs,
            codec
        );
        const mediaSectionIdx = this._remoteSdp.getNextMediaSectionIdx();
        ...
        if (encodings && encodings.length > 1) {
            logger$a.debug("send() | enabling legacy simulcast");
        ...
        if (encodings) {
            logger$a.debug("send() | applying given encodings");
            const parameters = transceiver.sender.getParameters();
        ...
        offerMediaObject = localSdpObject.media[mediaSectionIdx.idx];
        // Set RTCP CNAME.
        sendingRtpParameters.rtcp.cname = sdpCommonUtils$8.getCname({
            offerMediaObject,
        });
        // Set RTP encodings.
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$8.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    async setupTransport({ localDtlsRole, localSdpObject }) {
        ...
        // Get our local DTLS parameters.
        const dtlsParameters = sdpCommonUtils$8.extractDtlsParameters({
            sdpObject: localSdpObject,
        });
        // Set our DTLS role.
        dtlsParameters.role = localDtlsRole;
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$a(result, mod, k[i]);
                    }
                }
            }
...
const utils$a = __importStar$a(utils$i);
const ortc$9 = __importStar$a(ortc$e);
const sdpCommonUtils$7 = __importStar$a(commonUtils);
const sdpPlanBUtils$3 = __importStar$a(planBUtils);
const HandlerInterface_1$8 = HandlerInterface$1;
const RemoteSdp_1$7 = RemoteSdp$1;
const logger$9 = new Logger_1$9.Logger("Chrome67");
const NAME$8 = "Chrome67";
const SCTP_NUM_STREAMS$7 = { OS: 1024, MIS: 1024 };
class Chrome67 extends HandlerInterface_1$8.HandlerInterface {
    /**
     * Creates a factory function.
     */
    static createFactory() {
    ...
    async getNativeRtpCapabilities() {
        ...
        try {
            ...
            const nativeRtpCapabilities = sdpCommonUtils$7.extractRtpCapabilities({
                sdpObject,
            });
            return nativeRtpCapabilities;
        } catch (error) {
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        if (codec) {
            logger$9.warn("send() | codec selection is not available in %s handler", this.name);
        }
        ...
        sendingRtpParameters.codecs = ortc$9.reduceCodecs(sendingRtpParameters.codecs);
        const sendingRemoteRtpParameters = utils$a.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        sendingRemoteRtpParameters.codecs = ortc$9.reduceCodecs(sendingRemoteRtpParameters.codecs);
        if (!this._transportReady) {
            await this.setupTransport({
                localDtlsRole: this._forcedLocalDtlsRole ?? "client",
                localSdpObject,
        ...
        if (track.kind === "video" && encodings && encodings.length > 1) {
            logger$9.debug("send() | enabling simulcast");
        ...
        // Set RTCP CNAME.
        sendingRtpParameters.rtcp.cname = sdpCommonUtils$7.getCname({
            offerMediaObject,
        });
        // Set RTP encodings.
        sendingRtpParameters.encodings = sdpPlanBUtils$3.getRtpEncodings({
            offerMediaObject,
            track,
        });
        // Complete encodings with given values.
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$7.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    async setupTransport({ localDtlsRole, localSdpObject }) {
        ...
        // Get our local DTLS parameters.
        const dtlsParameters = sdpCommonUtils$7.extractDtlsParameters({
            sdpObject: localSdpObject,
        });
        // Set our DTLS role.
        dtlsParameters.role = localDtlsRole;
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$9(result, mod, k[i]);
                    }
                }
            }
...
const utils$9 = __importStar$9(utils$i);
const ortc$8 = __importStar$9(ortc$e);
const sdpCommonUtils$6 = __importStar$9(commonUtils);
const sdpPlanBUtils$2 = __importStar$9(planBUtils);
const HandlerInterface_1$7 = HandlerInterface$1;
const RemoteSdp_1$6 = RemoteSdp$1;
const logger$8 = new Logger_1$8.Logger("Chrome55");
const NAME$7 = "Chrome55";
const SCTP_NUM_STREAMS$6 = { OS: 1024, MIS: 1024 };
class Chrome55 extends HandlerInterface_1$7.HandlerInterface {
    /**
     * Creates a factory function.
     */
    static createFactory() {
    ...
    async getNativeRtpCapabilities() {
        ...
        try {
            ...
            const nativeRtpCapabilities = sdpCommonUtils$6.extractRtpCapabilities({
                sdpObject,
            });
            return nativeRtpCapabilities;
        } catch (error) {
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        sendingRtpParameters.codecs = ortc$8.reduceCodecs(sendingRtpParameters.codecs);
        const sendingRemoteRtpParameters = utils$9.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        sendingRemoteRtpParameters.codecs = ortc$8.reduceCodecs(sendingRemoteRtpParameters.codecs);
        if (!this._transportReady) {
            await this.setupTransport({
                localDtlsRole: this._forcedLocalDtlsRole ?? "client",
                localSdpObject,
        ...
        offerMediaObject = localSdpObject.media.find((m) => m.type === track.kind);
        // Set RTCP CNAME.
        sendingRtpParameters.rtcp.cname = sdpCommonUtils$6.getCname({
            offerMediaObject,
        });
        // Set RTP encodings.
        sendingRtpParameters.encodings = sdpPlanBUtils$2.getRtpEncodings({
            offerMediaObject,
            track,
        });
        // Complete encodings with given values.
    ...
    async replaceTrack(
        ...
        throw new errors_1$7.UnsupportedError("not implemented");
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    async setRtpEncodingParameters(localId, params) {
        throw new errors_1$7.UnsupportedError("not supported");
    }
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    async getSenderStats(localId) {
        throw new errors_1$7.UnsupportedError("not implemented");
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$6.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    async setupTransport({ localDtlsRole, localSdpObject }) {
        ...
        // Get our local DTLS parameters.
        const dtlsParameters = sdpCommonUtils$6.extractDtlsParameters({
            sdpObject: localSdpObject,
        });
        // Set our DTLS role.
        dtlsParameters.role = localDtlsRole;
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$8(result, mod, k[i]);
                    }
                }
            }
...
const utils$8 = __importStar$8(utils$i);
const ortc$7 = __importStar$8(ortc$e);
const sdpCommonUtils$5 = __importStar$8(commonUtils);
const sdpUnifiedPlanUtils$3 = __importStar$8(unifiedPlanUtils);
const HandlerInterface_1$6 = HandlerInterface$1;
const RemoteSdp_1$5 = RemoteSdp$1;
const scalabilityModes_1$3 = scalabilityModes;
const logger$7 = new Logger_1$7.Logger("Firefox120");
const NAME$6 = "Firefox120";
const SCTP_NUM_STREAMS$5 = { OS: 16, MIS: 2048 };
class Firefox120 extends HandlerInterface_1$6.HandlerInterface {
    /**
     * Creates a factory function.
     */
    static createFactory() {
    ...
    async getNativeRtpCapabilities() {
        ...
        try {
            ...
            const nativeRtpCapabilities = sdpCommonUtils$5.extractRtpCapabilities({
                sdpObject,
            });
            return nativeRtpCapabilities;
        } catch (error) {
    ...
    async send({ track, encodings, codecOptions, codec, onRtpSender }) {
        ...
        // This may throw.
        sendingRtpParameters.codecs = ortc$7.reduceCodecs(sendingRtpParameters.codecs, codec);
        const sendingRemoteRtpParameters = utils$8.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$7.reduceCodecs(
            sendingRemoteRtpParameters.codecs,
            codec
        );
        ...
        // Set RTCP CNAME.
        sendingRtpParameters.rtcp.cname = sdpCommonUtils$5.getCname({
            offerMediaObject,
        });
        // Set RTP encodings by parsing the SDP offer if no encodings are given.
        if (!encodings) {
        ...
        // one if just a single encoding has been given.
            ...
            const newEncodings = sdpUnifiedPlanUtils$3.getRtpEncodings({
                offerMediaObject,
            });
            Object.assign(newEncodings[0], encodings[0]);
            sendingRtpParameters.encodings = newEncodings;
    ...
    async pauseSending(localId) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$7.debug("pauseSending() [localId:%s]", localId);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
        }
        ...
        this._remoteSdp.pauseMediaSection(localId);
        const offer = await this._pc.createOffer();
        logger$7.debug("pauseSending() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$7.debug("pauseSending() | calling pc.setRemoteDescription() [answer:%o]", answer);
        await this._pc.setRemoteDescription(answer);
    }
    async resumeSending(localId) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$7.debug("resumeSending() [localId:%s]", localId);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
        }
        ...
        this._remoteSdp.resumeSendingMediaSection(localId);
        const offer = await this._pc.createOffer();
        logger$7.debug("resumeSending() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$7.debug("resumeSending() | calling pc.setRemoteDescription() [answer:%o]", answer);
        await this._pc.setRemoteDescription(answer);
    }
    async replaceTrack(localId, track) {
        this.assertNotClosed();
        this.assertSendDirection();
        if (track) {
            logger$7.debug("replaceTrack() [localId:%s, track.id:%s]", localId, track.id);
        } else {
            logger$7.debug("replaceTrack() [localId:%s, no track]", localId);
        }
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
    ...
    async setMaxSpatialLayer(localId, spatialLayer) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$7.debug("setMaxSpatialLayer() [localId:%s, spatialLayer:%s]", localId, spatialLayer);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated transceiver not found");
        }
        ...
        this._remoteSdp.muxMediaSectionSimulcast(localId, parameters.encodings);
        const offer = await this._pc.createOffer();
        logger$7.debug("setMaxSpatialLayer() | calling pc.setLocalDescription() [offer:%o]", offer);
        await this._pc.setLocalDescription(offer);
        const answer = { type: "answer", sdp: this._remoteSdp.getSdp() };
        logger$7.debug(
            "setMaxSpatialLayer() | calling pc.setRemoteDescription() [answer:%o]",
    ...
    async setRtpEncodingParameters(localId, params) {
        this.assertNotClosed();
        this.assertSendDirection();
        logger$7.debug("setRtpEncodingParameters() [localId:%s, params:%o]", localId, params);
        const transceiver = this._mapMidTransceiver.get(localId);
        if (!transceiver) {
            throw new Error("associated RTCRtpTransceiver not found");
        }
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$5.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    async pauseReceiving(localIds) {
        ...
        this.assertRecvDirection();
        for (const localId of localIds) {
            logger$7.debug("pauseReceiving() [localId:%s]", localId);
            const transceiver = this._mapMidTransceiver.get(localId);
            if (!transceiver) {
                throw new Error("associated RTCRtpTransceiver not found");
            }
        ...
        const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
        logger$7.debug("pauseReceiving() | calling pc.setRemoteDescription() [offer:%o]", offer);
        await this._pc.setRemoteDescription(offer);
        const answer = await this._pc.createAnswer();
        logger$7.debug("pauseReceiving() | calling pc.setLocalDescription() [answer:%o]", answer);
        await this._pc.setLocalDescription(answer);
    }
    async resumeReceiving(localIds) {
        this.assertNotClosed();
        this.assertRecvDirection();
        for (const localId of localIds) {
            logger$7.debug("resumeReceiving() [localId:%s]", localId);
            const transceiver = this._mapMidTransceiver.get(localId);
            if (!transceiver) {
                throw new Error("associated RTCRtpTransceiver not found");
            }
        ...
        const offer = { type: "offer", sdp: this._remoteSdp.getSdp() };
        logger$7.debug("resumeReceiving() | calling pc.setRemoteDescription() [offer:%o]", offer);
        await this._pc.setRemoteDescription(offer);
        const answer = await this._pc.createAnswer();
        logger$7.debug("resumeReceiving() | calling pc.setLocalDescription() [answer:%o]", answer);
        await this._pc.setLocalDescription(answer);
    }
    async getReceiverStats(localId) {
        this.assertRecvDirection();
    ...
    async setupTransport({ localDtlsRole, localSdpObject }) {
        ...
        // Get our local DTLS parameters.
        const dtlsParameters = sdpCommonUtils$5.extractDtlsParameters({
            sdpObject: localSdpObject,
        });
        // Set our DTLS role.
        dtlsParameters.role = localDtlsRole;
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$7(result, mod, k[i]);
                    }
                }
            }
...
const utils$7 = __importStar$7(utils$i);
const ortc$6 = __importStar$7(ortc$e);
...
class Firefox60 extends HandlerInterface_1$5.HandlerInterface {
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        const sendingRemoteRtpParameters = utils$7.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$6.reduceCodecs(
            sendingRemoteRtpParameters.codecs,
            codec
        );
        // NOTE: Firefox fails sometimes to properly anticipate the closed media
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$4.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$6(result, mod, k[i]);
                    }
                }
            }
...
const utils$6 = __importStar$6(utils$i);
const ortc$5 = __importStar$6(ortc$e);
const sdpCommonUtils$3 = __importStar$6(commonUtils);
const sdpUnifiedPlanUtils$1 = __importStar$6(unifiedPlanUtils);
const ortcUtils$1 = __importStar$6(utils$f);
...
class Safari12 extends HandlerInterface_1$4.HandlerInterface {
    ...
    async send({ track, encodings, codecOptions, codec, onRtpSender }) {
        ...
        const sendingRemoteRtpParameters = utils$6.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$5.reduceCodecs(
            sendingRemoteRtpParameters.codecs,
            codec
        );
        const mediaSectionIdx = this._remoteSdp.getNextMediaSectionIdx();
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$3.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$5(result, mod, k[i]);
                    }
                }
            }
...
const utils$5 = __importStar$5(utils$i);
const ortc$4 = __importStar$5(ortc$e);
...
class Safari11 extends HandlerInterface_1$3.HandlerInterface {
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        sendingRtpParameters.codecs = ortc$4.reduceCodecs(sendingRtpParameters.codecs);
        const sendingRemoteRtpParameters = utils$5.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        sendingRemoteRtpParameters.codecs = ortc$4.reduceCodecs(sendingRemoteRtpParameters.codecs);
        if (!this._transportReady) {
            await this.setupTransport({
                localDtlsRole: this._forcedLocalDtlsRole ?? "client",
                localSdpObject,
        ...
        // Set RTP encodings.
        sendingRtpParameters.encodings = sdpPlanBUtils$1.getRtpEncodings({
            offerMediaObject,
            track,
        });
        // Complete encodings with given values.
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$2.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$4(result, mod, k[i]);
                    }
                }
            }
...
const utils$4 = __importStar$4(utils$i);
/**
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$3(result, mod, k[i]);
                    }
                }
            }
...
const utils$3 = __importStar$3(utils$i);
const ortc$3 = __importStar$3(ortc$e);
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$2(result, mod, k[i]);
                    }
                }
            }
...
const utils$2 = __importStar$2(utils$i);
const ortc$2 = __importStar$2(ortc$e);
const sdpCommonUtils$1 = __importStar$2(commonUtils);
const sdpUnifiedPlanUtils = __importStar$2(unifiedPlanUtils);
const ortcUtils = __importStar$2(utils$f);
...
const NAME$1 = "ReactNativeUnifiedPlan";
...
class ReactNativeUnifiedPlan extends HandlerInterface_1$1.HandlerInterface {
    ...
    async send({ track, encodings, codecOptions, codec, onRtpSender }) {
        ...
        const sendingRemoteRtpParameters = utils$2.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        // This may throw.
        sendingRemoteRtpParameters.codecs = ortc$2.reduceCodecs(
            sendingRemoteRtpParameters.codecs,
            codec
        );
        const mediaSectionIdx = this._remoteSdp.getNextMediaSectionIdx();
    ...
    async sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol }) {
        ...
        const dataChannel = this._pc.createDataChannel(label, options);
        // Increase next id.
        this._nextSendSctpStreamId = ++this._nextSendSctpStreamId % SCTP_NUM_STREAMS$1.MIS;
        // If this is the first DataChannel we need to create the SDP answer with
        // m=application section.
        if (!this._hasDataChannelMediaSection) {
            const offer = await this._pc.createOffer();
    ...
    (function () {
        ...
        return function (mod) {
            ...
            if (mod != null) {
                for (var k = ownKeys(mod), i = 0; i < k.length; i++) {
                    if (k[i] !== "default") {
                        __createBinding$1(result, mod, k[i]);
                    }
                }
            }
...
const utils$1 = __importStar$1(utils$i);
const ortc$1 = __importStar$1(ortc$e);
...
class ReactNative extends HandlerInterface_1.HandlerInterface {
    ...
    async send({ track, encodings, codecOptions, codec }) {
        ...
        sendingRtpParameters.codecs = ortc$1.reduceCodecs(sendingRtpParameters.codecs);
        const sendingRemoteRtpParameters = utils$1.clone(
            this._sendingRemoteRtpParametersByKind[track.kind]
        );
        sendingRemoteRtpParameters.codecs = ortc$1.reduceCodecs(sendingRemoteRtpParameters.codecs);
        if (!this._transportReady) {
            await this.setupTransport({
                localDtlsRole: this._forcedLocalDtlsRole ?? "client",
                localSdpObject,
...
const utils = __importStar(utils$i);
const ortc = __importStar(ortc$e);
...
class Bus {
    ...
    async _handlePayload(payload) {
        ...
        if (responseTo) {
            ...
            const response = await ((_a = this.onRequest) === null || _a === void 0
                ? void 0
...
(function (CLIENT_MESSAGE) {
    ...
    /** Signals that the client wants to start or stop the transcription of the call */
    CLIENT_MESSAGE["SET_TRANSCRIPTION"] = "SET_TRANSCRIPTION";
...
class SfuClient extends EventTarget {
    ...
    setTranscription(active) {
        var _a;
        (_a = this._bus) === null || _a === void 0
            ? void 0
            : _a.send({
                  name: CLIENT_MESSAGE.SET_TRANSCRIPTION,
                  payload: { active },
              });
    }
    /**
