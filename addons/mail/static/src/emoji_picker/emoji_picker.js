...
export class EmojiPicker extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            this.emojis = emojis;
            this.codepointsToEmoji = this.emojis.reduce((acc, emoji) => {
                acc[emoji.codepoints] = emoji;
                return acc;
            }, {});
    ...
    get recentEmojis() {
        return Object.entries(this.recent)
            ...
            .slice(0, 42)
            .map((r) => r[0])
            .map((codepoints) => this.codepointsToEmoji[codepoints]);
    }
    ...
    onKeydown(ev) {
        switch (ev.key) {
            ...
            case "ArrowDown": {
                const newIndex = this.state.activeEmojiIndex + EMOJI_PER_ROW;
                if (newIndex <= this.itemsAmount - 1) {
                    this.state.activeEmojiIndex = newIndex;
                }
                break;
            }
            case "ArrowRight": {
                const newIndex = Math.min(this.state.activeEmojiIndex + 1, this.itemsAmount - 1);
    ...
    selectEmoji(ev) {
        ...
        if (codepoints) {
            ...
            this.recent[codepoints] = (this.recent[codepoints] ?? 0) + 1;
