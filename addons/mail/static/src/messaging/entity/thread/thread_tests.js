...
QUnit.test('inbox & starred mailboxes', async function (assert) {
    ...
    await this.start();
    const mailboxInbox = this.env.entities.Thread.mailboxFromId('inbox');
    const mailboxStarred = this.env.entities.Thread.mailboxFromId('starred');
...
QUnit.test('create (channel)', async function (assert) {
    ...
    await this.start();
    assert.notOk(this.env.entities.Partner.fromId(9));
    assert.notOk(this.env.entities.Partner.fromId(10));
    ...
    assert.ok(thread);
    assert.ok(this.env.entities.Partner.fromId(9));
    assert.ok(this.env.entities.Partner.fromId(10));
    assert.ok(this.env.entities.Thread.channelFromId(100));
    const partner9 = this.env.entities.Partner.fromId(9);
    const partner10 = this.env.entities.Partner.fromId(10);
    assert.strictEqual(thread, this.env.entities.Thread.channelFromId(100));
...
QUnit.test('create (chat)', async function (assert) {
    ...
    assert.notOk(this.env.entities.Thread.channelFromId(200));
    ...
    assert.ok(channel);
    assert.ok(this.env.entities.Thread.channelFromId(200));
    assert.ok(this.env.entities.Partner.fromId(5));
    const partner = this.env.entities.Partner.fromId(5);
    assert.strictEqual(channel, this.env.entities.Thread.channelFromId(200));
