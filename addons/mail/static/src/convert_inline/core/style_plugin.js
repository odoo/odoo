...
export class StylePlugin extends Plugin {
    ...
    computeDynamicValues(element, styleInfo) {
        ...
        for (const [propertyName, propertyInfo] of styleInfo) {
            ...
            if (this.isDynamicCSSValue(propertyInfo.value)) {
                ...
                if (simpleVarsValue !== undefined) {
                    propertyInfo.value = simpleVarsValue;
                } else {
                    // TODO EGGMAIL: fix failure case
                    // It is possible that the computed value for a specific
                    // propertyName is not defined (shorthand not defined, longhand defined)
                    // e.g. border may have '' as a value, but each longhand for border
                    // might be defined
    ...
    splitSimpleVars(value) {
        const parts = [];
        let cursor = 0;
        const registerValuePart = (index) => {
            parts.push({ value: value.slice(cursor, index) });
        };
        for (const match of value.matchAll(/var\(\s*(--[\w-]+)\s*\)/g)) {
            const index = match.index ?? 0;
            const raw = match[0];
            const simpleVar = match[1];
            if (index > cursor) {
                registerValuePart(index);
            }
            parts.push({ simpleVar, raw });
            cursor = index + raw.length;
        }
        if (cursor < value.length) {
            registerValuePart(value.length);
        }
        return parts;
    }

    /**
     * Recursively try to identify simple var values. Only support simple
     ...
     */
    getSimpleVarsValue(value, element, computedStyle) {
        const valueParts = this.splitSimpleVars(value.trim());
