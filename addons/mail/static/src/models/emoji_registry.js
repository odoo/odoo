    ...
    recordMethods: {
        async _populateFromEmojiData() {
            await executeGracefully(emojiCategoriesData.map(category => () => {
                ...
                const emojiCount = emojisData.reduce((acc, emoji) => emoji.category === category.name ? acc + 1 : acc, 0);
                this.update({
            ...
            this.update({ allEmojiCount: emojisData.length }); // Set the total number of emojis to the category before all emojis are processed and created.
