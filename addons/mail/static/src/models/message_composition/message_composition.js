...
import { replace, clear, unlink } from '@mail/model/model_field_command';
        ...
        _cleanMentionedChannels() {
            ...
            for (const channel of this.mentionedChannels) {
                const fromIndex = namesIndex[channel.name] !== undefined
                    ...
                    0;
                const index = this.rawBody.indexOf(`#${channel.name}`, fromIndex);
                if (index !== -1) {
            ...
            this.update({
                mentionedChannels: unlink(unmentionedChannels),
            });
        },
        /**
        ...
        _cleanMentionedPartners() {
            ...
            for (const partner of this.mentionedPartners) {
                const fromIndex = namesIndex[partner.name] !== undefined
                    ...
                    0;
                const index = this.rawBody.indexOf(`@${partner.name}`, fromIndex);
                if (index !== -1) {
            ...
            this.update({
                mentionedPartners: unlink(unmentionedPartners)
            });
        },
    },
    fields: {
