...
var ChatWindowManager =  AbstractService.extend({
    ...
    openChat: function (session, options) {
        ...
        if (!chatSession) {
            ...
            chatSession.window.on("toggle_star_status", null, function (ev) {
                self.call('chat_manager', 'toggleStarStatus', ev.data.messageID);
            });

            chatSession.window.on("fold_channel", null, function (ev) {
                self.call('chat_manager', 'foldChannel', ev.data.channelID, ev.data.folded);
            });
    ...
    _openChatWithoutSession: function () {
        ...
        if (!this.newChatSession) {
            ...
            this.newChatSession.window.on('open_dm_session', null, function (ev) {
                self.newChatSession.partner_id = ev.data.partnerID;
                var dm = self.call('chat_manager', 'getDmFromPartnerID', ev.data.partnerID);
                if (!dm) {
                    self.call('chat_manager', 'openAndDetachDm', ev.data.partnerID);
                } else {
     ...
     * @param {Object} query
     */
    _onAnyoneListening: function (ev) {
        _.each(this.chatSessions, function (session) {
            if (ev.data.channel.id === session.id && session.window.thread.is_at_bottom() && !session.window.is_hidden) {
                ev.data.query.is_displayed = true;
            }
        });
    },
    /**
    ...
    _onDiscussOpen: function (ev) {
        this.displayState.chatWindowsHidden = ev.data.value;
        if (ev.data.value) {
    ...
    _onNewMessage: function (ev) {
        this._updateSessions(ev.data.message, true);
    },
    /**
     * @private
     * @param {Object} channel
    ...
    _onUpdateDmPresence: function (ev) {
        _.each(this.chatSessions, function (session) {
            if (ev.data.channel.id === session.id) {
                session.window.update_status(ev.data.channel.status);
            }
        });
    },
    /**
    ...
    _onUnsubscribeFromChannel: function (ev) {
        var self = this;
        _.each(self.chatSessions, function (session) {
            if (ev.data.channelID === session.id) {
