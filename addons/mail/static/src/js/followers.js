...
var Followers = AbstractField.extend({
     ...
     * @param {Array} [ids.partnerIDs] the partner ids
     * @param {Array} [ids.channelIDs] the channel ids
     */
    ...
    _updateSubscription: function (event, followerID, isChannel) {
        ...
        if (followerID !== undefined) {
            ...
            if (isChannel) {
                ids.channelIDs = [followerID];
            } else {
                ids.partnerIDs = [followerID];
            }
        } else {
            subtypes = this.$('.o_followers_actions input[type="checkbox"]');
            ids.partnerIDs = [this.partnerID];
        }
    ...
    _onFollowButtonClicked: function () {
        if (!this._isFollower) {
            ...
            this._unfollow({partnerIDs: [this.partnerID]});
        }
    },
    /**
     * @private
    ...
    _onRemoveFollower: function (ev) {
        ...
        if (resModel === 'res.partner') {
            return this._unfollow({partnerIDs: [resID]});
        } else {
            return this._unfollow({channelIDs: [resID]});
        }
    },
    /**
     * @private
