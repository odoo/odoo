...
var ChatterComposer = composer.BasicComposer.extend({
    ...
    events: _.extend({}, composer.BasicComposer.prototype.events, {
     ...
     *
     * @private
     * @param {Array} checkedSuggestedPartners list of 'recipient' partners to complete informations or validate
    ...
    _checkSuggestedPartners: function (checkedSuggestedPartners) {
        ...
        $.when(def).pipe(function (result) {
            ...
            _.each(recipientPopups, function (partnerInfo) {
                ...
                var parsedEmail = utils.parse_email(partnerName);
            ...
            $.when.apply($, emailDefs).then(function () {
                ...
                $.when(def).pipe(function (result) {
                    ...
                    var recipientPopups = result.concat(recipientsToCheck);
                    _.each(recipientPopups, function (partnerInfo) {
                        if (partnerInfo.partner_id && _.indexOf(partnerInfo.partner_id, recipientIDsToRemove) === -1) {
    ...
    _onOpenFullComposer: function () {
        ...
        if (this.options.isLog) {
            ...
            recipientDoneDef = this._checkSuggestedPartners(checkedSuggestedPartners);
        }
