...
/** Sentinel older than any real write date, used as a baseline. */
 ...
 *
 * REPLACE is never versioned, as its date can't be defined.
 */
export class ManyFieldVersion {
    ...
    /**
     * Returns a replace command, equivalent to all the commands in history, along with
     * the write date of the last history entry that mentioned each local id (whether it
     * ended up present or removed). Used as a fallback version for values with none of
     * their own (e.g. many2many, or the original REPLACE).
     */
    _generateReplaceFromHistory() {
        const positionByLocalId = {};
        const versionByLocalId = {};
        for (let idx = 0; idx < this.history.length; idx++) {
            const { cmd, writeDate } = this.history[idx];
            const [name, values] = cmd;
            for (const value of values) {
                const localId = this.TargetModel.localId(value);
                versionByLocalId[localId] = writeDate;
        ...
        const sortedValues = Object.values(positionByLocalId)
            ...
            .map((p) => p.value);
        return [["REPLACE", sortedValues, { versionByLocalId }]];
    }
}
