...
export class RecordListInternal {
     ...
     *   ManyFieldVersion history). Used as historyVersion for the echo below.
     */
        ...
        return store.MAKE_UPDATE(function recordListAssign() {
            ...
            const newRecords = vals.map((val) =>
                self.insert(
                    ...
                    { historyVersion: versionByLocalId?.[TargetModel.localId(val)] }
                )
            );
     ...
     * @param {import("./field_version").WriteDate} [historyVersion] Version to echo when
     *   `val` carries none of its own, i.e. when it comes from a REPLACE regenerated
     *   from ManyFieldVersion history.
     */
    insert(val, fn, { inv = true, mode = "ADD", historyVersion } = {}) {
        const recordList = this.recordList;
        ...
        if (inv) {
            this.echoInverseIfAny(val, mode === "ADD" ? "ADD.noinv" : "DELETE.noinv", {
                ...
                version: val?.__version__ ?? historyVersion,
            });
        }
        /** @type {R} */
        let newRecordProxy;
