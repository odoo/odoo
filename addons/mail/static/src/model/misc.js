 ...
 * `[mode, value]` or `[mode, value, version]` arrays (see `ManyFieldVersion`). `version`
 * covers the whole command, except for ADD, which carries one per value instead, inside
 * each value's own `__version__`.
 */
