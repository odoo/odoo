...
QUnit.test('create: should create and link a record for an empty x2one field', async function (assert) {
    ...
    const contact = this.env.services.messaging.models['test.contact'].create({ id: 10 });
    contact.update({ address: create({ id: 20 })});
    const address = this.env.services.messaging.models['test.address'].findFromIdentifyingData({ id: 20 });
    assert.strictEqual(
        contact.address,
        address,
...
QUnit.test('create: should create and replace a record for a non-empty x2one field', async function (assert) {
    ...
    const contact = this.env.services.messaging.models['test.contact'].create({
        id: 10,
        address: create({ id: 10 }),
    });
    const address10 = this.env.services.messaging.models['test.address'].findFromIdentifyingData({ id: 10 });
    contact.update({ address: create({ id: 20 })});
    const address20 = this.env.services.messaging.models['test.address'].findFromIdentifyingData({ id: 20 });
    assert.strictEqual(
        contact.address,
        address20,
...
QUnit.test('create: should create and link a record for an empty x2many field', async function (assert) {
    ...
    const task = this.env.services.messaging.models['test.task'].findFromIdentifyingData({ id: 10 });
    assert.strictEqual(
...
QUnit.test('create: should create and add a record for a non-empty x2many field', async function (assert) {
    ...
    const task10 = this.env.services.messaging.models['test.task'].findFromIdentifyingData({ id: 10 });
    contact.update({ tasks: create({ id: 20 }) });
    const task20 = this.env.services.messaging.models['test.task'].findFromIdentifyingData({ id: 20 });
    assert.strictEqual(
        contact.tasks.length,
        2,
