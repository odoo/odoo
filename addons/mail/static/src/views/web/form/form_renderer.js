...
patch(FormRenderer.prototype, {
    setup() {
        ...
        if (this.env.services["mail.store"]) {
            this.discussStore = useService("mail.store");
        }
     ...
     */
    hasFileViewer() {
        if (!this.discussStore || this.uiService.size < SIZES.XXL || !this.props.record.resId) {
            return false;
        }
        this.messagingState.thread = this.discussStore.Thread.insert({
