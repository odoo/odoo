 ...
 * row it belongs to. A row stacking its columns vertically is ignored: its
 * `align-items` and their `align-self` are then horizontal alignments.
 *
 * @param {Element} td
...
function _getVerticalAlign(td) {
    ...
    if (getComputedStyle(row).flexDirection.startsWith("column")) {
        return "top";
    }
    const alignment = td.style.alignSelf || row.style.alignItems;
