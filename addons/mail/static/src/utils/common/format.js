...
export async function prettifyMessageContent(rawBody, validRecords = []) {
    ...
    if (codesFences) {
        body = body
            .replaceAll("&#x60;", "`")
            .replaceAll(codeFencesRegexp, "--code-fence-placeholder--");
    }
...
export function isMessageContentEmpty(body) {
    return (
        !body ||
        ["", "<p></p>", "<p><br></p>", "<p><br/></p>"].includes(
            body.replace('<span class="o-mail-Message-edited"></span>', "").replace(/\s/g, "")
        )
    );
}
