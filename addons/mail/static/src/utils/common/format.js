...
export async function prettifyMessageContent(
    rawBody,
    { isHtmlBody, validMentions = [], allowEmojiLoading = true } = {}
) {
    ...
    let body;
    if (isHtmlBody) {
        body = markup(escapeAndCompactHtmlContent(rawBody).replace(/&nbsp;/g, " "));
    } else {
        body = htmlReplace(escapeAndCompactTextContent(rawBody), /&nbsp;/g, " ");
        body = htmlTrim(body);
