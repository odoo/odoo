...
export async function prettifyMessageContent(rawBody, validRecords = []) {
    ...
    let body = markupHtmlReplace(escapedAndCompactContent, /&nbsp;/g, " ");
    body = markupHtmlTrim(body);
...
function linkify(text) {
    ...
    while ((match = urlRegexp.exec(text)) !== null) {
        result = markupHtmlJoin(result, text.slice(curIndex, match.index));
    ...
    return markupHtmlJoin(result, text.slice(curIndex));
}
...
export function escapeAndCompactTextContent(content) {
    ...
    let value = markupHtmlTrim(content);
    value = markupHtmlReplace(value, /(\r|\n){2,}/g, markup("<br/><br/>"));
    value = markupHtmlReplace(value, /(\r|\n)/g, markup("<br/>"));

    // prevent html space collapsing
    value = markupHtmlReplace(value, / /g, markup("&nbsp;"));
    value = markupHtmlReplace(value, /([^>])&nbsp;([^<])/g, markup("$1 $2"));
    return value;
}
...
function generateMentionsLinks(body, { partners = [], threads = [] }) {
    ...
    for (const partner of partners) {
        ...
        body = markupHtmlReplace(body, text, placeholder);
    }
    ...
    for (const mention of mentions) {
        ...
        body = markupHtmlReplace(body, mention.placeholder, markup(link.outerHTML));
    }
    return markupHtmlJoin(body); // ensure markup if not already a markup
}
...
async function _generateEmojisOnHtml(htmlString) {
    ...
    for (const emoji of emojis) {
        for (const source of [...emoji.shortcodes, ...emoji.emoticons]) {
            const escapedSource = markupHtmlJoin(String(source));
            const regexp = new RegExp("(\\s|^)(" + escapeRegExp(escapedSource) + ")(?=\\s|$)", "g");
            htmlString = markupHtmlReplace(htmlString, regexp, "$1" + emoji.codepoints);
        }
    }
    return markupHtmlJoin(htmlString); // ensure markup if not already a markup
}
...
export function htmlToTextContentInline(htmlString) {
    if (!(htmlString instanceof Markup)) {
        return htmlString; // already text content
    }
    htmlString = markupHtmlReplace(htmlString, /<br\s*\/?>/gi, " ");
    const div = document.createElement("div");
    try {
        setElementContent(div, htmlString);
    } catch {
