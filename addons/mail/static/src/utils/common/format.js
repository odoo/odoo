...
export async function prettifyMessageContent(rawBody, validRecords = []) {
    ...
    // Extract all code codeFencesRegexp from the body
    const codesFences = body.replaceAll("&#x60;", "`").match(codeFencesRegexp);
    // remove all the code codeFencesRegexp to avoid processing them
    if (codesFences) {
    ...
    // add code codeFencesRegexp back in the message
    if (codesFences) {
