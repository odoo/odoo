...
export class ImageFormatPlugin extends Plugin {
     ...
     * converted to a mail-supported equivalent (JPEG).
     */
    async sanitizeImages() {
        const promises = [];
        promises.concat(this.forEachSvg((svg) => this.sanitizeSvg(svg)));
        promises.concat(this.forEachImg((img) => this.sanitizeImage(img)));
        promises.concat(this.forEachBackgroundImg((el) => this.sanitizeImage(el)));
