 ...
 * assign, the `maxTileCount` the video filter caps at, and whether the caller has to crop.
 *
 * @param {Object} param0
 ...
 *  order. Synthetic entries are allowed, to reserve a slot.
 ...
 * @param {string} [param0.insetCorner] corner the user dropped the inset in. Unset until they move
 *  it, in which case the layout takes the corner the stage has room for.
...
/**
 * How many tiles fit at `minTileWidth` without shrinking further: the point past which the video
 * filter drops the video-less ones.
 *
 * @param {Stage} stage
...
/**
 * Lay the sidebar cards, one 16:9 card per slot. They shrink rather than overflow: the sidebar
 * shares the stage's scroll, so overflowing it would scroll the focused tile out of view.
 *
 * @param {Object<string, Rect>} rects written in place
