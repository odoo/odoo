...
class CreateChatDialog extends Component {
    ...
    get createText() {
        if (this.invitePeopleState.selectedUsers.length === 1) {
            return _t("Open Chat");
        }
    ...
    onClickConfirm() {
        const selectedUsersId = this.invitePeopleState.selectedUsers.map((p) => p.id);
        const user_ids = [...new Set([this.store.self_user?.id, ...selectedUsersId])];
        if (user_ids.length === 1) {
            this.store.createGroupChat({ user_ids });
        } else {
            this.store.startChat(user_ids);
        }
        this.props.close();
    }
}
...
export class DiscussCommandPalette {
    ...
    buildResults(filtered) {
        ...
        if (this.store.self_user?.share === false) {
            users = Object.values(this.store["res.users"].records).filter(
                (user) =>
                    user.share === false &&
                    user.partner_id &&
                    cleanTerm(user.displayName).includes(this.cleanedTerm) &&
                    (!filtered || !filtered.has(user))
            );
    ...
    makeDiscussCommand(channelOrUser, category) {
        ...
        if (channelOrUser === NEW_GROUP_CHAT) {
            const name = this.options.searchValue.trim();
            return {
                Component: DiscussCommand,
                action: () => {
