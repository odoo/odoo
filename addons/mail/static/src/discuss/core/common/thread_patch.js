...
const threadPatch = {
        ...
        useEffect(
            (loadNewer, mountedAndLoaded) => {
                if (
                    loadNewer ||
                    !mountedAndLoaded ||
                    !this.props.thread.channel?.self_member_id ||
                ...
                if (Math.abs(el.scrollTop + el.clientHeight - el.scrollHeight) <= 1) {
                    this.props.thread.channel.self_member_id.hideUnreadBanner = true;
                }
            },
    ...
    /** @override */
    fetchMessages() {
        if (this.props.thread.channel?.self_member_id && this.props.thread.scrollUnread) {
            toRaw(this.props.thread).loadAround(
                this.props.thread.channel?.self_member_id.new_message_separator
            );
        } else {
            super.fetchMessages();
        }
    ...
    get newMessageBannerText() {
        if (this.props.thread.channel?.self_member_id?.message_unread_counter > 1) {
            return _t(
                "%s new messages",
                this.props.thread.channel?.self_member_id.message_unread_counter
            );
        }
    ...
    async onClickUnreadMessagesBanner() {
        await this.props.thread.loadAround(
            this.props.thread.channel?.self_member_id.new_message_separator_ui
        );
