...
registerThreadAction("add-to-favorites", {
     ...
     */
    condition: ({ channel, store, isDiscussContent }) =>
     ...
     */
    onSelected: async ({ channel, store, inDiscussApp }) => {
...
registerThreadAction("invite-people", {
    ...
    actionPanelComponentProps: ({ channel }) => ({ channel }),
    actionPanelOpen({ inMeetingView, isDiscussSidebarChannelActions, store, channel, rootRef }) {
    ...
    condition: ({ channel, pipWindow, isDiscussContent, chatWindow }) =>
        channel &&
        !pipWindow &&
        (!chatWindow || chatWindow.isOpen) &&
...
registerThreadAction("member-list", {
    actionPanelClose: ({ action, inDiscussApp, store, nextActiveAction }) => {
        if (
            action.condition &&
            inDiscussApp &&
