...
class ChannelActionDialog extends Component {
    ...
    static template = xml`
        <Dialog size="'md'" title="props.title" footer="false" contentClass="'o-bg-body o-rounded-bubble'" bodyClass="'p-1'">
        ...
        setup(action) {
            ...
            if (!component.props.chatWindow) {
                action.popover = usePopover(NotificationSettings, {
                    ...
                    position: "bottom-end",
                    fixedPosition: true,
                    popoverClass: `o-mail-NotificationSettings-popover o-rounded-bubble m-2 ${action.panelOuterClass}`,
                    arrow: false,
                });
            }
