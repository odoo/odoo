...
export class Rtc {
    ...
    constructor(env, services) {
        ...
        this.discussStore.ringingThreads = this.ringingThreads;
        ...
        this.env.bus.addEventListener(
            ...
            ({ detail: { commands = [], record, channel } }) => {
                if (record) {
    ...
    endCall(channel = this.state.channel) {
        ...
        if (this.state.channel?.localId === channel.localId) {
            this.clear();
    ...
    async toggleCall(channel, { video } = {}) {
        ...
        const isActiveCall = Boolean(
            this.state.channel && this.state.channel.localId === channel.localId
        );
        if (this.state.channel) {
    ...
    async joinCall(channel, { video = false } = {}) {
        ...
        this.threadService.update(this.state.channel, { rtcSessions, invitedMembers });
        this.state.selfSession = this.discussStore.rtcSessions[sessionId];
    ...
    clear() {
        for (const session of Object.values(this.discussStore.rtcSessions)) {
            this.disconnect(session);
        }
    ...
    insertSession(data) {
        let session;
        if (this.discussStore.rtcSessions[data.id]) {
            session = this.discussStore.rtcSessions[data.id];
        } else {
            session = new RtcSession();
            session._store = this.discussStore;
        }
        ...
        if (channelMember) {
            ...
            if (channelMemberRecord.channel) {
                channelMemberRecord.channel.rtcSessions[session.id] = session;
            }
        }
        this.discussStore.rtcSessions[session.id] = session;
        // return reactive version
        return this.discussStore.rtcSessions[session.id];
    }

    /**
     * @param {import("@mail/discuss/rtc/rtc_session_model").id} id
     */
    deleteSession(id) {
        const session = this.discussStore.rtcSessions[id];
        if (session) {
            if (this.state.selfSession && session.id === this.state.selfSession.id) {
                this.endCall();
            }
            delete this.discussStore.channels[createLocalId("discuss.channel", session.channelId)]
                ?.rtcSessions[id];
            this.disconnect(session);
        }
        delete this.discussStore.rtcSessions[id];
    }
