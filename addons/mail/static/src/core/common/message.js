...
export class Message extends Component {
    ...
    async onClickReplyMessage() {
        ...
        const default_body = markup(
            `<div class="o_mail_hidden_content d-none">\
            <span>On ${datetime} ${this.escape(author.name)} ${authorEmail} wrote </span>\
            <blockquote>${getNonEditableMentions(this.message.body)}</blockquote></div>`
        );
        const params = {
            context: {
    ...
    async onClickFowardMessage() {
        ...
        const newAttachmentIds = await this.env.services.orm.call("ir.attachment", "copy", [attachmentIds], {
