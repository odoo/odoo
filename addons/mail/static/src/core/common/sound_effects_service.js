 ...
 */
export function playSound(soundEffectName, { loop = false, volume } = {}) {
    if (typeof browser.Audio === "undefined") {
        return;
    }
    const soundEffect = sounds[soundEffectName];
    if (!soundEffect) {
        return;
    }
 ...
 */
export function stopPlaying(soundEffectName) {
    const soundEffect = sounds[soundEffectName];
    if (soundEffect) {
        if (soundEffect.audio) {
            soundEffect.audio.pause();
        ...
        for (const soundEffect of Object.values(sounds)) {
            if (soundEffect.audio) {
                soundEffect.audio.pause();
                soundEffect.audio.currentTime = 0;
            }
