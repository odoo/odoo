...
import { createElementWithContent, htmlJoin, htmlMarkup } from "@web/core/utils/html";
...
export class Composer extends Component {
    ...
    setup() {
        ...
        this.OR_PRESS_SEND_KEYBIND = _t("or press %(send_keybind)s", {
            send_keybind: htmlJoin(
                this.sendKeybinds.map((key) => htmlMarkup`<samp>${key}</samp>`),
                " + "
            ),
        });
    ...
    get CANCEL_OR_SAVE_EDIT_TEXT() {
        if (this.ui.isSmall) {
            return _t(
                ...
                {
                    open_button: htmlMarkup`<button class='btn px-1 py-0' data-type="${EDIT_CLICK_TYPE.CANCEL}">`,
                    close_button: htmlMarkup`</button>`,
                    icon: htmlMarkup`<i class='fa fa-times-circle pe-1' data-type="${EDIT_CLICK_TYPE.CANCEL}"></i>`,
                    open_em: htmlMarkup`<em data-type="${EDIT_CLICK_TYPE.CANCEL}">`,
                    close_em: htmlMarkup`</em>`,
                }
            );
        } else {
            const tags = {
                open_samp: htmlMarkup`<samp>`,
                close_samp: htmlMarkup`</samp>`,
                open_em: htmlMarkup`<em>`,
                close_em: htmlMarkup`</em>`,
                open_cancel: htmlMarkup`<a role="button" href="#" data-type="${EDIT_CLICK_TYPE.CANCEL}">`,
                close_cancel: htmlMarkup`</a>`,
                open_save: htmlMarkup`<a role="button" href="#" data-type="${EDIT_CLICK_TYPE.SAVE}">`,
                close_save: htmlMarkup`</a>`,
            };
    ...
    formatDefaultBodyForFullComposer(defaultBody, signature = "") {
        if (signature) {
            defaultBody = htmlMarkup`${defaultBody}<br/>${signature}`;
        }
        return htmlMarkup`<div>${defaultBody}</div>`; // as to not wrap in <p> by html_sanitize
    }
