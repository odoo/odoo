...
import { MailMessage } from "@mail/core/common/model_definitions";
...
function setup() {
    this.attachment_ids = fields.Many("ir.attachment", { inverse: "message" });
    this.author_id = fields.One("res.partner");
    this.author_guest_id = fields.One("mail.guest");
    this.body = fields.Html("");
    this.call_history_ids = fields.Many("discuss.call.history");
    this.richBody = fields.Html("", {
        compute() {
    ...
    this.richTranslationValue = fields.Html("", {
        compute() {
            if (!this.store.emojiLoader.loaded) {
                loadEmoji();
            }
    ...
    this.composer = fields.One("Composer", { inverse: "message", onDelete: (r) => r?.delete() });
    this.date = fields.Datetime();
    /** @type {string} */
    this.default_subject;
    /** @type {boolean} */
    this.edited = fields.Attr(false, {
        compute() {
            return Boolean(
    ...
    this.hasLink = fields.Attr(false, {
        compute() {
            if (this.isBodyEmpty) {
                return false;
            }
    ...
    this.hasMailNotificationSummary = fields.Attr(false, {
        compute() {
            return Boolean(
    ...
    this.incoming_email_cc;
    /** @type {Array[Array[string]]} */
    this.incoming_email_to;
    /** @type {boolean} */
    this.is_transient;
    this.message_link_preview_ids = fields.Many("mail.message.link.preview", {
        inverse: "message_id",
    });
    /** @type {number[]} */
     ...
     * @type {() => {} | undefined}
     */
    this.postFailRedo = undefined;
    this.reactions = fields.Many("MessageReactions", {
        inverse: "message",
        /**
    ...
    this.notification_ids = fields.Many("mail.notification", { inverse: "mail_message_id" });
    this.partner_ids = fields.Many("res.partner");
    this.subtype_id = fields.One("mail.message.subtype");
    this.thread = fields.One("mail.thread");
    this.threadAsNeedaction = fields.One("mail.thread", {
        compute() {
            if (this.needaction) {
                return this.thread;
            }
    ...
    this.threadAsNewest = fields.One("mail.thread");
    this.threadAsInEdition = fields.One("mail.thread", {
        compute() {
            if (this.composer) {
                return this.thread;
            }
    ...
    this.scheduledDatetime = fields.Datetime();
    this.onlyEmojis = fields.Attr(false, {
        compute() {
    ...
    this.write_date = fields.Datetime();
    /** @type {undefined|Boolean} */
    this.needaction;
    this.starred = false;
    this.showTranslation = false;
    this.isSelfAuthored = fields.Attr(false, {
        compute() {
            return Boolean(this.author?.eq(this.effectiveSelf));
        },
    });
    this.isPending = false;
    this.isEmpty = fields.Attr(false, {
        /** @this {import("models").Message} */
        compute() {
            return this.computeIsEmpty();
        },
    });
    this.isBodyEmpty = fields.Attr(undefined, {
        compute() {
            return !this.body || isEmptyBlock(createElementWithContent("div", this.body));
        },
    });
    this.inlineBody = fields.Html("", {
        /** @this {import("models").Message} */
        compute() {
...
export const Message = MailMessage;
