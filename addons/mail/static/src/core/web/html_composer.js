...
import { MAIL_PLUGINS, MAIL_SMALL_UI_PLUGINS } from "./plugins/plugin_sets";
import { MailAttachmentDropzone } from "@mail/core/common/mail_attachment_dropzone";
import { MessageConfirmDialog } from "@mail/core/common/message_confirm_dialog";
import { prettifyMessageContent, isMarkup, isEmptyHtmlBody } from "@mail/utils/common/format";
...
import { useComposerActions } from "../common/composer_actions";
 ...
 */
export class HtmlComposer extends Component {
    static components = {
        AttachmentList,
        Dropdown,
        DropdownItem,
    ...
    static template = "mail.HtmlComposer";
    ...
    get wysiwygConfigs() {
        return {
            ...
            onEditorReady: () => {
                if (isEmptyHtmlBody(this.props.composer.body)) {
                    this.restoreContent();
                }
                const body = createDocumentFragmentFromContent(this.props.composer.body).body;
                for (const el of body.querySelectorAll(".o-mail-Message-edited")) {
                    el.remove();
                }
                if (!isEmptyHtmlBody(body.innerHTML)) {
                    setElementContent(this.wysiwyg.editor.editable, markup(body.innerHTML));
                }
                this.wysiwyg.editor.shared.selection.setCursorEnd(
                    this.wysiwyg.editor.editable.lastChild
                );
    ...
    get isSendButtonDisabled() {
        ...
        return (
            !this.state.active ||
            (isEmptyHtmlBody(this.props.composer.body) && attachments.length === 0) ||
            attachments.some(({ uploading }) => Boolean(uploading))
        );
    }

    get hasSuggestions() {
        return Boolean(document.querySelector(".o-mail-Composer-suggestion"));
    }
    ...
    onKeydown(ev) {
        ...
        switch (ev.key) {
            case "ArrowUp":
                if (!this.env.inChatter && isEmptyHtmlBody(this.props.composer.body)) {
    ...
    async onClickFullComposer(ev) {
        ...
        const body = this.props.composer.body;
    ...
    clear() {
        ...
        if (this.wysiwyg.editor?.editable) {
            setElementContent(this.wysiwyg.editor.editable, markup("<p><br/></p>"));
            this.wysiwyg.editor.shared.selection.setCursorEnd(
                this.wysiwyg.editor.editable.lastChild
            );
            this.wysiwyg.editor.shared.history.addStep();
    ...
    async processMessage(cb) {
        ...
        if (attachments.some(({ uploading }) => uploading)) {
            ...
            !isEmptyHtmlBody(this.props.composer.body) ||
            ...
            await cb(this.props.composer.body);
    ...
    saveContent() {
        const composer = toRaw(this.props.composer);
        const saveContentToLocalStorage = ({ body, emailAddSignature, replyToMessageId }) => {
    ...
    restoreContent() {
        ...
        if (config.body) {
            ...
            composer.body = markup(config.body);
        }
