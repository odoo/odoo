    ...
    get wysiwygConfigs() {
        return {
            ...
            onEditorReady: () => {
                ...
                if (!isEmptyHtmlBody(body)) {
                    setElementContent(this.wysiwyg.editor.editable, body);
                }
                this.wysiwyg.editor.shared.selection.setCursorEnd(
                    this.wysiwyg.editor.editable.lastChild
                );
        ...
        switch (ev.key) {
            case "ArrowUp":
                if (!this.env.inChatter && isEmptyHtmlBody(this.props.composer.body)) {
                    ...
                    if (messageToEdit) {
                        messageToEdit.enterEditMode(messageToEdit.thread);
                    }
                }
                break;
            case "Enter": {
    ...
    saveContent() {
        ...
        if (this.state.isFullComposerOpen) {
            ...
            saveContentToLocalStorage({
                body: !isMarkup(composer.body) && composer.body,
                htmlBody: isMarkup(composer.body) && composer.body,
    ...
    restoreContent() {
        ...
        try {
            ...
            if (config.body) {
                ...
                composer.body = config.body;
            }
            if (!isEmptyHtmlBody(config.htmlBody)) {
                composer.emailAddSignature = config.emailAddSignature;
                composer.body = markup(config.htmlBody);
            }
