...
export function useRefToModel({ fieldName, modelName, propNameAsRecordLocalId, refName }) {
    ...
    const record = component.env.models[modelName].get(component.props[propNameAsRecordLocalId]);
    const ref = useRef(refName);
    if (record) {
        record.update({ [fieldName]: ref });
    }
    onWillUpdateProps(nextProps => {
        const currentRecord = component.env.models[modelName].get(component.props[propNameAsRecordLocalId]);
        if (currentRecord) {
            currentRecord.update({ [fieldName]: clear() });
        }
        const nextRecord = component.env.models[modelName].get(nextProps[propNameAsRecordLocalId]);
        if (nextRecord) {
