...
QUnit.test("recent category (basic)", async (assert) => {
    ...
    await openDiscuss(channelId);
    await click("button[aria-label='Emojis']");
    assert.containsNone($, ".o-EmojiPicker-header [title='Frequently used']");
    await click(".o-EmojiPicker-content .o-Emoji:contains(😀)");
    await click("button[aria-label='Emojis']");
    assert.containsOnce($, ".o-EmojiPicker-header [title='Frequently used']");
    assert.containsOnce(
        $,
...
QUnit.test("first category should be highlight by default", async (assert) => {
    ...
    await openDiscuss(channelId);
    await click("button[aria-label='Emojis']");
    assert.containsOnce($, ".o-EmojiPicker-header .o-Emoji:eq(0).bg-200");
});
