...
test("Opening full composer in 'send message' mode should copy selected suggested recipients", async () => {
    ...
    await click("button", { text: "Send message" });
    await contains(".o-mail-RecipientsInput .o_tag[title='John Jane']", {
        contains: [".o_tag_badge_text:contains(John Jane)"],
    });
    await contains(".o-mail-RecipientsInput .o_tag[title='john@test.be']", {
        contains: [".o_tag_badge_text:contains(john@test.be)"],
    });
