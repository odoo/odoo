...
test("Mail Composer scheduled date field", async () => {
    const pyEnv = await startServer();
    registerArchs({
        "mail.compose.message,false,form": `<form><field name="scheduled_date" widget="mail_composer_scheduled_date"/></form>`,
    });
    ...
    await contains(".o_field_mail_composer_scheduled_date button", { text: "" });
    await click(".o_field_mail_composer_scheduled_date button");
    ...
    await contains(".o_field_mail_composer_scheduled_date button", {
...
test("Mail Scheduled Message scheduled date field", async () => {
    const pyEnv = await startServer();
    registerArchs({
        "mail.scheduled.message,false,form": `<form><field name="scheduled_date" widget="mail_scheduled_message_scheduled_date"/></form>`,
    });
    ...
    // button should show the scheduled date
    await contains(".o_field_mail_scheduled_message_scheduled_date button", {
        text: "Sending Oct 21, 1:00 PM",
    });
    await click(".o_field_mail_scheduled_message_scheduled_date button");
    // should open the dialog to select the schedule date
    await contains(".modal");
