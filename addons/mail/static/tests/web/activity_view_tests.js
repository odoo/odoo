...
QUnit.test("Activity view rendering", async (assert) => {
    ...
    async function toggleFilterProgressBar(activityTypeName, state, activateFilter) {
        await click(selProgressBar(activityTypeName, state));
    ...
    function selCellCounterContains(recordName, type, contains) {
        return `${selCellCounter(recordName, type)}:contains('${contains}')`;
    }
    ...
    for (const todoDisplayDone of [true, false]) {
        ...
        if (todoDisplayDone) {
            await contains(selCellCounterContains('Prototype 1', 'to do', '2 / 3'));
        } else {
            await contains(selCellCounterContains('Prototype 1', 'to do', '2'));
            await contains(selCellCounterContains('Prototype 1', 'to do', '/'), { count: 0 });
        }
        await contains(selCellCounterContains('Prototype 1', 'upload', '2'));
        await contains(selCellCounterContains('Prototype 1', 'meeting', '1 / 2'));
        if (todoDisplayDone) {
            await contains(selCellCounterContains('Prototype 2', 'to do', '1 / 2'));
        } else {
            await contains(selCellCounterContains('Prototype 2', 'to do', '1'), { count: 0 });
            await contains(selCellCounterContains('Prototype 2', 'to do', '/'), { count: 0 });
        }
        // Check cells avatars
        ...
        await contains(selProgressBar('to do', 'planned'));
        await contains(selProgressBar('to do', 'overdue'));
        await contains(selProgressBar('to do', 'today'));
        await contains(selProgressBar('call', 'planned'), { count: 0 });
        await contains(selProgressBar('call', 'overdue'), { count: 0 });
        await contains(selProgressBar('call', 'today'), { count: 0 });
        await contains(selProgressBar('meeting', 'planned'));
        await contains(selProgressBar('meeting', 'overdue'), { count: 0 });
        await contains(selProgressBar('meeting', 'today'), { count: 0 });
        await contains(selProgressBar('upload', 'planned'), { count: 0 });
        await contains(selProgressBar('upload', 'overdue'), { count: 0 });
        await contains(selProgressBar('upload', 'today'));
        // Header: Check counters
        await contains(selHeaderOngoingActivityCounter('to do', 3));
        if (todoDisplayDone) {
            await contains(selHeaderTotalActivityCounter('to do', 5));
        } else {
            await contains(selHeaderTotalActivityCounter('to do', 5), { count: 0 });
        }
        await contains(selHeaderCounter('call'), { count: 0 });
        await contains(selHeaderOngoingActivityCounter('meeting', 1));
        await contains(selHeaderTotalActivityCounter('meeting', 3));
        await contains(selHeaderOngoingActivityCounter('upload', 1));
        await contains(selHeaderTotalActivityCounter('upload', 3));
        ...
        await contains(".o_activity_summary_cell.done", { count: 4 });
        await contains(selHeaderOngoingActivityCounter('to do', 1));
        if (todoDisplayDone) {
            await contains(selHeaderTotalActivityCounter('to do', 5));
        } else {
            await contains(selHeaderTotalActivityCounter('to do', 5), { count: 0 });
    ...
    for (const [recordName, activityType, nByStatus, ongoingActivityDefs, completedActivityDefs] of [
        ...
        for (const status of ['planned', 'today', 'overdue', 'done']) {
            if (status in nByStatus) {
                ...
                await contains(`.o-mail-ActivityListPopover-${status} .rounded-pill:contains('${nByStatus[status]}')`);
            } else {
