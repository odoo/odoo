...
const SINGLE_FIELD_CASES = [
    {
        ...
        steps: [
            {
                values: { name: "v1" },
                meta: { snapshot: { xmin: 10, xmax: 10, xip_list: [], current_xact_id: null } },
                expected: { name: "v1" },
            },
            {
                values: { name: "v3" },
                meta: { snapshot: { xmin: 40, xmax: 40, xip_list: [], current_xact_id: null } },
                expected: { name: "v3" },
                description: "V3 read's snapshot is newer.",
            },
            {
                values: { name: "v2" },
                meta: { snapshot: { xmin: 30, xmax: 30, xip_list: [], current_xact_id: null } },
                expected: { name: "v3" },
    ...
    },
    {
        ...
        steps: [
            ...
            },
            {
                values: { name: "v2" },
                meta: {
                    snapshot: { xmin: 20, xmax: 20, xip_list: [], current_xact_id: 20 },
            ...
            },
            {
                values: { name: "v4" },
                meta: {
                    snapshot: { xmin: 20, xmax: 20, xip_list: [], current_xact_id: 35 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v4" },
            ...
            },
            {
                values: { name: "v6" },
                meta: {
                    snapshot: { xmin: 50, xmax: 50, xip_list: [], current_xact_id: 50 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v6" },
            ...
            {
                values: { name: "v7" },
                meta: { snapshot: { xmin: 60, xmax: 60, xip_list: [], current_xact_id: null } },
                expected: { name: "v7" },
            ...
            },
            {
                values: { name: "v8" },
                meta: {
                    snapshot: { xmin: 60, xmax: 60, xip_list: [], current_xact_id: 60 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v8" },
        ...
        steps: [
            {
                values: { name: "v1" },
                meta: {
                    snapshot: { xmin: 10, xmax: 10, xip_list: [], current_xact_id: 10 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v1" },
            },
            {
                values: { name: "v3" },
                meta: {
                    snapshot: { xmin: 30, xmax: 30, xip_list: [], current_xact_id: 30 },
                    written_fields_by_record: { Thread: { 1: ["name"] } },
                },
                expected: { name: "v3" },
    ...
    },
    {
        ...
        steps: [
            ...
            },
            {
                ...
                meta: { snapshot: { xmin: 20, xmax: 20, xip_list: [], current_xact_id: null } },
    ...
    },
    {
        ...
        steps: [
            ...
            {
                values: { name: "READ" },
                meta: { snapshot: { xmin: 5, xmax: 5, xip_list: [], current_xact_id: null } },
                expected: { name: "WRITE" },
...
const MANY_FIELD_CASES = [
    ...
    },
    {
        ...
        steps: [
            ...
            {
                values: [["DELETE", [1, 2, 3]]],
                meta: { snapshot: { xmin: 15, xmax: 15, xip_list: [], current_xact_id: null } },
                expected: [1, 2, 3],
                description:
...
test("out-of-order delete does not pile up inverse echoes", async () => {
    ...
    store.insert({
        ...
        __store_version__: { snapshot: { xmin: 10, xmax: 10, xip_list: [] } },
    });
    store.insert({
        Thread: { id: 1, messages: [["ADD", [6]]] },
        __store_version__: { snapshot: { xmin: 30, xmax: 30, xip_list: [] } },
    });
    store.insert({
        Thread: { id: 1, messages: [["DELETE", [1]]] },
        __store_version__: { snapshot: { xmin: 20, xmax: 20, xip_list: [] } },
    });
...
test("Inverse of relations are properly versioned", async () => {
    ...
    store.insert({
        ...
        __store_version__: { snapshot: { xmin: 1, xmax: 1, xip_list: [] } },
    });
    expect(store.Thread.get(1).messages.map((m) => m.id)).toEqual([1, 2]);
    expect(store.Message.get(1).thread.id).toBe(1);
    store.insert({
        Thread: { id: 1, messages: [["DELETE", [1]]] },
        __store_version__: { snapshot: { xmin: 3, xmax: 3, xip_list: [] } },
    });
    ...
    store.insert({
        ...
        __store_version__: { snapshot: { xmin: 2, xmax: 2, xip_list: [] } },
    });
    expect(store.Message.get(1).thread).toBe(undefined);
    expect(store.Thread.get(1).messages.map((m) => m.id)).toEqual([2]);
    store.insert({
        Message: { id: 1, thread: 1 },
        __store_version__: { snapshot: { xmin: 4, xmax: 4, xip_list: [] } },
    });
    expect(store.Message.get(1).thread.id).toBe(1);
    expect(
        store.Thread.get(1)
...
test("drop snapshots from a not yet upgraded server", async () => {
    (class Thread extends Record {
        static id = "id";
        id;
        name;
    ...
    store.insert({
        Thread: { id: 1, name: "v1" },
        // Legacy shape: no xip_list, only the now-removed xip_bitmap field.
