...
function triggerEvent(el, selector, eventType, eventInit, options = {}) {
    ...
    console.log;
    manuallyDispatchProgrammaticEvent(target, "keydown");
    // InputEvent is required to simulate the insert text.
    manuallyDispatchProgrammaticEvent(target, "beforeinput", {
        inputType: eventType,
    });
    manuallyDispatchProgrammaticEvent(target, "input", {
        inputType: eventType,
    });
    // KeyUpEvent is not required but is triggered like the browser would.
    manuallyDispatchProgrammaticEvent(target, "keyup");
