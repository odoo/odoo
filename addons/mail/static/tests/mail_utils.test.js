...
test("addLink: utility function and special entities", () => {
    const testInputs = [
        ...
        [
            htmlMarkup`<p>https://example.com/?&amp;currency_id</p>`,
        ...
        [
            htmlMarkup`<p>https://example.com/'yeah'</p>`,
        ...
        [
            htmlMarkup`https://odoo.com/%5B%5D`,
...
test("addLink: linkify inside text node (1 occurrence)", async () => {
    const content = htmlMarkup`<p>some text https://somelink.com</p>`;
...
test("addLink: linkify inside text node (2 occurrences)", () => {
    ...
    // stringified representation, we continue deeper assertion with query
    // selectors.
    const content = htmlMarkup`<p>some text https://somelink.com and again https://somelink2.com ...</p>`;
    const linkified = parseAndTransform(content, addLink);
    const fragment = document.createDocumentFragment();
    const div = document.createElement("div");
    fragment.appendChild(div);
