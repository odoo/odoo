...
QUnit.test('marked as read thread notifications are ordered by last message date', async function (assert) {
    ...
    assert.containsN(
        ...
        2,
        "there should be two channel previews"
    );
    const channelPreviewElList = document.querySelectorAll('.o_ChannelPreviewView');
    assert.strictEqual(
        channelPreviewElList[0].querySelector(':scope .o_ChannelPreviewView_name').textContent,
        'Channel 2020',
        "First channel in the list should be the channel of 2020 (more recent last message)"
    );
    assert.strictEqual(
        channelPreviewElList[1].querySelector(':scope .o_ChannelPreviewView_name').textContent,
        'Channel 2019',
...
QUnit.test('thread notifications are re-ordered on receiving a new message', async function (assert) {
    ...
    assert.containsN(
        ...
        '.o_ChannelPreviewView',
        2,
        "there should still be two channel previews"
    );
    const channelPreviewElList = document.querySelectorAll('.o_ChannelPreviewView');
    assert.strictEqual(
        channelPreviewElList[0].querySelector(':scope .o_ChannelPreviewView_name').textContent,
