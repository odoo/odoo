    ...
    QUnit.module("model_field_commands", {}, function () {
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                const address20 = messaging.models["TestAddress"].findFromIdentifyingData({
                    id: 20,
                });
                assert.strictEqual(
                    contact.address,
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.strictEqual(contact.tasks[0], task, "should link the new record");
                assert.strictEqual(
                    task.responsible,
                    contact,
                    "the inverse relation should be set as well"
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.strictEqual(contact.tasks[1], task20, "new record should be added");
                assert.strictEqual(
                    task20.responsible,
                    contact,
                    "the inverse relation should be set as well"
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.strictEqual(contact.tasks.length, 1, "should have 1 record");
                assert.strictEqual(contact.tasks[0], task, "the original task should be kept");
                assert.strictEqual(
                    task.title,
                    "task 10 updated",
