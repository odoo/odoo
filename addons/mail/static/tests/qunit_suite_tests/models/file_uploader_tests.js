    ...
    QUnit.module("components", {}, function () {
        QUnit.module("file_uploader", {}, function () {
            ...
            QUnit.test("no conflicts between file uploaders", async function (assert) {
                ...
                await dropFiles(".o_ChatWindow .o_DropZoneView", [file2]);
                await contains(
                    ".o_ChatWindow .o_ComposerView .o_AttachmentCard:not(.o-isUploading)"
                );
                await triggerEvents(".o_ChatWindow .o_ComposerTextInputView_textarea", [
                    ["keydown", { key: "Enter" }],
                ]);
                await contains(".o_Chatter .o_AttachmentCard");
                await contains(".o_ChatWindow .o_MessageView .o_AttachmentCard");
            });
        });
    });
});
