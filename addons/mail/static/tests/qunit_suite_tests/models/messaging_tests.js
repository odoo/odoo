    ...
    QUnit.module("models", {}, function () {
        QUnit.module.skipRefactoring("messaging_tests.js", {}, function () {
            ...
            QUnit.test("openChat: open new chat for user", async function (assert) {
                ...
                const partner = messaging.models["Partner"].findFromIdentifyingData({
                    id: resPartnerId1,
                });
                ...
                await messaging.openChat({ partnerId: resPartnerId1 });
                const chat = messaging.models["Partner"].findFromIdentifyingData({
                    id: resPartnerId1,
                }).dmChatWithCurrentPartner;
                assert.ok(chat, "a chat should exist with the target partner");
                assert.strictEqual(
                    chat.thread.threadViews.length,
                    1,
            ...
            QUnit.test("openChat: open existing chat for user", async function (assert) {
                ...
                const { messaging } = await start();
                const existingChat = messaging.models["Partner"].findFromIdentifyingData({
                    id: resPartnerId1,
                }).dmChatWithCurrentPartner;
                assert.ok(existingChat, "a chat should initially exist with the target partner");
                assert.strictEqual(
                    existingChat.thread.threadViews.length,
                    0,
