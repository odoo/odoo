    ...
    QUnit.module("utils", {}, function () {
        QUnit.module.skipRefactoring("timer", {}, function () {
            ...
            QUnit.test("timer insert (duration: 0ms)", async function (assert) {
                ...
                await advanceTime(0);
                assert.notOk(timer.timeoutId, "timer should have timed out on insert after 0ms");
            });
            ...
            QUnit.test("timer insert (duration: 1000s)", async function (assert) {
                ...
                await advanceTime(0);
                assert.ok(timer.timeoutId, "timer should not have timed out on insert after 0ms");
                ...
                await advanceTime(1);
                assert.notOk(timer.timeoutId, "timer should have timed out on insert after 10s");
            });
        });
    });
});
