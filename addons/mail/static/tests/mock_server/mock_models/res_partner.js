...
export class ResPartner extends webModels.ResPartner {
    ...
    search_for_channel_invite(search_term, channel_id, limit = 30, with_portal_users = false) {
        ...
        const { partner_ids, terms_truncated } = this._search_for_channel_invite(
            store,
            search_term,
            channel_id,
            limit,
        ...
        if (this._allow_invite_by_email(channel) && partner_ids.length < limit) {
            ...
            if (emailsNormalized.length) {
                ...
                selectable_emails = emailsNormalized
                    .filter((email) => !knownEmails.has(email))
                    .slice(0, limit - partner_ids.length);
            }
        }
        return {
            emails_already_sent,
    ...
    _search_for_channel_invite(
        ...
        const { terms, terms_truncated } = this._split_search_terms(search_term, limit);
        const lowerTerms = terms.map((term) => term.toLowerCase()); // simulates ILIKE
     ...
     * @param {number} limit
     */
    _split_search_terms(searchTerm, limit) {
        // simulates search_term.split(",") on the Python side
        const terms = (searchTerm || "")
            .split(",")
            .map((term) => term.trim())
            .filter((term) => term.length > 0);
        return { terms: terms.slice(0, limit), terms_truncated: terms.length > limit };
    }
