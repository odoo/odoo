import { Surface, SurfaceManager } from "@mail/discuss/call/common/stage/surface_manager";

import { describe, expect, test } from "@odoo/hoot";

describe.current.tags("desktop");

test("reconcile creates surfaces for the desired surface data", () => {
    const manager = new SurfaceManager();
    const surfaces = manager.reconcile([
        { key: "session_main_1", type: "camera" },
        { key: "session_main_2", type: "camera" },
    ]);
    expect(surfaces.map((surface) => surface.key)).toEqual(["session_main_1", "session_main_2"]);
    expect(manager.size).toBe(2);
    expect(surfaces[0]).toBeInstanceOf(Surface);
...
test("a reused surface exposes a new data object, so cardData changes identity", () => {
    // Merging into the surface would keep the prop reference, so a camera turning on would
