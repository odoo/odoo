import {
    ...
    GeometryRenderer,
} from "@mail/discuss/call/common/stage/geometry_renderer";

import { describe, expect, test } from "@odoo/hoot";

/**
 * The interpolation belongs to the browser, so what is asserted is what the renderer hands over:
 * the inline resting geometry, the keyframes overriding it, and that a new target cancels the
 * running animation rather than queueing one.
 */
...
/** The `transform` of every keyframe of the last animation started on `el`. */
function lastKeyframes(el) {
    return lastAnimation(el).keyframes.map((keyframe) => keyframe.transform);
}

describe("handing a surface its target", () => {
...
describe("the latest target wins, nothing is ever queued", () => {
...
describe("interruptions mid-animation", () => {
