...
describe("sizing and placing the tile grid", () => {
...
describe("the sidebar column takes its room out of the grid's", () => {
...
describe("the inset rides on the main tile", () => {
...
describe("a stage taller than it is wide, where the source ratio no longer fits", () => {
    ...
    test("fillMainWidth switches the portrait fill off, so a portrait stage must not ask for it", () => {
        // Exclusive: `fillMainWidth` sizes the card from the width, so a portrait stage asking for
        // both spends three quarters of a phone on black.
        const surfaces = [
            { key: "main", placement: MAIN },
            { key: "inset", placement: INSET },
        ];
    ...
    test("a taller stage hands its tiles the extra height", () => {
        // The tile shape is bounded by the stage, so a longer phone gets a longer, bigger tile.
    ...
    test("a portrait grid picks the column count with the largest tile", () => {
        // Area wins over fidelity: 2x2 gives each tile 58500px², one uncropped column only 39900.
        const { rects, cropsTiles } = computeLayout({
            width: 390,
            height: 600,
            aspectRatio: 16 / 9,
    ...
    test("a portrait stage that the source ratio already fits is not cropped", () => {
        // 3 stacked cells are wider than 16:9, and a tile is never stretched past the source
        // ratio: it letterboxes horizontally, and the full height is still used.
        const { rects, cropsTiles } = computeLayout({
            width: 390,
            height: 600,
            aspectRatio: 16 / 9,
...
describe("the knobs the stage profile hands over", () => {
...
describe("a stage with no height of its own", () => {
    ...
    test("a stage that is not laid out yet is not mistaken for an auto-height one", () => {
        // Same collapsed measurement, but on a stage that has a height: the engine must not
        // invent a grid from the width alone, which a stageHeight write would then lock in.
        const { rects, stageHeight } = computeLayout({
            width: 1600,
            height: 0,
            aspectRatio: 16 / 9,
...
describe("the inputs a caller could break it with", () => {
