...
QUnit.test("make voice message in chat", async (assert) => {
    ...
    assert.containsOnce(document.body, "button[title='Voice Message']");
    await click("button[title='Voice Message']");
    assert.containsOnce(document.body, ".o-mail-VoiceRecorder");
    assert.containsOnce(document.body, ".o-mail-VoiceRecorder:contains(00 : 00)");
    ...
    assert.containsOnce(document.body, "button[title='Stop Recording']");
    await click("button[title='Stop Recording']");
    assert.containsOnce(document.body, ".o-mail-VoicePlayer");
    // wait for audio stream decode + drawing of waves
    await voicePlayerDrawing;
    await nextAnimationFrame();
    assert.containsOnce(document.body, ".o-mail-VoicePlayer button[title='Play']");
    assert.containsN(document.body, ".o-mail-VoicePlayer canvas", 2); // 1 for global waveforms, 1 for played waveforms
    assert.containsOnce(document.body, ".o-mail-VoicePlayer:contains(00 : 04)"); // duration of call_02_in_.mp3
    assert.containsOnce(document.body, ".o-mail-VoiceRecorder button[title='Voice Message']");
