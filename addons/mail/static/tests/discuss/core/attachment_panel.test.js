...
test("Empty attachment panel", async () => {
    ...
    await click(".o-mail-DiscussHeader button[title='Attachments']");
...
test("Can toggle allow public upload", async () => {
    ...
    await click(".o-mail-DiscussHeader button[title='Attachments']", { target: env1 });
    await contains(".o-mail-ActionPanel", {
        contains: ["label", { text: "File upload is disabled for external users" }],
        target: env1,
    });
    await openDiscuss(channelId, { target: env2 });
    await click(".o-mail-DiscussHeader button[title='Attachments']", { target: env2 });
    await contains(".o-mail-ActionPanel", {
        contains: ["label", { text: "File upload is disabled for external users" }],
        target: env2,
    });
