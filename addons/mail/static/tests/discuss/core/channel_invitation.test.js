...
test("Pasting at least two terms selects a partner matched by exact name instead of listing them as selectable", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({
        name: "Demo Partner",
...
test("Pasting at least two terms selects a partner matched by exact email instead of listing them as selectable", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({
        name: "Demo Partner",
        email: "demo@partner.com",
...
test("Pasting at least two brand-new emails selects them instead of listing them as selectable", async () => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({
        channel_member_ids: [Command.create({ partner_id: serverState.partnerId })],
        channel_type: "group",
...
test("Clearing the search text after pasting a batch unmatches the auto-selected partner and email", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({
        name: "Demo Partner",
        email: "demo@partner.com",
    ...
    pyEnv["res.users"].create({ partner_id: partnerId });
    pyEnv["res.users"].create({ partner_id: otherPartnerId });
    const channelId = pyEnv["discuss.channel"].create({
        channel_member_ids: [Command.create({ partner_id: serverState.partnerId })],
        channel_type: "group",
    });
    ...
    await contains(".o-discuss-ChannelInvitation-selectedList button:text(Demo Partner)");
    await contains(".o-discuss-ChannelInvitation-selectedList button:text(new1@test.com)");
    await insertText(".o-discuss-ChannelInvitation-search", "", { replace: true });
    await pasteMulti(".o-discuss-ChannelInvitation-search", {
        "text/plain": "Other Partner\nnew2@test.com",
    });
    await contains(".o-discuss-ChannelInvitation-selectedList button:text(Other Partner)");
    await contains(".o-discuss-ChannelInvitation-selectedList button:text(new2@test.com)");
    await contains(".o-discuss-ChannelInvitation-selectedList button", { count: 2 });
});

test("Clicking the chip of a partner selected from a pasted batch unmatches it instead of leaving it stuck", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({
        name: "Demo Partner",
        email: "demo@partner.com",
    ...
    await contains(".o-discuss-ChannelInvitation-selectedList button:text(Demo Partner)");
    await click(".o-discuss-ChannelInvitation-selectedList button:text(Demo Partner)");
    await contains(".o-discuss-ChannelInvitation-selectable:has(:text('Demo Partner'))");
});
