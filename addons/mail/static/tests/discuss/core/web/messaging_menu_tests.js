...
QUnit.test('"Start a conversation" item selection opens chat', async (assert) => {
    ...
    assert.containsOnce(document.body, ".o-mail-ChatWindow-name[title='Gandalf']");
});
...
QUnit.test('"New channel" item selection opens channel (existing)', async (assert) => {
    ...
    await click(".o-discuss-ChannelSelector-suggestion");
    assert.containsOnce(document.body, ".o-mail-ChatWindow-name[title='Gryffindors']");
});
...
QUnit.test('"New channel" item selection opens channel (new)', async (assert) => {
    ...
    await click(".o-discuss-ChannelSelector-suggestion");
    assert.containsOnce(document.body, ".o-mail-ChatWindow-name[title='slytherins']");
});
...
QUnit.test("new message [REQUIRE FOCUS]", async (assert) => {
    ...
    assert.containsOnce(document.body, ".o-discuss-ChannelSelector input:focus");
});
