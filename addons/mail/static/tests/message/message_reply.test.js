...
test("reply action in Chatter prepends mention", async () => {
    const pyEnv = await startServer();
    ...
    await contains(".o-mail-Composer-coreHeader", { text: "Replying to Partner B" });
...
test("reply action in Chatter to self does not prepend mention", async () => {
    ...
    await click(".o-mail-Message-actions button[title='Reply']");
    await contains(".o-mail-Composer-coreHeader", { text: "Replying to Mitchell Admin" });
    await contains("button.active", { text: "Log note" });
    await contains(".o-mail-Composer-input:focus", { value: "" });
});
