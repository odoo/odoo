...
test("rendering with chat push notification default permissions", async () => {
    ...
    await contains(".o-mail-NotificationItem", {
        text: "Suggestion: Enable chat push notifications",
    });
});
...
test("rendering with PWA installation request", async () => {
    ...
    await contains(".o-mail-NotificationItem-name", { text: "Suggestion: Install Odoo" });
...
test("multiple grouped notifications by document model, sorted by the most recent message of each group", async () => {
    ...
    await contains(":nth-child(1 of .o-mail-NotificationItem)", { text: "Failure: Companies" });
    await contains(":nth-child(2 of .o-mail-NotificationItem)", { text: "Failure: Contact" });
});
