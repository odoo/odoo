...
QUnit.test("mark unread channel as read", async (assert) => {
    ...
    await click(".o_menu_systray i[aria-label='Messages']");
    await afterNextRender(() => triggerEvent($(".o-mail-NotificationItem")[0], null, "mouseenter"));
    assert.containsOnce($, ".o-mail-NotificationItem [title='Mark As Read']");

    try {
        console.log($(".o-mail-NotificationItem [title='Mark As Read']"));
        setInterval(() => console.log($(".o-mail-NotificationItem [title='Mark As Read']")), 500);
