...
test("Category fold is locally persistent (saved in local storage)", async () => {
    ...
    await openDiscuss();
    await contains(
        ".o-mail-DiscussSidebarCategory:contains('Channels') i[data-icon='chevron_forward']"
    );
    await click(".o-mail-DiscussSidebarCategory-toggler:contains('Channels')");
    await contains(
        ".o-mail-DiscussSidebarCategory:contains('Channels') i[data-icon='keyboard_arrow_down']"
    );
    await contains(
        ".o-mail-DiscussSidebarCategory:contains('Direct messages') i[data-icon='chevron_forward']"
    ); // unchanged
    ...
    await contains(
        ".o-mail-DiscussSidebarCategory:contains('Direct messages') i[data-icon='keyboard_arrow_down']"
    );
    await contains(
        ".o-mail-DiscussSidebarCategory:contains('Channels') i[data-icon='chevron_forward']"
    ); // unchanged
...
test("add and remove channel from favorites updates sidebar", async () => {
    ...
    await openDiscuss();
    await click(
        `${channelContainerSelector} ${generalChannelSelector} button [data-icon='more_horiz']`
    );
    await click(".o-dropdown-item:contains('Add to Favorites')");
    await click(
        `${favoriteContainerSelector} ${generalChannelSelector} button [data-icon='more_horiz']`
    );
