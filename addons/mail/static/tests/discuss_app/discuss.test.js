...
test("receive new chat from chat plays sound only when out-of-focus", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Dumbledore" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
    ...
    let isFocused = false;
    mockService("presence", { isOdooFocused: () => isFocused });
    ...
    // simulate odoo tab has focus + receiving a new message
    isFocused = true;
    await withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
    ...
    await animationFrame();
    await waitForSteps([]);
    // simulate odoo tab has no focus + receiving a new message
    isFocused = false;
    await withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
