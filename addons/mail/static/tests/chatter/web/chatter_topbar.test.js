...
test("rendering with multiple partner followers", async () => {
    ...
    await contains(".o-mail-Follower:eq(0):contains(Jean Michang)");
    await contains(".o-mail-Follower:eq(1):contains(Eden Hazard)");
});
...
test("log note toggling", async () => {
    ...
    await start();
    await openFormView("res.partner", partnerId);
    await contains("button:not(.active):contains(Log note)");
    await contains(".o-mail-Composer", { count: 0 });
    await click("button:contains(Log note)");
    await contains("button.active:contains(Log note)");
...
test("send message toggling", async () => {
    ...
    await start();
    await openFormView("res.partner", partnerId);
    await contains("button:not(.active):contains(Send message)");
    await contains(".o-mail-Composer", { count: 0 });
    await click("button:contains(Send message)");
    await contains("button.active:contains(Send message)");
...
test("attachment counter without attachments", async () => {
    ...
    await openFormView("res.partner", partnerId);
    await contains("button[aria-label='Attach files']");
    await contains("button[aria-label='Attach files']:contains(0)", { count: 0 });
});
