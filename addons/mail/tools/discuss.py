...
class Store:
    ...
    def to(
        ...
        *,
        ...
        for main_channel in bus_channel._bus_channels():
            key = (main_channel, bus_subchannel)
            # Batching is only supported for `mail.record/insert`.
            if notification_type == "mail.record/insert" and key in stores:
            ...
            if notification_type == "mail.record/insert":
                stores[key] = store
            result[key] = store
        return result
    ...
    class Stores(dict[object | tuple, "Store"]):
        ...
        def __getattr__(self, name):
            if name not in {"add", "delete", "_add_field_list"}:
    ...
    class FieldList(UserList):
        ...
        def __init__(self, store, records):
            ...
            for s in (store._internal_store or {}).values():
                self._internal_field_list = Store.FieldList(s, records)
        ...
        def __init__(self, records, bus_target):
            ...
            for record in records:
                ...
                self._field_lists_by_record[record] = [
                    Store.FieldList(store, record)
                    for bus_channel in target._bus_channels()
                    for store in Store.to(bus_channel, bus_subchannel).values()
                ]
