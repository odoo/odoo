...
class Store:
    ...
    def _get_records_data(self, records, fields):
        abstract_fields = [field for field in fields if isinstance(field, Store.AbstractField)]
        ...
        for record, data in zip(records, records_data):
            for field in abstract_fields:
                if not field.predicate_func or field.predicate_func(record):
    ...
    class AbstractField:
        """Base class for fields to be added to the store."""

        def __init__(self, field_name, *, predicate_func=None, rename=None):
    ...
    class Attr(AbstractField):
        ...
        def __init__(self, field_name, value_or_func=None, *, predicate_func=None, rename=None):
            super().__init__(field_name, predicate_func=predicate_func, rename=rename)
            self.value_or_func = value_or_func

        def _get_value(self, record):
            if self.value_or_func is None and self.field_name in record._fields:
                return record[self.field_name]
            if callable(self.value_or_func):
                return self.value_or_func(record)
            return self.value_or_func

    class Relation(AbstractField):
        ...
        def _get_value(self, record):
            ...
            if self.field_name == "thread" and "thread" not in record._fields:
                ...
                target = (record.sudo() if self.sudo else record)[self.field_name]
    ...
    class Many(Relation):
        ...
        def _copy_with_records(self, records, fields=None, **kwargs):
            ...
            res.sorted = kwargs.get("sorted", self.sorted)
            return res
        ...
        def _sort_recods(self):
            if self.sorted:
                self.records = self.records.sorted(self.sorted)
                self.sorted = None
