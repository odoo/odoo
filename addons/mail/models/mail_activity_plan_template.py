...
class MailActivityPlanTemplate(models.Model):
    ...
    next_activities = fields.Many2many('mail.activity.type', string='Next Activities', compute='_compute_next_activities', store=True)
    ...
    def _compute_next_activities(self):
        for template in self:
            template.next_activities = False
            activity_type = template.activity_type_id
            if activity_type.triggered_next_type_id:
                template.next_activities = [Command.set([activity_type.triggered_next_type_id.id])]
            elif activity_type.suggested_next_type_ids:
                template.next_activities = [Command.set(activity_type.suggested_next_type_ids.ids)]
