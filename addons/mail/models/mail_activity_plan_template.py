...
class MailActivityPlanTemplate(models.Model):
    ...
    activity_type_id = fields.Many2one(
        ...
        domain="['|', ('res_model', '=', False), '&', ('res_model', '!=', False), ('res_model', '=', parent.dedicated_to_res_model)]",
    ...
    dedicated_to_res_model = fields.Char(related="plan_id.dedicated_to_res_model")
    ...
    def _check_activity_type_res_model(self):
        ...
        "plan_id.res_model_ids". That's why this method is called by those models
        when the mentioned fields are updated.
        As template can be created before plan in the interface, template.plan_id.res_model_ids
        can return False while it shouldn't. To avoid that, we use a computed, precomputed stored
        field on plan: dedicated_to_res_model (through the related field).
        """
        for template in self:
            activity_type_model = template.activity_type_id.res_model
            if activity_type_model and template.dedicated_to_res_model != activity_type_model:
