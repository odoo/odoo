...
class LinkPreviewMessage(models.Model):
    ...
    message_id = fields.Many2one("mail.message", index=True)
    is_hidden = fields.Boolean(default=False)

    def init(self):
        self.env.cr.execute("CREATE UNIQUE INDEX IF NOT EXISTS mail_link_preview_id_message_id ON %s (link_preview_id, message_id)" % self._table)
    ...
    def _unlink_and_notify(self):
        ...
        for link_preview_message in self:
            link_preview_message._bus_send_store(
                ...
                {"linkPreviews": Store.many(link_preview_message.link_preview_id, "DELETE", only_id=True)},
            )
        self.unlink()
