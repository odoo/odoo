...
class MailTemplate(models.Model):
    ...
    def send_mail_batch(self, res_ids, force_send=False, raise_exception=False, email_values=None,
        ...
        :param list res_ids: IDs of records on which template will be rendered
        ...
        for res_ids_chunk in tools.split_every(gen_batch_size, res_ids):
            ...
            if sending_email_layout_xmlid and self.lang:
                res_ids_langs = self._render_lang(res_ids_chunk)
                langs = [res_ids_langs[res_id] for res_id in res_ids_chunk]
                res_ids_companies = records._mail_get_companies(default=self.env.company)
                companies = [res_ids_companies[res_id] for res_id in res_ids_chunk]
            else:
                langs = [False] * len(res_ids_chunk)
                companies = [self.env.company] * len(res_ids_chunk)

            for values, record, lang, company in zip(values_list, records, langs, companies):
            ...
            # manage attachments
            for mail, attachments, attachment_ids in zip(mails, attachments_list, attachments_ids_list):
                attachment_ids += [
                    (0, 0, {
                        'name': name,
                        'datas': datas,
