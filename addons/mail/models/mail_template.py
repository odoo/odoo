    ...
    def _compute_has_dynamic_reports(self):
        number_of_dynamic_reports_per_model = dict(
            self.env['ir.actions.report'].sudo()._read_group(
                domain=[('binding_model_id', 'in', self.model_id.ids)],
                groupby=['binding_model_id'],
                aggregates=['id:count'],
                having=[('__count', '>', 0)]))
        for template in self:
            template.has_dynamic_reports = template.model_id in number_of_dynamic_reports_per_model
