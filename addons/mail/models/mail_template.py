...
class MailTemplate(models.Model):
    ...
    report_template_ids = fields.Many2many(
        'ir.actions.report', 'mail_template_ir_actions_report_rel',
...
'default_template_id' : %d}""" % (template.model, template.id),
    ...
    def _generate_template_attachments(self, res_ids, render_fields,
        ...
        :param list render_fields: list of fields to render on template which
          are specific to recipients, e.g. attachment_ids or report_template_ids;
        ...
        # pre-browse records to be sure to have it in cache
    ...
    def _generate_template_recipients(self, res_ids, render_fields,
                                      partners_only=False, render_results=None):
        ...
        :return: updated (or new) render_results. It holds a 'partner_ids' key
          ...
          generated based on 'partner_to'. Îf ``partners_only`` is False emails
          are present, otherwise added in ``partner_ids``.
        """
        self.ensure_one()
        if render_results is None:
            render_results = {}
    ...
    def _generate_template_scheduled_date(self, res_ids, render_results=None):
        ...
        for res_id in res_ids:
            scheduled_date = scheduled_dates.get(res_id)
            if scheduled_date:
    ...
    def _generate_template_static(self, res_ids, render_fields, render_results=None):
        """ Render static values based on template 'self'. Those are not rendered
        and do not support dynamic values.
    ...
    def _generate_template(self, res_ids, render_fields,
        ...
        :param boolean partners_only: transform emails into partners (see
        ...
        for _lang, (template, template_res_ids) in self._classify_per_lang(res_ids).items():
            ...
            template._generate_template_static(
                template_res_ids,
                render_fields_set,
                render_results=render_results
            )
