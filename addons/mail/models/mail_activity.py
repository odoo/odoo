        ...
        for (res_id, activity_type_id), activities in grouped_activities:
            ...
            data = {
                'count_by_state': dict(Counter(
                    map(lambda a: self._compute_state_from_dates(a['date_done'], a['date_deadline'], user_tz),
                        activities)
                )),
                'ids': [v['id'] for v in activities],
                'o_closest_date': date_done if is_all_activities_done else date_deadline,
                'state': self._compute_state_from_dates(is_all_activities_done, date_deadline, user_tz),
