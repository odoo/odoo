...
class MailThread(models.AbstractModel):
    ...
    def _routing_create_bounce_email(self, email_from, body_html, message, **mail_values):
        bounce_to = mail.decode_message_header(message, 'Return-Path') or email_from
        bounce_mail_values = {
            'author_id': False,
            'body_html': body_html,
        ...
        if bounce_from := self.env.company.bounce_email:
            email_from = mail.formataddr(('MAILER-DAEMON', bounce_from))
        if not email_from:
            ...
            if not any(catchall_email in message['To'] for catchall_email in catchall_aliases):
                email_from = mail.decode_message_header(message, 'To')
        if not email_from:
            email_from = mail.formataddr(('MAILER-DAEMON', self.env.user.email_normalized))
    ...
    def _detect_is_bounce(self, message, message_dict):
        ...
        email_to_list = [
            mail.email_normalize(e) or e
            for e in (mail.email_split(message_dict['to']) or [''])
        ]
        if bounce_aliases and any(email in bounce_aliases for email in email_to_list):
            return True

        email_from = message_dict['email_from']
        email_from_localpart = (mail.email_split(email_from) or [''])[0].split('@', 1)[0].lower()
    ...
    def _detect_loop_sender(self, message, message_dict, routes):
        ...
        email_from_normalized = mail.email_normalize(email_from)
        ...
        for model in models:
            ...
            if mail_incoming_messages_count >= LOOP_THRESHOLD:
                ...
                references = (
                    ...
                    + mail.generate_tracking_message_id('loop-detection-bounce-email')
                )
    ...
    def _detect_write_to_catchall(self, msg_dict):
        ...
        email_to_list = [
            mail.email_normalize(e) or e
            for e in (mail.email_split(msg_dict['to']) or [''])
        ]
    ...
    def message_route(self, message, message_dict, model=None, thread_id=None, custom_values=None):
        ...
        msg_references = [
            ...
            for ref in mail.mail_header_msgid_re.findall(thread_references)
        ...
        email_from = message_dict['email_from']
        email_to_list = [e.lower() for e in (mail.email_split(message_dict['to']) or [''])]
        ...
        rcpt_tos_list = [e.lower() for e in (mail.email_split(message_dict['recipients']) or [''])]
    ...
    def _message_parse_extract_payload(self, message, message_dict, save_original=False):
        ...
        if message.get_content_maintype() == 'text':
            ...
            if message.get_content_type() == 'text/plain':
                # text/plain -> <pre/>
                body = mail.append_content_to_html('', body, preserve=True)
            elif message.get_content_type() == 'text/html':
                # we only strip_classes here everything else will be done in by html field of mail.message
                body = mail.html_sanitize(body, sanitize_tags=False, strip_classes=True)
        else:
            alternative = False
            mixed = False
            html = ''
            for part in message.walk():
                ...
                if part.get_content_type() == 'text/plain' and (not alternative or not body):
                    body = mail.append_content_to_html(body, mail.ustr(content,
                ...
                # 3) text/html -> raw
                    ...
                    if not append_content:
                        ...
                        body = mail.append_content_to_html(body, html, plaintext=False)
                    # we only strip_classes here everything else will be done in by html field of mail.message
                    body = mail.html_sanitize(body, sanitize_tags=False, strip_classes=True)
    ...
    def _message_parse_extract_bounce(self, email_message, message_dict):
        ...
        if dsn_part and len(dsn_part.get_payload()) > 1:
            ...
            final_recipient_data = mail.decode_message_header(dsn, 'Final-Recipient')
            # old servers may hold void or invalid Final-Recipient header
            if final_recipient_data and ";" in final_recipient_data:
                bounced_email = mail.email_normalize(final_recipient_data.split(';', 1)[1].strip())
            if bounced_email:
    ...
    def message_parse(self, message, save_original=False):
        ...
        if message.get('Subject'):
            msg_dict['subject'] = mail.decode_message_header(message, 'Subject')

        email_from = mail.decode_message_header(message, 'From', separator=',')
        email_cc = mail.decode_message_header(message, 'cc', separator=',')
        email_from_list = mail.email_split_and_format(email_from)
        email_cc_list = mail.email_split_and_format(email_cc)
        ...
        msg_dict['recipients'] = ','.join(set(formatted_email
            for address in [
                mail.decode_message_header(message, 'Delivered-To', separator=','),
                mail.decode_message_header(message, 'To', separator=','),
                mail.decode_message_header(message, 'Cc', separator=','),
                mail.decode_message_header(message, 'Resent-To', separator=','),
                mail.decode_message_header(message, 'Resent-Cc', separator=',')
            ] if address
            for formatted_email in mail.email_split_and_format(address))
        )
        ...
        partner_ids = [x.id for x in self._mail_find_partner_from_emails(mail.email_split(msg_dict['recipients']), records=self) if x]
        msg_dict['partner_ids'] = partner_ids
        # compute references to find if email_message is a reply to an existing thread
        msg_dict['references'] = mail.decode_message_header(message, 'References')
        msg_dict['in_reply_to'] = mail.decode_message_header(message, 'In-Reply-To').strip()

        if message.get('Date'):
            try:
                date_hdr = mail.decode_message_header(message, 'Date')
    ...
    def _get_bounced_message_data(self, message, message_dict):
        ...
        for header in headers:
            value = mail.decode_message_header(message, header)
            references = mail.mail_header_msgid_re.findall(value)
        ...
        reference_ids.extend(mail.mail_header_msgid_re.findall(message_dict['in_reply_to']))
        reference_ids.extend(mail.mail_header_msgid_re.findall(message_dict['references']))
    ...
    def _get_parent_message(self, msg_dict):
        ...
        reference_ids = mail.mail_header_msgid_re.findall(msg_dict.get('references') or '')
        if reference_ids:
            parent = self.env['mail.message'].search(
    ...
    def _mail_find_user_for_gateway(self, email_value, alias=None):
        ...
        normalized_email = mail.email_normalize(email_value)
    ...
    def _mail_find_partner_from_emails(self, emails, records=None, force_create=False, extra_domain=False):
        ...
        normalized_emails = [email_normalized
                             for email_normalized in (mail.email_normalize(contact, strict=False) for contact in emails)
                             if email_normalized
                            ]
        ...
        for contact in emails:
            normalized_email = mail.email_normalize(contact, strict=False)
    ...
    def _process_attachments_for_post(self, attachments, attachment_ids, message_values):
        ...
        if attachments: # generate
            ...
            if body:
                root = lxml.html.fromstring(mail.ustr(body))
    ...
    def message_notify(self, *,
        ...
        msg_values = {
            ...
            'subtype_id': subtype_id,
            # recipients
            'message_id': mail.generate_tracking_message_id('message-notify'),
            'partner_ids': partner_ids,
            # notification
    ...
    def _message_log_batch(self, bodies, subject=False,
        ...
        base_message_values = {
            ...
            'message_id': mail.generate_tracking_message_id('message-notify'),  # why? this is all but a notify
            'partner_ids': partner_ids,
    ...
    def _notify_by_web_push_prepare_payload(self, message, msg_vals=False):
        ...
        payload['options']['body'] = mail.html2plaintext(body)
