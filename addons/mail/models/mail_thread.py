...
class MailThread(models.AbstractModel):
    ...
    def _generate_empty_attachment_link_names(self, html):
        """ Add a label to document links generated by web editor.
        These links can be identified by having o_image class and file name
        set in title attribute.
        A width:100% style is also required to display the links in a single
        line.
        """
        if not html:
            return html
        ...
        def _sub_text_with_title(match):
            return match.group(1) + match.group(2) + match.group(3) + match.group(4) + ';width:100%' + match.group(5) + match.group(2) + '<a>'

        html = re.sub(r"""(<a(?=\s)[^>]*\sclass="[^"]*o_image[^"]*"[^>]*\stitle=")([^"]+)("[^>]*\sstyle=")([^"]+)("[^>]*>)</a>""", _sub_text_with_title, html)
   ...
   Only allowed people can contact us at this address.</p>
    ...
    def _notify_record_by_email(self, message, recipients_data, msg_vals=False,
        ...
        for recipients_group_data in recipients_groups_data:
            ...
            mail_body = self._generate_empty_attachment_link_names(mail_body)
