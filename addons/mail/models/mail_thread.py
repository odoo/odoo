...
class MailThread(models.AbstractModel):
    ...
    def _message_track_post_template(self, changes):
        ...
        for _field_name, (template, post_kwargs) in templates.items():
            ...
            if composition_mode == 'mass_mail':
                self._fallback_lang().message_mail_with_source(template, **post_kwargs)
            else:
                self._fallback_lang().message_post_with_source(template, **post_kwargs)
        return True
    ...
    def message_post(self, *,
        ...
        self._raise_for_invalid_fields([kwargs.keys()], {'model', 'res_id', 'subtype'})
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        return self._message_set_main_attachment_id([att[1] for att in msg_vals['attachment_ids'] or []])
    ...
    def message_mail_with_source(self, source_ref, render_values=None,
        ...
        through mail.thread and lessen usage of composer itself.s
        ...
        # preliminary value safety check
        self._raise_for_invalid_fields([kwargs], {'body', 'composition_mode', 'model', 'res_id', 'values'})
    ...
    def message_post_with_source(self, source_ref, render_values=None,
        ...
        # preliminary value safety check
        self._raise_for_invalid_fields(
            [kwargs.keys()],
            {'body', 'composition_mode', 'model', 'res_id', 'values'}
        )
    ...
    def message_notify(self, *,
        ...
        # preliminary value safety check
        self._raise_for_invalid_fields([kwargs.keys()], {'message_id', 'message_type'})
    ...
    def _message_log_with_view(self, views_or_xmlid, render_values=None,
        ...
        :param views_or_xmlid: either an xmlid of a view (template), either directly
          an ir.ui.view record. Used to render body;
        :param dict render_values: additional rendering values for qweb context;
        :param str message_type: one of 'notification' or 'comment';
        ...
        """
        self._raise_for_invalid_fields([kwargs.keys()], {'body', 'bodies'})
    ...
    def _message_create(self, values_list):
        ...
        # preliminary value safety check
        self._raise_for_invalid_fields(
            (values.keys() for values in values_list),
            self._get_invalid_field_names()
        )
    ...
    def _get_invalid_field_names(self):
        return {
            'author_avatar',
            'canned_response_ids',
            'child_ids',
    ...
    def _get_source_from_ref(self, source_ref):
        ...
        if isinstance(source_ref, models.BaseModel):
            ...
            if res_model == 'mail.template':
                template = self.env['mail.tempalte'].browse(res_id)
    ...
    def _raise_for_invalid_fields(self, field_names_list, invalid_field_names):
        """ Helper to warn about invalid fields.

        :param iter field_names_list: an iterable containing field names
        :param set invalid_field_names: set of fields that should not be present
          in field_names_list
        """
        conflicting_field_names = set()
        for field_names in field_names_list:
            conflicting_field_names |= invalid_field_names & set(field_names)
        if conflicting_field_names:
            raise ValueError(
                _('Those values are not supported when posting or notifying: %(field_names)s',
                  field_names=', '.join(conflicting_field_names))
            )
    ...
    def _notify_thread_by_email(self, message, recipients_data, msg_vals=False,
        ...
        # group recipients into groups that receive the same global email layout
        ...
        # prepare values for notification layout
        ...
        for recipients_group_data in recipients_groups_data:
            ...
            recipients_ids_all |= set(recipients_ids)
        ...
        if resend_existing and recipients_ids_all:
            existing_notifications = self.env['mail.notification'].sudo().search([
                ('mail_message_id', '=', message.id),
                ('notification_type', '=', 'email'),
                ('res_partner_id', 'in', list(recipients_ids_all))
            ])

        emails = SafeMail.create([mail_values for _rids, mail_values in mail_lines_all])
        notification_lines = []
        for mail, (mail_recipient_ids, _mail_values) in zip(emails, mail_lines_all):
            if resend_existing:
                notifs = existing_notifications.filtered(
                    lambda notif: notif.res_partner_id.id in mail_recipient_ids
                )
                tocreate_pids = [rid for rid in mail_recipient_ids if rid not in notifs.res_partner_id.ids]
                if notifs:
                    notifs.write({
                ...
                tocreate_pids = mail_recipient_ids
            notification_lines += [{
                'author_id': message.author_id.id,
            ...
            } for recipient_id in tocreate_pids]

        if notification_lines:
            SafeNotification.create(notification_lines)
