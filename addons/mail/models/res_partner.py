...
class Partner(models.Model):
    ...
    def _notify_prepare_email_values(self, message, notif_values):
        ...
        if message.res_id and message.model:
            custom_values = self.env['mail.thread']._notify_specific_email_values_on_record(message, record=self.env[message.model].browse(message.res_id))
    ...
    def _notify_send(self, body, subject, recipients, **mail_values):
        ...
        for email_chunk in split_every(50, recipients.ids):
            ...
            recipient_values = self.env['mail.thread']._notify_email_recipients_on_record(message, email_chunk, record=tig)
            create_values = {
                'body_html': body,
                'subject': subject,
            }
    ...
    def _notify(self, message, layout=False, force_send=False, send_after_commit=True, values=None):
        ...
        recipients = self.env['mail.thread']._notify_classify_recipients_on_record(message, self, record=tig)
