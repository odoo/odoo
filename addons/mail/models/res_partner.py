...
class Partner(models.Model):
    ...
    def _notify(self, message, record, recipient_data, layout=False, force_send=False, send_after_commit=True, values=None):
        ...
        base_mail_values.update(self.env['mail.thread']._notify_specific_email_values_on_record(message, record=record))

        # classify recipients: actions / no action
        recipients = self.env['mail.thread']._notify_classify_recipients_on_record(message, email_rdata, record=record)
        ...
        for rvals in recipients.values():
            ...
            body = base_template.render(template_values, engine='ir.qweb', minimal_qcontext=True),
            ...
            for email_chunk in split_every(50, rvals['recipients']):
                recipient_values = self.env['mail.thread']._notify_email_recipients_on_record(message, email_chunk, record=record)
