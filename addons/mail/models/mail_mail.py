...
class MailMail(models.Model):
    ...
    def _filter_mail_mail_servers(self, mail_servers):
        ...
        return mail_servers.filtered(lambda server: server.owner_user_id in [self.env['res.users'], self.create_uid])
    ...
    def _split_by_mail_configuration(self):
        ...
        all_mail_servers = IrMailServer.sudo().search([], order='sequence, id')
    ...
    def send(self, auto_commit=False, raise_exception=False, post_send_callback=None):
        ...
        for mail_server_id, alias_domain_id, smtp_from, batch_ids in self._split_by_mail_configuration():
            ...
            try:
                IrMailServer = self.env['ir.mail_server']
                smtp_session = IrMailServer.connect(mail_server_id=mail_server_id, smtp_from=smtp_from)
