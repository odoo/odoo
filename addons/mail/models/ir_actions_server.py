...
class IrActionsServer(models.Model):
    ...
    def _compute_activity_plan_id(self):
        to_reset = self.filtered(lambda act: act.state != 'next_activity' or (act.activity_plan_id.res_model and act.model_id.model != act.activity_plan_id.res_model) or act.activity_type_id)
        to_reset.activity_plan_id = False

    @api.onchange('activity_plan_id')
    def _onchange_activity_plan_id(self):
        if self.activity_plan_id:
            self._compute_activity_type_id()
    ...
    def _onchange_activity_type_id(self):
        if self.activity_type_id:
            self._compute_activity_plan_id()
    ...
    @api.depends('state', 'activity_plan_id')
    def _compute_activity_user_type(self):
        to_reset = self.filtered(lambda act: act.state != 'next_activity' or (act.activity_plan_id and not act.activity_plan_id.has_user_on_demand))
