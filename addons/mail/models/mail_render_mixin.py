...
from odoo.tools import is_html_empty, html_normalize
...
class MailRenderMixin(models.AbstractModel):
    ...
    def _has_complex_expression(self):
        for template in self.sudo():
            for fname, field in template._fields.items():
                ...
                if engine in ('qweb', 'qweb_view'):
                    if self._has_complex_expression_template_qweb(template[fname]):
                        return True
                else:
                    if self._has_complex_expression_template_inline_template(template[fname]):
                        return True
        return False

    @api.model
    def _has_complex_expression_template_qweb(self, template_src):
        if template_src:
            try:
    ...
    @api.model
    def _has_complex_expression_template_inline_template(self, template_txt):
        if template_txt:
    ...
    def _check_access_right_dynamic_template(self):
        if not self.env.su and not self.env.user.has_group('mail.group_mail_template_editor') and self._has_complex_expression():
    ...
    def _render_template_qweb(self, template_src, model, res_ids,
        ...
        if not self._has_complex_expression_template_qweb(template_src):
            # do not call the qweb engine
            return self._render_template_qweb_static(template_src, model, res_ids)
    ...
    @api.model
    def _render_template_qweb_static(self, template_src, model, res_ids):
        ...
        for record in records:
            def replace(match):
                ...
                if not self.env['ir.qweb']._is_expression_allowed(expr):
                    raise SyntaxError("Invalid expression for the static mode")

                try:
                    value = reduce(lambda r, f: r[f], expr.split('.')[1:], record) or default
                except KeyError:
                    value = default
    ...
    def _render_template_inline_template(self, template_txt, model, res_ids,
        ...
        if not self._has_complex_expression_template_inline_template(str(template_txt)):
            # do not call the qweb engine
            return self._render_template_inline_template_static(str(template_txt), model, res_ids)
    ...
    @api.model
    def _render_template_inline_template_static(self, template_txt, model, res_ids):
        """Render the inline template in static mode, without calling safe eval."""
        records = self.env[model].browse(res_ids)
        result = {}
        for record in records:
            renderer = []
            for string, expression, default in parse_inline_template(str(template_txt)):
                renderer.append(string)
                if expression:
                    if not self.env['ir.qweb']._is_expression_allowed(expression):
                        raise SyntaxError("Invalid expression for the static mode")
                    try:
                        value = reduce(lambda r, f: r[f], expression.split('.')[1:], record) or default
                    except KeyError:
                        value = default
