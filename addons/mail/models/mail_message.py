    ...
    * snailmail.letter (snailmail addon)
    ...
    def _to_store_defaults(self):
        ...
        field_names = [
            ...
            Store.Many("partner_ids", "name", rename="recipients", sudo=True),
    ...
    def _author_to_store(self, store: Store):
        for message in self:
            ...
            if guest_author := message.sudo().author_guest_id:
                data["author"] = Store.One(guest_author, self.env["mail.guest"]._avatar_fields())
            # sudo: mail.message: access to author is allowed
                ...
                data["author"] = Store.One(
                    ...
                    [
                        *self.env["res.partner"]._avatar_fields(),
                        Store.Many("user_ids", ["active", "share"]),
                    ],
                )
            store.add(message, data)
