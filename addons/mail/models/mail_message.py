...
class Message(models.Model):
    ...
    @api.multi
    def set_thread_read(self):
        print 'setting thread as read'
        return True

    @api.multi
    def set_message_needaction(self, needaction):
        if needaction:
            self.write({'partner_ids': [(4, self.env.user.partner_id.id)]})
        return True
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        """
        # print '_search', args, offset, limit, order, count, access_rights_uid
    ...
    def check_access_rule(self, operation):
        ...
        if operation == 'create':
            ...
            # print 'not_parent_ids', not_parent_ids
        ...
        if operation == 'read':
            ...
            for doc_model, doc_ids in model_record_ids.items():
                followers = self.env['mail.followers'].sudo().search([
                    ...
                    # ('channel_id', '=', self.env.user.channel_id.id)
                    ])
        ...
        if not other_ids:
            return
        # print 'mail_message check access rule', other_ids
        raise AccessError(
            _('The requested operation cannot be completed due to security restrictions. Please contact your system administrator.\n\n(Document type: %s, Operation: %s)') %
            (self._description, operation))
