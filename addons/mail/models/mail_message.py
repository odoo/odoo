    ...
    def _find_allowed_doc_ids(self, model_ids):
        ...
        for doc_model, doc_dict in model_ids.items():
            if not IrModelAccess.check(doc_model, 'read', False):
                continue
            documents = self.env[doc_model].with_context(active_test=False).search([('id', 'in', list(doc_dict))])
            allowed_documents = self.env[doc_model].with_prefetch(documents.ids)
            operation_res_ids = documents._mail_group_by_operation_for_mail_message_operation('read')
            # filter for each operation
            for operation, documents in operation_res_ids.items():
                if operation == "read":  # already implied by 'search'
                    allowed_documents += documents
                else:
                    allowed_documents += documents._filtered_access(operation)
            allowed_ids |= {
