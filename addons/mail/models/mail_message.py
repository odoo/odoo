    ...
    * snailmail.letter (snailmail addon)
    ...
    def _to_store_defaults(self):
        ...
        field_names = [
            ...
            Store.Many("attachment_ids", sorted="id", sudo=True),
            "body",
            "create_date",
            "date",
            ...
            Store.Attr(
                "link_preview_ids",
                lambda m: Store.Many(m.sudo().link_preview_ids.filtered(lambda l: not l.is_hidden)),
            ),
            "message_format",
            "message_type",
        ...
        if self.env.user._is_internal():
            field_names.append(
                Store.Attr(
                    "notification_ids",
                    lambda m: Store.Many(m.notification_ids._filtered_for_web_client()),
                )
            )
        return field_names
    ...
    def _to_store(self, store: Store, fields, *, format_reply=True, msg_vals=None,
        ...
        record_fields = [
            ...
            Store.Attr(
                ...
                predicate_func=lambda record: record._name != "discuss.channel",
            ),
            Store.Attr(
                "module_icon",
                lambda record: modules.module.get_module_icon(self.env[record._name]._original_module),
                predicate_func=lambda record: self.env[record._name]._original_module,
            ),
        ]
        if for_current_user and add_followers and non_channel_records:
