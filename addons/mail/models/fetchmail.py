...
class POP3Connection:
    ...
    def check_unread_messages(self):
        (num_messages, total_size) = self.stat()
        self.list()
    ...
    def retrieve_unread_messages(self):
        while self._unread_messages:
            num = self._unread_messages.pop()
            (header, messages, octets) = self.retr(num)
            message = (b'\n').join(messages)
            yield num, message
...
odoo_mailgate: "|/path/to/odoo-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
    ...
    def _fetch_mails(self, **kw):
        ...
        if not self.search_count(MAIL_SERVER_DOMAIN):
            ...
            self.env['ir.cron']._notify_progress(done=0, remaining=0, deactivate=True)
    ...
    def _fetch_mail(self, batch_limit=50):
        ...
        for server in self:
            ...
            try:
                ...
                for message_num, message in itertools.islice(server_connection.retrieve_unread_messages(), batch_limit):
                    count += 1
                    total_remaining -= 1
                    try:
                ...
                if not server.error_date:
                    ...
                    _logger.info("Deactivating %s server %s (too many failures)", *server_type_and_name)
                    server.set_draft()
            finally:
                try:
