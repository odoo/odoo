...
class MailTracking(models.Model):
    ...
    def _create_tracking_values(self, initial_value, new_value, col_name, col_info, record):
        ...
        if col_info['type'] in {'integer', 'float', 'char', 'text', 'datetime'}:
            ...
            values.update({
                'old_value_char': ', '.join(value.display_name or _('Unnamed %(record_model_name)s (%(record_id)s)', record_model_name=_(value._description), record_id=value.id) for value in initial_value) if initial_value else '',
                'new_value_char': ', '.join(value.display_name or _('Unnamed %(record_model_name)s (%(record_id)s)', record_model_name=_(value._description), record_id=value.id) for value in new_value) if new_value else '',
            })
        else:
