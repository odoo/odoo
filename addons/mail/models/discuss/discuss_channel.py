...
class DiscussChannel(models.Model):
    ...
    @api.depends('channel_type', 'default_display_mode', 'image_128', 'name', 'uuid',
    ...
    def _generate_avatar(self):
        ...
        if self.channel_type == "chat":
            ...
            if not correspondent or correspondent.image_128:
                return False
            return self._generate_text_avatar(_avatar_initials(correspondent.name), str(correspondent.id))
        if self.channel_type == "group":
            if self.default_display_mode == "video_full_screen":
                return self._generate_text_avatar(str(self.create_date.day) if self.create_date else "", str(self.id))
            bgcolor, fgcolor = get_light_ui_color_from_seed(str(self.id))
            avatar = group_avatar.replace('fill="#875a7b"', f'fill="{bgcolor}"').replace('fill="#ffffff"', f'fill="{fgcolor}"')
            return BinaryBytes(avatar.encode())
        return self._generate_text_avatar(_avatar_initials(self.name)[:1], str(self.id))

    def _generate_text_avatar(self, text, seed):
        """Build an SVG avatar showing ``text`` over a light, ``seed``-stable color.
