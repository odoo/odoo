...
class DiscussChannel(models.Model):
    ...
    def _is_accessible_for_current_user(self):
        """ Check that the current user isn't restricted from accessing this channel by its
        group restriction (independently of any invitation token they may have).
        :return: True if there is no group restriction, or the current user belongs to the
            required group.
        """
        self.ensure_one()
        # sudo - res.groups: can access group public id of parent channel to determine if we
        # can access the channel.
        group_public_id = self.sudo().group_public_id or self.parent_channel_id.sudo().group_public_id
        return not group_public_id or group_public_id in self.env.user.all_group_ids
