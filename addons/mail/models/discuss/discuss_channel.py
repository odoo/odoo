...
class DiscussChannel(models.Model):
        ...
        if "member_indices" in vals and (
            ...
            raise UserError(
                self.env._(
                    ...
                    channel_names=", ".join(failing_channels.mapped("display_name")),
                ),
            )
    ...
    def _get_or_create_chat(self, partners_to):
        ...
        # sudo: discuss.channel - a chat is not readable by a member that left, but must still be found
        ...
        if channel and create_members:
            # sudo: discuss.channel - a member that left cannot write the chat, only its own combination is added
