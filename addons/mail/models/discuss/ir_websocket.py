    ...
    def _build_bus_channel_list(self, channels):
        ...
        for channel in channels:
            if isinstance(channel, str):
                match = re.findall(r'discuss\.channel.*?(\b\d+\b)', channel)
                if match:
                    channel_ids.append(int(match[0]))
        guest = self.env["mail.guest"]._get_guest_from_context()
        if guest:
            channels.append(guest)
        channels.extend(self.env["discuss.channel"].search(["|", ("is_member", "=", True), ("id", "in", channel_ids)]))
