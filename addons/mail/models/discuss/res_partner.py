...
class ResPartner(models.Model):
    ...
    def get_mention_suggestions_from_channel(self, channel_id, search, limit=8):
        ...
        store.add(channel, {"authorizedGroupId": channel.group_public_id.id or None})
        for p in partners:
            store.add(p,{
                "group_id": next((group.id for group in p.user_ids.groups_id if group.id == channel.group_public_id.id), None)
            })
