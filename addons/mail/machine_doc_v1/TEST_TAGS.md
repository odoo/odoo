...
(`tests/`, 72 `test_*.py` files) and JavaScript HOOT (`static/tests/`, 156 `*.test.js`).
...
etc.). Of **142** tagged classes, **113** carry `post_install`/`-at_install`. Note both
...
**41 of the 72 test files carry no topic tag at all** and are reachable only by the module
