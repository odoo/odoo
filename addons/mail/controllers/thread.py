...
class ThreadController(http.Controller):
    ...
    def mail_thread_messages(self, thread_model, thread_id, search_term=None, before=None, after=None, around=None, limit=30, **kwargs):
        thread = request.env[thread_model]._get_thread_with_access(thread_id, **kwargs)
        if not thread:
            raise Forbidden()
        domain = self._get_fetch_domain(thread_model, thread_id)
        # sudo: mail.thread - reading messages on accessible thread is allowed
        res = thread.sudo().env["mail.message"]._message_fetch(domain, search_term=search_term, before=before, after=after, around=around, limit=limit)
