...
class ThreadController(http.Controller):
    ...
    def _prepare_message_data(self, post_data, thread, partner_emails=None, **kwargs):
        res = {
            key: value
        ...
        res["partner_ids"] = self._filter_message_post_partners(
            partners,
            post_data.get("partner_ids_mention_token", {}),
        ).ids
        return res

    def _filter_message_post_partners(self, partners, token_by_pid):
        return partners.filtered(
            lambda p: (not self.env.user.share and p.has_access("read"))
            or (
                (token := token_by_pid.get(str(p.id)))
                and verify_limited_field_access_token(p, "id", token, scope="mail.message_mention")
            ),
        )
    ...
    def mail_message_post(self, thread_model, thread_id, post_data, context=None, **kwargs):
        ...
        message = thread.sudo().message_post(
            **self._prepare_message_data(post_data, thread, **kwargs),
        )
    ...
    def mail_message_update_content(self, message_id, body, attachment_ids, attachment_tokens=None, partner_ids=None, **kwargs):
        ...
        update_data = {
            ...
            "partner_ids_mention_token": kwargs.get("partner_ids_mention_token", {}),
        }
        thread._message_update_content(
            message,
            **self._prepare_message_data(update_data, thread, **kwargs),
        )
