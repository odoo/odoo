...
class PollController(ThreadController):
    ...
    def poll_create(
        ...
        thread_target = thread._store_target()
        Store.to(thread_target[0], bus_subchannel=thread_target[1])
        return poll.id
    ...
    def poll_vote(self, poll_id, option_ids):
        ...
        Store.to(self_bus_channel).add(options_sudo, ["selected_by_self"])
        thread_target = thread._store_target()
        store = Store.to(thread_target[0], bus_subchannel=thread_target[1])
    ...
    def poll_remove_vote(self, poll_id):
        ...
        if not votes_sudo:
            ...
            Store.to(user or guest).add(poll_sudo.option_ids, ["selected_by_self"])
            return
        options_sudo = votes_sudo.option_id
        poll_sudo = votes_sudo.option_id.poll_id
        votes_sudo.unlink()
        Store.to(user or guest).add(options_sudo, ["selected_by_self"])
