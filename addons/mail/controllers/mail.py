...
class MailController(http.Controller):
    ...
    def _redirect_to_messaging(cls, channel_id=None, highlight_message_id=None):
        url_params = {}
        if channel_id:
            url_params["active_id"] = channel_id
        if highlight_message_id:
            url_params["highlight_message_id"] = highlight_message_id
        url = f"/odoo/action-mail.action_discuss?{urlencode(url_params)}"
    ...
    def _redirect_to_record(cls, model, res_id, access_token=None, **kwargs):
        ...
        if record_action['type'] == 'ir.actions.act_url':
            split_url = urlsplit(record_action["url"])
            action_url_params = parse_qs(split_url.query)
            action_url_params.update(url_params)
            split_url = split_url._replace(query=urlencode(action_url_params, doseq=True))
