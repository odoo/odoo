...
class LinkPreviewController(http.Controller):
    @http.route("/discuss/link_preview", methods=["POST"], type="json", auth="public")
    @add_guest_to_context
    def discuss_link_preview(self, message_id):
        if not request.env["mail.link.preview"]._is_link_preview_enabled():
            return
        guest = request.env["mail.guest"]._get_guest_from_context()
        message = guest.env["discuss.message"].search([("id", "=", int(message_id))])
        if not message:
            return
    ...
    @http.route("/discuss/link_preview/hide", methods=["POST"], type="json", auth="public")
    @add_guest_to_context
    def discuss_link_preview_hide(self, link_preview_ids):
        guest = request.env["mail.guest"]._get_guest_from_context()
