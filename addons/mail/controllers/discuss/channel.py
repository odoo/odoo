...
class ChannelController(http.Controller):
    @http.route("/discuss/channel/get", methods=["POST"], type="jsonrpc", readonly=True)
    def discuss_channel_get(self, partners_to, pin=True, force_open=False):
        channel = request.env["discuss.channel"]._channel_get(partners_to, pin, force_open)
        return Store(channel).get_result()

    @http.route("/discuss/channel/create", methods=["POST"], type="jsonrpc")
    def discuss_channel_create(self, name, group_id):
        channel = request.env["discuss.channel"]._channel_create(name, group_id)
        return Store(channel).get_result()

    @http.route("/discuss/channel/create_group", methods=["POST"], type="jsonrpc")
