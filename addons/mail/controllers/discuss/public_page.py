...
class PublicPageController(http.Controller):
    ...
    def _response_discuss_channel_invitation(self, store, channel, guest_email=None):
        # Channel access type takes precedence over token.
        if (channel.access_type == "internal" and not self.env.user._is_internal()) or (
            channel.access_type == "invite_only"
            and not channel.self_member_id
            and not channel.parent_channel_id.self_member_id
        ):
