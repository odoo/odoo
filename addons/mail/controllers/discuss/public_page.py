...
class PublicPageController(http.Controller):
    ...
    @http.route("/discuss/channel/add_guest", methods=["POST"], type="json", auth="public")
    @add_guest_to_context
    def add_guest_to_channel(self, channel_id):
        channel = request.env["discuss.channel"].browse(channel_id).exists()
        guest = request.env["mail.guest"]._get_guest_from_context()
        if channel and guest:
            return channel.add_members(guest_ids=guest.ids)
        return False
    ...
    def discuss_channel(self, channel_id):
        ...
        guest_already_member = channel.env["discuss.channel.member"].search([('channel_id', '=', channel.id), ('guest_id', '=', guest.id)])
        store = Store()
        if guest and not guest_already_member:
            store.add({"isChannelTokenSecret": True, "shouldDisplayWelcomeViewInitially": True})
