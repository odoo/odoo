...
class AttachmentController(http.Controller):
    ...
    def mail_attachment_upload(self, ufile, thread_id, thread_model, is_pending=False, **kwargs):
        ...
        try:
            ...
            res = {"data": Store(attachment, extra_fields="as_author_access_token").get_result()}
        except AccessError:
    ...
    def mail_attachment_delete(self, attachment_id, access_token=None):
        ...
        if not attachment:
            ...
            return
        attachment._check_attachments_access([access_token])
        message = request.env["mail.message"].search(
            [("attachment_ids", "in", attachment.ids)], limit=1
        )
        # sudo: ir.attachment: access is validated with _check_attachments_access
