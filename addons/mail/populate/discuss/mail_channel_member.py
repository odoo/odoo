...
class ChannelMember(models.Model):
    ...
    _populate_dependencies = ["res.partner", "res.users", "mail.channel"]

    def _populate(self, size):
        res = super()._populate(size)
        random = populate.Random("mail.channel.member")
        ...
        for batch in batches:
            count += len(batch)
            _logger.info("Batch of mail.channel.member: %s/%s", count, total)
            res += self.env["mail.channel.member"].create(batch)
        return res
