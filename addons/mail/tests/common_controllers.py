...
class MailControllerThreadCommon(MailControllerCommon):
    ...
    def _execute_message_fetch_subtests(self, subtests, record, allowed):
        for user_data, *args in subtests:
            route_kw = args[0] if args else {}
            user = user_data if user_data._name == "res.users" else self.user_public
            guest = user_data if user_data._name == "mail.guest" else self.env["mail.guest"]
            ...
            self._authenticate_pseudo_user(user_data)
            with self.subTest(record=record, user=user, guest=guest, route_kw=route_kw):
                if allowed:
                    ...
                    self.assertEqual(self._message_fetch(record, route_kw), {})
            messages.unlink()
    ...
    def _message_fetch(self, record, route_kw):
        res = self.make_jsonrpc_request(
            ...
            params={
                "fetch_params": [
                    ["/mail/thread/messages", self._message_fetch_params(record, route_kw)]
                ]
            },
        )
        res.pop("__store_version__", None)
        return res
