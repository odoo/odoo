...
class TestCallHistoryLog(MailCommon):
    ...
    def test_logging_the_call_keeps_the_message_it_posted(self):
    ...
    def test_a_recording_uploaded_after_the_call_was_logged_shows_on_its_message(self):
        """The recording is uploaded once the call is over, hence after the message
        logging it was rendered: that message gets the recording icon all the same."""
        message = self._log_call_on_a_new_partner()
    ...
    def test_a_recording_landing_later_is_pushed_to_the_open_chatter(self):
        """The chatter of the document is likely open when the recording lands, the
        meeting having just taken place: the new body reaches it over the bus instead
    ...
    def test_a_recording_uploaded_before_the_call_was_logged_shows_on_its_message(self):
    ...
    def test_a_recording_of_an_unlogged_call_has_no_message_to_update(self):
    ...
    def test_a_call_logged_while_ongoing_is_completed_when_its_last_attendee_leaves(self):
        """Logging a call that is still going on only schedules its activity (see
        `is_call_ongoing`): the call ending is what marks it done and posts it, going
        through the RTC session leaving rather than through `_link_to_activity` by hand."""
        partner = self.env["res.partner"].create({"name": "Test Partner"})
    ...
    def test_an_unknown_caller_gets_no_contact_guessed(self):
        """A call placed to a number no contact holds is about nobody: the wizard leaves
        its contact empty rather than offering a stranger, which Mark Done would then
        persist as the contact the call was with."""
