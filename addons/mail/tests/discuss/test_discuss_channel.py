...
from odoo.addons.base.models.avatar_mixin import get_light_ui_color_from_seed
...
class TestChannelInternals(MailCommon, HttpCase):
    ...
    def _expected_default_avatar(self, text, seed):
        """Rebuild the SVG produced by ``_generate_text_avatar`` for a given text/seed.

        :param str text: the initials or number drawn on the avatar.
        :param str seed: value the light color pair is derived from.
        :return: the expected avatar bytes.
        :rtype: bytes
        """
        bgcolor, fgcolor = get_light_ui_color_from_seed(seed)
        ...
        return (
            ...
            f"<text fill='{fgcolor}' font-size='{font_size}' font-weight='bold' text-anchor='middle' x='90' y='124' font-family='sans-serif'>{html_escape(text)}</text>"
            "</svg>"
        ).encode()
    ...
    def test_channel_should_generate_correct_default_avatar(self):
        ...
        # regular group: the group glyph recolored with the light palette
        bgcolor_group, fgcolor_group = get_light_ui_color_from_seed(str(private_group.id))
        expected_group = group_avatar.replace('fill="#875a7b"', f'fill="{bgcolor_group}"').replace('fill="#ffffff"', f'fill="{fgcolor_group}"').encode()
