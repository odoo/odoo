...
class TestPartner(MailCommon):
    ...
    def test_res_partner_find_or_create_from_emails(self):
        ...
        for (sample, exp_name, exp_email), partner in zip(new_samples, partners):
            exp_company = self.env.company if sample in [
                'Raoul chirurgiens-dentistes.fr',  # invalid email -> create a new partner
                '"New Customer" <new.customer@test.EXAMPLE.com>',  # new customer
                'Invalid'  # invalid email -> create a new partner
    ...
    def test_res_partner_find_or_create_from_emails_dupes(self):
        ...
        # results
        expected_partner_name = "Formatted Customer"  # first found email will setup partner info
        expected_partner_email = 'test.customer@test.dupe.example.com'  # normalized version of given email
        self.assertEqual(len(partners), len(samples))
        self.assertTrue(len(set(partner.id for partner in partners)) == 1 and partners[0].id, 'Should have a unique new partner')
        for partner in partners:
            self.assertEqual(partner.email, expected_partner_email)
            self.assertEqual(partner.name, expected_partner_name)

        new_samples = [
            '"Another Customer" <test.customer2@TEST.DUPE.EXAMPLE.COM',
