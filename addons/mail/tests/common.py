...
class MailCase(common.TransactionCase, MockEmail):
    ...
    def assertMailNotifications(self, messages, recipients_info, bus_notif_count=1):
        ...
        for message_info in recipients_info:
            ...
            if (
                ('subtype_id' in message_values and not message_values['subtype_id'])
                or ('subtype_id' not in message_values and 'subtype' in message_info and not message_info['subtype'])
            ):
                msubtype = None
            else:
