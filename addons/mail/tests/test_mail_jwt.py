...
class TestMailJwt(BaseCase):
    ...
    def test_invalid_json_payload(self):
        ...
        with self.assertRaises(json.JSONDecodeError):
            jwt.verify(
                f"{header}.{bad_payload}.sig",
                secret,
                algorithm=jwt.Algorithm.HS256,
            )

    def test_header_and_payload_must_be_objects(self):
        secret = base64.urlsafe_b64encode(b"secret").decode("ascii")
        valid_header = {"typ": "JWT", "alg": "HS256"}
        for value in (None, False, 5, "header", []):
            for header, payload in ((value, {}), (valid_header, value)):
                with self.subTest(header=header, payload=payload):
                    ...
                    with self.assertRaisesRegex(ValueError, "must be JSON objects"):
                        jwt.verify(
                            f"{header_part}.{payload_part}.AAAA",
                            secret,
                            algorithm=jwt.Algorithm.HS256,
                        )
    ...
    def test_key_without_padding(self):
        raw_secret = b"secretkey123"  # length multiple of 3 bytes = 4 base64 chars
        secret_no_padding = base64.urlsafe_b64encode(raw_secret).decode("ascii")
        self.assertNotIn("=", secret_no_padding)
        claims = {"test": "data"}
        token = jwt.sign(
            claims,
            secret_no_padding,
