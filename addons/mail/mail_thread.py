...
class mail_thread(osv.AbstractModel):
    ...
    def message_subscribe(self, cr, uid, ids, partner_ids, subtype_ids=None, context=None):
        ...
        return self._message_subscribe(cr, uid, ids, partner_ids, subtype_ids=subtype_ids, context=context)

    def _message_subscribe(self, cr, uid, ids, partner_ids, subtype_ids=None, access_rights_uid=None, raise_exception=True, context=None):
        """ Private and enhanced version of follower subscription method. This
        method allows to specify the user used for access rights computation.

        As this method is private and not callable through XML-RPC this is not
        ...
        # # check for security: as most of followers creation / udpate is done as
        # # ADMIN for security and perf reasons, be sure to have the rights to
        # # modify followers
        # user_pid = self.pool['res.users'].browse(cr, uid, uid, context=context).partner_id.id
        # if set(partner_ids) == set([user_pid]):
        #     access_right_mode = 'read'
        # else:
        #     access_right_mode = 'write'
        # access_rights_uid = access_rights_uid or uid
        # try:
        #     self.check_access_rights(cr, access_rights_uid, access_right_mode)
        #     self.check_access_rule(cr, access_rights_uid, ids, access_right_mode)
        ...
        self.write(cr, uid, ids, {'message_subscription_ids': command}, context=context)
        return True
    ...
    def _message_auto_subscribe_misc(self, cr, uid, records, values, context=None):
        ...
        record_followers = {}  # {res_id: {followers-like dict}}
        # if context.get('mail_subscribe_uid'):
        pid = self.pool['res.users'].browse(cr, SUPERUSER_ID, uid).partner_id.id
        followers.setdefault(pid, set())
        return followers, record_followers

    def _message_auto_subscribe_from_fields(self, cr, uid, records, values, context=None):
        """ UPDATE ME """
        followers = {}  # {partner_id: subtype_ids}
        dynamic_fields = []
        print '_message_auto_subscribe_from_fields', records, values
        for name, field in {fname: self._fields[fname] for fname in values.keys()}.items():
            if getattr(field, 'track_subscribe', False) == 'self':
                followers.setdefault(values[name], None)
            elif getattr(field, 'track_subscribe', False) and callable(field.track_subscribe):
                print 'found field', name, field
                # external = self.resolve_2many_commands(cr, uid, name, values[name], ['id'], context=context)
                # print '\t', external
        ...
        if dynamic_fields:
            for record in records:
                for field in dynamic_fields:
                    prout = getattr(field, 'track_subscribe')(self, cr, uid, record, context=context)
                    print prout
                    for caca in prout:
                        record_followers.setdefault(record.id, dict()).setdefault(caca, None)

        print 'ending from fields', followers, record_followers
        return followers, record_followers
    ...
    def message_auto_subscribe(self, cr, uid, records, values, context=None):
        ...
        def _add_followers_dict(src_dict):
            for partner_id, subtype_ids in src_dict.iteritems():
                followers.setdefault(partner_id, set()).update(subtype_ids)

        def _add_record_followers_dict(src_dict):
            for res_id in src_dict:
                record_followers.setdefault(res_id, dict())
                for partner_id, subtype_ids in src_dict.iteritems():
                    record_followers[res_id].setdefault(partner_id, set()).update(subtype_ids)

        misc_nf, misc_rf = self._message_auto_subscribe_misc(cr, uid, records, values, context=context)
        # print misc_nf, misc_rf
        _add_followers_dict(misc_nf)
        _add_record_followers_dict(misc_rf)

        fields_nf, fields_rf = self._message_auto_subscribe_from_fields(cr, uid, records, values, context=context)
        # print fields_nf, fields_rf
        _add_followers_dict(fields_nf)
        _add_record_followers_dict(fields_rf)

        st_nf, st_rf = self._message_auto_subscribe_from_subtypes(cr, uid, records, values, context=context)
        # print st_nf, st_rf
        _add_followers_dict(st_nf)
        _add_record_followers_dict(st_rf)

        print 'followers', followers
        print 'record_followers', record_followers

        command = self.pool['mail.followers']._add_follower_command(cr, uid, self._name, None, followers.keys(), followers, context)  # tde: update none
        print command
        # for pid, subtypes in followers.iteritems():
        #     subtypes = list(subtypes) if subtypes is not None else None
        #     self.message_subscribe(cr, uid, ids, [pid], subtypes, context=context)
        # for res_id, fol_data in record_followers.iteritems():
        #     for pid, subtypes in fol_data.iteritems():
        #         subtypes = list(subtypes) if subtypes is not None else None
        #         self.message_subscribe(cr, uid, [res_id], [pid], subtypes, context=context)

        # self._message_auto_subscribe_notify(cr, uid, ids, followers.keys(), context=context)
