...
class MailThread(models.AbstractModel):
    ...
    def _set_followers(self):
        ...
        for command in self.message_follower_ids:
            if command.id:
                new |= command
            # else:
            #     import pdb
    ...
    # ------------------------------------------------------
    # Technical methods (TDE FIXME: to class when migrating)
    # ------------------------------------------------------
    ...
    def message_redirect_action(self):
        ...
        if msg_id and not (model and res_id):
            msg = self.env['mail.message'].browse(msg_id).exists()  # TDE: check exists in new api
            try:
    ...
    def message_get_reply_to(self, res_ids, default=None):
        # TDE FIXME: ids, model, multi, ... ??? not clear
        ...
        left_ids = set(res_ids).difference(set(aliases.keys()))  # TDE CLEANME: some strange computation here
        if left_ids:
    ...
    def message_get_email_values(self, notif_mail=None):
        ...
        # TDE FIXME: add some explanations here
        self.ensure_one()
        res = dict()
        return res
    ...
    def _message_find_partners(self, message, header_fields=['From']):
        ...
        return filter(lambda x: x, self._find_partner_from_emails(tools.email_split(s)))  # TDE CLEAN: recordset cleaning + call
    ...
    def message_route_process(self, message, message_dict, routes):
        # TDE FIXME: clean weird stuff with mail.thread and things like that
    ...
    def _message_add_suggested_recipient(self, result, partner=None, email=None, reason=''):
        # TDE FIXME; clean calls to that method
    ...
    def message_post(self, body='', subject=None, message_type='notification',
        ...
        attachment_ids = self._message_preprocess_attachments(
            attachments, kwargs.pop('attachment_ids', []), model, self.ids and self.ids[0] or None)  # TDE FIXME/ was using thread_id
        ...
        if subtype:
            ...
            subtype_id = self.env.ref(subtype).id  # TDE FIXME check for raise if not found
        ...
        return new_message  # TDE/ check for api returns
    ...
    def _message_auto_subscribe_notify(self, partner_ids):
        ...
        for record_id in self.ids:
            MailMessage = self.env['mail.message'].sudo()  # TDE FIXME/ not sure for sudo
    ...
    def message_change_thread(self, new_thread):
        ...
        MailMessage = self.env['mail.message']  # TDE FIXME/ not sure for sudo
