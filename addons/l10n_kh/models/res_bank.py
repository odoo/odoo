...
class ResPartnerBankAccount(models.Model):
        ...
        for bank in self.filtered(lambda b: b.country_code == "KH"):
            if bank.proxy_type not in [
                ...
                raise ValidationError(
                    self.env._("The proxy type must be Bakong Account ID")
                )
            if bank.proxy_type in ["bakong_id_solo", "bakong_id_merchant"] and (
                ...
                raise ValidationError(
                    self.env._("Please enter a valid Bakong Account ID.")
                )
    ...
    def _compute_display_qr_setting(self):
        ...
        super(ResPartnerBankAccount, self - bank_kh)._compute_display_qr_setting()

    @api.depends("country_code")
    def _compute_country_proxy_keys(self):
        bank_kh = self.filtered(lambda b: b.country_code == "KH")
        bank_kh.country_proxy_keys = "bakong_id_solo,bakong_id_merchant"
        super(ResPartnerBankAccount, self - bank_kh)._compute_country_proxy_keys()
