...
# PINT AE aligns only this subset of UNCL5305; the base account_edi_ubl_cii selection also
# carries EU/ES/IT codes (G, K, L, M, B) the FTA rejects, so the field is whitelisted in the
# form view. https://docs.peppol.eu/poac/ae/pint-ae/trn-invoice/codelist/Aligned-TaxCategoryCodes/
AE_TAX_CATEGORY_CODES = ['S', 'E', 'O', 'AE', 'Z', 'N']
...
class AccountTax(models.Model):
    ...
    ubl_cii_tax_category_code = fields.Selection(
        # Label follows the FTA code list wording; it is the category used for the margin scheme
        # (see the spec's own "Margin scheme" example, and ibr-116-ae).
        selection_add=[('N', 'N - Standard rate additional VAT')],
        ondelete={'N': 'cascade'},
    )
    ...
    l10n_ae_available_tax_category_codes = fields.Json(compute='_compute_l10n_ae_available_tax_category_codes')
    l10n_ae_available_exemption_reason_codes = fields.Json(compute='_compute_l10n_ae_available_exemption_reason_codes')

    def _compute_l10n_ae_available_tax_category_codes(self):
        for tax in self:
            tax.l10n_ae_available_tax_category_codes = AE_TAX_CATEGORY_CODES
