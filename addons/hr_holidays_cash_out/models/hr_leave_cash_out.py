...
class HrLeaveCashOut(models.Model):
    ...
    leave_type_request_unit = fields.Selection(related="leave_type_id.request_unit")
    ...
    def _check_allow_cash_out_request(self):
        for record in self:
    ...
    @api.constrains('leave_type_id')
    def _check_valid_allocation(self):
        for record in self:
            if not record.leave_type_id.has_valid_allocation:
                msg = "The selected leave type does have a valid allocation."
                raise ValidationError(msg)

    @api.constrains("leave_type_id")
    def _check_allow_employee_request(self):
    ...
    def _compute_display_name(self):
        for cash_out in self:
            time_off_type_display = cash_out.leave_type_id.name or ""
            target = cash_out.employee_id.name or ""
            time_off_type_unit = cash_out.leave_type_request_unit or ""
            if time_off_type_unit and cash_out.quantity > 1:
                time_off_type_unit += "s"
            if time_off_type_display and target:
                time_off_type_display = " on " + time_off_type_display + ": "
            elif target:
                target += ": "
            elif time_off_type_display:
                time_off_type_display += ": "
            cash_out.display_name = _(
                "%(person)s%(leave_type)s%(quantity)s %(unit)s",
                person=target,
                leave_type=time_off_type_display,
                quantity=cash_out.quantity,
                unit=time_off_type_unit,
            )
    ...
    def _compute_company_id(self):
        for cash_out in self:
            cash_out.company_id = (
                cash_out.employee_company_id
                or cash_out.department_id.company_id
                or self.env.company
            )
    ...
    def _compute_from_employee_id(self):
        for cash_out in self:
            if not cash_out.employee_id:
                cash_out.leave_type_id = False
            elif cash_out.employee_id.user_id != self.env.user and cash_out._origin.employee_id != cash_out.employee_id:
                if cash_out.employee_id and not cash_out.leave_type_id.with_context(employee_id=cash_out.employee_id.id).has_valid_allocation:
                    cash_out.leave_type_id = False
...
is approved, validated or refused.",
    ...
    def _action_validate(self, check_state=True):
        ...
        if check_state and any(not cash_out.can_validate for cash_out in self):
            raise UserError(_("You can't validate this cash out."))
    ...
    def action_cancel(self):
        ...
        if not self.can_cancel:
            raise ValidationError(_("This cash-out cannot be cancelled."))
        ...
        for cash_out in cash_outs:
            ...
            if responsible:
                self.env["mail.thread"].sudo().message_notify(
                    ...
                    subject=_("Refused Leave Cash-out"),
                    body=_(
    ...
    def create(self, vals_list):
        ...
        if any(not vals.get("employee_id") for vals in vals_list):
            raise UserError(
                _(
                    "There is no employee set on the cash out. Please make sure you're logged in the correct company.",
                ),
            )
        cash_outs = super(
        ...
        for cash_out in cash_outs:
            ...
            cash_out_sudo.add_follower(cash_out.employee_id.id)
            ...
            if cash_out.validation_type == "no_validation":
                ...
                cash_out_sudo.message_post(
                    body=_("The cash out has been automatically approved"),
    ...
    def write(self, values):
        ...
        for cash_out in self:
            if employee_id:
                cash_out.add_follower(employee_id)
        ...
        for leave_type, cash_outs in sorted_cash_outs.items():
            if not leave_type.requires_allocation:
                raise ValidationError(
                    _("You must select a Time Off Type that requires allocation"),
                )
