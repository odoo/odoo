...
class EventRegistration(models.Model):
    ...
    state = fields.Selection(default='draft', compute="_compute_registration_state", store=True, readonly=False)
    sale_status = fields.Selection(string="Sale Status", default=None, selection=[
            ('to_pay', 'Not Sold'),
            ('sold', 'Sold'),
            ('free', 'Free'),
        ...
        for record in self:
            ...
            if not order:
                ...
                if float_is_zero(order._get_event_sale_total(), precision_digits=order.currency_id.rounding):
                    ...
                    if order._get_event_sale_state() and record.state != 'cancel':
