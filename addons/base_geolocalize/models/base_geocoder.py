...
class BaseGeocoder(models.AbstractModel):
    ...
    @api.model
    def geo_find_reverse(self, latitude, longitude, **kw):
        """Use a location provider API to convert Latitude and Longitude coordinates to addresses.
        Here we use Openstreetmap Nominatim by default.
        :param latitude: Latitude GPS Coordinates
        :param longitude: Longitude GPS Coordinate
        :return: Formatted Address or None if not found
        """
        provider = self._get_provider().tech_name
        try:
            service = getattr(self, '_call_reverse_' + provider)
            result = service(latitude, longitude, **kw)
        except AttributeError:
            raise UserError(_(
                'Provider %s is not implemented for reverse geolocation service.',
                provider))
        except UserError:
            raise
        except Exception:
    ...
    @api.model
    def _call_reverse_openstreetmap(self, latitude, longitude, **kw):
        """
        Use Openstreemap Nominatim service to retrieve address from Latitude, Longitude
        :return: Address display_name
        """
        if not (latitude and longitude):
            _logger.info('invalid latitude and longitude given')
            return None
        url = 'https://nominatim.openstreetmap.org/search'
        try:
            headers = {'User-Agent': 'Odoo (http://www.odoo.com/contactus)'}
            response = requests.get(url, headers=headers, params={'format': 'json', 'q': f"{latitude},{longitude}"})
            _logger.info('openstreetmap nominatim service called')
            if response.status_code != 200:
                _logger.warning('Request to openstreetmap failed.\nCode: %s\nContent: %s', response.status_code, response.content)
            result = response.json()
    ...
    @api.model
    def _call_reverse_googlemap(self, latitude, longitude, **kw):
        """ Use google maps API. It won't work without a valid API key.
        ...
        url = "https://maps.googleapis.com/maps/api/geocode/json"
        params = {'sensor': 'false', 'latlng': f"{latitude}, {longitude}", 'key': apikey}
        try:
            result = requests.get(url, params).json()
        except Exception as e:
            self._raise_query_error(e)
        ...
        try:
            ...
            if result['status'] != 'OK':
                ...
                error_msg = _('Unable to geolocate, received the error:\n%s'
                              ...
                              'Geocoding, Maps Static, Maps Javascript.\n', result.get('error_message'))
                raise UserError(error_msg)
            geo_reverse = result['results'][0]['formatted_address']
            return geo_reverse
        except (KeyError, ValueError):
            _logger.debug('Unexpected Gmaps API answer %s', result.get('error_message', ''))
            return None
