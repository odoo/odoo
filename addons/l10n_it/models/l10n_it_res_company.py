...
class ResCompany(models.Model):
    ...
    codice_fiscale = fields.Char(string="Codice Fiscale", size=16, related='partner_id.codice_fiscale',
                                 store=True, readonly=False, help="16 character fiscal code of your company")
    tax_system = fields.Selection(selection=TAX_SYSTEM, string="Tax System", help="Please select the Tax system to which you are subjected.")

    # PEC server
    mail_pec_server_id = fields.Many2one('ir.mail_server', string="Server PEC", help="Configure your PEC-mail server to send electronic invoices.")
    address_recipient_fatturapa = fields.Char(string="Government PEC-mail",
                                              help="Enter Government PEC-mail address. Ex: sdi01@pec.fatturapa.it")
    address_send_fatturapa = fields.Char(string="Company PEC-mail", help="Enter your company PEC-mail address. Ex: yourcompany@pec.mail.it")


    # Economic and Administrative Index
    has_eco_admin_index = fields.Boolean(default=False,
                                         ...
                                         Civil Code)")
    eco_admin_index_office = fields.Many2one('res.country.state', domain="[('country_id','=','IT')]",
                                             string="Province of the register-of-companies office")
    eco_admin_index_number = fields.Char(string="Number in register of companies", size=20,
                                         help="This field must contain the number under which the\
                                           seller/provider is listed on the register of companies.")
    eco_admin_index_share_capital = fields.Float(default="0.0", size=15, string="Share capital actually paid up",
                                                 ...
                                                 financial statement")
    eco_admin_index_sole_shareholder = fields.Selection([("NO", "Not a limited liability company"),
                                                         ("SU", "Socio unico"),
                                                         ("SM", "Più soci")],
                                                        string="Shareholder")
    eco_admin_index_liquidation_state = fields.Selection([("LS", "The company is in a state of liquidation"),
                                                          ("LN", "The company is not in a state of liquidation")],
                                                         string="Liquidation state")


    # Tax representative
    has_tax_representative = fields.Boolean(default=False,
                                            ...
                                            Italy")
    tax_representative_partner_id = fields.Many2one('res.partner', string='Tax representative partner')

    @api.constrains('has_eco_admin_index',
                    'eco_admin_index_office',
                    'eco_admin_index_number',
                    'eco_admin_index_share_capital',
                    'eco_admin_index_sole_shareholder',
                    'eco_admin_index_liquidation_state')
    def _check_eco_admin_index(self):
        for record in self:
            if not record.has_eco_admin_index:
                continue
            if not record.eco_admin_index_office\
               or not record.eco_admin_index_number\
               or not record.eco_admin_index_share_capital\
               or not record.eco_admin_index_sole_shareholder\
               or not record.eco_admin_index_liquidation_state:
                raise ValidationError("All fields about the Economic and Administrative Index must be completed.")

    @api.constrains('has_tax_representative',
                    'tax_representative_partner_id')
    def _check_tax_representative(self):
        for record in self:
            if not record.has_tax_representative:
                continue
            if not record.tax_representative_partner_id:
                raise ValidationError("You must select a tax representative.")
            if not record.tax_representative_partner_id.vat:
                raise ValidationError("Your tax representative partner must have a tax number.")
            if not record.tax_representative_partner_id.country_id:
