...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _get_demo_data(self, company=False):
        ...
        if company and company.account_fiscal_country_id.code == 'IT':
            demo_data = {'res.partner': self._l10n_it_get_demo_data_partner(), **demo_data}
        return demo_data

    @api.model
    def _l10n_it_get_demo_data_partner(self):
        return {
            'res_partner_it_b2b': {
    ...
    def _get_demo_data_bank(self, company=False):
        if not company or company.account_fiscal_country_id.code != 'IT':
            return super()._get_demo_data_bank(company)
        return {
            'bank_it_b2b': {
                'partner_id': 'res_partner_it_b2b',
                'acc_number': 'IT62P0300203280491121199673',
                'company_id': company.id,
            },
            'bank_it_b2c': {
                'partner_id': 'res_partner_it_b2c',
                'acc_number': 'IT40H0300203280131617863276',
                'company_id': company.id,
            },
            'bank_it_pa': {
                'partner_id': 'res_partner_it_pa',
                'acc_number': 'IT28K0300203280151118269669',
                'company_id': company.id,
            },
            'bank_eu_b2b': {
                'partner_id': 'res_partner_eu_b2b',
                'acc_number': 'DE22500105171521155351',
                'company_id': company.id,
            },
            'bank_extraeu_b2b': {
                'partner_id': 'res_partner_extraeu_b2b',
                'acc_number': '5790765029392936',
                'company_id': company.id,
            },
        }
    ...
    def _get_demo_data_move(self, company=False):
        demo_data_move = super()._get_demo_data_move(company)
        if not company or company.account_fiscal_country_id.code != 'IT':
        ...
        def get_date(day):
            return (today + relativedelta(day=day)).strftime('%Y-%m-%d')

        def create_move_lines(lines_vals):
            """Create a list of Command.create() from a list of (name, quantity, price_unit, tax_xml_suffix) tuples."""
            return [
                Command.create({
                    'name': name,
                    'quantity': quantity,
                    'price_unit': price_unit,
                    'tax_ids': self.ref(tax_xml_suffix),
                })
                for name, quantity, price_unit, tax_xml_suffix in lines_vals
            ]

        demo_data_move.update({
            'demo_inv_sale_multi_01': {
                'company_id': company.id,
                'move_type': 'out_invoice',
                'partner_id': 'res_partner_it_b2b',
                'invoice_date': get_date(18),
    ...
    def _post_load_demo_data(self, company=False):
        ...
        for move in moves:
            try:
                ...
                _logger.exception('Error while posting l10n_it demo data')
