...
class AccountMove(models.Model):
    ...
    l10n_jo_payment_method = fields.Selection(
        selection=[
            ('cash', 'Cash'),
            ('receivable', 'Receivable'),
        ],
        string="JoFatara Payment Method",
        compute='_compute_l10n_jo_payment_method',
        readonly=False, store=True,
    )
    ...
    def _compute_l10n_jo_payment_method(self):
        for move in self.filtered(lambda m: m.l10n_jo_edi_is_needed):
            if move.partner_id.is_company:
                move.l10n_jo_payment_method = 'receivable'
            else:
                move.l10n_jo_payment_method = 'cash'
    ...
    def _get_invoice_payment_method_code(self):
        return '1' if self.l10n_jo_payment_method == 'cash' else '2'
    ...
    def _l10n_jo_validate_fields(self):
        ...
        if not self.l10n_jo_payment_method:
