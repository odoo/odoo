...
class RazorpayController(http.Controller):
    ...
    def razorpay_callback(self, **data):
        ...
        if code:
            ...
            request_url = url_join(request.env['payment.provider']._get_razorpay_oauth_url(), '/iap/razorpay_proxy/v1/get_access_token')
            try:
                params = {
                    'dbuuid': dbuuid,
                    'code': data.get('code'),
    ...
    def _get_payment_provider_web_url(self, razorpay_provider, is_successful=True):
        ...
        redirect_url = f'/web#model={razorpay_provider._name}'\
                       f'&id={razorpay_provider.id}'\
                       f'&action={request.env.ref(view_ref).id}'\
                       f'&view_type=form'
