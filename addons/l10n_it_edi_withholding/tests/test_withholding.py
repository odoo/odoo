...
class TestWithholdingAndPensionFundTaxes(TestItEdiCommon):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()

        conditions = [('company_id.id', '=', cls.company.id), ('type_tax_use', '=', 'sale')]
        cls.withholding_sale_tax = cls.env['account.tax'].search(conditions + [('description', '=', '20vwc')], limit=1)
        cls.withholding_sale_tax_23 = cls.env['account.tax'].search(conditions + [('description', '=', '23vwo')], limit=1)
        cls.pension_fund_sale_tax = cls.env['account.tax'].search(conditions + [('description', '=', '4vcp')], limit=1)
        cls.enasarco_sale_tax = cls.env['account.tax'].search(conditions + [('description', '=', 'enasarcov')], limit=1)
    ...
    def test_withholding_taxes_export(self):
        ...
        """
        invoice_etree = etree.fromstring(self.withholding_tax_invoice._export_as_xml())
    ...
    def test_withholding_taxes_import(self):
        ...
        invoice = self.FatturaPA._create_invoice_from_xml_tree("IT00470550013_withh.xml", imported_etree)
        invoice_data = self.get_real_client_invoice_data()
    ...
    def test_pension_fund_taxes_export(self):
        ...
        """
        invoice_etree = etree.fromstring(self.pension_fund_tax_invoice._export_as_xml())
        invoice_data = self.get_real_client_invoice_data()
        expected_etree = self.with_applied_xpath(
            etree.fromstring(self.edi_basis_xml),
            '''
            ...
            <xpath expr="//DatiGeneraliDocumento//ImportoTotaleDocumento" position="before">
                <DatiCassaPrevidenziale>
                    ...
                    <AliquotaIVA>22.00</AliquotaIVA>
                    <RiferimentoAmministrazione>4vcp</RiferimentoAmministrazione>
                </DatiCassaPrevidenziale>
            </xpath>
            <xpath expr="//DatiGeneraliDocumento//ImportoTotaleDocumento" position="inside">
    ...
    def test_pension_fund_taxes_import(self):
        ...
        invoice = self.FatturaPA._create_invoice_from_xml_tree("IT00470550013_pfund.xml", imported_etree)
        invoice_data = self.get_real_client_invoice_data()
        for line in invoice.line_ids.filtered(lambda x: x.name in [data[0] for data in invoice_data.lines]):
            withholding_taxes = line.tax_ids.filtered(lambda x: x.l10n_it_withholding_type)
            pension_fund_taxes = line.tax_ids.filtered(lambda x: x.l10n_it_pension_fund_type)
    ...
    def test_enasarco_tax_export(self):
        ...
        """
        invoice_etree = etree.fromstring(self.enasarco_tax_invoice._export_as_xml())
        invoice_data = self.get_real_client_invoice_data()
        expected_etree = self.with_applied_xpath(
            etree.fromstring(self.edi_basis_xml),
            '''
    ...
    def test_enasarco_tax_import(self):
        ...
        invoice = self.FatturaPA._create_invoice_from_xml_tree("IT00470550013_enasarco.xml", imported_etree)
        invoice_data = self.get_real_client_invoice_data()
        for line in invoice.line_ids.filtered(lambda x: x.name in [data[0] for data in invoice_data.lines]):
            self.assertEqual(self.enasarco_sale_tax, line.tax_ids.filtered(lambda x: x.l10n_it_pension_fund_type == 'TC07'))
