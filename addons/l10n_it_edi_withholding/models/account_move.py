...
class AccountMove(models.Model):
    ...
    def _l10n_it_edi_import_invoice(self, invoice, data, is_new):
        ...
        for other_data_element in tree.xpath('//AltriDatiGestionali'):
            data_kind_element = other_data_element.xpath('./TipoDato')
            text_element = other_data_element.xpath('./RiferimentoTesto')
            if not data_kind_element or not text_element:
                continue
            ...
            if data_kind == 'cassa-prev' and ('enasarco' in data_text or 'tc07' in data_text):
                enasarco_count += 1
                parent_element = other_data_element.xpath('..')[0]
                number_element = other_data_element.xpath('./RiferimentoNumero')
                if number_element:
                    enasarco_amount = float(number_element[0].text)
                price_unit = get_float(parent_element, './PrezzoUnitario')
                vat_rate = get_float(parent_element, './/AliquotaIVA')

        if parent_element is not None and enasarco_count == 1 and enasarco_amount and price_unit == 0.0:
            base_amount = self._get_l10_it_edi_get_taxable_amount_from_summary_data(parent_element.xpath('..')[0], vat_rate)
            enasarco_percentage = -self.env.company.currency_id.round(enasarco_amount / base_amount * 100) if base_amount else 0.0
            ...
            if enasarco_tax := self._l10n_it_edi_search_tax_for_import(self.company_id, enasarco_percentage, domain, vat_only=False):
                self.invoice_line_ids.filtered(lambda line: vat_rate in line.tax_ids.mapped('amount')).tax_ids |= enasarco_tax

        return self

    def _get_l10_it_edi_get_taxable_amount_from_summary_data(self, element, vat_rate):
        taxable_amount = 0
        for summary_data_element in element.xpath('.//DatiRiepilogo'):
            summary_vat_rate = get_float(summary_data_element, './/AliquotaIVA')
            if vat_rate == summary_vat_rate:
