...
class AccountMoveSend(models.TransientModel):
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if not invoice_data.get("l10n_rs_edi_send") or not invoice.l10n_rs_edi_is_eligible:
                continue
            self._check_l10n_rs_edi_required_info(invoice)
            ...
            if error := invoice.with_company(invoice.company_id)._l10n_rs_edi_send(xml, self.l10n_rs_edi_send_cir_checkbox):
                invoice_data["error"] = {
    ...
    def _check_l10n_rs_edi_required_info(self, invoice):
        if not invoice.company_id.l10n_rs_edi_api_key:
            raise ValidationError('eFaktura API Key is missing')
        return
