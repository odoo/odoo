...
export class TimePeriodSelectionField extends SelectionField {
    get options() {
        // This field widget replaces three last option by the current month,
        // the next month and the after next month for the last year.
        // For instance, if the current date is 12 January 2025,
        ...
        for (const option of this.props.record.fields[this.props.name].selection) {
            if (option[0] === "last_year") {
                options.push([option[0], `${_t(date1.monthLong)} ${date1.year}`]);
            } else if (option[0] === "last_year_2") {
                options.push([option[0], `${_t(date2.monthLong)} ${date2.year}`]);
            } else if (option[0] === "last_year_3") {
                options.push([option[0], `${_t(date3.monthLong)} ${date3.year}`]);
