...
import { getSuggestToggleState, getMonthlyDemandRange, toServerDatetime } from "../utils";
...
export class PurchaseStockProductCatalogSearchModel extends AccountProductCatalogSearchModel {
    setup() {
        ...
        useSubEnv({
            suggestState: this.suggest,
            computeTotalEstimatedPrice: () => this.computeTotalEstimatedPrice(),
        });
    }
    ...
    async computeTotalEstimatedPrice() {
        this._editSuggestContext();
     ...
     */
    _editSuggestContext() {
        const [start, limit] = getMonthlyDemandRange(this.suggest.basedOn);
