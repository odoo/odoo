...
class TestPurchaseOrderSuggest(PurchaseTestCommon):
    ...
    def assertEstimatedPrice(self, po_suggest, price, based_on='one_month', days=30, factor=100, deduce_forecast_quantity=True, warehouse=False):
        ...
        po_suggest.deduce_forecast_quantity = deduce_forecast_quantity
    ...
    def _create_and_process_delivery(self, products_and_quantities, date=False, warehouse=False):
    ...
    def test_purchase_order_suggest_quantities(self):
        ...
        self._create_and_process_delivery(
        ...
        self.assertEstimatedPrice(po_suggest, 3375, factor=125)
        # Check estimed price for 3 months.
        self.assertEstimatedPrice(po_suggest, 1233.33, based_on='three_months')
        self.assertEstimatedPrice(po_suggest, 3700, based_on='three_months', days=90)
        # Check estimed price for current year.
        self.assertEstimatedPrice(po_suggest, 452.05, based_on='one_year')
        ...
        po_suggest.action_purchase_order_suggest()
        self.assertRecordValues(po.order_line, [
            {'product_id': self.product_1.id, 'product_qty': 8},
            {'product_id': product_2.id, 'product_qty': 3},
            {'product_id': product_3.id, 'product_qty': 6},
        ])
    ...
    def test_purchase_order_suggest_quantities_deduce_forecast_quantity(self):
        """ Ensures that when the option "Deduce forecast quantity" is checked,
        the forecast quantity is deduced from the suggested quantity"""
        today = fields.Datetime.now()
        self.env['stock.quant']._update_available_quantity(self.product_1, self.stock_location, 12)
        # Do a delivery in the past.
        self._create_and_process_delivery([(self.product_1, 12)], date=today - timedelta(days=10))
        ...
        # Check estimed price with the deduction of the forecast quantity.
        self.assertEstimatedPrice(po_suggest, 600, deduce_forecast_quantity=True)
        po_suggest.action_purchase_order_suggest()
        self.assertRecordValues(po.order_line, [
            {'product_id': self.product_1.id, 'product_qty': 6},
        ])
        # Check estimed price without the deduction of the forecast quantity.
        self.assertEstimatedPrice(po_suggest, 1200, deduce_forecast_quantity=False)
        po_suggest.action_purchase_order_suggest()
        self.assertRecordValues(po.order_line, [
    ...
    def test_purchase_order_suggest_quantities_multiwarehouse(self):
        ...
        self._create_and_process_delivery([(self.product_1, 5)], date)
        self._create_and_process_delivery([(self.product_1, 10)], date, warehouse=self.warehouse_1)
