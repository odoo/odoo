...
class TestStockValuationWithCOA(PurchaseTestCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.product = cls.product_fifo_auto
    ...
    def test_anglosaxon_valuation_price_total_diff_discount(self):
        ...
        # Create PO
        order = self._create_purchase(self.product, price_unit=110.0)

        # Receive the goods
        receipt = self._receive(order, quantity=1)

        # Create an invoice with a different price and a discount
        self._create_bill(purchase_order=order, price_unit=100.0, discount=10.0)

        # The bill revalues the FIFO receipt move down to its net billed price (110 - 20).
        self.assertEqual(receipt.value, 90.0)
        self.assertEqual(self.product.total_value, 90.0)
    ...
    def test_anglosaxon_valuation_discount(self):
        ...
        # Create PO
        order = self._create_purchase(self.product)

        # Receive the goods
        receipt = self._receive(order, quantity=1)

        # Create an invoice with a different price and a discount
        self._create_bill(purchase_order=order, discount=10.0, tax_ids=[Command.clear()])
    ...
    def test_anglosaxon_valuation_price_unit_diff_discount(self):
        ...
        # Create PO
        order = self._create_purchase(self.product, price_unit=90.0)
    ...
    def test_pdiff_lot_valuation(self):
        ...
        po = self._create_purchase(product=product, quantity=3.0)
        ...
        self._create_bill(purchase_order=po, price_unit=150.0)
    ...
    def test_purchase_with_backorders_and_return_and_price_changes(self):
        ...
        """
        product = self.product_avco_auto
        po = self._create_purchase(product, quantity=100, price_unit=10.0)
    ...
    def test_invoice_on_ordered_qty_with_backorder_and_different_currency_manual(self):
        ...
        usd_currency = self.env.ref('base.USD')
        self.env.company.currency_id = usd_currency.id
        self.product_fifo.purchase_method = 'purchase'
    ...
    def test_bill_with_zero_qty(self):
        ...
        bill01 = self._create_bill(purchase_order=po, post=False)
    ...
    def _test_fifo_and_returns_common(self):
        ...
        Receive & Bill 1 @ 10
        """
        po = self._create_purchase(self.product, price_unit=10.0)
    ...
    def test_fifo_return_and_receive_all_on_backorder(self):
        ...
        po = self._create_purchase(self.product, quantity=4, price_unit=25.0)
        ...
        self.assertRecordValues(self.product, [{
            'qty_available': 5,
            'total_value': 125.0,
            'standard_price': 25.0,
        }])
    ...
    def test_fifo_return_twice_and_bill(self):
        ...
        po = self._create_purchase(self.product, price_unit=25.0)

        receipt01 = self._receive(po, quantity=1)

        receipt01_return = self._make_return(receipt01, receipt01.quantity)
    ...
    def test_fifo_bill_return_refund(self):
        ...
        self._create_bill(purchase_order=po)
        self._make_return(receipt01, receipt01.quantity)
    ...
    def test_incoming_with_negative_qty(self):
        ...
        po = self._create_purchase(self.product, quantity=-2, price_unit=10.0)
    ...
    def test_return_a_return_avco_prod_with_exchange_diff(self):
        ...
        """
        avco_prod = self.product_avco_auto
        ...
        receipt2 = self._receive(purchase_order)

        receipt2_return1 = self._make_return(receipt2, receipt2.quantity)
        ...
        self._create_bill(purchase_order=purchase_order)
    ...
    def test_manual_non_standard_cost_bill_post(self):
        ...
        bill = self._create_bill(purchase_order=purchase_order, price_unit=120)
    ...
    def test_100_percent_discount(self):
        ...
        receipt = self._receive(purchase_order)
        self._create_bill(purchase_order=purchase_order)
        self.assertEqual(receipt.value, 0)
    ...
    def test_standard_valuation_return_credit_note(self):
        ...
        po = self._create_purchase(product=product)
        receipt_po = self._receive(po)

        self._create_bill(purchase_order=po)  # Bill
        self._make_return(receipt_po, receipt_po.quantity)
        self._create_bill(purchase_order=po)  # Refund
    ...
    def test_move_value_invoice_manual_rate(self):
        ...
        eur_currency = self.other_currency
        po = self._create_purchase(
            product=product,
        ...
        receipt_po = self._receive(po)
        self.assertEqual(receipt_po.value, 50)

        bill = self._create_bill(purchase_order=po, post=False)
        with Form(bill) as move_form:
            move_form.invoice_currency_rate = 4
        bill.action_post()
        self.assertEqual(receipt_po.value, 25)
    ...
    def test_price_diff_with_partial_bills_and_delivered_qties(self):
        ...
        """
        product = self.product  # fifo, real_time
