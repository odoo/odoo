...
class TestStockValuationWithCOA(AccountTestInvoicingCommon):
    ...
    def test_manual_non_standard_cost_bill_post(self):
        ...
        purchase_order.button_confirm()
        purchase_order.picking_ids.button_validate()
        scrap_location = self.env['stock.location'].search([('scrap_location', '=', True), ('company_id', '=', self.env.company.id)], limit=1)
        with Form(self.env['stock.move.line'].with_context(is_scrap=True, default_company_id=self.env.company.id), view='stock.view_scrap_move_line_form2') as move_line_form:
            move_line_form.product_id = product
            move_line_form.quantity = 10
            move_line_form.location_id = self.env.ref('stock.stock_location_suppliers')
            move_line_form.location_dest_id = scrap_location
            scrap_move_line = move_line_form.save()
        scrap_move_line.do_scrap()
        purchase_order.action_create_invoice()
        bill = purchase_order.invoice_ids
