...
class TestPurchaseOrderProcess(PurchaseTestCommon):
    ...
    def test_00_cancel_purchase_order_flow(self):
        ...
        self.assertEqual(purchase_order.state, 'done', 'Purchase: PO state should be "Purchase')
    ...
    def test_02_vendor_delay_report_partially_cancelled_purchase_order(self):
        ...
        purchase_order.line_ids.filtered(lambda l: l.product_id == self.product).product_qty = 0
        ...
        self.assertEqual(self.vendor.purchase_line_ids.mapped('qty_transferred'), [0.0, 3.0])
