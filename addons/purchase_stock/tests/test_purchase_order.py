...
class TestPurchaseOrder(ValuationReconciliationTestCommon):
    ...
    def test_00_purchase_order_flow(self):
        ...
        self.assertFalse(self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a), 'Purchase: confirming the order should not add the partner to the product suppliers')
    ...
    def test_po_edit_after_receive_2_steps_route(self):
        ...
        self.po = self.env['purchase.order'].create(self.po_vals)
        self.po.button_confirm()
        self.po.order_line.move_ids.picking_id.move_ids.quantity = 1
        Form.from_action(self.env, self.po.order_line.move_ids.picking_id.button_validate()).save().process()
        self.assertEqual(self.po.order_line.move_ids.picking_id.sorted('id').move_ids.mapped('product_uom_qty'), [1.0, 1.0, 4.0, 4.0])
        self.po.order_line[0].product_qty = 3
        self.assertEqual(self.po.order_line.move_ids.picking_id.sorted('id').move_ids.mapped('product_uom_qty'), [1.0, 1.0, 2.0, 4.0])
