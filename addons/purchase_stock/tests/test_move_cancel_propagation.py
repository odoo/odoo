...
class TestMoveCancelPropagation(PurchaseTestCommon):
    ...
    def test_01_cancel_draft_purchase_order_one_steps(self):
        ...
        """
        self.warehouse_1.write({'delivery_steps': 'ship_only', 'reception_steps': 'one_step'})
        self.picking_out.write({'location_id': self.picking_type_out.default_location_src_id.id})
        self.move.write({'location_id': self.picking_type_out.default_location_src_id.id})
    ...
    def test_03_cancel_draft_purchase_order_two_steps_push(self):
        ...
        """
        self.warehouse_1.write({'delivery_steps': 'pick_ship', 'reception_steps': 'two_steps'})
        self.move.write({
            'picking_id': False,
        ...
        """ Check the picking and moves status related PO, When canceling purchase order
            Ex.
                ...
                3) Cancel 'draft' purchase order should cancel releted < Receiption Picking  IN>
                  not < PICK, PACK, SHIP >
        """
        self.warehouse_1.write({'delivery_steps': 'pick_pack_ship', 'reception_steps': 'three_steps'})
        self.move.write({
            'picking_id': False,
            'picking_type_id': self.warehouse_1.pick_type_id.id,
            'location_final_id': self.customer_location.id,
    ...
    def test_cancel_move_lines_operation(self):
        ...
        product_car = self.env['product.product'].create({
            ...
            'is_storable': True,
            'route_ids': [
                (4, self.warehouse_1.mto_pull_id.route_id),
                (4, self.warehouse_1.buy_pull_id.route_id)
            ],
            'seller_ids': [(6, 0, [seller.id])],
        })
