...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            procurement_partner = procurement.values.get('procurement_partner') or supplier.partner_id  # Use procurement_partner to assign a PO to a non vendor partner

            if not (supplier or procurement_partner):
            ...
            procurement.values['procurement_partner'] = procurement_partner
    ...
    def _find_procurement_supplier(self, company_id, procurement):
        """ Finds the best partner to attach the PO to (ie the vendor at PO level not line level).
            Returns None if no supplier is found (eg no pricelist at all). """
        supplier = False
        if procurement.values.get('supplierinfo_id'):
            ...
            supplier = self._pick_supplier(
                ...
                partner_id=procurement.values.get('procurement_partner'),
                qty=procurement.product_qty,
                uom_id=procurement.product_uom,
                date_planned=max(procurement_date_planned.date(), fields.Date.today()),
                params={"force_uom": procurement.values.get('force_uom')},
            )

        return supplier

    def _pick_supplier(self, company, product_id, partner_id=False, qty=None, uom_id=False, date_planned=None, params=False):
        """ Flex find best supplier, dropping constraints (eg. min_qty, uom...) progressievely, except partner_id"""
        product = product_id.with_company(company)
        supplier = product._select_seller(partner_id=partner_id, quantity=qty, date=date_planned, uom_id=uom_id, params=params) or \
                   product._select_seller(partner_id=partner_id, quantity=None, date=date_planned, uom_id=uom_id, params=params) or \
                   product._select_seller(partner_id=partner_id, quantity=None, uom_id=uom_id, params=params) or \
                   product._select_seller(partner_id=partner_id, quantity=None, params=params) or \
                   product._select_seller(partner_id=partner_id, quantity=None)

        if not supplier and not partner_id:  # Last fallback, also matching expired pricelists
            supplier = product._select_seller(quantity=None) or product._prepare_sellers()[:1]
        return supplier
