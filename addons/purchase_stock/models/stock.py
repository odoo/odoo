...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _compute_allowed_partner_ids(self):
        for orderpoint in self:
            sellers = orderpoint.product_id.seller_ids.filtered_domain(
                self.env['product.supplierinfo']._check_company_domain(orderpoint.company_id),
            )
    ...
    def _search_effective_supplier_id(self, operator, value):
        ...
        if tuple(value) == (False,):
            orderpoints = self.search([('partner_id', '=', False)]).filtered(
                lambda orderpoint: not orderpoint.effective_supplier_id,
            )
        else:
            suppliers = self.env['product.supplierinfo'].search([('id', 'in', value)])
            orderpoints = self.search([
                '|', ('partner_id', '=', False), ('partner_id', 'in', suppliers.partner_id.ids),
            ]).filtered(lambda orderpoint: orderpoint.effective_supplier_id in suppliers)
    ...
    def _search_allowed_partner_ids(self, operator, value):
        ...
        if tuple(value) == (False,):
            orderpoints = self.search([('partner_id', '=', False)]).filtered(
                lambda orderpoint: not orderpoint.allowed_partner_ids,
            )
        else:
            partners = self.env['res.partner'].search([('id', 'in', value)])
            orderpoints = self.search([
                '|', ('partner_id', '=', False), ('partner_id', 'in', partners.ids),
            ]).filtered(lambda orderpoint: orderpoint.allowed_partner_ids & partners)
        return [('id', 'in', orderpoints.ids)]
    ...
    def _get_default_supplier(self):
        self.ensure_one()
        if not self.show_partner or not self.product_id:
            return self.env['product.supplierinfo']
        planned_date = self._get_orderpoint_procurement_date() - relativedelta.relativedelta(days=int(self.get_horizon_days()))
        dates_info = self.product_id._get_dates_info(planned_date, self.location_id, route_ids=self.route_id)
