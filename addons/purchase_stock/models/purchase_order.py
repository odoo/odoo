...
class PurchaseOrder(models.Model):
    ...
    def button_approve(self, force=False):
        ...
        # Do not create a new receipt unless existing receipts were all cancelled or returned (if the PO was cancelled and redrafted).
        # if all(line._is_net_zero() for line in self.order_line):
        # Do not create a new receipt if any quantity at all was received through an existing receipt.
        if not self.env['stock.move'].search_count([('picking_id', 'in', self.picking_ids.ids),
                                                    ('state', '=', 'done'),
    ...
    def button_cancel(self):
        ...
        fulfilled_purchase_orders = self.filtered(lambda po: any(move.state == 'done' for move in po.order_line.move_ids)
                                                  and any(line.qty_received for line in po.order_line))
        if fulfilled_purchase_orders:
            raise UserError(self.env._("Unable to cancel purchase order(s): %s since they have receipts that are already done. The moves must be returned first.",
                              fulfilled_purchase_orders.mapped('display_name')))
        for order in self:
