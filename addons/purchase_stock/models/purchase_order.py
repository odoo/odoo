    ...
    def _create_picking(self):
        for order in self.filtered(lambda po: po.state == 'purchase'):
            if any(product.type == 'consu' for product in order.order_line.product_id):
                ...
                moves = moves.filtered(lambda x: x.state not in ('done', 'cancel')).with_context({'move_picking_partner_id': self.partner_id, 'purchase_priority': order.priority}).sudo()._action_confirm()
                seq = 0
