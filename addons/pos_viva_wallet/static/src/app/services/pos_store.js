...
patch(PosStore.prototype, {
    ...
    async pay() {
        ...
        if (vivaWalletPaymentMethod && refundedOrder) {
            ...
            if (
                currentOrder.currency.equal(
                    Math.abs(currentOrder.getTotalDue()),
                    refundedOrder.amount_total
                )
            ) {
                ...
                vivaWalletPaymentlines.forEach((pi) => {
                    ...
                    if (currentOrder.currency.isNegative(currentDue)) {
                        const paymentLine = currentOrder.addPaymentline(pi.payment_method_id);
                        paymentLine.setAmount(-Math.min(Math.abs(currentDue), pi.amount));
                        paymentLine.updateRefundPaymentLine(pi);
                    }
                });
                if (currentOrder.currency.isNegative(currentOrder.getDue())) {
                    paymentIds.forEach((pi) => {
                        const currentDue = currentOrder.getDue();
                        if (
                            currentOrder.currency.isNegative(currentDue) < 0 &&
