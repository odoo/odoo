...
class PosVivaWalletController(http.Controller):
    ...
    def notification(self, company_id, token):
        ...
        payment_method_sudo = request.env['pos.payment.method'].sudo().search([('use_payment_terminal', '=', 'viva_wallet'), ('company_id.id', '=', company_id), ('pos_payment_provider_id.viva_wallet_webhook_verification_key', '=', token)], limit=1)
        if payment_method_sudo:
            ...
            return json.dumps({'Key': payment_method_sudo.pos_payment_provider_id.viva_wallet_webhook_verification_key})
        else:
