...
class Data_RecycleModel(models.Model):
    ...
    def _recycle_records(self, batch_commits=False):
        ...
        for recycle_model in self:
            ...
            if recycle_model.recycle_mode == 'automatic' and not self.env.context.get('data_cleaning_detect_only'):
    ...
    def create(self, vals_list):
        ...
        for record, vals in zip(records, vals_list):
            record.with_context(data_cleaning_detect_only=True)._refresh_records_to_process(vals)
        return records

    def write(self, vals):
        result = super().write(vals)
        # A configuration change only refreshes what is pending. Automatic rules
        # are only applied by the cron.
        self.with_context(data_cleaning_detect_only=True)._refresh_records_to_process(vals)
        return result
    ...
    def action_find_records_to_recycle(self):
        self.sudo().with_context(data_cleaning_detect_only=True)._recycle_records()
    ...
    def refresh_recycle_records(self):
        ...
        """
        self.search([]).with_context(data_cleaning_detect_only=True)._recycle_records(batch_commits=True)
