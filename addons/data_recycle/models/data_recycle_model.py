...
class Data_RecycleModel(models.Model):
    ...
    active = fields.Boolean(related='unified_id.active')
    name = fields.Char(string='Name', compute='_compute_name', store=True, readonly=False, required=True, copy=True)

    res_model_id = fields.Many2one(related='unified_id.res_model_id')
    res_model_name = fields.Char(related='unified_id.res_model_name')
    recycle_record_ids = fields.One2many('data_recycle.record', 'recycle_model_id')

    recycle_mode = fields.Selection(related='unified_id.cleaning_mode')
    ...
    notify_user_ids = fields.Many2many(related='unified_id.notify_user_ids')
    notify_frequency = fields.Integer(related='unified_id.notify_frequency')
    notify_frequency_period = fields.Selection(related='unified_id.notify_frequency_period')
    last_notification = fields.Datetime(related='unified_id.last_notification')
