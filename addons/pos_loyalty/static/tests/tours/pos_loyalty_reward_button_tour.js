...
registry.category("web_tour.tours").add("test_loyalty_program_among_pos_categ", {
    steps: () =>
        [
            ...
            // Adding the letter tray automatically triggers the claim as it is already
            // scanned in this order. The reward is applied to the cheapest eligible product,
            // so: 1.98 + 5.28 - 0.20 (10% discount on 1.98, rounded) = 7.06.
            ProductScreen.clickDisplayedProduct("Letter Tray"),
            PosLoyalty.orderTotalIs("7.06"),
            ProductScreen.clickDisplayedProduct("Test Chair"),
            // 1.98 + 5.28 + 1.73 - 0.17 (10% of 1.73 chepest of (1.98, 1.73)) = 8.81 (because these are not exact numbers there are rounded)
            PosLoyalty.orderTotalIs("8.81"),
        ].flat(),
});
