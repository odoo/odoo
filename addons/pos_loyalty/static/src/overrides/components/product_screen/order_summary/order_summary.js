    ...
    manageGiftCard() {
        this.dialog.add(ManageGiftCardPopup, {
            ...
            getPayload: async (code, points) => {
                ...
                const existingGiftCard = res.length > 0 ? res[0] : null;
                const newGiftCardCode = res.length === 0 ? code : null;
                this.currentOrder.processGiftCard(existingGiftCard, newGiftCardCode, points);
