...
const PosLoyaltyOrderline = (Orderline) =>
    class PosLoyaltyOrderline extends Orderline {
        export_as_JSON() {
...
const PosLoyaltyOrder = (Order) =>
    class PosLoyaltyOrder extends Order {
        ...
        _get_ignored_product_ids_total_discount() {
            ...
            const giftCardPrograms = this.pos.programs.filter(
        ...
        get_last_orderline() {
            const orderLines = super
                .get_orderlines(...arguments)
            ...
            await mutex.exec(async () => {
                ...
                if (couponsToFetch.length) {
                    await this.pos.fetchCoupons(
                        [["id", "in", couponsToFetch]],
                        couponsToFetch.length
                    );
        ...
        _updateRewardLines() {
            ...
            for (const claimedReward of productRewards
                .concat(otherRewards)
                .concat(paymentRewards)) {
        ...
        async _updatePrograms() {
            ...
            const programs = [...programsToCheck].map(
                (programId) => this.pos.program_by_id[programId]
            );
        ...
        async _couponForProgram(program) {
            ...
            return new PosLoyaltyCard(
                null,
                null,
                program.id,
                (this.get_partner() || { id: -1 }).id,
                0
            );
        }
        _programIsApplicable(program) {
            if (
                ...
                !program.rules.find(
                    (rule) =>
                        rule.mode === "auto" || this.codeActivatedProgramRules.includes(rule.id)
                )
            ) {
                return false;
            }
        ...
        pointsForPrograms(programs) {
            ...
            const totalsPerProgram = Object.fromEntries(
                programs.map((program) => [
                    program.id,
                    { untaxed: totalUntaxed, taxed: totalTaxed },
                ])
            );
        ...
        _getDiscountableOnSpecific(reward) {
            ...
            for (const lines of Object.values(discountLinesPerReward)) {
                ...
                const commonLines = linesToDiscount.filter((line) =>
                    discountedLines.includes(line)
                );
