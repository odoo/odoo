...
patch(PosOrder.prototype, {
    get manualAvailableRewards() {
        return this.models["loyalty.program"]
            .filter((p) => !["ewallet", "gift_card"].includes(p.program_type))
            .flatMap((program) => program.availableRewards(this));
    },
    get automaticAvailableRewards() {
        return this.models["loyalty.program"]
            .filter(
                (p) => !["ewallet", "gift_card"].includes(p.program_type) && p.trigger === "auto"
            )
            .flatMap((program) => program.availableRewards(this));
    },
    get availableRewards() {
        return this.models["loyalty.program"].flatMap((program) => program.availableRewards(this));
    },
    applyAutoProgramRewards() {
        const toApply = this.automaticAvailableRewards;
        debugger;
    },
