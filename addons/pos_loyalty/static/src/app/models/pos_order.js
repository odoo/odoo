...
patch(PosOrder.prototype, {
    ...
    _getCheapestLinesInfo(reward) {
        ...
        for (let line of validLines) {
            discountedQuantity = Math.floor(
                (restQuantity + line.getQuantity()) / reward.repeater
            )
            ...
            else if (discountedQuantity > 0) {  // Only discount 1 product as it clears the wallet
    ...
    _getRewardLineValuesDiscount(args) {
        ...
        if (rewardAppliesTo === "cheapest") {
            let previousCheapestRewardLines = this.getOrderlines().filter(
            ...
            rewardCode = previousCheapestRewardLines.length ?
                previousCheapestRewardLines[0].reward_identifier_code : rewardCode
        }
