    ...
    def onchange_journal_voucher(self, cr, uid, ids, line_ids=False, tax_id=False, price=0.0, partner_id=False, journal_id=False, ttype=False, company_id=False, context=None):
        ...
        if journal.type in ('sale','sale_refund'):
            ...
            if not journal.default_credit_account_id or not journal.default_debit_account_id:
                raise UserError(_('Error!'), _('Please define default credit/debit accounts on the journal "%s".') % (journal.name))
    ...
    def unlink(self, cr, uid, ids, context=None):
        for t in self.read(cr, uid, ids, ['state'], context=context):
            if t['state'] not in ('draft', 'cancel'):
                raise UserError(_('Invalid Action!'), _('Cannot delete voucher(s) which are already opened or paid.'))
        ...
        for line in voucher.line_ids:
            ...
            if line.amount == line.amount_unreconciled:
                if not line.move_line_id:
                    raise UserError(_('Wrong voucher line'),_("The invoice you are willing to pay is not valid anymore."))
            ...
            if move_line.get('account_tax_id', False):
                ...
                if not (tax_data.base_code_id and tax_data.tax_code_id):
                    raise UserError(_('No Account Base Code and Account Tax Code!'),_("You have to configure account base code and account tax code on the '%s' tax!") % (tax_data.name))
