...
#    _rec_name = 'number'
    ...
    _columns = {
        ...
        'writeoff_acc_id': fields.many2one('account.account', 'Counterpart Account', readonly=True, states={'draft': [('readonly', False)]}, domain=[('deprecated', '=', False)]),
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            for rec_ids in rec_list_ids:
                if len(rec_ids) >= 2:
                    reconcile = move_line_pool.reconcile_partial(cr, uid, rec_ids, writeoff_acc_id=voucher.writeoff_acc_id.id, writeoff_period_date=voucher.date, writeoff_journal_id=voucher.journal_id.id)
        return True
