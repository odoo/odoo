...
patch(Persona.prototype, {
    ...
    async fetchPublicHolidayStatus() {
        try {
            if (!this.isPublicHoliday && this.userId) {
                const data = await rpc("/holidays/is_public_holiday", { userId: this.userId });
                this.isPublicHoliday = data;
            }
        } catch {
    ...
    get outOfOfficeText() {
        ...
        if (this.isPublicHoliday && this.id !== this.store.odoobot.id) {
            return _t(`Out of office due to ${this.isPublicHoliday} (Public Holiday).`);
        }
