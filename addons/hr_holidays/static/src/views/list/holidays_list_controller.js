...
export class HolidaysListController extends ListController {
    ...
    getStaticActionMenuItems() {
        ...
        if (this.model.root.selection.every((record) => record.data.can_approve)) {
            menuItems.approve = {
                ...
                callback: async () => await this.executeAction("action_approve"),
            };
        }
        ...
        if (this.model.root.selection.every((record) => record.data.can_refuse)) {
            menuItems.refuse = {
                ...
                callback: async () => await this.executeAction("action_refuse"),
            };
        }
    ...
    async executeAction(functionName, records = null) {
        const targetRecords = records || this.model.root.selection;
        await this.orm.call(this.props.resModel, functionName, [
            targetRecords.map((record) => record.resId),
        ]);
