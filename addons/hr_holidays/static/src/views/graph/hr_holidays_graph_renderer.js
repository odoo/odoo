...
export class HrHolidaysGraphRenderer extends GraphRenderer {
    getBarChartData() {
        // Split the data into 2 stacks, `Allocation` and `Time Off`.
    ...
    _removeLeaveTypeFromDomains(domains){
        ...
        */
        // Create a deep copy of the domains array to prevent changing the original domains.
    ...
    _computeBalanceData(datasets){
        // Initialize the datapoints to zeros
        let data = new Array(datasets[0].trueLabels.length).fill(0);
        // Update the data of the balanceDataset such that balance = Allocation - Time Off.
        for (const dataset of datasets){
            for (let i = 0; i < dataset.data.length; i++){
                if (dataset.stack === "Allocation"){
                    data[i] += dataset.data[i];
                }
                else if (dataset.stack === "Time Off"){
                    data[i] -= dataset.data[i];
                }
            }
        }
    ...
    _replaceLeaveTypeWithBalance(label, leaveType) {
        const pattern = this._createLeaveTypePattern(leaveType);
        // Use `replace()` to substitute the term with 'Balance'
