"""Temporary debug instrumentation for the hr_holidays quality-improvement campaign.

Four loggers under ``odoo.addons.hr_holidays.debug``; enable one or all with
``--log-handler odoo.addons.hr_holidays.debug:DEBUG`` (or ``.logic``, ``.performance``,
``.pipeline``, ``.lifecycle``). Every call site imports this module as ``dbg``,
so ``grep -rn 'dbg\\.' addons/hr_holidays`` lists the whole instrumentation for removal.
...
- ``pipeline``: hand-offs between stages of a flow (allocation -> consumed
  leaves -> balance; leave -> resource leave -> meeting; accrual cron -> level ->
  carryover).
...
_ROOT = "odoo.addons.hr_holidays.debug"
DEBUG = logging.DEBUG
