...
class HrLeaveAllocation(models.Model):
    ...
    last_accrual = fields.Date("Date of the last accrual allocation", readonly=False)
    # lastcall stores the date of the last executed accrual event (accruals, carryover_date, ...)
    lastcall = fields.Date(export_string_translation=False, readonly=False)
    ...
    def get_next_carryover_date(self, date_from, date_from_included=True):
        """
    ...
    def _get_maximum_leave_yearly_days(self, level):
        ...
        return level.maximum_leave_yearly / self.employee_id._get_hours_per_day(self.date_from)
    ...
    def _is_accrued_gain_start(self):
        self.ensure_one()
        return self.accrual_plan_id.accrued_gain_time == 'start'

    def _is_accrued_gain_end(self):
        self.ensure_one()
        return self.accrual_plan_id.accrued_gain_time == 'end'

    def _get_current_accrual_plan_level_id(self, date, lvl_intervals=False):
        self.ensure_one()
        if not self.accrual_plan_id:
            return False
        if not lvl_intervals:
            lvl_intervals = self.accrual_plan_id.get_lvls_intervals(self.date_from)
        return self.accrual_plan_id._get_current_accrual_plan_level_id(date, lvl_intervals)

    def get_current_accrual_plan_level_id(self, date, lvl_intervals=False):
        self.ensure_one()
        plan_level_id = self._get_current_accrual_plan_level_id(date, lvl_intervals)
        if not plan_level_id:
            return (False, False)
        return plan_level_id[-1]

    def _get_current_accrual_plan_level_id_for_accrual(self, date, lvl_intervals=False):
        """
        Unlike "get_current_accrual_plan_level_id", this method will return the current level considering the level transition.
        For example, during level transition, if the accrued_gain_time is "end", then we want to get the previous plan level as
        we need to compute number_of_days, expiring_days, ... depending on the previous level policy.

        Returns (current_level, current_level_idx), (accrual_level, accrual_level_idx)
        Where (current_level, current_level_idx) would be the same return as "get_current_accrual_plan_level_id"
        And (accrual_level, accrual_level_idx) is the current level considering level transition
        """
        self.ensure_one()
        plan_level_id = self._get_current_accrual_plan_level_id(date, lvl_intervals)
        if not plan_level_id:
            return (False, False)
        if len(plan_level_id) == 2 and self._is_accrued_gain_end():
            return plan_level_id[-1], plan_level_id[0]
        return plan_level_id[-1], plan_level_id[-1]
    ...
    def _process_accrual_plan_level(self, level, period_start, start_date, period_end, end_date):
        """
    ...
    def _update_carryover_expiration_date(self, carryover_date, current_level):
        self.ensure_one()
    ...
    def _process_expiration_date(self, current_level, leaves_taken, next_carryover_date):
        self.ensure_one()
        ...
        self._update_carryover_expiration_date(next_carryover_date, current_level)
    ...
    def _process_accrual_start(self, current_level, next_accrual, leaves_taken):
        ...
        current_lvl_start_date = get_lvl_start_date(self.date_from, current_level)
        if self.nextcall == period_start or self.nextcall == current_lvl_start_date:
            ...
            if current_level.cap_accrued_time:
                current_level_maximum_leave = self._get_lvl_max_leave_days(current_level)
            self._add_days_to_allocation(current_level, current_level_maximum_leave, leaves_taken, period_start,
                period_end, accrual_start_date, accrual_end_date)
    ...
    def _process_accrual_end(self, current_level, current_level_idx, lvl_intervals, first_level_start_date, leaves_taken):
        ...
        current_level_last_date = self.accrual_plan_id.get_lvl_last_date(self.date_from, current_level_idx, lvl_intervals)
    ...
    def _has_expiring_days(self):
        self.ensure_one()
        return self.carried_over_days_expiration_date and self.nextcall < self.carried_over_days_expiration_date

    def _set_first_nextcall(self, level_ids=False):
        self.ensure_one()
        if self._is_accrued_gain_start():
            self.nextcall = self.lastcall
            return
        ...
        nextcall = first_level._get_next_date(self.lastcall)
        if first_level.can_be_carryover:
            carryover_date = self.get_next_carryover_date(self.lastcall)
            nextcall = min(carryover_date, nextcall)

        if len(level_ids) > 1:
            second_level_start_date = self.accrual_plan_id.get_lvl_last_date(self.date_from, 0)
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False, log=True):
        ...
        for allocation in self:
            ...
            # [BUG] compute leaves_taken here because computing leaves_taken call hr_employee_get_consumed_leaves ->
            #       _get_future_leaves_on which call _process_accrual_plans and reset nextcall to False !!!!
            ...
            if not allocation.nextcall:
                ...
                allocation._set_first_nextcall(level_ids)
                if log:
            ...
            while allocation.nextcall <= date_to:
                (current_level, current_level_idx), (accrual_level, accrual_level_idx) = \
                    allocation._get_current_accrual_plan_level_id_for_accrual(allocation.nextcall, lvl_intervals)
                ...
                # 1. Passing from one level to another
                ...
                if current_level.can_be_carryover:
                    carryover_date = allocation.get_next_carryover_date(allocation.nextcall)
                ...
                if allocation._is_accrued_gain_start():
                    allocation._process_accrual_start(accrual_level, next_accrual, leaves_taken)
                else:
                    allocation._process_accrual_end(accrual_level, accrual_level_idx, lvl_intervals, first_level_start_date, leaves_taken)
