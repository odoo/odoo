...
class HolidaysAllocation(models.Model):
    ...
    def _get_hours_per_day(self):
        return self.employee_id._get_calendars(self.date_from)[self.employee_id.id].hours_per_day
    ...
    def _get_title(self):
        ...
        if self.type_request_unit == 'hour':
            return _(
                ...
                duration=self.number_of_days * self._get_hours_per_day(),
            )
        return _(
    ...
    def _compute_number_of_hours_display(self):
        for allocation in self:
            allocation.number_of_hours_display = (allocation.number_of_days * allocation._get_hours_per_day())
        ...
        for allocation in self:
            ...
            if allocation.type_request_unit:
                allocation.number_of_days = float(value) if allocation.type_request_unit != 'hour' \
                    else float(value) /allocation._get_hours_per_day()
            else:
    ...
    def _inverse_number_of_hours_display(self):
        for allocation in self:
            allocation.number_of_days = allocation.number_of_hours_display / allocation._get_hours_per_day()
    ...
    def _add_days_to_allocation(self, current_level, current_level_maximum_leave, leaves_taken, period_start, period_end):
        ...
        if current_level.cap_accrued_time_yearly:
            maximum_leave_yearly = current_level.maximum_leave_yearly\
                ...
                else current_level.maximum_leave_yearly / self._get_hours_per_day()
    ...
    def _process_accrual_plan_level(self, level, start_period, start_date, end_period, end_date):
        ...
        if level.added_value_type == 'hour':
            added_value = added_value / self._get_hours_per_day()
        period_prorata = 1
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False, log=True):
        ...
        for allocation in self:
            ...
            leaves_taken = allocation.leaves_taken if allocation.holiday_status_id.request_unit in ["day", "half_day"] \
                else allocation.leaves_taken / allocation._get_hours_per_day()
            ...
            while allocation.nextcall <= date_to:
                ...
                if current_level.cap_accrued_time:
                    current_level_maximum_leave = current_level.maximum_leave \
                        ...
                        else current_level.maximum_leave / allocation._get_hours_per_day()
                    ...
                    if current_level.action_with_unused_accruals in ['lost', 'maximum']:
                        ...
                        if current_level.action_with_unused_accruals == 'maximum':
                            postpone_max_days = current_level.postpone_max_days \
                                if current_level.added_value_type == 'day' \
                                else current_level.postpone_max_days / allocation._get_hours_per_day()
