...
class HrLeaveAllocation(models.Model):
    ...
    def _get_next_carryover_date(self, date_from):
        self.ensure_one()
        carryover_date = self._get_carryover_date(date_from)
        if carryover_date == date_from:
    ...
    def _process_accrual_start(self, current_level, next_accruals, leaves_taken):
        self.ensure_one()
        ...
        if self.nextcall == period_start or self.nextcall == current_lvl_start_date:
            ...
            accrual_end_date = next_accruals
    ...
    def _process_accrual_end(self, current_level, current_level_idx, lvl_intervals, first_level_start_date, leaves_taken):
        ...
        if self.nextcall == period_end or self.nextcall == current_level_last_date:
            accrual_start_date = self.last_accrual if self.last_accrual else first_level_start_date
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False, log=True):
        ...
        for allocation in self:
            ...
            while allocation.nextcall <= date_to:
                ...
                if allocation._is_accrued_gain_start():
                    allocation._process_accrual_start(accrual_level, next_accruals, leaves_taken)
                else:
