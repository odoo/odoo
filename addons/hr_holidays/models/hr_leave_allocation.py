...
class HolidaysAllocation(models.Model):
    ...
    def _process_accrual_plan_level(self, level, start_period, start_date, end_period, end_date):
        ...
        self.ensure_one()
        datetime_min_time = datetime.min.time()
        employee_timezone = timezone(self.employee_id.tz)
        start_dt = datetime.combine(start_date, datetime_min_time).replace(tzinfo=employee_timezone)
        end_dt = datetime.combine(end_date, datetime_min_time).replace(tzinfo=employee_timezone)
        employee_calendar_periods = self.employee_id._get_calendar_periods(start_dt, end_dt)[self.employee_id]
        # Use the employee's working schedule If the employee doesn't have any contracts,
        if not employee_calendar_periods:
            employee_calendar_periods = [(start_dt, end_dt, self.employee_id.resource_calendar_id)]
        total_added_value = 0
        for cal_period_start, cal_period_end, calendar in employee_calendar_periods:
            cal_period_start = cal_period_start.astimezone(employee_timezone)
            cal_period_end = cal_period_end.astimezone(employee_timezone)
