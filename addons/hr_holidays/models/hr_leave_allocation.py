...
class HolidaysAllocation(models.Model):
    ...
    employee_resource_calendar_id = fields.Many2one("resource.calendar", compute="_compute_employee_resource_calendar_id", required=True, readonly=True)
    ...
    number_of_hours_display = fields.Float(
        'Duration (hours)', compute='_compute_number_of_hours_display', inverse="_inverse_number_of_hours_display", store=True,
        help="For an Accrual Allocation, this field contains the theorical amount of time given to the employee, due to a previous start date, on the first run of the plan. This can be manually edited.")
    ...
    def _get_title(self):
        ...
        if self.type_request_unit == 'hour':
            return _(
                ...
                duration=self.number_of_days * self.employee_resource_calendar_id.hours_per_day,
            )
        return _(
    ...
    @api.depends('number_of_days', 'employee_resource_calendar_id')
    def _compute_number_of_hours_display(self):
        for allocation in self:
            allocation.number_of_hours_display = (allocation.number_of_days
                                                  * allocation.employee_resource_calendar_id.hours_per_day)
    ...
    def _inverse_duration_display(self):
        for allocation in self:
            try:
                value = float(allocation.duration_display.split(' ')[0].replace(',','.'))
            except ValidationError:
                raise ValidationError(_("Cannot convert allocated days/hours value : '%s' to float", allocation.duration_display))

            if allocation.type_request_unit:
                allocation.number_of_days = float(value) if allocation.type_request_unit != 'hour' \
                    else float(value) / allocation.employee_resource_calendar_id.hours_per_day
            else:
                raise ValidationError(_("No allocation value unit defined"))
    ...
    def _inverse_number_of_hours_display(self):
        for allocation in self:
            allocation.number_of_days = allocation.number_of_hours_display / allocation.employee_resource_calendar_id.hours_per_day
    ...
    @api.depends('date_from', 'employee_id.resource_calendar_id')
    def _compute_employee_resource_calendar_id(self):
        for allocation in self:
            allocation.employee_resource_calendar_id = allocation.employee_id\
                ._get_calendars(allocation.date_from)[allocation.employee_id.id]
    ...
    def _add_days_to_allocation(self, current_level, current_level_maximum_leave, leaves_taken, period_start, period_end):
        ...
        if current_level.cap_accrued_time_yearly:
            maximum_leave_yearly = current_level.maximum_leave_yearly\
                ...
                else current_level.maximum_leave_yearly / self.employee_resource_calendar_id.hours_per_day
    ...
    def _process_accrual_plan_level(self, level, start_period, start_date, end_period, end_date):
        ...
        if level.added_value_type == 'hour':
            added_value = added_value / self.employee_resource_calendar_id.hours_per_day
        period_prorata = 1
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False, log=True):
        ...
        for allocation in self:
            ...
            leaves_taken = allocation.leaves_taken if allocation.holiday_status_id.request_unit in ["day", "half_day"] \
                else allocation.leaves_taken / allocation.employee_resource_calendar_id.hours_per_day
            ...
            while allocation.nextcall <= date_to:
                ...
                if current_level.cap_accrued_time:
                    current_level_maximum_leave = current_level.maximum_leave \
                        ...
                        else current_level.maximum_leave / allocation.employee_resource_calendar_id.hours_per_day
