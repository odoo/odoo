...
class ResPartner(models.Model):
    ...
    def _to_store_default_fields(self):
        return super()._to_store_default_fields() + ["out_of_office_date_end"]
    ...
    def _to_store_field_computes(self):
        def out_of_office_date_end(partner):
            ...
            date = min(dates) if dates and all(dates) else False
            state = min(states) if states and all(states) else False
            return date if state == "validate" else False

        return super()._to_store_field_computes() | {
            "out_of_office_date_end": out_of_office_date_end
        }
