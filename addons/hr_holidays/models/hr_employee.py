...
class HrEmployee(models.Model):
    ...
    def _compute_leave_status(self):
        ...
        upcoming_leaves = leaves.filtered(lambda leave: leave.date_from > now)
        holidays = leaves - upcoming_leaves

        holidays_by_employee_id = holidays.grouped(lambda leave: leave.employee_id.id)
        upcoming_leaves_by_employee_id = upcoming_leaves.grouped(lambda leave: leave.employee_id.id)
        ...
        # employees not off right now, but whose leave starts within the next day
        for employee in employees_not_on_leave:
            emp_leaves = upcoming_leaves_by_employee_id.get(employee.id)
            if not emp_leaves:
                continue
            latest_emp_leave = emp_leaves[0]
            employee.leave_date_from = min(emp_leaves.mapped('date_from'))
            employee.leave_date_to = latest_emp_leave.date_to.date()
        ...
        for employee in employees_with_leave_end:
            ...
            employee.leave_date_from = min(emp_holidays.mapped('date_from'))
            employee.leave_request_date_from_period = latest_emp_holiday.request_date_from_period
            employee.leave_request_duration = latest_emp_holiday.request_duration
