...
class HrLeaveAccrualPlan(models.Model):
    ...
    is_carryover = fields.Boolean(default=True, store=True, readonly=False, inverse="_inverse_is_carryover")
    ...
    def _compute_summary(self):
        for plan in self:
            ...
            if plan.accrued_gain_time == "start":
                start_or_end = _("This accrual plan is accrued at the <b>start of each period</b>, ")
            else:
                start_or_end = _("This accrual plan is accrued at the <b>end of each period</b>, ")
            if plan.is_based_on_worked_time:
                based_on_worked_time = _("<b>based on the worked time.</b>")
            else:
                based_on_worked_time = _("<b>based on the whole calendar days.</b>")
            ...
            if plan.is_carryover:
                if plan.carryover_date == "year_start":
                    carryover_date = _(" <b>at the start of the year.</b>")
                elif plan.carryover_date == "allocation":
                    carryover_date = _(" <b>at the allocation date.</b>")
                else:
                    carryover_date = _(" <b>at the %(day)s of %(month)s.</b>", day=carryover_day, month=carryover_month)
            else:
                carryover_date = ""
            ...
            plan.summary = start_or_end + based_on_worked_time + carryover + carryover_date + transition_mode
    ...
    def _inverse_is_carryover(self):
        for plan in self:
            if not plan.is_carryover:
                plan.level_ids.write({'action_with_unused_accruals': 'lost'})
