...
class HrLeaveAccrualLevel(models.Model):
    ...
    def _check_dates(self):
        for level in self:
            if level.frequency == 'weekly' and not level.week_day:
                raise UserError(_("Weekday must be selected to use the frequency weekly"))

            if level.frequency == 'bimonthly' and int(level.first_day) >= int(level.second_day):
                raise UserError(_("The first day must be lower than the second day."))
    ...
    def _get_next_date(self, last_call):
        ...
        if self.frequency == 'bimonthly':
            ...
            if last_call < second_date:
                return second_date
            return last_call + relativedelta(months=1, day=int(self.first_day))
        ...
        if self.frequency == 'biyearly':
            ...
            if last_call < second_date:
                return second_date
            return last_call + relativedelta(years=1, month=int(self.first_month), day=int(self.first_month_day))

        if self.frequency == 'yearly':
            date = last_call + relativedelta(month=int(self.yearly_month), day=int(self.yearly_day))
            if last_call < date:
                return date
            return last_call + relativedelta(years=1, month=int(self.yearly_month), day=int(self.yearly_day))
    ...
    def _get_previous_date(self, last_call):
        ...
        if self.frequency == 'bimonthly':
            ...
            if last_call >= first_date:
                return first_date
            return last_call + relativedelta(months=-1, day=int(self.second_day))
        ...
        if self.frequency == 'biyearly':
            ...
            if last_call >= first_date:
                return first_date
            return last_call + relativedelta(years=-1, month=int(self.second_month), day=int(self.second_month_day))
