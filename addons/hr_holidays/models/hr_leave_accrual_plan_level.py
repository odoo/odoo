...
class HrLeaveAccrualLevel(models.Model):
    ...
    cap_accrued_time_yearly = fields.Boolean(string="Milestone cap", compute="_compute_cap_accrued_time_yearly",
    ...
    _valid_postpone_max_days_value = models.Constraint(
        "CHECK(action_with_unused_accruals <> 'maximum' OR COALESCE(postpone_max_days, 0) > 0)",
    ...
    @api.depends('is_carryover')
    def _compute_action_with_unused_accruals(self):
        for level in self:
            if not level.is_carryover:
    ...
    @api.depends('action_with_unused_accruals')
    def _compute_cap_accrued_time_yearly(self):
        for level in self:
            if level.action_with_unused_accruals == 'lost':
