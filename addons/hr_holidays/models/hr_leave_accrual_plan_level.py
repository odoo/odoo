...
class HrLeaveAccrualLevel(models.Model):
    ...
    def _check_min_amount(self):
        for level in self:
            errors = ""
            if level.added_value < 1:
                errors += _("\n- You must set a minimum amount of accrued days/hours for this accrual plan milestone.")
            if level.postpone_max_days < 1 and level.action_with_unused_accruals == 'maximum':
                errors += _("\n- You must set a maximum quantity to carry over > 0")
            if level.maximum_leave < 1 and level.cap_accrued_time:
                    errors += _("\n- You must set a milestone cap > 0")
            if level.action_with_unused_accruals != 'lost':
                if level.accrual_validity_count < 1 and level.accrual_validity:
                    errors += _("\n- You must set a carry-over validity cap > 0")
                if level.maximum_leave_yearly < 1 and level.cap_accrued_time_yearly:
                    errors += _("\n- You must set a yearly cap > 0")
            if errors:
                raise UserError(_("You can not set value(s) to 0 for this/these field(s) : " + errors))
