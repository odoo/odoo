...
class HrLeavePublicHoliday(models.Model):
    _name = 'hr.leave.public.holiday'
    _description = 'Public Holiday'

    name = fields.Char(string='Holiday Name', required=True)
    date_from = fields.Datetime(string='Start Date', default=fields.Datetime.today(), required=True, copy=False)
    date_to = fields.Datetime(string='End Date', default=lambda self: fields.Datetime.today() + relativedelta(days=1), required=True, copy=False)

    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company, domain="[('id', 'in', allowed_company_ids)]")
    resource_calendar_ids = fields.Many2many(
        'resource.calendar', 'calendar_public_holiday_rel', 'leave_id', 'calendar_id',
        string='Working Hours', domain="[('company_id', 'in', [company_id, False])]")
    work_address_ids = fields.Many2many('res.partner', string='Work Address', domain="[('is_company', '=', True)]")
    country_id = fields.Many2one('res.country', string='Country', domain=lambda self: [('id', 'in', self.env.companies.country_id.ids)])

    @api.constrains('date_from', 'date_to', 'resource_calendar_ids')
    def _check_compare_dates(self):
        all_existing_leaves = self.env['hr.leave.public.holiday'].search([
        ...
        for record in self:
            ...
            if record.resource_calendar_ids:
                existing_leaves = existing_leaves.filtered(lambda l: not l.resource_calendar_ids or l.resource_calendar_ids == record.resource_calendar_ids)
            if existing_leaves:
    ...
    def _prepare_public_holidays_values(self, vals_list):
        for vals in vals_list:
            ...
            if not vals.get('resource_calendar_ids') or \
            ...
            calendar_ids = [calendar[1] for calendar in vals.get('resource_calendar_ids')]
            calendars_tz = set(self.env['resource.calendar'].browse(calendar_ids).mapped('tz'))
            if len(calendars_tz) != 1:
                raise ValidationError(self.env._('All calendars must be in the same timezone.'))
            calendar_tz = pytz.timezone(calendars_tz.pop())
    ...
    def copy_data(self, default=None):
        ...
        return [dict(vals, name=self.env._("%s (copy)", holiday.name)) for holiday, vals in zip(self, vals_list)]
