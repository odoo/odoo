...
class HrHolidaysStatus(models.Model):
    ...
    active = fields.Boolean('Active', help="If the active field is set to false, it will allow you to hide the leave type without removing it.", default=True)
    ...
    virtual_remaining_leaves = fields.Float(compute='_compute_user_left_days', string='Virtual Remaining Leaves', help='Maximum Leaves Allowed - Leaves Already Taken - Leaves Waiting Approval')
    ...
    def get_days(self, employee_id):
        result = {sid: {'max_leaves':0, 'leaves_taken':0, 'remaining_leaves':0, 'virtual_remaining_leaves':0} for sid in self.ids}
        ...
        for holiday in holidays:
            ...
            if holiday.type == 'add' and holiday.state == 'validate':
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        to the method. """
        status_ids = super(HrHolidaysStatus, self)._search(args, offset=offset, limit=limit, order=order, count=count, access_rights_uid=access_rights_uid)
