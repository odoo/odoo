    ...
    def _compute_leave_type_increases_duration(self):
        ...
        for leave in self:
            ...
            if leave.leave_type_request_unit == 'day' and leave.holiday_status_requires_allocation and days < leave.number_of_days:
                leave.leave_type_increases_duration = _("According to your working schedule you are expected to work"
    ...
    def action_approve(self, check_state=True):
        ...
        for leave in self:
            if check_state and leave.can_validate or not check_state and leave.validation_type != "both":
                ...
                raise UserError(_('You cannot approve this leave.'))
    ...
    def _check_approval_update(self, state, raise_if_not_possible=True):
        ...
        for holiday in self:
            ...
            if holiday.state == state:
                ...
                error_message = _('Not possible state. State Approve is only used for leave needed 2 approvals')
            elif holiday.state == 'cancel':
                error_message = _('A cancelled leave cannot be modified.')

            # Leave in past
            elif is_in_past and not is_officer:
                error_message = _('Only a Time Off Officer can change the state of a leave in the past.')

            elif state not in dict_all_possible_state.get(holiday.state, {}):
                if state == 'cancel':
                    error_message = _('You can only cancel your own leave. You can cancel a leave only if this leave \
is approved, validated or refused.')
                elif state == 'confirm':
                    error_message = _('You can\'t reset a leave. Cancel/delete this one and create an other')
                elif state == 'validate1':
                    if not is_time_off_manager:
                        error_message = _('Only a Time Off Officer/Manager can approve a leave.')
                    else:
                        error_message = _('You can\'t approve a validated leave.')
                elif state == "validate":
                    if not is_time_off_manager:
                        error_message = _('Only a Time Off Officer/Manager can validate a leave.')
                    elif holiday.state == "refuse":
                        error_message = _('You can\'t approve this refused leave.')
                    else:
                        error_message = _('You can only validate a leave with validation by Time Off Manager.')
                elif state == "refuse":
                    if not is_time_off_manager:
                        error_message = _('Only a Time Off Officer/Manager can refuse a leave.')
                    else:
                        error_message = _('You can\'t refuse a leave with validation by Time Off Officer.')
            elif state != "cancel":
                try:
