...
class HrLeave(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context.get('holiday_status_display_name', True) and 'holiday_status_id' in fields_list and not defaults.get('holiday_status_id'):
            lt = self.env['hr.leave.type'].search(['|', ('requires_allocation', '=', False), ('has_valid_allocation', '=', True)], limit=1, order='sequence')
            if lt:
    ...
    def _compute_dashboard_warning_message(self):
        ...
        for holiday in self:
            ...
            if holidays_only_have_uid:
                msg = _('You\'ve already booked time off which overlaps with this period:')
            else:
                msg = _('An employee already booked time off which overlaps with this period:')
    ...
    def _compute_leave_type_increases_duration(self):
        ...
        for leave in self:
            ...
            if leave.leave_type_request_unit == 'day' and days < leave.number_of_days:
                leave.leave_type_increases_duration = _("According to your working schedule You are expected to work %(days)s days in this period, but %(nb_days)s days will be used because this leave %(leave_type_name)s can only be taken by days.", days=days, nb_days=leave.number_of_days, leave_type_name=leave.holiday_status_id.name)
            else:
    ...
    def _unlink_if_correct_states(self):
        error_message = _('Oops! %(state)s Time-Off requests can only be deleted by Administrators.')
    ...
    def _check_approval_update(self, state):
        ...
        for holiday in self:
            ...
            # Leave in past
            if is_in_past and not is_officer:
                raise UserError(_('Only a Time Off Officer can change the state of a leave in the past.'))
