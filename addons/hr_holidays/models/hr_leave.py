...
class HolidaysRequest(models.Model):
    ...
    def _check_approval_update(self, state):
        ...
        for holiday in self:
            ...
            if state == 'draft':
                if holiday.employee_id != current_employee and not is_officer:
                    raise UserError(_('Only a Leave Officer or Manager can reset other people leaves.'))
                continue
            ...
            if holiday.employee_id == current_employee and not is_officer:
                raise UserError(_('Only a Leave Officer or Manager can approve its own requests.'))

            if (state == 'validate1' and val_type == 'both') or (state == 'validate' and val_type == 'manager'):
                manager = holiday.employee_id.parent_id or holiday.employee_id.department_id.manager_id
                if (manager and manager != current_employee) and not is_officer:
                    raise UserError(_('You must be either %s\'s manager or Leave Officer / Manager to approve this leave') % (holiday.employee_id.name))

            if state == 'validate' and val_type == 'both' and not is_officer:
                raise UserError(_('Only an Leave Officer / Manager can apply the second approval on leave requests.'))
