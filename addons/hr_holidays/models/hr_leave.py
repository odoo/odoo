...
%(versions)s""",
    ...
    def _check_validity(self):
        ...
        for (leave_type, date_from), leaves in sorted_leaves.items():
            ...
            if leave_type.allows_negative:
                ...
                for employee in employees:
                    if ((len(leave_data[employee]) == 0)
                        or (leave_data[employee] and leave_data[employee][0][1]['virtual_remaining_leaves'] < -max_excess)):
                        leave_issues['invalid_allocation'].append(employee.name)
                continue
        ...
        if not is_leave_user and any(leave.has_mandatory_day for leave in self):
            leave_issues['mandatory_day'].append(employee.name)

        if any(leave_issues.values()):
            messages = []
            if leave_issues['invalid_allocation']:
                names = "\n".join(f"- {name}" for name in leave_issues['invalid_allocation'])
                messages.append(f"No valid allocation for:\n{names}")
            if leave_issues['mandatory_day']:
                names = "\n".join(f"- {name}" for name in leave_issues['mandatory_day'])
                messages.append(f"Mandatory leave day violation:\n{names}")
            full_message = "\n\n".join(messages)
            raise ValidationError(f"Leave issues detected:\n\n{full_message}")
