...
class HrLeave(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context.get('holiday_status_display_name', True) and 'holiday_status_id' in fields_list and not defaults.get('holiday_status_id'):
            ...
            lts = self.env['hr.leave.type'].search(domain, order='sequence')
            selected_leave_type = next(
                (
                    leave_types for leave_types in lts
                    if (defaults.get('request_unit_hours') and leave_types['request_unit'] == 'hour') or (not defaults.get('request_unit_hours'))
                ),
                lts[0] if lts else None,
            )
