...
been taken for this time off type. Changing it now would affect existing employee balances."))
    ...
    @api.model
    def _get_country_domain(self, country):
        if country and self.search_count([('country_id', '=', country.id)], limit=1):
            return Domain('country_id', '=', country.id)
        return Domain('country_id', '=', False)
