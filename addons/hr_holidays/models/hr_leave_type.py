        ...
        for record in self:
            ...
            if record.requires_allocation == "yes" and not self._context.get("from_manager_leave_form"):
                ...
                if record.request_unit == "hour":
                    name = _("%(name)s (%(remaining_time)s remaining out of %(maximum)s hours)", name=record.name, remaining_time=remaining_time, maximum=maximum)
                else:
                    name = _("%(name)s (%(remaining_time)s remaining out of %(maximum)s days)", name=record.name, remaining_time=remaining_time, maximum=maximum)
            ...
            for leave_type in leave_type_requires_allocation:
                ...
                if closest_allocation.date_to:
                    ...
                    calendar_attendance = calendar._work_intervals_batch(
                        ...
                        datetime.combine(closest_allocation.date_to, time.max).replace(tzinfo=pytz.UTC),
