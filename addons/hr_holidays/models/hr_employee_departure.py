...
class HrEmployeeDeparture(models.Model):
    ...
    def action_register(self):
        ...
        for departure in self:
            if not departure.do_cancel_time_off_requests:
                continue
            employee_leaves = self.env['hr.leave'].search([
                ('employee_id', 'in', departure.employee_id.id),
            ...
            if employee_leaves:
                ...
                changes_leaves = leaves_with_departure.filtered(lambda leave: leave.date_to.date() <= departure.departure_date)
