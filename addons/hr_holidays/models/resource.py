...
class ResourceCalendar(models.Model):
    ...
    def _public_holidays_intervals_batch(self, start_dt, end_dt, resources=None, domain=None, tz=None):
        if not domain:
            domain = []
        resources_list = self._get_resources_list(resources)
    ...
    def _get_resources_list(self, resources=None):
        if not resources:
            if self.env.user.employee:
                return [self.env.user.employee_id.resource_id, self.env['resource.resource']]
            return [self.env['resource.resource']]
        return list(resources) + [self.env['resource.resource']]
