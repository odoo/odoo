...
class HrLeaveGenerateMultiWizard(models.TransientModel):
    ...
    def action_generate_time_off(self):
        ...
        if employees_with_conflicts and not leaves:
            ...
            notify_message = self.env._('No time off requests were created. %(failure_count)s employee(s) have overlapping time off: %(employees)s',
                                        failure_count=len(employees_with_conflicts),
                                        employees=', '.join(employees_with_conflicts.mapped('name')))
        elif employees_with_conflicts:
            if leaves:
                notify_type = 'warning'
                notify_message = self.env._('%(success_count)s time off request(s) created successfully. %(failure_count)s employee(s) skipped due to overlapping time off: %(employees)s',
            ...
            notify_message = self.env._('%(success_count)s time off request(s) created successfully.', success_count=len(leaves))
