...
class HrHolidaySummaryReport(models.AbstractModel):
    ...
    def _get_leaves_summary(self, start_date, empid, holiday_type):
        ...
        holidays = HrHolidays.search([('employee_id', '=', empid), ('state', 'in', holiday_type), ('type', '=', 'remove'), ('date_from', '<=', str(end_date)), ('date_to', '>=', str(start_date))])
            ...
            for department in HrDepartment.browse(data['depts']):
                ...
                for employee in employees:
                    res[len(res)-1]['data'].append({
                        'emp': employee.name,
                        'display': self._get_leaves_summary(data['date_from'], employee.id, data['holiday_type']),
                        'sum': self.sum
                    })
        elif 'emp' in data:
    ...
    def _get_holidays_status(self):
        res = []
        holidays = self.env['hr.holidays.status'].search([])
        for holiday in holidays:
