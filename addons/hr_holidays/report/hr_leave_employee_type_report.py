...
class HrLeaveEmployeeTypeReport(models.Model):
    ...
    def _get_report_records(self):
        ...
        for employee, employee_allocation_data in allocation_data.items():
            ...
            for _name, data, _requires_allocation, allocation_work_entry_type_id in employee_allocation_data:
                if allocation_work_entry_type_id not in scoped_type_ids:
                    continue
                if not data['max_leaves'] and not data['virtual_remaining_leaves']:
                    continue
