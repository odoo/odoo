...
class TestHrLeaveMandatoryDays(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_emp_leave_manager = new_test_user(cls.env, login='timeoff_officer', groups='base.group_user', company_ids=[(6, 0, cls.company.ids)], company_id=cls.company.id)
    ...
    def test_approve_leave_timeoff_officer_on_mandatory_days(self):
        """
            ...
            Expectation: The timeoff officer should be able to approve the request.
        """
        leave_type = self.env['hr.leave.type'].create({
            'name': 'Sick Time Off',
            'time_type': 'leave',
        ...
        employee_leave.with_user(self.user_emp_leave_manager).action_approve()
        self.assertEqual(employee_leave.state, 'validate1', 'employee leave state should be in validate1 stage')
