...
class TestLeaveRequests(TestHrHolidaysCommon):
    ...
    def test_current_leave_status(self):
        ...
        def run_validation_flow(leave_validation_type):
            ...
            employee._compute_current_leave()
    ...
    def test_mail_update_with_both_validation(self):
        """Test the email notification flow for Time Off when 'both' validation is required
            (i.e., approval from both the Time Off Officer and the Employee's Approver).
        """
        self.holidays_type_4.responsible_ids = [Command.link(self.user_employee.id)]
        self.employee_hruser.leave_manager_id = self.user_hrmanager_id
        ...
        message = self.env['mail.message'].search([('res_id', '=', test_holiday_2.id)], limit=1)
        self.assertEqual(message.subject, "I'm requesting 1 days of Limited with 2 approvals from 2022-03-09 to 2022-03-09")
        self.assertEqual(message.partner_ids, self.user_hrmanager.partner_id, "The mail should be sent to the Employee's Approver.")

        test_holiday_2.action_approve()

        message = self.env['mail.message'].search([('res_id', '=', test_holiday_2.id)], limit=1)
        self.assertEqual(message.subject, "I'm requesting 1 days of Limited with 2 approvals from 2022-03-09 to 2022-03-09")
        self.assertEqual(message.partner_ids, self.user_employee.partner_id, "The mail should be sent to the Time Off Officer after first approval.")
