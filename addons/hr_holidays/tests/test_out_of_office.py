...
class TestOutOfOffice(TestHrHolidaysCommon):
    ...
    def test_leave_ooo(self):
        ...
        partner_info = next(c['partner'] for c in channel_info['members'] if c['partner']['email'] == partner.email)
        partner2_info = next(c['partner'] for c in channel_info['members'] if c['partner']['email'] == partner2.email)
