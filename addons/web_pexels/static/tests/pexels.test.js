...
test("Pexels is inserted in the Media Dialog", async () => {
    const imageRecord = {
        id: 1,
        name: "logo",
        mimetype: "image/png",
    ...
    onRpc("/web_pexels/attachment/add", (args) => [
        { ...imageRecord, description: "pexels_image" },
    ]);
    ...
    await waitFor(".o-wysiwyg img[alt='pexels_image']");
    expect(".o-wysiwyg img[alt='pexels_image']").toHaveCount(1);
});

test("Pexels error is displayed when there is no key", async () => {
    const imageRecord = {
        id: 1,
        name: "logo",
        mimetype: "image/png",
    ...
    contains("input.o_we_search").edit("cat");
    await fetchDef;
    await waitFor(".pexels_error");
    expect(".pexels_error").toHaveCount(1);
});
