...
def odoo_payments_proxy_control():
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kw):
            ...
            if not secret or not msg_signature or not msg_timestamp:
                raise Forbidden()

            if int(msg_timestamp) + TIMEOUT < int(time.time()):
                _logger.debug('HTTP request validation failed due to invalid timestamp for route "%s"', request.httprequest.path)
                raise Forbidden()
