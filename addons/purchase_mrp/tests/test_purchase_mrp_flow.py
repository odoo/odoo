...
class TestPurchaseMrpFlow(AccountTestInvoicingCommon):
    ...
    def test_purchase_kit_bill_before_reception_component_cost_exactly_aligns_with_kit_product_cost(self):
        ...
        self.assertRecordValues(
            ...
            [
                {'account_id': stock_input_account.id,       'reconciled': True,    'debit': 10.0,   'credit':  0.0},
                {'account_id': tax_paid_account.id,          'reconciled': False,   'debit':  1.5,   'credit':  0.0},
                {'account_id': account_payable_account.id,   'reconciled': False,   'debit':  0.0,   'credit': 11.5},
                {'account_id': stock_input_account.id,       'reconciled': True,    'debit':  0.0,   'credit':  5.0},
                {'account_id': stock_valuation_account.id,   'reconciled': False,   'debit':  5.0,   'credit':  0.0},
                {'account_id': stock_input_account.id,       'reconciled': True,    'debit':  0.0,   'credit':  5.0},
                {'account_id': stock_valuation_account.id,   'reconciled': False,   'debit':  5.0,   'credit':  0.0},
            ]
        )
