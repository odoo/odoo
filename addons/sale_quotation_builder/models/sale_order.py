...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals.update(self._inject_quotation_description(vals))
        return super(SaleOrderLine, self).create(vals_list)

    def write(self, values):
        values.update(self._inject_quotation_description(values))
    ...
    def _inject_quotation_description(self, values):
        ...
        if not values.get('website_description') and values.get('product_id'):
            ...
            return dict(website_description=product.quotation_description)
        return dict()
