...
describe("date command", () => {
    ...
    test.tags("desktop");
    test("manual time input should not round minutes when editing time", async () => {
        const dateUTC = DateTime.now().toUTC().toISO();
        await setupEditor(
            `<p><span data-embedded="date" data-embedded-props='{"date":"${dateUTC}","type":"time"}'></span></p>`,
            {
                config: configWithEmbeddings,
            }
        );
        ...
        await press("enter");
        await animationFrame();
        expect('[data-embedded="date"] span').toHaveText("4:12 PM");
    });
    ...
    test.tags("desktop");
    test("manual time input should not round minutes when inserting datetime", async () => {
        const { editor } = await setupEditor("<p>[]<br></p>", {
            config: configWithEmbeddings,
        });
        await insertText(editor, "/datetime");
        ...
        await contains(".o_datetime_buttons button").click();
        await expectElementCount(".o_datetime_picker", 0);
        expect('[data-embedded="date"] span').toHaveText("Apr 6, 2026, 10:17 AM");
    });
