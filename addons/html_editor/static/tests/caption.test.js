...
import { manuallyDispatchProgrammaticEvent, click, press, queryOne, waitFor, dblclick } from "@odoo/hoot-dom";
...
import { makeMockEnv, onRpc, contains } from "@web/../tests/web_test_helpers";
...
const configWithEmbeddedCaption = {
    ...
    resources: { embedded_components: [CaptionPluginWithPredictableId, ...MAIN_EMBEDDINGS.filter(plugin => plugin.id !== "caption")] },
};
const setupEditorWithEmbeddedCaption = async content => {
    return await setupEditor(content, { config: configWithEmbeddedCaption });
};
...
const addLinkToImage = async url => {
    ...
    if (url) {
        ...
        await contains(".o-we-linkpopover input.o_we_href_input_link", { timeout: 1500 }).edit("odoo.com");
    }
}
...
test("previewing an image with a caption shows the caption as title", async () => {
    ...
    // In real life the blur event has the time to resolve between the two
    // clicks (of the double click).
    await click("img");
    await animationFrame();
    await click("img");
    await animationFrame();
