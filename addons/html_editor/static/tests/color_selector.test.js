...
test("can render and apply gradient color", async () => {
    ...
    await animationFrame();
    expect("i[data-icon='A']").toHaveStyle({
        borderImage:
...
test("selected text color is shown in the toolbar and update when hovering", async () => {
    ...
    await animationFrame();
    expect("i[data-icon='A']").toHaveStyle({ borderBottomColor: "rgb(255, 0, 255)" });
    // Hover out
...
test("selected text color is not shown in the toolbar after removeFormat", async () => {
    ...
    await animationFrame();
    expect("i[data-icon='A']").toHaveStyle({ borderBottomColor: defaultTextColor });
});
...
test("selected color is shown and updates when selection change", async () => {
    ...
    expect(".o_font_color_selector").toHaveCount(0);
    await animationFrame();
    expect("i[data-icon='A']").toHaveStyle({ borderBottomColor: "rgb(150, 255, 0)" });
    setSelection({
    ...
    await waitUntil(
        () => queryOne("i[data-icon='A']").style.borderBottomColor === "rgb(255, 156, 0)"
    );
    expect("i[data-icon='A']").toHaveStyle({ borderBottomColor: "rgb(255, 156, 0)" });
});
