...
test("should add opacity to custom background colors but not to theme colors", async () => {
    ...
    await click(".o_color_button[data-color='#94BD7B']"); // Select a custom color.
    await animationFrame();
    await waitFor(".o-we-toolbar");
    ...
    expect(getContent(el)).toBe(
        `<p><font style="background-color: rgba(148, 189, 123, 0.6);">[test]</font></p>`
    );
    // Verify paintbrush border bottom color has no opacity.
    expect("i.fa-paint-brush").toHaveStyle({ borderBottomColor: "rgb(148, 189, 123)" });

    await click(".o-select-color-background");
    await animationFrame();
    expect(".o_font_color_selector").toHaveCount(1);

    await click(".o_color_button[data-color='o-color-1']"); // Select a theme color
    await animationFrame();
    await waitFor(".o-we-toolbar");
