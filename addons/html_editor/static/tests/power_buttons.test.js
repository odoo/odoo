...
describe("visibility", () => {
    ...
    test("should show power buttons on block p tag contains strong (bold)", async () => {
        await setupEditor(`<p>${strong("[]\u200B", "first")}</p>`);
        expect(".o_we_power_buttons").toBeVisible();
    });

    test("should show power buttons on block p tag contains em (italic)", async () => {
        await setupEditor(`<p>${em("[]\u200B", "first")}</p>`);
        expect(".o_we_power_buttons").toBeVisible();
    });

    test("should show power buttons on block p tag contains u (underline)", async () => {
        await setupEditor(`<p>${u("[]\u200B", "first")}</p>`);
        expect(".o_we_power_buttons").toBeVisible();
    });

    test("should show power buttons on block p tag contains s (strikethrough)", async () => {
