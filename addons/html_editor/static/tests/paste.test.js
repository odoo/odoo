...
describe("onDrop", () => {
    test("should drop text from htmlTransferItem before the selected text", async () => {
        const { el } = await setupEditor("<p>a[b]cd</p>");
        const pElement = el.firstChild;
        const dragdata = new DataTransfer();
        dragdata.setData("text/html", "b");
        await dispatch(pElement, "dragstart", { dataTransfer: dragdata });

        const textNode = pElement.childNodes[pElement.childNodes.length - 1];

        patchWithCleanup(document, {
            caretPositionFromPoint: () => ({ offsetNode: textNode, offset: 1 }),
        });

        await dispatch(pElement, "drop", { dataTransfer: dragdata });
        await tick();

        expect(getContent(el)).toBe("<p>acb[]d</p>");
    });
    test("should drop text from htmlTransferItem after the selected text", async () => {
        const { el } = await setupEditor("<p>a[b]cd</p>");
        const pElement = el.firstChild;
