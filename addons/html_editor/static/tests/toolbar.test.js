...
test("toolbar works: displays correct font size on input", async () => {
    ...
    expect(".o_font_selector_menu").toHaveCount(1);
...
test("toolbar works: font size dropdown closes on Enter and Tab key press", async () => {
    ...
    await press("Enter");
    await animationFrame();
    expect(".o_font_selector_menu").toHaveCount(0);
...
test("toolbar works: ArrowUp/Down moves focus to font size dropdown", async () => {
    ...
    const fontSelectorMenu = queryOne(".o_font_selector_menu");
    await press("ArrowDown");
    await animationFrame();
    expect(".o_font_selector_menu").toHaveCount(1);
    expect(getActiveElement()).toBe(fontSelectorMenu.firstElementChild);
    ...
    await animationFrame();
    expect(".o_font_selector_menu").toHaveCount(1);
    expect(getActiveElement()).toBe(fontSelectorMenu.lastElementChild);
});
