...
test("toolbar enable link button when deep selection is text (1)", async () => {
    await setupEditor("[<ul><li>test</li></ul>]");

    await waitFor(".o-we-toolbar");
    expect(".btn[name='link']").not.toHaveClass("disabled");
});

test("toolbar enable link button when deep selection is text (2)", async () => {
    await setupEditor(`<div>[<hr contenteditable="false"><div>test</div>]</div>`);
