...
test("playlists are embedded with the right url", () => {
    ...
    for (const [platform, url, embedUrl] of [
        ...
        [
            "dailymotion",
            "https://www.dailymotion.com/playlist/x61w5b",
            "https://geo.dailymotion.com/player.html?playlist=x61w5b",
        ],
        // Dailymotion plays the video, then the playlist.
        [
            "dailymotion",
            "https://geo.dailymotion.com/player.html?video=x7svr6t&playlist=x61w5b",
            "https://geo.dailymotion.com/player.html?video=x7svr6t&playlist=x61w5b",
        ],
        // The custom player of an embed url is kept.
        [
            "dailymotion",
            "https://geo.dailymotion.com/player/xk3d1.html?playlist=x61w5b",
            "https://geo.dailymotion.com/player/xk3d1.html?playlist=x61w5b",
        ],
        [
            "twitch",
...
test("a playlist has no video thumbnail", async () => {
    ...
    const list = "PL4fGSI1pDJn6O1LS0XSdF3RyO0Rq_LDeI";
    expect(getThumbnailUrl("youtube", `https://www.youtube.com/playlist?list=${list}`)).toBe("");
    expect(getThumbnailUrl("dailymotion", "https://www.dailymotion.com/playlist/x61w5b")).toBe("");
...
test("a clip is embedded with the clip player", () => {
    const clip = "UnrulyShySalamanderPogChamp";
    ...
    for (const url of [
        `https://www.twitch.tv/monstercat/clip/${clip}`,
