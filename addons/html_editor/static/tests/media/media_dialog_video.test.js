...
describe("media dialog video", () => {
    ...
    describe("video files", () => {
        ...
        test("should insert a video file with the default options", async () => {
            ...
            // The container holds what the `<video>` element is rebuilt from.
            expect(containerEl.dataset.baseUrl).toBe("https://example.com/video/my-video.mp4");
            expect(containerEl.dataset.embedUrl).toBe("https://example.com/video/my-video.mp4");
        ...
        test("should apply the selected options on the inserted video file", async () => {
            ...
            // The options are stored on the container as well, since the
            // `<video>` element is rebuilt from it.
            const containerEl = await waitFor("div.media_iframe_video");
            const options = JSON.parse(containerEl.dataset.videoOptions);
            expect(options.loop).toBe(true);
            expect(options.autoplay).toBe(true);
            expect(options.hideControls).toBe(true);
        });
