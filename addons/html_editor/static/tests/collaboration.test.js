...
describe("Collaboration with embedded components", () => {
    ...
    describe("Embedded state", () => {
        ...
        test("A user change to the embedded state is properly applied for every other collaborator", async () => {
            const peerInfos = await setupMultiEditor({
                peerIds: ["c1", "c2"],
        ...
        test("Undo and Redo can overwrite a collaborator changes to the embedded state", async () => {
            ...
            // a collaborator, and the current user can not undo it. Instead, the history allows the
            // user to undo his own last step. In summary:
            // - undo for user goes from the current state (which can be set by the collaborator)
            // to the state before his own last step.
            // - redo for user goes from the current state (which can be set by the collaborator)
        ...
        test("A change from a collaborator with the same values as the previous change done by the user is properly applied", async () => {
            const peerInfos = await setupMultiEditor({
                peerIds: ["c1", "c2"],
