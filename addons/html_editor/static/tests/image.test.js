...
async function openImageDropdown(buttonName) {
    await click("img");
    await waitFor(".o-we-toolbar");
...
describe("Image size preview with mouse hover", () => {
    test.tags("desktop");
    test("should preview different image sizes on hover and revert when the mouse leaves", async () => {
        const { el } = await setupEditor(`<p><img src="/web/static/img/logo2.png"></p>`);
        await openImageDropdown("image_size");
...
describe("Image size preview with keyboard", () => {
    test.tags("desktop");
    test("should preview different image sizes while navigating with keyboard and revert on Escape", async () => {
        await setupEditor(`<p><img src="/web/static/img/logo2.png"></p>`);
        await openImageDropdown("image_size");
        await expectElementCount(".image_size_selector", 1);
    ...
    test("should revert preview when Escape closes the dropdown (existing size)", async () => {
        await setupEditor(`<p><img src="/web/static/img/logo2.png" style="width: 50%;"></p>`);
        await openImageDropdown("image_size");
        await expectElementCount(".image_size_selector", 1);
        await press("ArrowDown");
        await press("ArrowDown");
...
describe("Image padding preview with mouse hover", () => {
    test.tags("desktop");
    test("should preview different image paddings on hover and revert when the mouse leaves", async () => {
        const { el } = await setupEditor(imageHtml);
        await openImageDropdown("image_padding");
    ...
    test("should revert preview when mouse leaves without applying image padding (existing padding)", async () => {
        const { el } = await setupEditor(imageP2Html);
        await openImageDropdown("image_padding");
        await expectElementCount(".image_padding_selector", 1);
        await hover(getDropdownItem("Large"));
        expect("img").toHaveClass("p-3");
...
describe("Image padding preview with keyboard", () => {
    test.tags("desktop");
    test("should preview different image paddings while navigating with keyboard and revert on Escape", async () => {
        await setupEditor(imageHtml);
        await openImageDropdown("image_padding");
        await expectElementCount(".image_padding_selector", 1);
        await press("ArrowDown");
...
describe("Image alignment preview with mouse hover", () => {
    test.tags("desktop");
    test("should preview different image alignments on hover and revert when the mouse leaves", async () => {
        const { el } = await setupEditor(imageHtml);
        await openImageDropdown("image_align");
    ...
    test("should revert preview when mouse leaves without applying image alignment (existing alignment)", async () => {
        const { el } = await setupEditor(imageFloatHtml);
        await openImageDropdown("image_align");
        await expectElementCount(".o-we-toolbar-dropdown", 1);
...
describe("Image alignment preview with keyboard", () => {
    test.tags("desktop");
    test("should preview different image alignments while navigating with keyboard and revert on Escape", async () => {
        await setupEditor(imageHtml);
        await openImageDropdown("image_align");
        await expectElementCount(".o-we-toolbar-dropdown", 1);
    ...
    test("should revert preview when Escape closes the dropdown (existing alignment)", async () => {
        await setupEditor(imageFloatHtml);
        await openImageDropdown("image_align");
        await expectElementCount(".o-we-toolbar-dropdown", 1);
