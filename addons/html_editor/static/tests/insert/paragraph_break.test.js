...
import { patchPrism, SYNTAX_HIGHLIGHTING_WRAPPER } from "../_helpers/syntax_highlighting";
...
describe("Selection collapsed", () => {
    ...
    describe("Pre", () => {
        describe("with syntax highlighting", () => {
            patchPrism();
            const testEnterInCodeBlock = (selectionStart, codeAfter) => async (editor) => {
            ...
            test("should insert a line break within the pre", async () => {
                await testEditor({
                    ...
                    contentBeforeEdit: SYNTAX_HIGHLIGHTING_WRAPPER("abcd", { highlight: false }),
                    stepFunction: testEnterInCodeBlock(2), // "ab[]cd"
                    contentAfterEdit: SYNTAX_HIGHLIGHTING_WRAPPER("ab<br>cd", { focused: true }),
                });
            });
            test("should insert a new line at the end of the pre", async () => {
                await testEditor({
                    contentBefore: "<pre>abc</pre>",
                    contentBeforeEdit: SYNTAX_HIGHLIGHTING_WRAPPER("abc", { highlight: false }),
                    stepFunction: testEnterInCodeBlock(3), // "abc[]"
                    contentAfterEdit: SYNTAX_HIGHLIGHTING_WRAPPER(
                        `<span id="plaintext">abc</span><br><br>`,
                        { highlight: false, focused: true } // Doing the highlight manually here.
                    ),
                });
            });
        });
