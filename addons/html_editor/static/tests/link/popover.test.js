...
describe("upload file via link popover", () => {
    ...
    test("can create a link to an uploaded file", async () => {
        ...
        await click(".o_we_apply_link");
        await animationFrame();
        // Created link has the correct href and label, and contains the oe_attachment_link class
        expect(cleanLinkArtifacts(getContent(el))).toBe(
            `<p><a class="oe_attachment_link" href="${expectedUrl}">file.txt[]</a><br></p>`
        );
    });
    ...
    test("link to attachment should have a download icon before it", async () => {
        await setupEditor(
            '<p><a class="oe_attachment_link" href="/web/content/1?download=true&unique=123">file.txt</a></p>'
        );
        const link = queryOne("a.oe_attachment_link");
        const beforeStyle = getComputedStyle(link, "before");
        expect(beforeStyle.content).toBe('"\uf019"'); // fa-download
    });
