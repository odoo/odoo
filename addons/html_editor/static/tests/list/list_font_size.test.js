...
test("should apply font-size to completely selected multiple list items", async () => {
    await testEditor({
        ...
        stepFunction: (editor) =>
            editor.dispatch("FORMAT_FONT_SIZE_CLASSNAME", { className: "h2-fs" }),
        contentAfter:
...
test("should apply font-size to completely selected list items and paragraph tag", async () => {
    await testEditor({
        ...
        stepFunction: (editor) =>
            editor.dispatch("FORMAT_FONT_SIZE_CLASSNAME", { className: "display-3-fs" }),
        contentAfter:
...
test("should remove font-size style on converting the text to a block tag", async () => {
    await testEditor({
        ...
        stepFunction: (editor) => editor.dispatch("SET_TAG", { tagName: "H1" }),
