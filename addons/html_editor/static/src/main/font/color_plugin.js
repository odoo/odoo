    ...
    _applyColor(color, mode, previewMode = false) {
        ...
        const getFonts = (selectedNodes) => {
            return selectedNodes.flatMap((node) => {
                let font = closestElement(node, "font") || closestElement(node, "span") || closestElement(node, '[style*="color"], [style*="background-color"], [style*="background-image"]');
                const children = font && descendants(font);
                if (
                    font && font.nodeName !== "T" &&
                    (font.nodeName !== "SPAN" || font.style[mode])
                ) {
