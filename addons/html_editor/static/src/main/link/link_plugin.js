...
export class LinkPlugin extends Plugin {
    static id = "link";
    static dependencies = ["dom", "history", "selection", "split", "lineBreak", "overlay", "color"];
    ...
    setup() {
        ...
        this.addDomListener(this.editable, "click", (ev) => {
            if (ev.target.tagName === "A" && ev.target.isContentEditable) {
                ev.preventDefault();
                // this.openLinkTools(ev.target); // the handle selection change should handle this
            }
        });
        ...
        this.detroyKeyboardCommand = this.services.command.add(
            "Create link",
            () => {
    ...
    openLinkTools(linkElement) {
        ...
        if (linkElement) {
            const linkWasExtended = this.extendsLinkToSelection(linkElement, selection);
            if (linkWasExtended) {
            ...
            if (this.linkInDocument && isImage) {
                ...
                if ((selectionTextContent && selectionTextContent === label) || isImage) {
                    ...
                    if (commonAncestor !== anchorClosestElement) {
                        this.dependencies.selection.forceCursorAfter(anchorClosestElement);
                    }
                    this.dependencies.dom.insert(link);
                } else if (label) {
        ...
        const props = {
            linkElement,
            isImage: isImage,
            onApply: applyCallback.bind(this),
            onRemove: () => {
    ...
    closeLinkTools(cursors = null) {
        ...
        this.linkInDocument = null;
        // Some unit tests fail when his.overlay.isOpen but the DOM don't contain the linkPopover yet.
        ...
        if (this.overlay.isOpen && document.querySelector(".o-we-linkpopover")) {
            ...
            if (link && link.isConnected) {
                ...
                if (cleanZWChars(link.innerText) === "" && !link.querySelector("img")) {
                    this.dependencies.selection.forceCursorAfter(link);
                    link.remove();
                } else if (cursors) {
                    cursors.restore();
                } else {
    ...
    handleSelectionChange(selectionData) {
        ...
        if (!selectionData.documentSelectionIsInEditable) {
            ...
            if (imageNode?.parentNode?.tagName === "A" && linksInSelection.length < 2) {
     ...
     * @return {boolean}
     */
    extendsLinkToSelection(linkElement, selection) {
        if (
