...
export class TablePlugin extends Plugin {
     ...
     * @param {HTMLTableRowElement} row
     */
    normalizeRowDimension(row) {
        const table = closestElement(row, "table");
        const rows = [...table.rows];
        const referenceRowHeight = parseFloat(getComputedStyle(row).height);
        rows.forEach((row) => {
            if (
                row.style.height &&
    ...
    resetRowSize(row) {
        row.style.height = "";
        this.normalizeRowDimension(row);
    }

    /**
     * @param {HTMLTableElement} table
     */
    normalizeColumnDimension(table) {
        const rows = [...table.rows];
    ...
    resetColumnSize(cell) {
        ...
        this.normalizeColumnDimension(table);
    }
