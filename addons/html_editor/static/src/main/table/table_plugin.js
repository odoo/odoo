...
export class TablePlugin extends Plugin {
    ...
    resources = {
        ...
        // `normalizeTable` must run last: it derives the `<colgroup>` from the
        // first row, which is only complete once the ragged rows have been
        // filled in.
        html_compatibility_processors: [
            this.normalizeTableStructure.bind(this),
            this.fillEmptyTableCells.bind(this),
            this.normalizeTable.bind(this),
        ],
    ...
    handlePasteTableIntoExistingTable(selection, clipboardRoot) {
        ...
        this.fillEmptyTableCells(sourceTable);
     ...
     * @param {Element} root
     */
    fillEmptyTableCells(root) {
        for (const cell of root.querySelectorAll("td, th")) {
            if (isEmptyBlock(cell) && !childNodes(cell).some(isBlock)) {
