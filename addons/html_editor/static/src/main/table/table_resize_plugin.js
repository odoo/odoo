...
export class TableResizePlugin extends Plugin {
    ...
    isHoveringTdBorder(ev) {
        ...
        if (ev.target && ["TD", "TH"].includes(target.nodeName) && target.isContentEditable) {
    ...
    resizeTable(ev, direction, target1, target2) {
        ...
        // clean the rest.
        if (direction === "col") {
            let hostCell = closestElement(table, "td, th");
            const hostCells = [];
            while (hostCell) {
                hostCells.push(hostCell);
                hostCell = closestElement(hostCell.parentElement, "td, th");
            }
        ...
        switch (position) {
            case "first": {
                ...
                if (newMargin >= 0 && newSize > MIN_SIZE) {
                    ...
                    const hostCell = closestElement(table.parentElement, "td, th");
    ...
    fitToContent(ev) {
        ...
        if (["left", "right"].includes(isHoveringTdBorder)) {
            ...
            const currentColumnCells = table.querySelectorAll(
                ...
                tr th:nth-of-type(${currentColumnIndex + 1})`
            );
            ...
            if (
                ...
                const siblingColumnCells = table.querySelectorAll(
                    ...
                    tr th:nth-of-type(${siblingColumnIndex + 1})`
                );
    ...
    onMousedown(ev) {
        ...
        if (isHoveringTdBorder) {
            ...
            if (isHoveringTdBorder === "top" && column) {
                ...
                if (isRTL) {
                    target1 = getAdjacentPreviousSiblings(ev.target).find((node) =>
                        ["TD", "TH"].includes(node.nodeName)
                    );
                    target2 = ev.target;
                } else {
                    target1 = ev.target;
                    target2 = getAdjacentNextSiblings(ev.target).find((node) =>
                        ["TD", "TH"].includes(node.nodeName)
                    );
                }
