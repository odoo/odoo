...
export class ResizePlugin extends Plugin {
    ...
    static shared = ["resetSize", "resetHeight", "resetWidth"];

    /** @type {import("plugins").EditorResources} */
    resources = {
        table_menu_commands: [
            {
                resetSize: this.resetSize.bind(this),
                resetRowHeight: this.resetHeight.bind(this),
                resetColumnWidth: this.resetWidth.bind(this),
            },
        ],
    };
    ...
    getResizeTargets(activeHover) {
        ...
        let [item, neighbor, itemMinSize, neighborMinSize] = [
            target1 || target2,
            target2,
            activeHover.resizingParameter.minSize,
            activeHover.resizingParameter.minSize,
        ];
    ...
    onPointerDown(ev) {
        ...
        const { item, neighbor, position, itemMinSize, neighborMinSize } = this.getResizeTargets(
            this.activeHover
        );

        this.isResizingElement = true;
        const handleResize = (ev) => {
            if ((item && !item.isConnected) || (neighbor && !neighbor.isConnected)) {
    ...
    handleResize(
        ...
        switch (position) {
            case "first": {
                ...
                const newSize = Math.max(currentSize + sizeDelta, itemMinSize);
                if (newMargin >= 0) {
                    const resizeContainerRect = resizeContainer.getBoundingClientRect();
    ...
    /**
     * Reset inline styling and sizes applied to container elements during resizing
     *
     * @param {HTMLElement} container
     * @param {Object} [options]
     * @param {string} [options.widthElementsSelector] - Selector for elements to clear width from
     * @param {string} [options.heightElementsSelector] - Selector for elements to clear height from
     * @param {string} [options.proxyElementSelector] - Selector for proxy elements to remove entirely
     * @returns {void}
     */
    resetSize(
        container,
        { widthElementsSelector, heightElementsSelector, proxyElementSelector } = {}
    ) {
        container.removeAttribute("style");
        if (proxyElementSelector) {
            container.querySelector(proxyElementSelector)?.remove();
        } else if (widthElementsSelector) {
            container.querySelectorAll(widthElementsSelector).forEach((el) => {
                removeStyle(el, "width");
            });
        }
        if (heightElementsSelector) {
            container.querySelectorAll(heightElementsSelector).forEach((el) => {
