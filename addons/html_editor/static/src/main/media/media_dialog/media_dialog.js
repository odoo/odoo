...
export class MediaDialog extends Component {
    ...
    setup() {
        ...
        this.addDefaultTabs();
        this.addExtraTabs();
        this.tabs.sort((a, b) => sequence(a) - sequence(b));
    ...
    get initialActiveTab() {
        ...
        if (this.props.media) {
            const correspondingTab = Object.keys(this.tabsDict).find((id) =>
                this.tabsDict[id].Component.tagNames.includes(this.props.media.tagName)
            );
            if (correspondingTab) {
    ...
    async renderMedia(selectedMedia) {
        const elements = await this.tabsDict[this.state.activeTab].Component.createElements(
            selectedMedia,
            { orm: this.orm }
        );
        elements.forEach((element) => {
            if (this.props.media) {
                ...
                if (this.state.activeTab === this.tabsDict.IMAGES.id) {
            ...
            for (const otherTab of Object.keys(this.tabsDict).filter(
                (key) => key !== this.state.activeTab
            )) {
                for (const property of this.tabsDict[otherTab].Component.mediaSpecificStyles) {
                    element.style.removeProperty(property);
                }
                element.classList.remove(...this.tabsDict[otherTab].Component.mediaSpecificClasses);
                const extraClassesToRemove = [];
                for (const name of this.tabsDict[otherTab].Component.mediaExtraClasses) {
                ...
                element.classList.remove(
                    ...extraClassesToRemove.filter((candidateName) => {
                        for (const name of this.tabsDict[this.state.activeTab].Component
                            .mediaExtraClasses) {
                            if (typeof name === "string") {
            ...
            element.classList.add(
                ...this.tabsDict[this.state.activeTab].Component.mediaSpecificClasses
            );
        });
        return elements;
    }
    ...
    async save() {
        ...
        const saveSelectedMedia =
            ...
            (this.state.activeTab !== this.tabsDict.ICONS.id ||
