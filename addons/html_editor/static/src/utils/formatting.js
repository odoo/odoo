...
export const formatsSpecs = {
    ...
    setFontSizeClassName: {
        isFormatted: (node, props) => {
            const boundaryElement = closestBlock(node)?.parentElement;
            ...
            // otherwise return true if any font size class is active.
            if (props?.className) {
                return props.className === activeFontSizeClass;
            }
            return !!activeFontSizeClass;
        },
        hasStyle: (node, props) =>
