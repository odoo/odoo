...
import { closestElement, closestPath, findNode, descendants } from "./dom_traversal";
...
export const formatsSpecs = {
    ...
    setFontSizeClassName: {
        ...
        addNeutralStyle: function (node, isParentNodeBlockAndCompletelySelected) {
            const block = closestBlock(node);
            if (isParentNodeBlockAndCompletelySelected) {
                const parentNode = node.parentElement;
                for (const element of [parentNode, ...descendants(parentNode).filter(isElement)]) {
                    if (this.hasStyle(element)) {
                        this.removeStyle(element);
                        if (
                            ["SPAN", "FONT"].includes(element.tagName) &&
                            !element.getAttributeNames().length
                        ) {
                ...
                unwrapContents(node);
            } else if (["H1", "H2", "H3", "H4", "H5", "H6"].includes(block.nodeName)) {
