...
export class HistoryPlugin extends Plugin {
    ...
    undo() {
        ...
        if (pos > 0) {
            ...
            this.revertMutations(this.steps[pos].mutations, true);
     ...
     * @param { boolean } forNewStep whether the mutations will be used to create a new step
     */
    ...
    makeSavePoint() {
        ...
        return () => {
            ...
            // as before.
            this.applyMutations(draftMutations, true);
            this.handleObserverRecords();
    ...
    revertStepsUntil(stepIndex) {
        ...
        for (let i = this.steps.length - 1; i > stepIndex; i--) {
            ...
            this.revertMutations(currentStep.mutations, true);
        ...
        for (let i = stepIndex + 1; i < this.steps.length; i++) {
            const currentStep = this.steps[i];
            if (!this.isReversibleStep(i)) {
                this.applyMutations(currentStep.mutations, true);
                this.processNewRecords(this.observer.takeRecords());
            }
        }
