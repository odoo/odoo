...
export class TogglePlugin extends Plugin {
    ...
    resources = {
        ...
        split_element_block_overrides: this.handleSplitElementBlock.bind(this),
    ...
    handleKeyDown(ev) {
        const selection = this.dependencies.selection.getEditableSelection();
        const targetEl = closestElement(selection.anchorNode, isBlock);
    ...
    _arrowDown(selection) {
        ...
        if (
            ...
            next = closestElement(container, (element) => {
                return element.dataset.embedded === "toggle" && element.nextSibling;
            }).nextSibling;
            anchorNode = firstLeaf(
