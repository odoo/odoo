...
export class DynamicTablePlugin extends Plugin {
        ...
        this.fieldPopover.open({
            ...
            props: {
                ...
                validate: async ({ path, fieldInfo }) => {
                    ...
                    const tr = this.createRowElement(target, basePath, path, fieldInfo);
    ...
    createRowElement(target, basePath, path, fieldInfo) {
        ...
        this.document.querySelectorAll("[t-as], [t-set]").forEach((el) => {
            for (const att of [el.getAttribute("t-as"), el.getAttribute("t-set")]) {
                if (!att || !att.startsWith(tAsPrefix)) {
                    continue;
                }
