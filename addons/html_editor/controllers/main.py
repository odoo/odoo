    ...
    def _attachment_create(self, name='', data=False, url=False, res_id=False, res_model='ir.ui.view'):
        ...
        if (
            ...
            if request.env.user.share:
                max_portal_file_size = int(request.env["ir.config_parameter"].sudo().get_param(
                    "html_editor.max_portal_file_size",
                    "100_000_000",
                ))
                if not (attachment.mimetype or '').startswith('image/'):
                    raise AccessError(self.env._("Non-internal users can only upload image."))
                if attachment.file_size >= max_portal_file_size:
                    raise AccessError(self.env._("Non-internal users can't upload large files."))
        else:
