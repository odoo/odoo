...
class HTML_Editor(http.Controller):
    ...
    def _attachment_create(self, name='', data=False, url=False, res_id=False, res_model='ir.ui.view'):
        ...
        if data:
            ...
            raise UserError(request.env._("You need to specify either data or url to create an attachment."))
    ...
    def add_data(self, name, data, is_image, quality=0, width=0, height=0, res_id=False, res_model='ir.ui.view', **kwargs):
        data = b64decode(data)
        if is_image:
            format_error_msg = request.env._("Uploaded image's format is not supported. Try with: %s", ', '.join(SUPPORTED_IMAGE_MIMETYPES.values()))
            try:
    ...
    def save_library_media(self, media):
        ...
        if response.status_code != requests.codes.ok:
            raise Exception(request.env._("ERROR: couldn't get download urls from media library."))
    ...
    def generate_text(self, prompt, conversation_history):
        try:
            ...
            if response['status'] == 'success':
                ...
                raise UserError(request.env._("Sorry, your prompt is too long. Try to say it in fewer words."))
            elif response['status'] == 'limit_call_reached':
                raise UserError(request.env._("You have reached the maximum number of requests for this service. Try again later."))
            else:
                raise UserError(request.env._("Sorry, we could not generate a response. Please try again later."))
        except AccessError:
            raise AccessError(request.env._("Oops, it looks like our AI is unreachable!"))
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                ...
                if not action:
                    return {'error_msg': request.env._("Action %s not found, link preview is not available, please check your url is correct", action_name)}
                action_type = action.type
                if action_type != 'ir.actions.act_window':
                    return {'other_error_msg': request.env._("Action %s is not a window action, link preview is not available", action_name)}
            ...
            return {'error_msg': request.env._("Link preview is not available because %s, please check if your url is correct", str(e))}
