...
class TableExporter(http.Controller):
    ...
    @http.route('/web_graph/check_xlsxwriter', type='json', auth='none')
    ...
    @http.route('/web_graph/export_xlsx', type='http', auth="user")
    def export_xlsx(self, data, token):
        jdata = simplejson.loads(data)
        nbr_measures = jdata['nbr_measures']
        workbook = xlsxwriter.Workbook(response.stream)
        worksheet = workbook.add_worksheet(jdata['title'][:30])
        header_bold = workbook.add_format({'bold': True, 'bg_color': 'BFBFBF'})
        header_plain = workbook.add_format({'bg_color': 'BFBFBF'})
