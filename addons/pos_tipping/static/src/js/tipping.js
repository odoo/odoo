...
var OrderSearchScreen = ScreenWidget.extend({
    ...
    search: function(term) {
        var self = this;
        this.filtered_open_orders = this.pos.db.get_unpaid_orders();
        this.filtered_open_orders.forEach(function (order) {
            order.customer_name = "";
            if (order.partner_id) {
                order.partner_name = self.pos.db.get_partner_by_id(order.partner_id).name;
            }
        });
        ...
        if (term) {
            this.filtered_open_orders = utils.full_search(
                this.filtered_open_orders,
                term,
                ['card_name', 'uid', 'amount_total', 'partner_name']
            );
        }
