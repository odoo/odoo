...
patch(ImageSelector.prototype, 'image_selector_unsplash', {
    ...
    async selectRecord(media) {
        await this.props.selectMedia({ ...media, mediaType: 'unsplashRecord', query: this.state.needle }, { multiSelect: this.props.multiSelect });
    },
        ...
        return {
            ...
            async uploadUnsplashRecords(records, { resModel, resId }, onUploaded) {
                ...
                const file = service.addFile({
                    id: service.fileId,
                    name: records.length > 1 ? env._t(`Uploading ${records.length} '${records[0].query}' images.`) : env._t(`Uploading '${records[0].query}' image.`),
                    size: null,
                    progress: 0,
                });
