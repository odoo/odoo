...
class EventEvent(models.Model):
    ...
    def _get_event_resource_urls(self, slot=False):
        ...
        ical_params = params
        if slot:
            ical_params.update(slot_id=slot.id)
        google_url = GOOGLE_CALENDAR_URL + werkzeug.urls.url_encode(params)
        iCal_url = f'/event/{self.id:d}/ics?{werkzeug.urls.url_encode(ical_params)}'
