...
class WebsiteEventController(http.Controller):
    ...
    def events(self, page=1, slug_tags=None, **searches):
        if (slug_tags or searches.get('tags')) and request.httprequest.method == 'GET' and not searches.get('prevent_redirect'):
    ...
    def _channel_search_tags_ids(self, search_tags):
        """ Input: %5B4%5D """
        ChannelTag = request.env['event.tag']
        try:
    ...
    def _extract_searched_event_tags(self, searches, slug_tags):
        ...
        if slug_tags:
            ...
            tags = self._channel_search_tags_ids(searches['tags'])
        return tags
