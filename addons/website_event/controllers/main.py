...
class WebsiteEventController(http.Controller):
    ...
    def events(self, page=1, slug_tags=None, **searches):
        ...
        item_list = JsonLd("ItemList")
        item_list_elements = []
        for idx, event in enumerate(events):
            item_list_element = JsonLd("ListItem", position=idx + 1)
        ...
        values['event_list_json_ld'] = JsonLd.render_json(item_list)
    ...
    def event_register(self, event, **post):
        ...
        values['event_json_ld'] = JsonLd.render_structured_data_list([event_schema, breadcrumb_list_schema])
