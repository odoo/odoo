...
export class WebsiteSaleService {
     ...
     * @param {Boolean} [isConfigured=false] - Whether the product is already configured. Defaults
     *      to false.
     *
     * @returns {void}
     */
     ...
     * @returns {void}
     */
    _openComboConfigurator(productTemplateId, productId, combos, remainingData, rest) {
     ...
     * @param {...*} [product.rest] - Locally unused data sent to the controllers.
     *
     * @returns {Object} - The data sent back by the server.
     */
    async _addToCart({
        productTemplateId,
        productId,
        ...
        if (isBuyNow || session.add_to_cart_action === 'go_to_cart') {
