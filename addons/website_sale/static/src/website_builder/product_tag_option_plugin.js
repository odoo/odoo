...
export class ProductTagColorAction extends BuilderAction {
    ...
    getValue({ editingElement: el }) {
        const tagElement = el.querySelector("[data-oe-model='product.tag']");
        return tagElement?.style.color;
    }

    async apply({ editingElement: el, value }) {
        const tagElement = el.querySelector("[data-oe-model='product.tag']");
        const tagId = parseInt(tagElement.dataset.oeId);
        await rpc("/shop/config/tag", { tag_id: tagId, color: value });
...
export class ProductTagImageAction extends BuilderAction {
    ...
    async apply({ editingElement: el, loadResult }) {
        ...
        this.setTagImage(el, tagId, imgEl.src);

        await rpc("/shop/config/tag", { tag_id: tagId, image: selectedMedia[0]["id"] });
    }

    setTagImage(editingElement, tagId, imageUrl) {
