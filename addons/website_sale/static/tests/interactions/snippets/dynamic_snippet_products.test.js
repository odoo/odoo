...
test.tags("desktop");
test("dynamic snippet products grid loads items and displays them through grid template", async () => {
    document.documentElement.dataset.mainObject = "product.public.category(2,)";
    onRpc("/website/snippet/filters", async (args) => {
        const json = JSON.parse(new TextDecoder().decode(await args.arrayBuffer()));
        expect(json.params.filter_id).toBe(3);
        ...
        expect(json.params.limit).toBe(6);
        expect(json.params.search_domain).toEqual([["public_categ_ids", "child_of", 2]]);
        return [
            `<div class="s_test_item" data-test-param="test">Some test record</div>`,
            `<div class="s_test_item" data-test-param="test2">Another test record</div>`,
            `<div class="s_test_item" data-test-param="test3">Yet another test record</div>`,
        ];
    });
    const { core } = await startInteractions(`
        <div id="wrapwrap">
            <section
                class="s_dynamic_snippet_products s_dynamic_snippet_products_grid s_dynamic pt32 pb32 o_colored_level s_product_product_borderless_1"
                data-snippet="s_dynamic_snippet_products_grid"
    ...
    `);
    expect(core.interactions).toHaveLength(4);
    expect(".dynamic_snippet_template .carousel").toHaveCount(0);
    const gridEl = queryOne(".dynamic_snippet_template .s_dynamic_snippet_grid_container");
    expect(gridEl).toHaveAttribute("data-grid-columns", "3");
    expect(gridEl).toHaveAttribute("data-mobile-columns", "2");
    const itemEls = queryAll(".s_test_item");
    expect(itemEls[0]).toHaveAttribute("data-test-param", "test");
    expect(itemEls[1]).toHaveAttribute("data-test-param", "test2");
    expect(itemEls[2]).toHaveAttribute("data-test-param", "test3");
    expect(itemEls[0].closest(".s_dynamic_snippet_grid_container")).toBe(gridEl);
    expect(itemEls[1].closest(".s_dynamic_snippet_grid_container")).toBe(gridEl);
    expect(itemEls[2].closest(".s_dynamic_snippet_grid_container")).toBe(gridEl);
    // Make sure element interactions are started.
    expect(itemEls[0]).toHaveAttribute("data-started", "*test*");
    expect(itemEls[1]).toHaveAttribute("data-started", "*test2*");
    expect(itemEls[2]).toHaveAttribute("data-started", "*test3*");
