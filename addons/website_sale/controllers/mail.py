...
class ShopPortalChatter(PortalChatter):
    ...
    def portal_chatter_post(self, res_model, res_id, message, redirect=None, attachment_ids='', attachment_tokens='', **kw):
        ...
        if res_model == 'product.template' and res_id:
            product = request.env['product.template'].browse(int(res_id)).exists()
        if not product:
            return super(ShopPortalChatter, self).portal_chatter_post(
            ...
            url = super(ShopPortalChatter, self).portal_chatter_post(
                res_model, res_id, message,
                redirect=redirect,
                attachment_ids='',
                attachment_tokens='',
                **kw)
            last_message = product.message_ids[-1]
            if last_message:
