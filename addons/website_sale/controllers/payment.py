...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    def shop_payment_transaction(self, order_id, access_token, **kwargs):
        ...
        if not kwargs.get('amount'):
            kwargs['amount'] = order_sudo._get_amount_need_pay()['amount_total']

        compare_amounts = order_sudo.currency_id.compare_amounts
        if compare_amounts(kwargs['amount'], order_sudo._get_amount_need_pay()['amount_total']):
            raise ValidationError(_("The cart has been updated. Please refresh the page."))
        if compare_amounts(order_sudo.amount_paid, order_sudo._get_amount_need_pay()['amount_total']) == 0:
