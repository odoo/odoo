...
class WebsiteSale(http.Controller):
    ...
    def get_attribute_value_ids(self, product):
        ...
        if request.website.pricelist_id.id != request.env.context.get('pricelist'):
            to_currency = self.get_pricelist().currency_id
            for p in product.product_variant_ids:
                price = request.website.currency_id.compute(p.lst_price, to_currency)
                attribute_value_ids.append([p.id, p.attribute_value_ids.filtered(lambda val: val.attribute_id.id in visible_attrs_ids).ids, p.price, price])
        else:
            attribute_value_ids = [[p.id, p.attribute_value_ids.filtered(lambda val: val.attribute_id.id in visible_attrs_ids).ids, p.price, p.lst_price]
                for p in product.product_variant_ids]
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        attributes_ids = list(set([v[0] for v in attrib_values]))
        ...
        if not local_context.get('pricelist'):
            ...
            local_context['pricelist'] = pricelist.id
        else:
        ...
        if search:
            post["search"] = search
        if category:
            category = category.with_context(local_context)
        ...
        products = Product.with_context(local_context).search(domain, limit=ppg, offset=pager['offset'], order='website_published desc, website_sequence desc')
        ...
        compute_currency = lambda price: request.env.user.company_id.currency_id.compute(price, pricelist.currency_id)
    ...
    def product(self, product, category='', search='', **kwargs):
        local_context = dict(request.env.context, active_id=product.id)
        ProdcutCategory = request.env['product.public.category']
        Rating = request.env['rating.rating']

        if category:
            category = ProdcutCategory.with_context(local_context).browse(int(category)).exists()
        ...
        categs = ProdcutCategory.with_context(local_context).search([('parent_id', '=', False)])
        ...
        if not request.env.context.get('pricelist'):
            local_context['pricelist'] = pricelist.id
            product = product.with_context(local_context)
    ...
    def pricelist(self, promo, **post):
        ...
        if not request.website.is_pricelist_available(pricelist.id):
    ...
    def cart(self, **post):
        ...
        if order:
            compute_currency = lambda price: order.company_id.currency_id.compute(price, order.pricelist_id.currency_id)
        else:
    ...
    def checkout_values(self, data=None):
        ...
        shippings = request.env['res.partner'].with_context(show_address=1).sudo()
        checkout = {}
        if not data:
        ...
        if shipping_id == partner.id:
            ...
            shippings += request.env['res.partner'].with_context(show_address=1).sudo().browse(shipping_id)
        elif shipping_id is None and shippings:
            shipping_id = shippings[0].id

        if shipping_id > 0:
            checkout.update(self.checkout_parse("shipping", request.env['res.partner'].with_context(show_address=1).sudo().browse(shipping_id)))
    ...
    def payment_transaction(self, acquirer_id):
        ...
        # update quotation
        order.write({'payment_acquirer_id': acquirer_id, 'payment_tx_id': request.session['sale_transaction_id']})
    ...
    def payment_get_status(self, sale_order_id, **post):
        ...
        if not order:
            ...
            tx = request.env['payment.transaction'].sudo().search(['|', ('sale_order_id', '=', order.id), ('reference', '=', order.name)], limit=1)
    ...
    def payment_confirmation(self, **post):
        ...
        if sale_order_id:
            return request.website.render("website_sale.confirmation", {'order': request.env['sale.order'].sudo().browse(sale_order_id)})
        else:
            return request.redirect('/shop')
    ...
    def add_product(self, name=None, category=0, **post):
        product = request.env['product.product'].create({'name': name or _("New Product"), 'public_categ_ids': category})
    ...
    def get_unit_price(self, product_ids, add_qty, use_order_pricelist=False, **kw):
        ...
        prices = request.env['product.pricelist'].search([]).price_rule_get_multi([(product, add_qty, partner) for product in products])
