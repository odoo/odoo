...
class TestSnippets(HttpCase):
    ...
    def test_website_category_url(self):
        ...
        self.env['website'].get_current_website().domain = "http://www.example-of-shop.com"
        ...
        with MockRequest(self.env, website=self.env['website'].get_current_website()):
            data = self.env['website.snippet.filter'].sudo()._prepare_category_list_data(parent_id=category.id)
