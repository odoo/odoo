...
class WebsiteSaleGMCCommon(ProductVariantsCommon, WebsiteSaleCommon):
    ...
    def mock_public_request(self, **kwargs):
        website = (
            kwargs.pop('website', self.website)
                .with_context(**kwargs.pop('context', {}))
    ...
    def update_items(self, **request_kwargs):
        with self.mock_public_request(**request_kwargs):
            self.items = (
                self.products
                    .with_context(**request_kwargs.get('context', {}))
                    ._prepare_gmc_items()
            )
