...
class TestVariantAvailability(HttpCase, WebsiteSaleCommon):
    ...
    def _resolve_combination(self, *requested_names, template=None):
        ...
        return set(
            template._get_available_combination(
                combination,
    ...
    def test_completion_untouched_without_requested_values(self):
        """A URL asking for nothing keeps the plain default, sold out or not."""
        with self._patch_availability(self.sofa.product_variant_ids):
            self.assertEqual(self._resolve_combination(), {"Wood", "White", "Drawers"})
