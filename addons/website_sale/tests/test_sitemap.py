...
class TestProductSitemapLastmod(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.related_records = [
            cls.image, cls.attribute_line, cls.attribute_line.product_template_value_ids,
        ]
    ...
    def _lastmod(self):
        ...
        return str(next(
            page["lastmod"].date() for page in self.website._enumerate_pages() if page["loc"] == loc
        ))
    ...
    def test_product_lastmod_from_related_records(self):
        """ An image, an attribute line or an attribute value dates the URL on its own. """
        for records in self.related_records:
            self._set_write_date(records, self.NEW_DATE)
            self.assertEqual(self._lastmod(), self.NEW_DATE[:10])
            self._set_write_date(records, self.OLD_DATE)

    def test_product_lastmod_ignores_variant_cost(self):
        """ Stock valuation rewrites the variant cost on each move, which the page does not show. """
        self.variants[0].standard_price = 42.0
        self.assertEqual(self._lastmod(), self.OLD_DATE[:10])
