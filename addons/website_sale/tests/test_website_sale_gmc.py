...
class TestWebsiteSaleGMCItems(TestWebsiteSaleGMCCommon):
    ...
    def test_gmc_product_prices(self):
        ...
        self.assertEqual('71.1 EUR', self.white_mouse_item['sale_price'], 'Christmas sales')
        self.assertEqual('89.1 EUR', self.black_mouse_item['sale_price'], 'Christmas sales')
        self.assertEqual('129.0 EUR', self.items[self.keyboard]['price'], 'Christmas no reduction.')
        # price in the data source must be equal to those at `link`
        self.start_tour(
            self.white_mouse_item['link'],
