...
class TestWebsiteSaleProductFilters(WebsiteSaleCommon, TestSaleProductAttributeValueCommon):
    ...
    def setUpClass(cls):
        ...
        accessories = cls.computer_case.product_variant_ids + cls.monitor.product_variant_id
        cls.computer.write({
            'company_id': False,
            'website_published': True,
            'accessory_product_ids': accessories.ids,
        })
    ...
    def test_latest_sold_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertCountEqual(
                ...
                computer.ids + (self.pink_case_L + self.pink_case_M + self.black_case_M).ids,
                '"Latest sold" filter should return 4 products without hiding variants',
            )
            self.assertEqual(
                with_variants[0]['product_id'],
                computer.id,
                "Without hiding variants, `computer` should be the most sold product",
            )
    ...
    def test_latest_viewed_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertCountEqual(
                [p['product_id'] for p in with_variants],
                viewed_products.ids,
                'Without hiding variants, "Latest viewed" filter should return viewed variants',
            )
    ...
    def test_recently_sold_with_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertCountEqual(
                ...
                '"Recently sold with" filter should return sold variants when not hiding variants',
            )
    ...
    def test_accessories_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertCountEqual(
                ...
                "Accessories filter should return 16 results when not hiding variants",
            )
            ...
            self.assertCountEqual(
                [p['product_id'] for p in no_variants],
                [self.computer_case.product_variant_id.id, self.monitor.product_variant_id.id],
    ...
    def test_alternative_products_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertCountEqual(
                ...
                "Alternative products filter should return 16 results when not hiding variants",
            )
