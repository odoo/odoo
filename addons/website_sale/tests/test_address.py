...
class TestCheckoutAddress(WebsiteSaleCommon):
    ...
    * address choice (/shop/checkout)
    ...
    def test_10_addresses_updates(self):
        ...
        self.assertTrue(partner_company.can_edit_country())
        self.assertTrue(all(p != p.commercial_partner_id for p in partner_company.child_ids))
        self.assertTrue(all(p.can_edit_country() for p in partner_company.child_ids))
        ...
        self.assertFalse(partner_company.can_edit_country())
        self.assertTrue(all(p.can_edit_country() for p in partner_company.child_ids))
        invoice = self.env['account.move'].create({
            'move_type': 'out_invoice',
        ...
        invoice.action_post()
        self.assertFalse(partner_1.can_edit_country())
