...
class Website(models.Model):
    ...
    def configurator_apply(
        ...
        for footer_id in ThemeUtils._footer_templates:
            footer_view = self.env['website'].viewref(footer_id)
            footer_updated = False
            try:
                arch_tree = etree.fromstring(footer_view.arch_db)
                ...
                if footer_id == 'website_sale.template_footer_website_sale':
                    ...
                    if not ecommerce_categories_node:
                        ...
                        # Logic for inserting eCommerce catagories in footer
                ...
                if footer_updated:
                    footer_view.with_context(website_id=website.id).write(
                        {'arch_db': etree.tostring(arch_tree), 'arch_updated': True},
                    )
        ...
        # For website editor to recognize the correct footer template (reason `isApplied` method of footer plugin)
        Assets.make_scss_customization(
