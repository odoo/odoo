...
class Website(models.Model):
    ...
    def _get_website_specific_extra_step(self):
        website_specific_extra_step = self.env['website.checkout.step'].sudo().search([
            ('website_id', '=', self.id),
            ('step_href', '=', '/shop/extra_info'),
        ], limit=1)

        if not website_specific_extra_step:
            generic_extra_step = self.env.ref('website_sale.checkout_step_extra')
            website_specific_extra_step =  generic_extra_step.copy({'website_id': self.id})

        return website_specific_extra_step
    ...
    def _get_checkout_steps(self):
        ...
        steps = self.env['website.checkout.step'].sudo().search(
            allowed_steps_domain, order='sequence'
        )
        return steps
