...
class Website(models.Model):
    ...
    def _get_checkout_specific_step(self, key):
        step = self.env['website.checkout.step'].sudo().search([
            ('website_id', '=', self.id),
            ('key', '=', key),
        ], limit=1)
        if not step:
            generic_step = self.env['website.checkout.step'].sudo().search([
                ('website_id', '=', False),
                ('key', '=', key),
            ], limit=1)
            step = generic_step.copy({'website_id': self.id}) if generic_step else step
        return step
    ...
    def _get_checkout_steps(self):
        ...
        steps = self.env['website.checkout.step'].sudo().search(allowed_steps_domain, order='sequence')
        return steps
    ...
    def _get_checkout_step_values(self, href):
        ...
        # If the next step is the checkout page, we need to redirect to the login page or the
        # checkout with try_skip_step option which is different from the default href
        ...
        if next_step.step_href == '/shop/checkout':
            ...
            next_href = (
                ('/web/login?redirect=' if redirect_to_sign_in else '')
