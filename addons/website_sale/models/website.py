...
class Website(models.Model):
    ...
    def _get_new_checkout_steps(self, current_step=None):
        # TODO-PDA adapt for steps list
        website_id = self.env.context.get('website_id') or self.env['website'].get_current_website().id

        steps = self.env['website.checkout.step'].get_checkout_steps(website_id=website_id)
        if current_step:
            # TODO-PDA not 2 searchs
            # TODO-PDA quid no next_step (if current = last step or with all same sequence)
            next_step = self.env['website.checkout.step'].search([('sequence', '>', steps.filtered(lambda step: step.xmlid == current_step).sequence), ('is_published','=', True)], limit=1)
            return next_step

        return steps


    def _get_next_step(self, current_step, allowed_steps_domain):
        """ Return the next step in the checkout process. """

        next_step_domain = expression.AND([allowed_steps_domain, [('sequence', '>', current_step.sequence)]])
        next_step = self.env['website.checkout.step'].search(next_step_domain, limit=1)

        return next_step

    def _get_back_step(self, current_step, allowed_steps_domain):
        """ Return the next step in the checkout process. """

        back_step_domain = expression.AND([allowed_steps_domain, [('sequence', '<', current_step.sequence)]])
        back_step = self.env['website.checkout.step'].search(back_step_domain, limit=1)

        return back_step


    def _get_step_specific_values(self, current_step_xmlid):
        """ Return the specific values for the current step. """

        current_step = self.env['website.checkout.step'].search([('xmlid', '=', current_step_xmlid)], limit=1)

        website_id = self.env.context.get('website_id') or \
            self.env['website'].get_current_website().id
        allowed_steps_domain = ['|',('website_id', '=', website_id), ('website_id', '=', False), ('is_published', '=', True)]

        next_step = self._get_next_step(current_step, allowed_steps_domain)
        back_step = self._get_back_step(current_step, allowed_steps_domain)

        return {
            'name': current_step.name,
            'main_button': next_step.main_button,
            'main_button_href': next_step.main_button_href,
            'back_button': back_step.back_button,
            'back_button_href': back_step.back_button_href,
        }
