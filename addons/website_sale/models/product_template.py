...
class ProductTemplate(models.Model):
    ...
    def _search_get_detail(self, website, order, options):
        ...
        search_fields = ['name', 'default_code', 'product_variant_ids.default_code', 'product_variant_ids.barcode', 'attribute_line_ids.attribute_id.name', 'attribute_line_ids.value_ids.name']
        ...
        mapping = {
            ...
            'rating': {'name': 'rating', 'type': 'text', 'match': False, 'skip_markup': True},
        ...
        if with_price:
            ...
            mapping['detail_strike'] = {'name': 'list_price', 'type': 'text', 'match': True, 'display_currency': options['display_currency']}
        if with_category:
    ...
    def _search_render_results(self, fetch_fields, mapping, icon, limit):
        ...
        for product, data in zip(self, results_data):
            ...
            if not matched_group and term in (product.name or '').lower():
                matched_group = 'title'
            ...
            if not matched_group and with_rating and data['rating']:
                if term in str(data['rating']):
                    matched_group = 'rating'

            if not matched_group and with_barcode:
                barcode = product.barcode or product.product_variant_id.barcode or ''
                if term in barcode.lower():
                    matched_group = 'barcode'
                    data['barcode'] = barcode

            if not matched_group and with_attributes and product.attribute_line_ids:
                attr_blocks = []
                color_blocks = []
                matched_attr = False
                matched_color = False
            ...
            if not matched_group and with_category:
                categs = product.public_categ_ids.filtered(lambda c: not c.website_id or c.website_id == current_website)
