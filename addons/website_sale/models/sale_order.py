...
class website(orm.Model):
    ...
    def sale_get_order(self, cr, uid, ids, force_create=False, code=None, update_pricelist=None, context=None):
        ...
        if not sale_order_id and (force_create or code):
            ...
            salesperson_id = affiliate_id if user_obj.exists(cr, SUPERUSER_ID, affiliate_id, context=context) else False
            for w in self.browse(cr, uid, ids):
                ...
                values.update({'user_id': salesperson_id and salesperson_id or w.salesperson_id.id})
...
class ir_http(orm.AbstractModel):
    ...
    def _dispatch(self):
        affiliate_id = parse_qs(urlparse(request.httprequest.query_string).path).get('affiliate_id')
        if affiliate_id:
            request.session['affiliate_id'] = int(affiliate_id[0])
