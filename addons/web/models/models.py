...
class Base(models.AbstractModel):
    ...
    def search_panel_global_counters(self, values_range, parent_name):
        ...
        :param values_range: dict of the form
            {
                id: { 'id': id, 'count': c, parent_name: parent_id, ... }
                ...
            }
        :param parent_name: string, indicates which key determines the parent
        """
        local_counters = lazymapping(lambda id: values_range[id]['count'])
    ...
    @api.model
    def search_panel_range(self, field_name, **kwargs):
        """
        ...
        if field.type in ['many2many', 'many2one']:
            ...
            # we fetch display_name but does not process it
            field_names['display_name'] = lambda record: None

            records = Comodel.with_context(hierarchical_naming=False).search_read(comodel_domain, field_names.keys())

            values_range = OrderedDict()
            for record in records:
                record_id = record['id']
                record['count'] = local_counters.get(record_id, 0)
        ...
        if field.type == 'selection':
            ...
            for value, label in selection:
                values_range[value] = {
                    'id': value,
                    'display_name': label,
                    'count': local_counters.get(value, 0),
                }
            return values_range
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        :param category_domain: domain generated by categories
                                (this parameter is used in search_panel_range)
        :param comodel_domain: domain of field values (if relational)
                                (this parameter is used in search_panel_range)
        ...
        values_range = self.search_panel_range(field_name, comodel_field_names=field_names, **kwargs)

        if parent_name and not kwargs.get('disable_counters'):
