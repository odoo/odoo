...
class Base(models.AbstractModel):
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        if not (expand or hierarchize or comodel_domain):
            ...
            # Sort domain_image values based on comodel field if field.type is many2one and order is given
            if field.type == 'many2one' and order:
                ids = [x.get('id') for x in values]
                comodel_domain.append(('id', 'in', ids))
                records = Comodel.search(comodel_domain, order=order)
                sorted_values = []
                for record in records:
                    sorted_values.append(next(item for item in values if item["id"] == record.id))
        ...
        comodel_records = Comodel.search_read(comodel_domain, field_names, limit=limit, order=order)
