...
class TestImage(HttpCase):
    ...
    def test_05_web_image_access_token(self):
        ...
        base_result = datetime(2021, 3, 22, 9, 59, 18)
        for i in range(14):
        ...
        with freeze_time(datetime(2021, 3, 1, 1, 2, 3)):
            ...
            self.assertEqual(
                get_datetime_from_record_field(self.env["ir.attachment"].browse(2), "raw"),
                datetime(2021, 3, 22, 9, 59, 18),
            )
            # a different record generates a different token
            self.assertEqual(
                get_datetime_from_record_field(self.env["ir.attachment"].browse(3), "raw"),
                datetime(2021, 3, 20, 7, 0, 17),
            )
            # a different field generates a different token
            self.assertEqual(
                get_datetime_from_record_field(self.env["ir.attachment"].browse(3), "datas"),
                datetime(2021, 3, 22, 19, 54, 26),
            )
            # a different model generates a different token
            self.assertEqual(
                get_datetime_from_record_field(self.env["res.partner"].browse(3), "raw"),
                datetime(2021, 3, 20, 18, 48, 18),
            )
