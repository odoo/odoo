    ...
    def test_01_click_everywhere_as_admin(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                ...
                self.browser_js("/web", "odoo.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "odoo.isReady === true", login="admin", timeout=600)
...
class TestMenusDemo(odoo.tests.HttpCase):
    ...
    def test_01_click_everywhere_as_demo(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                _logger.runbot('Testing %s', menus[app_id]['name'])
                self.browser_js("/web", "odoo.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "odoo.isReady === true", login="demo", timeout=600)
