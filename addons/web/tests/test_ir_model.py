...
class TestIrModel(TransactionCase):
    ...
    def test_group_expansion(self):
        ...
        groups = self.env['x_bananas'].web_read_group(domain=[],
                                                  groupby=['x_ripeness_id'],
                                                  aggregates=['__count'])['groups']
        expected = [{
            'x_ripeness_id': self.ripeness_green,
            '__count': 3,
            '__domain_part': [('x_ripeness_id', '=', self.ripeness_green[0])],
        }, {
            'x_ripeness_id': self.ripeness_okay,
            '__count': 0,
            '__domain_part': [('x_ripeness_id', '=', self.ripeness_okay[0])],
        }, {
            'x_ripeness_id': self.ripeness_gone,
            '__count': 0,
            '__domain_part': [('x_ripeness_id', '=', self.ripeness_gone[0])],
        }]
