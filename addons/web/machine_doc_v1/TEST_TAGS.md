...
| `web_http` | HttpCase (url_open, no browser) | 136 tests | ~5 min |
| `web_tour` | HttpCase (start_tour/browser_js) | 7 tests | ~2 min |
| `web_js` | Full JS suites (HOOT) | 37 tests | ~1-2 hr † |
| `addon_js` | HOOT suites of addons with no runner of their own | 96 tests | depends on the DB's module set |
