...
export class MockServer {
    ...
    getDateSortingValue(record, fieldName) {
        // extract every range start (from) related to fieldName
        const values = Object.entries(record.__range)
                             .filter(entry => entry[0].startsWith(fieldName))
                             .map(entry => entry[1] && entry[1].from);
