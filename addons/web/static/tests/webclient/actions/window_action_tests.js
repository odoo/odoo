...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test(
        ...
        <form>
          <header>
            ...
            await doAction(webClient, 3);
            // Sort records
            await click(webClient.el.querySelector(".o_list_view th.o_column_sortable"));
            // Get to the form view of the model, on the first record
            await click(webClient.el.querySelector(".o_data_cell"));
            // Execute another action by clicking on the button within the form
            await click(webClient.el.querySelector('button[name="8"]'));
        }
    );
    ...
    QUnit.test("breadcrumbs are updated when display_name changes", async function (assert) {
        ...
        await editInput(webClient.el, ".o_field_widget[name=display_name] input", "New name");
    ...
    QUnit.test('save when leaving a "dirty" view', async function (assert) {
        ...
        // edit record
        await click(webClient.el.querySelector(".o_control_panel button.o_form_button_edit"));
        await editInput(webClient.el, '.o_field_widget[name="foo"] input', "pinkypie");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await click(webClient.el, ".o_field_many2one .o_form_uri");
            assert.strictEqual(
                webClient.el.querySelector(".o_control_panel .breadcrumb").textContent,
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await click(webClient.el.querySelector(".o_control_panel .o_form_button_edit"));
            assert.containsOnce(webClient, ".o_form_view .o_form_editable");
            await editInput(webClient.el, ".o_field_widget[name=foo] input", "val");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await doAction(webClient, 3, { viewType: "form" });
            assert.containsOnce(webClient, ".o_action_manager .o_form_view .o_form_editable");
            await editInput(webClient.el, ".o_field_widget[name=display_name] input", "TEST");
            // Open dialog without saving should not ask to discard
            await doAction(webClient, 5);
            assert.containsOnce(webClient, ".o_action_manager .o_form_view .o_form_editable");
            assert.containsOnce(webClient, ".o_dialog .o_view_controller");
        }
    );
    ...
    QUnit.skip("window action in target new fails (onchange)", async (assert) => {
        ...
        const webClient = await createWebClient({ serverData, mockRPC });
        await doAction(webClient, 24);
        await click(webClient.el, ".o_form_view button[name='5']");
    ...
    QUnit.test("action and load_views rpcs are cached", async function (assert) {
        ...
        await doAction(webClient, 1);
        assert.containsOnce(webClient, ".o_kanban_view");
        assert.verifySteps(["/web/action/load", "load_views", "web_search_read"]);
