...
const updateComodelRelationalFields = (model, record, originalRecord) => {
    for (const fname in record) {
        ...
        const comodel_inverse_field = coModel._fields[inverseFieldName];
        ...
        if (record[fname]) {
            for (const relatedRecordId of relatedRecordIds) {
                ...
                if (comodel_inverse_field.type === "many2one_reference") {
                    data[comodel_inverse_field.model_name_ref_fname] = model._name;
                }
            ...
            const model_many2one_field =
                comodel_inverse_field.inverse_fname_by_model_name[model._name];
...
export class Model extends Array {
    ...
    create(valuesList, kwargs = {}) {
        ...
        for (const values of valuesList) {
            ...
            updateComodelRelationalFields(this, record);
        }
    ...
    write(idOrIds, values, kwargs = {}) {
        const ids = ensureArray(idOrIds);
        for (const id of ids) {
            const [originalRecord] = this.search_read([["id", "=", id]]);
            this._write(values, id);
            const updatedRecord = this.find((record) => record.id === id);
            updateComodelRelationalFields(this, updatedRecord, originalRecord);
        }
        this._filter([["id", "in", ids]])._applyComputesAndValidate();
        return true;
    }
