...
export class MockServer {
    ...
    async _loadModels() {
        ...
        if (this.modelNamesToFetch.size) {
            serverModels = await fetchModelDefinitions(this.modelNamesToFetch);
            ...
            for (const record of model._records) {
                ...
                for (const [fieldName, fieldDef] of Object.entries(model._fields)) {
                    ...
                    if ("default" in fieldDef) {
                        ...
                        record[fieldName] ??=
                            typeof def === "function" ? def.call(model, record) : def;
                    }
        ...
        // Third iteration: validate records once all default records have been created.
        // This is needed because relational fields validation depends on actual records.
        for (const model of models) {
