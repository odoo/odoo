...
beforeEach(() => {
    ...
    patchWithCleanup(window, { innerHeight: CONTAINER_HEIGHT, innerWidth: CONTAINER_WIDTH });
});
...
test("updates on resize", async () => {
    ...
    patchWithCleanup(window, {
        innerHeight: CONTAINER_HEIGHT / 2,
        innerWidth: CONTAINER_WIDTH / 2,
    });
    resize(window);
    await animationFrame();
...
test("[api] required params only", async () => {
...
test("[api] with empty rows and columns", async () => {
    class C extends Component {
        static template = xml`
...
test("[api] with 1 row and 1 column", async () => {
    class C extends Component {
        static template = xml`
            <div t-ref="pseudoScrollable"/>
        `;
...
test("[api] with columns only", async () => {
    class C extends Component {
        static template = xml`
            <div t-ref="pseudoScrollable"/>
        `;
...
test("[api] onChange", async () => {
    ...
    patchWithCleanup(window, {
        innerHeight: CONTAINER_HEIGHT / 2 + Number.EPSILON,
        innerWidth: CONTAINER_WIDTH / 2 + Number.EPSILON,
    });
    resize(window);
    await animationFrame();
    expect([]).toVerifySteps();
    // it can also receive the changed indexes of a single direction
    patchWithCleanup(window, { innerWidth: CONTAINER_WIDTH * 2 });
    resize(window);
    await animationFrame();
    ...
    await animationFrame(); // not needed, but just to be sure
    expect([]).toVerifySteps();
