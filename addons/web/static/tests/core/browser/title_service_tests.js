...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Infinite Laundry" });
    assert.strictEqual(env.services.title.current, "Infinite Laundry");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Infinite Laundry", chat: null });
    assert.strictEqual(env.services.title.current, "Infinite Laundry");
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Infinite Laundry", action: "Import" });
    assert.strictEqual(env.services.title.current, "Odoo - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Infinite Laundry", action: "Import" });
    parts.action = "Export";
