...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Linen Master" });
    assert.strictEqual(env.services.title.current, "Linen Master");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Linen Master", chat: null });
    assert.strictEqual(env.services.title.current, "Linen Master");
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Linen Master", action: "Import" });
    assert.strictEqual(env.services.title.current, "Odoo - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Linen Master", action: "Import" });
    parts.action = "Export";
