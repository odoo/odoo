...
QUnit.module("Components", (hooks) => {
    ...
    QUnit.test("can be rendered", async (assert) => {
        ...
        await mount(Input, target, { env, props: { onChange: () => {} } });
        assert.containsOnce(target, 'input[type="text"]');
        assert.hasClass(target.querySelector("input"), "o_input");
    });

    QUnit.test("can be rendered with a given value", async (assert) => {
        const env = await makeTestEnv();
        await mount(Input, target, { env, props: { value: "foo", onChange: () => {} } });
        assert.strictEqual(target.querySelector("input").value, "foo");
    });

    QUnit.test("can be rendered with a custom class name and id", async (assert) => {
        const env = await makeTestEnv();
        await mount(Input, target, {
            env,
            props: { className: "custom", id: "myId", onChange: () => {} },
        });
        assert.hasClass(target.querySelector("input"), "custom");
        assert.strictEqual(target.querySelector("input").id, "myId");
    });
    ...
    QUnit.test("call onChange prop when some change occurs", async (assert) => {
        ...
        Parent.template = xml`<Input onChange="onChange"/>`;
        Parent.components = { Input };

        await mount(Parent, target, { env });
        assert.containsOnce(target, "input");
        await editInput(target, "input", "foo");
        assert.strictEqual(value, "foo");
        await editInput(target, "input", "bar");
    ...
    QUnit.test("can confirm value by pressing ENTER", async (assert) => {
        ...
        Parent.template = xml`<Input onChange.bind="onChange"/>`;
        Parent.components = { Input };

        await mount(Parent, target, { env });
        assert.containsOnce(target, "input");
        target.querySelector("input").value = "foo";
        await triggerEvent(target, "input", "keydown", { key: "Enter" });
        assert.strictEqual(value, "foo");
    });

    QUnit.test("Do not update the input with props while editing", async (assert) => {
        const env = await makeTestEnv();
        ...
        Parent.template = xml`<Input className="'custom'" onChange.bind="onChange" value="state.value"/>`;
        Parent.components = { Input };
