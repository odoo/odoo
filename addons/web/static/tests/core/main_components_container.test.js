...
test("unmounts erroring main component", async () => {
    ...
    let compA;
    class MainComponentA extends Component {
        static template = xml`<span><t t-if="this.state.shouldThrow" t-esc="this.error"/>MainComponentA</span>`;
        static props = ["*"];
        setup() {
            compA = this;
...
test("unmounts erroring main component: variation", async () => {
    ...
    let compB;
    class MainComponentB extends Component {
        static template = xml`<span><t t-if="this.state.shouldThrow" t-esc="this.error"/>MainComponentB</span>`;
        static props = ["*"];
        setup() {
            compB = this;
            this.state = useState({ shouldThrow: false });
