...
test("performRPC: web_read_group, no group", async function (assert) {
    ...
    expect(result.groups).toEqual([{ __count: 0, __domain_part: [] }]);
});
...
test("performRPC: web_read_group, group by char", async () => {
    ...
    expect(result.groups).toEqual([
        { name: "aaa", __domain_part: [["name", "=", "aaa"]], __count: 1 },
        { name: "ddd", __domain_part: [["name", "=", "ddd"]], __count: 1 },
        { name: "mmm", __domain_part: [["name", "=", "mmm"]], __count: 1 },
        { name: "xxx", __domain_part: [["name", "=", "xxx"]], __count: 1 },
        { name: "zzz", __domain_part: [["name", "=", "zzz"]], __count: 2 },
    ]);
});
...
test("performRPC: web_read_group, group by boolean", async () => {
    ...
    expect(result.groups).toEqual([
        { bool: false, __domain_part: [["bool", "=", false]], __count: 2 },
        { bool: true, __domain_part: [["bool", "=", true]], __count: 4 },
    ]);
});
...
test("performRPC: web_read_group, group by date", async () => {
    ...
    expect(result.map((x) => x.__domain_part)).toEqual([
        [
            ["date", ">=", "2016-04-01"],
            ["date", "<", "2016-05-01"],
        ],
...
test("performRPC: web_read_group, group by date with number granularity", async () => {
    ...
    for (const { granularity, result, count } of allGranularity) {
        ...
        expect(response.map((x) => x.__domain_part)).toEqual(
...
test("performRPC: web_read_group, group by datetime with number granularity", async () => {
    ...
    for (const { granularity, result, count } of allGranularity) {
        ...
        expect(response.groups.map((x) => x.__domain_part)).toEqual(
    ...
    expect(response).toEqual([
        {
            ...
            __domain_part: [["partner_ids", "=", 1]],
            __count: 2,
        },
        {
            partner_ids: [2, "Raoul"],
            __domain_part: [["partner_ids", "=", 2]],
            __count: 2,
        },
        {
            partner_ids: false,
            __domain_part: [["partner_ids", "=", false]],
            __count: 3,
        },
    ]);
});
...
test("performRPC: web_read_group, group by m2o", async () => {
    ...
    expect(result).toEqual([
        {
            partner_id: [2, "Raoul"],
            __domain_part: [["partner_id", "=", 2]],
            __count: 1,
        },
        {
            partner_id: [1, "Jean-Michel"],
            __domain_part: [["partner_id", "=", 1]],
            __count: 2,
        },
        {
            partner_id: false,
...
test("performRPC: web_read_group, group by integer", async () => {
    ...
    expect(result).toEqual([
        ...
        {
            __domain_part: [["foo", "=", 12]],
            foo: 12,
            __count: 1,
        },
        {
            __domain_part: [["foo", "=", 17]],
            foo: 17,
            __count: 1,
        },
        {
            __domain_part: [["foo", "=", 42]],
            foo: 42,
            __count: 1,
        },
    ]);
...
test("performRPC: web_read_group, group by selection", async () => {
    ...
    expect(result).toEqual([
        { select: "new", __domain_part: [["select", "=", "new"]], __count: 3 },
        { select: "dev", __domain_part: [["select", "=", "dev"]], __count: 1 },
        { select: "done", __domain_part: [["select", "=", "done"]], __count: 2 },
    ]);
});
...
test("performRPC: web_read_group with special measure specifications", async () => {
    ...
    expect(result).toEqual([
        {
            __count: 2,
            __domain_part: [["bool", "=", false]],
            bool: false,
            'float:sum': 0,
        },
        {
            __count: 4,
            __domain_part: [["bool", "=", true]],
            bool: true,
            'float:sum': 2,
        },
    ]);
