...
import { queryAllContents, queryOne } from "@odoo/hoot-dom";
...
const checkTooltip = (view, { title, lines }, index, datasetIndex = null) => {
    ...
    expect(queryAllContents(`table tbody tr td span.o_label`)).toEqual(lineLabels);
    expect(queryAllContents(`table tbody tr td.o_value`)).toEqual(lineValues);
};
...
test("not use a many2one as a measure by default", async () => {
    ...
    expect(queryAllContents(".o-dropdown .o_menu_item")).toEqual(["Foo", "Revenue", "Count"]);
});
...
test("graph measures should be alphabetically sorted (exception: 'Count' is last)", async () => {
    ...
    expect(queryAllContents(".o-dropdown .o_menu_item")).toEqual([
        "Bouh",
        "Foo",
        "Revenue",
        "Count",
