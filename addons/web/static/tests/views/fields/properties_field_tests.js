...
QUnit.module("Fields", (hooks) => {
    ...
    QUnit.test("properties: access to parent", async function (assert) {
        ...
        assert.ok(document.querySelector(".picker-switch .fa-check"),
            "Should not close the definition popover after selecting a date");
        await click(document, ".picker-switch .fa-check");
        assert.ok(target.querySelector(".o_property_field_popover"),
            "Should not close the definition popover after selecting a date");
        ...
        assert.strictEqual(datetimeLabel.innerText, "My Datetime",
        ...
        assert.strictEqual(propertyValue.innerText, "char value",
    ...
    QUnit.test("properties: selection", async function (assert) {
        ...
        assert.strictEqual(document.activeElement, options[3].querySelector("input"),
            "Should focus the new option");

        await editInput(target, ".o_field_property_selection_option:nth-child(4) input", "New option");

        // Press enter to add a second new option
        await triggerEvent(target, ".o_field_property_selection_option:nth-child(4) input",
            "keydown", { key: "Enter" });
        await nextTick();
        ...
        assert.strictEqual(document.activeElement, options[4].querySelector("input"),
            "Should focus the new option");

        // Up arrow should give the focus to the previous option
        // because the new option is empty and lost focus, it should be removed
        await triggerEvent(target, ".o_field_property_selection_option:nth-child(5) input",
            "keydown", { key: "ArrowUp" });
        ...
        assert.deepEqual(optionValues, ["A", "C", "New option"],
    ...
    QUnit.test("properties: tags", async function (assert) {
        ...
        assert.deepEqual(getVisibleTags(tagsComponent, ".dropdown-item"), ["A", "B", "C"],
        ...
        assert.deepEqual(getVisibleTags(target, ".o_property_field_value .o_tag"), ["B"],
        ...
        await click(target, ".o_property_field_value .o_input_dropdown input");
        assert.deepEqual(getVisibleTags(tagsComponent, ".dropdown-item"), ["A", "C"],
        ...
        assert.deepEqual(getVisibleTags(target, ".o_property_field_value .o_tag"), ["B", "D"],
    ...
    QUnit.test("properties: many2many", async function (assert) {
        ...
        const getSelectedUsers = () => {
            const selectedUsers = target.querySelectorAll(".o_property_field_value .o_tag_badge_text");
