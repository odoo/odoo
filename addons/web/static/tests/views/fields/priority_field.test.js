...
test("PriorityField can write after adding a record -- kanban", async () => {
    ...
    expect(".o_kanban_record [data-icon='star']").toHaveCount(0);

    await click(".o_priority button.o_priority_star[data-icon='star']");
    // wait for web_save
    await animationFrame();
    expect.verifySteps(['web_save [[1],{"selection":"1"}]']);
    expect(".o_kanban_record [data-icon='star']").toHaveCount(1);
    ...
    expect(".o_kanban_record [data-icon='star']").toHaveCount(2);
});
...
test("PriorityField in editable list view", async () => {
    ...
    expect(
        ".o_data_row:first-child .o_priority button.o_priority_star[data-icon='star']"
    ).toHaveCount(1, {
    ...
    expect(
        ".o_data_row:last-child .o_priority button.o_priority_star[data-icon='star']"
    ).toHaveCount(2, {
...
test("PriorityField hover in editable list view", async () => {
    ...
    // hover last star
    const star =
        ".o_data_row:first-child .o_priority button.o_priority_star[data-icon='star']:last";
    await hover(star);
    await animationFrame();
...
test("PriorityField with readonly attribute", async () => {
    ...
    expect("span.o_priority_star[data-icon='star']").toHaveCount(2, {
...
test('PriorityField edited by the smart action "Set priority..."', async () => {
    ...
    expect("button[data-icon='star']").toHaveCount(1);
    ...
    await animationFrame();
    expect("button[data-icon='star']").toHaveCount(2);
});
