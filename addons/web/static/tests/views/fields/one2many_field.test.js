...
test("use the limit attribute in arch (in field o2m inline tree view)", async () => {
...
test("one2many (who contains name) with tree view and without form view", async () => {
...
test("column widths are kept when adding first record in o2m", async () => {
    await mountView({
        type: "form",
        resModel: "partner",
        arch: `
    ...
    const width = queryOne('th[data-name="date"]').offsetWidth;

    await contains(".o_field_x2many_list_row_add a").click();

    expect(".o_data_row").toHaveCount(1);
    expect('th[data-name="date"]').toHaveProperty("offsetWidth", width);
});

test("column widths are kept when editing a record in o2m", async () => {
    Partner._records[0].p = [2];
    ...
    const width = queryOne('th[data-name="date"]').style.width;

    await contains(".o_data_row .o_data_cell").click();
    expect(queryOne('th[data-name="date"]').style.width).toBe(width);
...
test("column widths are kept when remove last record in o2m", async () => {
    Partner._records[0].p = [2];
...
test("column widths are correct after toggling optional fields", async () => {
    Partner._records[0].p = [2];
...
test("prevent the dialog in readonly x2many tree view with option no_open True", async () => {
    await mountView({
        type: "form",
        resModel: "partner",
        arch: `
