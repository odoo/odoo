...
test("use incorrect formula", async () => {
    ...
    await contains(".o_field_widget[name=float_field] input").edit("=eee", { confirm: false });
    await clickSave();
    ...
    await contains(".o_field_widget[name=float_field] input").edit("=3*eee", { confirm: false });
    await clickSave();
