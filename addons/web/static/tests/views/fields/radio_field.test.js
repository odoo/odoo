...
test("two radio field with same selection", async () => {
    ...
    // click on Red
    await click(queryFirst("[name='color_2'] label"));
    expect("[name='color'] input.o_radio_input:checked").toHaveAttribute("data-value", "black");
...
test("radio field has o_horizontal or o_vertical class", async () => {
    ...
    const rectT = queryRect(".o_radio_item:first", { root: verticalRadio });
    const rectB = queryRect(".o_radio_item:last", { root: verticalRadio });
    ...
    const rectL = queryRect(".o_radio_item:first", { root: horizontalRadio });
    const rectR = queryRect(".o_radio_item:last", { root: horizontalRadio });
