...
test("BadgeSelectionField widget in list with the color_field option", async () => {
    ...
    expect(`.o_field_selection_badge[name="product_id"]`).toHaveCount(2);
    ...
    expect(`span.btn-secondary.badge.border:contains(xphone)`).toHaveCount(1);
    expect(`span.btn-secondary.badge:contains(xpad)`).toHaveCount(1);
    // The 'border' class should be set only on the current product
    expect(`span.btn-secondary.badge.border:contains(xpad)`).toHaveCount(0);

    // Select the second product.
    await contains(`span.btn-secondary:contains(xpad)`).click();

    expect(`span.btn-secondary.badge.border:contains(xpad)`).toHaveCount(1);
    expect(`span.btn-secondary.badge:contains(xphone)`).toHaveCount(1);
    // The 'border' class should be set only on the new product
    expect(`span.btn-secondary.badge.border:contains(xphone)`).toHaveCount(0);
...
test("BadgeSelectionField widget in list without the color_field option", async () => {
    ...
    expect(`div.o_field_selection_badge span:contains(xphone)`).toHaveCount(1);
    expect(`div.o_field_selection_badge span:contains(xpad)`).toHaveCount(0);
    // Ensure that the 'badge' display is not used.
    expect(`.o_field_selection_badge[name="product_id"] .badge`).toHaveCount(0);
    expect(`.o_field_selection_badge[name="product_id"] .badge`).toHaveCount(0);
    ...
    // Ensure that the 'badge' display is not used.
    expect("span.btn-secondary.badge").toHaveCount(0);
    expect("span.btn-secondary.badge").toHaveCount(0);
