...
class Partner extends models.Model {
    ...
    name = fields.Char({ string: "Name", default: "My little Name Value", searchable: true, trim: true });
    int_field = fields.Integer({ string: "int_field", sortable: true, searchable: true });
    partner_ids = fields.One2many({ string: "one2many field", relation: "res.partner", searchable: true });
...
test("char field with size attribute", async () => {
    Partner._fields.name = fields.Char({ string: "Name", default: "My little Name Value", searchable: true, trim: true, size: 5});
    await mountView({ type: "form", resModel: "res.partner", resId: 1 });
...
test`desktop`("char field in editable list view", async () => {
    await mountView({
        type: "list",
...
test("char field translatable", async () => {
    ...
    onRpc(async (route, params) => {
        ...
        if (route === "/web/dataset/call_kw/res.partner/update_field_translations") {
            expect(params.args[2]).toEqual({ en_US: "bar", es_ES: false }, {
                message: "the new translation value should be written and the value false voids the translation"
            } );
...
test("html field translatable", async () => {
    ...
    onRpc(async (route, params) => {
        ...
        if (route === "/web/dataset/call_kw/res.partner/update_field_translations") {
            expect(params.args[2]).toEqual({ en_US: { "first paragraph": "first paragraph modified" } }, {
    ...
    expect(".o_field_char input[type='text']").toHaveValue("<p>first paragraph</p><p>second paragraph</p>", {
...
test`desktop`("input field: change value before pending onchange returns", async () => {
...
test("input field: change value before pending onchange returns (2)", async () => {
    Partner._fields.int_field = fields.Integer({ string: "Foo2", trim: false, onChange: (obj) => {
        if (obj.int_field === 7) {
            obj.name = "blabla";
        } else {
            obj.name = "tralala";
...
test`desktop`("input field: change value before pending onchange returns (with fieldDebounce)", async () => {
    ...
    Partner._fields.product_id = fields.Many2one({ string: "Product", relation: "product", onChange: (obj) => {
        obj.int_field = obj.product_id ? 7 : false;
    }});
    let def;

    await mountView({ type: "form", resModel: "res.partner", arch: `
        <form>
            <field name="partner_ids">
                <tree editable="bottom">
                    <field name="product_id"/>
...
test("onchange return value before editing input", async () => {
    Partner._fields.name = fields.Char({ string: "Name", default: "My little Name Value", trim: true, onChange: (obj) => {
        obj.name = "yop";
    }});
    await mountView({ type: "form", resModel: "res.partner", resId: 1 });
    expect(".o_field_widget[name='name'] input").toHaveValue("yop");
...
test`desktop`("input field: change value before pending onchange renaming", async () => {
    Partner._fields.product_id = fields.Many2one({ string: "Product", relation: "product", onChange: (obj) => {
...
test`desktop`("input field: set and remove value, then wait for onchange", async () => {
    Partner._fields.product_id = fields.Many2one({ string: "Product", relation: "product", onChange: (obj) => {
...
test("char field: correct value is used to evaluate the modifiers", async () => {
    ...
    Partner._fields.name = fields.Char({ string: "Name", onChange: (obj) => {
        if (obj.name === "a") {
...
test("edit a char field should display the status indicator buttons without flickering", async () => {
    ...
    Partner._fields.name = fields.Char({ string: "Name", onChange: (obj) => {obj.display_name = "cc";}});
