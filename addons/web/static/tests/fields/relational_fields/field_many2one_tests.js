...
QUnit.module('fields', {}, function () {
    ...
    QUnit.module('relational_fields', {
        ...
        QUnit.test('context in many2one and default get', async function (assert) {
            ...
            // AAB FIXME 2: check this one
            // there is a context on the many2one, used in nameget, but nameget isn't called anymore
            // here I added option 'always_reload' to force the nameget calls -> use an existing
            // mechanism instead of guessing (according to the context) whether the nameget should be
            // done or not (note that the context could be define for the name_search or name_create,
            // so it would be a shame to do a nameget for nothing)
                ...
                mockRPC: function (route, args) {
                    if (args.method === 'create') {
                        assert.deepEqual(args.args[0], {
                            ...
                            timmy: [
                                [6, false, []],
                                // AAB FIXME 1: we can't distinguish a value coming from a default_get
