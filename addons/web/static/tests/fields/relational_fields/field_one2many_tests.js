...
QUnit.module('fields', {}, function () {
    ...
    QUnit.module('relational_fields', {
        ...
        QUnit.test('one2many kanban: edition', async function (assert) {
            ...
            // AAB FIXME 3:
            // for 'default_get', we only sent fields in view, i.e. display_name and foo, not color
            // for 'onchange', we send (like before) all known fields in the spec, so in this
            // particular case, we get a value for 'color' whereas we didn't before. I think it is
            // ok, but this is a change of behavior.
        ...
        QUnit.test('one2many list with inline form view', async function (assert) {
            ...
            var form = await createView({
                ...
                mockRPC: function (route, args) {
                    if (args.method === 'write') {
                        // AAB FIXME 3: with default_get, we only sent the fields in the (form) view,
                        // so we didn't get a value for "foo". With the onchange, we always put all
                        // fields we know in the spec (including "foo"), so we now get the default
                        // value for "foo". As LPE said, this seems ok (and actually, I don't know
                        // how we could prevent this), but this is a change of behavior
        ...
        QUnit.skip('one2many with several pages, onchange and default order', async function (assert) {
            ...
            assert.verifySteps([
                ...
                'onchange',
                'read [1]', // AAB FIXME 4: this test's purpose is to assert that this rpc isn't
                            ...
                            // is took into account but the basicmodel doesn't care it concerns a
