...
QUnit.module("Fields", (hooks) => {
    ...
    QUnit.skip("AceEditorField on text fields works", async function (assert) {
        assert.expect(2);
        ...
        testUtils
            .createView({
                ...
                model: "partner",
                data: this.data,
                arch: '<form string="Partners">' + '<field name="foo" widget="ace"/>' + "</form>",
                res_id: 1,
            })
            .then(function (form) {
                assert.ok("ace" in window, "the ace library should be loaded");
                assert.ok(
                    form.$("div.ace_content").length,
