    ...
    QUnit.module(
        "widgets",
        {
            beforeEach: function () {
                ...
                this.mockSession = {
                    async user_has_group(g) {
                        return g === "base.group_allow_export";
                    },
                };
        ...
        function () {
            ...
            QUnit.test("exporting all data in list view", async function (assert) {
                ...
                assert.strictEqual(
                    $("div.o_tree_column:contains(Activities)").length,
                    1,
                    "the Activities field should be in the list of exportable fields"
                );
                assert.strictEqual(
                    $(".modal .o_export_field").length,
                    1,
                    "There should be only one export field"
                );
                assert.strictEqual(
                    $(".modal .o_export_field").data("field_id"),
                    "foo",
                    "There should be only one export field"
                );
                ...
                await testUtils.dom.click($(".modal button:contains(Export)"));
                await testUtils.dom.click($(".modal button:contains(Close)"));
                list.destroy();
                framework.blockUI = blockUI;
                framework.unblockUI = unblockUI;
                assert.verifySteps(["block UI", "/web/export/csv", "unblock UI"]);
            });
            ...
            QUnit.test("saving fields list when exporting data", async function (assert) {
                ...
                await testUtils.dom.click(
                    $(".modal .o_export_tree_item:contains(Activities) .o_add_field")
                );
                assert.strictEqual(
                    $(".modal .o_fields_list .o_export_field").length,
                    2,
                    "there should be two items in the fields list"
                );
                // Save as template
                await testUtils.fields.editAndTrigger(
                    $(".modal .o_exported_lists_select"),
                    "new_template",
                    ["change"]
                );
                await testUtils.fields.editInput(
                    $(".modal .o_save_list .o_save_list_name"),
                    "fields list"
                );
                await testUtils.dom.click($(".modal .o_save_list .o_save_list_btn"));

                assert.verifySteps(["create"], "create should have been called");
            ...
            QUnit.test("Export dialog UI test", async function (assert) {
                ...
                assert.strictEqual(
                    $(".modal .o_export_tree_item:visible").length,
                    2,
                    "There should be only two items visible"
                );
                await testUtils.dom.click($(".modal .o_export_search_input"));
                const searchInput = document.querySelector(".modal .o_export_search_input");
                searchInput.value = "Activities";
                searchInput.dispatchEvent(new Event("input", { keyCode: 65 }));
                assert.strictEqual(
                    $(".modal .o_export_tree_item:visible").length,
                    1,
                    "Only match item visible"
                );
                // Add field
                await testUtils.dom.click($(".modal div:contains(Activities) .o_add_field"));
                assert.strictEqual(
                    $(".modal .o_fields_list li").length,
                    2,
                    "There should be two fields in export field list."
                );
                assert.strictEqual(
                    $(".modal .o_fields_list li:eq(1)").text(),
                    "Activities",
