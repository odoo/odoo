/** @odoo-module alias=@web/../tests/webclient/actions/window_action_tests default=false */
...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("A new form view can be reloaded after a failed one", async function (assert) {
        ...
        await nextTick(); // wait for the update of the router
        assert.strictEqual(browser.location.pathname, "/odoo/action-3/1");
        ...
        await nextTick(); // wait for the update of the router
        assert.strictEqual(browser.location.pathname, "/odoo/action-3/2");
    ...
    QUnit.test("pushState also changes the title of the tab", async (assert) => {
        ...
        assert.strictEqual(titleService.current, '{"action":"Partners"}');
        await click(target.querySelector(".o_data_row .o_data_cell"));
        assert.strictEqual(titleService.current, '{"action":"First record"}');
        await click(target.querySelector(".o_pager_next"));
        assert.strictEqual(titleService.current, '{"action":"Second record"}');
    });
