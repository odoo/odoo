...
QUnit.module("Search", (hooks) => {
    ...
    QUnit.test("search panel is available on list and kanban by default", async (assert) => {
        ...
        assert.containsOnce(webclient, ".o_kanban_view .o_content.o_component_with_search_panel");
        assert.containsOnce(webclient, ".o_content.o_component_with_search_panel .o_search_panel");

        await switchView(webclient, "pivot");

        assert.containsOnce(webclient, ".o_pivot_view .o_content");
        assert.containsNone(webclient, ".o_pivot_view .o_content .o_search_panel");

        await switchView(webclient, "list");
        await legacyExtraNextTick();

        assert.containsOnce(webclient, ".o_list_view .o_content.o_component_with_search_panel");
        assert.containsOnce(webclient, ".o_content.o_component_with_search_panel .o_search_panel");

        await click(webclient.el.querySelector(".o_data_row .o_data_cell"));
        await legacyExtraNextTick();

        assert.containsOnce(webclient, ".o_form_view .o_content");
        assert.containsNone(webclient, ".o_form_view .o_content .o_search_panel");
    });
    ...
    QUnit.test("search panel with view_types attribute", async (assert) => {
        ...
        assert.containsOnce(webclient, ".o_list_view .o_content");
    ...
    QUnit.test('after onExecuteAction, selects "All" as default category value', async (assert) => {
        ...
        await click(webclient.el.querySelector(".o_form_view .o_form_nosheet button"));
    ...
    QUnit.skip("scroll position is kept when switching between controllers", async (assert) => {
        ...
        assert.containsOnce(webclient, ".o_kanban_view .o_content");
