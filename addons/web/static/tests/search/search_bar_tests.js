...
QUnit.module("Search", (hooks) => {
    ...
    QUnit.test(
        "should wait label promises for many2many search defaults (simple value)",
        async function (assert) {
            ...
            await makeWithSearch({
                ...
                `,
                context: { search_default_m2m: "abc" },
            });
            assert.containsOnce(target, ".o_control_panel");
            assert.strictEqual(getFacetTexts(target)[0].replace("\n", ""), "M2Mabc");
        }
    ...
    QUnit.test(
        "should wait label promises for many2many search defaults (array value)",
        async function (assert) {
            serverData.models.partner.fields.m2m = {
                string: "M2M",
                type: "many2many",
