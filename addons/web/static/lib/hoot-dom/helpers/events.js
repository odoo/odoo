...
const triggerFocus = async (target) => {
    ...
    if (previous !== target.ownerDocument.body) {
        ...
        await Promise.resolve().then(() => previous.blur());
        if (!$hasFocus()) {
    ...
    if (isNodeFocusable(target)) {
        ...
        await Promise.resolve().then(() => target.focus());
        if (!$hasFocus()) {
...
export async function dispatch(target, type, eventInit) {
    ...
    await Promise.resolve().then(() => target.dispatchEvent(event));
...
export async function scroll(target, position, options) {
    ...
    await Promise.resolve().then(() => element.scrollTo(scrollOptions));
