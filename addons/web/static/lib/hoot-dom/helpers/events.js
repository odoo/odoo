 ...
 *
 * @typedef {{
 *  delay?: (() => any) | Promise<any> | number | false | null;
 * }} EventOptions
 *
...
const dispatchEventSequence = async (target, eventSequence, eventInit) => {
    for (const eventType of eventSequence) {
        ...
        const event = await dispatch(eventTarget, eventType, eventInit);
...
const setupEvents = (type) => {
    ...
    return async (options) => {
        const delay = options?.delay ?? animationFrame;
        if (delay) {
            switch (typeof delay) {
                case "function": {
                    await delay();
...
export async function drag(target, options) {
    ...
    const cancel = expectIsDragging(
        ...
        async function cancel(options) {
            ...
            dragEvents.push(...(await finalizeEvents({ delay, ...options })));
