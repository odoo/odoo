...
/**
 * @param {keyof import("../core/runner").Runner} varName
 * @param {string} colorClassName
 */
const issueTemplate = (varName, colorClassName) => /* xml */ `
    <t t-foreach="this.runner['${varName}']" t-as="key" t-key="key">
        <t t-set="issue" t-value="this.runner['${varName}'][key]" />
        <div
...
export class HootReporting extends Component {
        ...
        <div class="${HootReporting.name} flex-1 overflow-y-auto">
            ...
            <t t-if="!this.filteredResults().length">
                <div class="flex items-center justify-center h-full">
                    ...
                    <t t-elif="!this.runner.reporting.tests">
                    ...
                    <t t-else="">
                        <div class="flex flex-col gap-3 p-5 rounded bg-gray-200 dark:bg-gray-800">
                            <h3 class="border-b border-gray pb-1">
                                ...
                                /
                                <span class="text-primary" t-out="this.runner.filteredTests().length" />
                                tests completed
                            </h3>
    ...
    setup() {
        const showdetail = this.config.showdetail();

        let didShowDetail = false;
        this.runner.afterPostTest((test) => {
            if (
                showdetail &&
    ...
    computeFilteredResults() {
        ...
        const results = [];
        for (const test of this.runner.finishedTests()) {
            const testResults = test.results();
            let matchFilter = false;
            switch (statusFilter) {
                case "failed": {
                    matchFilter = !test.config.skip && testResults.some((r) => !r.pass);
                    break;
                }
                case "passed": {
                    matchFilter =
                        !test.config.todo && !test.config.skip && testResults.some((r) => r.pass);
                    break;
                }
                case "skipped": {
                ...
                default: {
                    matchFilter = Boolean(selectedSuiteId) || testResults.some((r) => !r.pass);
                    break;
                }
            }
    ...
    getEmptyMessage() {
        ...
        return {
            ...
            selectedSuiteName: selectedSuiteId && this.runner.suites.get(selectedSuiteId).name,
        };
    }
