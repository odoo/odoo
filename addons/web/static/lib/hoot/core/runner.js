...
const { clearTimeout, console, document, Math, performance, Promise, setTimeout, window } =
    globalThis;
...
export class TestRunner {
     ...
     */
    #include(type, ...ids) {
        const values = this.state.includeMap[type];
        for (const id of ids) {
    ...
    #isIncluded(job) {
        ...
        const includeValues = isSuite ? this.state.includeMap.suites : this.state.includeMap.tests;
        if (job.id in includeValues) {
            return includeValues[job.id];
        }

        // Priority 2: excluded if one of the job tags is in the tag "exlude" set
        const tagEntries = Object.entries(this.state.includeMap.tags);
