...
export function makeExpect(params) {
    ...
    function afterTest(options) {
        ...
        if (!(currentResult.counts.assertion + currentResult.counts.query)) {
            currentResult.registerEvent("assertion", {
                label: "assertions",
            ...
            currentResult.expectedAssertions !== currentResult.counts.assertion
        ) {
            currentResult.registerEvent("assertion", {
                label: "assertions",
                message: [
        ...
        if (
            ...
            currentResult.expectedErrors !== currentResult.counts.error
        ) {
            currentResult.registerEvent("assertion", {
                label: "errors",
                message: [
    ...
    function beforeTest(test) {
        ...
        removeInteractionListener = addInteractionListener(["interaction", "query"], onInteraction);
    }
    ...
    constructor(type, [name, args, returnValue]) {
        ...
        for (let i = 0; i < args.length; i++) {
            if (args[i] !== undefined && (i === 0 || typeof args[i] !== "object")) {
                this.messageParts.push(new FormattedString(args[i]));
            }
        }
        if (type !== "interaction" && returnValue) {
