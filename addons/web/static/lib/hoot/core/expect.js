...
const afterTest = (test) => {
    ...
    if (
        ...
        currentResult.expectedErrors !== currentResult.errors.length
    ) {
        registerAssertion(
            new Assertion({
                label: "errors",
                message: `expected ${currentResult.expectedErrors} errors, but ${currentResult.errors.length} were thrown`,
                pass: false,
            })
        );
    }
    ...
    if (test.config.todo) {
        if (currentResult.pass) {
            ...
            currentResult.errors.push(
                new HootError(
                    `tests tagged with "todo" are expected to fail (this means that your test actually passes, which is a good thing!)`
                )
            );
        } else {
...
export class Matchers {
    ...
    toHaveAttribute(attribute, value, options) {
        ...
        return this.#resolve({
            ...
            transform: queryAll,
            predicate: each((node) =>
                expectsValue ? node.getAttribute(attribute) === value : node.hasAttribute(attribute)
            ),
            message: (pass) =>
                options?.message ||
                (pass
            ...
            details: (actual) => {
                const attrValue = actual.getAttribute(attribute);
                const details = [
