...
export class ElementMap extends Map {
     ...
     * @param {{
     *  target: Target;
     *  map?: (element: Element) => T;
     *  predicate?: (value: T, element: Element, map: ElementMap) => boolean;
     * }} [options]
     */
    constructor(target, { map, predicate } = {}) {
        const mapValues = [];
        for (const element of queryAll(target)) {
            mapValues.push([element, map ? map(element) : element]);
        }
        ...
        this.runPredicate = this.runPredicate.bind(this);
    }
    ...
    getFailed(mapFn, not) {
        const result = [];
        let groupIndex = 1;
        for (const [el, value] of this.entries()) {
            const passPredicate = this.predicate(value, el, this);
            const details = Markup.resolveDetails(
                mapFn(value, el, this),
                not ? !passPredicate : passPredicate
            );
            result.push(
                new Markup({
                    content: el,
    ...
    runPredicate(not) {
        ...
        for (const [el, value] of this) {
            const passPredicate = this.predicate(value, el, this);
            if (not ? passPredicate : !passPredicate) {
                return false;
            }
        }
        return true;
...
export class Markup {
     ...
     * @param {boolean} pass
     */
    static resolveDetails(details, pass) {
        const result = [];
