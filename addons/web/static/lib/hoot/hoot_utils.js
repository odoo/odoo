...
export function formatTime(value, unit) {
    ...
    const seconds = (value / 1_000) % 60;
    value -= seconds * 1_000;

    const minutes = (value / 60_000) % 60;
...
export function makeCallbacks() {
    ...
    const addCallback = (type, callback, once) => {
        ...
        if (once) {
            ...
            callback = (...args) => {
                callbackRegistry[type] = callbackRegistry[type].filter((fn) => fn !== callback);
        ...
        if (type.startsWith("after")) {
            callbackRegistry[type].unshift(callback);
        } else {
    ...
    /** @type {Record<string, ((...args: any[]) => MaybePromise<((...args: any[]) => void) | void>)[]>} */
    const callbackRegistry = {};

    return { add: addCallback, call };
}
