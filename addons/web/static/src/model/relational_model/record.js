...
import { htmlJoin, htmlMarkup } from "@web/core/utils/html";
    ...
    _checkValidity({ silent, displayNotification, removeInvalidOnly } = {}) {
        ...
        if (!isValid && displayNotification) {
            const items = [...this._invalidFields].map(
                (fieldName) => htmlMarkup`<li>${this.fields[fieldName].string || fieldName}</li>`,
                this
            );
            this._closeInvalidFieldsNotification = this.model.notification.add(
                htmlMarkup`<ul>${htmlJoin(items)}</ul>`,
                {
            ...
            if (succeeded) {
                ...
                this.model._closeUrgentSaveNotification = this.model.notification.add(
                    _t(
                        ...
                        { icon: htmlMarkup`<i class="fa fa-cloud-upload fa-fw"></i>` }
                    ),
                    { sticky: true }
                );
            }
