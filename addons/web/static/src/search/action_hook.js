    ...
    function setScrollFromState() {
        ...
        if (scrolling) {
            if (component.env.isSmall) {
                params.rootRef.el.scrollTop = (scrolling.root && scrolling.root.top) || 0;
                params.rootRef.el.scrollLeft = (scrolling.root && scrolling.root.left) || 0;
            } else if (scrolling.content) {
                const contentEl =
                    params.rootRef.el.querySelector(
                        ".o_component_with_search_panel > .o_renderer_with_searchpanel," +
                            ".o_component_with_search_panel > .o_renderer"
                    ) || params.rootRef.el.querySelector(".o_content");
                if (contentEl) {
