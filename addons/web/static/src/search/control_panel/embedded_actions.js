...
export class EmbeddedActionsPanel extends Component {
    ...
    setup() {
        ...
        useEffect(() => {
            const embeddedInfos = this.props.state.embeddedInfos;
            if (!this.root() || !embeddedInfos.showEmbedded) {
                return;
            }
            const timer = setTimeout(() => {
                if (
                    embeddedInfos.showEmbedded &&
                    embeddedInfos.visibleEmbeddedActions.length === 1
                ) {
                    this.root().querySelector(".btn[name='openEmbeddedActions']")?.click();
                }
            }, 100);
