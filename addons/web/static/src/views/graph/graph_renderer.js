...
export class GraphRenderer extends Component {
    ...
    getScaleOptions() {
        ...
        const { allIntegers, fields, groupBy, measure, measures, mode, stacked, widgets } =
            this.model.metaData;
        if (mode === "pie") {
            return {};
        }
        ...
        const yAxe = {
            ...
            ticks: {
                callback: (value) => this.formatValue(value, allIntegers, widgets[measure]),
                color:
                    cookie.get("color_scheme") === "dark"
                        ? getColor(15, cookie.get("color_scheme"))
                        : null,
    ...
    getTooltipItems(data, metaData, tooltipModel) {
        ...
        for (const item of sortedDataPoints) {
            ...
            let measureWidget = metaData.widgets[measure];
