...
function useSmoothStickyTop({ scrollableRef, stickyRef }) {
    const baseClass = "o-smooth-sticky-top";
    ...
    const onScroll = () => {
        ...
        el.classList.toggle(visibleClass, delta <= 0);
        el.classList.toggle(invisibleClass, delta > 0);
        oldScrollTop = scrollTop;
        return;
    };
    useListener(scrollableRef, "scroll", onScroll);
    useEffect(() => {
        const sticky = stickyRef();
        if (sticky) {
            sticky.classList.add(baseClass);
        }
    });
}
...
class LanguageButtons extends Component {
    ...
    model = usePlugin(TranslateModel);
    languages = useProps.static(
        "languages",
        types.signal().optional(() => this.model.languages)
    );
    onChange = useProps.static(
        "onChange",
...
export class TranslateXml extends Component {
    ...
    setup() {
        useSmoothStickyTop({ scrollableRef: this.rootRef, stickyRef: this.langButtonsRef });
    }
...
export class TranslationDialog extends Component {
    ...
    setup() {
        ...
        this.model = usePlugin(TranslateModel);
