...
const BLACKLISTED_MENUS = [
    ...
    "account_accountant.menu_action_auto_reconcile",
];
...
function waitForCondition(stopCondition) {
    return new Promise(function (resolve, reject) {
        ...
        function checkCondition(timeLimit) {
            ...
            if (stopCondition() && !hasPendingRPC() && !hasScheduledTask()) {
                resolve();
            } else {
                if (timeLimit > 0) {
                    // recursive call until the resolve or the timeout
                    browser.setTimeout(checkCondition, interval, timeLimit - interval);
                } else {
        ...
        browser.setTimeout(checkCondition, interval, initialTime);
    });
}
...
async function testFilters() {
    ...
    const simpleFilterSel = ".o_control_panel .o_filter_menu > .dropdown-item.o_menu_item:not(.o_add_custom_filter)";
