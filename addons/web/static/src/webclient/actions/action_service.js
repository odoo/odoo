...
export function makeActionManager(env, router = _router) {
  ...
  function _preprocessAction(action, context = {}) {
    ...
    if (action.type === "ir.actions.act_window") {
      // FIX: action.views is undefined for some activity popovers → JS crash
      if (!action.views || !Array.isArray(action.views)) {
        action.views = [[false, "form"]]; // safe fallback
      }
  ...
  function _getViewInfo(view, action, views, props = {}) {
    ...
    const openFormView = (
      resId,
      { activeIds, readonly, force, newWindow } = {}
    ) => {
      if (target !== "new") {
        if (_getView("form")) {
          return switchView(
  ...
  async function _updateUI(controller, options = {}) {
    ...
    controller.config.isReloadingController =
      controller === controllerStack.at(-1);
  ...
  async function _executeActWindowAction(action, options) {
    ...
    if (env.isSmall) {
      view =
        _findView(views, view.multiRecord, action.mobile_view_mode) || view;
    }
