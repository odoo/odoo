                ...
                _.each(events, function (event) {
                    ...
                    _.each(data, function (_value) {
                        ...
                        var f = {
                            ...
                            'value': value,
                            'label': x2many? false: fieldUtils.format[field.type](_value, field) || _t("Undefined"),
            ...
            return $.when.apply($, defs).then(function () {
                _.each(self.data.filters, function (filter) {
                    ...
                    if (filter.filters.length && (filter.filters[0].avatar_model in to_read)) {
                        _.each(filter.filters, function (f) {
                            var new_label = to_read[f.avatar_model][f.value];
                            if (typeof new_label !== typeof undefined && new_label !== false){
