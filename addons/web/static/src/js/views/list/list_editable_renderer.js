...
ListRenderer.include({
    ...
    _onAddRecord: function (ev) {
        ...
        var self = this;
        this.unselectRow().then(function () {
            self.trigger('add_record', {context: ev.currentTarget.dataset.context && [ev.currentTarget.dataset.context]}); // TODO write a test, the deferred was not considered
    ...
    */
    ...
    _onWindowClicked: function (event) {
        ...
        if ($(event.data.$event.target).parents('.ui-autocomplete').length) {
            return;
        }

        // ignore clicks in modals, except if the list is in a modal, and the
        // click is performed in that modal
        var $clickModal = $(event.data.$event.target).closest('.modal');
        ...
        if (!document.contains(event.data.$event.target)) {
            return;
        }

        // ignore clicks if target is inside the list. In that case, they are
        // handled directly by the renderer.
        if (this.el.contains(event.data.$event.target) && this.el !== event.data.$event.target) {
            return;
        }
