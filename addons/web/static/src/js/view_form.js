...
instance.web.FormView = instance.web.View.extend(instance.web.form.FieldManagerMixin, {
    ...
    load_form: function(data) {
        ...
        this.$el.on('click', '.oe_form_field_status:not(.oe_form_status_clickable)', function (e) {
            if((self.get("actual_mode") == "view") || (self.get("actual_mode") == "edit")) {
                var $button = self.$el.find(".oe_highlight:not(.oe_form_invisible)").css({'float':'left','margin-left':'2px','margin-right':'3px','position':'relative','left':'2px','white-space':'nowrap'});
                $button.openerpBounce();
                e.stopPropagation();
            }
         });
