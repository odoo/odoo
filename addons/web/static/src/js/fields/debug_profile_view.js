...
odoo.define('web.debug_profile_view', function (require) {
'use strict';
 ...
 */
const DebugViewProfiling = basicFfields.AceEditor.extend({
    template: "web.DebugViewProfiling",
    ...
    willStart: async function () {
        ...
        for (const view of this.views) {
            ...
            view.query = 0;
            for (const line of this.profileLines) {
                if (line.view_id === view.id) {
                    if (line.xpath === '') {
                        view.delay += line.delay;
                        view.query += line.query;
                    }
                }
            }
        ...
        this.$('.ace_tag-open, .ace_end-tag-close, .ace_end-tag-open, .ace_qweb').each((i, node) => {
            ...
            if ($node.hasClass('ace_end-tag-close')) {
                ...
                if ((delay || query) && !$node.children('.o_info').length) {
                    $(qweb.render('web.DebugViewProfiling.hover', { delay: this._formatDelay(delay), query: query })).prependTo($node);
                }
                ...
                if (delays.length && !$row.children('.o_info').length) {
                    $(qweb.render('web.DebugViewProfiling.info', {
...
registry.add('debug_profile_view', DebugViewProfiling);
