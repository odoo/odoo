import { Plugin, useListener } from "@odoo/owl";
...
export class ErrorPlugin extends Plugin {
    ...
    setup() {
        useListener(window, "error", this.onError.bind(this));
        useListener(window, "unhandledrejection", this.onUnhandledRejection.bind(this));
    }
    ...
    handleError(uncaughtError) {
        ...
        for (const [name, handler] of errorHandlerRegistry.getEntries()) {
            try {
                ...
                if (this.shouldLogError(uncaughtError)) {
