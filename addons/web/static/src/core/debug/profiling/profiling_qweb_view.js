...
odoo.define('web.profiling_qweb_view', function (require) {
'use strict';

const registry = require('web.field_registry');
const basicFfields = require('web.basic_fields');
...
const ProfilingQwebView = basicFfields.AceEditor.extend({
    ...
    init: function () {
        ...
        const results = this.value && JSON.parse(this.value)[0].results || {archs: {}, data: []};
    ...
    _renderProfilingInformation: function () {
        ...
        this.$('.ace_tag-open, .ace_end-tag-close, .ace_end-tag-open, .ace_qweb').each((i, node) => {
            const $node = $(node);
            ...
            if ($node.hasClass('ace_end-tag-close')) {
                ...
                let query = 0;
                for (const line of this.profileLines) {
                    if (line.view_id === this.viewID && line.xpath === xpath && line.directive.includes(directive)) {
                        delay += line.delay;
                        query += line.query;
                    }
                }
                ...
                if ((delay || query) && !$node.children('.o_info').length) {
                    $(qweb.render('web.ProfilingQwebView.hover', { delay: this._formatDelay(delay), query: query })).prependTo($node);
                }
                ...
                for (const line of this.profileLines) {
                    if (line.view_id === this.viewID && (closed ? line.xpath.startsWith(xpath) : line.xpath === xpath)) {
