...
registry.category("services").add("clear_field_registry", {
    start(env) {
 ...
 */
export async function loadFields(resModel, options = {}) {
...
async function _loadPropertyDefinitions(fieldDefs, name, domain = []) {
    const { definition_record: definitionRecord, definition_record_field: definitionRecordField } =
        fieldDefs[name];
 ...
 * @returns {Promise<object[]>}
 */
export async function loadPropertyDefinitions(resModel, fieldName, domain) {
 ...
 * @returns {Promise<Object>}
 */
export async function loadPath(resModel, path = "*") {
    const fieldDefs = await loadFields(resModel);
