 ...
 * @param {Partial<T> & { [P: string | symbol]: any }} extension The object containing the patched properties
 */
 ...
 * @param {new (...args: any[]) => any} klass
 */
function patchClass(klass) {
    ...
    const klassProps = getPropertyMap(klass);
    // Do not override "prototype" property.
    klassProps.delete("prototype");
    if (!isPropUserDefined(klass, "name")) {
        // Override the "name" property only if it is set.
        klassProps.delete("name");
    }
    if (!isPropUserDefined(klass, "length")) {
        // Override the "length" property only if it is set.
        klassProps.delete("length");
    }
    const baseKlass = Reflect.getPrototypeOf(klass);
    applyPatch(baseKlass, klass, klassProps);

    return () => {
        removePatch(baseKlass, klass);
 ...
 *
 * @overload
 * @param {new (...args: any[]) => any} objToPatch The object to patch
 * @returns {() => void} Returns an unpatch function
 */
