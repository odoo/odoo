...
from . import get_absolute_path, defineCommand, get_tooling, CONFIG
...
def make_js_configs(addons_path):
    ...
    jsconfig_file = None
    base_jsconfig = get_tooling().joinpath("_jsconfig.json")
    if base_jsconfig.exists():
        with open(base_jsconfig) as f:
    ...
    for abs_path, repo in repos.items():
        ...
        jsconfig = {
            "extends": str(get_tooling().joinpath("_jsconfig.json")),
            "compilerOptions": {
                "plugins": [
...
def create_js_configs(addons_path):
    for path, jsconfig in make_js_configs(addons_path):
        logger.info(f"Create jsconfig at {path}")
...
def remove_js_configs(addons_path):
    for repo_path, repo_obj in get_repositories(addons_path):
        ...
        if jsconf_file.exists():
            logger.info(f"Removed {jsconf_file}")
            jsconf_file.unlink()
        else:
            logger.info(f"{jsconf_file} doesn't exist.")
