...
def build_search_index(icons: dict[str, dict]) -> dict:
    """Build a compact inverted index for fast prefix search.

    :returns: a dict with keys ``t`` (sorted tokens), ``i`` (token → icon
        index list), ``c`` (compact icon list: ``n`` = name, ``f`` = has_fill).
    """
    # Stable ordering for reproducible output
    icon_order = sorted(icons.keys())

    # Token → set of icon indices
    index: dict[str, set[int]] = {}
    for idx, name in enumerate(icon_order):
        info = icons[name]
        haystack = f"{name} {info.get('tags', '')}".lower()
        for word in haystack.split():
            index.setdefault(word, set()).add(idx)

    # Convert sets to sorted lists for deterministic JSON
    compact_index = {
        token: sorted(idxs) for token, idxs in index.items()
    }
    sorted_tokens = sorted(compact_index.keys())

    compact_icons = [
        {'n': name, 'f': icons[name]['has_fill']}
...
def main() -> None:
    ...
    search_index = build_search_index(icons)
    (ms_dir / 'search_index.json').write_text(
