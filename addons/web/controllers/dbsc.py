...
class DBSCController(Controller):
    ...
    def dbsc_refresh(self, **kw):
        ...
        device_dbsc = request.env['res.device.dbsc'].sudo().search([
        ...
        challenge = session.pop('dbsc_refresh_challenge', None)  # Single use refresh challenge
        jws = request.httprequest.headers.get('Secure-Session-Response')

        if not (jws and challenge):
        ...
        if not device_dbsc._dbsc_refresh(jws, challenge):
            raise Forbidden()
