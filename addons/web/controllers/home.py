...
class Home(http.Controller):
    ...
    def _prepare_change_password_layout_values(self):
        """ This method is meant to be overridden by dependencies to add values in the layout of the
    ...
    def web_change_password(self, **kwargs):
        values = self._prepare_change_password_layout_values()
        if request.httprequest.method == 'POST':
            if kwargs['new_password'] != kwargs['confirm_new_password']:
                values['error'] = _('The new password and its confirmation must be identical.')
            else:
                try:
                    request.env['res.users'].change_password(kwargs['current_password'], kwargs['new_password'])
                    values['message'] = _('Your password has been successfully changed.')
                    ...
                    values['error'] = _('The current password you provided is incorrect. Your password was not changed.')
                except odoo.exceptions.UserError as e:
                    values['error'] = str(e)
        return request.render('web.change_password', values)
