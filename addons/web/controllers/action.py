    ...
    def load(self, action_id, context=None):
        ...
        try:
            ...
            try:
                ...
                raise MissingActionError(request.env._("The action “%s” does not exist.", action_id)) from exc

        base_action = Actions.browse([action_id]).sudo().read(['type'])
        if not base_action:
            raise MissingActionError(request.env._("The action “%s” does not exist", action_id))
            ...
            try:
                if action.get('action'):
                    ...
                    if record_id:
                        ...
                        if record_id == 'new':
                            results.append({'display_name': request.env._("New")})
                        elif act['res_model']:
