...
class WebClient(openerpweb.Controller):
    ...
    def translations(self, req, mods, lang):
        ...
        for mod in messages:
            if not messages[mod]['messages']:
                addons_path = openerpweb.addons_manifest[mod]['addons_path']
                for l in langs:
                    f_name = os.path.join(addons_path, mod, "i18n", l + ".po")
                    ...
                    for x in po:
                        if x.id and x.string and "openerp-web" in x.auto_comments:
                            messages[mod]["messages"].append({'id': x.id, 'string': x.string})
