...
class TestHrCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        # Creating two users and assigning each a group related to Human Resource Management
        ...
        self.job_developer = self.env(user=self.res_users_hr_manager.id).ref('hr.job_developer')
        self.employee_niv = self.env(user=self.res_users_hr_manager.id).ref('hr.employee_niv')
