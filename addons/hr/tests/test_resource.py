...
class TestResource(TestHrCommon):
    ...
    def setUpClass(cls):
        ...
        cls.employee_sharlene = cls.env['hr.employee'].create({
            'name': 'Sharlene Rhodes',
            'departure_date': '2022-06-01',
            'resource_calendar_id': cls.calendar_40h.id,
        })
        cls.employee_sharlene_create_date = '2021-01-01 10:00:00'
        cls.env.cr.execute("UPDATE hr_employee SET create_date=%s WHERE id=%s",
                           (cls.employee_sharlene_create_date, cls.employee_sharlene.id))

    def test_calendars_validity_within_period_default(self):
        calendars = self.employee_sharlene.resource_id._get_calendars_validity_within_period(
        ...
        self.assertEqual(1, len(calendars[self.employee_sharlene.resource_id.id]), "Niv should only have one calendar")
        niv_entry = calendars[self.employee_sharlene.resource_id.id]
