...
export class LocationPicker extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            try {
                const geoPermission = await browser.navigator.permissions.query({
                    name: "geolocation",
                });
    ...
    setupMap() {
        ...
        if (!mapEl || !this.state.shouldLoadMap) {
            return;
        }

        if (typeof L === "undefined") {
            this.notification.add(_t("Map library failed to load. Please refresh the page."), {
                type: "danger",
            });
            return;
        }
    ...
    useCurrentLocation() {
        if (!navigator.geolocation) {
            this.notification.add(_t("Your browser does not support geolocation."), {
                type: "warning",
            });
            return;
        }
    ...
    async onSaveLocation() {
        if (!this.state.selectedLocation) {
            this.notification.add(_t("Please select a location on the map before saving."), {
                type: "warning",
            });
            return;
        }
        ...
        try {
            ...
            if (!navigator.onLine) {
                ...
                errorMessage = _t(
                    "The selected coordinates are not valid. Please choose a different location."
                );
            } else if (
                error instanceof RPCError &&
                ...
                errorMessage = _t(
                    "You do not have permission to modify work locations. Please contact your administrator."
                );
            } else {
                throw error;
            }
