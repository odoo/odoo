...
export class VersionQuickAccess extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            const contractTemplateId = this.props.record.data[this.props.name].id;
            if (!contractTemplateId) return;
            this.globalVersionTemplates = await this.orm.searchRead("hr.version", [["employee_id", "=", false]], ["id"]);
        });
    }
