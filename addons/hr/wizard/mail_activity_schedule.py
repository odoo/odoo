...
class MailActivitySchedule(models.TransientModel):
    ...
    def _get_activity_summary(self, record, template):
        if self.res_model == 'hr.employee' and template.plan_id:
            record.activity_plans_ids |= self.plan_id
            return f"{template.plan_id.name} - {template.summary if template.summary else template.activity_type_id.name}"
        else:
            return super()._get_activity_summary(record, template)
