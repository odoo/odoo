...
class HrEmployee(models.Model):
    ...
    def _default_history_ids(self):
        return self.env['hr.employee.history'].create([{'employee_id': self.id}])
    ...
    history_ids = fields.One2many(
        ...
        copy=False)
    current_history_id = fields.Many2one(
        'hr.employee.history',
        compute='_compute_current_history_id',
        store=True)
    selected_history_id = fields.Many2one(
        'hr.employee.history',
        compute='_compute_selected_history_id',
    ...
    country_id = fields.Many2one(related='current_history_id.country_id', store=True, readonly=False)
    identification_id = fields.Char(related='current_history_id.identification_id', store=True, readonly=False)
    ssnid = fields.Char(related='current_history_id.ssnid', store=True, readonly=False)
    passport_id = fields.Char(related='current_history_id.passport_id', store=True, readonly=False)

    private_street = fields.Char(related='current_history_id.private_street', store=True, readonly=False)
    private_street2 = fields.Char(related='current_history_id.private_street2', store=True, readonly=False)
    private_city = fields.Char(related='current_history_id.private_city', store=True, readonly=False)
    private_state_id = fields.Many2one(related='current_history_id.private_state_id', store=True, readonly=False)
    private_zip = fields.Char(related='current_history_id.private_zip', store=True, readonly=False)
    private_country_id = fields.Many2one(related='current_history_id.private_country_id', store=True, readonly=False)

    distance_home_work = fields.Integer(related='current_history_id.distance_home_work', store=True, readonly=False)
    km_home_work = fields.Integer(related='current_history_id.km_home_work', store=True, readonly=False)
    distance_home_work_unit = fields.Selection(related='current_history_id.distance_home_work_unit',
                                               store=True, readonly=False)

    marital = fields.Selection(related='current_history_id.marital', store=True, readonly=False)
    spouse_complete_name = fields.Char(related='current_history_id.spouse_complete_name', store=True, readonly=False)
    spouse_birthdate = fields.Date(related='current_history_id.spouse_birthdate', store=True, readonly=False)
    children = fields.Integer(related='current_history_id.children', store=True, readonly=False)

    certificate = fields.Selection(related='current_history_id.certificate', store=True, readonly=False)
    study_field = fields.Char(related='current_history_id.study_field', store=True, readonly=False)

    # Work Information
    department_id = fields.Many2one(related='current_history_id.department_id', store=True, readonly=False)
    job_id = fields.Many2one(related='current_history_id.job_id', store=True, readonly=False)
    parent_id = fields.Many2one(related='current_history_id.parent_id', store=True, readonly=False)

    address_id = fields.Many2one(related='current_history_id.address_id', store=True, readonly=False)
    work_location_id = fields.Many2one(related='current_history_id.work_location_id', store=True, readonly=False)

    departure_reason_id = fields.Many2one(related='current_history_id.departure_reason_id', store=True, readonly=False)
    departure_description = fields.Html(related='current_history_id.departure_description', store=True, readonly=False)
    departure_date = fields.Date(related='current_history_id.departure_date', store=True, readonly=False)

    resource_calendar_id = fields.Many2one(related='current_history_id.resource_calendar_id', store=True, readonly=False)
    tz = fields.Selection(related='current_history_id.tz', store=True, readonly=False)
    ...
    def _compute_current_history_id(self):
        ...
        for record in self:
            history = record._get_history(today)
            record.current_history_id = history

    @api.depends('history_ids')
    def _compute_selected_history_id(self):
        today = fields.Date.today()
        for record in self:
            history = record._get_history(today)
            record.selected_history_id = history

    def _get_history(self, date):
        history = self.history_ids.filtered(
            lambda h:
            (not h.date_from and not h.date_to) or
            (not h.date_from and h.date_to >= date) or
            (not h.date_to and h.date_from <= date) or
            (h.date_from and h.date_to and h.date_from <= date <= h.date_to))
        return history[0] if history else False
    ...
    def write(self, vals):
        # next_history_ids = self.env['hr.employee.history'].search([
        #     ('employee_id', '=', self.id),
