...
from odoo.tools import convert, format_time, SQL
...
class HrEmployee(models.Model):
    ...
    _inherits = {'hr.contract': 'version_id'}
    ...
    def _create(self, vals_list):
        ...
        for (employee, version_id) in zip(result, versions):
            self.env['hr.contract'].browse(version_id).employee_id = employee.id
        return result
    ...
    def _compute_version_id(self):
        for employee in self:
            if self._context.get('version_id'):
                version = self.env['hr.contract'].search(
    ...
    def _search_version_id(self, operator, value):
        return [('id', 'in', self.env['hr.contract'].search([('id', operator, value)]).mapped('employee_id.id'))]
    ...
    def _field_to_sql(self, alias, fname, query=None, flush: bool = True) -> SQL:
        ...
        if fname == 'version_id':
            return SQL("""(
                SELECT c.id 
                FROM "hr_contract" c 
                WHERE c.employee_id = "hr_employee"."id"
                ORDER BY c.date_version DESC 
                LIMIT 1
            )""")
    ...
    def create_version(self, date=fields.Date.today()):
        if type(date) is str:
            date = isoparse(date).date()

        version_to_copy = self._get_version(date)
        if not version_to_copy:
            version_to_copy = self.env['hr.contract'].search([('employee_id', '=', self.employee_id)], limit=1)

        if version_to_copy.date_version == date:
            return version_to_copy.id

        return version_to_copy.copy({'date_version': date}).id
    ...
    def _get_calendars(self, date_from=None):
        ...
        if not date_from:
            return res
        contracts = self.env['hr.contract'].sudo().search([('employee_id', 'in', self.ids)]).filtered(
            lambda c: c.date_start <= date_from and (not c.date_end or c.date_end >= date_from))
    ...
    def _get_calendar_periods(self, start, stop):
        ...
        contracts_by_employee = self.env['hr.contract'].sudo()._read_group(
            domain=[('employee_id', 'in', self.ids)], groupby=['employee_id'], aggregates=['id:recordset'])

        for employee, contracts in contracts_by_employee:
            for contract in contracts.filtered(lambda c: c.date_start <= stop.date() and (not c.date_end or c.date_end >= start.date())):
    ...
    @api.model
    def _get_all_contracts(self, date_from, date_to):
        """
    ...
    def _get_unusual_days(self, date_from, date_to=None):
        ...
        employee_contracts = self.env['hr.contract'].sudo().search([('employee_id', '=', self.id)]).filtered(
            lambda c: c.date_start <= date_to_date and (not c.date_end or c.date_end >= date_from_date))
    ...
    def _employee_attendance_intervals(self, start, stop, lunch=False):
        ...
        if not lunch:
            ...
            valid_contracts = self.sudo()._get_contracts(start, stop)
    ...
    def _get_expected_attendances(self, date_from, date_to):
        self.ensure_one()
        valid_contracts = self.sudo()._get_contracts(date_from, date_to)
        employee_tz = timezone(self.tz) if self.tz else None
        if not valid_contracts:
            calendar = self.resource_calendar_id or self.company_id.resource_calendar_id
