            ...
            for date_start, date_end, versions in dates_per_employee[version.employee_id]:
                ...
                if date_start <= contract_date_end and version.contract_date_start <= date_to:
                    raise ValidationError(_(
                        ...
                        overlaps='\n'.join(
                            [f'Version {format_date_abbr(v.env, v.date_version)}: '
                             f'from {format_date_abbr(v.env, v.contract_date_start)} '
                             f'to {format_date_abbr(v.env, v.contract_date_end) if v.contract_date_end else "Indefinite"}'
