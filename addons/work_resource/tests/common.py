...
class TestResourceCommon(TransactionCase):
    ...
    def setUp(self):
        ...
        self.test_context = context = dict(tz='UTC')
        ...
        self.calendar = self.env['work.calendar'].create(
            {
                'name': 'TestCalendar',
                'attendance_ids': [
                    (0, 0, {
        ...
        self.resource1_id = self.env['work.resource'].create(
            {
                'name': 'TestResource1',
                'resource_type': 'user',
        ...
        self.leave1_id = self.env['work.calendar.leave'].create(
            {
                'name': 'GenericLeave',
                'calendar_id': self.calendar.id,
        ...
        self.leave2_id = self.env['work.calendar.leave'].create(
            {
                'name': 'ResourceLeave',
                'calendar_id': self.calendar.id,
        ...
        self.leave3_id = self.env['work.calendar.leave'].create(
            {
                'name': 'ResourceLeave2',
                'calendar_id': self.calendar.id,
                'resource_id': self.resource1_id,
