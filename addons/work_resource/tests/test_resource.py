...
class TestIntervals(TestResourceCommon):
    ...
    def test_interval_clean(self):
        cleaned_intervals = self.env['work.calendar'].interval_clean(self.intervals)
    ...
    def test_interval_remove(self):
        ...
        result = self.env['work.calendar'].interval_remove_leaves(working_interval, self.intervals)
    ...
    def test_interval_schedule_hours(self):
        cleaned_intervals = self.env['work.calendar'].interval_clean(self.intervals)
        result = self.env['work.calendar'].interval_schedule_hours(cleaned_intervals, 5.5)
    ...
    def test_interval_schedule_hours_backwards(self):
        ...
        result = self.env['work.calendar'].interval_schedule_hours(cleaned_intervals, 5.5, remove_at_end=False)
        self.assertEqual(len(result), 2)
        # First interval: 03, 8-10 untouched
...
class TestCalendarBasics(TestResourceCommon):
    ...
    def test_calendar_working_day_intervals_limited_attendances(self):
        ...
        attendance = self.env['work.calendar.attendance'].search(
            [('name', '=', 'Att3')])
        attendance.write({
...
class ResourceWorkingHours(TestResourceCommon):
    ...
    def test_calendar_working_hours_default_calendar(self):
        ...
        res = self.env['work.calendar'].with_context(tz='UTC').get_working_hours(
            Datetime.from_string('2013-02-12 06:00:00'),
    ...
    def test_calendar_days_scheduling_without_calendar(self):
        ...
        res = self.env['work.calendar'].with_context(tz='UTC').schedule_days_get_date(5, day_date=Datetime.from_string('2013-02-12 09:08:07'), default_interval=(8, 16))
