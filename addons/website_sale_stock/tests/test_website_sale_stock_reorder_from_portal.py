...
class TestWebsiteSaleStockReorderFromPortal(HttpCase, WebsiteSaleStockCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partially_available_product = cls._create_product(name="partially_available_product")
        user_admin = cls.env.ref("base.user_admin")
        order = cls.env["sale.order"].create({
            "partner_id": user_admin.partner_id.id,
            "state": "sale",
            "order_line": [
                Command.create({"product_id": cls.available_product.id, "product_uom_qty": 1}),
                Command.create({"product_id": cls.unavailable_product.id, "product_uom_qty": 1}),
                Command.create({
