...
class ProductTemplate(models.Model):
    ...
    def _get_additionnal_combination_info(self, product_or_template, quantity, uom, date, website):
        ...
        if product_or_template.type == 'combo':
            ...
            max_quantities = [
                ...
                if (max_quantity := combo._get_max_quantity(website, website.current_sale_order_id)) is not None
            ]
            if max_quantities:
        ...
        if product_or_template.is_product_variant:
            ...
            if not product_sudo.allow_out_of_stock_order:
                cart_quantity = product_sudo.uom_id._compute_quantity(
                    website.current_sale_order_id._get_cart_qty(product_sudo.id),
                    to_unit=uom,
                )
    ...
    def _get_additional_configurator_data(
        ...
        if (website := ir_http.get_request_website()) and product_or_template.is_product_variant:
            max_quantity = product_or_template._get_max_quantity(website, website.current_sale_order_id, **kwargs)
