...
class TestCrossBorderVat(TransactionCase):
    ...
    def test_foreign_partner_invalid_vat_fails(self):
        ...
        expected_error_regex = (
            r"The VAT number \[XYZ88472910\] for partner \[Bad Vendor\] does not seem to be valid\.\s*"
            r"Note: the expected format is FR23334175221"
        )

        with self.assertRaisesRegex(ValidationError, expected_error_regex):
