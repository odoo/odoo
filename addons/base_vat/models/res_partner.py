...
class ResPartner(models.Model):
    ...
    def _run_vat_checks(self, country, vat, partner_name='', validation='error'):
        ...
        prefixed_country = ''
        if country in self._get_eu_prefixed_countries() and vat_prefix:
            country_code = EU_EXTRA_VAT_CODES_INV.get(vat_prefix, vat_prefix)
            if country_code in self._get_eu_prefixed_countries().mapped('code'):
                vat = vat_number
                prefixed_country = vat_prefix
                split_vat = True
                while split_vat: # For the case where the user would accidently repeat the prefix e.g. BEBEBE0477472701
                    vat_prefix, vat_number = self._split_vat(vat)
                    if vat_prefix == prefixed_country:
                        vat = vat[2:]
                    else:
                        split_vat = False
        ...
        if not self._check_vat_number(code_to_check, vat):
    ...
    def fix_eu_vat_number(self, country_id, vat):
        """ Better to avoid this method and use the standard run_vat_checks"""
    ...
    def check_vat_al(self, vat):
        ...
        if len(number) == 10 and self._check_vat_al_re.match(number):
            return True
        return False
    ...
    def _get_vat_required_valid(self, company=None, fp_exists=False):
        ...
        vat_required_valid = super()._get_vat_required_valid(company=company, fp_exists=fp_exists)
        eu_countries = self.env.ref('base.europe').country_ids
        if (
            company and self.with_company(company).perform_vies_validation
            and (fp_exists or company.country_id in eu_countries)
        ):
