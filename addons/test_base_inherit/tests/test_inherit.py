...
class TestXMLIDS(common.TransactionCase):
    def test_xml_ids(self):
        ...
        self.assertCountEqual(xml_ids.get(ir_field.id), [
            'test_base_new_api.field_test_new_api_selection__state',
            'test_base_inherit.field_test_new_api_selection__state',
        ])

        foo, bar, baz = ir_field.selection_ids
        xml_ids = (foo + bar + baz)._get_external_ids()
        self.assertCountEqual(xml_ids.get(foo.id), [
            'test_base_new_api.selection__test_new_api_selection__state__foo',
        ])
        self.assertCountEqual(xml_ids.get(bar.id), [
            'test_base_new_api.selection__test_new_api_selection__state__bar',
            'test_base_inherit.selection__test_new_api_selection__state__bar',
        ])
        self.assertCountEqual(xml_ids.get(baz.id), [
            'test_base_inherit.selection__test_new_api_selection__state__baz',
        ])
