...
test("OnlinePaymentErrorsTour: invalid online payment lines are refused and dropped", async () => {
    ...
    Utils.applyFlatProductPrice(store, 4.8);
    ...
    await Utils.clickPaymentMethod("Online payment");
    await Utils.clickPaymentMethod("Online payment");
    await Utils.clickPaymentline({ name: "Online payment", amount: "0.00" });
    await Utils.deletePaymentline({ name: "Online payment", amount: "0.00" });
    expect(Utils.getOnlinePaymentLines(order).map((line) => line.getAmount())).toEqual([46]);

    await Utils.clickPaymentline({ name: "Cash", amount: "2.00" });
