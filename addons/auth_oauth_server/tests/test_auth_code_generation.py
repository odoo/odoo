...
class TestAuthCodeGeneration(OauthServerCommon):
    ...
    def test_consent_screen_shows_the_correct_client_name(self):
        # Without passing client_name, the consent screen will show the name of the registered client.
        self.authenticate('internal_user', 'internal_user')
        response = self._authorize()
        self.assertEqual(response.status_code, 200)
        self.assertIn(
        ...
        # Passing client_name overrides the name of the registered client for the consent screen.
        self.authenticate('internal_user', 'internal_user')
        response = self._authorize(client_name='ChatGPT')
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            f"<strong>ChatGPT</strong> is requesting access to your Odoo account for <strong>{self.resource.label}</strong>",
            re.sub(r"\s+", " ", response.text)
        )
    ...
    def test_inactive_client_cannot_generate_an_auth_code(self):
        ...
        self.assertEqual(authorize_response.status_code, 400, authorize_response.text)
        self.assertEqual(authorize_response.json()['error'], 'authorization_failed')
        self.assertEqual(authorize_response.json()['error_description'], "Unknown client_id")
        ...
        self.assertEqual(consent_response.status_code, 400, consent_response.text)
        self.assertEqual(consent_response.json()['error_description'], "Unknown client_id")
        self.assertFalse(self.env['oauth.authorization.code'].search([('client_id', '=', client.id)]))
