...
from odoo.addons.test_mail.tests.common import TestMailCase, TestRecipients
from odoo.exceptions import AccessError
from odoo.tools import mute_logger


class TestMessagePost(TestMailCase, TestRecipients):
    ...
    def test_post_needaction(self):
        ...
        with self.assertNotifications([{'content': 'Body', 'notif': [{'partner': self.partner_employee, 'type': 'inbox'}]}]):
        ...
        with self.assertNotifications([{'content': 'NewBody', 'notif': [
        ...
        with self.assertNotifications([{'content': 'ToPortal', 'notif': [
            {'partner': self.partner_1, 'type': 'email'},
    ...
    def test_post_inactive_follower(self):
        ...
        with self.assertNotifications([{'content': 'Test', 'notif': [{'partner': self.partner_employee, 'type': 'inbox'}]}]):
            self.test_record.message_post(
        ...
        with self.assertNotifications([{'content': 'Test', 'notif': []}]):
            self.test_record.message_post(
                body='Test', message_type='comment', subtype='mail.mt_comment')
    ...
    def test_post_notifications(self):
        ...
        with self.assertNotifications([{'content': _body, 'notif': [
            {'partner': self.partner_1, 'type': 'email'},
    ...
    def test_post_answer(self):
        ...
        self.assertNotSentEmail(self.user_employee.partner_id)
    ...
    def test_post_portal_ok(self):
        ...
        with self.assertNotifications([{'content': '<p>Test</p>', 'notif': [
            {'partner': self.partner_employee, 'type': 'inbox'},
            {'partner': self.partner_1, 'type': 'email'}]}
