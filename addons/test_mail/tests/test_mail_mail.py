...
class TestMailMail(MailCommon):
    ...
    def test_mail_mail_send_configuration(self):
        ...
        # test 'mail.mail_mail_queue_batch_size': cron fetch size
        for batch_size, exp_send_count in [
            (3, 3),
            ...
            with self.subTest(batch_size=batch_size), \
                ...
                self.env['ir.config_parameter'].sudo().set_param('mail.mail_mail_queue_batch_size', batch_size)
                mails = self.env['mail.mail'].create([
                    {
                        'auto_delete': False,
        ...
        self.env['ir.config_parameter'].sudo().set_param('mail.mail_mail_queue_batch_size', False)
        for batch_size, exp_call_count in [
            ...
            with self.subTest(batch_size=batch_size), \
                 self.mock_mail_gateway():
                self.env['ir.config_parameter'].sudo().set_param('mail.session.batch.size', batch_size)
                mails = self.env['mail.mail'].create([
                    {
                        'auto_delete': False,
                        'body_html': f'Batch Email {idx}',
