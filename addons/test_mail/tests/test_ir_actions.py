...
from odoo.addons.test_base.tests.test_ir_actions import TestServerActionsBase
...
class TestServerActionsEmail(TestServerActionsBase):
    ...
    def test_action_email(self):
        email_template = self.env['mail.template'].create({
            ...
            'partner_to': '%s' % self.test_record.id,
        ...
        mail = self.env['mail.mail'].search([('subject', '=', 'About Testing Base Record')])
        self.assertEqual(len(mail), 1)
        # check email content
        self.assertEqual(mail.body, '<p>Dear Testing Base Record, your parent is False</p>')

    def test_action_followers(self):
        self.test_record.message_unsubscribe(self.test_record.message_partner_ids.ids)
        self.action.write({
            'state': 'followers',
        ...
        self.action.with_context(self.context).run()
        self.assertEqual(self.test_record.message_partner_ids, self.env.ref('base.partner_admin') | self.env.ref('base.partner_demo'))
        self.assertEqual(self.test_record.message_channel_ids, self.env.ref('mail.channel_all_employees'))
