...
class TestChannelModeration(common.Moderation):
    ...
    def test_send_guidelines(self):
        ...
        self.assertEmails(False, self.partner_employee | self.partner_employee_2, email_from=self.env['res.company']._get_current_company().catchall or self.env['res.company']._get_current_company().email)
