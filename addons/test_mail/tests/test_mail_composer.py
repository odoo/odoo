...
class TestComposerResultsMass(TestMailComposer):
    ...
    def test_mail_composer_scalability(self):
        ...
        for (render_size, send_size), (exp_mail_create_count, exp_force_send) in zip(
            [
                (False, False),  # unset
                (8, 0),  # send: defaults to 50 hence force_send in test
            ...
            with self.subTest(render_size=render_size, send_size=send_size):
                self.env['ir.config_parameter'].sudo().set_param(
                    "mail.batch_size", render_size
                )
                self.env['ir.config_parameter'].sudo().set_param(
                    "mail.mail_force_send_limit", send_size
                )
                composer_form = Form(self.env['mail.compose.message'].with_context(
                ...
                if exp_force_send:
                    self.assertTrue(
                        all(mail.state == 'sent' for mail in self._new_mails)
                    )
                else:
                    self.assertTrue(
                        all(mail.state == 'outgoing' for mail in self._new_mails)
                    )
