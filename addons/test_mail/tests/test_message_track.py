...
class TestTrackingTemplate(TestTrackingCommon):
    ...
    def test_message_track_template(self):
        ...
        test_record = self.test_ticket_record.with_env(self.env)
        test_record.write({'mail_template': self.env.ref('test_mail.mail_test_ticket_tracking_tpl').id})
        self.flush_tracking()
    ...
    def test_message_track_template_at_create(self):
        ...
        with self.mock_mail_gateway():
            test_ticket = self.env['mail.test.ticket'].with_user(self.user_employee).create({
                'name': 'Test',
                'customer_id': self.user_admin.partner_id.id,
        ...
        # one email send due to template
        self.assertSentEmail(self.partner_employee, [self.partner_admin], body='<p>Hello Test</p>')
