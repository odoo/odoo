...
class TestTrackingInternals(MailCommon):
    ...
    def test_mail_track_2many(self):
        ...
        child4_tracking = f'mail.test.track.all.o2m,{test_record.one2many_field[3].id}'
        self.flush_tracking()
        last_message = test_record.message_ids[0]
        self.assertTracking(
            last_message,
            [
                ...
                ('one2many_field', 'one2many', '', ', '.join(('Child1', 'Child2', 'Child3', child4_tracking))),
            ]
        )
        ...
        self.assertTracking(
            last_message,
            [('one2many_field', 'one2many', ', '.join(('Child1', 'Child2', 'Child3', child4_tracking)), ', '.join(('Child2', 'Child3', child4_tracking)))]
        )
