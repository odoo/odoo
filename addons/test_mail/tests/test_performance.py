...
class TestMailAPIPerformance(BaseMailPerformance):
    ...
    def test_adv_activity_full(self):
        ...
        with self.assertQueryCount(__system__=17, employee=21):
    ...
    def test_mail_composer(self):
        ...
        with self.assertQueryCount(__system__=32, employee=39):
            composer.send_mail()
    ...
    def test_mail_composer_nodelete(self):
        ...
        with self.assertQueryCount(__system__=25, employee=32):
            composer.send_mail()
    ...
    def test_mail_composer_w_template(self):
        ...
        with self.assertQueryCount(__system__=12, employee=14):
            composer = self.env['mail.compose.message'].with_context({
                'default_composition_mode': 'comment',
                'default_model': test_record._name,
                'default_res_id': test_record.id,
        ...
        with self.assertQueryCount(__system__=33, employee=40):
            composer.send_mail()
    ...
    def test_message_post_one_inbox_notification(self):
        ...
        with self.assertQueryCount(__system__=13, employee=18):
            record.message_post(
...
class TestMailComplexPerformance(BaseMailPerformance):
    ...
    def test_complex_message_post_template(self):
        ...
        with self.assertQueryCount(__system__=73, employee=75):
    ...
    def test_complex_tracking_subscription_create(self):
        ...
        with self.assertQueryCount(__system__=108, employee=109):
            rec = self.env['mail.test.ticket'].create({
                'name': 'Test',
