...
class TestActivityRights(TestActivityCommon):
    ...
    def test_activity_security_user_noaccess_manual(self):
        ...
        # can web_read_group activities if access to the document
        read_group_result = self.env['mail.activity'].with_user(self.user_employee).web_read_group(
            [('id', '=', test_activity.id)],
            ['summary'],
            ['__count'],
        )['groups']
        ...
        with patch.object(MailTestActivity, '_check_access', autospec=True, side_effect=_employee_crash):
            with self.assertRaises(exceptions.AccessError):
                self.env['mail.activity'].with_user(self.user_employee).web_read_group(
                    [('id', '=', test_activity.id)],
                    ['summary'],
                    ['__count'],
                )['groups']
...
class TestActivityMixin(TestActivityCommon):
    ...
    def test_mail_activity_mixin_search_state_basic(self):
        ...
        with patch('odoo.addons.mail.models.mail_activity.datetime', MockedDatetime), \
            ...
            self.assertEqual(
                {(e['activity_state'], e['__count']) for e in Model.web_read_group(**read_group_params)['groups']},
                {('today', 1), ('overdue', 1)})
...
class TestORM(TestActivityCommon):
    ...
    def test_week_grouping(self):
        ...
        with freeze_time("2024-09-24 10:00:00"):
            ...
            groups = MailTestActivityCtx.web_read_group(domain, groupby=[groupby])['groups']
