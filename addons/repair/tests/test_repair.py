...
class TestRepairCommon(common.TransactionCase):
    ...
    def _create_repair_order_with_moves_and_services(self):
        return self.env['repair.order'].create({
...
class TestRepair(TestRepairCommon):
    ...
    def test_invoice_creation_logic(self):
        ...
        repair_order.action_create_invoice()
        invoice = repair_order.invoice_ids[-1]
        ...
        self.assertRecordValues(part_move1, [
            {'qty_invoiced': 1, 'qty_to_invoice': 0},
        ])
        self.assertRecordValues(service_line, [
            {'qty_invoiced': 2, 'qty_to_invoice': 0},
        ])
        ...
        repair_order.action_create_invoice()
        refund_invoice = repair_order.invoice_ids[-1]

        # If there is any negative `qty_to_invoice` a credit note is created
        self.assertEqual(refund_invoice.move_type, 'out_refund')
        self.assertRecordValues(part_move1, [
            {'qty_invoiced': 3, 'qty_to_invoice': 0},
        ])
        # Stock moves' invoice lines are created before services, negative quantity in this case
        # because its a quantity that needs to be invoiced in a credit note.
        self.assertEqual(refund_invoice.invoice_line_ids[0].quantity, -2)
