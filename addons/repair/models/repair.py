        ...
        for repair in self:
            ...
            move = Move.create({
                ...
                'move_line_ids': [(0, 0, {'product_id': repair.product_id.id,
                                           ...
                                           'product_uom_id': repair.uom_id.id or repair.product_id.uom_id.id,
...
class RepairLine(models.Model):
    ...
    def _onchange_product_uom(self):
        ...
        if pricelist and self.product_id and self.type != 'remove':
            price = pricelist._get_product_price(self.product_id, self.product_uom_qty, uom=self.uom_id)
            if price is False:
                warning = {
...
class RepairFee(models.Model):
    ...
    price_unit = fields.Float('Unit Price', required=True, digits='Product Price')
    uom_id = fields.Many2one('uom.uom', 'Product Unit of Measure', required=True, domain="[('category_id', '=', product_uom_category_id)]")
    product_uom_category_id = fields.Many2one(related='product_id.uom_id.category_id')
    price_subtotal = fields.Float('Subtotal', compute='_compute_price_total_and_subtotal', store=True, digits=0)
    price_total = fields.Float('Total', compute='_compute_price_total_and_subtotal', store=True, digits=0)
    tax_id = fields.Many2many(
