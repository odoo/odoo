...
class StockMove(models.Model):
    ...
    def _compute_qty_to_invoice(self):
        ...
        for move in self:
            if move.repair_line_type != 'add':
                if move.qty_invoiced:
                    move.qty_to_invoice = move.qty_invoiced * -1
                else:
    ...
    def write(self, vals):
        ...
        repair_moves = self.env['stock.move']
        moves_to_create_sale_line = self.env['stock.move']
        for move in self:
            if not move.repair_id:
                continue
            # checks vals update
            if (
                ...
                moves_to_create_sale_line |= move
        ...
        moves_to_create_sale_line._create_repair_sale_order_line()
        return res
    ...
    def _get_repair_invoice_line_commands(self, is_refund_type=False):
        if not self:
            return []
        invoice_lines_commands = []
        for move in self:
            if not move.qty_to_invoice or (move.repair_line_type != 'add' and not is_refund_type):
                continue
            move_qty = move.qty_to_invoice
            if is_refund_type:
                move_qty *= -1
            line_vals = {
            ...
            if move.repair_id.under_warranty:
                ...
                line_vals['price_unit'] = move.price_unit

            invoice_lines_commands.append(Command.create(line_vals))

        return invoice_lines_commands
