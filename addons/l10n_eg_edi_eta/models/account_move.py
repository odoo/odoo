...
from odoo.tools import BinaryBytes, float_is_zero, float_compare
...
class AccountMove(models.Model):
    ...
    l10n_eg_eta_submission_ids = fields.One2many(
        ...
        comodel_name='l10n_eg_edi.eta.submission',
    ...
    l10n_eg_signing_time = fields.Datetime(string="Signing Time", copy=False)
    l10n_eg_is_signed = fields.Boolean(string="Is Signed", copy=False)
    ...
    def _compute_eta_long_id(self):
        for move in self:
            last_submission = move.l10n_eg_eta_submission_ids[-1:]
            move.l10n_eg_long_id = last_submission.eta_document_longid if last_submission else False

    @api.depends('l10n_eg_eta_submission_ids', 'state')
    def _compute_l10n_eg_edi_submission_values(self):
        for move in self:
            if last_submission := move.l10n_eg_eta_submission_ids[-1:]:
    ...
    def _compute_l10n_eg_edi_uuid(self):
        for move in self:
            move.l10n_eg_uuid = move.l10n_eg_eta_submission_ids[-1:].eta_document_uuid
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        for move in self:
            if (
                ...
                and move.l10n_eg_eta_submission_ids
                and move.l10n_eg_edi_submission_state != 'rejected'
            ):
    ...
    def action_l10n_eg_cancel_invoice(self):
        eg_moves = self.filtered(lambda inv: (
            inv.country_code == "EG"
            and inv.l10n_eg_edi_submission_state in {"accepted", "test"}
        ...
        return self.env['l10n_eg_edi.cancel.wizard'].with_context(default_move_ids=eg_moves.ids)._get_records_action(
    ...
    def action_post_sign_invoices(self):
        invoices = self.filtered(lambda inv: (
            inv.country_code == 'EG'
            and inv.state == 'posted'
            and inv.l10n_eg_edi_submission_state not in {'test', 'accepted'}
        ))
        if not invoices:
            return
        ...
        invoices.l10n_eg_signing_time = signing_time
    ...
    def _get_l10n_eg_edi_alerts(self):
        ...
        if missing_branch_details := self.journal_id.l10n_eg_branch_id.filtered(
            lambda p: p._check_l10n_eg_missing_address_data(issuer=True),
        ):
            alerts.update({
        ...
        if moves_without_journal_config := self.filtered(
            lambda inv: not (
                inv.journal_id.l10n_eg_branch_id
                and inv.journal_id.l10n_eg_activity_type_id
                and inv.journal_id.l10n_eg_branch_identifier
            ),
        ):
            alerts.update({
                'eg_eta_edi_journal_not_configured': {
                    'level': 'danger',
                    'message': self.env._("Please configure Egyptian ETA Settings on the Journal(s)."),
        ...
        if moves_with_future_date := self.filtered(
            lambda inv: inv.invoice_date > fields.Datetime.now().date(),
        ):
            alerts.update({
                'eg_eta_edi_moves_with_future_dates': {
                    'level': 'danger',
                    'message': self.env._("Please set the invoice date to be either less than or equal to today."),
        ...
        if amls_without_tax := amls_to_check.filtered(lambda line: not line.tax_ids):
            alerts.update({
                'eg_eta_edi_no_tax_lines': {
                    'level': 'danger',
                    'message': self.env._("Please make sure that there is tax in each invoice lines."),
                    'action_text': self.env._("View Invoices"),
                    'action': amls_without_tax._get_records_action(),
                },
            })

        if lines_without_product := amls_to_check.filtered(lambda line: not line.product_id):
            alerts.update({
                'eg_eta_edi_lines_without_product': {
                    'level': 'danger',
                    'message': self.env._("Please make sure that following invoice lines have a product in them."),
                    'action_text': self.env._("View Invoices"),
                    'action': lines_without_product.move_id._get_records_action(),
                },
            })

        if invalid_line_names := amls_to_check.filtered(lambda line: len(line.name or '') > 500):
            alerts.update({
    ...
    def _generate_l10n_eg_edi_json(self, is_demo=False):
        ...
        base_lines_aggregated_values_total_amount = AccountTax._aggregate_base_lines_tax_details(
        ...
        eta_invoice = {
            ...
            'documentType': ETA_DOCUMENT_TYPE_CODE.get(self.move_type, ''),
    ...
    def _l10n_eg_eta_prepare_address_data(self, partner, issuer=False):
        ...
        individual_type = partner._l10n_eg_get_partner_tax_type(issuer)
    ...
    def _l10n_eg_eta_send_invoice(self, notify=False):
        ...
        _response, data = self._l10n_eg_edi_eta_request(
            ...
            body=body,
            headers=headers,
            is_prod=self.l10n_eg_edi_api_mode == 'production',
        )
        if data.get('error'):
            return data
    ...
    def _l10n_eg_log_and_update_attachment(self, response, submission_id, success, notify=False):
        ...
        if not success:
            ...
            submission_values.update({
                'state': 'test' if self.l10n_eg_edi_api_mode != 'production' else 'accepted',
            ...
            self.l10n_eg_edi_submission_state = 'test' if self.l10n_eg_edi_api_mode != 'production' else 'accepted'
        ...
        if notify:
            self.env.user._bus_send('simple_notification', {
                ...
                'message': (
                    self.env._("Document successfully accepted!")
                    if success
                    else self.env._("Document is rejected. Please check it and try again.")
                ),
            })
    ...
    def _l10n_eg_get_eta_invoice_pdf(self, access_token):
        ...
        attachment = self.env['ir.attachment'].create({
            'name': self.env._('ETA_INVOICE_PDF_%s', self.name),
            'res_id': self.id,
            'res_model': self._name,
            'type': 'binary',
            'raw': pdf_content,
    ...
    def _l10n_eg_edi_cancel_invoices(self, cancel_reason, notify=False):
        """
        Called from the Cancel wizard to cancel single / multiple invoices.
        If the company is in demo mode, it will skip the API call and create a cancel log directly.
        """
        eg_moves = self.filtered(lambda inv: (
            inv.country_code == 'EG'
            and inv.l10n_eg_edi_submission_state in ['test', 'accepted']
        ))
        if not eg_moves:
            self.env.user._bus_send('simple_notification', {
                'type': 'warning',
    ...
    def _l10n_eg_edi_process_cancel_invoice(self, cancel_reason, notify=False):
        ...
        self._l10n_eg_edi_post_cancel(cancel_reason, notify=notify)

    def _l10n_eg_edi_post_cancel(self, cancel_reason, notify=False):
        self.env['l10n_eg_edi.eta.submission'].create({
            'move_id': self.id,
            'state': 'cancel',
            'message': self.env._("Invoice cancelled on ETA: \n%s", cancel_reason),
        })
        if notify:
            self.env.user._bus_send('simple_notification', {
                'type': 'info',
    ...
    def _l10n_eg_edi_simulate_send_invoices(self, notify=False):
        for move in self:
            move._l10n_eg_log_and_update_attachment(
                ETA_INVOICE_DUMMY_RESPONSE,
                ETA_DUMMY_SUBMISSION_ID,
                success=True,
                notify=notify,
            )
    ...
    def _l10n_eg_get_eta_qr_domain(self, is_production=True):
        return is_production and ETA_DOMAINS['invoice.production'] or ETA_DOMAINS['invoice.preproduction']

    @api.model
    def _l10n_eg_get_eta_api_domain(self, is_prod=True):
        return is_prod and ETA_DOMAINS['production'] or ETA_DOMAINS['preproduction']

    @api.model
    def _l10n_eg_get_eta_token_domain(self, is_prod=True):
        return is_prod and ETA_DOMAINS['token.production'] or ETA_DOMAINS['token.preproduction']
    ...
    @api.model
    def _l10n_eg_get_api_domain(self, is_access_request, is_prod):
        return (
            is_access_request
            and self._l10n_eg_get_eta_token_domain(is_prod)
            or self._l10n_eg_get_eta_api_domain(is_prod)
        )

    @api.model
    def _l10n_eg_edi_eta_request(self, url, method, body, headers, is_prod=True, is_access_token_req=False, timeout=20):
        request_url = self._l10n_eg_get_api_domain(is_access_token_req, is_prod) + url
        try:
    ...
    def _l10n_eg_parse_error(self, message):
        """This method returns error in the same format as ETA returns, so handling any error becomes simpler."""
        return {
            'error': {
                'code': '000',
                'message': message,
