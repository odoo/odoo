...
class PosOrder(models.Model):
    ...
    def _l10n_jo_validate_fields(self):
        error_msgs = []
        if self.refunded_order_id and not self.l10n_jo_edi_pos_return_reason:
        ...
        if len(self.payment_ids.payment_method_id.mapped('l10n_jo_edi_pos_is_cash')) > 1:
            error_msgs.append(self.env._("Please select consistent payment methods. Order payment methods should have 'JoFotara Cash' value consistent."))
        return "\n".join(error_msgs)
    ...
    def sync_from_ui(self, orders):
        # EXTENDS 'point_of_sale'
        """Entrypoint for EDI through the POS.
        Amends the response sent back to the POS after EDI to include fields needed for the receipt."""
    ...
    def button_l10n_jo_edi_pos(self):
        if error_message := self._l10n_jo_edi_send():
            ...
            message = self.env._("Submitted to JoFotara Successfully")
