...
class TestViesIAP(HttpCase):
    ...
    def test_vies_iap_valid_vat(self):
        ...
        with (
            self.assertLogs('odoo.addons.account_vat_vies.models.res_partner', logging.INFO) as log_catcher,
        ):
    ...
    def test_vies_iap_controller(self):
        ...
        with (
            self.assertLogs('odoo.addons.account_vat_vies.controllers.webhook', level="WARNING") as log_catcher,
        ):
            self.url_open(
                '/account_vat_vies/1/webhook_update_vies',
                data={
    ...
    def test_vies_iap_cron(self):
        ...
        self.env.ref('account_vat_vies.vies_iap_check_update').method_direct_trigger()
        self.assertTrue(self.partner.vies_valid)
