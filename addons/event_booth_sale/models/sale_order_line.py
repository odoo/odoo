...
class SaleOrderLine(models.Model):
    ...
    event_booth_category_id = fields.Many2one('event.booth.category', string='Booths Category')
    event_booth_pending_ids = fields.Many2many(
        'event.booth', 'event_booth_registration',
        'sale_order_line_id', 'event_booth_id', string='Pending Booths')
    ...
    def _compute_name_short(self):
        ...
        for record in wbooth:
            record.name_short = record.event_booth_pending_ids._get_booth_multiline_description()
        super(SaleOrderLine, self - wbooth)._compute_name_short()

    @api.onchange('product_id')
    def _onchange_booth_product_id(self):
        if self.event_id and (not self.product_id or self.product_id.id not in self.event_id.mapped('event_booth_pending_ids.product_id.id')):
            self.event_id = None
