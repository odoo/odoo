...
class Survey(http.Controller):
    ...
    def _prepare_survey_finished_values(self, survey, answer, token=False):
        values = {'survey': survey, 'answer': answer, 'finished': True}
        if token:
            values['token'] = token
    ...
    def _prepare_survey_data(self, survey_sudo, answer_sudo, **post):
        """ This method prepares all the data needed for template rendering, in function of the survey user input state.
            :param post: can contain 2 values useful for this method:
                - start_survey : comes from 'begin' route and force the survey to start for the current user, if the
                                 survey user input state is 'new'
        ...
        if answer_sudo.state == 'new':
            if 'start_survey' not in post:  # Start page
        ...
        else:  # answer_sudo.state == 'in_progress'
    ...
    def _prepare_question_html(self, survey_sudo, answer_sudo, **post):
        ...
        return request.env.ref('survey.survey_form_content').render(data).decode('UTF-8')
    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        # Avoid submitting answers if time limit reached for too long - avoid frontend timer hacking
        if answer_sudo.is_time_limit_reached:
            extra_buffer_time = timedelta(seconds=10)
            stop_time = answer_sudo.start_datetime + timedelta(minutes=survey_sudo.time_limit)
            if fields.Datetime.now() > (stop_time + extra_buffer_time):
