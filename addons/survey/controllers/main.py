...
class Survey(http.Controller):
    ...
    def _prepare_survey_data(self, survey_sudo, answer_sudo, **post):
        ...
        if survey_sudo.questions_layout != 'page_per_question':
            ...
            data.update({
                ...
                'triggered_questions_by_answer': {
                    answer.id: [
                        question.id for question in triggered_questions_by_answer[answer]
                    ] for answer in triggered_questions_by_answer.keys()},
                'selected_answers': [answer.id for answer in selected_answers]
            })
        ...
        if 'previous_page_id' in post:
            ...
            new_previous_id = survey_sudo._get_next_question(answer_sudo, previous_page_or_question_id, go_back=True).id
            data.update({
        ...
        if answer_sudo.state == 'in_progress':
            if answer_sudo.is_session_answer:
                ...
                next_page_or_question = survey_sudo._get_next_question(
                    answer_sudo,
            ...
            if survey_sudo.questions_layout != 'one_page':
                data.update({
                    'previous_page_id': survey_sudo._get_next_question(answer_sudo, next_page_or_question.id, go_back=True).id
                })
    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        if answer_sudo.survey_time_limit_reached or survey_sudo.questions_layout == 'one_page':
            ...
            if not answer_sudo.is_session_answer:
                next_page = survey_sudo._get_next_question(answer_sudo, page_or_question_id)
                if not next_page:
                    answer_sudo._mark_done()
