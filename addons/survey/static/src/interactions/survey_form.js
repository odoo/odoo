...
export class SurveyForm extends Interaction {
    ...
    onChoiceItemChange(ev) {
        ...
        if (surveyLastTriggeringAnswers) {
            const currentSelectedAnswers = Array.from(this.el.querySelectorAll(`
        ...
        const isQuestionComplete = this.checkConditionalQuestionsConfiguration(targetEl, questionEl);
    ...
    showCorrectAnswers(correctAnswers, submitPromise, options) {
        ...
        this.addListener(nextPageButtonEl, "click", (ev) => {
            ev.preventDefault();
            this.showingCorrectAnswers = false;
