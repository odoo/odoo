...
publicWidget.registry.SurveyResultPagination = publicWidget.Widget.extend({
     ...
     */
    start: function () {
        this.limit = this.$el.data("record_limit");
        this.$questionsEl.find('tbody tr:lt(' + this.limit + ')').removeClass('d-none');
...
publicWidget.registry.SurveyResultChart = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        this.graphData = this.$el.data("graph_data");

        switch (this.$el.data("graph_type")) {
            case 'multi_bar':
                this.chartConfig = this._getMultibarChartConfig();
                break;
            case 'bar':
                this.chartConfig = this._getBarChartConfig();
                break;
            case 'pie':
                this.chartConfig = this._getPieChartConfig();
                break;
            case 'doughnut':
                this.chartConfig = this._getDoughnutChartConfig();
                break;
        }
    ...
    */
    start: function () {
        ...
        this.$('.pagination').each(function (){
            ...
            $(this).find('li:first').addClass('active').find('a').click();
        });

        this.$('.survey_graph').each(function () {
