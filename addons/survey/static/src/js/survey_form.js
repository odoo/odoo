...
publicWidget.registry.SurveyFormWidget = publicWidget.Widget.extend({
    ...
    */
    start: function () {
        var self = this;
        this.options = this.$target.data()
        // If review mode, just display
        // If take survey, initialize data, widget and fields,
    ...
    /*
    * Checks, for already answered page, if the 'other' choice is checked.
    * Clear the comment textarea and disable (or hide) if if not checked (just in case) or focus on it and enable (or show) it.
    *
    * @private
    * @param {Event} event
    */
    ...
    _onBreadcrumbClick: function (event) {
        ...
        $('<input>').attr({
            type: 'hidden',
            name: 'breadcrumb_redirect',
            value: $valObj.attr('href')
        }).appendTo('.js_surveyform');
