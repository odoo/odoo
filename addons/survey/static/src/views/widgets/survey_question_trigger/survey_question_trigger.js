...
export class SurveyQuestionTriggerWidget extends Component {
    setup() {
        ...
        useEffect(() => {
            if (this.button && this.button.el) {
                ...
                if (triggerError === "MISSING_TRIGGER_ERROR") {
                ...
                } else if (triggerError === "MISPLACED_TRIGGER_WARNING") {
                    this.state.surveyQuestionTriggerTooltip = sprintf(
     ...
     * record data (is_placed_before_trigger) for two main reasons:
     *   1. Performance: we avoid saving the survey each time a line is moved.
     *   2. UX: We avoid cleaning data silently for missing triggers and
     *     instead show a warning message on which the user can act. This is
     *     valid for errors and warnings described (see trigger_fields.js).
     *
