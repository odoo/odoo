...
class Survey(models.Model):
    ...
    def _get_next_question(self, user_input, page_or_question_id, go_back=False):
        """
        ...
        if survey.has_conditional_questions and survey.questions_layout == 'page_per_question' and triggered_questions_by_answer:
            while True:
                if go_back == 1 and current_page_index - 1 >= 0:
                    next_question = pages_or_questions[current_page_index - 1]
                elif current_page_index + 1 < len(pages_or_questions):
                    next_question = pages_or_questions[current_page_index + 1]
                else:  # If no more question to display
                    return Question

                triggering_answer = triggering_answer_by_question.get(next_question)
                if not triggering_answer or triggering_answer in selected_answers:
                    # If next question found
                    return next_question
                current_page_index += 1 if not go_back else -1
        else:
            next_question = pages_or_questions[current_page_index + (1 if not go_back else -1)]
    ...
    def _get_conditional_maps(self):
        ...
        for question in self.question_ids:
            triggering_answer_by_question[question] = question.is_conditional and question.conditional_answer_id

            if question.is_conditional:
                if question.conditional_answer_id in triggered_questions_by_answer:
                    question_list = triggered_questions_by_answer[question.conditional_answer_id] + [question]
                    triggered_questions_by_answer[question.conditional_answer_id] = question_list
                else:
                    triggered_questions_by_answer[question.conditional_answer_id] = [question]
    ...
    def _get_session_next_question(self):
        ...
        if not self.session_question_id:
            ...
            next_question = self._get_next_question(most_voted_answers, self.session_question_id.id)
