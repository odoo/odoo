...
class SurveySurvey(models.Model):
    """This template allows to extend actions linked to the "not found" buttons."""

    _inherit = 'survey.survey'

    @api.model
    def action_setup_survey_type_template(self, template_key):
        # Manage the action
        template_values = self._get_survey_type_template_values(template_key)
        return self.env['survey.survey'].create(template_values).action_show_sample()

    @api.model
    def get_survey_type_templates_data(self):
        # Render all the elements for the button views
        return {
            'survey': {
