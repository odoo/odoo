...
class SurveyQuestion(models.Model):
    ...
    # Conditional display
    is_conditional = fields.Boolean(string='Conditional Display', copy=False,
                                    help="""If checked, this question will be displayed only if the specified
                                            conditional answer have been selected in a previous question""")
    conditional_question_id = fields.Many2one('survey.question', string="Conditional Question", copy=False,
                                              compute="_compute_conditional_question_id", store=True, readonly=False,
                                              domain="""[('survey_id', '=', survey_id),
                                                         '&', ('question_type', 'in', ['simple_choice', 'multiple_choice']),
                                                         '|',
                                                             ('sequence', '<', sequence),
                                                             '&', ('sequence', '=', sequence), ('id', '<', id)]""",
                                              help="Question that contains answer that need to be selected to display the current question.")
    conditional_answer_id = fields.Many2one('survey.question.answer', string="Conditional Answer", copy=False,
                                            compute="_compute_conditional_answer_id", store=True, readonly=False,
                                            domain="[('question_id', '=', conditional_question_id)]",
                                            help="Answer that needs to be selected to display the current question.")
    ...
    def _compute_conditional_question_id(self):
        ...
        for question in self:
            if not question.is_conditional or question.conditional_question_id is None:
                question.conditional_question_id = False

    @api.depends('conditional_question_id')
    def _compute_conditional_answer_id(self):
        ...
        for question in self:
            if not question.conditional_question_id \
                    or question.conditional_question_id != question.conditional_answer_id.question_id\
                    or question.conditional_answer_id is None:
                question.conditional_answer_id = False
