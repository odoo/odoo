...
class SurveyQuestion(models.Model):
    ...
    def _validate_time(self, answer):
        ...
        if self.validation_required:
            # Answer is not in the right range
            with contextlib.suppress(Exception):
                if (self.validation_min_time or self.validation_max_time) and \
                    not (self.validation_min_time <= float_time_answer <= self.validation_max_time):
                    return {self.id: self.validation_error_msg or _('The answer you entered is not valid.')}
        return {}
    ...
    def _get_correct_answers(self):
        ...
        for question in self - choices_questions:
            ...
            if question.question_type == 'date':
                ...
                answer = tools.format_time(self.env, float_to_time(answer), time_format="short")
