...
class SurveyInvite(models.TransientModel):
    ...
    def open_template_creation_wizard(self):
        """ hit 'save as template' button it will open the wizard to save the temaplte.
            `create_mail_template` is called when click on save. """
    ...
    def cancel_save_template(self):
        """ After canceling the 'save as template' wizard this will restore the previous fields
            As the user may want to edit the template. """
        self.ensure_one()
        return _reopen(self, self.id, self.model, context={**self.env.context, 'dialog_size': 'large'})
