odoo.define('website_form_builder.editor', function (require) {
'use strict';
        ...
        .then(function(options_list) {
            ...
            .call("get_authorized_fields", [options_list[0].id], {context: website.get_context()}).then(function(data) {
                ...
                $.each(data, function(n, val) {
                    ...
                    l = val.required ? val.string + ' *' : val.string;
                    
                    form.data('fields')[val.type][n] = {name: n, label : l, relation : rel,selection : sel, type: val.type, required: val.required};
...
var EditFormFieldOptionEditor = Class.extend({
    ...
    fieldModel : function() {
        return qweb.render('webiste.form.editor.wizard.option.template');
    },
        ...
        this.table.find('tr').each(function(i,elem) {
            ...
            if(label && value) {
                if((label.length > 0) && (value.length > 0)) {
                    ...
                    fullOptions += qweb.render('website.form.editor.template.option.'+type,params);
                }
            }
        });
    ...
    loadData: function (wizard,target, get) {
        ...
        if(!dataList.length) {
            parent.append('<td colspan="3"><strong class="center">'+_t('No Data')+'</strong></td>');
            tb_append = tb_last.filter('.o_form_builder_form-editor-add');
...
snippet_editor.options.formBuilder = snippet_editor.Option.extend({
    wizard: null,
    ...
    on_prompt: function () {
        ...
        .then(function(options_list) {
            ...
            self.wizard = $(qweb.render('website.form.editor.wizard.template.modelSelection',{'options': options}));
            ...
            self.call = editor.LinkDialog.prototype.call;
            self.fetch_pages = editor.LinkDialog.prototype.fetch_pages;

            self.wizard.find('input#success').select2({
                enable: true,
                initSelection: function(elem, callback) {
                    var value = self.$target.attr('data-success') || '';
                    callback({id:value, text:value});
                },
                query: function(query) {
                    var data = {results:[{id:query.term, text:query.term}]};
                    self.fetch_pages(query.term).then(function (tlist) {
                        $.each(tlist, function(i, obj) {
                            console.log(i, obj);
                            data.results.push({id: obj.loc, text:obj.loc});
                        });
                        query.callback(data);
                    });
                },
    ...
    check_for_save: function () {
        ...
        if(required_error) {
            ...
            form.prepend($(qweb.render('website.form.editor.error',{'message':message})).attr('contenteditable', false));
            return false;
        }
        return true;
...
snippet_editor.options.formBuilderFields = snippet_editor.Option.extend({
    model: null,
    fields: null,
    old_action: null,
    wizard: null,
    ...
    hiddenLoadData: function() {
        ...
        this.$target.addClass('css_non_editable_mode_hidden');
    ...
    on_prompt: function () {
        ...
        fieldWizardTemplate = (qweb.has_template('website.form.editor.wizard.template.'+type)) ?
                                qweb.render('website.form.editor.wizard.template.'+type) : '';
        ...
        this.loadData = function() {
         ...
         // Load template
        this.wizard = $(qweb.render('website.form.editor.wizard.template.default',{subTemplate: fieldWizardTemplate}));
    ...
    drop_and_build_snippet: function() {
        ...
        var form = this.$target.closest('form');
        this._super();
        this.getModel(form.data('model')).then(function() {
    ...
    start : function () {
        ...
        var self = this;
        this.getModel = getModel;
        this.optionEditor = new EditFormFieldOptionEditor(this);
        /*if(this.$target.data('form') == 'hidden'){
            this.$target.addClass('css_non_editable_mode_hidden');
        }*/
        var form = this.$target.closest('form');
        this.getModel(form.data('model'));
        this._super();
    },
    clean_for_save: function () {
        ...
        /*if(this.$target.data('form') == 'hidden') {
            this.$target.removeClass('css_non_editable_mode_hidden');
        }*/
