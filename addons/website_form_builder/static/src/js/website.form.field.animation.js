odoo.define('website_form_builder.animation', function (require) {
'use strict';
...
var snippet_animation = require('website.snippets.animation');
var registry  = snippet_animation.registry;
var Animation = snippet_animation.Animation;

var _t        = core._t;
var qweb      = core.qweb;

website.add_template_file('/website_form_builder/static/src/xml/website.form.template.xml');
...
var getDataForm = {
    'input[type=text]':     function($field) {return $field.val() ? _.object([$field.attr('name')], [$field.val()]) : null;},
...
registry.form_builder_send = Animation.extend({
    ...
    send: function(e) {
        ...
        var progress = $(qweb.render('website.form.editor.progress'))
                            .appendTo('body')
                            .modal({"keyboard" :true});
        ...
        ajax.post('/website_form/'+model,args)
        .then(function(data) {
            progress.modal('hide');
            ...
            if(!(data && (data.id || (data.fail_required && data.fail_required.length)))) {
                self.$target.find('.o_form_builder_form-danger').show(500)
                            .parent().find('.form-group').hide(500)
                            .closest('form').removeClass('o_form_builder_send-success')
                                            .addClass('o_form_builder_send-failed');
                return;
            }
            ...
            if(success_page) {
                ...
                self.$target.find('.o_form_builder_form-success').show(500)
                            .parent().find('.form-group').hide(500)
                            .closest('form').removeClass('o_form_builder_send-failed')
                                            .addClass('o_form_builder_send-success');
            }
        })
        .fail(function(data){
            progress.modal('hide');
...
registry.form_builder_input_file = Animation.extend({
    selector: ".input-file",
    start: function () {
...
registry.form_builder_autocomplete = Animation.extend({
