...
class DocController(http.Controller):
    """The registry's own contract: every model this reader may reach, its
    fields and its public methods, as JSON documents a client reads before it
    calls `/json/2`. The playground that renders them is `api_doc`.
    """
    ...
    def doc_index(self):
        ...
        :raises AccessError: the user is not in ``rpc.group_allow_doc``
        """
    ...
    def _check_doc_access(self):
        ...
        :raises AccessError: the user is not in ``rpc.group_allow_doc``
        """
        if not self.env.user.has_group("rpc.group_allow_doc"):
            raise AccessError(
                self.env._(
                    "This page is only accessible to %s users.",
                    self.env.ref("rpc.group_allow_doc").sudo().name,
                )
            )
    ...
    def _doc_cache_key(self, scope, generation):
        ...
        :param str generation: from
            :func:`~odoo.addons.rpc.tools.cache.doc_cache_generation`
