    ...
    async loadRecords(data) {
        ...
        for (const rawRecord of rawRecords) {
            ...
            if (rawRecord.recurring_maintenance && !rawRecord.done && !rawRecord.archive) {
                ...
                let [unit, interval] = [rawRecord.repeat_unit + 's', rawRecord.repeat_interval]
                let date = deserializeDateTime(rawRecord.schedule_date);
                date = this._getNextDate(date, unit, interval);
                let endDate =  deserializeDateTime(rawRecord.schedule_end);
                endDate = this._getNextDate(endDate, unit, interval);
                let counter = 1;
                while (date <= end) {
                    if (date > start) {
