...
class TestCalendarWithRecurrence(HttpCase):
    ...
    def test_dblclick_event_from_calendar(self):
        ...
        requests = self.env['maintenance.request'].create([{
            ...
            'name': 'clean the room',
            'schedule_date': datetime.now().replace(second=0, microsecond=0),
        ...
        self.assertEqual(request.schedule_end, request.schedule_date + relativedelta(hours=2), "The event modification should update the request by 2 hours")
