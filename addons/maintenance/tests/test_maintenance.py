...
class TestEquipment(TransactionCase):
    ...
    def test_update_multiple_maintenance_request_record(self):
        ...
        maintenance_requests = self.env['maintenance.request'].search([('name', 'in', ['m_1', 'm_2'])])
        try:
            maintenance_requests.write({'kanban_state': 'blocked', 'stage_id': self.ref('maintenance.stage_0')})
        except ValueError:
            self.fail("The write method of maintenance.request should be able to update multiple records")
