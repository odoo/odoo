...
class TeamTeam(models.Model):
    ...
    use_maintenance = fields.Boolean(
        string="Maintenance",
        help="The team repairs and maintains equipment: it receives maintenance requests.",
    )
    ...
    maintenance_todo_request_count = fields.Integer(
        string="Number of Requests",
        compute="_compute_maintenance_todo_requests",
    )
    maintenance_todo_request_count_date = fields.Integer(
        string="Number of Requests Scheduled",
        compute="_compute_maintenance_todo_requests",
    )
    maintenance_todo_request_count_high_priority = fields.Integer(
        string="Number of Requests in High Priority",
        compute="_compute_maintenance_todo_requests",
    )
    maintenance_todo_request_count_block = fields.Integer(
        string="Number of Requests Blocked",
        compute="_compute_maintenance_todo_requests",
    )
    maintenance_todo_request_count_unscheduled = fields.Integer(
    ...
    def _get_usages(self):
        return super()._get_usages() | {
            "maintenance": TeamUsage(
                ...
                alias_model="maintenance.request",
            ),
        }
    ...
    def _compute_maintenance_todo_requests(self):
        ...
        for team in self:
            ...
            team.maintenance_todo_request_count = sum(
                count for (_, _, _, count) in data
            )
            team.maintenance_todo_request_count_date = sum(
                count for (schedule_date, _, _, count) in data if schedule_date
            )
            team.maintenance_todo_request_count_high_priority = sum(
                count for (_, priority, _, count) in data if priority == "3"
            )
            team.maintenance_todo_request_count_block = sum(
                count
                for (_, _, kanban_state, count) in data
                if kanban_state == "blocked"
            )
            team.maintenance_todo_request_count_unscheduled = (
                team.maintenance_todo_request_count
                - team.maintenance_todo_request_count_date
            )
