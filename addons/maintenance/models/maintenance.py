...
class MaintenanceMixin(models.AbstractModel):
    ...
    def _compute_maintenance_request(self):
        for record in self:
            ...
            failure_dates = [(req.schedule_date or req.create_date).date() for req in maintenance_requests]
            record.mttr = len(maintenance_requests) and sum(int((req.close_date - failure_date).days)
                for req, failure_date in zip(maintenance_requests, failure_dates)) / len(maintenance_requests) or 0
