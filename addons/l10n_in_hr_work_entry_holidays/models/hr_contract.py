...
class HrContract(models.Model):
    ...
    def _get_contract_work_entries_values(self, date_start, date_stop):
        ...
        work_entry_values = super()._get_contract_work_entries_values(date_start, date_stop)
        ...
        for contract in in_contracts:
            ...
            attendance_intervals = list(contract_calendar_id._attendance_intervals_batch(start_dt, end_dt, resources=resource, tz=tz)[resource.id])
            ...
            for leave in leaves_by_employee[employee]:
                ...
                public_holidays = self.env['resource.calendar.leaves'].search([
                    ...
                    ('calendar_id', 'in', [False, contract_calendar_id.id]),
                ...
                self._update_public_holiday_work_entry_values(
                    employee,leave, leave_work_entry_type, leave_start_dt, leave_end_dt, work_entry_values, public_holidays
                )
                leave_dates = {
                ...
                work_entry_values.extend(
                    {
                        'name': "%s: %s" % (leave_work_entry_type.name or _('Undefined Type'), employee.name),
    ...
    def _adjust_leave_dates(self, leave, start_dt, end_dt, tz, contract_calendar_id):
        ...
        leave_start_dt = leave_start_dt.replace(hour=0, minute=0, second=0)
        leave_end_dt = leave_end_dt.replace(hour=23, minute=59, second=59)
        return leave_start_dt, leave_end_dt

    def _update_public_holiday_work_entry_values(self, employee, leave, leave_work_entry_type, leave_start_dt, leave_end_dt, work_entry_values, public_holidays):
