...
class GamificationBadgeUser(models.Model):
    ...
    def _check_employee_related_user(self):
        for badge_user in self:
            if badge_user.user_id and badge_user.employee_id and badge_user.employee_id not in badge_user.user_id.employee_ids:
...
class GamificationBadge(models.Model):
    ...
    def _compute_granted_employees_count(self):
        for badge in self:
            badge.granted_employees_count = self.env['gamification.badge.user'].search_count([('badge_id', '=', badge.id), ('employee_id', '!=', False)])
