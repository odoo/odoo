...
class StockMove(models.Model):
    ...
    def _get_value_from_extra(self, quantity, at_date=None):
        accounting_data_dict = super()._get_value_from_extra(quantity, at_date=at_date)
        lcs_dict = self._get_landed_cost(at_date=at_date)
        for record in self:
            accounting_data = accounting_data_dict[record.id]
            lcs = lcs_dict.get(record)
            if not lcs:
                continue
            lcs_desc = []
            for lc in lcs:
                ...
                if vendor_bill:
                    desc = self.env._("+ %(value)s from %(vendor_bill)s (Landed Cost: %(landed_cost)s)",
                        value=record.company_currency_id.format(value),
