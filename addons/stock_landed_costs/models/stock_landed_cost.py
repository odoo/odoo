...
class StockLandedCost(models.Model):
                ...
                for valuation in cost.valuation_adjustment_lines:
                    ...
                    if valuation.cost_line_id and valuation.cost_line_id.id == line.id:
                        ...
                        if valuation.id not in towrite_dict:
                            ...
                            towrite_dict[valuation.id] += value
                if (r_diff := self.currency_id.round(line.price_unit - value_split)) != 0:
                    towrite_dict[max(towrite_dict.keys())] += r_diff
    ...
    def _check_sum(self):
        ...
        rounding = self.currency_id.rounding or self.company.currency_id.rounding
        for landed_cost in self:
            total_amount = sum(landed_cost.valuation_adjustment_lines.mapped('additional_landed_cost'))
            if not tools.float_is_zero(total_amount - landed_cost.amount_total, precision_rounding=rounding):
                return False

            val_to_cost_lines = defaultdict(lambda: 0.0)
            for val_line in landed_cost.valuation_adjustment_lines:
                val_to_cost_lines[val_line.cost_line_id] += val_line.additional_landed_cost
            if any(not tools.float_is_zero(cost_line.price_unit - val_amount, precision_rounding=rounding)
