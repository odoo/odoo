...
class TestStockLandedCostsLots(TestLotValuation):
    _test_user_groups = (
        'stock.group_stock_manager',  # stock.landed.cost and its lines are manager-only
        'product.group_product_manager',  # product.product/template/category create/write are the subject
        'account.group_account_invoice',  # account.move (bills/invoices/COGS) generated by valuation are asserted here
        'account.group_account_readonly',  # FIXME: valuation closing reads account.fiscal.year (account_accountant.compute_fiscalyear_dates) without sudo
        'purchase.group_purchase_user',  # these tests receive their lots through a purchase order
    )
    ...
    def test_landed_cost_when_partially_sold(self):
        ...
        self.assertRecordValues(lots, [
            # the empty lot keeps its receipt cost, without a share of the landed cost
            {'product_qty': 0, 'standard_price': 10000, 'total_value': 0},
