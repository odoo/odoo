...
class stock_landed_cost(osv.osv):
    ...
    def get_valuation_lines(self, cr, uid, ids, picking_ids=None, context=None):
        ...
        for picking in picking_obj.browse(cr, uid, picking_ids):
            for move in picking.move_lines:
                ...
                lines.append(vals)
        if not lines:
            raise Warning(_('Error!'), _('The selected picking does not contain any move that would be impacted by landed costs. Landed costs are only possible for products configured in real time valuation with real price costing method. Please make sure it is the case, or you selected the correct picking'))
        return lines
    ...
    def _create_accounting_entries(self, cr, uid, line, move_id, qty_out, context=None):
        ...
        if not credit_account_id:
            raise Warning(_('Error!'), _('Please configure Stock Expense Account for product: %s.') % (cost_product.name))
    ...
    def button_validate(self, cr, uid, ids, context=None):
        ...
        for cost in self.browse(cr, uid, ids, context=context):
            if not cost.valuation_adjustment_lines or not self._check_sum(cr, uid, cost, context=context):
                raise Warning(_('Error!'), _('You cannot validate a landed cost which has no valid valuation lines.'))
