...
class PaymentProvider(models.Model):
    ...
    def _setup_provider(self, code):
        """ Override of `payment` to migrate Ogone tokens to Worldline.
        Remove ogone from account payment method.
        """
        super()._setup_provider(code)
        worldline_provider = self.env['payment.provider'].search([('code', '=', code)])
        ogone_provider = self.env['payment.provider'].search([('code', '=', 'ogone')], limit=1)
        if ogone_provider:
            # Reassign tokens from Ogone to Worldline
            tokens = self.env['payment.token'].search([('provider_id', '=', ogone_provider.id)])
            if tokens:
                tokens.write({'provider_id': worldline_provider.id})
                _logger.info("Transferred %d token(s) from Ogone to Worldline", len(tokens))

            ogone_provider.write({'state': 'disabled'})
            # Remove the Ogone payment method
            self.env['account.payment.method'].search([('code', '=', 'ogone')]).unlink()
