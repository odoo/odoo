...
export class PortalPasskeyCreate extends Interaction {
    ...
    async onClick() {
        this.services.dialog.add(InputConfirmationDialog, {
            ...
            confirm: async ({ inputEl }) => {
                ...
                if(name.length > 0) {
                    const create_action = await this.waitFor(handleCheckIdentity(
                        this.waitFor(this.services.orm.call("res.users", "action_create_passkey", [ user.userId ])),
                        this.services.orm,
    ...
    async createPasskey(serverOptions, name) {
        ...
        const [new_key] = await this.waitFor(this.services.orm.create("auth.passkey.key.create", [{ name }]));
        await this.waitFor(handleCheckIdentity(
            this.waitFor(this.services.orm.call("auth.passkey.key.create", "make_key", [new_key, registration])),
            this.services.orm,
            this.services.dialog,
        ));
        location.reload();
