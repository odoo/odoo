...
export const CogMenuItem = {
    ...
    isDisplayed: async ({ config, searchModel }) => {
        const current_company_id = searchModel.env.services.company.currentCompany.id
        const data = await searchModel.orm.read("res.company", [current_company_id], ["country_code"]);
        return (
