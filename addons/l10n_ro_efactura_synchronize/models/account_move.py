...
class AccountMove(models.Model):
    ...
    def _compute_l10n_ro_edi_index(self):
        for move in self:
            if move.country_code == 'RO':
                documents = move.l10n_ro_edi_document_ids \
                                .filtered(lambda doc: bool(doc.key_loading)) \
                                .sorted()
                move.l10n_ro_edi_index = (documents and documents[0].key_loading) or move.l10n_ro_edi_index or False
            else:
                move.l10n_ro_edi_index = False
