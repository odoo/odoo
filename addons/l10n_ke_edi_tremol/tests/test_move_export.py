...
class TestKeMoveExport(AccountTestInvoicingCommon):
    ...
    def test_export_wire_tax_rate_remap(self):
        """The stored tax_rate is remapped to the VAT class byte expected by KRA."""
        ...
        stored_rates = [case[0] for case in cases if case[0]]
        ...
        taxes = self.env['account.tax'].create([
            ...
            for rate, amount, *_values in cases
        ])
        ...
        for case, generated, expected in zip(cases, generated_lines, expected_lines):
            with self.subTest(stored_tax_rate=case[0]):
