...
class HrLeave(models.Model):
    ...
    def _l10n_in_apply_sandwich_rule(self, leave_days, public_holidays, employee_leaves):
        ...
        def find_linked_leave(self, start_date, step):
            current = start_date + timedelta(days=step)
            while is_non_working_day(current):
                current += timedelta(days=step)
            linked_leave = leaves_by_date.get(current)
            if linked_leave:
                if linked_leave.linked_sandwich_leave_id:
                    return None
                else:
                    start_edge = linked_leave.date_from.date()
                    end_edge = linked_leave.date_to.date()
                    if start_edge in holiday_dates or end_edge in holiday_dates:
                        linked_leave.has_edge_public_leave = True
                    else :
                        linked_leave.has_edge_public_leave = False
                    return linked_leave
            return None
        ...
        linked_before = find_linked_leave(self, date_from, -1)
        linked_after = find_linked_leave(self, date_to, 1)
