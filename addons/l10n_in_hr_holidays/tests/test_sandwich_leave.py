...
class TestSandwichLeave(TransactionCase):
    ...
    def test_sandwich_leave(self):
        with freeze_time('2023-08-15'):
            ...
            leave = before_holiday_leave._l10n_in_apply_sandwich_rule(
                before_holiday_leave.number_of_days, public_holiday, employee_leaves)
            self.assertEqual(leave, 1, "The total leaves should be 1")
            sandwiched_leave = after_holiday_leave._l10n_in_apply_sandwich_rule(
                after_holiday_leave.number_of_days, public_holiday, employee_leaves)
