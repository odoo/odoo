...
class TestMailSchedule(EventMailCommon):
    ...
    @mute_logger('odoo.addons.base.models.ir_model', 'odoo.models')
    @users('user_eventmanager')
    def test_event_mail_schedule_on_cancelled_or_ended_event(self):
        """ Test mail scheduling for following events:
            - Event in Ended Stage
            - Cancelled Event
        """
        ...
        # event in ended stage
        test_event.stage_id = self.env.ref('event.event_stage_done')
        self.assertEqual(test_event.event_mail_ids.mail_state, 'cancelled', 'Mail state should be cancelled')
        self.execute_event_cron(freeze_date=now_start)
        self.assertEqual(len(self._new_mails), 0, 'No mail should be sent for ended event')
        # event moved into announced stage
