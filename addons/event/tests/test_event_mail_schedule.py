...
class EventMailCommon(EventCase, MailCase, CronMixinCase):
    ...
    def setUpClass(cls):
        ...
        with cls.mock_datetime_and_now(cls, cls.reference_now):
            # create with admin to force create_date and user tz
...
class TestMailSchedule(EventMailCommon):
    ...
    def test_event_mail_schedule_on_slot(self):
        ...
        registrations_slot_1 = registrations.filtered(lambda r: r.event_slot_id == test_event.slot_ids[0])
        _registrations_slot_2 = registrations.filtered(lambda r: r.event_slot_id == test_event.slot_ids[1])
