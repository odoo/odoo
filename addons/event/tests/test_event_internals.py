...
class TestEventData(TestEventInternalsCommon):
    ...
    def test_event_slots_w_tickets_seats(self):
        ...
        for _ in range(5):
            ...
            slot1_registrations_data.extend(((slot1, slot_ticket1), (slot1, slot_ticket3)))
        for _ in range(10):
            # For slot2: 10 standard and 10 VIP tickets (= max of VIP tickets)
            slot2_registrations_data.extend(((slot2, slot_ticket2), (slot2, slot_ticket4)))
        ...
        self.assertEqual(slot_ticket2.seats_available, min(slot_ticket2.event_id.seats_max, slot_ticket2.seats_max) - 10)
        self.assertEqual(slot_ticket3.seats_available, min(slot_ticket3.event_id.seats_max, slot_ticket3.seats_max) - 5)
        ...
        # Check seats when archiving 1 registration to slot_ticket1 and 2 to slot_ticket2
        seats_expected_after_archiving = [
            ...
            (slot_ticket2, 'seats_available', slot_ticket2.seats_available + 2),
            (slot_ticket2, 'seats_reserved', slot_ticket2.seats_reserved - 2),
            (slot_ticket2, 'seats_taken', slot_ticket2.seats_taken - 2),
            (slot_ticket2, 'seats_used', slot_ticket2.seats_used),
        ]
        ...
        self.assertEqual(slot2_registrations[0].event_ticket_id, slot_ticket2)
        self.assertEqual(slot2_registrations[2].event_ticket_id, slot_ticket2)
        ...
        seats_expected_after_unarchiving = [
            ...
            (slot_ticket1, 'seats_taken', slot_ticket1.seats_taken + 1),
            (slot_ticket1, 'seats_used', slot_ticket1.seats_used),
            (slot_ticket2, 'seats_available', slot_ticket2.seats_available),
            (slot_ticket2, 'seats_reserved', slot_ticket2.seats_reserved),
            (slot_ticket2, 'seats_taken', slot_ticket2.seats_taken),
            (slot_ticket2, 'seats_used', slot_ticket2.seats_used),
        ]
