...
class EventQuestion(models.Model):
    ...
    event_type_ids = fields.Many2many('event.type', string='Event Type', index='btree_not_null')
    event_ids = fields.Many2many('event.event', string='Event', index='btree_not_null')
    ...
    def action_view_question_answers(self):
        ...
        # Fetch only registration answers for which the event and the value_answer_id are linked to
        # the question.
        action['domain'] = [
            ('event_id.question_ids', 'in', self.id), ('value_answer_id.question_id', 'in', self.id)
        ]
