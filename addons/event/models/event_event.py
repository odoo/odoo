...
class EventEvent(models.Model):
    ...
    def write(self, vals):
        ...
        # Delete the event registration answers with the removed questions.
        if vals.get('question_ids'):
            removed_question_ids = [question for command, question in vals.get('question_ids') if command == 3]
            self.registration_ids.registration_answer_ids.filtered(lambda a: a.question_id.id in removed_question_ids).unlink()
