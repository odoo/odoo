...
class EventEvent(models.Model):
    ...
    def _compute_question_ids(self):
        ...
        """
        for event in self:
            # Can only add default questions if the event has not yet been created.
            ...
            if self._origin.question_ids:
                # Keep questions with ettendee answers for the event.
                questions_tokeep_ids = event.registration_ids.registration_answer_ids.filtered(
                    lambda registration_answer: registration_answer.question_id in self._origin.question_ids
                ).question_id.ids
            else:
