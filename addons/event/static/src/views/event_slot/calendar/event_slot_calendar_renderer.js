...
export class EventSlotCalendarCommonRenderer extends CalendarCommonRenderer {
    ...
    getDayCellClassNames(info) {
        ...
        // Event datetime range format: "yyyy-mm-dd HH:mm:ss"
        const rangeStartDate = this.props.model.data.event?.start?.split(" ")[0];
        const rangeEndDate = this.props.model.data.event?.end?.split(" ")[0];
        if (
            rangeStartDate &&
            rangeEndDate &&
