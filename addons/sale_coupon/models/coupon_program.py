...
class CouponProgram(models.Model):
    ...
    def _filter_programs_from_common_rules(self, order, next_order=False):
        ...
        # adding the current program applied to the list if it is the last order. this is because the order is set as
        # invalid before since its last use was consumed.
        if (order.code_promo_program_id.promo_applicability == 'on_current_order'
                and order.code_promo_program_id.order_count ==  order.code_promo_program_id.maximum_use_number):
            programs += order.code_promo_program_id
