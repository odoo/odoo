...
/**
 * Migrate the global filters to the new format.
 * - Remove the type
 * - Add the operator
 * - Compute and add the target. The target is set to the first field matching
 *   that is linked to a direct field (not a chain)
 */
function migrate13to14(data) {
    for (const globalFilter of data.globalFilters || []) {
        if (globalFilter.type === "relation") {
            ...
            if (
                globalFilter.relation === "res.users" &&
                globalFilter.defaultValue === "current_user"
            ) {
                globalFilter.defaultValue = ["uid"];
            }
            delete globalFilter.modelName;
            delete globalFilter.includeChildren;
            delete globalFilter.defaultValueDisplayNames;
            const fieldMatching = findFieldMatching(data, globalFilter.id);
            globalFilter.target = fieldMatching
                ? {
                      model: fieldMatching.model,
                      field: fieldMatching.chain,
                  }
                : {
        ...
        if (globalFilter.type === "text") {
            ...
            const fieldMatching = findFieldMatching(data, globalFilter.id);
            globalFilter.target = fieldMatching
                ? { model: fieldMatching.model, field: fieldMatching.chain }
                : undefined;
            if (!globalFilter.target) {
                globalFilter.parameterType = "text";
            }
        }
        if (globalFilter.type === "date") {
            ...
            globalFilter.target = fieldMatching
                ...
                : undefined;
            if (!globalFilter.target) {
                globalFilter.parameterType = "date";
            }
        }
...
function findFieldMatching(data, filterId) {
    // pivots
    for (const pivot of Object.values(data.pivots || {})) {
        if (pivot.fieldMatching && filterId in pivot.fieldMatching) {
            if (pivot.fieldMatching[filterId].chain?.split(".").length === 1) {
                return { ...pivot.fieldMatching[filterId], model: pivot.model };
            }
        }
    }
    // lists
    for (const list of Object.values(data.lists || {})) {
        if (list.fieldMatching && filterId in list.fieldMatching) {
            if (list.fieldMatching[filterId].chain?.split(".").length === 1) {
                return { ...list.fieldMatching[filterId], model: list.model };
            }
        }
    }
    // charts
    for (const figure of Object.values(data.figures || {})) {
        if (
            ...
            figure.data.fieldMatching[filterId].chain?.split(".").length === 1
        ) {
            return {
