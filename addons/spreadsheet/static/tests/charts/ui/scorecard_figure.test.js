 ...
 * ensuring that y=0 always falls in the key region, not the title region.
 */
...
async function clickCanvas(fixture, region) {
    ...
    const position = region === "baseline" ? "bottom" : "top";
    await click(canvas, { position });
}
...
test("clicking scorecard in dashboard mode navigates to odoo link", async function () {
    ...
    await animationFrame();
    // y=0 → key area; A1 has no list formula so falls back to odoo link
    await clickCanvas(fixture, "key");
...
test("clicking key area opens seeRecord when key cell has a list record", async function () {
    ...
    // createSpreadsheetWithList inserts the list at A1:D11.
    // A2 is the first data row and has an ODOO.LIST.VALUE formula.
...
test("clicking baseline area opens seeRecord when baseline cell has a list record", async function () {
    ...
    const { model } = await createSpreadsheetWithList();
    const fixture = await mountSpreadsheet(model);
    // A1 is the list header (ODOO.LIST, not visible for seeRecord).
    // A2 is a data row (ODOO.LIST.VALUE, visible for seeRecord).
    createScorecardChart(model, { keyValue: "A1", baseline: "A2" });
    model.updateMode("dashboard");
    await animationFrame();
