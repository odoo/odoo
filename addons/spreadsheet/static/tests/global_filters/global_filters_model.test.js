...
test("Can add a selection filter", async () => {
    ...
    model.dispatch("ADD_GLOBAL_FILTER", { filter });
    model.dispatch("SET_GLOBAL_FILTER_VALUE", { id: "42", value: ["after"] });
...
test("Check selection filter domain", async () => {
    ...
    model.dispatch("SET_GLOBAL_FILTER_VALUE", { id: "42", value: ["before"] });
    expect(model.getters.getGlobalFilterDomain("42", fieldMatching).toString()).toEqual(
        `[("position", "in", ["before"])]`
    );
    model.dispatch("SET_GLOBAL_FILTER_VALUE", { id: "42", value: ["after", "before"] });
    expect(model.getters.getGlobalFilterDomain("42", fieldMatching).toString()).toEqual(
