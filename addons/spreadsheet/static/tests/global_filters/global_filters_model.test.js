...
test("Can delete a global filter", async function () {
    ...
    expect(result).toEqual(DispatchResult.Success);
    expect(model.getters.getGlobalFilters().length).toBe(0);
    const computedDomain = model.getters.getPivotComputedDomain("PIVOT#1");
    expect(computedDomain).toEqual([]);
});
...
test("Can edit a global filter", async function () {
    ...
    expect(result).toEqual(DispatchResult.Success);
    expect(model.getters.getGlobalFilters().length).toBe(1);
    expect(model.getters.getGlobalFilters()[0].defaultValue.yearOffset).toEqual(0);
});
...
test("Adding new DataSource with a different model won't set up its field matching", async function () {
    ...
    expect(fieldMatching).toEqual(undefined);
});
test("Can save a value to an existing global filter", async function () {
    ...
    expect(result).toEqual(DispatchResult.Success);
    expect(model.getters.getGlobalFilters().length).toBe(1);
    expect(model.getters.getGlobalFilterDefaultValue(gf.id).yearOffset).toEqual(-1);
    expect(model.getters.getGlobalFilterValue(gf.id).period).toEqual("february");
    expect(model.getters.getGlobalFilterValue(gf.id).yearOffset).toEqual(0);
    result = await setGlobalFilterValue(model, {
        id: gf.id,
        value: { period: "march", yearOffset: 0 },
    });
    expect(result).toEqual(DispatchResult.Success);
    expect(model.getters.getGlobalFilterValue(gf.id).period).toEqual("march");
    expect(model.getters.getGlobalFilterValue(gf.id).yearOffset).toEqual(0);
    const computedDomain = model.getters.getPivotComputedDomain("PIVOT#1");
    expect(computedDomain.length).toBe(3);
...
test("Can import/export filters", async function () {
    ...
    expect(filter1.defaultValue.yearOffset).toEqual(-1);
    expect(model.getters.getGlobalFilterValue(filter1.id).yearOffset).toEqual(-1, {
        message: "it should have applied the default value",
    });
    expect(filter2.defaultValue.yearOffset).toEqual(-1);
    expect(model.getters.getGlobalFilterValue(filter2.id).yearOffset).toEqual(-1, {
        message: "it should have applied the default value",
    });
    ...
    expect(filter1.defaultValue.yearOffset).toEqual(-1);
    expect(newModel.getters.getGlobalFilterValue(filter1.id).yearOffset).toEqual(-1, {
        message: "it should have applied the default value",
    });
    expect(filter2.defaultValue.yearOffset).toEqual(-1);
    expect(newModel.getters.getGlobalFilterValue(filter2.id).yearOffset).toEqual(-1, {
        message: "it should have applied the default value",
    });
...
test("Can undo-redo a MOVE_GLOBAL_FILTER", async function () {
    ...
    expect(model.getters.getGlobalFilters()[1].id).toEqual(lastYearFilterId);

    model.dispatch("REQUEST_UNDO");
    expect(model.getters.getGlobalFilters()[0].id).toEqual(lastYearFilterId);
...
test("Can set a value to a date filter from the SET_MANY_GLOBAL_FILTER_VALUE command", async function () {
    ...
    expect(model.getters.getGlobalFilterValue("42")).toEqual(undefined);
});
...
test("can move a global filter", function () {
    ...
    moveGlobalFilter(model, lastYearFilterId, 1);
    expect(model.getters.getGlobalFilters()[2].id).toEqual(lastYearFilterId);
