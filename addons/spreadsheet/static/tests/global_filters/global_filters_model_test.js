...
QUnit.module("spreadsheet > Global filters model", {}, () => {
    ...
    QUnit.test("getFiltersMatchingPivot works with date=false", async function (assert) {
        ...
        const matchingYearForYearGranularity = model.getters.getFiltersMatchingPivot(
            '=ODOO.PIVOT.HEADER(1,"expected_revenue","date:year",2024)'
        );
        assert.deepEqual(matchingYearForYearGranularity, [
            { filterId: "43", value: { yearOffset: 0 } },
        ]);

        const matchingYearForMonthGranularity = model.getters.getFiltersMatchingPivot(
            '=ODOO.PIVOT.HEADER(1,"expected_revenue","date:month",2024)'
        );
        assert.deepEqual(matchingYearForMonthGranularity, [
            { filterId: "43", value: { period: "january", yearOffset: 0 } },
        ]);

        const matchingYearForQuarterGranularity = model.getters.getFiltersMatchingPivot(
            '=ODOO.PIVOT.HEADER(1,"expected_revenue","date:quarter",2024)'
        );
        assert.deepEqual(matchingYearForQuarterGranularity, [
            { filterId: "43", value: { period: "first_quarter", yearOffset: 0 } },
        ]);
        ...
        assert.deepEqual(errorStrings, [
            { filterId: "43", value: { period: undefined, yearOffset: 0 } },
        ]);

        const emptyString = model.getters.getFiltersMatchingPivot(
            '=ODOO.PIVOT.HEADER(1,"expected_revenue","date:quarter","")'
        );
