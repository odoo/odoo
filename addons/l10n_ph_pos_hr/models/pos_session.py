...
class PosSession(models.Model):
    ...
    def _l10n_ph_get_audit_approver(self, payload, passcode):
        ...
        if approver_id:
            ...
            # The offline-sync replay (pos_order._l10n_ph_process_pending_audit_actions)
            # calls this sudo'd and never carries a passcode: the approver was already
            # PIN-checked client-side when the action was queued. Any non-sudo (live)
            # call must still prove the passcode, or a forged approver_id would let a
            # cashier self-approve a void as someone else without their PIN.
            if not self.env.is_superuser() and approver.pin != passcode:
    ...
    def l10n_ph_log_order_line_action(self, payload):
        ...
        if action_type == "line_void":
            self.config_id.l10n_ph_void_counter = (
                self.config_id.l10n_ph_void_counter or 0
            ) + 1
            void_counter = self.config_id.l10n_ph_void_counter
        return {
