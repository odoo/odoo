    ...
    def _get_sequence_values(self, name=False, code=False):
        ...
        values.update({
            'subcontracting_type_id': {
                ...
                'prefix': self.code + '/' + (self._extract_sequence_code(self.subcontracting_type_id.sequence_code) or (('SBC' + str(count) + '/') if count else 'SBC/')),
                'padding': 5,
                'company_id': self.company_id.id
            },
            'subcontracting_resupply_type_id': {
                'name': _('%(name)s Sequence Resupply Subcontractor', name=self.name),
                'prefix': self.code + '/' + (self._extract_sequence_code(self.subcontracting_resupply_type_id.sequence_code) or (('RES' + str(count) + '/') if count else 'RES/')),
                'padding': 5,
                'company_id': self.company_id.id
            },
        })
