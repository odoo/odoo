...
class MrpBom(models.Model):
    ...
    def _compute_bom_cost(self, product, boms_to_recompute=False):
        ...
        if self.type == 'subcontract':
            ...
            seller_info = product.with_company(company)._select_seller(
                quantity=self.product_qty,
                uom_id=self.uom_id,
                params={'subcontractor_ids': self.subcontractor_ids},
            )
            if seller_info:
                seller_price = seller_info['currency_id']._convert(seller_info['price'], company.currency_id, company)
                cost += seller_info['uom_id']._compute_price(seller_price, self.product_tmpl_id.uom_id)
        return cost
