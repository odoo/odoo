...
class TestL10nPkEdiPos(CommonPosTest):
    ...
    def _buyer_payload(self, partner):
        order = self._create_order(
            [{"product_id": self.pk_product.id, "qty": 1}], order_data={"partner_id": partner.id}
        )
        return order._l10n_pk_edi_pos_generate_json()
    ...
    def test_generate_json_payload(self):
        ...
        # A buyer is either a registered business, identified by its NTN, or a consumer, identified
        # by its CNIC; holding both is rejected by `_check_identifier_combination`.
        business = self.env["res.partner"].create(
            {"name": "PK Business", "phone": "+92 300 1234567", "country_id": self.env.ref("base.pk").id, "vat": "4174942"}
        )
        payload = self._buyer_payload(business)
        self.assertEqual(payload["BuyerName"], "PK Business")
        self.assertEqual(payload["BuyerPhoneNumber"], "+92 300 1234567")
        self.assertEqual(payload["BuyerNTN"], "4174942")
        self.assertEqual(payload["BuyerCNIC"], "")

        consumer = self.env["res.partner"].create({"name": "PK Consumer", "country_id": self.env.ref("base.pk").id})
        consumer._set_additional_identifier("PK_CN", "4210112345678")
        payload = self._buyer_payload(consumer)
        self.assertEqual(payload["BuyerName"], "PK Consumer")
        self.assertEqual(payload["BuyerNTN"], "")
    ...
    def test_bad_data_never_reaches_the_fbr(self):
        ...
        for expected in ("FBR Shop ID", "FBR Payment Code", "negative prices", "HS Code", "Internal Reference"):
    ...
    def test_order_without_payment_is_reported_as_cash(self):
        """A new shop reports cash out of the box, and a zero-total order closed without any
        payment line is reported as cash too rather than blocked forever."""
        cash = self.env["pos.config"]._create_cash_payment_method()
        self.assertEqual(cash.l10n_pk_edi_pos_fbr_payment_code, "1")

        # No payment_data, so no pos.payment is ever created for this order.
        order, _refund = self.create_backend_pos_order({
            "pos_config": self.pos_config_usd,
            "line_data": [{"product_id": self.pk_product.id, "qty": 1, "price_unit": 0.0}],
        })
        self.assertFalse(order.payment_ids)
        self.assertFalse(order.amount_total)
        self.assertFalse(order._l10n_pk_edi_pos_check_data())
        self.assertEqual(order._l10n_pk_edi_pos_generate_json()["PaymentMode"], 1)
...
class TestL10nPkEdiPosTours(TestPointOfSaleHttpCommon):
    ...
    def test_fbr_service_fee_line_added(self):
        self.main_pos_config.write({"l10n_pk_edi_pos_enabled": True, "l10n_pk_edi_pos_charge_service_fee": True})
        self.main_pos_config.with_user(self.pos_user).open_ui()
    ...
    def test_fbr_service_fee_product_is_configurable(self):
        ...
        self.main_pos_config.write(
            {
                "l10n_pk_edi_pos_enabled": True,
                "l10n_pk_edi_pos_charge_service_fee": True,
                "l10n_pk_edi_pos_service_fee_product_id": fee_product.id,
            }
        )
        self.main_pos_config.with_user(self.pos_user).open_ui()
