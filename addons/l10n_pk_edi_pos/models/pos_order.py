...
class PosOrder(models.Model):
    ...
    def _l10n_pk_edi_pos_connect_to_server(self, mode, auth_token, payload):
    ...
    def _l10n_pk_edi_pos_reported_lines(self):
        ...
        return lines.filtered(lambda ln: ln.qty)
    ...
    def _l10n_pk_edi_pos_check_data(self):
        ...
        # The Global Discount button is already hidden from the cashier, so this only
        # catches a price typed in by hand or an order encoded from the back end.
        if negative_price := lines.filtered(lambda ln: ln.price_unit < 0):
            errors.append(self.env._(
                "The FBR does not accept negative prices. Remove these lines: %s.",
    ...
    def _l10n_pk_edi_pos_payment_mode(self):
        ...
        # a sale that was not paid, so it is reported as cash.
        if not codes:
            return 1
    ...
    def _l10n_pk_edi_pos_line_details(self):
        ...
        for base_line in base_lines:
            ...
            # The FBR reports the further tax in its own field, so it is kept apart from the sales tax.
            ...
            # SaleValue is the price before any discount, which no tax detail holds, so the line is
            # priced a second time without its discount. A 3rd Schedule product is taxed on its
            # printed retail price whatever the shop charges, so that one starts from the sales price.
            ...
            # TaxRate must match the base the FBR recomputes the tax on, which for a 3rd Schedule
            # product is that undiscounted retail base rather than what was actually charged.
            ...
            # 1 and 3 are a sale and a refund; 11 and 12 are their 3rd Schedule counterparts.
            if is_refund:
            ...
            items.append({
                ...
                # The FBR reads the discount as the gap between that base and what is charged.
    ...
    def _l10n_pk_edi_pos_send(self):
        ...
        try:
            ...
            response = self._l10n_pk_edi_pos_connect_to_server(mode, auth_token, payload)
        ...
        if error_message:
            ...
            self._l10n_pk_edi_pos_mark_successful(response, mode, payload)

    def _l10n_pk_edi_pos_mark_successful(self, response, mode, payload):
        self.ensure_one()
        self.write({
