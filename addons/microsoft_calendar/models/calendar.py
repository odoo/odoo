...
class CalendarEvent(models.Model):
    ...
    def _odoo_attendee_commands_m(self, microsoft_event):
        ...
        additional_values = {
            email_normalize(a['emailAddress']['address']): {'name': a['emailAddress']['name']}
            for a in microsoft_attendees
            if email_normalize(a.get('emailAddress', {}).get('address')) and a.get('emailAddress', {}).get('name')
        }
        partners = self.env['mail.thread']._partner_find_from_emails_single(emails, no_create=False, additional_values=additional_values)
