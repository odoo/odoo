...
class MicrosoftSync(models.AbstractModel):
    ...
    # this event ID is provided by the Microsoft Graph API and identifies the event in the
    # organizer's calendar (used to sync from Odoo to Microsoft as updates on the organizer Microsoft event
    # is then dispatched on attendees Microsoft events)
    ms_organizer_event_id = fields.Char('Microsoft Organizer Event Id')
    ...
    def _sync_recurrence_microsoft2odoo(self, microsoft_events: MicrosoftEvent):
        ...
        for recurrent_master in recurrent_masters:
            ...
            to_create = recurrents.filter(lambda e: e.seriesMasterId == new_calendar_recurrence['ms_organizer_event_id'])
    ...
    def _restart_microsoft_sync(self):
        """ Turns on the microsoft synchronization for all the events of a given user. """
        raise NotImplementedError()
