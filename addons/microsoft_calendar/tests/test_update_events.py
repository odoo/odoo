...
class TestUpdateEvents(TestCommon):
    ...
    def test_update_attendee_of_exception_does_not_recreate_recurrence_events(self, mock_get_events):
        ...
        recurrence = self.env["calendar.recurrence"].search([('ms_organizer_event_id', '=', 'REC123')])
    ...
    @patch.object(MicrosoftSync, '_write_from_microsoft', autospec=True)
    ...
    def test_update_microsoft_recurrence_relinks_orphaned_occurrences_by_timeslot(self):
        ...
        occurrences in Odoo and brings them back with brand-new Outlook ids.
        """
    ...
    def test_update_microsoft_recurrence_relinks_orphaned_allday_occurrences_by_timeslot(self):
        recurrence = self.recurrence
        master_id = recurrence.ms_organizer_event_id
        occurrences = recurrence.calendar_event_ids.sorted('start')

        for i, occurrence in enumerate(occurrences):
            day = (self.start_date + timedelta(days=i * self.recurrent_event_interval)).date()
            occurrence.with_context(dont_notify=True).write({
                'allday': True,
                'start': datetime.combine(day, datetime.min.time()),
                'stop': datetime.combine(day, datetime.min.time()),
                'microsoft_id': False,
                'microsoft_recurrence_master_id': False,
                'need_sync_m': False,
                'follow_recurrence': True,
        ...
        instances = MicrosoftEvent([
            {
                ...
                'type': 'occurrence',
                'start': {
                    'dateTime': occurrence.start.strftime("%Y-%m-%dT%H:%M:%S.0000000"),
                    'timeZone': 'UTC',
                },
                'end': {
                    'dateTime': (occurrence.stop + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S.0000000"),
                    'timeZone': 'UTC',
                },
                'isAllDay': True,
            }
        ...
        for occurrence in recurrence.calendar_event_ids:
            self.assertTrue(occurrence.microsoft_id, "the all-day occurrence should be re-linked")
            self.assertEqual(occurrence.microsoft_recurrence_master_id, master_id)
