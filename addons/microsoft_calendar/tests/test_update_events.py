...
class TestUpdateEvents(TestCommon):
    ...
    def test_remove_attendee_of_recurrence_from_outlook_organizer_calendar(self, mock_get_events):
        ...
        # arrange
        attendee_partner = self.attendee_user.partner_id
        organizer_partner = self.organizer_user.partner_id
        recurrence_events = self.recurrence.calendar_event_ids
        # the recurrence fixture only has the attendee as partner; add the organizer
        # so we can assert it is preserved when the attendee is removed on Outlook
        recurrence_events.with_context(dont_notify=True).write({'partner_ids': [(4, organizer_partner.id)]})
        ...
        for event in recurrence_events:
            ...
            self.assertIn(
                self.organizer_user.partner_id,
                event.partner_ids,
                "The organizer must be kept on every event",
            )
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_update_start_later_of_all_events_of_recurrence_removes_attendee(self, mock_get_events):
        """ Move the whole recurrence later in Outlook while also removing an attendee. """

        # arrange
        # the recurrence fixture only has the attendee as partner; add the organizer
        # so we can assert it is preserved through the recurrence recreation
        self.recurrence.calendar_event_ids.with_context(dont_notify=True).write(
            {'partner_ids': [(4, self.organizer_user.partner_id.id)]}
        )
        events = self._prepare_outlook_events_for_all_events_start_date_update(
            self.recurrent_events_count, new_start_date=datetime(2021, 9, 23, 10, 0, 0),
        )
        events = [dict(e, attendees=[]) for e in events]
        mock_get_events.return_value = (MicrosoftEvent(events), None)
        ...
        for event in recurrence.calendar_event_ids:
            self.assertNotIn(
                ...
                event.partner_ids,
                "The attendee removed from the series on Outlook must be removed from every recreated event",
            )
            self.assertNotIn(
                self.attendee_user.partner_id,
                event.attendee_ids.partner_id,
                "The attendee record must be removed from every recreated event too",
            )
            self.assertIn(
                self.organizer_user.partner_id,
                event.partner_ids,
