...
class TestDeleteEvents(TestCommon):
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_delete_one_event_and_future_from_recurrence_from_odoo_calendar(self, mock_delete):

        # arrange
        idx = 4
        events_to_remove = self.recurrent_events[idx:]
        events_to_remove_ids = [e.ms_organizer_event_id for e in events_to_remove]
        events_to_remove_count = len(events_to_remove)

        # act
        self.recurrent_events[idx].with_user(self.organizer_user).action_mass_deletion('future_events')
        self.call_post_commit_hooks()

        # assert
        self.assertFalse(events_to_remove.exists())
        self.assertEqual(len(self.recurrence.calendar_event_ids), self.recurrent_events_count - events_to_remove_count)

        self.assertEqual(mock_delete.call_count, events_to_remove_count)
        for eid in events_to_remove_ids:
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_delete_first_event_and_future_from_recurrence_from_odoo_calendar(self, mock_delete):
        # arrange
        idx = 0
        ...
        self.assertEqual(mock_delete.call_count, events_to_remove_count + 1)
    ...
    @patch.object(MicrosoftCalendarService, 'delete')
    def test_delete_all_events_from_recurrence_from_odoo_calendar(self, mock_delete):
        ...
        # act
        self.recurrent_events[idx].with_user(self.organizer_user).action_mass_deletion('all_events')
        self.call_post_commit_hooks()
