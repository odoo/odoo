...
class StockMove(models.Model):
    ...
    transfer_partner = fields.Char(compute="_compute_transfer_partner", store=True)

    @api.depends('picking_id', 'picking_id.partner_id')
    def _compute_transfer_partner(self):
        for move in self:
            if move.picking_id and move.picking_id.partner_id:
                move.transfer_partner = f"{move.picking_id.name} - {move.picking_id.partner_id.name}"
            else:
                move.transfer_partner = f"{move.picking_id.name}"
