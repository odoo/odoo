...
class StockAddToWave(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if len(picking_types) > 1:
            raise UserError(self.env._("The selected transfers should belong to the same operation type"))
        return res
    ...
    def attach_pickings(self):
        ...
        if self.line_ids:
            ...
            if len(company) > 1:
                raise UserError(self.env._("The selected operations should belong to a unique company."))
            return self.line_ids._add_to_wave(self.wave_id)
        if self.picking_ids:
            company = self.picking_ids.company_id
            if len(company) > 1:
                raise UserError(self.env._("The selected transfers should belong to a unique company."))
        else:
