...
def migrate(cr, version):
    ...
    env["mrp_subcontracting_dropshipping.config"]._for_each(companies)
    env.flush_all()
    assignments = ", ".join(f"{column} = c.{column}" for column in present)
    cr.execute(
        f"UPDATE mrp_subcontracting_dropshipping_config x SET {assignments} FROM res_company c WHERE c.id = x.company_id"
    )
    env.invalidate_all()
