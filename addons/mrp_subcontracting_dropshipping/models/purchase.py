    ...
    def _get_destination_location(self):
        ...
        if self.dest_address_id:
            if self.mrp_production_count:
                mrp_production_ids = self._get_mrp_productions(True)
            ...
            elif not self.sale_order_count:
                in_bom_products = False
                not_in_bom_products = False
                for order_line in self.order_line:
                    if any(bom_line.bom_id.type == 'subcontract' and self.dest_address_id in bom_line.bom_id.subcontractor_ids for bom_line in order_line.product_id.bom_line_ids):
