...
class AccountChartTemplate(models.Model):
    ...
    def load_for_current_company(self, sale_tax_rate, purchase_tax_rate):
        ...
        if self.env['res.company']._get_current_company().country_id.code == 'NL':
            account = self.env['account.account'].search([('code', '=', '999999'), ('company_id', '=', self.env['res.company']._get_current_company().id)])
            if account:
