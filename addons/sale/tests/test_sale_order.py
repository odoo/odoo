...
class TestSaleOrder(SaleCommon):
    ...
    def test_computes_auto_fill(self):
        ...
        self.env["sale.order.line"].create([
            ...
            {"display_type": "line_section", "name": "Dummy section", "order_id": order.id},
            {"product_id": free_product.id, "order_id": order.id},
            {"product_id": dummy_product.id, "order_id": order.id},
        ])
    ...
    def test_sale_order_send_to_self(self):
        ...
        sale_order = (
            ...
            .create({"partner_id": self.sale_user.partner_id.id})
        )
        ...
        self.assertEqual(
            ...
            'Sale: author should be in composer recipients thanks to "partner_to" field set on'
            " template",
        )
        self.assertEqual(
            mail_message.partner_ids,
    ...
    def test_unlink_cancel(self):
        """Test deleting and cancelling sales orders depending on their state and on the user's
        rights."""
    ...
    def test_zero_quantity(self):
        """Test that changing the uom do not change the quantity, even if 0."""
        order_line = self.sale_order.order_line[0]
    ...
    def test_discount_rounding(self):
        """Check the discount is properly rounded and the price subtotal computed with this rounded
        discount."""
    ...
    def test_sale_warnings(self):
        ...
        self.env["sale.order.line"].create([
            ...
            {"order_id": sale_order.id, "product_id": product_with_warning1.id},
            {"order_id": sale_order2.id, "product_id": product_with_warning1.id},
            {"order_id": sale_order2.id, "product_id": product_with_warning2.id},
            # Warnings for duplicate products should not appear.
            {"order_id": sale_order2.id, "product_id": product_with_warning1.id},
        ])
    ...
    def test_sale_order_unit_price_recompute_on_product_change(self):
        ...
        """
        product2 = self.env["product.product"].create({"name": "Test Product2", "list_price": 0.0})
...
class TestSalesTeam(SaleCommon):
    ...
    def setUpClass(cls):
        ...
        # set up users
        cls.sale_team_2 = cls.env["crm.team"].create({"name": "Test Sales Team (2)"})
    ...
    def test_sale_order_analytic_distribution_change(self):
        ...
        so_no_analytic_account = self.env["sale.order"].create({"partner_id": partner.id})
    ...
    def test_qty_delivered_on_creation(self):
        """Check that the qty delivered of sol is automatically set to 0.0 when an so is created."""
        sale_order = self.env["sale.order"].create({
            "partner_id": self.partner.id,
