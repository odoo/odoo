...
class TestSaleSections(SaleCommon):
    ...
    def test_sale_order_line_parent_id(self):
        """Check that `parent_id` is assigned correctly:
            - Regular lines without a preceding (sub)section should have no parent.
            - Lines and subsections under a section should have the section as their parent.
            - Lines under a subsection should have the subsection as their parent.
        """
        self.assertFalse(self.sections_sale_order.order_line[0].parent_id)
        self.assertEqual(self.sections_sale_order.order_line[2].parent_id, self.sections_sale_order.order_line[1])
        self.assertEqual(self.sections_sale_order.order_line[3].parent_id, self.sections_sale_order.order_line[1])
        self.assertEqual(self.sections_sale_order.order_line[4].parent_id, self.sections_sale_order.order_line[3])
    ...
    def test_sale_order_sections_totals(self):
        self.assertEqual(self.sections_sale_order.order_line[1]._get_section_totals('price_subtotal'), 1200.00)
        self.assertEqual(self.sections_sale_order.order_line[3]._get_section_totals('price_subtotal'), 700.00)
        self.assertEqual(self.sections_sale_order.order_line[7]._get_section_totals('price_subtotal'), 300.00)
