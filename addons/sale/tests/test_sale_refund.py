...
class TestSaleRefund(TestSaleCommon):
    ...
    def setUpClass(cls):
        ...
        (cls.sol_prod_order, cls.sol_serv_deliver, cls.sol_serv_order, cls.sol_prod_deliver) = (
    ...
    def test_refund_create(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                self.assertFalse(
                    line.invoice_lines,
                    "The line based on delivered qty are not invoiced, so they should not be linked"
                    " to invoice line",
                )
            else:
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(
                        line.qty_invoiced,
                        5.0,
                        "The ordered (prod) sale line are totally invoiced (qty invoiced come from"
                        " the invoice lines)",
                    )
                else:
                    self.assertEqual(
                    ...
                    self.assertEqual(
                        line.qty_invoiced,
                        3.0,
                        "The ordered (serv) sale line are totally invoiced (qty invoiced = the"
                        " invoice lines)",
                    )
                self.assertEqual(
                    line.untaxed_amount_to_invoice,
                    line.price_unit * line.qty_to_invoice,
                    "Amount to invoice is now set as qty to invoice * unit price since no price"
                    " change on invoice, for ordered products",
                )
                self.assertEqual(
                    line.untaxed_amount_invoiced,
        ...
        # At this time, the invoice 1 is opend (validated) and its refund is in draft, so the
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                self.assertEqual(
                    line.qty_to_invoice,
                    5.0,
                    "As the refund is created, the invoiced quantity cancel each other (consu ordered)",
                )
                self.assertEqual(
                    line.qty_invoiced,
                    0.0,
                    "The qty to invoice should have decreased as the refund is created for ordered consu SO line",
                )
                self.assertEqual(
                    line.untaxed_amount_to_invoice,
                    0.0,
                    "Amount to invoice is zero as the refund is not validated",
                )
                self.assertEqual(
                    line.untaxed_amount_invoiced,
                    line.price_unit * 5,
                    "Amount invoiced is now set as unit price * ordered qty - refund qty) even if the ",
                )
                self.assertEqual(
                    len(line.invoice_lines),
                    2,
                ...
                self.assertEqual(
                    line.qty_invoiced,
                    0.0,
                    "The qty to invoice should have decreased as the refund is created for ordered service SO line",
                )
                self.assertEqual(
                    line.untaxed_amount_to_invoice,
                    0.0,
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                self.assertEqual(
                    line.qty_to_invoice,
                    2.0,
                    "The qty to invoice does not change when confirming the new invoice (2)",
                )
                self.assertEqual(
                    line.qty_invoiced,
                    3.0,
                    "The ordered (prod) sale line does not change on invoice 2 confirmation",
                )
                self.assertEqual(
                    line.untaxed_amount_to_invoice,
                    line.price_unit * 5,
                ...
                self.assertEqual(
                    line.qty_invoiced,
                    2.0,
                    "The ordered (serv) sale line does not change on invoice 2 confirmation",
                )
                self.assertEqual(
                    line.untaxed_amount_to_invoice,
                    line.price_unit * 3,
                    "Amount to invoice is now set as unit price * ordered qty - refund qty) even if the ",
                )
                self.assertEqual(
                    line.untaxed_amount_invoiced,
                    0.0,
    ...
    def test_refund_invoice_with_downpayment(self):
        ...
        self.assertEqual(
            sol_product.untaxed_amount_to_invoice,
            sol_product.price_unit * 5,
            "Amount to invoice is now set as qty to invoice * unit price since no price change on invoice, as refund is validated",
        )
        self.assertEqual(
            sol_product.untaxed_amount_invoiced,
            0.0,
            "Amount invoiced decreased as the refund is now confirmed",
        )
        self.assertEqual(
            len(sol_product.invoice_lines),
            2,
        ...
        self.assertEqual(
            sol_downpayment.untaxed_amount_to_invoice,
            -(sol_product.price_unit * 5) / 2,
            "Amount to invoice decreased as the refund is now confirmed",
        )
        self.assertEqual(
            sol_downpayment.untaxed_amount_invoiced,
            (sol_product.price_unit * 5) / 2,
            "Amount invoiced is now set as half of all products' total amount to invoice, as refund is validated",
        )
        self.assertEqual(
            len(sol_downpayment.invoice_lines),
            3,
