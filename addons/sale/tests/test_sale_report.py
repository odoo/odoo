...
class TestSaleReportCurrencyRate(TestSaleCommon):
    ...
    def setUpClass(cls):
        ...
        usd = cls.env['res.currency'].with_context(active_test=False).search([('name', '=', 'USD')])
        usd.action_unarchive()
        eur = cls.env['res.currency'].with_context(active_test=False).search([('name', '=', 'EUR')])
        eur.action_unarchive()
        cls.usd_cmp = cls.env['res.company'].create({'name': 'USD Company', 'currency_id': usd.id})
        cls.eur_cmp = cls.env['res.company'].create({'name': 'EUR Company', 'currency_id': eur.id})
    ...
    def test_sale_report_foreign_currency(self):
        ...
        ars_domain = [('name', '=', 'ARS')]
        ars = self.env['res.currency'].with_context(active_test=False).search(ars_domain)

        partner = self.env['res.partner'].create({'name': 'Test partner'})
        product = self.env['product.product'].create({
            'name': 'Test product', 'list_price': 100, 'currency_id': usd.id,
        })
        ...
        for company in companies:
            ...
            for date in (past_day, today):
                for pricelist in pricelists:
                    ...
                    # pricelist.
                    price_for_so_company = product.list_price / expected_product_currency_rate
                    ...
                    print("expected_reported_amount: ", expected_reported_amount)
        ...
        ).search([('name', 'in', [so.name for so in sale_orders])])
