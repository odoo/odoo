...
class TestTaxesDownPaymentSale(TestTaxCommonSale, TestTaxesDownPayment):
    ...
    def test_down_payment_invoice_multiple_taxes_and_accounts(self):
            ...
            order_line=[
                ...
                for name, product_id, price_unit, discount, tax_ids in (
                    ...
                    (
                        ...
                        50.0,
                        tax_15,
                    ),  # These next 2 lines will be merged together because same account, same tax
        ...
        self.assertRecordValues(
            so, [{"amount_untaxed": 14000.0, "amount_tax": 2450.0, "amount_total": 16450.0}]
        )
        ...
        self.assertRecordValues(
            dp_invoice, [{"amount_untaxed": 4200.0, "amount_tax": 735.0, "amount_total": 4935.0}]
        )
    ...
    def test_down_payment_invoice_manual_removing_of_tax(self):
        ...
        self.assertRecordValues(
            so, [{"amount_untaxed": 1000.0, "amount_tax": 150.0, "amount_total": 1150.0}]
        )
        ...
        self.assertRecordValues(
            dp_invoice_1, [{"amount_untaxed": 300.0, "amount_tax": 0.0, "amount_total": 300.0}]
        )
        ...
        self.assertRecordValues(
            dp_invoice_2, [{"amount_untaxed": 200.0, "amount_tax": 30.0, "amount_total": 230.0}]
        )
    ...
    def test_down_payment_invoice_foreign_currency_different_dates(self):
        ...
        self.assertRecordValues(
            so, [{"amount_untaxed": 1200.0, "amount_tax": 180.0, "amount_total": 1380.0}]
        )
        ...
        with freeze_time("2016-01-01"):
            ...
            self.assertRecordValues(
                dp_invoice, [{"amount_untaxed": 360.0, "amount_tax": 54.0, "amount_total": 414.0}]
            )
            self.assertRecordValues(
                dp_invoice.line_ids,
                [
    ...
    def test_down_payment_invoice_then_refunded_then_invoiced_again(self):
        ...
        so = self._create_sale_order_one_line(
            name="line_1", product_id=product, price_unit=1000.0, tax_ids=tax_15
        )
        ...
        dp_invoice_1.action_post()
        self.assertRecordValues(
            dp_invoice_1, [{"amount_untaxed": 300.0, "amount_tax": 45.0, "amount_total": 345.0}]
        )
        ...
        dp_invoice_2.action_post()
        self.assertRecordValues(
            dp_invoice_2, [{"amount_untaxed": 500.0, "amount_tax": 75.0, "amount_total": 575.0}]
        )
    ...
    def test_down_payment_100_first_then_0_final_invoice_round_per_line(self):
        ...
        final_invoice = self._create_final_invoice(so)
        self.assertRecordValues(
            final_invoice, [{"amount_untaxed": 0.0, "amount_tax": 0.0, "amount_total": 0.0}]
        )
        self.assert_invoice_tax_totals_summary(
            final_invoice,
            {
    ...
    def test_down_payment_with_global_discount(self):
        ...
        so = self._create_sale_order_one_line(
            product_id=product, price_unit=14990.00, tax_ids=tax_0
        )
        self.assertRecordValues(
            so, [{"amount_untaxed": 14990.00, "amount_tax": 0.0, "amount_total": 14990.00}]
        )
        ...
        dp_lines = AccountTax._prepare_down_payment_lines(
            ...
            grouping_function=lambda line: {"special_type": line["special_type"]},
        )
    ...
    def test_down_payment_account_prediction(self):
        ...
        new_partner = self.env["res.partner"].create({"name": "Arthur Morgan"})
        ...
        self.assertEqual(
            invoice.invoice_line_ids.account_id, self.company_data["default_account_revenue"]
        )
    ...
    def test_down_payment_invoice_manual_removing_of_tax_affecting_subsequent_taxes(self):
        ...
        self.assertRecordValues(
            so, [{"amount_untaxed": 100.0, "amount_tax": 21.0, "amount_total": 121.0}]
        )

        dp_invoice = self._create_down_payment_invoice(so, "percent", 50)
        self.assertRecordValues(
            dp_invoice, [{"amount_untaxed": 50.0, "amount_tax": 10.5, "amount_total": 60.5}]
        )

        dp_invoice.invoice_line_ids.tax_ids = [Command.set(tax_10_b.ids)]
        self.assertRecordValues(
            dp_invoice, [{"amount_untaxed": 50.0, "amount_tax": 5.0, "amount_total": 55.0}]
        )
