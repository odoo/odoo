    ...
    def test_down_payment_invoice_multiple_taxes_and_accounts(self):
        ...
        so = self._create_sale_order(
            order_line=self._create_line_list_vals([
                ...
                for product_id, price_unit, discount, tax_ids in (
                    (product_1, 2000.0, 50.0, None),  # Standalone line, no tax
                    (product_1, 4000.0, 50.0, tax_15),  # These next 2 lines will be merged together because same account, same tax
                    (product_2, 3000.0, None, tax_15),
                    (product_3, 3000.0, None, tax_15),  # Line linked to the same tax as the ones above but doesn't have the same account
                    (product_1, 5000.0, None, tax_10 + tax_15),  # Multiple taxes on the line. One tax detail will be squashed but not the other
                )
            ])
        )
    ...
    def test_downpayment_invoice_lines_with_down_payment_account(self):
        ...
        sale_order = self._create_sale_order(
            order_line=self._create_line_list_vals([
                ...
                for product, product_uom_qty in (
                    (self.company_data['product_order_no'], 1),
                    (self.company_data['product_order_sales_price'], 2),
                    (self.company_data['product_order_cost'], 3),
                )
            ])
        )
