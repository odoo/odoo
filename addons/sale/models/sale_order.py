...
class SaleOrder(models.Model):
    ...
    def _compute_note(self):
        ...
        for order in self:
            ...
            if order.terms_type == 'html' and order.company_id.invoice_terms_html:
                baseurl = html_keep_url(order._get_note_url() + '/terms')
                context = {'lang': order.partner_id.lang or self.env.user.lang}
                order.note = _('Terms & Conditions: %s', baseurl)
                del context
            elif not is_html_empty(order.company_id.invoice_terms):
                if order.partner_id.lang:
                    order = order.with_context(lang=order.partner_id.lang)
                order.note = order.company_id.invoice_terms

    @api.model
    def _get_note_url(self):
        return self.company_id.get_base_url()
