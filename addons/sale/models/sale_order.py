...
class SaleOrder(models.Model):
    ...
    def _compute_invoice_status(self):
        ...
        if not confirmed_orders:
            return
        line_invoice_status_all = self.env['sale.order.line']._aggregate([
                ('order_id', 'in', confirmed_orders.ids),
                ('is_downpayment', '=', False),
                ('display_type', '=', False),
            ],
            ['invoice_status:array_agg_distinct'],
            ['order_id'])
        for order in confirmed_orders:
            line_invoice_status = line_invoice_status_all.get_agg(order, 'invoice_status:array_agg_distinct', [])
