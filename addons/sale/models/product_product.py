...
class ProductProduct(models.Model):
    ...
    def _compute_sales_count(self):
        ...
        res_aggregate = self.env['sale.report']._aggregate(domain, ['product_uom_qty:sum'], ['product_id'])
        for product in self:
            if not product.id:
                product.sales_count = 0.0
                continue
            product.sales_count = float_round(res_aggregate.get_agg(product, 'product_uom_qty:sum', 0), precision_rounding=product.uom_id.rounding)
    ...
    def _filter_to_unlink(self):
        ...
        lines = self.env['sale.order.line']._aggregate(domain, groupby=['product_id'])
