...
class SaleOrderLine(models.Model):
    ...
    def _get_display_price(self, product):
        ...
        context_product = dict(product.env.context)
        final_price, rule_id = self.order_id.pricelist_id.with_context(context_product).get_product_price_rule(product, self.product_uom_qty or 1.0, self.order_id.partner_id)
        base_price, currency_id = self.with_context(context_product)._get_real_price_currency(product, rule_id, self.product_uom_qty, self.product_uom, self.order_id.pricelist_id.id)
    ...
    def _onchange_discount(self):
        ...
        context_product = dict(product.env.context)
        price, rule_id = self.order_id.pricelist_id.with_context(context_product).get_product_price_rule(product, self.product_uom_qty or 1.0, self.order_id.partner_id)
        new_list_price, currency_id = self.with_context(context_product)._get_real_price_currency(product, rule_id, self.product_uom_qty, self.product_uom, self.order_id.pricelist_id.id)
