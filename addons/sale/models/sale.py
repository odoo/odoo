...
class SaleOrder(models.Model):
    ...
    expected_date_mode = fields.Selection([('auto', 'Computed'), ('manual', 'Manual')], default='auto')
    ...
    @api.depends('order_line.customer_lead', 'date_order', 'order_line.state', 'expected_date_mode')
    def _compute_expected_date(self):
        """ For service and consumable, we only take the min dates. This method is extended in sale_stock to
            take the picking_policy of SO into account.
        """
        for order in self:
            if order.expected_date_mode == 'manual':
                order.expected_date = order.expected_date_manual
            else:
                dates_list = []
    ...
    def _inverse_expected_date(self):
        """ When writing on expected_date, if the value should be modify manually (`expected_date_mode` = 'manual' only),
            then we put the value in `expected_date_manual`. Otherwise, `expected_date_manual` should be False since the
            expected date is automatically compute by other mecanisms.
        """
        for order in self:
            order.expected_date_mode = 'manual'
