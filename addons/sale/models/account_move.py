...
class AccountMove(models.Model):
    ...
    def _compute_team_id(self):
        ...
        for ((user_id, company_id, partner_id), moves) in groupby(
            sale_moves,
            key=lambda m: (m.invoice_user_id.id, m.company_id.id, m.partner_id)
        ):
            default_team_id = self.env.context.get('default_team_id', False) or partner_id.team_id.id
