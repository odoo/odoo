...
class sale_make_invoice(osv.osv_memory):
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        if order.state == 'draft':
            raise Warning(_('Warning!'), _('You cannot create invoice when sales order is not confirmed.'))
        return False
    ...
    def make_invoices(self, cr, uid, ids, context=None):
        ...
        for sale_order in order_obj.browse(cr, uid, context.get(('active_ids'), []), context=context):
            if sale_order.state != 'manual':
                raise Warning(_('Warning!'), _("You shouldn't manually invoice the following sale order %s") % (sale_order.name))
