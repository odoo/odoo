...
class PaymentWizard(models.TransientModel):
    ...
    def _get_default_payment_method(self):
        return self.env.company_id.sale_onboarding_payment_method or 'digital_signature'
    ...
    def _set_payment_acquirer_onboarding_step_done(self):
        """ Override. """
        self.env.company_id.set_onboarding_step_done('sale_onboarding_order_confirmation_state')
    ...
    def add_payment_methods(self, *args, **kwargs):
        self.env.company_id.sale_onboarding_payment_method = self.payment_method
        if self.payment_method == 'digital_signature':
            self.env.company_id.portal_confirmation_sign = True
        if self.payment_method in ('paypal', 'stripe', 'manual'):
            self.env.company_id.portal_confirmation_pay = True
