        ...
        select_str = """
            WITH currency_rate as (%s)
             SELECT min(l.id) as id,
                    ...
                    sum(l.price_reduce * l.qty_to_invoice / COALESCE(NULLIF(cr.rate, 0), 1.0)) as amount_to_invoice,
                    sum(l.price_reduce * l.qty_invoiced / COALESCE(NULLIF(cr.rate, 0), 1.0)) as amount_invoiced,
                    count(*) as nbr,
                    s.name as name,
