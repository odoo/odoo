...
class SaleReport(models.Model):
    ...
    def _select_sale(self):
        select_ = f"""
            ...
            CASE WHEN l.product_id IS NOT NULL OR l.display_type = 'downpayment' THEN SUM(l.untaxed_amount_to_invoice
                / {self._case_value_or_one('s.currency_rate')}
                * {self._case_value_or_one('account_currency_table.rate')}
                ) ELSE 0
            END AS untaxed_amount_to_invoice,
            CASE WHEN l.product_id IS NOT NULL OR l.display_type = 'downpayment' THEN SUM(l.untaxed_amount_invoiced
                / {self._case_value_or_one('s.currency_rate')}
                * {self._case_value_or_one('account_currency_table.rate')}
                ) ELSE 0
