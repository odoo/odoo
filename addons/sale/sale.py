...
class sale_order(osv.osv):
    ...
    def onchange_project_id(self, cr, uid, ids, project, context=None):
        if not project:
            return {'value': {'contract_status': False}}
        contract_status = self.pool['account.analytic.account'].browse(cr, uid ,project, context=context).state
        return {'value': {'contract_state': contract_status}}
