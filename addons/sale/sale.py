...
class sale_order(osv.osv):
    ...
    def _get_default_company(self, cr, uid, context=None):
        ...
        if not company_id:
            raise Warning(_('Error!'), _('There is no default company for the current user!'))
        return company_id
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for s in sale_orders:
            if s['state'] in ['draft', 'cancel']:
                ...
                raise Warning(_('Invalid Action!'), _('In order to delete a confirmed sales order, you must cancel it before!'))
...
class sale_order_line(osv.osv):
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        for line in self.browse(cr, uid, ids, context=context):
            if line.invoiced:
                raise Warning(_('Invalid Action!'), _('You cannot cancel a sales order line that has already been invoiced.'))
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.state not in ['draft', 'cancel']:
                raise Warning(_('Invalid Action!'), _('Cannot delete a sales order line which is in state \'%s\'.') %(rec.state,))
