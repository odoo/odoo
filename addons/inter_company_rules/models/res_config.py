...
class inter_company_rules_configuration(models.TransientModel):
    ...
    set_type = fields.Selection([('so_and_po', 'SO and PO setting for inter company'),
    ...
    def onchange_set_type(self):
        if self.set_type == 'invoice_and_refunds':
            self.so_from_po = False
            self.po_from_so = False
            self.auto_validation = False

        elif self.set_type == 'so_and_po':
    ...
    def set_inter_company_configuration(self):
        if self.company_id:
            vals = {
                ...
                'auto_generate_invoices': True if self.set_type == 'invoice_and_refunds' else False,
