...
class inter_company_rules_configuration(models.TransientModel):
    ...
    rule_type = fields.Selection([
        ...
        help='Select the type to setup inter company rules '
             'in selected company.')
    ...
    def onchange_company_id(self):
        if self.company_id:
            rule_type = False
            if (self.company_id.so_from_po or self.company_id.po_from_so or
                    self.company_id.auto_validation):
                rule_type = 'so_and_po'
        ...
        if self.company_id:
            vals = {
                ...
                'auto_generate_invoices': (
                    True if self.rule_type == 'invoice_and_refunds'
                    else False),
