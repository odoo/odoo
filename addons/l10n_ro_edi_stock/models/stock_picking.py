...
class Picking(models.Model):
    ...
    def _compute_l10n_ro_edi_stock_enable(self):
        for picking in self:
            picking.l10n_ro_edi_stock_enable = picking.company_id.country_id.code == 'RO'
    ...
    def _compute_l10n_ro_edi_stock_enable_send(self):
        for picking in self:
            picking.l10n_ro_edi_stock_enable_send = (picking.l10n_ro_edi_stock_enable
                                                     ...
                                                     and picking.l10n_ro_edi_stock_state in (False, 'sending_failed')
                                                     and not picking.l10n_ro_edi_stock_interface_id._get_last_sent_document())

    @api.depends('company_id', 'state', 'l10n_ro_edi_stock_state')
    def _compute_l10n_ro_edi_stock_enable_fetch(self):
        for picking in self:
            picking.l10n_ro_edi_stock_enable_fetch = picking.l10n_ro_edi_stock_enable and picking.l10n_ro_edi_stock_state == 'sending'

    @api.depends('l10n_ro_edi_stock_document_message')
    def _compute_l10n_ro_edi_stock_enable_send_info(self):
        for picking in self:
            picking.l10n_ro_edi_stock_enable_send_info = picking.l10n_ro_edi_stock_document_message and True

    @api.depends('l10n_ro_edi_stock_state')
    def _compute_l10n_ro_edi_stock_enable_amend(self):
        for picking in self:
            picking.l10n_ro_edi_stock_enable_amend = picking.l10n_ro_edi_stock_enable and (picking.l10n_ro_edi_stock_state == 'sent' or (picking.l10n_ro_edi_stock_state == 'sending_failed' and picking.l10n_ro_edi_stock_interface_id._get_last_sent_document()))

    @api.depends('l10n_ro_edi_stock_state')
    def _compute_l10n_ro_edi_stock_fields_readonly(self):
        for picking in self:
            picking.l10n_ro_edi_stock_fields_readonly = picking.l10n_ro_edi_stock_state == 'sending'
