...
from odoo.addons.l10n_ro_edi_stock.models.l10n_ro_edi_stock_document import DOCUMENT_STATES, _make_etransport_request
...
class L10nRoEdiStockETransportInterface(models.Model):
    ...
    def _compute_current_document_data(self):
        for interface in self:
            ...
            interface.l10n_ro_edi_stock_state = document.state if document else None
            interface.l10n_ro_edi_stock_document_message = document.message if document else False
            interface.l10n_ro_edi_stock_document_load_id = document.load_id if document else None
            interface.l10n_ro_edi_stock_document_uit = document.uit if document else None
    ...
    def _check_fetch_values(self, errors=None) -> list:
        ...
        match self.l10n_ro_edi_stock_state:
            case 'sending_failed':
                if self._get_last_sent_document():
    ...
    def _create_document_sending(self, load_id: str, uit: str):
        self.ensure_one()
    ...
    def _create_document_sending_failed(self, message: str, load_id: str | None = None, uit: str | None = None):
        self.ensure_one()
        return self.env['l10n_ro.edi.stock.document'].create({
            'interface_id': self.id,
    ...
    def _create_document_sent(self, load_id: str, uit: str):
        self.ensure_one()
        return self.env['l10n_ro.edi.stock.document'].create({
            'interface_id': self.id,
            'state': 'sent',
    ...
    def _get_failed_documents(self):
        self.ensure_one()
        return self.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state == 'sending_failed')

    def _get_sending_and_failed_documents(self):
        self.ensure_one()
        return self.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state in ('sending_failed', 'sending'))

    def _get_last_sent_document(self):
        self.ensure_one()
        sent_documents = self.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state == 'sent').sorted()
    ...
    def _send_etransport_document(self, send_type: str):
        ...
        if errors := self._validate_data():
            ...
            if send_type == 'amend':
                last_sent_document = self._get_last_sent_document()
                self._create_document_sending_failed(message='\n'.join(errors), load_id=last_sent_document.load_id, uit=last_sent_document.uit)
            else:
                self._create_document_sending_failed(message='\n'.join(errors))
            return

        template = self.env['ir.ui.view']._render_template('l10n_ro_edi_stock.l10n_ro_template_etransport', values={
            'send_type': send_type,
            'interface': self,
        ...
        result = _make_etransport_request(
        ...
        if 'error' in result:
            ...
            if send_type == 'amend':
                last_sent_document = self._get_last_sent_document()
                self._create_document_sending_failed(message=result['error'], load_id=last_sent_document.load_id, uit=last_sent_document.uit)
            else:
                self._create_document_sending_failed(message=result['error'])
        else:
            self._get_sending_and_failed_documents().unlink()

            content = result['content']
            uit = content['UIT'] if send_type == 'send' else self._get_last_sent_document().uit
            self._create_document_sending(load_id=content['index_incarcare'], uit=uit)

    def _fetch_document_status(self):
        session = requests.Session()
        documents_to_delete = self.env['l10n_ro.edi.stock.document']
        to_fetch = self.filtered(lambda inter: inter.l10n_ro_edi_stock_state == 'sending')

        for interface in to_fetch:
            current_sending_document = interface.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state == 'sending')[0]

            if errors := interface._check_fetch_values():
                documents_to_delete |= interface._get_failed_documents()
                interface._create_document_sending_failed(message='\n'.join(errors), load_id=current_sending_document.load_id, uit=current_sending_document.uit)
                continue

            result = _make_etransport_request(
                company=interface.l10n_ro_edi_stock_company_id,
                endpoint=f'stareMesaj/{current_sending_document.load_id}',
                method='get',
                session=session,
            )
            ...
            if 'error' in result:
                documents_to_delete |= interface._get_failed_documents()
                interface._create_document_sending_failed(message=result['error'], load_id=current_sending_document.load_id, uit=current_sending_document.uit)
            else:
                documents_to_delete |= interface._get_sending_and_failed_documents()
                match state := result['content']['stare']:
                    case 'ok':
                        interface._create_document_sent(load_id=current_sending_document.load_id, uit=current_sending_document.uit)
                    case 'in prelucrare':
                        # Document is still being sent
                        interface._create_document_sending(load_id=current_sending_document.load_id, uit=current_sending_document.uit)
                    case 'XML cu erori nepreluat de sistem':
                        interface._create_document_sending_failed(message=_("XML contains errors."), load_id=current_sending_document.load_id, uit=current_sending_document.uit)
                    case _:
