...
class ETransportAPI:
    ...
    @staticmethod
    def _make_etransport_request(company, endpoint: str, method: str, session=None, data=None):
        ...
        try:
            if session:
                ...
                response = requests.request(method=method, url=url, data=data, headers=headers, timeout=10)
        ...
        if response_data['ExecutionStatus'] == 1:
            errors = ETransportAPI._cleanup_errors([error['errorMessage'] for error in response_data['Errors']])
            return {'error': '\n'.join(errors)}
