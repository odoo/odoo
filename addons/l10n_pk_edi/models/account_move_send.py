...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_all_extra_edis(self):
        ...
        res.update({
            'pk_edi_send': {
                ...
                'help': _("Send the E-Invoice to Federal Board of Revenue (Pakistan).")
            }
        })
        return res
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if pk_moves := moves.filtered(lambda m:
            'pk_edi_send' in moves_data[m]['extra_edis']
    ...
    def _hook_invoice_document_before_pdf_report_render(self, invoice, invoice_data):
        ...
        if (
            ('pk_edi_send' in invoice_data['extra_edis'] and not invoice.l10n_pk_edi_attachment_id)
            or (invoice_data['invoice_edi_format'] == 'pk_edi_json' and invoice._l10n_pk_edi_get_default_enable())
        ):
            if errors := invoice._l10n_pk_edi_validate():
                invoice_data['error'] = {
