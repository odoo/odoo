...
class AccountMove(models.Model):
    ...
    l10n_pk_edi_cancel_remarks = fields.Char(string="Cancel Remarks", copy=False)
    ...
    def _l10n_pk_edi_validate(self):
        ...
        alerts = {}
        buyers = self.env['res.partner']
        sellers = self.env['res.partner']
        products = self.env['product.product']

        for move in self:
            buyer, seller = _get_buyer_seller(move)
            buyers += buyer
            sellers += seller
            products += move.invoice_line_ids.product_id

        seller_checks = ['partner_vat_missing', 'partner_vat_invalid', 'partner_full_address_missing']
        buyer_alerts = buyers._l10n_pk_edi_export_check()
        seller_alerts = sellers._l10n_pk_edi_export_check(seller_checks)
        product_alerts = products._l10n_pk_edi_export_check()
        receipt_alerts = self._l10n_pk_edi_export_check()
        company_alerts = self.company_id._l10n_pk_edi_export_check()

        alerts.update(**buyer_alerts if buyer_alerts else {})
        alerts.update(**seller_alerts if seller_alerts else {})
        alerts.update(**product_alerts if product_alerts else {})
        alerts.update(**receipt_alerts if receipt_alerts else {})
        alerts.update(**company_alerts if company_alerts else {})
        return alerts
    ...
    def _get_l10n_pk_edi_tax_details_by_line(self, tax_details):
        ...
        for tax_detail in tax_details.values():
            line_code = tax_detail['taxes_data'][0]['tax'].tax_group_id.id
            ...
            l10n_pk_tax_detail['rate'] += tax_detail['taxes_data'][0]['tax'].amount
    ...
    def _get_l10n_pk_edi_partner_address(self, partner):
        ...
        address = ', '.join(part for part in address if part)
        return address
    ...
    def _l10n_pk_edi_generate_invoice_json(self):
        ...
        json_payload = {
            ...
            'saleType': 'T1000017',
            'salesValue': self._l10n_pk_edi_round_value(total_sale_value + tax_details.get('tax_amount') - total_discount),
            'totalSalesTaxApplicable': self._l10n_pk_edi_round_value(tax_details.get('tax_amount')),
