...
class AccountMove(models.Model):
    ...
    def _get_l10n_pk_edi_line_details(self, move_lines):
        ...
        # fields_get translates the selection labels using its own recordset's env, so lang is
        # carried by with_context - the codes sent to the FBR are the English labels.
        uom_code_selection = dict(self.env['uom.uom'].with_context(lang='en_US').fields_get(
            ['l10n_pk_edi_uom_code'], ['selection'],
        )['l10n_pk_edi_uom_code']['selection'])
        sale_type_selection = dict(self.env['account.move.line'].with_context(lang='en_US').fields_get(
        ...
        for line in invoice_lines:
            ...
            net_retail_price = (product.lst_price * line.quantity) - sales_tax_total
            ...
            item_payload = {
                ...
                'rate': rate,
                'uoM': uom_code_selection.get(line.product_uom_id.l10n_pk_edi_uom_code or FBR_UOM_CODE_OTHERS, "Others"),
                'quantity': json_float_round(line.quantity, 4),
                'totalValues': json_float_round(line.price_total, decimal_places),
                'valueSalesExcludingST': json_float_round(line.price_subtotal, decimal_places),
                'fixedNotifiedValueOrRetailPrice': json_float_round(net_retail_price, decimal_places) if is_third_schedule else "",
                'salesTaxApplicable': json_float_round(sales_tax_total, decimal_places),
                'salesTaxWithheldAtSource': json_float_round(abs(tax_values.get('withholding_tax_total', 0.0)), decimal_places),
                'extraTax': "",
                'furtherTax': json_float_round(tax_values.get('further_tax_total', 0.0), decimal_places),
                'sroScheduleNo': line.l10n_pk_edi_sro_id.name,
                'fedPayable': 0.0,
                'discount': json_float_round(line.price_unit * line.quantity * line.discount / 100, decimal_places),
    ...
    def _l10n_pk_edi_generate_invoice_json(self):
        ...
        partner_fields = self.env['res.partner'].with_context(lang='en_US').fields_get(
            ['l10n_pk_edi_fbr_customer_status'], ['selection'],
        )
        fbr_customer_status_selection = dict(partner_fields['l10n_pk_edi_fbr_customer_status']['selection'])
        ...
        json_payload = {
            ...
            'invoiceTotalAmount': json_float_round(abs(self.amount_total), self.currency_id.decimal_places),
            ...
            'buyerRegistrationType': fbr_customer_status_selection.get(buyer.l10n_pk_edi_fbr_customer_status, "Unregistered"),
        ...
        if self.move_type == 'out_invoice' and self.debit_origin_id and self.debit_origin_id.l10n_pk_edi_status in {'sent', 'sent_test'}:
            json_payload['reason'] = self.l10n_pk_edi_refund_reason or ''
    ...
    def _l10n_pk_edi_handle_response(self, response):
        ...
        if result := self.env['iap.account']._l10n_pk_edi_parse_response(response):
            ...
            self.message_post(
                body=Markup('<span class="text-danger">%s</span>') % Markup('<br/>').join(
