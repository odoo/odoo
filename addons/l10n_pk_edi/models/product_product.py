...
class ProductTemplate(models.Model):
    ...
    l10n_pk_edi_sale_type = fields.Selection(
        ...
        string="Sale Type", default='T1000017'
    )
    l10n_pk_edi_schedule_code = fields.Selection(
        ...
        string="Schedule Code", default='S1000012'
    )
    ...
    def _l10n_pk_edi_export_check(self):
        ...
        if invalid_records := self.filtered(lambda record: not all(record[field] for field in (
            ...
            errors['l10n_pk_edi_product_value_missing'] = {
                'level': 'danger',
                'message': _("Product(s) should have a Sale Type, Schedule Code and HS Code."),
        ...
        if invalid_records := self.filtered(
            ...
            errors['l10n_pk_edi_product_hscode_invalid'] = {
                'level': 'danger',
                'message': _("Product(s) has invalid HS Code. It should contains exactly 8 digits."),
                'action_text': _("View Product(s)"),
                'action': invalid_records._get_records_action(name=_("Check Product(s)")),
            }
        return errors
