...
class IapAccount(models.Model):
    ...
    def _l10n_pk_edi_parse_response(self, response):
        """Parse FBR response. Returns dict with status/errors/error_response, or None if valid.

        'errors' is a list of plain strings; formatting them is left to the caller.
        """
        ...
        if error := response.get('error'):
            ...
            if (validation_res := error.get('validationResponse')) and validation_res.get('status') == 'Invalid':
                if validation_res.get('error'):
                    errors.append(validation_res['error'])
                for item in (validation_res.get('invoiceStatuses') or []):
                    if item.get('error'):
                        errors.append(f"Line {item.get('itemSNo')} {item['error']}")
            if not errors:
                # A fault description is more specific than the generic error message.
                errors = [(error.get('fault') or {}).get('description') or error.get('message') or '']
            return {
                'status': 'failed',
                'errors': errors,
                'error_response': self._l10n_pk_edi_compose_error_response(
                    error.get('code') or 'SERVER_ERROR', '\n'.join(errors),
                ),
            }
