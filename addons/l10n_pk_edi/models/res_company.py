...
class ResCompany(models.Model):
    ...
    def _l10n_pk_edi_prepare_scenario_payload(self, json_payload):
        ...
        # FBR resolves the buyer NTN/CNIC against its own records, so only a scenario that declares
        # a registered buyer may carry l10n_pk_edi_test_vat: that number belongs to a registered
        # business, and sending it while buyerRegistrationType says "Unregistered" makes FBR reject
        # the line. Unregistered scenarios keep the dummy CNIC from the fixture.
        if payload.get('buyerRegistrationType') == 'Registered':
    ...
    def _l10n_pk_edi_run_single_test_scenario(self, filename, json_payload):
        ...
        if not auth_token:
            return False
        _logger.info("l10n_pk_edi: scenario %s payload: %s", scenario_id, json_payload)
        try:
            ...
            if failure := self.env['iap.account']._l10n_pk_edi_parse_response(validate_res):
                _logger.warning(
                    "l10n_pk_edi: scenario %s rejected at validation: %s",
                    scenario_id, "; ".join(failure["errors"]),
                )
                return False
            ...
            if failure := self.env['iap.account']._l10n_pk_edi_parse_response(post_res):
                _logger.warning(
                    "l10n_pk_edi: scenario %s rejected at posting: %s",
                    scenario_id, "; ".join(failure["errors"]),
                )
                return False
            return True
