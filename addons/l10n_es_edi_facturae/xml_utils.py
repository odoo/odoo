...
def _get_uri(uri, reference, base_uri=""):
    ...
    if uri.startswith("#"):
        ...
        if len(results) > 1:
            raise UserError(f"Ambiguous reference URI {uri} resolved to {len(results)} nodes")  # nosem: missing-gettext

    raise UserError(f'URI {uri} not found')  # nosem: missing-gettext
