    ...
    def test_out_invoice_rounding(self):
        ...
        with freeze_time(self.frozen_today):
            invoice = self.create_invoice(
                ...
                move_type='out_invoice',
                invoice_line_ids=[
                    {'price_unit': 2.0200, 'quantity': 25.0, 'tax_ids': [self.tax.id]},
                    {'price_unit': 7.2900, 'quantity': 2.0, 'tax_ids': [self.tax.id]},
                    {'price_unit': 7.3200, 'quantity': 5.0, 'tax_ids': [self.tax.id]},
                    {'price_unit': 9.5000, 'quantity': 25.0, 'tax_ids': [self.tax.id]},
                ],
            )
            invoice.action_post()
            generated_file, errors = invoice._l10n_es_edi_facturae_render_facturae()
