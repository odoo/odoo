...
class TestEdiFacturaeXmls(AccountTestInvoicingCommon):
    ...
    @classmethod
    def _create_invoice(cls, **kwargs):
    ...
    def test_cannot_generate_unsigned_xml(self):
        ...
        with freeze_time(self.frozen_today), \
                ...
                self._mock_sha1():
            invoice = self._create_invoice(partner_id=self.partner_a.id, move_type='out_invoice', invoice_line_ids=[{'price_unit': 100.0, 'tax_ids': [self.tax.id]}])
            invoice.action_post()
