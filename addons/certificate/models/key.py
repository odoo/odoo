...
class CertificateKey(models.Model):
    ...
    def _decrypt(self, message, hashing_algorithm='sha256'):
        ...
        private_key = serialization.load_pem_private_key(base64.b64decode(self.with_context(bn_size=False).pem_key), None)
    ...
    def _encrypt(self, message, hashing_algorithm='sha256'):
        ...
        public_key = serialization.load_pem_public_key(base64.b64decode(self.with_context(bn_size=False).pem_key))
        if not isinstance(public_key, rsa.RSAPublicKey):
            raise UserError(_(
                "Unsupported asymmetric cryptography algorithm '%s'. Currently supported for encryption: RSA.",
                repr(public_key),
            ))
