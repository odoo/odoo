...
class PosPaymentMethod(models.Model):
    ...
    def paytm_make_payment_request(self, amount, transaction_id, reference_id, timestamp):
        ...
        if self.pos_payment_provider_id.paytm_accept_payment == 'auto':
            body['autoAccept'] = 'True'
        body['paymentMode'] = self.pos_payment_provider_id.paytm_allowed_payment_modes.upper()
        head = self._paytm_get_request_head(body)
        head_error = head.get('error')
        if head_error:
            return {'error': '%s' % head_error}
        merchantExtendedInfo = {'paymentMode': self.pos_payment_provider_id.paytm_allowed_payment_modes.upper()}
        if self.pos_payment_provider_id.paytm_accept_payment == 'auto':
    ...
    def _paytm_get_request_body(self, transaction_id, reference_id, timestamp):
        ...
        return {
            'paytmMid': self.pos_payment_provider_id.paytm_mid,
    ...
    def _paytm_get_request_head(self, body):
        paytm_signature = self._paytm_generate_signature(body, self.pos_payment_provider_id.paytm_merchant_key)
        error = isinstance(paytm_signature, dict) and paytm_signature.get('error')
        if error:
            return {'error': '%s' % error}
        return {
            'requestTimeStamp' : body["transactionDateTime"],
            'channelId': self.pos_payment_provider_id.paytm_channel_id,
