...
class TestDashboardProject(TestProjectDashboardCommon):
    ...
    def test_display_project_related_sols_in_the_dashboard(self):
        ...
        self.assertFalse(sol_dashboard_project.project_id)
        sale_item_data = self.dashboard_project.get_sale_items_data(limit=5, with_action=False, section_id='service_revenues')
        self.assertEqual(
            [item['id'] for item in sale_item_data['sol_items']],
            [sol_dashboard_project.id],
            "`dashboard_project` should only show `sol_new_project`, matched via the analytic_distribution."
        )

        self.assertFalse(sol_new_project.project_id)
        sale_item_data = new_project.get_sale_items_data(limit=5, with_action=False, section_id='service_revenues')
        self.assertEqual(
            [item['id'] for item in sale_item_data['sol_items']],
            [sol_new_project.id],
            "`new_project` should only show `sol_new_project`, matched via the analytic_distribution."
        )
