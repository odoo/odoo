...
class TestDashboardProject(TestProjectDashboardCommon):
    ...
    def test_get_sale_item_data_various_sols(self):
        ...
        expected_dict = [{
            ...
            'qty_delivered': 0.0,
            'qty_invoiced': 0.0,
            'product_uom_id': (unit_uom_id, 'Units'),
            'product_id': (self.material_product.id, 'Material'),
        }]
        ...
        expected_dict = [{
            'id': sol_service_1.id,
            'name': '[SERV-ORDERED2] Service Milestone',
            'product_uom_qty': 1.0,
            'qty_delivered': 0.0,
            'qty_invoiced': 0.0,
            'product_uom_id': (hour_uom_id, 'Hours'),
            'product_id': (self.product_milestone.id,
            '[SERV-ORDERED2] Service Milestone'),
        }, {
            'id': sol_service_2.id,
            'name': '[SERV-ORDERED2] Product prepaid',
            'product_uom_qty': 1.0,
            'qty_delivered': 0.0,
            'qty_invoiced': 0.0,
            'product_uom_id': (hour_uom_id, 'Hours'),
            'product_id': (self.product_prepaid.id,
            '[SERV-ORDERED2] Product prepaid'),
        }, {
            'id': sol_service_4.id,
            'name': '[SERV-ORDERED2] Service Delivery',
            'product_uom_qty': 1.0,
            'qty_delivered': 0.0,
            'qty_invoiced': 0.0,
            'product_uom_id': (hour_uom_id, 'Hours'),
            'product_id': (self.dashboard_product_delivery_service.id,
            '[SERV-ORDERED2] Service Delivery'),
        }]
