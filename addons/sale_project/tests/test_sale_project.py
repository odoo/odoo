...
class TestSaleProject(TestSaleProjectCommon):
    ...
    def test_sale_order_with_project_task(self):
        ...
        self.assertEqual(
            ...
            f"{sale_order.name} - {self.product_order_service2.display_name}",
            "Task name in global project should include SO name & product name",
        )
        self.assertEqual(
            str(so_line_order_task_in_global.task_id.description),
            '<p>Description for global task.</p>',
        )
        #  service_tracking 'task_in_project'
        ...
        self.assertEqual(
            ...
            "Task name in new project should only include product name",
        )
        self.assertEqual(
            str(so_line_order_new_task_new_project.task_id.description),
            '<p>Description for new project task.</p>',
        )
        self.assertEqual(
            so_line_order_new_task_new_project2.task_id.name,
            self.product_order_service3.display_name,
            "Task name in new project (no description) should be the product name."
        )
        # service_tracking 'project_only'
