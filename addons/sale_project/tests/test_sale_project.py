...
class TestSaleProject(HttpCase, TestSaleProjectCommon):
    ...
    def test_analytics_on_so_confirmation_no_project(self):
        ...
        sol_no_project, sol_task_in_global_project, sol_task_in_template_project, sol_new_project = self.env['sale.order.line'].create([
            {'order_id': so.id, 'product_id': self.product_order_service1.id},
        ...
        self.assertEqual(
            so.project_id,
            (sol_task_in_global_project | sol_task_in_template_project | sol_new_project).project_id.sorted('sequence')[0],
            "The project of the SO should be set to the project with the lowest sequence."
        )
        self.assertNotEqual(
