...
class TestSaleProject(TestSaleProjectCommon):
    ...
    def test_onchange_partner_id(self):
        ...
        with Form(self.project_global.task_ids) as task_form:
            ...
            self.assertFalse(task_form.sale_line_id, "Sale Line Item must be set to False as we have changed partner.")
            self.assertFalse(task_form.sale_order_id,  "Sale Order must be set to False as we have changed partner.")
            # Setting the partner should bring SOL made for the partner in the current task if previous any
            task_form.partner_id = self.partner
            self.assertEqual(task_form.sale_line_id, sale_order.order_line, "Task should have the last SOL of The customer if exist.")
            self.assertEqual(task_form.sale_order_id, sale_order, "Task should have the Sale Order from Sale Line Item.")
