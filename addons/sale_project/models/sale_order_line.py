...
class SaleOrderLine(models.Model):
    ...
    def _compute_analytic_distribution(self):
        no_project_line_ids = set()
        for line in self:
            if line._origin:
                continue
            project = line.product_id.project_id or line.product_id.project_template_id or line.order_id.project_id
            accounts = project._get_analytic_account_ids()
            if not (line.product_id and accounts):
                no_project_line_ids.add(line.id)
                continue
            distribution = {reduce(lambda acc, el: f"{acc},{el}", accounts.ids): 100}
            line.analytic_distribution = distribution
        super(SaleOrderLine, self.browse(no_project_line_ids))._compute_analytic_distribution()
    ...
    def _timesheet_create_project_prepare_values(self):
        ...
        return {
            ...
            'analytic_account_id': self.order_id.analytic_account_id.id,  # TODO
    ...
    def _timesheet_create_project(self):
        ...
        project_to_copy = self.product_id.project_template_id or self.order_id.project_id
        if project_to_copy:
            values['name'] = "%s - %s" % (values['name'], project_to_copy.name)
            project = project_to_copy.copy(values)
            project.tasks.write({
                'sale_line_id': self.id,
                'partner_id': self.order_id.partner_id.id,
            })
            ...
            account_id = self.order_id.analytic_account_id
            if account_id:
                values[account_id.root_plan_id._column_name()] = account_id.id
        ...
        project.analytic_sale_order_id = self.order_id
        return project
    ...
    def _timesheet_service_generation(self):
        ...
        # If the SO generates projects on confirmation and the project of the SO is not set, set it to the project with the lowest sequence
        so_lines = so_line_task_global_project + so_line_new_project
        so = so_lines.order_id
        if not so.project_id and so_lines.project_id:
            so.project_id = so_lines.project_id.sorted('sequence')[0]
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if not values.get('analytic_distribution'):
            if self.project_id.analytic_account_id:
                ...
                analytic_accounts = project_analytic_accounts
