...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        for line in confirmed_lines:
            if line.task_id and not line in has_task_lines:
    ...
    def _timesheet_create_project(self):
        ...
        if project_template:
            ...
            project_order = self.order_id.project_id
            other_plans_fnames = [plan._column_name() for plan in other_plans]
            values.update({fname: project_order[fname].id for fname in other_plans_fnames if project_order[fname]})
    ...
    def _timesheet_service_generation(self):
        ...
        # we store the projects of SOLs of type 'task_global_project' per sale order
        map_global_projects_per_order = defaultdict(set)
        # and projects of SOLs of types 'project_only' or 'task_in_project' per sale order
        map_new_projects_per_order = defaultdict(set)
        # and SOLs of type 'task_global_project' that need to fallback on the project's SO per sale order
        map_lines_no_project_per_order = defaultdict(set)
        ...
        for so_line in so_line_task_global_project:
            if not so_line.task_id:
                ...
                if project and so_line.product_uom_qty > 0:
                    ...
                    map_global_projects_per_order[so_line.order_id].add(project.id)
                else:
                    map_lines_no_project_per_order[so_line.order_id].add(so_line.id)
        ...
        for so_line in so_line_new_project:
            ...
            if project:
                map_new_projects_per_order[so_line.order_id].add(project.id)
            so_line._handle_milestones(project)

        # If the SO generates projects or create tasks in projects on confirmation and the project's SO is not set, set it to the project with the lowest sequence
        # Also set the analytic account of the generated projects to the account of the project's SO or create a new one
        orders = (so_line_task_global_project | so_line_new_project).order_id
        for order in orders:
            new_projects = self.env['project.project'].browse(map_new_projects_per_order.get(order))
            if new_projects:
                new_projects.account_id = order.project_account_id or self.env['account.analytic.account'].create(order._prepare_analytic_account_data())
            if not order.project_id:
                order.project_id = (new_projects | self.env['project.project'].browse(map_global_projects_per_order.get(order))).sorted('sequence')[:1]

            # Also create tasks in the project's SO for SOLs having no project configured on their product
            # Or raise an error if such SOLs exist but not project is set on the SO
            lines_no_project = self.env['sale.order.line'].browse(map_lines_no_project_per_order.get(order)).filtered(lambda sol: sol.product_uom_qty > 0)
            if lines_no_project:
                if order.project_id:
