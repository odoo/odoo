...
class TestIdChartTemplate(AccountTestInvoicingCommon):
    ...
    def test_no_duplicate_utility_accounts(self):
        ...
        self.assertEqual(company.account_journal_suspense_account_id.code, '11120001')
        self.assertEqual(
            company.account_journal_suspense_account_id,
            ChartTemplate.ref('l10n_id_11120001'),
        )
        extra_suspense = Account.search([
            ('company_ids', 'in', company.id),
            ('name', '=', 'Bank Suspense Account'),
        ])
        self.assertFalse(
            extra_suspense,
            f"Unexpected auto-created Bank Suspense Account {extra_suspense.mapped('code')}",
        )
