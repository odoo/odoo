...
class HrAttendanceError(models.TransientModel):
    ...
    init_date = fields.Date('Starting Date', required=True, default=fields.Date.today)
    end_date = fields.Date('Ending Date', required=True, default=fields.Date.today)
    ...
    def print_report(self):
        ...
        self.env.cr.execute("SELECT id FROM hr_attendance WHERE employee_id IN %s "
                        "AND to_char(name,'YYYY-mm-dd')<=%s "
                        "AND to_char(name,'YYYY-mm-dd')>=%s "
                        "AND action IN %s ORDER BY name" , (tuple(self.env.context['active_ids']), date_to, date_from, tuple(['sign_in','sign_out'])))
        attendance_ids = [x[0] for x in self.env.cr.fetchall()]
        if not attendance_ids:
            raise UserError(_('No records are found for your selection!'))
        employees = self.env['hr.attendance'].browse(attendance_ids).mapped('employee_id')
        datas = {'init_date': date_from, 'end_date': date_to,
                 'max_delay': self.max_delay, 'emp_ids': employees.ids}
