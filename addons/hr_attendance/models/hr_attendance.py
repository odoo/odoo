...
class HrAttendance(models.Model):
    ...
    def _compute_overtime_hours(self):
        if self.employee_id:
            ...
            day_starts = [self._get_day_start_and_day(att.employee_id, att.check_in) for att in self]
            date_min, date_max = min(date for dummy, date in day_starts), max(date for dummy, date in day_starts)
            time_min, time_max = min(time for time, dummy in day_starts), max(time for time, dummy in day_starts)

            # Find at least all attendances that happen on the same day and for the same employee
            # as an attendance in self
            all_attendances = self | self.env['hr.attendance'].search(
                [
                    ('employee_id', 'in', self.employee_id.ids),
                    ('check_in', '>=', time_min),
                    ('check_in', '<=', time_max + timedelta(days=1)),
                ],
            )
            attendances_by_date_and_employee = defaultdict(list)
            for att in all_attendances:
                dummy, date = att._get_day_start_and_day(att.employee_id, att.check_in)
                if date_min <= date <= date_max:
                    att.overtime_hours = 0
                    attendances_by_date_and_employee[date, att.employee_id.id].append(att)

            overtimes = self.env['hr.attendance.overtime'].search_fetch(
                [
                    ('employee_id', 'in', self.employee_id.ids),
                    ('date', '>=', date_min),
                    ('date', '<=', date_max),
                ],
                field_names = ['employee_id', 'date', 'duration']
            )
            for ot in overtimes:
                ...
                for att in sorted(attendances, key=lambda att: att.check_in, reverse=True):
    ...
    def _get_day_start_and_day(self, employee, dt):
        ...
        if not dt.tzinfo:
            date_employee_tz = pytz.utc.localize(dt).astimezone(pytz.timezone(employee._get_calendar_tz(dt)))
        else:
            date_employee_tz = dt
