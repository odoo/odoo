    ...
    def _cron_auto_check_out(self):
        ...
        previous_duration = self.env['hr.attendance']._read_group(
            domain=[
                ...
                ('check_out', '!=', False)], groupby=['employee_id', 'id'])

        mapped_previous_duration = defaultdict(lambda: defaultdict(float))
        for rec in previous_duration:
            for previous_attendance in rec[1]:
                mapped_previous_duration[rec[0]][check_in_tz(previous_attendance).date()] += previous_attendance.worked_hours
