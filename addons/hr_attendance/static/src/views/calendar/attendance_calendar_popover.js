...
// icons for known attendance fields shown in the popover (decorative; label is shown by the field widget)
const FIELD_ICONS = {
    work_entry_type_id: `<i class="oi oi-fw text-400" data-icon="label" aria-hidden="true"/>`,
    break_duration: `<i class="oi oi-fw text-400" data-icon="pause_circle" aria-hidden="true"/>`,
};
    ...
    getDefaultPopoverBody() {
        ...
        items.push(`
            <div class="alert alert-warning py-1 px-2 mb-0 small" invisible="not source_stale">
                <i class="oi oi-fw me-1" data-icon="warning" role="img" aria-label="Warning" title="Warning"/>
        ...
        if (this.time) {
            ...
            items.push(`
                <div class="d-flex align-items-baseline gap-2">
                    <i class="oi oi-fw text-400" data-icon="schedule" aria-hidden="true"/>
                    <span class="fw-bold">${this.time}</span>${duration}
                </div>
            `);
        }

        // 4. premium pay options
        const ppNode = fieldNodes.find(
            (fn) => fn.name === "category_options_ids" && !["1", "True"].includes(fn.invisible)
        );
        if (ppNode) {
            const widget = `widget="${ppNode.widget || ppNode.type}"`;
            const options = `options='${JSON.stringify(ppNode.options)}'`;
            items.push(
                `<div class="d-flex align-items-baseline gap-2" invisible="not category_options_ids">` +
                    `<field name="category_options_ids" ${options} ${widget}/>` +
                `</div>`
            );
        }
