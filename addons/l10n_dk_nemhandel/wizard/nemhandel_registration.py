...
class NemhandelRegistration(models.TransientModel):
    ...
    receiver_registration = fields.Boolean(string='Register as a receiver', default=True)
    ...
    def _onchange_phone_number(self):
        for wizard in self:
            if wizard.phone_number:
                wizard.company_id._sanitize_nemhandel_phone_number(wizard.phone_number)
    ...
    def _action_open_nemhandel_form(self, reopen=True):
        action_dict = {
            ...
            'view_mode': 'form',
            'res_model': 'nemhandel.registration',
            'target': 'new',
    ...
    @handle_demo
    def button_nemhandel_sender_registration(self):
        """
        ...
        - Allows the user to become a sender but not a receiver on the Nemhandel network.
        Basically, a Sender does not exist on the nemhandel network. They use our
        Access Point to send invoices to Nemhandel participants without having to register
        themselves.
        """
        self.ensure_one()
        ...
        if not modules.module.current_test:
            self.env.cr.commit()
        vat = company.vat[2:] if company.vat and company.vat[:2].isalpha() else company.vat
        params = {
            'company_details': {
        ...
        if edi_user.edi_mode != 'demo':
            return self.button_send_nemhandel_verification_code()
        return self._action_open_nemhandel_form()

    @handle_demo
    def button_nemhandel_receiver_registration(self):
        """
        The second (optional) step in Nemhandel registration.
        The user can choose to become a Receiver and officially register on the Nemhandel
        network, i.e. receive documents from other Nemhandel participants.
        """
        self.ensure_one()
        try:
            self.edi_user_id._nemhandel_register_sender_as_receiver()
    ...
    def button_send_nemhandel_verification_code(self):
        """
    ...
    def button_check_nemhandel_verification_code(self):
        ...
        self.l10n_dk_nemhandel_proxy_state = 'sender'
        ...
        return self._action_send_notification(
            ...
            message=_("You can now send invoices via Nemhandel."),
        )
