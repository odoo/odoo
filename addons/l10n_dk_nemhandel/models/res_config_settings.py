...
class ResConfigSettings(models.TransientModel):
    ...
    def button_nemhandel_receiver_registration(self):
        ...
        """
        self.ensure_one()
        self.nemhandel_edi_user._nemhandel_register_sender_as_receiver()
        if self.l10n_dk_nemhandel_proxy_state == 'receiver':
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': _("Registered to receive documents via Nemhandel."),
                    'type': 'success',
