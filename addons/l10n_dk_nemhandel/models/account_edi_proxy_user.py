...
class AccountEdiProxyClientUser(models.Model):
    ...
    @api.model
    def _nemhandel_can_send_domain(self):
    ...
    def _cron_nemhandel_get_participant_status(self):
        edi_users = self.search([('company_id.l10n_dk_nemhandel_proxy_state', 'in', ['in_verification', 'sender'])])
    ...
    def _nemhandel_get_participant_status(self):
        for edi_user in self:
            try:
                proxy_user = edi_user._make_request(f"{edi_user._get_server_url()}/api/nemhandel/1/participant_status")
            except AccountEdiProxyError as e:
                _logger.error('Error while updating Nemhandel participant status: %s', e)
                continue

            if proxy_user['nemhandel_state'] in {'sender', 'receiver', 'rejected'}:
    ...
    def _nemhandel_register_sender_as_receiver(self):
        self.ensure_one()

        company = self.company_id
        edi_identification = self._get_proxy_identification(company, 'nemhandel')

        if company.l10n_dk_nemhandel_proxy_state != 'sender':
            ...
            raise UserError(
                _('Cannot register a user with a %s application', nemhandel_state_translated))
    ...
    # TODO check if we want to be able to easily extend to other services (with new modules or not)
