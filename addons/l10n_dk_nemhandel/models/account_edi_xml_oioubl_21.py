...
class AccountEdiXmlOIOUBL21(models.AbstractModel):
    ...
    def _get_partner_party_vals(self, partner, role):
        ...
        if partner.nemhandel_identifier_type and partner.nemhandel_identifier_value:
            prefix = 'DK%s' if partner.nemhandel_identifier_type == '0184' else '%s'
            vals.update({
                'endpoint_id': prefix % partner.nemhandel_identifier_value,
                'endpoint_id_attrs': {'schemeID': partner.nemhandel_identifier_type},
            })
    ...
    def _get_tax_unece_codes(self, customer, supplier, tax):
        ...
        res = super()._get_tax_unece_codes(customer, supplier, tax)
        tax_code_mapping = {
            # Simple mapping between tax type provided in UBL and what is accepted in OIOUBL
            # https://docs.peppol.eu/poacc/billing/3.0/codelist/UNCL5305/
            # https://www.oioubl.info/codelists/en/urn_oioubl_id_taxcategoryid-1.1.html
        ...
        res.update({'tax_category_code': tax_code_mapping.get(res.get('tax_category_code'))})
        return res
