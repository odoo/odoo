...
class ResCompany(models.Model):
    ...
    def _get_default_vat_disabled_tax(self):
        ...
        return self._get_or_create_chart_template_record('account.tax', 'attn_VAT-OUT-00-NA-S', {})
    ...
    def _inverse_vat_disabled(self):
        ...
        for company in self.filtered(lambda c: c.vat_disabled_available and c.account_fiscal_country_id.code == 'BE' and c.chart_template):
            ...
            no_subject_to_vat_fp = company._get_or_create_chart_template_record('account.fiscal.position', 'fiscal_position_template_7', chart_template_data)
            ...
            default_purchase_tax = company._get_or_create_chart_template_record('account.tax', company_data['account_purchase_tax_id'], chart_template_data)
            vat_disabled_purchase_21 = company._get_or_create_chart_template_record('account.tax', 'attn_VAT-IN-21-ND', chart_template_data)
            vat_disabled_purchase_12 = company._get_or_create_chart_template_record('account.tax', 'attn_VAT-IN-12-ND', chart_template_data)
            vat_disabled_purchase_06 = company._get_or_create_chart_template_record('account.tax', 'attn_VAT-IN-06-ND', chart_template_data)
            vat_disabled_purchase_00 = company._get_or_create_chart_template_record('account.tax', 'attn_VAT-IN-00-ND', chart_template_data)
            ...
            if company.vat_disabled:
                ...
                company.account_purchase_receipt_fiscal_position_id = company._get_or_create_chart_template_record('account.fiscal.position', company_data['account_purchase_receipt_fiscal_position_id'], chart_template_data)
