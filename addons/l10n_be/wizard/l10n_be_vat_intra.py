...
class partner_vat_intra(osv.osv_memory):
    ...
    def _get_datas(self, cr, uid, ids, context=None):
        ...
        if len(wiz_data.period_code) != 6:
            raise Warning(_('Error!'), _('Period code is not valid.'))

        if not wiz_data.period_ids:
            raise Warning(_('Insufficient Data!'),_('Please select at least one Period.'))

        p_id_list = obj_partner.search(cr, uid, [('vat','!=',False)], context=context)
        if not p_id_list:
            raise Warning(_('Insufficient Data!'),_('No partner has a VAT number associated with him.'))
    ...
    <ns2:Representative>
        ...
        for client in xml_data['clientlist']:
            if not client['vatnum']:
                raise Warning(_('Insufficient Data!'),_('No vat number defined for %s.') % client['partner_name'])
