...
class partner_vat(osv.osv_memory):
    ...
    def get_partner(self, cr, uid, ids, context=None):
        ...
        if not period_ids:
             raise UserError(_('Insufficient Data!'), _('No data for the selected year.'))

        partners = []
        partner_ids = obj_partner.search(cr, uid, [('vat_subjected', '!=', False), ('vat','ilike','BE%')], context=context)
        if not partner_ids:
             raise UserError(_('Error'),_('No belgium contact with a VAT number in your database.'))
        ...
        if not partners:
            raise UserError(_('Insufficient Data!'), _('No data found for the selected year.'))
...
class partner_vat_list(osv.osv_memory):
    ...
    def create_xml(self, cr, uid, ids, context=None):
        ...
        if not company_vat:
            raise UserError(_('Insufficient Data!'),_('No VAT number associated with the company.'))
...
<ns2:ClientListingConsignment xmlns="http://www.minfin.fgov.be/InputCommon" xmlns:ns2="http://www.minfin.fgov.be/ClientListingConsignment" ClientListingsNbr="1">
    <ns2:Representative>
        ...
        if not client_datas:
            raise UserError(_('Data Insufficient!'),_('No data available for the client.'))
        data_client_info = ''
...
""" % annual_listing_data
    ...
    def print_vatlist(self, cr, uid, ids, context=None):
        ...
        if not datas['client_datas']:
            raise UserError(_('Error!'), _('No record to print.'))
