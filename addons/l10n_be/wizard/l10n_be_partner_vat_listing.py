...
class partner_vat(osv.osv_memory):
    ...
    def get_partner(self, cr, uid, ids, context=None):
        ...
        if not period_ids:
             raise Warning(_('Insufficient Data!'), _('No data for the selected year.'))

        partners = []
        partner_ids = obj_partner.search(cr, uid, [('vat_subjected', '!=', False), ('vat','ilike','BE%')], context=context)
        if not partner_ids:
             raise Warning(_('Error'),_('No belgium contact with a VAT number in your database.'))
        ...
        if not partners:
            raise Warning(_('Insufficient Data!'), _('No data found for the selected year.'))
...
class partner_vat_list(osv.osv_memory):
    ...
    def create_xml(self, cr, uid, ids, context=None):
        ...
        if not company_vat:
            raise Warning(_('Insufficient Data!'),_('No VAT number associated with the company.'))
        ...
        if not email:
            raise Warning(_('Insufficient Data!'),_('No email address associated with the company.'))
        if not phone:
            raise Warning(_('Insufficient Data!'),_('No phone associated with the company.'))
        annual_listing_data = {
            'issued_by': issued_by,
...
<ns2:ClientListingConsignment xmlns="http://www.minfin.fgov.be/InputCommon" xmlns:ns2="http://www.minfin.fgov.be/ClientListingConsignment" ClientListingsNbr="1">
    <ns2:Representative>
        ...
        if not client_datas:
            raise Warning(_('Data Insufficient!'),_('No data available for the client.'))
        data_client_info = ''
...
""" % annual_listing_data
    ...
    def print_vatlist(self, cr, uid, ids, context=None):
        ...
        if not datas['client_datas']:
            raise Warning(_('Error!'), _('No record to print.'))
