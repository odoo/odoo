...
class TestSalePurchaseProject(TestSalePurchase, TestReInvoice):
    ...
    def test_purchase_order_reinvoiced_so_count(self):
        ...
        project = self.env['project.project'].create({
            'name': 'SO Project',
            f'{self.analytic_plan._column_name()}': self.test_analytic_account_1.id,
        })
        ...
        invoice.action_post()
        self.assertEqual(
            invoice.invoice_line_ids.reinvoiced_so_line_id,
        ...
        self.assertEqual(
            ...
            1,
            "There should be one SO from which reinvoicing was done from the purchase order's bills",
        )
