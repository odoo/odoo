...
def migrate(cr, version):
    ...
    chart_template_code = 'br'
    companies = env["res.company"].search(
        [("chart_template", "=", chart_template_code), ("parent_id", "=", None)], order="parent_path"
    )

    ChartTemplate = env["account.chart.template"]
    all_taxes_tpl = ChartTemplate._get_account_tax(chart_template_code)
    all_accounts_tpl = ChartTemplate._get_account_account(chart_template_code)
    all_tax_groups_tpl = ChartTemplate._get_account_tax_group(chart_template_code)

    all_keys = NEW_TAXES | NEW_ACCOUNT_ACCOUNTS | NEW_TAX_GROUPS
    wanted_xmlids = [f"{c.id}_{k}" for c in companies for k in all_keys]
    ...
    existing_xmlids = {row for (row,) in cr.fetchall()}
    for company in companies:

        def is_missing(imd_name):
            return f"{company.id}_{imd_name}" not in existing_xmlids

        taxes = {k: v for k, v in all_taxes_tpl.items() if k in NEW_TAXES and is_missing(k)}
        accounts = {k: v for k, v in all_accounts_tpl.items() if k in NEW_ACCOUNT_ACCOUNTS and is_missing(k)}
        tax_groups = {k: v for k, v in all_tax_groups_tpl.items() if k in NEW_TAX_GROUPS and is_missing(k)}
        if taxes or accounts or tax_groups:
