    ...
    def modify_production_order_state(self, cr, uid, ids, action):
        ...
        if action == 'start':
            if prod_obj.state =='confirmed':
                ...
                raise UserError(_('Error!'),_('Manufacturing order cannot be started in state "%s"!') % (prod_obj.state,))
        else:
    ...
    def check_operation(self, cr, uid, vals):
        ...
        if not oper_objs:
            if code.start_stop!='start':
                raise UserError(_('Sorry!'),_('Operation is not started yet!'))
                return False
        else:
            for oper in oper_objs:
                 code_lst.append(oper.code_id.start_stop)
            if code.start_stop=='start':
                    if 'start' in code_lst:
                        raise UserError(_('Sorry!'),_('Operation has already started! You can either Pause/Finish/Cancel the operation.'))
                        return False
            if code.start_stop=='pause':
                    if  code_lst[len(code_lst)-1]!='resume' and code_lst[len(code_lst)-1]!='start':
                        raise UserError(_('Error!'),_('In order to Pause the operation, it must be in the Start or Resume state!'))
                        return False
            if code.start_stop=='resume':
                if code_lst[len(code_lst)-1]!='pause':
                   raise UserError(_('Error!'),_('In order to Resume the operation, it must be in the Pause state!'))
                   return False

            if code.start_stop=='done':
               if code_lst[len(code_lst)-1]!='start' and code_lst[len(code_lst)-1]!='resume':
                  raise UserError(_('Sorry!'),_('In order to Finish the operation, it must be in the Start or Resume state!'))
                  return False
               if 'cancel' in code_lst:
                  raise UserError(_('Sorry!'),_('Operation is Already Cancelled!'))
                  return False
            if code.start_stop=='cancel':
               if  not 'start' in code_lst :
                   raise UserError(_('Error!'),_('No operation to cancel.'))
                   return False
               if 'done' in code_lst:
                  raise UserError(_('Error!'),_('Operation is already finished!'))
                  return False
        return True
