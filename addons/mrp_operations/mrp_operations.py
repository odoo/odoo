...
class mrp_production_workcenter_line(osv.osv):
    def _get_date_end(self, cr, uid, ids, field_name, arg, context=None):
        ...
        date_and_hours_by_cal = [(op.date_planned, op.hour, op.workcenter_id.calendar_id.id) for op in ops if
                                 op.date_planned]
    ...
    _columns = {
        ...
        'delay': fields.float('Working Hours',
        ...
        'production_state': fields.related('production_id', 'state',
                                           type='selection',
                                           selection=[('draft', 'Draft'), ('confirmed', 'Waiting Goods'),
                                                      ('ready', 'Ready to Produce'), ('in_production', 'In Production'),
                                                      ('cancel', 'Canceled'), ('done', 'Done')],
                                           string='Production Status', readonly=True),
        'product': fields.related('production_id', 'product_id', type='many2one', relation='product.product',
                                  string='Product',
                                  readonly=True),
        'qty': fields.related('production_id', 'product_qty', type='float', string='Qty', readonly=True, store=True),
        'uom': fields.related('production_id', 'product_uom', type='many2one', relation='product.uom',
        ...
        if action == 'start':
            if prod_obj.state == 'confirmed':
                ...
                raise osv.except_osv(_('Error!'),
                                     _('Manufacturing order cannot be started in state "%s"!') % (prod_obj.state,))
        else:
            ...
            if flag:
                for production in prod_obj_pool.browse(cr, uid, [prod_obj.id], context=None):
                    if production.move_lines or production.move_created_ids:
                        prod_obj_pool.action_produce(cr, uid, production.id, production.product_qty, 'consume_produce',
                                                     context=None)
        ...
        if vals.get('date_planned', False) and update:
            for prod in self.browse(cr, uid, ids, context=context):
                if prod.production_id.workcenter_lines:
                    ...
                    prod_obj.write(cr, uid, [prod.production_id.id], {'date_start': dstart}, context=context,
                                   mini=False)
        return result
...
class mrp_production(osv.osv):
    ...
    _columns = {
        'allow_reorder': fields.boolean('Free Serialisation',
                                        help="Check this to be able to move independently all production orders, without moving dependent ones."),
    }
                        ...
                        if l.production_id.state not in ('done', 'cancel'):
                            ...
                            if l.production_id.date_start > dt.strftime('%Y-%m-%d %H:%M:%S'):
                                self.write(cr, uid, [l.production_id.id],
                                           {'date_start': dt.strftime('%Y-%m-%d %H:%M:%S')}, mini=True)
        return True
    ...
    def write(self, cr, uid, ids, vals, context=None, update=True, mini=True):
        ...
        if (vals.get('workcenter_lines', False) or vals.get('date_start', False) or vals.get('date_planned',
                                                                                             False)) and update:
...
class mrp_operations_operation_code(osv.osv):
    ...
    _columns = {
        ...
        'start_stop': fields.selection(
            [('start', 'Start'), ('pause', 'Pause'), ('resume', 'Resume'), ('cancel', 'Cancelled'), ('done', 'Done')],
...
class mrp_operations_operation(osv.osv):
    ...
    def calc_delay(self, cr, uid, vals):
        ...
        oper_ids = self.search(cr, uid, [('production_id', '=', vals['production_id']),
        ...
        if not oper_objs:
            ...
            if code.start_stop == 'start':
                if 'start' in code_lst:
                    raise osv.except_osv(_('Sorry!'), _(
                        'Operation has already started! You can either Pause/Finish/Cancel the operation.'))
                    return False
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'date_start' in vals:
            ...
            wc_op_id = self.pool.get('mrp.production.workcenter.line').search(cr, uid, [
                ('workcenter_id', '=', vals['workcenter_id']), ('production_id', '=', vals['production_id'])])
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        wc_op_id = workcenter_pool.search(cr, uid, [('workcenter_id', '=', vals['workcenter_id']),
        ...
        if code.start_stop in ('start', 'done', 'pause', 'cancel', 'resume'):
            if not wc_op_id:
                ...
                wc_op_id.append(workcenter_pool.create(cr, uid, {'production_id': vals['production_id'],
            ...
            if code.start_stop == 'done':
                ...
                self.pool.get('mrp.production').write(cr, uid, vals['production_id'],
    ...
    _columns = {
        ...
        'date_start': fields.datetime('Start Date'),
        'date_finished': fields.datetime('End Date'),
        'order_date': fields.function(_get_order_date, string='Order Date', type='date',
                                      store={'mrp.production': (_order_date_search_production, ['date_planned'], 10)}),
    }
    _defaults = {
