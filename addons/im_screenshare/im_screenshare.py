...
class Controller(openerp.addons.bus.bus.Controller):
    ...
    def start(self, mode='record', **kwargs):
        ...
        if( mode == 'record'):
            ...
            data = {
                'name' : "New Screen Record (%d-%s)" % (uid, datetime.datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
                'user_id' : uid,
            }
...
class im_screenshare_record(osv.Model):
    ...
    def _event_count(self, cr, uid, ids, field_name, arg, context=None):
        Event = self.pool['im_screenshare.record.event']
        return {
            rec_id: Event.search_count(cr, uid, [('screen_record_id', '=', rec_id)], context=context) for rec_id in ids
        }
    ...
    def _get_duration(self, cr, uid, ids, fields, arg, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if events:
                ...
                result[record.id] = (end_date - start_date).seconds # seconds
            else:
                result[record.id] = 0
        return result
    ...
    _columns = {
        ...
        'duration' : fields.function(_get_duration, string='Duration', help="Difference between first and last events of the record, in seconds."),
        'user_id' : fields.many2one('res.users', 'User', readonly=True),
        'event_ids' : fields.one2many('im_screenshare.record.event', 'screen_record_id', 'Events', readonly=True),
        'description' : fields.text('Description'),
        'event_count': fields.function(_event_count, string='# Event', type='integer'),
    }
    _defaults = {
...
class im_screenshare_record_event(osv.Model):
    ...
    _columns = {
        'screen_record_id' : fields.many2one('im_screenshare.record', 'Screen Record', readonly=True, ondelete='cascade'),
