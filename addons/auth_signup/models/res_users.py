    ...
    def _action_reset_password(self, signup_type="reset"):
        ...
        for user in self:
            ...
            with contextlib.closing(self.env.cr.savepoint()):
                ...
                account_created_template = portal_account_created_template if is_portal else internal_account_created_template
                if account_created_template:
                    portal_email_from = {'email_from': user.company_id.email_formatted or self.env.user.email_formatted} if is_portal else {}
