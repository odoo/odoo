    ...
    def send_mail_test(self):
        ...
        if invalid_candidates:
            notification_messages.append(
                self.env._(
                    "Mailing addresses incorrect: %s", ", ".join(invalid_candidates)
                )
            )
        ...
        for mail_sudo in mails_sudo:
            if mail_sudo.state == "sent":
                notification_messages.append(
                    self.env._(
                        "Test mailing successfully sent to %s", mail_sudo.email_to
                    )
                )
            elif mail_sudo.state == "exception":
                notification_messages.append(
                    self.env._(
                        "Test mailing could not be sent to %s:", mail_sudo.email_to
                    )
