...
class TestMailingABTesting(MassMailCommon):
    ...
    def _create_ab_testing_mailings(self, nbr_testing_mailings, testing_mode='based_on'):
        ...
        for i in range(nbr_testing_mailings):
            values = dict({
                'ab_testing_mailing_id': ab_testing.id,
            ...
            values.update({
                'subject': 'Test %s' % (i + 1),
                'ab_testing_version': chr(ord('A') + i),
            })
    ...
    def test_mailing_ab_testing_auto_flow(self):
        ...
        self.assertEqual(ab_testing.mailing_ids[0].ab_testing_contact_pc, 0.125)
        self.assertEqual(ab_testing.mailing_ids[1].ab_testing_contact_pc, 0.125)
        ...
        ab_testing.mailing_ids.invalidate_cache()
        winner_mailing = ab_testing.mailing_ids.filtered(lambda mailing: mailing.ab_testing_is_winner)
        self.assertEqual(ab_testing.state, 'done')
        self.assertEqual(winner_mailing.subject, 'Test 1')
        self.assertEqual(winner_mailing.ab_testing_version, 'A - Final')
        self.assertEqual(winner_mailing.ab_testing_contact_pc, 0.75)
        self.assertEqual(mailing_a.ab_testing_contact_pc, 0.125)
        self.assertEqual(mailing_b.ab_testing_contact_pc, 0.125)
    ...
    def test_mailing_ab_testing_manual_flow(self):
        ...
        self.assertEqual(winner_mailing.ab_testing_version, 'B - Final')
        self.assertEqual(winner_mailing.ab_testing_contact_pc, 0.75)
        self.assertEqual(mailing_a.ab_testing_contact_pc, 0.125)
        self.assertEqual(mailing_b.ab_testing_contact_pc, 0.125)
