...
class MassMailingContact(models.Model):
    ...
    name_is_split = fields.Boolean(
        ...
        default=lambda self: self._get_name_is_split(),
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        if not self._get_name_is_split():
    ...
    def _compute_name_is_split(self):
        name_is_split = self._get_name_is_split()
        for record in self:
    ...
    def action_view_mailing_contacts(self):
        """ Show all mailing contacts.

        The method adds a system parameter in the context (name_is_split) to configure the views
        and eval the context.
        """
