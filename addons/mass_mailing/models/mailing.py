...
class MailingMailing(models.Model):
    ...
    def _check_is_template_mailing_type(self):
        for mailing in self:
            if mailing.is_template and mailing.mailing_type != 'mail':
                raise ValidationError(_("Templates are only supported for Email mailings."))
    ...
    def action_fetch_templates(self, extra_domain=None):
        """Return all active mailings set as template and skip mailings with empty body."""
    ...
    def action_save_as_template(self):
        self.ensure_one()
        if tools.is_html_empty(self.body_arch):
            raise UserError(_("No design has been created yet!"))
        ...
        for attachment in self.attachment_ids:
            new_attachments |= attachment.copy({'res_id': self.id})
        template.attachment_ids = [Command.set(new_attachments.ids)]

        return template

    def action_new_mailing_from_template(self):
        self.ensure_one()
        new_attachments = self.env['ir.attachment']
        for attachment in self.attachment_ids:
            new_attachments |= attachment.copy({'res_id': self.id})
        return {
            ...
            'context': {
                **clean_context(self.env.context),
                'default_attachment_ids': new_attachments.ids,
