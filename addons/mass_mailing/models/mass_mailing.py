...
<p class="o_message_paragraph">Join our Marketing newsletter and get <strong>this white paper instantly</strong></p>"""
    ...
    popup_content = fields.Html(string="Website Popup Content", translate=True, required=True, sanitize=False, default=_default_popup_content)
    popup_redirect_url = fields.Char(string="Website Popup Redirect URL", default='/')

    def _compute_contact_nbr(self):
        Contacts = self.env['mail.mass_mailing.contact']
        for group in Contacts.read_group([('list_id', 'in', self.ids), ('opt_out', '!=', True)], ['list_id'], ['list_id']):
            self.browse(group['list_id'][0]).contact_nbr = group['list_id_count']
...
class MassMailingContact(models.Model):
    ...
    def on_change_opt_out(self):
        self.unsubscription_date = self.opt_out and fields.Datetime.now()
...
class MassMailingCampaign(models.Model):
    ...
    source_id = fields.Many2one(default=lambda self: self.env.ref('utm.utm_source_newsletter'))
    medium_id = fields.Many2one(default=lambda self: self.env.ref('utm.utm_medium_email'))
    ...
    def _compute_clicks_ratio(self):
        ...
        for record in self.env.cr.dictfetchall():
            self.browse(record['id']).clicks_ratio = 100 * record['nb_clicks'] / record['nb_mails']
    ...
    def _compute_total_mailings(self):
        for campaign in self.env['mail.mass_mailing'].read_group([('mass_mailing_campaign_id', 'in', self.ids)], ['mass_mailing_campaign_id'], ['mass_mailing_campaign_id']):
            self.browse(campaign['mass_mailing_campaign_id'][0]).total_mailings = campaign['mass_mailing_campaign_id_count']

    @api.onchange('name')
    def on_change_campaign_name(self):
        if self.name:
            if self.campaign_id:
                self.campaign_id.write({'name': self.name})
            else:
                self.campaign_id = self.campaign_id.create({'name': self.name})
    ...
    def get_recipients(self, model=None):
        ...
        domain = [('mass_mailing_campaign_id', '=', self.id)]
        if model:
            domain += [('model', '=', model)]
        return set(self.env['mail.mail.statistics'].search(domain).ids)
...
class MassMailing(models.Model):
    ...
    opened_ratio = fields.Integer(compute="_compute_statistics", string='Opened Ratio')
    replied_ratio = fields.Integer(compute="_compute_statistics", string='Replied Ratio')
    bounced_ratio = fields.Integer(compute="_compute_statistics", String='Bouncded Ratio')
    next_departure = fields.Datetime(compute="_compute_next_departure", string='Next Departure')

    def _compute_total(self):
        for mass_mailing in self:
            mass_mailing.total = len(mass_mailing.get_recipients())
    ...
    def _compute_next_departure(self):
        cron_next_call = self.env.ref('mass_mailing.ir_cron_mass_mailing_queue').nextcall
        for mass_mailing in self:
            schedule_date = mass_mailing.schedule_date
            if schedule_date:
                if fields.Datetime.now() > schedule_date:
    ...
    def update_opt_out(self, mailing_id, email, res_ids, value):
        ...
        model = self.env[mailing.mailing_model]
    ...
    def send_mail(self):
        ...
        for mailing in self:
            ...
            if self.env.context:
                comp_ctx = dict(self.env.context, active_ids=res_ids)
            else:
            ...
            composer_values = {
                ...
                'body': self.convert_links()[mailing.id],
            ...
            composer = self.env['mail.compose.message'].with_context(comp_ctx).create(composer_values)
            composer.with_context(comp_ctx).send_mail(auto_commit=True)
            mailing.state = 'done'
        return True
    ...
    def _process_mass_mailing_queue(self):
        mass_mailings = self.sudo().search([('state', 'in', ('in_queue', 'sending')), '|', ('schedule_date', '<', fields.Datetime.now()), ('schedule_date', '=', False)])
