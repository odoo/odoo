...
class MailMailStats(models.Model):
    ...
    mail_mail_id = fields.Many2one('mail.mail', string='Mail', ondelete='set null', select=True)
    ...
    mass_mailing_campaign_id = fields.Many2one(
        related='mass_mailing_id.mass_mailing_campaign_id', ondelete='set null',
    ...
    def _compute_state(self):
        self.update({'state': 'outgoing', 'state_update': fields.Datetime.now()})
        for stat in self:
            if stat.exception:
    ...
    def _get_ids(self, mail_mail_ids=None, mail_message_ids=None, domain=None):
