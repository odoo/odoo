...
class MailingList(models.Model):
    ...
    def action_merge(self, src_lists):
        ...
        coalesced_select_fields = SQL(','.join(f"COALESCE({field}, '')" for field in select_fields))
        coalesced_target_contact_select_fields = SQL(
            ','.join(f"COALESCE(target_contact.{field}, '')" for field in unformated_select_fields)
        )
        ...
        distinct_contact_subscriptions_cte = SQL("""
SELECT DISTINCT ON (%(select_fields)s)
                   src_sub.id AS subscription_id
              FROM mailing_contact src_contact
              JOIN mailing_subscription src_sub ON src_contact.id = src_sub.contact_id
             WHERE src_sub.list_id IN %(src_list_ids)s
               AND (%(contact_condition)s)
               AND (%(coalesced_select_fields)s) NOT IN (
            SELECT %(coalesced_target_contact_select_fields)s
          ...
          -- order by IS TRUE as otherwise NULLs will always be at the top
          ORDER BY %(select_fields)s, src_sub.opt_out IS TRUE DESC
        """,
        coalesced_select_fields=coalesced_select_fields,
        coalesced_target_contact_select_fields=coalesced_target_contact_select_fields,
        ...
        # now for every distinct contact, update their first subscription to be linked to the target list
        update_query = SQL("""
              WITH new_sub AS (%(distinct_contact_subscriptions_cte)s)
            UPDATE mailing_subscription AS src_sub
               SET list_id = %(target_list_id)s
             WHERE src_sub.id IN (
                   SELECT new_sub.subscription_id
                     FROM new_sub
            )
        """,
        distinct_contact_subscriptions_cte=distinct_contact_subscriptions_cte,
        target_list_id=self.id,
        )
    ...
    def _mailing_list_get_contact_condition(self):
        return SQL("src_contact.email_normalized NOT IN (select email from mail_blacklist where active = TRUE)")
