...
class IrActionsActions(models.Model):
    ...
    def _get_bindings(self, model_name):
        ...
        provided the user has at least composer creation permissions (i.e. "can send emails").
        """
        ...
        if getattr(self.env[model_name], '_mailing_enabled', False) and model_name not in self._get_send_email_binding_excluded_models() \
            and self.env['mail.compose.message'].has_access('create'):
            if not self.env['mail.compose.message'].has_access('create') and not self.env.user.has_group('mass_mailing.group_mass_mailing_user'):
                return bindings
