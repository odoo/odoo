...
publicWidget.registry.MailingPortalSubscription = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        // Form
        this.listInfo = [...document.querySelectorAll("#o_mailing_subscription_form_manage input")].map((node) => {
            const listInfo = {
    ...
    _updateDisplay: function () {
        ...
        this._updateDisplayFeedback({ cleanFeedback: true });
    },
    ...
    _onBlocklistAddClick: async function (event) {
        ...
        return await rpc("/mailing/blocklist/add", {
    ...
    _onBlocklistRemoveClick: async function (event) {
        event.preventDefault();
        return await rpc("/mailing/blocklist/remove", {
            document_id: this.customerData.documentId,
            email: this.customerData.email,
            hash_token: this.customerData.hashToken,
            mailing_id: this.customerData.mailingId,
    ...
    _updateInfoBlocklist: function (infoKey) {
        ...
        if (infoKey !== undefined) {
            const infoContent = renderToElement("mass_mailing.portal.blocklist_update_info", {
                infoKey: infoKey,
            });
    ...
    _onFeedbackSendClick: async function (event) {
        ...
        return await rpc("/mailing/feedback", {
            ...
            if (result === true) {
                this._updateDisplayFeedback({ setReadonly: true });
    ...
    /*
     * Display feedback (visual tips) to the user concernin
        g the last done action.
     */
    _updateInfoFeedback: function (infoKey) {
        const feedbackInfo = document.getElementById("o_mailing_subscription_feedback_info");
        if (infoKey !== undefined) {
            const infoContent = renderToElement("mass_mailing.portal.feedback_update_info", {
                infoKey: infoKey,
            });
    ...
    _onSendForm: async function (event) {
        event.preventDefault();
        const selectedOptOutReason = document.querySelector(
            "div#o_mailing_subscription_feedback form input.o_mailing_subscription_opt_out_reason:checked"
        );
        ...
        return await rpc("/mailing/list/update", {
            csrf_token: formData.get("csrf_token"),
            document_id: this.customerData.documentId,
            email: this.customerData.email,
            opt_out_reason_id: optoutReasonId,
    ...
    _updateInfoForm: function (infoKey) {
        const updateInfo = document.getElementById("o_mailing_subscription_update_info");
        const infoContent = renderToFragment("mass_mailing.portal.list_form_update_status", {
            infoKey: infoKey,
        });
