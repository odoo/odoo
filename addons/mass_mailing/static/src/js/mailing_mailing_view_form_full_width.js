...
const throttleByAnimationFrame = (fn) => {
    let triggered = false;
    let calledBetweenFrame = false;
    return (...args) => {
        if (!triggered) {
            triggered = true;
            fn(...args);
            window.requestAnimationFrame(() => {
                triggered = false;
                if (calledBetweenFrame) {
                    return fn(...args);
                }
                calledBetweenFrame = false;
            });
        } else {
            calledBetweenFrame = true;
        }
    }
}
...
export class MassMailingFullWidthViewController extends formView.Controller {
    setup() {
        ...
        onMounted(() => {
            $('.o_content').on('scroll.repositionMailingEditorSidebar', throttleByAnimationFrame(this._repositionMailingEditorSidebar.bind(this)));
        });
        onWillUnmount(() => {
