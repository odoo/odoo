...
SyntaxError: Failed to execute 'querySelectorAll' on 'Element': '.button0:contins(brol)' is not a valid selector.`,
    ];
...
test("check for undeterminisms", async () => {
    ...
    await macroIsComplete(macro);
    expect.verifySteps([
        "log: [1/3] Tour tour_und → Step .button0",
        "log: [2/3] Tour tour_und → Step .button1",
...
[
  {
    "type": "childList",
    "target": "<button class=\\"button1\\">Text has changed :)</button>",
    "addedNodes": [
      "Text has changed :)"
    ],
    "removedNodes": [
...
UNDETERMINISM: element (button) has changed in 300ms for trigger .button1`,
    ]);
});
