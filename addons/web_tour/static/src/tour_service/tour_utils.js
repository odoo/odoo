...
export function serializeChanges(beforeElement, afterElement, changes = [], deep = 0) {
    ...
    // if (node === "" && afterElement.nodeType === Node.TEXT_NODE) {
    //     return;
    // }
    const hasChanged = {};
    if (maxLength) {
    ...
    new Set([...beforeAttrNames, ...afterAttrNames]).forEach((name) => {
        const oldValue = beforeElement.getAttribute(name);
        const newValue = afterElement.getAttribute(name);
    ...
    const sBeforeNodes = (beforeNodes || []).map(serializeNode);
    const sAfterNodes = (afterNodes || []).map(serializeNode);
    const addedNodes = sAfterNodes.filter((node) => !sBeforeNodes.includes(node));
    const removedNodes = sBeforeNodes.filter((node) => !sAfterNodes.includes(node));
    if (addedNodes.length) {
        hasChanged.addedNodes = addedNodes;
    }
    if (removedNodes.length) {
        hasChanged.removedNodes = removedNodes;
    }
