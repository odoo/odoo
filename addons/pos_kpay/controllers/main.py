...
class PosKpayController(http.Controller):
    ...
    @http.route('/pos_kpay/<string:kpay_unique_id>/notification', type='json', methods=['POST'], auth='public', save_session=False)
    def notification(self, kpay_unique_id):
        ...
        try:
            ...
            signaturePayload = '\n'.join([method, path, timestamp, nonce_str, request.httprequest.data.decode()]) + '\n'
            public_key = load_pem_public_key(f"-----BEGIN PUBLIC KEY-----{kpay_pm_sudo.kpay_public_key}-----END PUBLIC KEY-----".encode(), backend=default_backend())
            if isinstance(public_key, RSAPublicKey):
                public_key.verify(signature=signature.encode(), data=signaturePayload.encode(), padding=padding.PKCS1v15(), algorithm=hashes.SHA256())
            else:
                _logger.error('Unsupported public key type')
                return
        except InvalidSignature as e:
            _logger.error('Invalid signature received from KPay: %s', e)
            # return
