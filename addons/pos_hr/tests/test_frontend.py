...
class TestPosHrHttpCommon(TestPointOfSaleHttpCommon):
    ...
    def setUpClass(cls):
        ...
        cls.pos_admin.group_ids |= cls.env.ref('hr.group_hr_manager')
...
class TestUi(TestPosHrHttpCommon):
    ...
    def test_cash_move_keeps_employee(self):
        # Reading back the session's statement lines - which the employee is written on - needs
        # the accounting rights the tour's user had.
        self.pos_admin.group_ids = [Command.link(self.env.ref('account.group_account_invoice').id)]
        self.main_pos_config.with_user(self.pos_admin).open_ui()
        session = self.main_pos_config.current_session_id
        session.set_opening_control(0, None)
        session.with_user(self.pos_admin).try_cash_in_out(
            'out', 50, 'Test cash out', False, employee_id=self.emp1.id)

        self.assertEqual(session.bank_statement_line_ids[:1].employee_id, self.emp1)
    ...
    def test_change_on_rights_reflected_directly(self):
        """When changes in employee rights (manager/cashier/restrictive) should
