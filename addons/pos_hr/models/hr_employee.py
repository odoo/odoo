...
class HrEmployee(models.Model):
    ...
    def _load_pos_data(self, data):
        ...
        minimal_user_ids = employees.filtered(lambda emp: emp.user_id and data['pos.config'][0]['group_pos_minimal_user_id'] in emp.user_id.groups_id.ids).mapped('id')
        ...
        for employee in employees:
            if employee['user_id'] and employee['user_id'] in manager_ids or employee['id'] in data['pos.config'][0]['advanced_employee_ids']:
                role = 'manager'
            elif employee['user_id'] and employee['user_id'] in minimal_user_ids or employee['id'] in data['pos.config'][0]['minimal_employee_ids']:
                role = 'minimal'
            else:
                role = 'cashier'
