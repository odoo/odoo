...
test("PosHrTour: wrong pin, cashier rights and per-employee orders", async () => {
    ...
    expect(Utils.loginScreenIsShown()).toBe(true);
    expect(store.accessRight.loggedCashier).toBeEmpty();

    await Utils.login("Employee2", "5678");
    await waitFor(".product-screen");
    expect(store.accessRight.loggedCashier.name).toBe("Employee2");

    await Utils.switchCashier("Administrator", "1234");
    await waitFor(".product-screen");
    expect(store.accessRight.loggedCashier.name).toBe("Administrator");
    ...
    await waitFor(".product-screen");
    expect(store.accessRight.loggedCashier.name).toBe("Employee1");
    await Utils.openMenu();
...
test("test_cashier_user_can_change_price: unrestricted price control lets a cashier set a price", async () => {
    ...
    expect('.numpad button:contains("Price")').not.toHaveAttribute("disabled");
...
test("a restrictive employee gets no Price numpad button", async () => {
    const store = await Utils.setupAndMountPosHrApp();
    createTestProduct(store, { id: 9100, name: "Untaxed Product", price: 100, taxes_id: [] });
    ...
    await waitFor(".product-screen");
    expect(store.accessRight.loggedCashier._role).toBe("restrictive");
    expect(store.accessRight.disablePriceButton).toBe(false);
    if (!Utils.isMobile()) {
        expect('.numpad button:contains("Price")').toHaveAttribute("disabled");
    }
    expect(store.getOrder().lines[0].price_unit).toBe(100);
});

test("test_restrictive_employee_refund: refund pads are hidden for a restrictive employee", async () => {
    const store = await Utils.setupAndMountPosHrApp();
...
test("test_cost_and_margin_visibility: financials hidden only for a restrictive employee", async () => {
    const store = await Utils.setupAndMountPosHrApp({
...
test("test_switch_cashier_with_badge: scanning a badge switches the cashier", async () => {
    ...
    expect(store.accessRight.loggedCashier.name).toBe("A Little Guy");
