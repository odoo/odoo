...
class EdiFormat(models.Model):
    ...
    def _get_edi_format_settings(self, document=None, stage=None, flow_type=None):
        ...
        return {
            ...
            'batching_key': (document.move_type, document.l10n_es_edi_csv) if stage == 'Initialized' and document and document.is_invoice() else False,
            'stages': {
                'send': {
                    'Initialized': {
                        'new_state': 'to_send',
                        'action': self._send_invoice_to_sii,
                    },
                    'Invoice Succesfully Sent To SII': {
                        'new_state': 'sent',
    ...
    def _l10n_es_edi_call_web_service_sign(self, invoices, info_list):
        ...
        if resp_state == 'Correcto':
            ...
            return {inv.id: {'success': True} for inv in invoices}
    ...
    # def _get_invoice_edi_content(self, move): todo check that
    #     if self.code != 'es_sii':
    ...
    def _is_format_required(self, document, document_type=''):
        ...
        return document_type in ('invoice', 'payment') and document.l10n_es_edi_is_required
    ...
    def _send_invoice_to_sii(self, flows):
        # OVERRIDE
        ...
        for flow in flows:
            if res.get(flow.res_id, {}).get('success'):
                res[flow.res_id]['attachment'] = flow._create_or_update_edi_files([{
                    'type': 'binary',
                    'name': 'jsondump.json',
