...
export class HomeCounters extends Interaction {
    ...
    async updateCounters() {
        ...
        const proms = [...Array(Math.min(numberRpc, needed.length)).keys()].map(async i => {
            const documentsCountersData = await this.waitFor(rpc("/my/counters", {
        ...
        await this.waitFor(Promise.all(proms));
...
registry
    ...
    .add("portal.home_counters", HomeCounters);
