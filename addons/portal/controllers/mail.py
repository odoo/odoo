...
class PortalChatter(http.Controller):
    ...
    def portal_chatter_post(self, thread_model, thread_id, post_data, **kwargs):
        ...
        attachment_tokens = post_data.get("attachment_tokens", [])
        message = post_data.get("body")
        if not self._portal_post_has_content(thread_model, thread_id, message,
        ...
        portal_security = post_data.get("portal_security", {})
        post_values.update(
            (fname, portal_security.get(fname) or post_data.get(fname)) for fname in self._portal_post_filter_params())
        post_values['_hash'] = portal_security.get('hash')
