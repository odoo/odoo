...
class PortalChatter(http.Controller):
    ...
    def portal_chatter_init(self, **kwargs):
        portal_partner = request.env["res.partner"]._get_portal_partner_from_context() or request.env.user.partner_id
        partner_data = portal_partner.mail_partner_format()[portal_partner]
        partner_data["is_user_publisher"] = bool(portal_partner.user_id) and portal_partner.user_id.has_group(
    ...
    def portal_message_fetch(self, thread_model, thread_id, domain=False, limit=10, after=None, before=None, **kw):
        ...
        if kw.get("token"):
            access_as_sudo = bool(request.env["res.partner"]._get_portal_partner_from_context())
