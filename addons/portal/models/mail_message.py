...
class MailMessage(models.Model):
        ...
        for message, values in zip(self, vals_list):
            ...
            if 'author_avatar_url' in properties_names:
                if options and options.get("token"):
                    ...
                    values['author_avatar_url'] = f'/mail/avatar/mail.message/{message.id}/author_avatar/50x50?_hash={options["_hash"]}&pid={options["pid"]}'
                else:
            ...
            values.update({
                'reactions': message.sudo().reaction_ids._reaction_groups().get(message.id, []),
    ...
    def _validate_access_for_current_persona(self, operation):
        if self.env["res.partner"]._get_portal_partner_from_context():
            return True
