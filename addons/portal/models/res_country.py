...
class ResCountry(models.Model):
    ...
    def _get_cities_data(self, state_id=False):
        self.ensure_one()
        domain = Domain("country_id", "=", self.id)
        if state_id:
            domain = Domain.AND([domain, Domain("state_id", "in", [state_id, False])])
    ...
    def _enforce_city_choice(self):
        ...
        return self.enforce_cities and bool(
            self.env["res.city"].search_count([("country_id", "=", self.id)], limit=1)
        )

    def _get_vat_label(self):
        return self.vat_label or self.env.company.country_id.vat_label or self.env._("VAT")
