...
def check_portal_access(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        portal_res_model = kwargs.pop("portal_res_model", None) or kwargs.get(
            "thread_model"
        )
        if portal_token or (portal_hash and portal_pid):
            ...
            has_access = _check_special_access(
                portal_res_model, portal_res_id, portal_token, portal_hash, portal_pid
            )
            if has_access:
                req.update_context(
                ...
                if req.env.user._is_public() and hasattr(record, "partner_id"):
                    req.update_context(portal_partner=record.partner_id)
...
class MailThread(models.AbstractModel):
    ...
    def _get_portal_access(self):
        return self.env.context.get("portal_token") or (
            self.env.context.get("portal_pid") and self.env.context.get("portal_hash")
        )
    ...
    def _message_compute_author(
        ...
        _hash, pid = self.env.context.get("portal_hash"), self.env.context.get(
            "portal_pid"
        )
        if _hash and pid:
            author_id = pid
