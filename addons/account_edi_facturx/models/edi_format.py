...
class EdiFormat(models.Model):
        ...
        return {
            ...
            'stages': {
                'send': {
                    'Initialized': {
                        'new_state': 'to_send',
                        'action': self._create_facturx_attachment,
                    },
                    'XML File Created': {
                        'new_state': 'sent',
    ...
    def _prepare_document_report(self, pdf_writer, edi_files):
        self.ensure_one()
        if self.code != 'facturx_1_0_05':
            return super()._prepare_document_report(pdf_writer, edi_files)
        if not edi_files.attachment_id:
            return

        pdf_writer.embed_odoo_attachment(edi_files.attachment_id, subtype='text/xml')
        if not pdf_writer.is_pdfa and str2bool(self.env['ir.config_parameter'].sudo().get_param('edi.use_pdfa', 'False')):
            ...
            html = self.env['ir.qweb']._render('account_edi_facturx.account_invoice_pdfa_3_facturx_metadata', {
                    'title': edi_files.edi_flow._get_documents().name,
    ...
    def _update_document_from_xml_tree(self, filename, tree, document):
        self.ensure_one()
        if self._is_facturx(filename, tree):
            return self._import_facturx(tree, document)
        return super()._update_document_from_xml_tree(filename, tree, document)
    ...
    def _create_facturx_attachment(self, flows):
        ...
        for flow in flows:
            ...
            attachment_vals = self._export_facturx(invoice)
