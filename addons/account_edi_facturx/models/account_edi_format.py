...
class AccountEdiFormat(models.Model):
    ...
    def _prepare_invoice_report(self, pdf_writer, edi_document):
        ...
        if not pdf_writer.is_pdfa and str2bool(self.env['ir.config_parameter'].sudo().get_param('edi.use_pdfa', 'False')):
            ...
            html = self.env['ir.qweb']._render('account_edi_facturx.account_invoice_pdfa_3_facturx_metadata', {
                    'title': edi_document.move_id.name,
                    'date': fields.Date.context_today(self),
                },
                raise_if_not_found=False)
            if html:
                pdf_writer.add_file_metadata(html.encode())
    ...
    def _export_facturx(self, invoice):
        ...
        xml_content += self.env['ir.qweb']._render('account_edi_facturx.account_invoice_facturx_export', template_values)
