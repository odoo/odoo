...
class ChatRoom(models.Model):
    ...
    participant_count = fields.Integer("Participant count", compute="_compute_participant_count", copy=False)
    # reporting fields
    ...
    def _compute_participant_count(self):
        for room in self:
            room.discuss_channel_id.ensure_one()
            room.participant_count = len(room.discuss_channel_id[0].rtc_session_ids)

    @api.model_create_multi
    def create(self, values_list):
        for values in values_list:
            if not values.get("discuss_channel_id"):
                ...
                values["discuss_channel_id"] = new_discuss_channel.id
        return super().create(values_list)

    def get_room_url(self):
        self.ensure_one()
        return self.discuss_channel_id.invitation_url
