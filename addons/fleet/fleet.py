...
class fleet_vehicle(osv.Model):
    ...
    def _cost_count(self, cr, uid, ids, field_name, arg, context=None):
        ...
        cost_dict = { data['vehicle_id'][0]: data['vehicle_id_count'] for data in cost_data }
        return { vehicle_id: cost_dict.get(vehicle_id) for vehicle_id in ids }

    def _contract_count(self, cr, uid, ids, field_name, arg, context=None):
        contract_data = self.pool['fleet.vehicle.log.contract'].read_group(cr, uid, [('vehicle_id', 'in', ids)], ['vehicle_id'], ['vehicle_id'], context=context)
        contract_dict = { data['vehicle_id'][0]: data['vehicle_id_count'] for data in contract_data }
        return { vehicle_id: contract_dict.get(vehicle_id) for vehicle_id in ids }

    def _service_count(self, cr, uid, ids, field_name, arg, context=None):
        service_data = self.pool['fleet.vehicle.log.services'].read_group(cr, uid, [('vehicle_id', 'in', ids)], ['vehicle_id'], ['vehicle_id'], context=context)
        service_dict = { data['vehicle_id'][0]: data['vehicle_id_count'] for data in service_data }
        return { vehicle_id: service_dict.get(vehicle_id) for vehicle_id in ids }

    def _fuel_logs_count(self, cr, uid, ids, field_name, arg, context=None):
        fuel_data = self.pool['fleet.vehicle.log.fuel'].read_group(cr, uid, [('vehicle_id', 'in', ids)], ['vehicle_id'], ['vehicle_id'], context=context)
        fuel_dict = { data['vehicle_id'][0]: data['vehicle_id_count'] for data in fuel_data }
        return { vehicle_id: fuel_dict.get(vehicle_id) for vehicle_id in ids }

    def _odometer_count(self, cr, uid, ids, field_name, arg, context=None):
        odometer_data = self.pool['fleet.vehicle.odometer'].read_group(cr, uid, [('vehicle_id', 'in', ids)], ['vehicle_id'], ['vehicle_id'], context=context)
        odometer_dict = { data['vehicle_id'][0]: data['vehicle_id_count'] for data in odometer_data }
        return { vehicle_id: odometer_dict.get(vehicle_id) for vehicle_id in ids }
    ...
    _columns = {
        ...
        'contract_count': fields.function(_contract_count, type='integer', string='Contracts'),
        'service_count': fields.function(_service_count, type='integer', string='Services'),
        'fuel_logs_count': fields.function(_fuel_logs_count, type='integer', string='Fuel Logs'),
        'odometer_count': fields.function(_odometer_count, type='integer', string='Odometer'),
