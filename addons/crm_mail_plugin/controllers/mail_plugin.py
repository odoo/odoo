...
class MailPluginController(mail_plugin.MailPluginController):
    ...
    def search_records(self, model, query, limit=30):
        if model == "crm.lead":
            ...
            return self._search_leads([('name', 'ilike', query)], limit=limit)

        return super().search_records(model, query, limit)

    def _search_leads(self, domain, limit=30):
        ...
        partner_leads = request.env['crm.lead'].search(domain, limit=limit, order='won_status')
    ...
    def _get_contact_data(self, partner, email, **kwargs):
        ...
        if email:
            domain.append([('email_normalized', '=', email_normalize(email))])

        if not domain:
            contact_values['leads'] = []
        else:
            contact_values['leads'], contact_values['lead_count'] = self._search_leads(Domain.OR(domain))
    ...
    def crm_lead_create(self, email_body, email_subject, partner_email, partner_name, partner_id=None):
