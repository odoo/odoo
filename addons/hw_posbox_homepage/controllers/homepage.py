...
class IotBoxOwlHomePage(http.Controller):
    ...
    def get_version_info(self):
        ...
        current_commit = subprocess.run([*git, "rev-parse", "HEAD"], stdout=subprocess.PIPE, check=False, text=True)
        current_branch = subprocess.run(
            [*git, "rev-parse", "--abbrev-ref", "HEAD"], stdout=subprocess.PIPE, check=False, text=True
        )
        ...
        last_available_commit = subprocess.run(
            [*git, "ls-remote", "origin", current_branch], stdout=subprocess.PIPE, check=False, text=True
        )
