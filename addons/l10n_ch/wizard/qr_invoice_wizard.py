...
class L10n_ChQr_InvoiceWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        def determine_invoices_text(nb_inv, inv_format):
            ...
            '''
            if nb_inv == 0:
                return self.env._("No invoice could be printed in the %s format.", inv_format)
            if nb_inv == 1:
                return self.env._("One invoice could be printed in the %s format.", inv_format)
            return self.env._("%(amount)s invoices could be printed in the %(format)s format.", amount=nb_inv, format=inv_format)
