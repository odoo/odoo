...
class ResPartnerBank(models.Model):
    ...
    @api.depends('account_number', 'formatted_account_number')
    ...
    def _l10n_ch_get_qr_vals(self, amount, currency, debtor_partner, free_communication, structured_communication):
        ...
        cred_name = filter_text(self.holder_name or self.partner_id.name)
        ...
        if qr_method == 'ch_qr':
            if not _partner_fields_set(self.partner_id):
                return _(
                    ...
                    "the payment (%s) must have the necessary postal address "
                    "information (zip, city and country).",
                    self.formatted_account_number,
                )
