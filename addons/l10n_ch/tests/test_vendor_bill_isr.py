...
class TestVendorBillISR(common.SavepointCase):
    ...
    def test_isr_ref(self):
        ...
        self.assertTrue(invoice._validate_isr_reference())
    ...
    def test_isr_missing_ref(self):
        """Forget to enter ISR referenec with ISR subscription account number
        ...
        form.invoice_payment_ref = False
        invoice = form.save()

        with self.assertRaises(ValidationError):
            invoice._validate_isr_reference()

    def test_isr_ref_postal(self):
        """Enter any reference with standard postal account number

        The vendor bill can be saved but no check is done on
        the ref as we consider it as free reference.
        """
        form = Form(self.env["account.move"].with_context(
            default_type="in_invoice"), view="l10n_ch.isr_invoice_form")
        form.partner_id = self.supplier2
        form.invoice_partner_bank_id = self.bank_acc_postal

        form.invoice_payment_ref = '1234'
        invoice = form.save()

        self.assertTrue(invoice._validate_isr_reference())

    def test_isr_ref_iban(self):
        """Enter any reference with IBAN account number
        ...
        form = Form(self.env["account.move"].with_context(
            default_type="in_invoice"), view="l10n_ch.isr_invoice_form")
        form.partner_id = self.supplier3
        form.invoice_partner_bank_id = self.bank_acc_iban
