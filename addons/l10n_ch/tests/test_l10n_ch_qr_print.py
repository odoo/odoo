...
class QRPrintTest(AccountTestInvoicingCommon):
    ...
    def test_action_send_and_print_and_info_message(self):
        ...
        debtor_partner_ch = self.env['res.partner'].create({
            'name': 'deb partner CH',
            'country_id': self.env.ref('base.ch').id,
            'zip': 2000,
            'city': False,
        ...
        debtor_partner_li = self.env['res.partner'].create({
            'name': 'deb partner LI',
        ...
        debtor_partner_be = self.env['res.partner'].create({
            'name': 'deb partner BE',
        ...
        invoice_ch = self.env['account.move'].create({
            'move_type': 'out_invoice',
            'partner_id': debtor_partner_ch.id,
            'partner_bank_id': self.qr_bank_account.id,
            'currency_id': self.env.ref('base.CHF').id,
            'invoice_date': '2019-01-01',
            'invoice_line_ids': [(0, 0, {'product_id': self.product_a.id})],
        })
        invoice_li = self.env['account.move'].create({
            'move_type': 'out_invoice',
            'partner_id': debtor_partner_li.id,
            'partner_bank_id': self.qr_bank_account.id,
            'currency_id': self.env.ref('base.CHF').id,
            'invoice_date': '2019-01-01',
            'invoice_line_ids': [(0, 0, {'product_id': self.product_a.id})],
        })
        invoice_be = self.env['account.move'].create({
            'move_type': 'out_invoice',
            'partner_id': debtor_partner_be.id,
            'partner_bank_id': self.qr_bank_account.id,
            'currency_id': self.env.ref('base.CHF').id,
            'invoice_date': '2019-01-01',
            'invoice_line_ids': [(0, 0, {'product_id': self.product_a.id})],
        })
        invoices = invoice_ch + invoice_li + invoice_be
        ...
        wizard = get_wizard()
        self.assertEqual(wizard.l10n_ch_partners_without_street_ids.ids, [debtor_partner_ch.id, debtor_partner_li.id])
        ...
        debtor_partner_ch.write({'city': 'Martigny'})
        wizard = get_wizard()
        self.assertEqual(wizard.l10n_ch_partners_without_street_ids.ids, [debtor_partner_ch.id, debtor_partner_li.id])
        self.assertRaisesRegex(UserError,
            "The partner must have the necessary postal address "
        ...
        debtor_partner_li.write({'city': 'Vaduz'})
        wizard = get_wizard()
        self.assertEqual(wizard.l10n_ch_partners_without_street_ids.ids, [debtor_partner_ch.id, debtor_partner_li.id])
        wizard.action_send_and_print()

        debtor_partner_ch.write({'street': 'Street Name 1'})
        wizard = get_wizard()
        self.assertEqual(wizard.l10n_ch_partners_without_street_ids.ids, [debtor_partner_li.id])
        wizard.action_send_and_print()

        debtor_partner_li.write({'street': 'Street Name 2'})
        wizard = get_wizard()
        self.assertEqual(wizard.l10n_ch_partners_without_street_ids.ids, [])
        wizard.action_send_and_print()
