...
class QRPrintTest(AccountTestInvoicingCommon):
    ...
    def _run_wkhtmltopdf(*args, **kwargs):
        # use the sample pdf to mock the pdf generation
        return file_open('base/tests/multi_kids_embedded.pdf', 'rb').read()
    ...
    def test_action_single_send_and_print_and_info_message(self):
        ...
        with patch.object(self.env.registry['ir.actions.report'], '_run_wkhtmltopdf', self._run_wkhtmltopdf):
    ...
    def test_action_batch_send_and_print_and_info_message(self):
        ...
        def run_move_send_cron():
            with patch.object(self.env.registry['ir.actions.report'], '_run_wkhtmltopdf', self._run_wkhtmltopdf):
                account_move_send_cron.ir_actions_server_id.with_context(
                    # In test env, the pdf generation is skipped and switches to html
                    # rendering. However when company.qr_code is not set, the CH QR code
                    # generation is only overriden for pdf rendering.
                    # 'force_report_rendering' simulates the normal behavior.
                    force_report_rendering=True,

                    # We need to specify a 'ir_cron_progress_id' to simulate a cron
                    # environment as the qr code errors behaves differently in a cron job.
                    # The errors are non blocking
                    ir_cron_progress_id=1,

                # Cannot use 'method_direct_trigger' as it cleans the context
                ).run()
