...
class ProjectProject(models.Model):
    ...
    def _compute_issue_count(self):
        issues_data = self.env['project.issue'].read_group([('project_id', 'in', self.ids), ('stage_id.fold', '=', False)], ['project_id'], ['project_id'])
        result = dict((data['project_id'][0], data['project_id_count']) for data in issues_data)
        for project in self:
            project.issue_count = result.get(project.id, 0)
