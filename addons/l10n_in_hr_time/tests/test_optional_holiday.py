...
class TestOptionalHoliday(TestHrHolidaysCommon):
    ...
    def setUpClass(cls):
        ...
        cls.optional_holiday = cls.env['l10n.in.hr.time.optional.holiday'].create({
            'name': 'optional holiday',
            'date': '2025-01-02',
        })

    def test_optional_holiday_valid_leave_request(self):
        with Form(self.env['hr.time'].with_context(default_employee_id=self.employee_emp_id), view="l10n_in_hr_time.hr_time_view_form_inherit") as leave_form:
    ...
    def test_optional_holiday_invalid_leave_request(self):
        with self.assertRaises(ValidationError):
            with Form(self.env['hr.time'].with_context(default_employee_id=self.employee_hruser_id), view="l10n_in_hr_time.hr_time_view_form_inherit") as leave_form:
                leave_form.work_entry_type_id = self.work_entry_type
