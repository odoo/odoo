...
class TestRestrictedEditor(odoo.tests.HttpCase):
    ...
    def test_03_restricted_editor_tester(self):
        """
        This tests ensure that, a restricted user, should not be able to edit ir.ui.view records
        """
        self.user_test = new_test_user(self.env, login='restricted', website_id=False)
        self.user_test.groups_id |= self.env.ref('website.group_website_restricted_editor') | self.env.ref('test_website.group_test_website_tester')
