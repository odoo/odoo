...
export class AccrualLevels extends Component {
    static template = "hr_time.AccrualLevels";
    static props = {
        ...standardFieldProps
    };
    ...
    setup() {
        ...
        useRecordObserver(async (record) => {
            ...
            this.state.data = await this.orm.read("hr.time.accrual.level", record.data[this.props.name]._currentIds);
    ...
    async openMilestone(id) {
        let action;
        if (id) {
            action = await this.orm.call("hr.time.accrual.plan", "action_open_accrual_plan_level",
                [this.props.record.evalContext.id], { level_id: id });
        } else {
            action = await this.orm.call("hr.time.accrual.plan", "action_create_accrual_plan_level",
