...
class HrTimeEmployeeTypeReport(models.Model):
    _name = 'hr.time.employee.type.report'
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'hr_time_employee_type_report')

        self.env.cr.execute("""
            CREATE or REPLACE view hr_time_employee_type_report as (
                WITH
                /* Validated leaves */
                validated_leaves as (
                    SELECT
                ...
                ordered_allocations as (
                    ...
                    FROM hr_time_allocation allocation
    ...
    def action_time_off_analysis(self):
        ...
        return {
            ...
            'res_model': 'hr.time.employee.type.report',
            'view_mode': 'pivot',
            'search_view_id': [self.env.ref('hr_time.view_search_hr_time_employee_type_report').id],
            'domain': domain,
            'help': _("""
            ...
            """, self.env.ref("hr_time.hr_time_allocation_action_form").id),
            'context': {
