...
class HrTimeReport(models.Model):
    _name = 'hr.time.report'
    _description = 'Time Off Summary / Report'
    _inherit = ["hr.manager.department.report"]
    _auto = False
    _order = "date_from DESC, employee_id"

    leave_id = fields.Many2one('hr.time', string="Time Off Request", readonly=True)
    allocation_id = fields.Many2one('hr.time.allocation', string="Allocation Request", readonly=True)
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'hr_time_report')

        self.env.cr.execute("""
            CREATE or REPLACE view hr_time_report as (
                ...
                from hr_time_allocation as allocation
    ...
    def action_open_record(self):
        ...
        return {
            ...
            'res_model': 'hr.time' if self.leave_id else 'hr.time.allocation',
        }
