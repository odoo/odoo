...
class HrTimeEmployeeReport(models.Model):
    _name = 'hr.time.employee.report'
    ...
    def _compute_leave_duration(self, report_records):
        ...
        leaves = self.env['hr.time'].browse(leave_ids)
        ...
        virtual_leaves = self.env['hr.time']
        for virtual_leave_data in virtual_leaves_data:
            virtual_leaves |= self.env['hr.time'].new(virtual_leave_data)
