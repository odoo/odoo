...
class HrTimeReportCalendar(models.Model):
    _name = 'hr.time.report.calendar'
    ...
    description = fields.Char("Description", readonly=True, groups='hr_time.group_hr_time_user')
    work_entry_type_id = fields.Many2one('hr.work.entry.type', readonly=True, string="Time Off Type",
        groups='hr_time.group_hr_time_user')
    ...
    leave_id = fields.Many2one(comodel_name='hr.time', readonly=True, groups='hr_time.group_hr_time_user')
    is_manager = fields.Boolean("Manager", compute="_compute_is_manager")

    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'hr_time_report_calendar')
        self.env.cr.execute("""CREATE OR REPLACE VIEW hr_time_report_calendar AS
        (SELECT
            hl.id AS id,
            hl.id AS leave_id,
    ...
    def _compute_is_manager(self):
        for leave in self:
            leave.is_manager = self.env.user.has_group('hr_time.group_hr_time_user') or leave.leave_manager_id == self.env.user

    def action_approve(self):
        current_user = self.env.user
        if current_user.has_group('hr_time.group_hr_time_user'):
