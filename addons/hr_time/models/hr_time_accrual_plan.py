...
from odoo.addons.hr_time.models.hr_time_accrual_plan_level import _get_selection_days


class HrTimeAccrualPlan(models.Model):
    _name = 'hr.time.accrual.plan'
    _description = "Accrual Plan"

    active = fields.Boolean(default=True)
    name = fields.Char('Name', required=True)
    employees_count = fields.Integer("Employees", compute='_compute_employee_count')
    level_ids = fields.One2many('hr.time.accrual.level', 'accrual_plan_id', copy=True, string="Milestones")
    allocation_ids = fields.One2many('hr.time.allocation', 'accrual_plan_id',
    ...
    def _compute_level_count(self):
        level_read_group = self.env['hr.time.accrual.level']._read_group(
    ...
    def _compute_employee_count(self):
        allocations_read_group = self.env['hr.time.allocation']._read_group(
            [('accrual_plan_id', 'in', self.ids)],
            ['accrual_plan_id'],
    ...
    def action_create_accrual_plan_level(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'hr.time.accrual.level',
            'view_mode': 'form',
            'views': [[False, 'form']],
            'view_id': self.env.ref('hr_time.hr_accrual_level_view_form').id,
            'target': 'new',
            'context': dict(
                self.env.context,
                new=True,
    ...
    def _prevent_used_plan_unlink(self):
        ...
        if self.env['hr.time.allocation'].search_count(domain):
