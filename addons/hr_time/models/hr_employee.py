...
class HrEmployee(models.Model):
    ...
    def _compute_allocation_count(self):
        ...
        data = self.env['hr.time.allocation']._read_group([
            ('employee_id', 'in', self.ids),
    ...
    def _compute_allocation_remaining_display(self):
        current_date = date.today()
        allocations = self.env['hr.time.allocation'].search([('employee_id', 'in', self.ids)])
    ...
    def _compute_show_leaves(self):
        show_leaves = self.env.user.has_group('hr_time.group_hr_time_user')
        for employee in self:
    ...
    def create(self, vals_list):
        ...
        approver_group = self.env.ref('hr_time.group_hr_time_responsible', raise_if_not_found=False)
        group_updates = []
    ...
    def write(self, vals):
        ...
        if 'leave_manager_id' in values:
            ...
            if values['leave_manager_id']:
                ...
                approver_group = self.env.ref('hr_time.group_hr_time_responsible', raise_if_not_found=False)
                if approver_group and not leave_manager.has_group('hr_time.group_hr_time_responsible'):
        ...
        if 'resource_calendar_id' in values and not self.env.context.get('no_leave_resource_calendar_update'):
            try:
                leaves = self.env['hr.time'].search([
                    ('employee_id', 'in', self.ids),
        ...
        if 'parent_id' in values or 'department_id' in values:
            ...
            allocations = self.env['hr.time.allocation'].sudo().search([
                ('state', '=', 'confirm'),
                ('employee_id', 'in', self.ids),
            ])
    ...
    def action_time_off_dashboard(self):
        return {
            ...
            'res_model': 'hr.time',
            'views': [[self.env.ref('hr_time.hr_time_employee_view_dashboard').id, 'calendar']],
    ...
    def get_public_holidays_data(self, date_start, date_end):
        ...
        employee_tz = ZoneInfo(employee._get_tz() if employee else self.env.user.tz or 'UTC')
        public_holidays = employee._get_public_holidays(date_start, date_end).sorted('date_from')
        ...
        return [
            {
                ...
                'end': datetime.combine(
                    bh.date_to.astimezone(employee_tz),
                    datetime.max.time()
                ).isoformat(),
                'endType': "datetime",
                'isAllDay': True,
                'start': datetime.combine(
                    bh.date_from.astimezone(employee_tz),
                    datetime.min.time()
                ).isoformat(),
                'startType': "datetime",
                'title': bh.name,
    ...
    def get_allocation_requests_amount(self):
        employee = self._get_contextual_employee()
        return self.env['hr.time.allocation'].search_count([
    ...
    def _get_mandatory_days(self, start_date, end_date):
        ...
        return self.env['hr.time.mandatory.day'].search(domain)
    ...
    def _get_consumed_leaves(self, work_entry_types, target_date=False, ignore_future=False):
        ...
        leaves = self.env['hr.time'].search(leaves_domain)
        leaves_per_employee_type = defaultdict(lambda: defaultdict(lambda: self.env['hr.time']))
        for leave in leaves:
            leaves_per_employee_type[leave.employee_id][leave.work_entry_type_id] |= leave

        allocations = self.env['hr.time.allocation'].with_context(active_test=False).search([
            ('employee_id', 'in', employees.ids),
            ('work_entry_type_id', 'in', work_entry_types.ids),
            ('state', '=', 'validate'),
        ])
        allocations_per_employee_type = defaultdict(lambda: defaultdict(lambda: self.env['hr.time.allocation']))
        ...
        to_recheck_leaves_per_work_entry_type = defaultdict(lambda:
            defaultdict(lambda: {
                ...
                'to_recheck_leaves': self.env['hr.time']
            })
        )
        ...
        for employee in employees:
            for work_entry_type in work_entry_types:
                ...
                allocations_with_date_to = self.env['hr.time.allocation']
                allocations_without_date_to = self.env['hr.time.allocation']
