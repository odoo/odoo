...
class ResourceCalendarLeaves(models.Model):
    ...
    holiday_id = fields.Many2one("hr.time", string='Time Off Request')
    ...
    def _reevaluate_leaves(self, time_domain_dict):
        ...
        leaves = self.env['hr.time'].search(domain)
        if not leaves:
            return

        previous_durations = leaves.mapped('number_of_days')
        previous_states = leaves.mapped('state')
        self.env.add_to_compute(self.env['hr.time']._fields['number_of_days'], leaves)
        self.env.add_to_compute(self.env['hr.time']._fields['duration_display'], leaves)
        leaves.sudo().write({
            'state': 'confirm',
        })
        sick_time_status = self.env['hr.work.entry.type'].search([('code', '=', 'LEAVE110')])
        leaves_to_recreate = self.env['hr.time']
