...
class HrTime(models.Model):
    ...
    private_name = fields.Char('Time Off Description', groups='hr_time.group_hr_time_responsible')
    ...
    def _compute_attachment_is_visible(self):
        is_privileged_user = self.env.user.has_groups('hr_time.group_hr_time_user,hr_time.group_hr_time_manager')
        for leave in self:
    ...
    def _check_validity(self):
        sorted_leaves = defaultdict(lambda: self.env['hr.time'])
        ...
        is_leave_user = self.env.user.has_group('hr_time.group_hr_time_user')
    ...
    def _check_double_validation_rules(self, employees, state):
        if self.env.user.has_group('hr_time.group_hr_time_manager'):
            return
    ...
    def create(self, vals_list):
        ...
        holidays = super(HrTime, self.with_context(mail_create_nosubscribe=True)).create(vals_list)
        ...
        self.env['hr.time.allocation'].invalidate_model(['leaves_taken', 'max_leaves'])  # missing dependency on compute
    ...
    def write(self, vals):
        values = vals
        is_officer = self.env.user.has_group('hr_time.group_hr_time_user') or self.env.is_superuser()
    ...
    def _unlink_if_correct_states(self):
        ...
        elif not self.env.user.has_group('hr_time.group_hr_time_manager'):
    ...
    def unlink(self):
        ...
        self.env['hr.time.allocation'].invalidate_model(['leaves_taken', 'max_leaves'])  # missing dependency on compute
        return super(HrTime, self.with_context(leave_skip_date_check=True)).unlink()
    ...
    def action_cancel(self):
        ...
        return {
            ...
            'target': 'new',
            'res_model': 'hr.time.cancel.leave',
            'view_mode': 'form',
            'views': [[False, 'form']],
            'context': {
    ...
    def action_approve(self, check_state=True):
        ...
        leave_to_approve = self.env['hr.time']
        leave_to_validate = self.env['hr.time']
        for leave in self:
    ...
    def _split_leaves(self, split_date_from, split_date_to=False):
        ...
        for leave in multi_day_leaves:
            ...
            for leave_vals in new_leave_vals:
                new_leave = self.env['hr.time'].new(leave_vals)
        ...
        if not new_leaves_vals:
            return self.env['hr.time']
        return self.env['hr.time'].with_context(
    ...
    def _action_validate(self, check_state=True):
        ...
        leaves_second_approver = self.env['hr.time']
        leaves_first_approver = self.env['hr.time']
    ...
    def _notify_manager(self):
        ...
        model_description = self.env['ir.model']._get('hr.time').name
    ...
    def _force_cancel(self, reason=None, msg_subtype='mail.mt_comment', notify_responsibles=True):
        ...
        if reason:
            model_description = self.env['ir.model']._get('hr.time').display_name
            for leave in leaves:
                body = self.env._(
    ...
    def open_pending_requests(self):
        ...
        if employee != user_employee:
            view_name = 'hr_time.hr_time_allocation_view_tree'
            context.update({'search_default_employee_id': employee.id})
        else:
            view_name = 'hr_time.hr_time_allocation_view_tree_my'
            domain = [('employee_id', '=', employee.id)]
        return {
            'name': _('Allocation Requests'),
            'type': 'ir.actions.act_window',
            'res_model': 'hr.time.allocation',
    ...
    def _get_to_clean_activities(self):
        return ['hr_time.mail_act_leave_approval', 'hr_time.mail_act_leave_second_approval']

    def activity_update(self):
        if self.env.context.get('mail_activity_automation_skip'):
            return

        to_clean, to_do, to_do_confirm_activity = self.env['hr.time'], self.env['hr.time'], self.env['hr.time']
        activity_vals = []
        today = fields.Date.today()
        model_id = self.env['ir.model']._get_id('hr.time')
        confirm_activity = self.env.ref('hr_time.mail_act_leave_approval')
        approval_activity = self.env.ref('hr_time.mail_act_leave_second_approval')
        for holiday in self:
        ...
        if to_do_confirm_activity:
            to_do_confirm_activity.activity_feedback(['hr_time.mail_act_leave_approval'])
        if to_do:
            to_do.activity_feedback(['hr_time.mail_act_leave_approval', 'hr_time.mail_act_leave_second_approval'])
    ...
    def _track_log_get_default_subtype(self, track_init_values):
        if 'state' in track_init_values and self.state == 'validate':
            ...
            return leave_notif_subtype or self.env.ref('hr_time.mt_leave')
    ...
    def message_subscribe(self, partner_ids=None, subtype_ids=None):
        ...
        if any(holiday.state in ['validate', 'validate1'] for holiday in self):
            self.check_access('read')
            return super(HrTime, self.sudo()).message_subscribe(partner_ids=partner_ids, subtype_ids=subtype_ids)
    ...
    def _notify_leave_request(self):
        ...
        subject = self.env.ref('hr_time.new_timeoff_request_template')._render_field('subject', self.ids, compute_lang=True)
        for holiday in self:
            if holiday.work_entry_type_id.leave_validation_type == 'no_validation':
                continue
            ...
            if not partner_ids:
                continue
            leave_msg = self.env.ref('hr_time.new_timeoff_request_template').with_context(
    ...
    def _cancel_invalid_leaves(self):
        ...
        accrual_allocations = self.env['hr.time.allocation'].search([
