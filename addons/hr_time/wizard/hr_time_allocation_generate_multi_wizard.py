...
class HrTimeAllocationGenerateMultiWizard(models.TransientModel):
    _name = 'hr.time.allocation.generate.multi.wizard'
    _inherit = ['hr.mixin']
    ...
    accrual_plan_id = fields.Many2one('hr.time.accrual.plan')
    ...
    def action_generate_allocations(self):
        ...
        if vals_list:
            allocations = self.env['hr.time.allocation'].with_context(
            ...
            return {
                ...
                "views": [[self.env.ref('hr_time.hr_time_allocation_view_tree').id, "list"], [self.env.ref('hr_time.hr_time_allocation_view_form_manager').id, "form"]],
                'view_mode': 'list',
