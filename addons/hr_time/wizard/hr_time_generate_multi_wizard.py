...
class HrTimeGenerateMultiWizard(models.TransientModel):
    _name = 'hr.time.generate.multi.wizard'
    _inherit = ['hr.mixin']
    ...
    def _prepare_employees_holiday_values(self, employees, date_from_tz, date_to_tz):
        ...
        validated = self.env.user.has_group('hr_time.group_hr_time_user') or self.work_entry_type_id.leave_validation_type == 'no_validation'
        values = []
    ...
    def action_generate_time_off(self):
        ...
        conflicting_leaves = self.env['hr.time'].with_context(
        ...
        leaves = self.env['hr.time'].with_context(
            tracking_disable=True,
            mail_activity_automation_skip=True,
            leave_fast_create=True,
        ...
        return {
            ...
            "views": [[self.env.ref('hr_time.hr_time_view_tree').id, "list"], [self.env.ref('hr_time.hr_time_view_form_manager').id, "form"]],
            'view_mode': 'list',
            'res_model': 'hr.time',
    ...
    def _compute_valid_work_entry_type_ids(self):
        ...
        allocations = self.env['hr.time.allocation'].search([
