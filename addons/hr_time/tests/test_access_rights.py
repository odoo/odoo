...
class TestHrHolidaysAccessRightsCommon(TestHrHolidaysCommon):
    ...
    def setUpClass(cls):
        ...
        cls.employee_leave = cls.env['hr.time'].with_user(cls.user_employee_id).create({
            'name': 'Test',
        ...
        cls.env['hr.time.mandatory.day'].search([]).unlink()
    ...
    def request_leave(self, user_id, request_date_from, number_of_days, values=None):
        ...
        return self.env['hr.time'].with_user(user_id).create(values)
...
class TestAccessRightsRead(TestHrHolidaysAccessRightsCommon):
    ...
    def test_leave_read_by_user_other(self):
        ...
        other_leave = self.env['hr.time'].with_user(self.user_hruser).create({
            'name': 'Test',
            'work_entry_type_id': self.work_entry_type.id,
...
class TestAccessRightsWrite(TestHrHolidaysAccessRightsCommon):
    ...
    def test_leave_creation_for_other_user(self):
        ...
        HolidaysEmployeeGroup = self.env['hr.time'].with_user(self.user_employee_id)
        with self.assertRaises(AccessError):
    ...
    def test_leave_hr_to_validate_by_manager(self):
        ...
        manager_leave = self.env['hr.time'].with_user(self.user_hrmanager_id).create({
            'name': 'Hol manager',
            'work_entry_type_id': self.work_entry_type.id,
            'employee_id': self.employee_hrmanager_id,
    ...
    def test_leave_manager_to_validate_by_user(self):
        ...
        hr_time = self.request_leave(self.user_hruser_id, date_utils.start_of(date.today() + relativedelta(days=7), 'week'), 1, values)
        with self.assertRaises(UserError):
            hr_time.with_user(self.user_employee_id).action_approve()
    ...
    def test_leave_manager_to_validate_by_holiday_user(self):
        ...
        hr_time = self.request_leave(self.user_hruser_id, date_utils.start_of(date.today() + relativedelta(days=7), 'week'), 1, values)
        hr_time.with_user(self.user_hruser_id).action_approve()
    ...
    def test_leave_double_validate(self):
        ...
        hr_time = self.request_leave(self.user_hruser_id, leave_date, 1, values)

        with self.assertRaises(UserError):
            hr_time.with_user(self.user_employee_id).action_approve()
        self.employee_hrmanager.leave_manager_id = self.user_responsible
        hr_time.with_user(self.user_responsible_id).action_approve()
    ...
    def test_leave_double_validate_holiday_manager(self):
        ...
        leave_start = date_utils.start_of(date.today() + relativedelta(days=15), 'week')
        hr_time = self.request_leave(self.user_hrmanager_id, leave_start, 1, values).with_user(self.user_hrmanager_id)
        hr_time.action_approve()
    ...
    # hr_time.group_hr_time_responsible
