...
class TestHrHolidaysCancelLeave(TestHrHolidaysCommon):
    ...
    def setUpClass(cls):
        ...
        cls.holiday = cls.env['hr.time'].with_context(mail_create_nolog=True, mail_notrack=True).with_user(cls.user_employee).create({
            'name': 'Time Off 1',
    ...
    def test_action_cancel_leave(self):
        ...
        self.env['hr.time.cancel.leave'].with_user(self.user_employee).with_context(default_leave_id=self.holiday.id) \
    ...
    def test_action_cancel_leave_from_another_person(self):
        ...
        with self.assertRaises(ValidationError, msg='The leave could not be cancel since it is leave in the past.'):
            self.env['hr.time.cancel.leave'].with_user(self.user_hruser).with_context(default_leave_id=self.holiday.id) \
                .new({'reason': 'Test remove holiday'}) \
                .action_cancel_leave()
