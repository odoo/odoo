...
class TestHrTimeMandatoryDays(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.employee_user = new_test_user(cls.env, login='user', groups='base.group_user,hr_time.group_hr_time_employee', company_ids=[(6, 0, cls.company.ids)], company_id=cls.company.id)
        cls.manager_user = new_test_user(cls.env, login='manager', groups='base.group_user,hr_time.group_hr_time_manager', company_ids=[(6, 0, cls.company.ids)], company_id=cls.company.id)
        cls.user_emp_leave_manager = new_test_user(cls.env, login='employee_leave_manager', groups='base.group_user,hr_time.group_hr_time_employee', company_ids=[(6, 0, cls.company.ids)], company_id=cls.company.id)
        cls.hr_user = new_test_user(cls.env, login='HR_user', groups='hr_time.group_hr_time_manager', company_ids=[(6, 0, cls.company.ids)], company_id=cls.company.id)
        ...
        cls.mandatory_day = cls.env['hr.time.mandatory.day'].create({
            'name': 'Super Event',
            'company_id': cls.company.id,
        ...
        cls.mandatory_week = cls.env['hr.time.mandatory.day'].create({
    ...
    def test_request_mandatory_days(self):
        ...
        self.env['hr.time'].with_user(self.employee_user.id).create({
            'name': 'coucou',
        ...
        self.env['hr.time'].with_user(self.manager_user.id).create({
            'name': 'coucou',
            'work_entry_type_id': self.work_entry_type.id,
            'employee_id': self.employee_emp.id,
    ...
    def test_get_mandatory_days(self):
        ...
        with Form(self.env['hr.time'].with_user(self.employee_user.id).with_context(default_employee_id=self.employee_emp.id)) as leave_form:
    ...
    def test_department_mandatory_days(self):
        ...
        self.env['hr.time.mandatory.day'].create({
    ...
    def test_multiple_employees_mandatory_days(self):
        ...
        leave_1, leave_2 = self.env['hr.time'].with_user(self.manager_user.id).create([{
            'name': 'have been given the black spot',
            'work_entry_type_id': self.work_entry_type.id,
            'employee_id': self.employee_emp.id,
            'request_date_from': datetime(2021, 11, 3),
    ...
    def test_cancel_leave_on_mandatory_days(self):
        ...
        self.env['hr.time.cancel.leave'].with_user(self.employee_user).create({
