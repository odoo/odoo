...
class TestHolidaysFlow(TestHrHolidaysCommon):
    ...
    def test_00_leave_request_flow_unlimited(self):
        ...
        self.assertEqual(hol1_user_group.state, 'confirm', 'hr_time: newly created leave request should be in confirm state')

        # HrUser validates the employee leave request -> should work
        hol1_user_group.action_approve()
        self.assertEqual(hol1_manager_group.state, 'validate', 'hr_time: validated leave request should be in validate state')
        ...
        self.assertEqual(hol12_user_group.state, 'confirm', 'hr_time: newly created leave request should be in confirm state')

        # HrManager validate the employee leave request
        hol12_manager_group.action_approve()
        self.assertEqual(hol1_user_group.state, 'validate', 'hr_time: validates leave request should be in validate state')
    ...
    def test_01_leave_request_flow_limited(self):
        ...
        with freeze_time('2022-01-15'):
            Requests = self.env['hr.time']
            Allocations = self.env['hr.time.allocation']
            HolidaysStatus = self.env['hr.work.entry.type']
            ...
            self.env['hr.time.allocation'].create([
                {
            ...
            def _check_holidays_status(work_entry_type, employee, ml, lt, rl, vrl):
                ...
                self.assertEqual(result['max_leaves'], ml,
                                'hr_time: wrong type days computation')
            ...
            self.assertEqual(hol2.state, 'validate',
                            'hr_time: second validation should lead to validate state')
            ...
            self.assertEqual(hol2.state, 'refuse',
                            'hr_time: refuse should lead to refuse state')
            ...
            self.assertEqual(hol2.state, 'refuse',
                            'hr_time: hr_user should not be able to reset a refused leave request')
            ...
            hol3.action_refuse()
            self.assertEqual(hol3.state, 'refuse', 'hr_time: refuse should lead to refuse state')
            # Validate it again
            hol3.action_approve()
            self.assertEqual(hol3.state, 'validate', 'hr_time: validation should lead to validate state')
    ...
    def test_10_leave_summary_reports(self):
        ...
        test_reports.try_report_action(self.env.cr, self.env.uid, 'action_hr_time_summary_employee', wiz_data=data_dict, context=ctx, our_module='hr_time')
    ...
    def test_sql_constraint_dates(self):
        ...
        with mute_logger('odoo.sql_db'), self.assertRaises(IntegrityError):
            self.env['hr.time'].create(leave_vals)
        ...
        leave = self.env['hr.time'].create(leave_vals)
