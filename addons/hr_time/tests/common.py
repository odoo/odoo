...
class TestHrHolidaysCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_hruser = mail_new_test_user(cls.env, login='armande', groups='base.group_user,hr_time.group_hr_time_user')
        cls.user_hruser_id = cls.user_hruser.id

        cls.user_hrmanager = mail_new_test_user(cls.env, login='bastien', groups='base.group_user,hr_time.group_hr_time_manager')
        cls.user_hrmanager_id = cls.user_hrmanager.id
        cls.user_hrmanager.tz = 'Europe/Brussels'

        cls.user_responsible = mail_new_test_user(cls.env, login='Titus', groups='base.group_user,hr_time.group_hr_time_responsible')
        cls.user_responsible_id = cls.user_responsible.id
        cls.user_employee = mail_new_test_user(cls.env, login='enguerran', password='enguerran', groups='hr_time.group_hr_time_employee')
    ...
    def _create_form_test_accrual_allocation(self, work_entry_type, date_from, employee, accrual_plan, date_to=None, creator_user=None):
        allocation = self.env['hr.time.allocation']
        if creator_user:
            allocation = allocation.with_user(creator_user)
        with Form(allocation, 'hr_time.hr_time_allocation_view_form_manager') as form:
