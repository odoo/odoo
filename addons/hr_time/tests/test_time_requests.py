...
class TestLeaveRequests(TestHrHolidaysCommon):
    ...
    def test_group_leave_conflicting_days_computation(self):
        ...
        self.env['hr.time.allocation'].with_user(self.user_hrmanager_id).create({
            'name': 'Annual Time Off',
            'employee_id': self.employee_emp_id,
            'work_entry_type_id': self.holidays_type_4.id,
            'number_of_days': 20,
        ...
        leave1, leave2 = self.env['hr.time'].with_user(self.user_employee_id).create([
            {
        ...
        leave_wizard.action_generate_time_off()
        generated_training = self.env['hr.time'].search([
            ('employee_id', '=', self.employee_emp_id),
            ('work_entry_type_id', '=', training_type.id),
    ...
    def test_group_leave_hourly_conflict(self):
        ...
        hourly_leave = self.env['hr.time'].with_user(self.user_employee_id).create({
            'name': 'Hourly Leave',
            'employee_id': self.employee_emp_id,
