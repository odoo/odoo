...
class TestChangeDepartment(TestHrHolidaysCommon):
    def test_employee_change_department_request_change_department(self):
        self.HolidaysEmployeeGroup = self.env['hr.time'].with_user(self.user_employee_id)
        ...
        self.assertEqual(hol1_employee_group.department_id, self.hr_dept, 'hr_time: non approved leave request should change department if employee change department')

        # flushing is needed after approving because the hr_time.department_id
        ...
        self.env.flush_all()
        self.employee_emp.department_id = self.rd_dept
        self.assertEqual(hol2_employee_group.department_id, self.hr_dept, 'hr_time: approved passed leave request should stay in previous department if employee change department')
        ...
        self.env.flush_all()
        self.employee_emp.department_id = self.rd_dept
        self.assertEqual(hol22_employee_group.department_id, self.rd_dept, 'hr_time: approved future leave request should change department if employee change department')
        ...
        self.assertEqual(hol3_user_group.department_id, self.rd_dept, 'hr_time: refused passed leave request should stay in previous department if employee change department')
        ...
        self.env.flush_all()
        self.employee_emp.department_id = self.hr_dept  # Change department
        self.assertEqual(hol32_employee_group.department_id, self.hr_dept, 'hr_time: refused future leave request should change department if employee change department')
