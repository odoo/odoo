...
class TestAllocations(TestHrHolidaysCommon):
    ...
    def test_allocation_multi_employee(self):
        employee_allocation = self.env['hr.time.allocation.generate.multi.wizard'].create({
            'name': 'Bank Holiday',
        ...
        num_of_allocations = self.env['hr.time.allocation'].search_count([('employee_id', '=', self.employee.id)])
    ...
    def test_create_group_allocation_without_hr_right(self):
        ...
        allocation_wizard = self.env['hr.time.allocation.generate.multi.wizard'].create({
            'work_entry_type_id': self.work_entry_type.id,
    ...
    def test_allocation_request_day(self):
        ...
        employee_allocation = self.env['hr.time.allocation'].create({
            'employee_id': self.employee.id,
            'work_entry_type_id': self.work_entry_type.id,
            'allocation_type': 'regular',
        })

        with Form(employee_allocation.with_context(is_employee_allocation=True), 'hr_time.hr_time_allocation_view_form_dashboard') as allocation:
    ...
    def test_allocation_type_hours_with_resource_calendar(self):
        ...
        hour_type_allocation = self.env['hr.time.allocation.generate.multi.wizard'].create({
        ...
        employee_allocation = self.env['hr.time.allocation'].search([
            ('employee_id', '=', self.employee.id),
        ])
        employee_emp_allocation = self.env['hr.time.allocation'].search([
    ...
    def test_allowed_change_allocation(self):
        ...
        leave_request = self.env['hr.time'].create({
            'name': 'Leave Request',
            'work_entry_type_id': self.work_entry_type_paid.id,
    ...
    def test_allocation_dropdown_after_period(self):
        ...
        second_allocation = self.env['hr.time.allocation'].create({
            'name': 'Alloc2',
            'employee_id': self.employee.id,
            'work_entry_type_id': work_entry_type.id,
            'number_of_days': 9,
    ...
    def test_create_allocation_from_company_with_no_employee_for_current_user(self):
        ...
        allocation_form = Form(self.env['hr.time.allocation'].with_user(self.user_hrmanager))
    ...
    def test_hr_time_allocation_balance(self):
        ...
        first_valid_allocation = self.env['hr.time.allocation'].sudo().create({
            'name': 'Alloc',
            'employee_id': self.employee.id,
            'work_entry_type_id': work_entry_type.id,
            'number_of_days': 10,
        ...
        second_valid_allocation = self.env['hr.time.allocation'].sudo().create({
            'name': 'Alloc',
            'employee_id': self.employee.id,
            'work_entry_type_id': work_entry_type.id,
            'number_of_days': 12,
    ...
    def test_allocation_request_with_date_from(self):
        allocation = self.env['hr.time.allocation'].with_user(self.user_hrmanager)
        allocation_view = 'hr_time.hr_time_allocation_view_form'
    ...
    def test_different_granularity_insufficient_hours(self):
        allocation_insufficient_hours = self.env['hr.time.allocation'].create({
    ...
    def test_leave_allocation_by_removing_employee(self):
        ...
        with self.assertRaises(AssertionError):  # AssertionError raised by Form as employee is required
            with Form(self.env['hr.time.allocation']) as allocation_form:
                allocation_form.allocation_type = "regular"
                allocation_form.work_entry_type_id = self.work_entry_type
                allocation_form.number_of_hours_display = 10
    ...
    def test_refuse_validated_allocation_with_leaves(self):
        ...
        def _create_allocation(days):
            return self.env['hr.time.allocation'].create({
