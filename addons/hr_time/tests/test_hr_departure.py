...
class TestHolidaysFlow(TestHrHolidaysCommon):
    ...
    def test_departure_leave_before_departure_date(self):
        leave = self.env['hr.time'].with_context(leave_fast_create=True).create({
    ...
    def test_departure_leave_with_departure_date(self):
        ...
        self.assertTrue(cancel_message in self.env['hr.time'].search([
    ...
    def _check_employee_leave(self):
        leaves_after_departure_date = self.env['hr.time'].search([
            ('employee_id', '=', self.employee.id),
        ...
        leaves_before_departure_date = self.env['hr.time'].search([
            ('employee_id', '=', self.employee.id),
            ('date_from', '<=', self.departure_date),
        ])
