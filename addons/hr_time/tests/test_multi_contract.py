...
from odoo.addons.hr_time.tests.common import TestHolidayContract
...
class TestHolidaysMultiContract(TestHolidayContract):
    ...
    def test_leave_multi_contracts_split(self):
        ...
        leaves = self.env['hr.time'].search([('employee_id', '=', self.jules_emp.id)])
    ...
    def test_contract_traceability_calculate_nbr_leave(self):
        ...
        leave_during_full_time, leave_during_partial_time = self.env['hr.time'].create([
            {
                'employee_id': employee.id,
                'work_entry_type_id': work_entry_type.id,
    ...
    def test_multi_contract_out_of_office(self):
        ...
        leave1 = self.env['hr.time'].create({
            'employee_id': employee.id,
            'work_entry_type_id': work_entry_type.id,
        ...
        leave2 = self.env['hr.time'].create({
            'employee_id': employee.id,
            'work_entry_type_id': work_entry_type.id,
    ...
    def test_multi_contracts_with_different_work_schedules(self):
        ...
        with Form(self.env['hr.time'].with_context(default_employee_id=employee.id)) as leave_form:
