...
class TestExpiringLeaves(HttpCase, TestHrHolidaysCommon):
    ...
    def setUpClass(cls):
        ...
        cls.accrual_plan_with_accrual_validity = cls.env['hr.time.accrual.plan'].with_context(tracking_disable=True).sudo().create({
    ...
    def test_no_carried_over_leaves(self):
        ...
        accrual_plan = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).sudo().create({
        ...
        allocation = self.env['hr.time.allocation'].sudo().create({
    ...
    def test_allocation_with_max_carryover_and_expiring_allocation(self):
        ...
        number_of_accrued_days = 20
        carryover_limit = 10
        accrual_plan_1 = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).sudo().create({
            'name': 'Test Accrual Plan',
            'can_be_carryover': True,
            'carryover_date': 'other',
            'carryover_day': 31,
        ...
        accrual_plan_2 = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).sudo().create({
        ...
        with freeze_time("2024-01-01"):
            allocation_with_carryover = self.env['hr.time.allocation'].sudo().create({
    ...
    def test_expiration_date_2(self):
        ...
        with freeze_time("2024-01-01"):
            self.env['hr.time.allocation'].with_user(self.user_hruser)._update_accrual()
    ...
    def test_carried_over_days_expiration_date(self):
        ...
        """
        accrual_plan_without_accrual_validity = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).sudo().create({
            'name': 'Test Accrual Plan',
            'carryover_date': 'other',
            'carryover_day': 1,
            'carryover_month': '4',
