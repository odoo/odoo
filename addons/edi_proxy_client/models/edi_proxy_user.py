...
from .edi_proxy_auth import OdooEdiProxyAuth
...
class EdiProxyClientUser(models.Model):
    ...
    """
    _name = 'edi_proxy_client.user'
    _description = 'EDI proxy user'
    ...
    def _get_demo_state(self):
        demo_state = self.env['ir.config_parameter'].sudo().get_param('edi_proxy_client.demo', False)
        return 'prod' if demo_state in ['prod', False] else 'test' if demo_state == 'test' else 'demo'

    def _get_server_url(self):
        return DEFAULT_TEST_SERVER_URL if self._get_demo_state() == 'test' else self.env['ir.config_parameter'].sudo().get_param('edi_proxy_client.edi_server_url', DEFAULT_SERVER_URL)
    ...
    def _make_request(self, url, params=False):
        ...
        try:
            ...
            raise EdiProxyError('connection_error',
        ...
        if 'error' in response:
            ...
            raise EdiProxyError('connection_error', message)
        ...
        if proxy_error:
            ...
            raise EdiProxyError(error_code, proxy_error['message'] or False)
    ...
    def _renew_token(self):
        ...
        try:
            with self.env.cr.savepoint(flush=False):
                self.env.cr.execute('SELECT * FROM edi_proxy_client_user WHERE id IN %s FOR UPDATE NOWAIT', [tuple(self.ids)])
    ...
    def _neutralize(self):
        ...
        self.env.cr.execute("""
            ...
            VALUES ('edi_proxy_client.demo', true)
