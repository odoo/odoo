...
(function (exports) {
  ...
  function ensureSetup() {
    ...
    const DiscussWidget = odoo.__DEBUG__.services["mail/static/src/widgets/discuss/discuss.js"];
    if (DiscussWidget) {
  ...
  async function ensureAppsMenu() {
    ...
    if (!appsMenu) {
      ...
      await waitForCondition(() => document.querySelector(".o_navbar_apps_menu .o_dropdown_menu"));
    }
  }
  ...
  async function testViews() {
    ...
    for (const switchButton of switchButtons) {
      ...
      await waitForCondition(
        () =>
          document.querySelector(".o_action_manager > .o_action.o_view_controller").dataset
            .viewType === viewType
      );
      await testFilters();
    }
    ...
    try {
      ...
      if (xmlId) {
        if (isEnterprise) {
          ...
          await triggerClick(document.querySelector(".o_navbar_apps_menu .o_dropdown_toggler"));
