...
class AccountEdiXmlUblTr(models.AbstractModel):
    ...
    def _get_additional_document_reference_vals_list(self, invoice):
        # EXTENDS account.edi.xml.ubl_20
        additional_document_reference_vals_list = super()._get_additional_document_reference_vals_list(invoice)
        if invoice.partner_id.l10n_tr_nilvera_customer_status == 'earchive':
            additional_document_reference_vals_list.append({
                'id': "ELEKTRONIK",
                'issue_date': invoice.invoice_date,
                'document_type_code': "SEND_TYPE",
            })
        return additional_document_reference_vals_list
