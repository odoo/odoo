...
class AccountEdiXmlUblTr(models.AbstractModel):
    ...
    def _add_withholding_document_tax_total_nodes(self, line_node, vals):
        ...
        aggregated_tax_details_by_withholding_tax = {'tax': defaultdict(dict), 'withholding_tax': defaultdict(dict)}
        ...
        for grouping_key, values in aggregated_tax_details.items():
            if grouping_key:
                ...
                aggregated_tax_details_by_withholding_tax[key][l10n_tr_tax_withheld][grouping_key] = values

        line_node['cac:TaxTotal'] = [
            self._get_withholding_tax_total_node({**vals, 'aggregated_tax_details': tax_details, 'role': 'line'})
            for tax_details in aggregated_tax_details_by_withholding_tax['tax'].values()
        ]
        if vals['document_type'] == 'invoice':
            line_node['cac:WithholdingTaxTotal'] = [
                ...
                for tax_details in aggregated_tax_details_by_withholding_tax['withholding_tax'].values()
            ]
