...
class AccountEdiXmlUblTr(models.AbstractModel):
    ...
    def _add_invoice_header_nodes(self, document_node, vals):
        ...
        document_node.update({
            ...
            'cbc:ProfileID': self._get_tr_profile_id(invoice),
            'cbc:ID': invoice_id,
            'cbc:CopyIndicator': 'false',
            'cbc:UUID': invoice.l10n_tr_nilvera_uuid,
            'cbc:DueDate': None,
            'cbc:InvoiceTypeCode': 'ISTISNA' if invoice.l10n_tr_is_export_invoice else invoice.l10n_tr_gib_invoice_type,
            'cbc:PricingCurrencyCode': invoice.currency_id.name.upper()
                if vals['currency_id'] != vals['company_currency_id'] else None,
            'cbc:LineCountNumeric': len(invoice.line_ids),
            'cbc:BuyerReference': None,  # Nilvera will reject any <BuyerReference> tag, so remove it
            'cbc:Note': html2plaintext(invoice.narration, include_references=False) if invoice.narration else None,
        })

        document_node['cac:OrderReference']['cbc:IssueDate'] = invoice.invoice_date

        if invoice.state == 'draft' and (not invoice.name or invoice.name == '/'):
            prefix = invoice._l10n_tr_nilvera_get_series_prefix()
            document_node['cac:OrderReference']['cbc:ID'] = prefix
            document_node['cbc:ID'] = prefix
        ...
        if vals['invoice'].currency_id.name != 'TRY':
            ...
            document_node['cbc:Note'].append(f'KUR : {self._l10n_tr_get_currency_conversion_rate(invoice):.6f} TL')
    ...
    def _get_additional_document_reference_vals(self, invoice):
        ...
        additional_document_vals.extend([
            {
                'cbc:ID': value,
    ...
    def _l10n_tr_get_ecommerce_sale_additional_reference_data(self, invoice):
        ...
        if 'website_id' in invoice._fields and invoice.website_id:
            website = invoice.website_id.domain
            if not website:
                ecom_errors.append(self.env._("Please configure a domain name in Website Settings."))
        else:
        ...
        # Payment Data
        sale_order = invoice.line_ids.sale_line_ids.order_id[:1] if 'sale_line_ids' in invoice.line_ids._fields else False
        transaction_ids = (
            sale_order and sale_order.transaction_ids
            or 'transaction_ids' in invoice._fields and invoice.transaction_ids
        )
        transaction = transaction_ids.filtered(lambda t: t.state in ('done', 'authorized'))[:1]
        payment = invoice.matched_payment_ids[:1]
        if transaction:
            provider = transaction.provider_id
        elif payment:
            provider = payment.payment_method_line_id.payment_provider_id \
                        if 'payment_provider_id' in payment.payment_method_line_id._fields \
                        else False

        if pay := transaction or payment:
            if pay.payment_method_code == 'wire_transfer':
                pm_code = 'EFT/HAVALE'
            elif provider and provider.code not in ('custom', 'demo', 'none'):
                pm_code = 'ODEMEARACISI'
            elif 'card' in pay.payment_method_code:
                pm_code = 'KREDIKARTI/BANKAKARTI'
            else:
                pm_code = 'DIGER'

            if provider:
                ecom_data.append(('INT_PAYMENT_AGENT_NAME', provider.name))
            ecom_data.extend([
                ('INT_PAYMENT_METHOD', pm_code),
                ('INT_PAYMENTDATE', pay.create_date.strftime('%d.%m.%Y')),
            ])
        else:
    ...
    def _l10n_tr_add_billing_reference_node(self, document_node, vals):
        ...
        document_node['cac:BillingReference'] = {
            "cac:InvoiceDocumentReference": {
                'cbc:ID': reverese_entry_id,
                'cbc:IssueDate': invoice.reversed_entry_id.invoice_date or invoice.l10n_tr_original_invoice_date,
                'cbc:DocumentTypeCode': "İADE",
                'cbc:DocumentDescription': "İade Edilen Fatura"
            },
        }
    ...
    def _add_invoice_delivery_nodes(self, document_node, vals):
        ...
        if 'picking_ids' in invoice._fields and invoice.picking_ids:
            document_node['cac:Delivery']['cbc:ID'] = invoice.picking_ids[0].name
            document_node['cac:Delivery']['cbc:ActualDeliveryDate'] = invoice.delivery_date
        else:
    ...
    def _add_invoice_exchange_rate_nodes(self, document_node, vals):
        ...
        if vals['currency_name'] != 'TRY':
            document_node['cac:PricingExchangeRate'] = {
                'cbc:SourceCurrencyCode': vals['currency_name'],
                'cbc:TargetCurrencyCode': 'TRY',
                'cbc:CalculationRate': self._l10n_tr_get_currency_conversion_rate(invoice),
                'cbc:Date': invoice.invoice_date,
            }
    ...
    def _l10n_tr_get_issuer_party_line_codes(self, move_line):
        return {
            ...
            'cac:PostalAddress': {
                'cbc:CitySubdivisionName': move_line.move_id.partner_id.city,
                'cbc:CityName': move_line.move_id.partner_id.state_id.with_context(lang='tr_TR').name,
                'cac:Country': {
                    'cbc:Name': move_line.move_id.partner_id.country_id.with_context(lang='tr_TR').name,
                },
            },
        }
    ...
    def _add_invoice_line_delivery_nodes(self, line_node, vals):
        ...
        if move_line.move_id.l10n_tr_is_export_invoice:
            line_node['cac:Delivery'] = {
                'cac:DeliveryAddress': {
                    'cbc:StreetName': move_line.move_id.partner_id.street,
                    'cbc:CitySubdivisionName': move_line.move_id.partner_id.city,
                    'cbc:CityName': move_line.move_id.partner_id.state_id.with_context(lang='tr_TR').name,
                    'cbc:PostalZone': move_line.move_id.partner_id.zip,
                    'cac:Country': {'cbc:Name': move_line.move_id.partner_id.country_id.with_context(lang='tr_TR').name},
                },
                'cac:DeliveryTerms': {
                ...
                'cac:Shipment': {
                    'cbc:ID': 'NO_ID',
                    'cac:GoodsItem': {'cbc:RequiredCustomsID': move_line.l10n_tr_ctsp_number or move_line.product_id.l10n_tr_ctsp_number},
                    'cac:ShipmentStage': {'cbc:TransportModeCode': move_line.move_id.l10n_tr_shipping_type},
                },
            }
        ...
        if move_line.move_id.l10n_tr_exemption_code_id.code == 702:
            line_node['cac:Delivery'] = {
                'cac:Shipment': {
                    'cbc:ID': 'NO_ID',
                    'cac:GoodsItem': {
                        'cbc:RequiredCustomsID': move_line.l10n_tr_ctsp_number or move_line.product_id.l10n_tr_ctsp_number,
                    },
                ...
                tax_subtotals.append(
                    {
                        ...
                        "cbc:Percent": percent_return,
                        "cac:TaxCategory": {
                            "cac:TaxScheme": {
                                "cbc:Name": "Gerçek Usulde KDV",
    ...
    def _get_address_node(self, vals):
        ...
        return {
            'cbc:StreetName': ' '.join(s for s in [partner.street, partner.street2] if s),
            'cbc:CitySubdivisionName': partner.city,
            'cbc:CityName': partner.state_id.name,
            'cbc:PostalZone': partner.zip,
            'cac:Country': {
                'cbc:IdentificationCode': partner.country_id.code,
                'cbc:Name': partner.country_id.with_context(lang='tr_TR').name,
            }
        }
    ...
    def _get_ministry_party_node(self):
        ...
        return {
            ...
            'cac:PostalAddress': {
                'cbc:CitySubdivisionName': 'Ulus',
                'cbc:CityName': 'Ankara',
                'cac:Country': {'cbc:Name': 'Türkiye'},
            },
            'cac:PartyTaxScheme': {
                'cac:TaxScheme': {'cbc:Name': 'Ulus'},
            },
        }
    ...
    def _get_party_node(self, vals):
        ...
        party_node = {
            ...
            'cac:PartyTaxScheme': {
                'cac:TaxScheme': {
                    'cbc:Name': partner.l10n_tr_tax_office_id.name
                }
            },
    ...
    def _get_tax_category_node(self, vals):
        ...
        if grouping_key.get('l10n_tr_tax_withheld'):
            return {
                'cac:TaxScheme': {
                    'cbc:Name': grouping_key['name'],
                    'cbc:TaxTypeCode': grouping_key['tax_type_code'],
                },
            }
        is_withholding = grouping_key['tax_category_code'] == '9015'
        tax_category_node = {
            'cac:TaxScheme': {
                'cbc:Name': 'KDV Tevkifatı' if is_withholding else 'Gerçek Usulde KDV',
                'cbc:TaxTypeCode': grouping_key['tax_category_code'],
            }
        }
        ...
        tax_category_node.update({
            'cbc:TaxExemptionReasonCode': tax_invoice_exemption.code,
            'cbc:TaxExemptionReason': tax_invoice_exemption.with_context(lang='tr_TR').name,
        })
        return tax_category_node
            ...
            'cac:TaxSubtotal': [
                {
                    ...
                    'cbc:Percent': grouping_key['percent_withheld'] if is_withholding else grouping_key['amount'],
    ...
    def _add_document_line_allowance_charge_nodes(self, line_node, vals):
        ...
        for allowance_charge_node in line_node['cac:AllowanceCharge']:
            ...
            allowance_charge_node['cbc:MultiplierFactorNumeric'] = self.format_float(discount_percentage, vals['currency_dp'])
