...
class Coupon(models.Model):
    ...
    def _generate_code(self):
        ...
        programs = self.env['coupon.program'].search([('id', '=', pid)])

        for rec in programs:
            program_name = rec.name

        return str(program_name)+"-"+str(random.getrandbits(16)).zfill(5)

    code = fields.Text('Code', default=_generate_code, required=True, readonly=True)
