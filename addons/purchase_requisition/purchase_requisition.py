    ...
    def make_purchase_order(self, cr, uid, ids, partner_id, context=None):
        ...
        for requisition in self.browse(cr, uid, ids, context=context):
            if not requisition.multiple_rfq_per_supplier and supplier.id in filter(lambda x: x, [rfq.state != 'cancel' and rfq.partner_id.id or None for rfq in requisition.purchase_ids]):
                raise Warning(_('Warning!'), _('You have already one %s purchase order for this partner, you must cancel this purchase order to create a new quotation.') % rfq.state)
    ...
    def generate_po(self, cr, uid, ids, context=None):
        ...
        for tender in self.browse(cr, uid, ids, context=context):
            if tender.state == 'done':
                raise Warning(_('Warning!'), _('You have already generate the purchase order(s).'))
            ...
            for po_line in tender.po_line_ids:
                if po_line.state == 'confirmed':
                    ...
                    break
            if not confirm:
                raise Warning(_('Warning!'), _('You have no line selected for buying.'))
