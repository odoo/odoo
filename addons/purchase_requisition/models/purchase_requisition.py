...
class PurchaseRequisition(models.Model):
    ...
    name = fields.Char(string='Call for Tenders Reference', required=True, copy=False, default=lambda self: self.env['ir.sequence'].next_by_code('purchase.order.requisition'))
    ...
    po_line_ids = fields.One2many('purchase.order.line', compute='_compute_po_lines', string='Products by vendor')
    ...
    def _compute_po_lines(self):
        for tender in self:
             tender.po_line_ids = tender.purchase_ids.mapped('order_line').ids
    ...
    def open_product_line(self):
        ...
        res['context'] = dict(self.env.context)
        res['context'].update({
    ...
    def make_purchase_order(self, partner_id):
        ...
        for requisition in self:
            ...
            purchase = PurchaseOrder.with_context(mail_create_nolog=True).create(requisition._prepare_purchase_order(supplier))
