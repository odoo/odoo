...
class MarketingCampaign(models.Model):
    ...
    object_id = fields.Many2one('ir.model', string='Resource', required=True,
...
Normal - the campaign runs normally and automatically sends all emails and reports (be very careful with this mode, you're live!)""")
    ...
    activity_ids = fields.One2many('marketing.campaign.activity', 'campaign_id', string='Activities')
    fixed_cost = fields.Float(string='Fixed Cost', digits_compute=dp.get_precision('Product Price'),
        help="Fixed cost for running this campaign. You may also specify variable cost and revenue on "
             "each campaign activity. Cost and Revenue statistics are included in Campaign Reporting.")
    segment_ids = fields.One2many('marketing.campaign.segment', 'campaign_id', string='Segments')
    segments_count = fields.Integer(compute='_compute_count_segments', string='Segments')

    def _compute_count_segments(self):
    ...
    def _find_duplicate_workitems(self, cr, uid, record, campaign_rec, context=None):
        return MarketingCampaign._find_duplicate_workitems(campaign_rec, record)
    ...
    def _find_duplicate_workitems(self, record):
        """Finds possible duplicates workitems for a record in this campaign, based on a uniqueness
           ...
           :param record: recordset to find duplicates workitems for.
        """
        ...
        if unique_field:
            ...
            if unique_value:
                ...
                similar_resources = self.env[self.object_id.model].search([
                    (unique_field.name, '=', unique_value)
                ])
                if similar_resources:
                    duplicate_workitem_domain = [('res_id','in', similar_resources.ids),
                                                 ('campaign_id','=', self.id)]
        return self.env['marketing.campaign.workitem'].search(duplicate_workitem_domain).ids
...
class MarketingCampaignSegment(models.Model):
    ...
    name = fields.Char(required=True)
    campaign_id = fields.Many2one('marketing.campaign', string='Campaign', required=True, inded=True, ondelete="cascade")
    ...
    sync_last_date = fields.Datetime(string='Last Synchronization',
    ...
    sync_mode = fields.Selection([
        ...
        ], string='Synchronization mode', default='create_date',
    ...
    date_run = fields.Datetime(string='Launch Date', help="Initial start date of this segment.")
    date_done = fields.Datetime(string='End Date', help="Date this segment was last closed or cancelled.")
    ...
    def _compute_date_next_sync(self):
        ...
        sync_job = self.sudo().env.ref('marketing_campaign.ir_cron_marketing_campaign_every_day', False)
        next_sync = sync_job and sync_job.nextcall or False
        for segment in self:
            segment.date_next_sync = next_sync
    ...
    def process_segment(self):
        Activity = self.env['marketing.campaign.activity']
        ...
        Workitem.process_all(campaigns.ids)
        return True
...
class MarketingCampaignActivity(models.Model):
    ...
    start = fields.Boolean('Start', index=True,
    ...
    condition = fields.Text(required=True, default='True',
        help="Python expression to decide whether the activity can be executed, otherwise it will be deleted or cancelled."
             "The expression may use the following [recordset(s)] variables:\n"
             "   - activity: the campaign activity\n"
             "   - workitem: the campaign workitem\n"
             "   - resource: the resource object this campaign item represents\n"
             "   - transitions: list of campaign transitions outgoing from this activity\n"
             "...- re: Python regular expression module")
    type = fields.Selection(_action_types, required=True, default='email',
    ...
    to_ids = fields.One2many('marketing.campaign.transition', 'activity_from_id', string='Next Activities')
    from_ids = fields.One2many('marketing.campaign.transition', 'activity_to_id', string='Previous Activities')
    variable_cost = fields.Float(string='Variable Cost', digits_compute=dp.get_precision('Product Price'),
        help="Set a variable cost if you consider that every campaign item that has reached this point "
             "has entailed a certain cost. You can get cost statistics in the Reporting section")
    revenue = fields.Float(digits=0,
        help="Set an expected revenue if you consider that every campaign item that has reached this point "
             "has generated a certain revenue. You can get revenue statistics in the Reporting section")
    signal = fields.Char(
        help='An activity with a signal can be called programmatically. '
             'Be careful, the workitem is always created when a signal is sent')
    keep_if_condition_not_met = fields.Boolean(string="Don't Delete Workitems",
        help="By activating this option, workitems that aren't executed because "
             "the condition is not met are marked as cancelled instead of being deleted.")
    ...
    def _process_wi_report(self, cr, uid, activity, workitem, context=None):
        return MarketingCampaignActivity._process_wi_report(activity, workitem)

    @api.v8
    def _process_wi_report(self, workitem):
        report_data, format = render_report(self_cr, self._uid, [], self.report_id.report_name, {}, context=self._context)
        attach_vals = {
            'name': '%s_%s_%s' % (self.report_id.report_name, self.name, workitem.partner_id.name),
            'datas_fname': '%s.%s' % (self.report_id.report_name, self.report_id.report_type),
    ...
    def _process_wi_email(self, cr, uid, activity, workitem, context=None):
        return MarketingCampaignActivity._process_wi_email(activity, workitem)
    ...
    def _process_wi_action(self, cr, uid, activity, workitem, context=None):
        return MarketingCampaignActivity._process_wi_action(activity, workitem)

    @api.v8
    def _process_wi_action(self, workitem):
        self.self.server_action_id.with_context(
...
class MarketingCampaignTransition(models.Model):
    ...
    activity_from_id = fields.Many2one('marketing.campaign.activity', string='Previous Activity',
        index=True, required=True, ondelete="cascade")
    activity_to_id = fields.Many2one('marketing.campaign.activity', string='Next Activity',
        required=True, ondelete="cascade")
    interval_nbr = fields.Integer(string='Interval Value', required=True, default=1)
    interval_type = fields.Selection(_interval_units, string='Interval Unit', required=True, default='days')

    trigger = fields.Selection([
            ('auto', 'Automatic'),
            ('time', 'Time'),
            ('cosmetic', 'Cosmetic'),  # fake plastic transition
        ], required=True, default='time',
    ...
    def _check_campaign(self):
        if self.filtered(lambda transition: transition.activity_from_id.campaign_id != transition.activity_to_id.campaign_id):
            raise ValidationError(_('The To/From Activity of transition must be of the same Campaign'))

    def _delta(self):
        self.ensure_one()
        if self.trigger != 'time':
            raise ValueError(_('Delta is only relevant for timed transition.'))
...
class MarketingCampaignWorkitem(models.Model):
    ...
    activity_id = fields.Many2one('marketing.campaign.activity', string='Activity', required=True, readonly=True)
    campaign_id = fields.Many2one('marketing.campaign', related='activity_id.campaign_id',
        string='Campaign', readonly=True, store=True)
    object_id = fields.Many2one('ir.model', related='activity_id.campaign_id.object_id',
        string='Resource', index=True, readonly=True, store=True)
    res_id = fields.Integer(string='Resource ID', index=True, readonly=True)
    res_name = fields.Char(string='Resource Name', compute='_compute_res_name', search='_search_res_name')
    date = fields.Datetime(string='Execution Date', readonly=True,
    ...
    state = fields.Selection([
            ...
            ('exception', 'Exception'),
            ('done', 'Done'),
        ], string='Status', default='todo', readonly=True, copy=False)
    ...
    def _search_res_name(self, operator, operand):
        """Returns a domain with ids of workitem whose `res_name` matches with the given `operand`"""
        if not operand:
            return []
        condition_name = [None, operator, operand]
        self._cr.execute("""select w.id, w.res_id, m.model  \
    ...
    def _process_one(self, cr, uid, workitem, context=None):
        return MarketingCampaignWorkitem._process_one(workitem)
    ...
    def _process_one(self):
        ...
        try:
            ...
            if activity.condition:
                if not eval(activity.condition, eval_context):
            ...
            if campaign_mode in ('manual', 'active'):
                result = activity.process(self.id)
            ...
            if result:
                ...
                for transition in activity.to_ids.filtered(lambda transition: transition.trigger != 'cosmetic'):
                    launch_date = False
    ...
    def preview(self):
        ...
        if self.activity_id.type == 'email':
            view_ref = self.env.ref('mail.email_template_preview_form', False)
            res = {
                ...
                'context': "{'template_id':%d,'default_res_id':%d}"%
                                (self.activity_id.email_template_id.id, self.res_id)
            }
