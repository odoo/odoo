...
class AccountMove(models.Model):
    ...
    def l10n_es_tbai_resend_bill(self):
        self.ensure_one()
        if self.l10n_es_tbai_post_document_id:
            self.l10n_es_tbai_post_document_id = False
        error = self.with_context(batuz_correction=True)._l10n_es_tbai_post()
        if error:
            error = error + "\n\n" + _("You may want to fix and resend or adapt the bill back to the earlier accepted "
                                       "submission as that is still the one that is valid now. ")
