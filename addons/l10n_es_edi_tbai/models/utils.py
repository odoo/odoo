...
L10N_ES_TBAI_NS_MAP = {"ds": "http://www.w3.org/2000/09/xmldsig#"}

def l10n_es_tbai_base64_print(string):
...
def l10n_es_tbai_get_uri(uri, reference):
...
def l10n_es_tbai_reference_digests(node):
    for reference in node.findall("ds:Reference", namespaces=L10N_ES_TBAI_NS_MAP):
        ref_node = l10n_es_tbai_get_uri(reference.get("URI", ""), reference)
        lib = hashlib.new("sha256")
        lib.update(ref_node)
        reference.find("ds:DigestValue", namespaces=L10N_ES_TBAI_NS_MAP).text = b64encode(lib.digest())


def l10n_es_tbai_long_to_bytes(n, blocksize=0):
    """
...
def l10n_es_tbai_fill_signature(node, private_key):
    signed_info_xml = node.find("ds:SignedInfo", namespaces=L10N_ES_TBAI_NS_MAP)
    ...
    node.find("ds:SignatureValue", namespaces=L10N_ES_TBAI_NS_MAP).text = l10n_es_tbai_base64_print(b64encode(signature))
