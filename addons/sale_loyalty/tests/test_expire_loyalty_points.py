...
class TestExpireLoyaltyPoints(TestSaleCouponCommon):
    ...
    def test_expire_loyalty_points(self):
        ...
        future_date = fields.Datetime.now() + timedelta(days=self.loyalty_program.expire_after + 1)
        with self.mock_datetime_and_now(future_date):
            for history_record in history_records:
                history_record._compute_is_line_expired()
                self.assertEqual(history_record.is_line_expired, 'expired')
