...
class TestProgramRules(TestSaleCouponCommon, PaymentCommon):
    ...
    def test_program_rules_min_amount_not_reached_and_specific_product(self):
        ...
        self.env['sale.order.line'].create([
            {'product_id': self.product_A.id, 'product_uom_qty': 1.0, 'order_id': order.id},
            {'product_id': self.product_B.id, 'product_uom_qty': 40.0, 'order_id': order.id},
        ])
    ...
    def test_program_rules_min_amount_reached_and_specific_product(self):
        ...
        self.env['sale.order.line'].create([
            {'product_id': self.product_A.id, 'product_uom_qty': 2.0, 'order_id': order.id},
            {'product_id': self.product_B.id, 'product_uom_qty': 20.0, 'order_id': order.id},
        ])
        self.assertEqual(len(order.order_line), 2)
        self.assertEqual(order.amount_untaxed, 300)
