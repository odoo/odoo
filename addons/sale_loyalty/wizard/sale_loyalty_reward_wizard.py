...
class SaleLoyaltyRewardWizard(models.TransientModel):
    ...
    applied_reward_description = fields.Char()
    ...
    def action_apply_coupon(self):
        ...
        if len(status) == 1:
            ...
            if len(rewards) == 1:
                ...
                self.order_id._update_programs_and_rewards()
                self.applied_reward_description = self.env._("%s applied!", rewards.description)
            else:
                self.applied_reward_description = self.env._(
                    "Coupon code '%s' applied!", self.coupon_code
                )
        return {
