...
export class GiftCardCopy extends Interaction {
    ...
    onClick(ev) {
        ...
        iconEl.dataset.icon = "check";
        iconEl.classList.add("text-success");
        clearTimeout(this.timeout);
        this.timeout = this.waitForTimeout(() => {
            iconEl.dataset.icon = "content_copy";
...
export class GiftCardSend extends Interaction {
    ...
    async onSubmit() {
        ...
        const code = this.el.elements.code.value;
        const email = this.el.elements.email.value;
        try {
            await this.waitFor(rpc("/gift_card/send", { code, email }));
        } catch (error) {
            if (error instanceof RPCError) {
                this.services.notification.add(
                    _t("Could not send the gift card. Please try again later."),
                    { type: "danger" },
                );
                return;
            }
        ...
        this.services.notification.add(_t("Gift card sent to %s", email), { type: "success" });
    }
}
