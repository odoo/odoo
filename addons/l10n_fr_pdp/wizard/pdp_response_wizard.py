...
class PdpResponseWizard(models.TransientModel):
    ...
    def _is_fully_paid(self, move):
        ...
        # Check that we have an actual payment; and e.g. the unpaid credit note is not just reconciled with an unpaid invoice
        payment_terms = move.line_ids.filtered(lambda line: line.display_type == 'payment_term')
        ...
        return bool(counterpart_amls.filtered(lambda m: m.move_type != counterpart_move_type))
