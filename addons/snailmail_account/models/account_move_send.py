...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if snailmail_moves_without_valid_address := moves.filtered(
            lambda m: moves_data[m]['sending_method'] == 'snailmail' and not self.env['snailmail.letter']._is_valid_address(m.partner_id)
        ):
    ...
    def _hook_if_success(self, moves_data):
        ...
        to_send = {
            ...
            if move_data['sending_method'] == 'snailmail' and self._is_applicable_to_move('snailmail', move)
        }
        if to_send:
