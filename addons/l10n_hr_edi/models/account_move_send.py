...
class AccountMoveSend(models.AbstractModel):
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        edi_user = next(iter(invoices_data)).company_id.account_edi_proxy_client_ids.filtered(lambda u: u.proxy_type == 'mojeracun')

        # This instead should call the _mer_send() method now, giving it only the move itself.
        try:
            ...
            for invoice, invoice_data in invoices_data_mer.items():
                invoice.mer_move_state = 'error'
            ...
            if response.get('error'):
                ...
                for message, (invoice, invoice_data) in zip(response['messages'], invoices_data_mer.items()):
                    invoice.mer_message_uuid = message['message_uuid']
                    invoice.mer_move_state = 'processing'
                    invoices |= invoice
                log_message = _('The document has been sent to the MojEracun Access Point for processing')
                invoices._message_log_batch(bodies={invoice.id: log_message for invoice in invoices})
                self.env.ref('l10n_hr_edi.ir_cron_mer_get_message_status')._trigger(at=fields.Datetime.now() + timedelta(minutes=5))
