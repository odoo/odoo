...
def _make_request(company, endpoint_type, params=False):
    ...
    if response.status_code != 200:
        try:
            ...
            error_message = False
        raise UserError(_("Error handling request: %s", error_message) if error_message else _("HTTP %s: Connection error.", response.status_code))
    ...
    if endpoint != endpoints['receive']:
        ...
        if 'error' in response_json:
            message = _('The url that this service requested returned an error. The url it tried to contact was %(url)s. %(error_message)s', url=url, error_message=response_json['error']['message'])
            ...
            message = _("MER service returned an error: Username '%(name)s': %(desc)s", name=response_json['Username'].get('Value'), desc=response_json['Username'].get('Messages'))
...
def _mer_api_receive_document(company, electronic_id):
    ...
    if response_bstr[:5] != b'<?xml':
        raise MojEracunServiceError('service_error', _("Failed to retrieve document XML for ElectronicId: %s", electronic_id))
