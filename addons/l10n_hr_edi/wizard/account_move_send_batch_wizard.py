...
class AccountMoveSendBatchWizard(models.TransientModel):
    ...
    def _compute_summary_data(self):
        ...
        for wizard in self:
            for move in wizard.move_ids.filtered(lambda m: wizard._get_default_sending_method(m) == 'eracun'):
    ...
    def action_send_and_print(self, force_synchronous=False, allow_fallback_pdf=False):
        # EXTENDS 'account'
        self.ensure_one()
        self.move_ids.filtered(lambda m: self._get_default_sending_method(m) == 'eracun').eracun_move_state = 'to_send'
