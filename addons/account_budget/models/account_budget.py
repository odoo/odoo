...
class CrossoveredBudgetLines(models.Model):
    ...
    def _compute_theoritical_amount(self):
        ...
        for line in self:
            # Used for the report
            line_date_from = datetime.from_date(line.date_from)
            line_date_to = datetime.from_date(line.date_to)
            ...
            if self.env.context.get('wizard_date_from') and self.env.context.get('wizard_date_to'):
                ...
                if date_from < line_date_from:
                    date_from = line_date_from
                elif date_from > line_date_to:
                    date_from = False

                if date_to > line_date_to:
                    date_to = line_date_to
                elif date_to < line_date_from:
                    date_to = False

                theo_amt = 0.00
                if date_from and date_to:
                    line_timedelta = line_date_to - line_date_from
                ...
                if line.paid_date:
                    ...
                    line_timedelta = line_date_to - line_date_from
                    elapsed_timedelta = now - line_date_from

                    if elapsed_timedelta.days < 0:
                        # If the budget line has not started yet, theoretical amount should be zero
                        theo_amt = 0.00
                    elif line_timedelta.days > 0 and now < line_date_to:
