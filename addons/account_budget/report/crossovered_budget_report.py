...
class budget_report(report_sxw.rml_parse):
    ...
    def funct(self, object, form, ids=None, done=None, level=1):
        ...
        for an_id in an_ids:
            if not an_id[0]:
                continue
            analytic_name = acc_analytic_obj.browse(self.cr, self.uid, [an_id[0]])
            res={
                'b_id': '-1',
                'a_id': '-1',
            ...
            line_ids = c_b_lines_obj.search(self.cr, self.uid, [('id', 'in', budget_ids), ('analytic_account_id','=',an_id[0]), ('date_to', '>=', d_from), ('date_from', '<=', d_to)])
