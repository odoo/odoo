...
class TestTheoreticalAmount(TestAccountBudgetCommon):
    ...
    def test_01(self):
        """Start"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-01-01 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, 0)

    def test_02(self):
        """After 24 hours"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-01-02 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -1)

    def test_03(self):
        """After 36 hours, we do no take the hours into account"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-01-02 12:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -1.0)

    def test_04(self):
        """After 48 hours"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-01-03 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -2)

    def test_05(self):
        """After 10 days"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-01-11 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -10)

    def test_06(self):
        """After 50 days"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-02-20 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -50)

    def test_07(self):
        """After 182 days, exactly half of the budget line"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-07-02 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -182)

    def test_08(self):
        """After 308 days at noon, we do not take the hours into account"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-11-05 12:00:00'))  # remember, remember
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -308.0)

    def test_09(self):
        """One day before"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-12-30 00:00:00'))
        self.mock_datetime.now.return_value = date
        self.assertAlmostEqual(self.line.theoritical_amount, -363)

    def test_10(self):
        """At last"""
        date = Datetime.to_string(Datetime.from_string(str(datetime.now().year + 1)+'-12-31 00:00:00'))
        self.mock_datetime.now.return_value = date
