...
class StockPicking(models.Model):
    ...
    def do_print_picking(self):
        report_action = super().do_print_picking()
        for picking in self.filtered(lambda p: p.company_id.account_fiscal_country_id.code == 'PT'):
            if picking.l10n_pt_print_version == 'none':
    ...
    def action_open_print_wizard(self):
        pickings_reprint = self.filtered(
            lambda p: p.company_id.account_fiscal_country_id.code == 'PT'
                      and p.l10n_pt_print_version != 'none'
        )
        if pickings_reprint:
            return {
                'name': 'Print Reason',
                ...
                'view_mode': 'form',
                'target': 'new',
                'context': {'active_ids': self.ids, 'res_model': 'stock.picking'},
            }
        return self.env.ref('stock.action_report_delivery').report_action(self)

    def update_l10n_pt_print_version(self):
        self.ensure_one()
        if self.l10n_pt_print_version == 'none':
            self.l10n_pt_print_version = 'original'
        elif self.l10n_pt_print_version == 'original':
