...
class AccountPayment(models.Model):
    ...
    def _prepare_move_liquidity_lines(self, default_values):
        """
        When paying with multiple own checks, split the single liquidity line into one line per
        check (on the outstanding account), so each check has its own outstanding line.
        """
        if not (self.payment_method_code == 'own_checks' and self.payment_type == 'outbound' and len(self.l10n_latam_new_check_ids) > 1):
            return super()._prepare_move_liquidity_lines(default_values)

        checks_total = sum(self.l10n_latam_new_check_ids.mapped('amount'))
        liquidity_lines = []
        liquidity_balance_total = 0.0
        for check in self.l10n_latam_new_check_ids:
            if check == self.l10n_latam_new_check_ids[-1]:
                liquidity_balance = self.company_id.currency_id.round(default_values['balance'] - liquidity_balance_total)
            else:
                liquidity_balance = self.company_id.currency_id.round(default_values['balance'] * check.amount / checks_total)
    ...
    def _l10n_latam_check_split_move(self):
        for payment in self.filtered(lambda x: x.payment_method_code == 'own_checks' and x.payment_type == 'outbound' and len(x.l10n_latam_new_check_ids) == 1):
            payment.l10n_latam_new_check_ids.outstanding_line_id = payment._seek_for_lines()[0].id
