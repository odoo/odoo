...
class l10nLatamAccountPaymentCheck(models.Model):
    ...
    def _prepare_void_move_vals(self):
        return {
                'ref': 'Void check',
                'journal_id': self.liquidity_line_id.move_id.journal_id.id,
                'line_ids': [Command.create({
                    'name': "Void check %s" % self.liquidity_line_id.name,
                    'date_maturity': self.liquidity_line_id.date_maturity,
                    'amount_currency': self.liquidity_line_id.amount_currency,
                    'currency_id': self.liquidity_line_id.currency_id.id,
                    'debit': self.liquidity_line_id.debit,
                    'credit': self.liquidity_line_id.credit,
                    'partner_id': self.liquidity_line_id.partner_id.id,
                    ...
                    Command.create({
                        ...
                        'credit': -self.liquidity_line_id.credit,
                        'partner_id': self.liquidity_line_id.partner_id.id,
                        'account_id': self.liquidity_line_id.account_id.id,
                     })]
            }

    @api.depends('liquidity_line_id.amount_residual')
    def _compute_issue_state(self):
        for rec in self:
            if not rec.liquidity_line_id:
                rec.issue_state = False
            elif rec.amount and not rec.liquidity_line_id.amount_residual:
                reconciled_line = rec.liquidity_line_id.full_reconcile_id.reconciled_line_ids - rec.liquidity_line_id
    ...
    def action_void(self):
        for rec in self.filtered('liquidity_line_id'):
            void_move = rec.env['account.move'].create(rec._prepare_void_move_vals())
            void_move.action_post()
            (void_move.line_ids[1] + rec.liquidity_line_id).reconcile()
    ...
    def action_show_reconciled_move(self):
        ...
        return move.get_formview_action()

    def action_show_journal_entry(self):
        self.ensure_one()
        return self.liquidity_line_id.move_id.get_formview_action()

    def _get_reconciled_move(self):
        reconciled_line = self.liquidity_line_id.full_reconcile_id.reconciled_line_ids - self.liquidity_line_id
