...
class TestOwnChecks(L10nLatamCheckTest):
    ...
    def test_pay_bill_with_several_checks_single_payment(self):
        ...
        payment_method_line = self.bank_journal._get_available_payment_method_lines('outbound').filtered_domain(
            [('code', '=', 'own_checks')])[:1]
        ...
        self.assertEqual(invoice.payment_state, 'in_payment',
                         "Bill must stay 'In Payment' until the checks are reconciled.")
        outstanding_lines = payment.l10n_latam_new_check_ids.outstanding_line_id
        self.assertEqual(len(outstanding_lines), 2, "Each check must get its own liquidity line.")
        self.assertEqual(outstanding_lines.move_id, payment.move_id,
                         "The liquidity lines must be on the payment entry itself, not on a split move.")
