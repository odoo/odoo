...
class TestThirdChecks(L10nLatamCheckTest):
    ...
    def test_02_third_party_check_delivery(self):
        ...
        # Check Delivery
        vals = {
            'l10n_latam_operation_ids': [Command.set([check.id])],
            'partner_id': self.partner_a.id,
        ...
        operations = self.env['account.payment'].search([('l10n_latam_operation_ids', '=', check.id), ('state', '=', 'posted')], order="date desc, id desc")
