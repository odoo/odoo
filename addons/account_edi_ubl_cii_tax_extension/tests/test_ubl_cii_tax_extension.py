...
class TestAccountEdiUblCiiTaxExtension(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.fixed_tax = cls.company_data['default_tax_sale'].copy({
            ...
            'amount_type': 'fixed',
            'ubl_cii_allowance_charge_reason': 'Discount',
            'ubl_cii_allowance_charge_reason_code': '42',
        })
        cls.fixed_tax2 = cls.fixed_tax.copy({
            'name': 'Fixed tax 2',
            'ubl_cii_allowance_charge_reason': 'Some Reason',
            'ubl_cii_allowance_charge_reason_code': '41',
        })
    ...
    def test_allowance_charge_fixed_taxes(self):
        ...
        self.assertEqual(allowance_charge_nodes[0].findtext('.//{*}AllowanceChargeReason'), 'Discount')
        self.assertEqual(allowance_charge_nodes[0].findtext('.//{*}AllowanceChargeReasonCode'), '42')
    ...
    def test_allowance_charge_multiple_taxes(self):
        ...
        self.assertEqual(allowance_charge_nodes[0].findtext('.//{*}AllowanceChargeReason'), 'Discount')
        self.assertEqual(allowance_charge_nodes[0].findtext('.//{*}AllowanceChargeReasonCode'), '42')
        self.assertEqual(allowance_charge_nodes[1].findtext('.//{*}AllowanceChargeReason'), 'Some Reason')
        self.assertEqual(allowance_charge_nodes[1].findtext('.//{*}AllowanceChargeReasonCode'), '41')

    def test_no_allowance_charge_codes(self):
        self.fixed_tax.write({
            'ubl_cii_allowance_charge_reason': False,
            'ubl_cii_allowance_charge_reason_code': False,
        })
