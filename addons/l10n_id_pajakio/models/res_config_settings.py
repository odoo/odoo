...
class ResConfigSettings(models.TransientModel):
    ...
    l10n_id_pajakio_test_email = fields.Char(
        string="Pajak.io Testing Account Email", config_parameter="l10n_id_pajakio.test_email")
    l10n_id_pajakio_test_password = fields.Char(
        string="Pajak.io Testing Account Password", config_parameter="l10n_id_pajakio.test_password")

    l10n_id_pajakio_email = fields.Char(
        string="Pajak.io Production Account Email", config_parameter="l10n_id_pajakio.email")
    l10n_id_pajakio_password = fields.Char(
        string="Pajak.io Production Account Password", config_parameter="l10n_id_pajakio.password")
    ...
    def action_register_company_pajakio(self):
        ...
        ).get_param('l10n_id_pajakio.mode')
    ...
    def action_link_company_iap(self):
        ...
        self.env['iap.account'].get(
            "l10n_id_pajakio_proxy")._get_account_information_from_iap()

        mode = self.env['ir.config_parameter'].sudo(
        ).get_param('l10n_id_pajakio.mode')
        client_id = self.env['ir.config_parameter'].get_param(
            'l10n_id_pajakio.test_client_id') if mode == "test" else self.env['ir.config_parameter'].get_param('l10n_id_pajakio.client_id')
        if not client_id:
            raise ValidationError(
                _("Pajak.io: Cannot link Pajak.io service because Client ID is not found. Please register your company first."))
        params = {
            "client_id": client_id
        }
    ...
    def _compute_l10n_id_pajakio_register_button(self):
        ...
        if client id is set, should hide all the sign in/up + register company buttons
        """
        config_param = self.env['ir.config_parameter'].sudo()
        mode = config_param.get_param('l10n_id_pajakio.mode')
        for record in self:
            if mode == "test":
                record.l10n_id_pajakio_hide_register = bool(config_param.get_param(
                    'l10n_id_pajakio.test_email') and config_param.get_param('l10n_id_pajakio.test_password'))
                record.l10n_id_pajakio_hide_register_company = not record.l10n_id_pajakio_hide_register or bool(
                    config_param.get_param('l10n_id_pajakio.test_client_id'))
            else:
                record.l10n_id_pajakio_hide_register = bool(config_param.get_param(
                    'l10n_id_pajakio.email') and config_param.get_param('l10n_id_pajakio.password'))
                record.l10n_id_pajakio_hide_register_company = bool(
                    config_param.get_param('l10n_id_pajakio.client_id'))

    def _compute_l10n_id_pajakio_linked(self):
        """ Compute whether the pajak.io service is linked or not """
        config_param = self.env['ir.config_parameter'].sudo()
        for record in self:
            record.l10n_id_pajakio_linked = bool(
                config_param.get_param('l10n_id_pajakio.active'))
