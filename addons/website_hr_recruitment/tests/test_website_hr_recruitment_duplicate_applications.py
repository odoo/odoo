...
class TestWebsiteHrRecruitmentForm(HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_detects_a_full_duplicate_of_an_ongoing_application(self):
        ...
        """
        res = self.url_open("/website/form/hr.applicant", data=self.new_applicant_data)
    ...
    def test_detects_a_email_duplicate_of_an_ongoing_application(self):
        """
        Assert that a to-do activity is added to a new applicant if another
        ongoing applicant with the same email exists.
        """
        new_applicant_data = {
            **self.new_applicant_data,
            "partner_phone": "5678",
            "linkedin_profile": "linkedin.com/in/totally-new-applicant",
        }
    ...
    def test_detects_a_phone_duplicate_of_an_ongoing_application(self):
        ...
        new_applicant_data = {
            **self.new_applicant_data,
            "email_from": "totallynewapplicant@example.com",
            "linkedin_profile": "linkedin.com/in/totally-new-applicant",
        }
        res = self.url_open("/website/form/hr.applicant", data=new_applicant_data)
        new_applicant = self.env["hr.applicant"].browse(res.json().get("id"))
    ...
    def test_detects_a_email_phone_duplicate_of_an_ongoing_application(self):
        """
        Assert that a to-do activity is added to a new applicant if another
    ...
    def test_detects_a_email_linkedin_duplicate_of_an_ongoing_application(self):
        """
        Assert that a to-do activity is added to a new applicant if another
    ...
    def test_prioritizes_a_refused_message_over_ongoing(self):
       ...
       """
        ...
        self.env["hr.applicant"].create(
            {
                "partner_name": "some other new applicant",
                "email_from": "test@example.com",
                "partner_phone": "1234",
                "linkedin_profile": "linkedin.com/in/test-applicant",
