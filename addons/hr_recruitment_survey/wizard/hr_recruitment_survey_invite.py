...
class HrRecruitmentSurveyInvite(models.TransientModel):
    ...
    def _prepare_answers(self, partners, emails):
        """
        super()._prepare_answers() would return the previous answer if it was
        not closed yet to reuse it.
        We always want to create a new survey, even if the previous one is not
        closed yet, so we'll override super()._prepare_answers() completely.
        """
        self.ensure_one()
        ...
        for applicant in self.applicant_ids:
            ...
            answers.write({'applicant_id': applicant.id})
