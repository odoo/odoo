...
class HrRecruitmentSurveyInvite(models.TransientModel):
    ...
    def _compute_survey_start_url(self):
        for invite in self:
            if invite.single_survey_id:
                invite.survey_start_url = werkzeug.urls.url_join(invite.single_survey_id.get_base_url(), invite.single_survey_id.get_start_url())
            else:
    ...
    def _compute_survey_users_login_required(self):
        for invite in self:
            invite.survey_users_login_required = any(invite.survey_ids.users_login_required)

    @api.depends('survey_ids')
    def _compute_survey_users_can_signup(self):
        for invite in self:
            invite.survey_users_can_signup = any(invite.survey_ids.users_can_signup)
