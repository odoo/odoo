...
class SurveyInvite(models.TransientModel):
    ...
    is_many_applicants = fields.Boolean(compute="_compute_is_many_applicants", store=False)
    ...
    def _compute_single_survey(self):
        for invite in self:
            if invite.survey_ids and len(invite.survey_ids.ids) == 1:
                invite.single_survey_id = self.env['survey.survey'].browse(invite.survey_ids.ids[0])
            else:
                invite.single_survey_id = False

    @api.depends('applicant_ids')
    def _compute_is_many_applicants(self):
        for invite in self:
            invite.is_many_applicants = len(invite.applicant_ids.ids) > 1

    @api.depends('survey_access_mode', 'survey_ids')
    def _compute_hide_url(self):
        for invite in self:
            invite.hide_url = invite.survey_access_mode != 'public' or len(invite.survey_ids.ids) > 1
    ...
    def _compute_survey_start_url(self):
        for invite in self:
            if len(invite.survey_ids.ids) > 1:
                ...
                invite.survey_start_url = werkzeug.urls.url_join(invite.single_survey_id.get_base_url(), invite.single_survey_id.get_start_url()) if invite.single_survey_id else False

    @api.depends('survey_ids')
    def _compute_survey_users_login_required(self):
        for invite in self:
            for survey in invite.survey_ids:
                if survey.users_login_required:
                    invite.survey_users_login_required = True
                    break
            invite.survey_users_login_required = False

    @api.depends('survey_ids')
    def _compute_survey_users_can_signup(self):
        for invite in self:
            for survey in invite.survey_ids:
                if survey.users_can_signup:
                    invite.survey_users_can_signup = True
                    break
            invite.survey_users_can_signup = False

    def _partners_access(self):
        for partner, survey in zip(self.applicant_ids.mapped('partner_id'), self.applicant_ids.mapped('survey_id')):
            if survey.users_login_required and self.partner_ids:
                if not survey.users_can_signup:
    ...
    def _onchange_emails(self):
        if len(self.survey_ids.ids) > 1:
            return
        if self.emails and (self.single_survey_id.users_login_required and not self.single_survey_id.users_can_signup):
    ...
    def _prepare_answers(self, partners, emails):
        ...
        for applicant in self.applicant_ids:
            existing_answers = self.env['survey.user_input'].search([
                ...
                ('partner_id', 'in', applicant.partner_id.ids)
            ])
    ...
    def _compute_subject(self):
        for invite in self:
            if invite.subject:
                ...
                if len(invite.survey_ids.ids) == 1:
                    invite.subject = _("Participate to %(survey_name)s", survey_name=invite.single_survey_id.display_name)
                else:
                    invite.subject = "Interview Invitation"
    ...
    # Security
    @api.depends('survey_ids', 'survey_ids.restrict_user_ids')
    ...
    def _compute_accessible_by_interviewer_user(self):
        ...
        for record in self:
            record.accessible_by_interviewer_user = all(
                any(
                    (current_user in job.interviewer_ids) or
                    any(current_user in app.interviewer_ids for app in job.application_ids)
