...
class MailGroup(http.Controller):
    ...
    def _get_archives(self, group_id):
        groups = request.env['mail.message'].read_group(
            [('model', '=', 'mail.channel'), ('res_id', '=', group_id)], ['subject', 'date'],
            groupby="date", orderby="date desc")
        for group in groups:
            group['date_begin'] = '%s' % group['__domain'][0][2][:10]
            group['date_end'] = '%s' % group['__domain'][1][2][:10]
        return groups

    @http.route("/groups", type='http', auth="public", website=True)
    def view(self, **post):
        groups = request.env['mail.channel'].search([('alias_id', '!=', False), ('alias_id.alias_name', '!=', False)])

        # compute statistics
        month_date = datetime.today() - relativedelta.relativedelta(months=1)
        messages = request.env['mail.message'].read_group(
            [('model', '=', 'mail.channel'), ('date', '>=', Datetime.to_string(month_date))],
            [], ['res_id'])
        message_data = { message['res_id']: message['res_id_count'] for message in messages }

        group_data = { group.id: {'monthly_message_nbr': message_data.get(group.id, 0)} for group in groups }
    ...
    def subscription(self, channel_id=0, subscription="on", email='', **kw):
        ...
        if subscribe:
            ...
            channel.check_access_rule('read')
            return channel.sudo().write({'channel_partner_ids': [(4, partner_id) for partner_id in partner_ids]})
    ...
    def thread_headers(self, group, page=1, mode='thread', date_begin=None, date_end=None, **post):
        Thread = request.env['mail.message']
        ...
        messages = Thread.search(domain, limit=self._thread_per_page, offset=pager['offset'])
        values = {
            'messages': messages,
            'group': group,
            'pager': pager,
    ...
    def render_messages(self, group, message, **post):
        ...
        return request.env['ir.ui.view'].render('website_mail_channel.messages_short', values, engine='ir.qweb')
