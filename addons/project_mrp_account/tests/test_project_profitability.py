...
class TestSaleProjectProfitabilityMrp(TestProjectProfitabilityCommon):
    ...
    def test_profitability_for_multi_level_mo(self):
        ...
        route_1 = self.env['stock.route'].create({'name': 'Route 1', 'sequence': 10})
        ...
        raw_material = self.env['product.product'].create({
            'name': 'raw_material',
            'type': 'consu',
            'route_ids': [Command.set([route_1.id])],
        })

        # Manually creating BoMs for our products to set up the hierarchy.
        main_product_bom, component_bom = self.env['mrp.bom'].create([
            {
        ...
        component_mo = self.env['mrp.production'].search(
            [('product_id', '=', component.id), ('project_id', '=', project.id)
        ...
        parent_aal, child_aal = self.env["account.analytic.line"].create([{
            "name": main_mo.name,
