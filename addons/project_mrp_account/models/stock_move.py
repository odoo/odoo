# Part of Odoo. See LICENSE file for full copyright and licensing details.

from odoo import fields, models


class StockMove(models.Model):
    _inherit = 'stock.move'

    def _get_analytic_distribution(self):
        if not self.raw_material_production_id:
            return super()._get_analytic_distribution()
        account_ids = self.raw_material_production_id.project_id._get_analytic_account_ids()
        if not account_ids:
            return super()._get_analytic_distribution()
        return {",".join((str(account_id) for account_id in account_ids.ids)): 100}

    def _account_analytic_entry_move(self):
        unvalid_moves = self.filtered(lambda m: m.raw_material_production_id and not m.raw_material_production_id.project_id)
        super(StockMove, self - unvalid_moves)._account_analytic_entry_move()
