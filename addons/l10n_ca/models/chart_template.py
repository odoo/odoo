...
class AccountChartTemplate(models.Model):
    ...
    def _create_cash_discount_loss_account(self, company, code_digits):
        if not self == self.env.ref('l10n_ca.ca_en_chart_template_en'):
            return super()._create_cash_discount_loss_account(company, code_digits)
        cash_discount_loss_account = self.env['account.account'].search([('company_id', '=', company.id), ('code', 'like', '550001')], limit=1)
    ...
    def _create_cash_discount_gain_account(self, company, code_digits):
        ...
        cash_discount_gain_account = self.env['account.account'].search([('company_id', '=', company.id), ('code', 'like', '420001')], limit=1)
