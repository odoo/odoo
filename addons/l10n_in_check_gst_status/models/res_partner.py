...
class ResPartner(models.Model):
    ...
    l10n_in_gstin_verified_status = fields.Selection([
            ("active", "Active"),
            ("inactive", "Inactive"),
            ("invalid", "Invalid"),
        ],
    ...
    l10n_in_gstin_status_color = fields.Selection([
            ("green", "Green"),
            ("yellow", "Yellow"),
            ("red", "Red"),
        ],
        compute="_compute_l10n_in_gstin_status_color",
    )
    ...
    @api.depends('l10n_in_gstin_verified_status', 'l10n_in_gstin_verified_date')
    def _compute_l10n_in_gstin_status_color(self):
        for partner in self:
            if partner.country_code == 'IN' and partner.l10n_in_gstin_verified_status:
                partner.l10n_in_gstin_status_color = 'red'
                if partner.l10n_in_gstin_verified_status == 'active':
                    diff = (fields.Date.today() - partner.l10n_in_gstin_verified_date).days
                    if diff >= 0 and diff <= 30:
                        partner.l10n_in_gstin_status_color = 'green'
                    elif diff > 30 and diff <= 60:
                        partner.l10n_in_gstin_status_color = 'yellow'
            else:
                partner.l10n_in_gstin_status_color = False
    ...
    def get_l10n_in_gstin_verified_status(self):
        ...
        if response.get('error') and any(e.get('code') == 'no-credit' for e in response.get('error')):
        ...
        if gst_status.casefold() == 'active':
            ...
            l10n_in_gstin_verified_status = 'inactive'
            self.message_post(
                body=_(
