...
class CrmLead2OpportunityPartner(models.TransientModel):
        ...
        if user_id:
            if team_id:
                user_in_team = self.env['crm.team'].search(
                    [('id', '=', team_id), '|', ('user_id', '=', user_id), ('member_ids', '=', user_id)], count=True)
    ...
    def _create_partner(self, lead, action, partner_id):
        ...
        if self.action == 'each_exist_or_create':
            partner_id = self.with_context(active_id=lead.id)._find_matching_partner()
            action = 'create'
...
class CrmLead2OpportunityMassConvert(models.TransientModel):
    ...
    force_assignation = fields.Boolean(help='If unchecked, this will leave the salesman of duplicated opportunities')
