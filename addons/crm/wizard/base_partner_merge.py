...
def sanitize_email(email):
    ...
    emails = [parseaddr(mail)[1]
              for item in result
              for mail in item.split()]
...
class MergePartnerAutomatic(models.TransientModel):
    ...
    def _merge(self, partner_ids, dst_partner=None):
        ...
        if odoo.SUPERUSER_ID != self.env.uid and len(set(partner.email for partner in partners)) > 1:
        ...
        if odoo.SUPERUSER_ID != self.env.uid and self._model_is_installed('account.move.line') and \
    ...
    def auto_set_parent_id(self):
        assert is_integer_list(self)

        # select partner who have one least invoice
        partner_treated = ['@gmail.com']
        self.env.cr.execute("""  SELECT p.id, p.email
        ...
        for id, email in self.env.cr.fetchall():
            # check email domain
            ...
            self.env.cr.execute("""  SELECT id,email
                            FROM res_partner
                            WHERE parent_id != %s AND id != %s AND email LIKE %s
                    """, (id, id, '%' + email))
            _logger.info("%r", self.env.cr.fetchall())
