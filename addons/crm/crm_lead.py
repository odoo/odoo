...
Update your business card, phone book, social media,... Send an email right now and see it here.""") % {
    ...
    def retrieve_sales_dashboard(self, cr, uid, context=None):
        ...
        opportunies_domain = [('type', '=', 'opportunity'), ('user_id', '=', uid)]

        opportunities = self.search_read(cr, uid, opportunies_domain, context=context)
        ...
        for opp in opportunities:
            ...
            if opp['date_deadline']:
                ...
                if date_deadline == date.today():
                    res['closing']['today'] = res['closing']['today'] + 1
                if date_deadline >= date.today() and date_deadline <= date.today() + timedelta(days=7):
                    res['closing']['next_7_days'] = res['closing']['next_7_days'] + 1
                if date_deadline < date.today():
                    res['closing']['overdue'] = res['closing']['overdue'] + 1

            # Next activities
            # TODO: do we count only 1 activity by opportunity or it can be more ?
            if opp['next_activity_id'] and opp['date_action']:
                ...
                if date_action == date.today():
                    res['activity']['today'] = res['activity']['today'] + 1
                if date_action >= date.today() and date_action <= date.today() + timedelta(days=7):
                    res['activity']['next_7_days'] = res['activity']['next_7_days'] + 1
                if date_action < date.today():
                    res['activity']['overdue'] = res['activity']['overdue'] + 1
            ...
            if opp['date_closed']:
                ...
                if date_closed <= date.today() and date_closed >= date.today().replace(day=1):
                    if opp['planned_revenue']:
                        res['won']['this_month'] = res['won']['this_month'] + opp['planned_revenue']
                elif date_closed < date.today().replace(day=1) and date_closed >= date.today().replace(day=1) - relativedelta(months=+1):
                    if opp['planned_revenue']:
                        res['won']['last_month'] = res['won']['last_month'] + opp['planned_revenue']
        ...
        for act in activites_done:
            if act['date']:
                ...
                if date_act <= date.today() and date_act >= date.today().replace(day=1):
                        res['done']['this_month'] = res['done']['this_month'] + 1
                elif date_act < date.today().replace(day=1) and date_act >= date.today().replace(day=1) - relativedelta(months=+1):
                    res['done']['last_month'] = res['done']['last_month'] + 1

        # Meetings
        meetings_domain = [
            ('start', '>=', date.today()),
            ('start', '<=', date.today() + timedelta(days=8))  # as start is a datetime, use 7 + 1 to get all events
        ]
        # We need to add 'mymeetings' in the context for the search to be correct.
        meetings = self.pool.get('calendar.event').search_read(cr, uid, meetings_domain, context=context.update({'mymeetings': 1}) if context else {'mymeetings': 1})
        for meeting in meetings:
            if meeting['start']:
                start = datetime.strptime(meeting['start'], tools.DEFAULT_SERVER_DATETIME_FORMAT).date()

                if start == date.today():
                    res['meeting']['today'] = res['meeting']['today'] + 1
                if start >= date.today() and start <= date.today() + timedelta(days=7):
                    res['meeting']['next_7_days'] = res['meeting']['next_7_days'] + 1

        res['nb_opportunities'] = len(opportunities)

        res['done']['target'] = self.pool('res.users').browse(cr, uid, uid, context=context).target_sales_done
        res['won']['target'] = self.pool('res.users').browse(cr, uid, uid, context=context).target_sales_won
