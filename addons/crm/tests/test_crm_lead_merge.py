...
class TestCrmLeadMerge(TestCrmCases):
    ...
    def test_crm_lead_merge(self):
        ...
        partner3_id = self.env.ref("base.res_partner_3").id
        SalesmanLead = self.env['crm.lead'].sudo(self.crm_salemanager.id)
        ...
        merge = self.env['crm.lead'].search([('name', '=', 'Test opportunity 1'), ('partner_id', '=', partner3_id)], limit=1)
        ...
        merge_lead = self.env['crm.lead'].search([('name', '=', 'Test lead 3'), ('partner_id', '=', self.env.ref("base.res_partner_1").id)], limit=1)
        self.assertTrue(merge_lead, 'Fail to create merge opportunity wizard')
        self.assertEqual(merge_lead.partner_id.id, self.env.ref("base.res_partner_1").id, 'Partner mismatch')
        ...
        merge_opp = self.env['crm.lead'].search([('name', '=', 'Test opportunity 2'), ('partner_id', '=', partner3_id)], limit=1)
