...
class TestLead2opportunityAssignSalesmen(TestCrmCases):
    ...
    def test_lead2opportunity_assign_salesmen(self):
        ...
        SalesmanLead = self.env['crm.lead'].sudo(self.crm_salesman.id)
        ...
        mass = CrmLead2OpportunityPartnerMass.sudo(self.crm_salesman.id).with_context(context).create({
        ...
        opps = self.env['crm.lead'].browse(lead_ids)
        i = 0
        for opp in opps:
