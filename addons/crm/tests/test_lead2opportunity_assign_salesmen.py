...
class TestLead2opportunityAssignSalesmen(TestCrmAccessGroupUsers):
    ...
    def test_lead2opportunity_assign_salesmen(self):
        ...
        test_crm_lead_01 = self.CrmLead.sudo(self.crm_salesman_id).create({
            'type': 'lead',
            'name': 'Test lead 1',
            'email_from': 'Raoul Grosbedon <raoul@grosbedon.fr>',
            'stage_id': self.ref('crm.stage_lead1')
        })
        test_crm_lead_02 = self.CrmLead.sudo(self.crm_salesman_id).create({
            'type':'lead',
            'name': 'Test lead 2',
            'email_from': 'Raoul Grosbedon <raoul@grosbedon.fr>',
            'stage_id': self.ref('crm.stage_lead1')
        })
        test_crm_lead_03 = self.CrmLead.sudo(self.crm_salesman_id).create({
            'type': 'lead',
            'name': 'Test lead 3',
            'email_from': 'Raoul Grosbedon <raoul@grosbedon.fr>',
            'stage_id': self.ref('crm.stage_lead1')
        })
        test_crm_lead_04 = self.CrmLead.sudo(self.crm_salesman_id).create({
            'type': 'lead',
            'name': 'Test lead 4',
            'email_from': 'Fabrice Lepoilu',
            'stage_id': self.ref('crm.stage_lead1')
        })
        test_crm_lead_05 = self.CrmLead.sudo(self.crm_salesman_id).create({
            'type': 'lead',
            'name': 'Test lead 5',
            'email_from': 'Fabrice Lepoilu',
            'stage_id': self.ref('crm.stage_lead1')
        })
        test_crm_lead_06 = self.CrmLead.sudo(self.crm_salesman_id).create({
            'type': 'lead',
            'name': 'Test lead 6',
