...
class TestCrmMailActivity(TestCrmCommon):
    ...
    def test_crm_activity_recipients(self):
        ...
        self.lead_1.message_subscribe(partner_ids={self.lead_1.id: [self.contact_1.id]})
        ...
        self.lead_1.message_subscribe(partner_ids={self.lead_1.id: [self.user_sales_manager.partner_id.id]}, subtype_ids=[self.env.ref('mail.mt_activities').id, self.env.ref('mail.mt_comment').id])
