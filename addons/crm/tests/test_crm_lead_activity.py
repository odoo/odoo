...
class TestCrmLeadActivity(TestCrmCases):
    ...
    def test_crm_lead_activity(self):
        ...
        activity_email_id = self.ref('crm.crm_activity_data_email')
        activity_call_id = self.ref('crm.crm_activity_data_call')

        # Salemanager create crm activity
        demo_activity = self.env['crm.activity'].sudo(self.crm_salemanager_id).create({
            'name': 'Demostration',
