...
class Lead(models.Model):
    ...
    def _pls_prepare_update_frequency_table(self, rebuild=False, target_state=False):
        ...
        team_ids = self.env['crm.team'].search([]).ids + [0]  # If team_id is unset, consider it as team 0
    ...
    def _pls_prepare_frequencies(self, lead_values, leads_pls_fields, target_state=None):
        ...
        frequencies = dict((field, {}) for field in set(leads_pls_fields + ['tag_id']))
        ...
        for values in lead_values:
            ...
            if 'tag_id' in leads_pls_fields:  # tag_id already treated here above.
                leads_pls_fields.remove('tag_id')
            for field in leads_pls_fields:
