...
from odoo.tools import command_re, email_re, email_split
...
class CrmLead(FormatAddress, models.Model):
    ...
    type = fields.Selection(
            ...
            index=True, required=True, default=lambda self: 'lead' if self.env['res.users'].has_group('crm.group_use_lead') else 'opportunity',
    ...
    date_closed = fields.Datetime(string='Closed Date', readonly=True, copy=False)
    ...
    def _compute_day_open(self):
        ...
        for lead in self.filtered(lambda l: l.date_open):
            ...
            lead.day_open = abs(int((date_open - date_create).days))
    ...
    def _compute_day_close(self):
        ...
        for lead in self.filtered(lambda l: l.date_closed):
            ...
            lead.day_close = abs(int((date_close - date_create).days))

    @api.multi
    def _compute_meeting_count(self):
        for lead in self:
            lead.meeting_count = self.env['calendar.event'].search_count([('opportunity_id', '=', lead.id)])
    ...
    def _onchange_stage_id_wrapper(self):
        values = self.onchange_stage_id(self.stage_id.id)['value']
    ...
    def on_change_partner_id(self, partner_id):
        ...
        if partner_id:
            ...
            partner_name = partner.parent_id.name or (partner.is_company and partner.name)
            values = {
                'partner_name': partner_name,
                'contact_name': (not partner.is_company and partner.name),
    ...
    def _onchange_partner_id(self):
        values = self.on_change_partner_id(self.partner_id.id)['value']
        for fname, value in values.iteritems():
            setattr(self, fname, value)
    ...
    def on_change_user(self, user_id):
        res = {'value': {}}
        if user_id and self.env.context.get('team_id'):
            team = self.env['crm.team'].browse(self.env.context['team_id'])
            if user_id in team.member_ids.ids:
                return res
        res['value']['team_id'] = self.env['crm.team']._get_default_team_id(user_id=user_id)
        return res

    @api.onchange('user_id')
    def _onchange_user_id_wrapper(self):
        """ When changing the user, also set a team_id or restrict team id
            to the ones user_id is member of. """
        values = self.on_change_user(self.user_id.id)['value']
        for fname, value in values.iteritems():
            setattr(self, fname, value)
    ...
    def _onchange_next_activity_id(self):
        ...
        if activity:
            date_action = False
            if activity.days:
                date_action = fields.Datetime.to_string(datetime.now() + timedelta(days=activity.days))
    ...
    def write(self, vals):
        ...
        if vals.get('stage_id') and 'probability' not in vals:
            vals.update(self.onchange_stage_id(vals.get('stage_id'))['value'])
    ...
    def stage_find(self, cr, uid, cases, team_id, domain=None, order='sequence', context=None):
        if isinstance(cases, (int, long)):
            new_cases = self.browse(cr, uid, cases, context=context)
        else:
            new_cases = self.browse(cr, uid, None, context=context)
            for case in cases:
                new_cases += case
        return CrmLead.stage_find(new_cases, team_id, domain=domain, order=order)
    ...
    def _merge_get_result_type(self, cr, uid, opps, context=None):
        ...
        """
        opportunity = self.browse()
        for opp in opps:
            opportunity += opp
        return CrmLead._merge_get_result_type(opportunity)
    ...
    def _merge_get_result_type(self):
        ...
        """
        if self.filtered(lambda o: o.type == 'opportunity'):
            return 'opportunity'
    ...
    def _mail_body(self, cr, uid, lead, fields, title=False, context=None):
        # `lead` could be one `browse_record`
        return CrmLead._mail_body(lead, fields, title=title)
    ...
    def merge_opportunity(self, user_id=False, team_id=False):
        ...
        if merged_data.get('team_id'):
            stage = self.env['crm.stage'].search(['|', ('team_id', '=', merged_data['team_id']), ('team_id', '=', False)], order='sequence', limit=1)
            if merged_data.get('stage_id') not in stage.ids:
                merged_data['stage_id'] = stage.id
    ...
    def _convert_opportunity_data(self, customer, team_id=False):
        ...
        if not self.stage_id:
            ...
            if stage_id:
                val['probability'] = self.env['crm.stage'].browse(stage_id).probability
        return val
    ...
    def convert_opportunity(self, partner_id, user_ids=False, team_id=False):
        ...
        for lead in self.filtered(lambda l: l.active or l.probability < 100):
            vals = lead._convert_opportunity_data(partner, team_id)
            lead.write(vals)
    ...
    def _lead_create_contact(self, name, is_company, parent_id=False):
        ...
        vals = {'name': name,
            ...
            'phone': self.phone,
            'mobile': self.mobile,
            'email': email_split(self.email_from) and email_split(self.email_from)[0] or False,
            'fax': self.fax,
            'title': self.title.id,
    ...
    def _create_lead_partner(self):
        ...
        contact_id = self._lead_create_contact(contact_name, False, partner_company_id) if contact_name else False
    ...
    def handle_partner_assignation(self, action='create', partner_id=False):
        ...
        for lead in self:
            ...
            if lead.partner_id:
                partners_dict[lead.id] = lead.partner_id.id
            if action == 'create':
                partner_id = lead._create_lead_partner()
                self.env['res.partner'].browse(partner_id).write({'team_id': lead.team_id.id})
            if partner_id:
    ...
    def redirect_opportunity_view(self):
        ...
        return {
            ...
            'views': [(form_view and form_view.id or False, 'form'),
                      (tree_view and tree_view.id or False, 'tree'), (False, 'kanban'),
...
Update your business card, phone book, social media,... Send an email right now and see it here.""") % {
                    'link': "<a href='mailto:%(email)s'>%(email)s</a>" % {'email': '%s@%s' % (alias.alias_name, alias.alias_domain)}
                }
    ...
    def _notification_group_recipients(self, message, recipients, done_ids, group_data):
        ...
        group_sale_salesman = self.env.ref('base.group_sale_salesman', False)
        for recipient in recipients.filtered(lambda r: r.id not in done_ids):
            if recipient.user_ids and group_sale_salesman and group_sale_salesman in recipient.user_ids.mapped('groups_id'):
    ...
    # TODO refactor: This seems wierd code as `ids` holds `crm.team` models records' `ids` and
    # it is browsed with `crm.lead` object in recursive call!
    @api.model
    def message_get_reply_to(self, ids, default=None):
        ...
        # Suspicious recusive call with other object's ids!!
    ...
    def message_new(self, msg, custom_values=None):
        ...
        if author_id:
            defaults.update(self.on_change_partner_id(msg.get('author_id'))['value'])
        if priority in dict(crm_stage.AVAILABLE_PRIORITIES):
            defaults['priority'] = priority
        defaults.update(custom_values)
        return super(CrmLead, self.with_context(create_context)).message_new(msg, custom_values=defaults)
