...
class CrmTeam(models.Model):
    ...
    def _allocate_leads(self, creation_delta_days=7):
        ...
        def _log_team_data(teams_data, global_data):
        ...
        while population:
            ...
            try:
                ...
                _log_team_data(teams_data, global_data)
                _logger.error("[Assign Lead] Crash while assigning lead %s (ID: %d) to team %s (ID: %d). Error: %s", candidate_lead.name, candidate_lead.id, team.name, team.id, str(e))
        ...
        for team, leads_to_assign_ids in leads_per_team.items():
            ...
            for lead in preferred_leads.sorted(lambda lead: (-lead.probability, id)):
                ...
                try:
                    ...
                    _logger.error("[Assign Lead] Crash while assigning preferred lead %s (ID: %d) to a member of team %s (ID: %d). Error: %s", lead.name, lead.id, team.name, team.id, str(e))
                    _logger.error("[Assign Lead] %d leads (IDs: %s) assigned to member %s of team %s (ID: %d) before crash.", len(result_data[member_found]['assigned']), list(result_data[member_found]['assigned'].ids), member_found.id, team.name, team.id)
                    raise
            ...
            for lead in to_assign.sorted(lambda lead: (-lead.probability, id)):
                ...
                try:
                    ...
                    _log_member_data(result_data)
                    _logger.error("[Assign Lead] Crash while assigning lead %s (ID: %d) to a member of team %s (ID: %d). Error: %s", lead.name, lead.id, team.name, team.id, str(e))
                    _logger.error("[Assign Lead] %d leads (IDs: %s) assigned to member %s of team %s (ID: %d) before crash.", len(result_data[member_found]['assigned']), list(result_data[member_found]['assigned'].ids), member_found.id, team.name, team.id)
                    raise
