...
class CustomerWizard(models.TransientModel):
    _name = 'mail_docsaway.customer_wizard'
    _rec_name = 'remaining'
    ...
    wiz_ids = fields.One2many('mail_docsaway.single_wizard', 'wizard_customer_id', string='Mails')
    ...
    def _on_change_ink(self):
        ...
        wiz_id = self.id
        my_wiz_ids = self.env['mail_docsaway.single_wizard'].search([('wizard_customer_id','=',wiz_id)])
        if len(my_wiz_ids) == 0:
            active_ids = self._context.get('active_ids', [])
            self.wiz_ids = self.env['mail_docsaway.api']._prepare_document_deliveries(active_ids, wiz_id, ink=self.ink)
        else:
    ...
    def _on_change_attachment_ids(self):
        ...
        mail_ids = self.env['mail_docsaway.api'].browse(active_ids)
