...
class res_partner(osv.osv):
    ...
    def unlink(self, cursor, user, ids, context=None):
        ...
        if parnter_id:
            raise UserError(_('Invalid Action!'), _('You cannot delete a partner which is assigned to project, but you can uncheck the active box.'))
...
class account_analytic_line(osv.osv):
   ...
   def on_change_account_id(self, cr, uid, ids, account_id):
       ...
       if acc.state == 'close' or acc.state == 'cancelled':
           raise UserError(_('Invalid Analytic Account!'), _('You cannot select a Analytic Account which is in Close or Cancelled state.'))
       return res
