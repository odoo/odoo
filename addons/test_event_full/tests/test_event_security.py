...
class TestEventSecurity(TestEventFullCommon):
    ...
    def test_event_access_event_question_and_event_answer(self):
        """ Check that some user groups have access to questions and answers only if they are linked to at
        least one published event. """
        user_internal_event = mail_new_test_user(
            self.env,
            name='User Internal Event',
            login='user_internal_event',
            email='user_internal_event@example.com',
        ...
        restricted_users = [user_internal_event, self.user_portal, self.user_public]
