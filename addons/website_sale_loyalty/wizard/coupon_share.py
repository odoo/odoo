...
class CouponShare(models.TransientModel):
    ...
    def _check_website(self):
        if self.filtered(
            lambda record: (
                record.program_website_id and record.program_website_id != record.website_id
            )
        ):
            raise ValidationError(
                self.env._("The shared website should correspond to the website of the program.")
            )
