...
class ProductProduct(models.Model):
    ...
    def _can_return_content(self, field_name=None, access_token=None):
        ...
        :rtype: bool
        """
        if field_name in ["image_%s" % size for size in [1920, 1024, 512, 256, 128]] and self.env[
            'loyalty.reward'
        ].sudo().search_count([('discount_line_product_id', '=', self.id)], limit=1):
            return True
    ...
    def _get_product_placeholder_filename(self):
        ...
        if (
            ...
            .sudo()
            .search_count([('discount_line_product_id', '=', self.id)], limit=1)
        ):
            if (
                self.env['loyalty.reward']
                .sudo()
