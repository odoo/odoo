...
class AccountMove(models.Model):
    ...
    @api.model
    def _l10n_ge_edi_refresh_all_statuses(self, companies=None):
        domain = [('move_type', 'in', ['out_invoice', 'out_refund']), ('l10n_ge_edi_state', 'in', RSGE_AWAITING_COUNTERPARTY_STATES)]
        if companies:
            domain.append(('company_id', 'in', companies.ids))
        for company, invoices in self.search(domain).grouped('company_id').items():
