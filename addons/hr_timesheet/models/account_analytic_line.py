...
class AccountAnalyticLine(models.Model):
    ...
    parent_task_id = fields.Many2one('project.task', related='task_id.parent_id', store=True, index='btree_not_null', init_column=lambda model: None)
    project_id = fields.Many2one(
        'project.project', 'Project', domain=_domain_project_id, index=True,
        compute='_compute_project_id', store=True, readonly=False, init_column=lambda model: None)
    ...
    department_id = fields.Many2one('hr.department', "Department", compute='_compute_department_id', store=True, compute_sudo=True, init_column=lambda model: None)
    manager_id = fields.Many2one('hr.employee', "Manager", related='employee_id.parent_id', store=True, init_column=lambda model: None)
