...
class AccountAnalyticLine(models.Model):
    ...
    def _apply_timesheet_label(self, view_arch):
        ...
        encoding_uom = self.env['res.company']._get_current_company().timesheet_encode_uom_id
        ...
        if any([field_name in values for field_name in ['unit_amount', 'employee_id', 'account_id']]):
            for timesheet in sudo_self:
                ...
                amount_converted = timesheet.employee_id.currency_id._convert(
                    amount, timesheet.account_id.currency_id, self.env['res.company']._get_current_company(), timesheet.date)
