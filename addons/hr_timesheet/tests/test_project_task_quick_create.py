...
class TestProjectTaskQuickCreate(TestCommonTimesheet):
    ...
    def test_create_task_with_valid_expressions(self):
        ...
        for expression, values in valid_expressions.items():
            task_form = Form(self.env['project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_customer.id, 'default_allocated_hours_manually_set': True}), view="project.quick_create_task_form")
