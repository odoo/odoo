    ...
    def test_multi_create_timesheets_from_calendar(self):
        ...
        with patch.object(self.env.registry['bus.bus'], '_sendone') as mock_send:
            ...
            mock_send.assert_called_with(self.env.user.partner_id, 'simple_notification', {
                'type': 'danger',
                'message': 'Some of the timesheet entries were not created as no valid work intervals were available'
            })
            ...
            mock_send.assert_called_with(self.env.user.partner_id, 'simple_notification', {
                'type': 'danger',
                'message': 'No timesheet entries were not created as no valid work intervals were available'
            })
            mock_send.reset_mock()
