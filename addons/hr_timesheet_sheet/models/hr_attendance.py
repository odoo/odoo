...
class HrAttendance(models.Model):
    ...
    def _get_current_sheet(self, employee_id, date=False):
        ...
        sheet_ids = sheet_obj.search([('date_from', '<=', att_tz_date_str),
                                      ('date_to', '>=', att_tz_date_str),
                                      ('employee_id', '=', employee_id)], limit=1)
        return sheet_ids or False

    # JAA FIX ME 'day' put in depends
    @api.depends('sheet_id.employee_id', 'sheet_id.date_from', 'sheet_id.date_to', 'employee_id', 'name')
    ...
    def create(self, vals):
        ...
        if sheet_id:
            ...
            ts = self.env['hr_timesheet_sheet.sheet'].browse(sheet_id.ids)
