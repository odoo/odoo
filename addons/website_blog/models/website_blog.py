...
class BlogPost(models.Model):
    ...
    def _check_for_action_post_publish(self):
        """ Override
        Send a notification only if the blog post has never been published.
        If the published_date is already set, the notification has already
        been sent.
        """
        self.ensure_one()
        if self.website_published and self.active and not self.published_date:
            self.blog_id.message_post_with_source(
    ...
    def write(self, vals):
        ...
        for post in self:
            result &= super(BlogPost, post).write(vals)
        return result
