...
publicWidget.registry.subscribe = publicWidget.Widget.extend({
    ...
    _getListId: function () {
        return this.el.closest(".s_newsletter_block")?.dataset.listId || this.el.dataset.listId;
    },
    ...
    _onSubscribeClick: async function () {
        var self = this;
        const inputName = this.el.querySelector("input").name;
        const subscribeValueElements = Array.from(
            this.el.querySelectorAll(".js_subscribe_value")
        ).filter((elem) => !!elem.getClientRects().length);
        const subscribeEmailElements = Array.from(
            this.el.querySelectorAll(".js_subscribe_email")
        ).filter((elem) => !!elem.getClientRects().length); // js_subscribe_email is kept by compatibility (it was the old name of js_subscribe_value)
        const subscribeElements = [
            ...new Set([...subscribeValueElements, ...subscribeEmailElements]),
        ];
        const input = subscribeElements.length > 0 ? subscribeElements[0] : null;
        ...
        rpc("/website_mass_mailing/subscribe", {
            ...
            if (toastType === 'success') {
                ...
                if (popup) {
                    const modal = Modal.getOrCreateInstance(popup);
                    modal.hide();
                }
            }
...
publicWidget.registry.fixNewsletterListClass = publicWidget.Widget.extend({
     ...
     */
    start() {
        this.target.classList.add("s_newsletter_list");
        return this._super(...arguments);
    },
});
