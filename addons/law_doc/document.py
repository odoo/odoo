...
class legal_file(osv.osv):
    _inherit = 'law.matter'

    _columns = {
    'attach_ids': fields.one2many('ir.attachment', 'matter_id', 'File Attachments', ondelete='cascade', readonly=True, states={'open':[('readonly',False)]}),
    'matter_dir': fields.many2one('document.directory', 'Directory', select=True, ondelete='restrict', readonly=True, states={'open':[('readonly',False)]}),

    }
    def write(self, cr, uid, ids, vals, context=None):
        res = super(legal_file, self).write(cr, uid, ids, vals, context=context)
        #_logger.error("my variable,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,, : %r", context)
        self.write_dir(cr, uid, ids, context=context)
        return res
    def write_dir(self, cr, uid, ids, context=None):
        matter = self.browse(cr, uid, ids, context=context) 
        values ={
            'name': matter.matter_id + '-' + matter.name,
            'parent_id': matter.client_id.client_dir.id,
        }
        return self.pool['document.directory'].write(cr, uid, matter.matter_dir.id, values, context=context)
    def create(self, cr, uid, vals, context=None):
        res = super(legal_file, self).create(cr, uid, vals, context=context)
        vals['matter_dir'] = self.create_dir(cr, uid, res, context=context)
        super(legal_file, self).write(cr, uid, res, vals, context=context)
        return res
    def create_dir(self, cr, uid, ids, context=None):
        matter = self.browse(cr, uid, ids, context=context)
        values ={
            'name': matter.matter_id + '-' + matter.name,
            'user_id': uid,
        ...
        return self.pool['document.directory'].create(cr, uid, values, context=context)


class legal_client(osv.osv):
    _inherit = 'res.partner'

    _columns = {
    'client_dir': fields.many2one('document.directory', 'Directory', select=True, ondelete='cascade'),

    }
    def write(self, cr, uid, ids, vals, context=None):
        res = super(legal_client, self).write(cr, uid, ids, vals, context=context)
        #_logger.error("my variable,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,, : %r", context)
        self.write_dir(cr, uid, ids, context=context)
        return res
    def write_dir(self, cr, uid, ids, context=None):
        client = self.browse(cr, uid, ids, context=context)
        values ={
            'name': client.name,
        }
        #_logger.error("my variable,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,, : %r", client.client_dir.id)
        return self.pool['document.directory'].write(cr, uid, client.client_dir.id, values, context=context)
    def create(self, cr, uid, vals, context=None):
        res = super(legal_client, self).create(cr, uid, vals, context=context)
	vals['client_dir'] = self.create_dir(cr, uid, res, context=context)
	super(legal_client, self).write(cr, uid, res, vals, context=context)
        return res
    def create_dir(self, cr, uid, ids, context=None):
        if context is None:
           context = {}
...
class document_file(osv.osv):
	...
	'create_uid':  fields.many2one('res.users', 'Created By', readonly=True),
        # Fields of document:
        'user_id': fields.many2one('res.users', 'File Owner', select=1),
        'parent_id': fields.many2one('document.directory', 'Directory', select=1, change_default=True, domain="[('user_id', '=', uid)]"),
        'index_content': fields.text('Indexed Content'),
        'client_id':fields.many2one('res.partner', 'Client Name', select=1, domain="[('customer', '=', True)]"),
        'file_type': fields.char('Content Type'),
	'matter_id': fields.many2one('law.matter', 'Matter', select=True, domain="[('client_id', '=', client_id)]"),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('res_id', False) and vals.get('res_model', False) and not vals.get('client_id', False):
            vals['client_id'] = self.__get_client_id(cr, uid, vals['res_model'], vals['res_id'], context)
    ...
    def __get_client_id(self, cr, uid, res_model, res_id, context=None):
        ...
        if obj_model._name == 'res.partner':
            return res_id
        elif 'client_id' in obj_model._columns and obj_model._columns['client_id']._obj == 'res.partner':
            bro = obj_model.browse(cr, uid, res_id, context=context)
            return bro.client_id.id
        return False
