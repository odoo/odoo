...
patch(PosStore.prototype, {
    ...
    async onProductInfoClick(productTemplate, productProduct = false) {
        ...
        const eventProductTemplate = productTemplate.event_id
            ? productTemplate
            : this.models["product.template"].get(`dummy_${event?.id}`);

        return this.dialog.add(EventInfoPopup, {
            productTemplate: eventProductTemplate || productTemplate,
            event: event,
        });
    },
});
