...
class SurveyQuestion(models.Model):
    ...
    is_lead_generating = fields.Boolean(
        'Lead generating', compute='_compute_is_lead_generating',
        help='At least one of the question answers can generate leads.')

    @api.depends('question_type', 'suggested_answer_ids', 'suggested_answer_ids.is_create_lead')
    def _compute_is_lead_generating(self):
        for question in self:
            if question.question_type in ['simple_choice', 'multiple_choice', 'matrix']:
                question.is_lead_generating = any(answer.is_create_lead for answer in question.suggested_answer_ids)
            else:
