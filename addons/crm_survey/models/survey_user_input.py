...
class SurveyUser_Input(models.Model):
    ...
    def _prepare_user_input_lead_values(self):
        ...
        lead_values = {
            ...
            'name': _('%s live session results', lead_name_prefix) if self.is_session_answer else _('%s survey results', lead_name_prefix),
    ...
    def _prepare_lead_values_from_user_input_lines(self):
        ...
        for question_id, input_lines in answers_grouped_by_question.items():
            ...
            # In summary, the else occurs when no response and no comment are left.
            if not all(line.skipped for line in input_lines):
                for input_line_index, input_line in enumerate(input_lines):
                    # Useful for comment field without suggestion chosen.
                    # e.g. for a matrix question with no choice selected but with a comment left,
                    # this check allows to skip the first input line indicating that no answer was selected.
                    # But the second input line containing the comment left will be retrieved afterwards.
                        ...
                        if question_id.question_type == 'text_box':
                            ...
                            if question_id.question_type in ['simple_choice', 'multiple_choice'] and input_line.answer_type == 'char_box':  # Comment case
                                ...
                                if input_line.suggested_answer_id and not input_line._get_answer_value():
                                    answer = escape(input_line.suggested_answer_id.value_image_filename or '')
            ...
            html_input_lines.append(Markup('<li>%s%s</li>' % (escape(question_id.title), answer_description)))
