    ...
    def _compute_partner_ids(self):
        ...
        for lot in self:
            if delivery_ids_by_lot.get(lot.id, []):
                ...
                lot.partner_ids = picking_ids.mapped(lambda p:
                    p.sale_id.partner_shipping_id or p.purchase_id.dest_address_id or p.partner_id if p.is_dropship else p.partner_id
                )
            else:
