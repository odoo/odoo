...
class TestPackingDelivery(TestPackingCommon):
    ...
    def test_multi_level_package_weight(self):
        ...
        self.assertEqual(res_boxA.with_context(picking_ids=[delivery.id]).weight, 5)      # 1 + 2 * 2 = 5kg
        self.assertEqual(res_boxB.with_context(picking_ids=[delivery.id]).weight, 11)     # 1 + 2 * 5 = 11kg
        self.assertEqual(res_big_box.with_context(picking_ids=[delivery.id]).weight, 20)  # 4 + 5 + 11 = 20kg
        self.assertEqual(res_pallet.with_context(picking_ids=[delivery.id]).weight, 30)   # 10 + 20 = 30kg
