...
class StockPicking(models.Model):
    ...
    def send_to_shipper(self):
        for batch, pickings in self.grouped("batch_id").items():
            ...
            for picks, shipment in results.items():
                for picking in picks:
                    picking.carrier_price = picking.carrier_id._apply_margins(shipment.get('exact_price', 0.0), picking.sale_id)
                    self._post_shipping_information(picking, shipment)
                    self._update_tracking_number(picking, shipment)
                    picking._add_delivery_cost_to_so()
                if picks and batch:
                    self._post_shipping_information(picks[0], shipment, batch)

    def _update_tracking_number(self, picking, shipping):
        if shipping.get('tracking_number', False):
            related_pickings = picking.env['stock.picking'] if picking.carrier_tracking_ref and shipping['tracking_number'] in picking.carrier_tracking_ref else picking
            ...
            without_tracking.carrier_tracking_ref = shipping['tracking_number']
            for p in related_pickings - without_tracking:
                p.carrier_tracking_ref += "," + shipping['tracking_number']

    def _post_shipping_information(self, picking, res, output_target=None):
        order_currency = picking.sale_id.currency_id or picking.company_id.currency_id
        carrier_name = picking.carrier_id.name
        target = output_target or picking

        if len(res.get("delivery_docs", [])):
            doc_msg = Markup('<b>%(header)s</b>') % {'header': _("Shipping Documents Attached")}
            target.message_post(body=doc_msg, attachment_ids=res["delivery_docs"].ids)

        if len(res.get("return_labels", [])):
            ret_msg = Markup('<b>%(header)s</b>') % {'header': _("Return Labels Generated")}
            target.message_post(body=ret_msg, attachment_ids=res["return_labels"].ids)

        if len(res.get("delivery_labels", [])):
            del_msg = Markup("<b>%(header)s</b>") % {'header': _("Delivery Labels Generated")}
            target.message_post(body=del_msg, attachment_ids=res["delivery_labels"].ids)
