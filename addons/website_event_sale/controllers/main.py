...
class WebsiteEventSaleController(WebsiteEventController):
    ...
    def _create_attendees_from_registration_post(self, event, registration_data):
        ...
        for info in [r for r in registration_data if r.get('event_ticket_id')]:
            ...
            cart_values = order.with_context(event_ticket_id=ticket.id, fixed_price=True, registration_in_creation=True)._cart_update(product_id=ticket.product_id.id, add_qty=1)
