...
class TestWebsiteEventSale(HttpCaseWithUserPortal, TestWebsiteEventSaleCommon):
    ...
    def test_website_event_sale_mandatory_signup_single_partner(self):
        ...
        existing_matching_partners = self.env['res.partner'].search([('email', '=', attendee_email)])
        ...
        new_matching_partners = self.env['res.partner'].search([('email', '=', attendee_email)])
        self.assertFalse(
        ...
        self.assertEqual(
            last_so.partner_id, public_partner,
