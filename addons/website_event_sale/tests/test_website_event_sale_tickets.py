...
class TestUi(HttpCaseWithUserDemo, TestWebsiteEventSaleCommon):
    ...
    def test_check_conversion_website_sale_event(self):
        # Ensure the conversion rate of different currencies is not 1 to 1
        res_currency_rate = self.env['res.currency.rate'].search([], limit=1)
        res_currency_rate.write({
            'company_rate': 6,
        ...
        eur_cur.rate_ids = res_currency_rate
        ...
        c2 = self.env.company
        # Make product of type event from company FR
