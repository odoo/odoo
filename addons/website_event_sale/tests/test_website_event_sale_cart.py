...
class TestWebsiteEventSaleCart(TestWebsiteEventSaleCommon, TestWebsiteSaleCartAbandonedCommon):
    def test_event_cart(self):
        cart1, cart2 = self.so1before, self.so2before
        carts = cart1 + cart2
        ...
        self.assertTrue(
            ...
            "Cart recovery email should be sent for availlable tickets",
        )
        ...
        self.assertFalse(
            ...
            "Cart recovery email should not be sent when ticket has no seats availlable",
        )
        ...
        self.assertTrue(
            self.send_mail_patched(cart2.id),
            "Cart recovery email can be sent after increasing seat count",
        )
