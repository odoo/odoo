...
class AccountMove(models.Model):
    ...
    def _compute_l10n_gr_edi_warnings(self):
        for move in self:
            ...
            if move._l10n_gr_edi_eligible_for_mydata():
                move.l10n_gr_edi_warnings = move._origin._l10n_gr_edi_get_pre_error_dict()
            else:
    ...
    def _l10n_gr_edi_get_extra_invoice_report_values(self):
        ...
        if document.state in ('invoice_sent', 'bill_sent'):
            mydata_url = url_quote_plus(document.mydata_url)
            return {
                'barcode_src': f'/report/barcode/?barcode_type=QR&value={mydata_url}&width=180&height=180',
    ...
    def _l10n_gr_edi_add_address_vals(self, values):
        ...
        values.update({
            ...
            **{conditional_address_key: None for conditional_address_key in conditional_address_keys},
        })

        if issuer_not_from_greece:
            values.update({
                'issuer_name': self.company_id.name.encode('utf-8'),
                'issuer_postal_code': self.company_id.zip,
                'issuer_city': (self.company_id.city or "").encode('utf-8') or None,
            })
        ...
        if inv_type_allows_counterpart:
            ...
            if partner_not_from_greece:
                values['counterpart_name'] = self.commercial_partner_id.name.encode('utf-8')
        ...
        if inv_type_require_counterpart or (inv_type_allows_counterpart and partner_not_from_greece):
            values.update({
                ...
                'counterpart_city': (self.commercial_partner_id.city or "").encode('utf-8') or None,
            })
    ...
    def _l10n_gr_edi_send_invoices(self):
        ...
        for company, invoices in self.grouped('company_id').items():
            ...
            result = self.env['l10n_gr_edi.document']._mydata_send_invoices(company, xml_content)
    ...
    def _l10n_gr_edi_send_expense_classification(self):
        ...
        for company, bills in self.grouped('company_id').items():
            ...
            result = self.env['l10n_gr_edi.document']._mydata_send_expense_classification(company, xml_content)
            self._l10n_gr_edi_handle_send_result(result, xml_vals)
