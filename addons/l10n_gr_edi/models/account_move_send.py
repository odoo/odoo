...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        gr_warning_invoices = moves.filtered('l10n_gr_edi_warnings')

        if len(gr_warning_invoices) == 1:
            alerts = gr_warning_invoices.l10n_gr_edi_warnings
        elif len(gr_warning_invoices) > 1:
            alerts['l10n_gr_edi_warning_not_ready_invoice'] = {
                'message': _("The following invoice(s) are not ready to be sent to MyDATA: \n%s",
                             '\n'.join(f"- {move.display_name}" for move in gr_warning_invoices)),
                'action_text': _("View Invoice(s)"),
                'action': gr_warning_invoices._get_records_action(name=_("Check Invoice(s)")),
            }
