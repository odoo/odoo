...
class L10nGrEdiReconciliation(models.Model):
    ...
    @api.model
    def _l10n_gr_edi_prepare_reconciliation_values(self, company, invoice, event, move, currencies, sync_datetime):
        def find_value(element, xpath):
            return element.xpath(f'string({xpath})')

        headers = invoice.xpath('./*[local-name()="invoiceHeader"]')
        summaries = invoice.xpath('./*[local-name()="invoiceSummary"]')
