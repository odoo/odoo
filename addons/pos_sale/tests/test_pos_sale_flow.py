...
class TestPoSSale(TestPointOfSaleHttpCommon):
    ...
    def test_settle_quotation_delivered_qty(self):
        ...
        self.assertEqual(sum(pos_order.picking_ids.move_line_ids.filtered('picked').mapped('quantity')), 1)
        ...
        self.assertEqual(sum(refund_order.picking_ids.move_line_ids.filtered('picked').mapped('quantity')), 1)
    ...
    def test_settle_groupable_lot_total_amount(self):
        ...
        self.product = self.env['product.product'].create({
            ...
            'uom_po_id': groupable_uom_unit.id,
        })
        ...
        sale_order = self.env['sale.order'].create({
            ...
            'order_line': [(0, 0, {
                ...
                'name': self.product.name,
                'product_uom_qty': 1,
                'product_uom': groupable_uom_dozens.id,
                'price_unit': 12.0,
            })],
        })
