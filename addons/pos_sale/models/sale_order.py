    ...
    def _compute_amount_invoiced(self):
        ...
        for order in self:
            if order.invoice_status == 'invoiced':
                continue
            # We need to account for the downpayment paid in POS with and without invoice
            order_amount = sum(order.sudo().pos_order_line_ids.filtered(lambda pol: pol.order_id.state in ['paid', 'done'] and pol.sale_order_line_id.is_downpayment).mapped('price_subtotal_incl'))
