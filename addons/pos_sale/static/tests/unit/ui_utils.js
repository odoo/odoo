...
export async function selectQuotation(saleOrderName, { facet } = {}) {
    await openQuotationList();
    if (facet) {
        await removeFacet(facet);
    }
...
export async function settleSaleOrder(saleOrderName, { loadSN, facet } = {}) {
    await selectQuotation(saleOrderName, { facet });
