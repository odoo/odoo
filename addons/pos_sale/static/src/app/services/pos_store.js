...
patch(PosStore.prototype, {
    ...
    async downPaymentSO(saleOrder, isPercentage) {
        ...
        const groupingFunction = (base_line) => true;
        const downPaymentBaseLines = accountTaxHelpers.prepare_down_payment_lines(
            baseLines,
            this.company,
            amountType,
            amount,
            "down_payment", // TODO: won't work with multiple down payment on the same order... is it a problem?
            groupingFunction
        );
        ...
        for (const baseLine of downPaymentBaseLines) {
            ...
            this.addLineToCurrentOrder({
                pos: this,
                order: saleOrder,
                product_id: downPaymentProduct,
                product_tmpl_id: downPaymentProduct.product_tmpl_id,
