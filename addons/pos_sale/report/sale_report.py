...
class SaleReport(models.Model):
    ...
    def _select_pos(self):
        select_ = f"""
            ...
            CASE WHEN pos.state IN ('invoiced', 'done') THEN SUM(l.qty) ELSE 0 END AS qty_invoiced,
            CASE WHEN pos.state NOT IN ('invoiced', 'done') THEN SUM(l.qty) ELSE 0 END AS qty_to_invoice,
            AVG(l.price_unit)
            ...
            SUM(l.price_subtotal)
                ...
                * {self._case_value_or_one('account_currency_table.rate')}
            AS price_subtotal,
            (CASE WHEN pos.state NOT IN ('invoiced', 'done') THEN SUM(l.price_subtotal) ELSE 0 END)
                / MIN({self._case_value_or_one('pos.currency_rate')})
                * {self._case_value_or_one('account_currency_table.rate')}
            AS amount_to_invoice,
            (CASE WHEN pos.state IN ('invoiced', 'done') THEN SUM(l.price_subtotal) ELSE 0 END)
                / MIN({self._case_value_or_one('pos.currency_rate')})
                * {self._case_value_or_one('account_currency_table.rate')}
            AS amount_invoiced,
            count(*) AS nbr,
