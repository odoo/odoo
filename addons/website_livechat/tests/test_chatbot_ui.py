...
class TestLivechatChatbotUI(TestLivechatChatbotUICommon):
    ...
    def _check_complete_chatbot_flow_result(self):
        ...
        expected_messages = [
            ...
            (
                f'<div class="o_mail_notification">invited <a href="#" data-oe-model="res.partner" data-oe-id="{operator_member.partner_id.id}">@El Deboulonnator</a> to the channel</div>',
        ...
        for body, operator, _ in expected_messages[-6:]:
            chatbot_op = bool(operator)
            if chatbot_op and not last_msg_from_chatbot:
                parts.append(Markup("<br/>"))
            if chatbot_op:
                parts.append(Markup("<strong>%s</strong><br/>") % html2plaintext(body))
            else:
                parts.append(Markup("%s<br/>") % html2plaintext(body))
            last_msg_from_chatbot = chatbot_op
