    ...
    def _get_recent_channels_match_sql(self):
        ...
        return SQL(
            ...
            SELECT source_channel.id AS source_channel_id,
                   candidate_channel.id AS candidate_channel_id
              FROM source_channels selected_channel
              JOIN discuss_channel source_channel
                ON source_channel.id = selected_channel.id
              JOIN discuss_channel candidate_channel
                ON candidate_channel.livechat_visitor_id = source_channel.livechat_visitor_id
            """,
            match_sql=match_sql,
        )
