...
class StockPickingBatch(models.Model):
    ...
    l10n_ro_edi_stock_document_message = fields.Char(compute='_compute_l10n_ro_edi_stock_current_document_data')
    l10n_ro_edi_stock_document_load_id = fields.Char(compute='_compute_l10n_ro_edi_stock_current_document_data', string="eTransport Id")
    l10n_ro_edi_stock_document_uit = fields.Char(compute='_compute_l10n_ro_edi_stock_current_document_data', string="eTransport UIT")
    l10n_ro_edi_stock_state = fields.Selection(selection=DOCUMENT_STATES, compute='_compute_l10n_ro_edi_stock_current_document_data', string="eTransport Status")
    ...
    l10n_ro_edi_stock_start_loc_type = fields.Selection(selection=LOCATION_TYPES, string="Location Type", compute='_compute_l10n_ro_edi_stock_default_location_type')

    l10n_ro_edi_stock_available_end_loc_types = fields.Char(compute='_compute_l10n_ro_edi_stock_available_location_types')
    l10n_ro_edi_stock_end_loc_type = fields.Selection(selection=LOCATION_TYPES, string="Location Type", compute='_compute_l10n_ro_edi_stock_default_location_type')
    ...
    def _compute_l10n_ro_edi_stock_current_document_data(self):
        for batch in self:
            document = batch._l10n_ro_edi_stock_get_current_document()

            batch.l10n_ro_edi_stock_state = document.state if document else False
            batch.l10n_ro_edi_stock_document_message = document.message if document else False
            batch.l10n_ro_edi_stock_document_load_id = document.l10n_ro_edi_stock_load_id if document else False
            batch.l10n_ro_edi_stock_document_uit = document.l10n_ro_edi_stock_uit if document else False
    ...
    def _l10n_ro_edi_stock_send_etransport_document(self, send_type: str):
        ...
        data = {
            ...
            'tranport_partner_id': self.picking_ids[0].carrier_id.l10n_ro_edi_stock_partner_id,
        ...
        cif = self.company_id.vat.replace('RO', '')
        result = ETransportAPI._make_etransport_request(
    ...
    def _l10n_ro_edi_stock_fetch_document_status(self):
        ...
        for batch in to_fetch:
            ...
            result = self.env['l10n_ro_edi.document']._l10n_ro_edi_stock_fetch_etransport_document(
                company=batch.company_id,
                load_id=current_sending_document.l10n_ro_edi_stock_load_id,
                session=session,
            )
