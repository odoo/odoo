...
class PortalTest(http.Controller):
    ...
    @http.route('/my/test_portal/<int:res_id>', type='http', auth='public', methods=['GET'], website=True)
    ...
    def test_no_partner_portal_record_page(self, res_id, **kwargs):
        allowed_params = request.env["mail.test.portal.no.partner"]._get_allowed_access_params()
        access_kwargs = {k: v for k, v in kwargs.items() if k in allowed_params}
        record = request.env["mail.test.portal.no.partner"]._get_thread_with_access(res_id, **access_kwargs)
