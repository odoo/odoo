...
class TestMailPerformance(FullBaseMailPerformance):
    ...
    def test_message_post_w_followers(self):
        ...
        with self.assertQueryCount(employee=104):  # test_mail_full: 104
...
class TestPortalFormatPerformance(FullBaseMailPerformance, HttpCase):
    """Test performance of the portal chatter fetch route with multiple messages
    ...
    def test_portal_fetch_messages(self):
        ...
        for record in self.record_ratings:
            ...
            with self.assertQueryCount(employee=37):  # tmf: 34
                route_params = {
                    "share_only": True,
                    "thread_id": record.id,
                ...
                res = self.make_jsonrpc_request(
                    route="/mail/store",
                    params={"fetch_params": [["/mail/thread/messages", route_params]]},
                )
            fetched_messages = res["mail.message"]
            fetched_ratings = res["rating.rating"]
            ...
            for fetched_msg, fetched_rating, record_msg in zip(
                ...
                self.assertEqual(fetched_msg["subtype_id"], self.env.ref("mail.mt_comment").id)
...
class TestRatingPerformance(FullBaseMailPerformance):
    ...
    def test_rating_last_value_perfs(self):
        with self.assertQueryCount(employee=335):  # tmf: 335
            self.create_ratings('mail.test.rating.thread')

        with self.assertQueryCount(employee=343):  # tmf: 343
            self.apply_ratings(1)

        with self.assertQueryCount(employee=302):  # tmf: 302
            self.apply_ratings(5)

    @users('employee')
    @warmup
    def test_rating_last_value_perfs_with_rating_mixin(self):
        with self.assertQueryCount(employee=378):  # tmf: 378
            self.create_ratings('mail.test.rating')

        with self.assertQueryCount(employee=384):  # tmf: 384
            self.apply_ratings(1)

        with self.assertQueryCount(employee=363):  # tmf: 363
            self.apply_ratings(5)
