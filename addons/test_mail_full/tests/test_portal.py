...
class TestPortalControllers(TestPortal):
    ...
    def test_portal_avatar_with_access_token(self):
        ...
        self.opener.post(
            url=f"{self.record_portal.get_base_url()}/mail/message/post",
            json={
                "params": {
        ...
        response = self.url_open(
            f"/web/image/res.partner/{self.record_portal.partner_id.id}/avatar_128?thread_model={self.record_portal._name}&thread_id={self.record_portal.id}&token={token}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers.get("Content-Type"), "image/png")
        self.assertRegex(
            response.headers.get("Content-Disposition", ""),
            rf"{self.record_portal.partner_id.name}\.png",
        )

        placeholder_response = self.url_open(
            f'/web/image/res.partner/{self.record_portal.partner_id.id}/avatar_128?thread_model={self.record_portal._name}&thread_id={self.record_portal.id}&token={token + "a"}'
        )  # false token
        ...
        self.assertRegex(
            placeholder_response.headers.get("Content-Disposition", ""), r"placeholder\.png"
        )

        no_token_response = self.url_open(
            f"/web/image/res.partner/{self.record_portal.partner_id.id}/avatar_128"
        )
        self.assertEqual(no_token_response.status_code, 200)
        self.assertEqual(no_token_response.headers.get("Content-Type"), "image/png")
        self.assertRegex(
            no_token_response.headers.get("Content-Disposition", ""), r"placeholder\.png"
        )
    ...
    def test_portal_avatar_with_hash_pid(self):
        ...
        response = self.url_open(
            f"/web/image/res.partner/{self.partner_2.id}/avatar_128?thread_model={self.record_portal._name}&thread_id={self.record_portal.id}&hash={_hash}&pid={pid}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers.get("Content-Type"), "image/png")
        self.assertRegex(
            response.headers.get("Content-Disposition", ""),
            rf"{self.partner_2.name}\.png",
        )

        placeholder_response = self.url_open(
            f'/web/image/res.partner/{self.partner_2.id}/avatar_128?thread_model={self.record_portal._name}&thread_id={self.record_portal.id}&hash={_hash + "a"}&pid={pid}'
        )  # false hash
        self.assertEqual(placeholder_response.status_code, 200)
        self.assertEqual(placeholder_response.headers.get("Content-Type"), "image/png")
        self.assertRegex(
