...
class TestPortalControllers(TestPortal):
    ...
    def test_portal_message_fetch(self):
        ...
        res = self.opener.post(
            url=f"{self.record_portal.get_base_url()}/mail/data",
            json={"params": {"fetch_params": [["/mail/chatter_fetch", chatter_fetch_params]]}},
        )
        res.raise_for_status()
        fetched_ids = [msg["id"] for msg in res.json()["result"]["mail.message"]]
        self.assertIn(
            msg_custom.id,
            fetched_ids,
