...
class TestSMSComposerMass(TestMailFullCommon):
    ...
    def test_composer_mass_active_ids(self):
        ...
        for partner in self.partners:
            self.assertSMS(partner, None, 'outgoing', content=self._test_body)
    ...
    def test_composer_mass_active_ids_w_blacklist(self):
        ...
        for partner in self.partners[5:]:
            self.assertSMS(partner, partner.phone_sanitized, 'outgoing', content=self._test_body)
        for partner in self.partners[:5]:
            self.assertSMS(partner, partner.phone_sanitized, 'canceled', error_code='sms_blacklist', content=self._test_body)
    ...
    def test_composer_mass_active_ids_w_blacklist_and_done(self):
        ...
        for partner in self.partners[5:8]:
            self.assertSMS(partner, partner.phone_sanitized, 'canceled', error_code='sms_duplicate', content=self._test_body)
        for partner in self.partners[:5]:
            self.assertSMS(partner, partner.phone_sanitized, 'canceled', error_code='sms_blacklist', content=self._test_body)
    ...
    def test_composer_mass_active_ids_w_template_and_lang(self):
        ...
        for record in self.records:
            if record.customer_id == self.partners[2]:
                self.assertSMS(record.customer_id, None, 'outgoing', content='Cher·e· %s ceci est un SMS.' % record.display_name)
            else:
                self.assertSMS(record.customer_id, None, 'outgoing', content='Dear %s this is an SMS.' % record.display_name)
    ...
    def test_composer_template_context_action(self):
        ...
        self.assertSMS(test_record_1.customer_id, None, 'outgoing', content='Dear %s this is an SMS.' % test_record_1.display_name)
        self.assertSMS(test_record_2.customer_id, None, 'outgoing', content="Hello %s ceci est en français." % test_record_2.display_name)
