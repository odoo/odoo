...
class TestMassSMS(test_mail_full_common.MassSMSBaseFunctionalTest, mass_mailing_sms_common.MockSMS):
    ...
    def test_mass_sms_internals(self):
        with self.sudo('marketing'):
            ...
            remaining_res_ids = mailing.get_remaining_recipients()
    ...
    def test_mass_sms_internals_errors(self):
        ...
        self.assertSMSStatistics(
            ...
            mailing, self.records | new_record_1, check_sms=True
        )
        self.assertSMSStatistics(
            [{'partner': new_record_2.customer_id, 'number': self.records_numbers[0], 'content': 'Dear %s this is a mass SMS.' % new_record_2.display_name, 'state': 'ignored'}],
            mailing, new_record_2, check_sms=True
        )
