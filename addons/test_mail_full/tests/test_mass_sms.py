...
class TestMassSMS(test_mail_full_common.TestSMSCommon):
    ...
    def test_mass_sms_internals_errors(self):
        ...
        self.assertSMSStatistics(
            [{'partner': new_record_2.customer_id, 'number': self.records_numbers[0], 'content': 'Dear %s this is a mass SMS.' % new_record_2.display_name, 'state': 'bounce', 'failure_type': 'sms_duplicate'}],
    ...
    def test_mass_sms_internals_done_ids(self):
        ...
        self.assertSMSStatistics(
            [{'partner': record.customer_id, 'number': self.records_numbers[i], 'content': 'Dear %s this is a mass SMS.' % record.display_name, 'state': 'bounce', 'failure_type': 'sms_duplicate'} for i, record in enumerate(self.records[:5])],
            self.mailing, self.records[:5], check_sms=True
        )
        # new traces generated
        self.assertSMSStatistics(
