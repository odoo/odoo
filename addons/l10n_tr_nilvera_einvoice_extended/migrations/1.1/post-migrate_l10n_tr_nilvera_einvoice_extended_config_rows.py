...
COLUMNS = ("l10n_tr_nilvera_export_alias",)
...
def migrate(cr, version):
    ...
    env["l10n_tr_nilvera_einvoice_extended.config"]._for_each(companies)
    env.flush_all()
    assignments = ", ".join(f"{column} = c.{column}" for column in present)
    cr.execute(
        f"UPDATE l10n_tr_nilvera_einvoice_extended_config x SET {assignments} FROM res_company c WHERE c.id = x.company_id"
    )
    env.invalidate_all()
