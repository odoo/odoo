...
class Lead(models.Model):
    ...
    def _iap_enrich_from_response(self, iap_response):
        ...
        for lead in self.search([('id', 'in', list(iap_response.keys()))]):  # handle unlinked data by performing a search
            iap_payload = iap_response.get(str(lead.id))
            if not iap_payload:
                lead.write({'iap_enrich_done': True})
                lead.message_post_with_view('crm_iap_enrich.mail_message_lead_enrich_notfound', subtype_id=self.env.ref('mail.mt_note').id)
                continue

            contact_vals = self.env['iap.enrich.api']._get_contact_vals_from_response(iap_payload)
            contact_vals['partner_name'] = contact_vals['name']
            lead_vals = {'iap_enrich_done': True}
            lead_vals.update(dict(
                (field_name, contact_vals[field_name])
                for field_name in ['city', 'country_id', 'partner_name', 'mobile', 'phone', 'reveal_id', 'state_id', 'street', 'zip']
                if not lead[field_name] and contact_vals[field_name]
            ))
            lead.write(lead_vals)

            template_values = iap_payload
