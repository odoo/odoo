...
class stock_picking_wave(osv.osv):
    ...
    def print_picking(self, cr, uid, ids, context=None):
        ...
        if not picking_ids:
            raise Warning(_('Error!'), _('Nothing to print.'))
        ...
        for wave in self.browse(cr, uid, ids, context=context):
            for picking in wave.picking_ids:
                ...
                if picking.state != 'assigned':
                    raise Warning(_('Warning'), _('Some pickings are still waiting for goods. Please check or force their availability before setting this wave to done.'))
