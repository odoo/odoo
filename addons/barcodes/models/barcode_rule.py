...
class BarcodeRule(models.Model):
    ...
    has_alias = fields.Boolean(compute='_compute_has_alias')
    ...
    alias = fields.Char(
        string='Alias', default='', help='The matched pattern will alias to this barcode')
    ...
    @api.depends('rule_part_ids', 'rule_part_ids.type')
    def _compute_has_alias(self):
        for rule in self:
            rule.has_alias = any(rp.type == 'alias' for rp in rule.rule_part_ids)
    ...
    def _check_pattern(self):
        for rule_group in self:
            try:
                ...
                raise ValidationError(_(
                    "The pattern \"%(pattern)s\" is not a valid Regex: %(error)s",
