                ...
                if self.env.ref(move['partner_id']).country_id in european_countries:
                    for line in move['invoice_line_ids']:
                        if move['move_type'] in ['in_invoice', 'in_refund']:
                            line[2]['tax_ids'] = [Command.set(eu_sale_tax.ids)]
                        else:
                            line[2]['tax_ids'] = [Command.set(eu_purchase_tax.ids)]
                else:
                    for line in move['invoice_line_ids']:
                        if move['move_type'] in ['in_invoice', 'in_refund']:
                            line[2]['tax_ids'] = [Command.set(non_eu_sale_tax.ids)]
                        else:
                            line[2]['tax_ids'] = [Command.set(non_eu_purchase_tax.ids)]
        return data
