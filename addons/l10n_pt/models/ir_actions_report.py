...
class IrActionsReport(models.Model):
    ...
    def _get_l10n_pt_versioned_report_html(self, doc, template, values):
        ...
        # Original print. Might be the first time any report is generated for this record, or it might be an overwrite
        # in case the record has been cancelled (which is treated as a new report with an Original and Reprint version)
        if doc.l10n_pt_print_version == 'original':
            html = super()._render_template(template, values)
            if attachment and not attachment.cancelled and doc.state == 'cancel':
                attachment.write({
