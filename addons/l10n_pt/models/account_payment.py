...
class AccountPayment(models.Model):
    ...
    l10n_pt_original_print_json = fields.Json(readonly=True, copy=False)
    ...
    def action_open_print_wizard(self):
        payments_reprint = self.filtered(lambda p: p.country_code == 'PT' and p.l10n_pt_print_version)
        if payments_reprint:
            return {
    ...
    def action_receipt_download_pdf(self):
        if self.filtered(lambda p: p.country_code == 'PT'):
            return {
                'type': 'ir.actions.act_url',
                'url': f'/account/download_receipt_documents/{",".join(map(str, self.ids))}/pdf',
                'target': 'download',
            }
        return self.env.ref('account.report_payment_receipt').report_action(self.ids)

    def _get_payment_legal_documents(self, filetype, allow_fallback=False):
        # EXTENDS account
        if filetype == 'pdf' and self.country_code == 'PT':
            filename = f"{self.name.replace(' ', '_').replace('/', '_')}.pdf"
            content, report_type = self.env['ir.actions.report']._pre_render_qweb_pdf('account.report_payment_receipt', self.ids)
            content_by_id = self.env['ir.actions.report']._get_splitted_report('account.report_payment_receipt', content, report_type)
            return {
                'filename': filename,
                'filetype': 'pdf',
    ...
    def _check_l10n_pt_dates(self):
        ...
        if self.l10n_pt_at_series_id:
            self.env.cr.execute(SQL("""
                SELECT MAX(date) AS max_payment_date
                  FROM account_payment
                 WHERE state = ANY(ARRAY['in_process', 'paid', 'cancel'])
                   AND l10n_pt_at_series_id = %s AND payment_type = 'inbound'
            """, self.l10n_pt_at_series_id.id))

            max_payment_date = self.env.cr.fetchone()[0]
    ...
    def _compute_l10n_pt_at_series_id(self):
        payments = self.filtered(
            lambda p: not p.l10n_pt_at_series_id
                      or p.l10n_pt_at_series_id.payment_journal_id != p.journal_id
        )
        for (company, journal), grouped_payments in payments.grouped(lambda o: (o.company_id, o.journal_id)).items():
            last_payment = self.env['account.payment'].search([
                ('company_id', '=', company.id),
                ('payment_type', '=', 'inbound'),
                ('journal_id', '=', journal.id)
            ], order='id desc', limit=1)
            last_at_series = last_payment.l10n_pt_at_series_id
            if last_at_series:
                payments.l10n_pt_at_series_id = last_at_series
            else:
                payments.l10n_pt_at_series_id = self.env['l10n_pt.at.series'].search([
                    ('company_id', '=', company.id),
