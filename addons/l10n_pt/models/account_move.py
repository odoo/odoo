...
class AccountMove(models.Model):
    ...
    def action_open_print_wizard(self):
        ...
        if (
            ...
            and self.l10n_pt_print_version != 'none'
        ):
            return {
                'name': 'Print Reason',
                ...
                'view_mode': 'form',
                'target': 'new',
                'context': {'active_id': self.id, 'res_model': 'account.move'},
            }
    ...
    def _get_move_hash_domain(self, common_domain=False, force_hash=False):
        ...
        if self.env.company.account_fiscal_country_id.code == 'PT':
            return [('state', 'in', ('posted', 'cancel')) if x == ('state', '=', 'posted') else x for x in domain]
            ...
            if self.move_type == 'out_refund' and self.reversed_entry_id:
                ...
                for index in range(len(self.invoice_line_ids)):
                    if self.invoice_line_ids[index].price_total > self.reversed_entry_id.invoice_line_ids[index].price_total:
                        raise UserError(_("The value or quantity per line of a reversal cannot exceed the value or quantity of the original line."))
