...
class AlarmManager(models.AbstractModel):
    ...
    def _send_reminder(self):
        ...
        all_events_ids = list(set(event_id for event_ids in events_by_alarm.values() for event_id in event_ids))
        for alarm_id, event_ids in events_by_alarm.items():
            alarm = self.env['calendar.alarm'].browse(alarm_id).with_prefetch(events_by_alarm.keys())
