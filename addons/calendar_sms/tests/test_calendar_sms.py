...
class TestCalendarSms(SingleTransactionCase, SMSCase):
    ...
    def test_send_reminder_match_both_events(self):
        ...
        with self.mockSMSGateway():
            self.env.ref('calendar.ir_cron_scheduler_alarm').method_direct_trigger()

            mail_message_24h = self.env['mail.message'].search([
                ('model', '=', 'calendar.event'),
                ('res_id', '=', self.event_24h.id),
                ('message_type', '=', 'sms'),
            ])

            mail_message_1h = self.env['mail.message'].search([
                ('model', '=', 'calendar.event'),
                ('res_id', '=', self.event_1h.id),
                ('message_type', '=', 'sms'),
            ])

            self.assertEqual(len(mail_message_24h), 1)
            self.assertEqual(len(mail_message_1h), 1)
            self.assertEqual(html.fromstring(str(mail_message_24h.body)).text_content(),self.sms_template_24h.body)
            self.assertEqual(html.fromstring(str(mail_message_1h.body)).text_content(),self.sms_template_1h.body)
