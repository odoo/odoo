...
class TestWorkEntryLeave(TestWorkEntryHolidaysBase):
    ...
    def test_create_work_entry_for_duration_based_employee_leave(self):
        ...
        # Two leaves of 6 hours on Wednesday, Thursday is the last day of the period
        for hour_from, hour_to in ((1, 7), (14, 20)):
            self.env['hr.leave'].create({
                'employee_id': self.jules_emp.id,
                'holiday_status_id': leave_type_paid.id,
                'request_date_from': date(2024, 9, 18),
                'request_date_to': date(2024, 9, 18),
                'request_hour_from': hour_from,
                'request_hour_to': hour_to,
            }).with_user(SUPERUSER_ID)._action_validate()

        entries = self.jules_emp.generate_work_entries(date(2024, 9, 18), date(2024, 9, 18))

        wednesday_entries = entries.filtered_domain([('date', '=', date(2024, 9, 18))])
        wednesday_leave = wednesday_entries.filtered_domain([('work_entry_type_id', '=', entry_type_paid.id)])
        wednesday_attendance = wednesday_entries - wednesday_leave
        self.assertEqual(sum(wednesday_leave.mapped('duration')), 7, "Wednesday leaves (6h + 6h) can't exceed the 7 hours of the day")
        self.assertFalse(wednesday_attendance, "No attendance should remain on Wednesday")
