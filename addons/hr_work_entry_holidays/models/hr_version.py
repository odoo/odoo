...
class HrVersion(models.Model):
    ...
    def _get_duration_based_real_attendances(self, start_dt, end_dt, attendances, leaves, worked_leaves):
        """ Return real attendance/overtime for a day it was recorded, or a duration-based
        block sized from the calendar's configured hours (minus any leave taken that
        day) for a day it wasn't. """
        calendar = self.resource_calendar_id
        ...
        recorded_attendances = Intervals([
            (start, stop, record) for start, stop, record in attendances
            if record and record._name in ('hr.attendance', 'hr.attendance.overtime.line')
        ], keep_distinct=True)
        consumed = leaves | worked_leaves
        duration_based_blocks = []
        start_day = tz.localize(datetime.combine(start_dt.astimezone(tz).date(), time.min))
        last_day = end_dt.astimezone(tz)
        ...
        recorded_days = {
            ...
            for interval_start, _interval_stop, _records in recorded_attendances & day_windows_intervals
        }
        recorded_days_intervals = Intervals([
            (current_day, current_day + timedelta(days=1), self.env['resource.calendar.leaves'])
            for current_day in day_windows
            if current_day.date() in recorded_days
        ], keep_distinct=True)
        recorded_day_attendances = attendances & recorded_days_intervals
        ...
        for current_day in day_windows:
            ...
            if day_hours:
                ...
                if remaining_hours:
                    duration_based_blocks.append(
                        (current_day, current_day + timedelta(hours=remaining_hours), self.env['resource.calendar.attendance']))
        duration_based_attendances = Intervals(duration_based_blocks, keep_distinct=True) - recorded_day_attendances
        return duration_based_attendances | (recorded_day_attendances - leaves - worked_leaves)
