...
class AbstractReport(models.AbstractModel):
    ...
    def render_html(self, data=None):
        ...
        if context.get('active_ids'):
            ...
            model_records = self.env[model].with_context(context).browse(context['active_ids'])
        else:
            ...
            model_records = self.env[model].with_context(context).browse(self.ids)
            context['active_model'] = model
            context['active_ids'] = self.ids
        # Generate the old style report
        wrapped_report = self._model._wrapped_report_class(self.env.cr, self.env.uid, '', context=context)
        wrapped_report.set_context(model_records, data, context['active_ids'])
