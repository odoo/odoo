...
class TestMyDATAPicking(AccountTestInvoicingCommon):
    ...
    def test_l10n_gr_edi_picking_send_success(self):
        ...
        self.env['l10n_gr_edi.document']._l10n_gr_edi_handle_send_result(mock_result, xml_vals)
    ...
    def test_l10n_gr_edi_picking_send_error(self):
        ...
        self.assertEqual(picking.l10n_gr_edi_state, 'delivery_note_error')
        self.assertFalse(picking.l10n_gr_edi_mark)

    @patch('odoo.addons.l10n_gr_edi.models.l10n_gr_edi_document._make_mydata_request')
    def test_l10n_gr_edi_send_delivery_note_raises_user_error(self, mock_request):
        picking = self._create_mydata_delivery_note()

        mock_request.return_value = {
            0: {'error': '[223] Unsupported invoice type'},
        }
