...
class SaleTimesheetCustomerPortal(TimesheetCustomerPortal):
    ...
    def _get_searchbar_inputs(self):
        return super()._get_searchbar_inputs() | {
            'so': {'input': 'so', 'label': request.env._('Search in Sales Order Item'), 'sequence': 50},
            'invoice': {'input': 'invoice', 'label': request.env._('Search in Invoice'), 'sequence': 80},
        }
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        try:
            if task.sale_order_id and self._document_check_access('sale.order', task.sale_order_id.id):
                ...
                title = request.env._('Quotation') if task.sale_order_id.state in ['draft', 'sent'] else request.env._('Sales Order')
