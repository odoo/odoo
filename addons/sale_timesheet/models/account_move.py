...
class AccountMove(models.Model):
    ...
    def _compute_has_timesheet_portal(self):
        ...
        for invoice in self:
            invoices = self.env['account.move'].sudo().search(['|', ('name', 'ilike', invoice.name), ('id', 'ilike', invoice.name)])
            domain = expression.AND([initial_domain, self.env['account.analytic.line']._timesheet_get_sale_domain(invoices.mapped('invoice_line_ids.sale_line_ids'), invoices)])
            invoice.has_timesheet_portal = Timesheet.sudo().search(domain, limit=1)
    ...
    def action_view_timesheet(self):
        ...
        default_so_line = next((sale_line for sale_line in self.line_ids.sale_line_ids if sale_line.is_service and sale_line.product_id.service_policy != 'delivered_timesheet'), False)
        if default_so_line:
            context['default_so_line'] = default_so_line.id
            context['default_timesheet_invoice_id'] = self.id
            context['default_task_id'] = default_so_line.task_id.id
            if default_so_line.project_id:
                context['default_project_id'] = default_so_line.project_id.id
            else:
                context['default_project_id'] = default_so_line.task_id.project_id.id
    ...
    def _link_timesheets_to_invoice(self, start_date=None, end_date=None, service_policies=('delivered_timesheet')):
        ...
        for line in self.filtered(lambda i: i.move_type in ('out_invoice', 'out_refund') and i.state == 'draft').invoice_line_ids:
            sale_line_delivery = line.sale_line_ids.filtered(lambda sol: sol.product_id.service_policy and sol.product_id.service_policy in service_policies)
    ...
    def _unlink_timesheets_from_invoice(self, service_policies):
        for move in self:
            if move.move_type in ('out_invoice', 'out_refund') and move.state == 'posted':
                timesheets = move.timesheet_ids.filtered(lambda t: t.so_line.product_id.service_policy and t.so_line.product_id.service_policy in service_policies)
