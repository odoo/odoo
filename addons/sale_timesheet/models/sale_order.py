...
class SaleOrderLine(models.Model):
    ...
    timesheet_ids = fields.One2many('account.analytic.line', 'so_line', "Timesheets", domain=[('is_timesheet', '=', True)])
    analytic_line_ids = fields.One2many(domain=[('is_timesheet', '=', False)])  # only analytic lines, not timesheets (since this field determine if SO line came from expense)
