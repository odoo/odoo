...
class SaleOrderLine(models.Model):
    ...
    def _recompute_qty_to_invoice(self, start_date, end_date):
        ...
        for line in lines_by_timesheet:
            ...
            remaining_qty = line.qty_delivered - line.qty_invoiced
            if line.invoice_lines.move_id & credit_notes:
                ...
                qty_to_invoice = min(period_qty, remaining_qty)
            elif (start_date or end_date) and line.id in lines_with_timesheets_outside_period:
                # Partial period: invoice only the selected timesheets.
                qty_to_invoice = period_qty
            elif (start_date or end_date) and period_qty <= 0.0:
                qty_to_invoice = 0.0
            elif start_date or end_date:
                # Period covers all remaining eligible timesheets: take the larger of
                # period and remaining so previous rounding can be absorbed, while
                # still invoicing period timesheets after manual over-invoicing.
                qty_to_invoice = max(period_qty, remaining_qty)
            else:
                qty_to_invoice = remaining_qty
