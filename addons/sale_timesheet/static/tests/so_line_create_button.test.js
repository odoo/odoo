...
import { clickFieldDropdownItem, contains, mountView, onRpc } from "@web/../tests/web_test_helpers";
...
test("test so_line_create_button widget: visibility conditions", async () => {
    ...
    await animationFrame();
    await focus("[name='sale_line_id'] input");
    expect("button[aria-label='Create Sales Order']").toHaveCount(0, {
        message:
    ...
    await animationFrame();
    await focus("[name='sale_line_id'] input");
    expect("button[aria-label='Create Sales Order']").toBeVisible({
        message:
