...
class TestSaleProject(TestReporting):
    ...
    def test_project_overview_by_project(self):
        ...
        self.assertEqual(float_compare(vals['dashboard']['profit']['total'], project_invoiced + project_timesheet_cost + expense.amount, precision_rounding=rounding), 0, "The total amount should be the one from the SO2 line, as we are in ordered quantity")
