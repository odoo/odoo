...
class TestUpsellWarning(TestCommonSaleTimesheet):
    ...
    def test_display_upsell_warning_incremental_timesheets(self):
        ...
        self.assertEqual(len(so.activity_search(['mail.mail_activity_data_todo'])), 0, 'No upsell warning should appear yet (4h < 6h).')
        ...
        # 8) Check if the SO has a 'mail.mail_activity_data_todo' activity
        self.assertEqual(len(so.activity_search(['mail.mail_activity_data_todo'])), 1, 'An upsell warning should appear after incremental timesheets exceed threshold (7h > 6h).')
