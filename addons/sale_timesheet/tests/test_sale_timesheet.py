...
class TestSaleTimesheet(TestCommonSaleTimesheet):
    ...
    def test_unit_allocated_hours_generated_project(self):
        """
        The total field allocated_hours should always be set in hours, indepently
        of the uom set in the settings to encode timesheets
        """

        # 1. Create a product whose uom is day
        days_uom = self.env.ref('uom.product_uom_day')
        product_order_timesheet_day = self.env['product.product'].create({
            'name': "Service Ordered, create task in new project, day uom",
            'standard_price': 10,
            'list_price': 20,
            'type': 'service',
            'invoice_policy': 'order',
            'uom_id': days_uom.id,
            'uom_po_id': days_uom.id,
            'service_type': 'timesheet',
            'service_tracking': 'task_in_project',
            'project_id': False,
        })

        # 2. Set the default encoding uom to days in the settings
        self.env.company.timesheet_encode_uom_id = days_uom

        # 3. Create SO with this product and confirm it
        sale_order = self.env['sale.order'].create({
            'partner_id': self.partner_a.id,
            'partner_invoice_id': self.partner_a.id,
            'partner_shipping_id': self.partner_a.id,
        ...
        so_line = self.env['sale.order.line'].create({
            'product_id': product_order_timesheet_day.id,
            'product_uom_qty': 10,
            'order_id': sale_order.id,
        })
        sale_order.action_confirm()
        ...
        # 4. Check that the allocated_hours field has been correctly set for task and project
        for record in [project, task]:
            self.assertEqual(
                record.allocated_hours,
                80.0,
                f"The allocated_hours field of {record} (generated by a SO confirmation) should be set in hours"
            )
