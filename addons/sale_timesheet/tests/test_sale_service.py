...
class TestSaleService(TestCommonSaleTimesheet):
    ...
    def test_prepaid_service_unit_uom_qty_delivered_method(self):
        ...
        # Case 2: Unit UoM + SO Header project with allow_timesheets=True
        self.sale_order.project_id = self.project_global
        self.assertEqual(
            sol.qty_delivered_method, 'timesheet',
            "Prepaid service with Unit UoM linked to a timesheets-enabled SO project "
        ...
        # Case 4: same SOL, disable timesheets on the SO project
        self.project_global.allow_timesheets = False
        self.assertEqual(
            sol.qty_delivered_method, 'manual',
        ...
        # Case 5: Verify manual delivery input does not change the ordered quantity
        self.sale_order.project_id = False
        self.assertEqual(sol.qty_delivered_method, 'manual')

        sol.qty_delivered = 2.0
        self.assertEqual(
            sol.product_uom_qty, 3.0,
            "Manually updating qty_delivered on a prepaid service must not alter the ordered quantity.",
        )
