...
const { Component, onMounted, onPatched, onWillUnmount, onWillUpdateProps, useComponent, useRef } = owl;
...
export class AttachmentViewer extends Component {
    ...
    setup() {
        ...
        component.props.record.update({ ['component']: component });
        onWillUpdateProps(nextProps => {
            ...
            if (currentRecord.exists() && currentRecord !== nextRecord) {
                currentRecord.update({ ['component']: {
                    name: 'clear',
                    value: undefined,
                }});
            }
            nextRecord.update({ ['component']: component });
        });
    ...
    _handleImageLoad() {
        ...
        const image = refs[`${this.attachmentViewer.attachment.href}`];
        if (
...
Object.assign(AttachmentViewer, {
    props: { record: Object },
    template: 'knowledge.AttachmentViewer',
});
