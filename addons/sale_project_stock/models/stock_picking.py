...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        if self.project_id.analytic_sale_order_id and self.picking_type_id.analytic_costs:
            reinvoicable_stock_moves = self.move_ids.filtered(lambda m: m.product_id.expense_policy in {'sales_price', 'cost'})
            if reinvoicable_stock_moves:
                sale_order = self.project_id.analytic_sale_order_id
                ...
                # Create SOLs in analytic_sale_order_id with reinvoicable stock moves
                ...
                for stock_move in reinvoicable_stock_moves:
                    ...
                    last_sequence += 1
                self.env['sale.order.line'].create(sale_line_values_to_create)
        return res
