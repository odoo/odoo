...
class AccountAnalyticLine(models.Model):
    ...
    billable_type = fields.Selection(selection_add=[('13_picking_entry_positive', 'Inventory Transfers (Revenue)'),
                                                    ('24_picking_entry_negative', 'Inventory Transfers (Costs)')])

    @api.depends('billable_type')
    def _compute_category_report(self):
        picking_revenues = self.filtered(lambda t: t.billable_type == '13_picking_entry_positive')
    ...
    def _set_billable_cost(self):
        ...
        aals_delivery.billable_type = '24_picking_entry_negative'
    ...
    def _get_invoice_type(self, invoice_type):
        if self.category == 'picking_entry':
            return '13_picking_entry_positive'
