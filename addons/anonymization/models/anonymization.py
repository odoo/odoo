...
class IrModelFieldsAnonymization(models.Model):
    ...
    def _get_global_state(self):
        ...
        if not ano_fields or len(ano_fields) == len(ano_fields.filtered(lambda field: field.state == "clear")):
            state = 'clear'  # all fields are clear
        elif len(ano_fields) == len(ano_fields.filtered(lambda field: field.state == "anonymized")):
