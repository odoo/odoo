...
class DriverController(http.Controller):
    ...
    def action(self, session_id, device_identifier, data):
        ...
        iot_device.data['owner'] = data['operation_id'] = session_id
        ...
        # Call action method on device
        result = iot_device.action(data)
        # Send back result to the server (make it available to ``event`` route)
        event_manager.device_changed(iot_device, result)
        return True
