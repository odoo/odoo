...
def certificate_is_valid():
    ...
    """
    if platform.system() == 'Windows':
        path = Path(get_path_nginx(), 'conf', 'nginx-cert.crt')
    else:
    ...
    cert_end_date = datetime.datetime.strptime(cert.get_notAfter().decode(), "%Y%m%d%H%M%SZ") - datetime.timedelta(days=10)

    cn = next((key[1].decode() for key in cert.get_subject().get_components() if key[0] == b'CN'), '')
    if cn == 'OdooTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
        _logger.info("The HTTPS certificate %s must be updated" % cn)
        return False

    _logger.info('Your certificate %s is valid until %s' % (cn, cert_end_date))
    return True
...
def ensure_certificate():
    ...
    """
    if not certificate_is_valid() and not load_certificate():
        _logger.error("Failed to load certificate")
        return
    _logger.info("Certificate is up to date")
...
def load_certificate():
    ...
    try:
        ...
        _logger.exception("An error occurred while trying to get the certificate from odoo.com.")
        return False
