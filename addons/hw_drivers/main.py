...
class Manager(Thread):
    ...
    def send_alldevices(self):
        ...
        """
        # We change the password only if db is connected (if we decorated the method
        # with @helpers.require_db we wouldn't be able to change the password from the
        # homepage if no db was connected)
        ...
        try:
            ...
            return json.loads(resp.data).get('result', '')
