...
try:
    ...
    # AFTER  https://github.com/daviddrysdale/python-phonenumbers/blob/v8.13.53/python/phonenumbers/data/region_MX.py
    def _hook_load_region_mx(code):
        if parse_version(phonenumbers.__version__) < parse_version('8.13.53'):
            _local_load_region(code)
        else:
            phonenumbers.data._load_region(code)
        _region_metadata = phonenumbers.PhoneMetadata._region_metadata
