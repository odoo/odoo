...
class ProductLabelLayout(models.TransientModel):
    ...
    def _get_packaging_barcode(self, product, packaging):
        products = product if product.is_product_variant else product.product_variant_ids
        packaging_barcode = products.product_uom_ids.filtered(lambda product_uom: product_uom.uom_id == packaging)[:1].barcode
        return packaging_barcode or ''
    ...
    def _get_label_requests(self):
        ...
        for record in records:
            ...
            if self.product_uom_ids:
                ...
                barcode_value = self._get_packaging_barcode(product, self.packaging_id)
            else:
