...
class ReportProductLabelBase(models.AbstractModel):
    ...
    def _organize_labels(self, labels, layout_wizard):
        slots_per_page = layout_wizard.rows * layout_wizard.columns
        if not labels:
            return []
        ...
        for page_start in range(0, len(labels), slots_per_page):
            ...
            organized_pages.append([
                page_labels[row_start:row_start + layout_wizard.columns]
                for row_start in range(0, slots_per_page, layout_wizard.columns)
            ])
        return organized_pages

    def _get_report_label_values(self, labels, layout_wizard):
        label_pages = self._organize_labels(labels, layout_wizard)
        return {
    ...
    def _prepare_data(self, docids, data):
        ...
        report_values.update(self._get_report_label_values(labels, layout_wizard))
