...
class TestUom(TransactionCase):
    ...
    def test_10_conversion(self):
        qty = self.uom_gram._compute_qty(1020000, self.uom_ton.id)
        self.assertEquals(qty, 1.02, "Converted quantity does not correspond.")

        price = self.uom_gram._compute_price(2, self.uom_ton.id)
        ...
        # by rev. fa2f7b86.
        qty = self.uom_dozen._compute_qty(1, self.uom_unit.id)
        ...
        qty = self.uom_gram._compute_qty(1234, self.uom_kgm.id)
    ...
    def test_20_rounding(self):
        ...
        qty = self.uom_unit._compute_qty(2, product_uom.id)
