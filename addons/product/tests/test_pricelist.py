...
class TestPricelist(TransactionCase):
    ...
    def test_20_pricelist_uom(self):
        ...
        spam_id = self.usb_adapter.copy({'name': '1 tonne of spam',
                                                'uom_id': self.uom_ton.id,
                                                'uom_po_id': self.uom_ton.id,
                                                'list_price': tonne_price,
                                                'type': 'consu'})

        self.env['product.pricelist.item'].create({'pricelist_id': self.public_pricelist.id,
                                                   'sequence': 10,
                                                   'applied_on': '0_product_variant',
                                                   'compute_price': 'formula',
                                                   'base': 'list_price',  # based on public price
        ...
        def test_unit_price(qty, uom, expected_unit_price):
            unit_price = pricelist.with_context({'uom': uom}).price_get(spam_id.id, qty)[pricelist.id]
