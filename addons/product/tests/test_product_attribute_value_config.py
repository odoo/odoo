    ...
    def test_get_first_possible_combination(self):
        ...
        for exclusion in computer_ram_32.excluded_value_ids:
            computer_ram_32.write({
                'excluded_value_ids': [Command.delete(exclusion.id)]
            })

        # Activates all exclusions at once
        computer_ram_32.write({
            'excluded_value_ids': [Command.link(value) for value in [computer_hdd_1.id, computer_hdd_2.id, computer_hdd_4.id, computer_ssd_256.id, computer_ssd_512.id]]
        })
