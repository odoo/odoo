...
class TestImportFiles(TransactionCase):
    ...
    def test_import_create_product_duplicate_xls(self):
        ...
        results = self.import_product_xls("product.product", filepath="product/static/xls/test_duplicated_variants_2.xls")
        self.assertFalse(results["messages"])
        self.assertEqual(self.env['product.product'].search([('id', 'not in', products.ids)]).mapped('display_name'), ['Sweat', 'Sweat 33'])
