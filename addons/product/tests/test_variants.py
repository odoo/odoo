...
class TestVariants(ProductVariantsCommon):
    ...
    def test_list_price(self):
        """ Ensure list_price/lst_price are correctly (re)computed depending on the
        company context, mirroring standard_price. Unlike cost_currency_id, currency_id
        stays pinned to the main company regardless of the active company. """
        one_variant_product = self.product
        self.assertEqual(one_variant_product.product_variant_count, 1)
        ...
        self.assertEqual(one_variant_product.currency_id, company_a.currency_id)
        self.assertEqual(one_variant_product.with_company(company_b).currency_id, company_a.currency_id)

        one_variant_template = one_variant_product.product_tmpl_id
        original_list_price = one_variant_template.list_price
        self.assertEqual(one_variant_product.lst_price, one_variant_template.list_price)

        one_variant_product.with_company(company_b).lst_price = 500.0
        self.assertEqual(
            one_variant_product.with_company(company_b).lst_price,
            one_variant_template.with_company(company_b).list_price,
        )
        self.assertEqual(500.0, one_variant_template.with_company(company_b).list_price)

        # company_a's own value must remain untouched by the company_b write
        self.assertEqual(one_variant_template.list_price, original_list_price)
        self.assertEqual(one_variant_product.lst_price, original_list_price)
