...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        ...
        self.ensure_one()
        # Prepare datas
        products_qty_partner, prod_tmpl_ids, prod_ids, categ_ids, date = self._prepare_datas(products_qty_partner, date, uom_id)
        # Load all rules
        items = self._compute_price_rule_get_items(prod_tmpl_ids, prod_ids, categ_ids, date)
        # Build Results
        results = self._generate_pricelist_results(products_qty_partner, date, items)
        return results

    def _prepare_datas(self, products_qty_partner, date, uom_id):
        if not date:
    ...
    def _generate_pricelist_results(self, products_qty_partner, date, items):
        results = {}
        for product, qty, partner in products_qty_partner:
            results[product.id] = 0.0
            suitable_rule = False
            qty_uom_id, qty_in_product_uom = self._get_uom_qty(product, qty)
            # if Public user tries to access standard price from website sale, need to call price_compute.
            # TDE SURPRISE: product can actually be a template
            price = product.price_compute('list_price')[product.id]
            price_uom = self.env['uom.uom'].browse([qty_uom_id])
            for rule in items:
                suitable_rule = self._check_rule_validity(rule, product, qty_in_product_uom)
                if suitable_rule:
                    price = self._get_price(suitable_rule, product, qty, partner, date, price_uom)
                    price = self._get_price_by_rule(suitable_rule, product, price, price_uom)
                    break
            final_price = self._convert_final_price(suitable_rule, product, price, date)
            results[product.id] = (final_price, suitable_rule and suitable_rule.id or False)
        return results
    ...
    def _get_uom_qty(self, product, qty):
        ...
        if qty_uom_id != product.uom_id.id:
            try:
                ...
                pass
        return qty_uom_id, qty_in_product_uom

    def _check_rule_validity(self, rule, product, qty_in_product_uom):
    ...
    def _get_price(self, rule, product, qty, partner, date, price_uom):
    ...
    def _get_price_by_rule(self, rule, product, price, price_uom):
        """ Compute the price according to compute_price  """
        if price is not False:
            if rule.compute_price == 'fixed':
                price = product._convert_to_price_uom(rule.fixed_price, price_uom)
                ...
                if rule.price_surcharge:
                    price_surcharge = product._convert_to_price_uom(rule.price_surcharge, price_uom)
                    price += price_surcharge
                if rule.price_min_margin:
                    price_min_margin = product._convert_to_price_uom(rule.price_min_margin, price_uom)
                    price = max(price, price_limit + price_min_margin)
                if rule.price_max_margin:
                    price_max_margin = product._convert_to_price_uom(rule.price_max_margin, price_uom)
                    price = min(price, price_limit + price_max_margin)
        return price

    def _convert_final_price(self, suitable_rule, product, price, date):
