...
class ProductUom(models.Model):
    ...
    active = fields.Boolean('Active',
        help="By unchecking the active field you can disable a unit of measure without deleting it.", default=True)
    ...
    def _compute_factor_inv(self):
        self.factor_inv = self._get_factor_inv(self.factor)

    def _compute_factor_inv_write(self):
        self.factor = self._get_factor_inv(self.factor)
    ...
    def name_create(self, name):
        ...
        categ_id = uom_categ.search([('name', '=', categ_misc)], limit=1).id
        if not categ_id:
            categ_id, _ = uom_categ.name_create(categ_misc)
        Productuom = self.create({self._rec_name: name,
                                        'category_id': categ_id,
                                        'factor': 1})
        return Productuom.name_get()[0]
    ...
    def _compute_qty(self, qty, to_uom_id=False, round=True, rounding_method='UP'):
        if not self.id or not qty or not to_uom_id:
            return qty
        to_uom = self.browse(to_uom_id)
        if to_uom.id == self.id:
    ...
    def _compute_price(self, price, to_uom_id=False):
        if (not self.id or not price or not to_uom_id
