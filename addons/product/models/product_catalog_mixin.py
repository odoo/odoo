...
class ProductCatalogMixin(models.AbstractModel):
    ...
    def _get_action_add_from_catalog_extra_context(self):
        label = _("Back")  # Default for inheriters that don't override "back_button_label" key
        if "state" in self._fields and isinstance(self.state, str):
            label = _("Back to Quotation") if any(s in self.state for s in ["draft", "sent"]) else _("Back to Order")
        return {
