...
class ProductPriceHistory(models.Model):
    ...
    def _get_default_company_id(self):
        return self._context.get('force_company', self.env['res.company']._get_current_company().id)
...
class ProductProduct(models.Model):
    ...
    def price_compute(self, price_type, uom=False, currency=False, company=False):
        ...
        if price_type == 'standard_price':
            ...
            products = self.with_context(force_company=company and company.id or self._context.get('force_company', self.env['res.company']._get_current_company().id)).sudo()
        ...
        for product in products:
            ...
            if currency:
                prices[product.id] = product.currency_id._convert(
                    prices[product.id], currency, product.company_id or self.env['res.company']._get_current_company(), fields.Date.today())
    ...
    def _set_standard_price(self, value):
        ...
        for product in self:
            PriceHistory.create({
                ...
                'cost': value,
                'company_id': self._context.get('force_company', self.env['res.company']._get_current_company().id),
            })
...
class SupplierInfo(models.Model):
    ...
    company_id = fields.Many2one(
        'res.company', 'Company',
        default=lambda self: self.env['res.company']._get_current_company().id, index=1)
