    ...
    def _price_rule_get_multi(self, products_by_qty_by_partner):
        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            if qty_uom_id != product.uom_id.id:
                try:
                    qty_in_product_uom = ProductUom.browse(self.env.context.get('uom'))._compute_qty(qty, product.uom_id.id)
                except UserError:
                    # Ignored - incompatible UoM in context, use default product UoM
                    pass

            # if Public user try to access standard price from website sale, need to call _price_get.
            price = ProductTemplate._price_get([product], 'list_price')[product.id]
            ...
            for rule in pricelist_items:
                ...
                if rule.base == 'pricelist' and rule.base_pricelist_id:
                    ...
                    price = ProductTemplate._price_get([product], rule.base)[product.id]
...
class ProductPricelistItem(models.Model):
    ...
    pricelist_id = fields.Many2one('product.pricelist', string='Pricelist', ondelete='cascade', index=True)
