...
class ProductProduct(models.Model):
    ...
    virtual_available = fields.Float(
        ...
        digits='Product Unit', help="Forecast quantity (Quantity On Hand - Outgoing + Incoming)")
    incoming_qty = fields.Float(
        'Incoming', compute='_compute_quantities', compute_sudo=False, search='_search_incoming_qty',
        digits='Product Unit', help="Quantity of planned incoming products")
    outgoing_qty = fields.Float(
        'Outgoing', compute='_compute_quantities', compute_sudo=False, search='_search_outgoing_qty',
        digits='Product Unit', help="Quantity of planned outgoing products.")
    ...
    def _compute_quantities(self):
        ...
        def need_forecast(product):
            return product.type != 'service' and product.is_storable
        products = self.with_context(prefetch_fields=False).filtered(need_forecast)
        forecasted = products._compute_forecasted_without_stock()
        for product in products:
            if forecasted[product.id]:
                product.update(forecasted[product.id])
