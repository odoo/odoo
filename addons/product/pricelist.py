...
class ProductPricelist(models.Model):
    ...
    def _default_get_item_ids(self):
        ProductPricelistItem = self.env['product.pricelist.item']
        fields_list = ProductPricelistItem._defaults.keys()
        vals = ProductPricelistItem.default_get(fields_list)
    ...
    item_ids = fields.One2many('product.pricelist.item', 'pricelist_id', string='Pricelist Items', copy=True, default=_default_get_item_ids)
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if name and operator == '=' and not args:
            ...
            pricelist_ids = [r[0] for r in cr.fetchall()]
            # regular search() to apply ACLs - may limit results below limit in some cases
            pricelists = self.search([('id', 'in', pricelist_ids)], limit=limit)
            if pricelists:
    ...
    def _price_rule_get_multi(self, products_by_qty_by_partner):
        self.ensure_one()
        date = self.env.context.get('date') and self.env.context['date'][0:10] or fields.Date.today()
        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            if suitable_rule and suitable_rule.compute_price != 'fixed' and suitable_rule.base != 'pricelist':
                price = self.env.user.company_id.currency_id.compute(price, self.currency_id)
