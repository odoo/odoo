...
class TestItEdiImport(TestItEdi):
    ...
    def test_transaction_id_several_bills_in_fewer_files(self):
        ...
        for m in created_moves:
            moves |= m
        for (filename, transaction_id), move in zip([
            ('IT01234567890_FPR03.xml', f'{1:0>11}'),
            ('IT01234567890_FPR02.xml.p7m', f'{2:0>11}'),
            ('IT01234567890_FPR03_2.xml', f'{1:0>11}'),
            ('IT01234567890_FPR02.xml_2.p7m', f'{2:0>11}'),
        ], moves):
            self.assertEqual(move.l10n_it_edi_attachment_id.name, filename)
            self.assertEqual(move.l10n_it_edi_transaction, transaction_id)
