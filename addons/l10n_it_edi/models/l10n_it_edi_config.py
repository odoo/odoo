...
class L10nItEdiConfig(models.Model):
    _name = "l10n_it_edi.config"
    _description = "A company's l10n it edi configuration"
    ...
    def _compute_l10n_it_edi_purchase_journal_id(self):
        missing = self.filtered(
            lambda config: (
                not config.l10n_it_edi_purchase_journal_id
                and config.company_id.country_code == "IT"
            )
        )
        ...
        for config in self:
            if config in missing:
                config.l10n_it_edi_purchase_journal_id = next(
                    (
                        journal
                        for journal in purchase_journals
                        if not journal.company_id
                        or journal.company_id == config.company_id
                    ),
                    self.env["account.journal"],
                )
            else:
                config.l10n_it_edi_purchase_journal_id = (
                    config.l10n_it_edi_purchase_journal_id
                )
    ...
    @api.depends(
        "company_id.account_edi_proxy_client_ids", "company_id.l10n_it_codice_fiscale"
    )
    def _compute_l10n_it_edi_proxy_user_id(self):
        for config in self:
            company = config.company_id
            edi_company = company._l10n_it_get_edi_company()
            config.l10n_it_edi_proxy_user_id = (
            ...
            if not config.l10n_it_edi_proxy_user_id and company.l10n_it_codice_fiscale:
                config.l10n_it_edi_proxy_user_id = self.env[
