...
class FetchmailServer(models.Model):
    ...
    def _search_edi_invoice(self, att_name, send_state=False):
        ...
        conditions = [
            ...
            ("edi_document_ids.edi_format_id.code", "=", "fattura_pa"),
            ("edi_document_ids.attachment_id.name", "=", att_name),
        ]
        if send_state:
    ...
    def fetch_mail(self):
        ...
        for server in self.filtered(lambda s: s.l10n_it_is_pec):
            ...
            try:
                ...
                dummy, data = imap_server.uid('search', None, *email_filter)
        ...
        for attachment in attachments:
            ...
            if attachment_ext != 'zip':
                if split_underscore[1] in ['RC', 'NS', 'MC', 'MT', 'EC', 'SE', 'NE', 'DT']:
                    ...
                    att_content_data = attachment.content.encode("utf-8")
    ...
    def _create_invoice_from_mail(self, att_content_data, att_name, from_address):
        ...
        existing = self.env['ir.attachment'].search([("name", "=", att_name), ("type", "=", "binary")])
        if existing:
        ...
        self.env['ir.attachment'].create({
            'name': att_name,
            'datas': base64.encodebytes(att_content_data),
            'type': 'binary'})
    ...
    def _message_receipt_invoice(self, receipt_type, attachment):
        ...
        try:
            tree = etree.fromstring(attachment.content.encode("utf-8"))
        except:
