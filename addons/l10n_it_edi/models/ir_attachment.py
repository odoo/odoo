...
def remove_extensions(filename):
    basename = filename
    for suffix in Path(filename).suffixes[::-1]:
...
class IrAttachment(models.Model):
    ...
    def _decode_edi_binary(self, filename, content):
        ...
        for file_data in to_process_old:
            ...
            if '.p7m' in suffixes or 'application/pkcs7-mime' in mimetype:
                ...
                if xml_content := remove_signature(file_data['content']):
                    if to_process_new := attachment._decode_edi_xml(
                        f"{remove_extensions(filename)}.xml",
                        xml_content
                    ):
