...
class ResConfigSettings(models.TransientModel):
    ...
    def _compute_l10n_it_edi_show_purchase_journal_id(self):
        for config in self:
            ...
            config.l10n_it_edi_show_purchase_journal_id = len(purchase_journals) >= 2
    ...
    def _set_l10n_it_edi_register(self):
        for config in self:
            ...
            if config.l10n_it_edi_register:  # Create a new proxy user or reactivate an existing one
                if not proxy_user:
                ...
                elif proxy_user and not proxy_user.active:  # if previously proxy_user has been created but deactivated, re-activate it
                    server_url = proxy_user._get_proxy_urls()['l10n_it_edi'][proxy_user.edi_mode]
                    proxy_user._make_request(f"{server_url}/api/l10n_it_edi/1/reactivate_user")
                    proxy_user.active = True

            else:  # Deactivate the current activated proxy user
                if proxy_user and proxy_user.active:
                    server_url = proxy_user._get_proxy_urls()['l10n_it_edi'][proxy_user.edi_mode]
                    proxy_user._make_request(f"{server_url}/api/l10n_it_edi/1/deactivate_user")
