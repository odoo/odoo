...
class AccountMove(models.Model):
    ...
    def _l10n_it_edi_send(self, attachments_vals):
        ...
        def _handle_move_result(move, header=None, post_message=None, state=None, id_transaction=None):
            if header is not None:
                move.l10n_it_edi_header = header
            if post_message is not None:
                move.sudo().message_post(body=post_message)
        ...
        for filename, vals in results.items():
            ...
            if 'error' in vals:
                ...
                _handle_move_result(move, state=False, id_transaction=False, header=header)
            else:
                id_transaction = vals.get('id_transaction')
                if vals['id_transaction'] == 'demo':
                    ...
                    message = nl2br(_("The e-invoice file %s was signed and sent to the SdI for processing.", filename))
                else:
                    message = _("The e-invoice file %s was sent to the SdI for processing.", filename)
                _handle_move_result(move, state='processing', id_transaction=id_transaction, header=message, post_message=message)
        return results
    ...
    def _l10n_it_edi_upload_error_message(self, error_code, error_description):
        ...
        errors_map = {
            ...
            'OOQE': _('Daily XML signature quota exceeded per user'),
