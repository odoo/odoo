...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_default_extra_edi_checkboxes(self, move):
        ...
        if self._is_it_edi_applicable(move):
            extra_edi['it_edi_send'] = {'checked': True, 'readonly': False, 'label': _('Send to Tax Agency'), 'help': _('Send the e-invoice XML to the Italian Tax Agency.')}
        return extra_edi
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if it_moves := moves.filtered(lambda m: moves_data[m]['sending_method'] == 'it_edi_send' or moves_data[m]['invoice_edi_format'] == 'it_edi_xml'):
    ...
    def _hook_invoice_document_before_pdf_report_render(self, invoice, invoice_data):
        ...
        if (
                ('it_edi_send' in invoice_data['extra_edi'] and not invoice.l10n_it_edi_attachment_id)
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for move, move_data in invoices_data.items():
            if 'it_edi_send' in move_data['extra_edi']:
                moves |= move
