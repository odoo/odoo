...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_it_edi_check_taxes_configuration(self, invoice):
        """
            Can be overridden by submodules like l10n_it_edi_extended, which also allows for withholding and pension_fund taxes.
        """
        errors = []
    ...
    def _l10n_it_edi_import_search_tax(self, company, percentage, extra_domain=None):
        ...
        taxes = self.env['account.tax'].search(conditions).filtered(
            lambda x: x.invoice_repartition_line_ids.mapped("factor_percent") == [100.0, 100.0])
    ...
    def _l10n_it_edi_get_extra_info(self, company, document_type, body_tree, extra_info):
        ...
        extra_info['simplified'] = self._l10n_it_is_simplified_document_type(document_type)
        return []
    ...
    def _import_fattura_pa(self, tree, invoice):
        ...
        for body_tree in tree.xpath('//FatturaElettronicaBody'):
            ...
            with Form(invoice_ctx) as invoice_form:
                ...
                extra_info = {}
                message_to_log = []
                message_to_log += self._l10n_it_edi_get_extra_info(company, document_type, body_tree, extra_info)
    ...
    def _import_fattura_pa_line(self, element, invoice_line_form, extra_info=None):
        ...
        if percentage is not None:
            ...
            tax = self._l10n_it_edi_import_search_tax(company, percentage, conditions)
            if tax:
        ...
        for other_data_element in element.xpath('.//AltriDatiGestionali'):
            ...
            enasarco_percentage = round(enasarco_amount / price_subtotal * 100)
            enasarco_tax = self._l10n_it_edi_import_search_tax(company, enasarco_percentage, [
                ('l10n_it_pension_fund_kind', '=', 'TC07'),
            ])
            if enasarco_tax:
                invoice_line_form.tax_ids.add(enasarco_tax)
            else:
                message_to_log.append("%s<br/>%s" % (
