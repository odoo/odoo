...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_it_edi_is_self_invoice(self, invoice):
        report_lines_xmlids = (
            invoice.line_ids
               ...
               .mapped("tag_ids")
               .mapped("tax_report_line_ids")
               .mapped(lambda x: x.get_external_id().get(x.id, '')))
    ...
    def _l10n_it_edi_check_ordinary_invoice_configuration(self, invoice):
        ...
        seller, buyer = invoice.company_id, invoice.commercial_partner_id
    ...
    def _l10n_it_edi_services_or_goods(self, invoice):
        product_lines = invoice.invoice_line_ids.filtered(lambda p: not p.display_type and p.product_id)
        services_or_goods = set([l.product_id.type for l in product_lines])
        if services_or_goods == set(['consu', 'service']):
            return "both"
        return services_or_goods and services_or_goods.pop() or "both"
    ...
    def _l10n_it_document_type_mapping(self):
        return {
            ...
            'TD17': dict(move_type='in_invoice', import_type='out_invoice', self_invoice=True, services_or_goods="services"),
