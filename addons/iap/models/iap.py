...
class IapAccount(models.Model):
    ...
    company_id = fields.Many2one('res.company', default=lambda self: self.env['res.company']._get_current_company())

    @api.model
    def get(self, service_name, force_create=True):
        account = self.search([('service_name', '=', service_name), ('company_id', 'in', [self.env['res.company']._get_current_company().id, False])])
