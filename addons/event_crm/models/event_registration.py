...
class EventRegistration(models.Model):
    ...
    def write(self, vals):
        ...
        self._update_lead_values(vals, old_registration_vals)
    ...
    def _get_lead_description(self, prefix):
        ...
        return "\n%s. %s (%s)\n" % (prefix, self.name, " - ".join(info_registration))

    def _update_lead_values(self, vals, old_registration_vals):
        # get fields that are updated
        lead_update_fields = [key for key, value in vals.items() if old_registration_vals.get(key) != value]
        ...
        if lead_update_fields:
            for lead in self.lead_ids:
                ...
                # don't use partner_id if there is more than 1 registration created by an "attendee" rule
                if len(self) > 1 and lead.event_lead_rule_id.lead_creation_basis == 'attendee' and lead_values.get('partner_id'):
                    # if the registration was created as public user, then we use some of the info of the new partner to update the lead
                    if old_registration_vals.get('partner_id') == self.env.ref('base.public_partner').id and not lead.email_from:
                        lead_values.update({
                            'name': "%s - %s" % (self.event_id.name, self.partner_id.name),
                            ...
                            'description': "%s\n%s" % (_("Participant:"), self[0]._get_lead_description(1)),
                        })
                    else:
                        lead_values.update({
                            'partner_id': False,
                            'contact_name': lead.registration_ids.name,
                        })
                # if registrations was created as public user, recompute the description for lead created by "per order" rule.
                elif lead.event_lead_rule_id.lead_creation_basis == 'order' and lead_values.get('partner_id'):
                    if old_registration_vals.get('partner_id') == self.env.ref('base.public_partner').id:
                ...
                # update the name of the lead accordingly
                if lead_values.get('partner_id'):
                    lead_values['name'] = "%s - %s" % (self.event_id.name, self.partner_id.name)
                elif 'name' in lead_update_fields and (
                    (lead.event_lead_rule_id.lead_creation_basis == 'order' and not self.partner_id) or
                    lead.event_lead_rule_id.lead_creation_basis == 'attendee'
                ):
                    lead_values['name'] = "%s - %s" % (self.event_id.name, self.name)
                elif lead.event_lead_rule_id.lead_creation_basis == 'order' and self.partner_id:
