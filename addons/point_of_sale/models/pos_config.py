...
class PosConfig(models.Model):
    ...
    def _get_config_count(self):
        return self.env['pos.config'].search_count([('company_id', '=', self.env.company.id)])
    ...
    def load_onboarding_clothes_scenario(self, with_demo_data=True):
        ...
        if with_demo_data or self._get_config_count() != 1:
            config._load_onboarding_clothes_demo_data(with_demo_data)
        return {'config_id': config.id}

    def _load_onboarding_clothes_demo_data(self, load_products=True):
        self.ensure_one()
    ...
    def _load_onboarding_bakery_demo_data(self, load_products=True):
        self.ensure_one()
    ...
    def _load_onboarding_furniture_demo_data(self, load_products=False):
        self.ensure_one()
