...
import odoo.release
from odoo import Command, SUPERUSER_ID, api, fields, models, tools
    ...
    def _check_company_payment(self):
        for config in self:
            if self.env['pos.payment.method'].search_count([('id', 'in', config.payment_method_ids.ids), ('company_id', '!=', config.company_id.id)]):
                raise ValidationError(self.env._("The payment methods for the point of sale %s must belong to its company.", config.name))
    ...
    def copy_data(self, default=None):
        ...
        if 'name' not in default:
            for config, vals in zip(self, vals_list):
                vals['name'] = self.env._("%s (copy)", config.name)
        return vals_list
