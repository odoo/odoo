...
class PosSession(models.Model):
    ...
    def _add_pos_taxes_computation(self, loaded_data):
        ...
        # Prepare the taxes computation for each product.
        all_tax_ids = set()
        for product_data in loaded_data['product.product']:
            ...
            taxes_computation['product'][product_data['id']] = taxes_key
            taxes_computation['taxes'][taxes_key] = {'tax_ids': tax_ids}

        # Prefetch the taxes.
        self.env['account.tax'].with_prefetch(list(all_tax_ids))

        # Prepare the taxes computation for each fiscal position.
        fp_ids = set(fp_data['id'] for fp_data in loaded_data['account.fiscal.position'])
        fiscal_positions = self.env['account.fiscal.position'].browse(list(fp_ids))
        for fiscal_position in fiscal_positions:
            for taxes_key, taxes_data in list(taxes_computation['taxes'].items()):
                taxes = self.env['account.tax'].browse(taxes_data['tax_ids'])
                ...
                taxes_computation['fiscal_position'].setdefault(fiscal_position.id, {})
                taxes_computation['fiscal_position'][fiscal_position.id][taxes_key] = new_taxes_key
                taxes_computation['taxes'][new_taxes_key] = {'tax_ids': new_tax_ids}
        ...
        for taxes_data in taxes_computation['taxes'].values():
            taxes = self.env['account.tax'].browse(taxes_data['tax_ids'])
