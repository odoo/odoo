...
class PosSession(models.Model):
    ...
    def _load_pos_data_models(self, config_id):
        return ['pos.config', 'pos.order', 'pos.order.line', 'pos.pack.operation.lot', 'pos.payment', 'pos.payment.method', 'pos.printer',
            ...
            'account.cash.rounding', 'account.fiscal.position', 'account.fiscal.position.tax', 'stock.picking.type', 'res.currency', 'pos.note', 'ir.ui.view', 'product.tag']
    ...
    def _should_capture_order(self, order: dict, is_draft: bool, is_paid: bool):
        def early_return(reason: str):
            _logger.info("order '%s' was not captured as %s", order.get('name'), reason)
            return False
    ...
    def _handle_order_process_fail(self, order: dict, exception: Exception):
    ...
    def _process_order_process_fail(self, order: dict, exception: Exception, uid: int = False):
        current_pos_order_data_hash = hash(PoSOrderData(order))
