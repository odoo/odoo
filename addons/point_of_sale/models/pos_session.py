...
class PosSession(models.Model):
    ...
    def _close_session_action(self):
        try:
            self_ctx = self.with_company(self.company_id)
            amount_residual = self_ctx._validate_session_accounting(
                check_validity=False,
            )
        except UserError:
            # Rollback the transaction to avoid any change in the database
            # as we just want to check the residual amount to balance
            self.env.cr.rollback()

        default_account = self._get_balancing_account()
        wizard = self.env['pos.close.session.wizard'].create({
            'amount_to_balance': amount_residual,
    ...
    def action_pos_session_closing_control(self):
        ...
        if not result['status'] and result['type'] == 'accounting_error':
            self.env.cr.rollback()
            # Rollback to avoid any change in the database as we just
            # want to show the error message and maybe open the reconciliation widget
    ...
    def delete_cash_in_out(self, absl_id, partner_id):
        ...
        if absl not in self.bank_statement_line_ids:
    ...
    def _handle_cash_statement_entries(self, payment_method_closing={}):
        ...
        if not float_is_zero(difference, precision_rounding=rounding):
            ...
            cash_pm._create_cash_payment_line(
                self,
                difference,
                partner,
                message,
    ...
    def _validate_session_accounting(self, check_validity=True):
        ...
        sale_orders = non_invoiced_orders.filtered(
            lambda order: not order.is_refund and order.amount_total > 0,
        )
        refund_orders = non_invoiced_orders - sale_orders
        sales_move = self._create_session_account_move(sale_orders, check_validity)
        refunds_move = self._create_session_account_move(refund_orders, check_validity)
    ...
    def _create_session_account_move(self, orders, check_validity=True):
        ...
        refund = orders[0].is_refund  # All orders are refunds or not
        ...
        lines = orders._prepare_account_move_line_data()
        ...
        move_ctx = move.with_context(
            ...
            check_move_validity=check_validity,
        )
        ...
        payment_lines = self.env['pos.order']._create_payment_moves(
            self,
            payments,
        )
    ...
    def _prepare_account_move_line_commands_for_reversal(self, order, invoice_to_reverse):
        ...
        for line in product_lines:
            reverse_move_lines.append(Command.create({
                'name': _("Partial reversal of %s", line.name),
    ...
    def _create_partial_reversal_move_from_session_closing(self, order):
        ...
        if rounding_line:
            ...
            reverse_move_lines.append(Command.create({
                'name': _("Partial reversal of rounding: %s", matching_line.name),
                'account_id': matching_line.account_id.id,
                'partner_id': matching_line.partner_id.id,
                'currency_id': order.company_id.currency_id.id,
        ...
        for idx, payment in enumerate(payment_lines):
            ...
            receivable_line = Command.create({
                'name': _("Reversal of payment: %s", matching_line.name),
                'account_id': matching_line.account_id.id,
                'partner_id': matching_line.partner_id.id,
                'currency_id': order.company_id.currency_id.id,
        ...
        return move_ctx.create({
            ...
            'ref': _("Partial reversal of invoice %s", original_move.name),
