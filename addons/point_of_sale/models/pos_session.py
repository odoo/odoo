...
class PosSession(models.Model):
    ...
    def _confirm_orders(self):
        ...
        for session in self:
            ...
            move_id = PosOrder.with_context(local_context)._create_account_move(
                session.start_at, session.name, session.config_id.journal_id.id, company_id)
    ...
    @api.depends('cash_control', 'cash_journal_id')
    ...
    def create(self, values):
        config_id = values.get(
            'config_id') or self.env.context.get('default_config_id')
        if not config_id:
