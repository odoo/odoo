...
class PosSession(models.Model):
    ...
    def get_session_orders(self, session):
        return session.order_ids

    def action_pos_session_closing_control(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        bank_payment_method_diffs = bank_payment_method_diffs or {}
        for session in self:
            if any(order.state == 'draft' for order in self.get_session_orders(session)):
    ...
    def _validate_session(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        ...
        if self.get_session_orders(self).filtered(lambda o: o.state != 'cancel') or self.sudo().statement_line_ids:
        ...
        if self.config_id.order_edit_tracking:
            edited_orders = self.get_session_orders(self).filtered(lambda o: o.is_edited)
    ...
    def close_session_from_ui(self, bank_payment_method_diff_pairs=None):
        ...
        open_order_ids = self.get_session_orders(self).filtered(lambda o: o.state == 'draft').ids
    ...
    def _cannot_close_session(self, bank_payment_method_diffs=None):
        ...
        """
        bank_payment_method_diffs = bank_payment_method_diffs or {}
        if any(order.state == 'draft' for order in self.get_session_orders(self)):
    ...
    def _check_if_no_draft_orders(self):
        draft_orders = self.get_session_orders(self).filtered(lambda order: order.state == 'draft')
        if draft_orders:
            raise UserError(_(
