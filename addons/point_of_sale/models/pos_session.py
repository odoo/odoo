...
class PosSession(models.Model):
    ...
    def _get_diff_vals(self, payment_method_id, diff_amount, account_payment=False):
        payment_method = self.env['pos.payment.method'].browse(payment_method_id)
        diff_compare_to_zero = self.currency_id.compare_amounts(diff_amount, 0)
        source_account = payment_method.outstanding_account_id or account_payment.outstanding_account_id
    ...
    def _apply_diff_on_account_payment_move(self, account_payment, payment_method, diff_amount):
        diff_vals = self._get_diff_vals(payment_method.id, diff_amount, account_payment)
        if not diff_vals:
            return
