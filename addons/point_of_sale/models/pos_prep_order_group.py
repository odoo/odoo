...
class PosPrepOrderGroup(models.Model):
    ...
    _description = 'Handle groups of preparation orders when splitting orders'
    _inherit = ['pos.load.mixin']

    pos_order_ids = fields.One2many('pos.order', 'prep_order_group_id', string='Orders')
    prep_order_ids = fields.One2many('pos.prep.order', 'prep_order_group_id', string='Preparation Orders')
    ...
    def update_last_order_change(self, order):
        ...
        """
        prep_lines_by_key = {}
        prep_group = order.prep_order_group_id or self.env['pos.prep.order.group'].create({
            'pos_order_ids': [(4, order.id)],
        })
        prep_order = self.env['pos.prep.order'].create({
            'pos_order_id': order.id,
            'prep_order_group_id': prep_group.id,
        })

        for line in prep_group.prep_order_ids.prep_line_ids:
