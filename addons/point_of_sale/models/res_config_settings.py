...
class ResConfigSettings(models.TransientModel):
    ...
    sale_tax_id = fields.Many2one('account.tax', string="Default Sale Tax", related='company_id.account_sale_tax_id', related_inverse=True)
    ...
    update_stock_quantities = fields.Selection(related="company_id.point_of_sale_update_stock_quantities", related_inverse=True)
    account_default_pos_receivable_account_id = fields.Many2one(string='Default Account Receivable (PoS)', related='company_id.account_default_pos_receivable_account_id', related_inverse=True)
    is_default_pricelist_displayed = fields.Boolean(compute="_compute_pos_pricelist_id", compute_sudo=True)
    barcode_nomenclature_id = fields.Many2one('barcode.nomenclature', related='company_id.nomenclature_id', related_inverse=True)

    # pos.config fields
    pos_module_pos_discount = fields.Boolean(related='pos_config_id.module_pos_discount', related_inverse=True)
    pos_module_pos_hr = fields.Boolean(related='pos_config_id.module_pos_hr', related_inverse=True)
    pos_module_pos_restaurant = fields.Boolean(related='pos_config_id.module_pos_restaurant', related_inverse=True)
    pos_is_order_printer = fields.Boolean(compute='_compute_pos_printer', store=True, readonly=False)
    pos_printer_ids = fields.Many2many(related='pos_config_id.printer_ids', related_inverse=True)

    pos_allowed_pricelist_ids = fields.Many2many('product.pricelist', compute='_compute_pos_allowed_pricelist_ids')
    pos_amount_authorized_diff = fields.Float(related='pos_config_id.amount_authorized_diff', related_inverse=True)
    ...
    pos_cash_rounding = fields.Boolean(related='pos_config_id.cash_rounding', related_inverse=True, string="Cash Rounding (PoS)")
    pos_company_has_template = fields.Boolean(related='pos_config_id.company_has_template')
    pos_default_bill_ids = fields.Many2many(related='pos_config_id.default_bill_ids', related_inverse=True)
    ...
    pos_iface_big_scrollbars = fields.Boolean(related='pos_config_id.iface_big_scrollbars', related_inverse=True)
    pos_iface_cashdrawer = fields.Boolean(string='Cashdrawer', compute='_compute_pos_iface_cashdrawer', readonly=False, store=True)
    pos_iface_customer_facing_display_local = fields.Boolean(related='pos_config_id.iface_customer_facing_display_local', related_inverse=True)
    ...
    pos_iface_customer_facing_display_background_image_1920 = fields.Image(related='pos_config_id.iface_customer_facing_display_background_image_1920', related_inverse=True)
    ...
    pos_iface_print_auto = fields.Boolean(related='pos_config_id.iface_print_auto', related_inverse=True)
    pos_iface_print_skip_screen = fields.Boolean(related='pos_config_id.iface_print_skip_screen', related_inverse=True)
    ...
    pos_iface_tax_included = fields.Selection(related='pos_config_id.iface_tax_included', related_inverse=True)
    pos_iface_tipproduct = fields.Boolean(related='pos_config_id.iface_tipproduct', related_inverse=True)
    pos_invoice_journal_id = fields.Many2one(related='pos_config_id.invoice_journal_id', related_inverse=True)
    pos_is_header_or_footer = fields.Boolean(related='pos_config_id.is_header_or_footer', related_inverse=True)
    pos_is_margins_costs_accessible_to_every_user = fields.Boolean(related='pos_config_id.is_margins_costs_accessible_to_every_user', related_inverse=True)
    pos_is_posbox = fields.Boolean(related='pos_config_id.is_posbox', related_inverse=True)
    pos_journal_id = fields.Many2one(related='pos_config_id.journal_id', related_inverse=True)
    pos_limit_categories = fields.Boolean(related='pos_config_id.limit_categories', related_inverse=True)
    pos_manual_discount = fields.Boolean(related='pos_config_id.manual_discount', related_inverse=True)
    pos_only_round_cash_method = fields.Boolean(related='pos_config_id.only_round_cash_method', related_inverse=True)
    pos_other_devices = fields.Boolean(related='pos_config_id.other_devices', related_inverse=True)
    pos_payment_method_ids = fields.Many2many(related='pos_config_id.payment_method_ids', related_inverse=True)
    pos_picking_policy = fields.Selection(related='pos_config_id.picking_policy', related_inverse=True)
    pos_picking_type_id = fields.Many2one(related='pos_config_id.picking_type_id', related_inverse=True)
    ...
    pos_restrict_price_control = fields.Boolean(related='pos_config_id.restrict_price_control', related_inverse=True)
    pos_rounding_method = fields.Many2one(related='pos_config_id.rounding_method', related_inverse=True)
    pos_route_id = fields.Many2one(related='pos_config_id.route_id', related_inverse=True)
    ...
    pos_set_maximum_difference = fields.Boolean(related='pos_config_id.set_maximum_difference', related_inverse=True)
    pos_ship_later = fields.Boolean(related='pos_config_id.ship_later', related_inverse=True)
    pos_start_category = fields.Boolean(related='pos_config_id.start_category', related_inverse=True)
    pos_tax_regime_selection = fields.Boolean(related='pos_config_id.tax_regime_selection', related_inverse=True)
    ...
    pos_use_pricelist = fields.Boolean(related='pos_config_id.use_pricelist', related_inverse=True)
    pos_warehouse_id = fields.Many2one(related='pos_config_id.warehouse_id', related_inverse=True, string="Warehouse (PoS)")
    point_of_sale_use_ticket_qr_code = fields.Boolean(related='company_id.point_of_sale_use_ticket_qr_code', related_inverse=True)
    pos_auto_validate_terminal_payment = fields.Boolean(related='pos_config_id.auto_validate_terminal_payment', related_inverse=True, string="Automatically validates orders paid with a payment terminal.")
    pos_trusted_config_ids = fields.Many2many(related='pos_config_id.trusted_config_ids', related_inverse=True)
    point_of_sale_ticket_unique_code = fields.Boolean(related='company_id.point_of_sale_ticket_unique_code', related_inverse=True)
