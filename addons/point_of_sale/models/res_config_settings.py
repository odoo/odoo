...
class ResConfigSettings(models.TransientModel):
    ...
    @api.depends('pos_iface_discount', 'pos_config_id')
    def _compute_pos_discount_default_value(self):
        for res_config in self:
            if res_config.pos_iface_discount:
                product = self.env.ref("point_of_sale.product_product_consumable", raise_if_not_found=False)
                res_config.pos_discount_product_id = product if product and (not product.company_id or product.company_id == res_config.pos_config_id.company_id) else False
            else:
