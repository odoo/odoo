...
class ResPartner(models.Model):
    ...
    def _compute_application_statistics_hook(self):
        ...
        for partner in self.filtered('pos_order_count'):
            stat_info = {'icon': 'shopping_bag', 'value': partner.pos_order_count, 'label': _('Shopping cart')}
    ...
    def _load_pos_data_domain(self, data):
        ...
        loaded_order_partner_ids = set(data.get('pos.order', self.env['pos.order']).partner_id.ids)
    ...
    def _load_pos_metadata(self, data, search_params={}):
        if not search_params.get('offset') or search_params.get('domain'):
            return super()._load_pos_metadata(data, search_params)

        # Get new partners for a given offset (and no domain)
        ...
        data[self._name] = {
            ...
            'records': self.search(domain, limit=search_params.get('limit', False)),
        }
        return data
    ...
    def _load_pos_data_fields(self, config):
        return [
            ...
            'additional_identifiers', 'available_additional_identifiers_metadata',
        ]
