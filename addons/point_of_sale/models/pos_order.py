...
class PosOrder(models.Model):
    ...
    def _compute_is_invoiced(self):
        for order in self:
            ...
            order.is_singly_invoiced = bool(order_move and order_move not in session_moves)
            order.is_globally_invoiced = bool(order_move and order_move in session_moves)
    ...
    def _get_order_tax_totals(self):
        ...
        use_rounding = self.payment_ids.filtered_domain([
    ...
    def _prepare_account_move_line_data_for_payments(self, partner=None):
        ...
        for pm, amount in combined.items():
            ...
            pm_account = pm.receivable_account_id or session._get_receivable_account()
            destination_account = partner_account or pm_account
    ...
    def _create_payment_moves(self, session, payments, partner=None):
        ...
        for pm_data in payments:
            ...
            if pm.type == 'cash':
                all_payment_lines |= pm._create_cash_payment_line(**arguments)
            elif pm.type == 'bank':
                all_payment_lines |= pm._create_bank_payment_line(**arguments)
    ...
    def _prepare_invoice_vals(self):
        ...
        is_refund = any(order.is_refund for order in self)
        is_total_negative = sum(order.amount_total for order in self) < 0
        move_type = 'out_refund' if is_refund or is_total_negative else 'out_invoice'
        ...
        check_rounding = config.cash_rounding and not config.only_round_cash_method
        is_cash_payments = any(p.payment_method_id.type == 'cash' for p in self.payment_ids)
    ...
    def _generate_pos_order_invoice(self):
        if not self.env['res.company']._with_locked_records(self, allow_raising=False):
            raise UserError(_("Some orders are dalready being invoiced. Please try again later."))
    ...
    def _generate_invoice_after_session_closing(self):
        ...
        # Create misc move to balance the invoice with the correct amount,
        # with a counterpart on the same account as the session closing
        # entry, so that the fiscal report is correct.
        sign = -1 if self.is_refund else 1
        counter_strike = self.env['account.move'].sudo().with_context(
            default_move_type='entry',
        ).create({
            'ref': _("Account transfer for invoice %s", invoice.name),
            'date': invoice.invoice_date,
            'journal_id': global_move.journal_id.id,
            'line_ids': [
                Command.create({
                    'name': _("Counterpart for invoice %s", invoice.name),
                    'account_id': invoice.partner_id.property_account_receivable_id.id,
                    'balance': -invoice.amount_total * sign,
                }),
                Command.create({
                    'name': _("Reversal for invoice %s payment", invoice.name),
        ...
        counter_strike.action_post()
        invoice_payment_line = invoice.line_ids.filtered(
            lambda line: line.display_type == 'payment_term',
        )
        line_for_invoice = counter_strike.line_ids.filtered(
            lambda line: line.account_id == invoice_payment_line.account_id,
        )

        if invoice_payment_line and line_for_invoice:
            to_reconcile = invoice_payment_line | line_for_invoice
            to_reconcile.with_context(skip_invoice_sync=True).reconcile()
