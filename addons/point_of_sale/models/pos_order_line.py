...
class PosOrderLine(models.Model):
    ...
    def _compute_amount_line_all(self, qty=None):
        ...
        taxes = tax_ids_after_fiscal_position
        if self.price_type == 'override':
            taxes = taxes.with_context(force_price_include=True)
        taxes = taxes.compute_all(price, self.order_id.currency_id, line_qty, product=self.product_id, partner=self.order_id.partner_id)
        return {
