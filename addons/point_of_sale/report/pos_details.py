...
class ReportPosDetails(models.AbstractModel):
    ...
    def _get_invoice(self, inv_id):
        if inv_id:
            self.env.cr.execute("SELECT number FROM account_invoice as ac where id = %s", (inv_id,))
            return self.env.cr.fetchone()[0] or 'Draft'
        else:
            return ''
    ...
    def _pos_sales_details(self, form):
        ...
        pos_orders = self.env['pos.order'].search([('date_order', '>=', form['date_start'] + ' 00:00:00'), ('date_order', '<=', form['date_end'] + ' 23:59:59'), ('user_id', 'in', user_ids), ('state', 'in', ['done', 'paid', 'invoiced']), ('company_id', '=', self.env.user.company_id.id)])
    ...
    def _get_sum_invoice_2(self, form):
        user_ids = form['user_ids'] or self._get_all_users()
        pos_orders = self.env['pos.order'].search([('date_order', '>=', form['date_start'] + ' 00:00:00'), ('date_order', '<=', form['date_end'] + ' 23:59:59'), ('user_id', 'in', user_ids), ('company_id', '=', self.env.user.company_id.id), ('invoice_id', '<>', False)])
        for order in pos_orders:
            for pol in order.lines:
    ...
    def _get_sum_discount(self, form):
        ...
        user_ids = form['user_ids'] or self._get_all_users()
        pos_orders = self.env['pos.order'].search([('date_order', '>=', form['date_start'] + ' 00:00:00'), ('date_order', '<=', form['date_end'] + ' 23:59:59'), ('user_id', 'in', user_ids), ('company_id', '=', self.env.user.company_id.id)])
        for order in pos_orders:
            for pol in order.lines:
    ...
    def _get_payments(self, form):
        ...
        user_ids = form['user_ids'] or self._get_all_users()
        pos_orders = self.env['pos.order'].search([('date_order', '>=', form['date_start'] + ' 00:00:00'), ('date_order', '<=', form['date_end'] + ' 23:59:59'), ('state', 'in', ['paid', 'invoiced', 'done']), ('user_id', 'in', user_ids), ('company_id', '=', self.env.user.company_id.id)])
        data = {}
        if pos_orders:
            statement_line = StatementLine.search([('pos_statement_id', 'in', pos_orders.ids)])
            if statement_line:
                a_l = []
                for r in statement_line:
                    a_l.append(r['id'])
                self.env.cr.execute("select aj.name,sum(amount) from account_bank_statement_line as absl,account_bank_statement as abs,account_journal as aj " \
    ...
    def _total_of_the_day(self):
        return self.total or 0.00

    def _sum_invoice(self, records):
        return reduce(lambda acc, obj: acc + obj.invoice_id.amount_total, [o for o in records if o.invoice_id and o.invoice_id.number], 0.0)
    ...
    def render_html(self, data=None):
        ...
        report = Report._get_report_from_name('point_of_sale.report_detailsofsales')
        docargs = {
            'doc_ids': self._ids,
