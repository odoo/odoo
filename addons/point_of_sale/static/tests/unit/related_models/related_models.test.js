...
describe("Related Model", () => {
    ...
    test("Keep raw datas not defined in models fields", () => {
        ...
        expect(order2._additionalField).not.toBeEmpty(); // Added to the record because it starts with _
        expect(order2._additionalField).toBe("Sample value");

        expect(order2.otherField).toBeEmpty(); // Not a model field because it does not start with _
        expect(order2.raw.otherField).toBeEmpty(); // Do not modify the raw data
    });
    ...
    test("x2many array is not modifiable", () => {
        ...
        expect(() => {
            order.lines.push("a");
        }).toThrow(/cannot be directly modified/i);
    });
    ...
    test("Adding record to the store should not compute getter", () => {
        ...
        expect(order.lines.length).toBe(1); // line 13 is  loaded  ...
    });
