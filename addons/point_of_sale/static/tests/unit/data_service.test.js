...
test("check that flushing works", async () => {
    defineModels({ PosSession, PosConfig });
    registry.category("services").content = Object.fromEntries(
        Object.entries(registry.category("services").content).filter(([name]) =>
            [
                "bus_service",
                "orm",
                "dialog",
