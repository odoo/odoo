...
describe("reAnnounce", () => {
    test("calls _webrtcAnnounce with id and device_uuid", async () => {
        const webrtc = await makeWebrtcService({ deviceUuid: "test-uuid" });
        let capturedArgs;
        patchWithCleanup(webrtc, {
            _webrtcAnnounce: async (peerId, deviceUuid) => {
                capturedArgs = [peerId, deviceUuid];
            },
        });

        await webrtc.reAnnounce();

        expect(capturedArgs).toEqual([webrtc.id, "test-uuid"]);
    });

    test("awaits _announcePending without calling _announce again when init is in-flight", async () => {
        const webrtc = await makeWebrtcService();
        webrtc._initialized = false;
        webrtc._announcePending = Promise.resolve();
        const calls = [];
        patchWithCleanup(webrtc, {
    ...
    test("calls _announce and sets _announcePending when not initialized and no pending announce", async () => {
        const webrtc = await makeWebrtcService();
        webrtc._initialized = false;
        webrtc._announcePending = null;
        const calls = [];
        patchWithCleanup(webrtc, {
            _announce: async () => calls.push("_announce"),
        });
    ...
    test("does not run _announce twice when called concurrently during _init", async () => {
        const webrtc = await makeWebrtcService();
        webrtc._initialized = false;
        webrtc._announcePending = null;
        ...
        patchWithCleanup(webrtc, {
            async _announce() {
                calls.push("_announce");
                await new Promise((r) => (resolveAnnounce = r));
                this._initialized = true;
            },
        });

        const first = webrtc.reAnnounce();
        const second = webrtc.reAnnounce(); // second call while first is still awaiting
        resolveAnnounce();
...
describe("_init", () => {
    test("calls _ensureAnnounced and returns the service instance", async () => {
        const webrtc = await makeWebrtcService();
        const calls = [];
        patchWithCleanup(webrtc, { _ensureAnnounced: async () => calls.push(true) });
    ...
    test("skips _ensureAnnounced and returns the service instance when group is not set", async () => {
        const webrtc = await makeWebrtcService();
        webrtc.group = null;
        webrtc._initialized = false;
        const calls = [];
...
describe("_ensureAnnounced", () => {
    test("calls _announce and clears _announcePending when none is in-flight", async () => {
        const webrtc = await makeWebrtcService();
        webrtc._announcePending = null;
        const calls = [];
        patchWithCleanup(webrtc, { _announce: async () => calls.push("_announce") });

        await webrtc._ensureAnnounced();

        expect(calls).toEqual(["_announce"]);
        expect(webrtc._announcePending).toBe(null);
    });

    test("awaits existing _announcePending without calling _announce again", async () => {
        const webrtc = await makeWebrtcService();
        let resolved = false;
        webrtc._announcePending = Promise.resolve().then(() => {
            resolved = true;
        });
        const calls = [];
        patchWithCleanup(webrtc, { _announce: async () => calls.push("_announce") });
    ...
    test("concurrent calls run _announce exactly once", async () => {
        const webrtc = await makeWebrtcService();
        webrtc._announcePending = null;
        let resolveAnnounce;
        const calls = [];
        ...
        const first = webrtc._ensureAnnounced();
        const second = webrtc._ensureAnnounced();
        resolveAnnounce();
        await Promise.all([first, second]);
...
describe("_announce", () => {
    test("announces, adds bus channel and starts heartbeat", async () => {
        const webrtc = await makeWebrtcService();
        webrtc.id = "peer-id";
        const deviceUuid = webrtc._deviceUuid;
        const channelCalls = [];
        ...
        patchWithCleanup(webrtc, {
            ...
            _startHeartbeat: () => heartbeatCalls.push(true),
        });
        ...
        expect(capturedArgs).toEqual(["peer-id", deviceUuid]);
        expect(webrtc.id).toBe("peer-id");
        expect(channelCalls).toEqual([odoo.access_token]);
...
describe("_webrtcAnnounce", () => {
    test("terminal posts to /pos/webrtc/announce with config_id, peer_id and device_uuid, no peer_group", async () => {
        const webrtc = await makeWebrtcService({ group: "terminal" });
        let capturedParams;
        onRpc("/pos/webrtc/announce", async (request) => {
            capturedParams = (await request.json()).params;
            return {};
        });

        await webrtc._webrtcAnnounce("peer-1", "device-1");

        expect(capturedParams).toEqual({
            config_id: webrtc.configId,
            peer_id: "peer-1",
            device_uuid: "device-1",
        });
    });

    test("customer_display posts to /pos_customer_display/webrtc/announce with access_token, no peer_group", async () => {
        const webrtc = await makeWebrtcService({
            group: "customer_display",
            deviceUuid: "device-1",
        });
        let capturedParams;
        onRpc("/pos_customer_display/webrtc/announce", async (request) => {
            capturedParams = (await request.json()).params;
            return {};
        });
        ...
        expect(capturedParams).toEqual({
            access_token: webrtc.busChannel,
            peer_id: "peer-1",
            device_uuid: "device-1",
        });
    });
...
describe("_webrtcSignal", () => {
    test("terminal posts to /pos/webrtc/signal with config_id and msg", async () => {
        const webrtc = await makeWebrtcService({ group: "terminal" });
        let capturedParams;
        onRpc("/pos/webrtc/signal", async (request) => {
            capturedParams = (await request.json()).params;
            return {};
        });

        await webrtc._webrtcSignal({ type: "ice", candidate: "a", to: "peer-1", from: webrtc.id });

        expect(capturedParams).toEqual({
            config_id: webrtc.configId,
            msg: { type: "ice", candidate: "a", to: "peer-1", from: webrtc.id },
        });
    });

    test("customer_display posts to /pos_customer_display/webrtc/signal with access_token and msg", async () => {
        const webrtc = await makeWebrtcService({
            group: "customer_display",
            deviceUuid: "device-1",
        });
        let capturedParams;
        onRpc("/pos_customer_display/webrtc/signal", async (request) => {
            capturedParams = (await request.json()).params;
            return {};
        });
...
describe("_onConnectionStateChange", () => {
    test("calls _closeConnection with different states", async () => {
        const webrtc = await makeWebrtcService();
        const calls = [];
        patchWithCleanup(webrtc, { _closeConnection: (peerId) => calls.push(peerId) });

        webrtc._onConnectionStateChange("peer-disconnected", "disconnected");
        webrtc._onConnectionStateChange("peer-failed", "failed");
        webrtc._onConnectionStateChange("peer-closed", "closed");
        webrtc._onConnectionStateChange("peer-connecting", "connecting");
        webrtc._onConnectionStateChange("peer-connected", "connected");
        webrtc._onConnectionStateChange("peer-new", "new");

        expect(calls).toEqual(["peer-failed", "peer-closed"]);
    });
...
describe("_onChannelMessage", () => {
    ...
    test("buffers a chunk message and does not call _onRemoteMessage until complete", async () => {
        const webrtc = await makeWebrtcService();
        ...
        webrtc._onChannelMessage(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 0, total: 2, data: "AB" })
        );

        expect(calls).toEqual([]);
    });

    test("calls _onRemoteMessage with the reassembled message once all chunks arrive", async () => {
        const webrtc = await makeWebrtcService();
        addFakePeer(webrtc, "peer-1");
        const calls = [];
        patchWithCleanup(webrtc, {
        ...
        webrtc._onChannelMessage(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 1, total: 2, data: "CD" })
        );

        expect(calls).toEqual([["peer-1", "ABCD"]]);
    });
});

describe("_reassembleIfChunk", () => {
    test("returns the raw string unchanged when it is not valid JSON", async () => {
        const webrtc = await makeWebrtcService();
        expect(webrtc._reassembleIfChunk("peer-1", "not-json")).toBe("not-json");
    });

    test("returns the raw string unchanged for a normal (non-chunk) message", async () => {
        const webrtc = await makeWebrtcService();
        const raw = JSON.stringify({ type: "ping" });
        expect(webrtc._reassembleIfChunk("peer-1", raw)).toBe(raw);
    });

    test("returns null when the peer has no connection entry", async () => {
        const webrtc = await makeWebrtcService();
        const raw = JSON.stringify({ type: "chunk", cid: "c1", index: 0, total: 2, data: "AB" });
        expect(webrtc._reassembleIfChunk("unknown-peer", raw)).toBe(null);
    });

    test("returns null while a chunked message is still incomplete", async () => {
        const webrtc = await makeWebrtcService();
        addFakePeer(webrtc, "peer-1");
        const raw = JSON.stringify({ type: "chunk", cid: "c1", index: 0, total: 2, data: "AB" });
        expect(webrtc._reassembleIfChunk("peer-1", raw)).toBe(null);
    });

    test("reassembles chunks in index order regardless of arrival order", async () => {
        const webrtc = await makeWebrtcService();
        addFakePeer(webrtc, "peer-1");
        webrtc._reassembleIfChunk(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 1, total: 3, data: "CD" })
        );
        webrtc._reassembleIfChunk(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 0, total: 3, data: "AB" })
        );
        const result = webrtc._reassembleIfChunk(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 2, total: 3, data: "EF" })
        );

        expect(result).toBe("ABCDEF");
    });

    test("clears the buffer after reassembling so it isn't reused by a later message", async () => {
        const webrtc = await makeWebrtcService();
        const peer = addFakePeer(webrtc, "peer-1");
        webrtc._reassembleIfChunk(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 0, total: 1, data: "AB" })
        );

        expect(peer.chunkBuffers.has("c1")).toBe(false);
    });

    test("tracks multiple concurrent chunked messages independently by cid", async () => {
        const webrtc = await makeWebrtcService();
        addFakePeer(webrtc, "peer-1");
        webrtc._reassembleIfChunk(
            "peer-1",
        ...
        const result1 = webrtc._reassembleIfChunk(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c2", index: 0, total: 1, data: "XY" })
        );
        expect(result1).toBe("XY");

        const result2 = webrtc._reassembleIfChunk(
            "peer-1",
            JSON.stringify({ type: "chunk", cid: "c1", index: 1, total: 2, data: "CD" })
        );
...
describe("_signal", () => {
    test("calls _webrtcSignal with message merged with to and from", async () => {
        const webrtc = await makeWebrtcService();
        let capturedMsg;
        patchWithCleanup(webrtc, {
            _webrtcSignal: async (msg) => {
                capturedMsg = msg;
            },
        });
...
describe("_onOffer", () => {
    ...
    test("rejects an offer when _shouldConnect is false for its group/deviceUuid", async () => {
        const webrtc = await makeWebrtcService(); // terminal
        patchWithCleanup(globalThis, { RTCPeerConnection: MockRTCPeerConnection });
    ...
    test("accepts an offer using its own group/deviceUuid when _shouldConnect is true", async () => {
        const webrtc = await makeWebrtcService({ deviceUuid: "display-x" }); // terminal
        patchWithCleanup(globalThis, { RTCPeerConnection: MockRTCPeerConnection });
        patchWithCleanup(webrtc, { _setupPeerConnection: () => {} });
