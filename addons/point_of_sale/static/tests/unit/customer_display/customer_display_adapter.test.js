...
test("the server is only notified while a customer display is connected", async () => {
    ...
    // A display shows up: it is now reachable, and asks for the current order
    // since it has nothing to show.
    const refreshId = store.customerDisplayRefreshId;
    store.customerDisplayAliveNotification({ device_uuid: getDeviceUuid(), needs_data: true });
    expect(store.customerDisplayRefreshId).toBe(refreshId + 1);
    ...
    store.customerDisplayAliveNotification({ device_uuid: getDeviceUuid(), needs_data: true });
    expect(store.customerDisplayRefreshId).toBe(refreshId + 2);
