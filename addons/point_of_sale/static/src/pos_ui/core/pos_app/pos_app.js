...
export class PosApp extends Component {
    setup() {
        ...
        this.env = env;
        this.env.services.pos = this.wowlEnv.services.pos;
        ...
        const listeners = [
            ["show-main-screen", this.__showScreen.bind(this)],
            ["toggle-mobile-searchbar", this._toggleMobileSearchBar.bind(this)],
            ["show-temp-screen", this.__showTempScreen.bind(this)],
            ["close-temp-screen", this.__closeTempScreen.bind(this)],
            ["close-pos", this._closePos.bind(this)],
            ["loading-skip-callback", () => this.env.proxy.stop_searching()],
            ["play-sound", this._onPlaySound.bind(this)],
            ["set-sync-status", this._onSetSyncStatus.bind(this)],
            ["show-notification", this._onShowNotification.bind(this)],
            ["close-notification", this._onCloseNotification.bind(this)],
            ["connect-to-proxy", this.connect_to_proxy.bind(this)],
        ];
        const root = useRef("root");
        useEffect(
            () => {
                for (const [type, handler] of listeners) {
                    root.el.addEventListener(type, handler);
                }
                return () => {
                    for (const [type, handler] of listeners) {
                        root.el.removeEventListener(type, handler);
                    }
                };
            },
            () => []
        ...
        this.pos.numberBuffer.activate();
    ...
    get startScreen() {
        if (this.state.uiState !== "READY") {
            console.warn(
                `Accessing startScreen of PoS application before 'state.uiState' is 'READY' is not recommended.`
            );
        }
    ...
    async start() {
        try {
            ...
            await this.dialog.add(ErrorTracebackPopup, {
                title,
                body,
    ...
    openCashControl() {
        if (this.shouldShowCashControl()) {
            this.dialog.add(CashOpeningPopup, { keepBehind: true });
        }
    }
    ...
    _showStartScreen() {
        ...
        this.__showScreen({ detail: { name, props } });
    }
...
PosApp.template = "point_of_sale.PosApp";
PosApp.components = {
    Transition,
