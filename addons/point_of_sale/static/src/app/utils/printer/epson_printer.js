...
const EPSON_ERRORS = {
    ...
    "DeviceNotFound": _t("Check the printer configuration for the 'Device ID' setting.\nIt should be set to: 'local_printer'"),
    ...
    'ERROR_CODE_REQUEST_BACKEND': _t("Failed to add receipt to print queue. Please check your internet connection and try again."),
    'ERROR_CODE_PRINTER_RESPONSE_TIMEOUT': _t("Printer took too long to respond with the printing result. Please check the printer and try again."),
    'ERROR_CODE_PRINTER_INACTIVE': _t("No printer is currently active for this POS config.\nPlease ensure that the printer is online and this url is set in its Server Direct Print configuration:\n"),
    'ERROR_CODE_PRINTER_OLD_FIRMWARE': _t("The printer firmware is too old, please update it on the printer settings page before using it in Point Of Sale"),
    'ERROR_CODE_PRINT_QUEUE_FULL': _t("Print queue is full, cannot add new receipt to print queue. Please retry."),
}
...
export class EpsonPrinter extends BasePrinter {
    ...
    epsonPrint(children) {
        ...
        if (this.layout_xml === "ServerDirectPrintLayout") {
            ...
            // The print job ID must have 1 to 30 characters and allowable characters are alphanumerical characters, underscore, hyphen and period.
     ...
     */
    openCashbox() {
        const pulse = epsonPrint([createElement("pulse")]);
...
export class EpsonServerDirectPrinter extends EpsonPrinter {
    ...
    setup({ epsonServerReceiptQueueUrl, posData, busService }) {
        super.setup(...arguments);
        // point_of_sale/epson_server_direct_print/${posAccessToken}/add_receipt_to_print_queue`
        this.epsonServerReceiptQueueUrl = epsonServerReceiptQueueUrl;
        ...
        posData.connectWebSocket(websocketChannel, (this.epsonServerDirectPrintNotification.bind(this)));
    }
    ...
    /*
        ...
        * This is used to handle the response from the printer after sending a print job.
    */
    epsonServerDirectPrintNotification(printer_response) {
        // The success/error notifications will not work with old firmware which doesn't handle job_id usage
        if (!printer_response?.printJobId) {
            printer_response.success = "false";
            printer_response.errorCode = "ERROR_CODE_PRINTER_OLD_FIRMWARE";
        }
        else if (printer_response.printJobId !== this.printJobId) {
            // Ignoring websocket notifications for other job ids
            return;
        }
        if (this.pendingPrintJobResolver) {
            this.pendingPrintJobResolver(printer_response);  // Resolves the printerResponsePromise
    ...
    */
    async sendPrintingJob(img) {
        ...
        const printerResponsePromise = new Promise((resolve) => {
            ...
            this.printJobTimeout = setTimeout(() => {
                resolve({'result': false, "errorCode": "ERROR_CODE_PRINTER_RESPONSE_TIMEOUT"});
            }, 15000);
        });
        const printJobId = this.printJobId;
        try {
            let printerResponse = await rpc(this.epsonServerReceiptQueueUrl, {
                job_id: printJobId,
                receipt_to_print: img,
            })
            if (!printerResponse.errorCode) {
                // Wait for the websocket notification to resolve
