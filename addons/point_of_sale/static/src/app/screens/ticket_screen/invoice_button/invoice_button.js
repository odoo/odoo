...
export class InvoiceButton extends Component {
    ...
    async _downloadInvoice(account_move_id) {
        try {
            await this.invoiceService.downloadPdf(account_move_id);
        } catch (error) {
    ...
    async _invoiceOrder() {
        ...
        if (this.isAlreadyInvoiced) {
            await this._downloadInvoice(order.account_move.id);
        ...
        await this.pos.data.read("pos.order", [orderId]);

        // Part 3: Download invoice.
        await this._downloadInvoice(res_id);
        this.props.onInvoiceOrder(orderId);
    }
    async click() {
        if (this.lock) {
