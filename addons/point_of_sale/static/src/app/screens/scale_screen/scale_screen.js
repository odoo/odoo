...
export class ScaleScreen extends Component {
    ...
    onMounted() {
        ...
        this._readScale();
        this.pricePerUom = this.productPrice; // Initialize price per UOM
    }
    onWillUnmount() {
    ...
    async _setWeight(tare = false) {
        if (!this.shouldRead) {
            return;
        }
        if (tare) {
            this.state.tareWeight = await this.hardwareProxy.readScale(true);
        } else {
    ...
    async handleTareButtonClick() {
        ...
        const tareWeight = await this.hardwareProxy.readScale(true);
