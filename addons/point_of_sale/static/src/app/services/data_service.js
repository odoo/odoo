...
export class PosData extends Reactive {
        ...
        Object.entries(models).forEach(([modelName, _]) => {
            models[modelName].addEventListener("create", ({ ids, vals }) => {
                ...
                this.queue.push([
                    "CREATE",
                    modelName,
                    omit(models[modelName].getBy("uuid", ids[0]).raw, "id"),
                ]);
                this.debouncedFlush();
            });
    ...
    async flush() {
        ...
        try {
            ...
            for (const [command, ...data] of this.queue) {
                if (command === "CREATE") {
                    this.indexedDB.create(data[0], [data[1]]);
                } else if (command === "UPDATE") {
                    if (data[2].active == false) {
                        this.indexedDB.delete(data[0], [data[1]]);
                    } else {
                        // this.indexedDB.create(
                        //     data[0],
                        //     this.models[data[0]].find((x) => x.uuid === data[1] || x.id === data[1])
                        //         .raw
                        // );
                        this.indexedDB.create(data[0], [data[2]]);
                    }
                } else if (command === "DELETE") {
                    this.indexedDB.delete(data[0], [data[1]]);
                }
