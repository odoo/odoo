...
export class PosStore extends Reactive {
    ...
    async addLineToCurrentOrder(vals, opts = {}, configure = true) {
        ...
        const productTemplate = vals.product_template_id;
        const values = {
        ...
        } else if (values.product_template_id.attribute_line_ids.length > 0) {
            // Verify price extra of variant products
            const priceExtra = values.product_template_id.attribute_line_ids
        ...
        // ---
        // This actions cannot be handled inside pos_order.js or pos_order_line.js
        if (values.product_template_id.isCombo() && configure) {
            const payload = await makeAwaitable(this.dialog, ComboConfiguratorPopup, {
                productTemplate: values.product_template_id,
            });
            ...
            const comboPrices = computeComboItems(
                values.product_template_id.product_variant_ids[0],
                payload,
                order.pricelist_id,
        ...
        // ---
        // This actions cannot be handled inside pos_order.js or pos_order_line.js
        if (values.product_template_id.isTracked() && configure) {
            const code = opts.code;
            let pack_lot_ids = {};
            const packLotLinesToEdit =
                (!values.product_template_id.isAllowOnlyOneLot() &&
                    this.get_order()
                        .get_orderlines()
                        .filter((line) => !line.get_discount())
                        .find((line) => line.product_id.id === values.product_template_id.id)
            ...
            if (code && code.type === "lot") {
                ...
                pack_lot_ids = await this.editLots(values.product_template_id, packLotLinesToEdit);
            }
        ...
        // This actions cannot be handled inside pos_order.js or pos_order_line.js
        if (
            values.product_template_id.to_weight &&
            this.config.iface_electronic_scale &&
            configure
        ) {
            if (values.product_template_id.isScaleAvailable) {
                this.isScaleScreenVisible = true;
                this.scaleData = {
                    productName: values.product_template_id?.display_name,
                    uomName: values.product_template_id.uom_id?.name,
                    uomRounding: values.product_template_id.uom_id?.rounding,
                ...
                await values.product_template_id._onScaleNotAvailable();
            }
        }

        // Handle price unit
        if (!values.product_template_id.isCombo() && vals.price_unit === undefined) {
        ...
        if (values.price_extra && !isScannedProduct) {
            const price = values.product_template_id.get_price(
                order.pricelist_id,
                values.qty,
                values.price_extra
            );
    ...
    getProducePriceDetails({ productTemplate, product, price }) {
        ...
        const selectedPrice = price === false ? basePrice : price;
