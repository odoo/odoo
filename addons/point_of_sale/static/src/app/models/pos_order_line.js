...
import { PosOrderlineTaxHelpers } from "./helpers/pos_order_line_tax_helpers";

export class PosOrderline extends PosOrderlineTaxHelpers {
    ...
    setLinePrice() {
        const details = this.getTaxDetails();
        const totalExcluded = details.tax_details.total_excluded;
        const totalIncluded = details.tax_details.total_included;

        if (this.price_subtotal !== totalExcluded) {
            this.price_subtotal = totalExcluded;
        }
        if (this.price_subtotal_incl !== totalIncluded) {
            this.price_subtotal_incl = totalIncluded;
        }
    }
        ...
        return [
            ...new Set(
                this.order_id.fiscal_position_id
                    ?.getTaxes(this.product_id.taxes_id)
