...
import { formatDate, serializeDateTime, deserializeDate } from "@web/core/l10n/dates";
...
export class PosOrder extends Base {
    ...
    getTaxTotalsOfLines(lines) {
        ...
        const documentSign =
            lines.length === 0 ||
            !lines.every((l) => lt(l.qty, 0, { decimals: currency.decimal_places }))
                ? 1
                : -1;
    ...
    setShippingDate(shippingDate) {
        if (shippingDate) {
            this.shipping_date = serializeDateTime(deserializeDate(shippingDate));
        } else {
