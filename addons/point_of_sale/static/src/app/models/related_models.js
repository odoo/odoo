...
export function createRelatedModels(modelDefs, modelClasses = {}, indexes = {}) {
        ...
        for (const name in vals) {
            ...
            if (X2MANY_TYPES.has(field.type)) {
                ...
                if (vals[name]) {
                    ...
                    if (exist) {
                        const toLink = vals[name]?.id ? vals[name] : records[comodelName].get(id);
                        connect(field, record, toLink);
                    } else if (models[field.relation]) {
    ...
    function replaceDataByKey(key, rawData) {
        ...
        for (const model in rawData) {
            ...
            for (const data of rawDataIdx) {
                const localRecord = models[model].getBy(key, data);
                if (localRecord) {
                    if (localRecord.uiState) {
                        uiState[data[key]] = { ...localRecord.uiState };
                    }
                    localRecord.delete();
                }
            }
    ...
    function loadData(rawData, load = [], fromSerialized = false) {
        ...
        for (const model in rawData) {
            ...
            for (const record of _records) {
                ...
                if (indexes[model].includes("uuid")) {
                    const existingRecord = models[model].getBy("uuid", record.uuid);
                    if (existingRecord && existingRecord.uuid) {
                        uiStateByUuid[existingRecord.uuid] = existingRecord.serializeState();
        ...
        for (const { raw, record } of modelToSetup) {
            ...
            if (record.uuid && uiStateByUuid[record.uuid]) {
                record.setupState(uiStateByUuid[record.uuid]);
            }
        }
