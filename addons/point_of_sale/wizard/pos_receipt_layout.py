...
class PosReceiptLayout(models.TransientModel):
    ...
    def _get_preivew_data(self):
        ...
        return {
            ...
            'product_data': self._get_demo_product_names(),
        }

    def _get_demo_product_names(self):
        domain = []
        if self.pos_config_id.iface_available_categ_ids:
            domain += [('pos_categ_ids', 'in', self.pos_config_id.iface_available_categ_ids.ids)]
        return self.env['product.template'].search_read(domain, ['name', 'list_price'], limit=3) or [
            {'name': 'Product 1', 'list_price': 11.35},
            {'name': 'Product 2', 'list_price': 13.0},
            {'name': 'Product 3', 'list_price': 12.2},
        ]
