...
class pos_open_statement(osv.osv_memory):
    ...
    def open_statement(self, cr, uid, ids, context=None):
        ...
        if not j_ids:
            raise UserError(_('No Cash Register Defined!'), _('You have to define which payment method must be available in the point of sale by reusing existing bank and cash through "Accounting / Configuration / Journals / Journals". Select a journal and check the field "PoS Payment Method" from the "Point of Sale" tab. You can also create new payment methods directly from menu "PoS Backend / Configuration / Payment Methods".'))
