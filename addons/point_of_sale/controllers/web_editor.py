...
class WebEditorPointOfSale(Web_Editor):
    ...
    def get_field_text_html(self, model=None, res_id=None, field=None, callback=None, **kwargs):
        ...
        extra_head = request.registry["ir.ui.view"].render_template(request.cr, request.uid, 'point_of_sale.extra_head', None, context=request.context)
    ...
    def get_snippets(self):
        return request.registry["ir.ui.view"].render_template(request.cr, request.uid, 'point_of_sale.customer_facing_display_snippets', None, context=request.context)
