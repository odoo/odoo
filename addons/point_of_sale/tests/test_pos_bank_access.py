...
class TestPosBankAccess(CommonPosTest):
    ...
    def test_get_partner_bank_id_refund_customer_bank(self):
        """A pure POS user cannot read an arbitrary customer's bank and has no move to lean on,
        so without sudo() the o2m read drops the refund's recipient bank."""
    ...
    def test_get_mail_attachments_invoice_render(self):
        """point_of_sale.group_pos_user already has an account.move read rule
