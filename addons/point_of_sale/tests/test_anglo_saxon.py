...
class TestAngloSaxonCommon(AccountTestInvoicingCommon):
    ...
    def _setup_fifo_consting_method(self):
    ...
    def _prepare_paid_pos_order(self, qty=1, **kwargs):
        """ Initiates a new POS session, creates a `pos.order` sells the specified
        quantity of `self.product` @450 per unit, and processes the payment using the `cash_payment_method`.

        :param qty: int - The number of product units to sell.
        :return: pos.order - The newly created POS order record.
        """
        ...
        pos_order_values = {
            ...
            'shipping_date': kwargs.get('shipping_date', False),
            'lines': [(0, 0, {
                'name': "OL/0001",
                'product_id': self.product.id,
                'price_unit': 450,
        ...
        pos_order_pos0 = self.PosOrder.create(pos_order_values)

        # make a payment to fully pay the order with cash payment method
...
class TestAngloSaxonFlow(TestAngloSaxonCommon):
    ...
    def test_create_account_move_line(self):
        pos_order_pos0 = self._prepare_paid_pos_order()

        # I check that the order is marked as paid
        self.assertEqual(pos_order_pos0.state, 'paid', 'Order should be in paid state.')
        self.assertEqual(pos_order_pos0.amount_paid, 450, 'Amount paid for the order should be updated.')
        ...
        self.assertFalse(pos_order_pos0.account_move, 'There should be no invoice in the order.')
        ...
        aml = current_session_id.move_id.line_ids
    ...
    def test_fifo_valuation_no_invoice(self):
        ...
        self._setup_fifo_consting_method()
        pos_order_pos0 = self._prepare_paid_pos_order(qty=7)
    ...
    def test_cogs_with_ship_later_no_invoicing(self):
        ...
        self._prepare_paid_pos_order(shipping_date='2023-01-01')

        # I close the current session to generate the journal entries
        current_session = self.pos_config.current_session_id
        current_session.post_closing_cash_details(450.0)
        current_session.close_session_from_ui()
        self.assertEqual(current_session.state, 'closed', 'Check that session is closed')
