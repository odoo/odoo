...
class TestAngloSaxonCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        account_type_rcv = self.env['account.account.type'].create({'name': 'RCV type', 'account_type': 'receivable'})
        account_type_oth = self.env['account.account.type'].create({'name': 'RCV type', 'account_type': 'other'})
