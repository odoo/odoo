...
class TestPoSOtherCurrencyConfig(TestPoSCommon):
    ...
    def test_pos_data_shared_product_cost_currency(self):
        ...
        sale-price and cost currency from the active company. standard_price is
        company-dependent with no fallback across companies, so it must be set
        explicitly for the active company. When the POS runs in a company whose
        currency differs from the main company, a price explicitly set for that
        company must be loaded as-is, not converted again through an exchange rate,
        since it is already expressed in the POS currency.
        """
        ...
        # Set both explicitly for the active company, so they are expressed in that
        # company's currency (the "other" currency) rather than inherited/converted.
