...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_to_payment(self):
        ...
        self.env.ref('point_of_sale.pos_config_main').open_session_cb()
        ...
        self.pos_order_pos0_line1 = self.env['pos.order.line'].create({
            ...
            'discount': 0.0,
            'qty': 2.0,
            'tax_ids': self.product3.taxes_id,
        ...
        self.pos_order_pos0_line2 = self.env['pos.order.line'].create({
            ...
            'discount': 0.0,
            'qty': 3.0,
            'tax_ids': self.product4.taxes_id,
            'order_id': self.pos_order_pos0.id
        })
    ...
    def test_order_to_invoice(self):
        ...
        # attached to it
        self.assertEqual(self.pos_order_pos1.state, 'paid', "State not correct")
        ...
        self.data, format = odoo_report.render_report(self.env.cr, self.env.uid, account_statement.id, 'point_of_sale.report_statement', {})
        ...
        self.data, format = odoo_report.render_report(self.env.cr, self.env.uid, account_statement.id, 'point_of_sale.report_usersproduct', {})
        if tools.config['test_report_directory']:
        ...
        self.data, format = odoo_report.render_report(self.env.cr, self.env.uid, self.pos_order_pos1.id, 'point_of_sale.report_invoice', {})
        if tools.config['test_report_directory']:
        ...
        self.data, format = odoo_report.render_report(self.env.cr, self.env.uid, self.pos_order_pos1.id, 'point_of_sale.report_saleslines', {})
        if tools.config['test_report_directory']:
        ...
        self.data, format = odoo_report.render_report(self.env.cr, self.env.uid, self.pos_order_pos1.id, 'point_of_sale.report_receipt', {})
        if tools.config['test_report_directory']:
        ...
        data_dict = {
            'date_start': datetime.date.today(), 'date_end': datetime.date.today(), 'user_ids': self.env.ref('base.user_root')}
        test_reports.try_report_action(self.env.cr, self.env.uid, 'action_report_pos_details', wiz_data=data_dict, context=ctx, our_module='point_of_sale')

        # In order to test the PDF reports defined on a Point of Sale, we will
        # print a POS Payment Report

        self.data, format = odoo_report.render_report(self.env.cr, self.env.uid, self.pos_order_pos1.id, 'point_of_sale.report_payment', {})
        if tools.config['test_report_directory']:
