...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_with_deleted_tax(self):
        ...
        # order_ids of the pos_session will always contains single order,
        # so it's safe to access the name field this way.
        pos_session_move = self.env['account.move'].search([
            ('ref', '=', pos_session.order_ids.name), ('journal_id', '=', pos_session.config_id.journal_id.id)])
