...
class TestPointOfSaleCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        account = self.env['account.account'].create({'name': 'Receivable', 'code': 'RCV00' , 'user_type_id': account_type.id, 'reconcile': True})
        self.partner.property_account_receivable_id = account
...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_create_account_move_line(self):
        ...
        self.cash_journal.loss_account_id = self.env['account.account'].search([('user_type_id.type', '=', 'other')], limit=1)
        ...
        self.assertEqual(aml_output.debit, self.product.standard_price, "Cost of Good Sold entry missing or mismatching")
        self.assertEqual(aml_expense.credit, self.product.standard_price, "Cost of Good Sold entry missing or mismatching")
