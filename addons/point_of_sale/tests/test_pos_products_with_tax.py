...
class TestPoSProductsWithTax(TestPoSCommon):
    ...
    def test_pos_loaded_product_taxes_on_branch(self):
        ...
        self.assertEqual(
            next(iter(filter(lambda p: p['id'] == product_all_taxes.id, pos_data['product.template']['data'])))['taxes_id'],
            tax_xx.ids
        )
        self.assertEqual(
            next(iter(filter(lambda p: p['id'] == product_no_xx_tax.id, pos_data['product.template']['data'])))['taxes_id'],
            tax_x.ids
        )
        tax_data_no_branch = next(iter(filter(lambda p: p['id'] == product_no_branch_tax.id, pos_data['product.template']['data'])))['taxes_id']
        ...
        self.assertEqual(
            next(iter(filter(lambda p: p['id'] == product_no_tax.id, pos_data['product.template']['data'])))['taxes_id'],
            []
        )
