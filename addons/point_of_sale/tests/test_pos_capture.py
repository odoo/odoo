...
def mocked_handle_order_process_fail(self, order: dict, exception: Exception, uid: int = False):
...
def mocked_should_capture_order(self, *args, **kwargs):
    return True
...
class TestPosCapture(TestPoSCommon):
    ...
    def test_capture_one_order(self):
        ...
        with patch.object(PosOrder, '_process_order', mocked_process_order),\
             patch.multiple(
                 PosSession,
                _handle_order_process_fail=mocked_handle_order_process_fail,
                _should_capture_order=mocked_should_capture_order),\
            ...
            try:
                ...
                self.fail("Should have raised an IntendedException")
    ...
    def test_capture_two_orders(self):
        ...
        order1 = [self.create_ui_order_data([(self.product1, 1)], uuid='12345-678-1996')]
        order2 = [self.create_ui_order_data([(self.product1, 1)], uuid='12345-678-1999')]  # Different order with same content but different uuid
        ...
        with patch.object(PosOrder, '_process_order', mocked_process_order),\
            ...
            try:
                self.env['pos.order'].sync_from_ui(order1)
                self.fail("Should have raised an IntendedException")
            except IntendedException:
                self.assert_activity_and_attachment(session, 1)

            try:
                self.env['pos.order'].sync_from_ui(order2)
                self.fail("Should have raised an IntendedException")
            except IntendedException:
    ...
    def test_capture_order_same_uuid(self):
        ...
        order1 = [self.create_ui_order_data([(self.product1, 1)], uuid='12345-678-1996')]
        order2 = [self.create_ui_order_data([(self.product1, 2)], uuid='12345-678-1996')]
