...
class POSTestTaxReport(TestAccountReportsCommon):
    ...
    def setUpClass(cls):
        ...
        cls.pos_tax_report_line_invoice_base = cls._create_tax_report_line(
            "Invoice Base", cls.tax_report, tag_name="-pos_invoice_base", sequence=0
        )
        cls.pos_tax_report_line_invoice_tax = cls._create_tax_report_line(
            "Invoice Tax", cls.tax_report, tag_name="-pos_invoice_tax", sequence=1
        )
        cls.pos_tax_report_line_refund_base = cls._create_tax_report_line(
            "Refund Base", cls.tax_report, tag_name="pos_refund_base", sequence=2
        )
        cls.pos_tax_report_line_refund_tax = cls._create_tax_report_line(
            "Refund Tax", cls.tax_report, tag_name="pos_refund_tax", sequence=3
        )
                ...
                "invoice_repartition_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "tag_ids": cls._get_tag_ids(
                                cls.pos_tax_report_line_invoice_base.expression_ids
                            ),
                        },
                    ),
                    (
                        ...
                        0,
                        {
                            ...
                            "tag_ids": cls._get_tag_ids(
                                cls.pos_tax_report_line_invoice_tax.expression_ids
                            ),
                        },
                    ),
                ],
                "refund_repartition_line_ids": [
                    (
                        ...
                        0,
                        {
                            "repartition_type": "base",
                            "tag_ids": cls._get_tag_ids(
                                cls.pos_tax_report_line_refund_base.expression_ids
                            ),
                        },
                    ),
                    (
                        ...
                        0,
                        {
                            "repartition_type": "tax",
                            "tag_ids": cls._get_tag_ids(
                                cls.pos_tax_report_line_refund_tax.expression_ids
                            ),
                        },
                    ),
                ],
        ...
        rep_ln_tax = (
            cls.pos_tax.invoice_repartition_line_ids
            + cls.pos_tax.refund_repartition_line_ids
        )
        rep_ln_tax.filtered(lambda x: x.repartition_type == "tax").write(
            {"account_id": pos_tax_account.id}
        )
        ...
        cls.pos_config.write(
            {"payment_method_ids": [(4, cls.pos_payment_method.id, 0)]}
        )

    def _create_and_pay_pos_order(self, qty, price_unit):
        tax_amount = (
            (self.pos_tax.amount / 100) * qty * price_unit
        )  # Only possible because the tax is 'percent' and price excluded. Don't do this at home !
        rounded_total = self.company_data["company"].currency_id.round(
            tax_amount + price_unit * qty
        )
        ...
        order = self.env["pos.order"].create(
            {
                ...
                "amount_tax": self.company_data["company"].currency_id.round(
                    tax_amount
                ),
                "amount_paid": 0,
                "amount_return": 0,
