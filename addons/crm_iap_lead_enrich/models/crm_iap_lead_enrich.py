        ...
        for lead_id, data in data_clearbit.items():
            ...
            if record.exists():
                if data:
                    ...
                    message_values = self._create_message_data(data)
                    record.message_post_with_view('crm_iap_lead_enrich.mail_message_lead_enrich_with data', values=message_values, subtype_id=self.env.ref('mail.mt_note').id)
                else:
