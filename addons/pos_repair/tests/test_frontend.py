...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def test_pos_repair(self):
        ...
        self.assertEqual(len(self.test_product3.stock_move_ids.ids), 2, "There should be 2 stock moves for the product created by the repair order")

        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour("/pos/ui?config_id=%d" % self.main_pos_config.id, 'PosRepairSettleOrder', login="pos_user")

        self.assertEqual(len(self.test_product3.stock_move_ids.ids), 2, "Paying for the order in PoS should not create new stock moves")
