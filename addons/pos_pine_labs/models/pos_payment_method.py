...
from .pine_labs_pos_request import PineLabsPosRequest
...
class PosPaymentMethod(models.Model):
    ...
    def _get_payment_terminal_selection(self):
        return super(PosPaymentMethod, self)._get_payment_terminal_selection() + [('pine_labs', 'Pine Labs')]
    ...
    def pine_labs_make_payment_request(self, data):
        ...
        :rtype: dict
        """
        pine_labs = PineLabsPosRequest(self)
        body = {
            'Amount': data['amount'],
            'TransactionNumber': data['transactionNumber'],
            'SequenceNumber': data['sequenceNumber']
        }
        response = pine_labs.call_pine_labs(endpoint='UploadBilledTransaction', payload=body)
    ...
    def pine_labs_fetch_payment_status(self, data):
        ...
        response = pine_labs.call_pine_labs(endpoint='GetCloudBasedTxnStatus', payload=body)
    ...
    def pine_labs_cancel_payment_request(self, data):
        ...
        response = pine_labs.call_pine_labs(endpoint='CancelTransactionForced', payload=body)
        if response.get('ResponseCode') == 0 and response.get('ResponseMessage') == "APPROVED":
            return {
                'responseCode': response['ResponseCode'],
