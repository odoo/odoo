...
class HrVersion(models.Model):
    ...
    def generate_work_entries(self, date_start, date_stop, force=False):
        ...
        for version in self:
            versions_by_company_tz[
                version.company_id,
                version.resource_calendar_id.tz or version.employee_id.tz,
            ] += version
    ...
    def _generate_work_entries(self, date_start, date_stop, force=False):
        ...
        utc = pytz.timezone('UTC')
        for version in self:
            version_tz = (version.resource_calendar_id or version.company_id.resource_calendar_id).tz or version.employee_id.tz
            tz = pytz.timezone(version_tz) if version_tz else pytz.utc
