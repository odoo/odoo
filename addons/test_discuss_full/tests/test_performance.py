...
class TestDiscussFullPerformance(HttpCase, MailCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.has_enterprise = cls.env['ir.module.module'].search_count([('name', '=', 'test_discuss_full_enterprise'), ('state', '=', 'installed')], limit=1)
    ...
    def _run_test(self, /, *, fn, queries, results):
        self.authenticate(self.users[0].login, self.password)
    ...
    def test_10_init_store_data(self):
        ...
        queries_init_store = [
            # _store_init_global_fields
            'SELECT "res_partner"."id" FROM ... %s',
            'SELECT r.id FROM ir_rule ...',
            'SELECT "res_users"."id", ... %s',
            'SELECT "res_groups_users_rel"... FROM "res_groups" ...',
            'SELECT "rule_group_rel"."rule_group_id", "rule_group_rel"."group_id" FROM "res_groups" ...',
            'SELECT ... FROM "ir_rule" WHERE ... %s',
            # store add odoobot
            'SELECT "res_partner"."id", ... %s',
            'SELECT r.id FROM ir_rule ...',
            'SELECT "rule_group_rel"."rule_group_id", "rule_group_rel"."group_id" FROM "res_groups" ...',
            'SELECT ... FROM "ir_rule" WHERE ... %s',
            'SELECT "res_company"."id", ... %s',
            'SELECT "res_users"."id", ...',
            'SELECT "res_groups"."id" FROM "res_groups" WHERE "res_groups"."id" IN %s',
            # settings
            'SELECT "res_users_settings"."id", ...',
            'SELECT "res_users_settings_embedded_action"."id", ...',
            'SELECT "res_users_settings"."id", ... %s',
            'SELECT "res_users_settings_volumes"."id", ...',
            *((
                'SELECT "voip_provider"."id", ... %s',
            ) if self.has_enterprise else ()),
            'SELECT "res_lang_res_users_settings_rel"...',
            'SELECT ... FROM "im_livechat_expertise" ...',
            # canned response
            'SELECT COUNT(*) FROM ... FROM "mail_canned_response" ...',
            # voip and AI
            *((
                'SELECT "mail_activity_type"."id" FROM ... LIMIT 1',
                'SELECT COUNT(*) FROM "voip_call" WHERE ...',
                'SELECT "res_partner"."id", ... %s',
                'SELECT "ai_agent"."id", ...',
            ) if self.has_enterprise else (
                'SELECT "res_partner"."id", ... %s',
            ...
            # _compute_im_status
            'SELECT "res_users"."id", "res_users"."partner_id", "res_users"."active" FROM "res_users" ...',
            'SELECT "mail_presence"."id", "mail_presence"."user_id" FROM ...',
            'SELECT "mail_presence"."id", "mail_presence"."guest_id", ...',
            # other fields
            'SELECT "hr_employee"."id", ...',
            'SELECT "res_users"."id", ... %s',
            'SELECT "res_partner"."id", ... %s',
            'SELECT "res_users"."id", ...',  # _compute_main_user_id
            'SELECT "hr_employee_location"."id" FROM ...',  # _compute_exceptional_location_id
            'SELECT "hr_employee"."id", ... %s',  # _compute_work_location_type
            'SELECT "hr_leave"."id" FROM ...',  # _compute_leave_status
        ]
        self._run_test(
            fn=test_fn,
            queries=queries_init_store,
    ...
    def test_20_init_messaging(self):
        ...
        queries_init_messaging = [
            # _process_request_for_all
            'SELECT "res_users"."id", ... %s',
            'SELECT "discuss_channel_member"... FROM "discuss_channel" ...',
            'SELECT "discuss_channel"."id" FROM ...',
            'SELECT "bus_bus"."id" FROM ... LIMIT 1',  # _bus_last_id
            # _store_init_messaging_global_fields
            'SELECT "discuss_channel_member"."id", ...',
            'SELECT count(mail_message.id) AS count, ...',  # _compute_message_unread
            'SELECT count(*) as needaction_count FROM ...',  # _get_needaction_count
            'SELECT "mail_message"."id", ... GROUP BY ...',
            'SELECT "discuss_channel"."id", ...',
            'SELECT "discuss_channel"."id", ... WHERE "discuss_channel"."id" IN %s AND ...',  # fetch with security rules
            'SELECT "discuss_channel_member"."channel_id", MAX(...',  # _compute_self_member_id
            'SELECT "discuss_channel_member"."channel_id", ARRAY_AGG(...',  # _compute_invated_member_ids
            'SELECT "discuss_channel_rtc_session"."id", ...',
            'SELECT "discuss_channel_rtc_session"."id", ... %s',
            'SELECT "discuss_channel_member"."id", ...',
            'SELECT "discuss_channel_member"."id", ... %s',
            'SELECT "res_partner"."id", ... %s',
            # AI
            *((
                'SELECT "ai_agent"."id", ...',
            ...
            'SELECT "mail_presence"."id", "mail_presence"."guest_id", ...',
            # other fields
            'SELECT "hr_employee"."id", "hr_employee"."user_id", "hr_employee"."active" FROM "hr_employee" ...',
            'SELECT "hr_employee_location"."id" FROM ...',  # _compute_exceptional_location_id
            'SELECT "hr_employee"."id", ... %s',  # _compute_work_location_type
            'SELECT "hr_leave"."id" FROM ...',  # _compute_leave_status
            'SELECT "res_users"."id", ... %s',
            'SELECT "bus_bus"."id" FROM "bus_bus" ORDER BY "bus_bus"."id" DESC LIMIT 1',  # _bus_last_id
            'SELECT "discuss_channel_member"."channel_id", COUNT(*) FROM ...',  # _compute_member_count
            'SELECT msg.res_id FROM mail_message msg RIGHT JOIN mail_notification ...',  # _compute_message_need_action
            'SELECT "ir_attachment"."id", ...',  # _compute_avatar_128
            'SELECT "discuss_channel_res_groups_rel"."discuss_channel_id", ...',
            'SELECT "res_groups"."id", ... %s',  # _compute_full_name
        ]
        self._run_test(
            ...
            queries=queries_init_messaging,
    ...
    def test_30_discuss_channels(self):
        ...
        queries_discuss_channels = [
            # _process_request_for_all
            'SELECT "res_users"."id", ... %s',
            'SELECT "discuss_channel_member"... FROM "discuss_channel" ...',
            'SELECT "discuss_channel"."id", ...',
            # _add_has_unpinned_channels_to_store
            'SELECT COUNT(*) FROM ... FROM "discuss_channel_member" ...',
            'SELECT "discuss_channel_member"."channel_id", MAX(...',  # _compute_self_member_id
            'SELECT "discuss_channel_member"."channel_id", ARRAY_AGG(...',  # _compute_invated_member_ids
            'SELECT "discuss_channel_rtc_session"."id", ...',
            'SELECT "discuss_channel_rtc_session"."id", ... %s',
            ...
            'SELECT channel.id, member.id FROM discuss_channel channel ...',
            'SELECT "discuss_channel_member"."id", ... %s',
            # _compute_livechat_member_type
            'SELECT "im_livechat_channel_member_history"."id", ...',
            'SELECT "im_livechat_channel_member_history"."id", ... %s',
            'SELECT "res_partner"."id", ... %s',
            # AI
            *((
                'SELECT "ai_agent"."id", ...',
            ...
            'SELECT "hr_employee"."id", ... %s',  # _compute_work_location_type
            'SELECT "hr_leave"."id" FROM ...',  # _compute_leave_status
            # _compute_livechat_username -> _compute_res_users_settings_id
            'SELECT "res_users_settings"."id", ...',
            'SELECT "res_users_settings"."id", ... %s',
            'SELECT "res_users"."id", ... %s',
            # _compute_im_status
            'SELECT "res_country"."id", ... %s',
            'SELECT "mail_presence"."id", "mail_presence"."guest_id" FROM ...',
            'SELECT "mail_guest"."id", ... %s',
            'SELECT "bus_bus"."id" FROM ... LIMIT 1',  # _bus_last_id
            'SELECT "discuss_channel_member"."channel_id", COUNT(*) FROM ...',  # _compute_member_count
            'SELECT msg.res_id FROM mail_message ...',  # _compute_message_need_action
            'SELECT "ir_attachment"."id", ...',  # _compute_avatar_128
            'SELECT "discuss_channel_res_groups_rel"."discuss_channel_id", ...',
            *((
                'SELECT "discuss_channel"."id", ... %s',
            ) if self.has_enterprise else ()),
            'SELECT "im_livechat_channel_member_history"."id", ...',
            'SELECT "discuss_channel_im_livechat_expertise_rel"...',
            'SELECT "res_groups"."id", ... %s',  # compute_full_name
            'SELECT count(mail_message.id) AS count, ...',  # _compute_message_unread
            'SELECT "im_livechat_channel"."id", ... %s',
            'SELECT last_message_id FROM discuss_channel ...',  # _get_last_messages
            # _compute_linked_message_ids
            'SELECT "mail_message"."id", ... GROUP BY "mail_message"."id"',
            'SELECT "mail_message"."id", ... GROUP BY "mail_message"."id"',
            'SELECT "mail_message"."id", ... %s',
            # store mail.message.schedule
            'SELECT "mail_message_schedule"."id" FROM ...',
            'SELECT "mail_message_res_partner_starred_rel"...',  # _compute_starred
            'SELECT "message_attachment_rel"...',
            'SELECT "mail_message_res_partner_rel"...',
            'SELECT "mail_message_reaction"."id", ...',
            # _compute_has_poll
            'SELECT "mail_poll"."id", "mail_poll"."start_message_id" FROM "mail_poll" ...',
            'SELECT "mail_poll"."id", "mail_poll"."end_message_id" FROM "mail_poll" ...',
            'SELECT "mail_message_link_preview"."id", ...',
            'SELECT "mail_notification"."id", ...',
            'SELECT "mail_tracking_value"."id", ...',
            'SELECT "rating_rating"."id", ...',
            # _filter_unimportant_notifications
            'SELECT "mail_notification"."id", ... %s',
            'SELECT "mail_message_subtype"."id", ... %s',
            'SELECT "discuss_call_history"."id", ...',
            'SELECT "mail_message_reaction"."id", ... %s',
            'SELECT "rating_rating"."res_id", COUNT...',  # _compute_rating_stats
            'SELECT "mail_message_subtype"."id", ... %s',
        ...
        self._run_test(
            ...
            queries=queries_discuss_channels,
    ...
    def _get_init_store_data_result(self):
        ...
        channel_types_with_seen_infos = result["Store"]["channel_types_with_seen_infos"] + ["whatsapp"]
        result["Store"].update(
            {
