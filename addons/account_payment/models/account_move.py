...
class AccountMove(models.Model):
    ...
    def _get_online_payment_error(self):
        ...
        if not (self.amount_residual or not transactions and not self.currency_id.is_zero(self.amount_residual) and self.amount_total):
            errors.append(_("There is no amount to be paid."))
        if self.state != 'posted':
            errors.append(_("This invoice isn't posted."))
        if self.payment_state not in ('not_paid', 'in_payment', 'partial'):
