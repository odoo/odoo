...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    def overdue_invoices_transaction(self, payment_reference, **kwargs):
        ...
        logged_in = not request.env.user._is_public()
        if not logged_in:
            raise ValidationError(request.env._("Please log in to pay your overdue invoices"))
        ...
        if not all(currency == currencies[0] for currency in currencies):
            raise ValidationError(request.env._("Impossible to pay all the overdue invoices if they don't share the same currency."))
    ...
    def payment_pay(self, *args, amount=None, invoice_id=None, access_token=None, **kwargs):
        ...
        if invoice_id:
            ...
            if not invoice_sudo:
                raise ValidationError(request.env._("The provided parameters are invalid."))
