...
class Event(models.Model):
    ...
    def _compute_sale_price_subtotal(self):
        ...
        event_subtotals = self.env['sale.order.line']._aggregate(
            [('event_id', 'in', self.ids),
             ('price_subtotal', '!=', 0)],
            ['price_subtotal:sum'],
            ['event_id', 'currency_id'],
        )
        # TODO: why _convert make a extra query each time, should be cached ?
        self.sale_price_subtotal = 0
        for [event, currency], [sum_price_subtotal] in event_subtotals.items(as_records=True):
