...
class IrAttachment(models.Model):
    ...
    def _unlink_except_validated_pdf_invoices(self):
        ...
        res = self.env['account.edi.document']._read_group(domain=[('move_id', 'in', attachments_to_check.mapped('res_id')), ('state', '=', 'sent')],
        ...
        if restricted_attachments:
            raise UserError(_("Oops! The invoice PDF(s) are linked to a validated EDI document and cannot be deleted according to ZATCA rules: %s", ', '.join(restricted_attachments.mapped('name'))))
