...
class AccountJournal(models.Model):
    ...
    l10n_sa_production_csid_certificate = fields.Many2one(string="PCSID Certificate", comodel_name="certificate.certificate")
    l10n_sa_production_csid_json = fields.Char("PCSID JSON", copy=False, groups="base.group_system",
                                               help="Production CSID data received from the Production CSID API "
                                                    "in dumped json format")
    l10n_sa_production_csid_validity = fields.Datetime(related="l10n_sa_production_csid_certificate.date_end")
    l10n_sa_compliance_csid_certificate = fields.Many2one(string="CCSID certificate", comodel_name="certificate.certificate")
    ...
    def _l10n_sa_api_onboard_journal(self, otp):
        ...
        try:
            ...
            if not self.company_id.sudo().l10n_sa_private_key:
                ec_private_key_sudo = self.env['certificate.key'].sudo()._generate_ec_private_key(name='CCSID private key')
                self.company_id.write({'l10n_sa_private_key': ec_private_key_sudo.id})
    ...
    def _l10n_sa_get_compliance_CSID(self, otp):
        ...
        cert_id = self.env['certificate.certificate'].sudo().create({
            ...
            'private_key': self.company_id.sudo().l10n_sa_private_key.id,
        }).id
        self.sudo().write({
            'l10n_sa_compliance_csid_json': json.dumps(CCSID_data),
            'l10n_sa_compliance_csid_certificate': cert_id,
    ...
    def _l10n_sa_get_production_CSID(self, OTP=None):
        ...
        if not self_sudo.l10n_sa_compliance_csid_json or not self_sudo.l10n_sa_compliance_csid_certificate:
        ...
        pcsid_certificate_id = self_sudo.env['certificate.certificate'].create({
            'name': 'PCSID Certificate',
            'content': b64decode(PCSID_data['binarySecurityToken']),
            'private_key': self.company_id.sudo().l10n_sa_private_key.id,
        }).id
        self.write({'l10n_sa_production_csid_certificate': pcsid_certificate_id})
    ...
    def _l10n_sa_run_compliance_checks(self):
        ...
        for fname, fval in compliance_files.items():
            ...
            prepared_xml = self._l10n_sa_prepare_compliance_xml(fname, fval, self_sudo.l10n_sa_compliance_csid_certificate, digital_signature)
    ...
    def _l10n_sa_api_get_pcsid(self):
        ...
        """
        self.ensure_one()
        if not self.sudo().l10n_sa_production_csid_json or not self.sudo().l10n_sa_production_csid_certificate:
            raise UserError(_("Please, make a request to obtain the Compliance CSID and Production CSID before sending "
                            "documents to ZATCA"))
        certificate = self.sudo().l10n_sa_production_csid_certificate
        ...
        return json.loads(self.sudo().l10n_sa_production_csid_json), certificate.id
    ...
    def _l10n_sa_load_edi_demo_data(self):
        self.ensure_one()
        self.company_id.l10n_sa_private_key = self.env['certificate.key']._generate_ec_private_key()
        self.write({
