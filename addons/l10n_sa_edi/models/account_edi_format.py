...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_sa_check_buyer_missing_info(self, invoice):
        ...
        if any(tax.l10n_sa_exemption_reason_code in ('VATEX-SA-HEA', 'VATEX-SA-EDU') for tax in
            ...
            if identification_scheme != 'NAT' or not (partner_id.l10n_sa_edi_additional_identification_number and identification_scheme != 'TIN'):
                missing.append(_('Please set the Identification Scheme as National ID and Identification Number as the respective '
    ...
    def _l10n_sa_post_zatca_edi(self, invoice):  # no batch ensure that there is only one invoice
        ...
        if not company.l10n_sa_edi_is_production:
            company.l10n_sa_edi_is_production = company.l10n_sa_api_mode == 'prod'
    ...
    def _check_move_configuration(self, invoice):
        ...
        if customer_missing_info:
            errors.append(
                _(
                    "- %(missing_fields)s",
                    missing_fields=", ".join(customer_missing_info),
                )
            )
