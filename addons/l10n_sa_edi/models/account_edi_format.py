...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_sa_export_zatca_invoice(self, invoice):
        ...
        """
        self.ensure_one()
        unsigned_xml = invoice._l10n_sa_generate_unsigned_data()
    ...
    def _l10n_sa_post_zatca_edi(self, invoice):  # no batch ensure that there is only one invoice
        """
            Post the invoice to ZATCA along with the required technical data.

            Generate chain index, one per invoice.
            For each submission attempt, the invoice XML is generated, signed, and submitted to ZATCA.
            The submission result and the exact XML content sent are returned.
        """
