...
class BaseDocumentLayout(models.TransientModel):
    ...
    def _default_company_details(self):
        ...
        if self.env.company.vat:
            additional_company_details += "VAT Number: " + self.env.company.vat + Markup('<br/>')
        if (code := self.env.company.l10n_sa_edi_additional_identification_scheme) and self.env.company.l10n_sa_edi_additional_identification_number:
            code_value = dict(self._fields['l10n_sa_edi_additional_identification_scheme']._description_selection(self.env)).get(code, '')
            additional_company_details += code_value + ": " + self.env.company.l10n_sa_edi_additional_identification_number
