...
class TestEdiZatca(TestSaEdiCommon):
    ...
    def testInvoiceWithDownpayment(self):
        ...
        with freeze_time(freeze):
            ...
            downpayment_wizard = self.env['sale.advance.payment.inv'].sudo().with_context(context).create({
            ...
            final_wizard = self.env['sale.advance.payment.inv'].sudo().with_context(context).create({})
