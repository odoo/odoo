...
class SaleOrder(models.Model):
    ...
    def _check_carrier_quotation(self, force_carrier_id=None):
        ...
        if self.only_services:
            ...
            if force_carrier_id or not carrier_id or carrier_id not in carrier_ids:
                for delivery in self.env['delivery.carrier'].browse(carrier_ids):
    ...
    def _get_delivery_methods(self):
        self.ensure_one()
        carriers = self.env['delivery.carrier'].with_context(order_id=self.id).search([('website_published', '=', True)])
    ...
    def _get_website_data(self, order):
        ...
        if not order.order_line.filtered(lambda line: line.product_id.type in ['consu', 'product']):
            return values
    ...
    def _get_shipping_country(self, values):
        ...
        for carrier in delivery_carriers:
            ...
            countries = countries | carrier.country_ids
            ...
            states = states | carrier.state_ids | values['states'].filtered(lambda state: state.country_id in state_countries)
