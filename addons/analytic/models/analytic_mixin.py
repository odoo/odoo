...
class AnalyticMixin(models.AbstractModel):
    ...
    def _modifiying_distribution_values(self, old_distribution, new_distribution):
        ...
        for old_key, old_val in old_distribution.items():
            remaining_key = tuple(sorted(
                account_id
                for account_id in old_key.split(',')
                if self.env['account.analytic.account'].browse(int(account_id)).plan_id.root_id in non_changing_plans
            ))
            if remaining_key:
        ...
        for new_key, new_val in new_distribution.items():
            remaining_key = tuple(sorted(
                account_id
                for account_id in new_key.split(',')
                if self.env['account.analytic.account'].browse(int(account_id)).plan_id.root_id not in non_changing_plans
            ))
            if remaining_key:
    ...
    def _merge_distribution(self, old_distribution: dict, new_distribution: dict) -> dict:
        ...
        return {
            ','.join(old_key + new_key): ratio * old_val * new_val / non_changing_amount
            for old_key, old_val in non_changing_values.items()
            for new_key, new_val in changing_values.items()
        } | additional_vals
