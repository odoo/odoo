...
test("connection considered as lost after failed reconnect attempt", async () => {
    ...
    await makeMockEnv();
    getService("bus_service").addEventListener("connect", () => asyncStep("connect"));
    getService("bus_service").addEventListener("reconnect", () => asyncStep("reconnect"));
    unlockBus();
...
test("computer sleep doesn't mark connection as lost", async () => {
    ...
    await makeMockEnv();
    getService("bus_service").addEventListener("connect", () => asyncStep("connect"));
    getService("bus_service").addEventListener("disconnect", () => asyncStep("disconnect"));
    getService("bus_service").addEventListener("reconnect", () => asyncStep("reconnect"));
    unlockBus();
    await getService("bus_service").start();
    await waitForSteps([["isConnectionLost", false], "connect"]);
