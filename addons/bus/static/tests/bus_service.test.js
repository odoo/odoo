...
test("pass stream position on initialization", async () => {
    patchWithCleanup(session, { bus_info: { stream_position: "1:1:" } });
    patchWithCleanup(WebsocketWorker.prototype, {
        _onClientMessage(_client, { action, data }) {
            if (action === "BUS:INITIALIZE_CONNECTION") {
                expect.step(`${action} - ${data["streamPosition"]}`);
            }
...
test("subscription stream position is captured from the initial call to update channel", async () => {
    // Large delay so stream_position can advance before the subscription is sent.
    ...
    addBusServiceListeners(["BUS:CONNECT", () => expect.step("BUS:CONNECT")]);
    onWebsocketEvent("subscribe", (data) =>
        expect.step(`subscribe - [${data.channels.toString()}] - ${data.stream_position}`)
    );
    await startBusService();
    const busService = getService("bus_service");
    await expect.waitForSteps(["BUS:CONNECT"]);
    await advanceTime(120_000);
    await expect.waitForSteps([`subscribe - [] - ${session.bus_info.stream_position}`]);
    let lastReceivedPosition = MockServer.env["bus.bus"]._sendone(
        serverState.partnerId,
        "message_type",
        "message_1"
    );
    await waitNotifications(["message_type", "message_1"]);
    expect(worker.lastStreamPosition).toBe(lastReceivedPosition);
    const fooStreamPosition = lastReceivedPosition;
    busService.addChannel("foo");
    await expect.waitForSteps(["add_channel - foo"]);
    lastReceivedPosition = MockServer.env["bus.bus"]._sendone(
        serverState.partnerId,
        "message_type",
        "message_2"
    );
    ...
    await advanceTime(120_000);
    await expect.waitForSteps([`subscribe - [bar,foo] - ${fooStreamPosition}`]);
});
