...
export const busService = {
    ...
    start(
        env,
        { main_tab: mainTab, multi_tab: multiTab, notification, "bus.parameters": params, worker }
    ) {
        ...
        function initializeWorkerConnection() {
            ...
            worker.send("initialize_connection", {
        ...
        function startWorker() {
            ...
            worker.registerHandler(handleMessage);
        ...
        const state = reactive({
            ...
            addChannel: async (channel) => {
                ...
                worker.send("add_channel", channel);
                worker.send("start");
                state.isActive = true;
            },
            deleteChannel: (channel) => {
                worker.send("delete_channel", channel);
            },
            setLoggingEnabled: (isEnabled) => worker.send("set_logging_enabled", isEnabled),
            downloadLogs: () => worker.send("request_logs"),
            forceUpdateChannels: () => worker.send("force_update_channels"),
            trigger: bus.trigger.bind(bus),
            removeEventListener: bus.removeEventListener.bind(bus),
            send: (eventName, data) => worker.send("send", { event_name: eventName, data }),
            start: async () => {
                if (!worker) {
                    startWorker();
                }
