...
export class WebsocketWorker {
    ...
    /**
     * Stream position used to resume notification delivery safely. Initialized when the
     * tab connects and updated with each received notification batch. This ensures
     * notifications are not missed across reconnects or channel subscription changes.
     *
     * See `Websocket._stream_position` for implementation details.
     *
     * @type {?string}
     */
    ...
    constructor(name) {
        ...
        /** @type {?{force: boolean, stream_position: string}} */
     ...
     * @param {string} [param0.streamPosition] The transaction threshold at
     * page load. Used to ensure no notifications are missed between page load and the
     ...
     */
    _initializeConnection(client, { db, streamPosition, uid, websocketURL, startTs }) {
        ...
        this.lastStreamPosition ||= streamPosition;
    ...
    _onWebsocketMessage(messageEv) {
        ...
        this.lastStreamPosition = payload.stream_position ?? this.lastStreamPosition;
        this._logDebug("_onWebsocketMessage", this.lastStreamPosition, messages);
    ...
    _debouncedUpdateChannels = debounce(() => {
        const { force, stream_position } = this.nextSubscribeData;
        this.nextSubscribeData = null;
    ...
    _scheduleUpdateChannels({ force = false } = {}) {
        this.nextSubscribeData ??= { stream_position: this.lastStreamPosition };
