...
class TestWebsocketCaryall(WebsocketCase):
    def test_pg_tx_mock(self):
        initial_xid = self.bus_tx_mock._next_xid
        self.assertEqual(self.bus_tx_mock.snapshot(), f"{initial_xid}:{initial_xid}:")
        tx1 = self.bus_tx_mock.tx(self.env.cr)
        self.assertEqual(
            self.bus_tx_mock.snapshot(),
            f"{initial_xid}:{initial_xid + 1}:{initial_xid}",
        )
        tx2 = self.bus_tx_mock.tx(self.env.cr)
        self.assertEqual(
            self.bus_tx_mock.snapshot(),
            f"{initial_xid}:{initial_xid + 2}:{initial_xid},{initial_xid + 1}",
        )
        ...
        self.assertEqual(
            self.bus_tx_mock.snapshot(),
            f"{initial_xid}:{initial_xid + 2}:{initial_xid}",
        )
        row = self.env["bus.bus"].search([])
        self.assertEqual(row.create_tx_id, initial_xid + 1)
        self.assertEqual(json.loads(row.message)["payload"], "from_tx2")
        tx1.commit()
        self.assertEqual(self.bus_tx_mock.snapshot(), f"{initial_xid + 2}:{initial_xid + 2}:")
        row = self.env["bus.bus"].search([("create_tx_id", "=", initial_xid)])
    ...
    def test_trigger_notification_unsupported_language(self):
        ...
        self.subscribe(websocket, ['my_channel'])
        self.bus_tx_mock.tx(self.env.cr).send('my_channel', 'notif_type', 'message').commit()
    ...
    def test_subscribe_to_custom_channel(self):
        ...
        with patch.object(IrWebsocket, '_build_bus_channel_list', return_value=[channel]):
            ...
            with self.bus_tx_mock.tx(self.env.cr) as tx:
                tx.send(channel, 'notif_on_global_channel', 'message')
                tx.send((channel, 'PRIVATE'), 'notif_on_private_channel', 'message')
            notifications = json.loads(websocket.recv())["notifications"]
    ...
    def test_websocket_check_outdated_subscription(self):
        self.bus_tx_mock.tx(self.env.cr).send('channel_A', 'some_notification').commit()
        self._reset_bus()
        websocket = self.websocket_connect()
