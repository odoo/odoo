...
class Websocket:
    ...
    def _trim_sent_notifs(self):
        ...
        """
        last_sent_id = (
            max(notif[0] for notif in self._sent_notifs)
            if self._sent_notifs
            else self._min_notif_id
        )
        self._sent_notifs = [notif for notif in self._sent_notifs if time.time() - notif[1] < 10]
        self._min_notif_id = (
            min(notif[0] for notif in self._sent_notifs) if self._sent_notifs else last_sent_id
        )
