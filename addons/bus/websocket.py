...
class Websocket:
    ...
    def __init__(self, sock, session, cookies):
        ...
        #   - TX100 COMMIT
        #
        # A fetch strategy based on either `id > last_seen_id` or `create_tx_id >
        # last_seen_tx_id` would therefore permanently miss TX100.
        #
        # To address this, the stream position is not represented by a single increasing
        # value. Instead, each fetch captures a PostgreSQL transaction snapshot
        # (`pg_current_snapshot()`).
        #
        ...
        # assigned (XMAX).
        #
        # The next fetch will only consider transactions that were uncertain during the
        # previous fetch:
        ...
        #
        # Newly subscribed channels cannot immediately adopt the current global stream
        # position, otherwise notifications committed between the subscription request and
        # the actual subscription on the server-side could be missed.
        #
        # Each new channel therefore starts from its own snapshot position and performs an
        # initial catch-up fetch. Passed the initial fetch, channels can join the
        # `active_channels` that share a single stream position.
        self._stream_position = None
        self._active_channels = set()
        self._waiting_channel_to_stream_position = {}
        # Websocket start up
        self.__selector = (
    ...
    @property
    def channels(self):
        return self._active_channels | self._waiting_channel_to_stream_position.keys()
    ...
    def subscribe(self, channels, stream_position):
        """Subscribe to the given channel. This method should only be called from within
        the websocket event loop, as part of the `subscribe` event processing.

        :param set channels: Set of channels to subscribe to.
        :param str stream_position: PostgreSQL's snapshot, used to determine the
            starting point of the notification stream.
        """
        if not self._stream_position:
            self._stream_position = stream_position
        self._active_channels &= channels
        self._waiting_channel_to_stream_position = {
            c: pos for c, pos in self._waiting_channel_to_stream_position.items() if c in channels
        }
        if new_channels := (
            channels - self._active_channels - self._waiting_channel_to_stream_position
        ):
            self._waiting_channel_to_stream_position.update(
                dict.fromkeys(new_channels, stream_position),
            )
    ...
    def _dispatch_bus_notifications(self):
        ...
        self._active_channels.update(self._waiting_channel_to_stream_position.keys())
        self._waiting_channel_to_stream_position.clear()
        if self._stream_position == snapshot and not notifications:
            return
        self._stream_position = snapshot
        self._send({'stream_position': snapshot, 'notifications': notifications})
