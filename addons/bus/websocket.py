...
class Websocket:
    ...
    def _disconnect(self, code, reason=None):
        ...
        ``self.close``.
        """
        if code not in (CloseCode.ABNORMAL_CLOSURE, CloseCode.KILL_NOW):
    ...
    def _send_control_command(self, command, data=None):
        self.__cmd_queue.put((command, next(_command_index), data))
        ...
        for message in websocket.get_messages():
            with WebsocketRequest(db, httprequest, websocket) as req:
                try:
                    ...
                    websocket._disconnect(CloseCode.SESSION_EXPIRED)
                except PoolError:
                    websocket._disconnect(CloseCode.TRY_LATER)
                except Exception:
...
def _kick_all(code=CloseCode.GOING_AWAY):
    ...
    for websocket in _websocket_instances:
        if websocket.state is ConnectionState.OPEN:
            websocket.close(ControlCommand.CLOSE, {'code': code})
