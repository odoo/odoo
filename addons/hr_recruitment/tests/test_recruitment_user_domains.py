...
@tagged('recruitment_user_domains')
class TestRecruitmentUserDomains(TransactionCase):
    ...
    def test_recruiter_domain_with_company(self):
        job = self.env['hr.job'].new({
            'company_id': self.company_a.id,
        })
        job._compute_user_domains()

        domain = job.user_id_domain
        matched_users = self.env['res.users'].search(domain)
    ...
    def test_recruiter_domain_without_company(self):
        job = self.env['hr.job'].new({
            'company_id': False
        })
        job._compute_user_domains()
