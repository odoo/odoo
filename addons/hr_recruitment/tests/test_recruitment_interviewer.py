...
class TestRecruitmentInterviewer(MailCommon):
    ...
    def test_interviewer_group(self):
        ...
        self.assertFalse(interviewer_group.id in self.simple_user.group_ids.all_implied_ids.ids, "Simple User should not be interviewer")

        self.job.interviewer_ids = self.simple_user.ids
        self.assertTrue(interviewer_group.id in self.simple_user.group_ids.all_implied_ids.ids, "Simple User should be added as interviewer")

        self.job.write({'interviewer_ids': [(5, 0, 0)]})
        self.assertFalse(interviewer_group.id in self.simple_user.group_ids.all_implied_ids.ids, "Simple User should be removed from interviewer")
        ...
        applicant.interviewer_ids = False
        self.assertTrue(interviewer_group.id in self.simple_user.group_ids.all_implied_ids.ids, "Simple User should stay interviewer")
