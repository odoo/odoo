...
class ApplicantGetRefuseReason(models.TransientModel):
    ...
    def _compute_send_mail(self):
        for wizard in self:
            ...
            wizard.send_mail = bool(template and not wizard.applicant_without_email)
    ...
    def action_refuse_reason_apply(self):
        ...
        if self.duplicates_count and self.duplicates:
            ...
            duplicate_domain = [('application_status', '!=', 'refused'), ('candidate_id', '=', applicant_id.candidate_id.id)]
            duplicates_ids = self.env['hr.applicant'].search(duplicate_domain)
            refused_applications |= duplicates_ids
            url = '/odoo/hr.applicant/%s' % (applicant_id.id)
            message = _(
                "Refused automatically because this application has been identified as a duplicate of %(link)s",
                link=Markup("<a href=%s>%s</a>") % (url, applicant_id.partner_name))
            duplicates_ids._message_log_batch(bodies={duplicate.id: message for duplicate in duplicates_ids})
