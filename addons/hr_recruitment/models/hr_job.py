...
class Job(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users",
        "Recruiter",
        domain="[('id', 'in', _allowed_user_ids)]",
        tracking=True,
    ...
    _allowed_user_ids = fields.Many2many('res.users', compute='_compute_allowed_user_ids', readonly=True)
    ...
    def _compute_allowed_user_ids(self):
        ...
        for job in self:
            job._allowed_user_ids = users_by_company.get(job.company_id, all_users)
