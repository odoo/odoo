odoo.define('gamification_quiz.quiz.question.form', function (require) {
'use strict';
...
var QuestionFormWidget = publicWidget.Widget.extend({
    template: 'gamification_quiz.quiz.question.input',
    xmlDependencies: ['/gamification_quiz/static/src/xml/quiz_create.xml'],
    events: {
        'click .o_quiz_js_quiz_validate_question': '_validateQuestion',
        'click .o_quiz_js_quiz_cancel_question': '_cancelValidation',
        'click .o_quiz_js_quiz_comment_answer': '_toggleAnswerLineComment',
        'click .o_quiz_js_quiz_add_answer': '_addAnswerLine',
        'click .o_quiz_js_quiz_remove_answer': '_removeAnswerLine',
        'click .o_quiz_js_quiz_remove_answer_comment': '_removeAnswerLineComment',
        'change .o_quiz_js_quiz_answer_comment > input[type=text]': '_onCommentChanged'
    },
    ...
    start: function () {
        ...
        return this._super.apply(this, arguments).then(function () {
            self.$('.o_quiz_quiz_question input').focus();
        });
    },
    ...
    _onCommentChanged: function (event) {
        ...
        var commentIcon = $(input).closest('.o_quiz_js_quiz_answer').find('.o_quiz_js_quiz_comment_answer');
        if (input.value.trim() !== '') {
    ...
    _toggleAnswerLineComment: function (ev) {
        var commentLine = $(ev.currentTarget).closest('.o_quiz_js_quiz_answer').find('.o_quiz_js_quiz_answer_comment').toggleClass('d-none');
    ...
    _addAnswerLine: function (ev) {
        $(ev.currentTarget).closest('.o_quiz_js_quiz_answer').after(QWeb.render('quiz.answer.line'));
    },
    ...
    _removeAnswerLine: function (ev) {
        if (this.$('.o_quiz_js_quiz_answer').length > 1) {
            $(ev.currentTarget).closest('.o_quiz_js_quiz_answer').remove();
        }
    },
    ...
    _removeAnswerLineComment: function (ev) {
        var commentLine = $(ev.currentTarget).closest('.o_quiz_js_quiz_answer_comment').addClass('d-none');
    ...
    _validateQuestion: function (ev) {
        this._createOrUpdateQuestion({
            save_and_new: $(ev.currentTarget).hasClass('o_quiz_js_quiz_create_next'),
            update: $(ev.currentTarget).hasClass('o_quiz_js_quiz_update'),
        });
    },

    /**
     * Handler when user click on the 'Cancel' button.
     * Calls a method from quiz.js widget
     ...
     * Triggers method from quiz.js to
    ...
    _createOrUpdateQuestion: function (options) {
        ...
        if (this._isValidForm($form)) {
            ...
            this._rpc({
                route: '/gamification_quiz/quiz/question_add_or_update',
                params: values
            ...
            this.$('.o_quiz_quiz_question input').focus();
        }
    },
    ...
    _isValidForm: function ($form) {
        return $form.find('.o_quiz_quiz_question input[type=text]').val().trim() !== "";
    },
     ...
     * @param $form
     * @returns {{id: *, sequence: *, question: *, object_id: *, answer_ids: Array}}
     * @private
     */
    _serializeForm: function ($form) {
        var answers = [];
        var sequence = 1;
        $form.find('.o_quiz_js_quiz_answer').each(function () {
            var value = $(this).find('.o_quiz_js_quiz_answer_value').val();
            if (value.trim() !== "") {
                var answer = {
                    'sequence': sequence++,
                    'text_value': value,
                    'is_correct': $(this).find('input[type=radio]').prop('checked') === true,
                    'comment': $(this).find('.o_quiz_js_quiz_answer_comment > input[type=text]').val().trim()
                };
                answers.push(answer);
            }
        });
        return {
            ...
            'question': $form.find('.o_quiz_quiz_question input[type=text]').val(),
            'model': this.model,
