odoo.define('gamification_quiz.quiz.finish', function (require) {
'use strict';
...
var QuizFinishModal = Dialog.extend({
    ...
    _animateProgressBar: function () {
        ...
        if (this.quiz.rankProgress.level_up) {
            this.$('.o_quiz_quiz_modal_title').text(_t('Level up!'));
            $progressBar.css('width', '100%');
            _.delay(function () {
                self.$('.o_quiz_quiz_modal_rank_lower_bound')
                    .text(self.quiz.rankProgress.new_rank.lower_bound);
                self.$('.o_quiz_quiz_modal_rank_upper_bound')
    ...
    _animateText: function () {
        var self = this;
       _.delay(function () {
            self.$('h4.o_quiz_quiz_modal_xp_gained').addClass('show in');
            self.$('.o_quiz_quiz_modal_dismiss').removeClass('d-none');
        }, 800);

        if (this.quiz.rankProgress.level_up) {
            _.delay(function () {
                self.$('.o_quiz_quiz_modal_rank_motivational').addClass('fade');
                _.delay(function () {
                    self.$('.o_quiz_quiz_modal_rank_motivational').html(
                        self.quiz.rankProgress.last_rank ?
                            self.quiz.rankProgress.description :
                            self.quiz.rankProgress.new_rank.motivational
                    );
                    self.$('.o_quiz_quiz_modal_rank_motivational').addClass('show in');
                }, 800);
            }, 800);
        }
    },
