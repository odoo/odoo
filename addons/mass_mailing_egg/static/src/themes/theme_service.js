...
// TODO EGGMAIL: use data-name directly instead of the class
...
export class ThemeModel extends Reactive {
    ...
    getThemeName(html) {
        const layout = this.computeLayout(html);
        if (!layout) {
            return;
        }
    ...
    computeLayout(html) {
        ...
        return fragment.querySelector(".o_layout");
    }

    getThemeOptions(html) {
        const themeName = this.getThemeName(html);
