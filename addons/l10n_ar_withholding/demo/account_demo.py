...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _post_load_demo_data(self, company=False):
        ...
        if company == self.env.ref('base.company_ri', raise_if_not_found=False):
            ...
            ba_wth_applied = self.with_company(company).ref('ex_tax_withholding_iibb_ba_applied')
            ...
            # Add to partner Adhoc: 1) wth tax 119 and it`s sequence, 2) ba wth
            ...
            self.env['l10n_ar.partner.tax'].create({'tax_id': ba_wth_applied.id, 'partner_id': self.env.ref('l10n_ar.res_partner_adhoc').id, 'company_id': company.id})

            # Add to partner Belgrano: 1) wth tax 78 and it`s sequence, 2) ba wth
            ...
            self.env['l10n_ar.partner.tax'].create({'tax_id': ba_wth_applied.id, 'partner_id': self.env.ref('l10n_ar.res_partner_mipyme').id, 'company_id': company.id})
