...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_l10n_ar_withholding_ids(self):
        ...
        if active_model == 'account.move.line':
            is_supplier_payment = self.line_ids.move_id.is_purchase_document()
        partner_taxes = self.env['l10n_ar.partner.tax'].search([
            *self.env['l10n_ar.partner.tax']._check_company_domain(self.company_id),
            ...
            ('tax_id.l10n_ar_withholding_payment_type', '=', 'supplier' if is_supplier_payment else 'customer')
        ])
        self.l10n_ar_withholding_ids = [Command.clear()] + [Command.create({'tax_id': x.tax_id.id}) for x in partner_taxes]

    def action_create_payments(self):
        if self.l10n_ar_withholding_ids:
            l10n_ar_states = [x.l10n_ar_state_id for x in self.l10n_ar_withholding_ids.tax_id.filtered(lambda x: x.l10n_ar_state_id)]
            repeated_states = [state for state in set(l10n_ar_states) if l10n_ar_states.count(state) > 1]
            if repeated_states:
                provinces = ", ".join([state.name for state in repeated_states])
                raise UserError(_("It is not allowed to pay more than one withholding with the same province. Repeated provinces: %s") % provinces)
