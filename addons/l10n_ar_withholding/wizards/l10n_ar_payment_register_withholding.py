...
class l10nArPaymentRegisterWithholding(models.TransientModel):
    ...
    def _tax_compute_all_helper(self):
        ...
        if self.tax_id.l10n_ar_tax_type in ['earnings_withholding', 'earnings_withholding_scale']:
            ...
            same_period_withholdings = abs(sum(self.env['account.move.line'].search([
                *self.env['account.move.line']._check_company_domain(self.tax_id.company_id),
                ('parent_state', '=', 'posted'),
                ('tax_line_id.l10n_ar_code', '=', self.tax_id.l10n_ar_code),
                ('tax_line_id.l10n_ar_tax_type', 'in', ['earnings_withholding', 'earnings_withholding_scale']),
                ('partner_id', 'child_of', self.payment_register_id.partner_id.commercial_partner_id.id),
                ('date', '<=', to_date), ('date', '>=', from_date)]).mapped('balance')))
            same_period_base = abs(sum(self.env['account.move.line'].search([
                *self.env['account.move.line']._check_company_domain(self.tax_id.company_id),
                ('parent_state', '=', 'posted'),
                ('tax_ids.l10n_ar_code', '=', self.tax_id.l10n_ar_code),
                ('tax_ids.l10n_ar_tax_type', 'in', ['earnings_withholding', 'earnings_withholding_scale']),
                ('partner_id', 'child_of', self.payment_register_id.partner_id.commercial_partner_id.id),
                ('date', '<=', to_date), ('date', '>=', from_date)]).mapped('balance')))
            net_amount = max(0, self.base_amount - self.tax_id.l10n_ar_non_taxable_amount + same_period_base)
        else:
        ...
        if self.tax_id.l10n_ar_tax_type in ['earnings_withholding', 'earnings_withholding_scale']:
            ...
            if self.tax_id.l10n_ar_tax_type == 'earnings_withholding_scale':
        ...
        l10n_ar_withholding_minimum_threshold = self.tax_id.l10n_ar_withholding_minimum_threshold
        if l10n_ar_withholding_minimum_threshold > tax_amount:
            tax_amount = 0.0
    ...
    def _compute_base_amount(self):
        for wth in self:
            if wth.tax_id.l10n_ar_tax_type == 'iibb_withholding_total':
