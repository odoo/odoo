...
class TestL10nArWithholdingArRi(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref='ar_ri'):
        ...
        cls.tax_wth_seq_test_2 = cls.env['ir.sequence'].create({
        'implementation': 'standard',
        'name': 'tax wth test',
        'padding': 8,
        'number_increment' :1,            
        ...
        cls.tax_wth_test_2 = cls.env['account.tax'].create({
            'description': 'wth_test_2',
            'name': 'withholding test 2',
            'l10n_ar_withholding': 'supplier',
            'l10n_ar_withholding_amount_type': 'total_amount',
            'l10n_ar_withholding_sequence_id': cls.tax_wth_seq_test_2.id,
            'amount': 1,                     
            'amount_type': 'percent',
        ...
        cls.company_data_3 = cls.setup_company_data(
            'company_3_data', chart_template='ar_ri', **{'country_id': cls.env.ref('base.ar').id})
        ...
        cls.in_invoice_wht_1 = cls.env['account.move'].create({
            ...
            'invoice_date': '2023-09-01',
            'partner_id': cls.partner_a.id,
            'invoice_line_ids': [(0, 0, {'product_id': cls.product_a.id, 'price_unit': 1000.0, 'tax_ids': [Command.set(cls.tax_21.ids)]})],
    ...
    def test_register_payment_wh(self):
        ...
        self.assertRecordValues(payment.line_ids.sorted('balance'), [
            # Liquidity line:
            { 'debit': 0.0, 'credit': 1200.0, 'currency_id': wizard.currency_id.id, 'amount_currency': -1200.0, 'reconciled': False}, 
            # base line:
            { 'debit': 0.0, 'credit': 1000.0, 'currency_id': wizard.currency_id.id, 'amount_currency': -1000.0, 'reconciled': False}, 
            # witholding line:
            { 'debit': 0.0, 'credit': 10.0, 'currency_id': wizard.currency_id.id, 'amount_currency': -10.0, 'reconciled': False}, 
            # base line:            
