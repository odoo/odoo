...
class TestAccountComposerPerformance(BaseMailAccountPerformance):
    ...
    def test_move_composer_multi(self):
        ...
        # QueryCount: 391 on l10n staging (l10n_generic_coa: 287 / com 323 / ent 325)
        ...
        for move, customer in zip(test_moves, test_customers):
            with self.subTest(move=move, customer=customer):
                if move.partner_id.lang == 'es_ES':
                    ...
                    _exp_move_name = f'Borrador de factura {move.name}'
                ...
                self.assertMailMail(
                    ...
                    email_values={
                        'attachments_info': [
                            ...
                            {'name': 'report.account.report_invoice_with_payments.html', 'type': 'text/plain'},
                        ],
        ...
        self.assertEqual(composer.res_id, test_moves[0].id)
    ...
    def test_move_composer_single(self):
        ...
        # QueryCount: 72 on l10n staging (l10n_generic_coa: 55 / com 57 / ent 57)
        ...
        # QueryCount: 110 on l10n staging (l10n_generic_coa: 99 / com 110)
        with self.mock_mail_gateway(mail_unlink_sent=False), \
             self.mock_mail_app():
            composer = composer_form.save()
            composer.send_and_print_action()
        ...
        self.assertEqual(composer.res_id, test_move.id)
    ...
    def test_move_composer_single_lang(self):
        ...
        # QueryCount: 74 on l10n staging (l10n_generic_coa: 56 / com 58 / ent 58)
        default_ctx = test_move.action_send_and_print()['context']
        default_ctx['default_template_id'] = move_template.id
        composer_form = Form(
            self.env['account.invoice.send'].with_context(default_ctx)
