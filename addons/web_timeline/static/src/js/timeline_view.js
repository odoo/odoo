...
openerp.web_timeline = function (session) {
        ...
        create_message_object: function (data, parent_message) {
            if (data.type === 'parent') {
                var message = new openerp.web_timeline.ThreadParent(this, _.extend(data, {'context': {
                    'default_model': data.model || false,
                    'default_res_id': data.res_id || false,
            ...
            this.parent_thread.ds_thread._model.call('message_post', [this.context.default_res_id], values)
                .done(function (message_id) {
                    ...
                    thread.message_fetch([["id", "=", message_id]], false, [message_id], 'default', function (arg, data) {
                        ...
                        // insert the message on dom
                        thread.insert_message(message, false, false, true, false);
                    });
        ...
        compute_emails_from: function () {
            ...
            if (this.parent_thread.parent_message) {
                ...
                _.each(this.options.root_thread.messages, function (msg) {
                    messages.push(msg); 
                    messages.concat(msg.get_childs());});
            }
    ...
    openerp.web_timeline.ThreadMessageCommon = openerp.web_timeline.MessageCommon.extend({
        ...
        get_childs: function () {
            return this.thread ? this.thread.get_childs() : [];
        },
        ...
        on_message_read_unread: function (read_value) {
            ...
            if (this.thread) {
                messages = this.thread.get_childs();
                if (! messages.length) {
                    messages = [this];
                    get_childs = true;
                }
            ...
            if (this.options.inbox && read_value) {
            ...
            this.ds_message.call('set_message_read', [message_ids, read_value, false, get_childs, this.context])
                .then(function (nb_read) {
                    // apply modification
                    ...
                    if (nb_read && self.parent_message && !self.options.to_me) {
                        if (read_value) self.parent_message.nb_messages -= nb_read;
                        else self.parent_message.nb_messages += nb_read;
        ...
        check_for_rerender: function () {
            ...
            return this.parent_thread.ds_message.call('message_read2', 
                [undefined, domain, [], 'child', this.context, this.parent_thread.id])
    ...
    openerp.web_timeline.ThreadParent = openerp.web_timeline.ThreadMessageCommon.extend({
        ...
        on_display_followers: function (event) {
            ...
            if (this.followers_data) {
                this.bindTooltipTo($target, this.followers_data);
                ...
                .then(function (records) {
                    ...
                    self.bindTooltipTo($target, self.followers_data);
                    $target.tooltip('hide').tooltip('show');
                    $(".tooltip").on("mouseleave", function () {
                        $(this).remove();
                    });
    ...
    openerp.web_timeline.ThreadMessage = openerp.web_timeline.ThreadMessageCommon.extend({
        ...
        on_star: function (event) {
            ...
            this.ds_message.call('set_message_starred', [[self.id], !this.is_favorite, true])
                .then(function (star) {
