...
def _nemhandel_service_group(identifier):
    return dedent(Markup('''
        ...
        <smp:ServiceGroup xmlns:smp="http://busdox.org/serviceMetadata/publishing/1.0/"
            ...
            <id:ParticipantIdentifier scheme="iso6523-actorid-upis">%s</id:ParticipantIdentifier>
...
@contextmanager
def mock_nemhandel_smp(identifiers):
    """Serve an SMP ServiceGroup for every identifier in ``identifiers``, and a 404 for any other one."""
    def registered(request):
        return next(
            (i for i in identifiers if request.url.endswith(quote_plus(f"iso6523-actorid-upis::{i}"))),
            None,
        )
    with MockHTTPClient(
        method='GET',
        matcher=lambda r: 'dataudveksling.dk' in r.url,
        return_status=lambda r: 200 if registered(r) else 404,
        return_body=lambda r: _nemhandel_service_group(registered(r)) if registered(r) else b'',
    ) as mock_response:
        yield mock_response
