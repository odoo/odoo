...
class GamificationGoalDefinition(models.Model):
    ...
    monetary = fields.Boolean(string='Monetary Value',
    ...
    computation_mode = fields.Selection([
        ...
        help="Defined how will be computed the goals. "
             "The result of the operation will be stored in the field 'Current'.")
    ...
    field_id = fields.Many2one('ir.model.fields', string='Field to Sum',
        help='The field containing the value to evaluate')
    field_date_id = fields.Many2one('ir.model.fields', string='Date Field',
        help='The date to use for the time period evaluated')
    domain = fields.Char(string="Filter Domain", default='[]', required=True,
    ...
    condition = fields.Selection([
        ...
        ], string='Goal Performance', default='higher', required=True,
    ...
    def _compute_full_suffix(self):
        for goal in self:
            suffix = goal.suffix
            monetary = goal.monetary
            if suffix and not monetary:
                goal.full_suffix = suffix
            elif monetary:
                # use the current user's company currency
                currency_symbol = self.env.user.company_id.currency_id.symbol
                if suffix:
                    goal.full_suffix = "%s %s" % (currency_symbol, suffix)
                else:
    ...
    def _check_domain_validity(self):
        for definition in self.filtered(lambda d: d.computation_mode in ('count', 'sum')):
            try:
                ...
                self.env[definition.model_id.model].search_count(domain)
    ...
    def create(self, vals):
        gd = super(GamificationGoalDefinition, self).create(vals)
        if vals.get('computation_mode') in ('count', 'sum'):
            gd._check_domain_validity()
        return gd
...
class GamificationGoal(models.Model):
    ...
    challenge_id = fields.Many2one(related='line_id.challenge_id', string="Challenge", store=True, readonly=True,
    ...
    target_goal = fields.Float(string='To Reach', required=True, track_visibility='always')  # no goal = global index
    ...
    def _check_remind_delay(self, cr, uid, goal, context=None):
        goal = self.browse(cr, uid, goal.id, context=context)
        return GamificationGoal._check_remind_delay(goal)
    ...
    def _get_write_values(self, cr, uid, goal, new_value, context=None):
        return GamificationGoal._check_remind_delay(goal, new_value)
    ...
    def _get_write_values(self, new_value):
        ...
        elif self.end_date and date.today() > fields.Date.from_string(self.end_date):
            # check goal failure
        ...
        for definition, goals in goals_by_definition.items():
            ...
            if definition.computation_mode == 'manually':
                ...
                for goal in goals:
                    ...
                    cxt = {
                        'self': self.env['gamification.goal'],
                        'object': goal,
                        'pool': self.pool,
                        'cr': self.env.cr,
