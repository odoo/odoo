...
from odoo import api, tools, SUPERUSER_ID, _
...
class WebKitParser(report_sxw):
    ...
    """
    def __init__(self, name, table, rml=False, parser=rml_parse, header=True, store=False, register=True):
        self.localcontext = {}
        report_sxw.__init__(self, name, table, rml, parser, header, store, register=register)
    ...
    def generate_pdf(self, comm_path, report_xml, header, footer, html_list, webkit_header=False):
        ...
        fd, out_filename = tempfile.mkstemp(suffix=".pdf", prefix="webkit.tmp.")
        ...
        if header:
            with tempfile.NamedTemporaryFile(suffix=".head.html", delete=False) as head_file:
                head_file.write(self._sanitize_html(header.encode('utf-8')))
            file_to_del.append(head_file.name)
            command.extend(['--header-html', head_file.name])
        if footer:
            with tempfile.NamedTemporaryFile(suffix=".foot.html", delete=False) as foot_file:
        ...
        for html in html_list:
            with tempfile.NamedTemporaryFile(suffix="%d.body.html" % count, delete=False) as html_file:
                count += 1
    ...
    def translate_call(self, parser_instance, src):
        ...
        res = ir_translation._get_source(name, 'report', parser_instance.localcontext.get('lang', 'en_US'), src)
        if res == src:
            # no translation defined, fallback on None (backward compatibility)
            res = ir_translation._get_source(None, 'report', parser_instance.localcontext.get('lang', 'en_US'), src)
        if not res:
            return src
        return res
    ...
    def create_single_pdf(self, cr, uid, ids, data, report_xml, context=None):
        ...
        found_xml_ids = env["ir.model.data"].search([["model", "=", "ir.actions.report.xml"], ["res_id", "=", report_xml.id]])
        xml_id = None
        if found_xml_ids:
            xml_id = "%s.%s" % (found_xml_ids.module, found_xml_ids.name)
        ...
        helper = WebKitHelper(cr, uid, report_xml.id, context)
        ...
        if xml_id in _extender_functions:
            for fct in _extender_functions[xml_id]:
                fct(env, cr, uid, parser_instance.localcontext, context)
    ...
    def create(self, cr, uid, ids, data, context=None):
        ...
        if not report_xml:
            return super(WebKitParser, self).create(cr, uid, ids, data, context)
