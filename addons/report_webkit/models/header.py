...
class HeaderHTML(models.Model):
    """HTML Header allows you to define HTML CSS and Page format"""

    _name = "ir.header_webkit"

    company_id = fields.Many2one('res.company', string='Company')
    html = fields.Text(string='webkit header', help="Set Webkit Report Header")
    footer_html = fields.Text(string='webkit footer', help="Set Webkit Report Footer.")
    css = fields.Text(string='Header CSS')
    name = fields.Char(required=True)
    margin_top = fields.Float(string='Top Margin (mm)')
    margin_bottom = fields.Float(string='Bottom Margin (mm)')
    margin_left = fields.Float(string='Left Margin (mm)')
    margin_right = fields.Float(string='Right Margin (mm)')
    ...
    format = fields.Selection([
        ...
        ], string='Paper size', required=True, help="Select Proper Paper size")
...
class HeaderImage(models.Model):
    ...
    name = fields.Char(required=True, help="Name of Image")
    type = fields.Char(required=True, help="Image type(png,gif,jpeg)")
