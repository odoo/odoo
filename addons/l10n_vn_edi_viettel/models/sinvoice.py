...
class L10n_Vn_Symbol(models.Model):
    ...
    @api.constrains('name', 'invoice_template_code', 'company_id')
    def _constrains_fetched_symbol_values(self):
        if self.filtered('is_fetched'):
            raise UserError(self.env._(
                'You cannot modify the symbol, template code or company of a fetched e-invoice symbol.'
            ))

    @api.ondelete(at_uninstall=False)
    def _unlink_if_fetched(self):
        if self.filtered('is_fetched'):
            raise UserError(self.env._('You cannot delete a symbol that was fetched from SInvoice. You can archive it instead.'))
