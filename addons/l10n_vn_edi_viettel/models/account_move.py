...
class AccountMove(models.Model):
    ...
    l10n_vn_edi_replacement_origin = fields.Many2one(
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        self.filtered(lambda m: m._l10n_vn_edi_is_sent() and m.l10n_vn_edi_invoice_state != 'canceled').show_reset_to_draft_button = False
    ...
    def _compute_l10n_vn_edi_invoice_state(self):
        ...
        """
        for invoice in self.filtered(lambda i: i.country_code == 'VN'):
    ...
    def _l10n_vn_edi_fetch_invoice_file_data(self, file_format):
        ...
        self.ensure_one()
        files_data = self._l10n_vn_edi_try_fetch_invoice_file_data(file_format)
    ...
    def _l10n_vn_edi_try_fetch_invoice_file_data(self, file_format):
        ...
        if not self._l10n_vn_edi_is_sent():
            raise UserError(_("In order to download the invoice's PDF file, you must first send it to SInvoice"))

        # Lock
        self._l10n_vn_edi_lock_records()

        request_response, error_message = self._l10n_vn_edi_send_request(
            method='POST',
    ...
    def _l10n_vn_edi_fetch_invoice_xml_file_data(self):
        ...
        """
        self.ensure_one()
        files_data = self._l10n_vn_edi_fetch_invoice_file_data('ZIP')
            ...
            with zipfile.ZipFile(io.BytesIO(inner_zip_bytes)) as inner_zip:
                for file in inner_zip.filelist:
                    if file.filename.endswith('.xml'):
                        return file.filename, 'application/xml', inner_zip.read(file), 'l10n_vn_edi_sinvoice_xml_file'
    ...
    def _l10n_vn_edi_fetch_invoice_pdf_file_data(self):
        ...
        """
        self.ensure_one()
        file_data = self._l10n_vn_edi_fetch_invoice_file_data('PDF')
        file_bytes = base64.b64decode(file_data['fileToBytes'])

        return file_data['fileName'], 'application/pdf', file_bytes, 'l10n_vn_edi_sinvoice_pdf_file'
    ...
    def action_l10n_vn_edi_update_payment_status(self):
        ...
        for invoice in self.filtered(lambda i: i.l10n_vn_edi_invoice_state == 'payment_state_to_update'):
            ...
            _request_response, error_message = invoice._l10n_vn_edi_send_request(
                method='POST',
                endpoint=endpoint,
                params=params,
            )
    ...
    def _need_cancel_request(self):
        # EXTEND 'account'
        return super()._need_cancel_request() or self._l10n_vn_edi_is_sent()
    ...
    def _post(self, soft=True):
        ...
        posted.filtered(
            lambda invoice:
                invoice.company_id.account_fiscal_country_id.code == 'VN'
    ...
    def _l10n_vn_edi_send_invoice(self, invoice_json_data):
        ...
        if not invoice_data:
            ...
            request_response, error_message = self._l10n_vn_edi_send_request(
                method='POST',
                endpoint=f'InvoiceAPI/InvoiceWS/createInvoice/{self.company_id.vat}',
    ...
    def _l10n_vn_edi_cancel_invoice(self, reason, agreement_document_name, agreement_document_date):
        ...
        # First, try to cancel the invoice inside Odoo
        ...
        _request_response, error_message = self._l10n_vn_edi_send_request(
            method='POST',
        ...
        self.with_context(no_new_invoice=True).message_post(
            body=_('The invoice has been canceled for reason: %s', reason),
        )
    ...
    def button_draft(self):
        ...
        cancelled_sinvoices = self.filtered(
            lambda i: i.company_id.country_id.code == 'VN' and i.state == 'cancel'
        )
    ...
    def _l10n_vn_edi_add_general_invoice_information(self, json_values):
        ...
        elif self.l10n_vn_edi_replacement_origin:  # 'Reverse and create invoice' is used to issue a replacement invoice
            adjustment_origin_invoice = self.l10n_vn_edi_replacement_origin
    ...
    def _l10n_vn_edi_add_buyer_information(self, json_values):
        ...
        buyer_information = {
            ...
            'buyerEmail': self.commercial_partner_id.email,
        ...
        if self.commercial_partner_id.bank_ids:
            bank_id = self.commercial_partner_id.bank_ids[0]
            buyer_information.update({
                'buyerBankName': bank_id.bank_name,
                'buyerBankAccount': bank_id.acc_number,
            })
    ...
    def _l10n_vn_edi_add_item_information(self, json_values):
        ...
        for line in self.invoice_line_ids.filtered(lambda ln: ln.display_type != 'line_section'):
    ...
    def _l10n_vn_edi_lookup_invoice(self):
        ...
        self.ensure_one()
        invoice_data, error_message = self._l10n_vn_edi_send_request(
            method='POST',
    ...
    @api.model
    def _l10n_vn_edi_send_request(self, method, endpoint, json_data=None, params=None):
        """ Send a request to the API based on the given parameters. In case of errors, the error message is returned. """
        url = urllib.parse.urljoin(SINVOICE_API_URL, endpoint)
        if json_data:
        ...
        if endpoint != '/auth/login':  # in which case we do not yet have the token.
            headers['Cookie'] = f'access_token={self._l10n_vn_edi_get_access_token()}'

        try:
            response = requests.request(method, url, json=json_data, params=params, headers=headers, timeout=SINVOICE_TIMEOUT)
    ...
    def _l10n_vn_edi_get_access_token(self):
        ...
        if self.company_id.l10n_vn_edi_token and self.company_id.l10n_vn_edi_token_expiry > datetime.now():
            return self.company_id.l10n_vn_edi_token

        data = {'username': self.company_id.l10n_vn_edi_username, 'password': self.company_id.l10n_vn_edi_password}
        request_response, error_message = self._l10n_vn_edi_send_request(
            method='POST',
            endpoint='/auth/login',  # The / here will cause the route to be added to the base domain of the API
            json_data=data
        )
        if error_message:
            raise UserError(error_message)
        if 'access_token' not in request_response:  # Just in case something else go wrong and it's missing the token
            raise UserError(_('Connection to the API failed, please try again later.'))

        access_token = request_response['access_token']
        access_token_expiry = datetime.now() + relativedelta(seconds=request_response['expires_in'])
    ...
    def _l10n_vn_edi_get_sent_statuses(self):
        self.ensure_one()
        return {'sent', 'payment_state_to_update', 'canceled', 'adjusted', 'replaced'}

    def _l10n_vn_edi_is_sent(self):
        """ Small helper that returns true if self has been sent to sinvoice. """
        self.ensure_one()
        return self.l10n_vn_edi_invoice_state in self._l10n_vn_edi_get_sent_statuses()

    def _l10n_vn_edi_is_edi_document(self):
        """ Small helper that returns true if self is an invoice that should interact with the EDI. """
        self.ensure_one()
        return self.company_id.account_fiscal_country_id.code == 'VN' and self.is_sale_document()

    def _l10n_vn_edi_lock_records(self):
        """ To avoid sending multiple times the same invoice from different transactions,
        we use this generic method to lock the records passed as parameter.
        """
        try:
            with self.env.cr.savepoint(flush=False):
                self._cr.execute(f'SELECT * FROM {self._table} WHERE id IN %s FOR UPDATE NOWAIT', [tuple(self.ids)])
        except OperationalError as e:
            if e.pgcode == '55P03':
                raise UserError(_("Some documents are already being processed by another user."))
            else:
                raise
