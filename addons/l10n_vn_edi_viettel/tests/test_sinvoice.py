...
class TestSInvoiceSymbol(TransactionCase):
    ...
    def _patch_sinvoice(self, mock_templates):
        ...
        with patch('odoo.addons.l10n_vn_edi_viettel.models.sinvoice.L10n_Vn_Symbol._l10n_vn_edi_lookup_symbols', **kwargs):
            yield
    ...
    def test_fetch_symbols_creates_new_symbols(self):
        ...
        with self._patch_sinvoice(self._mock_api_fetch_symbols([('C23TSA', '1/001'), ('C23TSB', '1/002')])):
            self.env['l10n_vn.symbol'].with_company(self.company_vn_1).action_fetch_symbols()

        symbols = self.env['l10n_vn.symbol'].search([('company_id', '=', self.company_vn_1.id)])
    ...
    def test_fetch_symbols_archives_removed_symbols(self):
        ...
        symbols = self.env['l10n_vn.symbol'].with_context(active_test=False).search([
    ...
    def test_fetch_symbols_multicompany(self):
        ...
        with self._patch_sinvoice(mock_lookup):
            multicompany_env['l10n_vn.symbol'].action_fetch_symbols()

        symbols_1 = self.env['l10n_vn.symbol'].search([('company_id', '=', self.company_vn_1.id)])
        symbols_2 = self.env['l10n_vn.symbol'].search([('company_id', '=', self.company_vn_2.id)])
        symbols_us = self.env['l10n_vn.symbol'].search([('company_id', '=', self.company_us.id)])
