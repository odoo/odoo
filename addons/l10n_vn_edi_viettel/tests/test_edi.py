...
class TestVNEDI(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref='vn'):
        ...
        cls.template = cls.env['sinvoice.template'].create({
            'name': '1/001',
            'template_invoice_type': '1',
        })
        cls.symbol = cls.env['sinvoice.symbol'].create({
            'name': 'K24TUT',
    ...
    def test_default_symbol_on_partner(self):
        ...
        self.partner_a.l10n_vn_edi_symbol = self.env['sinvoice.symbol'].create({
            'name': 'K24TUD',
    ...
    def test_invoice_foreign_currency(self):
        ...
        self.assertEqual(json_data['generalInvoiceInfo']['exchangeRate'], 2.0)
    ...
    def test_cancel_invoice(self):
        ...
        with patch('odoo.addons.l10n_vn_edi_viettel.models.account_move.AccountMove._l10n_vn_edi_send_request', return_value=(None, None)):
    ...
    def _send_invoice(self, invoice):
        pdf_response = 'sinvoice.pdf', 'application/pdf', b'pdf file', 'l10n_vn_edi_sinvoice_pdf_file'
        xml_response = 'sinvoice.xml', 'application/xml', b'xml file', 'l10n_vn_edi_sinvoice_xml_file'
        # the only api call we will go through here is the one to send the invoice to sinvoice. The remaining are already covered by the files.
        request_response = {
            'result': {
        ...
        with patch('odoo.addons.l10n_vn_edi_viettel.models.account_move.AccountMove._l10n_vn_edi_fetch_invoice_pdf_file_data', return_value=pdf_response), \
             ...
             patch('odoo.addons.l10n_vn_edi_viettel.models.account_move.AccountMove._l10n_vn_edi_send_request', return_value=(request_response, None)):
