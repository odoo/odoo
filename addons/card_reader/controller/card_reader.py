...
class CardReader(http.Controller):
    ...
    def payment_handler(self, **k):
        return request.env['card_reader.mercury_transaction'].do_payment(k, request.session)

    @http.route('/pos/send_reversal', type='json', auth='user')
    def reversal_handler(self, **k):
        return request.env['card_reader.mercury_transaction'].do_reversal(k, request.session, False)

    @http.route('/pos/send_voidsale', type='json', auth='user')
    def voidsale_handler(self, **k):
        return request.env['card_reader.mercury_transaction'].do_reversal(k, request.session, True)
