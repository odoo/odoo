...
class ChatbotScriptStep(models.Model):
    ...
    def _process_step_create_lead_and_forward(self, discuss_channel, posted_message):
        ...
        team = lead.team_id
        team_operators = self.env["res.users"]
        if team and lead:
            team_operators = team.crm_team_member_ids.filtered(lambda m: m._get_assignment_quota() > 0).user_id
        posted_message = self._process_step_forward_operator(discuss_channel.with_context(crm_team_id=lead.team_id), posted_message, team_operators)
        assigned_user = next((operator for operator in team_operators if operator.partner_id == discuss_channel.livechat_operator_id), False)
        if assigned_user:
            lead.sudo().user_id = assigned_user
        return posted_message
