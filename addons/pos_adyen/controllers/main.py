...
class PosAdyenController(http.Controller):
    ...
    @http.route('/pos_adyen/notification', type='json', methods=['POST'], auth='public', csrf=False)
    def notification(self):
        ...
        # Temporary fix, this whole system should be changed.
        ...
        try:
            ...
            if not pos_hmac or not consteq(pos_hmac, adyen_pm_sudo._get_hmac(msg_header['SaleID'], msg_header['ServiceID'], msg_header['POIID'], data['SaleToPOIResponse']['PaymentResponse']['SaleData']['SaleTransactionID']['TransactionID'])):
                _logger.warning('Received invalid Adyen event notification (invalid hmac): \n%s', pprint.pformat(data))
                return
            ...
            _logger.warning('Received invalid Adyen event notification: \n%s', pprint.pformat(data))
            return
