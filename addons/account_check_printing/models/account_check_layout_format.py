...
class check_layout_format(models.Model):
    ...
    _sql_constraints = [('name_uniq', 'unique(name)', 'A layout with same name already exists.'),
                        ('check_char_seperator_length', 'CHECK (length(check_date_seperator) < 2)', 'The date seperator can be of atmost one character')]
    ...
    aif_width_area = fields.Float(string="Width Area for figure", required=True)
    ...
    def _check_check_dimensions(self):
        for record in self:
            if (record.check_width + record.check_margin_left) > record.paper_width:
                raise ValidationError("The check width cannot be greater than the paper width")
            if (record.check_height + record.check_margin_top) > record.paper_height:
    ...
    def _check_payee(self):
        for record in self:
            ...
            if record.payee_width_area > record.check_width:
                raise ValidationError("The width area for payee name must be less than the check width")
    ...
    def _check_aiw(self):
        for record in self:
            ...
            if record.aiw_line1_width_area > record.check_width:
            ...
            if record.aiw_line2_width_area > record.check_width:
                raise ValidationError("The width area for amount in words line 2 must be less than the check width")

    @api.constrains('aif_dist_from_top', 'aif_dist_from_left', 'aif_width_area')
    def _check_aif(self):
        for record in self:
            ...
            if record.aif_width_area > record.check_width:
                raise ValidationError("The width area for amount in figures must be less than the check width")
