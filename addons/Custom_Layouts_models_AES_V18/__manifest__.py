{
    "name": "Custom_Layouts_models_AES_V18",
    "version": "1.0",
    "category": "Tools",
    "summary": "Agencements personnalisés pour les documents PDF",
    "description": "Ajoute de nouveaux agencements pour les factures, devis, bons, etc.",
    "author": "Alain Gansonré",
    "depends": ["base","sale","web","account", "uom"],
    "data": [
        'views/external_layout_modern.xml',
        'views/report_invoice_document_remove_due_date.xml',
        'views/external_layout_sale_order.xml',
        'views/report_invoice_numbering.xml',
        'views/report_invoice_tax.xml',
        'views/report_invoice_add_reference.xml',
        'views/report_saleorder_line_numbering.xml',
        'views/hide_invoice_totals.xml',
        'views/hide_sale_order_totals.xml',
        'views/report_invoice_modern.xml',
        'views/report_invoice_inherit.xml',
        'views/report_sale_inherit.xml',
        'views/report_hide_discount.xml',
        'views/report_sale_order_hide_discount.xml',
        'views/report_invoice_hide_incoterm.xml',
        'views/report_saleorder_hide_narration.xml',
        'views/report_invoice_hide_narration.xml',
        'data/report_layouts.xml',
        'data/report_layout_sale_order.xml',
        'data/report_saleorder_compact.xml',
        'data/paperformat_no_header.xml',
    ],
    "installable": True,
    "application": False,
    "auto_install": False,
}
