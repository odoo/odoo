...
class stock_move(osv.Model):
    ...
    def _rectify_account_entry(self, cr, uid, move, produced_price_unit, context=None):
        ...
        accounts = self.pool['stock.quant']._get_accounting_data_for_entry_move(cr, uid, move.quant_ids, move, context=context)
        if not accounts:
            return False
        ...
        for line in account_move_line.browse(cr, uid, dummy_line_matching, context=context):
            if line.credit or (not line.debit and line.account_id.id == debit_account_id):
