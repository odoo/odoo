...
class WebsiteSlidesSurvey(WebsiteSlides):
    ...
    def create_slide(self, *args, **post):
        ...
        if create_new_survey:
            ...
            if not request.env['survey.survey'].has_access('create'):
                return {'error': request.env._('You are not allowed to create a survey.')}
            ...
            try:
                ...
                return {'error': request.env._('You are not allowed to link a certification.')}
    ...
    def _slide_mark_completed(self, slide):
        if slide.slide_category == 'certification':
            raise werkzeug.exceptions.Forbidden(request.env._("Certification slides are completed when the survey is succeeded."))
