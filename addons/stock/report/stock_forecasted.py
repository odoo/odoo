...
class StockForecasted(models.AbstractModel):
    ...
    def _get_report_lines(self, product_template_ids, product_ids, wh_location_ids, wh_stock_location, read=True):
        ...
        def _reconcile_out_with_ins(lines, out, ins, demand, product_rounding, in_id_to_in_data, ins_per_product, read=True):
            ins_to_remove = []
            for in_id in ins:
            ...
            for in_id in ins_to_remove:
                ...
                for dest in in_data['move_dests']:
                    dest_ids_to_in_ids_by_product_id[product_id][dest].remove(in_id)
        ...
        dest_ids_to_in_ids_by_product_id, in_id_to_in_data = defaultdict(lambda: defaultdict(OrderedSet)), {}
        ...
        for in_ in ins:
            ...
            for dest in in_id_to_in_data[in_.id]['move_dests']:
                dest_ids_to_in_ids_by_product_id[product_id][dest].add(in_.id)
        ...
        for product in (ins | outs).product_id:
            ...
            for out in outs_per_product[product.id]:
                ...
                demand_out = _reconcile_out_with_ins(lines, out, dest_ids_to_in_ids_by_product_id[product.id][out.id], demand_out, product_rounding, in_id_to_in_data, ins_per_product, read=read)
            ...
            for (demand, out) in unreconciled_outs:
                demand = _reconcile_out_with_ins(lines, out, ins_per_product[product.id], demand, product_rounding, in_id_to_in_data, ins_per_product, read=read)
