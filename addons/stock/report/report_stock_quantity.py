...
class ReportStockQuantity(models.Model):
                ...
                WITH
                    existing_sm (id, product_id, tmpl_id, product_qty, date, state, company_id, whs_id, whd_id) AS (
                        ...
                        WHERE pt.type = 'product' AND
                            ...
                            (m.state IN ('draft', 'waiting', 'confirmed', 'partially_available', 'assigned') or m.date >= ((now() AT TIME ZONE '{timezone}')::date - interval '3month'))
                    ),
    ...
    def _from(self):
        ...
        return f"""
                FROM (SELECT
                            ...
                            END AS state,
                            (m.date AT TIME ZONE 'UTC' AT TIME ZONE '{timezone}')::date AS date,
                            CASE
                        ...
                        SELECT
                            ...
                            pp.product_tmpl_id,
                            'forecast' as state,
                            (date.* AT TIME ZONE 'UTC' AT TIME ZONE '{timezone}')::date,
                            q.quantity as product_qty,
                            q.company_id,
                            wh.id as warehouse_id
                        FROM
                            GENERATE_SERIES((now() AT TIME ZONE '{timezone}')::date - interval '3month',
                            (now() AT TIME ZONE '{timezone}')::date + interval '3 month', '1 day'::interval) date,
                            stock_quant q
                        ...
                        SELECT
                            ...
                            GENERATE_SERIES(
                            CASE
                                WHEN m.state = 'done' THEN (now() AT TIME ZONE '{timezone}')::date - interval '3month'
                                ELSE (m.date AT TIME ZONE 'UTC' AT TIME ZONE '{timezone}')::date
                            END,
                            CASE
                                WHEN m.state != 'done' THEN (now() AT TIME ZONE '{timezone}')::date + interval '3 month'
                                ELSE (m.date AT TIME ZONE 'UTC' AT TIME ZONE '{timezone}')::date - interval '1 day'
    ...
    def _table_query(self):
        ...
        """
        return '%s %s %s %s' % (self._with(), self._select(), self._from(), self._group_by())
