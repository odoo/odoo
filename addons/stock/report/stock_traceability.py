...
class StockTraceabilityReport(models.TransientModel):
    ...
    def _make_dict_move(self, level, parent_id, move_line, unfoldable=False):
        ...
        picking_partner_name = move_line.picking_partner_id.name
        location_source, location_destination = (
            (picking_partner_name, move_line.location_dest_id.name) if move_line.picking_id.picking_type_code == 'incoming'
            else (move_line.location_id.name, picking_partner_name) if move_line.picking_id.picking_type_code == 'outgoing'
