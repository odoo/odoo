...
class StockTraceabilityReport(models.TransientModel):
    ...
    def _make_dict_move(self, level, parent_id, move_line, unfoldable=False):
        ...
        data = [{
            ...
            'lot_id': move_line.lot_id.id,
            'location_source': move_line.location_id.with_context(include_wh_code=True).display_name,
            'location_destination': move_line.location_dest_id.with_context(include_wh_code=True).display_name,
            'reference_id': ref,
            'res_id': res_id,
            'res_model': res_model}]
        return data
