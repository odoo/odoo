...
class TestStockFlow(TestStockCommon):
    ...
    def test_00_picking_create_and_transfer_quantity(self):
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.productA.id), ('location_id', '=', self.stock_location.id)])
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.productB.id), ('location_id', '=', self.stock_location.id), ('quantity', '!=', 0)])
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.productC.id), ('location_id', '=', self.stock_location.id), ('quantity', '!=', 0)])
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(sum(total_qty), 2.0, 'Expecting 2.0 Unit, got %.4f Unit on location stock!' % (sum(total_qty)))
        ...
        quant = self.StockQuantObj.search([('product_id', '=', self.productD.id), ('location_id', '=', self.stock_location.id), ('quantity', '!=', 0)], limit=1)
        ...
        # Check quants and available quantity for product A.
        quants = self.StockQuantObj.search([('product_id', '=', self.productA.id), ('location_id', '=', self.stock_location.id), ('quantity', '!=', 0)])
        total_qty = [quant.quantity for quant in quants]
        ...
        # Check quants and available quantity for product D.
        quants = self.StockQuantObj.search([('product_id', '=', self.productD.id), ('location_id', '=', self.stock_location.id), ('quantity', '!=', 0)])
        total_qty = [quant.quantity for quant in quants]
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.DozA.id), ('location_id', '=', self.stock_location.id)])
        total_qty = [quant.quantity for quant in quants]
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.SDozA.id), ('location_id', '=', self.stock_location.id)])
        total_qty = [quant.quantity for quant in quants]
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.SDozARound.id), ('location_id', '=', self.stock_location.id)])
        total_qty = [quant.quantity for quant in quants]
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.gB.id), ('location_id', '=', self.stock_location.id)])
        total_qty = [quant.quantity for quant in quants]
        ...
        quants = self.StockQuantObj.search([('product_id', '=', self.kgB.id), ('location_id', '=', self.stock_location.id)])
        total_qty = [quant.quantity for quant in quants]
        ...
        backorder_wizard = self.env[(res_dict_for_back_order.get('res_model'))].browse(res_dict_for_back_order.get('res_id')).with_context(res_dict_for_back_order['context'])
        backorder_wizard.process()
        quants = self.StockQuantObj.search([('product_id', '=', productKG.id), ('location_id', '=', self.stock_location.id)])
        total_qty = [quant.quantity for quant in quants]
    ...
    def test_20_create_inventory_with_packs_and_lots(self):
        ...
        quants = self.StockQuantObj.search([('product_id', '=', packproduct.id), ('location_id', '=', self.stock_location.id), ('package_id', '=', pack1.id)])
        ...
        quants = self.StockQuantObj.search([('product_id', '=', lotproduct.id), ('location_id', '=', self.stock_location.id), ('lot_id', '=', lot1.id)])
        total_qty = sum([quant.quantity for quant in quants])
        self.assertEqual(total_qty, 10, 'Expecting 10 units lot of lotproduct, but we got %.4f on location stock!' % (total_qty))
        quants = self.StockQuantObj.search([('product_id', '=', lotproduct.id), ('location_id', '=', self.stock_location.id), ('lot_id', '=', False)])
        total_qty = sum([quant.quantity for quant in quants])
    ...
    def test_30_check_with_no_incoming_lot(self):
        ...
        picking_in = self.picking_type_in
        ...
        self.StockPackObj.create({'product_id': self.productA.id, 'move_id': move_out.id, 'product_uom_id': move_out.product_uom.id, 'lot_id': lot2.id, 'quantity': 1.0, 'location_id': self.stock_location.id, 'location_dest_id': self.customer_location.id})
        self.StockPackObj.create({'product_id': self.productA.id, 'move_id': move_out.id, 'product_uom_id': move_out.product_uom.id, 'lot_id': lot3.id, 'quantity': 2.0, 'location_id': self.stock_location.id, 'location_dest_id': self.customer_location.id})
        picking_out._action_done()
        self.StockQuantObj.search([('product_id', '=', self.productA.id), ('location_id', '=', self.stock_location.id)])
    ...
    def test_40_pack_in_pack(self):
        ...
        quant = self.StockQuantObj.search([('product_id', '=', self.productA.id), ('location_id', '=', self.customer_location.id)])
    ...
    def test_backorder_setting(self):
        ...
        """
        picking_type_ask = self.picking_type_in
    ...
    def test_assign_done_sml_and_validate_it(self):
        ...
        receipt_form = Form(self.PickingObj)
        receipt_form.picking_type_id = self.picking_type_in
