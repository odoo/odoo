...
class TestStockFlow(TestStockCommon):
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        backorder_wizard = self.env[res_dict_for_back_order.get('res_model')].with_context(res_dict_for_back_order['context']).browse(res_dict_for_back_order.get('res_id'))
    ...
    def test_80_partial_picking_without_backorder(self):
        ...
        move_a.move_line_ids.qty_done = 4
        backorder_wizard = self.env['stock.backorder.confirmation'].with_context({'to_validate_pick_ids': picking.id}).create({'pick_ids': [(4, picking.id)]})
