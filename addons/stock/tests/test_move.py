...
class TestStockMove(TestStockCommon):
    ...
    def test_link_assign_9(self):
        ...
        second only validate 2 and create a backorder for the last one. Check that the last serial
        number is reserved in full on the backorder.
        """
        ...
        # Missing 1 unit means our 3-pack is lacking 0,33, which means 0,99 units to be backordered, rounded up to 1,0.
        backordered_move._action_assign()
        lot3 = self.env['stock.lot'].search([('name', '=', "lot3")])
        self.assertEqual(backordered_move.state, 'assigned')
        self.assertRecordValues(backordered_move.move_line_ids, [
            {'quantity_product_uom': 1.0, 'uom_id': self.uom_unit.id, 'lot_id': lot3.id},
        ])
