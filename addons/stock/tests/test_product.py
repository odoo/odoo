...
class TestVirtualAvailable(TestStockCommon):
    ...
    def test_is_storable_and_tracking(self):
        """
        Play different configurations to ensure that both tracking,
        is_storable are correctly computed
        This test sometimes uses `Form` to avoid any flush mechanism
        """
        template = self.env['product.template'].create({
