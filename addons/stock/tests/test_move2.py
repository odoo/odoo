...
class TestSinglePicking(TestStockCommon):
    ...
    def test_merge_chained_moves_multi_confirm(self):
        ...
        self.assertTrue(move_receipt_1.move_dest_ids, "No move created from push rules")
        self.assertFalse(
            (move1_receipt_2 | move2_receipt_2).exists().move_dest_ids,
        ...
        self.assertEqual(
            receipt2.move_ids.filtered(lambda m: m.product_id == self.productB).mapped(
                "product_uom_qty"
            ),
            [3],
        )
        self.assertEqual(
            receipt2.move_ids.filtered(lambda m: m.product_id == self.productA).mapped(
                "product_uom_qty"
            ),
            [5],
        )
