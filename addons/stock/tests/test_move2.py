...
class TestRoutes(TestStockCommon):
    ...
    def test_mtso_mto_adjust_03(self):
        ...
        # We alter one rule and we set it to 'mts_else_mto'
        rule = self.env['procurement.group']._get_rule(product_A, final_location, {'warehouse_id': self.warehouse_1})
        rule.procure_method = 'mts_else_mto'

        self.env['stock.quant']._update_available_quantity(product_A, self.stock_location, 4.0)
