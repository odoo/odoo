...
class TestPickShip(TestStockCommon):
    ...
    def test_mto_resupply_cancel_ship(self):
        ...
        resupply_route = self.env['stock.route'].search([
            ...
            ('supplied_wh_id', '=', self.warehouse_1.id)
        ])
...
class TestStockUOM(TestStockCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.env.ref('product.decimal_product_uom').sudo().digits = 7
    ...
    def test_update_product_move_line_with_different_uom(self):
        ...
        self.env['stock.quant']._update_available_quantity(product_LtDA, self.stock_location, 149)
        # quantity in hand converted to kg is more precise than the DA
        self.env['stock.quant']._update_available_quantity(product_GtDA, self.stock_location, 149.88)
        ...
        self.env['stock.quant']._update_available_quantity(product_LtDA, self.stock_location, 149.88)
        self.env['stock.quant']._update_available_quantity(product_GtDA, self.stock_location, 149.88)
...
class TestRoutes(TestStockCommon):
    ...
    def test_cross_dock(self):
        ...
        self.env['procurement.group'].run([self.env['procurement.group'].Procurement(
            self.product1, 5, self.uom_unit, self.customer_location,
        ...
        self.assertEqual(self.env['stock.quant']._get_available_quantity(self.product1, self.customer_location), 5)
