...
registry.category("web_tour.tours").add('test_enable_product_tracking', { steps: () => [
    {
...
registry.category("web_tour.tours").add('test_disable_product_tracking', { steps: () => [
    {
        trigger: "div[name=is_storable] input",
        run: "click",
    },
    Dialog.is({ title: "Stop Tracking" }),
    Dialog.bodyIs(
        "Are you sure you want to stop tracking this product?\nAll existing traceability will be lost"
    ),
    Dialog.confirm("Yes"),
    ...stepUtils.saveForm(),
]});
