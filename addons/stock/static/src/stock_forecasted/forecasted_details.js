...
export class ForecastedDetails extends Component {
    ...
    async _unreserve(line) {
        let model = "stock.forecasted_product_product";
        let method = "action_unreserve_linked_picks";
        let args = [line.move_out.id];
    ...
    async _onClickChangePriority(modelName, record) {
        const priority = record.priority == "0" ? "1" : "0";
        await this.orm.call(modelName, "write", [[record.id], { priority }]);
        this.props.reloadReport();
    }
    ...
    isOnHand(line) {
        const productLines = this.OnHandLinesPerProduct[line.product.id];
        return productLines && productLines.includes(this.lines[this.line_index]);
    }

    isReconciled(line) {
        const productLines = this.ReconciledLinesPerProduct[line.product.id];
        return productLines && productLines.includes(this.lines[this.line_index]);
    }
