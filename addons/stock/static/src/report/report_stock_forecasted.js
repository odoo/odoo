...
export default class ReportStockForecasted extends ReportClientAction {
    setup() {
        this.context = this.props.action.context;
        this.props.name = this.props.action.name;
        this.props.display_name = this.props.action.display_name;
        ...
        this.props.report_url = `/report/html/stock.${reportName}/${this.productId}`;

        this.props.options = {
            report_name: reportName,
            report_file: this.props.report_url,
            data: {
                model_name: 'report.stock.report_product_product_replenishment',
            },
    ...
    openReplenishment(ev) {
        ...
        const on_close = (res) => {
            if (res && res.special) { // Do nothing when the wizard is discarded.
                return;
            }
        ...
        return this.trigger('do-action', { action, options: { on_close } });
    }
...
// ReportStockForecasted.template = "stock.replenish_report";

registry.category("actions").add("stock.replenish_report", ReportStockForecasted);
