        ...
        return this._super.apply(this, arguments).then(function () {
            ...
            self.$('.col-droppable').droppable({
                ...
                drop: function (e, ui) {
                    ...
                    var $headers = self.$('.col-header');
                    var $currentHeader = $headers[dateIdx - 1];
                    var $startHeader = $headers[0];
                    self._rpc({
                        model: 'stock.move',
                        method: 'reschedule',
                        args: [move_id, {
                            'date_expected': $currentHeader.dataset['date'],
                            'unfold_product_ids': unfold_product_ids,
                            'date_from': $startHeader.dataset['date']
                        }],
