...
class StockPutInPack(models.TransientModel):
    ...
    def _onchange_package_type_id(self):
        ...
        if self.package_type_id and self.package_type_id.sequence_id:
            self.package_name = False

    def action_put_in_pack(self):
        context = self._get_put_in_pack_context()
        if self.package_ids:
            return self.package_ids.with_context(**context).action_put_in_pack(self.result_package_id.id, self.package_type_id.id, self.package_name)
        return self.move_line_ids.with_context(**context).action_put_in_pack(self.result_package_id.id, self.package_type_id.id, self.package_name)
