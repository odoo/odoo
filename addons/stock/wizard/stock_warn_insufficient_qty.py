...
class StockWarnInsufficientQtyScrap(models.TransientModel):
    ...
    scrap_move_line_id = fields.Many2one('stock.move.line', 'Scrap')

    def _get_reference_document_company_id(self):
        return self.scrap_move_line_id.company_id

    def action_done(self):
        return self.with_context(clean_context(self.env.context)).scrap_move_line_id.do_scrap()

    def action_cancel(self):
        return self.scrap_move_line_id.sudo().unlink()
