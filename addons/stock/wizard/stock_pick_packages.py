...
class PickPack(models.TransientModel):
    ...
    def action_add_packages(self):
        values = self._pick_packages(self.package_ids)
        ...
        self.picking_id.move_lines._action_confirm()

    def _pick_packages(self, packages):
        result = []
        for package in packages:
            for quant in package.quant_ids:
                result.append(self._pick_quant(quant, package))
        return result

    def _pick_quant(self, quant, package, quantity=0):
        destination_location = self.env.context.get('destination_location')
        if not destination_location:
            destination_location = self.picking_id.location_dest_id.id
        common_values = {
        ...
        move_values = {
            ...
            'state': self.picking_id.state != ('draft') and self.picking_id.state or 'confirmed',
        }
        move_line_values = {
