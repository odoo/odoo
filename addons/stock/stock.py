    ...
    def create_chained_picking(self, cr, uid, moves, context=None):
        ...
        for picking, chained_moves in self._chain_compute(cr, uid, moves, context=context).items():
            ...
            for todo in moves_by_type.values():
                ...
                if pickid:
                    self.pool.get('stock.picking').signal_workflow(cr, uid, [pickid], 'button_confirm')
        if new_moves:
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.state in ('confirmed', 'waiting'):
                ...
                if res:
                    ...
                    self.invalidate_cache(cr, uid, ['location_id', 'product_qty', 'product_uos_qty'], [move.id], context=context)
