...
class StockLot(models.Model):
    ...
    def _get_new_serial(self, company, product):
        if product.tracking == 'lot':
            ...
            if exist_lot:
                return self.env['stock.lot']._get_next_serial(company, product)
            return name

        return self.env['stock.lot']._get_next_serial(company, product) or self.env['ir.sequence'].next_by_code('stock.lot.serial')
