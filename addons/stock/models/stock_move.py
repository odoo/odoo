...
class StockMove(models.Model):
    ...
    def _compute_product_qty(self):
        if self.product_uom:
            self.product_qty = self.product_uom._compute_qty_obj(self.product_uom_qty, self.product_id.uom_id)
    ...
    def _compute_string_qty_information(self):
        ...
        for move in other_moves:
            ...
            total_available = move.product_id.uom_id._compute_qty_obj(total_available, move.product_uom, round=False)
            ...
            if move.reserved_availability:
                if move.reserved_availability != total_available:
                    ...
                    reserved_available = move.product_id.uom_id._compute_qty_obj(move.reserved_availability, move.product_uom, round=False)
        ...
        for ops in operations:
            ...
            if not (ops.product_id and ops.pack_lot_ids):
                ...
                for pack_lot in ops.pack_lot_ids:
                    lot_qty[pack_lot.lot_id.id] = ops.product_uom_id._compute_qty_obj(pack_lot.qty, ops.product_id.uom_id)
                for record in ops.linked_move_operation_ids:
                    move_qty = record.qty
                    move = record.move_id
                    domain = main_domain[move.id]
    ...
    def action_done(self):
        ...
        for operation in operations:
            ...
            lot_quantities = dict((pack_lot.lot_id.id, operation.product_uom_id._compute_qty_obj(
    ...
    def split(self, qty, restrict_lot_id=False, restrict_partner_id=False):
        ...
        uom_qty = self.product_id.uom_id._compute_qty_obj(qty, self.product_uom, rounding_method='HALF-UP')
        defaults = {
