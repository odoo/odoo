    ...
    def _compute_package_ids(self):
        for move in self:
            if move.state in ['done', 'cancel']:
                move.package_ids = move.move_line_ids.package_history_id.final_dest_id
            else:
                # Only display the top-level packages until the move is done.
                move.package_ids = move.move_line_ids.result_package_id.mapped(lambda p: p.final_package_dest_id or p)
...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def action_add_packages(self):
        ...
        return {
            ...
            'view_mode': 'list',
            'views': [(self.env.ref('stock.view_stock_package_add_package_list').id, 'list')],
            'target': 'new',
