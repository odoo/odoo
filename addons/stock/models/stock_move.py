...
class StockMove(models.Model):
    ...
    def _push_apply(self):
        for move in self:
            ...
            route_ids = move.product_id.route_ids | move.product_id.categ_id.total_route_ids | move.route_ids
            if not self.env.context.get('force_company', False) and move.location_dest_id.company_id == self.env.user.company_id:
                rules = self.env['procurement.group']._search_rule(route_ids, move.product_id, warehouse_id, domain)
            else:
                rules = self.sudo().env['procurement.group']._search_rule(route_ids, move.product_id, warehouse_id, domain)
