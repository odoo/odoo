...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        consumed_quant = set()
        quants = self.env['stock.quant']._get_reserve_quantity(self.product_id, self.location_id, qty)
        for ml in self.move_line_ids:
            ml_qty = ml.quantity
            ...
            qty -= taken_qty
            if self.product_id.uom_id.compare(qty, 0.0) <= 0:
                continue
