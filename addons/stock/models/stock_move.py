...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def _set_lot_ids(self):
        for move in self:
            # NOTE: this early return is removed because even after the demand is fulfilled we should allow adding the
            # lots and assigning the ml a proper lot/serial to it. Otherwise when selecting the lot after demand is
            # fulfilled the quantity will add up +1 to it but the lot won't be assigned to the ml and thus wont't be
            # added in the Tracking column too.
            move_lines_commands = []
            mls = move.move_line_ids
            mls_with_lots = mls.filtered(lambda ml: ml.lot_id)
            mls_without_lots = (mls - mls_with_lots)
            qty_to_assign = move.quantity - move._quantity_sml()
            for ml in mls_with_lots:
                if ml.quantity and ml.lot_id not in move.lot_ids:
                    move_lines_commands.append((2, ml.id))
                    qty_to_assign -= ml.uom_id._compute_quantity(ml.quantity, move.uom_id)
            ls = move.move_line_ids.lot_id
            is_over_demand = move.quantity > move.product_uom_qty
            ...
            for lot in move.lot_ids:
                if lot not in ls:
                    if mls_without_lots[:1]:  # Updates an existing line without serial number.
                        ...
                        qty_assigned = sum(mlc[2].get('quantity', 0) for mlc in move_lines_commands if mlc[0] == 0)
                        qty_to_reserve = 1 if is_over_demand or is_serial_tracked else abs(qty_to_assign) - qty_assigned
        ...
        if self.product_id.tracking == 'serial':
            ...
            quants_by_lot = self.env['stock.quant']._read_group(
                domain=[
                    ...
                    ('lot_id', 'in', newly_added_lots._origin.mapped('id')),
