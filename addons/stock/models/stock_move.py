...
class StockMove(models.Model):
    ...
    def _qweb_prepare_qcontext(self, view_id, domain):
        ...
        for i in range(DAYS):
            headers.append({'text': self.env['ir.qweb.field.date'].value_to_html(default_expected_datetime.date(), {'format': 'dd MMMM yyyy'}),
        ...
        values['act_today'] = actions['today']
    ...
    def _prepare_actions(self, unfold_product_ids):
        ...
        actions = {
            ...
            'today': {
                ...
                'type': 'action',
                'action_id': 'stock.action_stock_forecast_qweb_redirect',
                'context': json.dumps({'date_from': False, 'unfold_poduct_ids': unfold_product_ids})
            },
            'select_period': {
                'label': _("Select date"),
                'type': 'action',
    ...
    def reschedule(self, values):
        ...
        ctx.update({'date_from': values['date_from']})
