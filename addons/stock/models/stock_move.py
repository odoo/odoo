...
Please change the quantity done or the rounding precision of your unit of measure.""",
    ...
    def _rollup_move_dests(self, seen=False):
        if not seen:
            seen = OrderedSet()
        unseen = OrderedSet(move_id for move_id in self.ids if move_id not in seen)
        if not unseen:
            return seen
        seen.update(unseen)
    ...
    def _get_forecast_availability_outgoing(self, warehouse, location_id=False):
        ...
        forecast_lines = self.env['stock.forecasted_product_product'].with_context(forecast_minimal_lines=True)._get_report_lines(False, self.product_id.ids, wh_location_query, location_id or warehouse.lot_stock_id, read=False)
