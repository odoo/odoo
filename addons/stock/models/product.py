...
class ProductTemplate(models.Model):
    ...
    tracking = fields.Selection([
        ...
        string="Tracking Method", store=True, readonly=False, precompute=True, compute='_compute_tracking',
        help="Technical field to hold the actual tracking method.")
    store_by = fields.Selection([
        ('untracked', 'None'),
        ('quantity', 'By Quantity'),
        ('lot', 'By Lots'),
        ('serial', 'By Unique Serial Number')],
        string="Tracking", compute='_compute_store_by', inverse='_set_store_by', search='_search_store_by',
    ...
    def _search_store_by(self, operator, value):
        ...
        storabilities = set()
        for v in value.pop():
            tracking_methods.add(v not in ('untracked', 'quantity') and v)
            storabilities.add(v != 'untracked')
        return ['&', ('tracking', operator, tracking_methods), ('is_storable', operator, storabilities)]
    ...
    def _set_store_by(self):
        for template in self:
            template.tracking = template.store_by not in ('untracked', 'quantity') and template.store_by
            template.is_storable = template.store_by not in ('untracked', False) and template.type == 'consu'
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if 'store_by' in vals:
                vals['tracking'] = vals['store_by'] not in ('untracked', 'quantity') and vals['store_by']
                vals['is_storable'] = vals['store_by'] != 'untracked'
        ...
        if any(product_tmpl_quantities):
            for product_tmpl, qty in zip(product_templates, product_tmpl_quantities):
                if qty > 0 and not product_tmpl.tracking:
