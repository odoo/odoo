...
class StockMoveLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        for move_line in mls:
            if move_line.move_id or (not move_line.picking_id and not move_line.env.context.get('is_scrap')):
                continue
    ...
    def _action_done(self):
        ...
        for ml in self:
            ...
            if qty_done_float_compared > 0:
                ...
                if not picking_type_id and not ml.is_inventory and not ml.lot_id and not ml.move_id.scrapped:
    ...
    def _get_linkable_moves(self):
        self.ensure_one()
        moves = self.picking_id.move_ids.filtered(lambda x: x.product_id == self.product_id and (x.scrapped if self.env.context.get('is_scrap') else True))
    ...
    def do_scrap(self):
        ...
        self.move_id.with_context(is_scrap=True)._action_done()
        for line in self:
