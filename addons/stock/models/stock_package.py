...
class StockPackage(models.Model):
    ...
    parent_package_id = fields.Many2one('stock.package', 'Container')
    ...
    @api.depends_context('formatted_display_name', 'show_dest_package')
    def _compute_display_name(self):
        ...
        for package in self:
            if package.env.context.get('formatted_display_name') and package.package_type_id and package.package_type_id.packaging_length and package.package_type_id.width and package.package_type_id.height:
                name = package.dest_complete_name if show_dest_package else package.complete_name
                package.display_name = f"{name}\t--{package.package_type_id.packaging_length} x {package.package_type_id.width} x {package.package_type_id.height}--"
            elif show_dest_package:
                package.display_name = package.dest_complete_name
            else:
                package.display_name = package.complete_name
    ...
    def _compute_content_description(self):
        for package in self:
            package_content = package.contained_quant_ids.mapped(lambda q: (q.quantity, q.product_id.display_name))
            package.content_description = ", ".join([f"{qty} {name}" for (qty, name) in package_content])
    ...
    def write(self, vals):
        if vals.get('package_type_id'):
            ...
            for package in self:
                if package.package_type_id.sequence_id != package_type.sequence_id:
                    # Only update if changed to another sequence / removed sequence.
                    # If no sequence in both package types, no need to re-generate a new name.
