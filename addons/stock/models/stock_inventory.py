        ...
        with self.env.norecompute():
            for inventory in self.filtered(lambda x: x.state not in ('done', 'cancel')):
                if (inventory.filter != 'partial') and not inventory.line_ids:
                    for val in inventory._get_inventory_lines_values():
                        val['inventory_id'] = inventory.id
                        self.env['stock.inventory.line'].create(val)
                inventory.write({'state': 'confirm', 'date': fields.Datetime.now()})
        self.recompute()
        return True
...
def _get_inventory_lines_values(self):
        ...
        domain = ' sq.location_id in %s AND pp.active = TRUE'
        ...
        self.env.cr.execute(
            """SELECT
                ...
                MIN(pt.uom_id) as product_uom_id
        ...
        for product_data in self.env.cr.dictfetchall():
            ...
            if product_data['product_id']:
                quant_product_ids.append(product_data['product_id'])
            vals.append(product_data)
        if self.exhausted:
            quant_products = Product.browse(quant_product_ids)
            exhausted_vals = self._get_exhausted_inventory_line(products_to_filter, quant_products)
            vals.extend(exhausted_vals)
        
        #prefetch type, to win time during the _check_product_id(self) on stock.inventory.line
        Product.browse(quant_product_ids).read(['type'])
        return vals
    ...
    def _get_exhausted_inventory_line(self, products, quant_products):
        ...
        #prefetch type, to win time during the _check_product_id(self) on stock.inventory.line
        exhausted_products.read(['type'])
        for product_id in exhausted_products.ids:
            vals.append({
