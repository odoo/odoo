...
class StockPickingType(models.Model):
    ...
    return_picking_type_id = fields.Many2one(
        ...
        check_company=True)
    allow_pack_in_pack = fields.Boolean('Multi-level Package', help="Allows to do a new put in pack if destination package is already set")
...
class StockPicking(models.Model):
    ...
    def _compute_packages_count(self):
        packages_by_pick = self.env['stock.move.line']._read_group([('picking_id', 'in', self.ids), ('picking_id.state', '!=', 'done')], ['picking_id'], ['result_package_id:count_distinct'])
        packages_by_pick = dict(packages_by_pick)

        for picking in self:
            if picking.state == 'done':
                # TODO QUWO: Need some special handling for histories, as there isn't a direct like like for `result_package_id`
                picking.packages_count = 0
    ...
    def _check_entire_pack(self):
        ...
        for package in self.move_line_ids.package_id:
            ...
            if pickings._check_move_lines_map_quant_package(package):
                ...
                if not package.package_type_id or package.package_type_id.package_use == 'disposable':
    ...
    def action_add_entire_packs(self, packages):
        ...
        if self.state not in ('done', 'cancel'):
            ...
            all_packages.picking_ids = [Command.link(self.id)]
            move_line_vals = self._prepare_entire_pack_move_line_vals(all_packages)
            self.env['stock.move.line'].create(move_line_vals)
            all_package_ids = set(all_packages.ids)
            for package in all_packages:
                # Need to set the current parent to dest package to keep the structure of the whole package tree.
                if package.parent_package_id.id in all_package_ids:
    ...
    def _prepare_entire_pack_move_line_vals(self, packages):
        ...
        for package_quant in packages.quant_ids:
            move_line_vals.append({
                ...
                'quantity': package_quant.quantity,  # TODO QUWO: available_quantity so that we don't over-reserve?
