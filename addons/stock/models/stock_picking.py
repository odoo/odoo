...
class StockPicking(models.Model):
    ...
    def _compute_products_availability(self):
        ...
        for picking in pickings:
            ...
            if any(
                ...
                ) == -1 for move in picking.move_ids
            ):
    ...
    def _compute_show_check_availability(self):
        ...
        for picking in self:
            ...
            picking.show_check_availability = any(
                ...
                float_compare(move.product_uom_qty, 0, precision_digits=self.env['decimal.precision'].precision_get('Product Unit of Measure'))
                for move in picking.move_ids
            )
    ...
    def _get_lot_move_lines_for_sanity_check(self, none_done_picking_ids, separate_pickings=True):
        ...
        def get_line_with_done_qty_ids(move_lines):
            ...
            return move_lines.filtered(
                ...
                ml.picked and
                float_compare(ml.quantity, 0, precision_digits=self.env['decimal.precision'].precision_get('Product Unit of Measure'))
            ).ids
    ...
    def button_validate(self):
        ...
        for move in draft_picking.move_ids:
            if float_is_zero(move.quantity, precision_digits=precision_digits) and\
               not float_is_zero(move.product_uom_qty, precision_digits=precision_digits):
    ...
    def action_split_transfer(self):
        precision_digits = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        if all(float_is_zero(m.quantity, precision_digits=precision_digits) for m in self.move_ids):
        ...
        if all(float_compare(m.quantity, m.product_uom_qty, precision_digits=precision_digits) == 0 for m in self.move_ids):
        ...
        if any(float_compare(m.quantity, m.product_uom_qty, precision_digits=precision_digits) > 0 for m in self.move_ids):
