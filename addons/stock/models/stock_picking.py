...
class StockPickingType(models.Model):
    ...
    sequence_prefix = fields.Char('Sequence Prefix', related='sequence_id.prefix', readonly=False)
        ...
        for vals in vals_list:
            if not vals.get('sequence_id') and vals.get('sequence_code'):
                if vals.get('warehouse_id'):
                    ...
                    vals['sequence_prefix'] = wh.code + '/' + vals['sequence_code'] + '/'
                else:
                    vals['sequence_id'] = self.env['ir.sequence'].sudo().create({
                    ...
                    vals['sequence_prefix'] = vals['sequence_code']
