...
class StockPicking(models.Model):
    ...
    volume_uom_name = fields.Char(string='Volume unit of measure label', compute='_compute_volume_uom_name', readonly=True)
    ...
    def _compute_weight(self):
        pickings_to_update = self.filtered(lambda picking: picking.state not in ('done', 'cancel'))
    ...
    def _compute_bulk_volume(self):
        ...
        for picking in self:
            picking.volume_bulk = picking_volumes[picking.id]

    def _get_result_package_ids_weight_and_volume(self):
        result_package_ids_weights = defaultdict(float)
        result_package_ids_volumes = defaultdict(float)
        result_package_ids = self.move_line_ids.result_package_id

        res_groups = self.env['stock.move.line']._read_group(
                [('result_package_id', 'in', result_package_ids.ids), ('product_id', '!=', False), ('picking_id', 'in', self.ids)],
                ['result_package_id', 'product_id', 'uom_id', 'quantity'],
                ['__count'],
        )
        for package, product, product_uom, quantity, count in res_groups:
            result_package_ids_weights[package.id] += (
                count
                * product_uom._compute_quantity(quantity, product.uom_id)
                * product.weight
            )

            result_package_ids_volumes[package.id] += (
                count
                * product_uom._compute_quantity(quantity, product.uom_id)
                * product.volume
            )

        return result_package_ids_weights, result_package_ids_volumes
    ...
    def _compute_shipping_weight(self):
        ...
        result_package_ids_weights, __ = self._get_result_package_ids_weight_and_volume()
        ...
        for picking in self:
            ...
            shipping_weight += sum(packages_weights[package.id] for package in outermost_packages)
            picking.shipping_weight = shipping_weight

    @api.depends(
        'move_line_ids.product_id.volume',
    ...
    def _compute_shipping_volume(self):
        outermost_packages = self.move_line_ids.result_package_id.outermost_package_id
        __, result_package_ids_volumes = self._get_result_package_ids_weight_and_volume()
