...
class Picking(models.Model):
    ...
    @api.depends('move_line_ids', 'move_lines', 'picking_type_id.use_create_lots', 'picking_type_id.use_existing_lots', 'state')
    def _compute_show_lots_text(self):
        group_production_lot_enabled = self.user_has_groups('stock.group_production_lot')
        for picking in self:
            if picking.move_lines.origin_returned_move_id:
                picking.show_lots_text = False
            elif not picking.move_line_ids:
                picking.show_lots_text = False
