            ...
            if not vals.get('sequence_id') and vals.get('sequence_code'):
                if vals.get('warehouse_id'):
                    ...
                    vals['sequence_id'] = self.env['ir.sequence'].sudo().create({
                        'name': _('%(warehouse_name)s Sequence %(sequence_code)s', warehouse_name=wh.name, sequence_code=vals['sequence_code']),
                        'prefix': wh.code + '/' + vals['sequence_code'] + '/', 'padding': 5,
                        'company_id': wh.company_id.id,
                    }).id
                else:
                    vals['sequence_id'] = self.env['ir.sequence'].sudo().create({
                        'name': _('Sequence %(sequence_code)s', sequence_code=vals['sequence_code']),
            ...
            for picking_type in self:
                if picking_type.warehouse_id:
                    picking_type.sequence_id.sudo().write({
                        'name': _('%(warehouse_name)s Sequence %(sequence_code)s', warehouse_name=picking_type.warehouse_id.name, sequence_code=vals['sequence_code']),
