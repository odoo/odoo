...
class StockQuant(models.Model):
    ...
    def _write_specific_fields(self, vals):  # TODO rename: _update_quant_by_moves
        """ Handle update of more specific fields"""
        # if any key of vals in specific fields => Move+ & Move- with all values
        move_vals = []
        for quant in self:
            generic_location = quant.product_id.with_company(quant.company_id).property_stock_inventory
            # 'Undo' actual quant
            move_vals.append(
            ...
            move_vals.append(
                quant._get_inventory_move_values(
                    ...
                    generic_location,
                    self.env['stock.location'].browse(vals['location_id']) if 'location_id' in vals else quant.location_id,  # vals.get('location_id', quant.location_id),
                    package_id=self.env['stock.quant.package'].browse(vals['package_id']) if 'package_id' in vals else quant.package_id,  # package_id=vals.get('package_id',quant.package_id),
                    product_id=self.env['product.product'].browse(vals['product_id']) if 'product_id' in vals else quant.product_id,  # product_id=vals.get('product_id', quant.product_id),
                    lot_id=self.env['stock.lot'].browse(vals['lot_id']) if 'lot_id' in vals else quant.lot_id,  # vals.get('location_id', quant.location_id),
                    owner_id=self.env['res.partner'].browse(vals['owner_id']) if 'owner_id' in vals else quant.owner_id,  # vals.get('location_id', quant.location_id),
                )
            )
    ...
    def write(self, vals):
        ...
        if self._is_inventory_mode():
            if any(quant.location_id.usage == 'inventory' for quant in self):
                ...
                return
            if any(field in vals.keys() for field in self._get_specific_fields_write()):
                self._write_specific_fields(vals)
            self = self.sudo()
    ...
    def _get_inventory_move_values(self, qty, location_id, location_dest_id, package_id=False, package_dest_id=False, product_id=False, lot_id=False, owner_id=False):
        ...
        return {
            ...
            'product_uom': product_id.uom_id if product_id else self.product_uom_id.id,
            'product_uom_qty': qty,
            ...
            'move_line_ids': [(0, 0, {
                'product_id': product_id.id if product_id else self.product_id.id,
                'product_uom_id': product_id.uom_id if product_id else self.product_uom_id,
                'quantity': qty,
                'location_id': location_id.id,
                'location_dest_id': location_dest_id.id,
                'company_id': self.company_id.id or self.env.company.id,
