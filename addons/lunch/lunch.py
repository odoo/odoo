...
class lunch_order(osv.Model):
    ...
    def get_previous_lunch_order_details(self, cr, uid, context=None):
        ...
        if pref_ids:
            ...
            for pref in preferences:
                product_obj = pref.product_id
                categories.setdefault(product_obj.category_id.name, {})
                detail = {
                    'name': product_obj.name,
                    'price': product_obj.price,
                    'pref_id': pref.id,
                }
                if product_obj.id in categories[product_obj.category_id.name]:
                    if pref.note not in categories[product_obj.category_id.name][product_obj.id]:
                        categories[product_obj.category_id.name][product_obj.id][pref.note] = detail
                else:
                    categories[product_obj.category_id.name][product_obj.id] = {}
                    categories[product_obj.category_id.name][product_obj.id][pref.note] = detail
...
class lunch_order_line(osv.Model):
    ...
    _columns = {
        'name': fields.related('product_id', 'name', readonly=True, type='char'),
