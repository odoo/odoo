...
class LunchOrderWizard(models.TransientModel):
    ...
    topping_ids_1 = fields.Many2many('lunch.topping', string="Extra Garniture",
                                   domain="[('category_id', '=', product_category), ('label', '=', 1)]",
                                   default=lambda self: self._default_order_line().topping_ids_1)
    topping_ids_2 = fields.Many2many('lunch.topping', string="Extra Garniture 2",
                                     domain="[('category_id', '=', product_category), ('label', '=', 2)]",
                                     default=lambda self: self._default_order_line().topping_ids_2)
    topping_ids_3 = fields.Many2many('lunch.topping', string="Extra Garniture 3",
                                     domain="[('category_id', '=', product_category), ('label', '=', 3)]",
    ...
    def _compute_available_toppings(self):
        for wizard in self:
            wizard.available_toppings_1 = bool(wizard.env['lunch.topping'].search_count([('category_id', '=', wizard.product_category.id), ('label', '=', 1)]))
    ...
    def _check_topping_quantity(self):
        ...
        for wizard in self:
            for index in range(1, 4):
                ...
                toppings = wizard['topping_ids_%s' % index].filtered(lambda x: x.label == index)
