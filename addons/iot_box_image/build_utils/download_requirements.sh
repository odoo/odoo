...
# Download and extract the piCore image if not present (recovery).
if ! file_exists *piCore*.img ; then
    wget "http://tinycorelinux.net/14.x/armv6/releases/RPi/piCore-14.1.0.zip" -O piCore.zip
    unzip piCore.zip 'piCore-14.1.0.img'
    rm -f piCore.zip # cleanup
fi
PICORE=$(echo *piCore*.img)
