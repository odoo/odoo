...
# Unfortunately labwc doesn't support hiding the mouse cursor, and the previous
# solution (unclutter) doesn't work with Wayland. For now, this hack of setting
# all the mouse cursor images to transparent is the only workaround.
# We can remove this when this feature is merged: https://github.com/labwc/labwc/issues/2614
cd /usr/share/icons/Adwaita/cursors
for FILENAME in *; do rm $FILENAME; ln -s transparent $FILENAME; done
wget https://github.com/celly/transparent-xcursor/raw/refs/heads/master/transparent
