...
from odoo.addons.hr_time.tests.common import TestHrHolidaysCommon
...
class TestAccrualAllocationsAttendance(TestHrHolidaysCommon):
    ...
    def test_frequency_hourly_attendance(self):
        with freeze_time("2017-12-05"):
            accrual_plan = self.env['hr.time.accrual.plan'].with_context(tracking_disable=True).create({
            ...
            allocation = self.env['hr.time.allocation'].with_user(self.user_hrmanager_id).with_context(tracking_disable=True).create({
    ...
    def test_accrual_allocation_based_on_attendance(self):
        accrual_plan = self.env['hr.time.accrual.plan'].create({
        ...
        with Form(self.env['hr.time.allocation'].with_user(self.user_hrmanager)) as allocation_form:
    ...
    def test_accrual_allocation_with_overlapping_attendance(self):
        ...
        with freeze_time("2024-4-1"):
            allocation = self.env['hr.time.allocation'].create({
                'name': 'Accrual allocation for employee',
                'accrual_plan_id': accrual_plan.id,
                'employee_id': self.employee_emp.id,
                'work_entry_type_id': self.work_entry_type.id,
