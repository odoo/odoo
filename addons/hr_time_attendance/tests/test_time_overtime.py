...
class TestHolidaysOvertime(HttpCase, TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = new_test_user(cls.env, login='user', groups='base.group_user,hr_time.group_hr_time_employee', company_id=cls.company.id).with_company(cls.company)
        cls.user_manager = new_test_user(cls.env, login='manager', groups='base.group_user,hr_time.group_hr_time_user,hr_attendance.group_hr_attendance_manager', company_id=cls.company.id).with_company(cls.company)
    ...
    def test_allocation_check_overtime_write(self):
        ...
        alloc = self.env['hr.time.allocation'].create({
    ...
    def test_leave_check_cancel(self):
        ...
        self.env['hr.time.cancel.leave'].with_user(self.user).with_context(default_leave_id=leave.id) \
