...
class HrTimeAttendanceReport(models.Model):
    _name = "hr.time.attendance.report"
    ...
    leave_ids = fields.Many2many("hr.time", string="Time Offs", compute="_compute_leave_attendance_fields")
    ...
    def _compute_leave_attendance_fields(self):
        ...
        leaves_by_employees = dict(self.env['hr.time']._read_group(
            domain=[
        ...
        for rec in self:
            leaves = leaves_by_employees.get(rec.employee_id, self.env['hr.time'])
