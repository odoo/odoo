...
class HrTimeAllocation(models.Model):
    ...
    def write(self, vals):
        ...
        if 'number_of_days' not in vals:
            return res
        if not self.env.user.has_group("hr_time.group_hr_time_user") and any(allocation.state not in ('draft', 'confirm') for allocation in self):
