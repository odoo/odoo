...
class Employee(models.Model):
    ...
    def _get_employee_calendar(self, date_from=None, date_to=None):
        calendars = super()._get_employee_calendar(date_from=date_from, date_to=date_to)
        ...
        contracts = self.sudo().contract_ids.filtered_domain([
            ('date_start', '<=', date_to),
            '|',
            ('date_end', '=', False),
            ('date_end', '>=', date_from),
        ...
        contract_calendars = sorted(
            [{
                'from': employee_tz_to_utc(datetime.combine(contract.date_start, datetime.min.time()), self),
                'to': contract.date_end and employee_tz_to_utc(datetime.combine(contract.date_end, datetime.max.time()), self),
