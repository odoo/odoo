...
class StockLot(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for lot in self.filtered(lambda lot: lot.use_expiration_date and lot.expiration_date):
            if now >= lot.expiration_date:
                lot.display_name += self.env._("\t--Expired--")
            elif lot.alert_date and now >= lot.alert_date:
                lot.display_name += self.env._("\t--Expire on %(date)s--", date=fields.Datetime.to_string(lot.expiration_date))
