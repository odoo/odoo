...
class AccountEdiFormat(models.Model):
    ...
    def _post_invoice_edi(self, invoices):
        ...
        if response.get("error"):
            ...
            if "4002" in error_codes:
                ...
                response = self._l10n_in_edi_get_ewaybill_by_irn(invoices.company_id, generate_json.get('Irn'))
    ...
    def _l10n_in_edi_generate_irn_ewaybill(self, company, json_payload):
        ...
        elif json_payload.get("TransMode") == '1' and not (json_payload.get("VehNo") and json_payload.get("VehType")):
            return {'error': [{
                'code': 'required',
                'message': _(
                    "Vehicle Number and Vehicle Type is required when E-waybill Transportation Mode is By Road"
                )}
            ]}
        elif json_payload.get("TransMode") in ('2', '3', '4') and not (
            json_payload.get("TransDocDt") and json_payload.get("TransDocNo")):
            return {'error': [{
                'code': 'required',
                'message': _(
                    "Transport document number and date is required "
                    "when E-waybill Transportation Mode is Rail,Air or Ship"
                )}
            ]}
        elif not (json_payload.get("TransId") or json_payload.get("TransMode")):
            return {'error': [{
                'code': 'required',
