...
class L10nInProductHsnReport(models.Model):
    ...
    def _from(self):
        from_str = """FROM account_move_line aml
            ...
            WHERE (aa.account_type NOT IN ('asset_receivable','liability_payable','asset_cash','liability_credit_card')) AND (aml.tax_line_id IS NOT NULL OR mt.account_tax_id IS NULL)
              AND am.state = 'posted'
        """
        return from_str
