...
class TestFiscal(L10nInTestInvoicingCommon):
    ...
    def test_l10n_in_fiscal_in_vendor_bills(self):
        ...
        vendor_bill_3.write({
            'partner_id': self.partner_a,  # Inter-State Partner
            'l10n_in_state_id': company_res_state.id,  # Company State
            # But due to SEZ GST Treatment, Fiscal Position should be `Export/SEZ`
            'l10n_in_gst_treatment': 'special_economic_zone',
        })
        self.assertEqual(
            vendor_bill_3.fiscal_position_id,
            self.env['account.chart.template'].ref('fiscal_position_in_export_sez_in')
        )
