...
class ProductTemplate(models.Model):
    ...
    def _compute_l10n_in_hsn_warning(self):
        ...
        for record in self:
            ...
            if (
                record._origin
                and 'IN' in record.fiscal_country_codes
                and record.sale_ok
                and not record.l10n_in_hsn_code
                and record.taxes_id
            ):
                warning = _("It is preferred to add HSN Code when Sales Tax(es) are applied to the product.")
            elif (
