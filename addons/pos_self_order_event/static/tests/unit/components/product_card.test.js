...
test("selectProduct adds a regular product to the cart", async () => {
...
test("selectProduct navigates to the event page for an event product", async () => {
    const store = await setupSelfPosEnv("kiosk", "counter", "each", {}, true);
    store.computeAvailableCategories();
    // A product that is not in the cart yet, so "nothing was added" is a real assertion
    // and not just `addToCart` merging into an existing line.
    ...
    patchWithCleanup(store.router, {
        navigate(routeName, routeParams) {
            navigations.push([routeName, routeParams]);
        },
    });

    comp.selectProduct();
    expect(navigations).toEqual([["event_page", { id: 1 }]]);
