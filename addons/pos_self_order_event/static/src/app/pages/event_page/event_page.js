import { Component, onWillUnmount, useState, useRef, onMounted } from "@odoo/owl";
...
export class EventPage extends Component {
    ...
    static components = { ComboStepper };
    ...
    setup() {
        ...
        onMounted(() => this.initObserver());
        onWillUnmount(() => this.cleanupObserver());
    }

    initObserver() {
        const el = this.eventNameRef.el;
        if (!el) {
            return;
        }
        this.observer = new IntersectionObserver(
            ([entry]) => (this.state.showStickyTitle = !entry.isIntersecting),
            { threshold: 0 }
        );
    ...
    cleanupObserver() {
        if (this.observer) {
            this.observer.unobserve(this.eventNameRef.el);
        }
    }
    get eventTickets() {
    ...
    seatAvailability(ticket) {
        ...
        if (this.event.is_multi_slots && this.state.selectedSlot) {
            ...
            return this.state.slotTicketAvailabilities?.[ticket.id]?.[slotId] ?? 0;
        }
        return ticket.seats_available || ticket.seats_max === 0;
    }
    ...
    async fetchSlotAvaiblity(slot) {
        ...
        const slotTicketAvailabilities = await this.selfOrder.data.call(
            "event.event",
            "get_slot_tickets_availability_pos",
            [this.event.id, slotTickets]
        );
