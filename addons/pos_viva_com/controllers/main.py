        ...
        if payment_method_sudo:
            if request.httprequest.data:
                ...
                if event_type != 1796:  # Transaction Payment Created
                    ...
                    _logger.error(request.env._('received a message for a terminal not registered in Odoo: %s', terminal_id))
            return json.dumps({'Key': payment_method_sudo.viva_com_webhook_verification_key})
        else:
            _logger.error(request.env._('received a message for a pos payment provider not registered.'))
