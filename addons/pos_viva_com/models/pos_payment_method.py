...
class PosPaymentMethod(models.Model):
    ...
    viva_com_latest_response = fields.Json() # used to buffer the latest asynchronous notification from Adyen.
...
def get_verification_key(endpoint, viva_com_merchant_id, viva_com_api_key):
    ...
    auth = requests.auth.HTTPBasicAuth(viva_com_merchant_id, viva_com_api_key)
    try:
        response = requests.get(f"{endpoint}/api/messages/config/token", auth=auth, timeout=TIMEOUT)
