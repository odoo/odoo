...
PortalChatter.include({
     ...
     */
     _preprocessMessage: function (message, index) {
        this._super.apply(this, arguments);
        if (this.options.display_rating) {
            message.rating_value = this.roundToHalf(message.rating_value);
            message.rating = this._preprocessCommentData(message.rating, index);
        }
    },
    ...
    _preprocessCommentData: function (rawRating, messageIndex) {
        ...
        var ratingData = {
            ...
            publisher_datetime: publisher_datetime_moment ? publisher_datetime_moment.format('MMMM Do YYYY, h:mm:ss a') : "",
