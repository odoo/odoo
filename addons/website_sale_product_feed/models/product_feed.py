...
class ProductFeed(models.Model):
    ...
    def _compute_url(self):
        ...
        for feed in self:
            ...
            feed.url = (
                feed.website_id.get_base_url().rstrip('/')
    ...
    def _render_gmc_feed(self):
        ...
        gmc_data = {
            ...
            'link': (
                self.website_id.get_base_url().rstrip('/')
    ...
    def _prepare_gmc_items(self):
        ...
        base_url = self.website_id.get_base_url().rstrip('/')
        ...
        def format_product_link(url_):
            ...
            return base_url + self.env['ir.http']._url_lang(url_, lang_code=self.lang_id.code)
    ...
    def _prepare_gmc_image_links(self, product, base_url):
        ...
        return {
            ...
            'image_link': (
                base_url + product._get_image_1920_url() if product.image_128 else ''
            ),
            # Supports up to 10 extra images
            'additional_image_link': [
                base_url + url for url in product._get_extra_image_1920_urls()[:10]
            ],
        }
