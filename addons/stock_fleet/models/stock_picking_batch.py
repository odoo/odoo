...
class StockPickingBatch(models.Model):
    ...
    def _compute_dock_id(self):
        for batch in self:
            if batch.picking_type_id != batch._origin.picking_type_id:
                batch.dock_id = False
            if batch.picking_ids:
                if len(batch.picking_ids.location_id) == 1 and batch.picking_ids.location_id in batch.allowed_dock_ids:
