...
class BaiwangClient:
    ...
    def issue_invoice(self, invoice_data: dict):
        ...
        # If proxy itself failed (network, config), raise so account_move doesn't mark it 'failed'
        if not result.get('success') and 'response' not in result:
            raise UserError(f"Baiwang proxy error: {result.get('error', 'Unknown error')}")

        # Pass the Baiwang payload (success or business failure) up
    ...
    def add_red_confirmation(self, red_form_data: dict):
        ...
        # If proxy itself failed (network, config), raise so account_move doesn't mark it 'failed'
        if not result.get('success') and 'response' not in result:
            raise UserError(f"Baiwang red form submission failed: {result.get('error', 'Unknown error')}")
