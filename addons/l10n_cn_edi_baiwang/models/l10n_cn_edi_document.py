...
class L10nCnEdiDocument(models.Model):
    ...
    def _cron_check_red_form_status(self):
        ...
        for company, docs in docs_by_company.items():
            ...
            for doc in docs:
                try:
                    self._l10n_cn_baiwang_process_pending_doc(client, doc)
                except UserError as e:
                    _logger.error("Baiwang EDI: Error polling UUID %s: %s", doc.baiwang_uuid, e)

    @api.model
    def _l10n_cn_baiwang_process_pending_doc(self, client, doc):
        """Query one red form pending confirmation, update its state, and notify the accountant."""
    ...
    def _pull_inbound_red_forms(self):
        ...
        for company in companies:
            ...
            try:
                self._l10n_cn_baiwang_pull_inbound_forms(company, start_date, end_date)
            except Exception as e:  # ruff: ignore[blind-except]
                _logger.error("Baiwang EDI: Error pulling inbound red forms for company %s: %s", company.name, e)

    @api.model
    def _l10n_cn_baiwang_pull_inbound_forms(self, company, start_date, end_date):
        """Create tracking documents for inbound red forms from the seller side and notify the buyer."""
        client = BaiwangClient(company)
