...
class TestPdpPosFlowLifecycle(TestPoSCommon):
    ...
    def test_pos_sale_and_refund_report_the_net_transaction(self):
        with self._patch_pos_date('2025-09-03'):
            session = self.open_new_session()
            orders_data = [
                self.create_ui_order_data([(self.pos_product, 1)]),
                self.create_ui_order_data([(self.pos_product, 1)]),
            ]
            created_orders = self.env['pos.order'].create_from_ui(orders_data)
            order = self.env['pos.order'].browse(created_orders[0]['id'])
        ...
        self.assertEqual(len(session.order_ids), 3)
        transaction = self._transaction_nodes(session.move_id.l10n_fr_pdp_last_flow_id)[0]
        self.assertAlmostEqual(float(transaction.findtext('TaxExclusiveAmount')), 100.0, places=2)
        self.assertAlmostEqual(float(transaction.findtext('TaxTotal')), 20.0, places=2)
