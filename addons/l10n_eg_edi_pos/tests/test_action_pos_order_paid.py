...
class TestL10nEgEdiPosActionPaid(TestL10nEgEdiPosCommon):
    ...
    def test_refund_with_business_buyer_does_not_raise(self):
        """The business-buyer guard only blocks new sales (``not refunded_order_id``).
        A refund of a business receipt that was created outside the guard — e.g. an
        order that predates EG EDI enablement, or was imported — must still submit.
        Both orders carry the business partner so the return-partner-match check in
        check_data is satisfied."""
        original = self._create_unpaid_order(partner=self.eg_individual_customer)
        with self._mock_eta(send_response=self._eta_accepts_any_uuid()):
            self._pay(original)
        original.partner_id = self.eg_business_customer
        refund = self._refund_of(original)  # inherits the business partner → match OK
        with self._mock_eta(send_response=self._eta_accepts_any_uuid()):
            self._pay(refund)
