...
class PosOrder(models.Model):
    ...
    def _l10n_eg_edi_pos_send(self):
        ...
        data = self._l10n_eg_edi_pos_post_submission(payload, token)
        if isinstance(data, bytes):
            response_data = json.loads(data)
        else:
            response_data = data
        envelope = {'request': payload, 'response': response_data}
        ...
        error_message = self._l10n_eg_edi_pos_postprocess_response(response_data)
    ...
    def _l10n_eg_edi_pos_post_submission(self, payload, token):
        ...
        client = ETAClient(
            is_production=not self.config_id.l10n_eg_edi_pos_preprod
        )

        return client.submit_receipt(
    ...
    def _l10n_eg_edi_pos_get_qr_url(self):
        ...
        return client.get_receipt_url(
            uuid=self.l10n_eg_edi_pos_uuid,
            date_order=self.date_order.strftime('%Y-%m-%dT%H:%M:%SZ'),
