...
class TestCounterGenerator(TransactionCase):
    ...
    def test_counter_zero_step_raises(self):
        with self.assertRaises(ValueError):
            Counter(target=self.stock_field, env=self.env, step=0)

    def test_counter_positive_step_end_before_start_raises(self):
        with self.assertRaises(ValueError):
            Counter(target=self.stock_field, env=self.env, start=5, step=1, end=3)

    def test_counter_negative_step_end_after_start_raises(self):
        with self.assertRaises(ValueError):
            Counter(target=self.stock_field, env=self.env, start=0, step=-1, end=5)
    ...
    def test_unique_counter_subjobs_use_disjoint_strides(self):
        ...
        generator_a = Counter(target=self.stock_field, env=None, job=job_a, start=10.0, step=5.0, unique=True)
        generator_b = Counter(target=self.stock_field, env=None, job=job_b, start=10.0, step=5.0, unique=True)
...
class TestCycleGenerator(TransactionCase):
    ...
    def test_cycle_loops_through_values(self):
        generator = Cycle(target=self.name_field, env=self.env, values=['a', 'b', 'c'])

        values = [generator.next({}) for _ in range(6)]
        self.assertEqual(values, ['a', 'b', 'c', 'a', 'b', 'c'])

    def test_cycle_single_value(self):
        generator = Cycle(target=self.name_field, env=self.env, values=['only'])

        values = [generator.next({}) for _ in range(3)]
        self.assertTrue(all(v == 'only' for v in values))

    def test_cycle_numeric_values(self):
        generator = Cycle(target=self.price_field, env=self.env, values=[1.0, 2.5, 3.75])
    ...
    def test_cycle_empty_values_raises(self):
        with self.assertRaises(ValueError):
            Cycle(target=self.name_field, env=self.env, values=[])

    def test_cycle_weights_raises(self):
        with self.assertRaises(ValueError):
            Cycle(target=self.name_field, env=self.env, values={'a': 1, 'b': 2})

    def test_cycle_null_ratio_is_zero(self):
        generator = Cycle(target=self.name_field, env=self.env, values=['x'])
        self.assertEqual(generator.null_ratio, 0)
...
class TestEvalGenerator(TransactionCase):
    ...
    def test_eval_generator(self):
        generator = Eval(target=self.name_field, env=self.env, expr='"Test Product"')
    ...
    def test_eval_generator_numeric(self):
        generator = Eval(target=self.price_field, env=self.env, expr='99.99')

        value = generator.next({})
        self.assertEqual(value, 99.99)

    def test_eval_generator_simple_expression(self):
        generator = Eval(target=self.price_field, env=self.env, expr='42 + 69 if True else 420')
    ...
    def test_eval_generator_dynamic_no_accept_lambda(self):
        with self.assertRaises(ValueError):
            Eval(target=self.name_field, env=self.env, expr='lambda x: 42')

    def test_static_generator_unique_raises_error(self):
        with self.assertRaises(ValueError) as cm:
            Eval(target=self.name_field, env=self.env, expr='"Static Value"', unique=True)
