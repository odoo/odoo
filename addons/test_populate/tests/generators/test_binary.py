...
class TestBinaryGenerator(TransactionCase):
    ...
    def test_binary_default_size(self):
        generator = Binary(target=self.binary_field, env=self.env)
    ...
    def test_binary_custom_size(self):
        generator = Binary(target=self.binary_field, env=self.env, size=512)
        value = generator.next({})
    ...
    def test_binary_null_ratio(self):
        generator = Binary(target=self.binary_field, env=self.env, null_ratio=1.0)
    ...
    def test_binary_invalid_size_raises(self):
        with self.assertRaises(ValueError):
            Binary(target=self.binary_field, env=self.env, size=0)

        with self.assertRaises(ValueError):
            Binary(target=self.binary_field, env=self.env, size=-10)
...
class TestImageGenerator(TransactionCase):
    ...
    def test_image_default_dimensions(self):
        generator = Image(target=self.binary_field, env=self.env)
        value = generator.next({})
    ...
    def test_image_custom_dimensions(self):
        generator = Image(target=self.binary_field, env=self.env, width=32, height=16)
        value = generator.next({})
    ...
    def test_image_null_ratio(self):
        generator = Image(target=self.binary_field, env=self.env, null_ratio=1.0)
        values = [generator.next({}) for _ in range(20)]
    ...
    def test_image_invalid_dimensions_raises(self):
        with self.assertRaises(ValueError) as cm:
            Image(target=self.binary_field, env=self.env, width=0, height=64)
        self.assertIn("positive integers", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            Image(target=self.binary_field, env=self.env, width=64, height=-1)
        self.assertIn("positive integers", str(cm.exception))
