...
class ConfirmDeliveryMultipleWizard(models.TransientModel):
    _name = 'print_docsaway.multiple_wizard'
    _rec_name = 'remaining'
    ...
    def _default_free_count(self):
        return self.env['v']._get_free_count()
    ...
    wiz_ids = fields.One2many('print_docsaway.single_wizard', 'wizard_id', string='Mails')
    ...
    def _on_change_ink(self):
        ...
        wiz_id = self.id
        my_wiz_ids = self.env['print_docsaway.single_wizard'].search([('wizard_id','=',wiz_id)])
        if len(my_wiz_ids) == 0:
            model_name = self._context.get('active_model', None)
            report_model = self._context.get('report_model', None)
            active_ids = self._context.get('active_ids', [])
            self.wiz_ids = self.env['print.docsaway']._prepare_multiple_deliveries(active_ids, model_name, report_model, wiz_id, ink=self.ink)
        else:
