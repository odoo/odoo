...
class ResCompany(models.Model):
    ...
    def _l10n_pt_sale_check_hash_integrity(self):
        ...
        self.env['sale.order']._l10n_pt_compute_missing_hashes(self.id)
        ...
        for at_series_line in at_series_lines:
            ...
            while order_ids := self.env.execute_query(SQL("FETCH %s FROM hashed_orders", INTEGRITY_HASH_BATCH_SIZE)):
                ...
                if not orders and not last_order:
                    results.append({
                        ...
                        'status': 'no_data',
                        'msg_cover': _('No sale orders found for this AT series.'),
                    })
                    continue
            ...
            if corrupted_order:
                results.append({
                    ...
                    'status': 'corrupted',
                    'msg_cover': _(
                        "Corrupted data on sales order with id %(id)s (%(name)s).",
                ...
                results.append({
                    ...
                    'series_at_code': at_series_line._get_at_code(),
                    'status': 'verified',
                    'msg_cover': _("Sales Orders are correctly hashed"),
    ...
    def _verify_hashed_sales_order(self, order, previous_hash, versioning_list, current_versioning_index):
        ...
        message = pt_hash_utils.get_message_to_hash(order.date_order, order.l10n_pt_hashed_on,
                                                    order._get_l10n_pt_sale_document_number(), order.amount_total,
                                                    previous_hash)
        return pt_hash_utils.verify_integrity(message, order.l10n_pt_sale_inalterable_hash,
