...
class SaleOrderLine(models.Model):
    ...
    @api.depends('order_id.l10n_pt_global_discount', 'l10n_pt_line_discount')
    def _compute_discount(self):
        ...
        """
        for line in self:
            global_discount = (line.order_id.l10n_pt_global_discount or 0.0) / 100
...
class SaleOrder(models.Model):
    ...
    def _action_cancel(self):
        ...
        action = self.env['ir.actions.actions']._for_xml_id('l10n_pt.action_l10n_pt_cancel')
        action['context'] = {
            'model': 'sale.order',
            'order_ids': self.ids,
        }
    ...
    def action_l10n_pt_create_sales_order(self):
        ...
        sales_order = self.with_context(create_sales_order=True).copy(default={'quotation_id': self.id})
    ...
    def _inverse_l10n_pt_global_discount(self):
        self.env.add_to_compute(self.env['sale.order.line']._fields['discount'], self)
        self.env.add_to_compute(self.env['sale.order.line']._fields['price_subtotal'], self)
        ...
        for (document_type, series), orders in sales_orders.grouped(lambda o: (o.l10n_pt_document_type, o.l10n_pt_at_series_id)).items():
            ...
            if at_series_line:
                ...
                raise RedirectWarning(
                    _("There is no AT series for the document type %(order_type)s registered under the series name %(series_name)s. "
                      "Create a new series or view existing series via the Accounting Settings.",
                      order_type=dict(orders._fields['l10n_pt_document_type'].selection).get(document_type),
    ...
    def _get_integrity_hash_fields(self):
        if self.company_id.account_fiscal_country_id.code != 'PT':
            return []
        return ['date_order', 'l10n_pt_hashed_on', 'name', 'l10n_pt_document_number', 'amount_total']
    ...
    @api.model
    def _find_last_order(self, company_id, at_series_line):
    ...
    def _l10n_pt_compute_missing_hashes(self, company_id=None):
        """
    ...
    def _compute_l10n_pt_qr_code_str(self):
        ...
        for order in self.filtered(lambda o: (
            ...
            pt_hash_utils.verify_prerequisites_qr_code(order, order.l10n_pt_sale_inalterable_hash, order.l10n_pt_atcud)
