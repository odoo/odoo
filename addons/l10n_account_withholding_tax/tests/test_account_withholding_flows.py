...
class TestL10nAccountWithholdingTaxesFlows(TestTaxCommon, AnalyticCommon):
    ...
    def test_tax_repartition_on_refund(self):
        """ Test that the partner is set on the lines generated by the withholding tax on payment system. """
        base_tag_plus = self.env["account.account.tag"].create({
            "name": "+Base Tag",
            "applicability": "taxes",
        })
        base_tag_minus = self.env["account.account.tag"].create({
            "name": "-Base Tag",
            "applicability": "taxes",
            "tax_negate": True,
        })
        tax_tag_plus = self.env["account.account.tag"].create({
            "name": "+Tax Tag",
            "applicability": "taxes",
        })
        tax_tag_minus = self.env["account.account.tag"].create({
            "name": "-Tax Tag",
            "applicability": "taxes",
            "tax_negate": True,
        })
        withholding_tax = self.percent_tax(
            ...
            withholding_sequence_id=self.withholding_sequence.id,
            invoice_repartition_line_ids=[
                Command.create({"repartition_type": "base", "factor_percent": 100.0, "tag_ids": base_tag_plus.ids}),
                Command.create({"repartition_type": "tax", "factor_percent": 100.0, "tag_ids": tax_tag_plus.ids}),
            ],
            refund_repartition_line_ids=[
                Command.create({"repartition_type": "base", "factor_percent": 100.0, "tag_ids": base_tag_minus.ids}),
                Command.create({"repartition_type": "tax", "factor_percent": 100.0, "tag_ids": tax_tag_minus.ids}),
            ],
        )
        invoice = self.env["account.move"].create({
        ...
        self.assertRecordValues(payment.move_id.line_ids, [
            ...
            {"balance": 1000.0,      "tax_tag_ids": []},
            # withholding line:
            {"balance": -10.0,       "tax_tag_ids": tax_tag_plus.ids},
            # base lines:
            {"balance": -1000.0,     "tax_tag_ids": base_tag_plus.ids},
            {"balance": 1000.0,      "tax_tag_ids": []},
        ])
        ...
        self.assertRecordValues(payment.move_id.line_ids, [
            ...
            {"balance": -1000.0,    "tax_tag_ids": []},
            # withholding line:
            {"balance": 10.0,       "tax_tag_ids": tax_tag_minus.ids},
            # base lines:
            {"balance": 1000.0,     "tax_tag_ids": base_tag_minus.ids},
            {"balance": -1000.0,    "tax_tag_ids": []},
        ])
