...
class SlidesPortalChatter(PortalChatter):
    ...
    def portal_chatter_post(self, thread_model, thread_id, post_data, **kw):
        ...
        result = super().portal_chatter_post(thread_model, thread_id, post_data, **kw)
    ...
    def mail_update_message(self, thread_model, thread_id, message_id, post_data):
        ...
        pid = int(portal_security['pid']) if portal_security.get('pid') else False
        if not _check_special_access(thread_model, thread_id, portal_security.get('token'), portal_security.get('hash'),
                                     pid):
            raise Forbidden()
