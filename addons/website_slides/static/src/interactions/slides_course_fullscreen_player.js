...
var VideoPlayerYouTube = publicWidget.Widget.extend({
    ...
    start: function () {
        var self = this;
        return Promise.all([this._super.apply(this, arguments), this.loadYoutubeAPI()]).then(
            function () {
    ...
    onPlayerStateChange(event) {
        ...
        if (event.data !== YT.PlayerState.ENDED) {
            ...
            this.tid = setInterval(() => {
                ...
                if (this.totalVideoTime && this.currentVideoTime > this.totalVideoTime - 30) {
                    clearInterval(this.tid);
...
var VideoPlayerVimeo = publicWidget.Widget.extend({
    ...
    start: function () {
        return this._super.apply(arguments).then(this.setupVideoPlayer.bind(this));
    },
...
var Sidebar = publicWidget.Widget.extend({
    events: {
        "click .o_wslides_fs_sidebar_list_item .o_wslides_fs_slide_name": "onTabClick",
    },
    init: function (parent, slideList, defaultSlide) {
        var result = this._super.apply(this, arguments);
        this.slides = slideList;
        this.slide = defaultSlide;
        return result;
    },
    start() {
        var self = this;
        return this._super.apply(this, arguments).then(function () {
            $(document).keydown(self.onKeyDown.bind(self));
        });
    },
    destroy() {
        $(document).unbind("keydown", this.onKeyDown.bind(this));
        return this._super.apply(this, arguments);
    },
    /**
    ...
    _onClickMiniQuiz: function (ev) {
        ...
        this.trigger_up("change_slide", this.slide);
    },
    /**
    ...
    updateSlide(slide) {
        ...
        var selector =
            ".o_wslides_fs_sidebar_list_item[data-id=" + slide.id + '][data-is-quiz!="1"]';
...
var Fullscreen = SlideCoursePage.extend({
        ...
        slidesDataList.forEach(function (slideData, index) {
            ...
            if (slideData.category === "video" && slideData.videoSourceType !== "vimeo") {
                ...
                slideData.embedUrl = slideData.embedCode
                    ? scheme + slideData.embedCode + separator + $.param(params)
                    : "";
    ...
    _renderSlide: async function () {
        ...
        try {
            ...
            if (["document", "infographic"].includes(slide.category)) {
                $content
                    .empty()
                    .append(renderToElement("website.slides.fullscreen.content", { widget: this }));
                ...
                if (slide._autoSetDone && !session.is_website_user) {
                    ...
                    if (slide.category === "document") {
                        ...
                        self.el
                            ...
                            .addEventListener("load", () => self.toggleSlideCompleted(slide));
                    } else {
                        return self.toggleSlideCompleted(slide);
                    }
                }
            });
    },
     ...
     */
    toggleSlideCompleted: async function (slide, completed = true) {
