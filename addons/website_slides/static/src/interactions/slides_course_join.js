...
import { SlidesCourseJoinPopup } from "@website_slides/js/components/course_join_popup";
...
export class WebsiteSlidesCourseJoinLink extends Interaction {
    ...
    async onJoinClick() {
        if (
            ...
            this.joinChannel(this.channel.id);
            return;
        }
        ...
        if (data.error) {
            if (data.error === "public_user") {
                this.popover.add(this.el, SlidesCourseJoinPopup, {
                    ...
                    isPartnerWithoutUser: this.user.isPartnerWithoutUser,
                });
