...
import { htmlEscape, htmlMarkup } from "@web/core/utils/html";
    ...
    async onClick(slideId, voteType) {
        ...
        if (!data.error) {
            ...
            if (data.error === "public_user") {
                const tags = {
                    a_login_open: htmlMarkup`<a href="/web/login?redirect=${encodeURIComponent(
                        document.URL
                    )}">`,
                    a_login_close: htmlMarkup`</a>`,
                    a_signup_open: htmlMarkup`<a href="/web/signup?redirect=${encodeURIComponent(
                        document.URL
                    )}">`,
                    a_signup_close: htmlMarkup`</a>`,
                };
                this.showAlert(
