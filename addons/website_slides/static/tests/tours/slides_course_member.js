...
import { dispatch, queryAll, queryOne } from "@odoo/hoot-dom";
...
// eLeaning: course completion
{
    ...
    run: function () {
        ...
        if (queryAll('.o_wslides_fs_sidebar_list_item.active:contains("Gardening: The Know-How")').length === 0) {
            return;
        }
    ...
    run: function () {
        ...
        if (queryAll('.o_wslides_fs_sidebar_list_item.active:contains("Home Gardening")').length === 0) {
            return;
        }
        // getting here means that navigation worked
        document
    ...
    run: function () {
        ...
        setTimeout(function () {
            dispatch(document, "keydown", {key: "ArrowRight"})
            // check that it selected the next/next tab
            if (queryAll('.o_wslides_fs_sidebar_list_item.active:contains("Mighty Carrots")').length === 0) {
                return;
            }
            // getting here means that navigation worked
            queryOne('.o_wslides_fs_sidebar_header').classList.add('navigation-success-3');
        }, 300);
    }
}, {
