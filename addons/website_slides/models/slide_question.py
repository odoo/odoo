...
class SlideQuestion(models.Model):
    ...
    # statistics
    attempts_count = fields.Integer(compute='_compute_statistics', store=True)
    attempts_avg = fields.Float(compute="_compute_statistics", digits=(6, 2), store=True)
    done_count = fields.Integer(compute="_compute_statistics", store=True)
    ...
    @api.depends('slide_id.slide_partner_ids.quiz_attempts_count')
