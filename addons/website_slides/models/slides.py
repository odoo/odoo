...
class Category(models.Model):
    ...
    def _count_presentations(self):
        result = dict.fromkeys(self.ids, dict())
        res = self.env['slide.slide'].read_group(
            [('website_published', '=', True), ('category_id', 'in', self.ids), ('channel_id', '=', self.channel_id.id)],
...
class Slide(models.Model):
    ...
    download_security = fields.Selection(
        [('none', 'No One'), ('user', 'Authentified User Only'), ('public', 'Everyone')],
    ...
    def send_share_email(self, email):
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        self.channel_id.share_template_id.with_context({'email': email, 'base_url': base_url}).send_mail(self.id)
    ...
    def _parse_youtube_document(self, document_id, only_preview_fields):
        key = 'AIzaSyDOWlmDW-7DbLmOR9ZsT5AOEXf4n6TFwQA'  # TDE: config.parameter
    ...
    def _parse_google_document(self, document_id, only_preview_fields):
        ...
        key = 'AIzaSyDOWlmDW-7DbLmOR9ZsT5AOEXf4n6TFwQA'  # TDE FIXME: config parameter
