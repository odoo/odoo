...
class ResPartner(models.Model):
    ...
    def _get_iap_vies_credentials(self):
        ...
        if (
            not self.env.ref('l10n_eu_account_vies.vies_iap_check_update', raise_if_not_found=False)
    ...
    def _get_iap_vies_endpoint(self):
        ...
        default_endpoint = test if self.env.ref('base.module_l10n_eu_account_vies').demo else prod
        ...
        try:
            req = requests.post(
                ...
                data={
                    ...
                    'webhook_url': self.get_base_url() + '/l10n_eu_account_vies/1/webhook_update_vies',
                    # See L10nEUAccountViesWebhookController
    ...
    def _get_vat_required_valid(self, company=None):
        ...
        vat_required_valid = super()._get_vat_required_valid(company)
        if (
