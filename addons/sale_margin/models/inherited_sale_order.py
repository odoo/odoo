...
class SaleOrderLine(models.Model):
    ...
    margin = fields.Float(
        compute='_product_margin', digits=dp.get_precision('Product Price'), store=True)
    ...
    def _product_margin(self):
        ...
        self.margin = currency.round(
            self.price_subtotal - ((self.purchase_price or self.product_id.standard_price) * self.product_uos_qty))
...
class SaleOrder(models.Model):
    ...
    def _product_margin(self):
        self.margin = sum(
            line.margin for line in self.order_line if line.state != 'cancel')
