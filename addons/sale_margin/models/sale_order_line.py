...
class SaleOrderLine(models.Model):
    ...
    margin = fields.Float(
        ...
        digits=0, min_digits='Product Price', store=True, groups="base.group_user", precompute=True)
    margin_percent = fields.Float(
        "Margin (%)", compute='_compute_margin', store=True, groups="base.group_user", precompute=True)
    purchase_price = fields.Float(
        string="Cost", compute="_compute_purchase_price",
        digits=0, min_digits='Product Price', store=True, readonly=False, copy=False, precompute=True,
