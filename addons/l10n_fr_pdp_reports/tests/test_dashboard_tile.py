...
class TestPdpDashboardTile(PdpTestCommon):
    def test_dashboard_tile_visibility_flags(self):
        ...
        self.company.write({"l10n_fr_pdp_enabled": True, "country_id": self.env.ref("base.be").id})
    ...
    def test_dashboard_warning_and_danger_thresholds(self):
        ...
        base_today = fields.Date.from_string("2025-03-05")
        with patch("odoo.fields.Date.context_today", return_value=base_today):
            ...
            flow.invalidate_recordset(["next_deadline_start", "next_deadline_end"])
            ...
            self.assertFalse(data.get("pdp_has_danger"))
