...
class L10n_Vn_Symbol(models.Model):
    _name = 'l10n_vn.symbol'
    _description = 'Vietnam E-Invoice Symbol'
    """
    ...
    def _compute_is_used(self):
        used_symbol_ids = set()
        if self.ids:
            linked_symbols = self.env['account.move']._read_group(
                domain=[
                    ('l10n_vn_symbol_id', 'in', self.ids),
                    ('state', '=', 'posted'),
                ],
                groupby=['l10n_vn_symbol_id'],
            )
    ...
    def _constrains_symbol_values(self):
        if self.filtered(lambda symbol: symbol.is_used):
            raise UserError(self.env._(
                'You cannot modify the symbol, template code or company of an already used e-invoice symbol.'
            ))

    @api.constrains('usage')
    def _constrains_symbol_usage(self):
        if self.filtered(lambda symbol: symbol.is_used):
            raise UserError(self.env._(
                'You cannot modify the usage of an already used e-invoice symbol.'
            ))

    @api.ondelete(at_uninstall=False)
    def _unlink_if_used(self):
        if self.filtered(lambda symbol: symbol.is_used):
            raise UserError(self.env._('You cannot delete a symbol that is used on a posted invoice or vendor bill. You can archive it instead.'))
