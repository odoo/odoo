...
class AccountMove(models.Model):
    ...
    l10n_vn_symbol_usage = fields.Selection(
        selection=[
            ('invoice', 'Invoice'),
            ('vendor_bill', 'Vendor Bill'),
        ],
        compute='_compute_l10n_vn_symbol_usage',
        copy=False,
    )
    l10n_vn_symbol_id = fields.Many2one(
        string='E-Invoice Symbol',
        comodel_name='l10n_vn.symbol',
        compute='_compute_l10n_vn_symbol_id',
        readonly=False,
        store=True,
        copy=False,
        tracking=True,
    ...
    def _compute_l10n_vn_symbol_usage(self):
        for move in self:
            if move.country_code != 'VN':
                move.l10n_vn_symbol_usage = False
                continue
            if move.is_sale_document():
                move.l10n_vn_symbol_usage = 'invoice'
            elif move.is_purchase_document():
                move.l10n_vn_symbol_usage = 'vendor_bill'
            else:
                move.l10n_vn_symbol_usage = False

    @api.depends('company_id', 'journal_id', 'move_type')
    def _compute_l10n_vn_symbol_id(self):
        """ Use the property l10n_vn_symbol_id to set a default invoice symbol. """
        for move in self:
            if move.country_code != 'VN':
                move.l10n_vn_symbol_id = False
                continue
            symbol = move.l10n_vn_symbol_id
            if move.is_sale_document():
                # Use journal's default symbol, fallback to company symbol if not set
                move.l10n_vn_symbol_id = (
                ...
                move.l10n_vn_symbol_id = symbol if symbol.usage == 'vendor_bill' else False
            else:
                move.l10n_vn_symbol_id = False
