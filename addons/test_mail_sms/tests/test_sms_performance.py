...
class TestSMSPerformance(BaseMailPerformance, sms_common.SMSCase):
    ...
    def test_message_sms_record_1_partner(self):
        ...
        with self.subTest("QueryCount"), self.mockSMSGateway(sms_allow_unlink=True), self.assertQueryCount(employee=37):
    ...
    def test_message_sms_record_default(self):
        record = self.test_record.with_user(self.env.user)
        with self.subTest("QueryCount"), self.mockSMSGateway(sms_allow_unlink=True), self.assertQueryCount(employee=38):
            messages = record._message_sms(
                body='Performance Test',
            )
