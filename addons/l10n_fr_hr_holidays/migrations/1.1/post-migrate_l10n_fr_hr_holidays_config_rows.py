...
COLUMNS = ("l10n_fr_reference_leave_type",)
...
def migrate(cr, version):
    ...
    env["l10n_fr_hr_holidays.config"]._for_each(companies)
    env.flush_all()
    assignments = ", ".join(f"{column} = c.{column}" for column in present)
    cr.execute(
        f"UPDATE l10n_fr_hr_holidays_config x SET {assignments} FROM res_company c WHERE c.id = x.company_id"
    )
    env.invalidate_all()
