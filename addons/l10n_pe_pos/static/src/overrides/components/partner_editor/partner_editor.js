...
patch(PartnerEditor.prototype, {
    setup() {
        ...
        this.changes.l10n_pe_district =
            this.props.partner.l10n_pe_district && this.props.partner.l10n_pe_district.id;
        return res;
    },
    async confirm() {
        if (!this.pos.isPeruvianCompany()) {
        ...
        const city = this.pos.models["res.city"].find(
            (city) => city.id === parseInt(this.changes.city_id)
        );
        const district = this.pos.models["l10n_pe.res.city.district"].find(
            (district) => district.id === parseInt(this.changes.l10n_pe_district)
        );
        if (
            !city ||
            parseInt(this.changes.state_id) !== city.state_id.id ||
            parseInt(this.changes.country_id) !== city.country_id.id
        ) {
