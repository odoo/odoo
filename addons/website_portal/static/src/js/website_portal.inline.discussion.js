...
(function () {
    ...
    qweb = openerp.qweb;
    website.add_template_file('/website_portal/static/src/xml/website_portal.inline.discussion.xml');
    website.portal_discussion = openerp.Class.extend({
        init: function(options) {
            var self = this ;
            self.discus_identifier;
            var defaults = {
                position: 'right',
                post_id: $('#portal_post_name').attr('data-portal-id'),
                content : false,
                public_user: false,
            };
        ...
        do_render: function(data) {
            ...
            if ($('#discussions_wrapper').length === 0 && self.settings.content.length > 0) {
                $('<div id="discussions_wrapper"></div>').insertAfter($('#portal_content'));
            }
        ...
        prepare_data : function(identifier, comment_count) {
            var self = this;
            return openerp.jsonRpc("/portalpost/get_discussion/", 'call', {
        ...
        prepare_multi_data : function(identifiers, comment_count) {
            var self = this;
            return openerp.jsonRpc("/portalpost/get_discussions/", 'call', {
                'post_id': self.settings.post_id,
                'paths': identifiers,
                'count': comment_count, //if true only get length of total comment, display on discussion thread.
            })
        ...
        get_discussion : function(source, callback) {
            ...
            elt.append(qweb.render("website.portal_discussion.popover", {'identifier': identifier , 'options': self.settings}));
            var comment = '';
            self.prepare_data(identifier,false).then(function(data){
                _.each(data, function(res){
                    comment += qweb.render("website.portal_discussion.comment", {'res': res});
                });
        ...
        post_discussion : function(identifier) {
            ...
            if(!val) return
            openerp.jsonRpc("/portalpost/post_discussion", 'call', {
                'portal_post_id': self.settings.post_id,
                'path': self.discus_identifier,
                'comment': val[0],
                'name' : val[1],
                'email': val[2],
            }).then(function(res){
                $(".popover ul.media-list").prepend(qweb.render("website.portal_discussion.comment", {'res': res[0]}))
