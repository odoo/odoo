...
class TestLatePaymentPenalties(AccountTestInvoicingCommon):
    ...
    def _set_manual_rate(self, company, rate):
        company.write({
            'l10n_fr_pdp_manual_late_payment_penalties_rate': rate,
    ...
    def test_rate_mode_can_be_changed_from_accounting_settings(self):
        ...
        self.assertEqual(
            self.company.l10n_fr_pdp_manual_late_payment_penalties_rate,
            12.4,
        )
        self.assertFalse(
    ...
    def test_manual_rate_is_snapshotted_when_invoice_is_posted(self):
        ...
        self.company.l10n_fr_pdp_manual_late_payment_penalties_rate = 15
    ...
    def test_iap_rate_is_requested_once_for_current_semester(self):
        ...
        with (
            ...
            patch(
                'odoo.addons.l10n_fr_pdp_invoice.models.late_payment_penalty_rate.iap_tools.iap_jsonrpc',
        ...
        self.assertRecordValues(
            self.env['l10n_fr_pdp.late.payment.penalty.rate'].search([]),
            [{
    ...
    def test_iap_rates_are_requested_in_batch(self):
        ...
        self._set_manual_rate(company_2, 11)
        company_2.peppol_eas = 'odemo'
        manual_invoice = self._create_invoice(
        ...
        with (
            ...
            (manual_invoice | first_invoice | second_invoice).action_post()

        self.assertEqual(manual_invoice.l10n_fr_pdp_late_payment_penalties_rate, 11)
        ...
        self.assertRecordValues(
            self.env['l10n_fr_pdp.late.payment.penalty.rate'].search([], order='period_start'),
            [
                {
    ...
    def test_billing_user_can_update_rate_cache(self):
        first_invoice = self._create_invoice(invoice_date=date(2026, 7, 1))
        second_invoice = self._create_invoice(invoice_date=date(2026, 7, 15))
        response = [{
            'period_start': '2026-07-01',
    ...
    def test_automatic_rate_does_not_replace_company_manual_rate(self):
        self.company.write({
            'l10n_fr_pdp_manual_late_payment_penalties_rate': 12.4,
            'l10n_fr_pdp_late_payment_penalties_automatic': True,
        })
    ...
    def test_iap_failure_keeps_rate_and_retries_on_next_invoice(self):
        self.company.write({
            'l10n_fr_pdp_manual_late_payment_penalties_rate': 12.15,
            'l10n_fr_pdp_late_payment_penalties_automatic': True,
        })
        ...
        with (
            patch('odoo.fields.Date.today', return_value=date(2026, 7, 15)),
            mute_logger(
                'odoo.addons.l10n_fr_pdp_invoice.models.late_payment_penalty_rate'
            ),
            patch(
                'odoo.addons.l10n_fr_pdp_invoice.models.late_payment_penalty_rate.iap_tools.iap_jsonrpc',
    ...
    def test_invalid_iap_response_is_ignored(self):
        with (
            mute_logger(
                'odoo.addons.l10n_fr_pdp_invoice.models.late_payment_penalty_rate'
            ),
            ...
            rates = self.env['l10n_fr_pdp.late.payment.penalty.rate']._fetch_rates(
                self.company,
                [date(2026, 7, 1)],
            )
