...
class AccountJournal(models.Model):
    ...
    compatible_edi_ids = fields.Many2many(comodel_name='account.edi.format',
                                          ...
                                          help='EDI format that supports transfers for this Operation type')
    ...
    def write(self, vals):
        ...
        if vals.get('edi_format_ids'):
            ...
            documents = self.env['account.edi.document'].search([
                ('picking_id.picking_type_id', 'in', self.ids),
    ...
    def _compute_compatible_edi_ids(self):
        ...
        for picktype in self:
            compatible_edis = edi_formats.filtered(lambda e: e._is_compatible_with_picking_type(picktype))
            picktype.compatible_edi_ids += compatible_edis
    ...
    def _compute_edi_format_ids(self):
        ...
        for picktype in self:
            picktype.edi_format_ids += edi_formats.filtered(lambda e: e._is_compatible_with_picking_type(picktype))
