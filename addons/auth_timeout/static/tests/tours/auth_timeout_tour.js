...
const assertRPC = {
    ...
    run: async function () {
        ...
        throw new Error("RPC was prevented unexpectedly");
    },
};
...
const assertNoRPC = {
    ...
    run: async function () {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            const result = await testRPC();
            if (result?.error?.data?.name === "odoo.addons.auth_timeout.models.ir_http.CheckIdentityException") {
                return;
            }
            await new Promise((r) => setTimeout(r, 50));
        }
        throw new Error("RPC was allowed unexpectedly");
    },
};
...
registry.category("web_tour.tours").add("auth_timeout_tour_lock_timeout", {
    ...
    steps: () => [
        ...
        {
            content: "Trigger an RPC call",
            trigger: "body",
            run: function () {
                rpc("/web/session/check");
            },
        },
        {
            content: "The session expired dialog should appear, click the close button to be redirected to the login",
            trigger: ".modal-dialog:has(.modal-title:contains('Session Expired')) footer .btn:contains('Close'):last",
            run: "click",
        },
        {
            content: "The login form is displayed",
            trigger: "form[action='/web/login']",
        },
    ],
});
