...
class IrHttp(models.AbstractModel):
    ...
    def _set_session_inactivity(self, session, inactivity_period=0, force=False):
        ...
        if inactive:
            next = time.time() + timeout - inactivity_period
            if not session.get("identity-check-next") or next < session["identity-check-next"]:
                session["identity-check-next"] = next
    ...
    def _authenticate(cls, endpoint):
        ...
        if request.session.uid is not None:
            if must_check_identity := cls._must_check_identity():
                ...
                elif endpoint.routing.get("check_identity") and must_check_identity.get("check_identity"):
    ...
    def _generate_routing_rules(self, modules, converters):
        """
        Override routing rules and enable identity checks for `auth=user` routes,
        except for a few exception listed in `ROUTES_NO_CHECK_IDENTITY`.

        e.g. `/web/webclient/load_menus` is an exception because it is called in Javascript using a simple `fetch`
        rather than a `rpc(...)` within a QWeb template (see `web.webclient_bootstrap` in `webclient_templates.xml`).
        ...
        :param modules: Loaded module names.
        :param converters: URL converters for route generation.
        :yield: (url, endpoint) tuples.
        """
        for url, endpoint in super()._generate_routing_rules(modules, converters):
            routing = endpoint.routing
            if routing["auth"] == "user" and "check_identity" not in routing and url not in ROUTES_NO_CHECK_IDENTITY:
                routing["check_identity"] = True
            yield url, endpoint
