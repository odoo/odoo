...
class MicrosoftOutlookMixin(models.AbstractModel):
    ...
    is_microsoft_outlook_configured = fields.Boolean('Is Outlook Credential Configured', compute='_compute_is_microsoft_outlook_configured')
    microsoft_outlook_refresh_token = fields.Char(string='Outlook Refresh Token', groups='base.group_system', copy=False)
    microsoft_outlook_access_token = fields.Char(string='Outlook Access Token', groups='base.group_system', copy=False)
    microsoft_outlook_access_token_expiration = fields.Integer(string='Outlook Access Token Expiration Timestamp', groups='base.group_system', copy=False)
    microsoft_outlook_uri = fields.Char(compute='_compute_outlook_uri', string='Authentication URI', help='The URL to generate the authorization code from Google', groups='base.group_system')
    ...
    def _compute_outlook_uri(self):
        ...
        for record in self:
            ...
            record.microsoft_outlook_uri = url_join(self._OUTLOOK_ENDPOINT, 'authorize?%s' % url_encode({
                ...
                'scope': 'offline_access %s' % self._SERVICE_SCOPE,
                'state': json.dumps({
                    'model': record._name,
                    'id': record.id,
    ...
    def _fetch_access_token(self, refresh_token):
        """Refresh the access token thanks to the refresh token.

        :return:
            access_token, access_token_expiration
        """
        response = self._fetch_token('refresh_token', refresh_token=refresh_token)
    ...
    def _generate_oauth2_string(self, user):
        ...
        :param refresh_token: Access token for the given Outlook account
        ...
        if not self.microsoft_outlook_access_token \
           ...
           or self.microsoft_outlook_access_token_expiration < now_timestamp:
            ...
            ) = self._fetch_access_token(self.microsoft_outlook_refresh_token)
            _logger.info(
