...
class CiiExportFacturXFR(TestCiiFacturXCommon, TestUblCiiFRCommon):
    ...
    def test_invoice_document_header(self):
        self.ensure_installed("l10n_fr_pdp")
    ...
    def test_invoice_multiple_taxes(self):
        ...
        invoice = self._create_invoice(
            ...
            partner_bank_id=self.recipient_bank,
            invoice_line_ids=[
                self._prepare_invoice_line(price_unit=100, product_id=self.product, tax_ids=self.tax_20),
                self._prepare_invoice_line(price_unit=25, product_id=self.product, tax_ids=self.tax_20),
                self._prepare_invoice_line(price_unit=50, product_id=self.product, tax_ids=self.tax_sale_a),
    ...
    def test_invoice_price_amount_rounding_precision(self):
        ...
        invoice = self._create_invoice(
            ...
            partner_bank_id=self.recipient_bank,
            invoice_line_ids=[
                self._prepare_invoice_line(price_unit=123.123, product_id=self.product, quantity=5, tax_ids=self.tax_20),
                self._prepare_invoice_line(price_unit=10.125, product_id=self.product, quantity=1, tax_ids=self.tax_20),
                self._prepare_invoice_line(price_unit=100.126, product_id=self.product, quantity=2, tax_ids=self.tax_20),
            ],
            post=True,
        )
    ...
    def test_invoice_price_amount_rounding_precision_with_price_included_taxes(self):
        self.tax_20.price_include_override = 'tax_included'
    ...
    def test_invoice_negative_price_unit(self):
        ...
        invoice = self._create_invoice(
            ...
            partner_bank_id=self.recipient_bank,
            invoice_line_ids=[
                self._prepare_invoice_line(price_unit=100.0, product_id=self.product, tax_ids=self.tax_20),
                self._prepare_invoice_line(price_unit=-10.0, product_id=self.product, tax_ids=self.tax_20),
            ],
            post=True,
        )
    ...
    def test_invoice_partner_siret(self):
        self.partner_fr.write({
            'company_registry': '93062183400322',
        })
        self.company.write({
            'company_registry': '65933441140047',
        })
    ...
    def test_invoice_immediate_payment_term(self):
        invoice = self._create_invoice(
            ...
            partner_bank_id=self.recipient_bank,
            invoice_line_ids=[
                self._prepare_invoice_line(price_unit=100.0, product_id=self.product, tax_ids=self.tax_sale_a),
                self._prepare_invoice_line(price_unit=100.0, product_id=self.product, tax_ids=self.tax_20),
            ],
            post=True,
        )
    ...
    def test_invoice_financial_account_iban(self):
        partner_bank = self.env['res.partner.bank'].create({
            'account_number': 'FR7730004028379876543210943',
            'partner_id': self.partner_fr.id,
        })
        partner_bank.write({'account_type': 'iban', 'allow_out_payment': True})
    ...
    def test_invoice_without_chorus_pro_fields(self):
        self.partner_fr.ref = 'CLI-2026-0042'
    ...
    def test_invoice_global_location_number(self):
        self.partner_fr.child_ids = self.env['res.partner'].create({
            'name': 'FR Partner Delivery',
            'type': 'delivery',
            'street': "Rue Napoléon, 55",
            'zip': "75000",
    ...
    def test_invoice_tax_withholding(self):
        ...
        invoice = self._create_invoice_one_line(
            product_id=self.product,
            price_unit=1000.0,
            tax_ids=self.tax_20 + tax_minus_10_67,
            partner_id=self.partner_fr,
            partner_bank_id=self.recipient_bank,
            post=True,
        )
