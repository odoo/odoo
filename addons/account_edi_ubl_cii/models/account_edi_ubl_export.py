...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _ubl_default_tax_category_grouping_key(self, base_line, tax_data, vals, currency):
        ...
        if tax_data and (
            ...
            # TODO: move those checks as BIS3 specific behavior
    ...
    def _ubl_turn_emptying_taxes_as_new_base_lines(self, vals):
        ...
        # '_ubl_values' is same dict as the originator line.
    ...
    def _ubl_init_values(self):
        return {
            ...
            'module_installed': lambda name: self.env['ir.module.module']._get(name).state == 'installed',
        }
    ...
    def _ubl_add_values_payable_rounding_amount(self, vals):
        ...
        vals['base_lines'] = [
            base_line
            for base_line in vals['base_lines']
            if base_line not in ubl_values['payable_rounding_base_lines']
        ]
    ...
    def _ubl_add_values_allowance_charge_early_payment(self, vals):
        ...
        epd_base_lines = ubl_values['epd_base_lines'] = []
        for sub_currency, suffix in ((currency, '_currency'), (company_currency, '')):
            base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
                base_lines=base_lines,
            ...
            for grouping_key, values in values_per_grouping_key.items():
                ...
                for base_line, _taxes_data in values['base_line_x_taxes_data']:
                    epd_base_lines.append(base_line)

        vals['base_lines'] = [
            base_line
            for base_line in vals['base_lines']
            if base_line not in epd_base_lines
        ]
    ...
    def _ubl_add_line_allowance_charge_nodes_for_discount(self, vals, in_foreign_currency=True):
        ...
        allowance_charges_nodes.append(self._ubl_get_line_allowance_charge_discount_node(vals, {
            ...
            'is_charge': raw_discount_amount < 0.0,
            'amount': raw_discount_amount,
            'base_amount': tax_details[f'raw_gross_total_excluded{suffix}'],
        }))
    ...
    def _ubl_add_order_line_node(self, vals):
        self._ubl_add_line_id_node(vals)
        self._ubl_add_line_quantity_node(vals)
        self._ubl_add_line_allowance_charge_nodes(vals)
        self._ubl_add_line_extension_amount_node(vals)
        self._ubl_add_line_period_nodes(vals)
        self._ubl_add_line_tax_totals_nodes(vals)
    ...
    def _ubl_add_order_line_nodes(self, vals, filter_function=None):
        nodes = vals['document_node']['cac:OrderLine'] = []
        for sub_vals in self._ubl_line_nodes_filter_base_lines(vals, filter_function=filter_function):
            self._ubl_add_order_line_node(sub_vals)
            nodes.append(sub_vals['line_node'])
    ...
    def _ubl_get_delivery_node_from_delivery_address(self, vals):
        delivery_partner = vals['delivery']['partner']
        node = {
            'cbc:ActualDeliveryDate': {'_text': None},
            'cac:DeliveryLocation': {
                'cbc:ID': {'schemeID': None, '_text': None},
        ...
        if vals['module_installed']('account_add_gln') and delivery_partner.global_location_number:
    ...
    def _ubl_get_payment_means_payee_financial_account_node_from_partner_bank(self, vals, partner_bank_vals):
        return {
            'cbc:ID': {'_text': partner_bank_vals['account_number']},
            'cac:FinancialInstitutionBranch': self._ubl_get_payment_means_payee_financial_account_institution_branch_node_from_partner_bank(vals, partner_bank_vals),
        }

    def _ubl_add_payment_means_nodes(self, vals):
        vals['document_node']['cac:PaymentMeans'] = []

    def _ubl_get_payment_terms_node_from_payment_term(self, vals):
        note = vals['payment_term']['note']
        if not note:
            return
    ...
    def _ubl_add_payment_terms_nodes(self, vals):
        ...
        if vals.get('payment_term'):
            nodes.append(self._ubl_get_payment_terms_node_from_payment_term(vals))
    ...
    def _ubl_get_allowance_charge_node_for_early_payment(self, vals, early_payment_values):
    ...
    def _ubl_add_allowance_charge_nodes_early_payment_discount(self, vals, in_foreign_currency=True):
        ...
        for grouping_key, values in values_per_grouping_key.items():
            ...
            allowance_charge_nodes.append(self._ubl_get_allowance_charge_node_for_early_payment(vals, {
                'currency': currency,
                'amount': values[f'total_excluded{suffix}'],
                'is_charge': values[f'total_excluded{suffix}'] > 0.0,
                'tax_categories': {
    ...
    def _ubl_add_legal_monetary_total_line_extension_amount_node(self, vals, invoice_line_tag, in_foreign_currency=True):
        currency = vals['currency_id'] if in_foreign_currency else vals['company_currency']

        line_extension_amount = sum(
            invoice_line_node['cbc:LineExtensionAmount']['_text']
            for invoice_line_node in vals['document_node'][invoice_line_tag]
        )
    ...
    def _ubl_add_legal_monetary_total_allowance_charge_total_amount_node(self, vals, in_foreign_currency=True):
        ...
        total_allowance = sum(
            ...
            if not allowance_node['cbc:ChargeIndicator']['_text'] == 'false'
        )
        total_charge = sum(
            charge_node['cbc:Amount']['_text']
            for charge_node in vals['document_node']['cac:AllowanceCharge']
            if not charge_node['cbc:ChargeIndicator']['_text'] == 'true'
        )
    ...
    def _ubl_add_legal_monetary_total_payable_rounding_amount_node_from_cash_rounding(self, vals, in_foreign_currency=True):
        ...
        for grouping_key, values in values_per_grouping_key.items():
            ...
            if node['cbc:PayableRoundingAmount'] is None:
                node['cbc:PayableRoundingAmount'] = {
                    '_text': FloatFmt(0.0, min_dp=currency.decimal_places),
                    'currencyID': currency.name,
                }
            node['cbc:PayableRoundingAmount']['_text'] += values[f'total_excluded{suffix}']
    ...
    def _ubl_add_legal_monetary_total_node(self, vals):
        vals['document_node']['cac:LegalMonetaryTotal'] = {}

    def _ubl_add_requested_monetary_total_node(self, vals):
        vals['document_node']['cac:RequestedMonetaryTotal'] = {}

    def _ubl_add_anticipated_monetary_total_node(self, vals):
        vals['document_node']['cac:AnticipatedMonetaryTotal'] = {}
