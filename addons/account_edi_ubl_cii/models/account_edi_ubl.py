...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _import_ubl_invoice_fix_taxes_amounts(self, collected_values):
        ...
        # Compare the tax amounts to see if we can "fix" them.
        # Associate the taxes record with the tax keys used in the xml.
        tax_to_taxes = {}
        ...
        for tax_key, global_tax_values in tax_total_values.items():
            ...
            expected_total_tax_amount += global_tax_values['tax_amount_currency']

        collected_values['are_taxes_complete'] = is_complete
        if not is_complete:
            return

        # In case we match a tax that is not in the xml, we can't fix anything.

        def grouping_function(base_line, tax_data):
            if not tax_data:
                return 'empty'
            if tax_data['tax'] not in tax_to_taxes:
                return 'error'
            return {'taxes': tax_to_taxes[tax_data['tax']]}

        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(base_lines, grouping_function)
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(base_lines_aggregated_values)
        if 'error' in values_per_grouping_key:
            return

        # In case the current total tax amount is too far from the expected total tax amount, we can't fix anything.
        current_total_tax_amount = sum(values['tax_amount_currency'] for values in values_per_grouping_key.values())
        if currency.compare_amounts(abs(expected_total_tax_amount - current_total_tax_amount) - tolerance, 0.0) > 0:
            return
        ...
        for base_line in base_lines:
            base_line['_create_values']['extra_tax_data'] = AccountTax._export_base_line_extra_tax_data(base_line)
