...
class AccountEdiXmlCii(models.AbstractModel):
    ...
    def _remove_allowance_charge_tax_effect(self, tax_details):
        ...
        allowance_charge_keys = [
            ...
            if k.get('ubl_cii_type') in ('allowance', 'charge')
        ]
    ...
    def _add_invoice_line_allowance_charge_vals(self, template_values):
        ...
        for line_vals in template_values['invoice_line_vals_list']:
            ...
            for grouping_key, tax_detail in tax_details_per_record.items():
                if allowance_charge_vals := grouping_key.get('allowance_charge_vals'):
                    line_vals['allowance_charge_vals_list'].append({
                        'charge_indicator': allowance_charge_vals['charge_indicator'],
            ...
            allowance_charge_keys = [
                ...
                if key.get('ubl_cii_type') in ('allowance', 'charge')
            ]
            for key in allowance_charge_keys:
                tax_details_per_record.pop(key)

            sum_allowances_charges_taxes = self._get_allowance_charge_sum(line_vals['allowance_charge_vals_list'])
            line_vals['line_total_amount'] = line_vals['line'].price_subtotal + sum_allowances_charges_taxes
            ...
            grouping_key = {
                ...
                **self._get_tax_exemption_reason(customer.commercial_partner_id, supplier, tax),
                ...
                'allowance_charge_vals': {
                    'charge_indicator': 'true' if tax.ubl_cii_type == 'charge' else 'false',
                    'allowance_charge_reason_code': tax.ubl_cii_charge_reason_code if tax.ubl_cii_type == 'charge' else tax.ubl_cii_allowance_reason_code,
                    'allowance_charge_reason':  tax.ubl_cii_charge_reason if tax.ubl_cii_type == 'charge' else tax.ubl_cii_allowance_reason,
                } if tax.ubl_cii_type in ('allowance', 'charge') else None,
            }
            return grouping_key
