...
class ResPartner(models.Model):
    ...
    def _compute_routing_scheme_endpoint(self):
        for partner in self:
            if override := partner.routing_identifier_override:
                partner.routing_scheme, partner.routing_endpoint = partner._parse_routing_identifier_override(override)
                ...
                partner.routing_scheme = partner.routing_endpoint = False
    ...
    def _parse_routing_identifier_override(self, identifier):
        ...
        available_schemes = self.available_routing_schemes or [
            eas for eas in dict(self._fields['routing_scheme'].selection)
            if eas not in DEPRECATED_ELECTRONIC_ADDRESS_SCHEMES_CODELIST
        ]
