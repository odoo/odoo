    ...
    def _get_report_values(self, docids, data=None):
        ...
        for move in moves:
            if not move.l10n_tw_edi_related_number:
                raise UserError(self.env._("Invoice %(move_name)s does not have Ecpay Invoice", move_name=move.name))
            ...
            if response_data.get('RtnCode') != 1:
                raise UserError(self.env._("Invoice %(move_name)s Error: %(error)s", move_name=move.name, error=response_data.get('RtnMsg')))
