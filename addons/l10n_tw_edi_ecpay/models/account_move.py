    ...
    def _l10n_tw_edi_prepare_item_list(self, json_data, is_allowance=False):
        ...
        for line in self.invoice_line_ids.filtered(lambda line: line.display_type == "product"):
            ...
            if self.l10n_tw_edi_is_b2b:
                item_price = round(self._l10n_tw_edi_convert_currency_to_twd(line.price_subtotal / line.quantity), 2)
                ...
                if line.tax_ids[0].price_include:
                    ...
                    item_amount = round(self._l10n_tw_edi_convert_currency_to_twd(line.price_unit * (1 + line.tax_ids[0].amount / 100) * line.quantity), 2)

            if not self.l10n_tw_edi_is_b2b and is_allowance and not line.tax_ids[0].price_include:
                item_price = round(item_price * (1 + line.tax_ids[0].amount / 100), 2)
            ...
            if self.l10n_tw_edi_is_b2b and is_allowance:
                item_list.append({
                    ...
                    "Tax": round(item_amount * line.tax_ids[0].amount / 100, 2)
                })
            else:
                item_list.append({
