...
class AccountMove(models.Model):
    ...
    def _compute_l10n_tw_edi_related_number(self):
        for move in self:
            move.l10n_tw_edi_related_number = base64.urlsafe_b64encode(move.name.encode("utf-8")).decode() if move.name else False
    ...
    def _l10n_tw_edi_check_tax_type_on_invoice_lines(self):
        ...
        if invoice_lines_tax_types:
            ...
            if len(invoice_lines_tax_types) > 1 and "4" in invoice_lines_tax_types:
                raise UserError(self.env._("Special tax rate cannot be mixed with other tax types. Please check your tax types on invoice lines."))
            if "4" in invoice_lines_tax_types and len({line.tax_ids[0].l10n_tw_edi_special_tax_type for line in product_lines if line.tax_ids and line.tax_ids[0].l10n_tw_edi_tax_type == "4" and line.tax_ids[0].l10n_tw_edi_special_tax_type}) > 1:
                raise UserError(self.env._("Special tax type cannot be mixed with other special tax types. Please check your special tax types on invoice lines."))

            if "2" in invoice_lines_tax_types and "3" in invoice_lines_tax_types:
                raise UserError(self.env._("Tax type 2 (Zero tax rate) and type 3 (Duty free) cannot be used together. Please check your tax types on invoice lines."))
        else:
            raise UserError(self.env._("Please fill in the tax on the invoice lines and select the Ecpay tax type for taxes."))
    ...
    def _l10n_tw_edi_generate_invoice_json(self):
        ...
        if self.l10n_tw_edi_invoice_type == "07" and tax_type not in ["1", "2", "3", "9"]:
            raise UserError(self.env._("Invoice type 07 must be used with tax type 1, 2 or 9. Please check the tax type on the invoice lines."))

        if self.l10n_tw_edi_invoice_type == "08" and tax_type not in ["3", "4"]:
            raise UserError(self.env._("Invoice type 08 must be used with tax type 3 or 4. Please check the tax type on the invoice lines."))
        ...
        if is_zero_tax_rate:
            if not self.l10n_tw_edi_clearance_mark and not self.l10n_tw_edi_zero_tax_rate_reason:
                raise UserError(self.env._(" Clearance mark and Zero tax rate reason is required for zero tax rate invoice."))
    ...
    def _l10n_tw_edi_send(self, json_content):
        ...
        if response_data.get("RtnCode") != 1:
            return [self.env._("Invoice %(name)s Error: %(error_message)s", name=self.name, error_message=response_data.get("RtnMsg"))]
        self.write({
        ...
        self._message_log(
            body=self.env._("The invoice has been successfully sent to Ecpay with Ecpay invoice number %(invoice_number)s.", invoice_number=response_data.get("InvoiceNo")),
        )
    ...
    def _l10n_tw_edi_run_invoice_invalid(self):
        ...
        if self.l10n_tw_edi_state == "invalid":
            raise UserError(self.env._("The invoice: %(invoice_id)s has already been invalidated", invoice_id=self.l10n_tw_edi_ecpay_invoice_id))
        ...
        if response_data.get("RtnCode") != 1:
            raise UserError(self.env._("Fail to invalidate invoice. Error message: %(error_message)s", error_message=response_data.get("RtnMsg")))
    ...
    def l10n_tw_edi_issue_allowance(self):
        ...
        self.ensure_one()
        if not self.l10n_tw_edi_ecpay_invoice_id:
            raise UserError(self.env._("You cannot issue an allowance for an invoice that was not sent to Ecpay. Invoice number: %(invoice_number)s", invoice_number=self.name))
        ...
        if self.l10n_tw_edi_allowance_notify_way == "email" and self.partner_id.email:
            ...
            raise UserError(self.env._("Customer %(notify_way)s is needed for notification", notify_way=self.l10n_tw_edi_allowance_notify_way))
        ...
        if response_data.get("RtnCode") != 1:
            raise UserError(self.env._("Fail to issue allowance for ECpay invoice. Error message: %(error_message)s", error_message=response_data.get("RtnMsg")))
