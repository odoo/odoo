...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_all_extra_edis(self):
        ...
        res.update({'tw_ecpay_send': {'label': self.env._("Send to Ecpay"), 'is_applicable': self._is_tw_edi_applicable}})
        return res
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if errors := invoice._l10n_tw_edi_send(json_content):
                invoice_data["error"] = {
                    "error_title": self.env._("Error when sending the invoices to the E-invoicing service."),
                    "errors": errors,
                }
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if errors := invoice._l10n_tw_edi_update_ecpay_invoice_info():
                invoice_data["error"] = {
                    "error_title": self.env._("Error when getting the invoice data from the E-invoicing service."),
                    "errors": errors,
                }
