...
class L10nTWITestEdi(TestAccountMoveSendCommon, HttpCase):
    ...
    def test_08b_downpayment_tax_amount_matches_invoice(self):
        ...
        cases = [
            {
                "down_payments": [(95315.0, 4766.0), (57189.0, 2860.0)],
                "invoice_amount": 190630,
                "expected_tax": 1906.0,
            },
            {
                # down_payments format: (down payment, tax charged)
                "down_payments": [(330.0, 17.0)],
                "invoice_amount": 1000,
                "expected_tax": 33.0,
            },
        ...
        for case in cases:
            down_payments = case["down_payments"]
            invoice_amount = case["invoice_amount"]
            expected_tax = case["expected_tax"]
            with self.subTest(down_payments=down_payments, invoice_amount=invoice_amount, expected_tax=expected_tax):
                invoice = self.init_invoice(
                    'out_invoice', partner=self.partner_b, amounts=[invoice_amount], taxes=self.tax_sale_a,
                )
                invoice.write({
                    "invoice_line_ids": [
                        Command.create({
                            ...
                            "tax_ids": self.tax_sale_a,
                            "extra_tax_data": {
                                "computation_key": f"down_payment,{key}",
                                "manual_total_excluded_currency": -price_unit,
                                "manual_total_excluded": -price_unit,
                                "manual_tax_amounts": {
                                    str(self.tax_sale_a.id): {
                                        "tax_amount_currency": -tax_amount,
                                        "tax_amount": -tax_amount,
                                        "base_amount_currency": -price_unit,
                                        "base_amount": -price_unit,
                                    },
                                },
                                "currency_id": self.env.ref('base.TWD').id,
                                "price_unit": price_unit,
                                "discount": 0.0,
                                "quantity": -1.0,
                                "rate": 1.0,
                        ...
                        for key, (price_unit, tax_amount) in enumerate(down_payments, start=1)
                    ],
                })
                invoice.action_post()
