...
class EcPayAPI():
    ...
    def call_ecpay_api(self, endpoint, json_data):
        ...
        if not response_json.get("Data"):
            raise UserError(self.env._("ECPay API Error: %(error_message)s, Error Code: %(error_code)s", error_message=response_json.get("TransMsg"), error_code=response_json.get("TransCode")))
