...
class L10nMyEDITestNewSubmission(TestAccountMoveSendCommon):
    ...
    def test_15_error_title_bucketing(self):
        ...
        with patch(CONTACT_PROXY_METHOD, new=self._test_16_mock_myinvois):
            with self.assertRaisesRegex(UserError, 'MyInvois returned the following issue'):
                self.basic_invoice.action_l10n_my_edi_send_invoice()

        with patch(CONTACT_PROXY_METHOD, new=self._test_16_mock_odoo):
    ...
    def _test_16_mock_myinvois(self, endpoint, params):
    ...
    def _test_16_mock_odoo(self, endpoint, params):
