...
class AccountMove(models.Model):
    ...
    def _cron_l10n_my_edi_synchronize_myinvois(self):
        ...
        for company in set(invoices_per_company.keys()):
            ...
            for submission_uid, invoices in company_invoice_per_submission_uid.items():
                ...
                for invoice in invoices:
                    ...
                    new_state = invoice_result['status']
                    if new_state == invoice.l10n_my_edi_state:
                        continue

                    # If the state changed, we update the invoice with the new state and an eventual reason.
                    invoice._l10n_my_edi_set_status(
                        state=new_state,
                        message=_('This invoice has been %(status)s for reason: %(reason)s', status=new_state, reason=invoice_result['reason']) if invoice_result.get('reason') else None,
                    )
    ...
    def _l10n_my_get_submission_status(self, submission_uid, proxy_user):
        ...
        If there are too many and the submission is paginated, this will fetch each page with a waiting time of 3s per call.
        ...
        if 'error' in result:
            ...
            if result['document_count'] <= 100:  # If so, we got all of it at once.
                ...
                for page in range(2, (result['document_count'] // 100) + 1):
                    time.sleep(1)  # To avoid any risks of throteling, we should wait a bit before continuing
