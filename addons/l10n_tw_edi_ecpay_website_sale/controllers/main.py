...
class WebsiteSaleL10nTW(WebsiteSale):
    def _is_valid_mobile_barcode(self, carrier_number, order):
        ...
        response_data = EcPayAPI(order.company_id).call_ecpay_api("/CheckBarcode", json_data)
    ...
    def _is_valid_tax_id(self, tax_id, order):
        ...
        response_data = EcPayAPI(order.company_id).call_ecpay_api("/GetCompanyNameByTaxID", json_data)
    ...
    def _is_valid_love_code(self, love_code, order):
        ...
        response_data = EcPayAPI(order.company_id).call_ecpay_api("/CheckLoveCode", json_data)
        return int(response_data.get("RtnCode")) == 1 and response_data.get("IsExist") == "Y"
    ...
    def l10n_tw_invoicing_info(self, **kw):
        ...
        if request.httprequest.method == 'GET':
            default_vals.update({
                'l10n_tw_edi_is_donate': "on" if order.l10n_tw_edi_love_code else False,
                'l10n_tw_edi_love_code': order.l10n_tw_edi_love_code,
                'l10n_tw_edi_carrier_type': order.l10n_tw_edi_carrier_type if order.l10n_tw_edi_carrier_type else "1",
    ...
    def _handle_extra_form_data(self, extra_form_data, address_values):
        ...
        if request.website.sudo().company_id.country_id.code == 'TW' and request.website.sudo().company_id._is_ecpay_enabled():
            ...
            if address_values.get('company_name'):  # B2B customer
                ...
                if not request.website.sale_get_order().partner_id.parent_id:
                    ...
                    request.website.sale_get_order().partner_id.parent_id = company_contact
                else:
                    request.website.sale_get_order().partner_id.parent_id.write({
                        'name': address_values.get('company_name'),
                        'vat': address_values.get('vat'),
                        'l10n_tw_edi_require_paper_format': l10n_tw_edi_is_print,
                    })
            else:  # B2C customer
                request.website.sale_get_order().partner_id.l10n_tw_edi_require_paper_format = l10n_tw_edi_is_print

            request.website.sale_get_order().l10n_tw_edi_is_print = l10n_tw_edi_is_print
