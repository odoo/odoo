...
class WebsiteSaleL10nTW(WebsiteSale):
    ...
    def l10n_tw_invoicing_info(self, **kw):
        ...
        if request.httprequest.method == 'GET':
            default_vals['is_donate'] = "on" if order.l10n_tw_edi_love_code else False
            default_vals['love_code'] = order.l10n_tw_edi_love_code
            default_vals['carrier_type'] = order.l10n_tw_edi_carrier_type if order.l10n_tw_edi_carrier_type else "1"
            default_vals['carrier_number'] = order.l10n_tw_edi_carrier_number
            default_vals['carrier_number_2'] = order.l10n_tw_edi_carrier_number_2
        ...
        if request.httprequest.method == 'POST':
            ...
            elif kw.get('l10n_tw_edi_is_donate') == 'on' and not self._is_valid_love_code(kw.get('l10n_tw_edi_love_code')):
    ...
    def _validate_address_values(self, address_values, partner_sudo, address_type, *args, **kwargs):
        ...
        if address_type == 'billing' and request.website.sudo().company_id.country_id.code == 'TW' and request.website.sudo().company_id._is_ecpay_enabled():
            ...
            if address_values.get('company_name'):  # B2B customer
                ...
                if not self._is_valid_tax_id(address_values.get('vat')):
