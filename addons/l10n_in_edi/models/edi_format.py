...
class EdiFormat(models.Model):
        ...
        return {
            ...
            'stages': {
                'send': {
                    'Initialized': {
                        'new_state': 'to_send',
                        'action': self._send_invoice_to_webservice,
                    },
                    'Invoice sent to the government': {
                        'new_state': 'sent',
                ...
                'cancel': {
                    'Initialized': {
                        'new_state': 'to_cancel',
                        'action': self._cancel_invoice_to_webservice,
                    },
                    'Invoice sent to the government': {
                        'new_state': 'cancelled',
                        'make_attachments_official': True,
    ...
    def _is_format_required(self, document, document_type=''):
        ...
        return (
                document_type in ('invoice', 'payment')
                and document.is_sale_document()
                and document.country_code == 'IN'
                and document.l10n_in_gst_treatment in (
                    "regular",
                    "composition",
                    "overseas",
    ...
    # def _get_invoice_edi_content(self, move): # todo todo todo
    #     if self.code != "in_einvoice_1_03":
    ...
    def _check_document_configuration(self, move):
        if self.code != "in_einvoice_1_03":
            return super()._check_document_configuration(move)
        error_message = []
    ...
    def _send_invoice_to_webservice(self, flows):
        res = {}
        ...
        if response.get("error"):
            ...
            attachment = flows._create_or_update_edi_files([{
                "name": json_name,
            ...
            res[invoices.id] = {"success": True, "attachment": attachment}
        return res

    def _cancel_invoice_to_webservice(self, flows):
        invoices = flows._get_documents()
        res = {}
        for flow in flows:
            invoice = invoices.filtered(lambda inv: inv.id == flow.res_id)
            ...
            if not response.get("error"):
                ...
                attachment = flow._create_or_update_edi_files([{
                    "name": json_name,
                    "raw": json_dump.encode(),
                    "res_model": "account.move",
                    "res_id": invoice.id,
                ...
                res[invoice.id] = {"success": True, "attachment": attachment}
        return res
