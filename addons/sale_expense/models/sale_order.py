...
class SaleOrder(models.Model):
    ...
    is_linked_to_expense_with_attachment = fields.Boolean(compute='_compute_is_linked_to_expense_with_attachment', compute_sudo=True)
    ...
    def _compute_is_linked_to_expense_with_attachment(self):
        ...
        for order in self:
            order.is_linked_to_expense_with_attachment = order_attachments_map.get(order.id, [])
    ...
    def _get_expense_attachments_not_linked_yet(self):
        ...
        return {
            order.id: [
                ...
                for attachment in order.expense_ids.attachment_ids.filtered(lambda a: a.checksum not in checksums.get(order.id, []))
            ]
            for order in self
        }
    ...
    def action_copy_reinvoiced_expense_receipts(self):
        self.ensure_one()
        if not self.sudo().expense_ids.attachment_ids:
