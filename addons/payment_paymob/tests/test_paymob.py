...
class PaymobTest(PaymobCommon, PaymentHttpCommon):
    ...
    def test_no_item_missing_from_rendering_values(self):
        ...
        API_URL and URL_PARAMS corresponding to the response of API request. """
        ...
        paymob_client_secret = tx.paymob_client_secret
        self.assertEqual(
            rendering_values['api_url'],
            f'{paymob_url}/unifiedcheckout/?publicKey={paymob_pk}&clientSecret={paymob_client_secret}',
        )
        self.assertEqual(rendering_values['url_params']['publicKey'], paymob_pk)
        self.assertEqual(rendering_values['url_params']['clientSecret'], paymob_client_secret)
    ...
    def test_compute_signature(self):
        ...
        computed_hmac = PaymobController._compute_signature(
            PaymobController, self.redirection_data, self.provider.paymob_hmac_key)
    ...
    def test_normalize_webhook_data(self):
        ...
        normalized_data = PaymobController._normalize_response(
            PaymobController, self.webhook_data, self.hmac_signature)
