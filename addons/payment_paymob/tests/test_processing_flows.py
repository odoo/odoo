...
class TestProcessingFlows(PaymobCommon, PaymentHttpCommon):
    ...
    def test_accept_notification_with_valid_signature(self):
        ...
        self._assert_does_not_raise(
            Forbidden, PaymobController._verify_signature, self.redirection_data, tx
        )

    @mute_logger('odoo.addons.payment_paymob.controllers.main', 'odoo.addons.payment.utils')
    def test_reject_notification_with_missing_signature(self):
        """Test the verification of a notification with a missing signature."""
        tx = self._create_transaction('redirect')
        payload = dict(self.redirection_data, hmac=None)
    ...
    def test_reject_notification_with_invalid_signature(self):
        ...
        tx = self._create_transaction('redirect')
        payload = dict(self.redirection_data, hmac='dummy')
        self.assertRaises(Forbidden, PaymobController._verify_signature, payload, tx)
