...
class TestMrpReportMoOverview(TestStockCommon):
    ...
    def setUpClass(cls):
        ...
        cls.sub_bom = cls.env['mrp.bom'].create({
            ...
            'type': 'subcontract',
            'subcontractor_ids': [Command.set([cls.vendor.id])],
            'bom_line_ids': [
                Command.create({'product_id': cls.comp_1.id, 'product_qty': 1.0}),
                Command.create({'product_id': cls.comp_2.id, 'product_qty': 1.0}),
            ]
        })
        ...
        cls.final_bom = cls.env['mrp.bom'].create({
            ...
            'product_qty': 1.0,
            'type': 'normal',
            'bom_line_ids': [Command.create({'product_id': cls.sub_product.id, 'product_qty': 2.0})]
        })
    ...
    def test_subcontracted_product_mo_cost(self):
        ...
        # verify cost without a PO or an RFQ
        ...
        # change the price of the PO and verify the cost
        po_line.price_unit = 50
        report = self.env['report.mrp.report_mo_overview']
        result_after_price_change = report._get_components_data(mo, level=1, current_index='')
        self.assertEqual(result_after_price_change[0]['summary']['mo_cost'], 180.0)
