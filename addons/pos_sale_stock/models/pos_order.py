        ...
        for pos_order in pos_orders:
            ...
            for so_line in so_lines:
                ...
                for stock_move in so_line.move_ids:
                    ...
                    def get_expected_qty_to_ship_later():
                        ...
                        if pos_pickings and all(pos_picking.state in ['waiting', 'confirmed', 'assigned'] for pos_picking in pos_pickings):
                            return sum((so_line._convert_qty(so_line, pos_line._get_qty_to_move(), 'p2s') for pos_line in
                    ...
                    for move in so_line_stock_move_ids.filtered(lambda m: m.picking_id and m.state in ['waiting', 'confirmed', 'assigned'] and m.product_id == stock_move.product_id):
            ...
            for picking in self.env['stock.picking'].browse(waiting_picking_ids):
                if all(is_product_uom_qty_zero(move) for move in picking.move_ids):
                    origin_moves = self.env['stock.move'].browse(picking.move_ids._rollup_move_origs())
                    picking.action_cancel()
                    (origin_moves - picking.move_ids).filtered(lambda m: m.state not in ['done', 'cancel'])._action_cancel()
                else:
