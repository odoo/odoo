...
class res_config_settings(osv.osv_memory, res_config_module_installation_mixin):
    ...
    def onchange_module(self, cr, uid, ids, field_value, module_name, context=None):
        ...
        if installed_module_ids and field_value:
            dep_ids = module_pool.uninstalled_module_dependencies(cr, uid, installed_module_ids, context=context)
            dep_name = [x['display_name'] for x in module_pool.search_read(
                cr, uid, ['|', ('id', '=', installed_module_ids), ('id', 'in', dep_ids), ('application', '=', 'True')], context=context)]
            if dep_name:
                message = '\n'.join(dep_name)
                return {
                    'warning': {
                        'title': _('Warning!'),
                        'message': _('Enabling this feature you will install following paid application/s \n%s') % message,
                    }
                }
        return {}
