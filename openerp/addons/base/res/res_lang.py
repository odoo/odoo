...
class lang(osv.osv):
    ...
    def load_language(self, cr, uid, langs):
        lang_ids = []
        langs = [langs] if isinstance(langs, basestring) else langs
        for lang in langs:
            # active if language is inactive
            context = {'active_test': False}
            language = self.search(cr, uid, [('code', '=', lang), ('active', '=', False)], context=context)
            if language:
                self.write(cr, uid, language, {'active': True}, context=context)
                lang_ids += language
                continue
            # if already loaded and active langauge
            language = self.search(cr, uid, [('code', '=', lang)])
            if language:
                lang_ids += language
                continue
