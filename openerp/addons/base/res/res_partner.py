...
class format_address(object):
    ...
    def fields_view_get_address(self, arch):
        fmt = self._scope.user.company_id.country_id.address_format or ''
...
@model
def _lang_get(self):
    languages = self._scope['res.lang'].search([])
...
class res_partner(osv.Model, format_address):
    ...
    def _default_category(self):
        category_id = self._scope.context.get('category_id', False)
    ...
    def _default_company(self):
        return self._scope['res.company']._company_default_get('res.partner')

    _defaults = {
        'active': True,
        'lang': model(lambda self: self._scope.lang),
        'tz': model(lambda self: self._scope.context.get('tz', False)),
        'customer': True,
    ...
    def onchange_state(self, state_id):
        if state_id:
            state = self._scope['res.country.state'].browse(state_id)
...
#   _constraints = [(_check_ean_key, 'Error: Invalid ean code', ['ean13'])]
    ...
    def write(self, vals):
        ...
        if vals.get('company_id'):
            company = self._scope['res.company'].browse(vals['company_id'])
            for partner in self:
                if partner.user_ids:
    ...
    def main_partner(self):
        ...
        return self._scope.ref('base.main_partner')
