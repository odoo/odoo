...
#   _constraints = [(_check_ean_key, 'Error: Invalid ean code', ['ean13'])]
            ...
            for partner in self:
                if partner.user_ids:
                    ...
                    if len(companies) > 1 or company not in companies:
                        raise Warning(_("Warning"),_("You can not change the company as the partner/user has multiple user linked with different companies."))
    ...
    def name_create(self, cr, uid, name, context=None):
        ...
        if context.get('force_email') and not email:
            raise Warning(_('Warning'), _("Couldn't create contact without email address!"))
