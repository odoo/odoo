...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_act_window(self, cr, rec, data_node=None, mode=None):
        ...
        if src_model:
            ...
            self.pool['ir.values'].set_action(cr, self.uid, xml_id, action_slot=keyword, model=model, action=value, res_id=res_id)
    ...
    def _tag_ir_set(self, cr, rec, data_node=None, mode=None):
        ...
        for model in res['models']:
            ...
            if isinstance(model, (list, tuple)):
                model, res_id = model
            if res['key'] == 'default':
                ir_values.set_default(cr, self.uid, model, field_name=res['name'], value=res['value'], condition=res['key2'])
            elif res['key'] == 'action':
                ir_values.set_action(cr, self.uid, res['name'], action_slot=res['key2'], model=model, action=res['value'], res_id=res_id)
