...
class OpTimetable(models.Model):
    _name = 'op.timetable'
    _description = 'Paskaita'
    #_rec_name = 'Paskaita'
    _order = 'period_id'
    xsemester_id = fields.Many2one('op.xsemester', 'Semestras' , required =True )
    start_date = fields.Date('Pradžia', related='xsemester_id.start_date')
    end_date = fields.Date('Pabaiga', related='xsemester_id.end_date')
    einamasis = fields.Boolean ("Einamasis" , related='xsemester_id.einamasis')
    sekantis = fields.Boolean("Sekantis", related='xsemester_id.sekantis')
    ...
    day = fields.Selection(
        [('1', 'Pirmadienis'), ('2', 'Antradienis'),
         ...
         ('5', 'Penktadienis'), ('6', 'Šeštadienis')], 'Diena', required= True)
    ...
    #_sql_constraints = [
    #    ('name_uniq', 'unique(xsemester_id, day,period_id,subject_id , batch_id,week )', 'Dėstytojas jau turi šią paskaitą šiuo laiku šiai grupei'),
    #    ]
    ...
    def onchange_start_date(self):
        ...
        if start_datetime and start_datetime.weekday() == 0:
            ...
            self.type = 'Saturday'
#    _sql_constraints = [
#        ('unique_name_timetable_id',
#         'unique(timetable,subject_id,batch_id,xsemester_id,period_id,day,faculty_id, )',
#         'Grupė turi jau tokią paskaita šiuo laiku!'),
#    ]        
