...
class FgXReport(models.AbstractModel):
    ...
    def action_print(self, session_id):
        ...
        for session_id in session_ids:
            ...
            result = self.env['pos.payment'].read_group([('session_id', '=', session_id.id), ('payment_method_id', 'in', cash_payment_method.ids), ('amount', '>', 0)], ['amount'], ['session_id'])
            if result:
        ...
        if session_ids:
            self.env['transaction.log'].sudo().create_transaction_log('xreport_generation', 'pos.session', session_ids[0].id)
        return data
