...
individual_features.add_argument("--opt-packages", help="Install optional packages, for multiworker, enterprise and other advanced features (enabled by opt)", action=strore_true_no_default)
...
def clone(repo, org='odoo'):
    check_repo_exist = 'git -C {base_dir / repo} status > /dev/null 2>&1'
...
def main():
    ...
    if subprocess.run(check_user_exist, shell=True).returncode != 0:
        ...
        Operation(
            ...
            '(sudo -u postgres createuser -d -R -S $(whoami) && createdb $(whoami))',
        )
