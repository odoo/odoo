...
def _prepare_testing(o):
    if not o.no_tarball or not o.no_debian:
        ...
        # Use rsync to copy requirements.txt (only way to make Docker use cache)
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_debian")],
                        cwd=os.path.join(o.odoo_dir))
        subprocess.call(["docker", "build", "-t", "odoo/odoo-debian-nightly-tests", "."],
    ...
    if not o.no_rpm:
        ...
        subprocess.call(["cp", "package.dfcentos", os.path.join(o.build_dir, "docker_centos", "Dockerfile")],
                        cwd=os.path.join(o.odoo_dir, "setup"))
        subprocess.call(["docker", "build", "-t", "odoo/odoo-centos-nightly-tests", "."],
                        cwd=os.path.join(o.build_dir, "docker_centos"))

def test_tgz(o):
    with docker('odoo/odoo-debian-nightly-tests', o.build_dir, o.pub) as wheezy:
...
def test_rpm(o):
    with docker('odoo/odoo-centos-nightly-tests', o.build_dir, o.pub) as centos7:
