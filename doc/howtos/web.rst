...
A ``<div>`` element is usually invisible to the user if it
has no content (or without specific styles giving it a size) which is why nothing
...
This allows generating and displaying any type of content, but
rapidly gets unwieldy when generating significant amounts of DOM (lots of
...
But because it's a very common operation, :class:`~openerp.Widget`
...
The ``argument`` is necessary in all methods, otherwise bad
things could happen (such as the application not being translated
correctly). That's why, when you call a model's method, you should always provide
that argument. The solution to achieve that is to use
...
defined in ``AbstractField`` or any other class. We will use this method
