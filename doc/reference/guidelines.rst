...
such methods are decorated with the ``api.multi`` decorator. Then you will have to iterate on ``self`` to treat each
record.
...
~~~~~~~~~~~~~~~~~~~~~
The context is a ``frozendict`` that cannot be modified. To call
...
- Variable name :
    ...
    - Odoo works with a record or a recordset, don't
      suffix variable names with *_id* or *_ids* if they don't contain an id or a list
      of ids.
