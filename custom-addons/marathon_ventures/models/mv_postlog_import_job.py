# -*- coding: utf-8 -*-
"""Postlog Data (Postlog) import job.

Deliberate mirror of models/mv_prelog_import_job.py - same field names, same
method names, same flow - so that a change to one is easy to port to the other.

Two intentional divergences from the prelog job, both because a postlog is the
single as-run record for a week rather than a plan that gets re-issued:

  * no `prelog_version` equivalent - there is one postlog upload per week.
  * no `replace_existing` flag - re-uploading a week that already has Postlog Data
    is blocked with an error instead of silently replacing it, because a
    replace would destroy schedule attachments a user may have corrected by
    hand. Clearing a week is an explicit action from the workbench.
"""
...
from ..services.postlog_import.config_loader import slugify_program_name
from ..services.postlog_import.engine import PostlogImportEngine

_logger = logging.getLogger(__name__)


class MvPostlogImportJob(models.Model):
    _name = "mv.postlog_import_job"
    _description = "Postlog Import Job"
    _order = "create_date desc, id desc"

    name = fields.Char(
        string="Job Name",
        default=lambda self: self.env["ir.sequence"].next_by_code("mv.postlog_import_job.name") or "New",
        copy=False,
        readonly=True,
    )
    ...
    skipped_row_count = fields.Integer(string="Skipped Rows", readonly=True)
    ...
    postlog_ids = fields.One2many("mv.spot_data", "import_job", string="Postlog Rows")

    @api.model
    def create_from_wizard(self, *, program, upload_file, upload_filename, import_week):
        user_email = (self.env.user.partner_id.email or "").strip()
        if not user_email:
            raise UserError(_("Your user account must have an email address before starting a Postlog upload."))

        # One upload per week: refuse rather than replace, so hand-corrected
        # schedule attachments are never silently destroyed.
        existing_count = self.env["mv.spot_data"].search_count(
            [
                ("import_program", "=", program.id),
        ...
        if existing_count:
            raise UserError(
                _(
                    "%(program)s already has %(count)s Postlog rows for the week of "
                    "%(week)s. A postlog is one upload per week, so this import was "
                    "stopped instead of replacing them. Delete that week from the Postlog "
                    "Data workbench first if you need to re-import it."
                )
                % {
    ...
    def _run_job(self):
        ...
        try:
            ...
            _logger.exception("Postlog Data import job %s failed.", self.id)
    ...
    def _process_rows(self):
        ...
        for row_index, row in enumerate(rows, start=engine._first_data_row_number()):
            ...
            rate_added = False
            try:
                vals = engine.build_row_vals(row, self.import_week, row_index)
                vals["import_job"] = self.id
                rate_value = float(vals.get("spot_rate") or 0.0)
                ...
                if not rate_added:
                    rate_value = vals.get("spot_rate") if vals else row.get("spot_rate")
                    try:
        ...
        summary = {
            ...
            "skipped_row_count": getattr(engine, "skipped_row_count", 0),
    ...
    def _create_result_attachments(self, *, engine, success_rows, error_rows):
        self.ensure_one()
        attachment_vals = {}
        success_name = self._attachment_filename("uploaded-postlogs")
        error_name = self._attachment_filename("errors")

        success_csv = self._csv_bytes(
            rows=success_rows,
            fieldnames=["Postlog Data Name", "Schedule"] + engine.export_field_names(),
        )
    ...
    def _send_completion_email(self):
        ...
        mail = self.env["mail.mail"].create(
            {
                "subject": _(
                    "Postlog Data Upload Complete %(issues)s errors, %(matched)s postlogs created for %(filename)s"
                )
                % {
        ...
        try:
            ...
            _logger.exception("Postlog Data import job %s email send failed.", self.id)
    ...
    def _send_failure_email(self, failure_message):
        ...
        mail = self.env["mail.mail"].create(
            {
                "subject": _("Postlog Data Upload Failed for %s") % self.upload_filename,
                "email_to": email_to,
                "email_from": self.env.company.email_formatted or self.env.user.email_formatted or False,
                "body_html": """
                    <p>Your Postlog upload of file <strong>{filename}</strong> failed.</p>
                    <p>Error: {message}</p>
                """.format(
                    filename=self.upload_filename,
        ...
        try:
            ...
            _logger.exception("Postlog Data import job %s failure email send failed.", self.id)
            return False, str(exc)
        return True, False
    ...
    def _build_email_body_html(self):
        self.ensure_one()
        return """
            <p>Your Postlog upload of file <strong>{filename}</strong> completed.</p>
            <p>{matched} rows were successfully uploaded as Postlog Data records to Odoo and matched to their schedules (see attached uploaded-postlogs.csv).</p>
            <p>{unmatched} rows were uploaded as Postlog Data records to Odoo but did not match a schedule (see attached errors.csv).</p>
    ...
    def _build_summary_text(self):
        self.ensure_one()
        return _(
            "Matched: %(matched)s, Unmatched: %(unmatched)s, Errors: %(errors)s, "
            "Skipped: %(skipped)s, Total Amount: %(amount)s"
        ) % {
            "matched": self.matched_count,
            "unmatched": self.unmatched_count,
            "errors": self.error_count,
            "skipped": self.skipped_row_count,
    ...
    def _attachment_filename(self, prefix):
        ...
        return f"{prefix}-{network_slug}-{self.import_week}.csv"
    ...
    def _build_success_csv_row(self, engine, postlog, raw_row, vals):
        row = {
            "Postlog Data Name": postlog.name or False,
            "Schedule": postlog.schedule.display_name if postlog.schedule else False,
        }
