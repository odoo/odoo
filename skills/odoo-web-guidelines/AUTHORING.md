...
- Guidelines live in `guidelines/`, one file per domain: `javascript.md`, and a new
  file the day a domain has guidelines of its own.
- Inside a file, one guideline per `#` section. The title states the rule: "Avoid
  getters", not "Getters".
...
- A **convention** is a choice that fits in one line and can be followed as stated.
  - Examples: 4-space indent, `o_<module>` class prefix.
...
- State the rule as something a reviewer can check against code with a yes or no.
  - Good: "every class starts with `o_<module>_`".
  - Bad: "use descriptive class names".
